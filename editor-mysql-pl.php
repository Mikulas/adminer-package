...
confirm($Na=""){return" onclick=\"return confirm('".'Czy jesteś pewien?'.($Na?" (' + $Na + ')":"")."');\"";}function
...
upload_error($k){$uc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Wgranie pliku było niemożliwe.'.($uc?" ".sprintf('Maksymalna wielkość pliku to %sB.',$uc):""):'Plik nie istnieje.');}function
...
enum_input($Rd,$c,$l,$W,$mb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$_);$K=($mb!==null?"<label><input type='$Rd'$c value='$mb'".((is_array($W)?in_array($mb,$W):$W===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($_[1]as$q=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$q+1:(is_array($W)?in_array($q+1,$W):$W===$V));$K.=" <label><input type='$Rd'$c value='".($q+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$K;}function
input($l,$W,$p){global$Td,$b,$u;$C=h(bracket_escape($l["field"]));echo"<td class='function'>";$gd=($u=="mssql"&&$l["auto_increment"]);if($gd&&!$_POST["save"])$p=null;$Mb=(isset($_GET["select"])||$gd?array("orig"=>'bez zmian'):array())+$b->editFunctions($l);$c=" name='fields[$C]'";if($l["type"]=="enum")echo
...
lang($Pd,$Bc){$Tc=($Bc==1?0:($Bc%10>1&&$Bc%10<5&&$Bc/10%10!=1?1:2));$Pd=str_replace("%d","%s",$Pd[$Tc]);$Bc=number_format($Bc,0,".",' ');return
...
<tr><th>Użytkownik<td><input type="hidden" name="auth[driver]" value="server"><input id="username" name="auth[username]" value="',h($_GET["username"]),'">
...
selectLinks($Cd,$P=""){$a=$Cd["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Nowy rekord'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Ostatnia strona'."'>&gt;&gt;</a>\n";}function
...
as$e=>$V)$x.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($L[$V]);echo"<a href='".h($x)."' title='".'Nowy rekord'."'>+</a> ";}}}function
...
selectVal($V,$x,$l){$K=($V===null?"&nbsp;":$V);$x=h($x);if(ereg('blob|bytea',$l["type"])&&!is_utf8($V)){$K=lang(array('%d bajt','%d bajty','%d bajtów'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$K="<img src='$x' alt='$K'>";}if(like_bool($l)&&$K!="&nbsp;")$K=($V?'tak':'nie');if($x)$K="<a href='$x'>$K</a>";if(!$x&&!like_bool($l)&&ereg('int|float|double|decimal',$l["type"]))$K="<div class='number'>$K</div>";elseif(ereg('date',$l["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($X,$f,$t){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Szukaj'."</legend><div>\n";$hc=array();foreach($X
as$v=>$V)$hc[$V["col"]]=$v;$q=0;$m=fields($_GET["select"]);foreach($f
as$C=>$Wa){$l=$m[$C];if(ereg("enum",$l["type"])||like_bool($l)){$v=$hc[$C];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($C)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'nie','tak'),$X[$v]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$v]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$C]);}elseif(is_array($Kc=$this->_foreignKeyOptions($_GET["select"],$C))){if($m[$C]["null"])$Kc[0]='('.'puste'.')';$v=$hc[$C];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($C)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Kc,$X[$v]["val"],true)."</select></div>\n";unset($f[$C]);}}$q=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$q][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$q][val]' value='".h($V["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'gdziekolwiek'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($D,$f,$t){$Mc=array();foreach($t
as$v=>$s){$D=array();foreach($s["columns"]as$V)$D[]=$f[$V];if(count(array_filter($D,'strlen'))>1&&$v!="PRIMARY")$Mc[$v]=implode(", ",$D);}if($Mc){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Mc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
auth_error($sb=null){global$g,$b,$Nd;$rd=session_name();$k="";if(!$_COOKIE[$rd]&&$_GET[$rd]&&ini_bool("session.use_only_cookies"))$k='Wymagana jest obsługa sesji w PHP.';elseif(isset($_GET["username"])){if(($_COOKIE[$rd]||$_GET[$rd])&&!$Nd)$k='Sesja wygasła, zaloguj się ponownie.';else{$G=&get_session("pwds");if($G!==null){$k=h($sb?$sb->getMessage():(is_string($g)?$g:'Nieprawidłowe dane logowania.'));$G=null;}unset_permanent();}}page_header('Zaloguj się',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Brak rozszerzenia',sprintf('Żadne z rozszerzeń PHP umożliwiających połączenie się z bazą danych (%s) nie jest dostępne.',implode(", ",$Uc)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$Nd=$_SESSION["token"];if($d&&$_POST["token"])$_POST["token"]=$Nd;$k=($_POST?($_POST["token"]==$Nd?"":'Nieprawidłowy token CSRF. Spróbuj wysłać formularz ponownie.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Przesłano zbyt dużo danych. Zmniejsz objętość danych lub zwiększ zmienną konfiguracyjną %s.','"post_max_size"')));if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}$g->select_db($b->database());function
...
';if($m){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($be?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}echo($be?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
