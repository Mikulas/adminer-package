...
confirm($Ia="",$hd=false){return" onclick=\"".($hd?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($Ia?" (' + $Ia + ')":"")."');\"";}function
...
upload_error($h){$jc=($h==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($h?'Wgranie pliku było niemożliwe.'.($jc?" ".sprintf('Maksymalna wielkość pliku to %sB.',$jc):""):'Plik nie istnieje.');}function
...
enum_input($Bd,$c,$i,$W,$fb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$i["length"],$x);$I=(isset($fb)?"<label><input type='$Bd'$c value='$fb'".((is_array($W)?in_array($fb,$W):$W===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($x[1]as$n=>$V){$V=stripcslashes(str_replace("''","'",$V));$za=(is_int($W)?$W==$n+1:(is_array($W)?in_array($n+1,$W):$W===$V));$I.=" <label><input type='$Bd'$c value='".($n+1)."'".($za?' checked':'').'>'.h($b->editVal($V,$i)).'</label>';}return$I;}function
input($i,$W,$l){global$Dd,$b,$Wb;$_=h(bracket_escape($i["field"]));echo"<td class='function'>";$Tc=($Wb=="mssql"&&$i["auto_increment"]);if($Tc&&!$_POST["save"]){$l=null;}$Fb=(isset($_GET["select"])||$Tc?array("orig"=>'bez zmian'):array())+$b->editFunctions($i);$c=" name='fields[$_]'";if($i["type"]=="enum"){echo
...
selectLinks($od,$N=""){$a=$od["Name"];if(isset($N)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Nowy rekord'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Ostatnia strona'."'>&gt;&gt;</a>\n";}function
...
as$d=>$V){$u.="&set".urlencode("[".bracket_escape($d)."]")."=".urlencode($J[$V]);}echo"<a href='".h($u)."' title='".'Nowy rekord'."'>+</a> ";}}}function
...
selectVal($V,$u,$i){$I=($V=="<i>NULL</i>"?"&nbsp;":$V);if(ereg('blob|bytea',$i["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtów'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V)){$I="<img src='$u' alt='$I'>";}}if(like_bool($i)&&$I!="&nbsp;"){$I='<img src="'.($V?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.3").'" alt="'.h($V).'">';}if($u){$I="<a href='$u'>$I</a>";}if(!$u&&!like_bool($i)&&ereg('int|float|double|decimal',$i["type"])){$I="<div class='number'>$I</div>";}elseif(ereg('date',$i["type"])){$I="<div class='datetime'>$I</div>";}return$I;}function
...
selectSearchPrint($X,$e,$q){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Szukaj'."</legend><div>\n";$Xb=array();foreach($X
...
as$_=>$Sa){$A=$this->_foreignKeyOptions($_GET["select"],$_);if(is_array($A)){if($j[$_]["null"]){$A[0]='('.'puste'.')';}$s=$Xb[$_];$n--;echo"<div>".h($Sa)."<input type='hidden' name='where[$n][col]' value='".h($_)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($A,$X[$s]["val"],true)."</select></div>\n";unset($e[$_]);}}$n=0;foreach($X
as$V){if(($V["col"]==""||$e[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$n][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($e,$V["col"],true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$n][val]' value='".h($V["val"])."'></div>\n";$n++;}}echo"<div><select name='where[$n][col]' onchange='selectAddRow(this);'><option value=''>(".'gdziekolwiek'.")".optionlist($e,null,true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators),"<input name='where[$n][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$e,$q){$_c=array();foreach($q
as$s=>$p){$B=array();foreach($p["columns"]as$V){$B[]=$e[$V];}if(count(array_filter($B,'strlen'))>1&&$s!="PRIMARY"){$_c[$s]=implode(", ",$B);}}if($_c){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$_c,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
auth_error($lb=null){global$f,$b,$yd;$cd=session_name();$h="";if(!$_COOKIE[$cd]&&$_GET[$cd]&&ini_bool("session.use_only_cookies")){$h='Wymagana jest obsługa sesji w PHP.';}elseif(isset($_GET["username"])){if(($_COOKIE[$cd]||$_GET[$cd])&&!$yd){$h='Sesja wygasła, zaloguj się ponownie.';}else{$E=&get_session("pwds");if(isset($E)){$h=h($lb?$lb->getMessage():(is_string($f)?$f:'Nieprawidłowe dane logowania.'));$E=null;}}}page_header('Zaloguj się',$h,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Brak rozszerzenia',sprintf('Żadne z rozszerzeń PHP umożliwiających połączenie się z bazą danych (%s) nie jest dostępne.',implode(", ",$Hc)),false);page_footer("auth");exit;}$f=connect();}if(is_string($f)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$yd=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$yd;}$h=($_POST?($_POST["token"]==$yd?"":'Nieprawidłowy token CSRF. Spróbuj wysłać formularz ponownie.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Przesłano zbyt dużo danych. Zmniejsz objętość danych lub zwiększ zmienną konfiguracyjną %s.','"post_max_size"')));$f->select_db($b->database());function
...
like_bool($i){return
ereg("bool|(tinyint|bit)\\(1\\)",$i["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$tc="RESTRICT|CASCADE|SET NULL|NO ACTION";$Xa[DRIVER]='Zaloguj się';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$f->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Ld=(isset($_GET["select"])?$_POST["edit"]:$X);$j=fields($a);foreach($j
as$_=>$i){if(!isset($i["privileges"][$Ld?"update":"insert"])||$b->fieldName($i)==""){unset($j[$_]);}}if($_POST&&!$h&&!isset($_GET["select"])){$v=$_POST["referer"];if($_POST["insert"]){$v=($Ld?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$v)){$v=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$v,'Rekord został usunięty.');}else{$N=array();foreach($j
as$_=>$i){$V=process_input($i);if($V!==false&&$V!==null){$N[idf_escape($_)]=($Ld?"\n".idf_escape($_)." = $V":$V);}}if($Ld){if(!$N){redirect($v);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$v,'Rekord został zaktualizowany.');}else{$H=insert_into($a,$N);$bc=($H?last_id():0);queries_redirect($v,sprintf('Rekord%s został dodany.',($bc?" $bc":"")),$H);}}}$pd=$b->tableName(table_status($a));page_header(($Ld?'Edytuj':'Dodaj'),$h,array("select"=>array($a,$pd)),$pd);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($X){$L=array();foreach($j
as$_=>$i){if(isset($i["privileges"]["select"])){$L[]=($_POST["clone"]&&$i["auto_increment"]?"'' AS ":(ereg("enum|set",$i["type"])?"1*".idf_escape($_)." AS ":"")).idf_escape($_);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Brak rekordów.'."\n";}echo'
...
';if($j){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Ld?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}}echo($Ld?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$j?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
