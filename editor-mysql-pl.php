...
nl_br($O){return
str_replace("\n","<br>",$O);}function
checkbox($A,$W,$Ba,$pc="",$Nc="",$lc=false){static$q=0;$q++;$I="<input type='checkbox' name='$A' value='".h($W)."'".($Ba?" checked":"").($Nc?' onclick="'.h($Nc).'"':'').($lc?" class='jsonly'":"")." id='checkbox-$q'>";return($pc!=""?"<label for='checkbox-$q'>$I".h($pc)."</label>":$I);}function
...
confirm($Oa=""){return" onclick=\"return confirm('".'Czy jesteÅ› pewien?'.($Oa?" (' + $Oa + ')":"")."');\"";}function
...
ini_bool($hc){$V=ini_get($hc);return(eregi('^(on|true|yes)$',$V)||(int)$V);}function
...
as$fc){if(ereg("PRIMARY|UNIQUE",$fc["type"])){$I=array();foreach($fc["columns"]as$t){if(!isset($J[$t]))continue
2;$I[$t]=$J[$t];}return$I;}}}function
where($X,$m=array()){global$s;$I=array();$Qb='(^[\w\(]+'.str_replace("_",".*",preg_quote(idf_escape("_"))).'\)+$)';foreach((array)$X["where"]as$t=>$V){$t=bracket_escape($t,1);$I[]=(preg_match($Qb,$t)?$t:idf_escape($t)).(($s=="sql"&&ereg('\\.',$V))||$s=="mssql"?" LIKE ".exact_value(addcslashes($V,"%_\\")):" = ".unconvert_field($m[$t],exact_value($V)));}foreach((array)$X["null"]as$t)$I[]=idf_escape($t)." IS NULL";return
...
upload_error($k){$_c=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($_c?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$_c):""):'Plik nie istnieje.');}function
...
as$p=>$Da){$jb=$Za[$Da];if(!isset($jb))$jb=$se.$se[0];$I.=$jb;if($p)$Za[]=$se.$jb[0];$se=$jb;}return$I;}global$b,$g,$eb,$hb,$qb,$k,$Rb,$Ub,$aa,$ic,$s,$ba,$qc,$Lc,$Yc,$Ed,$S,$Yd,$be,$ie,$ca;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$aa=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$Wc=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$aa);if(version_compare(PHP_VERSION,'5.2.0')>=0)$Wc[]=true;call_user_func_array('session_set_cookie_params',$Wc);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Eb);if(function_exists("set_magic_quotes_runtime"))set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'pl';}function
lang($Xd,$Ic=null){if(is_array($Xd)){$ad=($Ic==1?0:($Ic%10>1&&$Ic%10<5&&$Ic/10%10!=1?1:2));$Xd=$Xd[$ad];}$Xd=str_replace("%d","%s",$Xd);$Ic=number_format($Ic,0,".",' ');return
...
true;}}return
false;}function
auto_increment(){$qa=" PRIMARY KEY";if($_GET["create"]!=""&&$_POST["auto_increment_col"]){foreach(indexes($_GET["create"])as$fc){if(in_array($_POST["fields"][$_POST["auto_increment_col"]]["orig"],$fc["columns"],true)){$qa="";break;}if($fc["type"]=="PRIMARY")$qa=" UNIQUE";}}return" AUTO_INCREMENT$qa";}function
...
routine($A,$Zd){global$g,$qb,$ic,$be;$ka=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$ae="((".implode("|",array_merge(array_keys($be),$ka)).")\\b(?:\\s*\\(((?:[^'\")]*|$qb)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$F="\\s*(".($Zd=="FUNCTION"?"":$ic).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$ae";$Pa=$g->result("SHOW CREATE $Zd ".idf_escape($A),2);preg_match("~\\(((?:$F\\s*,?)*)\\)\\s*".($Zd=="FUNCTION"?"RETURNS\\s+$ae\\s+":"")."(.*)~is",$Pa,$x);$m=array();preg_match_all("~$F\\s*,?~is",$x[1],$y,PREG_SET_ORDER);foreach($y
...
selectLinks($Jd,$N=""){$a=$Jd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Nowy rekord'."</a>\n";}function
...
as$e=>$V)$v.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($J[$V]);echo"<a href='".h($v)."' title='".'Nowy rekord'."'>+</a> ";}}}function
...
selectVal($V,$v,$l){$I=($V===null?"&nbsp;":$V);$v=h($v);if(ereg('blob|bytea',$l["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$I="<img src='$v' alt='$I'>";}if(like_bool($l)&&$I!="&nbsp;")$I=($V?'tak':'nie');if($v)$I="<a href='$v'>$I</a>";if(!$v&&!like_bool($l)&&ereg('int|float|double|decimal',$l["type"]))$I="<div class='number'>$I</div>";elseif(ereg('date',$l["type"]))$I="<div class='datetime'>$I</div>";return$I;}function
...
as$A=>$Xa){$l=$m[$A];if(ereg("enum",$l["type"])||like_bool($l)){$t=$nc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'nie','tak'),$X[$t]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$t]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$A]);}elseif(is_array($Rc=$this->_foreignKeyOptions($_GET["select"],$A))){if($m[$A]["null"])$Rc[0]='('.'puste'.')';$t=$nc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Rc,$X[$t]["val"],true)."</select></div>\n";unset($f[$A]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$p][val]' value='".h($V["val"])."' onsearch='selectSearchSearch(this);'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'gdziekolwiek'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$p][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$f,$r){$Tc=array();foreach($r
as$t=>$fc){$B=array();foreach($fc["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$t!="PRIMARY")$Tc[$t]=implode(", ",$B);}if($Tc){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectActionPrint($r){echo"<fieldset><legend>".'CzynnoÅ›Ä‡'."</legend><div>","<input type='submit' value='".'pokaÅ¼'."'>","</div></fieldset>\n";}function
selectCommandPrint(){return
true;}function
selectImportPrint(){return
true;}function
selectEmailPrint($lb,$f){if($lb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Nadawca'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Temat'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Dodaj'."'>\n";echo"<p>".'ZaÅ‚Ä…czniki'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($lb)==1?'<input type="hidden" name="email_field" value="'.h(key($lb)).'">':html_select("email_field",$lb)),"<input type='submit' name='email' value='".'WyÅ›lij'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
selectColumnsProcess($f,$r){return
array(array(),array());}function
selectSearchProcess($m,$r){$I=array();foreach((array)$_GET["where"]as$t=>$X){$Fa=$X["col"];$Oc=$X["op"];$V=$X["val"];if(($t<0?"":$Fa).$V!=""){$La=array();foreach(($Fa!=""?array($Fa=>$m[$Fa]):$m)as$A=>$l){if($Fa!=""||is_numeric($V)||!ereg('int|float|double|decimal',$l["type"])){$A=idf_escape($A);if($Fa!=""&&$l["type"]=="enum")$La[]=(in_array(0,$V)?"$A IS NULL OR ":"")."$A IN (".implode(", ",array_map('intval',$V)).")";else{$Pd=ereg('char|text|enum|set',$l["type"]);$W=$this->processInput($l,(!$Oc&&$Pd&&ereg('^[^%]+$',$V)?"%$V%":$V));$La[]=$A.($W=="NULL"?" IS".($Oc==">="?" NOT":"")." $W":(in_array($Oc,$this->operators)||$Oc=="="?" $Oc $W":($Pd?" LIKE $W":" IN (".str_replace(",","', '",$W).")")));if($t<0&&$V=="0")$La[]="$A IS NULL";}}}$I[]=($La?"(".implode(" OR ",$La).")":"0");}}return$I;}function
selectOrderProcess($m,$r){$gc=$_GET["index_order"];if($gc!="")unset($_GET["order"][1]);if($_GET["order"])return
array(idf_escape(reset($_GET["order"])).($_GET["desc"]?" DESC":""));foreach(($gc!=""?array($r[$gc]):$r)as$fc){if($gc!=""||$fc["type"]=="INDEX"){$Xa=false;foreach($fc["columns"]as$V){if(ereg('date|timestamp',$m[$V]["type"])){$Xa=true;break;}}$I=array();foreach($fc["columns"]as$V)$I[]=idf_escape($V).($Xa?" DESC":"");return$I;}}return
array();}function
selectLimitProcess(){return(isset($_GET["limit"])?$_GET["limit"]:"50");}function
selectLengthProcess(){return"100";}function
selectEmailProcess($X,$Jb){if($_POST["email_append"])return
true;if($_POST["email"]){$wd=0;if($_POST["all"]||$_POST["check"]){$l=idf_escape($_POST["email_field"]);$Gd=$_POST["email_subject"];$z=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$Gd.$z",$y);$K=get_rows("SELECT DISTINCT $l".($y[1]?", ".implode(", ",array_map('idf_escape',array_unique($y[1]))):"")." FROM ".table($_GET["select"])." WHERE $l IS NOT NULL AND $l != ''".($X?" AND ".implode(" AND ",$X):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$m=fields($_GET["select"]);foreach($this->rowDescriptions($K,$Jb)as$J){$nd=array('{\\'=>'{');foreach($y[1]as$V)$nd['{$'."$V}"]=$this->editVal($J[$V],$m[$V]);$kb=$J[$_POST["email_field"]];if(is_mail($kb)&&send_mail($kb,strtr($Gd,$nd),strtr($z,$nd),$_POST["email_from"],$_FILES["email_files"]))$wd++;}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(array('WysÅ‚ano %d e-mail.','WysÅ‚ano %d e-maile.','WysÅ‚ano %d e-maili.'),$wd));}return
false;}function
selectQueryBuild($L,$X,$Sb,$B,$u,$D){return"";}function
messageQuery($G){return" <span class='time'>".@date("H:i:s")."</span><!--\n".str_replace("--","--><!-- ",$G)."\n-->";}function
editFunctions($l){$I=array();if($l["null"]&&ereg('blob',$l["type"]))$I["NULL"]='puste';$I[""]=($l["null"]||$l["auto_increment"]||like_bool($l)?"":"*");if(ereg('date|time',$l["type"]))$I["now"]='teraz';if(eregi('_(md5|sha1)$',$l["field"],$x))$I[]=strtolower($x[1]);return$I;}function
...
auth_error($ub=null){global$g,$b,$S;$zd=session_name();$k="";if(!$_COOKIE[$zd]&&$_GET[$zd]&&ini_bool("session.use_only_cookies"))$k='Wymagana jest obsÅ‚uga sesji w PHP.';elseif(isset($_GET["username"])){if(($_COOKIE[$zd]||$_GET[$zd])&&!$S)$k='Sesja wygasÅ‚a, zaloguj siÄ™ ponownie.';else{$E=&get_session("pwds");if($E!==null){$k=h($ub?$ub->getMessage():(is_string($g)?$g:'NieprawidÅ‚owe dane logowania.'));$E=null;}unset_permanent();}}page_header('Zaloguj siÄ™',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Brak rozszerzenia',sprintf('Å»adne z rozszerzeÅ„ PHP umoÅ¼liwiajÄ…cych poÅ‚Ä…czenie siÄ™ z bazÄ… danych (%s) nie jest dostÄ™pne.',implode(", ",$bd)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$S=$_SESSION["token"];if($d&&$_POST["token"])$_POST["token"]=$S;$k='';if($_POST){if($_POST["token"]!=$S){$hc="max_input_vars";$Ac=ini_get($hc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$t){$V=ini_get($t);if($V&&(!$Ac||$V<$Ac)){$hc=$t;$Ac=$V;}}}$k=(!$_POST["token"]&&$Ac?sprintf('Przekroczono maksymalnÄ… liczbÄ™ pÃ³l. ZwiÄ™ksz %s.',"'$hc'"):'NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST")$k=sprintf('PrzesÅ‚ano zbyt duÅ¼o danych. Zmniejsz objÄ™toÅ›Ä‡ danych lub zwiÄ™ksz zmiennÄ… konfiguracyjnÄ… %s.',"'post_max_size'");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}$g->select_db($b->database());function
...
like_bool($l){return
ereg("bool|(tinyint|bit)\\(1\\)",$l["full_type"]);}$Lc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$eb[DRIVER]='Zaloguj siÄ™';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$m=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$m),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$m=fields($a);$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$m):""):where($_GET,$m));$je=(isset($_GET["select"])?$_POST["edit"]:$X);foreach($m
as$A=>$l){if(!isset($l["privileges"][$je?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$w=$_POST["referer"];if($_POST["insert"])$w=($je?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$w))$w=ME."select=".urlencode($a);$r=indexes($a);$ee=unique_array($_GET["where"],$r);$id="\nWHERE $X";if(isset($_POST["delete"])){$G="FROM ".table($a);query_redirect("DELETE".($ee?" $G$id":limit1($G,$id)),$w,'Rekord zostaÅ‚ usuniÄ™ty.');}else{$N=array();foreach($m
as$A=>$l){$V=process_input($l);if($V!==false&&$V!==null)$N[idf_escape($A)]=($je?"\n".idf_escape($A)." = $V":$V);}if($je){if(!$N)redirect($w);$G=table($a)." SET".implode(",",$N);query_redirect("UPDATE".($ee?" $G$id":limit1($G,$id)),$w,'Rekord zostaÅ‚ zaktualizowany.');}else{$H=insert_into($a,$N);$sc=($H?last_id():0);queries_redirect($w,sprintf('Rekord%s zostaÅ‚ dodany.',($sc?" $sc":"")),$H);}}}$Kd=$b->tableName(table_status($a,true));page_header(($je?'Edytuj':'Dodaj'),$k,array("select"=>array($a,$Kd)),$Kd);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($m
as$A=>$l){if(isset($l["privileges"]["select"])){$ma=convert_field($l);if($_POST["clone"]&&$l["auto_increment"])$ma="''";if($s=="sql"&&ereg("enum|set",$l["type"]))$ma="1*".idf_escape($A);$L[]=($ma?"$ma AS ":"").idf_escape($A);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false)echo"<p class='error'>".'Brak rekordÃ³w.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$m)echo"<p class='error'>".'Brak uprawnieÅ„ do edycji tej tabeli'."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($m
as$A=>$l){echo"<tr><th>".$b->fieldName($l);$Wa=$_GET["set"][bracket_escape($A)];if($Wa===null){$Wa=$l["default"];if($l["type"]=="bit"&&ereg("^b'([01]*)'\$",$Wa,$ld))$Wa=$ld[1];}$W=($J!==null?($J[$A]!=""&&$s=="sql"&&ereg("enum|set",$l["type"])?(is_array($J[$A])?array_sum($J[$A]):+$J[$A]):$J[$A]):(!$je&&$l["auto_increment"]?"":(isset($_GET["select"])?false:$Wa)));if(!$_POST["save"]&&is_string($W))$W=$b->editVal($W,$l);$o=($_POST["save"]?(string)$_POST["function"][$A]:($je&&$l["on_update"]=="CURRENT_TIMESTAMP"?"now":($W===false?null:($W!==null?'':'NULL'))));if(ereg("time",$l["type"])&&$W=="CURRENT_TIMESTAMP"){$W="";$o="now";}input($l,$W,$o);echo"\n";}echo"</table>\n";}echo'<p>
';if($m){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($je?'Zapisz i kontynuuj edycjÄ™':'Zapisz i dodaj nastÄ™pny')."' title='Ctrl+Shift+Enter'>\n";}echo($je?"<input type='submit' name='delete' value='".'UsuÅ„'."' onclick=\"return confirm('".'Czy jesteÅ› pewien?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status($a);$r=indexes($a);$m=fields($a);$Lb=column_foreign_keys($a);$Kc="";if($R["Oid"]=="t"){$Kc=($s=="sqlite"?"rowid":"oid");$r[]=array("type"=>"PRIMARY","columns"=>array($Kc));}parse_str($_COOKIE["adminer_import"],$ha);$rd=array();$f=array();$Od=null;foreach($m
as$t=>$l){$A=$b->fieldName($l);if(isset($l["privileges"]["select"])&&$A!=""){$f[$t]=html_entity_decode(strip_tags($A),ENT_QUOTES);if(is_shortable($l))$Od=$b->selectLengthProcess();}$rd+=$l["privileges"];}list($L,$Sb)=$b->selectColumnsProcess($f,$r);$kc=count($Sb)<count($L);$X=$b->selectSearchProcess($m,$r);$B=$b->selectOrderProcess($m,$r);$u=$b->selectLimitProcess();$Pb=($L?implode(", ",$L):"*".($Kc?", $Kc":"")).convert_fields($f,$m,$L)."\nFROM ".table($a);$Tb=($Sb&&$kc?"\nGROUP BY ".implode(", ",$Sb):"").($B?"\nORDER BY ".implode(", ",$B):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$fe=>$J){$ma=convert_field($m[key($J)]);echo$g->result("SELECT".limit($ma?$ma:idf_escape(key($J))." FROM ".table($a)," WHERE ".where_check($fe,$m).($X?" AND ".implode(" AND ",$X):"").($B?" ORDER BY ".implode(", ",$B):""),1));}exit;}if($_POST&&!$k){$re=$X;if(is_array($_POST["check"]))$re[]="((".implode(") OR (",array_map('where_check',$_POST["check"]))."))";$re=($re?"\nWHERE ".implode(" AND ",$re):"");$cd=$he=null;foreach($r
as$fc){if($fc["type"]=="PRIMARY"){$cd=array_flip($fc["columns"]);$he=($L?$cd:array());break;}}foreach((array)$he
...
as$A=>$V){$V=process_input($m[$A]);if($V!==null){if($_POST["clone"])$N[idf_escape($A)]=($V!==false?$V:idf_escape($A));elseif($V!==false)$N[]=idf_escape($A)." = $V";}}$G.=($_POST["clone"]?" (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a):" SET\n".implode(",\n",$N));}if($_POST["delete"]||$N){$Ja="UPDATE";if($_POST["delete"]){$Ja="DELETE";$G="FROM $G";}if($_POST["clone"]){$Ja="INSERT";$G="INTO $G";}if($_POST["all"]||($he===array()&&$_POST["check"])||$kc){$H=queries("$Ja $G$re");$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$H=queries($Ja.limit1($G,"\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V,$m)));if(!$H)break;$ia+=$g->affected_rows;}}}$z=lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordÃ³w.'),$ia);if($_POST["clone"]&&$H&&$ia==1){$sc=last_id();if($sc)$z=sprintf('Rekord%s zostaÅ‚ dodany.'," $sc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$z,$H);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+kliknij wartoÅ›Ä‡, aby jÄ… edytowaÄ‡.';else{$H=true;$ia=0;foreach($_POST["val"]as$fe=>$J){$N=array();foreach($J
as$t=>$V){$t=bracket_escape($t,1);$N[]=idf_escape($t)." = ".(ereg('char|text',$m[$t]["type"])||$V!=""?$b->processInput($m[$t],$V):"NULL");}$G=table($a)." SET ".implode(", ",$N);$qe=" WHERE ".where_check($fe,$m).($X?" AND ".implode(" AND ",$X):"");$H=queries("UPDATE".($kc?" $G$qe":limit1($G,$qe)));if(!$H)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordÃ³w.'),$ia),$H);}}elseif(is_string($Bb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$H=true;$Ia=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Bb,$y);$ia=count($y[0]);begin();$xd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($y[0]as$t=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$xd]*)$xd~",$V.$xd,$yc);if(!$t&&!array_diff($yc[1],$Ia)){$Ia=$yc[1];$ia--;}else{$N=array();foreach($yc[1]as$p=>$Fa)$N[idf_escape($Ia[$p])]=($Fa==""&&$m[$Ia[$p]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Fa))));$H=insert_update($a,$N,$cd);if(!$H)break;}}if($H)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('%d rekord zostaÅ‚ zaimportowany.','%d rekordy zostaÅ‚y zaimportowane.','%d rekordÃ³w zostaÅ‚o zaimportowanych.'),$ia),$H);queries("ROLLBACK");}else$k=upload_error($Bb);}}$Kd=$b->tableName($R);if(is_ajax())ob_start();page_header('pokaÅ¼'.": $Kd",$k);$N=null;if(isset($rd["insert"])){$N="";foreach((array)$_GET["where"]as$V){if(count($Lb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$N.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($R,$N);if(!$f)echo"<p class='error'>".'Nie udaÅ‚o siÄ™ pobraÄ‡ danych z tabeli'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($X,$f,$r);$b->selectOrderPrint($B,$f,$r);$b->selectLimitPrint($u);$b->selectLengthPrint($Od);$b->selectActionPrint($r);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$n=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$D=floor(max(0,$n-1)/$u);}$G=$b->selectQueryBuild($L,$X,$Sb,$B,$u,$D);if(!$G)$G="SELECT".limit((+$u&&$Sb&&$kc&&$s=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Pb,($X?"\nWHERE ".implode(" AND ",$X):"").$Tb,($u!=""?+$u:null),($D?$u*$D:0),"\n");echo$b->selectQuery($G);$H=$g->query($G);if(!$H)echo"<p class='error'>".error()."\n";else{if($s=="mssql"&&$D)$H->seek($u*$D);$mb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($D&&$s=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last")$n=(+$u&&$Sb&&$kc?($s=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($G) x")):count($K));if(!$K)echo"<p class='message'>".'Brak rekordÃ³w.'."\n";else{$ua=$b->backwardKeys($a,$Kd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Sb&&$L?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'edytuj'."</a>");$Gc=array();$Rb=array();reset($L);$jd=1;foreach($K[0]as$t=>$V){if($t!=$Kc){$V=$_GET["columns"][key($L)];$l=$m[$L?($V?$V["col"]:current($L)):$t];$A=($l?$b->fieldName($l,$jd):"*");if($A!=""){$jd++;$Gc[$t]=$A;$e=idf_escape($t);$ac=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($t);$Xa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($ac.($B[0]==$e||$B[0]==$t||(!$B&&$kc&&$Sb[0]==$e)?$Xa:'')).'">';echo(!$L||$V?apply_sql_function($V["fun"],$A):h(current($L)))."</a>";echo"<span class='column hidden'>","<a href='".h($ac.$Xa)."' title='".'malejÄ…co'."' class='text'> â†“</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($t)).'\'); return false;" title="'.'Szukaj'.'" class="text jsonly"> =</a>';echo"</span>";}$Rb[$t]=$V["fun"];next($L);}}$vc=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
as$t=>$V)$vc[$t]=max($vc[$t],min(40,strlen(utf8_decode($V))));}}echo($ua?"<th>".'Relacje':"")."</thead>\n";if(is_ajax()){if($u%2==1&&$D%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($K,$Lb)as$_=>$J){$ee=unique_array($K[$_],$r);if(!$ee){$ee=array();foreach($K[$_]as$t=>$V){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$t))$ee[$t]=$V;}}$fe="";foreach($ee
as$t=>$V){if(strlen($V)>64){$t="MD5(".(strpos($t,'(')?$t:idf_escape($t)).")";$V=md5($V);}$fe.="&".($V!==null?urlencode("where[".bracket_escape($t)."]")."=".urlencode($V):"null%5B%5D=".urlencode($t));}echo"<tr".odd().">".(!$Sb&&$L?"":"<td>".checkbox("check[]",substr($fe,1),in_array(substr($fe,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($kc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$fe)."'>".'edytuj'."</a>"));foreach($J
