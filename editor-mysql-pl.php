...
as$W=>$p){unset($ra[$c][$W]);if(is_array($p)){$ra[$c][stripslashes($W)]=$p;$ra[]=&$ra[$c][stripslashes($W)];}else{$ra[$c][stripslashes($W)]=($zb?$p:stripslashes($p));}}}}}function
...
nl_br($F){return
str_replace("\n","<br>",$F);}function
checkbox($f,$i,$cb,$xc="",$hb="",$bd=false){static$_=0;$_++;$b="<input type='checkbox' name='$f' value='".h($i)."'".($cb?" checked":"").($hb?' onclick="'.h($hb).'"':'').($bd?" class='jsonly'":"")." id='checkbox-$_'>";return($xc!=""?"<label for='checkbox-$_'>$b".h($xc)."</label>":$b);}function
optionlist($Z,$Xc=null,$gc=false){$b="";foreach($Z
as$W=>$p){$nc=array($W=>$p);if(is_array($p)){$b.='<optgroup label="'.h($W).'">';$nc=$p;}foreach($nc
as$c=>$a){$b.='<option'.($gc||is_string($c)?' value="'.h($c).'"':'').(($gc||is_string($c)?(string)$c:$a)===$Xc?' selected':'').'>'.h($a);}if(is_array($p)){$b.='</optgroup>';}}return$b;}function
...
print_fieldset($_,$Dd,$Ed=false,$hb=""){echo"<fieldset><legend><a href='#fieldset-$_' onclick=\"".h($hb)."return !toggle('fieldset-$_');\">$Dd</a></legend><div id='fieldset-$_'".($Ed?"":" class='hidden'").">\n";}function
...
pagination($y,$_d){return" ".($y==$_d?$y+1:'<a href="'.h(remove_from_uri("page").($y?"&page=$y":"")).'">'.($y+1)."</a>");}function
get_file($c,$Rb=false){$R=$_FILES[$c];if(!$R||$R["error"]){return$R["error"];}$b=file_get_contents($Rb&&ereg('\\.gz$',$R["name"])?"compress.zlib://$R[tmp_name]":($Rb&&ereg('\\.bz2$',$R["name"])?"compress.bzip2://$R[tmp_name]":$R["tmp_name"]));if($Rb){$Qc=substr($b,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$Qc,$Rd)){$b=iconv("utf-16","utf-8",$b);}elseif($Qc=="\xEF\xBB\xBF"){$b=substr($b,3);}}return$b;}function
...
as$W=>$p){$ra[$c."[$W]"]=$p;}}elseif(!in_array($c,$Kd)){echo'<input type="hidden" name="'.h($c).'" value="'.h($a).'">';}}}function
...
enum_input($ua,$L,$e,$i,$Ib=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($Ib)?"<label><input type='$ua'$L value='$Ib'".((is_array($i)?in_array($Ib,$i):$i===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($U[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$cb=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ua'$L value='".($n+1)."'".($cb?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$z){global$_a,$j,$ka;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$Ac=($ka=="mssql"&&$e["auto_increment"]);if($Ac&&!$_POST["save"]){$z=null;}$da=(isset($_GET["select"])||$Ac?array("orig"=>'bez zmian'):array())+$j->editFunctions($e);$L=" name='fields[$f]'";if($e["type"]=="enum"){echo
nbsp($da[""])."<td>".$j->editInput($_GET["edit"],$e,$L,$i);}else{$ca=0;foreach($da
as$c=>$a){if($c===""||!$a){break;}$ca++;}$ya=($ca?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($e["field"])))."]']; if ($ca > f.selectedIndex) f.selectedIndex = $ca;\"":"");$L.=$ya;echo(count($da)>1?html_select("function[$f]",$da,!isset($z)||in_array($z,$da)||isset($da[$z])?$z:"","functionChange(this);"):nbsp(reset($da))).'<td>';$zc=$j->editInput($_GET["edit"],$e,$L,$i);if($zc!=""){echo$zc;}elseif($e["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);foreach($U[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$cb=(is_int($i)?($i>>$n)&1:in_array($a,explode(",",$i),true));echo" <label><input type='checkbox' name='fields[$f][$n]' value='".(1<<$n)."'".($cb?' checked':'')."$ya>".h($j->editVal($a,$e)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$e["type"])&&ini_bool("file_uploads")){echo"<input type='file' name='fields-$f'$ya>";}elseif(ereg('text|lob',$e["type"])){echo"<textarea ".($ka!="sqlite"||ereg("\n",$i)?"cols='50' rows='12'":"cols='30' rows='1' style='height: 1.2em;'")."$L>".h($i).'</textarea>';}else{$Ob=(!ereg('int',$e["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$e["length"],$m)?((ereg("binary",$e["type"])?2:1)*$m[1]+($m[3]?1:0)+($m[2]&&!$e["unsigned"]?1:0)):($_a[$e["type"]]?$_a[$e["type"]]+($e["unsigned"]?0:1):0));echo"<input value='".h($i)."'".($Ob?" maxlength='$Ob'":"").(ereg('char|binary',$e["type"])&&$Ob>20?" size='40'":"")."$L>";}}}function
...
is_mail($Fa){$Gc='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Za='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$K="$Gc+(\\.$Gc+)*@($Za?\\.)+$Za";return
preg_match("(^$K(,\\s*$K)*\$)i",$Fa);}function
is_url($F){$Za='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($Za?\\.)+$Za(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$F,$m)?strtolower($m[1]):"");}global$j,$g,$va,$id,$Ub,$v,$da,$nd,$db,$wc,$ka,$od,$Od,$ub,$mc,$ea,$Nd,$_a,$qd,$_b;if(!isset($_SERVER["REQUEST_URI"])){$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"].($_SERVER["QUERY_STRING"]!=""?"?$_SERVER[QUERY_STRING]":"");}$db=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$bb=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$db);if(version_compare(PHP_VERSION,'5.2.0')>=0){$bb[]=true;}call_user_func_array('session_set_cookie_params',$bb);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$zb);if(function_exists("set_magic_quotes_runtime")){set_magic_quotes_runtime(false);}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
...
dsn($ud,$D,$P,$td='auth_error'){set_exception_handler($td);parent::__construct($ud,$D,$P);restore_exception_handler();$this->setAttribute(13,array('Min_PDOStatement'));$this->server_info=$this->getAttribute(4);}function
...
connect($E,$D,$P){mysqli_report(MYSQLI_REPORT_OFF);list($dd,$Ab)=explode(":",$E,2);$b=@$this->real_connect(($E!=""?$dd:ini_get("mysqli.default_host")),($E.$D!=""?$D:ini_get("mysqli.default_user")),($E.$D.$P!=""?$P:ini_get("mysqli.default_pw")),null,(is_numeric($Ab)?$Ab:ini_get("mysqli.default_port")),(!is_numeric($Ab)?$Ab:null));if($b){if(method_exists($this,'set_charset')){$this->set_charset("utf8");}else{$this->query("SET NAMES utf8");}}return$b;}function
result($l,$e=0){$k=$this->query($l);if(!$k){return
...
limit($l,$o,$C,$Fc=0,$sa=" "){return" $l$o".(isset($C)?$sa."LIMIT $C".($Fc?" OFFSET $Fc":""):"");}function
limit1($l,$o){return
...
foreign_keys($h){global$g,$ub;static$K='`(?:[^`]|``)+`';$b=array();$Hc=$g->result("SHOW CREATE TABLE ".table($h),1);if($Hc){preg_match_all("~CONSTRAINT ($K) FOREIGN KEY \\(((?:$K,? ?)+)\\) REFERENCES ($K)(?:\\.($K))? \\(((?:$K,? ?)+)\\)(?: ON DELETE ($ub))?(?: ON UPDATE ($ub))?~",$Hc,$U,PREG_SET_ORDER);foreach($U
as$m){preg_match_all("~$K~",$m[2],$ac);preg_match_all("~$K~",$m[5],$Y);$b[idf_unescape($m[1])]=array("db"=>idf_unescape($m[4]!=""?$m[3]:$m[4]),"table"=>idf_unescape($m[4]!=""?$m[4]:$m[3]),"source"=>array_map('idf_unescape',$ac[0]),"target"=>array_map('idf_unescape',$Y[0]),"on_delete"=>$m[6],"on_update"=>$m[7],);}}return$b;}function
...
alter_table($h,$f,$q,$kd,$jd,$Dc,$za,$kb,$xd){$ia=array();foreach($q
as$e){$ia[]=($e[1]?($h!=""?($e[0]!=""?"CHANGE ".idf_escape($e[0]):"ADD"):" ")." ".implode($e[1]).($h!=""?" $e[2]":""):"DROP ".idf_escape($e[0]));}$ia=array_merge($ia,$kd);$yc="COMMENT=".q($jd).($Dc?" ENGINE=".q($Dc):"").($za?" COLLATE ".q($za):"").($kb!=""?" AUTO_INCREMENT=$kb":"").$xd;if($h==""){return
queries("CREATE TABLE ".table($f)." (\n".implode(",\n",$ia)."\n) $yc");}if($h!=$f){$ia[]="RENAME TO ".table($f);}$ia[]=$yc;return
...
routine($f,$ua){global$g,$Ub,$wc,$_a;$Id=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Bc="((".implode("|",array_merge(array_keys($_a),$Id)).")(?:\\s*\\(((?:[^'\")]*|$Ub)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$K="\\s*(".($ua=="FUNCTION"?"":$wc).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Bc";$gb=$g->result("SHOW CREATE $ua ".idf_escape($f),2);preg_match("~\\(((?:$K\\s*,?)*)\\)".($ua=="FUNCTION"?"\\s*RETURNS\\s+$Bc":"")."\\s*(.*)~is",$gb,$m);$q=array();preg_match_all("~$K\\s*,?~is",$m[1],$U,PREG_SET_ORDER);foreach($U
as$ha){$f=str_replace("``","`",$ha[2]).$ha[3];$q[]=array("field"=>$f,"type"=>strtolower($ha[5]),"length"=>preg_replace_callback("~$Ub~s",'normalize_enum',$ha[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$ha[8] $ha[7]"))),"full_type"=>$ha[4],"inout"=>strtoupper($ha[1]),"collation"=>strtolower($ha[9]),);}if($ua!="FUNCTION"){return
...
rowDescriptions($w,$Xa){$b=$w;foreach($w[0]as$c=>$a){if(list($h,$_,$f)=$this->_foreignColumn($Xa,$c)){$Kc=array();foreach($w
as$d){$Kc[$d[$c]]=exact_value($d[$c]);}$Vb=$this->_values[$h];if(!$Vb){$Vb=get_key_vals("SELECT $_, $f FROM ".table($h)." WHERE $_ IN (".implode(", ",$Kc).")");}foreach($w
as$x=>$d){if(isset($d[$c])){$b[$x][$c]=(string)$Vb[$d[$c]];}}}}return$b;}function
selectVal($a,$t,$e){$b=($a=="<i>NULL</i>"?"&nbsp;":$a);if(ereg('blob|bytea',$e["type"])&&!is_utf8($a)){$b=lang(array('%d bajt','%d bajty','%d bajtów'),strlen($a));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$a)){$b="<img src='$t' alt='$b'>";}}if(like_bool($e)&&$b!="&nbsp;"){$b='<img src="'.($a?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.2").'" alt="'.h($a).'">';}if($t){$b="<a href='$t'>$b</a>";}if(!$t&&!like_bool($e)&&ereg('int|float|double|decimal',$e["type"])){$b="<div class='number'>$b</div>";}elseif(ereg('date',$e["type"])){$b="<div class='datetime'>$b</div>";}return$b;}function
...
selectSearchPrint($o,$A,$G){$o=(array)$_GET["where"];echo'<fieldset><legend>'.'Szukaj'."</legend><div>\n";$Wb=array();foreach($o
as$c=>$a){$Wb[$a["col"]]=$c;}$n=0;$q=fields($_GET["select"]);foreach($q
as$f=>$e){$na=$A[$f];if(ereg("enum",$e["type"])&&$na!=""){$c=$Wb[$f];$n--;echo"<div>".h($na)."<input type='hidden' name='where[$n][col]' value='".h($f)."'>:",enum_input("checkbox"," name='where[$n][val][]'",$e,(array)$o[$c]["val"],($e["null"]?0:null)),"</div>\n";unset($A[$f]);}}foreach($A
as$f=>$na){$Z=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Z)){if($q[$f]["null"]){$Z[0]='('.'puste'.')';}$c=$Wb[$f];$n--;echo"<div>".h($na)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Z,$o[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($o
...
as$c=>$T){$V=array();foreach($T["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Fb[$c]=implode(", ",$V);}}if($Fb){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Fb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
selectSearchProcess($q,$G){$b=array();foreach((array)$_GET["where"]as$c=>$o){$qa=$o["col"];$Va=$o["op"];$a=$o["val"];if(($c<0?"":$qa).$a!=""){$Qa=array();foreach(($qa!=""?array($qa=>$q[$qa]):$q)as$f=>$e){if($qa!=""||is_numeric($a)||!ereg('int|float|double|decimal',$e["type"])){$f=idf_escape($f);if($qa!=""&&$e["type"]=="enum"){$Qa[]=(in_array(0,$a)?"$f IS NULL OR ":"")."$f IN (".implode(", ",array_map('intval',$a)).")";}else{$cc=ereg('char|text|enum|set',$e["type"]);$i=$this->processInput($e,(!$Va&&$cc&&ereg('^[^%]+$',$a)?"%$a%":$a));$Qa[]=$f.($i=="NULL"?" IS".($Va==">="?" NOT":"")." $i":(in_array($Va,$this->operators)||$Va=="="?" $Va $i":($cc?" LIKE $i":" IN (".str_replace(",","', '",$i).")")));if($c<0&&$a=="0"){$Qa[]="$f IS NULL";}}}}$b[]=($Qa?"(".implode(" OR ",$Qa).")":"0");}}return$b;}function
selectOrderProcess($q,$G){$tb=$_GET["index_order"];if($tb!=""){unset($_GET["order"][1]);}if($_GET["order"]){return
array(idf_escape(reset($_GET["order"])).($_GET["desc"]?" DESC":""));}foreach(($tb!=""?array($G[$tb]):$G)as$T){if($tb!=""||$T["type"]=="INDEX"){$na=false;foreach($T["columns"]as$a){if(ereg('date|timestamp',$q[$a]["type"])){$na=true;break;}}$b=array();foreach($T["columns"]as$a){$b[]=idf_escape($a).($na?" DESC":"");}return$b;}}return
array();}function
selectLimitProcess(){return(isset($_GET["limit"])?$_GET["limit"]:"30");}function
selectLengthProcess(){return"100";}function
selectEmailProcess($o,$Xa){if($_POST["email_append"]){return
true;}if($_POST["email"]){$rc=0;if($_POST["all"]||$_POST["check"]){$e=idf_escape($_POST["email_field"]);$sb=$_POST["email_subject"];$X=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$sb.$X",$U);$w=get_rows("SELECT DISTINCT $e".($U[1]?", ".implode(", ",array_map('idf_escape',array_unique($U[1]))):"")." FROM ".table($_GET["select"])." WHERE $e IS NOT NULL AND $e != ''".($o?" AND ".implode(" AND ",$o):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$q=fields($_GET["select"]);foreach($this->rowDescriptions($w,$Xa)as$d){$Kb=array('{\\'=>'{');foreach($U[1]as$a){$Kb['{$'."$a}"]=$this->editVal($d[$a],$q[$a]);}$Fa=$d[$_POST["email_field"]];if(is_mail($Fa)&&send_mail($Fa,strtr($sb,$Kb),strtr($X,$Kb),$_POST["email_from"],$_FILES["email_files"])){$rc++;}}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(array('Wysłano %d e-mail.','Wysłano %d e-maile.','Wysłano %d e-maili.'),$rc));}return
false;}function
...
';if($Ua=="auth"){$ca=true;foreach((array)$_SESSION["pwds"]["server"][""]as$D=>$P){if(isset($P)){if($ca){echo"<p onclick='eventStop(event);'>\n";$ca=false;}echo"<a href='".h(auth_url("server","",$D))."'>".($D!=""?h($D):"<i>".'puste'."</i>")."</a><br>\n";}}}else{echo'<form action="" method="post">
<p class="logout">
...
_foreignColumn($Xa,$H){foreach((array)$Xa[$H]as$pb){if(count($pb["source"])==1){$f=$this->rowDescription($pb["table"]);if($f!=""){$_=idf_escape($pb["target"][0]);return
array($pb["table"],$_,$f);}}}}function
...
as$c=>$a){$na=(is_array($a)?$a[1]:$a);if($na!=""){echo'<a href="'.h(ME."$c=").urlencode(is_array($a)?$a[0]:$a).'">'.h($na).'</a> &raquo; ';}}}echo"$tc\n";}}echo"<span id='loader'></span>\n","<h2>$uc</h2>\n";restart_session();$oc=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$kc=$_SESSION["messages"][$oc];if($kc){echo"<div class='message'>".implode("</div>\n<div class='message'>",$kc)."</div>\n";unset($_SESSION["messages"][$oc]);}$fa=&get_session("dbs");if(DB!=""&&$fa&&!in_array(DB,$fa,true)){$fa=null;}if($v){echo"<div class='error'>$v</div>\n";}define("PAGE_HEADER",1);}function
...
long2str($p,$Pb){$xa='';foreach($p
as$a){$xa.=pack('V',$a);}if($Pb){return
substr($xa,0,end($p));}return$xa;}function
str2long($xa,$Pb){$p=array_values(unpack('V*',str_pad($xa,4*ceil(strlen($xa)/4),"\0")));if($Pb){$p[]=strlen($xa);}return$p;}function
xxtea_mx($M,$N,$S,$W){return
int32((($M>>5&0x7FFFFFF)^$N<<2)+(($N>>3&0x1FFFFFFF)^$M<<4))^int32(($S^$N)+($W^$M));}function
encrypt_string($Pa,$c){if($Pa==""){return"";}$c=array_values(unpack("V*",pack("H*",md5($c))));$p=str2long($Pa,true);$x=count($p)-1;$M=$p[$x];$N=$p[0];$Sb=floor(6+52/($x+1));$S=0;while($Sb-->0){$S=int32($S+0x9E3779B9);$Ta=$S>>2&3;for($O=0;$O<$x;$O++){$N=$p[$O+1];$Ba=xxtea_mx($M,$N,$S,$c[$O&3^$Ta]);$M=int32($p[$O]+$Ba);$p[$O]=$M;}$N=$p[0];$Ba=xxtea_mx($M,$N,$S,$c[$O&3^$Ta]);$M=int32($p[$x]+$Ba);$p[$x]=$M;}return
long2str($p,false);}function
decrypt_string($Pa,$c){if($Pa==""){return"";}$c=array_values(unpack("V*",pack("H*",md5($c))));$p=str2long($Pa,false);$x=count($p)-1;$M=$p[$x];$N=$p[0];$Sb=floor(6+52/($x+1));$S=int32($Sb*0x9E3779B9);while($S){$Ta=$S>>2&3;for($O=$x;$O>0;$O--){$M=$p[$O-1];$Ba=xxtea_mx($M,$N,$S,$c[$O&3^$Ta]);$N=int32($p[$O]-$Ba);$p[$O]=$N;}$M=$p[$x];$Ba=xxtea_mx($M,$N,$S,$c[$O&3^$Ta]);$N=int32($p[0]-$Ba);$p[0]=$N;$S=int32($S-0x9E3779B9);}return
...
as$c=>$e){$f=$j->fieldName($e);if(isset($e["privileges"]["select"])&&$f!=""){$A[$c]=html_entity_decode(strip_tags($f));if(ereg('text|lob',$e["type"])){$Oa=$j->selectLengthProcess();}}$_c+=$e["privileges"];}list($u,$Q)=$j->selectColumnsProcess($A,$G);$o=$j->selectSearchProcess($q,$G);$V=$j->selectOrderProcess($q,$G);$C=$j->selectLimitProcess();$Ia=($u?implode(", ",$u):($B["Oid"]=="t"?"oid, ":"")."*")."\nFROM ".table($r);$Eb=($Q&&count($Q)<count($u)?"\nGROUP BY ".implode(", ",$Q):"").($V?"\nORDER BY ".implode(", ",$V):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$ja=>$d){echo$g->result("SELECT".limit(idf_escape(key($d))." FROM ".table($r)," WHERE ".where_check($ja).($o?" AND ".implode(" AND ",$o):"").($V?" ORDER BY ".implode(", ",$V):""),1));}exit;}if($_POST&&!$v){$Ic="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$yb=$Ja=null;foreach($G
as$T){if($T["type"]=="PRIMARY"){$yb=array_flip($T["columns"]);$Ja=($u?$yb:array());break;}}foreach((array)$Ja
as$c=>$a){if(in_array(idf_escape($c),$u)){unset($Ja[$c]);}}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($r);$j->dumpTable($r,"");if(!is_array($_POST["check"])||$Ja===array()){$Ga=$o;if(is_array($_POST["check"])){$Ga[]="($Ic)";}$l="SELECT $Ia".($Ga?"\nWHERE ".implode(" AND ",$Ga):"").$Eb;}else{$Mc=array();foreach($_POST["check"]as$a){$Mc[]="(SELECT".limit($Ia,"\nWHERE ".($o?implode(" AND ",$o)." AND ":"").where_check($a).$Eb,1).")";}$l=implode(" UNION ALL ",$Mc);}$j->dumpData($r,"table",$l);exit;}if(!$j->selectEmailProcess($o,$Ma)){if($_POST["save"]||$_POST["delete"]){$k=true;$oa=0;$l=table($r);$s=array();if(!$_POST["delete"]){foreach($A
as$f=>$a){$a=process_input($q[$f]);if($a!==null){if($_POST["clone"]){$s[idf_escape($f)]=($a!==false?$a:idf_escape($f));}elseif($a!==false){$s[]=idf_escape($f)." = $a";}}}$l.=($_POST["clone"]?" (".implode(", ",array_keys($s)).")\nSELECT ".implode(", ",$s)."\nFROM ".table($r):" SET\n".implode(",\n",$s));}if($_POST["delete"]||$s){$Bb="UPDATE";if($_POST["delete"]){$Bb="DELETE";$l="FROM $l";}if($_POST["clone"]){$Bb="INSERT";$l="INTO $l";}if($_POST["all"]||($Ja===array()&&$_POST["check"])||count($Q)<count($u)){$k=queries($Bb." $l".($_POST["all"]?($o?"\nWHERE ".implode(" AND ",$o):""):"\nWHERE $Ic"));$oa=$g->affected_rows;}else{foreach((array)$_POST["check"]as$a){$k=queries($Bb.limit1($l,"\nWHERE ".where_check($a)));if(!$k){break;}$oa+=$g->affected_rows;}}}queries_redirect(remove_from_uri("page"),lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordów.'),$oa),$k);}elseif(!$_POST["import"]){if(!$_POST["val"]){$v='Kliknij podwójnie wartość, aby ją edytować.';}else{$k=true;$oa=0;foreach($_POST["val"]as$ja=>$d){$s=array();foreach($d
as$c=>$a){$c=bracket_escape($c,1);$s[]=idf_escape($c)." = ".(ereg('char|text',$q[$c]["type"])||$a!=""?$j->processInput($q[$c],$a):"NULL");}$l=table($r)." SET ".implode(", ",$s);$Ga=" WHERE ".where_check($ja).($o?" AND ".implode(" AND ",$o):"");$k=queries("UPDATE".(count($Q)<count($u)?" $l$Ga":limit1($l,$Ga)));if(!$k){break;}$oa+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordów.'),$oa),$k);}}elseif(is_string($R=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ib["output"])."&format=".urlencode($_POST["separator"]));$k=true;$Ca=array_keys($q);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$R,$U);$oa=count($U[0]);begin();$sa=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($U[0]as$c=>$a){preg_match_all("~((\"[^\"]*\")+|[^$sa]*)$sa~",$a.$sa,$Jb);if(!$c&&!array_diff($Jb[1],$Ca)){$Ca=$Jb[1];$oa--;}else{$s=array();foreach($Jb[1]as$n=>$qa){$s[idf_escape($Ca[$n])]=($qa==""&&$q[$Ca[$n]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$qa))));}$k=insert_update($r,$s,$yb);if(!$k){break;}}}if($k){queries("COMMIT");}queries_redirect(remove_from_uri("page"),lang(array('%d rekord został zaimportowany.','%d rekordy zostały zaimportowane.','%d rekordów zostało zaimportowanych.'),$oa),$k);queries("ROLLBACK");}else{$v=upload_error($R);}}}$Na=$j->tableName($B);page_header('pokaż'.": $Na",$v);session_write_close();$s=null;if(isset($_c["insert"])){$s="";foreach((array)$_GET["where"]as$a){if(count($Ma[$a["col"]])==1&&($a["op"]=="="||(!$a["op"]&&!ereg('[_%]',$a["val"])))){$s.="&set".urlencode("[".bracket_escape($a["col"])."]")."=".urlencode($a["val"]);}}}$j->selectLinks($B,$s);if(!$A){echo"<p class='error'>".'Nie udało się pobrać danych z tabeli'.($q?".":": ".error())."\n";}else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($r).'">',"</div>\n";$j->selectColumnsPrint($u,$A);$j->selectSearchPrint($o,$A,$G);$j->selectOrderPrint($V,$A,$G);$j->selectLimitPrint($C);$j->selectLengthPrint($Oa);$j->selectActionPrint();echo"</form>\n";$y=$_GET["page"];if($y=="last"){$ma=$g->result("SELECT COUNT(*) FROM ".table($r).($o?" WHERE ".implode(" AND ",$o):""));$y=floor(max(0,$ma-1)/$C);}$l="SELECT".limit((+$C&&$Q&&count($Q)<count($u)&&$ka=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Ia,($o?"\nWHERE ".implode(" AND ",$o):"").$Eb,($C!=""?+$C:null),($y?$C*$y:0),"\n");echo$j->selectQuery($l);$k=$g->query($l);if(!$k){echo"<p class='error'>".error()."\n";}else{if($ka=="mssql"){$k->seek($C*$y);}$Cb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$w=array();while($d=$k->fetch_assoc()){if($y&&$ka=="oracle"){unset($d["RNUM"]);}$w[]=$d;}if($_GET["page"]!="last"){$ma=(+$C&&$Q&&count($Q)<count($u)?($ka=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($l) x")):count($w));}if(!$w){echo"<p class='message'>".'Brak rekordów.'."\n";}else{$Mb=$j->backwardKeys($r,$Na);echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Q&&$u?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'edytuj'."</a>");$Lb=array();$da=array();reset($u);$Sc=1;foreach($w[0]as$c=>$a){if($B["Oid"]!="t"||$c!="oid"){$a=$_GET["columns"][key($u)];$e=$q[$u?($a?$a["col"]:current($u)):$c];$f=($e?$j->fieldName($e,$Sc):"*");if($f!=""){$Sc++;$Lb[$c]=$f;$H=idf_escape($c);echo'<th><a href="'.h(remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($c).($V[0]==$H||$V[0]==$c||(!$V&&count($Q)<count($u)&&$Q[0]==$H)?'&desc%5B0%5D=1':'')).'">'.(!$u||$a?apply_sql_function($a["fun"],$f):h(current($u)))."</a>";}$da[$c]=$a["fun"];next($u);}}$Tb=array();if($_GET["modify"]){foreach($w
as$d){foreach($d
as$c=>$a){$Tb[$c]=max($Tb[$c],min(40,strlen(utf8_decode($a))));}}}echo($Mb?"<th>".'Relacje':"")."</thead>\n";foreach($j->rowDescriptions($w,$Ma)as$x=>$d){$Hb=unique_array($w[$x],$G);$ja="";foreach($Hb
as$c=>$a){$ja.="&".(isset($a)?urlencode("where[".bracket_escape($c)."]")."=".urlencode($a):"null%5B%5D=".urlencode($c));}echo"<tr".odd().">".(!$Q&&$u?"":"<td>".checkbox("check[]",substr($ja,1),in_array(substr($ja,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($Q)<count($u)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($r).$ja)."'>".'edytuj'."</a>"));foreach($d
as$c=>$a){if(isset($Lb[$c])){$e=$q[$c];if($a!=""&&(!isset($Cb[$c])||$Cb[$c]!="")){$Cb[$c]=(is_mail($a)?$Lb[$c]:"");}$t="";$a=$j->editVal($a,$e);if(!isset($a)){$a="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$e["type"])&&$a!=""){$t=h(ME.'download='.urlencode($r).'&field='.urlencode($c).$ja);}if($a===""){$a="&nbsp;";}elseif($Oa!=""&&ereg('text|blob',$e["type"])&&is_utf8($a)){$a=shorten_utf8($a,max(0,+$Oa));}else{$a=h($a);}if(!$t){foreach((array)$Ma[$c]as$la){if(count($Ma[$c])==1||end($la["source"])==$c){$t="";foreach($la["source"]as$n=>$ac){$t.=where_link($n,$la["target"][$n],$w[$x][$ac]);}$t=h(($la["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($la["db"]),ME):ME).'select='.urlencode($la["table"]).$t);if(count($la["source"])==1){break;}}}}if($c=="COUNT(*)"){$t=h(ME."select=".urlencode($r));$n=0;foreach((array)$_GET["where"]as$p){if(!array_key_exists($p["col"],$Hb)){$t.=h(where_link($n++,$p["col"],$p["val"],$p["op"]));}}foreach($Hb
as$W=>$p){$t.=h(where_link($n++,$W,$p));}}}if(!$t){if(is_mail($a)){$t="mailto:$a";}if($ec=is_url($d[$c])){$t=($ec=="http"&&$db?$d[$c]:"$ec://www.adminer.org/redirect/?url=".urlencode($d[$c]));}}$_=h("val[$ja][".bracket_escape($c)."]");$i=$_POST["val"][$ja][bracket_escape($c)];$vc=h(isset($i)?$i:$d[$c]);$Zc=strpos($a,"<i>...</i>");$qc=is_utf8($a)&&$w[$x][$c]==$d[$c]&&!$da[$c];$sc=ereg('text|lob',$e["type"]);echo(($_GET["modify"]&&$qc)||isset($i)?"<td>".($sc?"<textarea name='$_' cols='30' rows='".(substr_count($d[$c],"\n")+1)."'>$vc</textarea>":"<input name='$_' value='$vc' size='$Tb[$c]'>"):"<td id='$_' ondblclick=\"".($qc?"selectDblClick(this, event".($Zc?", 2":($sc?", 1":"")).")":"alert('".h('Użyj linku edycji aby zmienić tę wartość.')."')").";\">".$j->selectVal($a,$t,$e));}}if($Mb){echo"<td>";}$j->backwardKeysPrint($Mb,$w[$x]);echo"</tr>\n";}echo"</table>\n";}if($w||$y){$Nb=true;if($_GET["page"]!="last"&&+$C&&count($Q)>=count($u)&&($ma>=$C||$y)){$ma=found_rows($B,$o);if($ma<max(1e4,2*($y+1)*$C)){ob_flush();flush();$ma=$g->result("SELECT COUNT(*) FROM ".table($r).($o?" WHERE ".implode(" AND ",$o):""));}else{$Nb=false;}}echo"<p class='pages'>";if(+$C&&$ma>$C){$Yb=floor(($ma-1)/$C);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Strona'."', '".($y+1)."'), event); return false;\">".'Strona'."</a>:",pagination(0,$y).($y>5?" ...":"");for($n=max(1,$y-4);$n<min($Yb,$y+5);$n++){echo
pagination($n,$y);}echo($y+5<$Yb?" ...":"").($Nb?pagination($Yb,$y):' <a href="'.h(remove_from_uri()."&page=last").'">'.'ostatni'."</a>");}echo" (".($Nb?"":"~ ").lang(array('%d rekord','%d rekordy','%d rekordów'),$ma).") ".checkbox("all",1,0,'wybierz wszystkie')."\n";if($j->selectCommandPrint()){?>
