...
upload_error($k){$vc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Wgranie pliku było niemożliwe.'.($vc?" ".sprintf('Maksymalna wielkość pliku to %sB.',$vc):""):'Plik nie istnieje.');}function
...
enum_input($Rd,$c,$l,$W,$mb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$z);$J=($mb!==null?"<label><input type='$Rd'$c value='$mb'".((is_array($W)?in_array($mb,$W):$W===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($z[1]as$q=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$q+1:(is_array($W)?in_array($q+1,$W):$W===$V));$J.=" <label><input type='$Rd'$c value='".($q+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$J;}function
input($l,$W,$p){global$Td,$b,$t;$B=h(bracket_escape($l["field"]));echo"<td class='function'>";$hd=($t=="mssql"&&$l["auto_increment"]);if($hd&&!$_POST["save"])$p=null;$Mb=(isset($_GET["select"])||$hd?array("orig"=>'bez zmian'):array())+$b->editFunctions($l);$c=" name='fields[$B]'";if($l["type"]=="enum")echo
...
lang($Pd,$Cc){$Uc=($Cc==1?0:($Cc%10>1&&$Cc%10<5&&$Cc/10%10!=1?1:2));$Pd=str_replace("%d","%s",$Pd[$Uc]);$Cc=number_format($Cc,0,".",' ');return
...
selectLinks($Cd,$O=""){$a=$Cd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'Nowy rekord'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Ostatnia strona'."'>&gt;&gt;</a>\n";}function
...
as$B=>$Wa){$l=$m[$B];if(ereg("enum",$l["type"])||like_bool($l)){$u=$ic[$B];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($B)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'nie','tak'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Lc=$this->_foreignKeyOptions($_GET["select"],$B))){if($m[$B]["null"])$Lc[0]='('.'puste'.')';$u=$ic[$B];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($B)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Lc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$q=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$q][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$q][val]' value='".h($V["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'gdziekolwiek'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators),"<input name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($C,$f,$s){$Nc=array();foreach($s
as$u=>$ac){$C=array();foreach($ac["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Nc[$u]=implode(", ",$C);}if($Nc){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Nc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';$this->databasesPrint($yc);if($yc!="db"&&$yc!="ns"){$S=table_status();if(!$S)echo"<p class='message'>".'Brak tabel.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($yc){}function
tablesPrint($Ed){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Ed
as$K){$B=$this->tableName($K);if(isset($K["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($K["Name"])."'".bold($_GET["select"]==$K["Name"])." title='".'Pokaż dane'."'>$B</a><br>\n";}}function
...
as$u=>$V)$Xd.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$Nb&&$M?"":"<td>".checkbox("check[]",substr($Xd,1),in_array(substr($Xd,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($fc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Xd)."'>".'edytuj'."</a>"));foreach($K
