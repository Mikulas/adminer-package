...
confirm($Pa=""){return" onclick=\"return confirm('".'Czy jesteÅ› pewien?'.($Pa?" (' + $Pa + ')":"")."');\"";}function
...
enum_input($ae,$c,$l,$W,$ob=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$z);$J=($ob!==null?"<label><input type='$ae'$c value='$ob'".((is_array($W)?in_array($ob,$W):$W===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($z[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$J.=" <label><input type='$ae'$c value='".($p+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$J;}function
input($l,$W,$o){global$g,$ce,$b,$t;$B=h(bracket_escape($l["field"]));echo"<td class='function'>";$od=($t=="mssql"&&$l["auto_increment"]);if($od&&!$_POST["save"])$o=null;$Sb=(isset($_GET["select"])||$od?array("orig"=>'bez zmian'):array())+$b->editFunctions($l);$c=" name='fields[$B]'";if($l["type"]=="enum")echo
...
lang($Yd,$Ic=null){if(is_array($Yd)){$ad=($Ic==1?0:($Ic%10>1&&$Ic%10<5&&$Ic/10%10!=1?1:2));$Yd=$Yd[$ad];}$Yd=str_replace("%d","%s",$Yd);$Ic=number_format($Ic,0,".",' ');return
...
selectLinks($Jd,$O=""){$a=$Jd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'Nowy rekord'."</a>\n";}function
...
as$B=>$Ya){$l=$m[$B];if(ereg("enum",$l["type"])||like_bool($l)){$u=$nc[$B];$p--;echo"<div>".h($Ya)."<input type='hidden' name='where[$p][col]' value='".h($B)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'nie','tak'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Rc=$this->_foreignKeyOptions($_GET["select"],$B))){if($m[$B]["null"])$Rc[0]='('.'puste'.')';$u=$nc[$B];$p--;echo"<div>".h($Ya)."<input type='hidden' name='where[$p][col]' value='".h($B)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Rc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$p=0;foreach($X
...
as$u=>$r){$C=array();foreach($r["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Tc[$u]=implode(", ",$C);}if($Tc){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
as$B=>$l){if(!isset($l["privileges"][$ke?"update":"insert"])||$b->fieldName($l)=="")unset($m[$B]);}if($_POST&&!$k&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($ke?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);$s=indexes($a);$fe=unique_array($_GET["where"],$s);$id="\nWHERE $X";if(isset($_POST["delete"])){$H="FROM ".table($a);query_redirect("DELETE".($fe?" $H$id":limit1($H,$id)),$x,'Rekord zostaÅ‚ usuniÄ™ty.');}else{$O=array();foreach($m
as$B=>$l){$V=process_input($l);if($V!==false&&$V!==null)$O[idf_escape($B)]=($ke?"\n".idf_escape($B)." = $V":$V);}if($ke){if(!$O)redirect($x);$H=table($a)." SET".implode(",",$O);query_redirect("UPDATE".($fe?" $H$id":limit1($H,$id)),$x,'Rekord zostaÅ‚ zaktualizowany.');}else{$I=insert_into($a,$O);$sc=($I?last_id():0);queries_redirect($x,sprintf('Rekord%s zostaÅ‚ dodany.',($sc?" $sc":"")),$I);}}}$Kd=$b->tableName(table_status1($a,true));page_header(($ke?'Edytuj':'Dodaj'),$k,array("select"=>array($a,$Kd)),$Kd);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($X){$M=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($ke?'Zapisz i kontynuuj edycjÄ™':'Zapisz i dodaj nastÄ™pny')."' title='Ctrl+Shift+Enter'>\n";}echo($ke?"<input type='submit' name='delete' value='".'UsuÅ„'."' onclick=\"return confirm('".'Czy jesteÅ› pewien?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
