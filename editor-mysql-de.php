...
upload_error($k){$vc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($vc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$vc):""):'Datei existiert nicht.');}function
...
selectLinks($Cd,$O=""){$a=$Cd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'Neuer Datensatz'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$B=>$Wa){$l=$m[$B];if(ereg("enum",$l["type"])||like_bool($l)){$u=$ic[$B];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($B)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Lc=$this->_foreignKeyOptions($_GET["select"],$B))){if($m[$B]["null"])$Lc[0]='('.'leer'.')';$u=$ic[$B];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($B)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Lc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$q=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$q][col]'><option value=''>(".'beliebig'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$q][val]' value='".h($V["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators),"<input name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($C,$f,$s){$Nc=array();foreach($s
as$u=>$ac){$C=array();foreach($ac["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Nc[$u]=implode(", ",$C);}if($Nc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Nc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';$this->databasesPrint($yc);if($yc!="db"&&$yc!="ns"){$S=table_status();if(!$S)echo"<p class='message'>".'Keine Tabellen.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($yc){}function
tablesPrint($Ed){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Ed
as$K){$B=$this->tableName($K);if(isset($K["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($K["Name"])."'".bold($_GET["select"]==$K["Name"])." title='".'Daten auswählen'."'>$B</a><br>\n";}}function
...
as$B=>$l){$V=process_input($l);if($V!==false&&$V!==null)$O[idf_escape($B)]=($be?"\n".idf_escape($B)." = $V":$V);}if($be){if(!$O)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $X"),$x,'Datensatz geändert.');}else{$I=insert_into($a,$O);$nc=($I?last_id():0);queries_redirect($x,sprintf('Datensatz%s hinzugefügt.',($nc?" $nc":"")),$I);}}}$Dd=$b->tableName(table_status($a));page_header(($be?'Ändern':'Hinzufügen'),$k,array("select"=>array($a,$Dd)),$Dd);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($X){$M=array();foreach($m
...
as$u=>$V)$Xd.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$Nb&&$M?"":"<td>".checkbox("check[]",substr($Xd,1),in_array(substr($Xd,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($fc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Xd)."'>".'ändern'."</a>"));foreach($K
