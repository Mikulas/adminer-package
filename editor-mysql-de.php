...
confirm($Ub=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($Ub?" (' + $Ub + ')":"")."');\"";}function
js_escape($A){return
addcslashes($A,"\r\n'\\/");}function
...
upload_error($v){$Fc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Hochladen von Datei fehlgeschlagen.'.($Fc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Fc):""):'Datei existiert nicht.');}function
...
input($e,$l,$_){global$Da,$j,$ya;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$ea=(isset($_GET["select"])?array("orig"=>'Original'):array())+$j->editFunctions($e);$P=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($sd,$Wa){$td=($Wa==1||(!$Wa&&'de'=='fr')?0:((!$Wa||$Wa>=5)&&ereg('cs|sk|ru','de')?2:1));return
...
selectLinks($Ka,$t=""){$q=$Ka["Name"];if(isset($t)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($q).$t).'">'.'Neuer Datensatz'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$J=>$a){$s.="&set".urlencode("[".bracket_escape($J)."]")."=".urlencode($d[$a]);}echo"<a href='".h($s)."' title='".'Neuer Datensatz'."'>+</a> ";}}}}function
...
selectSearchPrint($p,$z,$E){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Suchen'."</legend><div>\n";$Cb=array();foreach($p
...
as$c=>$Q){$X=array();foreach($Q["columns"]as$a){$X[]=$this->fieldName(array("field"=>$a,"comment"=>$z[$a]));}if(count(array_filter($X,'strlen'))>1&&$c!="PRIMARY"){$Bb[$c]=implode(", ",$X);}}if($Bb){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Bb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
mail($Ga,email_header($ib),$qc.$V.$db,$mb);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Mb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$Aa[DRIVER]='Login';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$q=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$q-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($q)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$q=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$ga=(isset($_GET["select"])?$_POST["edit"]:$p);$r=fields($q);foreach($r
as$f=>$e){if(!isset($e["privileges"][$ga?"update":"insert"])||$j->fieldName($e)==""){unset($r[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$O=$_POST["referer"];if($_POST["insert"]){$O=($ga?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$O)){$O=ME."select=".urlencode($q);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($q)," WHERE $p"),$O,'Datensatz gelöscht.');}else{$t=array();foreach($r
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($ga?"\n".idf_escape($f)." = $a":$a);}}if($ga){if(!$t){redirect($O);}query_redirect("UPDATE".limit1(table($q)." SET".implode(",",$t),"\nWHERE $p"),$O,'Datensatz geändert.');}else{$h=insert_into($q,$t);$sc=($h?last_id():0);queries_redirect($O,sprintf('Datensatz%s hinzugefügt.',($sc?" $sc":"")),$h);}}}$Ta=$j->tableName(table_status($q));page_header(($ga?'Ändern':'Hinzufügen'),$v,array("select"=>array($q,$Ta)),$Ta);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$w=array();foreach($r
