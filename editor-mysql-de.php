...
confirm($Dc="",$kd=false){return" onclick=\"".($kd?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Dc?" (' + $Dc + ')":"")."');\"";}function
js_escape($B){return
addcslashes($B,"\r\n'\\/");}function
...
upload_error($u){$dc=($u==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($u?'Hochladen von Datei fehlgeschlagen.'.($dc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$dc):""):'Datei existiert nicht.');}function
...
enum_input($wa,$P,$e,$h,$ac=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($ac)?"<label><input type='$wa'$P value='$ac'".((is_array($h)?in_array($ac,$h):$h===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($U[1]as$o=>$a){$a=stripcslashes(str_replace("''","'",$a));$Wa=(is_int($h)?$h==$o+1:(is_array($h)?in_array($o+1,$h):$h===$a));$b.=" <label><input type='$wa'$P value='".($o+1)."'".($Wa?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$h,$y){global$Da,$j,$qa;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$ec=($qa=="mssql"&&$e["auto_increment"]);if($ec&&!$_POST["save"]){$y=null;}$Y=(isset($_GET["select"])||$ec?array("orig"=>'Original'):array())+$j->editFunctions($e);$P=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($_d,$Ca){$wd=($Ca==1||(!$Ca&&'de'=='fr')?0:('de'=='sl'&&(!$Ca||$Ca>2)?1:0)+((!$Ca||$Ca>=5)&&ereg('cs|sk|ru|sl|pl','de')?2:1));return
...
selectLinks($Sa,$s=""){$r=$Sa["Name"];if(isset($s)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($r).$s).'">'.'Neuer Datensatz'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$I=>$a){$t.="&set".urlencode("[".bracket_escape($I)."]")."=".urlencode($d[$a]);}echo"<a href='".h($t)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
...
selectSearchPrint($p,$_,$G){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Suchen'."</legend><div>\n";$Ob=array();foreach($p
...
as$f=>$ra){$ja=$this->_foreignKeyOptions($_GET["select"],$f);if($ja){if($q[$f]["null"]){$ja[0]='('.'leer'.')';}$c=$Ob[$f];$o--;echo"<div>".h($ra)."<input type='hidden' name='where[$o][col]' value='".h($f)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($ja,$p[$c]["val"],true)."</select></div>\n";unset($_[$f]);}}$o=0;foreach($p
...
as$c=>$V){$R=array();foreach($V["columns"]as$a){$R[]=$_[$a];}if(count(array_filter($R,'strlen'))>1&&$c!="PRIMARY"){$Jb[$c]=implode(", ",$R);}}if($Jb){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Jb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
selectLimitPrint($D){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo
...
selectEmailPrint($xb,$_){if($xb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<p>".'Von'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Betreff'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea><br>\n";echo
html_select("email_addition",$_,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Hinzufügen'."'>\n";echo"<p>".'Anhänge'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($xb)==1?'<input type="hidden" name="email_field" value="'.h(key($xb)).'">':html_select("email_field",$xb)),"<input type='submit' name='email' value='".'Abschicken'."' onclick=\"return this.form['delete'].onclick();\">\n","</div></fieldset>\n";}}function
...
auth_error($cc=null){global$g,$j,$Z;$kb=session_name();$u="";if(!$_COOKIE[$kb]&&$_GET[$kb]&&ini_bool("session.use_only_cookies")){$u='Sitzungen müssen aktiviert sein.';}elseif(isset($_GET["username"])){if(($_COOKIE[$kb]||$_GET[$kb])&&!$Z){$u='Sitzungsdauer abgelaufen, bitte erneut anmelden.';}else{$M=&get_session("pwds");if(isset($M)){$u=h($cc?$cc->getMessage():(is_string($g)?$g:'Ungültige Anmelde-Informationen.'));$M=null;}}}page_header('Login',$u,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$j->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstützten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$Gd)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$j->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$Z=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$Z;}$u=($_POST?($_POST["token"]==$Z?"":'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('POST data zu gross. Reduzieren Sie die Grösse oder vergrössern Sie den Wert %s in der Konfiguration.','"post_max_size"')));$g->select_db($j->database());function
...
mail($Fa,email_header($ub),$lc.$W.$rb,$qb);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$wb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$ta[DRIVER]='Login';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$r=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$r-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($r)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$r=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$la=(isset($_GET["select"])?$_POST["edit"]:$p);$q=fields($r);foreach($q
as$f=>$e){if(!isset($e["privileges"][$la?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$u&&!isset($_GET["select"])){$F=$_POST["referer"];if($_POST["insert"]){$F=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$F)){$F=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$F,'Datensatz gelöscht.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($la?"\n".idf_escape($f)." = $a":$a);}}if($la){if(!$s){redirect($F);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $p"),$F,'Datensatz geändert.');}else{$l=insert_into($r,$s);$Gc=($l?last_id():0);queries_redirect($F,sprintf('Datensatz%s hinzugefügt.',($Gc?" $Gc":"")),$l);}}}$Xa=$j->tableName(table_status($r));page_header(($la?'Ändern':'Hinzufügen'),$u,array("select"=>array($r,$Xa)),$Xa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$w=array();foreach($q
