...
confirm($Pa=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($Pa?" (' + $Pa + ')":"")."');\"";}function
...
selectLinks($Jd,$O=""){$a=$Jd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'Neuer Datensatz'."</a>\n";}function
...
as$u=>$r){$C=array();foreach($r["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Tc[$u]=implode(", ",$C);}if($Tc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
as$B=>$l){if(!isset($l["privileges"][$ke?"update":"insert"])||$b->fieldName($l)=="")unset($m[$B]);}if($_POST&&!$k&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($ke?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);$s=indexes($a);$fe=unique_array($_GET["where"],$s);$id="\nWHERE $X";if(isset($_POST["delete"])){$H="FROM ".table($a);query_redirect("DELETE".($fe?" $H$id":limit1($H,$id)),$x,'Datensatz gelÃ¶scht.');}else{$O=array();foreach($m
as$B=>$l){$V=process_input($l);if($V!==false&&$V!==null)$O[idf_escape($B)]=($ke?"\n".idf_escape($B)." = $V":$V);}if($ke){if(!$O)redirect($x);$H=table($a)." SET".implode(",",$O);query_redirect("UPDATE".($fe?" $H$id":limit1($H,$id)),$x,'Datensatz geÃ¤ndert.');}else{$I=insert_into($a,$O);$sc=($I?last_id():0);queries_redirect($x,sprintf('Datensatz%s hinzugefÃ¼gt.',($sc?" $sc":"")),$I);}}}$Kd=$b->tableName(table_status1($a,true));page_header(($ke?'Ã„ndern':'HinzufÃ¼gen'),$k,array("select"=>array($a,$Kd)),$Kd);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($X){$M=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($ke?'Speichern und weiter bearbeiten':'Speichern und nÃ¤chsten hinzufÃ¼gen')."' title='Ctrl+Shift+Enter'>\n";}echo($ke?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
