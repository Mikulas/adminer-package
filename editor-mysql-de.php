...
confirm($Oa=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($Oa?" (' + $Oa + ')":"")."');\"";}function
...
upload_error($k){$_c=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($_c?" ".sprintf('Maximal erlaubte DateigrÃ¶sse ist %sB.',$_c):""):'Datei existiert nicht.');}function
...
selectLinks($Jd,$N=""){$a=$Jd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Neuer Datensatz'."</a>\n";}function
...
as$e=>$V)$v.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($J[$V]);echo"<a href='".h($v)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
...
as$A=>$Xa){$l=$m[$A];if(ereg("enum",$l["type"])||like_bool($l)){$t=$nc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'no','yes'),$X[$t]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$t]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$A]);}elseif(is_array($Rc=$this->_foreignKeyOptions($_GET["select"],$A))){if($m[$A]["null"])$Rc[0]='('.'leer'.')';$t=$nc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Rc,$X[$t]["val"],true)."</select></div>\n";unset($f[$A]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'beliebig'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$p][val]' value='".h($V["val"])."' onsearch='selectSearchSearch(this);'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$p][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$f,$s){$Tc=array();foreach($s
as$t=>$r){$B=array();foreach($r["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$t!="PRIMARY")$Tc[$t]=implode(", ",$B);}if($Tc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
selectLimitPrint($u){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo
...
as$A=>$l){if(!isset($l["privileges"][$je?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$w=$_POST["referer"];if($_POST["insert"])$w=($je?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$w))$w=ME."select=".urlencode($a);$s=indexes($a);$ee=unique_array($_GET["where"],$s);$id="\nWHERE $X";if(isset($_POST["delete"])){$G="FROM ".table($a);query_redirect("DELETE".($ee?" $G$id":limit1($G,$id)),$w,'Datensatz gelÃ¶scht.');}else{$N=array();foreach($m
as$A=>$l){$V=process_input($l);if($V!==false&&$V!==null)$N[idf_escape($A)]=($je?"\n".idf_escape($A)." = $V":$V);}if($je){if(!$N)redirect($w);$G=table($a)." SET".implode(",",$N);query_redirect("UPDATE".($ee?" $G$id":limit1($G,$id)),$w,'Datensatz geÃ¤ndert.');}else{$H=insert_into($a,$N);$sc=($H?last_id():0);queries_redirect($w,sprintf('Datensatz%s hinzugefÃ¼gt.',($sc?" $sc":"")),$H);}}}$Kd=$b->tableName(table_status($a,true));page_header(($je?'Ã„ndern':'HinzufÃ¼gen'),$k,array("select"=>array($a,$Kd)),$Kd);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($je?'Speichern und weiter bearbeiten':'Speichern und nÃ¤chsten hinzufÃ¼gen')."' title='Ctrl+Shift+Enter'>\n";}echo($je?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
