...
confirm($wc="",$jd=false){return" onclick=\"".($jd?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($wc?" (' + $wc + ')":"")."');\"";}function
js_escape($C){return
addcslashes($C,"\r\n'\\/");}function
...
upload_error($u){$Yb=($u==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($u?'Hochladen von Datei fehlgeschlagen.'.($Yb?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Yb):""):'Datei existiert nicht.');}function
...
lang($Zc,$za){$Bd=($za==1||(!$za&&'de'=='fr')?0:('de'=='sl'&&(!$za||$za>2)?1:0)+((!$za||$za>=5)&&ereg('cs|sk|ru|sl','de')?2:1));return
...
selectSearchPrint($p,$z,$F){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Suchen'."</legend><div>\n";$Kb=array();foreach($p
...
as$f=>$oa){$ka=$this->_foreignKeyOptions($_GET["select"],$f);if($ka){if($q[$f]["null"]){$ka[0]='('.'leer'.')';}$c=$Kb[$f];$o--;echo"<div>".h($oa)."<input type='hidden' name='where[$o][col]' value='".h($f)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($ka,$p[$c]["val"],true)."</select></div>\n";unset($z[$f]);}}$o=0;foreach($p
as$a){if(($a["col"]==""||$z[$a["col"]])&&"$a[col]$a[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'beliebig'.")".optionlist($z,$a["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$a["op"]),"<input name='where[$o][val]' value='".h($a["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='selectAddRow(this);'><option value=''>(".'beliebig'.")".optionlist($z,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($S,$z,$F){$Eb=array();foreach($F
as$c=>$T){$S=array();foreach($T["columns"]as$a){$S[]=$z[$a];}if(count(array_filter($S,'strlen'))>1&&$c!="PRIMARY"){$Eb[$c]=implode(", ",$S);}}if($Eb){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Eb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
';if($Ma!="db"&&$Ma!="ns"){$A=table_status();if(!$A){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($A);}}}}function
...
as$f=>$e){if(!isset($e["privileges"][$la?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$u&&!isset($_GET["select"])){$G=$_POST["referer"];if($_POST["insert"]){$G=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$G)){$G=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$G,'Datensatz gelöscht.');}else{$t=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($la?"\n".idf_escape($f)." = $a":$a);}}if($la){if(!$t){redirect($G);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$t),"\nWHERE $p"),$G,'Datensatz geändert.');}else{$k=insert_into($r,$t);$zc=($k?last_id():0);queries_redirect($G,sprintf('Datensatz%s hinzugefügt.',($zc?" $zc":"")),$k);}}}$Xa=$j->tableName(table_status($r));page_header(($la?'Ändern':'Hinzufügen'),$u,array("select"=>array($r,$Xa)),$Xa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$y=array();foreach($q
