...
confirm($Ja="",$jd=false){return" onclick=\"".($jd?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Ja?" (' + $Ja + ')":"")."');\"";}function
...
upload_error($h){$kc=($h==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($h?'Hochladen von Datei fehlgeschlagen.'.($kc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$kc):""):'Datei existiert nicht.');}function
...
selectLinks($qd,$N=""){$a=$qd["Name"];if(isset($N)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Neuer Datensatz'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$d=>$V){$v.="&set".urlencode("[".bracket_escape($d)."]")."=".urlencode($J[$V]);}echo"<a href='".h($v)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
...
selectSearchPrint($X,$e,$r){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Suchen'."</legend><div>\n";$Yb=array();foreach($X
as$t=>$V){$Yb[$V["col"]]=$t;}$o=0;$j=fields($_GET["select"]);foreach($e
as$A=>$Ta){$i=$j[$A];if(ereg("enum",$i["type"])||like_bool($i)){$t=$Yb[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'>:",(like_bool($i)?" <select name='where[$o][val]'>".optionlist(array(""=>"",'no','yes'),$X[$t]["val"],true)."</select>":enum_input("checkbox"," name='where[$o][val][]'",$i,(array)$X[$t]["val"],($i["null"]?0:null))),"</div>\n";unset($e[$A]);}elseif(is_array($_c=$this->_foreignKeyOptions($_GET["select"],$A))){if($j[$A]["null"]){$_c[0]='('.'leer'.')';}$t=$Yb[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($_c,$X[$t]["val"],true)."</select></div>\n";unset($e[$A]);}}$o=0;foreach($X
as$V){if(($V["col"]==""||$e[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'beliebig'.")".optionlist($e,$V["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$o][val]' value='".h($V["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($e,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$e,$r){$Bc=array();foreach($r
as$t=>$q){$B=array();foreach($q["columns"]as$V){$B[]=$e[$V];}if(count(array_filter($B,'strlen'))>1&&$t!="PRIMARY"){$Bc[$t]=implode(", ",$B);}}if($Bc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Bc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
';if($nc!="db"&&$nc!="ns"){$R=table_status();if(!$R){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($R);}}}}function
tablesPrint($sd){echo"<p id='tables'>\n";foreach($sd
as$J){$A=$this->tableName($J);if(isset($J["Engine"])&&$A!=""){echo"<a href='".h(ME).'select='.urlencode($J["Name"])."'".bold($_GET["select"]==$J["Name"])." title='".'Daten auswählen'."'>$A</a><br>\n";}}}function
...
as$A=>$i){if(!isset($i["privileges"][$Nd?"update":"insert"])||$b->fieldName($i)==""){unset($j[$A]);}}if($_POST&&!$h&&!isset($_GET["select"])){$w=$_POST["referer"];if($_POST["insert"]){$w=($Nd?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$w)){$w=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$w,'Datensatz gelöscht.');}else{$N=array();foreach($j
as$A=>$i){$V=process_input($i);if($V!==false&&$V!==null){$N[idf_escape($A)]=($Nd?"\n".idf_escape($A)." = $V":$V);}}if($Nd){if(!$N){redirect($w);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$w,'Datensatz geändert.');}else{$H=insert_into($a,$N);$cc=($H?last_id():0);queries_redirect($w,sprintf('Datensatz%s hinzugefügt.',($cc?" $cc":"")),$H);}}}$rd=$b->tableName(table_status($a));page_header(($Nd?'Ändern':'Hinzufügen'),$h,array("select"=>array($a,$rd)),$rd);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($X){$L=array();foreach($j
as$A=>$i){if(isset($i["privileges"]["select"])){$L[]=($_POST["clone"]&&$i["auto_increment"]?"'' AS ":(ereg("enum|set",$i["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Keine Daten.'."\n";}echo'
...
';if($j){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Nd?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}}echo($Nd?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$j?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
