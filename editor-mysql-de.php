...
remove_slashes($Zc,$Ab=false){if(get_magic_quotes_gpc()){while(list($u,$V)=each($Zc)){foreach($V
as$hc=>$U){unset($Zc[$u][$hc]);if(is_array($U)){$Zc[$u][stripslashes($hc)]=$U;$Zc[]=&$Zc[$u][stripslashes($hc)];}else$Zc[$u][stripslashes($hc)]=($Ab?$U:stripslashes($U));}}}}function
...
nl_br($O){return
str_replace("\n","<br>",$O);}function
checkbox($A,$W,$Ba,$kc="",$Ic="",$gc=false){static$r=0;$r++;$I="<input type='checkbox' name='$A' value='".h($W)."'".($Ba?" checked":"").($Ic?' onclick="'.h($Ic).'"':'').($gc?" class='jsonly'":"")." id='checkbox-$r'>";return($kc!=""?"<label for='checkbox-$r'>$I".h($kc)."</label>":$I);}function
...
confirm($Na=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($Na?" (' + $Na + ')":"")."');\"";}function
print_fieldset($r,$oc,$ge=false,$Ic=""){echo"<fieldset><legend><a href='#fieldset-$r' onclick=\"".h($Ic)."return !toggle('fieldset-$r');\">$oc</a></legend><div id='fieldset-$r'".($ge?"":" class='hidden'").">\n";}function
...
as$ac){if(ereg("PRIMARY|UNIQUE",$ac["type"])){$I=array();foreach($ac["columns"]as$u){if(!isset($J[$u]))continue
...
where($Aa,$m);}function
where_link($q,$e,$W,$Kc="="){return"&where%5B$q%5D%5Bcol%5D=".urlencode($e)."&where%5B$q%5D%5Bop%5D=".urlencode(($W!==null?$Kc:"IS NULL"))."&where%5B$q%5D%5Bval%5D=".urlencode($W);}function
...
get_file($u,$Ua=false){$n=$_FILES[$u];if(!$n||$n["error"])return$n["error"];$I=file_get_contents($Ua&&ereg('\\.gz$',$n["name"])?"compress.zlib://$n[tmp_name]":($Ua&&ereg('\\.bz2$',$n["name"])?"compress.bzip2://$n[tmp_name]":$n["tmp_name"]));if($Ua){$wd=substr($I,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$wd,$fd))$I=iconv("utf-16","utf-8",$I);elseif($wd=="\xEF\xBB\xBF")$I=substr($I,3);}return$I;}function
upload_error($k){$wc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($wc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$wc):""):'Datei existiert nicht.');}function
...
hidden_fields($Zc,$Zb=array()){while(list($u,$V)=each($Zc)){if(is_array($V)){foreach($V
as$hc=>$U)$Zc[$u."[$hc]"]=$U;}elseif(!in_array($u,$Zb))echo'<input type="hidden" name="'.h($u).'" value="'.h($V).'">';}}function
hidden_fields_get(){echo(sid()?'<input type="hidden" name="'.session_name().'" value="'.h(session_id()).'">':''),(SERVER!==null?'<input type="hidden" name="'.DRIVER.'" value="'.h(SERVER).'">':""),'<input type="hidden" name="username" value="'.h($_GET["username"]).'">';}function
column_foreign_keys($Q){global$b;$I=array();foreach($b->foreignKeys($Q)as$Gb){foreach($Gb["source"]as$V)$I[$V][]=$Gb;}return$I;}function
enum_input($Rd,$c,$l,$W,$mb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$tc);$I=($mb!==null?"<label><input type='$Rd'$c value='$mb'".((is_array($W)?in_array($mb,$W):$W===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($tc[1]as$q=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$q+1:(is_array($W)?in_array($q+1,$W):$W===$V));$I.=" <label><input type='$Rd'$c value='".($q+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$I;}function
input($l,$W,$p){global$Td,$b,$t;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$id=($t=="mssql"&&$l["auto_increment"]);if($id&&!$_POST["save"])$p=null;$Mb=(isset($_GET["select"])||$id?array("orig"=>'Original'):array())+$b->editFunctions($l);$c=" name='fields[$A]'";if($l["type"]=="enum")echo
nbsp($Mb[""])."<td>".$b->editInput($_GET["edit"],$l,$c,$W);else{$Bb=0;foreach($Mb
as$u=>$V){if($u===""||!$V)break;$Bb++;}$Hc=($Bb?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($l["field"])))."]']; if ($Bb > f.selectedIndex) f.selectedIndex = $Bb;\"":"");$c.=$Hc;echo(count($Mb)>1?html_select("function[$A]",$Mb,$p===null||in_array($p,$Mb)||isset($Mb[$p])?$p:"","functionChange(this);"):nbsp(reset($Mb))).'<td>';$ec=$b->editInput($_GET["edit"],$l,$c,$W);if($ec!="")echo$ec;elseif($l["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$tc);foreach($tc[1]as$q=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?($W>>$q)&1:in_array($V,explode(",",$W),true));echo" <label><input type='checkbox' name='fields[$A][$q]' value='".(1<<$q)."'".($Ba?' checked':'')."$Hc>".h($b->editVal($V,$l)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$l["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$A'$Hc>";elseif(($Hd=ereg('text|lob',$l["type"]))||ereg("\n",$W)){if($Hd&&$t!="sqlite")$c.=" cols='50' rows='12'";else{$K=min(12,substr_count($W,"\n")+1);$c.=" cols='30' rows='$K'".($K==1?" style='height: 1.2em;'":"");}echo"<textarea$c>".h($W).'</textarea>';}else{$xc=(!ereg('int',$l["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$l["length"],$y)?((ereg("binary",$l["type"])?2:1)*$y[1]+($y[3]?1:0)+($y[2]&&!$l["unsigned"]?1:0)):($Td[$l["type"]]?$Td[$l["type"]]+($l["unsigned"]?0:1):0));echo"<input".(ereg('int',$l["type"])?" type='number'":"")." value='".h($W)."'".($xc?" maxlength='$xc'":"").(ereg('char|binary',$l["type"])&&$xc>20?" size='40'":"")."$c>";}}}function
...
search_tables(){global$b,$g;$_GET["where"][0]["op"]="LIKE %%";$_GET["where"][0]["val"]=$_POST["query"];$Jb=false;foreach(table_status()as$Q=>$R){$A=$b->tableName($R);if(isset($R["Engine"])&&$A!=""&&(!$_POST["tables"]||in_array($Q,$_POST["tables"]))){$H=$g->query("SELECT".limit("1 FROM ".table($Q)," WHERE ".implode(" AND ",$b->selectSearchProcess(fields($Q),array())),1));if(!$H||$H->fetch_row()){if(!$Jb){echo"<ul>\n";$Jb=true;}echo"<li>".($H?"<a href='".h(ME."select=".urlencode($Q)."&where[0][op]=".urlencode($_GET["where"][0]["op"])."&where[0][val]=".urlencode($_GET["where"][0]["val"]))."'>$A</a>\n":"$A: <span class='error'>".error()."</span>\n");}}}echo($Jb?"</ul>":"<p class='message'>".'Keine Tabellen.')."\n";}function
dump_headers($Wb,$_c=false){global$b;$I=$b->dumpHeaders($Wb,$_c);$Pc=$_POST["output"];if($Pc!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($Wb).".$I".($Pc!="file"&&!ereg('[^0-9a-z]',$Pc)?".$Pc":""));session_write_close();return$I;}function
...
false;$ab=dirname($zb);unlink($zb);}}$zb="$ab/adminer.key";$I=@file_get_contents($zb);if($I)return$I;$Kb=@fopen($zb,"w");if($Kb){$I=md5(uniqid(mt_rand(),true));fwrite($Kb,$I);fclose($Kb);}return$I;}function
...
lzw_decompress($wa){$Za=256;$xa=8;$Ea=array();$jd=0;$kd=0;for($q=0;$q<strlen($wa);$q++){$jd=($jd<<8)+ord($wa[$q]);$kd+=8;if($kd>=$xa){$kd-=$xa;$Ea[]=$jd>>$kd;$jd&=(1<<$kd)-1;$Za++;if($Za>>$xa)$xa++;}}$Ya=range("\0","\xFF");$I="";foreach($Ea
as$q=>$Da){$ib=$Ya[$Da];if(!isset($ib))$ib=$ke.$ke[0];$I.=$ib;if($q)$Ya[]=$ke.$ib[0];$ke=$ib;}return$I;}global$b,$g,$db,$gb,$ob,$k,$Mb,$Pb,$aa,$dc,$t,$ba,$lc,$Gc,$Tc,$zd,$S,$Qd,$Td,$ae,$ca;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$aa=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$Rc=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$aa);if(version_compare(PHP_VERSION,'5.2.0')>=0)$Rc[]=true;call_user_func_array('session_set_cookie_params',$Rc);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Ab);if(function_exists("set_magic_quotes_runtime"))set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'de';}function
lang($Pd,$Dc){$Vc=($Dc==1?0:1);$Pd=str_replace("%d","%s",$Pd[$Vc]);$Dc=number_format($Dc,0,".",' ');return
...
get_databases($Cb){global$g;$I=get_session("dbs");if($I===null){$G=($g->server_info>=5?"SELECT SCHEMA_NAME FROM information_schema.SCHEMATA":"SHOW DATABASES");$I=($Cb?slow_query($G):get_vals($G));restart_session();set_session("dbs",$I);stop_session();}return$I;}function
...
alter_table($Q,$A,$m,$Db,$Ka,$nb,$Ga,$pa,$Sc){$la=array();foreach($m
as$l)$la[]=($l[1]?($Q!=""?($l[0]!=""?"CHANGE ".idf_escape($l[0]):"ADD"):" ")." ".implode($l[1]).($Q!=""?$l[2]:""):"DROP ".idf_escape($l[0]));$la=array_merge($la,$Db);$xd="COMMENT=".q($Ka).($nb?" ENGINE=".q($nb):"").($Ga?" COLLATE ".q($Ga):"").($pa!=""?" AUTO_INCREMENT=$pa":"").$Sc;if($Q=="")return
...
<tr><th>Benutzer<td><input type="hidden" name="auth[driver]" value="server"><input id="username" name="auth[username]" value="',h($_GET["username"]),'">
...
selectLinks($Dd,$N=""){$a=$Dd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Neuer Datensatz'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$Q=>$sa){foreach($sa["keys"]as$Ia){$w=ME.'select='.urlencode($Q);$q=0;foreach($Ia
as$e=>$V)$w.=where_link($q++,$e,$J[$V]);echo"<a href='".h($w)."'>".h($sa["name"])."</a>";$w=ME.'edit='.urlencode($Q);foreach($Ia
as$e=>$V)$w.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($J[$V]);echo"<a href='".h($w)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
selectQuery($G){return"<!--\n".str_replace("--","--><!-- ",$G)."\n-->\n";}function
rowDescription($Q){foreach(fields($Q)as$l){if(ereg("varchar|character varying",$l["type"]))return
idf_escape($l["field"]);}return"";}function
rowDescriptions($K,$Fb){$I=$K;foreach($K[0]as$u=>$V){if(list($Q,$r,$A)=$this->_foreignColumn($Fb,$u)){$Yb=array();foreach($K
as$J)$Yb[$J[$u]]=exact_value($J[$u]);$Xa=$this->_values[$Q];if(!$Xa)$Xa=get_key_vals("SELECT $r, $A FROM ".table($Q)." WHERE $r IN (".implode(", ",$Yb).")");foreach($K
as$_=>$J){if(isset($J[$u]))$I[$_][$u]=(string)$Xa[$J[$u]];}}}return$I;}function
selectLink($V,$l){}function
selectVal($V,$w,$l){$I=($V===null?"&nbsp;":$V);$w=h($w);if(ereg('blob|bytea',$l["type"])&&!is_utf8($V)){$I=lang(array('%d Byte','%d Bytes'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$I="<img src='$w' alt='$I'>";}if(like_bool($l)&&$I!="&nbsp;")$I=($V?'yes':'no');if($w)$I="<a href='$w'>$I</a>";if(!$w&&!like_bool($l)&&ereg('int|float|double|decimal',$l["type"]))$I="<div class='number'>$I</div>";elseif(ereg('date',$l["type"]))$I="<div class='datetime'>$I</div>";return$I;}function
...
selectSearchPrint($X,$f,$s){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Suchen'."</legend><div>\n";$ic=array();foreach($X
as$u=>$V)$ic[$V["col"]]=$u;$q=0;$m=fields($_GET["select"]);foreach($f
as$A=>$Wa){$l=$m[$A];if(ereg("enum",$l["type"])||like_bool($l)){$u=$ic[$A];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($A)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$A]);}elseif(is_array($Mc=$this->_foreignKeyOptions($_GET["select"],$A))){if($m[$A]["null"])$Mc[0]='('.'leer'.')';$u=$ic[$A];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($A)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Mc,$X[$u]["val"],true)."</select></div>\n";unset($f[$A]);}}$q=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$q][col]'><option value=''>(".'beliebig'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$q][val]' value='".h($V["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$f,$s){$Oc=array();foreach($s
as$u=>$ac){$B=array();foreach($ac["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$u!="PRIMARY")$Oc[$u]=implode(", ",$B);}if($Oc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Oc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
selectLimitPrint($v){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo
html_select("limit",array("","30","100"),$v),"</div></fieldset>\n";}function
selectLengthPrint($Id){}function
selectActionPrint($s){echo"<fieldset><legend>".'Aktion'."</legend><div>","<input type='submit' value='".'Daten zeigen von'."'>","</div></fieldset>\n";}function
selectCommandPrint(){return
true;}function
selectImportPrint(){return
true;}function
selectEmailPrint($kb,$f){if($kb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Von'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Betreff'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Hinzufügen'."'>\n";echo"<p>".'Anhänge'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($kb)==1?'<input type="hidden" name="email_field" value="'.h(key($kb)).'">':html_select("email_field",$kb)),"<input type='submit' name='email' value='".'Abschicken'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
...
dumpHeaders($Wb,$_c=false){$vb="csv";header("Content-Type: text/csv; charset=utf-8");return$vb;}function
homepage(){return
true;}function
...
';if($zc=="auth"){$Bb=true;foreach((array)$_SESSION["pwds"]["server"][""]as$T=>$E){if($E!==null){if($Bb){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Bb=false;}echo"<a href='".h(auth_url("server","",$T))."'>".($T!=""?h($T):"<i>".'leer'."</i>")."</a><br>\n";}}}else{echo'<form action="" method="post">
<p class="logout">
<input type="submit" name="logout" value="Abmelden" id="logout">
<input type="hidden" name="token" value="',$S,'">
</p>
</form>
';$this->databasesPrint($zc);if($zc!="db"&&$zc!="ns"){$R=table_status();if(!$R)echo"<p class='message'>".'Keine Tabellen.'."\n";else$this->tablesPrint($R);}}}function
databasesPrint($zc){}function
tablesPrint($Fd){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Fd
as$J){$A=$this->tableName($J);if(isset($J["Engine"])&&$A!="")echo"<a href='".h(ME).'select='.urlencode($J["Name"])."'".bold($_GET["select"]==$J["Name"])." title='".'Daten auswählen'."'>$A</a><br>\n";}}function
_foreignColumn($Fb,$e){foreach((array)$Fb[$e]as$Eb){if(count($Eb["source"])==1){$A=$this->rowDescription($Eb["table"]);if($A!=""){$r=idf_escape($Eb["target"][0]);return
array($Eb["table"],$r,$A);}}}}function
_foreignKeyOptions($Q,$e,$W=null){global$g;if(list($Gd,$r,$A)=$this->_foreignColumn(column_foreign_keys($Q),$e)){$I=&$this->_values[$Gd];if($I===null){$R=table_status($Gd);$I=($R["Rows"]>1000?"":array(""=>"")+get_key_vals("SELECT $r, $A FROM ".table($Gd)." ORDER BY 2"));}if(!$I&&$W!==null)return$g->result("SELECT $A FROM ".table($Gd)." WHERE $r = ".q($W));return$I;}}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);function
...
as$A=>$l){echo"<tr><th>".$b->fieldName($l);$Va=$_GET["set"][bracket_escape($A)];$W=($J!==null?($J[$A]!=""&&$t=="sql"&&ereg("enum|set",$l["type"])?(is_array($J[$A])?array_sum($J[$A]):+$J[$A]):$J[$A]):(!$be&&$l["auto_increment"]?"":(isset($_GET["select"])?false:($Va!==null?$Va:$l["default"]))));if(!$_POST["save"]&&is_string($W))$W=$b->editVal($W,$l);$p=($_POST["save"]?(string)$_POST["function"][$A]:($be&&$l["on_update"]=="CURRENT_TIMESTAMP"?"now":($W===false?null:($W!==null?'':'NULL'))));if($l["type"]=="timestamp"&&$W=="CURRENT_TIMESTAMP"){$W="";$p="now";}input($l,$W,$p);echo"\n";}echo"</table>\n";}echo'<p>
';if($m){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($be?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}echo($be?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status($a);$s=indexes($a);$m=fields($a);$Hb=column_foreign_keys($a);$Fc="";if($R["Oid"]=="t"){$Fc=($t=="sqlite"?"rowid":"oid");$s[]=array("type"=>"PRIMARY","columns"=>array($Fc));}parse_str($_COOKIE["adminer_import"],$ha);$ld=array();$f=array();$Id=null;foreach($m
as$u=>$l){$A=$b->fieldName($l);if(isset($l["privileges"]["select"])&&$A!=""){$f[$u]=html_entity_decode(strip_tags($A));if(is_shortable($l))$Id=$b->selectLengthProcess();}$ld+=$l["privileges"];}list($L,$Nb)=$b->selectColumnsProcess($f,$s);$fc=count($Nb)<count($L);$X=$b->selectSearchProcess($m,$s);$B=$b->selectOrderProcess($m,$s);$v=$b->selectLimitProcess();$Lb=($L?implode(", ",$L):"*".($Fc?", $Fc":""));if($t=="sql"){foreach($f
as$u=>$V){$ma=convert_field($m[$u]);if($ma)$Lb.=", $ma AS ".idf_escape($u);}}$Lb.="\nFROM ".table($a);$Ob=($Nb&&$fc?"\nGROUP BY ".implode(", ",$Nb):"").($B?"\nORDER BY ".implode(", ",$B):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Xd=>$J){$ma=convert_field($m[key($J)]);echo$g->result("SELECT".limit(($ma?$ma:idf_escape(key($J)))." FROM ".table($a)," WHERE ".where_check($Xd,$m).($X?" AND ".implode(" AND ",$X):"").($B?" ORDER BY ".implode(", ",$B):""),1));}exit;}if($_POST&&!$k){$je="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$Xc=$Zd=null;foreach($s
as$ac){if($ac["type"]=="PRIMARY"){$Xc=array_flip($ac["columns"]);$Zd=($L?$Xc:array());break;}}foreach((array)$Zd
as$u=>$V){if(in_array(idf_escape($u),$L))unset($Zd[$u]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Zd===array()){$ie=$X;if(is_array($_POST["check"]))$ie[]="($je)";$G="SELECT $Lb".($ie?"\nWHERE ".implode(" AND ",$ie):"").$Ob;}else{$Vd=array();foreach($_POST["check"]as$V)$Vd[]="(SELECT".limit($Lb,"\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V,$m).$Ob,1).")";$G=implode(" UNION ALL ",$Vd);}$b->dumpData($a,"table",$G);exit;}if(!$b->selectEmailProcess($X,$Hb)){if($_POST["save"]||$_POST["delete"]){$H=true;$ia=0;$G=table($a);$N=array();if(!$_POST["delete"]){foreach($f
as$A=>$V){$V=process_input($m[$A]);if($V!==null){if($_POST["clone"])$N[idf_escape($A)]=($V!==false?$V:idf_escape($A));elseif($V!==false)$N[]=idf_escape($A)." = $V";}}$G.=($_POST["clone"]?" (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a):" SET\n".implode(",\n",$N));}if($_POST["delete"]||$N){$Ja="UPDATE";if($_POST["delete"]){$Ja="DELETE";$G="FROM $G";}if($_POST["clone"]){$Ja="INSERT";$G="INTO $G";}if($_POST["all"]||($Zd===array()&&$_POST["check"])||$fc){$H=queries("$Ja $G".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $je"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$H=queries($Ja.limit1($G,"\nWHERE ".where_check($V,$m)));if(!$H)break;$ia+=$g->affected_rows;}}}$z=sprintf('%d Artikel betroffen.',$ia);if($_POST["clone"]&&$H&&$ia==1){$nc=last_id();if($nc)$z=sprintf('Datensatz%s hinzugefügt.'," $nc");}queries_redirect(remove_from_uri("page"),$z,$H);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+klick zum Bearbeiten des Wertes.';else{$H=true;$ia=0;foreach($_POST["val"]as$Xd=>$J){$N=array();foreach($J
as$u=>$V){$u=bracket_escape($u,1);$N[]=idf_escape($u)." = ".(ereg('char|text',$m[$u]["type"])||$V!=""?$b->processInput($m[$u],$V):"NULL");}$G=table($a)." SET ".implode(", ",$N);$ie=" WHERE ".where_check($Xd,$m).($X?" AND ".implode(" AND ",$X):"");$H=queries("UPDATE".($fc?" $G$ie":limit1($G,$ie)));if(!$H)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d Artikel betroffen.',$ia),$H);}}elseif(is_string($n=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$H=true;$Ia=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$n,$tc);$ia=count($tc[0]);begin();$rd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($tc[0]as$u=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$rd]*)$rd~",$V.$rd,$uc);if(!$u&&!array_diff($uc[1],$Ia)){$Ia=$uc[1];$ia--;}else{$N=array();foreach($uc[1]as$q=>$Fa)$N[idf_escape($Ia[$q])]=($Fa==""&&$m[$Ia[$q]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Fa))));$H=insert_update($a,$N,$Xc);if(!$H)break;}}if($H)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('%d Datensatz importiert.','%d Datensätze wurden importiert.'),$ia),$H);queries("ROLLBACK");}else$k=upload_error($n);}}$Ed=$b->tableName($R);if(is_ajax())ob_start();page_header('Daten zeigen von'.": $Ed",$k);$N=null;if(isset($ld["insert"])){$N="";foreach((array)$_GET["where"]as$V){if(count($Hb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$N.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($R,$N);if(!$f)echo"<p class='error'>".'Auswahl der Tabelle fehlgeschlagen'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($X,$f,$s);$b->selectOrderPrint($B,$f,$s);$b->selectLimitPrint($v);$b->selectLengthPrint($Id);$b->selectActionPrint($s);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$o=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$D=floor(max(0,$o-1)/$v);}$G=$b->selectQueryBuild($L,$X,$Nb,$B,$v,$D);if(!$G)$G="SELECT".limit((+$v&&$Nb&&$fc&&$t=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Lb,($X?"\nWHERE ".implode(" AND ",$X):"").$Ob,($v!=""?+$v:null),($D?$v*$D:0),"\n");echo$b->selectQuery($G);$H=$g->query($G);if(!$H)echo"<p class='error'>".error()."\n";else{if($t=="mssql")$H->seek($v*$D);$lb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($D&&$t=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last")$o=(+$v&&$Nb&&$fc?($t=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($G) x")):count($K));if(!$K)echo"<p class='message'>".'Keine Daten.'."\n";else{$ua=$b->backwardKeys($a,$Ed);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Nb&&$L?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ändern'."</a>");$Bc=array();$Mb=array();reset($L);$dd=1;foreach($K[0]as$u=>$V){if($u!=$Fc){$V=$_GET["columns"][key($L)];$l=$m[$L?($V?$V["col"]:current($L)):$u];$A=($l?$b->fieldName($l,$dd):"*");if($A!=""){$dd++;$Bc[$u]=$A;$e=idf_escape($u);$Vb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Wa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Vb.($B[0]==$e||$B[0]==$u||(!$B&&$fc&&$Nb[0]==$e)?$Wa:'')).'">';echo(!$L||$V?apply_sql_function($V["fun"],$A):h(current($L)))."</a>";echo"<span class='column hidden'>","<a href='".h($Vb.$Wa)."' title='".'absteigend'."' class='text'> ↓</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Suchen'.'" class="text jsonly"> =</a>';echo"</span>";}$Mb[$u]=$V["fun"];next($L);}}$qc=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
as$u=>$V)$qc[$u]=max($qc[$u],min(40,strlen(utf8_decode($V))));}}echo($ua?"<th>".'Relationen':"")."</thead>\n";if(is_ajax()){if($v%2==1&&$D%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($K,$Hb)as$_=>$J){$Wd=unique_array($K[$_],$s);$Xd="";foreach($Wd
as$u=>$V)$Xd.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$Nb&&$L?"":"<td>".checkbox("check[]",substr($Xd,1),in_array(substr($Xd,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($fc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Xd)."'>".'ändern'."</a>"));foreach($J
as$u=>$V){if(isset($Bc[$u])){$l=$m[$u];if($V!=""&&(!isset($lb[$u])||$lb[$u]!=""))$lb[$u]=(is_mail($V)?$Bc[$u]:"");$w="";$V=$b->editVal($V,$l);if($V!==null){if(ereg('blob|bytea|raw|file',$l["type"])&&$V!="")$w=ME.'download='.urlencode($a).'&field='.urlencode($u).$Xd;if($V==="")$V="&nbsp;";elseif($Id!=""&&is_shortable($l))$V=shorten_utf8($V,max(0,+$Id));else$V=h($V);if(!$w){foreach((array)$Hb[$u]as$Gb){if(count($Hb[$u])==1||end($Gb["source"])==$u){$w="";foreach($Gb["source"]as$q=>$ud)$w.=where_link($q,$Gb["target"][$q],$K[$_][$ud]);$w=($Gb["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($Gb["db"]),ME):ME).'select='.urlencode($Gb["table"]).$w;if(count($Gb["source"])==1)break;}}}if($u=="COUNT(*)"){$w=ME."select=".urlencode($a);$q=0;foreach((array)$_GET["where"]as$U){if(!array_key_exists($U["col"],$Wd))$w.=where_link($q++,$U["col"],$U["val"],$U["op"]);}foreach($Wd
as$hc=>$U)$w.=where_link($q++,$hc,$U);}}if(!$w&&($w=$b->selectLink($J[$u],$l))===null){if(is_mail($J[$u]))$w="mailto:$J[$u]";if($ad=is_url($J[$u]))$w=($ad=="http"&&$aa?$J[$u]:"$ad://www.adminer.org/redirect/?url=".urlencode($J[$u]));}$r=h("val[$Xd][".bracket_escape($u)."]");$W=$_POST["val"][$Xd][bracket_escape($u)];$Qb=h($W!==null?$W:$J[$u]);$sc=strpos($V,"<i>...</i>");$hb=is_utf8($V)&&$K[$_][$u]==$J[$u]&&!$Mb[$u];$Hd=ereg('text|lob',$l["type"]);echo(($_GET["modify"]&&$hb)||$W!==null?"<td>".($Hd?"<textarea name='$r' cols='30' rows='".(substr_count($J[$u],"\n")+1)."'>$Qb</textarea>":"<input name='$r' value='$Qb' size='$qc[$u]'>"):"<td id='$r' onclick=\"selectClick(this, event, ".($sc?2:($Hd?1:0)).($hb?"":", '".h('Benutzen Sie den Link zum editieren dieses Wertes.')."'").");\">".$b->selectVal($V,$w,$l));}}if($ua)echo"<td>";$b->backwardKeysPrint($ua,$K[$_]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n",(!$Nb&&$L?"":"<script type='text/javascript'>tableCheck();</script>\n");}if(($K||$D)&&!is_ajax()){$rb=true;if($_GET["page"]!="last"&&+$v&&!$fc&&($o>=$v||$D)){$o=found_rows($R,$X);if($o<max(1e4,2*($D+1)*$v))$o=reset(slow_query("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):"")));else$rb=false;}echo"<p class='pages'>";if(+$v&&($o===false||$o>$v)){$vc=($o===false?$D+(count($K)>=$v?2:1):floor(($o-1)/$v));echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Seite'."', '".($D+1)."'), event); return false;\">".'Seite'."</a>:",pagination(0,$D).($D>5?" ...":"");for($q=max(1,$D-4);$q<min($vc,$D+5);$q++)echo
pagination($q,$D);echo($D+5<$vc?" ...":"").($rb&&$o!==false?pagination($vc,$D):' <a href="'.h(remove_from_uri("page")."&page=last").'">'.'letzte'."</a>");}echo($o!==false?" (".($rb?"":"~ ").lang(array('%d Datensatz','%d Datensätze'),$o).")":""),(+$v&&($o===false?count($K)+1:$o-$D*$v)>$v?' <a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" onclick="return !selectLoadMore(this, '.(+$v).', \''.'Loading'.'\');">'.'Load more data'.'</a>':'')," ".checkbox("all",1,0,'Gesamtergebnis')."\n";if($b->selectCommandPrint()){echo'<fieldset><legend>Ändern</legend><div>
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$o,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$Ib=$b->dumpFormat();if($Ib){print_fieldset("export",'Exportieren');$Pc=$b->dumpOutput();echo($Pc?html_select("output",$Pc,$ha["output"])." ":""),html_select("format",$Ib,$ha["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$K);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ha["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($lb,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$S'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["script"])){if($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));elseif(list($Q,$r,$A)=$b->_foreignColumn(column_foreign_keys($_GET["source"]),$_GET["field"])){$H=$g->query("SELECT $r, $A FROM ".table($Q)." WHERE ".(ereg('^[0-9]+$',$_GET["value"])?"$r = $_GET[value] OR ":"")."$A LIKE ".q("$_GET[value]%")." ORDER BY 2 LIMIT 11");for($q=0;$q<10&&($J=$H->fetch_row());$q++)echo"<a href='".h(ME."edit=".urlencode($Q)."&where".urlencode("[".bracket_escape(idf_unescape($r))."]")."=".urlencode($J[0]))."'>".h($J[1])."</a><br>\n";if($q==10)echo"...\n";}exit;}else{page_header('Server',"",false);if($b->homepage()){echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' value='".'Suchen'."'>\n";if($_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^tables\[/);"><th>'.'Tabelle'.'<td>'.'Datensätze'."</thead>\n";foreach(table_status()as$Q=>$J){$A=$b->tableName($J);if(isset($J["Engine"])&&$A!=""){echo'<tr'.odd().'><td>'.checkbox("tables[]",$Q,in_array($Q,(array)$_POST["tables"],true),"","formUncheck('check-all');"),"<th><a href='".h(ME).'select='.urlencode($Q)."'>$A</a>";$V=number_format($J["Rows"],0,'.',' ');echo"<td align='right'><a href='".h(ME."edit=").urlencode($Q)."'>".($J["Engine"]=="InnoDB"&&$V?"~ $V":$V)."</a>";}}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","</form>\n";}}page_footer();