...
selectLinks($rd,$O=""){$a=$rd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'Neuer Datensatz'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$u=>$q){$C=array();foreach($q["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Dc[$u]=implode(", ",$C);}if($Dc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Dc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
editFunctions($j){$J=array();if($j["null"]&&ereg('blob',$j["type"]))$J["NULL"]='leer';$J[""]=($j["null"]||$j["auto_increment"]||like_bool($j)?"":"*");if(ereg('date|time',$j["type"]))$J["now"]='jetzt';if(eregi('_(md5|sha1)$',$j["field"],$y))$J[]=strtolower($y[1]);return$J;}function
...
';if($oc!="db"&&$oc!="ns"){$S=table_status();if(!$S)echo"<p class='message'>".'Keine Tabellen.'."\n";else$this->tablesPrint($S);}}}function
tablesPrint($td){echo"<p id='tables' onmouseover='menuOver(this);' onmouseout='menuOut(this);'>\n";foreach($td
as$K){$B=$this->tableName($K);if(isset($K["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($K["Name"])."'".bold($_GET["select"]==$K["Name"])." title='".'Daten auswählen'."'>$B</a><br>\n";}}function
...
as$B=>$j){if(!isset($j["privileges"][$Pd?"update":"insert"])||$b->fieldName($j)=="")unset($k[$B]);}if($_POST&&!$i&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($Pd?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$x,'Datensatz gelöscht.');else{$O=array();foreach($k
as$B=>$j){$V=process_input($j);if($V!==false&&$V!==null)$O[idf_escape($B)]=($Pd?"\n".idf_escape($B)." = $V":$V);}if($Pd){if(!$O)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $X"),$x,'Datensatz geändert.');}else{$I=insert_into($a,$O);$dc=($I?last_id():0);queries_redirect($x,sprintf('Datensatz%s hinzugefügt.',($dc?" $dc":"")),$I);}}}$sd=$b->tableName(table_status($a));page_header(($Pd?'Ändern':'Hinzufügen'),$i,array("select"=>array($a,$sd)),$sd);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($X){$M=array();foreach($k
as$B=>$j){if(isset($j["privileges"]["select"]))$M[]=($_POST["clone"]&&$j["auto_increment"]?"'' AS ":($s=="sql"&&ereg("enum|set",$j["type"])?"1*".idf_escape($B)." AS ":"")).idf_escape($B);}$K=array();if($M){$L=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$K=(isset($_GET["select"])&&count($L)!=1?null:reset($L));}}if($K===false)echo"<p class='error'>".'Keine Daten.'."\n";echo'
...
as$B=>$V){$V=process_input($k[$B]);if($V!==null){if($_POST["clone"])$O[idf_escape($B)]=($V!==false?$V:idf_escape($B));elseif($V!==false)$O[]=idf_escape($B)." = $V";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ea="UPDATE";if($_POST["delete"]){$Ea="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ea="INSERT";$H="INTO $H";}if($_POST["all"]||($Nd===array()&&$_POST["check"])||count($n)<count($M)){$I=queries("$Ea $H".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $Xd"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$I=queries($Ea.limit1($H,"\nWHERE ".where_check($V)));if(!$I)break;$ia+=$g->affected_rows;}}}$_=sprintf('%d Artikel betroffen.',$ia);if($_POST["clone"]&&$I&&$ia==1){$dc=last_id();if($dc)$_=sprintf('Datensatz%s hinzugefügt.'," $dc");}queries_redirect(remove_from_uri("page"),$_,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$i='Doppelklick zum Bearbeiten des Wertes.';else{$I=true;$ia=0;foreach($_POST["val"]as$Ld=>$K){$O=array();foreach($K
as$u=>$V){$u=bracket_escape($u,1);$O[]=idf_escape($u)." = ".(ereg('char|text',$k[$u]["type"])||$V!=""?$b->processInput($k[$u],$V):"NULL");}$H=table($a)." SET ".implode(", ",$O);$Wd=" WHERE ".where_check($Ld).($X?" AND ".implode(" AND ",$X):"");$I=queries("UPDATE".(count($n)<count($M)?" $H$Wd":limit1($H,$Wd)));if(!$I)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d Artikel betroffen.',$ia),$I);}}elseif(is_string($l=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Da=array_keys($k);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$l,$z);$ia=count($z[0]);begin();$ed=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($z[0]as$u=>$V){preg_match_all("~((\"[^\"]*\")+|[^$ed]*)$ed~",$V.$ed,$jc);if(!$u&&!array_diff($jc[1],$Da)){$Da=$jc[1];$ia--;}else{$O=array();foreach($jc[1]as$o=>$Aa)$O[idf_escape($Da[$o])]=($Aa==""&&$k[$Da[$o]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Aa))));$I=insert_update($a,$O,$Mc);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('%d Datensatz importiert.','%d Datensätze wurden importiert.'),$ia),$I);queries("ROLLBACK");}else$i=upload_error($l);}}$sd=$b->tableName($S);page_header('Daten zeigen von'.": $sd",$i);session_write_close();$O=null;if(isset($Yc["insert"])){$O="";foreach((array)$_GET["where"]as$V){if(count($Bb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$O.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($S,$O);if(!$f)echo"<p class='error'>".'Auswahl der Tabelle fehlgeschlagen'.($k?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($X,$f,$r);$b->selectOrderPrint($C,$f,$r);$b->selectLimitPrint($v);$b->selectLengthPrint($wd);$b->selectActionPrint($r);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$Eb=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$E=floor(max(0,$Eb-1)/$v);}$H="SELECT".limit((+$v&&$n&&count($n)<count($M)&&$s=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Gb,($X?"\nWHERE ".implode(" AND ",$X):"").$Ib,($v!=""?+$v:null),($E?$v*$E:0),"\n");echo$b->selectQuery($H);$I=$g->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($s=="mssql")$I->seek($v*$E);$fb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$s=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$Eb=(+$v&&$n&&count($n)<count($M)?($s=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Keine Daten.'."\n";else{$ta=$b->backwardKeys($a,$sd);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$n&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ändern'."</a>");$rc=array();$Hb=array();reset($M);$Sc=1;foreach($L[0]as$u=>$V){if($u!=$vc){$V=$_GET["columns"][key($M)];$j=$k[$M?($V?$V["col"]:current($M)):$u];$B=($j?$b->fieldName($j,$Sc):"*");if($B!=""){$Sc++;$rc[$u]=$B;$e=idf_escape($u);$Pb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Ta="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Pb.($C[0]==$e||$C[0]==$u||(!$C&&count($n)<count($M)&&$n[0]==$e)?$Ta:'')).'">';echo(!$M||$V?apply_sql_function($V["fun"],$B):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($Pb.$Ta)."' title='".'absteigend'."' class='text'> ↓</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Suchen'.'" class="text jsonly"> =</a>';echo"</span>";}$Hb[$u]=$V["fun"];next($M);}}$gc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
...
as$u=>$V)$Ld.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$n&&$M?"":"<td>".checkbox("check[]",substr($Ld,1),in_array(substr($Ld,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($n)<count($M)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Ld)."'>".'ändern'."</a>"));foreach($K
