...
enum_input($ua,$J,$e,$i,$Jb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$R);$b=(isset($Jb)?"<label><input type='$ua'$J value='$Jb'".((is_array($i)?in_array($Jb,$i):$i===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($R[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$bb=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ua'$J value='".($n+1)."'".($bb?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
...
as$f=>$ma){$Z=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Z)){if($q[$f]["null"]){$Z[0]='('.'leer'.')';}$c=$Ib[$f];$n--;echo"<div>".h($ma)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Z,$p[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($p
...
as$c=>$S){$V=array();foreach($S["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Nb[$c]=implode(", ",$V);}}if($Nb){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Nb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
as$f=>$e){if(!isset($e["privileges"][$ba?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$I=$_POST["referer"];if($_POST["insert"]){$I=($ba?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$I)){$I=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$I,'Datensatz gelöscht.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($ba?"\n".idf_escape($f)." = $a":$a);}}if($ba){if(!$s){redirect($I);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $p"),$I,'Datensatz geändert.');}else{$k=insert_into($r,$s);$Lc=($k?last_id():0);queries_redirect($I,sprintf('Datensatz%s hinzugefügt.',($Lc?" $Lc":"")),$k);}}}$La=$j->tableName(table_status($r));page_header(($ba?'Ändern':'Hinzufügen'),$v,array("select"=>array($r,$La)),$La);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$u=array();foreach($q
...
as$c=>$a){$ia.="&".(isset($a)?urlencode("where[".bracket_escape($c)."]")."=".urlencode($a):"null%5B%5D=".urlencode($c));}echo"<tr".odd().">".(!$M&&$u?"":"<td>".checkbox("check[]",substr($ia,1),in_array(substr($ia,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($M)<count($u)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($r).$ia)."'>".'ändern'."</a>"));foreach($d
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$fa,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
