...
upload_error($k){$uc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($uc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$uc):""):'Datei existiert nicht.');}function
...
selectLinks($Cd,$P=""){$a=$Cd["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Neuer Datensatz'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a>\n";}function
...
as$e=>$V)$x.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($L[$V]);echo"<a href='".h($x)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
...
selectSearchPrint($X,$f,$t){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Suchen'."</legend><div>\n";$hc=array();foreach($X
as$v=>$V)$hc[$V["col"]]=$v;$q=0;$m=fields($_GET["select"]);foreach($f
as$C=>$Wa){$l=$m[$C];if(ereg("enum",$l["type"])||like_bool($l)){$v=$hc[$C];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($C)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'no','yes'),$X[$v]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$v]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$C]);}elseif(is_array($Kc=$this->_foreignKeyOptions($_GET["select"],$C))){if($m[$C]["null"])$Kc[0]='('.'leer'.')';$v=$hc[$C];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($C)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Kc,$X[$v]["val"],true)."</select></div>\n";unset($f[$C]);}}$q=0;foreach($X
...
as$v=>$s){$D=array();foreach($s["columns"]as$V)$D[]=$f[$V];if(count(array_filter($D,'strlen'))>1&&$v!="PRIMARY")$Mc[$v]=implode(", ",$D);}if($Mc){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Mc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
selectLimitPrint($w){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo
...
';$this->databasesPrint($xc);if($xc!="db"&&$xc!="ns"){$S=table_status();if(!$S)echo"<p class='message'>".'Keine Tabellen.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($xc){}function
tablesPrint($Ed){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Ed
as$L){$C=$this->tableName($L);if(isset($L["Engine"])&&$C!="")echo"<a href='".h(ME).'select='.urlencode($L["Name"])."'".bold($_GET["select"]==$L["Name"])." title='".'Daten auswählen'."'>$C</a><br>\n";}}function
...
as$C=>$l){if(!isset($l["privileges"][$be?"update":"insert"])||$b->fieldName($l)=="")unset($m[$C]);}if($_POST&&!$k&&!isset($_GET["select"])){$y=$_POST["referer"];if($_POST["insert"])$y=($be?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$y))$y=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$y,'Datensatz gelöscht.');else{$P=array();foreach($m
as$C=>$l){$V=process_input($l);if($V!==false&&$V!==null)$P[idf_escape($C)]=($be?"\n".idf_escape($C)." = $V":$V);}if($be){if(!$P)redirect($y);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $X"),$y,'Datensatz geändert.');}else{$J=insert_into($a,$P);$mc=($J?last_id():0);queries_redirect($y,sprintf('Datensatz%s hinzugefügt.',($mc?" $mc":"")),$J);}}}$Dd=$b->tableName(table_status($a));page_header(($be?'Ändern':'Hinzufügen'),$k,array("select"=>array($a,$Dd)),$Dd);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($X){$N=array();foreach($m
as$C=>$l){if(isset($l["privileges"]["select"])){$ma=convert_field($l);if($_POST["clone"]&&$l["auto_increment"])$ma="''";if($u=="sql"&&ereg("enum|set",$l["type"]))$ma="1*".idf_escape($C);$N[]=($ma?"$ma AS ":"").idf_escape($C);}}$L=array();if($N){$M=get_rows("SELECT".limit(implode(", ",$N)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$L=(isset($_GET["select"])&&count($M)!=1?null:reset($M));}}if($L===false)echo"<p class='error'>".'Keine Daten.'."\n";echo'
