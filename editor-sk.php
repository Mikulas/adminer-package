...
upload_error($m){$pd=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Súbor sa nepodarilo nahrať.'.($pd?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$pd):""):'Súbor neexistuje.');}function
...
h($g->error);}function
check_sqlite_name($C){global$g;$Xb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Xb)\$~",$C)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$Xb));return
false;}return
true;}function
create_database($l,$d){global$g;if(file_exists($l)){$g->error='Súbor existuje.';return
...
as$l){if(!@unlink($l)){$g->error='Súbor existuje.';return
false;}}return
true;}function
rename_database($C,$d){global$g;if(!check_sqlite_name($C))return
false;$g->Min_SQLite(":memory:");$g->error='Súbor existuje.';return@rename(DB,$C);}function
...
set_schema($Ce){global$g,$Bf,$We;$K=$g->query("SET search_path TO ".idf_escape($Ce));foreach(types()as$U){if(!isset($Bf[$U])){$Bf[$U]=0;$We['Užívateľské typy'][]=$U;}}return$K;}function
...
<tr><th>Používateľ<td><input type="hidden" name="auth[driver]" value="server"><input name="auth[username]" id="username" value="',h($_GET["username"]),'" autocapitalize="off">
...
selectVal($X,$_,$n,$Pd){$K=($X===null?"&nbsp;":$X);$_=h($_);if(preg_match('~blob|bytea~',$n["type"])&&!is_utf8($X)){$K=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($Pd));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$Pd))$K="<img src='$_' alt='$K'>";}if(like_bool($n)&&$K!="&nbsp;")$K=($X?'yes':'no');if($_)$K="<a href='$_'".(is_url($_)?" rel='noreferrer'":"").">$K</a>";if(!$_&&!like_bool($n)&&preg_match('~int|float|double|decimal~',$n["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$n["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($Z,$f,$w){$Z=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Vyhľadať'."</legend><div>\n";$Xc=array();foreach($Z
as$y=>$X)$Xc[$X["col"]]=$y;$s=0;$o=fields($_GET["select"]);foreach($f
as$C=>$tb){$n=$o[$C];if(preg_match("~enum~",$n["type"])||like_bool($n)){$y=$Xc[$C];$s--;echo"<div>".h($tb)."<input type='hidden' name='where[$s][col]' value='".h($C)."'>:",(like_bool($n)?" <select name='where[$s][val]'>".optionlist(array(""=>"",'no','yes'),$Z[$y]["val"],true)."</select>":enum_input("checkbox"," name='where[$s][val][]'",$n,(array)$Z[$y]["val"],($n["null"]?0:null))),"</div>\n";unset($f[$C]);}elseif(is_array($Md=$this->_foreignKeyOptions($_GET["select"],$C))){if($o[$C]["null"])$Md[0]='('.'prázdne'.')';$y=$Xc[$C];$s--;echo"<div>".h($tb)."<input type='hidden' name='where[$s][col]' value='".h($C)."'><input type='hidden' name='where[$s][op]' value='='>: <select name='where[$s][val]'>".optionlist($Md,$Z[$y]["val"],true)."</select></div>\n";unset($f[$C]);}}$s=0;foreach($Z
as$X){if(($X["col"]==""||$f[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$s][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,$X["col"],true)."</select>",html_select("where[$s][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$s][val]' value='".h($X["val"])."' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";$s++;}}echo"<div><select name='where[$s][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoľvek'.")".optionlist($f,null,true)."</select>",html_select("where[$s][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$s][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($E,$f,$w){$Od=array();foreach($w
as$y=>$v){$E=array();foreach($v["columns"]as$X)$E[]=$f[$X];if(count(array_filter($E,'strlen'))>1&&$y!="PRIMARY")$Od[$y]=implode(", ",$E);}if($Od){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Od,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';if($wd=="auth"){$fc=true;foreach((array)$_SESSION["pwds"]as$Qf=>$Me){foreach($Me[""]as$V=>$H){if($H!==null){if($fc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$fc=false;}echo"<a href='".h(auth_url($Qf,"",$V))."'>".($V!=""?h($V):"<i>".'prázdne'."</i>")."</a><br>\n";}}}}else{$this->databasesPrint($wd);if($wd!="db"&&$wd!="ns"){$S=table_status('',true);if(!$S)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($wd){}function
tablesPrint($T){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($T
as$L){$C=$this->tableName($L);if(isset($L["Engine"])&&$C!="")echo"<a href='".h(ME).'select='.urlencode($L["Name"])."'".bold($_GET["select"]==$L["Name"]||$_GET["edit"]==$L["Name"],"select")." title='".'Vypísať dáta'."'>$C</a><br>\n";}}function
...
as$Qc=>$X){if($X[0]<$lf)unset($Pc[$Qc]);}}$Oc=&$Pc[$b->bruteForceKey()];if(!$Oc)$Oc=array($lf+30*60,0);$Oc[1]++;$Ke=serialize($Pc);rewind($oc);fwrite($oc,$Ke);ftruncate($oc,strlen($Ke));flock($oc,LOCK_UN);fclose($oc);}$Ca=$_POST["auth"];if($Ca){$Pc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Oc=$Pc[$b->bruteForceKey()];$Ad=($Oc[1]>30?$Oc[0]-time():0);if($Ad>0)auth_error(sprintf('Too many unsuccessful logins, try again in %d minute(s).',ceil($Ad/60)));session_regenerate_id();$Qf=$Ca["driver"];$O=$Ca["server"];$V=$Ca["username"];$H=(string)$Ca["password"];$l=$Ca["db"];set_password($Qf,$O,$V,$H);$_SESSION["db"][$Qf][$O][$V][$l]=true;if($Ca["permanent"]){$y=base64_encode($Qf)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($l);$he=$b->permanentLogin(true);$Yd[$y]="$y:".base64_encode($he?encrypt_string($H,$he):"");cookie("adminer_permanent",implode(" ",$Yd));}if(count($_POST)==1||DRIVER!=$Qf||SERVER!=$O||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($Qf,$O,$V,$l));}elseif($_POST["logout"]){if($wc&&!verify_token()){page_header('Odhlásiť','Neplatný token CSRF. Odošlite formulár znova.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Odhlásenie prebehlo v poriadku.');}}elseif($Yd&&!$_SESSION["pwds"]){session_regenerate_id();$he=$b->permanentLogin();foreach($Yd
...
auth_error($m){global$b,$wc;$m=h($m);$Ne=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Ne]||$_GET[$Ne])&&!$wc)$m='Session vypršala, prihláste sa prosím znova.';else{add_invalid_login();$H=get_password();if($H!==null){if($H===false)$m.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Ne]&&$_GET[$Ne]&&ini_bool("session.use_only_cookies"))$m='Session premenné musia byť povolené.';$G=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$G["lifetime"]);page_header('Prihlásiť sa',$m,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$ce)),false);page_footer("auth");exit;}$g=connect();}$_b=new
Min_Driver($g);if(!is_object($g)||!$b->login($_GET["username"],get_password()))auth_error((is_string($g)?$g:'Neplatné prihlasovacie údaje.'));if($Ca&&$_POST["token"])$_POST["token"]=$tf;$m='';if($_POST){if(!verify_token()){$Kc="max_input_vars";$qd=ini_get($Kc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$qd||$X<$qd)){$Kc=$y;$qd=$X;}}}$m=(!$_POST["token"]&&$qd?sprintf('Bol prekročený maximálny počet povolených polí. Zvýšte prosím %s.',"'$Kc'"):'Neplatný token CSRF. Odošlite formulár znova.'.' '.'If you did not send this request from Adminer then close this page.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
