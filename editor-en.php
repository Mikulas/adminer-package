...
set_schema($xe){global$h,$uf,$Pe;$K=$h->query("SET search_path TO ".idf_escape($xe));foreach(types()as$U){if(!isset($uf[$U])){$uf[$U]=0;$Pe['User types'][]=$U;}}return$K;}function
...
selectLinks($Ve,$P=""){$a=$Ve["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'New item'."</a>\n";}function
...
long2str($W,true);}$h='';$uc=$_SESSION["token"];if(!$uc)$_SESSION["token"]=rand(1,1e6);$mf=get_token();$Sd=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($z)=explode(":",$X);$Sd[$z]=$X;}}$Aa=$_POST["auth"];if($Aa){session_regenerate_id();$wb=$Aa["driver"];$O=$Aa["server"];$V=$Aa["username"];$H=$Aa["password"];$m=$Aa["db"];set_password($wb,$O,$V,$H);$_SESSION["db"][$wb][$O][$V][$m]=true;if($Aa["permanent"]){$z=base64_encode($wb)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($m);$be=$b->permanentLogin(true);$Sd[$z]="$z:".base64_encode($be?encrypt_string($H,$be):"");cookie("adminer_permanent",implode(" ",$Sd));}if(count($_POST)==1||DRIVER!=$wb||SERVER!=$O||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($wb,$O,$V,$m));}elseif($_POST["logout"]){if($uc&&!verify_token()){page_header('Logout','Invalid CSRF token. Send the form again.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$z)set_session($z,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Logout successful.');}}elseif($Sd&&!$_SESSION["pwds"]){session_regenerate_id();$be=$b->permanentLogin();foreach($Sd
...
as$D=>$o){if(!isset($o["privileges"][$Bf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$dd=$_POST["referer"];if($_POST["insert"])$dd=($Bf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$dd))$dd=ME."select=".urlencode($a);$x=indexes($a);$xf=unique_array($_GET["where"],$x);$he="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($dd,'Item has been deleted.',$wb->delete($a,$he,!$xf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Bf){if(!$P)redirect($dd);queries_redirect($dd,'Item has been updated.',$wb->update($a,$P,$he,!$xf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$wb->insert($a,$P);$Yc=($J?last_id():0);queries_redirect($dd,sprintf('Item%s has been inserted.',($Yc?" $Yc":"")),$J);}}}$We=$b->tableName(table_status1($a,true));page_header(($Bf?'Edit':'Insert'),$n,array("select"=>array($a,$We)),$We);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$z=>$X){if(!$Z)$L[$z]=null;$p[$z]=array("field"=>$z,"null"=>($z!="itemName()"),"auto_increment"=>($z=="itemName()"));}}}if($L===false)echo"<p class='error'>".'No rows.'."\n";echo'
<div id="message"></div>
...
';if($p){echo"<input type='submit' value='".'Save'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Bf?'Save and continue edit'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'Save and insert next')."' title='Ctrl+Shift+Enter'>\n";}echo($Bf?"<input type='submit' name='delete' value='".'Delete'."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$z=>$X){if(strlen($X)>64&&($y=="sql"||$y=="pgsql")){$z="MD5(".(strpos($z,'(')?$z:idf_escape($z)).")";$X=md5($X);}$yf.="&".($X!==null?urlencode("where[".bracket_escape($z)."]")."=".urlencode($X):"null%5B%5D=".urlencode($z));}echo"<tr".odd().">".(!$s&&$N?"":"<td>".checkbox("check[]",substr($yf,1),in_array(substr($yf,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Nc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$yf)."'>".'edit'."</a>"));foreach($L
