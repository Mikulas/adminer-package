...
* @version 3.3.2
*/error_reporting(6135);$cd=(!ereg('^(unsafe_raw)?$',ini_get("filter.default")));if($cd||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$b){$te=filter_input_array(constant("INPUT$b"),FILTER_UNSAFE_RAW);if($te){$$b=$te;}}}if(isset($_GET["file"])){header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
remove_slashes($pb,$cd=false){if(get_magic_quotes_gpc()){while(list($e,$b)=each($pb)){foreach($b
as$Ka=>$z){unset($pb[$e][$Ka]);if(is_array($z)){$pb[$e][stripslashes($Ka)]=$z;$pb[]=&$pb[$e][stripslashes($Ka)];}else{$pb[$e][stripslashes($Ka)]=($cd?$z:stripslashes($z));}}}}}function
bracket_escape($Q,$Hf=false){static$Ie=array(':'=>':1',']'=>':2','['=>':3');return
strtr($Q,($Hf?array_flip($Ie):$Ie));}function
h($I){return
htmlspecialchars(str_replace("\0","",$I),ENT_QUOTES);}function
nbsp($I){return(trim($I)!=""?h($I):"&nbsp;");}function
nl_br($I){return
str_replace("\n","<br>",$I);}function
checkbox($f,$q,$db,$Ke="",$Gc="",$Ff=false){static$R=0;$R++;$c="<input type='checkbox' name='$f' value='".h($q)."'".($db?" checked":"").($Gc?' onclick="'.h($Gc).'"':'').($Ff?" class='jsonly'":"")." id='checkbox-$R'>";return($Ke!=""?"<label for='checkbox-$R'>$c".h($Ke)."</label>":$c);}function
optionlist($Bc,$Lf=null,$Ue=false){$c="";foreach($Bc
as$Ka=>$z){$Ve=array($Ka=>$z);if(is_array($z)){$c.='<optgroup label="'.h($Ka).'">';$Ve=$z;}foreach($Ve
as$e=>$b){$c.='<option'.($Ue||is_string($e)?' value="'.h($e).'"':'').(($Ue||is_string($e)?(string)$e:$b)===$Lf?' selected':'').'>'.h($b);}if(is_array($z)){$c.='</optgroup>';}}return$c;}function
html_select($f,$Bc,$q="",$Gb=true){if($Gb){return"<select name='".h($f)."'".(is_string($Gb)?' onchange="'.h($Gb).'"':"").">".optionlist($Bc,$q)."</select>";}$c="";foreach($Bc
as$e=>$b){$c.="<label><input type='radio' name='".h($f)."' value='".h($e)."'".($e==$q?" checked":"").">".h($b)."</label>";}return$c;}function
confirm($Rc="",$Tf=false){return" onclick=\"".($Tf?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Rc?" (' + $Rc + ')":"")."');\"";}function
print_fieldset($R,$Uf,$Vf=false,$Gc=""){echo"<fieldset><legend><a href='#fieldset-$R' onclick=\"".h($Gc)."return !toggle('fieldset-$R');\">$Uf</a></legend><div id='fieldset-$R'".($Vf?"":" class='hidden'").">\n";}function
...
where_check($b){parse_str($b,$af);remove_slashes(array(&$af));return
where($af);}function
...
cookie($f,$q){global$xc;$mc=array($f,(ereg("\n",$q)?"":$q),time()+2592000,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$xc);if(version_compare(PHP_VERSION,'5.2.0')>=0){$mc[]=true;}return
...
auth_url($Ib,$F,$P){global$ma;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($ma))."|username|".session_name()),$k);return"$k[1]?".(sid()?SID."&":"").($Ib!="server"||$F!=""?urlencode($Ib)."=".urlencode($F)."&":"")."username=".urlencode($P).($k[2]?"&$k[2]":"");}function
...
query_redirect($i,$fa,$ta,$id=true,$lf=true,$ne=false){global$g,$n,$o;if($lf){$ne=!$g->query($i);}$Ld="";if($i){$Ld=$o->messageQuery("$i;");}if($ne){$n=error().$Ld;return
false;}if($id){redirect($fa,$ta.$Ld);}return
true;}function
...
false;}}return
true;}function
queries_redirect($fa,$ta,$id){return
query_redirect(queries(),$fa,$ta,$id,false,!$id);}function
...
get_file($e,$Md=false){$Ga=$_FILES[$e];if(!$Ga||$Ga["error"]){return$Ga["error"];}$c=file_get_contents($Md&&ereg('\\.gz$',$Ga["name"])?"compress.zlib://$Ga[tmp_name]":($Md&&ereg('\\.bz2$',$Ga["name"])?"compress.bzip2://$Ga[tmp_name]":$Ga["tmp_name"]));if($Md){$Jb=substr($c,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$Jb,$xe)){$c=iconv("utf-16","utf-8",$c);}elseif($Jb=="\xEF\xBB\xBF"){$c=substr($c,3);}}return$c;}function
upload_error($n){$ye=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Súbor sa nepodarilo nahrať.'.($ye?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$ye):""):'Súbor neexistuje.');}function
...
hidden_fields($pb,$xf=array()){while(list($e,$b)=each($pb)){if(is_array($b)){foreach($b
as$Ka=>$z){$pb[$e."[$Ka]"]=$z;}}elseif(!in_array($e,$xf)){echo'<input type="hidden" name="'.h($e).'" value="'.h($b).'">';}}}function
...
enum_input($_,$Ua,$d,$q,$ac=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);$c=(isset($ac)?"<label><input type='$_'$Ua value='$ac'".((is_array($q)?in_array($ac,$q):$q===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($sa[1]as$l=>$b){$b=stripcslashes(str_replace("''","'",$b));$db=(is_int($q)?$q==$l+1:(is_array($q)?in_array($l+1,$q):$q===$b));$c.=" <label><input type='$_'$Ua value='".($l+1)."'".($db?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$N){global$S,$o,$v;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Fe=($v=="mssql"&&$d["auto_increment"]);if($Fe&&!$_POST["save"]){$N=null;}$aa=(isset($_GET["select"])||$Fe?array("orig"=>'originál'):array())+$o->editFunctions($d);$Ua=" name='fields[$f]'";if($d["type"]=="enum"){echo
nbsp($aa[""])."<td>".$o->editInput($_GET["edit"],$d,$Ua,$q);}else{$_a=0;foreach($aa
as$e=>$b){if($e===""||!$b){break;}$_a++;}$Gb=($_a?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($d["field"])))."]']; if ($_a > f.selectedIndex) f.selectedIndex = $_a;\"":"");$Ua.=$Gb;echo(count($aa)>1?html_select("function[$f]",$aa,!isset($N)||in_array($N,$aa)||isset($aa[$N])?$N:"","functionChange(this);"):nbsp(reset($aa))).'<td>';$Ge=$o->editInput($_GET["edit"],$d,$Ua,$q);if($Ge!=""){echo$Ge;}elseif($d["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);foreach($sa[1]as$l=>$b){$b=stripcslashes(str_replace("''","'",$b));$db=(is_int($q)?($q>>$l)&1:in_array($b,explode(",",$q),true));echo" <label><input type='checkbox' name='fields[$f][$l]' value='".(1<<$l)."'".($db?' checked':'')."$Gb>".h($o->editVal($b,$d)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$d["type"])&&ini_bool("file_uploads")){echo"<input type='file' name='fields-$f'$Gb>";}elseif(ereg('text|lob',$d["type"])){echo"<textarea ".($v!="sqlite"||ereg("\n",$q)?"cols='50' rows='12'":"cols='30' rows='1' style='height: 1.2em;'")."$Ua>".h($q).'</textarea>';}else{$ud=(!ereg('int',$d["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$d["length"],$k)?((ereg("binary",$d["type"])?2:1)*$k[1]+($k[3]?1:0)+($k[2]&&!$d["unsigned"]?1:0)):($S[$d["type"]]?$S[$d["type"]]+($d["unsigned"]?0:1):0));echo"<input value='".h($q)."'".($ud?" maxlength='$ud'":"").(ereg('char|binary',$d["type"])&&$ud>20?" size='40'":"")."$Ua>";}}}function
...
password_file(){$Qc=ini_get("upload_tmp_dir");if(!$Qc){if(function_exists('sys_get_temp_dir')){$Qc=sys_get_temp_dir();}else{$ka=@tempnam("","");if(!$ka){return
false;}$Qc=dirname($ka);unlink($ka);}}$ka="$Qc/adminer.key";$c=@file_get_contents($ka);if($c){return$c;}$Ja=@fopen($ka,"w");if($Ja){$c=md5(uniqid(mt_rand(),true));fwrite($Ja,$c);fclose($Ja);}return$c;}function
is_mail($uf){$Ce='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$zc='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$ca="$Ce+(\\.$Ce+)*@($zc?\\.)+$zc";return
preg_match("(^$ca(,\\s*$ca)*\$)i",$uf);}function
is_url($I){$zc='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($zc?\\.)+$zc(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$I,$k)?strtolower($k[1]):"");}global$o,$g,$ma,$Ob,$Cb,$n,$aa,$kb,$xc,$cc,$v,$Bf,$Wf,$Ea,$Na,$O,$Xf,$S,$jb,$Ac;if(!isset($_SERVER["REQUEST_URI"])){$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"].($_SERVER["QUERY_STRING"]!=""?"?$_SERVER[QUERY_STRING]":"");}$xc=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$mc=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$xc);if(version_compare(PHP_VERSION,'5.2.0')>=0){$mc[]=true;}call_user_func_array('session_set_cookie_params',$mc);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$cd);if(function_exists("set_magic_quotes_runtime")){set_magic_quotes_runtime(false);}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'sk';}function
lang($Qf,$Yc){$yc=($Yc==1?0:($Yc&&$Yc<5?1:2));return
sprintf($Qf[$yc],$Yc);}if(extension_loaded('pdo')){class
Min_PDO
extends
...
SQLite3($ka);$ld=$this->_link->version();$this->server_info=$ld["versionString"];}function
...
indexes($h,$G=null){$c=array();$Pa=array();foreach(fields($h)as$d){if($d["primary"]){$Pa[]=$d["field"];}}if($Pa){$c[""]=array("type"=>"PRIMARY","columns"=>$Pa,"lengths"=>array());}foreach(get_rows("PRAGMA index_list(".table($h).")")as$a){$c[$a["name"]]["type"]=($a["unique"]?"UNIQUE":"INDEX");$c[$a["name"]]["lengths"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($a["name"]).")")as$Fc){$c[$a["name"]]["columns"][]=$Fc["name"];}}return$c;}function
...
q($b);}function
check_sqlite_name($f){global$g;$He="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($He)\$~",$f)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$He));return
false;}return
true;}function
...
alter_table($h,$f,$p,$La,$za,$tb,$U,$Sa,$ob){$w=array();foreach($p
...
support($ub){return
ereg('^(view|trigger|variables|status|dump)$',$ub);}$v="sqlite";$S=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$Na=array_keys($S);$jb=array();$gc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","");$aa=array("hex","length","lower","round","unixepoch","upper");$kb=array("avg","count","count distinct","group_concat","max","min","sum");$Ob=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$ma["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$_c=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
AND contype = 'f'::char
ORDER BY conkey, conname")as$a){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$a['definition'],$k)){$a['source']=array_map('trim',explode(',',$k[1]));$a['table']=$k[2];if(preg_match('~(.+)\.(.+)~',$k[2],$Vb)){$a['ns']=$Vb[1];$a['table']=$Vb[2];}$a['target']=array_map('trim',explode(',',$k[3]));$a['on_delete']=(preg_match("~ON DELETE ($Ea)~",$k[4],$Vb)?$Vb[1]:'');$a['on_update']=(preg_match("~ON UPDATE ($Ea)~",$k[4],$Vb)?$Vb[1]:'');$c[$a['conname']]=$a;}}return$c;}function
view($f){global$g;return
...
alter_table($h,$f,$p,$La,$za,$tb,$U,$Sa,$ob){$w=array();$gb=array();foreach($p
as$d){$D=idf_escape($d[0]);$b=$d[1];if(!$b){$w[]="DROP $D";}else{$wd=$b[5];unset($b[5]);if(isset($b[6])&&$d[0]==""){$b[1]=($b[1]=="bigint"?" big":" ")."serial";}if($d[0]==""){$w[]=($h!=""?"ADD ":"  ").implode($b);}else{if($D!=$b[0]){$gb[]="ALTER TABLE ".table($h)." RENAME $D TO $b[0]";}$w[]="ALTER $D TYPE$b[1]";if(!$b[6]){$w[]="ALTER $D ".($b[3]?"SET$b[3]":"DROP DEFAULT");$w[]="ALTER $D ".($b[2]==" NULL"?"DROP NOT":"SET").$b[2];}}if($d[0]!=""||$wd!=""){$gb[]="COMMENT ON COLUMN ".table($h).".$b[0] IS ".($wd!=""?substr($wd,9):"''");}}}$w=array_merge($w,$La);if($h==""){array_unshift($gb,"CREATE TABLE ".table($f)." (\n".implode(",\n",$w)."\n)");}elseif($w){array_unshift($gb,"ALTER TABLE ".table($h)."\n".implode(",\n",$w));}if($h!=""&&$h!=$f){$gb[]="ALTER TABLE ".table($h)." RENAME TO ".table($f);}if($h!=""||$za!=""){$gb[]="COMMENT ON TABLE ".table($f)." IS ".q($za);}if($Sa!=""){}foreach($gb
...
0;}function
explain($g,$i){return$g->query("EXPLAIN $i");}function
found_rows($u,$s){global$g;if(ereg(" rows=([0-9]+)",$g->result("EXPLAIN SELECT * FROM ".idf_escape($u["Name"]).($s?" WHERE ".implode(" AND ",$s):"")),$xe)){return$xe[1];}return
false;}function
types(){return
...
set_schema($Oa){global$g,$S,$Na;$c=$g->query("SET search_path TO ".idf_escape($Oa));foreach(types()as$_){if(!isset($S[$_])){$S[$_]=0;$Na['Užívateľské typy'][]=$_;}}return$c;}function
...
found_rows($u,$s){}function
alter_table($h,$f,$p,$La,$za,$tb,$U,$Sa,$ob){$w=$Ha=array();foreach($p
...
reset($H);}function
support($ub){return
ereg("view|drop_col|variables|status",$ub);}$v="oracle";$S=array();$Na=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$S+=$b;$Na[$e]=array_keys($b);}$jb=array();$gc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$aa=array("length","lower","round","upper");$kb=array("avg","count","count distinct","max","min","sum");$Ob=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ma["mssql"]="MS SQL";if(isset($_GET["mssql"])){$_c=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
foreign_keys($h){global$g,$Ea;static$ca='`(?:[^`]|``)+`';$c=array();$we=$g->result("SHOW CREATE TABLE ".table($h),1);if($we){preg_match_all("~CONSTRAINT ($ca) FOREIGN KEY \\(((?:$ca,? ?)+)\\) REFERENCES ($ca)(?:\\.($ca))? \\(((?:$ca,? ?)+)\\)(?: ON DELETE ($Ea))?(?: ON UPDATE ($Ea))?~",$we,$sa,PREG_SET_ORDER);foreach($sa
...
apply_queries("DROP DATABASE",$A,'idf_escape');}function
rename_database($f,$U){if(create_database($f,$U)){$Wb=array();foreach(tables_list()as$h=>$_){$Wb[]=table($h)." TO ".idf_escape($f).".".table($h);}if(!$Wb||queries("RENAME TABLE ".implode(", ",$Wb))){queries("DROP DATABASE ".idf_escape(DB));return
true;}}return
false;}function
auto_increment(){$Ad=" PRIMARY KEY";if($_GET["create"]!=""&&$_POST["auto_increment_col"]){foreach(indexes($_GET["create"])as$x){if(in_array($_POST["fields"][$_POST["auto_increment_col"]]["orig"],$x["columns"],true)){$Ad="";break;}if($x["type"]=="PRIMARY"){$Ad=" UNIQUE";}}}return" AUTO_INCREMENT$Ad";}function
alter_table($h,$f,$p,$La,$za,$tb,$U,$Sa,$ob){$w=array();foreach($p
as$d){$w[]=($d[1]?($h!=""?($d[0]!=""?"CHANGE ".idf_escape($d[0]):"ADD"):" ")." ".implode($d[1]).($h!=""?" $d[2]":""):"DROP ".idf_escape($d[0]));}$w=array_merge($w,$La);$Sb="COMMENT=".q($za).($tb?" ENGINE=".q($tb):"").($U?" COLLATE ".q($U):"").($Sa!=""?" AUTO_INCREMENT=$Sa":"").$ob;if($h==""){return
queries("CREATE TABLE ".table($f)." (\n".implode(",\n",$w)."\n) $Sb");}if($h!=$f){$w[]="RENAME TO ".table($f);}$w[]=$Sb;return
...
drop_tables($E){return
queries("DROP TABLE ".implode(", ",array_map('table',$E)));}function
move_tables($E,$X,$ea){$Wb=array();foreach(array_merge($E,$X)as$h){$Wb[]=table($h)." TO ".idf_escape($ea).".".table($h);}return
...
false;}}foreach($X
as$h){$f=($ea==DB?table("copy_$h"):idf_escape($ea).".".table($h));$Zb=view($h);if(!queries("DROP VIEW IF EXISTS $f")||!queries("CREATE VIEW $f AS $Zb[select]")){return
false;}}return
true;}function
...
routine($f,$_){global$g,$Cb,$cc,$S;$rf=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$ve="((".implode("|",array_merge(array_keys($S),$rf)).")(?:\\s*\\(((?:[^'\")]*|$Cb)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$ca="\\s*(".($_=="FUNCTION"?"":$cc).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$ve";$ja=$g->result("SHOW CREATE $_ ".idf_escape($f),2);preg_match("~\\(((?:$ca\\s*,?)*)\\)".($_=="FUNCTION"?"\\s*RETURNS\\s+$ve":"")."\\s*(.*)~is",$ja,$k);$p=array();preg_match_all("~$ca\\s*,?~is",$k[1],$sa,PREG_SET_ORDER);foreach($sa
as$eb){$f=str_replace("``","`",$eb[2]).$eb[3];$p[]=array("field"=>$f,"type"=>strtolower($eb[5]),"length"=>preg_replace_callback("~$Cb~s",'normalize_enum',$eb[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$eb[8] $eb[7]"))),"full_type"=>$eb[4],"inout"=>strtoupper($eb[1]),"collation"=>strtolower($eb[9]),);}if($_!="FUNCTION"){return
...
<tr><th>Systém<td>',html_select("driver",$ma,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($bd,$r=""){echo'<p class="tabs">';$ua=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($bd)){$ua["view"]='Zmeniť pohľad';}else{$ua["create"]='Zmeniť tabuľku';}if(isset($r)){$ua["edit"]='Nová položka';}foreach($ua
...
selectQuery($i){global$v;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$v'>".h(str_replace("\n"," ",$i))."</code> <a href='".h(ME)."sql=".urlencode($i)."'>".'Upraviť'."</a></p>\n";}function
rowDescription($h){return"";}function
rowDescriptions($H,$Cf){return$H;}function
selectVal($b,$y,$d){$c=($b!="<i>NULL</i>"&&ereg("char|binary",$d["type"])&&!ereg("var",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$c=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($y?"<a href='$y'>$c</a>":$c);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($J,$B){global$aa,$kb;print_fieldset("select",'Vypísať',$J);$l=0;$Je=array('Funkcie'=>$aa,'Agregácia'=>$kb);foreach($J
as$e=>$b){$b=$_GET["columns"][$e];echo"<div>".html_select("columns[$l][fun]",array(-1=>"")+$Je,$b["fun"]),"(<select name='columns[$l][col]'><option>".optionlist($B,$b["col"],true)."</select>)</div>\n";$l++;}echo"<div>".html_select("columns[$l][fun]",array(-1=>"")+$Je,"","this.nextSibling.nextSibling.onchange();"),"(<select name='columns[$l][col]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>)</div>\n","</div></fieldset>\n";}function
selectSearchPrint($s,$B,$K){print_fieldset("search",'Vyhľadať',$s);foreach($K
as$l=>$x){if($x["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$x["columns"]))."</i>) AGAINST"," <input name='fulltext[$l]' value='".h($_GET["fulltext"][$l])."'>",checkbox("boolean[$l]",1,isset($_GET["boolean"][$l]),"BOOL"),"<br>\n";}}$l=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$l][col]'><option value=''>(".'kdekoľvek'.")".optionlist($B,$b["col"],true)."</select>",html_select("where[$l][op]",$this->operators,$b["op"]),"<input name='where[$l][val]' value='".h($b["val"])."'></div>\n";$l++;}}echo"<div><select name='where[$l][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoľvek'.")".optionlist($B,null,true)."</select>",html_select("where[$l][op]",$this->operators,"="),"<input name='where[$l][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($cb,$B,$K){print_fieldset("sort",'Zotriediť',$cb);$l=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$l]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$l]",1,isset($_GET["desc"][$e]),'zostupne')."</div>\n";$l++;}}echo"<div><select name='order[$l]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>","<label><input type='checkbox' name='desc[$l]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($Nb){if(isset($Nb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($Nb).'">',"</div></fieldset>\n";}}function
...
selectColumnsProcess($B,$K){global$aa,$kb;$J=array();$qa=array();foreach((array)$_GET["columns"]as$e=>$b){if($b["fun"]=="count"||(isset($B[$b["col"]])&&(!$b["fun"]||in_array($b["fun"],$aa)||in_array($b["fun"],$kb)))){$J[$e]=apply_sql_function($b["fun"],(isset($B[$b["col"]])?idf_escape($b["col"]):"*"));if(!in_array($b["fun"],$kb)){$qa[]=$J[$e];}}}return
array($J,$qa);}function
selectSearchProcess($p,$K){global$v;$c=array();foreach($K
as$l=>$x){if($x["type"]=="FULLTEXT"&&$_GET["fulltext"][$l]!=""){$c[]="MATCH (".implode(", ",array_map('idf_escape',$x["columns"])).") AGAINST (".q($_GET["fulltext"][$l]).(isset($_GET["boolean"][$l])?" IN BOOLEAN MODE":"").")";}}foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){$Kb=" $b[op]";if(ereg('IN$',$b["op"])){$Eb=process_length($b["val"]);$Kb.=" (".($Eb!=""?$Eb:"NULL").")";}elseif(!$b["op"]){$Kb.=$b["val"];}elseif($b["op"]=="LIKE %%"){$Kb=" LIKE ".$this->processInput($p[$b["col"]],"%$b[val]%");}elseif(!ereg('NULL$',$b["op"])){$Kb.=" ".$this->processInput($p[$b["col"]],$b["val"]);}if($b["col"]!=""){$c[]=idf_escape($b["col"]).$Kb;}else{$ib=array();foreach($p
as$f=>$d){if(is_numeric($b["val"])||!ereg('int|float|double|decimal',$d["type"])){$f=idf_escape($f);$ib[]=($v=="sql"&&ereg('char|text|enum|set',$d["type"])&&!ereg('^utf8',$d["collation"])?"CONVERT($f USING utf8)":$f);}}$c[]=($ib?"(".implode("$Kb OR ",$ib)."$Kb)":"0");}}}return$c;}function
...
false;}function
messageQuery($i){global$v;static$Rc=0;restart_session();$R="sql-".($Rc++);$Wa=&get_session("queries");if(strlen($i)>1e6){$i=ereg_replace('[\x80-\xFF]+$','',substr($i,0,1e6))."\n...";}$Wa[$_GET["db"]][]=$i;return" <a href='#$R' onclick=\"return !toggle('$R');\">".'SQL príkaz'."</a><div id='$R' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($i,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Wa[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$c);}function
editInput($h,$d,$Ua,$q){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ua value='-1' checked><i>".'originál'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ua value=''".(isset($q)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ua,$d,$q,0);}return"";}function
...
dumpTable($h,$V,$hd=false){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($V){dump_csv(array_keys(fields($h)));}}elseif($V){$ja=create_sql($h,$_POST["auto_increment"]);if($ja){if($V=="DROP+CREATE"){echo"DROP ".($hd?"VIEW":"TABLE")." IF EXISTS ".table($h).";\n";}if($hd){$ja=preg_replace('~^([A-Z =]+) DEFINER=`'.preg_replace('~@(.*)~','`@`(%|\\1)',logged_user()).'`~','\\1',$ja);}echo($V!="CREATE+ALTER"?$ja:($hd?substr_replace($ja," OR REPLACE",6,0):substr_replace($ja," IF NOT EXISTS",12,0))).";\n\n";}if($V=="CREATE+ALTER"&&!$hd){$i="SELECT COLUMN_NAME, COLUMN_DEFAULT, IS_NULLABLE, COLLATION_NAME, COLUMN_TYPE, EXTRA, COLUMN_COMMENT FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ".q($h)." ORDER BY ORDINAL_POSITION";echo"DELIMITER ;;
CREATE PROCEDURE adminer_alter (INOUT alter_command text) BEGIN
	...
	DECLARE add_columns text DEFAULT '";$p=array();$Pb="";foreach(get_rows($i)as$a){$xa=$a["COLUMN_DEFAULT"];$a["default"]=(isset($xa)?q($xa):"NULL");$a["after"]=q($Pb);$a["alter"]=escape_string(idf_escape($a["COLUMN_NAME"])." $a[COLUMN_TYPE]".($a["COLLATION_NAME"]?" COLLATE $a[COLLATION_NAME]":"").(isset($xa)?" DEFAULT ".($xa=="CURRENT_TIMESTAMP"?$xa:$a["default"]):"").($a["IS_NULLABLE"]=="YES"?"":" NOT NULL").($a["EXTRA"]?" $a[EXTRA]":"").($a["COLUMN_COMMENT"]?" COMMENT ".q($a["COLUMN_COMMENT"]):"").($Pb?" AFTER ".idf_escape($Pb):" FIRST"));echo", ADD $a[alter]";$p[]=$a;$Pb=$a["COLUMN_NAME"];}echo"';
...
";}}}function
dumpData($h,$V,$i){global$g,$v;$Ze=($v=="sqlite"?0:1048576);if($V){if($_POST["format"]=="sql"&&$V=="TRUNCATE+INSERT"){echo
truncate_sql($h).";\n";}if($_POST["format"]=="sql"){$p=fields($h);}$j=$g->query($i,1);if($j){$vc="";$vb="";while($a=$j->fetch_assoc()){if($_POST["format"]!="sql"){if($V=="table"){dump_csv(array_keys($a));$V="INSERT";}dump_csv($a);}else{if(!$vc){$vc="INSERT INTO ".table($h)." (".implode(", ",array_map('idf_escape',array_keys($a))).") VALUES";}foreach($a
as$e=>$b){$a[$e]=(isset($b)?(ereg('int|float|double|decimal',$p[$e]["type"])?$b:q($b)):"NULL");}$la=implode(",\t",$a);if($V=="INSERT+UPDATE"){$r=array();foreach($a
as$e=>$b){$r[]=idf_escape($e)." = $b";}echo"$vc ($la) ON DUPLICATE KEY UPDATE ".implode(", ",$r).";\n";}else{$la=($Ze?"\n":" ")."($la)";if(!$vb){$vb=$vc.$la;}elseif(strlen($vb)+4+strlen($la)<$Ze){$vb.=",$la";}else{echo"$vb;\n";$vb=$vc.$la;}}}}if($_POST["format"]=="sql"&&$V!="INSERT+UPDATE"&&$vb){$vb.=";\n";echo$vb;}}elseif($_POST["format"]=="sql"){echo"-- ".str_replace("\n"," ",$g->error)."\n";}}}function
dumpHeaders($Mc,$nd=false){$Ma=$_POST["output"];$Hb=($_POST["format"]=="sql"?"sql":($nd?"tar":"csv"));header("Content-Type: ".($Ma=="bz2"?"application/x-bzip":($Ma=="gz"?"application/x-gzip":($Hb=="tar"?"application/x-tar":($Hb=="sql"||$Ma!="file"?"text/plain":"text/csv")."; charset=utf-8"))));if($Ma=="bz2"){ob_start('bzcompress',1e6);}if($Ma=="gz"){ob_start('gzencode',1e6);}return$Hb;}function
homepage(){echo'<p>'.($_GET["ns"]==""?'<a href="'.h(ME).'database=">'.'Zmeniť databázu'."</a>\n":""),(support("scheme")?"<a href='".h(ME)."scheme='>".($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému')."</a>\n":""),($_GET["ns"]!==""?'<a href="'.h(ME).'schema=">'.'Schéma databázy'."</a>\n":""),(support("privileges")?"<a href='".h(ME)."privileges='>".'Oprávnenia'."</a>\n":"");return
true;}function
navigation($Ub){global$Ac,$g,$O,$v,$ma;echo'<h1>
',$this->name(),' <span class="version">',$Ac,'</span>
<a href="http://www.adminer.org/#download" id="version">',(version_compare($Ac,$_COOKIE["adminer_version"])<0?h($_COOKIE["adminer_version"]):""),'</a>
</h1>
';if($Ub=="auth"){$_a=true;foreach((array)$_SESSION["pwds"]as$Ib=>$Nf){foreach($Nf
as$F=>$Rf){foreach($Rf
as$P=>$T){if(isset($T)){if($_a){echo"<p onclick='eventStop(event);'>\n";$_a=false;}echo"<a href='".h(auth_url($Ib,$F,$P))."'>($ma[$Ib]) ".h($P.($F!=""?"@$F":""))."</a><br>\n";}}}}}else{$A=get_databases();echo'<form action="" method="post">
<p class="logout">
';if(DB==""||!$Ub){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL príkaz'."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}}echo'<input type="submit" name="logout" value="Odhlásiť" onclick="eventStop(event);">
<input type="hidden" name="token" value="',$O,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($A?html_select("db",array(""=>"(".'databáza'.")")+$A,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrať"',($A?" class='hidden'":""),' onclick="eventStop(event);">
';if($Ub!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$Ub){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$E=tables_list();if(!$E){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($E);$ua=array();foreach($E
...
as$h=>$_){echo'<a href="'.h(ME).'select='.urlencode($h).'"'.bold($_GET["select"]==$h).">".'vypísať'."</a> ",'<a href="'.h(ME).'table='.urlencode($h).'"'.bold($_GET["table"]==$h).">".$this->tableName(array("Name"=>$h))."</a><br>\n";}}}$o=(function_exists('adminer_object')?adminer_object():new
Adminer);if(!isset($o->operators)){$o->operators=$gc;}function
page_header($We,$n="",$qc=array(),$Xe=""){global$Bf,$o,$g,$ma;header("Content-Type: text/html; charset=utf-8");if($o->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}$bf=$We.($Xe!=""?": ".h($Xe):"");$cf=strip_tags($bf.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$o->name());if(is_ajax()){header("X-AJAX-Title: ".rawurlencode($cf));}else{echo'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN">
...
<div id="content">
<?php
}if(isset($qc)){$y=substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1);echo'<p id="breadcrumb"><a href="'.h($y?$y:".").'">'.$ma[DRIVER].'</a> &raquo; ';$y=substr(preg_replace('~(db|ns)=[^&]*&~','',ME),0,-1);$F=(SERVER!=""?h(SERVER):'Server');if($qc===false){echo"$F\n";}else{echo"<a href='".($y?h($y):".")."' accesskey='1' title='Alt+Shift+1'>$F</a> &raquo; ";if($_GET["ns"]!=""||(DB!=""&&is_array($qc))){echo'<a href="'.h($y."&db=".urlencode(DB).(support("scheme")?"&ns=":"")).'">'.h(DB).'</a> &raquo; ';}if(is_array($qc)){if($_GET["ns"]!=""){echo'<a href="'.h(substr(ME,0,-1)).'">'.h($_GET["ns"]).'</a> &raquo; ';}foreach($qc
as$e=>$b){$uc=(is_array($b)?$b[1]:$b);if($uc!=""){echo'<a href="'.h(ME."$e=").urlencode(is_array($b)?$b[0]:$b).'">'.h($uc).'</a> &raquo; ';}}}echo"$We\n";}}echo"<span id='loader'></span>\n","<h2>$bf</h2>\n";restart_session();$le=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$gf=$_SESSION["messages"][$le];if($gf){echo"<div class='message'>".implode("</div>\n<div class='message'>",$gf)."</div>\n";unset($_SESSION["messages"][$le]);}$A=&get_session("dbs");if(DB!=""&&$A&&!in_array(DB,$A,true)){$A=null;}if($n){echo"<div class='error'>$n</div>\n";}define("PAGE_HEADER",1);}function
...
long2str($z,$Kd){$la='';foreach($z
as$b){$la.=pack('V',$b);}if($Kd){return
substr($la,0,end($z));}return$la;}function
str2long($la,$Kd){$z=array_values(unpack('V*',str_pad($la,4*ceil(strlen($la)/4),"\0")));if($Kd){$z[]=strlen($la);}return$z;}function
xxtea_mx($Ba,$Aa,$Da,$Ka){return
int32((($Ba>>5&0x7FFFFFF)^$Aa<<2)+(($Aa>>3&0x1FFFFFFF)^$Ba<<4))^int32(($Da^$Aa)+($Ka^$Ba));}function
encrypt_string($rc,$e){if($rc==""){return"";}$e=array_values(unpack("V*",pack("H*",md5($e))));$z=str2long($rc,true);$ga=count($z)-1;$Ba=$z[$ga];$Aa=$z[0];$ba=floor(6+52/($ga+1));$Da=0;while($ba-->0){$Da=int32($Da+0x9E3779B9);$sc=$Da>>2&3;for($va=0;$va<$ga;$va++){$Aa=$z[$va+1];$Rb=xxtea_mx($Ba,$Aa,$Da,$e[$va&3^$sc]);$Ba=int32($z[$va]+$Rb);$z[$va]=$Ba;}$Aa=$z[0];$Rb=xxtea_mx($Ba,$Aa,$Da,$e[$va&3^$sc]);$Ba=int32($z[$ga]+$Rb);$z[$ga]=$Ba;}return
long2str($z,false);}function
decrypt_string($rc,$e){if($rc==""){return"";}$e=array_values(unpack("V*",pack("H*",md5($e))));$z=str2long($rc,false);$ga=count($z)-1;$Ba=$z[$ga];$Aa=$z[0];$ba=floor(6+52/($ga+1));$Da=int32($ba*0x9E3779B9);while($Da){$sc=$Da>>2&3;for($va=$ga;$va>0;$va--){$Ba=$z[$va-1];$Rb=xxtea_mx($Ba,$Aa,$Da,$e[$va&3^$sc]);$Aa=int32($z[$va]-$Rb);$z[$va]=$Aa;}$Ba=$z[$ga];$Rb=xxtea_mx($Ba,$Aa,$Da,$e[$va&3^$sc]);$Aa=int32($z[0]-$Rb);$z[0]=$Aa;$Da=int32($Da-0x9E3779B9);}return
long2str($z,true);}$g='';$O=$_SESSION["token"];if(!$_SESSION["token"]){$_SESSION["token"]=rand(1,1e6);}$yb=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$b){list($e)=explode(":",$b);$yb[$e]=$b;}}if(isset($_POST["server"])){session_regenerate_id();$_SESSION["pwds"][$_POST["driver"]][$_POST["server"]][$_POST["username"]]=$_POST["password"];if($_POST["permanent"]){$e=base64_encode($_POST["driver"])."-".base64_encode($_POST["server"])."-".base64_encode($_POST["username"]);$Lc=$o->permanentLogin();$yb[$e]="$e:".base64_encode($Lc?encrypt_string($_POST["password"],$Lc):"");cookie("adminer_permanent",implode(" ",$yb));}if(count($_POST)==($_POST["permanent"]?5:4)||DRIVER!=$_POST["driver"]||SERVER!=$_POST["server"]||$_GET["username"]!==$_POST["username"]){redirect(auth_url($_POST["driver"],$_POST["server"],$_POST["username"]));}}elseif($_POST["logout"]){if($O&&$_POST["token"]!=$O){page_header('Odhlásiť','Neplatný token CSRF. Odošlite formulár znova.');page_footer("db");exit;}else{foreach(array("pwds","dbs","queries")as$e){set_session($e,null);}$e=base64_encode(DRIVER)."-".base64_encode(SERVER)."-".base64_encode($_GET["username"]);if($yb[$e]){unset($yb[$e]);cookie("adminer_permanent",implode(" ",$yb));}redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),'Odhlásenie prebehlo v poriadku.');}}elseif($yb&&!$_SESSION["pwds"]){session_regenerate_id();$Lc=$o->permanentLogin();foreach($yb
as$e=>$b){list(,$Mf)=explode(":",$b);list($Ib,$F,$P)=array_map('base64_decode',explode("-",$e));$_SESSION["pwds"][$Ib][$F][$P]=decrypt_string(base64_decode($Mf),$Lc);}}function
auth_error($ff=null){global$g,$o,$O;$Kc=session_name();$n="";if(!$_COOKIE[$Kc]&&$_GET[$Kc]&&ini_bool("session.use_only_cookies")){$n='Session premenné musia byť povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Kc]||$_GET[$Kc])&&!$O){$n='Session vypršala, prihláste sa prosím znova.';}else{$T=&get_session("pwds");if(isset($T)){$n=h($ff?$ff->getMessage():(is_string($g)?$g:'Neplatné prihlasovacie údaje.'));$T=null;}}}page_header('Prihlásiť sa',$n,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$o->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$_c)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$o->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$O=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$O;}$n=($_POST?($_POST["token"]==$O?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));function
connect_error(){global$g,$O,$n,$ma;$A=array();if(DB!=""){page_header('Databáza'.": ".h(DB),'Nesprávna databáza.',true);}else{if($_POST["db"]&&!$n){queries_redirect(substr(ME,0,-1),'Databázy boli odstránené.',drop_databases($_POST["db"]));}page_header('Vybrať databázu',$n,false);echo"<p><a href='".h(ME)."database='>".'Vytvoriť novú databázu'."</a>\n";foreach(array('privileges'=>'Oprávnenia','processlist'=>'Zoznam procesov','variables'=>'Premenné','status'=>'Stav',)as$e=>$b){if(support($e)){echo"<a href='".h(ME)."$e='>$b</a>\n";}}echo"<p>".sprintf('Verzia %s: %s cez PHP rozšírenie %s',$ma[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Prihlásený ako: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"]){set_session("dbs",null);}$A=get_databases();if($A){$Jd=support("scheme");$Z=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td>&nbsp;<th>".'Databáza'."<td>".'Porovnávanie'."<td>".'Tabuľky'."</thead>\n";foreach($A
as$t){$kd=h(ME)."db=".urlencode($t);echo"<tr".odd()."><td>".checkbox("db[]",$t,in_array($t,(array)$_POST["db"])),"<th><a href='$kd'>".h($t)."</a>","<td><a href='$kd".($Jd?"&amp;ns=":"")."&amp;database='>".nbsp(db_collation($t,$Z))."</a>","<td align='right'><a href='$kd&amp;schema=' id='tables-".h($t)."'>?</a>","\n";}echo"</table>\n","<p><input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /db/)",1).">\n";echo"<input type='hidden' name='token' value='$O'>\n","<a href='".h(ME)."refresh=1' onclick='eventStop(event);'>".'Obnoviť'."</a>\n","</form>\n";}}page_footer("db");if($A){echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}}if(isset($_GET["status"])){$_GET["variables"]=$_GET["status"];}if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect")){if(DB!=""){set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"])){redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());}if(!set_schema($_GET["ns"])){page_header('Schéma'.": ".h($_GET["ns"]),'Neplatné schéma.',true);page_footer("ns");exit;}}function
select($j,$G=null,$Oc=""){$ua=array();$K=array();$B=array();$df=array();$S=array();odd('');for($l=0;$a=$j->fetch_row();$l++){if(!$l){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($na=0;$na<count($a);$na++){$d=$j->fetch_field();$f=$d->name;$Ia=$d->orgtable;$wc=$d->orgname;if($Oc){$ua[$na]=($f=="table"?"table=":($f=="possible_keys"?"indexes=":null));}elseif($Ia!=""){if(!isset($K[$Ia])){$K[$Ia]=array();foreach(indexes($Ia,$G)as$x){if($x["type"]=="PRIMARY"){$K[$Ia]=array_flip($x["columns"]);break;}}$B[$Ia]=$K[$Ia];}if(isset($B[$Ia][$wc])){unset($B[$Ia][$wc]);$K[$Ia][$wc]=$na;$ua[$na]=$Ia;}}if($d->charsetnr==63){$df[$na]=true;}$S[$na]=$d->type;$f=h($f);echo"<th".($Ia!=""||$d->name!=$wc?" title='".h(($Ia!=""?"$Ia.":"").$wc)."'":"").">".($Oc?"<a href='$Oc".strtolower($f)."' target='_blank' rel='noreferrer'>$f</a>":$f);}echo"</thead>\n";}echo"<tr".odd().">";foreach($a
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($df[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($S[$e]==254){$b="<code>$b</code>";}}if(isset($ua[$e])&&!$B[$ua[$e]]){if($Oc){$y=$ua[$e].urlencode($a[array_search("table=",$ua)]);}else{$y="edit=".urlencode($ua[$e]);foreach($K[$ua[$e]]as$Jc=>$na){$y.="&where".urlencode("[".bracket_escape($Jc)."]")."=".urlencode($a[$na]);}}$b="<a href='".h(ME.$y)."'>$b</a>";}echo"<td>$b";}}echo($l?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
format_time($Jb,$Hc){return" <span class='time'>(".sprintf('%.3f s',max(0,array_sum(explode(" ",$Hc))-array_sum(explode(" ",$Jb)))).")</span>";}function
edit_type($e,$d,$Z,$ha=array()){global$Na,$S,$jb,$Ea;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($S[$d["type"]])?array():array($d["type"]))+$Na+($ha?array('Cudzie kľúče'=>$ha):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($Z,$d["collation"]).'</select>',($jb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($jb,$d["unsigned"]).'</select>':''),($ha?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Ea),$d["on_delete"])."</select> ":" ");}function
process_length($W){global$Cb;return(preg_match("~^\\s*(?:$Cb)(?:\\s*,\\s*(?:$Cb))*\\s*\$~",$W)&&preg_match_all("~$Cb~",$W,$sa)?implode(",",$sa[0]):preg_replace('~[^0-9,+-]~','',$W));}function
process_type($d,$lc="COLLATE"){global$jb;return" $d[type]".($d["length"]!=""?"(".process_length($d["length"]).")":"").(ereg('int|float|double|decimal',$d["type"])&&in_array($d["unsigned"],$jb)?" $d[unsigned]":"").(ereg('char|text|enum|set',$d["type"])&&$d["collation"]?" $lc ".q($d["collation"]):"");}function
process_field($d,$Cc){return
array(idf_escape($d["field"]),process_type($Cc),($d["null"]?" NULL":" NOT NULL"),(isset($d["default"])?" DEFAULT ".(($d["type"]=="timestamp"&&eregi('^CURRENT_TIMESTAMP$',$d["default"]))||($d["type"]=="bit"&&ereg("^([0-9]+|b'[0-1]+')\$",$d["default"]))?$d["default"]:q($d["default"])):""),($d["on_update"]?" ON UPDATE $d[on_update]":""),(support("comment")&&$d["comment"]!=""?" COMMENT ".q($d["comment"]):""),($d["auto_increment"]?auto_increment():null),);}function
type_class($_){foreach(array('char'=>'text','date'=>'time|year','binary'=>'blob','enum'=>'set',)as$e=>$b){if(ereg("$e|$b",$_)){return" class='$e'";}}}function
edit_fields($p,$Z,$_="TABLE",$Ne=0,$ha=array(),$_b=false){global$cc;echo'<thead><tr class="wrap">
';if($_=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($_=="TABLE"?'Názov stĺpca':'Názov parametra'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Dĺžka
<td>Voľby
';if($_=="TABLE"){echo'<td>NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym title="Auto Increment">AI</acronym>
<td',($_POST["defaults"]?"":" class='hidden'"),'>Východzie hodnoty
',(support("comment")?"<td".($_b?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$l=>$d){$l++;$sd=$d[($_POST?"orig":"field")];$Me=(isset($_POST["add"][$l-1])||(isset($d["field"])&&!$_POST["drop_col"][$l]))&&(support("drop_col")||$sd=="");echo'<tr',($Me?"":" style='display: none;'"),'>
',($_=="PROCEDURE"?"<td>".html_select("fields[$l][inout]",explode("|",$cc),$d["inout"]):""),'<th>';if($Me){echo'<input name="fields[',$l,'][field]" value="',h($d["field"]),'" onchange="',($d["field"]!=""||count($p)>1?"":"editingAddRow(this, $Ne); "),'editingNameChange(this);" maxlength="64">';}echo'<input type="hidden" name="fields[',$l,'][orig]" value="',h($sd),'">
...
',(support("comment")?"<td".($_b?"":" class='hidden'")."><input name='fields[$l][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $Ne, 1);'>&nbsp;"."<input type='image' name='up[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.2' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.2' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($sd==""||support("drop_col")?"<input type='image' name='drop_col[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.2' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
process_fields(&$p){ksort($p);$L=0;if($_POST["up"]){$wb=0;foreach($p
as$e=>$d){if(key($_POST["up"])==$e){unset($p[$e]);array_splice($p,$wb,0,array($d));break;}if(isset($d["field"])){$wb=$L;}$L++;}}if($_POST["down"]){$oa=false;foreach($p
...
grant($ia,$ra,$B,$Ab){if(!$ra){return
true;}if($ra==array("ALL PRIVILEGES","GRANT OPTION")){return($ia=="GRANT"?queries("$ia ALL PRIVILEGES$Ab WITH GRANT OPTION"):queries("$ia ALL PRIVILEGES$Ab")&&queries("$ia GRANT OPTION$Ab"));}return
queries("$ia ".preg_replace('~(GRANT OPTION)\\([^)]*\\)~','\\1',implode("$B, ",$ra).$B).$Ab);}function
drop_create($Ha,$ja,$fa,$Le,$Ef,$Df,$f){if($_POST["drop"]){return
query_redirect($Ha,$fa,$Le,true,!$_POST["dropped"]);}$Za=$f!=""&&($_POST["dropped"]||queries($Ha));$Gf=queries($ja);if(!queries_redirect($fa,($f!=""?$Ef:$Df),$Gf)&&$Za){redirect(null,$Le);}return$Za;}function
tar_file($ka,$jd){$c=pack("a100a8a8a8a12a12",$ka,644,0,0,decoct(strlen($jd)),decoct(time()));$Oe=8*32;for($l=0;$l<strlen($c);$l++){$Oe+=ord($c{$l});}$c.=sprintf("%06o",$Oe)."\0 ";return$c.str_repeat("\0",512-strlen($c)).$jd.str_repeat("\0",511-(strlen($jd)+511)%
512);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Ea="RESTRICT|CASCADE|SET NULL|NO ACTION";$Cb="'(?:''|[^'\\\\]|\\\\.)*+'";$cc="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["callf"])){$_GET["call"]=$_GET["callf"];}if(isset($_GET["function"])){$_GET["procedure"]=$_GET["function"];}if(isset($_GET["download"])){$m=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$m-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($m)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["table"])){$m=$_GET["table"];$p=fields($m);if(!$p){$n=error();}$u=($p?table_status($m):array());page_header(($p&&is_view($u)?'Pohľad':'Tabuľka').": ".h($m),$n);$o->selectLinks($u);$za=$u["Comment"];if($za!=""){echo"<p>".'Komentár'.": ".h($za)."\n";}if($p){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Stĺpec'."<td>".'Typ'.(support("comment")?"<td>".'Komentár':"")."</thead>\n";foreach($p
as$d){echo"<tr".odd()."><th>".h($d["field"]),"<td title='".h($d["collation"])."'>".h($d["full_type"]).($d["null"]?" <i>NULL</i>":"").($d["auto_increment"]?" <i>".'Auto Increment'."</i>":""),(isset($d["default"])?" [<b>".h($d["default"])."</b>]":""),(support("comment")?"<td>".nbsp($d["comment"]):""),"\n";}echo"</table>\n";if(!is_view($u)){echo"<h3>".'Indexy'."</h3>\n";$K=indexes($m);if($K){echo"<table cellspacing='0'>\n";foreach($K
as$f=>$x){ksort($x["columns"]);$Db=array();foreach($x["columns"]as$e=>$b){$Db[]="<i>".h($b)."</i>".($x["lengths"][$e]?"(".$x["lengths"][$e].")":"");}echo"<tr title='".h($f)."'><th>$x[type]<td>".implode(", ",$Db)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($m).'">'.'Zmeniť indexy'."</a>\n";if(fk_support($u)){echo"<h3>".'Cudzie kľúče'."</h3>\n";$ha=foreign_keys($m);if($ha){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'Cieľ'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($v!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($ha
as$f=>$C){echo"<tr title='".h($f)."'>","<th><i>".implode("</i>, <i>",array_map('h',$C["source"]))."</i>","<td><a href='".h($C["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($C["db"]),ME):($C["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($C["ns"]),ME):ME))."table=".urlencode($C["table"])."'>".($C["db"]!=""?"<b>".h($C["db"])."</b>.":"").($C["ns"]!=""?"<b>".h($C["ns"])."</b>.":"").h($C["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$C["target"]))."</i>)","<td>".nbsp($C["on_delete"])."\n","<td>".nbsp($C["on_update"])."\n";if($v!="sqlite"){echo'<td><a href="'.h(ME.'foreign='.urlencode($m).'&name='.urlencode($f)).'">'.'Zmeniť'.'</a>';}}echo"</table>\n";}if($v!="sqlite"){echo'<p><a href="'.h(ME).'foreign='.urlencode($m).'">'.'Pridať cudzí kľúč'."</a>\n";}}if(support("trigger")){echo"<h3>".'Triggery'."</h3>\n";$fd=triggers($m);if($fd){echo"<table cellspacing='0'>\n";foreach($fd
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Zmeniť'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$xb=array();$Pe=array();$f="adminer_schema";$Te=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$Te,$sa,PREG_SET_ORDER);foreach($sa
as$l=>$k){$xb[$k[1]]=array($k[2],$k[3]);$Pe[]="\n\t'".js_escape($k[1])."': [ $k[2], $k[3] ]";}$Yb=0;$Se=-1;$Oa=array();$Re=array();$Qe=array();foreach(table_status()as$a){if(!isset($a["Engine"])){continue;}$yc=0;$Oa[$a["Name"]]["fields"]=array();foreach(fields($a["Name"])as$f=>$d){$yc+=1.25;$d["pos"]=$yc;$Oa[$a["Name"]]["fields"][$f]=$d;}$Oa[$a["Name"]]["pos"]=($xb[$a["Name"]]?$xb[$a["Name"]]:array($Yb,0));foreach($o->foreignKeys($a["Name"])as$b){if(!$b["db"]){$ya=$Se;if($xb[$a["Name"]][1]||$xb[$b["table"]][1]){$ya=min(floatval($xb[$a["Name"]][1]),floatval($xb[$b["table"]][1]))-1;}else{$Se-=.1;}while($Qe[(string)$ya]){$ya-=.0001;}$Oa[$a["Name"]]["references"][$b["table"]][(string)$ya]=array($b["source"],$b["target"]);$Re[$b["table"]][$a["Name"]][(string)$ya]=$b["target"];$Qe[(string)$ya]=true;}}$Yb=max($Yb,$Oa[$a["Name"]]["pos"][0]+2.5+$yc);}echo'<div id="schema" style="height: ',$Yb,'em;">
<script type="text/javascript">
tablePos = {',implode(",",$Pe)."\n",'};
...
';foreach($Oa
as$f=>$h){echo"<div class='table' style='top: ".$h["pos"][0]."em; left: ".$h["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($f).'"><b>'.h($f)."</b></a><br>\n";foreach($h["fields"]as$d){$b='<span'.type_class($d["type"]).' title="'.h($d["full_type"].($d["null"]?" NULL":'')).'">'.h($d["field"]).'</span>';echo($d["primary"]?"<i>$b</i>":$b)."<br>\n";}foreach((array)$h["references"]as$bc=>$kc){foreach($kc
as$ya=>$ad){$jc=$ya-$xb[$f][1];$l=0;foreach($ad[0]as$Ca){echo"<div class='references' title='".h($bc)."' id='refs$ya-".($l++)."' style='left: $jc"."em; top: ".$h["fields"][$Ca]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$jc)."em;'></div></div>\n";}}}foreach((array)$Re[$f]as$bc=>$kc){foreach($kc
as$ya=>$B){$jc=$ya-$xb[$f][1];$l=0;foreach($B
as$ea){echo"<div class='references' title='".h($bc)."' id='refd$ya-".($l++)."' style='left: $jc"."em; top: ".$h["fields"][$ea]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=3.3.2'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$jc)."em;'></div></div>\n";}}}echo"</div>\n";}foreach($Oa
as$f=>$h){foreach((array)$h["references"]as$bc=>$kc){foreach($kc
as$ya=>$ad){$Uc=$Yb;$td=-10;foreach($ad[0]as$e=>$Ca){$Qd=$h["pos"][0]+$h["fields"][$Ca]["pos"];$Vd=$Oa[$bc]["pos"][0]+$Oa[$bc]["fields"][$ad[1][$e]]["pos"];$Uc=min($Uc,$Qd,$Vd);$td=max($td,$Qd,$Vd);}echo"<div class='references' id='refl$ya' style='left: $ya"."em; top: $Uc"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($td-$Uc)."em;'></div></div>\n";}}}echo'</div>
<p><a href="',h(ME."schema=".urlencode($Te)),'" id="schema-link">Permanentný odkaz</a>
';}elseif(isset($_GET["dump"])){$m=$_GET["dump"];if($_POST){$Xd="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$e){$Xd.="&$e=".urlencode($_POST[$e]);}cookie("adminer_export",substr($Xd,1));$Hb=dump_headers(($m!=""?$m:DB),(DB==""||count((array)$_POST["tables"]+(array)$_POST["data"])>1));$qb=($_POST["format"]=="sql");if($qb){echo"-- Adminer $Ac ".$ma[DRIVER]." dump
...
as$t){if($g->select_db($t)){if($qb&&ereg('CREATE',$V)&&($ja=$g->result("SHOW CREATE DATABASE ".idf_escape($t),1))){if($V=="DROP+CREATE"){echo"DROP DATABASE IF EXISTS ".idf_escape($t).";\n";}echo($V=="CREATE+ALTER"?preg_replace('~^CREATE DATABASE ~','\\0IF NOT EXISTS ',$ja):$ja).";\n";}if($qb){if($V){echo
use_sql($t).";\n\n";}if(in_array("CREATE+ALTER",array($V,$_POST["table_style"]))){echo"SET @adminer_alter = '';\n\n";}$mb="";if($_POST["routines"]){foreach(array("FUNCTION","PROCEDURE")as$Ra){foreach(get_rows("SHOW $Ra STATUS WHERE Db = ".q($t),null,"-- ")as$a){$mb.=($V!='DROP+CREATE'?"DROP $Ra IF EXISTS ".idf_escape($a["Name"]).";;\n":"").$g->result("SHOW CREATE $Ra ".idf_escape($a["Name"]),2).";;\n\n";}}}if($_POST["events"]){foreach(get_rows("SHOW EVENTS",null,"-- ")as$a){$mb.=($V!='DROP+CREATE'?"DROP EVENT IF EXISTS ".idf_escape($a["Name"]).";;\n":"").$g->result("SHOW CREATE EVENT ".idf_escape($a["Name"]),3).";;\n\n";}}if($mb){echo"DELIMITER ;;\n\n$mb"."DELIMITER ;\n\n";}}if($_POST["table_style"]||$_POST["data_style"]){$X=array();foreach(table_status()as$a){$h=(DB==""||in_array($a["Name"],(array)$_POST["tables"]));$he=(DB==""||in_array($a["Name"],(array)$_POST["data"]));if($h||$he){if(!is_view($a)){if($Hb=="tar"){ob_start();}$o->dumpTable($a["Name"],($h?$_POST["table_style"]:""));if($he){$o->dumpData($a["Name"],$_POST["data_style"],"SELECT * FROM ".table($a["Name"]));}if($qb&&$_POST["triggers"]&&$h&&($fd=trigger_sql($a["Name"],$_POST["table_style"]))){echo"\nDELIMITER ;;\n$fd\nDELIMITER ;\n";}if($Hb=="tar"){echo
tar_file((DB!=""?"":"$t/")."$a[Name].csv",ob_get_clean());}elseif($qb){echo"\n";}}elseif($qb){$X[]=$a["Name"];}}}foreach($X
as$Zb){$o->dumpTable($Zb,$_POST["table_style"],true);}if($Hb=="tar"){echo
pack("x512");}}if($V=="CREATE+ALTER"&&$qb){$i="SELECT TABLE_NAME, ENGINE, TABLE_COLLATION, TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE()";echo"DELIMITER ;;
CREATE PROCEDURE adminer_alter (INOUT alter_command text) BEGIN
...
CALL adminer_alter(@adminer_alter);
DROP PROCEDURE adminer_alter;
";}if(in_array("CREATE+ALTER",array($V,$_POST["table_style"]))&&$qb){echo"SELECT @adminer_alter;\n";}}}if($qb){echo"-- ".$g->result("SELECT NOW()")."\n";}exit;}page_header('Export',"",($_GET["export"]!=""?array("table"=>$_GET["export"]):array()),DB);echo'
<form action="" method="post">
<table cellspacing="0">
';$Yd=array('','USE','DROP+CREATE','CREATE');$fe=array('','DROP+CREATE','CREATE');$ee=array('','TRUNCATE+INSERT','INSERT');if($v=="sql"){$Yd[]='CREATE+ALTER';$fe[]='CREATE+ALTER';$ee[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$a);if(!$a){$a=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}if(!isset($a["events"])){$a["routines"]=$a["events"]=($_GET["dump"]=="");$a["triggers"]=$a["table_style"];}echo"<tr><th>".'Výstup'."<td>".html_select("output",$o->dumpOutput(),$a["output"],0)."\n";echo"<tr><th>".'Formát'."<td>".html_select("format",$o->dumpFormat(),$a["format"],0)."\n";echo($v=="sqlite"?"":"<tr><th>".'Databáza'."<td>".html_select('db_style',$Yd,$a["db_style"]).(support("routine")?checkbox("routines",1,$a["routines"],'Procedúry'):"").(support("event")?checkbox("events",1,$a["events"],'Udalosti'):"")),"<tr><th>".'Tabuľky'."<td>".html_select('table_style',$fe,$a["table_style"]).checkbox("auto_increment",1,$a["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$a["triggers"],'Triggery'):""),"<tr><th>".'Dáta'."<td>".html_select('data_style',$ee,$a["data_style"]),'</table>
<p><input type="submit" value="Export">

<table cellspacing="0">
';$rd=array();if(DB!=""){$db=($m!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$db onclick='formCheck(this, /^tables\\[/);'>".'Tabuľky'."</label>","<th style='text-align: right;'><label>".'Dáta'."<input type='checkbox' id='check-data'$db onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$X="";foreach(table_status()as$a){$f=$a["Name"];$tc=ereg_replace("_.*","",$f);$db=($m==""||$m==(substr($m,-1)=="%"?"$tc%":$f));$Db="<tr><td>".checkbox("tables[]",$f,$db,$f,"formUncheck('check-tables');");if(is_view($a)){$X.="$Db\n";}else{echo"$Db<td align='right'><label>".($a["Engine"]=="InnoDB"&&$a["Rows"]?"~ ":"").$a["Rows"].checkbox("data[]",$f,$db,"","formUncheck('check-data');")."</label>\n";}$rd[$tc]++;}echo$X;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($m==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Databáza'."</label></thead>\n";$A=get_databases();if($A){foreach($A
as$t){if(!information_schema($t)){$tc=ereg_replace("_.*","",$t);echo"<tr><td>".checkbox("databases[]",$t,$m==""||$m=="$tc%",$t,"formUncheck('check-databases');")."</label>\n";$rd[$tc]++;}}}else{echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}}echo'</table>
</form>
';$_a=true;foreach($rd
as$e=>$b){if($e!=""&&$b>1){echo($_a?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$_a=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$j=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q($_GET["db"])." LIKE Db")." ORDER BY Host, User");if(!$j){echo'<form action=""><p>
...
';$j=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");}echo"<table cellspacing='0'>\n","<thead><tr><th>&nbsp;<th>".'Používateľ'."<th>".'Server'."</thead>\n";while($a=$j->fetch_assoc()){echo'<tr'.odd().'><td><a href="'.h(ME.'user='.urlencode($a["User"]).'&host='.urlencode($a["Host"])).'">'.'upraviť'.'</a><td>'.h($a["User"])."<td>".h($a["Host"])."\n";}echo"</table>\n",'<p><a href="'.h(ME).'user=">'.'Vytvoriť používateľa'."</a>";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$o->dumpTable("","");$o->dumpData("","table",$_POST["query"]);exit;}restart_session();$Kf=&get_session("queries");$Wa=&$Kf[DB];if(!$n&&$_POST["clear"]){$Wa=array();redirect(remove_from_uri("history"));}page_header('SQL príkaz',$n);if(!$n&&$_POST){$Ja=false;$i=$_POST["query"];if($_POST["webfile"]){$Ja=@fopen((file_exists("adminer.sql")?"adminer.sql":(file_exists("adminer.sql.gz")?"compress.zlib://adminer.sql.gz":"compress.bzip2://adminer.sql.bz2")),"rb");$i=($Ja?fread($Ja,1e6):false);}elseif($_FILES&&$_FILES["sql_file"]["error"]!=4){$i=get_file("sql_file",true);}if(is_string($i)){if(function_exists('memory_get_usage')){@ini_set("memory_limit",max(ini_get("memory_limit"),2*strlen($i)+memory_get_usage()+8e6));}if($i!=""&&strlen($i)<1e6){$ba=$i.(ereg(';$',$i)?"":";");if(!$Wa||end($Wa)!=$ba){$Wa[]=$ba;}}$nc="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\n)";if(!ini_bool("session.use_cookies")){session_write_close();}$Dd=";";$L=0;$ac=true;$G=connect();if(is_object($G)&&DB!=""){$G->select_db(DB);}$zb=0;$Dc=array();$Jf='[\'"'.($v=="sql"?'`#':($v=="sqlite"?'`[':($v=="mssql"?'[':''))).']|/\\*|-- |$'.($v=="pgsql"?'|\\$[^$]*\\$':'');$If=microtime();parse_str($_COOKIE["adminer_export"],$je);$ke=$o->dumpFormat();unset($ke["sql"]);while($i!=""){if(!$L&&$v=="sql"&&preg_match("~^$nc*DELIMITER\\s+(.+)~i",$i,$k)){$Dd=$k[1];$i=substr($i,strlen($k[0]));}else{preg_match('('.preg_quote($Dd)."|$Jf)",$i,$k,PREG_OFFSET_CAPTURE,$L);$oa=$k[0][0];if(!$oa&&$Ja&&!feof($Ja)){$i.=fread($Ja,1e5);}else{$L=$k[0][1]+strlen($oa);if(!$oa&&rtrim($i)==""){break;}if($oa&&$oa!=$Dd){while(preg_match('('.($oa=='/*'?'\\*/':($oa=='['?']':(ereg('^-- |^#',$oa)?"\n":preg_quote($oa)."|\\\\."))).'|$)s',$i,$k,PREG_OFFSET_CAPTURE,$L)){$la=$k[0][0];$L=$k[0][1]+strlen($la);if(!$la&&$Ja&&!feof($Ja)){$L-=strlen($oa);$i.=fread($Ja,1e5);}elseif($la[0]!="\\"){break;}}}else{$ac=false;$ba=substr($i,0,$k[0][1]);$zb++;$Db="<pre id='sql-$zb'><code class='jush-$v'>".shorten_utf8(trim($ba),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$Db;ob_flush();flush();}$Jb=microtime();if($g->multi_query($ba)&&is_object($G)&&preg_match("~^$nc*USE\\b~isU",$ba)){$G->query($ba);}do{$j=$g->store_result();$Hc=microtime();$Wd=format_time($Jb,$Hc).(strlen($ba)<1000?" <a href='".h(ME)."sql=".urlencode(trim($ba))."'>".'Upraviť'."</a>":"");if($g->error){echo($_POST["only_errors"]?$Db:""),"<p class='error'>".'Chyba v dotaze'.": ".error()."\n";$Dc[]=" <a href='#sql-$zb'>$zb</a>";if($_POST["error_stops"]){break
2;}}elseif(is_object($j)){select($j,$G);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n","<p>".($j->num_rows?lang(array('%d riadok','%d riadky','%d riadkov'),$j->num_rows):"").$Wd;$R="export-$zb";$Od=", <a href='#$R' onclick=\"return !toggle('$R');\">".'Export'."</a><span id='$R' class='hidden'>: ".html_select("output",$o->dumpOutput(),$je["output"])." ".html_select("format",$ke,$je["format"])."<input type='hidden' name='query' value='".h($ba)."'>"." <input type='submit' name='export' value='".'Export'."' onclick='eventStop(event);'><input type='hidden' name='token' value='$O'></span>\n";if($G&&preg_match("~^($nc|\\()*SELECT\\b~isU",$ba)&&($qf=explain($G,$ba))){$R="explain-$zb";echo", <a href='#$R' onclick=\"return !toggle('$R');\">EXPLAIN</a>$Od","<div id='$R' class='hidden'>\n";select($qf,$G,($v=="sql"?"http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/explain-output.html#explain_":""));echo"</div>\n";}else{echo$Od;}echo"</form>\n";}}else{if(preg_match("~^$nc*(CREATE|DROP|ALTER)$nc+(DATABASE|SCHEMA)\\b~isU",$ba)){restart_session();set_session("dbs",null);session_write_close();}if(!$_POST["only_errors"]){echo"<p class='message' title='".h($g->info)."'>".lang(array('Príkaz prebehol v poriadku, bol zmenený %d záznam.','Príkaz prebehol v poriadku boli zmenené %d záznamy.','Príkaz prebehol v poriadku bolo zmenených %d záznamov.'),$g->affected_rows)."$Wd\n";}}$Jb=$Hc;}while($g->next_result());$i=substr($i,$L);$L=0;}}}}if($ac){echo"<p class='message'>".'Žiadne príkazy na vykonanie.'."\n";}elseif($_POST["only_errors"]){echo"<p class='message'>".lang(array('Bol vykonaný %d dotaz.','Boli vykonané %d dotazy.','Bolo vykonaných %d dotazov.'),$zb-count($Dc)).format_time($If,microtime())."\n";}elseif($Dc&&$zb>1){echo"<p class='error'>".'Chyba v dotaze'.": ".implode("",$Dc)."\n";}}else{echo"<p class='error'>".upload_error($i)."\n";}}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
<p>';$ba=$_GET["sql"];if($_POST){$ba=$_POST["query"];}elseif($_GET["history"]=="all"){$ba=$Wa;}elseif($_GET["history"]!=""){$ba=$Wa[$_GET["history"]];}textarea("query",$ba,20);echo($_POST?"":"<script type='text/javascript'>document.getElementsByTagName('textarea')[0].focus();</script>\n"),"<p>".(ini_bool("file_uploads")?'Nahranie súboru'.': <input type="file" name="sql_file"'.($_FILES&&$_FILES["sql_file"]["error"]!=4?'':' onchange="this.form[\'only_errors\'].checked = true;"').'> (&lt; '.ini_get("upload_max_filesize").'B)':'Nahrávánie súborov nie je povolené.'),'<p>
<input type="submit" value="Vykonať" title="Ctrl+Enter">
<input type="hidden" name="token" value="',$O,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$yd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$yd[]=".$e";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($yd?"[".implode("|",$yd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Wa){print_fieldset("history",'História',$_GET["history"]!="");foreach($Wa
as$e=>$b){echo'<a href="'.h(ME."sql=&history=$e").'">'.'Upraviť'."</a> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$pa?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$fa=$_POST["referer"];if($_POST["insert"]){$fa=($pa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$fa)){$fa=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$fa,'Položka bola vymazaná.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($pa?"\n".idf_escape($f)." = $b":$b);}}if($pa){if(!$r){redirect($fa);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$fa,'Položka bola aktualizovaná.');}else{$j=insert_into($m,$r);$Ud=($j?last_id():0);queries_redirect($fa,sprintf('Položka%s bola vložená.',($Ud?" $Ud":"")),$j);}}}$Fa=$o->tableName(table_status($m));page_header(($pa?'Upraviť':'Vložiť'),$n,array("select"=>array($m,$Fa)),$Fa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$J=array();foreach($p
as$f=>$d){if(isset($d["privileges"]["select"])){$J[]=($_POST["clone"]&&$d["auto_increment"]?"'' AS ":(ereg("enum|set",$d["type"])?"1*".idf_escape($f)." AS ":"")).idf_escape($f);}}$a=array();if($J){$H=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($m)," WHERE $s",(isset($_GET["select"])?2:1)));$a=(isset($_GET["select"])&&count($H)!=1?null:reset($H));}}if($a===false){echo"<p class='error'>".'Žiadne riadky.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($p){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($pa?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($pa?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["create"])){$m=$_GET["create"];$Sd=array('HASH','LINEAR HASH','KEY','LINEAR KEY','RANGE','LIST');$Rd=referencable_primary($m);$ha=array();foreach($Rd
as$Fa=>$d){$ha[str_replace("`","``",$Fa)."`".str_replace("`","``",$d["field"])]=$Fa;}$Zc=array();$dd=array();if($m!=""){$Zc=fields($m);$dd=table_status($m);}if($_POST&&!$_POST["fields"]){$_POST["fields"]=array();}if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"]){if($_POST["drop"]){query_redirect("DROP TABLE ".table($m),substr(ME,0,-1),'Tabuľka bola odstránená.');}else{$p=array();$La=array();ksort($_POST["fields"]);$Hd=reset($Zc);$Pb="FIRST";foreach($_POST["fields"]as$e=>$d){$C=$ha[$d["type"]];$Cc=(isset($C)?$Rd[$C]:$d);if($d["field"]!=""){if(!$d["has_default"]){$d["default"]=null;}$xa=eregi_replace(" *on update CURRENT_TIMESTAMP","",$d["default"]);if($xa!=$d["default"]){$d["on_update"]="CURRENT_TIMESTAMP";$d["default"]=$xa;}if($e==$_POST["auto_increment_col"]){$d["auto_increment"]=true;}$ie=process_field($d,$Cc);if($ie!=process_field($Hd,$Hd)){$p[]=array($d["orig"],$ie,$Pb);}if(isset($C)){$La[idf_escape($d["field"])]=($m!=""?"ADD":" ")." FOREIGN KEY (".idf_escape($d["field"]).") REFERENCES ".table($ha[$d["type"]])." (".idf_escape($Cc["field"]).")".(ereg("^($Ea)\$",$d["on_delete"])?" ON DELETE $d[on_delete]":"");}$Pb="AFTER ".idf_escape($d["field"]);}elseif($d["orig"]!=""){$p[]=array($d["orig"]);}if($d["orig"]!=""){$Hd=next($Zc);}}$ob="";if(in_array($_POST["partition_by"],$Sd)){$zd=array();if($_POST["partition_by"]=='RANGE'||$_POST["partition_by"]=='LIST'){foreach(array_filter($_POST["partition_names"])as$e=>$b){$q=$_POST["partition_values"][$e];$zd[]="\nPARTITION ".idf_escape($b)." VALUES ".($_POST["partition_by"]=='RANGE'?"LESS THAN":"IN").($q!=""?" ($q)":" MAXVALUE");}}$ob.="\nPARTITION BY $_POST[partition_by]($_POST[partition])".($zd?" (".implode(",",$zd)."\n)":($_POST["partitions"]?" PARTITIONS ".(+$_POST["partitions"]):""));}elseif($m!=""&&support("partitioning")){$ob.="\nREMOVE PARTITIONING";}$ta='Tabuľka bola zmenená.';if($m==""){cookie("adminer_engine",$_POST["Engine"]);$ta='Tabuľka bola vytvorená.';}queries_redirect(ME."table=".urlencode($_POST["name"]),$ta,alter_table($m,$_POST["name"],$p,$La,$_POST["Comment"],($_POST["Engine"]&&$_POST["Engine"]!=$dd["Engine"]?$_POST["Engine"]:""),($_POST["Collation"]&&$_POST["Collation"]!=$dd["Collation"]?$_POST["Collation"]:""),($_POST["Auto_increment"]!=""?+$_POST["Auto_increment"]:""),$ob));}}page_header(($m!=""?'Zmeniť tabuľku':'Vytvoriť tabuľku'),$n,array("table"=>$m),$m);$a=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($S["int"])?"int":(isset($S["integer"])?"integer":"")))),"partition_names"=>array(""),);if($_POST){$a=$_POST;if($a["auto_increment_col"]){$a["fields"][$a["auto_increment_col"]]["auto_increment"]=true;}process_fields($a["fields"]);}elseif($m!=""){$a=$dd;$a["name"]=$m;$a["fields"]=array();if(!$_GET["auto_increment"]){$a["Auto_increment"]="";}foreach($Zc
as$d){$d["has_default"]=isset($d["default"]);if($d["on_update"]){$d["default"].=" ON UPDATE $d[on_update]";}$a["fields"][]=$d;}if(support("partitioning")){$fc="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($m);$j=$g->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $fc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($a["partition_by"],$a["partitions"],$a["partition"])=$j->fetch_row();$a["partition_names"]=array();$a["partition_values"]=array();foreach(get_rows("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $fc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION")as$Fc){$a["partition_names"][]=$Fc["PARTITION_NAME"];$a["partition_values"][]=$Fc["PARTITION_DESCRIPTION"];}$a["partition_names"][]="";}}$Z=collations();$Ed=floor(extension_loaded("suhosin")?(min(ini_get("suhosin.request.max_vars"),ini_get("suhosin.post.max_vars"))-13)/10:0);if($Ed&&count($a["fields"])>$Ed){echo"<p class='error'>".h(sprintf('Bol prekročený maximálny počet povolených polí. Zvýšte prosím %s a %s.','suhosin.post.max_vars','suhosin.request.max_vars'))."\n";}$Fd=engines();foreach($Fd
as$tb){if(!strcasecmp($tb,$a["Engine"])){$a["Engine"]=$tb;break;}}echo'
<form action="" method="post" id="form">
<p>
Názov tabuľky: <input name="name" maxlength="64" value="',h($a["name"]),'">
';if($m==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Fd?html_select("Engine",array(""=>"(".'úložisko'.")")+$Fd,$a["Engine"]):""),' ',($Z&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$Z,$a["Collation"]):""),' <input type="submit" value="Uložiť">
<table cellspacing="0" id="edit-fields" class="nowrap">
';$_b=($_POST?$_POST["comments"]:$a["Comment"]!="");if(!$_POST&&!$_b){foreach($a["fields"]as$d){if($d["comment"]!=""){$_b=true;break;}}}edit_fields($a["fields"],$Z,"TABLE",$Ed,$ha,$_b);echo'</table>
<p>
Auto Increment: <input name="Auto_increment" size="6" value="',h($a["Auto_increment"]),'">
<label class="jsonly"><input type="checkbox" name="defaults" value="1"',($_POST["defaults"]?" checked":""),' onclick="columnShow(this.checked, 5);">Východzie hodnoty</label>
',(support("comment")?checkbox("comments",1,$_b,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($_b?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
';if(support("partitioning")){$ce=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Sd,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($ce||!$a["partition_by"]?" class='hidden'":""),'>
...
';}echo'</form>
';}elseif(isset($_GET["indexes"])){$m=$_GET["indexes"];$Nc=array("PRIMARY","UNIQUE","INDEX");$u=table_status($m);if(eregi("MyISAM|M?aria",$u["Engine"])){$Nc[]="FULLTEXT";}$K=indexes($m);if($v=="sqlite"){unset($Nc[0]);unset($K[""]);}if($_POST&&!$n&&!$_POST["add"]){$w=array();foreach($_POST["indexes"]as$x){$f=$x["name"];if(in_array($x["type"],$Nc)){$B=array();$Xb=array();$r=array();ksort($x["columns"]);foreach($x["columns"]as$e=>$D){if($D!=""){$W=$x["lengths"][$e];$r[]=idf_escape($D).($W?"(".(+$W).")":"");$B[]=$D;$Xb[]=($W?$W:null);}}if($B){$sb=$K[$f];if($sb){ksort($sb["columns"]);ksort($sb["lengths"]);if($x["type"]==$sb["type"]&&array_values($sb["columns"])===$B&&(!$sb["lengths"]||array_values($sb["lengths"])===$Xb)){unset($K[$f]);continue;}}$w[]=array($x["type"],$f,"(".implode(", ",$r).")");}}}foreach($K
as$f=>$sb){$w[]=array($sb["type"],$f,"DROP");}if(!$w){redirect(ME."table=".urlencode($m));}queries_redirect(ME."table=".urlencode($m),'Indexy boli zmenené.',alter_indexes($m,$w));}page_header('Indexy',$n,array("table"=>$m),$m);$p=array_keys(fields($m));$a=array("indexes"=>$K);if($_POST){$a=$_POST;if($_POST["add"]){foreach($a["indexes"]as$e=>$x){if($x["columns"][count($x["columns"])]!=""){$a["indexes"][$e]["columns"][]="";}}$x=end($a["indexes"]);if($x["type"]||array_filter($x["columns"],'strlen')||array_filter($x["lengths"],'strlen')){$a["indexes"][]=array("columns"=>array(1=>""));}}}else{foreach($a["indexes"]as$e=>$x){$a["indexes"][$e]["name"]=$e;$a["indexes"][$e]["columns"][]="";}$a["indexes"][]=array("columns"=>array(1=>""));}echo'
<form action="" method="post">
<table cellspacing="0" class="nowrap">
<thead><tr><th>Typ indexu<th>Stĺpec (dĺžka)<th>Názov</thead>
';$na=1;foreach($a["indexes"]as$x){echo"<tr><td>".html_select("indexes[$na][type]",array(-1=>"")+$Nc,$x["type"],($na==count($a["indexes"])?"indexesAddRow(this);":1))."<td>";ksort($x["columns"]);$l=1;foreach($x["columns"]as$e=>$D){echo"<span>".html_select("indexes[$na][columns][$l]",array(-1=>"")+$p,$D,($l==count($x["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($v=="sql"?"":$_GET["indexes"]."_")."');"),"<input name='indexes[$na][lengths][$l]' size='2' value='".h($x["lengths"][$e])."'> </span>";$l++;}echo"<td><input name='indexes[$na][name]' value='".h($x["name"])."'>\n";$na++;}echo'</table>
<p>
<input type="submit" value="Uložiť">
<noscript><p><input type="submit" name="add" value="Pridať ďalší"></noscript>
<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["database"])){if($_POST&&!$n&&!isset($_POST["add_x"])){restart_session();if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$_POST["name"]){if(DB!=""){$_GET["db"]=$_POST["name"];queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_POST["name"]),'Databáza bola premenovaná.',rename_database($_POST["name"],$_POST["collation"]));}else{$A=explode("\n",str_replace("\r","",$_POST["name"]));$de=true;$wb="";foreach($A
as$t){if(count($A)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$de=false;}$wb=$t;}}queries_redirect(ME."db=".urlencode($wb),'Databáza bola vytvorená.',$de);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$n,array(),DB);$Z=collations();$f=DB;$lc=null;if($_POST){$f=$_POST["name"];$lc=$_POST["collation"];}elseif(DB!=""){$lc=db_collation(DB,$Z);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$ia){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ia,$k)&&$k[1]){$f=stripcslashes(idf_unescape("`$k[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Z?html_select("collation",array(""=>"(".'porovnávanie'.")")+$Z,$lc):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložiť">
<?php
if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Pridať ďalší'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$n){$y=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"]){query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$y,'Schéma bola odstránená.');}else{$y.=urlencode($_POST["name"]);if($_GET["ns"]==""){query_redirect("CREATE SCHEMA ".idf_escape($_POST["name"]),$y,'Schéma bola vytvorená.');}elseif($_GET["ns"]!=$_POST["name"]){query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($_POST["name"]),$y,'Schéma bola zmenená.');}else{redirect($y);}}}page_header($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému',$n);$a=array("name"=>$_GET["ns"]);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Uložiť">
';if($_GET["ns"]!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["call"])){$Qa=$_GET["call"];page_header('Zavolať'.": ".h($Qa),$n);$Ra=routine($Qa,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Eb=array();$mb=array();foreach($Ra["fields"]as$l=>$d){if(substr($d["inout"],-3)=="OUT"){$mb[$l]="@".idf_escape($d["field"])." AS ".idf_escape($d["field"]);}if(!$d["inout"]||substr($d["inout"],0,2)=="IN"){$Eb[]=$l;}}if(!$n&&$_POST){$Zd=array();foreach($Ra["fields"]as$e=>$d){if(in_array($e,$Eb)){$b=process_input($d);if($b===false){$b="''";}if(isset($mb[$e])){$g->query("SET @".idf_escape($d["field"])." = $b");}}$Zd[]=(isset($mb[$e])?"@".idf_escape($d["field"]):$b);}$i=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($Qa)."(".implode(", ",$Zd).")";echo"<p><code class='jush-$v'>".h($i)."</code> <a href='".h(ME)."sql=".urlencode($i)."'>".'Upraviť'."</a>\n";if(!$g->multi_query($i)){echo"<p class='error'>".error()."\n";}else{$G=connect();if(is_object($G)){$G->select_db(DB);}do{$j=$g->store_result();if(is_object($j)){select($j,$G);}else{echo"<p class='message'>".lang(array('Procedúra bola zavolaná, bol zmenený %d záznam.','Procedúra bola zavolaná, boli zmenené %d záznamy.','Procedúra bola zavolaná, bolo zmenených %d záznamov.'),$g->affected_rows)."\n";}}while($g->next_result());if($mb){select($g->query("SELECT ".implode(", ",$mb)));}}}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["foreign"])){$m=$_GET["foreign"];if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"]){query_redirect("ALTER TABLE ".table($m)."\nDROP ".($v=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($m),'Cudzí kľúč bol odstránený.');}else{$Ca=array_filter($_POST["source"],'strlen');ksort($Ca);$ea=array();foreach($Ca
as$e=>$b){$ea[$e]=$_POST["target"][$e];}query_redirect("ALTER TABLE ".table($m).($_GET["name"]!=""?"\nDROP FOREIGN KEY ".idf_escape($_GET["name"]).",":"")."\nADD FOREIGN KEY (".implode(", ",array_map('idf_escape',$Ca)).") REFERENCES ".table($_POST["table"])." (".implode(", ",array_map('idf_escape',$ea)).")".(ereg("^($Ea)\$",$_POST["on_delete"])?" ON DELETE $_POST[on_delete]":"").(ereg("^($Ea)\$",$_POST["on_update"])?" ON UPDATE $_POST[on_update]":""),ME."table=".urlencode($m),($_GET["name"]!=""?'Cudzí kľúč bol zmenený.':'Cudzí kľúč bol vytvorený.'));$n='Zdrojové a cieľové stĺpce musia mať rovnaký datový typ, nad cieľovými stĺpcami musí byť definovaný index a odkazované dáta musia existovať.'."<br>$n";}}page_header('Cudzí kľúč',$n,array("table"=>$m),$m);$a=array("table"=>$m,"source"=>array(""));if($_POST){$a=$_POST;ksort($a["source"]);if($_POST["add"]){$a["source"][]="";}elseif($_POST["change"]||$_POST["change-js"]){$a["target"]=array();}}elseif($_GET["name"]!=""){$ha=foreign_keys($m);$a=$ha[$_GET["name"]];$a["source"][]="";}$Ca=array_keys(fields($m));$ea=($m===$a["table"]?$Ca:array_keys(fields($a["table"])));$ae=array();foreach(table_status()as$f=>$u){if(fk_support($u)){$ae[]=$f;}}echo'
<form action="" method="post">
<p>
';if($a["db"]==""&&$a["ns"]==""){echo'Cieľová tabuľka:
',html_select("table",$ae,$a["table"],"this.form['change-js'].value = '1'; if (!ajaxForm(this.form)) this.form.submit();"),'<input type="hidden" name="change-js" value="">
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
';if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["event"])){$lb=$_GET["event"];$Pd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$od=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($lb),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Pd)&&isset($od[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($lb!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($lb!=""?"ALTER EVENT ".idf_escape($lb).$Td.($lb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$od[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($lb!=""?'Upraviť udalosť'.": ".h($lb):'Vytvoriť udalosť'),$n);$a=array();if($_POST){$a=$_POST;}elseif($lb!=""){$H=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($lb));$a=reset($H);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Pd,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$od,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($lb!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["procedure"])){$Qa=$_GET["procedure"];$Ra=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$xd=routine_languages();$Za=false;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"]){$r=array();$p=(array)$_POST["fields"];ksort($p);foreach($p
as$d){if($d["field"]!=""){$r[]=(ereg("^($cc)\$",$d["inout"])?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Za=drop_create("DROP $Ra ".idf_escape($Qa),"CREATE $Ra ".idf_escape($_POST["name"])." (".implode(", ",$r).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$xd)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$Qa);}page_header(($Qa!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($Qa):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$n);$Z=get_vals("SHOW CHARACTER SET");sort($Z);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Qa!=""){$a=routine($Qa,$Ra);$a["name"]=$Qa;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
',($xd?'Jazyk'.": ".html_select("language",$xd,$a["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$Z,$Ra);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$a["returns"],$Z);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="submit" value="Uložiť">
';if($Qa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["sequence"])){$Bb=$_GET["sequence"];if($_POST&&!$n){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($Bb),$y,'Sekvencia bola odstránená.');}elseif($Bb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$y,'Sekvencia bola vytvorená.');}elseif($Bb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($Bb)." RENAME TO ".idf_escape($_POST["name"]),$y,'Sekvencia bola zmenená.');}else{redirect($y);}}page_header($Bb!=""?'Pozmeniť sekvenciu'.": ".h($Bb):'Vytvoriť sekvenciu',$n);$a=array("name"=>$Bb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Uložiť">
';if($Bb!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["type"])){$Ec=$_GET["type"];if($_POST&&!$n){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Ec),$y,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ bol vytvorený.');}}page_header($Ec!=""?'Pozmeniť typ'.": ".h($Ec):'Vytvoriť typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($Ec!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["trigger"])){$m=$_GET["trigger"];$Ic=trigger_options();$be=array("INSERT","UPDATE","DELETE");$Za=false;if($_POST&&!$n&&in_array($_POST["Timing"],$Ic["Timing"])&&in_array($_POST["Event"],$be)&&in_array($_POST["Type"],$Ic["Type"])){$ge=" $_POST[Timing] $_POST[Event]";$Ab=" ON ".table($m);$Za=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($v=="pgsql"?$Ab:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($v=="mssql"?$Ab.$ge:$ge.$Ab).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($m),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$n,array("table"=>$m));$a=array("Trigger"=>$m."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Ic["Timing"],$a["Timing"],"if (/^".preg_quote($m,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($m)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$be,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Ic["Type"],$a["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["user"])){$pd=$_GET["user"];$ra=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$a){foreach(explode(",",($a["Privilege"]=="Grant option"?"":$a["Context"]))as$Pc){$ra[$Pc][$a["Privilege"]]=$a["Comment"];}}$ra["Server Admin"]+=$ra["File access on server"];$ra["Databases"]["Create routine"]=$ra["Procedures"]["Create routine"];unset($ra["Procedures"]["Create routine"]);$ra["Columns"]=array();foreach(array("Select","Insert","Update","References")as$b){$ra["Columns"][$b]=$ra["Tables"][$b];}unset($ra["Server Admin"]["Usage"]);foreach($ra["Tables"]as$e=>$b){unset($ra["Databases"][$e]);}$hc=array();if($_POST){foreach($_POST["objects"]as$e=>$b){$hc[$b]=(array)$hc[$b]+(array)$_POST["grants"][$e];}}$hb=array();$Sc="";if(isset($_GET["host"])&&($j=$g->query("SHOW GRANTS FOR ".q($pd)."@".q($_GET["host"])))){while($a=$j->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$a[0],$k)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$k[1],$sa,PREG_SET_ORDER)){foreach($sa
as$b){if($b[1]!="USAGE"){$hb["$k[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$hb["$k[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$k)){$Sc=$k[1];}}}if($_POST&&!$n){$ec=(isset($_GET["host"])?q($pd)."@".q($_GET["host"]):"''");$nb=q($_POST["user"])."@".q($_POST["host"]);$Nd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $ec",ME."privileges=",'Používateľ bol odstránený.');}else{if($ec!=$nb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $nb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Nd");}elseif($_POST["pass"]!=$Sc||!$_POST["hashed"]){queries("SET PASSWORD FOR $nb = ".($_POST["hashed"]?$Nd:"PASSWORD($Nd)"));}if(!$n){$oc=array();foreach($hc
as$Xa=>$ia){if(isset($_GET["grant"])){$ia=array_filter($ia);}$ia=array_keys($ia);if(isset($_GET["grant"])){$oc=array_diff(array_keys(array_filter($hc[$Xa],'strlen')),$ia);}elseif($ec==$nb){$Ye=array_keys((array)$hb[$Xa]);$oc=array_diff($Ye,$ia);$ia=array_diff($ia,$Ye);unset($hb[$Xa]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$Xa,$k)&&(!grant("REVOKE",$oc,$k[2]," ON $k[1] FROM $nb")||!grant("GRANT",$ia,$k[2]," ON $k[1] TO $nb"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($ec!=$nb){queries("DROP USER $ec");}elseif(!isset($_GET["grant"])){foreach($hb
as$Xa=>$oc){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Xa,$k)){grant("REVOKE",array_keys($oc),$k[2]," ON $k[1] FROM $nb");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$n);if($ec!=$nb){$g->query("DROP USER $nb");}}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$pd@$_GET[host]"):'Vytvoriť používateľa'),$n,array("privileges"=>array('','Oprávnenia')));if($_POST){$a=$_POST;$hb=$hc;}else{$a=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$a["pass"]=$Sc;if($Sc!=""){$a["hashed"]=true;}$hb[DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$l=0;foreach($hb
as$Xa=>$ia){echo'<th>'.($Xa!="*.*"?"<input name='objects[$l]' value='".h($Xa)."' size='10'>":"<input type='hidden' name='objects[$l]' value='*.*' size='10'>*.*");$l++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Pc=>$uc){foreach((array)$ra[$Pc]as$pc=>$za){echo"<tr".odd()."><td".($uc?">$uc<td":" colspan='2'").' lang="en" title="'.h($za).'">'.h($pc);$l=0;foreach($hb
as$Xa=>$ia){$f="'grants[$l][".h(strtoupper($pc))."]'";$q=$ia[strtoupper($pc)];if($Pc=="Server Admin"&&$Xa!=(isset($hb["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($q?" selected":"").">".'Povoliť'."<option value='0'".($q=="0"?" selected":"").">".'Zakázať'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($q?" checked":"").($pc=="All privileges"?" id='grants-$l-all'":($pc=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$l-all');\"")).">";}$l++;}}}echo"</table>\n",'<p>
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Cd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$Cd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$Cd),$Cd||!$_POST["kill"]);}page_header('Zoznam procesov',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".(($v=="sql"?$e=="Info"&&$b!="":$e=="current_query"&&$b!="<IDLE>")?"<code class='jush-$v'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Upraviť'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
';if(support("kill")){echo($l+1)."/".sprintf('%d celkom',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Ukončiť'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["select"])){$m=$_GET["select"];$u=table_status($m);$K=indexes($m);$p=fields($m);$ha=column_foreign_keys($m);if($u["Oid"]=="t"){$K[]=array("type"=>"PRIMARY","columns"=>array("oid"));}parse_str($_COOKIE["adminer_import"],$ed);$ef=array();$B=array();$Nb=null;foreach($p
as$e=>$d){$f=$o->fieldName($d);if(isset($d["privileges"]["select"])&&$f!=""){$B[$e]=html_entity_decode(strip_tags($f));if(ereg('text|lob',$d["type"])){$Nb=$o->selectLengthProcess();}}$ef+=$d["privileges"];}list($J,$qa)=$o->selectColumnsProcess($B,$K);$s=$o->selectSearchProcess($p,$K);$cb=$o->selectOrderProcess($p,$K);$M=$o->selectLimitProcess();$fc=($J?implode(", ",$J):($u["Oid"]=="t"?"oid, ":"")."*")."\nFROM ".table($m);$Bd=($qa&&count($qa)<count($J)?"\nGROUP BY ".implode(", ",$qa):"").($cb?"\nORDER BY ".implode(", ",$cb):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Va=>$a){echo$g->result("SELECT".limit(idf_escape(key($a))." FROM ".table($m)," WHERE ".where_check($Va).($s?" AND ".implode(" AND ",$s):"").($cb?" ORDER BY ".implode(", ",$cb):""),1));}exit;}if($_POST&&!$n){$ue="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$Pa=$ic=null;foreach($K
as$x){if($x["type"]=="PRIMARY"){$Pa=array_flip($x["columns"]);$ic=($J?$Pa:array());break;}}foreach((array)$ic
as$e=>$b){if(in_array(idf_escape($e),$J)){unset($ic[$e]);}}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($m);$o->dumpTable($m,"");if(!is_array($_POST["check"])||$ic===array()){$Tb=$s;if(is_array($_POST["check"])){$Tb[]="($ue)";}$i="SELECT $fc".($Tb?"\nWHERE ".implode(" AND ",$Tb):"").$Bd;}else{$se=array();foreach($_POST["check"]as$b){$se[]="(SELECT".limit($fc,"\nWHERE ".($s?implode(" AND ",$s)." AND ":"").where_check($b).$Bd,1).")";}$i=implode(" UNION ALL ",$se);}$o->dumpData($m,"table",$i);exit;}if(!$o->selectEmailProcess($s,$ha)){if($_POST["save"]||$_POST["delete"]){$j=true;$rb=0;$i=table($m);$r=array();if(!$_POST["delete"]){foreach($B
as$f=>$b){$b=process_input($p[$f]);if($b!==null){if($_POST["clone"]){$r[idf_escape($f)]=($b!==false?$b:idf_escape($f));}elseif($b!==false){$r[]=idf_escape($f)." = $b";}}}$i.=($_POST["clone"]?" (".implode(", ",array_keys($r)).")\nSELECT ".implode(", ",$r)."\nFROM ".table($m):" SET\n".implode(",\n",$r));}if($_POST["delete"]||$r){$Wc="UPDATE";if($_POST["delete"]){$Wc="DELETE";$i="FROM $i";}if($_POST["clone"]){$Wc="INSERT";$i="INTO $i";}if($_POST["all"]||($ic===array()&&$_POST["check"])||count($qa)<count($J)){$j=queries($Wc." $i".($_POST["all"]?($s?"\nWHERE ".implode(" AND ",$s):""):"\nWHERE $ue"));$rb=$g->affected_rows;}else{foreach((array)$_POST["check"]as$b){$j=queries($Wc.limit1($i,"\nWHERE ".where_check($b)));if(!$j){break;}$rb+=$g->affected_rows;}}}queries_redirect(remove_from_uri("page"),sprintf('%d položiek bolo ovplyvnených.',$rb),$j);}elseif(!$_POST["import"]){if(!$_POST["val"]){$n='Dvojkliknite na políčko, ktoré chcete zmeniť.';}else{$j=true;$rb=0;foreach($_POST["val"]as$Va=>$a){$r=array();foreach($a
as$e=>$b){$e=bracket_escape($e,1);$r[]=idf_escape($e)." = ".(ereg('char|text',$p[$e]["type"])||$b!=""?$o->processInput($p[$e],$b):"NULL");}$i=table($m)." SET ".implode(", ",$r);$Tb=" WHERE ".where_check($Va).($s?" AND ".implode(" AND ",$s):"");$j=queries("UPDATE".(count($qa)<count($J)?" $i$Tb":limit1($i,$Tb)));if(!$j){break;}$rb+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$rb),$j);}}elseif(is_string($Ga=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ed["output"])."&format=".urlencode($_POST["separator"]));$j=true;$ib=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Ga,$sa);$rb=count($sa[0]);begin();$bb=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($sa[0]as$e=>$b){preg_match_all("~((\"[^\"]*\")+|[^$bb]*)$bb~",$b.$bb,$Gd);if(!$e&&!array_diff($Gd[1],$ib)){$ib=$Gd[1];$rb--;}else{$r=array();foreach($Gd[1]as$l=>$Jc){$r[idf_escape($ib[$l])]=($Jc==""&&$p[$ib[$l]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Jc))));}$j=insert_update($m,$r,$Pa);if(!$j){break;}}}if($j){queries("COMMIT");}queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$rb),$j);queries("ROLLBACK");}else{$n=upload_error($Ga);}}}$Fa=$o->tableName($u);page_header('Vypísať'.": $Fa",$n);session_write_close();$r=null;if(isset($ef["insert"])){$r="";foreach((array)$_GET["where"]as$b){if(count($ha[$b["col"]])==1&&($b["op"]=="="||(!$b["op"]&&!ereg('[_%]',$b["val"])))){$r.="&set".urlencode("[".bracket_escape($b["col"])."]")."=".urlencode($b["val"]);}}}$o->selectLinks($u,$r);if(!$B){echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($p?".":": ".error())."\n";}else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($m).'">',"</div>\n";$o->selectColumnsPrint($J,$B);$o->selectSearchPrint($s,$B,$K);$o->selectOrderPrint($cb,$B,$K);$o->selectLimitPrint($M);$o->selectLengthPrint($Nb);$o->selectActionPrint();echo"</form>\n";$Y=$_GET["page"];if($Y=="last"){$fb=$g->result("SELECT COUNT(*) FROM ".table($m).($s?" WHERE ".implode(" AND ",$s):""));$Y=floor(max(0,$fb-1)/$M);}$i="SELECT".limit((+$M&&$qa&&count($qa)<count($J)&&$v=="sql"?"SQL_CALC_FOUND_ROWS ":"").$fc,($s?"\nWHERE ".implode(" AND ",$s):"").$Bd,($M!=""?+$M:null),($Y?$M*$Y:0),"\n");echo$o->selectQuery($i);$j=$g->query($i);if(!$j){echo"<p class='error'>".error()."\n";}else{if($v=="mssql"){$j->seek($M*$Y);}$Tc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$H=array();while($a=$j->fetch_assoc()){if($Y&&$v=="oracle"){unset($a["RNUM"]);}$H[]=$a;}if($_GET["page"]!="last"){$fb=(+$M&&$qa&&count($qa)<count($J)?($v=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($i) x")):count($H));}if(!$H){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{$Id=$o->backwardKeys($m,$Fa);echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$qa&&$J?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$_d=array();$aa=array();reset($J);$oe=1;foreach($H[0]as$e=>$b){if($u["Oid"]!="t"||$e!="oid"){$b=$_GET["columns"][key($J)];$d=$p[$J?($b?$b["col"]:current($J)):$e];$f=($d?$o->fieldName($d,$oe):"*");if($f!=""){$oe++;$_d[$e]=$f;$D=idf_escape($e);echo'<th><a href="'.h(remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($e).($cb[0]==$D||$cb[0]==$e||(!$cb&&count($qa)<count($J)&&$qa[0]==$D)?'&desc%5B0%5D=1':'')).'">'.(!$J||$b?apply_sql_function($b["fun"],$f):h(current($J)))."</a>";}$aa[$e]=$b["fun"];next($J);}}$Xb=array();if($_GET["modify"]){foreach($H
as$a){foreach($a
as$e=>$b){$Xb[$e]=max($Xb[$e],min(40,strlen(utf8_decode($b))));}}}echo($Id?"<th>".'Vzťahy':"")."</thead>\n";foreach($o->rowDescriptions($H,$ha)as$ga=>$a){$vd=unique_array($H[$ga],$K);$Va="";foreach($vd
...
as$e=>$b){if(isset($_d[$e])){$d=$p[$e];if($b!=""&&(!isset($Tc[$e])||$Tc[$e]!="")){$Tc[$e]=(is_mail($b)?$_d[$e]:"");}$y="";$b=$o->editVal($b,$d);if(!isset($b)){$b="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$d["type"])&&$b!=""){$y=h(ME.'download='.urlencode($m).'&field='.urlencode($e).$Va);}if($b===""){$b="&nbsp;";}elseif($Nb!=""&&ereg('text|blob',$d["type"])&&is_utf8($b)){$b=shorten_utf8($b,max(0,+$Nb));}else{$b=h($b);}if(!$y){foreach((array)$ha[$e]as$C){if(count($ha[$e])==1||end($C["source"])==$e){$y="";foreach($C["source"]as$l=>$Ca){$y.=where_link($l,$C["target"][$l],$H[$ga][$Ca]);}$y=h(($C["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($C["db"]),ME):ME).'select='.urlencode($C["table"]).$y);if(count($C["source"])==1){break;}}}}if($e=="COUNT(*)"){$y=h(ME."select=".urlencode($m));$l=0;foreach((array)$_GET["where"]as$z){if(!array_key_exists($z["col"],$vd)){$y.=h(where_link($l++,$z["col"],$z["val"],$z["op"]));}}foreach($vd
as$Ka=>$z){$y.=h(where_link($l++,$Ka,$z));}}}if(!$y){if(is_mail($b)){$y="mailto:$b";}if($pe=is_url($a[$e])){$y=($pe=="http"&&$xc?$a[$e]:"$pe://www.adminer.org/redirect/?url=".urlencode($a[$e]));}}$R=h("val[$Va][".bracket_escape($e)."]");$q=$_POST["val"][$Va][bracket_escape($e)];$qe=h(isset($q)?$q:$a[$e]);$hf=strpos($b,"<i>...</i>");$Ee=is_utf8($b)&&$H[$ga][$e]==$a[$e]&&!$aa[$e];$De=ereg('text|lob',$d["type"]);echo(($_GET["modify"]&&$Ee)||isset($q)?"<td>".($De?"<textarea name='$R' cols='30' rows='".(substr_count($a[$e],"\n")+1)."'>$qe</textarea>":"<input name='$R' value='$qe' size='$Xb[$e]'>"):"<td id='$R' ondblclick=\"".($Ee?"selectDblClick(this, event".($hf?", 2":($De?", 1":"")).")":"alert('".h('Pre zmenu tejto hodnoty použite odkaz upraviť.')."')").";\">".$o->selectVal($b,$y,$d));}}if($Id){echo"<td>";}$o->backwardKeysPrint($Id,$H[$ga]);echo"</tr>\n";}echo"</table>\n";}if($H||$Y){$qd=true;if($_GET["page"]!="last"&&+$M&&count($qa)>=count($J)&&($fb>=$M||$Y)){$fb=found_rows($u,$s);if($fb<max(1e4,2*($Y+1)*$M)){ob_flush();flush();$fb=$g->result("SELECT COUNT(*) FROM ".table($m).($s?" WHERE ".implode(" AND ",$s):""));}else{$qd=false;}}echo"<p class='pages'>";if(+$M&&$fb>$M){$md=floor(($fb-1)/$M);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($Y+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$Y).($Y>5?" ...":"");for($l=max(1,$Y-4);$l<min($md,$Y+5);$l++){echo
pagination($l,$Y);}echo($Y+5<$md?" ...":"").($qd?pagination($md,$Y):' <a href="'.h(remove_from_uri()."&page=last").'">'.'posledný'."</a>");}echo" (".($qd?"":"~ ").lang(array('%d riadok','%d riadky','%d riadkov'),$fb).") ".checkbox("all",1,0,'celý výsledok')."\n";if($o->selectCommandPrint()){?>
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$fb,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Export');$Ma=$o->dumpOutput();echo($Ma?html_select("output",$Ma,$ed["output"])." ":""),html_select("format",$o->dumpFormat(),$ed["format"])," <input type='submit' name='export' value='".'Export'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($o->selectImportPrint()){print_fieldset("import",'Import',!$H);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ed["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$O'>\n","</div></fieldset>\n";}$o->selectEmailPrint(array_filter($Tc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Sb=isset($_GET["status"]);page_header($Sb?'Stav':'Premenné');$ze=($Sb?show_status():show_variables());if(!$ze){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($ze
as$e=>$b){echo"<tr>","<th><code class='jush-".$v.($Sb?"status":"set")."'>".h($e)."</code>","<td>".nbsp($b);}echo"</table>\n";}}elseif(isset($_GET["script"])){header("Content-Type: text/javascript; charset=utf-8");if($_GET["script"]=="db"){$gd=array("Data_length"=>0,"Index_length"=>0,"Data_free"=>0);foreach(table_status()as$u){$R=js_escape($u["Name"]);json_row("Comment-$R",nbsp($u["Comment"]));if(!is_view($u)){foreach(array("Engine","Collation")as$e){json_row("$e-$R",nbsp($u[$e]));}foreach($gd+array("Auto_increment"=>0,"Rows"=>0)as$e=>$b){if($u[$e]!=""){$b=number_format($u[$e],0,'.',' ');json_row("$e-$R",($e=="Rows"&&$u["Engine"]=="InnoDB"&&$b?"~ $b":$b));if(isset($gd[$e])){$gd[$e]+=($u["Engine"]!="InnoDB"||$e!="Data_free"?$u[$e]:0);}}elseif(array_key_exists($e,$u)){json_row("$e-$R");}}}}foreach($gd
as$e=>$b){json_row("sum-$e",number_format($b,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$t=>$b){json_row("tables-".js_escape($t),$b);}json_row("");}exit;}else{$_e=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($_e&&!$n&&!$_POST["search"]){$j=true;$ta="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$j=truncate_tables($_POST["tables"]);}$ta='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$j=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ta='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$j=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ta='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"]){$j=drop_views($_POST["views"]);}if($j&&$_POST["tables"]){$j=drop_tables($_POST["tables"]);}$ta='Tabuľka bola odstránená.';}elseif($_POST["tables"]&&($j=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$j->fetch_assoc()){$ta.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ta,$j);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$n,true);if($o->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$Xc=tables_list();if(!$Xc){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabuľka'.'<td>'.'Typ'.'<td>'.'Porovnávanie'.'<td>'.'Veľkosť dát'.'<td>'.'Veľkosť indexu'.'<td>'.'Voľné miesto'.'<td>'.'Auto Increment'.'<td>'.'Riadky'.(support("comment")?'<td>'.'Komentár':'')."</thead>\n";foreach($Xc
as$f=>$_){$Zb=(isset($_)&&!eregi("table",$_));echo'<tr'.odd().'><td>'.checkbox(($Zb?"views[]":"tables[]"),$f,in_array($f,$_e,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($Zb){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{foreach(array("Engine"=>"","Collation"=>"","Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$e=>$y){echo($y?"<td align='right'><a href='".h(ME."$y=").urlencode($f)."' id='$e-".h($f)."'>?</a>":"<td id='$e-".h($f)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($Xc)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$e){echo"<td align='right' id='sum-$e'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p>".($v=="sql"?"<input type='submit' value='".'Analyzovať'."'> <input type='submit' name='optimize' value='".'Optimalizovať'."'> <input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$A=(support("scheme")?schemas():get_databases());if(count($A)!=1&&$v!="sqlite"){$t=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($A?html_select("target",$A,$t):'<input name="target" value="'.h($t).'">')," <input type='submit' name='move' value='".'Presunúť'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$O'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";}if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$Ae=routines();if($Ae){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($Ae
as$a){echo'<tr'.odd().'>','<th><a href="'.h(ME).($a["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($a["ROUTINE_NAME"]).'">'.h($a["ROUTINE_NAME"]).'</a>','<td>'.h($a["ROUTINE_TYPE"]),'<td>'.h($a["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($a["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($a["ROUTINE_NAME"]).'">'.'Zmeniť'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Vytvoriť procedúru'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Vytvoriť funkciu'."</a>\n";}if(support("sequence")){echo"<h3>".'Sekvencia'."</h3>\n";$Be=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema()");if($Be){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."</thead>\n";odd('');foreach($Be
