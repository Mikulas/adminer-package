...
* @version 3.3.0
*/error_reporting(6135);$Le=(!ereg('^(unsafe_raw)?$',ini_get("filter.default"))||ini_get("filter.default_flags"));if($Le){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$b){$Nd=filter_input_array(constant("INPUT$b"),FILTER_UNSAFE_RAW);if($Nd){$$b=$Nd;}}}if(isset($_GET["file"])){header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
nl_br($K){return
str_replace("\n","<br>",$K);}function
checkbox($f,$q,$fb,$Te="",$Cc="",$Of=false){static$S=0;$S++;$c="<input type='checkbox' name='$f' value='".h($q)."'".($fb?" checked":"").($Cc?" onclick=\"$Cc\"":"").($Of?" class='jsonly'":"")." id='checkbox-$S'>";return($Te!=""?"<label for='checkbox-$S'>$c".h($Te)."</label>":$c);}function
optionlist($Bc,$Lf=null,$te=false){$c="";foreach($Bc
...
html_select($f,$Bc,$q="",$Hb=true){if($Hb){return"<select name='".h($f)."'".(is_string($Hb)?" onchange=\"$Hb\"":"").">".optionlist($Bc,$q)."</select>";}$c="";foreach($Bc
as$e=>$b){$c.="<label><input type='radio' name='".h($f)."' value='".h($e)."'".($e==$q?" checked":"").">".h($b)."</label>";}return$c;}function
confirm($Oc="",$Sf=false){return" onclick=\"".($Sf?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Oc?" (' + $Oc + ')":"")."');\"";}function
js_escape($K){return
addcslashes($K,"\r\n'\\/");}function
...
cookie($f,$q){global$uc;$ic=array($f,(ereg("\n",$q)?"":$q),time()+2592000,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$uc);if(version_compare(PHP_VERSION,'5.2.0')>=0){$ic[]=true;}return
...
query_redirect($i,$ga,$ua,$Vc=true,$Cf=true,$Ee=false){global$g,$n,$o;if($Cf){$Ee=!$g->query($i);}$Cd="";if($i){$Cd=$o->messageQuery("$i;");}if($Ee){$n=error().$Cd;return
false;}if($Vc){redirect($ga,$ua.$Cd);}return
true;}function
...
false;}}return
true;}function
queries_redirect($ga,$ua,$Vc){return
query_redirect(queries(),$ga,$ua,$Vc,false,!$Vc);}function
...
get_file($e,$Id=false){$Da=$_FILES[$e];if(!$Da||$Da["error"]){return$Da["error"];}$c=file_get_contents($Id&&ereg('\\.gz$',$Da["name"])?"compress.zlib://$Da[tmp_name]":($Id&&ereg('\\.bz2$',$Da["name"])?"compress.bzip2://$Da[tmp_name]":$Da["tmp_name"]));if($Id){$Eb=substr($c,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$Eb,$Yf)){$c=iconv("utf-16","utf-8",$c);}elseif($Eb=="\xEF\xBB\xBF"){$c=substr($c,3);}}return$c;}function
upload_error($n){$Je=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Súbor sa nepodarilo nahrať.'.($Je?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Je):""):'Súbor neexistuje.');}function
...
enum_input($z,$Ta,$d,$q,$cc=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);$c=(isset($cc)?"<label><input type='$z'$Ta value='$cc'".((is_array($q)?in_array($cc,$q):$q===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($sa[1]as$k=>$b){$b=stripcslashes(str_replace("''","'",$b));$fb=(is_int($q)?$q==$k+1:(is_array($q)?in_array($k+1,$q):$q===$b));$c.=" <label><input type='$z'$Ta value='".($k+1)."'".($fb?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$L){global$Q,$o,$u;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Fe=($u=="mssql"&&$d["auto_increment"]);if($Fe&&!$_POST["save"]){$L=null;}$aa=(isset($_GET["select"])||$Fe?array("orig"=>'originál'):array())+$o->editFunctions($d);$Ta=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
dump_headers($Fc,$ud=false){global$o;$c=$o->dumpHeaders($Fc,$ud);$Na=$_POST["output"];if($Na!="text"){header("Content-Disposition: attachment; filename=".friendly_url($Fc!=""?$Fc:(SERVER!=""?SERVER:"localhost")).".$c".($Na!="file"&&!ereg('[^0-9a-z]',$Na)?".$Na":""));}session_write_close();return$c;}function
...
is_mail($tf){$He='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$pc='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$ca="$He+(\\.$He+)*@($pc?\\.)+$pc";return
preg_match("(^$ca(,\\s*$ca)*\$)i",$tf);}function
is_url($K){$pc='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($pc?\\.)+$pc(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$K,$l)?strtolower($l[1]):"");}function
print_fieldset($S,$rf,$yf=false,$Cc=""){echo"<fieldset><legend><a href='#fieldset-$S' onclick=\"$Cc"."return !toggle('fieldset-$S');\">$rf</a></legend><div id='fieldset-$S'".($yf?"":" class='hidden'").">\n";}function
bold($qf){return($qf?" class='active'":"");}global$o,$g,$na,$Pb,$Ab,$n,$aa,$gb,$uc,$Sb,$u,$xf,$Vf,$eb,$La,$O,$Uf,$Q,$ib,$jc;if(!isset($_SERVER["REQUEST_URI"])){$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"].($_SERVER["QUERY_STRING"]!=""?"?$_SERVER[QUERY_STRING]":"");}$uc=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$ic=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$uc);if(version_compare(PHP_VERSION,'5.2.0')>=0){$ic[]=true;}call_user_func_array('session_set_cookie_params',$ic);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE));if(function_exists("set_magic_quotes_runtime")){set_magic_quotes_runtime(false);}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'sk';}function
lang($jf,$ed){$mc=($ed==1?0:($ed&&$ed<5?1:2));return
sprintf($jf[$mc],$ed);}if(extension_loaded('pdo')){class
Min_PDO
extends
...
indexes($h,$G=null){$c=array();$Ja=array();foreach(fields($h)as$d){if($d["primary"]){$Ja[]=$d["field"];}}if($Ja){$c[""]=array("type"=>"PRIMARY","columns"=>$Ja,"lengths"=>array());}foreach(get_rows("PRAGMA index_list(".table($h).")")as$a){$c[$a["name"]]["type"]=($a["unique"]?"UNIQUE":"INDEX");$c[$a["name"]]["lengths"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($a["name"]).")")as$bd){$c[$a["name"]]["columns"][]=$bd["name"];}}return$c;}function
...
q($b);}function
check_sqlite_name($f){global$g;$Ie="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ie)\$~",$f)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$Ie));return
false;}return
true;}function
...
support($xb){return
ereg('^(view|trigger|variables|status|dump)$',$xb);}$u="sqlite";$Q=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$La=array_keys($Q);$ib=array();$bc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","");$aa=array("hex","length","lower","round","unixepoch","upper");$gb=array("avg","count","count distinct","group_concat","max","min","sum");$Pb=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$na["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$xc=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
as$d){$C=idf_escape($d[0]);$b=$d[1];if(!$b){$v[]="DROP $C";}else{$Hd=$b[5];unset($b[5]);if(isset($b[6])&&$d[0]==""){$b[1]=($b[1]=="bigint"?" big":" ")."serial";}if($d[0]==""){$v[]=($h!=""?"ADD ":"  ").implode($b);}else{if($C!=$b[0]){$jb[]="ALTER TABLE ".table($h)." RENAME $C TO $b[0]";}$v[]="ALTER $C TYPE$b[1]";if(!$b[6]){$v[]="ALTER $C ".($b[3]?"SET$b[3]":"DROP DEFAULT");$v[]="ALTER $C ".($b[2]==" NULL"?"DROP NOT":"SET").$b[2];}}if($d[0]!=""||$Hd!=""){$jb[]="COMMENT ON COLUMN ".table($h).".$b[0] IS ".($Hd!=""?substr($Hd,9):"''");}}}$v=array_merge($v,$Ma);if($h==""){array_unshift($jb,"CREATE TABLE ".table($f)." (\n".implode(",\n",$v)."\n)");}elseif($v){array_unshift($jb,"ALTER TABLE ".table($h)."\n".implode(",\n",$v));}if($h!=""&&$h!=$f){$jb[]="ALTER TABLE ".table($h)." RENAME TO ".table($f);}if($h!=""||$xa!=""){$jb[]="COMMENT ON TABLE ".table($f)." IS ".q($xa);}if($Sa!=""){}foreach($jb
...
reset($H);}function
support($xb){return
ereg("view|drop_col|variables|status",$xb);}$u="oracle";$Q=array();$La=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$Q+=$b;$La[$e]=array_keys($b);}$ib=array();$bc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$aa=array("length","lower","round","upper");$gb=array("avg","count","count distinct","max","min","sum");$Pb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$na["mssql"]="MS SQL";if(isset($_GET["mssql"])){$xc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
apply_queries("DROP DATABASE",$A,'idf_escape');}function
rename_database($f,$U){if(create_database($f,$U)){$Vb=array();foreach(tables_list()as$h=>$z){$Vb[]=table($h)." TO ".idf_escape($f).".".table($h);}if(!$Vb||queries("RENAME TABLE ".implode(", ",$Vb))){queries("DROP DATABASE ".idf_escape(DB));return
true;}}return
false;}function
...
alter_table($h,$f,$p,$Ma,$xa,$ub,$U,$Sa,$rb){$v=array();foreach($p
as$d){$v[]=($d[1]?($h!=""?($d[0]!=""?"CHANGE ".idf_escape($d[0]):"ADD"):" ")." ".implode($d[1]).($h!=""?" $d[2]":""):"DROP ".idf_escape($d[0]));}$v=array_merge($v,$Ma);$Yb="COMMENT=".q($xa).($ub?" ENGINE=".q($ub):"").($U?" COLLATE ".q($U):"").($Sa!=""?" AUTO_INCREMENT=$Sa":"").$rb;if($h==""){return
...
drop_tables($D){return
queries("DROP TABLE ".implode(", ",array_map('table',$D)));}function
move_tables($D,$W,$ha){$Vb=array();foreach(array_merge($D,$W)as$h){$Vb[]=table($h)." TO ".idf_escape($ha).".".table($h);}return
queries("RENAME TABLE ".implode(", ",$Vb));}function
...
false;}}foreach($W
as$h){$f=($ha==DB?table("copy_$h"):idf_escape($ha).".".table($h));$Xb=view($h);if(!queries("DROP VIEW IF EXISTS $f")||!queries("CREATE VIEW $f AS $Xb[select]")){return
false;}}return
true;}function
...
routine($f,$z){global$g,$Ab,$Sb,$Q;$Nf=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Me="((".implode("|",array_merge(array_keys($Q),$Nf)).")(?:\\s*\\(((?:[^'\")]*|$Ab)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$ca="\\s*(".($z=="FUNCTION"?"":$Sb).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Me";$ja=$g->result("SHOW CREATE $z ".idf_escape($f),2);preg_match("~\\(((?:$ca\\s*,?)*)\\)".($z=="FUNCTION"?"\\s*RETURNS\\s+$Me":"")."\\s*(.*)~is",$ja,$l);$p=array();preg_match_all("~$ca\\s*,?~is",$l[1],$sa,PREG_SET_ORDER);foreach($sa
...
<tr><th>Systém<td>',html_select("driver",$na,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Zc,$r=""){echo'<p class="tabs">';$ta=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Zc)){$ta["view"]='Zmeniť pohľad';}else{$ta["create"]='Zmeniť tabuľku';}if(isset($r)){$ta["edit"]='Nová položka';}foreach($ta
as$e=>$b){echo" <a href='".h(ME)."$e=".urlencode($Zc["Name"]).($e=="edit"?$r:"")."'".bold(isset($_GET[$e])).">$b</a>";}echo"\n";}function
foreignKeys($h){return
foreign_keys($h);}function
backwardKeys($h,$bg){return
array();}function
backwardKeysPrint($cg,$a){}function
selectQuery($i){global$u;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$u'>".h(str_replace("\n"," ",$i))."</code> <a href='".h(ME)."sql=".urlencode($i)."'>".'Upraviť'."</a></p>\n";}function
rowDescription($h){return"";}function
rowDescriptions($H,$Jf){return$H;}function
selectVal($b,$x,$d){$c=($b!="<i>NULL</i>"&&ereg("char|binary",$d["type"])&&!ereg("var",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$c=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($x?"<a href='$x'>$c</a>":$c);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($I,$B){global$aa,$gb;print_fieldset("select",'Vypísať',$I);$k=0;$Qe=array('Funkcie'=>$aa,'Agregácia'=>$gb);foreach($I
...
selectSearchPrint($s,$B,$J){print_fieldset("search",'Vyhľadať',$s);foreach($J
...
selectOrderPrint($cb,$B,$J){print_fieldset("sort",'Zotriediť',$cb);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'zostupne')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>","<label><input type='checkbox' name='desc[$k]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
...
false;}function
messageQuery($i){global$u;static$Oc=0;restart_session();$S="sql-".($Oc++);$Ya=&get_session("queries");if(strlen($i)>1e6){$i=ereg_replace('[\x80-\xFF]+$','',substr($i,0,1e6))."\n...";}$Ya[$_GET["db"]][]=$i;return" <a href='#$S' onclick=\"return !toggle('$S');\">".'SQL príkaz'."</a><div id='$S' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($i,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ya[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
dumpTable($h,$V,$dd=false){if($_POST["format"]!="sql"){echo"\xef\xbb\xbf";if($V){dump_csv(array_keys(fields($h)));}}elseif($V){$ja=create_sql($h,$_POST["auto_increment"]);if($ja){if($V=="DROP+CREATE"){echo"DROP ".($dd?"VIEW":"TABLE")." IF EXISTS ".table($h).";\n";}if($dd){$ja=preg_replace('~^([A-Z =]+) DEFINER=`'.preg_replace('~@(.*)~','`@`(%|\\1)',logged_user()).'`~','\\1',$ja);}echo($V!="CREATE+ALTER"?$ja:($dd?substr_replace($ja," OR REPLACE",6,0):substr_replace($ja," IF NOT EXISTS",12,0))).";\n\n";}if($V=="CREATE+ALTER"&&!$dd){$i="SELECT COLUMN_NAME, COLUMN_DEFAULT, IS_NULLABLE, COLLATION_NAME, COLUMN_TYPE, EXTRA, COLUMN_COMMENT FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ".q($h)." ORDER BY ORDINAL_POSITION";echo"DELIMITER ;;
...
truncate_sql($h).";\n";}if($_POST["format"]=="sql"){$p=fields($h);}$j=$g->query($i,1);if($j){$gc="";$ob="";while($a=$j->fetch_assoc()){if($_POST["format"]!="sql"){if($V=="table"){dump_csv(array_keys($a));$V="INSERT";}dump_csv($a);}else{if(!$gc){$gc="INSERT INTO ".table($h)." (".implode(", ",array_map('idf_escape',array_keys($a))).") VALUES";}foreach($a
as$e=>$b){$a[$e]=(isset($b)?(ereg('int|float|double|decimal',$p[$e]["type"])?$b:q($b)):"NULL");}$ma=implode(",\t",$a);if($V=="INSERT+UPDATE"){$r=array();foreach($a
as$e=>$b){$r[]=idf_escape($e)." = $b";}echo"$gc ($ma) ON DUPLICATE KEY UPDATE ".implode(", ",$r).";\n";}else{$ma=($xe?"\n":" ")."($ma)";if(!$ob){$ob=$gc.$ma;}elseif(strlen($ob)+4+strlen($ma)<$xe){$ob.=",$ma";}else{echo"$ob;\n";$ob=$gc.$ma;}}}}if($_POST["format"]=="sql"&&$V!="INSERT+UPDATE"&&$ob){$ob.=";\n";echo$ob;}}elseif($_POST["format"]=="sql"){echo"-- ".str_replace("\n"," ",$g->error)."\n";}}}function
dumpHeaders($Fc,$ud=false){$Na=$_POST["output"];$Fb=($_POST["format"]=="sql"?"sql":($ud?"tar":"csv"));header("Content-Type: ".($Na=="bz2"?"application/x-bzip":($Na=="gz"?"application/x-gzip":($Fb=="tar"?"application/x-tar":($Fb=="sql"||$Na!="file"?"text/plain":"text/csv")."; charset=utf-8"))));if($Na=="bz2"){ob_start('bzcompress',1e6);}if($Na=="gz"){ob_start('gzencode',1e6);}return$Fb;}function
...
true;}function
navigation($Ub){global$jc,$g,$O,$u,$na;echo'<h1>
',$this->name(),' <span class="version">',$jc,'</span>
<a href="http://www.adminer.org/#download" id="version">',(version_compare($jc,$_COOKIE["adminer_version"])<0?h($_COOKIE["adminer_version"]):""),'</a>
</h1>
';if($Ub=="auth"){$ya=true;foreach((array)$_SESSION["pwds"]as$Jb=>$Af){foreach($Af
as$F=>$_f){foreach($_f
...
select($j,$G=null,$Ec=""){$ta=array();$J=array();$B=array();$_e=array();$Q=array();odd('');for($k=0;$a=$j->fetch_row();$k++){if(!$k){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($la=0;$la<count($a);$la++){$d=$j->fetch_field();$f=$d->name;$Ga=$d->orgtable;$qc=$d->orgname;if($Ec){$ta[$la]=($f=="table"?"table=":($f=="possible_keys"?"indexes=":null));}elseif($Ga!=""){if(!isset($J[$Ga])){$J[$Ga]=array();foreach(indexes($Ga,$G)as$w){if($w["type"]=="PRIMARY"){$J[$Ga]=array_flip($w["columns"]);break;}}$B[$Ga]=$J[$Ga];}if(isset($B[$Ga][$qc])){unset($B[$Ga][$qc]);$J[$Ga][$qc]=$la;$ta[$la]=$Ga;}}if($d->charsetnr==63){$_e[$la]=true;}$Q[$la]=$d->type;$f=h($f);echo"<th".($Ga!=""||$d->name!=$qc?" title='".h(($Ga!=""?"$Ga.":"").$qc)."'":"").">".($Ec?"<a href='$Ec".strtolower($f)."' target='_blank' rel='noreferrer'>$f</a>":$f);}echo"</thead>\n";}echo"<tr".odd().">";foreach($a
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($_e[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($Q[$e]==254){$b="<code>$b</code>";}}if(isset($ta[$e])&&!$B[$ta[$e]]){if($Ec){$x=$ta[$e].urlencode($a[array_search("table=",$ta)]);}else{$x="edit=".urlencode($ta[$e]);foreach($J[$ta[$e]]as$Nc=>$la){$x.="&where".urlencode("[".bracket_escape($Nc)."]")."=".urlencode($a[$la]);}}$b="<a href='".h(ME.$x)."'>$b</a>";}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($e,$d,$Z,$da=array()){global$La,$Q,$ib,$eb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($Q[$d["type"]])?array():array($d["type"]))+$La+($da?array('Cudzie kľúče'=>$da):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($Z,$d["collation"]).'</select>',($ib?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($ib,$d["unsigned"]).'</select>':''),($da?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($eb,$d["on_delete"])."</select> ":" ");}function
...
process_type($d,$wc="COLLATE"){global$ib;return" $d[type]".($d["length"]!=""?"(".process_length($d["length"]).")":"").(ereg('int|float|double|decimal',$d["type"])&&in_array($d["unsigned"],$ib)?" $d[unsigned]":"").(ereg('char|text|enum|set',$d["type"])&&$d["collation"]?" $wc ".q($d["collation"]):"");}function
process_field($d,$Ic){return
array(idf_escape($d["field"]),process_type($Ic),($d["null"]?" NULL":" NOT NULL"),(isset($d["default"])?" DEFAULT ".(($d["type"]=="timestamp"&&eregi('^CURRENT_TIMESTAMP$',$d["default"]))||($d["type"]=="bit"&&ereg("^([0-9]+|b'[0-1]+')\$",$d["default"]))?$d["default"]:q($d["default"])):""),($d["on_update"]?" ON UPDATE $d[on_update]":""),(support("comment")&&$d["comment"]!=""?" COMMENT ".q($d["comment"]):""),($d["auto_increment"]?auto_increment():null),);}function
type_class($z){foreach(array('char'=>'text','date'=>'time|year','binary'=>'blob','enum'=>'set',)as$e=>$b){if(ereg("$e|$b",$z)){return" class='$e'";}}}function
edit_fields($p,$Z,$z="TABLE",$ef=0,$da=array(),$Db=false){global$Sb;echo'<thead><tr class="wrap">
...
',(support("comment")?"<td".($Db?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$k=>$d){$k++;$nd=$d[($_POST?"orig":"field")];$cf=(isset($_POST["add"][$k-1])||(isset($d["field"])&&!$_POST["drop_col"][$k]))&&(support("drop_col")||$nd=="");echo'<tr',($cf?"":" style='display: none;'"),'>
',($z=="PROCEDURE"?"<td>".html_select("fields[$k][inout]",explode("|",$Sb),$d["inout"]):""),'<th>';if($cf){echo'<input name="fields[',$k,'][field]" value="',h($d["field"]),'" onchange="',($d["field"]!=""||count($p)>1?"":"editingAddRow(this, $ef); "),'editingNameChange(this);" maxlength="64">';}echo'<input type="hidden" name="fields[',$k,'][orig]" value="',h($nd),'">
...
',(support("comment")?"<td".($Db?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $ef, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($nd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
tar_file($ia,$od){$c=pack("a100a8a8a8a12a12",$ia,644,0,0,decoct(strlen($od)),decoct(time()));$Ue=8*32;for($k=0;$k<strlen($c);$k++){$Ue+=ord($c{$k});}$c.=sprintf("%06o",$Ue)."\0 ";return$c.str_repeat("\0",512-strlen($c)).$od.str_repeat("\0",511-(strlen($od)+511)%
512);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$eb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$Ab="'(?:''|[^'\\\\]|\\\\.)*+'";$Sb="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["callf"])){$_GET["call"]=$_GET["callf"];}if(isset($_GET["function"])){$_GET["procedure"]=$_GET["function"];}if(isset($_GET["download"])){$m=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$m-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($m)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["table"])){$m=$_GET["table"];$p=fields($m);if(!$p){$n=error();}$E=($p?table_status($m):array());page_header(($p&&is_view($E)?'Pohľad':'Tabuľka').": ".h($m),$n);$o->selectLinks($E);$xa=$E["Comment"];if($xa!=""){echo"<p>".'Komentár'.": ".h($xa)."\n";}if($p){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Stĺpec'."<td>".'Typ'.(support("comment")?"<td>".'Komentár':"")."</thead>\n";foreach($p
...
as$f=>$w){ksort($w["columns"]);$yb=array();foreach($w["columns"]as$e=>$b){$yb[]="<i>".h($b)."</i>".($w["lengths"][$e]?"(".$w["lengths"][$e].")":"");}echo"<tr title='".h($f)."'><th>$w[type]<td>".implode(", ",$yb)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($m).'">'.'Zmeniť indexy'."</a>\n";if(fk_support($E)){echo"<h3>".'Cudzie kľúče'."</h3>\n";$da=foreign_keys($m);if($da){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'Cieľ'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($u!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($da
as$f=>$_){echo"<tr title='".h($f)."'>","<th><i>".implode("</i>, <i>",array_map('h',$_["source"]))."</i>","<td><a href='".h($_["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($_["db"]),ME):($_["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($_["ns"]),ME):ME))."table=".urlencode($_["table"])."'>".($_["db"]!=""?"<b>".h($_["db"])."</b>.":"").($_["ns"]!=""?"<b>".h($_["ns"])."</b>.":"").h($_["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$_["target"]))."</i>)","<td>".nbsp($_["on_delete"])."\n","<td>".nbsp($_["on_update"])."\n";if($u!="sqlite"){echo'<td><a href="'.h(ME.'foreign='.urlencode($m).'&name='.urlencode($f)).'">'.'Zmeniť'.'</a>';}}echo"</table>\n";}if($u!="sqlite"){echo'<p><a href="'.h(ME).'foreign='.urlencode($m).'">'.'Pridať cudzí kľúč'."</a>\n";}}if(support("trigger")){echo"<h3>".'Triggery'."</h3>\n";$Gc=triggers($m);if($Gc){echo"<table cellspacing='0'>\n";foreach($Gc
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Zmeniť'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB);$vb=array();$Re=array();$f="adminer_schema";$We=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$We,$sa,PREG_SET_ORDER);foreach($sa
as$k=>$l){$vb[$l[1]]=array($l[2],$l[3]);$Re[]="\n\t'".js_escape($l[1])."': [ $l[2], $l[3] ]";}$ec=0;$Ye=-1;$Ia=array();$Xe=array();$Ze=array();foreach(table_status()as$a){if(!isset($a["Engine"])){continue;}$mc=0;$Ia[$a["Name"]]["fields"]=array();foreach(fields($a["Name"])as$f=>$d){$mc+=1.25;$d["pos"]=$mc;$Ia[$a["Name"]]["fields"][$f]=$d;}$Ia[$a["Name"]]["pos"]=($vb[$a["Name"]]?$vb[$a["Name"]]:array($ec,0));foreach($o->foreignKeys($a["Name"])as$b){if(!$b["db"]){$za=$Ye;if($vb[$a["Name"]][1]||$vb[$b["table"]][1]){$za=min(floatval($vb[$a["Name"]][1]),floatval($vb[$b["table"]][1]))-1;}else{$Ye-=.1;}while($Ze[(string)$za]){$za-=.0001;}$Ia[$a["Name"]]["references"][$b["table"]][(string)$za]=array($b["source"],$b["target"]);$Xe[$b["table"]][$a["Name"]][(string)$za]=$b["target"];$Ze[(string)$za]=true;}}$ec=max($ec,$Ia[$a["Name"]]["pos"][0]+2.5+$mc);}echo'<div id="schema" style="height: ',$ec,'em;">
<script type="text/javascript">
tablePos = {',implode(",",$Re)."\n",'};
em = document.getElementById(\'schema\').offsetHeight / ',$ec,';
...
';foreach($Ia
as$f=>$h){echo"<div class='table' style='top: ".$h["pos"][0]."em; left: ".$h["pos"][1]."em;' onmousedown='schemaMousedown(this, event);'>",'<a href="'.h(ME).'table='.urlencode($f).'"><b>'.h($f)."</b></a><br>\n";foreach($h["fields"]as$d){$b='<span'.type_class($d["type"]).' title="'.h($d["full_type"].($d["null"]?" NULL":'')).'">'.h($d["field"]).'</span>';echo($d["primary"]?"<i>$b</i>":$b)."<br>\n";}foreach((array)$h["references"]as$dc=>$hc){foreach($hc
as$za=>$Wc){$sc=$za-$vb[$f][1];$k=0;foreach($Wc[0]as$Ha){echo"<div class='references' title='".h($dc)."' id='refs$za-".($k++)."' style='left: $sc"."em; top: ".$h["fields"][$Ha]["pos"]."em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: ".(-$sc)."em;'></div></div>\n";}}}foreach((array)$Xe[$f]as$dc=>$hc){foreach($hc
as$za=>$B){$sc=$za-$vb[$f][1];$k=0;foreach($B
as$ha){echo"<div class='references' title='".h($dc)."' id='refd$za-".($k++)."' style='left: $sc"."em; top: ".$h["fields"][$ha]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=3.3.0'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$sc)."em;'></div></div>\n";}}}echo"</div>\n";}foreach($Ia
as$f=>$h){foreach((array)$h["references"]as$dc=>$hc){foreach($hc
as$za=>$Wc){$Uc=$ec;$md=-10;foreach($Wc[0]as$e=>$Ha){$Ve=$h["pos"][0]+$h["fields"][$Ha]["pos"];$Se=$Ia[$dc]["pos"][0]+$Ia[$dc]["fields"][$Wc[1][$e]]["pos"];$Uc=min($Uc,$Ve,$Se);$md=max($md,$Ve,$Se);}echo"<div class='references' id='refl$za' style='left: $za"."em; top: $Uc"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($md-$Uc)."em;'></div></div>\n";}}}echo'</div>
<p><a href="',h(ME."schema=".urlencode($We)),'" id="schema-link">Permanentný odkaz</a>
';}elseif(isset($_GET["dump"])){$m=$_GET["dump"];if($_POST){$bf="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$e){$bf.="&$e=".urlencode($_POST[$e]);}cookie("adminer_export",substr($bf,1));$Fb=dump_headers(($m!=""?$m:DB),(DB==""||count((array)$_POST["tables"]+(array)$_POST["data"])>1));$wb=($_POST["format"]=="sql");if($wb){echo"-- Adminer $jc ".$na[DRIVER]." dump
...
as$Xb){$o->dumpTable($Xb,$_POST["table_style"],true);}if($Fb=="tar"){echo
...
<table cellspacing="0">
';$Md=array('','USE','DROP+CREATE','CREATE');$Vd=array('','DROP+CREATE','CREATE');$Ud=array('','TRUNCATE+INSERT','INSERT');if($u=="sql"){$Md[]='CREATE+ALTER';$Vd[]='CREATE+ALTER';$Ud[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$a);if(!$a){$a=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}if(!isset($a["events"])){$a["routines"]=$a["events"]=($_GET["dump"]=="");$a["triggers"]=$a["table_style"];}echo"<tr><th>".'Výstup'."<td>".html_select("output",$o->dumpOutput(),$a["output"],0)."\n";echo"<tr><th>".'Formát'."<td>".html_select("format",$o->dumpFormat(),$a["format"],0)."\n";echo($u=="sqlite"?"":"<tr><th>".'Databáza'."<td>".html_select('db_style',$Md,$a["db_style"]).(support("routine")?checkbox("routines",1,$a["routines"],'Procedúry'):"").(support("event")?checkbox("events",1,$a["events"],'Udalosti'):"")),"<tr><th>".'Tabuľky'."<td>".html_select('table_style',$Vd,$a["table_style"]).checkbox("auto_increment",1,$a["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$a["triggers"],'Triggery'):""),"<tr><th>".'Dáta'."<td>".html_select('data_style',$Ud,$a["data_style"]),'</table>
<p><input type="submit" value="Export">

<table cellspacing="0">
';$kd=array();if(DB!=""){$fb=($m!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$fb onclick='formCheck(this, /^tables\\[/);'>".'Tabuľky'."</label>","<th style='text-align: right;'><label>".'Dáta'."<input type='checkbox' id='check-data'$fb onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$W="";foreach(table_status()as$a){$f=$a["Name"];$vc=ereg_replace("_.*","",$f);$fb=($m==""||$m==(substr($m,-1)=="%"?"$vc%":$f));$yb="<tr><td>".checkbox("tables[]",$f,$fb,$f,"formUncheck('check-tables');");if(is_view($a)){$W.="$yb\n";}else{echo"$yb<td align='right'><label>".($a["Engine"]=="InnoDB"&&$a["Rows"]?"~ ":"").$a["Rows"].checkbox("data[]",$f,$fb,"","formUncheck('check-data');")."</label>\n";}$kd[$vc]++;}echo$W;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($m==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Databáza'."</label></thead>\n";$A=get_databases();if($A){foreach($A
as$t){if(!information_schema($t)){$vc=ereg_replace("_.*","",$t);echo"<tr><td>".checkbox("databases[]",$t,$m==""||$m=="$vc%",$t,"formUncheck('check-databases');")."</label>\n";$kd[$vc]++;}}}else{echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}}echo'</table>
</form>
';$ya=true;foreach($kd
as$e=>$b){if($e!=""&&$b>1){echo($ya?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$ya=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$j=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q($_GET["db"])." LIKE Db")." ORDER BY Host, User");if(!$j){echo'<form action=""><p>
...
';$j=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");}echo"<table cellspacing='0'>\n","<thead><tr><th>&nbsp;<th>".'Používateľ'."<th>".'Server'."</thead>\n";while($a=$j->fetch_assoc()){echo'<tr'.odd().'><td><a href="'.h(ME.'user='.urlencode($a["User"]).'&host='.urlencode($a["Host"])).'">'.'upraviť'.'</a><td>'.h($a["User"])."<td>".h($a["Host"])."\n";}echo"</table>\n",'<p><a href="'.h(ME).'user=">'.'Vytvoriť používateľa'."</a>";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$o->dumpTable("","");$o->dumpData("","table",$_POST["query"]);exit;}restart_session();$vf=&get_session("queries");$Ya=&$vf[DB];if(!$n&&$_POST["clear"]){$Ya=array();redirect(remove_from_uri("history"));}page_header('SQL príkaz',$n);if(!$n&&$_POST){$Ka=false;$i=$_POST["query"];if($_POST["webfile"]){$Ka=@fopen((file_exists("adminer.sql")?"adminer.sql":(file_exists("adminer.sql.gz")?"compress.zlib://adminer.sql.gz":"compress.bzip2://adminer.sql.bz2")),"rb");$i=($Ka?fread($Ka,1e6):false);}elseif($_FILES&&$_FILES["sql_file"]["error"]!=4){$i=get_file("sql_file",true);}if(is_string($i)){if(function_exists('memory_get_usage')){@ini_set("memory_limit",max(ini_get("memory_limit"),2*strlen($i)+memory_get_usage()+8e6));}if($i!=""&&strlen($i)<1e6){$ba=$i.(ereg(';$',$i)?"":";");if(!$Ya||end($Ya)!=$ba){$Ya[]=$ba;}}$kc="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\n)";if(!ini_bool("session.use_cookies")){session_write_close();}$qd=";";$M=0;$cc=true;$G=connect();if(is_object($G)&&DB!=""){$G->select_db(DB);}$zb=0;$Tc=array();$Mf='[\'`"]'.($u=="pgsql"?'|\\$[^$]*\\$':($u=="mssql"||$u=="sqlite"?'|\\[':'')).'|/\\*|-- |#';$Rf=microtime();parse_str($_COOKIE["adminer_export"],$ne);$be=$o->dumpFormat();unset($be["sql"]);while($i!=""){if(!$M&&$u=="sql"&&preg_match("~^$kc*DELIMITER\\s+(.+)~i",$i,$l)){$qd=$l[1];$i=substr($i,strlen($l[0]));}else{preg_match('('.preg_quote($qd)."|$Mf|\$)",$i,$l,PREG_OFFSET_CAPTURE,$M);$qa=$l[0][0];$M=$l[0][1]+strlen($qa);if(!$qa&&$Ka&&!feof($Ka)){$i.=fread($Ka,1e5);}else{if(!$qa&&rtrim($i)==""){break;}if($qa&&$qa!=$qd){while(preg_match('('.($qa=='/*'?'\\*/':($qa=='['?']':(ereg('^-- |^#',$qa)?"\n":preg_quote($qa)."|\\\\."))).'|$)s',$i,$l,PREG_OFFSET_CAPTURE,$M)){$ma=$l[0][0];$M=$l[0][1]+strlen($ma);if(!$ma&&$Ka&&!feof($Ka)){$i.=fread($Ka,1e6);}elseif($ma[0]!="\\"){break;}}}else{$cc=false;$ba=substr($i,0,$l[0][1]);$zb++;$yb="<pre id='sql-$zb'><code class='jush-$u'>".shorten_utf8(trim($ba),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$yb;ob_flush();flush();}$Eb=microtime();if($g->multi_query($ba)&&is_object($G)&&preg_match("~^$kc*USE\\b~isU",$ba)){$G->query($ba);}do{$j=$g->store_result();$Qc=microtime();$pe=format_time($Eb,$Qc).(strlen($ba)<1000?" <a href='".h(ME)."sql=".urlencode(trim($ba))."'>".'Upraviť'."</a>":"");if($g->error){echo($_POST["only_errors"]?$yb:""),"<p class='error'>".'Chyba v dotaze'.": ".error()."\n";$Tc[]=" <a href='#sql-$zb'>$zb</a>";if($_POST["error_stops"]){break;}}elseif(is_object($j)){select($j,$G);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n","<p>".($j->num_rows?lang(array('%d riadok','%d riadky','%d riadkov'),$j->num_rows):"").$pe;$S="export-$zb";$re=", <a href='#$S' onclick=\"return !toggle('$S');\">".'Export'."</a><span id='$S' class='hidden'>: ".html_select("output",$o->dumpOutput(),$ne["output"])." ".html_select("format",$be,$ne["format"])."<input type='hidden' name='query' value='".h($ba)."'>"." <input type='submit' name='export' value='".'Export'."' onclick='eventStop(event);'><input type='hidden' name='token' value='$O'></span>";if($G&&preg_match("~^($kc|\\()*SELECT\\b~isU",$ba)&&($Df=explain($G,$ba))){$S="explain-$zb";echo", <a href='#$S' onclick=\"return !toggle('$S');\">EXPLAIN</a>$re\n","<div id='$S' class='hidden'>\n";select($Df,$G,($u=="sql"?"http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/explain-output.html#explain_":""));echo"</div>\n";}else{echo"$re\n";}echo"</form>\n";}}else{if(preg_match("~^$kc*(CREATE|DROP|ALTER)$kc+(DATABASE|SCHEMA)\\b~isU",$ba)){restart_session();set_session("dbs",null);session_write_close();}if(!$_POST["only_errors"]){echo"<p class='message' title='".h($g->info)."'>".lang(array('Príkaz prebehol v poriadku, bol zmenený %d záznam.','Príkaz prebehol v poriadku boli zmenené %d záznamy.','Príkaz prebehol v poriadku bolo zmenených %d záznamov.'),$g->affected_rows)."$pe\n";}}$Eb=$Qc;}while($g->next_result());$i=substr($i,$M);$M=0;}}}}if($cc){echo"<p class='message'>".'Žiadne príkazy na vykonanie.'."\n";}elseif($_POST["only_errors"]){echo"<p class='message'>".lang(array('Bol vykonaný %d dotaz.','Boli vykonané %d dotazy.','Bolo vykonaných %d dotazov.'),$zb-count($Tc)).format_time($Rf,microtime())."\n";}elseif($Tc&&$zb>1){echo"<p class='error'>".'Chyba v dotaze'.": ".implode("",$Tc)."\n";}}else{echo"<p class='error'>".upload_error($i)."\n";}}echo'
...
<input type="hidden" name="token" value="',$O,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$pd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$pd[]=".$e";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($pd?"[".implode("|",$pd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Ya){print_fieldset("history",'História',$_GET["history"]!="");foreach($Ya
as$e=>$b){echo'<a href="'.h(ME."sql=&history=$e").'">'.'Upraviť'."</a> <code class='jush-$u'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$oa?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ga=$_POST["referer"];if($_POST["insert"]){$ga=($oa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ga)){$ga=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$ga,'Položka bola vymazaná.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($oa?"\n".idf_escape($f)." = $b":$b);}}if($oa){if(!$r){redirect($ga);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$ga,'Položka bola aktualizovaná.');}else{$j=insert_into($m,$r);$je=($j?last_id():0);queries_redirect($ga,sprintf('Položka%s bola vložená.',($je?" $je":"")),$j);}}}$Ea=$o->tableName(table_status($m));page_header(($oa?'Upraviť':'Vložiť'),$n,array("select"=>array($m,$Ea)),$Ea);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$I=array();foreach($p
...
';if($p){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($oa?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($oa?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$Ea=>$d){$da[str_replace("`","``",$Ea)."`".str_replace("`","``",$d["field"])]=$Ea;}$Ac=array();$Hc=array();if($m!=""){$Ac=fields($m);$Hc=table_status($m);}if($_POST&&!$_POST["fields"]){$_POST["fields"]=array();}if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"]){if($_POST["drop"]){query_redirect("DROP TABLE ".table($m),substr(ME,0,-1),'Tabuľka bola odstránená.');}else{$p=array();$Ma=array();ksort($_POST["fields"]);$rd=reset($Ac);$Ib="FIRST";foreach($_POST["fields"]as$e=>$d){$_=$da[$d["type"]];$Ic=(isset($_)?$de[$_]:$d);if($d["field"]!=""){if(!$d["has_default"]){$d["default"]=null;}$_a=eregi_replace(" *on update CURRENT_TIMESTAMP","",$d["default"]);if($_a!=$d["default"]){$d["on_update"]="CURRENT_TIMESTAMP";$d["default"]=$_a;}if($e==$_POST["auto_increment_col"]){$d["auto_increment"]=true;}$qe=process_field($d,$Ic);if($qe!=process_field($rd,$rd)){$p[]=array($d["orig"],$qe,$Ib);}if(isset($_)){$Ma[idf_escape($d["field"])]=($m!=""?"ADD":" ")." FOREIGN KEY (".idf_escape($d["field"]).") REFERENCES ".table($da[$d["type"]])." (".idf_escape($Ic["field"]).")".(in_array($d["on_delete"],$eb)?" ON DELETE $d[on_delete]":"");}$Ib="AFTER ".idf_escape($d["field"]);}elseif($d["orig"]!=""){$p[]=array($d["orig"]);}if($d["orig"]!=""){$rd=next($Ac);}}$rb="";if(in_array($_POST["partition_by"],$ee)){$ld=array();if($_POST["partition_by"]=='RANGE'||$_POST["partition_by"]=='LIST'){foreach(array_filter($_POST["partition_names"])as$e=>$b){$q=$_POST["partition_values"][$e];$ld[]="\nPARTITION ".idf_escape($b)." VALUES ".($_POST["partition_by"]=='RANGE'?"LESS THAN":"IN").($q!=""?" ($q)":" MAXVALUE");}}$rb.="\nPARTITION BY $_POST[partition_by]($_POST[partition])".($ld?" (".implode(",",$ld)."\n)":($_POST["partitions"]?" PARTITIONS ".(+$_POST["partitions"]):""));}elseif($m!=""&&support("partitioning")){$rb.="\nREMOVE PARTITIONING";}$ua='Tabuľka bola zmenená.';if($m==""){cookie("adminer_engine",$_POST["Engine"]);$ua='Tabuľka bola vytvorená.';}queries_redirect(ME."table=".urlencode($_POST["name"]),$ua,alter_table($m,$_POST["name"],$p,$Ma,$_POST["Comment"],($_POST["Engine"]&&$_POST["Engine"]!=$Hc["Engine"]?$_POST["Engine"]:""),($_POST["Collation"]&&$_POST["Collation"]!=$Hc["Collation"]?$_POST["Collation"]:""),($_POST["Auto_increment"]!=""?+$_POST["Auto_increment"]:""),$rb));}}page_header(($m!=""?'Zmeniť tabuľku':'Vytvoriť tabuľku'),$n,array("table"=>$m),$m);$a=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($Q["int"])?"int":(isset($Q["integer"])?"integer":"")))),"partition_names"=>array(""),);if($_POST){$a=$_POST;if($a["auto_increment_col"]){$a["fields"][$a["auto_increment_col"]]["auto_increment"]=true;}process_fields($a["fields"]);}elseif($m!=""){$a=$Hc;$a["name"]=$m;$a["fields"]=array();if(!$_GET["auto_increment"]){$a["Auto_increment"]="";}foreach($Ac
as$d){$d["has_default"]=isset($d["default"]);if($d["on_update"]){$d["default"].=" ON UPDATE $d[on_update]";}$a["fields"][]=$d;}if(support("partitioning")){$Zb="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($m);$j=$g->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Zb ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($a["partition_by"],$a["partitions"],$a["partition"])=$j->fetch_row();$a["partition_names"]=array();$a["partition_values"]=array();foreach(get_rows("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Zb AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION")as$bd){$a["partition_names"][]=$bd["PARTITION_NAME"];$a["partition_values"][]=$bd["PARTITION_DESCRIPTION"];}$a["partition_names"][]="";}}$Z=collations();$id=floor(extension_loaded("suhosin")?(min(ini_get("suhosin.request.max_vars"),ini_get("suhosin.post.max_vars"))-13)/10:0);if($id&&count($a["fields"])>$id){echo"<p class='error'>".h(sprintf('Bol prekročený maximálny počet povolených polí. Zvýšte prosím %s a %s.','suhosin.post.max_vars','suhosin.request.max_vars'))."\n";}$Jd=engines();foreach($Jd
as$ub){if(!strcasecmp($ub,$a["Engine"])){$a["Engine"]=$ub;break;}}echo'
<form action="" method="post" id="form">
<p>
Názov tabuľky: <input name="name" maxlength="64" value="',h($a["name"]),'">
';if($m==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Jd?html_select("Engine",array(""=>"(".'úložisko'.")")+$Jd,$a["Engine"]):""),' ',($Z&&!ereg("sqlite|mssql",$u)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$Z,$a["Collation"]):""),' <input type="submit" value="Uložiť">
<table cellspacing="0" id="edit-fields" class="nowrap">
';$Db=($_POST?$_POST["comments"]:$a["Comment"]!="");if(!$_POST&&!$Db){foreach($a["fields"]as$d){if($d["comment"]!=""){$Db=true;break;}}}edit_fields($a["fields"],$Z,"TABLE",$id,$da,$Db);echo'</table>
<p>
...
',(support("comment")?checkbox("comments",1,$Db,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Db?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
';if(support("partitioning")){$Zd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$ee,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Zd||!$a["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$n&&!isset($_POST["add_x"])){restart_session();if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$_POST["name"]){if(DB!=""){$_GET["db"]=$_POST["name"];queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_POST["name"]),'Databáza bola premenovaná.',rename_database($_POST["name"],$_POST["collation"]));}else{$A=explode("\n",str_replace("\r","",$_POST["name"]));$me=true;$pb="";foreach($A
as$t){if(count($A)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$me=false;}$pb=$t;}}queries_redirect(ME."db=".urlencode($pb),'Databáza bola vytvorená.',$me);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$n,array(),DB);$Z=collations();$f=DB;$wc=null;if($_POST){$f=$_POST["name"];$wc=$_POST["collation"];}elseif(DB!=""){$wc=db_collation(DB,$Z);}elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$ka){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ka,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Z?html_select("collation",array(""=>"(".'porovnávanie'.")")+$Z,$wc):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložiť">
<?php
if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Pridať ďalší'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$n){$x=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"]){query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$x,'Schéma bola odstránená.');}else{$x.=urlencode($_POST["name"]);if($_GET["ns"]==""){query_redirect("CREATE SCHEMA ".idf_escape($_POST["name"]),$x,'Schéma bola vytvorená.');}elseif($_GET["ns"]!=$_POST["name"]){query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($_POST["name"]),$x,'Schéma bola zmenená.');}else{redirect($x);}}}page_header($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému',$n);$a=array("name"=>$_GET["ns"]);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Uložiť">
';if($_GET["ns"]!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["call"])){$Pa=$_GET["call"];page_header('Zavolať'.": ".h($Pa),$n);$Qa=routine($Pa,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Kb=array();$lb=array();foreach($Qa["fields"]as$k=>$d){if(substr($d["inout"],-3)=="OUT"){$lb[$k]="@".idf_escape($d["field"])." AS ".idf_escape($d["field"]);}if(!$d["inout"]||substr($d["inout"],0,2)=="IN"){$Kb[]=$k;}}if(!$n&&$_POST){$ie=array();foreach($Qa["fields"]as$e=>$d){if(in_array($e,$Kb)){$b=process_input($d);if($b===false){$b="''";}if(isset($lb[$e])){$g->query("SET @".idf_escape($d["field"])." = $b");}}$ie[]=(isset($lb[$e])?"@".idf_escape($d["field"]):$b);}$i=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($Pa)."(".implode(", ",$ie).")";echo"<p><code class='jush-$u'>".h($i)."</code> <a href='".h(ME)."sql=".urlencode($i)."'>".'Upraviť'."</a>\n";if(!$g->multi_query($i)){echo"<p class='error'>".error()."\n";}else{$G=connect();if(is_object($G)){$G->select_db(DB);}do{$j=$g->store_result();if(is_object($j)){select($j,$G);}else{echo"<p class='message'>".lang(array('Procedúra bola zavolaná, bol zmenený %d záznam.','Procedúra bola zavolaná, boli zmenené %d záznamy.','Procedúra bola zavolaná, bolo zmenených %d záznamov.'),$g->affected_rows)."\n";}}while($g->next_result());if($lb){select($g->query("SELECT ".implode(", ",$lb)));}}}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["foreign"])){$m=$_GET["foreign"];if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"]){query_redirect("ALTER TABLE ".table($m)."\nDROP ".($u=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($m),'Cudzí kľúč bol odstránený.');}else{$Ha=array_filter($_POST["source"],'strlen');ksort($Ha);$ha=array();foreach($Ha
as$e=>$b){$ha[$e]=$_POST["target"][$e];}query_redirect("ALTER TABLE ".table($m).($_GET["name"]!=""?"\nDROP FOREIGN KEY ".idf_escape($_GET["name"]).",":"")."\nADD FOREIGN KEY (".implode(", ",array_map('idf_escape',$Ha)).") REFERENCES ".table($_POST["table"])." (".implode(", ",array_map('idf_escape',$ha)).")".(in_array($_POST["on_delete"],$eb)?" ON DELETE $_POST[on_delete]":"").(in_array($_POST["on_update"],$eb)?" ON UPDATE $_POST[on_update]":""),ME."table=".urlencode($m),($_GET["name"]!=""?'Cudzí kľúč bol zmenený.':'Cudzí kľúč bol vytvorený.'));$n='Zdrojové a cieľové stĺpce musia mať rovnaký datový typ, nad cieľovými stĺpcami musí byť definovaný index a odkazované dáta musia existovať.'."<br>$n";}}page_header('Cudzí kľúč',$n,array("table"=>$m),$m);$a=array("table"=>$m,"source"=>array(""));if($_POST){$a=$_POST;ksort($a["source"]);if($_POST["add"]){$a["source"][]="";}elseif($_POST["change"]||$_POST["change-js"]){$a["target"]=array();}}elseif($_GET["name"]!=""){$da=foreign_keys($m);$a=$da[$_GET["name"]];$a["source"][]="";}$Ha=array_keys(fields($m));$ha=($m===$a["table"]?$Ha:array_keys(fields($a["table"])));$ge=array();foreach(table_status()as$f=>$E){if(fk_support($E)){$ge[]=$f;}}echo'
<form action="" method="post">
<p>
';if($a["db"]==""&&$a["ns"]==""){echo'Cieľová tabuľka:
',html_select("table",$ge,$a["table"],"this.form['change-js'].value = '1'; if (!ajaxForm(this.form)) this.form.submit();"),'<input type="hidden" name="change-js" value="">
...
<p>
ON DELETE: ',html_select("on_delete",array(-1=>"")+$eb,$a["on_delete"]),' ON UPDATE: ',html_select("on_update",array(-1=>"")+$eb,$a["on_update"]),'<p>
<input type="submit" value="Uložiť">
...
';if($Xa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["event"])){$mb=$_GET["event"];$ke=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Gd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($mb),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$ke)&&isset($Gd[$_POST["STATUS"]])){$oe="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($mb!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($mb!=""?"ALTER EVENT ".idf_escape($mb).$oe.($mb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$oe)."\n".$Gd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($mb!=""?'Upraviť udalosť'.": ".h($mb):'Vytvoriť udalosť'),$n);$a=array();if($_POST){$a=$_POST;}elseif($mb!=""){$H=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($mb));$a=reset($H);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$ke,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Gd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($mb!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["procedure"])){$Pa=$_GET["procedure"];$Qa=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$xd=routine_languages();$Xa=false;if($_POST&&!$n&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"]){$r=array();$p=(array)$_POST["fields"];ksort($p);foreach($p
as$d){if($d["field"]!=""){$r[]=(ereg("^($Sb)\$",$d["inout"])?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Xa=drop_create("DROP $Qa ".idf_escape($Pa),"CREATE $Qa ".idf_escape($_POST["name"])." (".implode(", ",$r).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$xd)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$Pa);}page_header(($Pa!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($Pa):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$n);$Z=get_vals("SHOW CHARACTER SET");sort($Z);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Pa!=""){$a=routine($Pa,$Qa);$a["name"]=$Pa;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($Pa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Xa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["sequence"])){$Cb=$_GET["sequence"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($Cb),$x,'Sekvencia bola odstránená.');}elseif($Cb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$x,'Sekvencia bola vytvorená.');}elseif($Cb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($Cb)." RENAME TO ".idf_escape($_POST["name"]),$x,'Sekvencia bola zmenená.');}else{redirect($x);}}page_header($Cb!=""?'Pozmeniť sekvenciu'.": ".h($Cb):'Vytvoriť sekvenciu',$n);$a=array("name"=>$Cb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Uložiť">
';if($Cb!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["type"])){$gd=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($gd),$x,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ bol vytvorený.');}}page_header($gd!=""?'Pozmeniť typ'.": ".h($gd):'Vytvoriť typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($gd!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["trigger"])){$m=$_GET["trigger"];$cd=trigger_options();$ae=array("INSERT","UPDATE","DELETE");$Xa=false;if($_POST&&!$n&&in_array($_POST["Timing"],$cd["Timing"])&&in_array($_POST["Event"],$ae)&&in_array($_POST["Type"],$cd["Type"])){$Td=" $_POST[Timing] $_POST[Event]";$_b=" ON ".table($m);$Xa=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$_b:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$_b.$Td:$Td.$_b).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($m),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$n,array("table"=>$m));$a=array("Trigger"=>$m."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$cd["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($m))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$ae,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$cd["Type"],$a["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Xa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["user"])){$Dd=$_GET["user"];$ra=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$a){foreach(explode(",",($a["Privilege"]=="Grant option"?"":$a["Context"]))as$Yc){$ra[$Yc][$a["Privilege"]]=$a["Comment"];}}$ra["Server Admin"]+=$ra["File access on server"];$ra["Databases"]["Create routine"]=$ra["Procedures"]["Create routine"];unset($ra["Procedures"]["Create routine"]);$ra["Columns"]=array();foreach(array("Select","Insert","Update","References")as$b){$ra["Columns"][$b]=$ra["Tables"][$b];}unset($ra["Server Admin"]["Usage"]);foreach($ra["Tables"]as$e=>$b){unset($ra["Databases"][$e]);}$lc=array();if($_POST){foreach($_POST["objects"]as$e=>$b){$lc[$b]=(array)$lc[$b]+(array)$_POST["grants"][$e];}}$kb=array();$ad="";if(isset($_GET["host"])&&($j=$g->query("SHOW GRANTS FOR ".q($Dd)."@".q($_GET["host"])))){while($a=$j->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$a[0],$l)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$l[1],$sa,PREG_SET_ORDER)){foreach($sa
as$b){if($b[1]!="USAGE"){$kb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$kb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$ad=$l[1];}}}if($_POST&&!$n){$Wb=(isset($_GET["host"])?q($Dd)."@".q($_GET["host"]):"''");$nb=q($_POST["user"])."@".q($_POST["host"]);$yd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $Wb",ME."privileges=",'Používateľ bol odstránený.');}else{if($Wb!=$nb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $nb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $yd");}elseif($_POST["pass"]!=$ad||!$_POST["hashed"]){queries("SET PASSWORD FOR $nb = ".($_POST["hashed"]?$yd:"PASSWORD($yd)"));}if(!$n){$zc=array();foreach($lc
as$ab=>$ka){if(isset($_GET["grant"])){$ka=array_filter($ka);}$ka=array_keys($ka);if(isset($_GET["grant"])){$zc=array_diff(array_keys(array_filter($lc[$ab],'strlen')),$ka);}elseif($Wb==$nb){$Pd=array_keys((array)$kb[$ab]);$zc=array_diff($Pd,$ka);$ka=array_diff($ka,$Pd);unset($kb[$ab]);}if(preg_match('~^(.+)\\s*(\\(.*\\))?$~U',$ab,$l)&&(!grant("REVOKE",$zc,$l[2]," ON $l[1] FROM $nb")||!grant("GRANT",$ka,$l[2]," ON $l[1] TO $nb"))){$n=true;break;}}}if(!$n&&isset($_GET["host"])){if($Wb!=$nb){queries("DROP USER $Wb");}elseif(!isset($_GET["grant"])){foreach($kb
as$ab=>$zc){if(preg_match('~^(.+)(\\(.*\\))?$~U',$ab,$l)){grant("REVOKE",array_keys($zc),$l[2]," ON $l[1] FROM $nb");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$n);if($Wb!=$nb){$g->query("DROP USER $nb");}}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$Dd@$_GET[host]"):'Vytvoriť používateľa'),$n,array("privileges"=>array('','Oprávnenia')));if($_POST){$a=$_POST;$kb=$lc;}else{$a=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$a["pass"]=$ad;if($ad!=""){$a["hashed"]=true;}$kb[DB!=""&&!isset($_GET["host"])?idf_escape($_GET["db"]).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$k=0;foreach($kb
as$ab=>$ka){echo'<th>'.($ab!="*.*"?"<input name='objects[$k]' value='".h($ab)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Yc=>$tc){foreach((array)$ra[$Yc]as$_c=>$xa){echo"<tr".odd()."><td".($tc?">$tc<td":" colspan='2'").' lang="en" title="'.h($xa).'">'.h($_c);$k=0;foreach($kb
as$ab=>$ka){$f="'grants[$k][".h(strtoupper($_c))."]'";$q=$ka[strtoupper($_c)];if($Yc=="Server Admin"&&$ab!=(isset($kb["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($q?" selected":"").">".'Povoliť'."<option value='0'".($q=="0"?" selected":"").">".'Zakázať'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($q?" checked":"").($_c=="All privileges"?" id='grants-$k-all'":($_c=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$k-all');\"")).">";}$k++;}}}echo"</table>\n",'<p>
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$_d=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$_d++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$_d),$_d||!$_POST["kill"]);}page_header('Zoznam procesov',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".($e=="Info"&&$b!=""?"<code class='jush-$u'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Upraviť'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>',($k+1)."/".sprintf('%d celkom',$g->result("SELECT @@max_connections")),'<p>
<input type="submit" value="Ukončiť">
<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["select"])){$m=$_GET["select"];$E=table_status($m);$J=indexes($m);$p=fields($m);$da=column_foreign_keys($m);if($E["Oid"]=="t"){$J[]=array("type"=>"PRIMARY","columns"=>array("oid"));}parse_str($_COOKIE["adminer_import"],$Dc);$he=array();$B=array();$Lb=null;foreach($p
as$e=>$d){$f=$o->fieldName($d);if(isset($d["privileges"]["select"])&&$f!=""){$B[$e]=html_entity_decode(strip_tags($f));if(ereg('text|lob',$d["type"])){$Lb=$o->selectLengthProcess();}}$he+=$d["privileges"];}list($I,$pa)=$o->selectColumnsProcess($B,$J);$s=$o->selectSearchProcess($p,$J);$cb=$o->selectOrderProcess($p,$J);$N=$o->selectLimitProcess();$Zb=($I?implode(", ",$I):($E["Oid"]=="t"?"oid, ":"")."*")."\nFROM ".table($m);$Kd=($pa&&count($pa)<count($I)?"\nGROUP BY ".implode(", ",$pa):"").($cb?"\nORDER BY ".implode(", ",$cb):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Ua=>$a){echo$g->result("SELECT".limit(idf_escape(key($a))." FROM ".table($m)," WHERE ".where_check($Ua).($s?" AND ".implode(" AND ",$s):"").($cb?" ORDER BY ".implode(", ",$cb):""),1));}exit;}if($_POST&&!$n){$Od="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$Ja=$yc=null;foreach($J
as$w){if($w["type"]=="PRIMARY"){$Ja=array_flip($w["columns"]);$yc=($I?$Ja:array());break;}}foreach((array)$yc
as$e=>$b){if(in_array(idf_escape($e),$I)){unset($yc[$e]);}}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($m);$o->dumpTable($m,"");if(!is_array($_POST["check"])||$yc===array()){$fc=$s;if(is_array($_POST["check"])){$fc[]="($Od)";}$i="SELECT $Zb".($fc?"\nWHERE ".implode(" AND ",$fc):"").$Kd;}else{$ce=array();foreach($_POST["check"]as$b){$ce[]="(SELECT".limit($Zb,"\nWHERE ".($s?implode(" AND ",$s)." AND ":"").where_check($b).$Kd,1).")";}$i=implode(" UNION ALL ",$ce);}$o->dumpData($m,"table",$i);exit;}if(!$o->selectEmailProcess($s,$da)){if($_POST["save"]||$_POST["delete"]){$j=true;$sb=0;$i=table($m);$r=array();if(!$_POST["delete"]){foreach($B
as$f=>$b){$b=process_input($p[$f]);if($b!==null){if($_POST["clone"]){$r[idf_escape($f)]=($b!==false?$b:idf_escape($f));}elseif($b!==false){$r[]=idf_escape($f)." = $b";}}}$i.=($_POST["clone"]?" (".implode(", ",array_keys($r)).")\nSELECT ".implode(", ",$r)."\nFROM ".table($m):" SET\n".implode(",\n",$r));}if($_POST["delete"]||$r){$Pc="UPDATE";if($_POST["delete"]){$Pc="DELETE";$i="FROM $i";}if($_POST["clone"]){$Pc="INSERT";$i="INTO $i";}if($_POST["all"]||($yc===array()&&$_POST["check"])||count($pa)<count($I)){$j=queries($Pc." $i".($_POST["all"]?($s?"\nWHERE ".implode(" AND ",$s):""):"\nWHERE $Od"));$sb=$g->affected_rows;}else{foreach((array)$_POST["check"]as$b){$j=queries($Pc.limit1($i,"\nWHERE ".where_check($b)));if(!$j){break;}$sb+=$g->affected_rows;}}}queries_redirect(remove_from_uri("page"),sprintf('%d položiek bolo ovplyvnených.',$sb),$j);}elseif(!$_POST["import"]){if(!$_POST["val"]){$n='Dvojkliknite na políčko, ktoré chcete zmeniť.';}else{$j=true;$sb=0;foreach($_POST["val"]as$Ua=>$a){$r=array();foreach($a
as$e=>$b){$e=bracket_escape($e,1);$r[]=idf_escape($e)." = ".(ereg('char|text',$p[$e]["type"])||$b!=""?$o->processInput($p[$e],$b):"NULL");}$i=table($m)." SET ".implode(", ",$r);$fc=" WHERE ".where_check($Ua).($s?" AND ".implode(" AND ",$s):"");$j=queries("UPDATE".(count($pa)<count($I)?" $i$fc":limit1($i,$fc)));if(!$j){break;}$sb+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$sb),$j);}}elseif(is_string($Da=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($Dc["output"])."&format=".urlencode($_POST["separator"]));$j=true;$hb=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Da,$sa);$sb=count($sa[0]);begin();$Za=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($sa[0]as$e=>$b){preg_match_all("~((\"[^\"]*\")+|[^$Za]*)$Za~",$b.$Za,$sd);if(!$e&&!array_diff($sd[1],$hb)){$hb=$sd[1];$sb--;}else{$r=array();foreach($sd[1]as$k=>$Nc){$r[idf_escape($hb[$k])]=($Nc==""&&$p[$hb[$k]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Nc))));}$j=insert_update($m,$r,$Ja);if(!$j){break;}}}if($j){queries("COMMIT");}queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$sb),$j);queries("ROLLBACK");}else{$n=upload_error($Da);}}}$Ea=$o->tableName($E);page_header('Vypísať'.": $Ea",$n);session_write_close();$r=null;if(isset($he["insert"])){$r="";foreach((array)$_GET["where"]as$b){if(count($da[$b["col"]])==1&&($b["op"]=="="||(!$b["op"]&&!ereg('[_%]',$b["val"])))){$r.="&set".urlencode("[".bracket_escape($b["col"])."]")."=".urlencode($b["val"]);}}}$o->selectLinks($E,$r);if(!$B){echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($p?".":": ".error())."\n";}else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($m).'">',"</div>\n";$o->selectColumnsPrint($I,$B);$o->selectSearchPrint($s,$B,$J);$o->selectOrderPrint($cb,$B,$J);$o->selectLimitPrint($N);$o->selectLengthPrint($Lb);$o->selectActionPrint();echo"</form>\n";$Y=$_GET["page"];if($Y=="last"){$Wa=$g->result("SELECT COUNT(*) FROM ".table($m).($s?" WHERE ".implode(" AND ",$s):""));$Y=floor(max(0,$Wa-1)/$N);}$i="SELECT".limit((+$N&&$pa&&count($pa)<count($I)&&$u=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Zb,($s?"\nWHERE ".implode(" AND ",$s):"").$Kd,($N!=""?+$N:null),($Y?$N*$Y:0),"\n");echo$o->selectQuery($i);$j=$g->query($i);if(!$j){echo"<p class='error'>".error()."\n";}else{if($u=="mssql"){$j->seek($N*$Y);}$Xc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$H=array();while($a=$j->fetch_assoc()){if($Y&&$u=="oracle"){unset($a["RNUM"]);}$H[]=$a;}if($_GET["page"]!="last"){$Wa=(+$N&&$pa&&count($pa)<count($I)?($u=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($i) x")):count($H));}if(!$H){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{$jd=$o->backwardKeys($m,$Ea);echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$pa&&$I?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$hd=array();$aa=array();reset($I);$Qd=1;foreach($H[0]as$e=>$b){if($E["Oid"]!="t"||$e!="oid"){$b=$_GET["columns"][key($I)];$d=$p[$I?($b?$b["col"]:current($I)):$e];$f=($d?$o->fieldName($d,$Qd):"*");if($f!=""){$Qd++;$hd[$e]=$f;$C=idf_escape($e);echo'<th><a href="'.h(remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($e).($cb[0]==$C||$cb[0]==$e||(!$cb&&count($pa)<count($I)&&$pa[0]==$C)?'&desc%5B0%5D=1':'')).'">'.(!$I||$b?apply_sql_function($b["fun"],$f):h(current($I)))."</a>";}$aa[$e]=$b["fun"];next($I);}}$ac=array();if($_GET["modify"]){foreach($H
as$a){foreach($a
as$e=>$b){$ac[$e]=max($ac[$e],min(40,strlen(utf8_decode($b))));}}}echo($jd?"<th>".'Vzťahy':"")."</thead>\n";foreach($o->rowDescriptions($H,$da)as$fa=>$a){$Ld=unique_array($H[$fa],$J);$Ua="";foreach($Ld
as$e=>$b){$Ua.="&".(isset($b)?urlencode("where[".bracket_escape($e)."]")."=".urlencode($b):"null%5B%5D=".urlencode($e));}echo"<tr".odd().">".(!$pa&&$I?"":"<td>".checkbox("check[]",substr($Ua,1),in_array(substr($Ua,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($pa)<count($I)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($m).$Ua)."'>".'upraviť'."</a>"));foreach($a
as$e=>$b){if(isset($hd[$e])){$d=$p[$e];if($b!=""&&(!isset($Xc[$e])||$Xc[$e]!="")){$Xc[$e]=(is_mail($b)?$hd[$e]:"");}$x="";$b=$o->editVal($b,$d);if(!isset($b)){$b="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$d["type"])&&$b!=""){$x=h(ME.'download='.urlencode($m).'&field='.urlencode($e).$Ua);}if($b===""){$b="&nbsp;";}elseif($Lb!=""&&ereg('text|blob',$d["type"])&&is_utf8($b)){$b=shorten_utf8($b,max(0,+$Lb));}else{$b=h($b);}if(!$x){foreach((array)$da[$e]as$_){if(count($da[$e])==1||end($_["source"])==$e){$x="";foreach($_["source"]as$k=>$Ha){$x.=where_link($k,$_["target"][$k],$H[$fa][$Ha]);}$x=h(($_["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($_["db"]),ME):ME).'select='.urlencode($_["table"]).$x);if(count($_["source"])==1){break;}}}}if($e=="COUNT(*)"){$x=h(ME."select=".urlencode($m));$k=0;foreach((array)$_GET["where"]as$y){if(!array_key_exists($y["col"],$Ld)){$x.=h(where_link($k++,$y["col"],$y["val"],$y["op"]));}}foreach($Ld
as$Oa=>$y){$x.=h(where_link($k++,$Oa,$y));}}}if(!$x){if(is_mail($b)){$x="mailto:$b";}if($fe=is_url($a[$e])){$x=($fe=="http"&&$uc?$a[$e]:"$fe://www.adminer.org/redirect/?url=".urlencode($a[$e]));}}$S=h("val[$Ua][".bracket_escape($e)."]");$q=$_POST["val"][$Ua][bracket_escape($e)];$Xd=h(isset($q)?$q:$a[$e]);$sf=strpos($b,"<i>...</i>");$Rd=is_utf8($b)&&$H[$fa][$e]==$a[$e]&&!$aa[$e];$Sd=ereg('text|lob',$d["type"]);echo(($_GET["modify"]&&$Rd)||isset($q)?"<td>".($Sd?"<textarea name='$S' cols='30' rows='".(substr_count($a[$e],"\n")+1)."'>$Xd</textarea>":"<input name='$S' value='$Xd' size='$ac[$e]'>"):"<td id='$S' ondblclick=\"".($Rd?"selectDblClick(this, event".($sf?", 2":($Sd?", 1":"")).")":"alert('".h('Pre zmenu tejto hodnoty použite odkaz upraviť.')."')").";\">".$o->selectVal($b,$x,$d));}}if($jd){echo"<td>";}$o->backwardKeysPrint($jd,$H[$fa]);echo"</tr>\n";}echo"</table>\n";}if($H||$Y){$Ed=true;if($_GET["page"]!="last"&&+$N&&count($pa)>=count($I)&&($Wa>=$N||$Y)){$Wa=$E["Rows"];if(!isset($Wa)||$s||($E["Engine"]=="InnoDB"&&$Wa<max(1e4,2*($Y+1)*$N))){ob_flush();flush();$Wa=$g->result("SELECT COUNT(*) FROM ".table($m).($s?" WHERE ".implode(" AND ",$s):""));}else{$Ed=false;}}echo"<p class='pages'>";if(+$N&&$Wa>$N){$wd=floor(($Wa-1)/$N);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($Y+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$Y).($Y>5?" ...":"");for($k=max(1,$Y-4);$k<min($wd,$Y+5);$k++){echo
pagination($k,$Y);}echo($Y+5<$wd?" ...":"").($Ed?pagination($wd,$Y):' <a href="'.h(remove_from_uri()."&page=last").'">'.'posledný'."</a>");}echo" (".($Ed?"":"~ ").lang(array('%d riadok','%d riadky','%d riadkov'),$Wa).") ".checkbox("all",1,0,'celý výsledok')."\n";if($o->selectCommandPrint()){?>
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$Wa,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Export');$Na=$o->dumpOutput();echo($Na?html_select("output",$Na,$Dc["output"])." ":""),html_select("format",$o->dumpFormat(),$Dc["format"])," <input type='submit' name='export' value='".'Export'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($o->selectImportPrint()){print_fieldset("import",'Import',!$H);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Dc["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$O'>\n","</div></fieldset>\n";}$o->selectEmailPrint(array_filter($Xc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Yb=isset($_GET["status"]);page_header($Yb?'Stav':'Premenné');$Yd=($Yb?show_status():show_variables());if(!$Yd){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Yd
as$e=>$b){echo"<tr>","<th><code class='jush-".$u.($Yb?"status":"set")."'>".h($e)."</code>","<td>".nbsp($b);}echo"</table>\n";}}elseif(isset($_GET["script"])){header("Content-Type: text/javascript; charset=utf-8");if($_GET["script"]=="db"){$Lc=array("Data_length"=>0,"Index_length"=>0,"Data_free"=>0);foreach(table_status()as$a){$S=js_escape($a["Name"]);json_row("Comment-$S",nbsp($a["Comment"]));if(!is_view($a)){foreach(array("Engine","Collation")as$e){json_row("$e-$S",nbsp($a[$e]));}foreach($Lc+array("Auto_increment"=>0,"Rows"=>0)as$e=>$b){if($a[$e]!=""){$b=number_format($a[$e],0,'.',' ');json_row("$e-$S",($e=="Rows"&&$a["Engine"]=="InnoDB"&&$b?"~ $b":$b));if(isset($Lc[$e])){$Lc[$e]+=($a["Engine"]!="InnoDB"||$e!="Data_free"?$a[$e]:0);}}elseif(array_key_exists($e,$a)){json_row("$e-$S");}}}}foreach($Lc
as$e=>$b){json_row("sum-$e",number_format($b,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$t=>$b){json_row("tables-".js_escape($t),$b);}json_row("");}exit;}else{$se=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($se&&!$n&&!$_POST["search"]){$j=true;$ua="";if($u=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$j=truncate_tables($_POST["tables"]);}$ua='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$j=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ua='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$j=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ua='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"]){$j=drop_views($_POST["views"]);}if($j&&$_POST["tables"]){$j=drop_tables($_POST["tables"]);}$ua='Tabuľka bola odstránená.';}elseif($_POST["tables"]&&($j=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$j->fetch_assoc()){$ua.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ua,$j);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$n,true);if($o->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$Sc=tables_list();if(!$Sc){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabuľka'.'<td>'.'Typ'.'<td>'.'Porovnávanie'.'<td>'.'Veľkosť dát'.'<td>'.'Veľkosť indexu'.'<td>'.'Voľné miesto'.'<td>'.'Auto Increment'.'<td>'.'Riadky'.(support("comment")?'<td>'.'Komentár':'')."</thead>\n";foreach($Sc
as$f=>$z){$Xb=(isset($z)&&!eregi("table",$z));echo'<tr'.odd().'><td>'.checkbox(($Xb?"views[]":"tables[]"),$f,in_array($f,$se,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($Xb){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{foreach(array("Engine"=>"","Collation"=>"","Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$e=>$x){echo($x?"<td align='right'><a href='".h(ME."$x=").urlencode($f)."' id='$e-".h($f)."'>?</a>":"<td id='$e-".h($f)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($Sc)),"<td>".nbsp($u=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$e){echo"<td align='right' id='sum-$e'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p>".($u=="sql"?"<input type='submit' value='".'Analyzovať'."'> <input type='submit' name='optimize' value='".'Optimalizovať'."'> <input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$A=(support("scheme")?schemas():get_databases());if(count($A)!=1&&$u!="sqlite"){$t=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($A?html_select("target",$A,$t):'<input name="target" value="'.h($t).'">')," <input type='submit' name='move' value='".'Presunúť'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$O'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";}if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$le=routines();if($le){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($le
