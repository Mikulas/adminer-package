...
confirm($cb=""){return" onclick=\"return confirm('".'Naozaj?'.($cb?" (' + $cb + ')":"")."');\"";}function
...
upload_error($l){$kd=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Súbor sa nepodarilo nahrať.'.($kd?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$kd):""):'Súbor neexistuje.');}function
...
enum_input($S,$ya,$m,$X,$Eb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$m["length"],$fd);$G=($Eb!==null?"<label><input type='$S'$ya value='$Eb'".((is_array($X)?in_array($Eb,$X):$X===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($fd[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ma=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$ya value='".($q+1)."'".($Ma?' checked':'').'>'.h($b->editVal($W,$m)).'</label>';}return$G;}function
input($m,$X,$p){global$T,$b,$v;$A=h(bracket_escape($m["field"]));echo"<td class='function'>";$Le=($v=="mssql"&&$m["auto_increment"]);if($Le&&!$_POST["save"])$p=null;$oc=(isset($_GET["select"])||$Le?array("orig"=>'originál'):array())+$b->editFunctions($m);$ya=" name='fields[$A]'";if($m["type"]=="enum")echo
...
lang($Nf,$Ad){$ne=($Ad==1?0:($Ad&&$Ad<5?1:2));$Nf=str_replace("%d","%s",$Nf[$ne]);$Ad=number_format($Ad,0,".",' ');return
...
q($W);}function
check_sqlite_name($A){global$h;$Xb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Xb)\$~",$A)){$h->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$Xb));return
false;}return
true;}function
create_database($k,$d){global$h;if(file_exists($k)){$h->error='Súbor existuje.';return
...
as$k){if(!@unlink($k)){$h->error='Súbor existuje.';return
false;}}return
true;}function
rename_database($A,$d){global$h;if(!check_sqlite_name($A))return
false;$h->Min_SQLite(":memory:");$h->error='Súbor existuje.';return@rename(DB,$A);}function
...
set_schema($Ve){global$h,$T,$mf;$G=$h->query("SET search_path TO ".idf_escape($Ve));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$mf['Užívateľské typy'][]=$S;}}return$G;}function
...
reset($I);}function
support($Zb){return
ereg("view|scheme|processlist|drop_col|variables|status",$Zb);}$v="oracle";$T=array();$mf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$mf[$w]=array_keys($W);}$cg=array();$Ld=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$oc=array("length","lower","round","upper");$tc=array("avg","count","count distinct","max","min","sum");$_b=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ub["mssql"]="MS SQL";if(isset($_GET["mssql"])){$qe=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Používateľ<td><input id="username" name="auth[username]" value="',h($_GET["username"]),'">
<tr><th>Heslo<td><input type="password" name="auth[password]">
<tr><th>Databáza<td><input name="auth[db]" value="',h($_GET["db"]);?>">
</table>
...
selectLinks($tf,$L=""){echo'<p class="tabs">';$ad=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($tf))$ad["view"]='Zmeniť pohľad';else$ad["create"]='Zmeniť tabuľku';if($L!==null)$ad["edit"]='Nová položka';foreach($ad
...
selectQuery($E){global$v;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$v'>".h(str_replace("\n"," ",$E))."</code> <a href='".h(ME)."sql=".urlencode($E)."'>".'Upraviť'."</a></p>\n";}function
rowDescription($O){return"";}function
rowDescriptions($I,$gc){return$I;}function
selectVal($W,$z,$m){$G=($W===null?"<i>NULL</i>":(ereg("char|binary",$m["type"])&&!ereg("var",$m["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$m["type"])&&!is_utf8($W))$G=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W));return($z?"<a href='$z'>$G</a>":$G);}function
editVal($W,$m){return(ereg("binary",$m["type"])?reset(unpack("H*",$W)):$W);}function
selectColumnsPrint($J,$g){global$oc,$tc;print_fieldset("select",'Vypísať',$J);$q=0;$nc=array('Funkcie'=>$oc,'Agregácia'=>$tc);foreach($J
...
selectLengthPrint($Df){if($Df!==null){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($Df).'">',"</div></fieldset>\n";}}function
selectActionPrint($u){echo"<fieldset><legend>".'Akcia'."</legend><div>","<input type='submit' value='".'Vypísať'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script type='text/javascript'>\n","var indexColumns = ";$g=array();foreach($u
...
false;}function
messageQuery($E){global$v;static$cb=0;restart_session();$r="sql-".($cb++);$vc=&get_session("queries");if(strlen($E)>1e6)$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";$vc[$_GET["db"]][]=array($E,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL príkaz'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($vc[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$G);}function
editInput($O,$m,$ya,$X){if($m["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$ya value='-1' checked><i>".'originál'."</i></label> ":"").($m["null"]?"<label><input type='radio'$ya value=''".($X!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ya,$m,$X,0);return"";}function
...
dumpOutput(){$G=array('text'=>'otvoriť','file'=>'uložiť');if(function_exists('gzencode'))$G['gz']='gzip';if(function_exists('bzcompress'))$G['bz2']='bzip2';return$G;}function
dumpFormat(){return
...
<p class="logout">
';if(DB==""||!$td){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL príkaz'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}echo'<input type="submit" name="logout" value="Odhlásiť">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
<form action="">
<p style="overflow: hidden;">
';hidden_fields_get();echo($j?html_select("db",array(""=>"(".'databáza'.")")+$j,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrať"',($j?" class='hidden'":""),'>
';if($td!="db"&&DB!=""&&$h->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':""))),"</p></form>\n";if($_GET["ns"]!==""&&!$td){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$Q=tables_list();if(!$Q)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{$this->tablesPrint($Q);$ad=array();foreach($Q
...
as$O=>$S){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O).">".'vypísať'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold($_GET["table"]==$O)." title='".'Zobraziť štruktúru'."'>".$this->tableName(array("Name"=>$O))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
auth_error($Pb=null){global$h,$b,$R;$ef=session_name();$l="";if(!$_COOKIE[$ef]&&$_GET[$ef]&&ini_bool("session.use_only_cookies"))$l='Session premenné musia byť povolené.';elseif(isset($_GET["username"])){if(($_COOKIE[$ef]||$_GET[$ef])&&!$R)$l='Session vypršala, prihláste sa prosím znova.';else{$C=&get_session("pwds");if($C!==null){$l=h($Pb?$Pb->getMessage():(is_string($h)?$h:'Neplatné prihlasovacie údaje.'));$C=null;}}}page_header('Prihlásiť sa',$l,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$qe)),false);page_footer("auth");exit;}$h=connect();}if(is_string($h)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$R=$_SESSION["token"];if($za&&$_POST["token"])$_POST["token"]=$R;$l=($_POST?($_POST["token"]==$R?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));function
connect_error(){global$b,$h,$R,$l,$ub;$j=array();if(DB!="")page_header('Databáza'.": ".h(DB),'Nesprávna databáza.',true);else{if($_POST["db"]&&!$l)queries_redirect(substr(ME,0,-1),'Databázy boli odstránené.',drop_databases($_POST["db"]));page_header('Vybrať databázu',$l,false);echo"<p><a href='".h(ME)."database='>".'Vytvoriť novú databázu'."</a>\n";foreach(array('privileges'=>'Oprávnenia','processlist'=>'Zoznam procesov','variables'=>'Premenné','status'=>'Stav',)as$w=>$W){if(support($w))echo"<a href='".h(ME)."$w='>$W</a>\n";}echo"<p>".sprintf('Verzia %s: %s cez PHP rozšírenie %s',$ub[DRIVER],"<b>$h->server_info</b>","<b>$h->extension</b>")."\n","<p>".sprintf('Prihlásený ako: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"])set_session("dbs",null);$j=$b->databases();if($j){$We=support("scheme");$e=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);'>\n","<thead><tr><td>&nbsp;<th>".'Databáza'."<td>".'Porovnávanie'."<td>".'Tabuľky'."</thead>\n";foreach($j
as$k){$Oe=h(ME)."db=".urlencode($k);echo"<tr".odd()."><td>".checkbox("db[]",$k,in_array($k,(array)$_POST["db"])),"<th><a href='$Oe'>".h($k)."</a>","<td><a href='$Oe".($We?"&amp;ns=":"")."&amp;database=' title='".'Zmeniť databázu'."'>".nbsp(db_collation($k,$e))."</a>","<td align='right'><a href='$Oe&amp;schema=' id='tables-".h($k)."' title='".'Schéma databázy'."'>?</a>","\n";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","<p><input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /db/)").">\n","<input type='hidden' name='token' value='$R'>\n","<a href='".h(ME)."refresh=1'>".'Obnoviť'."</a>\n","</form>\n";}}page_footer("db");if($j)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(!(DB!=""?$h->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect")){if(DB!="")set_session("dbs",null);connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){page_header('Schéma'.": ".h($_GET["ns"]),'Neplatné schéma.',true);page_footer("ns");exit;}}function
...
as$w=>$W){if($W===null)$W="<i>NULL</i>";elseif($Fa[$w]&&!is_utf8($W))$W="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$w]==254)$W="<code>$W</code>";}if(isset($ad[$w])&&!$g[$ad[$w]]){if($yc){$O=$H[array_search("table=",$ad)];$z=$ad[$w].urlencode($Sd[$O]!=""?$Sd[$O]:$O);}else{$z="edit=".urlencode($ad[$w]);foreach($u[$ad[$w]]as$Pa=>$Lc)$z.="&where".urlencode("[".bracket_escape($Pa)."]")."=".urlencode($H[$Lc]);}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";return$G;}function
...
edit_type($w,$m,$e,$hc=array()){global$mf,$T,$cg,$Hd;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$m["type"]||isset($T[$m["type"]])?array():array($m["type"]))+$mf+($hc?array('Cudzie kľúče'=>$hc):array()),$m["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$m["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($e,$m["collation"]).'</select>',($cg?"<select name='$w"."[unsigned]'".(!$m["type"]||ereg('(int|float|double|decimal)$',$m["type"])?"":" class='hidden'").'><option>'.optionlist($cg,$m["unsigned"]).'</select>':''),($hc?"<select name='$w"."[on_delete]'".(ereg("`",$m["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Hd),$m["on_delete"])."</select> ":" ");}function
...
';if($S=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($S=="TABLE"?'Názov stĺpca':'Názov parametra'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Dĺžka
<td>Voľby
...
',(support("comment")?"<td".($Va?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($Va?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $wa, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Td==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$A=>$t){ksort($t["columns"]);$ve=array();foreach($t["columns"]as$w=>$W)$ve[]="<i>".h($W)."</i>".($t["lengths"][$w]?"(".$t["lengths"][$w].")":"");echo"<tr title='".h($A)."'><th>$t[type]<td>".implode(", ",$ve)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($a).'">'.'Zmeniť indexy'."</a>\n";if(fk_support($P)){echo"<h3>".'Cudzie kľúče'."</h3>\n";$hc=foreign_keys($a);if($hc){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'Cieľ'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($v!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($hc
as$A=>$o){echo"<tr title='".h($A)."'>","<th><i>".implode("</i>, <i>",array_map('h',$o["source"]))."</i>","<td><a href='".h($o["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($o["db"]),ME):($o["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($o["ns"]),ME):ME))."table=".urlencode($o["table"])."'>".($o["db"]!=""?"<b>".h($o["db"])."</b>.":"").($o["ns"]!=""?"<b>".h($o["ns"])."</b>.":"").h($o["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$o["target"]))."</i>)","<td>".nbsp($o["on_delete"])."\n","<td>".nbsp($o["on_update"])."\n",($v=="sqlite"?"":'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($A)).'">'.'Zmeniť'.'</a>');}echo"</table>\n";}if($v!="sqlite")echo'<p><a href="'.h(ME).'foreign='.urlencode($a).'">'.'Pridať cudzí kľúč'."</a>\n";}if(support("trigger")){echo"<h3>".'Triggery'."</h3>\n";$Tf=triggers($a);if($Tf){echo"<table cellspacing='0'>\n";foreach($Tf
as$w=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Zmeniť'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$wf=array();$xf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$fd,PREG_SET_ORDER);foreach($fd
...
<p><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">Permanentný odkaz</a>
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Wa=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W))$Wa[]=".$w";}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($Wa?"[".implode("|",$Wa)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($vc){print_fieldset("history",'História',$_GET["history"]!="");foreach($vc
as$w=>$W){list($D,$Ef)=$W;echo'<a href="'.h(ME."sql=&history=$w").'">'.'Upraviť'."</a> <span class='time'>".@date("H:i:s",$Ef)."</span> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$D)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$A=>$m){if(!isset($m["privileges"][$dg?"update":"insert"])||$b->fieldName($m)=="")unset($n[$A]);}if($_POST&&!$l&&!isset($_GET["select"])){$bd=$_POST["referer"];if($_POST["insert"])$bd=($dg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$bd))$bd=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$bd,'Položka bola vymazaná.');else{$L=array();foreach($n
as$A=>$m){$W=process_input($m);if($W!==false&&$W!==null)$L[idf_escape($A)]=($dg?"\n".idf_escape($A)." = $W":$W);}if($dg){if(!$L)redirect($bd);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$bd,'Položka bola aktualizovaná.');}else{$F=insert_into($a,$L);$Tc=($F?last_id():0);queries_redirect($bd,sprintf('Položka%s bola vložená.',($Tc?" $Tc":"")),$F);}}}$uf=$b->tableName(table_status($a));page_header(($dg?'Upraviť':'Vložiť'),$l,array("select"=>array($a,$uf)),$uf);$H=null;if($_POST["save"])$H=(array)$_POST["fields"];elseif($Z){$J=array();foreach($n
as$A=>$m){if(isset($m["privileges"]["select"]))$J[]=($_POST["clone"]&&$m["auto_increment"]?"'' AS ":($v=="sql"&&ereg("enum|set",$m["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false)echo"<p class='error'>".'Žiadne riadky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($n){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($dg?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($dg?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$n?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Va,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($H["Comment"]).'" maxlength="60"'.($Va?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
';if(support("partitioning")){$ge=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$fe,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($ge||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$l&&!isset($_POST["add_x"])){restart_session();$A=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Databáza bola premenovaná.',rename_database($A,$_POST["collation"]));}else{$j=explode("\n",str_replace("\r","",$A));$nf=true;$Sc="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$_POST["collation"]))$nf=false;$Sc=$k;}}queries_redirect(ME."db=".urlencode($Sc),'Databáza bola vytvorená.',$nf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$l,array(),DB);$e=collations();$A=DB;$Qa=null;if($_POST){$A=$_POST["name"];$Qa=$_POST["collation"];}elseif(DB!="")$Qa=db_collation(DB,$e);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$pc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$pc,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input id="name" name="name" value="'.h($A).'" maxlength="64">')."\n".($e?html_select("collation",array(""=>"(".'porovnávanie'.")")+$e,$Qa):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložiť">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Pridať ďalší'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$l){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'Schéma bola odstránená.');else{$A=trim($_POST["name"]);$z.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$z,'Schéma bola vytvorená.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$z,'Schéma bola zmenená.');else
redirect($z);}}page_header($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému',$l);$H=$_POST;if(!$H)$H=array("name"=>$_GET["ns"]);echo'
<form action="" method="post">
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$wb=false;if($_POST&&!$l){$A=trim($_POST["name"]);$wb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($A)),'Pohľad bol odstránený.','Pohľad bol zmenený.','Pohľad bol vytvorený.',$a);}page_header(($a!=""?'Zmeniť pohľad':'Vytvoriť pohľad'),$l,array("table"=>$a),$a);$H=$_POST;if(!$H&&$a!=""){$H=view($a);$H["name"]=$a;}echo'
<form action="" method="post">
<p>Názov: <input name="name" value="',h($H["name"]),'" maxlength="64">
<p>';textarea("select",$H["select"]);echo'<p>
';if($wb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Ic=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$kf=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$l){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Udalosť bola odstránená.');elseif(in_array($_POST["INTERVAL_FIELD"],$Ic)&&isset($kf[$_POST["STATUS"]])){$Ue="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Ue.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Ue)."\n".$kf[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Upraviť udalosť'.": ".h($aa):'Vytvoriť udalosť'),$l);$H=$_POST;if(!$H&&$aa!=""){$I=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$H=reset($I);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>Názov<td><input name="EVENT_NAME" value="',h($H["EVENT_NAME"]),'" maxlength="64">
<tr><th>Začiatok<td><input name="STARTS" value="',h("$H[EXECUTE_AT]$H[STARTS]"),'">
<tr><th>Koniec<td><input name="ENDS" value="',h($H["ENDS"]),'">
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ic,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$kf,$H["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($aa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$m){if($m["field"]!="")$L[]=(ereg("^($Fc)\$",$m["inout"])?"$m[inout] ":"").idf_escape($m["field"]).process_type($m,"CHARACTER SET");}$wb=drop_create("DROP $Pe ".idf_escape($da),"CREATE $Pe ".idf_escape(trim($_POST["name"]))." (".implode(", ",$L).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$Qe)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$da);}page_header(($da!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($da):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$l);$e=get_vals("SHOW CHARACTER SET");sort($e);$H=array("fields"=>array());if($_POST){$H=$_POST;$H["fields"]=(array)$H["fields"];process_fields($H["fields"]);}elseif($da!=""){$H=routine($da,$Pe);$H["name"]=$da;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($H["name"]),'" maxlength="64">
',($Qe?'Jazyk'.": ".html_select("language",$Qe,$H["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($H["fields"],$e,$Pe);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$H["returns"],$e);}echo'</table>
<p>';textarea("definition",$H["definition"]);echo'<p>
<input type="submit" value="Uložiť">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($wb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$l){$z=substr(ME,0,-1);$A=trim($_POST["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekvencia bola odstránená.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$z,'Sekvencia bola vytvorená.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$z,'Sekvencia bola zmenená.');else
redirect($z);}page_header($fa!=""?'Pozmeniť sekvenciu'.": ".h($fa):'Vytvoriť sekvenciu',$l);$H=$_POST;if(!$H)$H=array("name"=>$fa);echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Uložiť">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$l){$z=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ bol odstránený.');else
query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$z,'Typ bol vytvorený.');}page_header($ga!=""?'Pozmeniť typ'.": ".h($ga):'Vytvoriť typ',$l);$H=$_POST;if(!$H)$H=array("as"=>"AS ");echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";else{echo"<input name='name' value='".h($H['name'])."'>\n";textarea("as",$H["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$Sf=trigger_options();$Qf=array("INSERT","UPDATE","DELETE");$wb=false;if($_POST&&!$l&&in_array($_POST["Timing"],$Sf["Timing"])&&in_array($_POST["Event"],$Qf)&&in_array($_POST["Type"],$Sf["Type"])){$Ff=" $_POST[Timing] $_POST[Event]";$Gd=" ON ".table($a);$wb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($v=="pgsql"?$Gd:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($v=="mssql"?$Gd.$Ff:$Ff.$Gd).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$l,array("table"=>$a));$H=$_POST;if(!$H)$H=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Sf["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$Qf,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Sf["Type"],$H["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($wb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$W){if($W[1]!="USAGE")$qc["$_[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$H[0]))$qc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_))$Ed=$_[1];}}if($_POST&&!$l){$Fd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$zd=q($_POST["user"])."@".q($_POST["host"]);$je=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $Fd",ME."privileges=",'Používateľ bol odstránený.');else{$fb=false;if($Fd!=$zd){$fb=queries(($h->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $zd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $je");$l=!$fb;}elseif($_POST["pass"]!=$Ed||!$_POST["hashed"])queries("SET PASSWORD FOR $zd = ".($_POST["hashed"]?$je:"PASSWORD($je)"));if(!$l){$Me=array();foreach($yd
...
as$Bd=>$Me){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Bd,$_))grant("REVOKE",array_keys($Me),$_[2]," ON $_[1] FROM $zd");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$l);if($fb)$h->query("DROP USER $zd");}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$ha@$_GET[host]"):'Vytvoriť používateľa'),$l,array("privileges"=>array('','Oprávnenia')));if($_POST){$H=$_POST;$qc=$yd;}else{$H=$_GET+array("host"=>$h->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$Ed;if($Ed!="")$H["hashed"]=true;$qc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>Server<td><input name="host" maxlength="60" value="',h($H["host"]),'">
<tr><th>Používateľ<td><input name="user" maxlength="16" value="',h($H["user"]),'">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$q=0;foreach($qc
as$Bd=>$pc){echo'<th>'.($Bd!="*.*"?"<input name='objects[$q]' value='".h($Bd)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$ab=>$pb){foreach((array)$ye[$ab]as$xe=>$Ua){echo"<tr".odd()."><td".($pb?">$pb<td":" colspan='2'").' lang="en" title="'.h($Ua).'">'.h($xe);$q=0;foreach($qc
as$Bd=>$pc){$A="'grants[$q][".h(strtoupper($xe))."]'";$X=$pc[strtoupper($xe)];if($ab=="Server Admin"&&$Bd!=(isset($qc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Povoliť'."<option value='0'".($X=="0"?" selected":"").">".'Zakázať'."</select>";else
...
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$Pc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$Pc++;}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$Pc),$Pc||!$_POST["kill"]);}page_header('Zoznam procesov',$l);echo'
<form action="" method="post">
...
as$w=>$W)echo"<td>".(($v=="sql"&&$w=="Info"&&ereg("Query|Killed",$H["Command"])&&$W!="")||($v=="pgsql"&&$w=="current_query"&&$W!="<IDLE>")||($v=="oracle"&&$w=="sql_text"&&$W!="")?"<code class='jush-$v'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($H["db"]!=""?"db=".urlencode($H["db"])."&":"")."sql=".urlencode($W)).'">'.'Upraviť'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($q+1)."/".sprintf('%d celkom',$h->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Ukončiť'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$A=>$W){$W=process_input($n[$A]);if($W!==null){if($_POST["clone"])$L[idf_escape($A)]=($W!==false?$W:idf_escape($A));elseif($W!==false)$L[]=idf_escape($A)." = $W";}}$E.=($_POST["clone"]?" (".implode(", ",array_keys($L)).")\nSELECT ".implode(", ",$L)."\nFROM ".table($a):" SET\n".implode(",\n",$L));}if($_POST["delete"]||$L){$Sa="UPDATE";if($_POST["delete"]){$Sa="DELETE";$E="FROM $E";}if($_POST["clone"]){$Sa="INSERT";$E="INTO $E";}if($_POST["all"]||($bg===array()&&$_POST["check"])||count($rc)<count($J)){$F=queries("$Sa $E".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $pg"));$ra=$h->affected_rows;}else{foreach((array)$_POST["check"]as$W){$F=queries($Sa.limit1($E,"\nWHERE ".where_check($W)));if(!$F)break;$ra+=$h->affected_rows;}}}$md=sprintf('%d položiek bolo ovplyvnených.',$ra);if($_POST["clone"]&&$F&&$ra==1){$Tc=last_id();if($Tc)$md=sprintf('Položka%s bola vložená.'," $Tc");}queries_redirect(remove_from_uri("page"),$md,$F);}elseif(!$_POST["import"]){if(!$_POST["val"])$l='Dvojkliknite na políčko, ktoré chcete zmeniť.';else{$F=true;$ra=0;foreach($_POST["val"]as$Zf=>$H){$L=array();foreach($H
as$w=>$W){$w=bracket_escape($w,1);$L[]=idf_escape($w)." = ".(ereg('char|text',$n[$w]["type"])||$W!=""?$b->processInput($n[$w],$W):"NULL");}$E=table($a)." SET ".implode(", ",$L);$og=" WHERE ".where_check($Zf).($Z?" AND ".implode(" AND ",$Z):"");$F=queries("UPDATE".(count($rc)<count($J)?" $E$og":limit1($E,$og)));if(!$F)break;$ra+=$h->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$ra),$F);}}elseif(is_string($ac=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($qa["output"])."&format=".urlencode($_POST["separator"]));$F=true;$Ra=array_keys($n);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$ac,$fd);$ra=count($fd[0]);begin();$af=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($fd[0]as$w=>$W){preg_match_all("~((\"[^\"]*\")+|[^$af]*)$af~",$W.$af,$gd);if(!$w&&!array_diff($gd[1],$Ra)){$Ra=$gd[1];$ra--;}else{$L=array();foreach($gd[1]as$q=>$Pa)$L[idf_escape($Ra[$q])]=($Pa==""&&$n[$Ra[$q]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Pa))));$F=insert_update($a,$L,$te);if(!$F)break;}}if($F)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$ra),$F);queries("ROLLBACK");}else$l=upload_error($ac);}}$uf=$b->tableName($P);page_header('Vypísať'.": $uf",$l);session_write_close();$L=null;if(isset($Ne["insert"])){$L="";foreach((array)$_GET["where"]as$W){if(count($hc[$W["col"]])==1&&($W["op"]=="="||(!$W["op"]&&!ereg('[_%]',$W["val"]))))$L.="&set".urlencode("[".bracket_escape($W["col"])."]")."=".urlencode($W["val"]);}}$b->selectLinks($P,$L);if(!$g)echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($n?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($J,$g);$b->selectSearchPrint($Z,$g,$u);$b->selectOrderPrint($Pd,$g,$u);$b->selectLimitPrint($y);$b->selectLengthPrint($Df);$b->selectActionPrint($u);echo"</form>\n";$be=$_GET["page"];if($be=="last"){$kc=$h->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$be=floor(max(0,$kc-1)/$y);}$E="SELECT".limit((+$y&&$rc&&count($rc)<count($J)&&$v=="sql"?"SQL_CALC_FOUND_ROWS ":"").$mc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$sc,($y!=""?+$y:null),($be?$y*$be:0),"\n");echo$b->selectQuery($E);$F=$h->query($E);if(!$F)echo"<p class='error'>".error()."\n";else{if($v=="mssql")$F->seek($y*$be);$Db=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$I=array();while($H=$F->fetch_assoc()){if($be&&$v=="oracle")unset($H["RNUM"]);$I[]=$H;}if($_GET["page"]!="last")$kc=(+$y&&$rc&&count($rc)<count($J)?($v=="sql"?$h->result(" SELECT FOUND_ROWS()"):$h->result("SELECT COUNT(*) FROM ($E) x")):count($I));if(!$I)echo"<p class='message'>".'Žiadne riadky.'."\n";else{$Da=$b->backwardKeys($a,$uf);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$rc&&$J?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$xd=array();$oc=array();reset($J);$Ce=1;foreach($I[0]as$w=>$W){if($w!=$Cd){$W=$_GET["columns"][key($J)];$m=$n[$J?($W?$W["col"]:current($J)):$w];$A=($m?$b->fieldName($m,$Ce):"*");if($A!=""){$Ce++;$xd[$w]=$A;$yc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($w);echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($yc).'">'.(!$J||$W?apply_sql_function($W["fun"],$A):h(current($J)))."</a>";echo"<span class='column hidden'>","<a href='".h("$yc&desc%5B0%5D=1")."' title='".'zostupne'."' class='text'> ↓</a>";if(!$W["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($w)).'\'); return false;" title="'.'Vyhľadať'.'" class="text jsonly"> =</a>';echo"</span>";}$oc[$w]=$W["fun"];next($J);}}$Yc=array();if($_GET["modify"]){foreach($I
as$H){foreach($H
as$w=>$W)$Yc[$w]=max($Yc[$w],min(40,strlen(utf8_decode($W))));}}echo($Da?"<th>".'Vzťahy':"")."</thead>\n";foreach($b->rowDescriptions($I,$hc)as$wd=>$H){$Yf=unique_array($I[$wd],$u);$Zf="";foreach($Yf
as$w=>$W)$Zf.="&".($W!==null?urlencode("where[".bracket_escape($w)."]")."=".urlencode($W):"null%5B%5D=".urlencode($w));echo"<tr".odd().">".(!$rc&&$J?"":"<td>".checkbox("check[]",substr($Zf,1),in_array(substr($Zf,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($rc)<count($J)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Zf)."'>".'upraviť'."</a>"));foreach($H
...
as$w=>$W)json_row("sum-$w",number_format($W,0,'.',' '));json_row("");}else{foreach(count_tables($b->databases())as$k=>$W)json_row("tables-".js_escape($k),$W);json_row("");}exit;}else{$_f=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($_f&&!$l&&!$_POST["search"]){$F=true;$md="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$F=truncate_tables($_POST["tables"]);$md='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$md='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$md='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"])$F=drop_views($_POST["views"]);if($F&&$_POST["tables"])$F=drop_tables($_POST["tables"]);$md='Tabuľka bola odstránená.';}elseif($v!="sql"){$F=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$md='Tables have been optimized.';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc())$md.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$md,$F);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$l,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$zf=tables_list();if(!$zf)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabuľka','<td>'.'Typ','<td>'.'Porovnávanie','<td>'.'Veľkosť dát','<td>'.'Veľkosť indexu','<td>'.'Voľné miesto','<td>'.'Auto Increment','<td>'.'Riadky',(support("comment")?'<td>'.'Komentár':''),"</thead>\n";foreach($zf
as$A=>$S){$lg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($lg?"views[]":"tables[]"),$A,in_array($A,$_f,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Zobraziť štruktúru'.'">'.h($A).'</a>';if($lg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'Zmeniť pohľad'.'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Vypísať dáta'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmeniť tabuľku'),"Index_length"=>array("indexes",'Zmeniť indexy'),"Data_free"=>array("edit",'Nová položka'),"Auto_increment"=>array("auto_increment=1&create",'Zmeniť tabuľku'),"Rows"=>array("select",'Vypísať dáta'),)as$w=>$z)echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($A)."' id='$w-".h($A)."' title='$z[1]'>?</a>":"<td id='$w-".h($A)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($zf)),"<td>".nbsp($v=="sql"?$h->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w)echo"<td align='right' id='sum-$w'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$v)?($v!="sqlite"?"<input type='submit' value='".'Analyzovať'."'> ":"")."<input type='submit' name='optimize' value='".'Optimalizovať'."'> ":"").($v=="sql"?"<input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)").">\n";$j=(support("scheme")?schemas():$b->databases());if(count($j)!=1&&$v!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'">')," <input type='submit' name='move' value='".'Presunúť'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$Re=routines();if($Re){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($Re
as$H){echo'<tr'.odd().'>','<th><a href="'.h(ME).($H["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($H["ROUTINE_NAME"]).'">'.h($H["ROUTINE_NAME"]).'</a>','<td>'.h($H["ROUTINE_TYPE"]),'<td>'.h($H["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($H["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($H["ROUTINE_NAME"]).'">'.'Zmeniť'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Vytvoriť procedúru'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Vytvoriť funkciu'."</a>\n";}if(support("sequence")){echo"<h3>".'Sekvencia'."</h3>\n";$bf=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema()");if($bf){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."</thead>\n";odd('');foreach($bf
