...
confirm($ib=""){return" onclick=\"return confirm('".'Naozaj?'.($ib?" (' + $ib + ')":"")."');\"";}function
...
upload_error($k){$Ed=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Súbor sa nepodarilo nahrať.'.($Ed?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Ed):""):'Súbor neexistuje.');}function
...
enum_input($S,$ya,$l,$X,$Rb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$_d);$H=($Rb!==null?"<label><input type='$S'$ya value='$Rb'".((is_array($X)?in_array($Rb,$X):$X===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($_d[1]as$p=>$W){$W=stripcslashes(str_replace("''","'",$W));$Oa=(is_int($X)?$X==$p+1:(is_array($X)?in_array($p+1,$X):$X===$W));$H.=" <label><input type='$S'$ya value='".($p+1)."'".($Oa?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$H;}function
input($l,$X,$o){global$g,$T,$b,$u;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$jf=($u=="mssql"&&$l["auto_increment"]);if($jf&&!$_POST["save"])$o=null;$Cc=(isset($_GET["select"])||$jf?array("orig"=>'originál'):array())+$b->editFunctions($l);$ya=" name='fields[$A]'";if($l["type"]=="enum")echo
...
h($g->error);}function
check_sqlite_name($A){global$g;$jc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($jc)\$~",$A)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$jc));return
false;}return
true;}function
create_database($j,$d){global$g;if(file_exists($j)){$g->error='Súbor existuje.';return
...
as$j){if(!@unlink($j)){$g->error='Súbor existuje.';return
false;}}return
true;}function
rename_database($A,$d){global$g;if(!check_sqlite_name($A))return
false;$g->Min_SQLite(":memory:");$g->error='Súbor existuje.';return@rename(DB,$A);}function
...
set_schema($vf){global$g,$T,$Nf;$H=$g->query("SET search_path TO ".idf_escape($vf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$Nf['Užívateľské typy'][]=$S;}}return$H;}function
...
unconvert_field($l,$H){return$H;}function
support($mc){return
ereg("view|scheme|processlist|drop_col|variables|status",$mc);}$u="oracle";$T=array();$Nf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$v=>$W){$T+=$W;$Nf[$v]=array_keys($W);}$Hg=array();$ie=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Cc=array("length","lower","round","upper");$Hc=array("avg","count","count distinct","max","min","sum");$Lb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Eb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Ke=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Používateľ<td><input name="auth[username]" id="username" value="',h($_GET["username"]),'" autocapitalize="off">
<tr><th>Heslo<td><input type="password" name="auth[password]">
<tr><th>Databáza<td><input name="auth[db]" value="',h($_GET["db"]);?>" autocapitalize="off">
</table>
...
selectLinks($Uf,$M=""){echo'<p class="tabs">';$wd=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Uf))$wd["view"]='Zmeniť pohľad';else$wd["create"]='Zmeniť tabuľku';if($M!==null)$wd["edit"]='Nová položka';foreach($wd
...
selectQuery($F){global$u,$R;return"<form action='".h(ME)."sql=' method='post'><p><span onclick=\"return !selectEditSql(event, this, '".'Vykonať'."');\">"."<code class='jush-$u'>".h(str_replace("\n"," ",$F))."</code>"." <a href='".h(ME)."sql=".urlencode($F)."'>".'Upraviť'."</a>"."</span><input type='hidden' name='token' value='$R'></p></form>\n";}function
rowDescription($O){return"";}function
rowDescriptions($J,$uc){return$J;}function
selectLink($W,$l){}function
selectVal($W,$x,$l){$H=($W===null?"<i>NULL</i>":(ereg("char|binary",$l["type"])&&!ereg("var",$l["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$l["type"])&&!is_utf8($W))$H=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($W,ENT_QUOTES)));return($x?"<a href='".h($x)."'>$H</a>":$H);}function
editVal($W,$l){return$W;}function
selectColumnsPrint($K,$f){global$Cc,$Hc;print_fieldset("select",'Vypísať',$K);$p=0;$Ac=array('Funkcie'=>$Cc,'Agregácia'=>$Hc);foreach($K
...
selectSearchPrint($Z,$f,$t){print_fieldset("search",'Vyhľadať',$Z);foreach($t
as$p=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$p]' value='".h($_GET["fulltext"][$p])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$p]",1,isset($_GET["boolean"][$p]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ma="this.nextSibling.onchange();";for($p=0;$p<=count($_GET["where"]);$p++){list(,$W)=each($_GET["where"]);if(!$W||("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators))){echo"<div><select name='where[$p][col]' onchange='$Ma'><option value=''>(".'kdekoľvek'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$p][op]",$this->operators,$W["op"],$Ma),"<input type='search' name='where[$p][val]' value='".h($W["val"])."' onchange='".($W?"selectFieldChange(this.form)":"selectAddRow(this)").";' onsearch='selectSearchSearch(this);'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($me,$f,$t){print_fieldset("sort",'Zotriediť',$me);$p=0;foreach((array)$_GET["order"]as$v=>$W){if(isset($f[$W])){echo"<div><select name='order[$p]' onchange='selectFieldChange(this.form);'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$p]",1,isset($_GET["desc"][$v]),'zostupne')."</div>\n";$p++;}}echo"<div><select name='order[$p]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>",checkbox("desc[$p]",1,false,'zostupne')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($hg){if($hg!==null){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($hg)."'>","</div></fieldset>\n";}}function
selectActionPrint($t){echo"<fieldset><legend>".'Akcia'."</legend><div>","<input type='submit' value='".'Vypísať'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script type='text/javascript'>\n","var indexColumns = ";$f=array();foreach($t
...
messageQuery($F){global$u;restart_session();$Kc=&get_session("queries");$q="sql-".count($Kc[$_GET["db"]]);if(strlen($F)>1e6)$F=ereg_replace('[\x80-\xFF]+$','',substr($F,0,1e6))."\n...";$Kc[$_GET["db"]][]=array($F,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$q' onclick=\"return !toggle('$q');\">".'SQL príkaz'."</a><div id='$q' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($F,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Kc[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$H);}function
editInput($O,$l,$ya,$X){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$ya value='-1' checked><i>".'originál'."</i></label> ":"").($l["null"]?"<label><input type='radio'$ya value=''".($X!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ya,$l,$X,0);return"";}function
...
dumpOutput(){$H=array('text'=>'otvoriť','file'=>'uložiť');if(function_exists('gzencode'))$H['gz']='gzip';return$H;}function
dumpFormat(){return
...
<p class="logout">
';if(DB==""||!$Nd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"]))." title='".'Import'."'>".'SQL príkaz'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}echo'<input type="submit" name="logout" value="Odhlásiť" id="logout">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
';$this->databasesPrint($Nd);if($_GET["ns"]!==""&&!$Nd&&DB!=""){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$Q=table_status('',true);if(!$Q)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{$this->tablesPrint($Q);$wd=array();foreach($Q
...
<p id="dbs">
';hidden_fields_get();$rb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo($i?"<select name='db'$rb>".optionlist(array(""=>"(".'databáza'.")")+$i,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Vybrať'."'".($i?" class='hidden'":"").">\n";if($Nd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br><select name='ns'$rb>".optionlist(array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':""))),"</p></form>\n";}function
tablesPrint($Q){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Q
as$O=>$Kf){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O||$_GET["edit"]==$O).">".'vypísať'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold(in_array($O,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])))." title='".'Zobraziť štruktúru'."'>".$this->tableName($Kf)."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
auth_error($bc=null){global$g,$b,$R;$Df=session_name();$k="";if(!$_COOKIE[$Df]&&$_GET[$Df]&&ini_bool("session.use_only_cookies"))$k='Session premenné musia byť povolené.';elseif(isset($_GET["username"])){if(($_COOKIE[$Df]||$_GET[$Df])&&!$R)$k='Session vypršala, prihláste sa prosím znova.';else{$D=&get_session("pwds");if($D!==null){$k=h($bc?$bc->getMessage():(is_string($g)?$g:'Neplatné prihlasovacie údaje.'));if($D===false)$k.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');$D=null;}unset_permanent();}}page_header('Prihlásiť sa',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$Ke)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$R=$_SESSION["token"];if($za&&$_POST["token"])$_POST["token"]=$R;$k='';if($_POST){if($_POST["token"]!=$R){$Wc="max_input_vars";$Fd=ini_get($Wc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$v){$W=ini_get($v);if($W&&(!$Fd||$W<$Fd)){$Wc=$v;$Fd=$W;}}}$k=(!$_POST["token"]&&$Fd?sprintf('Bol prekročený maximálny počet povolených polí. Zvýšte prosím %s.',"'$Wc'"):'Neplatný token CSRF. Odošlite formulár znova.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$k=sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.',"'post_max_size'");if(isset($_GET["sql"]))$k.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}function
connect_error(){global$b,$g,$R,$k,$Eb;$i=array();if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('Databáza'.": ".h(DB),'Nesprávna databáza.',true);}else{if($_POST["db"]&&!$k)queries_redirect(substr(ME,0,-1),'Databázy boli odstránené.',drop_databases($_POST["db"]));page_header('Vybrať databázu',$k,false);echo"<p><a href='".h(ME)."database='>".'Vytvoriť novú databázu'."</a>\n";foreach(array('privileges'=>'Oprávnenia','processlist'=>'Zoznam procesov','variables'=>'Premenné','status'=>'Stav',)as$v=>$W){if(support($v))echo"<a href='".h(ME)."$v='>$W</a>\n";}echo"<p>".sprintf('Verzia %s: %s cez PHP rozšírenie %s',$Eb[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Prihlásený ako: %s',"<b>".h(logged_user())."</b>")."\n";$ef="<a href='".h(ME)."refresh=1'>".'Obnoviť'."</a>\n";$i=$b->databases();if($i){$wf=support("scheme");$Wa=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr><td>&nbsp;<th>".'Databáza'."<td>".'Porovnávanie'."<td>".'Tabuľky'."</thead>\n";foreach($i
as$j){$of=h(ME)."db=".urlencode($j);echo"<tr".odd()."><td>".checkbox("db[]",$j,in_array($j,(array)$_POST["db"])),"<th><a href='$of'>".h($j)."</a>","<td><a href='$of".($wf?"&amp;ns=":"")."&amp;database=' title='".'Zmeniť databázu'."'>".nbsp(db_collation($j,$Wa))."</a>","<td align='right'><a href='$of&amp;schema=' id='tables-".h($j)."' title='".'Schéma databázy'."'>?</a>","\n";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","<p><input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /db/)").">\n","<input type='hidden' name='token' value='$R'>\n",$ef,"</form>\n";}else
echo"<p>$ef";}page_footer("db");if($i)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('Schéma'.": ".h($_GET["ns"]),'Neplatné schéma.',true);page_footer("ns");exit;}}function
...
edit_type($v,$l,$Wa,$vc=array()){global$Nf,$T,$Hg,$ee;echo'<td><select name="',$v,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$Nf+($vc?array('Cudzie kľúče'=>$vc):array()),$l["type"]),'</select>
<td><input name="',$v,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$v"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($Wa,$l["collation"]).'</select>',($Hg?"<select name='$v"."[unsigned]'".(!$l["type"]||ereg('((^|[^o])int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Hg,$l["unsigned"]).'</select>':''),(isset($l['on_update'])?"<select name='$v"."[on_update]'".($l["type"]=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),$l["on_update"]).'</select>':''),($vc?"<select name='$v"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$ee),$l["on_delete"])."</select> ":" ");}function
...
';if($S=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($S=="TABLE"?'Názov stĺpca':'Názov parametra'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Dĺžka
<td>Voľby
...
',(support("comment")?"<td".($bb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($bb?"":" class='hidden'")."><input name='fields[$p][comment]' value='".h($l["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.7.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.7.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($qe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.7.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
';if($m){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Ig?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($Ig?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<p>
Názov tabuľky: <input name="name" maxlength="64" value="',h($I["name"]),'" autocapitalize="off">
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($Ub?html_select("Engine",array(""=>"(".'úložisko'.")")+$Ub,$I["Engine"]):""),' ',($Wa&&!ereg("sqlite|mssql",$u)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$Wa,$I["Collation"]):""),' <input type="submit" value="Uložiť">
...
',checkbox("defaults",1,true,'Východzie hodnoty',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($bb?" checked":"").">".'Komentár'."</label>".' <input name="Comment" id="Comment" value="'.h($I["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($bb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if(support("partitioning")){$Ae=ereg('RANGE|LIST',$I["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$I["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$_e,$I["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($I["partition"]),'">)
Oddiely: <input type="number" name="partitions" class="size" value="',h($I["partitions"]),'"',($Ae||!$I["partition_by"]?" class='hidden'":""),'>
...
';$fd=1;foreach($I["indexes"]as$s){echo"<tr><td>".html_select("indexes[$fd][type]",array(-1=>"")+$Sc,$s["type"],($fd==count($I["indexes"])?"indexesAddRow(this);":1))."<td>";ksort($s["columns"]);$p=1;foreach($s["columns"]as$v=>$e){echo"<span>".html_select("indexes[$fd][columns][$p]",array(-1=>"")+$m,$e,($p==count($s["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($u=="sql"?"":$_GET["indexes"]."_")."');"),($u=="sql"||$u=="mssql"?"<input type='number' name='indexes[$fd][lengths][$p]' class='size' value='".h($s["lengths"][$v])."'>":""),($u!="sql"?checkbox("indexes[$fd][descs][$p]",1,$s["descs"][$v],'zostupne'):"")," </span>";$p++;}echo"<td><input name='indexes[$fd][name]' value='".h($s["name"])."' autocapitalize='off'>\n";$fd++;}echo'</table>
<p>
<input type="submit" value="Uložiť">
<noscript><p><input type="submit" name="add" value="Pridať ďalší"></noscript>
<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["database"])){$I=$_POST;if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();$A=trim($I["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Databáza bola premenovaná.',rename_database($A,$I["collation"]));}else{$i=explode("\n",str_replace("\r","",$A));$Pf=true;$nd="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$I["collation"]))$Pf=false;$nd=$j;}}queries_redirect(ME."db=".urlencode($nd),'Databáza bola vytvorená.',$Pf);}}else{if(!$I["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$k,array(),DB);$Wa=collations();$A=DB;if($_POST)$A=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$Wa);elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$Dc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Dc,$z)&&$z[1]){$A=stripcslashes(idf_unescape("`$z[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input name="name" id="name" value="'.h($A).'" maxlength="64" autocapitalize="off">')."\n".($Wa?html_select("collation",array(""=>"(".'porovnávanie'.")")+$Wa,$I["collation"]):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Uložiť">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.1' alt='+' title='".'Pridať ďalší'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){$I=$_POST;if($_POST&&!$k){$x=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$x,'Schéma bola odstránená.');else{$A=trim($I["name"]);$x.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$x,'Schéma bola vytvorená.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$x,'Schéma bola zmenená.');else
redirect($x);}}page_header($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému',$k);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">
<p><input name="name" id="name" value="',h($I["name"]);?>" autocapitalize="off">
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Uložiť">
<?php
if($_GET["ns"]!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header('Zavolať'.": ".h($da),$k);$pf=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Qc=array();$ue=array();foreach($pf["fields"]as$p=>$l){if(substr($l["inout"],-3)=="OUT")$ue[$p]="@".idf_escape($l["field"])." AS ".idf_escape($l["field"]);if(!$l["inout"]||substr($l["inout"],0,2)=="IN")$Qc[]=$p;}if(!$k&&$_POST){$La=array();foreach($pf["fields"]as$v=>$l){if(in_array($v,$Qc)){$W=process_input($l);if($W===false)$W="''";if(isset($ue[$v]))$g->query("SET @".idf_escape($l["field"])." = $W");}$La[]=(isset($ue[$v])?"@".idf_escape($l["field"]):$W);}$F=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$La).")";echo"<p><code class='jush-$u'>".h($F)."</code> <a href='".h(ME)."sql=".urlencode($F)."'>".'Upraviť'."</a>\n";if(!$g->multi_query($F))echo"<p class='error'>".error()."\n";else{$h=connect();if(is_object($h))$h->select_db(DB);do{$G=$g->store_result();if(is_object($G))select($G,$h);else
echo"<p class='message'>".lang(array('Procedúra bola zavolaná, bol zmenený %d záznam.','Procedúra bola zavolaná, boli zmenené %d záznamy.','Procedúra bola zavolaná, bolo zmenených %d záznamov.'),$g->affected_rows)."\n";}while($g->next_result());if($ue)select($g->query("SELECT ".implode(", ",$ue)));}}echo'
<form action="" method="post">
...
';}if($A!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=$_POST;if($_POST&&!$k){$A=trim($I["name"]);$wa=" AS\n$I[select]";$y=ME."table=".urlencode($A);$_='Pohľad bol zmenený.';if(!$_POST["drop"]&&$a==$A&&$u!="sqlite")query_redirect(($u=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($A).$wa,$y,$_);else{$eg=$A."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A).$wa,"DROP VIEW ".table($A),"CREATE VIEW ".table($eg).$wa,"DROP VIEW ".table($eg),($_POST["drop"]?substr(ME,0,-1):$y),'Pohľad bol odstránený.',$_,'Pohľad bol vytvorený.',$a,$A);}}if(!$_POST&&$a!=""){$I=view($a);$I["name"]=$a;if(!$k)$k=$g->error;}page_header(($a!=""?'Zmeniť pohľad':'Vytvoriť pohľad'),$k,array("table"=>$a),$a);echo'
<form action="" method="post">
<p>Názov: <input name="name" value="',h($I["name"]),'" maxlength="64" autocapitalize="off">
<p>';textarea("select",$I["select"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$ad=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Lf=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");$I=$_POST;if($_POST&&!$k){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Udalosť bola odstránená.');elseif(in_array($I["INTERVAL_FIELD"],$ad)&&isset($Lf[$I["STATUS"]])){$uf="\nON SCHEDULE ".($I["INTERVAL_VALUE"]?"EVERY ".q($I["INTERVAL_VALUE"])." $I[INTERVAL_FIELD]".($I["STARTS"]?" STARTS ".q($I["STARTS"]):"").($I["ENDS"]?" ENDS ".q($I["ENDS"]):""):"AT ".q($I["STARTS"]))." ON COMPLETION".($I["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$uf.($aa!=$I["EVENT_NAME"]?"\nRENAME TO ".idf_escape($I["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($I["EVENT_NAME"]).$uf)."\n".$Lf[$I["STATUS"]]." COMMENT ".q($I["EVENT_COMMENT"]).rtrim(" DO\n$I[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Upraviť udalosť'.": ".h($aa):'Vytvoriť udalosť'),$k);if(!$I&&$aa!=""){$J=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$I=reset($J);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>Názov<td><input name="EVENT_NAME" value="',h($I["EVENT_NAME"]),'" maxlength="64" autocapitalize="off">
<tr><th title="datetime">Začiatok<td><input name="STARTS" value="',h("$I[EXECUTE_AT]$I[STARTS]"),'">
<tr><th title="datetime">Koniec<td><input name="ENDS" value="',h($I["ENDS"]),'">
<tr><th>Každých<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$ad,$I["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Lf,$I["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$I=$_POST;if($_POST&&!$k){$x=substr(ME,0,-1);$A=trim($I["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$x,'Sekvencia bola odstránená.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$x,'Sekvencia bola vytvorená.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$x,'Sekvencia bola zmenená.');else
redirect($x);}page_header($fa!=""?'Pozmeniť sekvenciu'.": ".h($fa):'Vytvoriť sekvenciu',$k);if(!$I)$I["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="Uložiť">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$I=$_POST;if($_POST&&!$k){$x=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$x,'Typ bol odstránený.');else
query_redirect("CREATE TYPE ".idf_escape(trim($I["name"]))." $I[as]",$x,'Typ bol vytvorený.');}page_header($ga!=""?'Pozmeniť typ'.": ".h($ga):'Vytvoriť typ',$k);if(!$I)$I["as"]="AS ";echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";else{echo"<input name='name' value='".h($I['name'])."' autocapitalize='off'>\n";textarea("as",$I["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$A=$_GET["name"];$yg=trigger_options();$wg=array("INSERT","UPDATE","DELETE");$I=(array)trigger($A)+array("Trigger"=>$a."_bi");if($_POST){if(!$k&&in_array($_POST["Timing"],$yg["Timing"])&&in_array($_POST["Event"],$wg)&&in_array($_POST["Type"],$yg["Type"])){$de=" ON ".table($a);$Fb="DROP TRIGGER ".idf_escape($A).($u=="pgsql"?$de:"");$y=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Fb,$y,'Trigger bol odstránený.');else{if($A!="")queries($Fb);queries_redirect($y,($A!=""?'Trigger bol zmenený.':'Trigger bol vytvorený.'),queries(create_trigger($de,$_POST)));if($A!="")queries(create_trigger($de,$I+array("Type"=>reset($yg["Type"]))));}}$I=$_POST;}page_header(($A!=""?'Zmeniť trigger'.": ".h($A):'Vytvoriť trigger'),$k,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$yg["Timing"],$I["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$wg,$I["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$yg["Type"],$I["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($I["Trigger"]),'" maxlength="64" autocapitalize="off">
...
as$W){if($W[1]!="USAGE")$Ec["$z[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$I[0]))$Ec["$z[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$I[0],$z))$be=$z[1];}}if($_POST&&!$k){$ce=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $ce",ME."privileges=",'Používateľ bol odstránený.');else{$Ud=q($_POST["user"])."@".q($_POST["host"]);$De=$_POST["pass"];if($De!=''&&!$_POST["hashed"]){$De=$g->result("SELECT PASSWORD(".q($De).")");$k=!$De;}$lb=false;if(!$k){if($ce!=$Ud){$lb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Ud IDENTIFIED BY PASSWORD ".q($De));$k=!$lb;}elseif($De!=$be)queries("SET PASSWORD FOR $Ud = ".q($De));}if(!$k){$mf=array();foreach($Sd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer' class='help'>".'Oprávnenia'."</a>";$p=0;foreach($Ec
as$Xd=>$Dc){echo'<th>'.($Xd!="*.*"?"<input name='objects[$p]' value='".h($Xd)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$p]' value='*.*' size='10'>*.*");$p++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$gb=>$xb){foreach((array)$Se[$gb]as$Re=>$ab){echo"<tr".odd()."><td".($xb?">$xb<td":" colspan='2'").' lang="en" title="'.h($ab).'">'.h($Re);$p=0;foreach($Ec
as$Xd=>$Dc){$A="'grants[$p][".h(strtoupper($Re))."]'";$X=$Dc[strtoupper($Re)];if($gb=="Server Admin"&&$Xd!=(isset($Ec["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Povoliť'."<option value='0'".($X=="0"?" selected":"").">".'Zakázať'."</select>";else
...
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$kd=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$kd++;}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$kd),$kd||!$_POST["kill"]);}page_header('Zoznam procesov',$k);echo'
<form action="" method="post">
...
as$v=>$W)echo"<td>".(($u=="sql"&&$v=="Info"&&ereg("Query|Killed",$I["Command"])&&$W!="")||($u=="pgsql"&&$v=="current_query"&&$W!="<IDLE>")||($u=="oracle"&&$v=="sql_text"&&$W!="")?"<code class='jush-$u'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($I["db"]!=""?"db=".urlencode($I["db"])."&":"")."sql=".urlencode($W)).'">'.'Klonovať'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($p+1)."/".sprintf('%d celkom',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Ukončiť'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$v=>$W)json_row("sum-$v",number_format($W,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$j=>$W)json_row("tables-".js_escape($j),$W);json_row("");}exit;}else{$bg=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($bg&&!$k&&!$_POST["search"]){$G=true;$_="";if($u=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$G=truncate_tables($_POST["tables"]);$_='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$G=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$_='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$G=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$_='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"])$G=drop_views($_POST["views"]);if($G&&$_POST["tables"])$G=drop_tables($_POST["tables"]);$_='Tabuľka bola odstránená.';}elseif($u!="sql"){$G=($u=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$_='Tables have been optimized.';}elseif(!$_POST["tables"])$_='Žiadne tabuľky.';elseif($G=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($I=$G->fetch_assoc())$_.="<b>".h($I["Table"])."</b>: ".h($I["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$_,$G);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabuľky a pohľady'."</h3>\n";$ag=tables_list();if(!$ag)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabuľka','<td>'.'Typ','<td>'.'Porovnávanie','<td>'.'Veľkosť dát','<td>'.'Veľkosť indexu','<td>'.'Voľné miesto','<td>'.'Auto Increment','<td>'.'Riadky',(support("comment")?'<td>'.'Komentár':''),"</thead>\n";foreach($ag
as$A=>$S){$Sg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Sg?"views[]":"tables[]"),$A,in_array($A,$bg,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Zobraziť štruktúru'.'">'.h($A).'</a>';if($Sg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'Zmeniť pohľad'.'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Vypísať dáta'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmeniť tabuľku'),"Index_length"=>array("indexes",'Zmeniť indexy'),"Data_free"=>array("edit",'Nová položka'),"Auto_increment"=>array("auto_increment=1&create",'Zmeniť tabuľku'),"Rows"=>array("select",'Vypísať dáta'),)as$v=>$x)echo($x?"<td align='right'><a href='".h(ME."$x[0]=").urlencode($A)."' id='$v-".h($A)."' title='$x[1]'>?</a>":"<td id='$v-".h($A)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($ag)),"<td>".nbsp($u=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$v)echo"<td align='right' id='sum-$v'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$u)?($u!="sqlite"?"<input type='submit' value='".'Analyzovať'."'> ":"")."<input type='submit' name='optimize' value='".'Optimalizovať'."'> ":"").($u=="sql"?"<input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)").">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$u!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Presunúť'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";if(support("routine")){echo"<h3 id='routines'>".'Procedúry'."</h3>\n";$rf=routines();if($rf){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($rf
as$I){echo'<tr'.odd().'>','<th><a href="'.h(ME).($I["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($I["ROUTINE_NAME"]).'">'.h($I["ROUTINE_NAME"]).'</a>','<td>'.h($I["ROUTINE_TYPE"]),'<td>'.h($I["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($I["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($I["ROUTINE_NAME"]).'">'.'Zmeniť'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Vytvoriť procedúru'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Vytvoriť funkciu'."</a>\n";}if(support("sequence")){echo"<h3 id='sequences'>".'Sekvencia'."</h3>\n";$Af=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema()");if($Af){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."</thead>\n";odd('');foreach($Af
as$W)echo"<tr".odd()."><th><a href='".h(ME)."sequence=".urlencode($W)."'>".h($W)."</a>\n";echo"</table>\n";}echo"<p><a href='".h(ME)."sequence='>".'Vytvoriť sekvenciu'."</a>\n";}if(support("type")){echo"<h3 id='user-types'>".'Užívateľské typy'."</h3>\n";$Mg=types();if($Mg){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."</thead>\n";odd('');foreach($Mg
as$W)echo"<tr".odd()."><th><a href='".h(ME)."type=".urlencode($W)."'>".h($W)."</a>\n";echo"</table>\n";}echo"<p><a href='".h(ME)."type='>".'Vytvoriť typ'."</a>\n";}if(support("event")){echo"<h3 id='events'>".'Udalosti'."</h3>\n";$J=get_rows("SHOW EVENTS");if($J){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."<td>".'Plán'."<td>".'Začiatok'."<td>".'Koniec'."<td></thead>\n";foreach($J
as$I){echo"<tr>","<th>".h($I["Name"]),"<td>".($I["Execute at"]?'V stanovený čas'."<td>".$I["Execute at"]:'Každých'." ".$I["Interval value"]." ".$I["Interval field"]."<td>$I[Starts]"),"<td>$I[Ends]",'<td><a href="'.h(ME).'event='.urlencode($I["Name"]).'">'.'Zmeniť'.'</a>';}echo"</table>\n";$Zb=$g->result("SELECT @@event_scheduler");if($Zb&&$Zb!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Zb)."\n";}echo'<p><a href="'.h(ME).'event=">'.'Vytvoriť udalosť'."</a>\n";}if($ag)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}}}page_footer();