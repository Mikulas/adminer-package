...
confirm($db=""){return" onclick=\"return confirm('".'Naozaj?'.($db?" (' + $db + ')":"")."');\"";}function
...
upload_error($k){$ld=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Súbor sa nepodarilo nahrať.'.($ld?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$ld):""):'Súbor neexistuje.');}function
...
enum_input($S,$ya,$l,$X,$Fb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$gd);$G=($Fb!==null?"<label><input type='$S'$ya value='$Fb'".((is_array($X)?in_array($Fb,$X):$X===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($gd[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ma=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$ya value='".($q+1)."'".($Ma?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$v;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$Me=($v=="mssql"&&$l["auto_increment"]);if($Me&&!$_POST["save"])$o=null;$pc=(isset($_GET["select"])||$Me?array("orig"=>'originál'):array())+$b->editFunctions($l);$ya=" name='fields[$A]'";if($l["type"]=="enum")echo
...
lang($Of,$Bd){$oe=($Bd==1?0:($Bd&&$Bd<5?1:2));$Of=str_replace("%d","%s",$Of[$oe]);$Bd=number_format($Bd,0,".",' ');return
...
q($W);}function
check_sqlite_name($A){global$g;$Yb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Yb)\$~",$A)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$Yb));return
false;}return
true;}function
create_database($j,$d){global$g;if(file_exists($j)){$g->error='Súbor existuje.';return
...
as$j){if(!@unlink($j)){$g->error='Súbor existuje.';return
false;}}return
true;}function
rename_database($A,$d){global$g;if(!check_sqlite_name($A))return
false;$g->Min_SQLite(":memory:");$g->error='Súbor existuje.';return@rename(DB,$A);}function
...
set_schema($We){global$g,$T,$nf;$G=$g->query("SET search_path TO ".idf_escape($We));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$nf['Užívateľské typy'][]=$S;}}return$G;}function
...
reset($I);}function
support($ac){return
ereg("view|scheme|processlist|drop_col|variables|status",$ac);}$v="oracle";$T=array();$nf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$nf[$w]=array_keys($W);}$dg=array();$Md=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$pc=array("length","lower","round","upper");$tc=array("avg","count","count distinct","max","min","sum");$Ab=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$vb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$re=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Systém<td>',html_select("auth[driver]",$vb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($uf,$L=""){echo'<p class="tabs">';$bd=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($uf))$bd["view"]='Zmeniť pohľad';else$bd["create"]='Zmeniť tabuľku';if($L!==null)$bd["edit"]='Nová položka';foreach($bd
...
selectVal($W,$z,$l){$G=($W===null?"<i>NULL</i>":(ereg("char|binary",$l["type"])&&!ereg("var",$l["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$l["type"])&&!is_utf8($W))$G=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W));return($z?"<a href='$z'>$G</a>":$G);}function
editVal($W,$l){return(ereg("binary",$l["type"])?reset(unpack("H*",$W)):$W);}function
selectColumnsPrint($J,$f){global$pc,$tc;print_fieldset("select",'Vypísať',$J);$q=0;$oc=array('Funkcie'=>$pc,'Agregácia'=>$tc);foreach($J
...
selectSearchPrint($Z,$f,$u){print_fieldset("search",'Vyhľadať',$Z);foreach($u
as$q=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ka="this.nextSibling.onchange();";for($q=0;$q<=count($_GET["where"]);$q++){list(,$W)=each($_GET["where"]);if(!$W||("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators))){echo"<div><select name='where[$q][col]' onchange='$Ka'><option value=''>(".'kdekoľvek'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$q][op]",$this->operators,$W["op"],$Ka),"<input name='where[$q][val]' value='".h($W["val"])."' onchange='".($W?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Qd,$f,$u){print_fieldset("sort",'Zotriediť',$Qd);$q=0;foreach((array)$_GET["order"]as$w=>$W){if(isset($f[$W])){echo"<div><select name='order[$q]' onchange='selectFieldChange(this.form);'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'zostupne')."</div>\n";$q++;}}echo"<div><select name='order[$q]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$q]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($Ef){if($Ef!==null){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($Ef).'">',"</div></fieldset>\n";}}function
...
explode("/",$G);}function
editInput($O,$l,$ya,$X){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$ya value='-1' checked><i>".'originál'."</i></label> ":"").($l["null"]?"<label><input type='radio'$ya value=''".($X!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ya,$l,$X,0);return"";}function
...
edit_type($w,$l,$Ra,$ic=array()){global$nf,$T,$dg,$Id;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$nf+($ic?array('Cudzie kľúče'=>$ic):array()),$l["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($Ra,$l["collation"]).'</select>',($dg?"<select name='$w"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($dg,$l["unsigned"]).'</select>':''),($ic?"<select name='$w"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Id),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Wa?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Wa?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $wa, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Ud==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Zmeniť'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$xf=array();$yf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$gd,PREG_SET_ORDER);foreach($gd
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Xa=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W))$Xa[]=".$w";}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($Xa?"[".implode("|",$Xa)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($vc){print_fieldset("history",'História',$_GET["history"]!="");foreach($vc
as$w=>$W){list($D,$Ff)=$W;echo'<a href="'.h(ME."sql=&history=$w").'">'.'Upraviť'."</a> <span class='time'>".@date("H:i:s",$Ff)."</span> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$D)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$A=>$l){if(!isset($l["privileges"][$eg?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$cd=$_POST["referer"];if($_POST["insert"])$cd=($eg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$cd))$cd=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$cd,'Položka bola vymazaná.');else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null)$L[idf_escape($A)]=($eg?"\n".idf_escape($A)." = $W":$W);}if($eg){if(!$L)redirect($cd);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$cd,'Položka bola aktualizovaná.');}else{$F=insert_into($a,$L);$Uc=($F?last_id():0);queries_redirect($cd,sprintf('Položka%s bola vložená.',($Uc?" $Uc":"")),$F);}}}$vf=$b->tableName(table_status($a));page_header(($eg?'Upraviť':'Vložiť'),$k,array("select"=>array($a,$vf)),$vf);$H=null;if($_POST["save"])$H=(array)$_POST["fields"];elseif($Z){$J=array();foreach($m
as$A=>$l){if(isset($l["privileges"]["select"]))$J[]=($_POST["clone"]&&$l["auto_increment"]?"'' AS ":($v=="sql"&&ereg("enum|set",$l["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false)echo"<p class='error'>".'Žiadne riadky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($m){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($eg?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($eg?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Wa,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($H["Comment"]).'" maxlength="60"'.($Wa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
';if(support("partitioning")){$he=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$ge,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($he||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();$A=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Databáza bola premenovaná.',rename_database($A,$_POST["collation"]));}else{$i=explode("\n",str_replace("\r","",$A));$of=true;$Tc="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"]))$of=false;$Tc=$j;}}queries_redirect(ME."db=".urlencode($Tc),'Databáza bola vytvorená.',$of);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$k,array(),DB);$Ra=collations();$A=DB;$Qa=null;if($_POST){$A=$_POST["name"];$Qa=$_POST["collation"];}elseif(DB!="")$Qa=db_collation(DB,$Ra);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input id="name" name="name" value="'.h($A).'" maxlength="64">')."\n".($Ra?html_select("collation",array(""=>"(".'porovnávanie'.")")+$Ra,$Qa):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložiť">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Pridať ďalší'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$k){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'Schéma bola odstránená.');else{$A=trim($_POST["name"]);$z.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$z,'Schéma bola vytvorená.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$z,'Schéma bola zmenená.');else
redirect($z);}}page_header($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému',$k);$H=$_POST;if(!$H)$H=array("name"=>$_GET["ns"]);echo'
<form action="" method="post">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$xb=false;if($_POST&&!$k){$A=trim($_POST["name"]);$xb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($A)),'Pohľad bol odstránený.','Pohľad bol zmenený.','Pohľad bol vytvorený.',$a);}page_header(($a!=""?'Zmeniť pohľad':'Vytvoriť pohľad'),$k,array("table"=>$a),$a);$H=$_POST;if(!$H&&$a!=""){$H=view($a);$H["name"]=$a;}echo'
<form action="" method="post">
<p>Názov: <input name="name" value="',h($H["name"]),'" maxlength="64">
<p>';textarea("select",$H["select"]);echo'<p>
';if($xb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Ic=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$lf=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$k){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Udalosť bola odstránená.');elseif(in_array($_POST["INTERVAL_FIELD"],$Ic)&&isset($lf[$_POST["STATUS"]])){$Ve="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Ve.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Ve)."\n".$lf[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Upraviť udalosť'.": ".h($aa):'Vytvoriť udalosť'),$k);$H=$_POST;if(!$H&&$aa!=""){$I=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$H=reset($I);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ic,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$lf,$H["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
as$l){if($l["field"]!="")$L[]=(ereg("^($Fc)\$",$l["inout"])?"$l[inout] ":"").idf_escape($l["field"]).process_type($l,"CHARACTER SET");}$xb=drop_create("DROP $Qe ".idf_escape($da),"CREATE $Qe ".idf_escape(trim($_POST["name"]))." (".implode(", ",$L).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$Re)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$da);}page_header(($da!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($da):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$k);$Ra=get_vals("SHOW CHARACTER SET");sort($Ra);$H=array("fields"=>array());if($_POST){$H=$_POST;$H["fields"]=(array)$H["fields"];process_fields($H["fields"]);}elseif($da!=""){$H=routine($da,$Qe);$H["name"]=$da;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($H["name"]),'" maxlength="64">
',($Re?'Jazyk'.": ".html_select("language",$Re,$H["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($H["fields"],$Ra,$Qe);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$H["returns"],$Ra);}echo'</table>
<p>';textarea("definition",$H["definition"]);echo'<p>
<input type="submit" value="Uložiť">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($xb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$z=substr(ME,0,-1);$A=trim($_POST["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekvencia bola odstránená.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$z,'Sekvencia bola vytvorená.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$z,'Sekvencia bola zmenená.');else
redirect($z);}page_header($fa!=""?'Pozmeniť sekvenciu'.": ".h($fa):'Vytvoriť sekvenciu',$k);$H=$_POST;if(!$H)$H=array("name"=>$fa);echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Uložiť">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ bol odstránený.');else
query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$z,'Typ bol vytvorený.');}page_header($ga!=""?'Pozmeniť typ'.": ".h($ga):'Vytvoriť typ',$k);$H=$_POST;if(!$H)$H=array("as"=>"AS ");echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$Tf=trigger_options();$Rf=array("INSERT","UPDATE","DELETE");$xb=false;if($_POST&&!$k&&in_array($_POST["Timing"],$Tf["Timing"])&&in_array($_POST["Event"],$Rf)&&in_array($_POST["Type"],$Tf["Type"])){$Gf=" $_POST[Timing] $_POST[Event]";$Hd=" ON ".table($a);$xb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($v=="pgsql"?$Hd:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($v=="mssql"?$Hd.$Gf:$Gf.$Hd).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$k,array("table"=>$a));$H=$_POST;if(!$H)$H=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Tf["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$Rf,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Tf["Type"],$H["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($xb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$Cd=>$Ne){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Cd,$_))grant("REVOKE",array_keys($Ne),$_[2]," ON $_[1] FROM $_d");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$k);if($gb)$g->query("DROP USER $_d");}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$ha@$_GET[host]"):'Vytvoriť používateľa'),$k,array("privileges"=>array('','Oprávnenia')));if($_POST){$H=$_POST;$qc=$zd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$Fd;if($Fd!="")$H["hashed"]=true;$qc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$q=0;foreach($qc
as$Cd=>$p){echo'<th>'.($Cd!="*.*"?"<input name='objects[$q]' value='".h($Cd)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$bb=>$qb){foreach((array)$ze[$bb]as$ye=>$Va){echo"<tr".odd()."><td".($qb?">$qb<td":" colspan='2'").' lang="en" title="'.h($Va).'">'.h($ye);$q=0;foreach($qc
as$Cd=>$p){$A="'grants[$q][".h(strtoupper($ye))."]'";$X=$p[strtoupper($ye)];if($bb=="Server Admin"&&$Cd!=(isset($qc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Povoliť'."<option value='0'".($X=="0"?" selected":"").">".'Zakázať'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Qc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$Qc++;}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$Qc),$Qc||!$_POST["kill"]);}page_header('Zoznam procesov',$k);echo'
<form action="" method="post">
...
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($q+1)."/".sprintf('%d celkom',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Ukončiť'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$A=>$W){$W=process_input($m[$A]);if($W!==null){if($_POST["clone"])$L[idf_escape($A)]=($W!==false?$W:idf_escape($A));elseif($W!==false)$L[]=idf_escape($A)." = $W";}}$E.=($_POST["clone"]?" (".implode(", ",array_keys($L)).")\nSELECT ".implode(", ",$L)."\nFROM ".table($a):" SET\n".implode(",\n",$L));}if($_POST["delete"]||$L){$Ta="UPDATE";if($_POST["delete"]){$Ta="DELETE";$E="FROM $E";}if($_POST["clone"]){$Ta="INSERT";$E="INTO $E";}if($_POST["all"]||($cg===array()&&$_POST["check"])||count($rc)<count($J)){$F=queries("$Ta $E".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $qg"));$ra=$g->affected_rows;}else{foreach((array)$_POST["check"]as$W){$F=queries($Ta.limit1($E,"\nWHERE ".where_check($W)));if(!$F)break;$ra+=$g->affected_rows;}}}$nd=sprintf('%d položiek bolo ovplyvnených.',$ra);if($_POST["clone"]&&$F&&$ra==1){$Uc=last_id();if($Uc)$nd=sprintf('Položka%s bola vložená.'," $Uc");}queries_redirect(remove_from_uri("page"),$nd,$F);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Dvojkliknite na políčko, ktoré chcete zmeniť.';else{$F=true;$ra=0;foreach($_POST["val"]as$ag=>$H){$L=array();foreach($H
as$w=>$W){$w=bracket_escape($w,1);$L[]=idf_escape($w)." = ".(ereg('char|text',$m[$w]["type"])||$W!=""?$b->processInput($m[$w],$W):"NULL");}$E=table($a)." SET ".implode(", ",$L);$pg=" WHERE ".where_check($ag).($Z?" AND ".implode(" AND ",$Z):"");$F=queries("UPDATE".(count($rc)<count($J)?" $E$pg":limit1($E,$pg)));if(!$F)break;$ra+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$ra),$F);}}elseif(is_string($bc=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($qa["output"])."&format=".urlencode($_POST["separator"]));$F=true;$Sa=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$bc,$gd);$ra=count($gd[0]);begin();$bf=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($gd[0]as$w=>$W){preg_match_all("~((\"[^\"]*\")+|[^$bf]*)$bf~",$W.$bf,$hd);if(!$w&&!array_diff($hd[1],$Sa)){$Sa=$hd[1];$ra--;}else{$L=array();foreach($hd[1]as$q=>$Pa)$L[idf_escape($Sa[$q])]=($Pa==""&&$m[$Sa[$q]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Pa))));$F=insert_update($a,$L,$ue);if(!$F)break;}}if($F)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$ra),$F);queries("ROLLBACK");}else$k=upload_error($bc);}}$vf=$b->tableName($P);page_header('Vypísať'.": $vf",$k);session_write_close();$L=null;if(isset($Oe["insert"])){$L="";foreach((array)$_GET["where"]as$W){if(count($ic[$W["col"]])==1&&($W["op"]=="="||(!$W["op"]&&!ereg('[_%]',$W["val"]))))$L.="&set".urlencode("[".bracket_escape($W["col"])."]")."=".urlencode($W["val"]);}}$b->selectLinks($P,$L);if(!$f)echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($J,$f);$b->selectSearchPrint($Z,$f,$u);$b->selectOrderPrint($Qd,$f,$u);$b->selectLimitPrint($y);$b->selectLengthPrint($Ef);$b->selectActionPrint($u);echo"</form>\n";$ce=$_GET["page"];if($ce=="last"){$lc=$g->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$ce=floor(max(0,$lc-1)/$y);}$E="SELECT".limit((+$y&&$rc&&count($rc)<count($J)&&$v=="sql"?"SQL_CALC_FOUND_ROWS ":"").$nc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$sc,($y!=""?+$y:null),($ce?$y*$ce:0),"\n");echo$b->selectQuery($E);$F=$g->query($E);if(!$F)echo"<p class='error'>".error()."\n";else{if($v=="mssql")$F->seek($y*$ce);$Eb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$I=array();while($H=$F->fetch_assoc()){if($ce&&$v=="oracle")unset($H["RNUM"]);$I[]=$H;}if($_GET["page"]!="last")$lc=(+$y&&$rc&&count($rc)<count($J)?($v=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($E) x")):count($I));if(!$I)echo"<p class='message'>".'Žiadne riadky.'."\n";else{$Da=$b->backwardKeys($a,$vf);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$rc&&$J?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$yd=array();$pc=array();reset($J);$De=1;foreach($I[0]as$w=>$W){if($w!=$Dd){$W=$_GET["columns"][key($J)];$l=$m[$J?($W?$W["col"]:current($J)):$w];$A=($l?$b->fieldName($l,$De):"*");if($A!=""){$De++;$yd[$w]=$A;$e=idf_escape($w);$yc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($w);$qb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($yc.($Qd[0]==$e||$Qd[0]==$w||(!$Qd&&count($rc)<count($J)&&$rc[0]==$e)?$qb:'')).'">';echo(!$J||$W?apply_sql_function($W["fun"],$A):h(current($J)))."</a>";echo"<span class='column hidden'>","<a href='".h($yc.$qb)."' title='".'zostupne'."' class='text'> ↓</a>";if(!$W["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($w)).'\'); return false;" title="'.'Vyhľadať'.'" class="text jsonly"> =</a>';echo"</span>";}$pc[$w]=$W["fun"];next($J);}}$Zc=array();if($_GET["modify"]){foreach($I
as$H){foreach($H
as$w=>$W)$Zc[$w]=max($Zc[$w],min(40,strlen(utf8_decode($W))));}}echo($Da?"<th>".'Vzťahy':"")."</thead>\n";foreach($b->rowDescriptions($I,$ic)as$xd=>$H){$Zf=unique_array($I[$xd],$u);$ag="";foreach($Zf
as$w=>$W)$ag.="&".($W!==null?urlencode("where[".bracket_escape($w)."]")."=".urlencode($W):"null%5B%5D=".urlencode($w));echo"<tr".odd().">".(!$rc&&$J?"":"<td>".checkbox("check[]",substr($ag,1),in_array(substr($ag,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($rc)<count($J)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$ag)."'>".'upraviť'."</a>"));foreach($H
...
as$w=>$W)json_row("sum-$w",number_format($W,0,'.',' '));json_row("");}else{foreach(count_tables($b->databases())as$j=>$W)json_row("tables-".js_escape($j),$W);json_row("");}exit;}else{$Af=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Af&&!$k&&!$_POST["search"]){$F=true;$nd="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$F=truncate_tables($_POST["tables"]);$nd='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$nd='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$nd='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"])$F=drop_views($_POST["views"]);if($F&&$_POST["tables"])$F=drop_tables($_POST["tables"]);$nd='Tabuľka bola odstránená.';}elseif($v!="sql"){$F=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$nd='Tables have been optimized.';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc())$nd.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$nd,$F);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$_f=tables_list();if(!$_f)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabuľka','<td>'.'Typ','<td>'.'Porovnávanie','<td>'.'Veľkosť dát','<td>'.'Veľkosť indexu','<td>'.'Voľné miesto','<td>'.'Auto Increment','<td>'.'Riadky',(support("comment")?'<td>'.'Komentár':''),"</thead>\n";foreach($_f
as$A=>$S){$mg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($mg?"views[]":"tables[]"),$A,in_array($A,$Af,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Zobraziť štruktúru'.'">'.h($A).'</a>';if($mg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'Zmeniť pohľad'.'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Vypísať dáta'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmeniť tabuľku'),"Index_length"=>array("indexes",'Zmeniť indexy'),"Data_free"=>array("edit",'Nová položka'),"Auto_increment"=>array("auto_increment=1&create",'Zmeniť tabuľku'),"Rows"=>array("select",'Vypísať dáta'),)as$w=>$z)echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($A)."' id='$w-".h($A)."' title='$z[1]'>?</a>":"<td id='$w-".h($A)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($_f)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w)echo"<td align='right' id='sum-$w'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$v)?($v!="sqlite"?"<input type='submit' value='".'Analyzovať'."'> ":"")."<input type='submit' name='optimize' value='".'Optimalizovať'."'> ":"").($v=="sql"?"<input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)").">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$v!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'">')," <input type='submit' name='move' value='".'Presunúť'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$Se=routines();if($Se){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($Se
