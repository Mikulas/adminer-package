...
confirm($qe=""){return" onclick=\"return confirm('".'Naozaj?'.($qe?" (' + $qe + ')":"")."');\"";}function
js_escape($D){return
addcslashes($D,"\r\n'\\/");}function
...
input($d,$r,$O){global$T,$p,$y;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$ca=(isset($_GET["select"])?array("orig"=>'originál'):array())+$p->editFunctions($d);$Sa=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($We,$pc){$oc=($pc==1||(!$pc&&'sk'=='fr')?0:((!$pc||$pc>=5)&&ereg('cs|sk|ru','sk')?2:1));return
...
set_schema($La){global$g,$T,$Oa;$c=$g->query("SET search_path TO ".idf_escape($La));foreach(types()as$_){if(!isset($T[$_])){$T[$_]=0;$Oa['Užívateľské typy'][]=$_;}}return$c;}function
...
reset($G);}function
support($mb){return
ereg("view|drop_col|variables|status",$mb);}$y="oracle";$T=array();$Oa=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$T+=$b;$Oa[$e]=array_keys($b);}$sb=array();$Zb=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL");$ca=array("length","lower","round","upper");$pb=array("avg","count","count distinct","max","min","sum");$Wb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ka["mssql"]="MS SQL";if(isset($_GET["mssql"])){$mc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Nc,$q=""){echo'<p class="tabs">';$Ka=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Nc)){$Ka["view"]='Zmeniť pohľad';}else{$Ka["create"]='Zmeniť tabuľku';}if(isset($q)){$Ka["edit"]='Nová položka';}foreach($Ka
...
selectQuery($j){global$y;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$y'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Upraviť'."</a>\n";}function
...
selectColumnsPrint($N,$B){global$ca,$pb;print_fieldset("select",'Vypísať',$N);$k=0;$he=array('Funkcie'=>$ca,'Agregácia'=>$pb);foreach($N
...
selectSearchPrint($t,$B,$J){print_fieldset("search",'Vyhľadať',$t);foreach($J
...
selectOrderPrint($nb,$B,$J){print_fieldset("sort",'Zotriediť',$nb);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'zostupne')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>",checkbox("desc[$k]",1,0,'zostupne')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($zb){if(isset($zb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($zb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$y;restart_session();$U="sql-".count($_SESSION["messages"]);$ib=&get_session("queries");$ib[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$U' onclick=\"return !toggle('$U');\">".'SQL príkaz'."</a><div id='$U' class='hidden'><pre class='jush-$y'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ib[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$c);}function
editInput($h,$d,$Sa,$r){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Sa value='-1' checked><i>".'originál'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Sa value=''".(isset($r)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Sa value='0'".($r===0?" checked":"")."><i>".'prázdne'."</i></label>".enum_input("radio",$Sa,$d,$r);}return"";}function
...
';if($Pb!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$Pb){echo'<p><a href="'.h(ME).'create=">'.bold('Vytvoriť novú tabuľku',$_GET["create"]==="")."</a>\n";$F=tables_list();if(!$F){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($F);$Ka=array();foreach($F
...
as$h=>$_){echo'<a href="'.h(ME).'select='.urlencode($h).'">'.bold('vypísať',$_GET["select"]==$h).'</a> ','<a href="'.h(ME).'table='.urlencode($h).'">'.bold($this->tableName(array("Name"=>$h)),$_GET["table"]==$h)."</a><br>\n";}}}$p=(function_exists('adminer_object')?adminer_object():new
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Ee[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($T[$e]==254){$b="<code>$b</code>";}}if(isset($Ka[$e])&&!$B[$Ka[$e]]){$x="edit=".urlencode($Ka[$e]);foreach($J[$Ka[$e]]as$_c=>$oa){$x.="&where".urlencode("[".bracket_escape($_c)."]")."=".urlencode($a[$oa]);}$b="<a href='".h(ME.$x)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($e,$d,$W,$ba=array()){global$Oa,$T,$sb,$bb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($T[$d["type"]])?array():array($d["type"]))+$Oa+($ba?array('Cudzie kľúče'=>$ba):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($W,$d["collation"]).'</select>',($sb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($sb,$d["unsigned"]).'</select>':''),($ba?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($bb,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Kb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($Kb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $Ne, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.1.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.1.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($hd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.1.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Kb;}function
...
';$cb=true;foreach($ed
as$e=>$b){if($e!=""&&$b>1){echo($cb?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$cb=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$i=$g->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$i){echo'<form action=""><p>
...
as$f=>$d){if(!isset($d["privileges"][$na?"update":"insert"])||$p->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ja=$_POST["referer"];if($_POST["insert"]){$ja=($na?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ja)){$ja=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$ja,'Položka bola vymazaná.');}else{$q=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$q[idf_escape($f)]=($na?"\n".idf_escape($f)." = $b":$b);}}if($na){if(!$q){redirect($ja);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$q),"\nWHERE $t"),$ja,'Položka bola aktualizovaná.');}else{$i=insert_into($m,$q);$ee=($i?last_id():0);queries_redirect($ja,sprintf('Položka%s bola vložená.',($ee?" $ee":"")),$i);}}}$Ba=$p->tableName(table_status($m));page_header(($na?'Upraviť':'Vložiť'),$n,array("select"=>array($m,$Ba)),$Ba);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$N=array();foreach($o
...
',($md?html_select("Engine",array(""=>"(".'úložisko'.")")+$md,$a["Engine"]):""),' ',($W&&!ereg("sqlite|mssql",$y)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$W,$a["Collation"]):""),' <input type="submit" value="Uložiť">
...
<?php echo(support("comment")?checkbox("","",$Kb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Kb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$K,'">
...
as$s){if(count($z)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$Qd=false;}$wb=$s;}}queries_redirect(ME."db=".urlencode($wb),'Databáza bola vytvorená.',$Qd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$n,array(),DB);$W=collations();$f=DB;$dc=null;if($_POST){$f=$_POST["name"];$dc=$_POST["collation"];}elseif(DB!=""){$dc=db_collation(DB,$W);}elseif($y=="sql"){foreach(get_vals("SHOW GRANTS")as$fa){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$fa,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeydown="return textareaKeydown(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($W?html_select("collation",array(""=>"(".'porovnávanie'.")")+$W,$dc):"");?>
...
<input type="hidden" name="token" value="',$K,'">
';if($Ua){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
</form>
';}elseif(isset($_GET["event"])){$gb=$_GET["event"];$Jd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$vd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($gb),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Jd)&&isset($vd[$_POST["STATUS"]])){$Gd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($gb!=""?"ALTER EVENT ".idf_escape($gb).$Gd.($gb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Gd)."\n".$vd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($gb!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'));}}page_header(($gb!=""?'Upraviť udalosť'.": ".h($gb):'Vytvoriť udalosť'),$n);$a=array();if($_POST){$a=$_POST;}elseif($gb!=""){$G=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($gb));$a=reset($G);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Jd,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$vd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Uložiť">
';if($gb!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
...
<input type="hidden" name="token" value="',$K,'">
';if($Ua){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($Ra!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
...
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Uložiť">
';if($yb!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}echo'</form>
';}elseif(isset($_GET["type"])){$Oc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Oc),$x,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ bol vytvorený.');}}page_header($Oc!=""?'Pozmeniť typ'.": ".h($Oc):'Vytvoriť typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$K,'">
...
as$b){if($b[1]!="USAGE"){$Za["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$Za["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$zc=$l[1];}}}if($_POST&&!$n){$Yb=(isset($_GET["host"])?q($nd)."@".q($_GET["host"]):"''");$jb=q($_POST["user"])."@".q($_POST["host"]);$zd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $Yb",ME."privileges=",'Používateľ bol odstránený.');}else{if($Yb!=$jb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $jb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $zd");}elseif($_POST["pass"]!=$zc||!$_POST["hashed"]){queries("SET PASSWORD FOR $jb = ".($_POST["hashed"]?$zd:"PASSWORD($zd)"));}if(!$n){$vc=array();foreach($rc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html'>".'Oprávnenia'."</a>";$k=0;foreach($Za
as$Wa=>$fa){echo'<th>'.($Wa!="*.*"?"<input name='objects[$k]' value='".h($Wa)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Ec=>$hc){foreach((array)$la[$Ec]as$sc=>$ya){echo"<tr".odd()."><td".($hc?">$hc<td":" colspan='2'").' lang="en" title="'.h($ya).'">'.h($sc);$k=0;foreach($Za
as$Wa=>$fa){$f="'grants[$k][".h(strtoupper($sc))."]'";$r=$fa[strtoupper($sc)];if($Ec=="Server Admin"&&$Wa!=(isset($Za["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($r?" selected":"").">".'Povoliť'."<option value='0'".($r=="0"?" selected":"").">".'Zakázať'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($r?" checked":"").($sc=="All privileges"?" id='grants-$k-all'":($sc=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$k-all');\"")).">";}$k++;}}}echo"</table>\n",'<p>
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$jd),$jd||!$_POST["kill"]);}page_header('Zoznam procesov',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".($e=="Info"&&$b!=""?"<code class='jush-$y'>".nbsp($b).'</code> <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Upraviť'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
<input type="hidden" name="token" value="',$K,'">
