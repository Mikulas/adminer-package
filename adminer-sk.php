...
confirm($gb=""){return" onclick=\"return confirm('".'Naozaj?'.($gb?" (' + $gb + ')":"")."');\"";}function
...
upload_error($l){$vd=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'SÃºbor sa nepodarilo nahraÅ¥.'.($vd?" ".sprintf('MaximÃ¡lna povolenÃ¡ veÄ¾kosÅ¥ sÃºboru je %sB.',$vd):""):'SÃºbor neexistuje.');}function
...
enum_input($S,$ya,$m,$X,$Lb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$m["length"],$qd);$H=($Lb!==null?"<label><input type='$S'$ya value='$Lb'".((is_array($X)?in_array($Lb,$X):$X===0)?" checked":"")."><i>".'prÃ¡zdne'."</i></label>":"");foreach($qd[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Na=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$H.=" <label><input type='$S'$ya value='".($q+1)."'".($Na?' checked':'').'>'.h($b->editVal($W,$m)).'</label>';}return$H;}function
input($m,$X,$p){global$h,$T,$b,$v;$A=h(bracket_escape($m["field"]));echo"<td class='function'>";$Xe=($v=="mssql"&&$m["auto_increment"]);if($Xe&&!$_POST["save"])$p=null;$vc=(isset($_GET["select"])||$Xe?array("orig"=>'originÃ¡l'):array())+$b->editFunctions($m);$ya=" name='fields[$A]'";if($m["type"]=="enum")echo
...
lang($cg,$Md){$ye=($Md==1?0:($Md&&$Md<5?1:2));$cg=str_replace("%d","%s",$cg[$ye]);$Md=number_format($Md,0,".",' ');return
...
q($W);}function
check_sqlite_name($A){global$h;$dc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($dc)\$~",$A)){$h->error=sprintf('ProsÃ­m vyberte jednu z koncoviek %s.',str_replace("|",", ",$dc));return
false;}return
true;}function
create_database($k,$e){global$h;if(file_exists($k)){$h->error='SÃºbor existuje.';return
...
as$k){if(!@unlink($k)){$h->error='SÃºbor existuje.';return
false;}}return
true;}function
rename_database($A,$e){global$h;if(!check_sqlite_name($A))return
false;$h->Min_SQLite(":memory:");$h->error='SÃºbor existuje.';return@rename(DB,$A);}function
...
set_schema($kf){global$h,$T,$Af;$H=$h->query("SET search_path TO ".idf_escape($kf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$Af['UÅ¾Ã­vateÄ¾skÃ© typy'][]=$S;}}return$H;}function
...
unconvert_field($m,$H){return$H;}function
support($fc){return
ereg('^(comment|view|scheme|processlist|sequence|trigger|type|variables|drop_col)$',$fc);}$v="pgsql";$T=array();$Af=array();foreach(array('ÄŒÃ­sla'=>array("smallint"=>5,"integer"=>10,"bigint"=>19,"boolean"=>1,"numeric"=>0,"real"=>7,"double precision"=>16,"money"=>20),'DÃ¡tum a Äas'=>array("date"=>13,"time"=>17,"timestamp"=>20,"timestamptz"=>21,"interval"=>0),'ReÅ¥azce'=>array("character"=>0,"character varying"=>0,"text"=>0,"tsquery"=>0,"tsvector"=>0,"uuid"=>0,"xml"=>0),'BinÃ¡rne'=>array("bit"=>0,"bit varying"=>0,"bytea"=>0),'SieÅ¥'=>array("cidr"=>43,"inet"=>43,"macaddr"=>17,"txid_snapshot"=>0),'Geometria'=>array("box"=>0,"circle"=>0,"line"=>0,"lseg"=>0,"path"=>0,"point"=>0,"polygon"=>0),)as$w=>$W){$T+=$W;$Af[$w]=array_keys($W);}$rg=array();$Xd=array("=","<",">","<=",">=","!=","~","!~","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$vc=array("char_length","lower","round","to_hex","to_timestamp","upper");$_c=array("avg","count","count distinct","max","min","sum");$Fb=array(array("char"=>"md5","date|time"=>"now",),array("int|numeric|real|money"=>"+/-","date|time"=>"+ interval/- interval","char|text"=>"||",));}$Ab["oracle"]="Oracle";if(isset($_GET["oracle"])){$Ae=array("OCI8","PDO_OCI");define("DRIVER","oracle");if(extension_loaded("oci8")){class
...
unconvert_field($m,$H){return$H;}function
support($fc){return
ereg("view|scheme|processlist|drop_col|variables|status",$fc);}$v="oracle";$T=array();$Af=array();foreach(array('ÄŒÃ­sla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'DÃ¡tum a Äas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'ReÅ¥azce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¡rne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$Af[$w]=array_keys($W);}$rg=array();$Xd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$vc=array("length","lower","round","upper");$_c=array("avg","count","count distinct","max","min","sum");$Fb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ab["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Ae=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
unconvert_field($m,$H){return$H;}function
support($fc){return
ereg('^(scheme|trigger|view|drop_col)$',$fc);}$v="mssql";$T=array();$Af=array();foreach(array('ÄŒÃ­sla'=>array("tinyint"=>3,"smallint"=>5,"int"=>10,"bigint"=>20,"bit"=>1,"decimal"=>0,"real"=>12,"float"=>53,"smallmoney"=>10,"money"=>20),'DÃ¡tum a Äas'=>array("date"=>10,"smalldatetime"=>19,"datetime"=>19,"datetime2"=>19,"time"=>8,"datetimeoffset"=>10),'ReÅ¥azce'=>array("char"=>8000,"varchar"=>8000,"text"=>2147483647,"nchar"=>4000,"nvarchar"=>4000,"ntext"=>1073741823),'BinÃ¡rne'=>array("binary"=>8000,"varbinary"=>8000,"image"=>2147483647),)as$w=>$W){$T+=$W;$Af[$w]=array_keys($W);}$rg=array();$Xd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL");$vc=array("len","lower","round","upper");$_c=array("avg","count","count distinct","max","min","sum");$Fb=array(array("date|time"=>"getdate",),array("int|decimal|real|float|money|datetime"=>"+/-","char|text"=>"+",));}$Ab=array("server"=>"MySQL")+$Ab;if(!defined("DRIVER")){$Ae=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
<tr><th>SystÃ©m<td>',html_select("auth[driver]",$Ab,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" autocapitalize="off">
<tr><th>PouÅ¾Ã­vateÄ¾<td><input name="auth[username]" id="username" value="',h($_GET["username"]),'" autocapitalize="off">
<tr><th>Heslo<td><input type="password" name="auth[password]">
<tr><th>DatabÃ¡za<td><input name="auth[db]" value="',h($_GET["db"]);?>" autocapitalize="off">
</table>
...
echo"<p><input type='submit' value='".'PrihlÃ¡siÅ¥ sa'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["adminer_permanent"],'TrvalÃ© prihlÃ¡senie')."\n";}function
login($nd,$D){return
true;}function
tableName($Hf){return
h($Hf["Name"]);}function
fieldName($m,$be=0){return'<span title="'.h($m["full_type"]).'">'.h($m["field"]).'</span>';}function
selectLinks($Hf,$M=""){echo'<p class="tabs">';$ld=array("select"=>'VypÃ­saÅ¥ dÃ¡ta',"table"=>'ZobraziÅ¥ Å¡truktÃºru');if(is_view($Hf))$ld["view"]='ZmeniÅ¥ pohÄ¾ad';else$ld["create"]='ZmeniÅ¥ tabuÄ¾ku';if($M!==null)$ld["edit"]='NovÃ¡ poloÅ¾ka';foreach($ld
...
selectQuery($F){global$v,$R;return"<form action='".h(ME)."sql=' method='post'><p><span onclick=\"return !selectEditSql(event, this, '".'VykonaÅ¥'."');\">"."<code class='jush-$v'>".h(str_replace("\n"," ",$F))."</code>"." <a href='".h(ME)."sql=".urlencode($F)."'>".'UpraviÅ¥'."</a>"."</span><input type='hidden' name='token' value='$R'></p></form>\n";}function
rowDescription($O){return"";}function
rowDescriptions($J,$mc){return$J;}function
selectLink($W,$m){}function
selectVal($W,$z,$m){$H=($W===null?"<i>NULL</i>":(ereg("char|binary",$m["type"])&&!ereg("var",$m["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$m["type"])&&!is_utf8($W))$H=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($W,ENT_QUOTES)));return($z?"<a href='".h($z)."'>$H</a>":$H);}function
editVal($W,$m){return$W;}function
selectColumnsPrint($K,$g){global$vc,$_c;print_fieldset("select",'VypÃ­saÅ¥',$K);$q=0;$tc=array('Funkcie'=>$vc,'AgregÃ¡cia'=>$_c);foreach($K
...
selectSearchPrint($Z,$g,$u){print_fieldset("search",'VyhÄ¾adaÅ¥',$Z);foreach($u
as$q=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$La="this.nextSibling.onchange();";for($q=0;$q<=count($_GET["where"]);$q++){list(,$W)=each($_GET["where"]);if(!$W||("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators))){echo"<div><select name='where[$q][col]' onchange='$La'><option value=''>(".'kdekoÄ¾vek'.")".optionlist($g,$W["col"],true)."</select>",html_select("where[$q][op]",$this->operators,$W["op"],$La),"<input type='search' name='where[$q][val]' value='".h($W["val"])."' onchange='".($W?"selectFieldChange(this.form)":"selectAddRow(this)").";' onsearch='selectSearch(this);'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($be,$g,$u){print_fieldset("sort",'ZotriediÅ¥',$be);$q=0;foreach((array)$_GET["order"]as$w=>$W){if(isset($g[$W])){echo"<div><select name='order[$q]' onchange='selectFieldChange(this.form);'><option>".optionlist($g,$W,true)."</select>",checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'zostupne')."</div>\n";$q++;}}echo"<div><select name='order[$q]' onchange='selectAddRow(this);'><option>".optionlist($g,null,true)."</select>","<label><input type='checkbox' name='desc[$q]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($Sf){if($Sf!==null){echo"<fieldset><legend>".'DÄºÅ¾ka textov'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($Sf)."'>","</div></fieldset>\n";}}function
selectActionPrint($u){echo"<fieldset><legend>".'Akcia'."</legend><div>","<input type='submit' value='".'VypÃ­saÅ¥'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script type='text/javascript'>\n","var indexColumns = ";$g=array();foreach($u
...
messageQuery($F){global$v;restart_session();$Bc=&get_session("queries");$r="sql-".count($Bc[$_GET["db"]]);if(strlen($F)>1e6)$F=ereg_replace('[\x80-\xFF]+$','',substr($F,0,1e6))."\n...";$Bc[$_GET["db"]][]=array($F,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL prÃ­kaz'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($F,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Bc[$_GET["db"]])-1)).'">'.'UpraviÅ¥'.'</a></div>';}function
...
explode("/",$H);}function
editInput($O,$m,$ya,$X){if($m["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$ya value='-1' checked><i>".'originÃ¡l'."</i></label> ":"").($m["null"]?"<label><input type='radio'$ya value=''".($X!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ya,$m,$X,0);return"";}function
...
dumpOutput(){$H=array('text'=>'otvoriÅ¥','file'=>'uloÅ¾iÅ¥');if(function_exists('gzencode'))$H['gz']='gzip';if(function_exists('bzcompress'))$H['bz2']='bzip2';return$H;}function
dumpFormat(){return
...
homepage(){echo'<p>'.($_GET["ns"]==""?'<a href="'.h(ME).'database=">'.'ZmeniÅ¥ databÃ¡zu'."</a>\n":""),(support("scheme")?"<a href='".h(ME)."scheme='>".($_GET["ns"]!=""?'PozmeniÅ¥ schÃ©mu':'VytvoriÅ¥ schÃ©mu')."</a>\n":""),($_GET["ns"]!==""?'<a href="'.h(ME).'schema=">'.'SchÃ©ma databÃ¡zy'."</a>\n":""),(support("privileges")?"<a href='".h(ME)."privileges='>".'OprÃ¡vnenia'."</a>\n":"");return
true;}function
...
<p class="logout">
';if(DB==""||!$Ed){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL prÃ­kaz'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}echo'<input type="submit" name="logout" value="OdhlÃ¡siÅ¥" id="logout">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
';$this->databasesPrint($Ed);if($_GET["ns"]!==""&&!$Ed&&DB!=""){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'VytvoriÅ¥ novÃº tabuÄ¾ku'."</a>\n";$Q=tables_list();if(!$Q)echo"<p class='message'>".'Å½iadne tabuÄ¾ky.'."\n";else{$this->tablesPrint($Q);$ld=array();foreach($Q
...
<p id="dbs">
';hidden_fields_get();echo($j?'<select name="db" onmousedown="dbMouseDown(event, this);" onchange="dbChange(this);">'.optionlist(array(""=>"(".'databÃ¡za'.")")+$j,DB).'</select>':'<input name="db" value="'.h(DB).'" autocapitalize="off">'),'<input type="submit" value="VybraÅ¥"',($j?" class='hidden'":""),'>
';if($Ed!="db"&&DB!=""&&$h->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schÃ©ma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':""))),"</p></form>\n";}function
tablesPrint($Q){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Q
as$O=>$S){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O).">".'vypÃ­saÅ¥'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold($_GET["table"]==$O)." title='".'ZobraziÅ¥ Å¡truktÃºru'."'>".$this->tableName(array("Name"=>$O))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
auth_error($Vb=null){global$h,$b,$R;$tf=session_name();$l="";if(!$_COOKIE[$tf]&&$_GET[$tf]&&ini_bool("session.use_only_cookies"))$l='Session premennÃ© musia byÅ¥ povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$tf]||$_GET[$tf])&&!$R)$l='Session vyprÅ¡ala, prihlÃ¡ste sa prosÃ­m znova.';else{$D=&get_session("pwds");if($D!==null){$l=h($Vb?$Vb->getMessage():(is_string($h)?$h:'NeplatnÃ© prihlasovacie Ãºdaje.'));$D=null;}unset_permanent();}}page_header('PrihlÃ¡siÅ¥ sa',$l,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½iadne rozÅ¡Ã­renie',sprintf('Nie je dostupnÃ© Å¾iadne z podporovanÃ½ch rozÅ¡Ã­renÃ­ (%s).',implode(", ",$Ae)),false);page_footer("auth");exit;}$h=connect();}if(is_string($h)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$R=$_SESSION["token"];if($d&&$_POST["token"])$_POST["token"]=$R;$l='';if($_POST){if($_POST["token"]!=$R){$Lc="max_input_vars";$wd=ini_get($Lc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$w){$W=ini_get($w);if($W&&(!$wd||$W<$wd)){$Lc=$w;$wd=$W;}}}$l=(!$_POST["token"]&&$wd?sprintf('Bol prekroÄenÃ½ maximÃ¡lny poÄet povolenÃ½ch polÃ­. ZvÃ½Å¡te prosÃ­m %s.',"'$Lc'"):'NeplatnÃ½ token CSRF. OdoÅ¡lite formulÃ¡r znova.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST")$l=sprintf('PrÃ­liÅ¡ veÄ¾kÃ© POST dÃ¡ta. ZmenÅ¡ite dÃ¡ta alebo zvÃ½Å¡te hodnotu konfiguraÄej direktÃ­vy %s.',"'post_max_size'");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}function
connect_error(){global$b,$h,$R,$l,$Ab;$j=array();if(DB!="")page_header('DatabÃ¡za'.": ".h(DB),'NesprÃ¡vna databÃ¡za.',true);else{if($_POST["db"]&&!$l)queries_redirect(substr(ME,0,-1),'DatabÃ¡zy boli odstrÃ¡nenÃ©.',drop_databases($_POST["db"]));page_header('VybraÅ¥ databÃ¡zu',$l,false);echo"<p><a href='".h(ME)."database='>".'VytvoriÅ¥ novÃº databÃ¡zu'."</a>\n";foreach(array('privileges'=>'OprÃ¡vnenia','processlist'=>'Zoznam procesov','variables'=>'PremennÃ©','status'=>'Stav',)as$w=>$W){if(support($w))echo"<a href='".h(ME)."$w='>$W</a>\n";}echo"<p>".sprintf('Verzia %s: %s cez PHP rozÅ¡Ã­renie %s',$Ab[DRIVER],"<b>$h->server_info</b>","<b>$h->extension</b>")."\n","<p>".sprintf('PrihlÃ¡senÃ½ ako: %s',"<b>".h(logged_user())."</b>")."\n";$Te="<a href='".h(ME)."refresh=1'>".'ObnoviÅ¥'."</a>\n";$j=$b->databases();if($j){$lf=support("scheme");$Ua=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr><td>&nbsp;<th>".'DatabÃ¡za'."<td>".'PorovnÃ¡vanie'."<td>".'TabuÄ¾ky'."</thead>\n";foreach($j
as$k){$df=h(ME)."db=".urlencode($k);echo"<tr".odd()."><td>".checkbox("db[]",$k,in_array($k,(array)$_POST["db"])),"<th><a href='$df'>".h($k)."</a>","<td><a href='$df".($lf?"&amp;ns=":"")."&amp;database=' title='".'ZmeniÅ¥ databÃ¡zu'."'>".nbsp(db_collation($k,$Ua))."</a>","<td align='right'><a href='$df&amp;schema=' id='tables-".h($k)."' title='".'SchÃ©ma databÃ¡zy'."'>?</a>","\n";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","<p><input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm("formChecked(this, /db/)").">\n","<input type='hidden' name='token' value='$R'>\n",$Te,"</form>\n";}else
echo"<p>$Te";}page_footer("db");if($j)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(!(DB!=""?$h->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){page_header('SchÃ©ma'.": ".h($_GET["ns"]),'NeplatnÃ© schÃ©ma.',true);page_footer("ns");exit;}}function
...
as$w=>$W){if($W===null)$W="<i>NULL</i>";elseif($Ga[$w]&&!is_utf8($W))$W="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$w]==254)$W="<code>$W</code>";}if(isset($ld[$w])&&!$g[$ld[$w]]){if($Ec){$O=$I[array_search("table=",$ld)];$z=$ld[$w].urlencode($ee[$O]!=""?$ee[$O]:$O);}else{$z="edit=".urlencode($ld[$w]);foreach($u[$ld[$w]]as$Sa=>$Uc)$z.="&where".urlencode("[".bracket_escape($Sa)."]")."=".urlencode($I[$Uc]);}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Å½iadne riadky.')."\n";return$H;}function
...
edit_type($w,$m,$Ua,$nc=array()){global$Af,$T,$rg,$Td;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$m["type"]||isset($T[$m["type"]])?array():array($m["type"]))+$Af+($nc?array('Cudzie kÄ¾ÃºÄe'=>$nc):array()),$m["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$m["type"])?"":" class='hidden'").'><option value="">('.'porovnÃ¡vanie'.')'.optionlist($Ua,$m["collation"]).'</select>',($rg?"<select name='$w"."[unsigned]'".(!$m["type"]||ereg('(int|float|double|decimal)$',$m["type"])?"":" class='hidden'").'><option>'.optionlist($rg,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='$w"."[on_update]'".($m["type"]=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),$m["on_update"]).'</select>':''),($nc?"<select name='$w"."[on_delete]'".(ereg("`",$m["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Td),$m["on_delete"])."</select> ":" ");}function
...
';if($S=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($S=="TABLE"?'NÃ¡zov stÄºpca':'NÃ¡zov parametra'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>DÄºÅ¾ka
<td>VoÄ¾by
';if($S=="TABLE"){echo'<td>NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym title="Auto Increment">AI</acronym>
<td>VÃ½chodzie hodnoty
',(support("comment")?"<td".($Za?"":" class='hidden'").">".'KomentÃ¡r':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.4' alt='+' title='".'PridaÅ¥ ÄalÅ¡Ã­'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($Za?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.4' alt='+' title='".'PridaÅ¥ ÄalÅ¡Ã­'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.4' alt='^' title='".'PresunÃºÅ¥ hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.4' alt='v' title='".'PresunÃºÅ¥ dolu'."'>&nbsp;":""),($fe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.4' alt='x' title='".'OdobraÅ¥'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$A=>$t){ksort($t["columns"]);$Fe=array();foreach($t["columns"]as$w=>$W)$Fe[]="<i>".h($W)."</i>".($t["lengths"][$w]?"(".$t["lengths"][$w].")":"");echo"<tr title='".h($A)."'><th>$t[type]<td>".implode(", ",$Fe)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($a).'">'.'ZmeniÅ¥ indexy'."</a>\n";if(fk_support($P)){echo"<h3>".'Cudzie kÄ¾ÃºÄe'."</h3>\n";$nc=foreign_keys($a);if($nc){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'CieÄ¾'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($v!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($nc
as$A=>$o){echo"<tr title='".h($A)."'>","<th><i>".implode("</i>, <i>",array_map('h',$o["source"]))."</i>","<td><a href='".h($o["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($o["db"]),ME):($o["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($o["ns"]),ME):ME))."table=".urlencode($o["table"])."'>".($o["db"]!=""?"<b>".h($o["db"])."</b>.":"").($o["ns"]!=""?"<b>".h($o["ns"])."</b>.":"").h($o["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$o["target"]))."</i>)","<td>".nbsp($o["on_delete"])."\n","<td>".nbsp($o["on_update"])."\n",($v=="sqlite"?"":'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($A)).'">'.'ZmeniÅ¥'.'</a>');}echo"</table>\n";}if($v!="sqlite")echo'<p><a href="'.h(ME).'foreign='.urlencode($a).'">'.'PridaÅ¥ cudzÃ­ kÄ¾ÃºÄ'."</a>\n";}if(support("trigger")){echo"<h3>".'Triggery'."</h3>\n";$ig=triggers($a);if($ig){echo"<table cellspacing='0'>\n";foreach($ig
as$w=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'ZmeniÅ¥'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'PridaÅ¥ trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('SchÃ©ma databÃ¡zy',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Kf=array();$Lf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$qd,PREG_SET_ORDER);foreach($qd
...
<p><a href="',h(ME."schema=".urlencode($ea)),'" id="schema-link">PermanentnÃ½ odkaz</a>
...
<table cellspacing="0">
';$pb=array('','USE','DROP+CREATE','CREATE');$Mf=array('','DROP+CREATE','CREATE');$nb=array('','TRUNCATE+INSERT','INSERT');if($v=="sql")$nb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$I);if(!$I)$I=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($I["events"])){$I["routines"]=$I["events"]=($_GET["dump"]=="");$I["triggers"]=$I["table_style"];}echo"<tr><th>".'VÃ½stup'."<td>".html_select("output",$b->dumpOutput(),$I["output"],0)."\n";echo"<tr><th>".'FormÃ¡t'."<td>".html_select("format",$b->dumpFormat(),$I["format"],0)."\n";echo($v=="sqlite"?"":"<tr><th>".'DatabÃ¡za'."<td>".html_select('db_style',$pb,$I["db_style"]).(support("routine")?checkbox("routines",1,$I["routines"],'ProcedÃºry'):"").(support("event")?checkbox("events",1,$I["events"],'Udalosti'):"")),"<tr><th>".'TabuÄ¾ky'."<td>".html_select('table_style',$Mf,$I["table_style"]).checkbox("auto_increment",1,$I["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$I["triggers"],'Triggery'):""),"<tr><th>".'DÃ¡ta'."<td>".html_select('data_style',$nb,$I["data_style"]),'</table>
<p><input type="submit" value="Export">
<input type="hidden" name="token" value="',$R,'">

<table cellspacing="0">
';$Ce=array();if(DB!=""){$Na=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Na onclick='formCheck(this, /^tables\\[/);'>".'TabuÄ¾ky'."</label>","<th style='text-align: right;'><label>".'DÃ¡ta'."<input type='checkbox' id='check-data'$Na onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Y="";foreach(table_status()as$P){$A=$P["Name"];$Be=ereg_replace("_.*","",$A);$Na=($a==""||$a==(substr($a,-1)=="%"?"$Be%":$A));$Fe="<tr><td>".checkbox("tables[]",$A,$Na,$A,"checkboxClick(event, this); formUncheck('check-tables');");if(is_view($P))$Y.="$Fe\n";else
echo"$Fe<td align='right'><label>".($P["Engine"]=="InnoDB"&&$P["Rows"]?"~ ":"").$P["Rows"].checkbox("data[]",$A,$Na,"","checkboxClick(event, this); formUncheck('check-data');")."</label>\n";$Ce[$Be]++;}echo$Y;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'DatabÃ¡za'."</label></thead>\n";$j=$b->databases();if($j){foreach($j
...
';$jc=true;foreach($Ce
as$w=>$W){if($w!=""&&$W>1){echo($jc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$w%")."'>".h($w)."</a>";$jc=false;}}}elseif(isset($_GET["privileges"])){page_header('OprÃ¡vnenia');$G=$h->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$wc=$G;if(!$G)$G=$h->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($wc?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".'PouÅ¾Ã­vateÄ¾'."<th>".'Server'."<th>&nbsp;</thead>\n";while($I=$G->fetch_assoc())echo'<tr'.odd().'><td>'.h($I["User"])."<td>".h($I["Host"]).'<td><a href="'.h(ME.'user='.urlencode($I["User"]).'&host='.urlencode($I["Host"])).'">'.'UpraviÅ¥'."</a>\n";if(!$wc||DB!="")echo"<tr".odd()."><td><input name='user' autocapitalize='off'><td><input name='host' value='localhost' autocapitalize='off'><td><input type='submit' value='".'UpraviÅ¥'."'>\n";echo"</table>\n","</form>\n",'<p><a href="'.h(ME).'user=">'.'VytvoriÅ¥ pouÅ¾Ã­vateÄ¾a'."</a>";}elseif(isset($_GET["sql"])){if(!$l&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$Cc=&get_session("queries");$Bc=&$Cc[DB];if(!$l&&$_POST["clear"]){$Bc=array();redirect(remove_from_uri("history"));}page_header('SQL prÃ­kaz',$l);if(!$l&&$_POST){$rc=false;$F=$_POST["query"];if($_POST["webfile"]){$rc=@fopen((file_exists("adminer.sql")?"adminer.sql":(file_exists("adminer.sql.gz")?"compress.zlib://adminer.sql.gz":"compress.bzip2://adminer.sql.bz2")),"rb");$F=($rc?fread($rc,1e6):false);}elseif($_FILES&&$_FILES["sql_file"]["error"]!=UPLOAD_ERR_NO_FILE)$F=get_file("sql_file",true);if(is_string($F)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($F)+memory_get_usage()+8e6));if($F!=""&&strlen($F)<1e6){$E=$F.(ereg(";[ \t\r\n]*\$",$F)?"":";");if(!$Bc||reset(end($Bc))!=$E){restart_session();$Bc[]=array($E,time());set_session("queries",$Cc);stop_session();}}$vf="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\n)";$tb=";";$B=0;$Lb=true;$i=connect();if(is_object($i)&&DB!="")$i->select_db(DB);$Xa=0;$Rb=array();$kd=0;$pe='[\'"'.($v=="sql"?'`#':($v=="sqlite"?'`[':($v=="mssql"?'[':''))).']|/\\*|-- |$'.($v=="pgsql"?'|\\$[^$]*\\$':'');$ag=microtime();parse_str($_COOKIE["adminer_export"],$pa);$Db=$b->dumpFormat();unset($Db["sql"]);while($F!=""){if(!$B&&preg_match("~^$vf*DELIMITER\\s+(\\S+)~i",$F,$_)){$tb=$_[1];$F=substr($F,strlen($_[0]));}else{preg_match('('.preg_quote($tb)."\\s*|$pe)",$F,$_,PREG_OFFSET_CAPTURE,$B);list($pc,$ye)=$_[0];if(!$pc&&$rc&&!feof($rc))$F.=fread($rc,1e5);else{if(!$pc&&rtrim($F)=="")break;$B=$ye+strlen($pc);if($pc&&rtrim($pc)!=$tb){while(preg_match('('.($pc=='/*'?'\\*/':($pc=='['?']':(ereg('^-- |^#',$pc)?"\n":preg_quote($pc)."|\\\\."))).'|$)s',$F,$_,PREG_OFFSET_CAPTURE,$B)){$if=$_[0][0];if(!$if&&$rc&&!feof($rc))$F.=fread($rc,1e5);else{$B=$_[0][1]+strlen($if);if($if[0]!="\\")break;}}}else{$Lb=false;$E=substr($F,0,$ye);$Xa++;$Fe="<pre id='sql-$Xa'><code class='jush-$v'>".shorten_utf8(trim($E),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$Fe;ob_flush();flush();}$xf=microtime();if($h->multi_query($E)&&is_object($i)&&preg_match("~^$vf*USE\\b~isU",$E))$i->query($E);do{$G=$h->store_result();$Mb=microtime();$Tf=format_time($xf,$Mb).(strlen($E)<1000?" <a href='".h(ME)."sql=".urlencode(trim($E))."'>".'UpraviÅ¥'."</a>":"");if($h->error){echo($_POST["only_errors"]?$Fe:""),"<p class='error'>".'Chyba v dotaze'.($h->errno?" ($h->errno)":"").": ".error()."\n";$Rb[]=" <a href='#sql-$Xa'>$Xa</a>";if($_POST["error_stops"])break
2;}elseif(is_object($G)){$ee=select($G,$i);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n","<p>".($G->num_rows?lang(array('%d riadok','%d riadky','%d riadkov'),$G->num_rows):"").$Tf;$r="export-$Xa";$ac=", <a href='#$r' onclick=\"return !toggle('$r');\">".'Export'."</a><span id='$r' class='hidden'>: ".html_select("output",$b->dumpOutput(),$pa["output"])." ".html_select("format",$Db,$pa["format"])."<input type='hidden' name='query' value='".h($E)."'>"." <input type='submit' name='export' value='".'Export'."'><input type='hidden' name='token' value='$R'></span>\n";if($i&&preg_match("~^($vf|\\()*SELECT\\b~isU",$E)&&($Zb=explain($i,$E))){$r="explain-$Xa";echo", <a href='#$r' onclick=\"return !toggle('$r');\">EXPLAIN</a>$ac","<div id='$r' class='hidden'>\n";select($Zb,$i,($v=="sql"?"http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/explain-output.html#explain_":""),$ee);echo"</div>\n";}else
echo$ac;echo"</form>\n";}}else{if(preg_match("~^$vf*(CREATE|DROP|ALTER)$vf+(DATABASE|SCHEMA)\\b~isU",$E)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($h->info)."'>".lang(array('PrÃ­kaz prebehol v poriadku, bol zmenenÃ½ %d zÃ¡znam.','PrÃ­kaz prebehol v poriadku boli zmenenÃ© %d zÃ¡znamy.','PrÃ­kaz prebehol v poriadku bolo zmenenÃ½ch %d zÃ¡znamov.'),$h->affected_rows)."$Tf\n";}$xf=$Mb;}while($h->next_result());$kd+=substr_count($E.$pc,"\n");$F=substr($F,$B);$B=0;}}}}if($Lb)echo"<p class='message'>".'Å½iadne prÃ­kazy na vykonanie.'."\n";elseif($_POST["only_errors"])echo"<p class='message'>".lang(array('Bol vykonanÃ½ %d dotaz.','Boli vykonanÃ© %d dotazy.','Bolo vykonanÃ½ch %d dotazov.'),$Xa-count($Rb)).format_time($ag,microtime())."\n";elseif($Rb&&$Xa>1)echo"<p class='error'>".'Chyba v dotaze'.": ".implode("",$Rb)."\n";}else
echo"<p class='error'>".upload_error($F)."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
<p>';$E=$_GET["sql"];if($_POST)$E=$_POST["query"];elseif($_GET["history"]=="all")$E=$Bc;elseif($_GET["history"]!="")$E=$Bc[$_GET["history"]][0];textarea("query",$E,20);echo($_POST?"":"<script type='text/javascript'>document.getElementsByTagName('textarea')[0].focus();</script>\n"),"<p>".(ini_bool("file_uploads")?'Nahranie sÃºboru'.': <input type="file" name="sql_file"'.($_FILES&&$_FILES["sql_file"]["error"]!=4?'':' onchange="this.form[\'only_errors\'].checked = true;"').'> (&lt; '.ini_get("upload_max_filesize").'B)':'NahrÃ¡vÃ¡nie sÃºborov nie je povolenÃ©.'),'<p>
<input type="submit" value="VykonaÅ¥" title="Ctrl+Enter">
',checkbox("error_stops",1,$_POST["error_stops"],'ZastaviÅ¥ pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'ZobraziÅ¥ iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$ab=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W))$ab[]=".$w";}echo
sprintf('SÃºbor %s na webovom serveri',"<code>adminer.sql".($ab?"[".implode("|",$ab)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'SpustiÅ¥ sÃºbor'.'">',"</div></fieldset>\n";if($Bc){print_fieldset("history",'HistÃ³ria',$_GET["history"]!="");for($W=end($Bc);$W;$W=prev($Bc)){$w=key($Bc);list($E,$Tf)=$W;echo'<a href="'.h(ME."sql=&history=$w").'">'.'UpraviÅ¥'."</a> <span class='time' title='".@date('Y-m-d',$Tf)."'>".@date("H:i:s",$Tf)."</span> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$E)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'VyÄistiÅ¥'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'UpraviÅ¥ vÅ¡etko'."</a>\n","</div></fieldset>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$A=>$m){if(!isset($m["privileges"][$sg?"update":"insert"])||$b->fieldName($m)=="")unset($n[$A]);}if($_POST&&!$l&&!isset($_GET["select"])){$md=$_POST["referer"];if($_POST["insert"])$md=($sg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$md))$md=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$md,'PoloÅ¾ka bola vymazanÃ¡.');else{$M=array();foreach($n
as$A=>$m){$W=process_input($m);if($W!==false&&$W!==null)$M[idf_escape($A)]=($sg?"\n".idf_escape($A)." = $W":$W);}if($sg){if(!$M)redirect($md);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$M),"\nWHERE $Z"),$md,'PoloÅ¾ka bola aktualizovanÃ¡.');}else{$G=insert_into($a,$M);$ed=($G?last_id():0);queries_redirect($md,sprintf('PoloÅ¾ka%s bola vloÅ¾enÃ¡.',($ed?" $ed":"")),$G);}}}$If=$b->tableName(table_status($a));page_header(($sg?'UpraviÅ¥':'VloÅ¾iÅ¥'),$l,array("select"=>array($a,$If)),$If);$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($n
as$A=>$m){if(isset($m["privileges"]["select"])){$wa=convert_field($m);if($_POST["clone"]&&$m["auto_increment"])$wa="''";if($v=="sql"&&ereg("enum|set",$m["type"]))$wa="1*".idf_escape($A);$K[]=($wa?"$wa AS ":"").idf_escape($A);}}$I=array();if($K){$J=get_rows("SELECT".limit(implode(", ",$K)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$I=(isset($_GET["select"])&&count($J)!=1?null:reset($J));}}if($I===false)echo"<p class='error'>".'Å½iadne riadky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($n){echo"<input type='submit' value='".'UloÅ¾iÅ¥'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($sg?'UloÅ¾iÅ¥ a pokraÄovaÅ¥ v ÃºpravÃ¡ch':'UloÅ¾iÅ¥ a vloÅ¾iÅ¥ ÄalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($sg?"<input type='submit' name='delete' value='".'ZmazaÅ¥'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$n?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<p>
NÃ¡zov tabuÄ¾ky: <input name="name" maxlength="64" value="',h($I["name"]),'" autocapitalize="off">
';if($a==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Ob?html_select("Engine",array(""=>"(".'ÃºloÅ¾isko'.")")+$Ob,$I["Engine"]):""),' ',($Ua&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porovnÃ¡vanie'.")")+$Ua,$I["Collation"]):""),' <input type="submit" value="UloÅ¾iÅ¥">
...
<label class="jsonly"><input type="checkbox" id="defaults" name="defaults" value="1" checked onclick="columnShow(this.checked, 5);">VÃ½chodzie hodnoty</label>
';if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?checkbox("comments",1,$Za,'KomentÃ¡r',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input name="Comment" id="Comment" value="'.h($I["Comment"]).'" maxlength="'.($h->server_info>=5.5?2048:60).'"'.($Za?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="UloÅ¾iÅ¥">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="OdstrÃ¡niÅ¥"',confirm(),'>';}if(support("partitioning")){$re=ereg('RANGE|LIST',$I["partition_by"]);print_fieldset("partition",'RozdeliÅ¥ podÄ¾a',$I["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$qe,$I["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($I["partition"]),'">)
Oddiely: <input type="number" name="partitions" class="size" value="',h($I["partitions"]),'"',($re||!$I["partition_by"]?" class='hidden'":""),'>
<table cellspacing="0" id="partition-table"',($re?"":" class='hidden'"),'>
<thead><tr><th>NÃ¡zov oddielu<th>Hodnoty</thead>
...
<p>
<input type="submit" value="UloÅ¾iÅ¥">
<noscript><p><input type="submit" name="add" value="PridaÅ¥ ÄalÅ¡Ã­"></noscript>
<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["database"])){if($_POST&&!$l&&!isset($_POST["add_x"])){restart_session();$A=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'DatabÃ¡za bola odstrÃ¡nenÃ¡.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'DatabÃ¡za bola premenovanÃ¡.',rename_database($A,$_POST["collation"]));}else{$j=explode("\n",str_replace("\r","",$A));$Cf=true;$dd="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$_POST["collation"]))$Cf=false;$dd=$k;}}queries_redirect(ME."db=".urlencode($dd),'DatabÃ¡za bola vytvorenÃ¡.',$Cf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'DatabÃ¡za bola zmenenÃ¡.');}}page_header(DB!=""?'ZmeniÅ¥ databÃ¡zu':'VytvoriÅ¥ databÃ¡zu',$l,array(),DB);$Ua=collations();$A=DB;$Ta=null;if($_POST){$A=$_POST["name"];$Ta=$_POST["collation"];}elseif(DB!="")$Ta=db_collation(DB,$Ua);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$wc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$wc,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input name="name" id="name" value="'.h($A).'" maxlength="64" autocapitalize="off">')."\n".($Ua?html_select("collation",array(""=>"(".'porovnÃ¡vanie'.")")+$Ua,$Ta):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="UloÅ¾iÅ¥">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.4' alt='+' title='".'PridaÅ¥ ÄalÅ¡Ã­'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$l){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'SchÃ©ma bola odstrÃ¡nenÃ¡.');else{$A=trim($_POST["name"]);$z.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$z,'SchÃ©ma bola vytvorenÃ¡.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$z,'SchÃ©ma bola zmenenÃ¡.');else
redirect($z);}}page_header($_GET["ns"]!=""?'PozmeniÅ¥ schÃ©mu':'VytvoriÅ¥ schÃ©mu',$l);$I=$_POST;if(!$I)$I=array("name"=>$_GET["ns"]);echo'
<form action="" method="post">
<p><input name="name" id="name" value="',h($I["name"]);?>" autocapitalize="off">
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="UloÅ¾iÅ¥">
<?php
if($_GET["ns"]!="")echo"<input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header('ZavolaÅ¥'.": ".h($da),$l);$ef=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$Hc=array();$ke=array();foreach($ef["fields"]as$q=>$m){if(substr($m["inout"],-3)=="OUT")$ke[$q]="@".idf_escape($m["field"])." AS ".idf_escape($m["field"]);if(!$m["inout"]||substr($m["inout"],0,2)=="IN")$Hc[]=$q;}if(!$l&&$_POST){$Ka=array();foreach($ef["fields"]as$w=>$m){if(in_array($w,$Hc)){$W=process_input($m);if($W===false)$W="''";if(isset($ke[$w]))$h->query("SET @".idf_escape($m["field"])." = $W");}$Ka[]=(isset($ke[$w])?"@".idf_escape($m["field"]):$W);}$F=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$Ka).")";echo"<p><code class='jush-$v'>".h($F)."</code> <a href='".h(ME)."sql=".urlencode($F)."'>".'UpraviÅ¥'."</a>\n";if(!$h->multi_query($F))echo"<p class='error'>".error()."\n";else{$i=connect();if(is_object($i))$i->select_db(DB);do{$G=$h->store_result();if(is_object($G))select($G,$i);else
echo"<p class='message'>".lang(array('ProcedÃºra bola zavolanÃ¡, bol zmenenÃ½ %d zÃ¡znam.','ProcedÃºra bola zavolanÃ¡, boli zmenenÃ© %d zÃ¡znamy.','ProcedÃºra bola zavolanÃ¡, bolo zmenenÃ½ch %d zÃ¡znamov.'),$h->affected_rows)."\n";}while($h->next_result());if($ke)select($h->query("SELECT ".implode(", ",$ke)));}}echo'
<form action="" method="post">
...
<input type="submit" value="UloÅ¾iÅ¥">
<noscript><p><input type="submit" name="add" value="PridaÅ¥ stÄºpec"></noscript>
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="OdstrÃ¡niÅ¥"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=($a==""?array():view($a));$I["name"]=$a;if($_POST){if(!$l){$A=trim($_POST["name"]);drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A)." AS\n$_POST[select]","CREATE VIEW ".table($a)." AS\n$I[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($A)),'PohÄ¾ad bol odstrÃ¡nenÃ½.','PohÄ¾ad bol zmenenÃ½.','PohÄ¾ad bol vytvorenÃ½.',$a);}$I=$_POST;}page_header(($a!=""?'ZmeniÅ¥ pohÄ¾ad':'VytvoriÅ¥ pohÄ¾ad'),$l,array("table"=>$a),$a);echo'
<form action="" method="post">
<p>NÃ¡zov: <input name="name" value="',h($I["name"]),'" maxlength="64" autocapitalize="off">
<p>';textarea("select",$I["select"]);echo'<p>
<input type="submit" value="UloÅ¾iÅ¥">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="OdstrÃ¡niÅ¥"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Pc=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$zf=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$l){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'UdalosÅ¥ bola odstrÃ¡nenÃ¡.');elseif(in_array($_POST["INTERVAL_FIELD"],$Pc)&&isset($zf[$_POST["STATUS"]])){$jf="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'UdalosÅ¥ bola zmenenÃ¡.':'UdalosÅ¥ bola vytvorenÃ¡.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$jf.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$jf)."\n".$zf[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'UpraviÅ¥ udalosÅ¥'.": ".h($aa):'VytvoriÅ¥ udalosÅ¥'),$l);$I=$_POST;if(!$I&&$aa!=""){$J=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$I=reset($J);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>NÃ¡zov<td><input name="EVENT_NAME" value="',h($I["EVENT_NAME"]),'" maxlength="64" autocapitalize="off">
<tr><th title="datetime">ZaÄiatok<td><input name="STARTS" value="',h("$I[EXECUTE_AT]$I[STARTS]"),'">
<tr><th title="datetime">Koniec<td><input name="ENDS" value="',h($I["ENDS"]),'">
<tr><th>KaÅ¾dÃ½ch<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Pc,$I["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$zf,$I["STATUS"]),'<tr><th>KomentÃ¡r<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$I["ON_COMPLETION"]=="PRESERVE",'Po dokonÄenÃ­ zachovat'),'</table>
<p>';textarea("EVENT_DEFINITION",$I["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="UloÅ¾iÅ¥">
';if($aa!=""){echo'<input type="submit" name="drop" value="OdstrÃ¡niÅ¥"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$ef=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$ff=routine_languages();$I=($da==""?array("fields"=>array()):routine($da,$ef));$I["name"]=$da;if($_POST){if(!$l&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"])drop_create("DROP $ef ".idf_escape($da),create_routine($ef,$_POST),create_routine($ef,$I),substr(ME,0,-1),'ProcedÃºra bola odstrÃ¡nenÃ¡.','ProcedÃºra bola zmenenÃ¡.','ProcedÃºra bola vytvorenÃ¡.',$da);$I=$_POST;$I["fields"]=(array)$I["fields"];process_fields($I["fields"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmeniÅ¥ funkciu':'ZmeniÅ¥ procedÃºru').": ".h($da):(isset($_GET["function"])?'VytvoriÅ¥ funkciu':'VytvoriÅ¥ procedÃºru')),$l);$Ua=get_vals("SHOW CHARACTER SET");sort($Ua);echo'
<form action="" method="post" id="form">
<p>NÃ¡zov: <input name="name" value="',h($I["name"]),'" maxlength="64" autocapitalize="off">
',($ff?'Jazyk'.": ".html_select("language",$ff,$I["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($I["fields"],$Ua,$ef);if(isset($_GET["function"])){echo"<tr><td>".'NÃ¡vratovÃ½ typ';edit_type("returns",$I["returns"],$Ua);}echo'</table>
<p>';textarea("definition",$I["definition"]);echo'<p>
<input type="submit" value="UloÅ¾iÅ¥">
';if($da!=""){echo'<input type="submit" name="drop" value="OdstrÃ¡niÅ¥"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$l){$z=substr(ME,0,-1);$A=trim($_POST["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekvencia bola odstrÃ¡nenÃ¡.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$z,'Sekvencia bola vytvorenÃ¡.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$z,'Sekvencia bola zmenenÃ¡.');else
redirect($z);}page_header($fa!=""?'PozmeniÅ¥ sekvenciu'.": ".h($fa):'VytvoriÅ¥ sekvenciu',$l);$I=$_POST;if(!$I)$I=array("name"=>$fa);echo'
<form action="" method="post">
<p><input name="name" value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="UloÅ¾iÅ¥">
';if($fa!="")echo"<input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$l){$z=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ bol odstrÃ¡nenÃ½.');else
query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$z,'Typ bol vytvorenÃ½.');}page_header($ga!=""?'PozmeniÅ¥ typ'.": ".h($ga):'VytvoriÅ¥ typ',$l);$I=$_POST;if(!$I)$I=array("as"=>"AS ");echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm().">\n";else{echo"<input name='name' value='".h($I['name'])."' autocapitalize='off'>\n";textarea("as",$I["as"]);echo"<p><input type='submit' value='".'UloÅ¾iÅ¥'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$hg=trigger_options();$fg=array("INSERT","UPDATE","DELETE");$I=(array)trigger($_GET["name"])+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$hg["Timing"])&&in_array($_POST["Event"],$fg)&&in_array($_POST["Type"],$hg["Type"])){$Sd=" ON ".table($a);drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($v=="pgsql"?$Sd:""),create_trigger($Sd,$_POST),create_trigger($Sd,$I+array("Type"=>reset($hg["Type"]))),ME."table=".urlencode($a),'Trigger bol odstrÃ¡nenÃ½.','Trigger bol zmenenÃ½.','Trigger bol vytvorenÃ½.',$_GET["name"]);}$I=$_POST;}page_header(($_GET["name"]!=""?'ZmeniÅ¥ trigger'.": ".h($_GET["name"]):'VytvoriÅ¥ trigger'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>ÄŒas<td>',html_select("Timing",$hg["Timing"],$I["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>UdalosÅ¥<td>',html_select("Event",$fg,$I["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$hg["Type"],$I["Type"]),'</table>
<p>NÃ¡zov: <input name="Trigger" value="',h($I["Trigger"]),'" maxlength="64" autocapitalize="off">
...
checkbox("hashed",1,$I["hashed"],'ZahaÅ¡ovanÃ©',"typePassword(this.form['pass'], this.checked);"),'</table>

';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer' class='help'>".'OprÃ¡vnenia'."</a>";$q=0;foreach($xc
as$Nd=>$wc){echo'<th>'.($Nd!="*.*"?"<input name='objects[$q]' value='".h($Nd)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'DatabÃ¡za',"Tables"=>'TabuÄ¾ka',"Columns"=>'StÄºpec',"Procedures"=>'ProcedÃºra',)as$eb=>$ub){foreach((array)$Ie[$eb]as$He=>$Ya){echo"<tr".odd()."><td".($ub?">$ub<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($He);$q=0;foreach($xc
as$Nd=>$wc){$A="'grants[$q][".h(strtoupper($He))."]'";$X=$wc[strtoupper($He)];if($eb=="Server Admin"&&$Nd!=(isset($xc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'PovoliÅ¥'."<option value='0'".($X=="0"?" selected":"").">".'ZakÃ¡zaÅ¥'."</select>";else
...
<input type="submit" value="UloÅ¾iÅ¥">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="OdstrÃ¡niÅ¥"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$ad=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$ad++;}queries_redirect(ME."processlist=",lang(array('Bol ukonÄenÃ½ %d proces.','Boli ukonÄenÃ© %d procesy.','Bolo ukonÄenÃ½ch %d procesov.'),$ad),$ad||!$_POST["kill"]);}page_header('Zoznam procesov',$l);echo'
<form action="" method="post">
...
as$w=>$W)echo"<td>".(($v=="sql"&&$w=="Info"&&ereg("Query|Killed",$I["Command"])&&$W!="")||($v=="pgsql"&&$w=="current_query"&&$W!="<IDLE>")||($v=="oracle"&&$w=="sql_text"&&$W!="")?"<code class='jush-$v'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($I["db"]!=""?"db=".urlencode($I["db"])."&":"")."sql=".urlencode($W)).'">'.'KlonovaÅ¥'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($q+1)."/".sprintf('%d celkom',$h->result("SELECT @@max_connections")),"<p><input type='submit' value='".'UkonÄiÅ¥'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$Wc=>$V)$z.=where_link($q++,$Wc,$V);}}if(!$z&&($z=$b->selectLink($I[$w],$m))===null){if(is_mail($I[$w]))$z="mailto:$I[$w]";if($Le=is_url($I[$w]))$z=($Le=="http"&&$ba?$I[$w]:"$Le://www.adminer.org/redirect/?url=".urlencode($I[$w]));}$r=h("val[$og][".bracket_escape($w)."]");$X=$_POST["val"][$og][bracket_escape($w)];$Ac=h($X!==null?$X:$I[$w]);$od=strpos($W,"<i>...</i>");$Gb=is_utf8($W)&&$J[$Hd][$w]==$I[$w]&&!$vc[$w];$Rf=ereg('text|lob',$m["type"]);echo(($_GET["modify"]&&$Gb)||$X!==null?"<td>".($Rf?"<textarea name='$r' cols='30' rows='".(substr_count($I[$w],"\n")+1)."'>$Ac</textarea>":"<input name='$r' value='$Ac' size='$jd[$w]'>"):"<td id='$r' onclick=\"selectClick(this, event, ".($od?2:($Rf?1:0)).($Gb?"":", '".h('Pre zmenu tejto hodnoty pouÅ¾ite odkaz upraviÅ¥.')."'").");\">".$b->selectVal($W,$z,$m));}}if($Ca)echo"<td>";$b->backwardKeysPrint($Ca,$J[$Hd]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n",(!$yc&&$K?"":"<script type='text/javascript'>tableCheck();</script>\n");}if(($J||$C)&&!is_ajax()){$Ub=true;if($_GET["page"]!="last"&&+$y&&!$Qc&&($qc>=$y||$C)){$qc=found_rows($P,$Z);if($qc<max(1e4,2*($C+1)*$y))$qc=reset(slow_query("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):"")));else$Ub=false;}if(+$y&&($qc===false||$qc>$y||$C)){echo"<p class='pages'>";$td=($qc===false?$C+(count($J)>=$y?2:1):floor(($qc-1)/$y));echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'StrÃ¡nka'."', '".($C+1)."'), event); return false;\">".'StrÃ¡nka'."</a>:",pagination(0,$C).($C>5?" ...":"");for($q=max(1,$C-4);$q<min($td,$C+5);$q++)echo
pagination($q,$C);if($td>0){echo($C+5<$td?" ...":""),($Ub&&$qc!==false?pagination($td,$C):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$td'>".'poslednÃ½'."</a>");}echo(($qc===false?count($J)+1:$qc-$C*$y)>$y?' <a href="'.h(remove_from_uri("page")."&page=".($C+1)).'" onclick="return !selectLoadMore(this, '.(+$y).', \''.'Loading'.'\');">'.'Load more data'.'</a>':'');}echo"<p>\n",($qc!==false?"(".($Ub?"":"~ ").lang(array('%d riadok','%d riadky','%d riadkov'),$qc).") ":""),checkbox("all",1,0,'celÃ½ vÃ½sledok')."\n";if($b->selectCommandPrint()){echo'<fieldset><legend>UpraviÅ¥</legend><div>
<input type="submit" value="UloÅ¾iÅ¥"',($_GET["modify"]?'':' title="'.'Ctrl+kliknite na polÃ­Äko, ktorÃ© chcete zmeniÅ¥.'.'" class="jsonly"');?>>
<input type="submit" name="edit" value="UpraviÅ¥">
<input type="submit" name="clone" value="KlonovaÅ¥">
<input type="submit" name="delete" value="ZmazaÅ¥" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$qc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$oc=$b->dumpFormat();if($oc){print_fieldset("export",'Export');$le=$b->dumpOutput();echo($le?html_select("output",$le,$qa["output"])." ":""),html_select("format",$oc,$qa["format"])," <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Import',!$J);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Kb,'strlen'),$g);echo"<p><input type='hidden' name='token' value='$R'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$yf=isset($_GET["status"]);page_header($yf?'Stav':'PremennÃ©');$_g=($yf?show_status():show_variables());if(!$_g)echo"<p class='message'>".'Å½iadne riadky.'."\n";else{echo"<table cellspacing='0'>\n";foreach($_g
...
as$w=>$W)json_row("sum-$w",number_format($W,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$h->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$k=>$W)json_row("tables-".js_escape($k),$W);json_row("");}exit;}else{$Of=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Of&&!$l&&!$_POST["search"]){$G=true;$yd="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$G=truncate_tables($_POST["tables"]);$yd='TabuÄ¾ka bola vyprÃ¡zdnenÃ¡.';}elseif($_POST["move"]){$G=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$yd='TabuÄ¾ka bola presunutÃ¡.';}elseif($_POST["copy"]){$G=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$yd='TabuÄ¾ky boli skopÃ­rovanÃ©.';}elseif($_POST["drop"]){if($_POST["views"])$G=drop_views($_POST["views"]);if($G&&$_POST["tables"])$G=drop_tables($_POST["tables"]);$yd='TabuÄ¾ka bola odstrÃ¡nenÃ¡.';}elseif($v!="sql"){$G=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$yd='Tables have been optimized.';}elseif(!$_POST["tables"])$yd='Å½iadne tabuÄ¾ky.';elseif($G=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($I=$G->fetch_assoc())$yd.="<b>".h($I["Table"])."</b>: ".h($I["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$yd,$G);}page_header(($_GET["ns"]==""?'DatabÃ¡za'.": ".h(DB):'SchÃ©ma'.": ".h($_GET["ns"])),$l,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'TabuÄ¾ky a pohÄ¾ady'."</h3>\n";$Nf=tables_list();if(!$Nf)echo"<p class='message'>".'Å½iadne tabuÄ¾ky.'."\n";else{echo"<form action='' method='post'>\n","<p>".'VyhÄ¾adaÅ¥ dÃ¡ta v tabuÄ¾kÃ¡ch'.": <input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'VyhÄ¾adaÅ¥'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'TabuÄ¾ka','<td>'.'Typ','<td>'.'PorovnÃ¡vanie','<td>'.'VeÄ¾kosÅ¥ dÃ¡t','<td>'.'VeÄ¾kosÅ¥ indexu','<td>'.'VoÄ¾nÃ© miesto','<td>'.'Auto Increment','<td>'.'Riadky',(support("comment")?'<td>'.'KomentÃ¡r':''),"</thead>\n";foreach($Nf
as$A=>$S){$Bg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Bg?"views[]":"tables[]"),$A,in_array($A,$Of,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'ZobraziÅ¥ Å¡truktÃºru'.'">'.h($A).'</a>';if($Bg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'ZmeniÅ¥ pohÄ¾ad'.'">'.'PohÄ¾ad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'VypÃ­saÅ¥ dÃ¡ta'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'ZmeniÅ¥ tabuÄ¾ku'),"Index_length"=>array("indexes",'ZmeniÅ¥ indexy'),"Data_free"=>array("edit",'NovÃ¡ poloÅ¾ka'),"Auto_increment"=>array("auto_increment=1&create",'ZmeniÅ¥ tabuÄ¾ku'),"Rows"=>array("select",'VypÃ­saÅ¥ dÃ¡ta'),)as$w=>$z)echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($A)."' id='$w-".h($A)."' title='$z[1]'>?</a>":"<td id='$w-".h($A)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($Nf)),"<td>".nbsp($v=="sql"?$h->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w)echo"<td align='right' id='sum-$w'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$v)?($v!="sqlite"?"<input type='submit' value='".'AnalyzovaÅ¥'."'> ":"")."<input type='submit' name='optimize' value='".'OptimalizovaÅ¥'."'> ":"").($v=="sql"?"<input type='submit' name='check' value='".'SkontrolovaÅ¥'."'> <input type='submit' name='repair' value='".'OpraviÅ¥'."'> ":"")."<input type='submit' name='truncate' value='".'VyprÃ¡zdniÅ¥'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'OdstrÃ¡niÅ¥'."'".confirm("formChecked(this, /tables|views/)").">\n";$j=(support("scheme")?schemas():$b->databases());if(count($j)!=1&&$v!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'PresunÃºÅ¥ do inej databÃ¡zy'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'" autocapitalize="off">')," <input type='submit' name='move' value='".'PresunÃºÅ¥'."'>",(support("copy")?" <input type='submit' name='copy' value='".'KopÃ­rovaÅ¥'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'VytvoriÅ¥ tabuÄ¾ku'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'VytvoriÅ¥ pohÄ¾ad'."</a>\n";if(support("routine")){echo"<h3>".'ProcedÃºry'."</h3>\n";$gf=routines();if($gf){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'NÃ¡zov'.'<td>'.'Typ'.'<td>'.'NÃ¡vratovÃ½ typ'."<td>&nbsp;</thead>\n";odd('');foreach($gf
as$I){echo'<tr'.odd().'>','<th><a href="'.h(ME).($I["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($I["ROUTINE_NAME"]).'">'.h($I["ROUTINE_NAME"]).'</a>','<td>'.h($I["ROUTINE_TYPE"]),'<td>'.h($I["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($I["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($I["ROUTINE_NAME"]).'">'.'ZmeniÅ¥'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'VytvoriÅ¥ procedÃºru'.'</a> ':'').'<a href="'.h(ME).'function=">'.'VytvoriÅ¥ funkciu'."</a>\n";}if(support("sequence")){echo"<h3>".'Sekvencia'."</h3>\n";$qf=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema()");if($qf){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ¡zov'."</thead>\n";odd('');foreach($qf
as$W)echo"<tr".odd()."><th><a href='".h(ME)."sequence=".urlencode($W)."'>".h($W)."</a>\n";echo"</table>\n";}echo"<p><a href='".h(ME)."sequence='>".'VytvoriÅ¥ sekvenciu'."</a>\n";}if(support("type")){echo"<h3>".'UÅ¾Ã­vateÄ¾skÃ© typy'."</h3>\n";$wg=types();if($wg){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ¡zov'."</thead>\n";odd('');foreach($wg
as$W)echo"<tr".odd()."><th><a href='".h(ME)."type=".urlencode($W)."'>".h($W)."</a>\n";echo"</table>\n";}echo"<p><a href='".h(ME)."type='>".'VytvoriÅ¥ typ'."</a>\n";}if(support("event")){echo"<h3>".'Udalosti'."</h3>\n";$J=get_rows("SHOW EVENTS");if($J){echo"<table cellspacing='0'>\n","<thead><tr><th>".'NÃ¡zov'."<td>".'PlÃ¡n'."<td>".'ZaÄiatok'."<td>".'Koniec'."<td></thead>\n";foreach($J
as$I){echo"<tr>","<th>".h($I["Name"]),"<td>".($I["Execute at"]?'V stanovenÃ½ Äas'."<td>".$I["Execute at"]:'KaÅ¾dÃ½ch'." ".$I["Interval value"]." ".$I["Interval field"]."<td>$I[Starts]"),"<td>$I[Ends]",'<td><a href="'.h(ME).'event='.urlencode($I["Name"]).'">'.'ZmeniÅ¥'.'</a>';}echo"</table>\n";$Tb=$h->result("SELECT @@event_scheduler");if($Tb&&$Tb!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Tb)."\n";}echo'<p><a href="'.h(ME).'event=">'.'VytvoriÅ¥ udalosÅ¥'."</a>\n";}if($Nf)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}}}page_footer();