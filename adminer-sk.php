...
confirm($Jc="",$_f=false){return" onclick=\"".($_f?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Jc?" (' + $Jc + ')":"")."');\"";}function
js_escape($J){return
addcslashes($J,"\r\n'\\/");}function
...
upload_error($n){$Be=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Súbor sa nepodarilo nahrať.'.($Be?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Be):""):'Súbor neexistuje.');}function
...
enum_input($z,$Ua,$d,$q,$Zb=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$ta);$c=(isset($Zb)?"<label><input type='$z'$Ua value='$Zb'".((is_array($q)?in_array($Zb,$q):$q===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($ta[1]as$k=>$b){$b=stripcslashes(str_replace("''","'",$b));$fb=(is_int($q)?$q==$k+1:(is_array($q)?in_array($k+1,$q):$q===$b));$c.=" <label><input type='$z'$Ua value='".($k+1)."'".($fb?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$K){global$S,$o,$v;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$te=($v=="mssql"&&$d["auto_increment"]);if($te&&!$_POST["save"]){$K=null;}$Z=(isset($_GET["select"])||$te?array("orig"=>'originál'):array())+$o->editFunctions($d);$Ua=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Df,$Ib){$Ac=($Ib==1||(!$Ib&&'sk'=='fr')?0:('sk'=='sl'&&(!$Ib||$Ib>2)?1:0)+((!$Ib||$Ib>=5)&&ereg('cs|sk|ru|sl|pl','sk')?2:1));return
...
q($b);}function
check_sqlite_name($f){global$g;$Je="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Je)\$~",$f)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$Je));return
false;}return
true;}function
...
set_schema($Na){global$g,$S,$Oa;$c=$g->query("SET search_path TO ".idf_escape($Na));foreach(types()as$z){if(!isset($S[$z])){$S[$z]=0;$Oa['Užívateľské typy'][]=$z;}}return$c;}function
...
reset($G);}function
support($rb){return
ereg("view|drop_col|variables|status",$rb);}$v="oracle";$S=array();$Oa=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$S+=$b;$Oa[$e]=array_keys($b);}$lb=array();$ec=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$Z=array("length","lower","round","upper");$ib=array("avg","count","count distinct","max","min","sum");$Qb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$la["mssql"]="MS SQL";if(isset($_GET["mssql"])){$yc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Ec,$r=""){echo'<p class="tabs">';$Ia=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Ec)){$Ia["view"]='Zmeniť pohľad';}else{$Ia["create"]='Zmeniť tabuľku';}if(isset($r)){$Ia["edit"]='Nová položka';}foreach($Ia
...
selectColumnsPrint($P,$B){global$Z,$ib;print_fieldset("select",'Vypísať',$P);$k=0;$Wd=array('Funkcie'=>$Z,'Agregácia'=>$ib);foreach($P
...
selectLengthPrint($yb){if(isset($yb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($yb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$v;static$Jc=0;restart_session();$R="sql-".($Jc++);$ab=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$ab[$_GET["db"]][]=$j;return" <a href='#$R' onclick=\"return !toggle('$R');\">".'SQL príkaz'."</a><div id='$R' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ab[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($se[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($S[$e]==254){$b="<code>$b</code>";}}if(isset($Ia[$e])&&!$B[$Ia[$e]]){$w="edit=".urlencode($Ia[$e]);foreach($I[$Ia[$e]]as$Fc=>$qa){$w.="&where".urlencode("[".bracket_escape($Fc)."]")."=".urlencode($a[$qa]);}$b="<a href='".h(ME.$w)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($e,$d,$Y,$fa=array()){global$Oa,$S,$lb,$db;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($S[$d["type"]])?array():array($d["type"]))+$Oa+($fa?array('Cudzie kľúče'=>$fa):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($Y,$d["collation"]).'</select>',($lb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($lb,$d["unsigned"]).'</select>':''),($fa?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($db,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Rb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($Rb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $Ge, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($kd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Rb;}function
...
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Zmeniť'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB);$wb=array();$Ke=array();$f="adminer_schema";preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]),$ta,PREG_SET_ORDER);foreach($ta
...
';$ya=true;foreach($rd
as$e=>$b){if($e!=""&&$b>1){echo($ya?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$ya=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$i=$g->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$i){echo'<form action=""><p>
...
<input type="hidden" name="token" value="',$N,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"]);$gd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$gd[]=".$e";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($gd?"[".implode("|",$gd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($ab){print_fieldset("history",'História',$_GET["history"]!="");foreach($ab
...
as$f=>$d){if(!isset($d["privileges"][$pa?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ga=$_POST["referer"];if($_POST["insert"]){$ga=($pa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ga)){$ga=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$ga,'Položka bola vymazaná.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($pa?"\n".idf_escape($f)." = $b":$b);}}if($pa){if(!$r){redirect($ga);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$ga,'Položka bola aktualizovaná.');}else{$i=insert_into($m,$r);$Yd=($i?last_id():0);queries_redirect($ga,sprintf('Položka%s bola vložená.',($Yd?" $Yd":"")),$i);}}}$Ga=$o->tableName(table_status($m));page_header(($pa?'Upraviť':'Vložiť'),$n,array("select"=>array($m,$Ga)),$Ga);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$P=array();foreach($p
...
';if($m==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($_d?html_select("Engine",array(""=>"(".'úložisko'.")")+$_d,$a["Engine"]):""),' ',($Y&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$Y,$a["Collation"]):""),' <input type="submit" value="Uložiť">
...
',(support("comment")?checkbox("","",$Rb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Rb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$N,'">
';if(support("partitioning")){$je=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$ae,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($je||!$a["partition_by"]?" class='hidden'":""),'>
...
as$t){if(count($_)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$Kd=false;}$sb=$t;}}queries_redirect(ME."db=".urlencode($sb),'Databáza bola vytvorená.',$Kd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$n,array(),DB);$Y=collations();$f=DB;$lc=null;if($_POST){$f=$_POST["name"];$lc=$_POST["collation"];}elseif(DB!=""){$lc=db_collation(DB,$Y);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$ia){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ia,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Y?html_select("collation",array(""=>"(".'porovnávanie'.")")+$Y,$lc):"");?>
...
';}elseif(isset($_GET["event"])){$pb=$_GET["event"];$Ae=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$yd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($pb),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Ae)&&isset($yd[$_POST["STATUS"]])){$Zd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($pb!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($pb!=""?"ALTER EVENT ".idf_escape($pb).$Zd.($pb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Zd)."\n".$yd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($pb!=""?'Upraviť udalosť'.": ".h($pb):'Vytvoriť udalosť'),$n);$a=array();if($_POST){$a=$_POST;}elseif($pb!=""){$G=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($pb));$a=reset($G);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ae,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$yd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($pb!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$N,'">
</form>
...
<input type="submit" value="Uložiť">
';if($Qa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$N,'">
</form>
';}elseif(isset($_GET["sequence"])){$Db=$_GET["sequence"];if($_POST&&!$n){$w=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($Db),$w,'Sekvencia bola odstránená.');}elseif($Db==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$w,'Sekvencia bola vytvorená.');}elseif($Db!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($Db)." RENAME TO ".idf_escape($_POST["name"]),$w,'Sekvencia bola zmenená.');}else{redirect($w);}}page_header($Db!=""?'Pozmeniť sekvenciu'.": ".h($Db):'Vytvoriť sekvenciu',$n);$a=array("name"=>$Db);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Uložiť">
';if($Db!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$N,'">
...
as$Za=>$ia){echo'<th>'.($Za!="*.*"?"<input name='objects[$k]' value='".h($Za)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Dc=>$xc){foreach((array)$ra[$Dc]as$kc=>$Aa){echo"<tr".odd()."><td".($xc?">$xc<td":" colspan='2'").' lang="en" title="'.h($Aa).'">'.h($kc);$k=0;foreach($jb
