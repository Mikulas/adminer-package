...
upload_error($n){$xe=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Súbor sa nepodarilo nahrať.'.($xe?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$xe):""):'Súbor neexistuje.');}function
...
input($c,$p,$O){global$T,$r,$_;$f=h(bracket_escape($c["field"]));echo"<td class='function'>";$V=(isset($_GET["select"])?array("orig"=>'originál'):array())+$r->editFunctions($c);$Qa=" name='fields[$f]'";if($c["type"]=="enum"){echo
...
lang($Ve,$nc){$jc=($nc==1||(!$nc&&'sk'=='fr')?0:((!$nc||$nc>=5)&&ereg('cs|sk|ru','sk')?2:1));return
...
false;}return
true;}function
create_database($s,$S){global$g;if(file_exists($s)){$g->error='Súbor existuje.';return
...
reset($F);}function
support($ob){return
ereg("view|drop_col|variables|status",$ob);}$_="oracle";$T=array();$Oa=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$T+=$b;$Oa[$e]=array_keys($b);}$nb=array();$Nb=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL");$V=array("length","lower","round","upper");$qb=array("avg","count","count distinct","max","min","sum");$Vb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ha[]="SQLSRV";$Ha[]="MSSQL";if(extension_loaded("sqlsrv")||extension_loaded("mssql")){$ja["mssql"]="MS SQL";}if(isset($_GET["mssql"])){define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Systém<td>',html_select("driver",$ja,DRIVER),'<tr><th>Server<td><input name="server" value="',h(SERVER),'">
...
selectLinks($Xc,$q=""){echo'<p class="tabs">';$Ka=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Xc)){$Ka["view"]='Zmeniť pohľad';}else{$Ka["create"]='Zmeniť tabuľku';}if(isset($q)){$Ka["edit"]='Nová položka';}foreach($Ka
...
selectQuery($j){global$_;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Stránka'.": ".'posledný'."'>&gt;&gt;</a> <code class='jush-$_'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Upraviť'."</a>\n";}function
rowDescription($h){return"";}function
rowDescriptions($F,$mf){return$F;}function
selectVal($b,$x,$c){$d=($b!="<i>NULL</i>"&&ereg("^char|binary",$c["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$c["type"])&&!is_utf8($b)){$d=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($x?"<a href='$x'>$d</a>":$d);}function
editVal($b,$c){return(ereg("binary",$c["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($C,$A){global$V,$qb;print_fieldset("select",'Vypísať',$C);$k=0;$ne=array('Funkcie'=>$V,'Agregácia'=>$qb);foreach($C
...
selectSearchPrint($t,$A,$K){print_fieldset("search",'Vyhľadať',$t);foreach($K
as$k=>$v){if($v["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$v["columns"]))."</i>) AGAINST"," <input name='fulltext[$k]' value='".h($_GET["fulltext"][$k])."'>",checkbox("boolean[$k]",1,isset($_GET["boolean"][$k]),"BOOL"),"<br>\n";}}$k=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$k][col]'><option value=''>(".'kdekoľvek'.")".optionlist($A,$b["col"],true)."</select>",html_select("where[$k][op]",$this->operators,$b["op"]),"<input name='where[$k][val]' value='".h($b["val"])."'></div>\n";$k++;}}echo"<div><select name='where[$k][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoľvek'.")".optionlist($A,null,true)."</select>",html_select("where[$k][op]",$this->operators),"<input name='where[$k][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($ub,$A,$K){print_fieldset("sort",'Zotriediť',$ub);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($A[$b])){echo"<div><select name='order[$k]'><option>".optionlist($A,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'zostupne')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($A,null,true)."</select>",checkbox("desc[$k]",1,0,'zostupne')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($_b){if(isset($_b)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($_b).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$_;restart_session();$U="sql-".count($_SESSION["messages"]);$eb=&get_session("queries");$eb[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$U' onclick=\"return !toggle('$U');\">".'SQL príkaz'."</a><div id='$U' class='hidden'><pre class='jush-$_'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($eb[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$d);}function
editInput($h,$c,$Qa,$p){if($c["type"]=="enum"){return($c["null"]?"<label><input type='radio'$Qa value=''".(isset($p)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Qa value='0'".($p===0?" checked":"")."><i>".'prázdne'."</i></label>";}return"";}function
...
dumpOutput($C,$p=""){$d=array('text'=>'otvoriť','file'=>'uložiť');if(function_exists('gzencode')){$d['gz']='gzip';}if(function_exists('bzcompress')){$d['bz2']='bzip2';}return
...
<p class="logout">
';if(DB==""||!$ac){echo"<a href='".h(ME)."sql='>".bold('SQL príkaz',isset($_GET["sql"]))."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."'>".bold('Export',isset($_GET["dump"]))."</a>\n";}}echo'<input type="hidden" name="token" value="',$L,'">
<input type="submit" name="logout" value="Odhlásiť">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($z?html_select("db",array(""=>"(".'databáza'.")")+$z,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrať"',($z?" class='hidden'":""),'>
';if($ac!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$ac){$D=tables_list();if(!$D){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($D);$Ka=array();foreach($D
...
as$h=>$y){echo'<a href="'.h(ME).'select='.urlencode($h).'">'.bold('vypísať',$_GET["select"]==$h).'</a> ','<a href="'.h(ME).'table='.urlencode($h).'">'.bold($this->tableName(array("Name"=>$h)),$_GET["table"]==$h)."</a><br>\n";}}}$r=(function_exists('adminer_object')?adminer_object():new
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Be[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($T[$e]==254){$b="<code>$b</code>";}}if(isset($Ka[$e])&&!$A[$Ka[$e]]){$x="edit=".urlencode($Ka[$e]);foreach($K[$Ka[$e]]as$yc=>$la){$x.="&where".urlencode("[".bracket_escape($yc)."]")."=".urlencode($a[$la]);}$b="<a href='".h(ME.$x)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($e,$c,$W,$ca=array()){global$Oa,$T,$nb,$fb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$c["type"]||isset($T[$c["type"]])?array():array($c["type"]))+$Oa+($ca?array('Cudzie kľúče'=>$ca):array()),$c["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($c["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$c["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($W,$c["collation"]).'</select>',($nb?"<select name='$e"."[unsigned]'".(!$c["type"]||ereg('(int|float|double|decimal)$',$c["type"])?"":" class='hidden'").'><option>'.optionlist($nb,$c["unsigned"]).'</select>':''),($ca?"<select name='$e"."[on_delete]'".(ereg("`",$c["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($fb,$c["on_delete"])."</select> ":" ");}function
...
';if($y=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($y=="TABLE"?'Názov stĺpca':'Názov parametra'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Dĺžka
<td>Voľby
...
',(support("comment")?"<td".($Cb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($Cb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($c["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $Pe, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Cd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Cb;}function
...
';$ib=true;foreach($_d
as$e=>$b){if($e!=""&&$b>1){echo($ib?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$ib=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$i=$g->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$i){echo'<form action=""><p>
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe');print_fieldset("webfile",'Zo serveru',$_POST["webfile"]);$md=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$md[]=".$e";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($md?"[".implode("|",$md)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($eb){print_fieldset("history",'História',$_GET["history"]!="");foreach($eb
as$e=>$b){echo'<a href="'.h(ME."sql=&history=$e").'">'.'Upraviť'."</a> <code class='jush-$_'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$c){if(!isset($c["privileges"][$na?"update":"insert"])||$r->fieldName($c)==""){unset($o[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ka=$_POST["referer"];if($_POST["insert"]){$ka=($na?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ka)){$ka=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$ka,'Položka bola vymazaná.');}else{$q=array();foreach($o
as$f=>$c){$b=process_input($c);if($b!==false&&$b!==null){$q[idf_escape($f)]=($na?"\n".idf_escape($f)." = $b":$b);}}if($na){if(!$q){redirect($ka);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$q),"\nWHERE $t"),$ka,'Položka bola aktualizovaná.');}else{$i=insert_into($m,$q);$ce=($i?last_id():0);queries_redirect($ka,sprintf('Položka%s bola vložená.',($ce?" $ce":"")),$i);}}}$Da=$r->tableName(table_status($m));page_header(($na?'Upraviť':'Vložiť'),$n,array("select"=>array($m,$Da)),$Da);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$C=array();foreach($o
...
<?php echo(support("comment")?checkbox("","",$Cb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Cb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložiť">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstrániť"',$Xa,'>';}if(support("partitioning")){$Vd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$de,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Vd||!$a["partition_by"]?" class='hidden'":""),'>
...
as$s){if(count($z)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$Md=false;}$rb=$s;}}queries_redirect(ME."db=".urlencode($rb),'Databáza bola vytvorená.',$Md);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$n,array(),DB);$W=collations();$f=DB;$xc=null;if($_POST){$f=$_POST["name"];$xc=$_POST["collation"];}elseif(DB!=""){$xc=db_collation(DB,$W);}elseif($_=="sql"){foreach(get_vals("SHOW GRANTS")as$ga){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ga,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea name="name" rows="10" cols="40" onkeydown="return textareaKeydown(this, event);">'.h($f).'</textarea><br>':'<input name="name" value="'.h($f).'" maxlength="64">')."\n".($W?html_select("collation",array(""=>"(".'porovnávanie'.")")+$W,$xc):""),'<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložiť">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$Xa>\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Pridať ďalší'."'>\n";}echo'</form>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Xa,'>';}echo'<input type="hidden" name="token" value="',$L,'">
</form>
...
';}elseif(isset($_GET["event"])){$lb=$_GET["event"];$fe=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$zd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($lb),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$fe)&&isset($zd[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($lb!=""?"ALTER EVENT ".idf_escape($lb).$Td.($lb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$zd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($lb!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'));}}page_header(($lb!=""?'Upraviť udalosť'.": ".h($lb):'Vytvoriť udalosť'),$n);$a=array();if($_POST){$a=$_POST;}elseif($lb!=""){$F=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($lb));$a=reset($F);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$fe,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$zd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložiť">
';if($lb!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Xa,'>';}echo'</form>
...
as$c){if($c["field"]!=""){$q[]=(in_array($c["inout"],$wc)?"$c[inout] ":"").idf_escape($c["field"]).process_type($c,"CHARACTER SET");}}$Ya=drop_create("DROP $Ta ".idf_escape($Ua),"CREATE $Ta ".idf_escape($_POST["name"])." (".implode(", ",$q).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$Ua);}page_header(($Ua!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($Ua):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$n);$W=get_vals("SHOW CHARACTER SET");sort($W);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Ua!=""){$a=routine($Ua,$Ta);$a["name"]=$Ua;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$W,$Ta);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$a["returns"],$W);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$L,'">
';if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Xa,'>';}echo'</form>
';}elseif(isset($_GET["sequence"])){$yb=$_GET["sequence"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($yb),$x,'Sekvencia bola odstránená.');}elseif($yb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$x,'Sekvencia bola vytvorená.');}elseif($yb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($yb)." RENAME TO ".idf_escape($_POST["name"]),$x,'Sekvencia bola zmenená.');}else{redirect($x);}}page_header($yb!=""?'Pozmeniť sekvenciu'.": ".h($yb):'Vytvoriť sekvenciu',$n);$a=array("name"=>$yb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložiť">
';if($yb!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$Xa>\n";}echo'</form>
';}elseif(isset($_GET["type"])){$Yc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Yc),$x,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ bol vytvorený.');}}page_header($Yc!=""?'Pozmeniť typ'.": ".h($Yc):'Vytvoriť typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$L,'">
';if($Yc!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$Xa>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Kc["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($m,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$ee,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Kc["Type"],$a["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$L,'">
';if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Xa,'>';}echo'</form>
...
as$b){if($b[1]!="USAGE"){$bb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$bb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$Nc=$l[1];}}}if($_POST&&!$n){$bc=(isset($_GET["host"])?q($ud)."@".q($_GET["host"]):"''");$cb=q($_POST["user"])."@".q($_POST["host"]);$Dd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $bc",ME."privileges=",'Používateľ bol odstránený.');}else{if($bc!=$cb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $cb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Dd");}elseif($_POST["pass"]!=$Nc||!$_POST["hashed"]){queries("SET PASSWORD FOR $cb = ".($_POST["hashed"]?$Dd:"PASSWORD($Dd)"));}if(!$n){$vc=array();foreach($qc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html'>".'Oprávnenia'."</a>";$k=0;foreach($bb
as$Za=>$ga){echo'<th>'.($Za!="*.*"?"<input name='objects[$k]' value='".h($Za)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Uc=>$hc){foreach((array)$ma[$Uc]as$uc=>$ua){echo"<tr".odd()."><td".($hc?">$hc<td":" colspan='2'").' lang="en" title="'.h($ua).'">'.h($uc);$k=0;foreach($bb
as$Za=>$ga){$f="'grants[$k][".h(strtoupper($uc))."]'";$p=$ga[strtoupper($uc)];if($Uc=="Server Admin"&&$Za!=(isset($bb["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($p?" selected":"").">".'Povoliť'."<option value='0'".($p=="0"?" selected":"").">".'Zakázať'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($p?" checked":"").($uc=="All privileges"?" id='grants-$k-all'":($uc=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$k-all');\"")).">";}$k++;}}}echo"</table>\n",'<p>
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',$Xa,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$bd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$bd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$bd),$bd||!$_POST["kill"]);}page_header('Zoznam procesov',$n);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$Sa,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Export');echo$r->dumpOutput(1,$pd["output"])." ".$r->dumpFormat(1,$pd["format"]);echo" <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}print_fieldset("import",'Import CSV',!$F);echo"<input type='hidden' name='token' value='$L'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;"),$pd["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>\n","</div></fieldset>\n";$r->selectEmailPrint(array_filter($Sc,'strlen'),$A);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ob=isset($_GET["status"]);page_header($Ob?'Stav':'Premenné');$Qd=($Ob?show_status():show_variables());if(!$Qd){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Qd
