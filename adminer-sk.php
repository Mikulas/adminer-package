...
confirm($ab="",$bf=false){return" onclick=\"".($bf?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($ab?" (' + $ab + ')":"")."');\"";}function
...
upload_error($k){$ed=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Súbor sa nepodarilo nahrať.'.($ed?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$ed):""):'Súbor neexistuje.');}function
...
enum_input($S,$xa,$l,$X,$Cb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Zc);$G=(isset($Cb)?"<label><input type='$S'$xa value='$Cb'".((is_array($X)?in_array($Cb,$X):$X===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($Zc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ja=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($q+1)."'".($Ja?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$v;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$Be=($v=="mssql"&&$l["auto_increment"]);if($Be&&!$_POST["save"]){$o=null;}$lc=(isset($_GET["select"])||$Be?array("orig"=>'originál'):array())+$b->editFunctions($l);$xa=" name='fields[$A]'";if($l["type"]=="enum"){echo
...
lang($Ef,$vd){$de=($vd==1?0:($vd&&$vd<5?1:2));return
...
q($W);}function
check_sqlite_name($A){global$g;$Ub="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ub)\$~",$A)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$Ub));return
false;}return
true;}function
...
set_schema($Le){global$g,$T,$df;$G=$g->query("SET search_path TO ".idf_escape($Le));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$df['Užívateľské typy'][]=$S;}}return$G;}function
...
reset($I);}function
support($Wb){return
ereg("view|drop_col|variables|status",$Wb);}$v="oracle";$T=array();$df=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$df[$w]=array_keys($W);}$Rf=array();$Ed=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$lc=array("length","lower","round","upper");$pc=array("avg","count","count distinct","max","min","sum");$yb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ge=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Systém<td>',html_select("driver",$sb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($kf,$L=""){echo'<p class="tabs">';$Uc=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($kf)){$Uc["view"]='Zmeniť pohľad';}else{$Uc["create"]='Zmeniť tabuľku';}if(isset($L)){$Uc["edit"]='Nová položka';}foreach($Uc
...
selectVal($W,$z,$l){$G=($W!="<i>NULL</i>"&&ereg("char|binary",$l["type"])&&!ereg("var",$l["type"])?"<code>$W</code>":$W);if(ereg('blob|bytea|raw|file',$l["type"])&&!is_utf8($W)){$G=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($W,ENT_QUOTES)));}return($z?"<a href='$z'>$G</a>":$G);}function
editVal($W,$l){return(ereg("binary",$l["type"])?reset(unpack("H*",$W)):$W);}function
selectColumnsPrint($J,$f){global$lc,$pc;print_fieldset("select",'Vypísať',$J);$q=0;$kc=array('Funkcie'=>$lc,'Agregácia'=>$pc);foreach($J
...
as$q=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$q=0;foreach((array)$_GET["where"]as$W){if("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators)){echo"<div><select name='where[$q][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$q][op]",$this->operators,$W["op"]),"<input name='where[$q][val]' value='".h($W["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoľvek'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",$this->operators,"="),"<input name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Id,$f,$u){print_fieldset("sort",'Zotriediť',$Id);$q=0;foreach((array)$_GET["order"]as$w=>$W){if(isset($f[$W])){echo"<div><select name='order[$q]'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'zostupne')."</div>\n";$q++;}}echo"<div><select name='order[$q]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$q]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($y){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($y)."'>","</div></fieldset>\n";}function
selectLengthPrint($vf){if(isset($vf)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($vf).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($E){global$v;static$ab=0;restart_session();$r="sql-".($ab++);$rc=&get_session("queries");if(strlen($E)>1e6){$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";}$rc[$_GET["db"]][]=$E;return" <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL príkaz'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($rc[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$G);}function
editInput($O,$l,$xa,$X){if($l["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$xa value='-1' checked><i>".'originál'."</i></label> ":"").($l["null"]?"<label><input type='radio'$xa value=''".(isset($X)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$xa,$l,$X,0);}return"";}function
...
<p class="logout">
';if(DB==""||!$nd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL príkaz'."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}}echo'<input type="submit" name="logout" value="Odhlásiť" onclick="eventStop(event);">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($i?html_select("db",array(""=>"(".'databáza'.")")+$i,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrať"',($i?" class='hidden'":""),' onclick="eventStop(event);">
';if($nd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$nd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$Q=tables_list();if(!$Q){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($Q);$Uc=array();foreach($Q
...
as$w=>$W){if(!isset($W)){$W="<i>NULL</i>";}elseif($Da[$w]&&!is_utf8($W)){$W="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W))."</i>";}elseif(!strlen($W)){$W="&nbsp;";}else{$W=h($W);if($T[$w]==254){$W="<code>$W</code>";}}if(isset($Uc[$w])&&!$f[$Uc[$w]]){if($uc){$z=$Uc[$w].urlencode($H[array_search("table=",$Uc)]);}else{$z="edit=".urlencode($Uc[$w]);foreach($u[$Uc[$w]]as$Ma=>$Hc){$z.="&where".urlencode("[".bracket_escape($Ma)."]")."=".urlencode($H[$Hc]);}}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($w,$l,$Oa,$ec=array()){global$df,$T,$Rf,$Ad;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$df+($ec?array('Cudzie kľúče'=>$ec):array()),$l["type"]),'</select>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Ld==""||support("drop_col")?"<input type='image' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ua=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W)){$Ua[]=".$w";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($Ua?"[".implode("|",$Ua)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($rc){print_fieldset("history",'História',$_GET["history"]!="");foreach($rc
as$w=>$W){echo'<a href="'.h(ME."sql=&history=$w").'">'.'Upraviť'."</a> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$W)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$A=>$l){if(!isset($l["privileges"][$Sf?"update":"insert"])||$b->fieldName($l)==""){unset($m[$A]);}}if($_POST&&!$k&&!isset($_GET["select"])){$Vc=$_POST["referer"];if($_POST["insert"]){$Vc=($Sf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$Vc)){$Vc=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$Vc,'Položka bola vymazaná.');}else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null){$L[idf_escape($A)]=($Sf?"\n".idf_escape($A)." = $W":$W);}}if($Sf){if(!$L){redirect($Vc);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$Vc,'Položka bola aktualizovaná.');}else{$F=insert_into($a,$L);$Oc=($F?last_id():0);queries_redirect($Vc,sprintf('Položka%s bola vložená.',($Oc?" $Oc":"")),$F);}}}$lf=$b->tableName(table_status($a));page_header(($Sf?'Upraviť':'Vložiť'),$k,array("select"=>array($a,$lf)),$lf);$H=null;if($_POST["save"]){$H=(array)$_POST["fields"];}elseif($Z){$J=array();foreach($m
as$A=>$l){if(isset($l["privileges"]["select"])){$J[]=($_POST["clone"]&&$l["auto_increment"]?"'' AS ":(ereg("enum|set",$l["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false){echo"<p class='error'>".'Žiadne riadky.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($m){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Sf?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($Sf?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Ta,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($H["Comment"]).'" maxlength="60"'.($Ta?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
';if(support("partitioning")){$Wd=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Vd,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($Wd||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header('Zavolať'.": ".h($da),$k);$Fe=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$xc=array();$Pd=array();foreach($Fe["fields"]as$q=>$l){if(substr($l["inout"],-3)=="OUT"){$Pd[$q]="@".idf_escape($l["field"])." AS ".idf_escape($l["field"]);}if(!$l["inout"]||substr($l["inout"],0,2)=="IN"){$xc[]=$q;}}if(!$k&&$_POST){$Ha=array();foreach($Fe["fields"]as$w=>$l){if(in_array($w,$xc)){$W=process_input($l);if($W===false){$W="''";}if(isset($Pd[$w])){$g->query("SET @".idf_escape($l["field"])." = $W");}}$Ha[]=(isset($Pd[$w])?"@".idf_escape($l["field"]):$W);}$E=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$Ha).")";echo"<p><code class='jush-$v'>".h($E)."</code> <a href='".h(ME)."sql=".urlencode($E)."'>".'Upraviť'."</a>\n";if(!$g->multi_query($E)){echo"<p class='error'>".error()."\n";}else{$h=connect();if(is_object($h)){$h->select_db(DB);}do{$F=$g->store_result();if(is_object($F)){select($F,$h);}else{echo"<p class='message'>".lang(array('Procedúra bola zavolaná, bol zmenený %d záznam.','Procedúra bola zavolaná, boli zmenené %d záznamy.','Procedúra bola zavolaná, bolo zmenených %d záznamov.'),$g->affected_rows)."\n";}}while($g->next_result());if($Pd){select($g->query("SELECT ".implode(", ",$Pd)));}}}echo'
<form action="" method="post">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ec,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$af,$H["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Hf["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$Gf,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Hf["Type"],$H["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($ub){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$W){if($W[1]!="USAGE"){$mc["$_[2]$W[2]"][$W[1]]=true;}if(ereg(' WITH GRANT OPTION',$H[0])){$mc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_)){$yd=$_[1];}}}if($_POST&&!$k){$zd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$td=q($_POST["user"])."@".q($_POST["host"]);$Zd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $zd",ME."privileges=",'Používateľ bol odstránený.');}else{$db=false;if($zd!=$td){$db=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $td IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Zd");$k=!$db;}elseif($_POST["pass"]!=$yd||!$_POST["hashed"]){queries("SET PASSWORD FOR $td = ".($_POST["hashed"]?$Zd:"PASSWORD($Zd)"));}if(!$k){$Ce=array();foreach($sd
...
as$wd=>$Ce){if(preg_match('~^(.+)(\\(.*\\))?$~U',$wd,$_)){grant("REVOKE",array_keys($Ce),$_[2]," ON $_[1] FROM $td");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$k);if($db){$g->query("DROP USER $td");}}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$ha@$_GET[host]"):'Vytvoriť používateľa'),$k,array("privileges"=>array('','Oprávnenia')));if($_POST){$H=$_POST;$mc=$sd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$yd;if($yd!=""){$H["hashed"]=true;}$mc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$q=0;foreach($mc
as$wd=>$p){echo'<th>'.($wd!="*.*"?"<input name='objects[$q]' value='".h($wd)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Ya=>$nb){foreach((array)$ne[$Ya]as$me=>$Sa){echo"<tr".odd()."><td".($nb?">$nb<td":" colspan='2'").' lang="en" title="'.h($Sa).'">'.h($me);$q=0;foreach($mc
as$wd=>$p){$A="'grants[$q][".h(strtoupper($me))."]'";$X=$p[strtoupper($me)];if($Ya=="Server Admin"&&$wd!=(isset($mc["*.*"])?"*.*":".*")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Povoliť'."<option value='0'".($X=="0"?" selected":"").">".'Zakázať'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$A value='1'".($X?" checked":"").($me=="All privileges"?" id='grants-$q-all'":($me=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$q-all');\"")).">";}$q++;}}}echo"</table>\n",'<p>
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Kc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W))){$Kc++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$Kc),$Kc||!$_POST["kill"]);}page_header('Zoznam procesov',$k);echo'
<form action="" method="post">
...
as$w=>$W){echo"<td>".(($v=="sql"?$w=="Info"&&$W!="":$w=="current_query"&&$W!="<IDLE>")?"<code class='jush-$v'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($H["db"]!=""?"db=".urlencode($H["db"])."&":"")."sql=".urlencode($W)).'">'.'Upraviť'.'</a>':nbsp($W));}echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
