...
upload_error($n){$Qe=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Súbor sa nepodarilo nahrať.'.($Qe?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Qe):""):'Súbor neexistuje.');}function
...
input($c,$p,$N){global$T,$r,$_;$f=h(bracket_escape($c["field"]));echo"<td class='function'>";$V=(isset($_GET["select"])?array("orig"=>'originál'):array())+$r->editFunctions($c);$Pa=" name='fields[$f]'";if($c["type"]=="enum"){echo
...
lang($Se,$hc){$qc=($hc==1||(!$hc&&'sk'=='fr')?0:((!$hc||$hc>=5)&&ereg('cs|sk|ru','sk')?2:1));return
...
q($b);}function
create_database($s,$R){global$g;if(file_exists($s)){$g->error='Súbor existuje.';return
false;}$x=new
...
false;}}return
true;}function
rename_database($f,$R){global$g;$g->Min_SQLite(":memory:");$g->error='Súbor existuje.';return@rename(DB,$f);}function
...
reset($D);}function
support($qb){return
ereg("view|drop_col|variables|status",$qb);}$_="oracle";$T=array();$Sa=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$T+=$b;$Sa[$e]=array_keys($b);}$pb=array();$Vb=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL");$V=array("length","lower","round","upper");$ob=array("avg","count","count distinct","max","min","sum");$Yb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ja[]="SQLSRV";$Ja[]="MSSQL";if(extension_loaded("sqlsrv")||extension_loaded("mssql")){$ja["mssql"]="MS SQL";}if(isset($_GET["mssql"])){define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Wc,$q=""){echo'<p class="tabs">';$Ia=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Wc)){$Ia["view"]='Zmeniť pohľad';}else{$Ia["create"]='Zmeniť tabuľku';}if(isset($q)){$Ia["edit"]='Nová položka';}foreach($Ia
...
selectColumnsPrint($C,$B){global$V,$ob;print_fieldset("select",'Vypísať',$C);$k=0;$ge=array('Funkcie'=>$V,'Agregácia'=>$ob);foreach($C
...
selectOrderPrint($ub,$B,$K){print_fieldset("sort",'Zotriediť',$ub);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'zostupne')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>",checkbox("desc[$k]",1,0,'zostupne')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($L){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($L)."'>","</div></fieldset>\n";}function
selectLengthPrint($Bb){if(isset($Bb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($Bb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$_;restart_session();$U="sql-".count($_SESSION["messages"]);$lb=&get_session("queries");$lb[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$U' onclick=\"return !toggle('$U');\">".'SQL príkaz'."</a><div id='$U' class='hidden'><pre class='jush-$_'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($lb[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$d);}function
editInput($h,$c,$Pa,$p){if($c["type"]=="enum"){return($c["null"]?"<label><input type='radio'$Pa value=''".(isset($p)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Pa value='0'".($p===0?" checked":"")."><i>".'prázdne'."</i></label>";}return"";}function
...
';if($Lc!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""){$E=tables_list();if(!$E){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($E);$Ia=array();foreach($E
...
auth_error($le=null){global$g,$r,$P;$Fc=session_name();$n="";if(!$_COOKIE[$Fc]&&$_GET[$Fc]&&ini_bool("session.use_only_cookies")){$n='Session premenné musia byť povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Fc]||$_GET[$Fc])&&!$P){$n='Session vypršala, prihláste sa prosím znova.';}else{$S=&get_session("pwds");if(isset($S)){$n=h($le?$le->getMessage():(is_string($g)?$g:'Neplatné prihlasovacie údaje.'));$S=null;}}}page_header('Prihlásiť sa',$n,null);echo"<form action='' method='post'>\n";$r->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])&&class_exists("Min_DB")){$g=connect();}if(is_string($g)||!$r->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$P=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$P;}$n=($_POST?($_POST["token"]==$P?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));function
connect_error(){global$g,$P,$n,$ja;$z=array();if(DB!=""){page_header('Databáza'.": ".h(DB),'Nesprávna databáza.',true);}else{if($_POST["db"]&&!$n){set_session("dbs",null);queries_redirect(substr(ME,0,-1),'Databázy boli odstránené.',drop_databases($_POST["db"]));}page_header('Vybrať databázu',$n,false);echo"<p><a href='".h(ME)."database='>".'Vytvoriť novú databázu'."</a>\n";foreach(array('privileges'=>'Oprávnenia','processlist'=>'Zoznam procesov','variables'=>'Premenné','status'=>'Stav',)as$e=>$b){if(support($e)){echo"<a href='".h(ME)."$e='>$b</a>\n";}}echo"<p>".sprintf('Verzia %s: %s cez PHP rozšírenie %s',$ja[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Prihlásený ako: %s',"<b>".h(logged_user())."</b>")."\n";$z=get_databases();if($z){$X=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td><input type='hidden' name='token' value='$P'>&nbsp;<th>".'Databáza'."<td>".'Porovnávanie'."<td>".'Tabuľky'."</thead>\n";foreach($z
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($oe[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($T[$e]==254){$b="<code>$b</code>";}}if(isset($Ia[$e])&&!$B[$Ia[$e]]){$x="edit=".urlencode($Ia[$e]);foreach($K[$Ia[$e]]as$Ic=>$na){$x.="&where".urlencode("[".bracket_escape($Ic)."]")."=".urlencode($a[$na]);}$b="<a href='".h(ME.$x)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($e,$c,$X,$ba=array()){global$Sa,$T,$pb,$db;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$c["type"]||isset($T[$c["type"]])?array():array($c["type"]))+$Sa+($ba?array('Cudzie kľúče'=>$ba):array()),$c["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($c["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$c["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($X,$c["collation"]).'</select>',($pb?"<select name='$e"."[unsigned]'".(!$c["type"]||ereg('(int|float|double|decimal)$',$c["type"])?"":" class='hidden'").'><option>'.optionlist($pb,$c["unsigned"]).'</select>':''),($ba?"<select name='$e"."[on_delete]'".(ereg("`",$c["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($db,$c["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Fb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($Fb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($c["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $me, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($td==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Fb;}function
...
';$fb=true;foreach($bd
as$e=>$b){if($e!=""&&$b>1){echo($fb?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$fb=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$i=$g->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$i){echo'<form action=""><p>
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe');print_fieldset("webfile",'Zo serveru',$_POST["webfile"]);$cd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$cd[]=".$e";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($cd?"[".implode("|",$cd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($lb){print_fieldset("history",'História',$_GET["history"]!="");foreach($lb
...
as$f=>$c){if(!isset($c["privileges"][$la?"update":"insert"])||$r->fieldName($c)==""){unset($o[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ka=$_POST["referer"];if($_POST["insert"]){$ka=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ka)){$ka=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$ka,'Položka bola vymazaná.');}else{$q=array();foreach($o
as$f=>$c){$b=process_input($c);if($b!==false&&$b!==null){$q[idf_escape($f)]=($la?"\n".idf_escape($f)." = $b":$b);}}if($la){if(!$q){redirect($ka);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$q),"\nWHERE $t"),$ka,'Položka bola aktualizovaná.');}else{$i=insert_into($m,$q);$Xd=($i?last_id():0);queries_redirect($ka,sprintf('Položka%s bola vložená.',($Xd?" $Xd":"")),$i);}}}$Fa=$r->tableName(table_status($m));page_header(($la?'Upraviť':'Vložiť'),$n,array("select"=>array($m,$Fa)),$Fa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$C=array();foreach($o
...
<?php echo(support("comment")?checkbox("","",$Fb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Fb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Uložiť">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstrániť"',$Va,'>';}if(support("partitioning")){$Wd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Rd,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Wd||!$a["partition_by"]?" class='hidden'":""),'>
...
as$s){if(count($z)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$Od=false;}$mb=$s;}}queries_redirect(ME."db=".urlencode($mb),'Databáza bola vytvorená.',$Od);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"])." COLLATE ".q($_POST["collation"]),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$n,array(),DB);$X=collations();$f=DB;$kc=null;if($_POST){$f=$_POST["name"];$kc=$_POST["collation"];}elseif(DB!=""){$kc=db_collation(DB,$X);}elseif($_=="sql"){foreach(get_vals("SHOW GRANTS")as$ga){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ga,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input name="name" value="'.h($f).'" maxlength="64">')."\n".($X?html_select("collation",array(""=>"(".'porovnávanie'.")")+$X,$kc):""),'<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Uložiť">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$Va>\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Pridať ďalší'."'>\n";}echo'</form>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Va,'>';}echo'<input type="hidden" name="token" value="',$P,'">
</form>
...
';}elseif(isset($_GET["event"])){$gb=$_GET["event"];$Gd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Dd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($gb),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Gd)&&isset($Dd[$_POST["STATUS"]])){$Md="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($gb!=""?"ALTER EVENT ".idf_escape($gb).$Md.($gb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Md)."\n".$Dd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($gb!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'));}}page_header(($gb!=""?'Upraviť udalosť'.": ".h($gb):'Vytvoriť udalosť'),$n);$a=array();if($_POST){$a=$_POST;}elseif($gb!=""){$D=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($gb));$a=reset($D);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Gd,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Dd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Uložiť">
';if($gb!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Va,'>';}echo'</form>
...
as$c){if($c["field"]!=""){$q[]=(in_array($c["inout"],$tc)?"$c[inout] ":"").idf_escape($c["field"]).process_type($c,"CHARACTER SET");}}$Za=drop_create("DROP $Ta ".idf_escape($Oa),"CREATE $Ta ".idf_escape($_POST["name"])." (".implode(", ",$q).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$Oa);}page_header(($Oa!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($Oa):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$n);$X=get_vals("SHOW CHARACTER SET");sort($X);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Oa!=""){$a=routine($Oa,$Ta);$a["name"]=$Oa;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$X,$Ta);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$a["returns"],$X);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$P,'">
';if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($Oa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Va,'>';}echo'</form>
';}elseif(isset($_GET["sequence"])){$zb=$_GET["sequence"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($zb),$x,'Sekvencia bola odstránená.');}elseif($zb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$x,'Sekvencia bola vytvorená.');}elseif($zb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($zb)." RENAME TO ".idf_escape($_POST["name"]),$x,'Sekvencia bola zmenená.');}else{redirect($x);}}page_header($zb!=""?'Pozmeniť sekvenciu'.": ".h($zb):'Vytvoriť sekvenciu',$n);$a=array("name"=>$zb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Uložiť">
';if($zb!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$Va>\n";}echo'</form>
';}elseif(isset($_GET["type"])){$Uc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Uc),$x,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ bol vytvorený.');}}page_header($Uc!=""?'Pozmeniť typ'.": ".h($Uc):'Vytvoriť typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$P,'">
';if($Uc!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$Va>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Kc["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($m,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$Ld,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Kc["Type"],$a["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$P,'">
';if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$Va,'>';}echo'</form>
...
as$b){if($b[1]!="USAGE"){$cb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$cb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$Hc=$l[1];}}}if($_POST&&!$n){$Xb=(isset($_GET["host"])?q($zd)."@".q($_GET["host"]):"''");$jb=q($_POST["user"])."@".q($_POST["host"]);$ud=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $Xb",ME."privileges=",'Používateľ bol odstránený.');}else{if($Xb!=$jb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $jb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $ud");}elseif($_POST["pass"]!=$Hc||!$_POST["hashed"]){queries("SET PASSWORD FOR $jb = ".($_POST["hashed"]?$ud:"PASSWORD($ud)"));}if(!$n){$wc=array();foreach($uc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html'>".'Oprávnenia'."</a>";$k=0;foreach($cb
as$Wa=>$ga){echo'<th>'.($Wa!="*.*"?"<input name='objects[$k]' value='".h($Wa)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Ac=>$fc){foreach((array)$ma[$Ac]as$dc=>$_a){echo"<tr".odd()."><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($_a).'">'.h($dc);$k=0;foreach($cb
...
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',$Va,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$rd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$rd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$rd),$rd||!$_POST["kill"]);}page_header('Zoznam procesov',$n);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$Na,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Export');echo$r->dumpOutput(1,$md["output"])." ".$r->dumpFormat(1,$md["format"]);echo" <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}print_fieldset("import",'Import CSV',!$D);echo"<input type='hidden' name='token' value='$P'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;"),$md["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>\n","</div></fieldset>\n";$r->selectEmailPrint(array_filter($Tc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Pb=isset($_GET["status"]);page_header($Pb?'Stav':'Premenné');$be=($Pb?show_status():show_variables());if(!$be){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($be
