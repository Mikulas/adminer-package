...
upload_error($k){$yd=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Súbor sa nepodarilo nahrať.'.($yd?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$yd):""):'Súbor neexistuje.');}function
...
enum_input($S,$za,$l,$X,$Ob=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$td);$G=($Ob!==null?"<label><input type='$S'$za value='$Ob'".((is_array($X)?in_array($Ob,$X):$X===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($td[1]as$p=>$W){$W=stripcslashes(str_replace("''","'",$W));$Pa=(is_int($X)?$X==$p+1:(is_array($X)?in_array($p+1,$X):$X===$W));$G.=" <label><input type='$S'$za value='".($p+1)."'".($Pa?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$u;$_=h(bracket_escape($l["field"]));echo"<td class='function'>";$Ze=($u=="mssql"&&$l["auto_increment"]);if($Ze&&!$_POST["save"])$o=null;$yc=(isset($_GET["select"])||$Ze?array("orig"=>'originál'):array())+$b->editFunctions($l);$za=" name='fields[$_]'";if($l["type"]=="enum")echo
...
q($W);}function
check_sqlite_name($_){global$g;$hc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($hc)\$~",$_)){$g->error=sprintf('Prosím vyberte jednu z koncoviek %s.',str_replace("|",", ",$hc));return
false;}return
true;}function
...
false;$g->Min_SQLite(":memory:");$g->error='Súbor existuje.';return@rename(DB,$_);}function
...
set_schema($lf){global$g,$T,$Bf;$G=$g->query("SET search_path TO ".idf_escape($lf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$Bf['Užívateľské typy'][]=$S;}}return$G;}function
...
unconvert_field($l,$G){return$G;}function
support($jc){return
ereg("view|scheme|processlist|drop_col|variables|status",$jc);}$u="oracle";$T=array();$Bf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Dátum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Reťazce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binárne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$v=>$W){$T+=$W;$Bf[$v]=array_keys($W);}$sg=array();$Zd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$yc=array("length","lower","round","upper");$Cc=array("avg","count","count distinct","max","min","sum");$Ib=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Cb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Ce=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($If,$L=""){echo'<p class="tabs">';$od=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($If))$od["view"]='Zmeniť pohľad';else$od["create"]='Zmeniť tabuľku';if($L!==null)$od["edit"]='Nová položka';foreach($od
...
selectQuery($E){global$u;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$u'>".h(str_replace("\n"," ",$E))."</code> <a href='".h(ME)."sql=".urlencode($E)."'>".'Upraviť'."</a></p>\n";}function
rowDescription($O){return"";}function
rowDescriptions($I,$qc){return$I;}function
selectVal($W,$y,$l){$G=($W===null?"<i>NULL</i>":(ereg("char|binary",$l["type"])&&!ereg("var",$l["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$l["type"])&&!is_utf8($W))$G=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W));return($y?"<a href='$y'>$G</a>":$G);}function
editVal($W,$l){return$W;}function
selectColumnsPrint($J,$f){global$yc,$Cc;print_fieldset("select",'Vypísať',$J);$p=0;$xc=array('Funkcie'=>$yc,'Agregácia'=>$Cc);foreach($J
...
as$p=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input name='fulltext[$p]' value='".h($_GET["fulltext"][$p])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$p]",1,isset($_GET["boolean"][$p]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Na="this.nextSibling.onchange();";for($p=0;$p<=count($_GET["where"]);$p++){list(,$W)=each($_GET["where"]);if(!$W||("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators))){echo"<div><select name='where[$p][col]' onchange='$Na'><option value=''>(".'kdekoľvek'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$p][op]",$this->operators,$W["op"],$Na),"<input name='where[$p][val]' value='".h($W["val"])."' onchange='".($W?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($de,$f,$t){print_fieldset("sort",'Zotriediť',$de);$p=0;foreach((array)$_GET["order"]as$v=>$W){if(isset($f[$W])){echo"<div><select name='order[$p]' onchange='selectFieldChange(this.form);'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$p]",1,isset($_GET["desc"][$v]),'zostupne')."</div>\n";$p++;}}echo"<div><select name='order[$p]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$p]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($Tf){if($Tf!==null){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($Tf).'">',"</div></fieldset>\n";}}function
...
messageQuery($E){global$u;static$ib=0;restart_session();$q="sql-".($ib++);$Ec=&get_session("queries");if(strlen($E)>1e6)$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";$Ec[$_GET["db"]][]=array($E,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$q' onclick=\"return !toggle('$q');\">".'SQL príkaz'."</a><div id='$q' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ec[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$G);}function
editInput($O,$l,$za,$X){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$za value='-1' checked><i>".'originál'."</i></label> ":"").($l["null"]?"<label><input type='radio'$za value=''".($X!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$za,$l,$X,0);return"";}function
...
dumpOutput(){$G=array('text'=>'otvoriť','file'=>'uložiť');if(function_exists('gzencode'))$G['gz']='gzip';if(function_exists('bzcompress'))$G['bz2']='bzip2';return$G;}function
dumpFormat(){return
...
<p class="logout">
';if(DB==""||!$Gd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL príkaz'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}echo'<input type="submit" name="logout" value="Odhlásiť" id="logout">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
';$this->databasesPrint($Gd);if($_GET["ns"]!==""&&!$Gd&&DB!=""){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$Q=tables_list();if(!$Q)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{$this->tablesPrint($Q);$od=array();foreach($Q
...
<p id="dbs">
';hidden_fields_get();echo($i?html_select("db",array(""=>"(".'databáza'.")")+$i,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrať"',($i?" class='hidden'":""),'>
...
as$O=>$S){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O).">".'vypísať'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold($_GET["table"]==$O)." title='".'Zobraziť štruktúru'."'>".$this->tableName(array("Name"=>$O))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$v=>$W){if($W===null)$W="<i>NULL</i>";elseif($Ia[$v]&&!is_utf8($W))$W="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$v]==254)$W="<code>$W</code>";}if(isset($od[$v])&&!$f[$od[$v]]){if($Hc){$O=$H[array_search("table=",$od)];$y=$od[$v].urlencode($ge[$O]!=""?$ge[$O]:$O);}else{$y="edit=".urlencode($od[$v]);foreach($t[$od[$v]]as$Ua=>$Xc)$y.="&where".urlencode("[".bracket_escape($Ua)."]")."=".urlencode($H[$Xc]);}$W="<a href='".h(ME.$y)."'>$W</a>";}echo"<td>$W";}}echo($p?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";return$G;}function
...
edit_type($v,$l,$Wa,$rc=array()){global$Bf,$T,$sg,$Vd;echo'<td><select name="',$v,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$Bf+($rc?array('Cudzie kľúče'=>$rc):array()),$l["type"]),'</select>
<td><input name="',$v,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$v"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($Wa,$l["collation"]).'</select>',($sg?"<select name='$v"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($sg,$l["unsigned"]).'</select>':''),($rc?"<select name='$v"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Vd),$l["on_delete"])."</select> ":" ");}function
...
<td',($_POST["defaults"]?"":" class='hidden'"),'>Východzie hodnoty
',(support("comment")?"<td".($bb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($bb?"":" class='hidden'")."><input name='fields[$p][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $wa, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($he==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$cb=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$v=>$W){if(extension_loaded($W))$cb[]=".$v";}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($cb?"[".implode("|",$cb)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Ec){print_fieldset("history",'História',$_GET["history"]!="");foreach($Ec
as$v=>$W){list($D,$Uf)=$W;echo'<a href="'.h(ME."sql=&history=$v").'">'.'Upraviť'."</a> <span class='time'>".@date("H:i:s",$Uf)."</span> <code class='jush-$u'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$D)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$_=>$l){if(!isset($l["privileges"][$tg?"update":"insert"])||$b->fieldName($l)=="")unset($m[$_]);}if($_POST&&!$k&&!isset($_GET["select"])){$pd=$_POST["referer"];if($_POST["insert"])$pd=($tg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$pd))$pd=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$pd,'Položka bola vymazaná.');else{$L=array();foreach($m
as$_=>$l){$W=process_input($l);if($W!==false&&$W!==null)$L[idf_escape($_)]=($tg?"\n".idf_escape($_)." = $W":$W);}if($tg){if(!$L)redirect($pd);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$pd,'Položka bola aktualizovaná.');}else{$F=insert_into($a,$L);$hd=($F?last_id():0);queries_redirect($pd,sprintf('Položka%s bola vložená.',($hd?" $hd":"")),$F);}}}$Jf=$b->tableName(table_status($a));page_header(($tg?'Upraviť':'Vložiť'),$k,array("select"=>array($a,$Jf)),$Jf);$H=null;if($_POST["save"])$H=(array)$_POST["fields"];elseif($Z){$J=array();foreach($m
as$_=>$l){if(isset($l["privileges"]["select"])){$xa=convert_field($l);if($_POST["clone"]&&$l["auto_increment"])$xa="''";if($u=="sql"&&ereg("enum|set",$l["type"]))$xa="1*".idf_escape($_);$J[]=($xa?"$xa AS ":"").idf_escape($_);}}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false)echo"<p class='error'>".'Žiadne riadky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($m){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($tg?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($tg?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<label class="jsonly"><input type="checkbox" name="defaults" value="1"',($_POST["defaults"]?" checked":""),' onclick="columnShow(this.checked, 5);">Východzie hodnoty</label>
',(support("comment")?checkbox("comments",1,$bb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($H["Comment"]).'" maxlength="60"'.($bb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
';if(support("partitioning")){$te=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$se,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($te||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();$_=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$_){if(DB!=""){$_GET["db"]=$_;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_),'Databáza bola premenovaná.',rename_database($_,$_POST["collation"]));}else{$i=explode("\n",str_replace("\r","",$_));$Cf=true;$gd="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"]))$Cf=false;$gd=$j;}}queries_redirect(ME."db=".urlencode($gd),'Databáza bola vytvorená.',$Cf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($_).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$k,array(),DB);$Wa=collations();$_=DB;$Va=null;if($_POST){$_=$_POST["name"];$Va=$_POST["collation"];}elseif(DB!="")$Va=db_collation(DB,$Wa);elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$zc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$zc,$z)&&$z[1]){$_=stripcslashes(idf_unescape("`$z[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($_,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($_).'</textarea><br>':'<input id="name" name="name" value="'.h($_).'" maxlength="64">')."\n".($Wa?html_select("collation",array(""=>"(".'porovnávanie'.")")+$Wa,$Va):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložiť">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Pridať ďalší'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$k){$y=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$y,'Schéma bola odstránená.');else{$_=trim($_POST["name"]);$y.=urlencode($_);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($_),$y,'Schéma bola vytvorená.');elseif($_GET["ns"]!=$_)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($_),$y,'Schéma bola zmenená.');else
redirect($y);}}page_header($_GET["ns"]!=""?'Pozmeniť schému':'Vytvoriť schému',$k);$H=$_POST;if(!$H)$H=array("name"=>$_GET["ns"]);echo'
<form action="" method="post">
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$Eb=false;if($_POST&&!$k){$_=trim($_POST["name"]);$Eb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($_)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_)),'Pohľad bol odstránený.','Pohľad bol zmenený.','Pohľad bol vytvorený.',$a);}page_header(($a!=""?'Zmeniť pohľad':'Vytvoriť pohľad'),$k,array("table"=>$a),$a);$H=$_POST;if(!$H&&$a!=""){$H=view($a);$H["name"]=$a;}echo'
<form action="" method="post">
<p>Názov: <input name="name" value="',h($H["name"]),'" maxlength="64">
<p>';textarea("select",$H["select"]);echo'<p>
';if($Eb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Sc=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$_f=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$k){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Udalosť bola odstránená.');elseif(in_array($_POST["INTERVAL_FIELD"],$Sc)&&isset($_f[$_POST["STATUS"]])){$kf="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$kf.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$kf)."\n".$_f[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Upraviť udalosť'.": ".h($aa):'Vytvoriť udalosť'),$k);$H=$_POST;if(!$H&&$aa!=""){$I=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$H=reset($I);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>Názov<td><input name="EVENT_NAME" value="',h($H["EVENT_NAME"]),'" maxlength="64">
<tr><th>Začiatok<td><input name="STARTS" value="',h("$H[EXECUTE_AT]$H[STARTS]"),'">
<tr><th>Koniec<td><input name="ENDS" value="',h($H["ENDS"]),'">
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Sc,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$_f,$H["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Eb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$y=substr(ME,0,-1);$_=trim($_POST["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$y,'Sekvencia bola odstránená.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($_),$y,'Sekvencia bola vytvorená.');elseif($fa!=$_)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($_),$y,'Sekvencia bola zmenená.');else
redirect($y);}page_header($fa!=""?'Pozmeniť sekvenciu'.": ".h($fa):'Vytvoriť sekvenciu',$k);$H=$_POST;if(!$H)$H=array("name"=>$fa);echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Uložiť">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$y=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$y,'Typ bol odstránený.');else
query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$y,'Typ bol vytvorený.');}page_header($ga!=""?'Pozmeniť typ'.": ".h($ga):'Vytvoriť typ',$k);$H=$_POST;if(!$H)$H=array("as"=>"AS ");echo'
<form action="" method="post">
<p>
';if($ga!="")echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";else{echo"<input name='name' value='".h($H['name'])."'>\n";textarea("as",$H["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$ig=trigger_options();$gg=array("INSERT","UPDATE","DELETE");$Eb=false;if($_POST&&!$k&&in_array($_POST["Timing"],$ig["Timing"])&&in_array($_POST["Event"],$gg)&&in_array($_POST["Type"],$ig["Type"])){$Vf=" $_POST[Timing] $_POST[Event]";$Ud=" ON ".table($a);$Eb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$Ud:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$Ud.$Vf:$Vf.$Ud).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$k,array("table"=>$a));$H=$_POST;if(!$H)$H=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$ig["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$gg,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$ig["Type"],$H["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Eb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$Pd=>$cf){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Pd,$z))grant("REVOKE",array_keys($cf),$z[2]," ON $z[1] FROM $Md");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$k);if($lb)$g->query("DROP USER $Md");}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$ha@$_GET[host]"):'Vytvoriť používateľa'),$k,array("privileges"=>array('','Oprávnenia')));if($_POST){$H=$_POST;$_c=$Ld;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$Sd;if($Sd!="")$H["hashed"]=true;$_c[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>Server<td><input name="host" maxlength="60" value="',h($H["host"]),'">
<tr><th>Používateľ<td><input name="user" maxlength="16" value="',h($H["user"]),'">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$p=0;foreach($_c
as$Pd=>$zc){echo'<th>'.($Pd!="*.*"?"<input name='objects[$p]' value='".h($Pd)."' size='10'>":"<input type='hidden' name='objects[$p]' value='*.*' size='10'>*.*");$p++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$gb=>$wb){foreach((array)$Ke[$gb]as$Je=>$ab){echo"<tr".odd()."><td".($wb?">$wb<td":" colspan='2'").' lang="en" title="'.h($ab).'">'.h($Je);$p=0;foreach($_c
as$Pd=>$zc){$_="'grants[$p][".h(strtoupper($Je))."]'";$X=$zc[strtoupper($Je)];if($gb=="Server Admin"&&$Pd!=(isset($_c["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$_><option><option value='1'".($X?" selected":"").">".'Povoliť'."<option value='0'".($X=="0"?" selected":"").">".'Zakázať'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$dd=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$dd++;}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$dd),$dd||!$_POST["kill"]);}page_header('Zoznam procesov',$k);echo'
<form action="" method="post">
...
as$v=>$W)echo"<td>".(($u=="sql"&&$v=="Info"&&ereg("Query|Killed",$H["Command"])&&$W!="")||($u=="pgsql"&&$v=="current_query"&&$W!="<IDLE>")||($u=="oracle"&&$v=="sql_text"&&$W!="")?"<code class='jush-$u'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($H["db"]!=""?"db=".urlencode($H["db"])."&":"")."sql=".urlencode($W)).'">'.'Upraviť'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
...
as$_=>$W){$W=process_input($m[$_]);if($W!==null){if($_POST["clone"])$L[idf_escape($_)]=($W!==false?$W:idf_escape($_));elseif($W!==false)$L[]=idf_escape($_)." = $W";}}$E.=($_POST["clone"]?" (".implode(", ",array_keys($L)).")\nSELECT ".implode(", ",$L)."\nFROM ".table($a):" SET\n".implode(",\n",$L));}if($_POST["delete"]||$L){$Ya="UPDATE";if($_POST["delete"]){$Ya="DELETE";$E="FROM $E";}if($_POST["clone"]){$Ya="INSERT";$E="INTO $E";}if($_POST["all"]||($rg===array()&&$_POST["check"])||$Tc){$F=queries("$Ya $E".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $Fg"));$ra=$g->affected_rows;}else{foreach((array)$_POST["check"]as$W){$F=queries($Ya.limit1($E,"\nWHERE ".where_check($W)));if(!$F)break;$ra+=$g->affected_rows;}}}$_d=sprintf('%d položiek bolo ovplyvnených.',$ra);if($_POST["clone"]&&$F&&$ra==1){$hd=last_id();if($hd)$_d=sprintf('Položka%s bola vložená.'," $hd");}queries_redirect(remove_from_uri("page"),$_d,$F);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Dvojkliknite na políčko, ktoré chcete zmeniť.';else{$F=true;$ra=0;foreach($_POST["val"]as$pg=>$H){$L=array();foreach($H
as$v=>$W){$v=bracket_escape($v,1);$L[]=idf_escape($v)." = ".(ereg('char|text',$m[$v]["type"])||$W!=""?$b->processInput($m[$v],$W):"NULL");}$E=table($a)." SET ".implode(", ",$L);$Eg=" WHERE ".where_check($pg).($Z?" AND ".implode(" AND ",$Z):"");$F=queries("UPDATE".($Tc?" $E$Eg":limit1($E,$Eg)));if(!$F)break;$ra+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$ra),$F);}}elseif(is_string($kc=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($qa["output"])."&format=".urlencode($_POST["separator"]));$F=true;$Xa=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$kc,$td);$ra=count($td[0]);begin();$qf=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($td[0]as$v=>$W){preg_match_all("~((\"[^\"]*\")+|[^$qf]*)$qf~",$W.$qf,$ud);if(!$v&&!array_diff($ud[1],$Xa)){$Xa=$ud[1];$ra--;}else{$L=array();foreach($ud[1]as$p=>$Ua)$L[idf_escape($Xa[$p])]=($Ua==""&&$m[$Xa[$p]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Ua))));$F=insert_update($a,$L,$Fe);if(!$F)break;}}if($F)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$ra),$F);queries("ROLLBACK");}else$k=upload_error($kc);}}$Jf=$b->tableName($P);if(is_ajax())ob_start();page_header('Vypísať'.": $Jf",$k);$L=null;if(isset($df["insert"])){$L="";foreach((array)$_GET["where"]as$W){if(count($rc[$W["col"]])==1&&($W["op"]=="="||(!$W["op"]&&!ereg('[_%]',$W["val"]))))$L.="&set".urlencode("[".bracket_escape($W["col"])."]")."=".urlencode($W["val"]);}}$b->selectLinks($P,$L);if(!$f)echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($J,$f);$b->selectSearchPrint($Z,$f,$t);$b->selectOrderPrint($de,$f,$t);$b->selectLimitPrint($x);$b->selectLengthPrint($Tf);$b->selectActionPrint($t);echo"</form>\n";$B=$_GET["page"];if($B=="last"){$uc=$g->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$B=floor(max(0,$uc-1)/$x);}$E=$b->selectQueryBuild($J,$Z,$Ac,$de,$x,$B);if(!$E)$E="SELECT".limit((+$x&&$Ac&&$Tc&&$u=="sql"?"SQL_CALC_FOUND_ROWS ":"").$wc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$Bc,($x!=""?+$x:null),($B?$x*$B:0),"\n");echo$b->selectQuery($E);$F=$g->query($E);if(!$F)echo"<p class='error'>".error()."\n";else{if($u=="mssql")$F->seek($x*$B);$Nb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$I=array();while($H=$F->fetch_assoc()){if($B&&$u=="oracle")unset($H["RNUM"]);$I[]=$H;}if($_GET["page"]!="last")$uc=(+$x&&$Ac&&$Tc?($u=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($E) x")):count($I));if(!$I)echo"<p class='message'>".'Žiadne riadky.'."\n";else{$Ea=$b->backwardKeys($a,$Jf);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Ac&&$J?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$Kd=array();$yc=array();reset($J);$Pe=1;foreach($I[0]as$v=>$W){if($v!=$Qd){$W=$_GET["columns"][key($J)];$l=$m[$J?($W?$W["col"]:current($J)):$v];$_=($l?$b->fieldName($l,$Pe):"*");if($_!=""){$Pe++;$Kd[$v]=$_;$e=idf_escape($v);$Hc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($v);$wb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Hc.($de[0]==$e||$de[0]==$v||(!$de&&$Tc&&$Ac[0]==$e)?$wb:'')).'">';echo(!$J||$W?apply_sql_function($W["fun"],$_):h(current($J)))."</a>";echo"<span class='column hidden'>","<a href='".h($Hc.$wb)."' title='".'zostupne'."' class='text'> ↓</a>";if(!$W["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($v)).'\'); return false;" title="'.'Vyhľadať'.'" class="text jsonly"> =</a>';echo"</span>";}$yc[$v]=$W["fun"];next($J);}}$md=array();if($_GET["modify"]){foreach($I
as$H){foreach($H
as$v=>$W)$md[$v]=max($md[$v],min(40,strlen(utf8_decode($W))));}}echo($Ea?"<th>".'Vzťahy':"")."</thead>\n";if(is_ajax()){if($x%2==1&&$B%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($I,$rc)as$Jd=>$H){$og=unique_array($I[$Jd],$t);$pg="";foreach($og
as$v=>$W)$pg.="&".($W!==null?urlencode("where[".bracket_escape($v)."]")."=".urlencode($W):"null%5B%5D=".urlencode($v));echo"<tr".odd().">".(!$Ac&&$J?"":"<td>".checkbox("check[]",substr($pg,1),in_array(substr($pg,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Tc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$pg)."'>".'upraviť'."</a>"));foreach($H
...
as$v=>$W)json_row("sum-$v",number_format($W,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$j=>$W)json_row("tables-".js_escape($j),$W);json_row("");}exit;}else{$Pf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Pf&&!$k&&!$_POST["search"]){$F=true;$_d="";if($u=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$F=truncate_tables($_POST["tables"]);$_d='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$_d='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$_d='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"])$F=drop_views($_POST["views"]);if($F&&$_POST["tables"])$F=drop_tables($_POST["tables"]);$_d='Tabuľka bola odstránená.';}elseif($u!="sql"){$F=($u=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$_d='Tables have been optimized.';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc())$_d.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$_d,$F);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$Of=tables_list();if(!$Of)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabuľka','<td>'.'Typ','<td>'.'Porovnávanie','<td>'.'Veľkosť dát','<td>'.'Veľkosť indexu','<td>'.'Voľné miesto','<td>'.'Auto Increment','<td>'.'Riadky',(support("comment")?'<td>'.'Komentár':''),"</thead>\n";foreach($Of
as$_=>$S){$Bg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Bg?"views[]":"tables[]"),$_,in_array($_,$Pf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($_).'" title="'.'Zobraziť štruktúru'.'">'.h($_).'</a>';if($Bg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($_).'" title="'.'Zmeniť pohľad'.'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($_).'" title="'.'Vypísať dáta'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmeniť tabuľku'),"Index_length"=>array("indexes",'Zmeniť indexy'),"Data_free"=>array("edit",'Nová položka'),"Auto_increment"=>array("auto_increment=1&create",'Zmeniť tabuľku'),"Rows"=>array("select",'Vypísať dáta'),)as$v=>$y)echo($y?"<td align='right'><a href='".h(ME."$y[0]=").urlencode($_)."' id='$v-".h($_)."' title='$y[1]'>?</a>":"<td id='$v-".h($_)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($_)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($Of)),"<td>".nbsp($u=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$v)echo"<td align='right' id='sum-$v'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$u)?($u!="sqlite"?"<input type='submit' value='".'Analyzovať'."'> ":"")."<input type='submit' name='optimize' value='".'Optimalizovať'."'> ":"").($u=="sql"?"<input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)").">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$u!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'">')," <input type='submit' name='move' value='".'Presunúť'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$hf=routines();if($hf){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($hf
as$H){echo'<tr'.odd().'>','<th><a href="'.h(ME).($H["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($H["ROUTINE_NAME"]).'">'.h($H["ROUTINE_NAME"]).'</a>','<td>'.h($H["ROUTINE_TYPE"]),'<td>'.h($H["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($H["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($H["ROUTINE_NAME"]).'">'.'Zmeniť'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Vytvoriť procedúru'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Vytvoriť funkciu'."</a>\n";}if(support("sequence")){echo"<h3>".'Sekvencia'."</h3>\n";$rf=get_vals("SELECT sequence_name FROM information_schema.sequences WHERE sequence_schema = current_schema()");if($rf){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."</thead>\n";odd('');foreach($rf
as$W)echo"<tr".odd()."><th><a href='".h(ME)."sequence=".urlencode($W)."'>".h($W)."</a>\n";echo"</table>\n";}echo"<p><a href='".h(ME)."sequence='>".'Vytvoriť sekvenciu'."</a>\n";}if(support("type")){echo"<h3>".'Užívateľské typy'."</h3>\n";$T=types();if($T){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Názov'."</thead>\n";odd('');foreach($T
