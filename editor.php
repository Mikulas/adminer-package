...
* @version 3.6.3
*/error_reporting(6135);$Fb=!ereg('^(unsafe_raw)?$',ini_get("filter.default"));if($Fb||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$te=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($te)$$X=$te;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");if(isset($_GET["file"])){if($_SERVER["HTTP_IF_MODIFIED_SINCE"]){header("HTTP/1.1 304 Not Modified");exit;}header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
print_fieldset($u,$uc,$De=false,$Rc=""){echo"<fieldset><legend><a href='#fieldset-$u' onclick=\"".h($Rc)."return !toggle('fieldset-$u');\">$uc</a></legend><div id='fieldset-$u'".($De?"":" class='hidden'").">\n";}function
...
ini_bool($hc){$X=ini_get($hc);return(eregi('^(on|true|yes)$',$X)||(int)$X);}function
...
where($Z,$q=array()){global$y;$J=array();foreach((array)$Z["where"]as$z=>$X)$J[]=idf_escape(bracket_escape($z,1)).(($y=="sql"&&ereg('\\.',$X))||$y=="mssql"?" LIKE ".exact_value(addcslashes($X,"%_\\")):" = ".unconvert_field($q[$z],exact_value($X)));foreach((array)$Z["null"]as$z)$J[]=idf_escape($z)." IS NULL";return
implode(" AND ",$J);}function
where_check($X,$q=array()){parse_str($X,$Ea);remove_slashes(array(&$Ea));return
where($Ea,$q);}function
where_link($t,$g,$Y,$Tc="="){return"&where%5B$t%5D%5Bcol%5D=".urlencode($g)."&where%5B$t%5D%5Bop%5D=".urlencode(($Y!==null?$Tc:"IS NULL"))."&where%5B$t%5D%5Bval%5D=".urlencode($Y);}function
cookie($D,$Y){global$aa;$ed=array($D,(ereg("\n",$Y)?"":$Y),time()+2592000,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$aa);if(version_compare(PHP_VERSION,'5.2.0')>=0)$ed[]=true;return
...
redirect($xc,$Fc=null){if($Fc!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($xc!==null?$xc:$_SERVER["REQUEST_URI"]))][]=$Fc;}if($xc!==null){if($xc=="")$xc=".";header("Location: $xc");exit;}}function
query_redirect($H,$xc,$Fc,$td=true,$xb=true,$Ab=false){global$j,$o,$c;if($xb)$Ab=!$j->query($H);$Ld="";if($H)$Ld=$c->messageQuery("$H;");if($Ab){$o=error().$Ld;return
false;}if($td)redirect($xc,$Fc.$Ld);return
true;}function
queries($H=null){global$j;static$rd=array();if($H===null)return
implode(";\n",$rd);$rd[]=(ereg(';$',$H)?"DELIMITER ;;\n$H;\nDELIMITER ":$H);return$j->query($H);}function
apply_queries($H,$S,$tb='table'){foreach($S
as$Q){if(!queries("$H ".$tb($Q)))return
false;}return
true;}function
queries_redirect($xc,$Fc,$td){return
query_redirect(queries(),$xc,$Fc,$td,false,!$td);}function
...
get_file($z,$Va=false){$Cb=$_FILES[$z];if(!$Cb||$Cb["error"])return$Cb["error"];$J=file_get_contents($Va&&ereg('\\.gz$',$Cb["name"])?"compress.zlib://$Cb[tmp_name]":($Va&&ereg('\\.bz2$',$Cb["name"])?"compress.bzip2://$Cb[tmp_name]":$Cb["tmp_name"]));if($Va){$Md=substr($J,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$Md,$ud))$J=iconv("utf-16","utf-8",$J);elseif($Md=="\xEF\xBB\xBF")$J=substr($J,3);}return$J;}function
upload_error($o){$Dc=($o==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($o?lang(1).($Dc?" ".lang(2,$Dc):""):lang(3));}function
repeat_pattern($gd,$vc){return
str_repeat("$gd{0,65535}",$vc/65535)."$gd{0,".($vc%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($P,$vc=80,$Sd=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{FFFF}]",$vc).")($)?)u",$P,$B))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$vc).")($)?)",$P,$B);return
h($B[1]).$Sd.(isset($B[2])?"":"<i>...</i>");}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($od,$dc=array()){while(list($z,$X)=each($od)){if(is_array($X)){foreach($X
as$mc=>$W)$od[$z."[$mc]"]=$W;}elseif(!in_array($z,$dc))echo'<input type="hidden" name="'.h($z).'" value="'.h($X).'">';}}function
...
enum_input($U,$sa,$p,$Y,$ob=null){global$c;preg_match_all("~'((?:[^']|'')*)'~",$p["length"],$Ac);$J=($ob!==null?"<label><input type='$U'$sa value='$ob'".((is_array($Y)?in_array($ob,$Y):$Y===0)?" checked":"")."><i>".lang(4)."</i></label>":"");foreach($Ac[1]as$t=>$X){$X=stripcslashes(str_replace("''","'",$X));$Fa=(is_int($Y)?$Y==$t+1:(is_array($Y)?in_array($t+1,$Y):$Y===$X));$J.=" <label><input type='$U'$sa value='".($t+1)."'".($Fa?' checked':'').'>'.h($c->editVal($X,$p)).'</label>';}return$J;}function
input($p,$Y,$s){global$oe,$c,$y;$D=h(bracket_escape($p["field"]));echo"<td class='function'>";$xd=($y=="mssql"&&$p["auto_increment"]);if($xd&&!$_POST["save"])$s=null;$Rb=(isset($_GET["select"])||$xd?array("orig"=>lang(5)):array())+$c->editFunctions($p);$sa=" name='fields[$D]'";if($p["type"]=="enum")echo
nbsp($Rb[""])."<td>".$c->editInput($_GET["edit"],$p,$sa,$Y);else{$Gb=0;foreach($Rb
as$z=>$X){if($z===""||!$X)break;$Gb++;}$Qc=($Gb?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($p["field"])))."]']; if ($Gb > f.selectedIndex) f.selectedIndex = $Gb;\"":"");$sa.=$Qc;echo(count($Rb)>1?html_select("function[$D]",$Rb,$s===null||in_array($s,$Rb)||isset($Rb[$s])?$s:"","functionChange(this);"):nbsp(reset($Rb))).'<td>';$jc=$c->editInput($_GET["edit"],$p,$sa,$Y);if($jc!="")echo$jc;elseif($p["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$p["length"],$Ac);foreach($Ac[1]as$t=>$X){$X=stripcslashes(str_replace("''","'",$X));$Fa=(is_int($Y)?($Y>>$t)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$D][$t]' value='".(1<<$t)."'".($Fa?' checked':'')."$Qc>".h($c->editVal($X,$p)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$p["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$D'$Qc>";elseif(($Yd=ereg('text|lob',$p["type"]))||ereg("\n",$Y)){if($Yd&&$y!="sqlite")$sa.=" cols='50' rows='12'";else{$L=min(12,substr_count($Y,"\n")+1);$sa.=" cols='30' rows='$L'".($L==1?" style='height: 1.2em;'":"");}echo"<textarea$sa>".h($Y).'</textarea>';}else{$Ec=(!ereg('int',$p["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$p["length"],$B)?((ereg("binary",$p["type"])?2:1)*$B[1]+($B[3]?1:0)+($B[2]&&!$p["unsigned"]?1:0)):($oe[$p["type"]]?$oe[$p["type"]]+($p["unsigned"]?0:1):0));echo"<input".(ereg('int',$p["type"])?" type='number'":"")." value='".h($Y)."'".($Ec?" maxlength='$Ec'":"").(ereg('char|binary',$p["type"])&&$Ec>20?" size='40'":"")."$sa>";}}}function
...
dump_headers($bc,$Jc=false){global$c;$J=$c->dumpHeaders($bc,$Jc);$bd=$_POST["output"];if($bd!="text")header("Content-Disposition: attachment; filename=".$c->dumpFilename($bc).".$J".($bd!="file"&&!ereg('[^0-9a-z]',$bd)?".$bd":""));session_write_close();return$J;}function
...
is_mail($lb){$qa='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$cb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$gd="$qa+(\\.$qa+)*@($cb?\\.)+$cb";return
preg_match("(^$gd(,\\s*$gd)*\$)i",$lb);}function
...
slow_query($H){global$c,$ge;$n=$c->database();if(support("kill")&&is_object($k=connect())&&($n==""||$k->select_db($n))){$pc=$k->result("SELECT CONNECTION_ID()");echo'<script type="text/javascript">
...
query($H,$pe=false){$I=parent::query($H);$this->error="";if(!$I){list(,$this->errno,$this->error)=$this->errorInfo();return
...
SQLite3($Db);$Ae=$this->_link->version();$this->server_info=$Ae["versionString"];}function
...
query($H,$pe=false){$Hc=($pe?"unbufferedQuery":"query");$I=@$this->_link->$Hc($H,SQLITE_BOTH,$o);$this->error="";if(!$I){$this->error=$o;return
...
fetch_field(){$D=$this->_result->fieldName($this->_offset++);$gd='(\\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($gd\\.)?$gd\$~",$D,$B)){$Q=($B[3]!=""?$B[3]:idf_unescape($B[2]));$D=($B[5]!=""?$B[5]:idf_unescape($B[4]));}return(object)array("name"=>$D,"orgname"=>$D,"orgtable"=>$Q,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
...
array();}function
limit($H,$Z,$_,$Nc=0,$Hd=" "){return" $H$Z".($_!==null?$Hd."LIMIT $_".($Nc?" OFFSET $Nc":""):"");}function
...
indexes($Q,$k=null){$J=array();$ld=array();foreach(fields($Q)as$p){if($p["primary"])$ld[]=$p["field"];}if($ld)$J[""]=array("type"=>"PRIMARY","columns"=>$ld,"lengths"=>array());foreach(get_rows("PRAGMA index_list(".table($Q).")")as$K){if(!ereg("^sqlite_",$K["name"])){$J[$K["name"]]["type"]=($K["unique"]?"UNIQUE":"INDEX");$J[$K["name"]]["lengths"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($K["name"]).")")as$Ad)$J[$K["name"]]["columns"][]=$Ad["name"];}}return$J;}function
...
alter_table($Q,$D,$q,$Ib,$Na,$pb,$f,$ta,$fd){$ye=($Q==""||$Ib);foreach($q
as$p){if($p[0]!=""||!$p[1]||$p[2]){$ye=true;break;}}$d=array();$ad=array();$md=false;foreach($q
as$p){if($p[1]){if($p[1][6])$md=true;$d[]=($ye?"  ":"ADD ").implode($p[1]);if($p[0]!="")$ad[$p[0]]=$p[1][0];}}if($ye){if($Q!=""){queries("BEGIN");foreach(foreign_keys($Q)as$r){$h=array();foreach($r["source"]as$g){if(!$ad[$g])continue
2;$h[]=$ad[$g];}$Ib[]="  FOREIGN KEY (".implode(", ",$h).") REFERENCES ".table($r["table"])." (".implode(", ",array_map('idf_escape',$r["target"])).") ON DELETE $r[on_delete] ON UPDATE $r[on_update]";}$x=array();foreach(indexes($Q)as$nc=>$w){$h=array();foreach($w["columns"]as$g){if(!$ad[$g])continue
2;$h[]=$ad[$g];}$h="(".implode(", ",$h).")";if($w["type"]!="PRIMARY")$x[]=array($w["type"],$nc,$h);elseif(!$md)$Ib[]="  PRIMARY KEY $h";}}$d=array_merge($d,$Ib);if(!queries("CREATE TABLE ".table($Q!=""?"adminer_$D":$D)." (\n".implode(",\n",$d)."\n)"))return
false;if($Q!=""){if($ad&&!queries("INSERT INTO ".table("adminer_$D")." (".implode(", ",$ad).") SELECT ".implode(", ",array_map('idf_escape',array_keys($ad)))." FROM ".table($Q)))return
false;$me=array();foreach(triggers($Q)as$le=>$be){$ke=trigger($le);$me[]="CREATE TRIGGER ".idf_escape($le)." ".implode(" ",$be)." ON ".table($D)."\n$ke[Statement]";}if(!queries("DROP TABLE ".table($Q)))return
false;queries("ALTER TABLE ".table("adminer_$D")." RENAME TO ".table($D));if(!alter_indexes($D,$x))return
false;foreach($me
as$ke){if(!queries($ke))return
...
apply_queries("DROP VIEW",$Ce);}function
drop_tables($S){return
apply_queries("DROP TABLE",$S);}function
move_tables($S,$Ce,$T){return
false;}function
...
show_status(){$J=array();foreach(get_vals("PRAGMA compile_options")as$Vc){list($z,$X)=explode("=",$Vc,2);$J[$z]=$X;}return$J;}function
convert_field($p){}function
unconvert_field($p,$J){return$J;}function
support($Bb){return
ereg('^(view|trigger|variables|status|dump|move_col|drop_col)$',$Bb);}$y="sqlite";$oe=array("integer"=>0,"real"=>0,"numeric"=>0,"text"=>0,"blob"=>0);$Pd=array_keys($oe);$ve=array();$Uc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT IN","IS NOT NULL","");$Rb=array("hex","length","lower","round","unixepoch","upper");$Ub=array("avg","count","count distinct","group_concat","max","min","sum");$ib=array(array(),array("integer|real|numeric"=>"+/-","text"=>"||",));}$eb["pgsql"]="PostgreSQL";if(isset($_GET["pgsql"])){$kd=array("PgSQL","PDO_PgSQL");define("DRIVER","pgsql");if(extension_loaded("pgsql")){class
...
query($H,$pe=false){$I=@pg_query($this->_link,$H);$this->error="";if(!$I){$this->error=pg_last_error($this->_link);return
...
indexes($Q,$k=null){global$j;if(!is_object($k))$k=$j;$J=array();$Xd=$k->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($Q));$h=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $Xd AND attnum > 0",$k);foreach(get_rows("SELECT relname, indisunique, indisprimary, indkey FROM pg_index i, pg_class ci WHERE i.indrelid = $Xd AND ci.oid = i.indexrelid",$k)as$K){$J[$K["relname"]]["type"]=($K["indisprimary"]=="t"?"PRIMARY":($K["indisunique"]=="t"?"UNIQUE":"INDEX"));$J[$K["relname"]]["columns"]=array();foreach(explode(" ",$K["indkey"])as$fc)$J[$K["relname"]]["columns"][]=$h[$fc];$J[$K["relname"]]["lengths"]=array();}return$J;}function
foreign_keys($Q){global$Pc;$J=array();foreach(get_rows("SELECT conname, pg_get_constraintdef(oid) AS definition
FROM pg_constraint
WHERE conrelid = (SELECT pc.oid FROM pg_class AS pc INNER JOIN pg_namespace AS pn ON (pn.oid = pc.relnamespace) WHERE pc.relname = ".q($Q)." AND pn.nspname = current_schema())
AND contype = 'f'::char
ORDER BY conkey, conname")as$K){if(preg_match('~FOREIGN KEY\s*\((.+)\)\s*REFERENCES (.+)\((.+)\)(.*)$~iA',$K['definition'],$B)){$K['source']=array_map('trim',explode(',',$B[1]));$K['table']=$B[2];if(preg_match('~(.+)\.(.+)~',$B[2],$_c)){$K['ns']=$_c[1];$K['table']=$_c[2];}$K['target']=array_map('trim',explode(',',$B[3]));$K['on_delete']=(preg_match("~ON DELETE ($Pc)~",$B[4],$_c)?$_c[1]:'NO ACTION');$K['on_update']=(preg_match("~ON UPDATE ($Pc)~",$B[4],$_c)?$_c[1]:'NO ACTION');$J[$K['conname']]=$K;}}return$J;}function
view($D){global$j;return
...
alter_table($Q,$D,$q,$Ib,$Na,$pb,$f,$ta,$fd){$d=array();$rd=array();foreach($q
as$p){$g=idf_escape($p[0]);$X=$p[1];if(!$X)$d[]="DROP $g";else{$_e=$X[5];unset($X[5]);if(isset($X[6])&&$p[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($p[0]=="")$d[]=($Q!=""?"ADD ":"  ").implode($X);else{if($g!=$X[0])$rd[]="ALTER TABLE ".table($Q)." RENAME $g TO $X[0]";$d[]="ALTER $g TYPE$X[1]";if(!$X[6]){$d[]="ALTER $g ".($X[3]?"SET$X[3]":"DROP DEFAULT");$d[]="ALTER $g ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($p[0]!=""||$_e!="")$rd[]="COMMENT ON COLUMN ".table($Q).".$X[0] IS ".($_e!=""?substr($_e,9):"''");}}$d=array_merge($d,$Ib);if($Q=="")array_unshift($rd,"CREATE TABLE ".table($D)." (\n".implode(",\n",$d)."\n)");elseif($d)array_unshift($rd,"ALTER TABLE ".table($Q)."\n".implode(",\n",$d));if($Q!=""&&$Q!=$D)$rd[]="ALTER TABLE ".table($Q)." RENAME TO ".table($D);if($Q!=""||$Na!="")$rd[]="COMMENT ON TABLE ".table($D)." IS ".q($Na);if($ta!=""){}foreach($rd
...
insert_into($Q,$O){return
queries("INSERT INTO ".table($Q).($O?" (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")":"DEFAULT VALUES"));}function
insert_update($Q,$O,$ld){global$j;$we=array();$Z=array();foreach($O
as$z=>$X){$we[]="$z = $X";if(isset($ld[idf_unescape($z)]))$Z[]="$z = $X";}return($Z&&queries("UPDATE ".table($Q)." SET ".implode(", ",$we)." WHERE ".implode(" AND ",$Z))&&$j->affected_rows)||queries("INSERT INTO ".table($Q)." (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).")");}function
last_id(){return
0;}function
explain($j,$H){return$j->query("EXPLAIN $H");}function
found_rows($R,$Z){global$j;if(ereg(" rows=([0-9]+)",$j->result("EXPLAIN SELECT * FROM ".idf_escape($R["Name"]).($Z?" WHERE ".implode(" AND ",$Z):"")),$ud))return$ud[1];return
false;}function
types(){return
...
set_schema($Cd){global$j,$oe,$Pd;$J=$j->query("SET search_path TO ".idf_escape($Cd));foreach(types()as$U){if(!isset($oe[$U])){$oe[$U]=0;$Pd[lang(12)][]=$U;}}return$J;}function
...
select_db($l){return
true;}function
query($H,$pe=false){$I=oci_parse($this->_link,$H);$this->error="";if(!$I){$o=oci_error($this->_link);$this->errno=$o["code"];$this->error=$o["message"];return
...
limit($H,$Z,$_,$Nc=0,$Hd=" "){return($Nc?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $H$Z) t WHERE rownum <= ".($_+$Nc).") WHERE rnum > $Nc":($_!==null?" * FROM (SELECT $H$Z) WHERE rownum <= ".($_+$Nc):" $H$Z"));}function
limit1($H,$Z){return" $H$Z";}function
...
count_tables($m){return
array();}function
table_status($D=""){$J=array();$Ed=q($D);foreach(get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($D!=""?" AND table_name = $Ed":"")."
UNION SELECT view_name, 'view', 0, 0 FROM user_views".($D!=""?" WHERE view_name = $Ed":""))as$K){if($D!="")return$K;$J[$K["Name"]]=$K;}return$J;}function
is_view($R){return$R["Engine"]=="view";}function
fk_support($R){return
true;}function
fields($Q){$J=array();foreach(get_rows("SELECT * FROM all_tab_columns WHERE table_name = ".q($Q)." ORDER BY column_id")as$K){$U=$K["DATA_TYPE"];$vc="$K[DATA_PRECISION],$K[DATA_SCALE]";if($vc==",")$vc=$K["DATA_LENGTH"];$J[$K["COLUMN_NAME"]]=array("field"=>$K["COLUMN_NAME"],"full_type"=>$U.($vc?"($vc)":""),"type"=>strtolower($U),"length"=>$vc,"default"=>$K["DATA_DEFAULT"],"null"=>($K["NULLABLE"]=="Y"),"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),);}return$J;}function
...
found_rows($R,$Z){}function
alter_table($Q,$D,$q,$Ib,$Na,$pb,$f,$ta,$fd){$d=$fb=array();foreach($q
...
unconvert_field($p,$J){return$J;}function
support($Bb){return
ereg("view|scheme|processlist|drop_col|variables|status",$Bb);}$y="oracle";$oe=array();$Pd=array();foreach(array(lang(13)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(14)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(15)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(16)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$z=>$X){$oe+=$X;$Pd[$z]=array_keys($X);}$ve=array();$Uc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$Rb=array("length","lower","round","upper");$Ub=array("avg","count","count distinct","max","min","sum");$ib=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$eb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$kd=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
Min_DB{var$extension="sqlsrv",$_link,$_result,$server_info,$affected_rows,$errno,$error;function
_get_error(){$this->error="";foreach(sqlsrv_errors()as$o){$this->errno=$o["code"];$this->error.="$o[message]\n";}$this->error=rtrim($this->error);}function
connect($N,$V,$G){$this->_link=@sqlsrv_connect($N,array("UID"=>$V,"PWD"=>$G,"CharacterSet"=>"UTF-8"));if($this->_link){$gc=sqlsrv_server_info($this->_link);$this->server_info=$gc['SQLServerVersion'];}else$this->_get_error();return(bool)$this->_link;}function
quote($P){return"'".str_replace("'","''",$P)."'";}function
select_db($l){return$this->query("USE ".idf_escape($l));}function
query($H,$pe=false){$I=sqlsrv_query($this->_link,$H);$this->error="";if(!$I){$this->_get_error();return
...
seek($Nc){for($t=0;$t<$Nc;$t++)sqlsrv_fetch($this->_result);}function
...
query($H,$pe=false){$I=mssql_query($H,$this->_link);$this->error="";if(!$I){$this->error=mssql_get_last_message();return
...
seek($Nc){mssql_data_seek($this->_result,$Nc);}function
...
limit($H,$Z,$_,$Nc=0,$Hd=" "){return($_!==null?" TOP (".($_+$Nc).")":"")." $H$Z";}function
limit1($H,$Z){return
...
WHERE o.schema_id = SCHEMA_ID(".q(get_schema()).") AND o.type IN ('S', 'U', 'V') AND o.name = ".q($Q))as$K){$U=$K["type"];$vc=(ereg("char|binary",$U)?$K["max_length"]:($U=="decimal"?"$K[precision],$K[scale]":""));$J[$K["name"]]=array("field"=>$K["name"],"full_type"=>$U.($vc?"($vc)":""),"type"=>$U,"length"=>$vc,"default"=>$K["default"],"null"=>$K["is_nullable"],"auto_increment"=>$K["is_identity"],"collation"=>$K["collation_name"],"privileges"=>array("insert"=>1,"select"=>1,"update"=>1),"primary"=>$K["is_identity"],);}return$J;}function
...
alter_table($Q,$D,$q,$Ib,$Na,$pb,$f,$ta,$fd){$d=array();foreach($q
...
insert_into($Q,$O){return
queries("INSERT INTO ".table($Q).($O?" (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")":"DEFAULT VALUES"));}function
insert_update($Q,$O,$ld){$we=array();$Z=array();foreach($O
as$z=>$X){$we[]="$z = $X";if(isset($ld[idf_unescape($z)]))$Z[]="$z = $X";}return
queries("MERGE ".table($Q)." USING (VALUES(".implode(", ",$O).")) AS source (c".implode(", c",range(1,count($O))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$we)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($O)).") VALUES (".implode(", ",$O).");");}function
...
queries("DROP TABLE ".implode(", ",array_map('table',$S)));}function
move_tables($S,$Ce,$T){return
apply_queries("ALTER SCHEMA ".idf_escape($T)." TRANSFER",array_merge($S,$Ce));}function
trigger($D){if($D=="")return
...
query($H,$pe=false){$I=@($pe?mysql_unbuffered_query($H,$this->_link):mysql_query($H,$this->_link));$this->error="";if(!$I){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
...
true;}function
select_db($l){return$this->query("USE ".idf_escape($l));}function
query($H,$pe=false){$this->setAttribute(1000,!$pe);return
parent::query($H,$pe);}}}function
idf_escape($v){return"`".str_replace("`","``",$v)."`";}function
table($v){return
idf_escape($v);}function
connect(){global$c;$j=new
Min_DB;$Ta=$c->credentials();if($j->connect($Ta[0],$Ta[1],$Ta[2])){$j->query("SET sql_quote_show_create = 1, autocommit = 1");return$j;}$J=$j->error;if(function_exists('iconv')&&!is_utf8($J)&&strlen($Bd=iconv("windows-1250","utf-8",$J))>strlen($J))$J=$Bd;return$J;}function
...
foreign_keys($Q){global$j,$Pc;static$gd='`(?:[^`]|``)+`';$J=array();$Sa=$j->result("SHOW CREATE TABLE ".table($Q),1);if($Sa){preg_match_all("~CONSTRAINT ($gd) FOREIGN KEY \\(((?:$gd,? ?)+)\\) REFERENCES ($gd)(?:\\.($gd))? \\(((?:$gd,? ?)+)\\)(?: ON DELETE ($Pc))?(?: ON UPDATE ($Pc))?~",$Sa,$Ac,PREG_SET_ORDER);foreach($Ac
as$B){preg_match_all("~$gd~",$B[2],$Kd);preg_match_all("~$gd~",$B[5],$T);$J[idf_unescape($B[1])]=array("db"=>idf_unescape($B[4]!=""?$B[3]:$B[4]),"table"=>idf_unescape($B[4]!=""?$B[4]:$B[3]),"source"=>array_map('idf_unescape',$Kd[0]),"target"=>array_map('idf_unescape',$T[0]),"on_delete"=>($B[6]?$B[6]:"RESTRICT"),"on_update"=>($B[7]?$B[7]:"RESTRICT"),);}}return$J;}function
view($D){global$j;return
...
apply_queries("DROP DATABASE",$m,'idf_escape');}function
rename_database($D,$f){if(create_database($D,$f)){$vd=array();foreach(tables_list()as$Q=>$U)$vd[]=table($Q)." TO ".idf_escape($D).".".table($Q);if(!$vd||queries("RENAME TABLE ".implode(", ",$vd))){queries("DROP DATABASE ".idf_escape(DB));return
true;}}return
false;}function
...
alter_table($Q,$D,$q,$Ib,$Na,$pb,$f,$ta,$fd){$d=array();foreach($q
as$p)$d[]=($p[1]?($Q!=""?($p[0]!=""?"CHANGE ".idf_escape($p[0]):"ADD"):" ")." ".implode($p[1]).($Q!=""?$p[2]:""):"DROP ".idf_escape($p[0]));$d=array_merge($d,$Ib);$Nd="COMMENT=".q($Na).($pb?" ENGINE=".q($pb):"").($f?" COLLATE ".q($f):"").($ta!=""?" AUTO_INCREMENT=$ta":"").$fd;if($Q=="")return
queries("CREATE TABLE ".table($D)." (\n".implode(",\n",$d)."\n) $Nd");if($Q!=$D)$d[]="RENAME TO ".table($D);$d[]=$Nd;return
...
drop_tables($S){return
queries("DROP TABLE ".implode(", ",array_map('table',$S)));}function
move_tables($S,$Ce,$T){$vd=array();foreach(array_merge($S,$Ce)as$Q)$vd[]=table($Q)." TO ".idf_escape($T).".".table($Q);return
queries("RENAME TABLE ".implode(", ",$vd));}function
copy_tables($S,$Ce,$T){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($S
...
false;}foreach($Ce
as$Q){$D=($T==DB?table("copy_$Q"):idf_escape($T).".".table($Q));$Be=view($Q);if(!queries("DROP VIEW IF EXISTS $D")||!queries("CREATE VIEW $D AS $Be[select]"))return
false;}return
true;}function
trigger($D){if($D=="")return
...
routine($D,$U){global$j,$qb,$ic,$oe;$na=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$ne="((".implode("|",array_merge(array_keys($oe),$na)).")\\b(?:\\s*\\(((?:[^'\")]*|$qb)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$gd="\\s*(".($U=="FUNCTION"?"":$ic).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$ne";$Ra=$j->result("SHOW CREATE $U ".idf_escape($D),2);preg_match("~\\(((?:$gd\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$ne\\s+":"")."(.*)~is",$Ra,$B);$q=array();preg_match_all("~$gd\\s*,?~is",$B[1],$Ac,PREG_SET_ORDER);foreach($Ac
as$dd){$D=str_replace("``","`",$dd[2]).$dd[3];$q[]=array("field"=>$D,"type"=>strtolower($dd[5]),"length"=>preg_replace_callback("~$qb~s",'normalize_enum',$dd[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$dd[8] $dd[7]"))),"null"=>1,"full_type"=>$dd[4],"inout"=>strtoupper($dd[1]),"collation"=>strtolower($dd[9]),);}if($U!="FUNCTION")return
...
insert_into($Q,$O){return
queries("INSERT INTO ".table($Q)." (".implode(", ",array_keys($O)).")\nVALUES (".implode(", ",$O).")");}function
insert_update($Q,$O,$ld){foreach($O
as$z=>$X)$O[$z]="$z = $X";$we=implode(", ",$O);return
queries("INSERT INTO ".table($Q)." SET $we ON DUPLICATE KEY UPDATE $we");}function
...
use_sql($l){return"USE ".idf_escape($l);}function
trigger_sql($Q,$Qd){$J="";foreach(get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($Q,"%_")),null,"-- ")as$K)$J.="\n".($Qd=='CREATE+ALTER'?"DROP TRIGGER IF EXISTS ".idf_escape($K["Trigger"]).";;\n":"")."CREATE TRIGGER ".idf_escape($K["Trigger"])." $K[Timing] $K[Event] ON ".table($K["Table"])." FOR EACH ROW\n$K[Statement];;\n";return$J;}function
show_variables(){return
...
login($yc,$G){global$j;$j->query("SET time_zone = ".q(substr_replace(@date("O"),":",-2,0)));return
true;}function
tableName($Vd){return
h($Vd["Comment"]!=""?$Vd["Comment"]:$Vd["Name"]);}function
fieldName($p,$Yc=0){return
h($p["comment"]!=""?$p["comment"]:$p["field"]);}function
selectLinks($Vd,$O=""){$b=$Vd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($b).$O).'">'.lang(25)."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".lang(26)."'>&gt;&gt;</a>\n";}function
foreignKeys($Q){return
foreign_keys($Q);}function
backwardKeys($Q,$Ud){$J=array();foreach(get_rows("SELECT TABLE_NAME, CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_COLUMN_NAME
...
as$z=>$X){$D=$this->tableName(table_status($z));if($D!=""){$Ed=preg_quote($Ud);$Hd="(:|\\s*-)?\\s+";$J[$z]["name"]=(preg_match("(^$Ed$Hd(.+)|^(.+?)$Hd$Ed\$)iu",$D,$B)?$B[2].$B[3]:$D);}else
...
rowDescriptions($L,$Kb){$J=$L;foreach($L[0]as$z=>$X){if(list($Q,$u,$D)=$this->_foreignColumn($Kb,$z)){$cc=array();foreach($L
as$K)$cc[$K[$z]]=exact_value($K[$z]);$Ya=$this->_values[$Q];if(!$Ya)$Ya=get_key_vals("SELECT $u, $D FROM ".table($Q)." WHERE $u IN (".implode(", ",$cc).")");foreach($L
as$C=>$K){if(isset($K[$z]))$J[$C][$z]=(string)$Ya[$K[$z]];}}}return$J;}function
selectLink($X,$p){}function
selectVal($X,$A,$p){$J=($X===null?"&nbsp;":$X);$A=h($A);if(ereg('blob|bytea',$p["type"])&&!is_utf8($X)){$J=lang(27,strlen($X));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$X))$J="<img src='$A' alt='$J'>";}if(like_bool($p)&&$J!="&nbsp;")$J=($X?lang(28):lang(29));if($A)$J="<a href='$A'>$J</a>";if(!$A&&!like_bool($p)&&ereg('int|float|double|decimal',$p["type"]))$J="<div class='number'>$J</div>";elseif(ereg('date',$p["type"]))$J="<div class='datetime'>$J</div>";return$J;}function
...
as$z=>$X)$oc[$X["col"]]=$z;$t=0;$q=fields($_GET["select"]);foreach($h
as$D=>$Xa){$p=$q[$D];if(ereg("enum",$p["type"])||like_bool($p)){$z=$oc[$D];$t--;echo"<div>".h($Xa)."<input type='hidden' name='where[$t][col]' value='".h($D)."'>:",(like_bool($p)?" <select name='where[$t][val]'>".optionlist(array(""=>"",lang(29),lang(28)),$Z[$z]["val"],true)."</select>":enum_input("checkbox"," name='where[$t][val][]'",$p,(array)$Z[$z]["val"],($p["null"]?0:null))),"</div>\n";unset($h[$D]);}elseif(is_array($Wc=$this->_foreignKeyOptions($_GET["select"],$D))){if($q[$D]["null"])$Wc[0]='('.lang(4).')';$z=$oc[$D];$t--;echo"<div>".h($Xa)."<input type='hidden' name='where[$t][col]' value='".h($D)."'><input type='hidden' name='where[$t][op]' value='='>: <select name='where[$t][val]'>".optionlist($Wc,$Z[$z]["val"],true)."</select></div>\n";unset($h[$D]);}}$t=0;foreach($Z
...
selectOrderPrint($Yc,$h,$x){$Zc=array();foreach($x
as$z=>$w){$Yc=array();foreach($w["columns"]as$X)$Yc[]=$h[$X];if(count(array_filter($Yc,'strlen'))>1&&$z!="PRIMARY")$Zc[$z]=implode(", ",$Yc);}if($Zc){echo'<fieldset><legend>'.lang(33)."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Zc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectSearchProcess($q,$x){$J=array();foreach((array)$_GET["where"]as$z=>$Z){$Ja=$Z["col"];$Sc=$Z["op"];$X=$Z["val"];if(($z<0?"":$Ja).$X!=""){$Oa=array();foreach(($Ja!=""?array($Ja=>$q[$Ja]):$q)as$D=>$p){if($Ja!=""||is_numeric($X)||!ereg('int|float|double|decimal',$p["type"])){$D=idf_escape($D);if($Ja!=""&&$p["type"]=="enum")$Oa[]=(in_array(0,$X)?"$D IS NULL OR ":"")."$D IN (".implode(", ",array_map('intval',$X)).")";else{$ae=ereg('char|text|enum|set',$p["type"]);$Y=$this->processInput($p,(!$Sc&&$ae&&ereg('^[^%]+$',$X)?"%$X%":$X));$Oa[]=$D.($Y=="NULL"?" IS".($Sc==">="?" NOT":"")." $Y":(in_array($Sc,$this->operators)||$Sc=="="?" $Sc $Y":($ae?" LIKE $Y":" IN (".str_replace(",","', '",$Y).")")));if($z<0&&$X=="0")$Oa[]="$D IS NULL";}}}$J[]=($Oa?"(".implode(" OR ",$Oa).")":"0");}}return$J;}function
selectOrderProcess($q,$x){$ec=$_GET["index_order"];if($ec!="")unset($_GET["order"][1]);if($_GET["order"])return
array(idf_escape(reset($_GET["order"])).($_GET["desc"]?" DESC":""));foreach(($ec!=""?array($x[$ec]):$x)as$w){if($ec!=""||$w["type"]=="INDEX"){$Xa=false;foreach($w["columns"]as$X){if(ereg('date|timestamp',$q[$X]["type"])){$Xa=true;break;}}$J=array();foreach($w["columns"]as$X)$J[]=idf_escape($X).($Xa?" DESC":"");return$J;}}return
array();}function
selectLimitProcess(){return(isset($_GET["limit"])?$_GET["limit"]:"30");}function
selectLengthProcess(){return"100";}function
selectEmailProcess($Z,$Kb){if($_POST["email_append"])return
true;if($_POST["email"]){$Gd=0;if($_POST["all"]||$_POST["check"]){$p=idf_escape($_POST["email_field"]);$Rd=$_POST["email_subject"];$Fc=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$Rd.$Fc",$Ac);$L=get_rows("SELECT DISTINCT $p".($Ac[1]?", ".implode(", ",array_map('idf_escape',array_unique($Ac[1]))):"")." FROM ".table($_GET["select"])." WHERE $p IS NOT NULL AND $p != ''".($Z?" AND ".implode(" AND ",$Z):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$q=fields($_GET["select"]);foreach($this->rowDescriptions($L,$Kb)as$K){$wd=array('{\\'=>'{');foreach($Ac[1]as$X)$wd['{$'."$X}"]=$this->editVal($K[$X],$q[$X]);$lb=$K[$_POST["email_field"]];if(is_mail($lb)&&send_mail($lb,strtr($Rd,$wd),strtr($Fc,$wd),$_POST["email_from"],$_FILES["email_files"]))$Gd++;}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(43,$Gd));}return
false;}function
selectQueryBuild($M,$Z,$Sb,$Yc,$_,$F){return"";}function
messageQuery($H){return" <span class='time'>".@date("H:i:s")."</span><!--\n".str_replace("--","--><!-- ",$H)."\n-->";}function
editFunctions($p){$J=array();if($p["null"]&&ereg('blob',$p["type"]))$J["NULL"]=lang(4);$J[""]=($p["null"]||$p["auto_increment"]||like_bool($p)?"":"*");if(ereg('date|time',$p["type"]))$J["now"]=lang(44);if(eregi('_(md5|sha1)$',$p["field"],$B))$J[]=strtolower($B[1]);return$J;}function
editInput($Q,$p,$sa,$Y){if($p["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$sa value='-1' checked><i>".lang(5)."</i></label> ":"").enum_input("radio",$sa,$p,($Y||isset($_GET["select"])?$Y:0),($p["null"]?"":null));$Wc=$this->_foreignKeyOptions($Q,$p["field"],$Y);if($Wc!==null)return(is_array($Wc)?"<select$sa>".optionlist($Wc,$Y,true)."</select>":"<input value='".h($Y)."'$sa class='hidden'><input value='".h($Wc)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($Q)."&field=".urlencode($p["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");if(like_bool($p))return'<input type="checkbox" value="'.h($Y?$Y:1).'"'.($Y?' checked':'')."$sa>";$Yb="";if(ereg('time',$p["type"]))$Yb=lang(45);if(ereg('date|timestamp',$p["type"]))$Yb=lang(46).($Yb?" [$Yb]":"");if($Yb)return"<input value='".h($Y)."'$sa> ($Yb)";if(eregi('_(md5|sha1)$',$p["field"]))return"<input type='password' value='".h($Y)."'$sa>";return'';}function
processInput($p,$Y,$s=""){if($s=="now")return"$s()";$J=$Y;if(ereg('date|timestamp',$p["type"])&&preg_match('(^'.str_replace('\\$1','(?P<p1>\\d*)',preg_replace('~(\\\\\\$([2-6]))~','(?P<p\\2>\\d{1,2})',preg_quote(lang(30)))).'(.*))',$Y,$B))$J=($B["p1"]!=""?$B["p1"]:($B["p2"]!=""?($B["p2"]<70?20:19).$B["p2"]:gmdate("Y")))."-$B[p3]$B[p4]-$B[p5]$B[p6]".end($B);$J=($p["type"]=="bit"&&ereg('^[0-9]+$',$Y)?$J:q($J));if($Y==""&&like_bool($p))$J="0";elseif($Y==""&&($p["null"]||!ereg('char|text',$p["type"])))$J="NULL";elseif(ereg('^(md5|sha1)$',$s))$J="$s($J)";return
...
dumpData($Q,$Qd,$H){global$j;$I=$j->query($H,1);if($I){while($K=$I->fetch_assoc()){if($Qd=="table"){dump_csv(array_keys($K));$Qd="INSERT";}dump_csv($K);}}}function
dumpFilename($bc){return
friendly_url($bc);}function
dumpHeaders($bc,$Jc=false){$yb="csv";header("Content-Type: text/csv; charset=utf-8");return$yb;}function
homepage(){return
true;}function
navigation($Ic){global$ba,$ge;echo'<h1>
...
';if($Ic=="auth"){$Gb=true;foreach((array)$_SESSION["pwds"]["server"][""]as$V=>$G){if($G!==null){if($Gb){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Gb=false;}echo"<a href='".h(auth_url("server","",$V))."'>".($V!=""?h($V):"<i>".lang(4)."</i>")."</a><br>\n";}}}else{echo'<form action="" method="post">
<p class="logout">
<input type="submit" name="logout" value="',lang(47),'" id="logout">
<input type="hidden" name="token" value="',$ge,'">
</p>
</form>
';$this->databasesPrint($Ic);if($Ic!="db"&&$Ic!="ns"){$R=table_status();if(!$R)echo"<p class='message'>".lang(6)."\n";else$this->tablesPrint($R);}}}function
databasesPrint($Ic){}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$K){$D=$this->tableName($K);if(isset($K["Engine"])&&$D!="")echo"<a href='".h(ME).'select='.urlencode($K["Name"])."'".bold($_GET["select"]==$K["Name"])." title='".lang(48)."'>$D</a><br>\n";}}function
...
Adminer);function
page_header($ce,$o="",$Da=array(),$de=""){global$a,$c,$j,$eb;header("Content-Type: text/html; charset=utf-8");if($c->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}$ee=$ce.($de!=""?": ".h($de):"");$fe=strip_tags($ee.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$c->name());echo'<!DOCTYPE html>
...
<div id="content">
';if($Da!==null){$A=substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1);echo'<p id="breadcrumb"><a href="'.h($A?$A:".").'">'.$eb[DRIVER].'</a> &raquo; ';$A=substr(preg_replace('~(db|ns)=[^&]*&~','',ME),0,-1);$N=(SERVER!=""?h(SERVER):lang(50));if($Da===false)echo"$N\n";else{echo"<a href='".($A?h($A):".")."' accesskey='1' title='Alt+Shift+1'>$N</a> &raquo; ";if($_GET["ns"]!=""||(DB!=""&&is_array($Da)))echo'<a href="'.h($A."&db=".urlencode(DB).(support("scheme")?"&ns=":"")).'">'.h(DB).'</a> &raquo; ';if(is_array($Da)){if($_GET["ns"]!="")echo'<a href="'.h(substr(ME,0,-1)).'">'.h($_GET["ns"]).'</a> &raquo; ';foreach($Da
as$z=>$X){$Xa=(is_array($X)?$X[1]:$X);if($Xa!="")echo'<a href="'.h(ME."$z=").urlencode(is_array($X)?$X[0]:$X).'">'.h($Xa).'</a> &raquo; ';}}echo"$ce\n";}}echo"<h2>$ee</h2>\n";restart_session();$xe=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$Gc=$_SESSION["messages"][$xe];if($Gc){echo"<div class='message'>".implode("</div>\n<div class='message'>",$Gc)."</div>\n";unset($_SESSION["messages"][$xe]);}$m=&get_session("dbs");if(DB!=""&&$m&&!in_array(DB,$m,true))$m=null;stop_session();if($o)echo"<div class='error'>$o</div>\n";define("PAGE_HEADER",1);}function
page_footer($Ic=""){global$c;echo'</div>

';switch_lang();echo'<div id="menu">
';$c->navigation($Ic);echo'</div>
';}function
int32($C){while($C>=2147483648)$C-=4294967296;while($C<=-2147483649)$C+=4294967296;return(int)$C;}function
long2str($W,$Ee){$Bd='';foreach($W
as$X)$Bd.=pack('V',$X);if($Ee)return
substr($Bd,0,end($W));return$Bd;}function
str2long($Bd,$Ee){$W=array_values(unpack('V*',str_pad($Bd,4*ceil(strlen($Bd)/4),"\0")));if($Ee)$W[]=strlen($Bd);return$W;}function
xxtea_mx($Je,$Ie,$Td,$mc){return
int32((($Je>>5&0x7FFFFFF)^$Ie<<2)+(($Ie>>3&0x1FFFFFFF)^$Je<<4))^int32(($Td^$Ie)+($mc^$Je));}function
encrypt_string($Od,$z){if($Od=="")return"";$z=array_values(unpack("V*",pack("H*",md5($z))));$W=str2long($Od,true);$C=count($W)-1;$Je=$W[$C];$Ie=$W[0];$qd=floor(6+52/($C+1));$Td=0;while($qd-->0){$Td=int32($Td+0x9E3779B9);$hb=$Td>>2&3;for($cd=0;$cd<$C;$cd++){$Ie=$W[$cd+1];$Kc=xxtea_mx($Je,$Ie,$Td,$z[$cd&3^$hb]);$Je=int32($W[$cd]+$Kc);$W[$cd]=$Je;}$Ie=$W[0];$Kc=xxtea_mx($Je,$Ie,$Td,$z[$cd&3^$hb]);$Je=int32($W[$C]+$Kc);$W[$C]=$Je;}return
long2str($W,false);}function
decrypt_string($Od,$z){if($Od=="")return"";$z=array_values(unpack("V*",pack("H*",md5($z))));$W=str2long($Od,false);$C=count($W)-1;$Je=$W[$C];$Ie=$W[0];$qd=floor(6+52/($C+1));$Td=int32($qd*0x9E3779B9);while($Td){$hb=$Td>>2&3;for($cd=$C;$cd>0;$cd--){$Je=$W[$cd-1];$Kc=xxtea_mx($Je,$Ie,$Td,$z[$cd&3^$hb]);$Ie=int32($W[$cd]-$Kc);$W[$cd]=$Ie;}$Je=$W[$C];$Kc=xxtea_mx($Je,$Ie,$Td,$z[$cd&3^$hb]);$Ie=int32($W[0]-$Kc);$W[0]=$Ie;$Td=int32($Td-0x9E3779B9);}return
long2str($W,true);}$j='';$ge=$_SESSION["token"];if(!$_SESSION["token"])$_SESSION["token"]=rand(1,1e6);$hd=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($z)=explode(":",$X);$hd[$z]=$X;}}$e=$_POST["auth"];if($e){session_regenerate_id();$_SESSION["pwds"][$e["driver"]][$e["server"]][$e["username"]]=$e["password"];$_SESSION["db"][$e["driver"]][$e["server"]][$e["username"]][$e["db"]]=true;if($e["permanent"]){$z=base64_encode($e["driver"])."-".base64_encode($e["server"])."-".base64_encode($e["username"])."-".base64_encode($e["db"]);$nd=$c->permanentLogin();$hd[$z]="$z:".base64_encode($nd?encrypt_string($e["password"],$nd):"");cookie("adminer_permanent",implode(" ",$hd));}if(count($_POST)==1||DRIVER!=$e["driver"]||SERVER!=$e["server"]||$_GET["username"]!==$e["username"]||DB!=$e["db"])redirect(auth_url($e["driver"],$e["server"],$e["username"],$e["db"]));}elseif($_POST["logout"]){if($ge&&$_POST["token"]!=$ge){page_header(lang(47),lang(51));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$z)set_session($z,null);unset_permanent();redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),lang(52));}}elseif($hd&&!$_SESSION["pwds"]){session_regenerate_id();$nd=$c->permanentLogin();foreach($hd
as$z=>$X){list(,$Ga)=explode(":",$X);list($db,$N,$V,$n)=array_map('base64_decode',explode("-",$z));$_SESSION["pwds"][$db][$N][$V]=decrypt_string(base64_decode($Ga),$nd);$_SESSION["db"][$db][$N][$V][$n]=true;}}function
unset_permanent(){global$hd;foreach($hd
as$z=>$X){list($db,$N,$V)=array_map('base64_decode',explode("-",$z));if($db==DRIVER&&$N==SERVER&&$n==$_GET["username"])unset($hd[$z]);}cookie("adminer_permanent",implode(" ",$hd));}function
auth_error($vb=null){global$j,$c,$ge;$Jd=session_name();$o="";if(!$_COOKIE[$Jd]&&$_GET[$Jd]&&ini_bool("session.use_only_cookies"))$o=lang(53);elseif(isset($_GET["username"])){if(($_COOKIE[$Jd]||$_GET[$Jd])&&!$ge)$o=lang(54);else{$G=&get_session("pwds");if($G!==null){$o=h($vb?$vb->getMessage():(is_string($j)?$j:lang(55)));$G=null;}unset_permanent();}}page_header(lang(23),$o,null);echo"<form action='' method='post'>\n";$c->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header(lang(56),lang(57,implode(", ",$kd)),false);page_footer("auth");exit;}$j=connect();}if(is_string($j)||!$c->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$ge=$_SESSION["token"];if($e&&$_POST["token"])$_POST["token"]=$ge;$o=($_POST?($_POST["token"]==$ge?"":lang(51)):($_SERVER["REQUEST_METHOD"]!="POST"?"":lang(58,'"post_max_size"')));if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}$j->select_db($c->database());function
email_header($Wb){return"=?UTF-8?B?".base64_encode($Wb)."?=";}function
send_mail($lb,$Rd,$Fc,$Qb="",$Eb=array()){$rb=(strncasecmp(PHP_OS,"win",3)?"\n":"\r\n");$Fc=str_replace("\n",$rb,wordwrap(str_replace("\r","","$Fc\n")));$Ca=uniqid("boundary");$ra="";foreach((array)$Eb["error"]as$z=>$X){if(!$X)$ra.="--$Ca$rb"."Content-Type: ".str_replace("\n","",$Eb["type"][$z]).$rb."Content-Disposition: attachment; filename=\"".preg_replace('~["\\n]~','',$Eb["name"][$z])."\"$rb"."Content-Transfer-Encoding: base64$rb$rb".chunk_split(base64_encode(file_get_contents($Eb["tmp_name"][$z])),76,$rb).$rb;}$za="";$Xb="Content-Type: text/plain; charset=utf-8$rb"."Content-Transfer-Encoding: 8bit";if($ra){$ra.="--$Ca--$rb";$za="--$Ca$rb$Xb$rb$rb";$Xb="Content-Type: multipart/mixed; boundary=\"$Ca\"";}$Xb.=$rb."MIME-Version: 1.0$rb"."X-Mailer: Adminer Editor".($Qb?$rb."From: ".str_replace("\n","",$Qb):"");return
mail($lb,email_header($Rd),$za.$Fc.$ra,$Xb);}function
like_bool($p){return
ereg("bool|(tinyint|bit)\\(1\\)",$p["full_type"]);}$Pc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$eb[DRIVER]=lang(23);if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$b=$_GET["download"];$q=fields($b);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$b-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$j->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($b)," WHERE ".where($_GET,$q),1));exit;}elseif(isset($_GET["edit"])){$b=$_GET["edit"];$q=fields($b);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$q):""):where($_GET,$q));$we=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($q
as$D=>$p){if(!isset($p["privileges"][$we?"update":"insert"])||$c->fieldName($p)=="")unset($q[$D]);}if($_POST&&!$o&&!isset($_GET["select"])){$xc=$_POST["referer"];if($_POST["insert"])$xc=($we?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$xc))$xc=ME."select=".urlencode($b);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($b)," WHERE $Z"),$xc,lang(59));else{$O=array();foreach($q
as$D=>$p){$X=process_input($p);if($X!==false&&$X!==null)$O[idf_escape($D)]=($we?"\n".idf_escape($D)." = $X":$X);}if($we){if(!$O)redirect($xc);query_redirect("UPDATE".limit1(table($b)." SET".implode(",",$O),"\nWHERE $Z"),$xc,lang(60));}else{$I=insert_into($b,$O);$tc=($I?last_id():0);queries_redirect($xc,lang(61,($tc?" $tc":"")),$I);}}}$Wd=$c->tableName(table_status($b));page_header(($we?lang(62):lang(40)),$o,array("select"=>array($b,$Wd)),$Wd);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($q
...
as$D=>$p){echo"<tr><th>".$c->fieldName($p);$Wa=$_GET["set"][bracket_escape($D)];$Y=($K!==null?($K[$D]!=""&&$y=="sql"&&ereg("enum|set",$p["type"])?(is_array($K[$D])?array_sum($K[$D]):+$K[$D]):$K[$D]):(!$we&&$p["auto_increment"]?"":(isset($_GET["select"])?false:($Wa!==null?$Wa:$p["default"]))));if(!$_POST["save"]&&is_string($Y))$Y=$c->editVal($Y,$p);$s=($_POST["save"]?(string)$_POST["function"][$D]:($we&&$p["on_update"]=="CURRENT_TIMESTAMP"?"now":($Y===false?null:($Y!==null?'':'NULL'))));if($p["type"]=="timestamp"&&$Y=="CURRENT_TIMESTAMP"){$Y="";$s="now";}input($p,$Y,$s);echo"\n";}echo"</table>\n";}echo'<p>
';if($q){echo"<input type='submit' value='".lang(65)."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($we?lang(66):lang(67))."' title='Ctrl+Shift+Enter'>\n";}echo($we?"<input type='submit' name='delete' value='".lang(68)."' onclick=\"return confirm('".lang(0)."');\">\n":($_POST||!$q?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$ge,'">
</form>
';}elseif(isset($_GET["select"])){$b=$_GET["select"];$R=table_status($b);$x=indexes($b);$q=fields($b);$Lb=column_foreign_keys($b);$Oc="";if($R["Oid"]=="t"){$Oc=($y=="sqlite"?"rowid":"oid");$x[]=array("type"=>"PRIMARY","columns"=>array($Oc));}parse_str($_COOKIE["adminer_import"],$ka);$_d=array();$h=array();$Zd=null;foreach($q
as$z=>$p){$D=$c->fieldName($p);if(isset($p["privileges"]["select"])&&$D!=""){$h[$z]=html_entity_decode(strip_tags($D));if(is_shortable($p))$Zd=$c->selectLengthProcess();}$_d+=$p["privileges"];}list($M,$Sb)=$c->selectColumnsProcess($h,$x);$kc=count($Sb)<count($M);$Z=$c->selectSearchProcess($q,$x);$Yc=$c->selectOrderProcess($q,$x);$_=$c->selectLimitProcess();$Qb=($M?implode(", ",$M):"*".($Oc?", $Oc":""));if($y=="sql"){foreach($h
as$z=>$X){$pa=convert_field($q[$z]);if($pa)$Qb.=", $pa AS ".idf_escape($z);}}$Qb.="\nFROM ".table($b);$Tb=($Sb&&$kc?"\nGROUP BY ".implode(", ",$Sb):"").($Yc?"\nORDER BY ".implode(", ",$Yc):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$se=>$K){$pa=convert_field($q[key($K)]);echo$j->result("SELECT".limit(($pa?$pa:idf_escape(key($K)))." FROM ".table($b)," WHERE ".where_check($se,$q).($Z?" AND ".implode(" AND ",$Z):"").($Yc?" ORDER BY ".implode(", ",$Yc):""),1));}exit;}if($_POST&&!$o){$Ge="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$ld=$ue=null;foreach($x
as$w){if($w["type"]=="PRIMARY"){$ld=array_flip($w["columns"]);$ue=($M?$ld:array());break;}}foreach((array)$ue
as$z=>$X){if(in_array(idf_escape($z),$M))unset($ue[$z]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($b);$c->dumpTable($b,"");if(!is_array($_POST["check"])||$ue===array()){$Fe=$Z;if(is_array($_POST["check"]))$Fe[]="($Ge)";$H="SELECT $Qb".($Fe?"\nWHERE ".implode(" AND ",$Fe):"").$Tb;}else{$qe=array();foreach($_POST["check"]as$X)$qe[]="(SELECT".limit($Qb,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$q).$Tb,1).")";$H=implode(" UNION ALL ",$qe);}$c->dumpData($b,"table",$H);exit;}if(!$c->selectEmailProcess($Z,$Lb)){if($_POST["save"]||$_POST["delete"]){$I=true;$la=0;$H=table($b);$O=array();if(!$_POST["delete"]){foreach($h
as$D=>$X){$X=process_input($q[$D]);if($X!==null){if($_POST["clone"])$O[idf_escape($D)]=($X!==false?$X:idf_escape($D));elseif($X!==false)$O[]=idf_escape($D)." = $X";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($b):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ma="UPDATE";if($_POST["delete"]){$Ma="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ma="INSERT";$H="INTO $H";}if($_POST["all"]||($ue===array()&&$_POST["check"])||$kc){$I=queries("$Ma $H".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $Ge"));$la=$j->affected_rows;}else{foreach((array)$_POST["check"]as$X){$I=queries($Ma.limit1($H,"\nWHERE ".where_check($X,$q)));if(!$I)break;$la+=$j->affected_rows;}}}$Fc=lang(69,$la);if($_POST["clone"]&&$I&&$la==1){$tc=last_id();if($tc)$Fc=lang(61," $tc");}queries_redirect(remove_from_uri("page"),$Fc,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$o=lang(70);else{$I=true;$la=0;foreach($_POST["val"]as$se=>$K){$O=array();foreach($K
as$z=>$X){$z=bracket_escape($z,1);$O[]=idf_escape($z)." = ".(ereg('char|text',$q[$z]["type"])||$X!=""?$c->processInput($q[$z],$X):"NULL");}$H=table($b)." SET ".implode(", ",$O);$Fe=" WHERE ".where_check($se,$q).($Z?" AND ".implode(" AND ",$Z):"");$I=queries("UPDATE".($kc?" $H$Fe":limit1($H,$Fe)));if(!$I)break;$la+=$j->affected_rows;}queries_redirect(remove_from_uri(),lang(69,$la),$I);}}elseif(is_string($Cb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ka["output"])."&format=".urlencode($_POST["separator"]));$I=true;$La=array_keys($q);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Cb,$Ac);$la=count($Ac[0]);begin();$Hd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($Ac[0]as$z=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Hd]*)$Hd~",$X.$Hd,$Bc);if(!$z&&!array_diff($Bc[1],$La)){$La=$Bc[1];$la--;}else{$O=array();foreach($Bc[1]as$t=>$Ja)$O[idf_escape($La[$t])]=($Ja==""&&$q[$La[$t]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Ja))));$I=insert_update($b,$O,$ld);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(71,$la),$I);queries("ROLLBACK");}else$o=upload_error($Cb);}}$Wd=$c->tableName($R);if(is_ajax())ob_start();page_header(lang(36).": $Wd",$o);$O=null;if(isset($_d["insert"])){$O="";foreach((array)$_GET["where"]as$X){if(count($Lb[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!ereg('[_%]',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$c->selectLinks($R,$O);if(!$h)echo"<p class='error'>".lang(72).($q?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($b).'">',"</div>\n";$c->selectColumnsPrint($M,$h);$c->selectSearchPrint($Z,$h,$x);$c->selectOrderPrint($Yc,$h,$x);$c->selectLimitPrint($_);$c->selectLengthPrint($Zd);$c->selectActionPrint($x);echo"</form>\n";$F=$_GET["page"];if($F=="last"){$Ob=$j->result("SELECT COUNT(*) FROM ".table($b).($Z?" WHERE ".implode(" AND ",$Z):""));$F=floor(max(0,$Ob-1)/$_);}$H=$c->selectQueryBuild($M,$Z,$Sb,$Yc,$_,$F);if(!$H)$H="SELECT".limit((+$_&&$Sb&&$kc&&$y=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Qb,($Z?"\nWHERE ".implode(" AND ",$Z):"").$Tb,($_!=""?+$_:null),($F?$_*$F:0),"\n");echo$c->selectQuery($H);$I=$j->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($y=="mssql")$I->seek($_*$F);$nb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($F&&$y=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$Ob=(+$_&&$Sb&&$kc?($y=="sql"?$j->result(" SELECT FOUND_ROWS()"):$j->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".lang(63)."\n";else{$ya=$c->backwardKeys($b,$Wd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Sb&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(73)."</a>");$Lc=array();$Rb=array();reset($M);$sd=1;foreach($L[0]as$z=>$X){if($z!=$Oc){$X=$_GET["columns"][key($M)];$p=$q[$M?($X?$X["col"]:current($M)):$z];$D=($p?$c->fieldName($p,$sd):"*");if($D!=""){$sd++;$Lc[$z]=$D;$g=idf_escape($z);$ac=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($z);$Xa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($ac.($Yc[0]==$g||$Yc[0]==$z||(!$Yc&&$kc&&$Sb[0]==$g)?$Xa:'')).'">';echo(!$M||$X?apply_sql_function($X["fun"],$D):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($ac.$Xa)."' title='".lang(74)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($z)).'\'); return false;" title="'.lang(31).'" class="text jsonly"> =</a>';echo"</span>";}$Rb[$z]=$X["fun"];next($M);}}$wc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$z=>$X)$wc[$z]=max($wc[$z],min(40,strlen(utf8_decode($X))));}}echo($ya?"<th>".lang(75):"")."</thead>\n";if(is_ajax()){if($_%2==1&&$F%2==1)odd();ob_end_clean();}foreach($c->rowDescriptions($L,$Lb)as$C=>$K){$re=unique_array($L[$C],$x);$se="";foreach($re
as$z=>$X)$se.="&".($X!==null?urlencode("where[".bracket_escape($z)."]")."=".urlencode($X):"null%5B%5D=".urlencode($z));echo"<tr".odd().">".(!$Sb&&$M?"":"<td>".checkbox("check[]",substr($se,1),in_array(substr($se,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($kc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($b).$se)."'>".lang(73)."</a>"));foreach($K
as$z=>$X){if(isset($Lc[$z])){$p=$q[$z];if($X!=""&&(!isset($nb[$z])||$nb[$z]!=""))$nb[$z]=(is_mail($X)?$Lc[$z]:"");$A="";$X=$c->editVal($X,$p);if($X!==null){if(ereg('blob|bytea|raw|file',$p["type"])&&$X!="")$A=ME.'download='.urlencode($b).'&field='.urlencode($z).$se;if($X==="")$X="&nbsp;";elseif($Zd!=""&&is_shortable($p))$X=shorten_utf8($X,max(0,+$Zd));else$X=h($X);if(!$A){foreach((array)$Lb[$z]as$r){if(count($Lb[$z])==1||end($r["source"])==$z){$A="";foreach($r["source"]as$t=>$Kd)$A.=where_link($t,$r["target"][$t],$L[$C][$Kd]);$A=($r["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($r["db"]),ME):ME).'select='.urlencode($r["table"]).$A;if(count($r["source"])==1)break;}}}if($z=="COUNT(*)"){$A=ME."select=".urlencode($b);$t=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$re))$A.=where_link($t++,$W["col"],$W["val"],$W["op"]);}foreach($re
as$mc=>$W)$A.=where_link($t++,$mc,$W);}}if(!$A&&($A=$c->selectLink($K[$z],$p))===null){if(is_mail($K[$z]))$A="mailto:$K[$z]";if($pd=is_url($K[$z]))$A=($pd=="http"&&$aa?$K[$z]:"$pd://www.adminer.org/redirect/?url=".urlencode($K[$z]));}$u=h("val[$se][".bracket_escape($z)."]");$Y=$_POST["val"][$se][bracket_escape($z)];$Vb=h($Y!==null?$Y:$K[$z]);$zc=strpos($X,"<i>...</i>");$jb=is_utf8($X)&&$L[$C][$z]==$K[$z]&&!$Rb[$z];$Yd=ereg('text|lob',$p["type"]);echo(($_GET["modify"]&&$jb)||$Y!==null?"<td>".($Yd?"<textarea name='$u' cols='30' rows='".(substr_count($K[$z],"\n")+1)."'>$Vb</textarea>":"<input name='$u' value='$Vb' size='$wc[$z]'>"):"<td id='$u' onclick=\"selectClick(this, event, ".($zc?2:($Yd?1:0)).($jb?"":", '".h(lang(76))."'").");\">".$c->selectVal($X,$A,$p));}}if($ya)echo"<td>";$c->backwardKeysPrint($ya,$L[$C]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n",(!$Sb&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if(($L||$F)&&!is_ajax()){$ub=true;if($_GET["page"]!="last"&&+$_&&!$kc&&($Ob>=$_||$F)){$Ob=found_rows($R,$Z);if($Ob<max(1e4,2*($F+1)*$_))$Ob=reset(slow_query("SELECT COUNT(*) FROM ".table($b).($Z?" WHERE ".implode(" AND ",$Z):"")));else$ub=false;}echo"<p class='pages'>";if(+$_&&($Ob===false||$Ob>$_)){$Cc=($Ob===false?$F+(count($L)>=$_?2:1):floor(($Ob-1)/$_));echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(77)."', '".($F+1)."'), event); return false;\">".lang(77)."</a>:",pagination(0,$F).($F>5?" ...":"");for($t=max(1,$F-4);$t<min($Cc,$F+5);$t++)echo
pagination($t,$F);echo($F+5<$Cc?" ...":"").($ub&&$Ob!==false?pagination($Cc,$F):' <a href="'.h(remove_from_uri("page")."&page=last").'">'.lang(78)."</a>");}echo($Ob!==false?" (".($ub?"":"~ ").lang(79,$Ob).")":""),(+$_&&($Ob===false?count($L)+1:$Ob-$F*$_)>$_?' <a href="'.h(remove_from_uri("page")."&page=".($F+1)).'" onclick="return !selectLoadMore(this, '.(+$_).', \''.lang(80).'\');">'.lang(81).'</a>':'')," ".checkbox("all",1,0,lang(82))."\n";if($c->selectCommandPrint()){echo'<fieldset><legend>',lang(62),'</legend><div>
