...
* @version 4.0.0
*/error_reporting(6135);$ec=!preg_match('~^(unsafe_raw)?$~',ini_get("filter.default"));if($ec||ini_get("filter.default_flags")){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$X){$Bf=filter_input_array(constant("INPUT$X"),FILTER_UNSAFE_RAW);if($Bf)$$X=$Bf;}}if(function_exists("mb_internal_encoding"))mb_internal_encoding("8bit");if(isset($_GET["file"])){if($_SERVER["HTTP_IF_MODIFIED_SINCE"]){header("HTTP/1.1 304 Not Modified");exit;}header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
remove_slashes($fe,$ec=false){if(get_magic_quotes_gpc()){while(list($y,$X)=each($fe)){foreach($X
as$Sc=>$W){unset($fe[$y][$Sc]);if(is_array($W)){$fe[$y][stripslashes($Sc)]=$W;$fe[]=&$fe[$y][stripslashes($Sc)];}else$fe[$y][stripslashes($Sc)]=($ec?$W:stripslashes($W));}}}}function
bracket_escape($u,$Fa=false){static$qf=array(':'=>':1',']'=>':2','['=>':3');return
strtr($u,($Fa?array_flip($qf):$qf));}function
h($Q){return
...
optionlist($Kd,$Ee=null,$Hf=false){$K="";foreach($Kd
as$Sc=>$W){$Ld=array($Sc=>$W);if(is_array($W)){$K.='<optgroup label="'.h($Sc).'">';$Ld=$W;}foreach($Ld
as$y=>$X)$K.='<option'.($Hf||is_string($y)?' value="'.h($y).'"':'').(($Hf||is_string($y)?(string)$y:$X)===$Ee?' selected':'').'>'.h($X);if(is_array($W))$K.='</optgroup>';}return$K;}function
...
print_fieldset($t,$dd,$Of=false,$Fd=""){echo"<fieldset><legend><a href='#fieldset-$t' onclick=\"".h($Fd)."return !toggle('fieldset-$t');\">$dd</a></legend><div id='fieldset-$t'".($Of?"":" class='hidden'").">\n";}function
...
get_key_vals($I,$i=null,$if=0){global$h;if(!is_object($i))$i=$h;$K=array();$i->timeout=$if;$J=$i->query($I);$i->timeout=0;if(is_object($J)){while($L=$J->fetch_row())$K[$L[0]]=$L[1];}return$K;}function
...
auth_url($Kf,$O,$V,$m=null){global$zb;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($zb))."|username|".($m!==null?"db|":"").session_name()),$A);return"$A[1]?".(sid()?SID."&":"").($Kf!="server"||$O!=""?urlencode($Kf)."=".urlencode($O)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($A[2]?"&$A[2]":"");}function
...
query_redirect($I,$hd,$rd,$oe=true,$Tb=true,$Yb=false){global$h,$n,$b;$hf="";if($Tb){$Pe=microtime(true);$Yb=!$h->query($I);$hf="; -- ".format_time($Pe,microtime(true));}$Ne="";if($I)$Ne=$b->messageQuery($I.$hf);if($Yb){$n=error().$Ne;return
false;}if($oe)redirect($hd,$rd.$Ne);return
true;}function
queries($I=null){global$h;static$ie=array();if($I===null)return
implode("\n",$ie);$Pe=microtime(true);$K=$h->query($I);$ie[]=(preg_match('~;$~',$I)?"DELIMITER ;;\n$I;\nDELIMITER ":$I)."; -- ".format_time($Pe,microtime(true));return$K;}function
apply_queries($I,$T,$Pb='table'){foreach($T
as$R){if(!queries("$I ".$Pb($R)))return
false;}return
true;}function
queries_redirect($hd,$rd,$oe){return
query_redirect(queries(),$hd,$rd,$oe,false,!$oe);}function
format_time($Pe,$Kb){return
lang(1,max(0,$Kb-$Pe));}function
...
null;foreach($bc
as$y=>$X)$bc[$y]=(array)$X;$K='';foreach($bc["error"]as$y=>$n){if($n)return$n;$D=$bc["name"][$y];$of=$bc["tmp_name"][$y];$fb=file_get_contents($nb&&preg_match('~\\.gz$~',$D)?"compress.zlib://$of":$of);if($nb){$Pe=substr($fb,0,3);if(function_exists("iconv")&&preg_match("~^\xFE\xFF|^\xFF\xFE~",$Pe,$pe))$fb=iconv("utf-16","utf-8",$fb);elseif($Pe=="\xEF\xBB\xBF")$fb=substr($fb,3);$K.=$fb."\n\n";}}return$K;}function
upload_error($n){$od=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?lang(2).($od?" ".lang(3,$od):""):lang(4));}function
repeat_pattern($Ud,$ed){return
str_repeat("$Ud{0,65535}",$ed/65535)."$Ud{0,".($ed%65535)."}";}function
is_utf8($X){return(preg_match('~~u',$X)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$X));}function
shorten_utf8($Q,$ed=80,$Ve=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{FFFF}]",$ed).")($)?)u",$Q,$A))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$ed).")($)?)",$Q,$A);return
h($A[1]).$Ve.(isset($A[2])?"":"<i>...</i>");}function
friendly_url($X){return
preg_replace('~[^a-z0-9_]~i','-',$X);}function
hidden_fields($fe,$Gc=array()){while(list($y,$X)=each($fe)){if(is_array($X)){foreach($X
as$Sc=>$W)$fe[$y."[$Sc]"]=$W;}elseif(!in_array($y,$Gc))echo'<input type="hidden" name="'.h($y).'" value="'.h($X).'">';}}function
...
input($o,$Y,$q){global$h,$xf,$b,$x;$D=h(bracket_escape($o["field"]));echo"<td class='function'>";if(is_array($Y)&&!$q){$wa=array($Y);if(version_compare(PHP_VERSION,5.4)>=0)$wa[]=JSON_PRETTY_PRINT;$Y=call_user_func_array('json_encode',$wa);$q="json";}$te=($x=="mssql"&&$o["auto_increment"]);if($te&&!$_POST["save"])$q=null;$sc=(isset($_GET["select"])||$te?array("orig"=>lang(6)):array())+$b->editFunctions($o);$Ba=" name='fields[$D]'";if($o["type"]=="enum")echo
nbsp($sc[""])."<td>".$b->editInput($_GET["edit"],$o,$Ba,$Y);else{$fc=0;foreach($sc
as$y=>$X){if($y===""||!$X)break;$fc++;}$Ed=($fc?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($o["field"])))."]']; if ($fc > f.selectedIndex) f.selectedIndex = $fc;\" onkeyup='keyupChange.call(this);'":"");$Ba.=$Ed;echo(count($sc)>1?"<select name='function[$D]' onchange='functionChange(this);'".on_help("getTarget(event).value.replace(/^SQL\$/, '')",1).">".optionlist($sc,$q===null||in_array($q,$sc)||isset($sc[$q])?$q:"")."</select>":nbsp(reset($sc))).'<td>';$Oc=$b->editInput($_GET["edit"],$o,$Ba,$Y);if($Oc!="")echo$Oc;elseif($o["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$o["length"],$B);foreach($B[1]as$s=>$X){$X=stripcslashes(str_replace("''","'",$X));$Ra=(is_int($Y)?($Y>>$s)&1:in_array($X,explode(",",$Y),true));echo" <label><input type='checkbox' name='fields[$D][$s]' value='".(1<<$s)."'".($Ra?' checked':'')."$Ed>".h($b->editVal($X,$o)).'</label>';}}elseif(preg_match('~blob|bytea|raw|file~',$o["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$D'$Ed>";elseif(($ef=preg_match('~text|lob~',$o["type"]))||preg_match("~\n~",$Y)){if($ef&&$x!="sqlite")$Ba.=" cols='50' rows='12'";else{$M=min(12,substr_count($Y,"\n")+1);$Ba.=" cols='30' rows='$M'".($M==1?" style='height: 1.2em;'":"");}echo"<textarea$Ba>".h($Y).'</textarea>';}elseif($q=="json")echo"<textarea$Ba cols='50' rows='12' class='jush-js'>".h($Y).'</textarea>';else{$qd=(!preg_match('~int~',$o["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$o["length"],$A)?((preg_match("~binary~",$o["type"])?2:1)*$A[1]+($A[3]?1:0)+($A[2]&&!$o["unsigned"]?1:0)):($xf[$o["type"]]?$xf[$o["type"]]+($o["unsigned"]?0:1):0));if($x=='sql'&&$h->server_info>=5.6&&preg_match('~time~',$o["type"]))$qd+=7;echo"<input".(preg_match('~int~',$o["type"])?" type='number'":"")." value='".h($Y)."'".($qd?" maxlength='$qd'":"").(preg_match('~char|binary~',$o["type"])&&$qd>20?" size='40'":"")."$Ba>";}}}function
...
dump_headers($Ec,$wd=false){global$b;$K=$b->dumpHeaders($Ec,$wd);$Od=$_POST["output"];if($Od!="text")header("Content-Disposition: attachment; filename=".$b->dumpFilename($Ec).".$K".($Od!="file"&&!preg_match('~[^0-9a-z]~',$Od)?".$Od":""));session_write_close();ob_flush();flush();return$K;}function
...
select_value($X,$_,$o,$ff){global$b,$aa;if(is_array($X)){$K="";foreach($X
as$Sc=>$W)$K.="<tr>".($X!=array_values($X)?"<th>".h($Sc):"")."<td>".select_value($W,$_,$o,$ff);return"<table cellspacing='0'>$K</table>";}if(!$_)$_=$b->selectLink($X,$o);if($_===null){if(is_mail($X))$_="mailto:$X";if($ge=is_url($X))$_=($ge=="http"&&$aa?$X:"$ge://www.adminer.org/redirect/?url=".urlencode($X));}$X=$b->editVal($X,$o);if($X!==null){if($X==="")$X="&nbsp;";elseif($ff!=""&&is_shortable($o))$X=shorten_utf8($X,max(0,+$ff));else$X=h($X);}$X=$b->selectVal($X,$_,$o);return$X;}function
is_mail($Gb){$za='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$xb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$Ud="$za+(\\.$za+)*@($xb?\\.)+$xb";return
is_string($Gb)&&preg_match("(^$Ud(,\\s*$Ud)*\$)i",$Gb);}function
...
slow_query($I){global$b,$pf;$m=$b->database();$if=$b->queryTimeout();if(support("kill")&&is_object($i=connect())&&($m==""||$i->select_db($m))){$Xc=$i->result("SELECT CONNECTION_ID()");echo'<script type="text/javascript">
var timeout = setTimeout(function () {
	ajax(\'',js_escape(ME),'script=kill\', function () {
	}, \'token=',$pf,'&kill=',$Xc,'\');
}, ',1000*$if,');
</script>
';}else$i=null;ob_flush();flush();$K=@get_key_vals($I,$i,$if);if($i){echo"<script type='text/javascript'>clearTimeout(timeout);</script>\n";ob_flush();flush();}return
array_keys($K);}function
get_token(){$le=rand(1,1e6);return($le^$_SESSION["token"]).":$le";}function
verify_token(){list($pf,$le)=explode(":",$_POST["token"]);return($le^$_SESSION["token"])==$pf;}function
lzw_decompress($Ka){$ub=256;$La=8;$Wa=array();$ve=0;$we=0;for($s=0;$s<strlen($Ka);$s++){$ve=($ve<<8)+ord($Ka[$s]);$we+=8;if($we>=$La){$we-=$La;$Wa[]=$ve>>$we;$ve&=(1<<$we)-1;$ub++;if($ub>>$La)$La++;}}$tb=range("\0","\xFF");$K="";foreach($Wa
as$s=>$Va){$Fb=$tb[$Va];if(!isset($Fb))$Fb=$Sf.$Sf[0];$K.=$Fb;if($s)$tb[]=$Sf.$Fb[0];$Sf=$Fb;}return$K;}function
on_help($ab,$Ke=0){return" onmouseover='helpMouseover(this, event, ".h($ab).", $Ke);' onmouseout='helpMouseout(this, event);'";}global$b,$h,$zb,$Db,$Mb,$n,$sc,$uc,$aa,$Nc,$x,$ba,$ad,$Dd,$Vd,$Se,$xc,$pf,$sf,$xf,$Df,$ca;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$aa=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);session_cache_limiter("");if(!defined("SID")){session_name("adminer_sid");$G=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$aa);if(version_compare(PHP_VERSION,'5.2.0')>=0)$G[]=true;call_user_func_array('session_set_cookie_params',$G);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$ec);if(get_magic_quotes_runtime())set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);$ad=array('en'=>'English','ar'=>'Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©','bn'=>'à¦¬à¦¾à¦‚à¦²à¦¾','ca'=>'CatalÃ ','cs'=>'ÄŒeÅ¡tina','de'=>'Deutsch','es'=>'EspaÃ±ol','et'=>'Eesti','fa'=>'ÙØ§Ø±Ø³ÛŒ','fr'=>'FranÃ§ais','hu'=>'Magyar','id'=>'Bahasa Indonesia','it'=>'Italiano','ja'=>'æ—¥æœ¬èª','ko'=>'í•œêµ­ì–´','lt'=>'LietuviÅ³','nl'=>'Nederlands','pl'=>'Polski','pt'=>'PortuguÃªs','pt-br'=>'PortuguÃªs (Brazil)','ro'=>'Limba RomÃ¢nÄƒ','ru'=>'Ğ ÑƒÑÑĞºĞ¸Ğ¹ ÑĞ·Ñ‹Ğº','sk'=>'SlovenÄina','sl'=>'Slovenski','sr'=>'Ğ¡Ñ€Ğ¿ÑĞºĞ¸','ta'=>'à®¤â€Œà®®à®¿à®´à¯','th'=>'à¸ à¸²à¸©à¸²à¹„à¸—à¸¢','tr'=>'TÃ¼rkÃ§e','uk'=>'Ğ£ĞºÑ€Ğ°Ñ—Ğ½ÑÑŒĞºĞ°','zh'=>'ç®€ä½“ä¸­æ–‡','zh-tw'=>'ç¹é«”ä¸­æ–‡',);function
get_lang(){global$ba;return$ba;}function
lang($u,$Ad=null){if(is_string($u)){$Zd=array_search($u,get_translations("en"));if($Zd!==false)$u=$Zd;}global$ba,$sf;$rf=($sf[$u]?$sf[$u]:$u);if(is_array($rf)){$Zd=($Ad==1?0:($ba=='cs'||$ba=='sk'?($Ad&&$Ad<5?1:2):($ba=='fr'?(!$Ad?0:1):($ba=='pl'?($Ad%10>1&&$Ad%10<5&&$Ad/10%10!=1?1:2):($ba=='sl'?($Ad%100==1?0:($Ad%100==2?1:($Ad%100==3||$Ad%100==4?2:3))):($ba=='lt'?($Ad%10==1&&$Ad%100!=11?0:($Ad%10>1&&$Ad/10%10!=1?1:2)):($ba=='ru'||$ba=='sr'||$ba=='uk'?($Ad%10==1&&$Ad%100!=11?0:($Ad%10>1&&$Ad%10<5&&$Ad/10%10!=1?1:2)):1)))))));$rf=$rf[$Zd];}$wa=func_get_args();array_shift($wa);$mc=str_replace("%d","%s",$rf);if($mc!=$rf)$wa[0]=number_format($Ad,0,".",lang(8));return
...
query($I,$yf=false){$J=parent::query($I);$this->error="";if(!$J){list(,$this->errno,$this->error)=$this->errorInfo();return
...
select($R,$N,$Z,$r,$E,$z,$F){global$b,$x;$Qc=(count($r)<count($N));$I=$b->selectQueryBuild($N,$Z,$r,$E,$z,$F);if(!$I)$I="SELECT".limit(($_GET["page"]!="last"&&+$z&&$r&&$Qc&&$x=="sql"?"SQL_CALC_FOUND_ROWS ":"").implode(", ",$N)."\nFROM ".table($R),($Z?"\nWHERE ".implode(" AND ",$Z):"").($r&&$Qc?"\nGROUP BY ".implode(", ",$r):"").($E?"\nORDER BY ".implode(", ",$E):""),($z!=""?+$z:null),($F?$z*$F:0),"\n");echo$b->selectQuery($I);return$this->_conn->query($I);}function
delete($R,$je,$z=0){$I="FROM ".table($R);return
queries("DELETE".($z?limit1($I,$je):" $I$je"));}function
update($R,$P,$je,$z=0,$Ge="\n"){$Jf=array();foreach($P
as$y=>$X)$Jf[]="$y = $X";$I=table($R)." SET$Ge".implode(",$Ge",$Jf);return
queries("UPDATE".($z?limit1($I,$je):" $I$je"));}function
insert($R,$P){return
...
SQLite3($cc);$Lf=$this->_link->version();$this->server_info=$Lf["versionString"];}function
...
query($I,$yf=false){$ud=($yf?"unbufferedQuery":"query");$J=@$this->_link->$ud($I,SQLITE_BOTH,$n);$this->error="";if(!$J){$this->error=$n;return
...
fetch_field(){$D=$this->_result->fieldName($this->_offset++);$Ud='(\\[.*]|"(?:[^"]|"")*"|(.+))';if(preg_match("~^($Ud\\.)?$Ud\$~",$D,$A)){$R=($A[3]!=""?$A[3]:idf_unescape($A[2]));$D=($A[5]!=""?$A[5]:idf_unescape($A[4]));}return(object)array("name"=>$D,"orgname"=>$D,"orgtable"=>$R,);}}}}elseif(extension_loaded("pdo_sqlite")){class
Min_SQLite
extends
...
extends
Min_SQL{function
insertUpdate($R,$M,$ce){$Jf=array();foreach($M
as$P)$Jf[]="(".implode(", ",$P).")";return
...
array();}function
limit($I,$Z,$z,$Bd=0,$Ge=" "){return" $I$Z".($z!==null?$Ge."LIMIT $z".($Bd?" OFFSET $Bd":""):"");}function
...
fields($R){global$h;$K=array();$ce="";foreach(get_rows("PRAGMA table_info(".table($R).")")as$L){$D=$L["name"];$U=strtolower($L["type"]);$ob=$L["dflt_value"];$K[$D]=array("field"=>$D,"type"=>(preg_match('~int~i',$U)?"integer":(preg_match('~char|clob|text~i',$U)?"text":(preg_match('~blob~i',$U)?"blob":(preg_match('~real|floa|doub~i',$U)?"real":"numeric")))),"full_type"=>$U,"default"=>(preg_match("~'(.*)'~",$ob,$A)?str_replace("''","'",$A[1]):($ob=="NULL"?null:$ob)),"null"=>!$L["notnull"],"privileges"=>array("select"=>1,"insert"=>1,"update"=>1),"primary"=>$L["pk"],);if($L["pk"]){if($ce!="")$K[$ce]["auto_increment"]=false;elseif(preg_match('~^integer$~i',$U))$K[$D]["auto_increment"]=true;$ce=$D;}}$Ne=$h->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));preg_match_all('~(("[^"]*+")+|[a-z0-9_]+)\s+text\s+COLLATE\s+(\'[^\']+\'|\S+)~i',$Ne,$B,PREG_SET_ORDER);foreach($B
as$A){$D=str_replace('""','"',preg_replace('~^"|"$~','',$A[1]));if($K[$D])$K[$D]["collation"]=trim($A[3],"'");}return$K;}function
indexes($R,$i=null){global$h;if(!is_object($i))$i=$h;$K=array();$Ne=$i->result("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ".q($R));if(preg_match('~\bPRIMARY\s+KEY\s*\((([^)"]+|"[^"]*")++)~i',$Ne,$A)){$K[""]=array("type"=>"PRIMARY","columns"=>array(),"lengths"=>array(),"descs"=>array());preg_match_all('~((("[^"]*+")+)|(\S+))(\s+(ASC|DESC))?(,\s*|$)~i',$A[1],$B,PREG_SET_ORDER);foreach($B
as$A){$K[""]["columns"][]=idf_unescape($A[2]).$A[4];$K[""]["descs"][]=(preg_match('~DESC~i',$A[5])?'1':null);}}if(!$K){foreach(fields($R)as$D=>$o){if($o["primary"])$K[""]=array("type"=>"PRIMARY","columns"=>array($D),"lengths"=>array(),"descs"=>array(null));}}$Oe=get_key_vals("SELECT name, sql FROM sqlite_master WHERE type = 'index' AND tbl_name = ".q($R),$i);foreach(get_rows("PRAGMA index_list(".table($R).")",$i)as$L){$D=$L["name"];if(!preg_match("~^sqlite_~",$D)){$K[$D]["type"]=($L["unique"]?"UNIQUE":"INDEX");$K[$D]["lengths"]=array();foreach(get_rows("PRAGMA index_info(".idf_escape($D).")",$i)as$ye)$K[$D]["columns"][]=$ye["name"];$K[$D]["descs"]=array();if(preg_match('~^CREATE( UNIQUE)? INDEX '.preg_quote(idf_escape($D).' ON '.idf_escape($R),'~').' \((.*)\)$~i',$Oe[$D],$pe)){preg_match_all('/("[^"]*+")+( DESC)?/',$pe[2],$B);foreach($B[2]as$X)$K[$D]["descs"][]=($X?'1':null);}}}return$K;}function
...
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){$Gf=($R==""||$hc);foreach($p
as$o){if($o[0]!=""||!$o[1]||$o[2]){$Gf=true;break;}}$c=array();$Nd=array();foreach($p
as$o){if($o[1]){$c[]=($Gf?$o[1]:"ADD ".implode($o[1]));if($o[0]!="")$Nd[$o[0]]=$o[1][0];}}if(!$Gf){foreach($c
as$X){if(!queries("ALTER TABLE ".table($R)." $X"))return
false;}if($R!=$D&&!queries("ALTER TABLE ".table($R)." RENAME TO ".table($D)))return
false;}elseif(!recreate_table($R,$D,$c,$Nd,$hc))return
false;if($Da)queries("UPDATE sqlite_sequence SET seq = $Da WHERE name = ".q($D));return
true;}function
recreate_table($R,$D,$p,$Nd,$hc,$w=array()){queries("BEGIN");if($R!=""){if(!$p){foreach(fields($R)as$y=>$o){$p[]=process_field($o,$o);$Nd[$y]=idf_escape($y);}}$de=false;foreach($p
as$y=>$o){if($o[6])$de=true;$p[$y]="  ".implode($o);}$Ab=array();foreach($w
as$y=>$X){if($X[2]=="DROP"){$Ab[$X[1]]=true;unset($w[$y]);}}foreach(indexes($R)as$Vc=>$v){$f=array();foreach($v["columns"]as$y=>$e){if(!$Nd[$e])continue
2;$f[]=$Nd[$e].($v["descs"][$y]?" DESC":"");}$f="(".implode(", ",$f).")";if(!$Ab[$Vc]){if($v["type"]!="PRIMARY"||!$de)$w[]=array($v["type"],$Vc,$f);}}foreach($w
as$y=>$X){if($X[0]=="PRIMARY"){unset($w[$y]);$hc[]="  PRIMARY KEY $X[2]";}}foreach(foreign_keys($R)as$Vc=>$kc){foreach($kc["source"]as$y=>$e){if(!$Nd[$e])continue
2;$kc["source"][$y]=idf_unescape($Nd[$e]);}if(!isset($hc[" $Vc"]))$hc[]=" ".format_foreign_key($kc);}}$p=array_merge($p,array_filter($hc));if(!queries("CREATE TABLE ".table($R!=""?"adminer_$D":$D)." (\n".implode(",\n",$p)."\n)"))return
false;if($R!=""){if($Nd&&!queries("INSERT INTO ".table("adminer_$D")." (".implode(", ",$Nd).") SELECT ".implode(", ",array_map('idf_escape',array_keys($Nd)))." FROM ".table($R)))return
false;$vf=array();foreach(triggers($R)as$uf=>$jf){$tf=trigger($uf);$vf[]="CREATE TRIGGER ".idf_escape($uf)." ".implode(" ",$jf)." ON ".table($D)."\n$tf[Statement]";}if(!queries("DROP TABLE ".table($R)))return
false;queries("ALTER TABLE ".table("adminer_$D")." RENAME TO ".table($D));if(!alter_indexes($D,$w))return
false;foreach($vf
as$tf){if(!queries($tf))return
...
drop_views($Nf){return
apply_queries("DROP VIEW",$Nf);}function
drop_tables($T){return
apply_queries("DROP TABLE",$T);}function
move_tables($T,$Nf,$cf){return
false;}function
...
query($I,$yf=false){$J=@pg_query($this->_link,$I);$this->error="";if(!$J){$this->error=pg_last_error($this->_link);return
...
as$P){$Ef=array();$Z=array();foreach($P
as$y=>$X){$Ef[]="$y = $X";if(isset($ce[idf_unescape($y)]))$Z[]="$y = $X";}if(!(($Z&&queries("UPDATE ".table($R)." SET ".implode(", ",$Ef)." WHERE ".implode(" AND ",$Z))&&$h->affected_rows)||queries("INSERT INTO ".table($R)." (".implode(", ",array_keys($P)).") VALUES (".implode(", ",$P).")")))return
false;}return
true;}}function
idf_escape($u){return'"'.str_replace('"','""',$u).'"';}function
table($u){return
...
indexes($R,$i=null){global$h;if(!is_object($i))$i=$h;$K=array();$af=$i->result("SELECT oid FROM pg_class WHERE relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = current_schema()) AND relname = ".q($R));$f=get_key_vals("SELECT attnum, attname FROM pg_attribute WHERE attrelid = $af AND attnum > 0",$i);foreach(get_rows("SELECT relname, indisunique::int, indisprimary::int, indkey, indoption FROM pg_index i, pg_class ci WHERE i.indrelid = $af AND ci.oid = i.indexrelid",$i)as$L){$qe=$L["relname"];$K[$qe]["type"]=($L["indisprimary"]?"PRIMARY":($L["indisunique"]?"UNIQUE":"INDEX"));$K[$qe]["columns"]=array();foreach(explode(" ",$L["indkey"])as$Jc)$K[$qe]["columns"][]=$f[$Jc];$K[$qe]["descs"]=array();foreach(explode(" ",$L["indoption"])as$Kc)$K[$qe]["descs"][]=($Kc&1?'1':null);$K[$qe]["lengths"]=array();}return$K;}function
...
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){$c=array();$ie=array();foreach($p
as$o){$e=idf_escape($o[0]);$X=$o[1];if(!$X)$c[]="DROP $e";else{$If=$X[5];unset($X[5]);if(isset($X[6])&&$o[0]=="")$X[1]=($X[1]=="bigint"?" big":" ")."serial";if($o[0]=="")$c[]=($R!=""?"ADD ":"  ").implode($X);else{if($e!=$X[0])$ie[]="ALTER TABLE ".table($R)." RENAME $e TO $X[0]";$c[]="ALTER $e TYPE$X[1]";if(!$X[6]){$c[]="ALTER $e ".($X[3]?"SET$X[3]":"DROP DEFAULT");$c[]="ALTER $e ".($X[2]==" NULL"?"DROP NOT":"SET").$X[2];}}if($o[0]!=""||$If!="")$ie[]="COMMENT ON COLUMN ".table($R).".$X[0] IS ".($If!=""?substr($If,9):"''");}}$c=array_merge($c,$hc);if($R=="")array_unshift($ie,"CREATE TABLE ".table($D)." (\n".implode(",\n",$c)."\n)");elseif($c)array_unshift($ie,"ALTER TABLE ".table($R)."\n".implode(",\n",$c));if($R!=""&&$R!=$D)$ie[]="ALTER TABLE ".table($R)." RENAME TO ".table($D);if($R!=""||$bb!="")$ie[]="COMMENT ON TABLE ".table($D)." IS ".q($bb);if($Da!=""){}foreach($ie
as$I){if(!queries($I))return
false;}return
true;}function
alter_indexes($R,$c){$ib=array();$_b=array();$ie=array();foreach($c
as$X){if($X[0]!="INDEX")$ib[]=($X[2]=="DROP"?"\nDROP CONSTRAINT ".idf_escape($X[1]):"\nADD".($X[1]!=""?" CONSTRAINT ".idf_escape($X[1]):"")." $X[0] ".($X[0]=="PRIMARY"?"KEY ":"").$X[2]);elseif($X[2]=="DROP")$_b[]=idf_escape($X[1]);else$ie[]="CREATE INDEX ".idf_escape($X[1]!=""?$X[1]:uniqid($R."_"))." ON ".table($R)." $X[2]";}if($ib)array_unshift($ie,"ALTER TABLE ".table($R).implode(",",$ib));if($_b)array_unshift($ie,"DROP INDEX ".implode(", ",$_b));foreach($ie
as$I){if(!queries($I))return
false;}return
true;}function
truncate_tables($T){return
...
true;}function
drop_views($Nf){return
queries("DROP VIEW ".implode(", ",array_map('table',$Nf)));}function
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$Nf,$cf){foreach($T
as$R){if(!queries("ALTER TABLE ".table($R)." SET SCHEMA ".idf_escape($cf)))return
false;}foreach($Nf
as$R){if(!queries("ALTER VIEW ".table($R)." SET SCHEMA ".idf_escape($cf)))return
false;}return
true;}function
...
0;}function
explain($h,$I){return$h->query("EXPLAIN $I");}function
found_rows($S,$Z){global$h;if(preg_match("~ rows=([0-9]+)~",$h->result("EXPLAIN SELECT * FROM ".idf_escape($S["Name"]).($Z?" WHERE ".implode(" AND ",$Z):"")),$pe))return$pe[1];return
false;}function
types(){return
...
set_schema($_e){global$h,$xf,$Se;$K=$h->query("SET search_path TO ".idf_escape($_e));foreach(types()as$U){if(!isset($xf[$U])){$xf[$U]=0;$Se[lang(13)][]=$U;}}return$K;}function
...
select_db($k){return
true;}function
query($I,$yf=false){$J=oci_parse($this->_link,$I);$this->error="";if(!$J){$n=oci_error($this->_link);$this->errno=$n["code"];$this->error=$n["message"];return
...
limit($I,$Z,$z,$Bd=0,$Ge=" "){return($Bd?" * FROM (SELECT t.*, rownum AS rnum FROM (SELECT $I$Z) t WHERE rownum <= ".($z+$Bd).") WHERE rnum > $Bd":($z!==null?" * FROM (SELECT $I$Z) WHERE rownum <= ".($z+$Bd):" $I$Z"));}function
limit1($I,$Z){return" $I$Z";}function
...
count_tables($l){return
array();}function
table_status($D=""){$K=array();$Be=q($D);foreach(get_rows('SELECT table_name "Name", \'table\' "Engine", avg_row_len * num_rows "Data_length", num_rows "Rows" FROM all_tables WHERE tablespace_name = '.q(DB).($D!=""?" AND table_name = $Be":"")."
UNION SELECT view_name, 'view', 0, 0 FROM user_views".($D!=""?" WHERE view_name = $Be":"")."
...
found_rows($S,$Z){}function
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){$c=$_b=array();foreach($p
...
set_schema($Ae){global$h;return$h->query("ALTER SESSION SET CURRENT_SCHEMA = ".idf_escape($Ae));}function
show_variables(){return
...
unconvert_field($o,$K){return$K;}function
support($ac){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$ac);}$x="oracle";$xf=array();$Se=array();foreach(array(lang(14)=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),lang(15)=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),lang(16)=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),lang(17)=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$xf+=$X;$Se[$y]=array_keys($X);}$Df=array();$Id=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$sc=array("length","lower","round","upper");$uc=array("avg","count","count distinct","max","min","sum");$Db=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$zb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ae=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
query($I,$yf=false){$J=sqlsrv_query($this->_link,$I);$this->error="";if(!$J){$this->_get_error();return
...
query($I,$yf=false){$J=mssql_query($I,$this->_link);$this->error="";if(!$J){$this->error=mssql_get_last_message();return
...
as$P){$Ef=array();$Z=array();foreach($P
as$y=>$X){$Ef[]="$y = $X";if(isset($ce[idf_unescape($y)]))$Z[]="$y = $X";}if(!queries("MERGE ".table($R)." USING (VALUES(".implode(", ",$P).")) AS source (c".implode(", c",range(1,count($P))).") ON ".implode(" AND ",$Z)." WHEN MATCHED THEN UPDATE SET ".implode(", ",$Ef)." WHEN NOT MATCHED THEN INSERT (".implode(", ",array_keys($P)).") VALUES (".implode(", ",$P).");"))return
false;}return
true;}function
begin(){return
...
limit($I,$Z,$z,$Bd=0,$Ge=" "){return($z!==null?" TOP (".($z+$Bd).")":"")." $I$Z";}function
limit1($I,$Z){return
...
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){$c=array();foreach($p
...
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$Nf,$cf){return
apply_queries("ALTER SCHEMA ".idf_escape($cf)." TRANSFER",array_merge($T,$Nf));}function
trigger($D){if($D=="")return
...
extends
Min_SQL{function
_chunkRequest($Fc,$qa,$G,$Ub=array()){global$h;foreach(array_chunk($Fc,25)as$Sa){$Rd=$G;foreach($Sa
as$s=>$t){$Rd["Item.$s.ItemName"]=$t;foreach($Ub
as$y=>$X)$Rd["Item.$s.$y"]=$X;}if(!sdb_request($qa,$Rd))return
false;}$h->affected_rows=count($Fc);return
true;}function
_extractIds($R,$je,$z){$K=array();if(preg_match_all("~itemName\(\) = (('[^']*+')+)~",$je,$B))$K=array_map('idf_unescape',$B[1]);else{foreach(sdb_request_all('Select','Item',array('SelectExpression'=>'SELECT itemName() FROM '.table($R).$je.($z?" LIMIT 1":"")))as$Rc)$K[]=$Rc->Name;}return$K;}function
select($R,$N,$Z,$r,$E,$z,$F){global$h;$h->next=$_GET["next"];$K=parent::select($R,$N,$Z,$r,$E,$z,$F);$h->next=0;return$K;}function
delete($R,$je,$z=0){return$this->_chunkRequest($this->_extractIds($R,$je,$z),'BatchDeleteAttributes',array('DomainName'=>$R));}function
update($R,$P,$je,$z=0,$Ge="\n"){$pb=array();$Pc=array();$s=0;$Fc=$this->_extractIds($R,$je,$z);$t=idf_unescape($P["`itemName()`"]);unset($P["`itemName()`"]);foreach($P
...
idf_escape($u);}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
limit($I,$Z,$z,$Bd=0,$Ge=" "){return" $I$Z".($z!==null?$Ge."LIMIT $z":"");}function
unconvert_field($o,$K){return$K;}function
fk_support($S){}function
engines(){return
array();}function
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){return($R==""&&sdb_request('CreateDomain',array('DomainName'=>$D)));}function
...
last_id(){}function
hmac($ua,$lb,$y,$ne=false){$Ma=64;if(strlen($y)>$Ma)$y=pack("H*",$ua($y));$y=str_pad($y,$Ma,"\0");$Tc=$y^str_repeat("\x36",$Ma);$Uc=$y^str_repeat("\x5C",$Ma);$K=$ua($Uc.pack("H*",$ua($Tc.$lb)));if($ne)$K=pack("H*",$K);return$K;}function
sdb_request($qa,$G=array()){global$b,$h;list($Bc,$G['AWSAccessKeyId'],$Ce)=$b->credentials();$G['Action']=$qa;$G['Timestamp']=gmdate('Y-m-d\TH:i:s+00:00');$G['Version']='2009-04-15';$G['SignatureVersion']=2;$G['SignatureMethod']='HmacSHA1';ksort($G);$I='';foreach($G
as$y=>$X)$I.='&'.rawurlencode($y).'='.rawurlencode($X);$I=str_replace('%7E','~',substr($I,1));$I.="&Signature=".urlencode(base64_encode(hmac('sha1',"POST\n".preg_replace('~^https?://~','',$Bc)."\n/\n$I",$Ce,true)));@ini_set('track_errors',1);$bc=@file_get_contents((preg_match('~^https?://~',$Bc)?$Bc:"http://$Bc"),false,stream_context_create(array('http'=>array('method'=>'POST','content'=>$I,'ignore_errors'=>1,))));if(!$bc){$h->error=$Wd;return
false;}libxml_use_internal_errors(true);$Tf=simplexml_load_string($bc);if(!$Tf){$n=libxml_get_last_error();$h->error=$n->message;return
false;}if($Tf->Errors){$n=$Tf->Errors->Error;$h->error="$n->Message ($n->Code)";return
false;}$h->error='';$bf=$qa."Result";return($Tf->$bf?$Tf->$bf:true);}function
sdb_request_all($qa,$bf,$G=array(),$if=0){$K=array();$Pe=($if?microtime(true):0);$z=(preg_match('~LIMIT\s+(\d+)\s*$~i',$G['SelectExpression'],$A)?$A[1]:0);do{$Tf=sdb_request($qa,$G);if(!$Tf)break;foreach($Tf->$bf
as$Fb)$K[]=$Fb;if($z&&count($K)>=$z){$_GET["next"]=$Tf->NextToken;break;}if($if&&microtime(true)-$Pe>$if)return
false;$G['NextToken']=$Tf->NextToken;if($z)$G['SelectExpression']=preg_replace('~\d+\s*$~',$z-count($K),$G['SelectExpression']);}while($Tf->NextToken);return$K;}$x="simpledb";$Id=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","IS NOT NULL");$sc=array();$uc=array("count");$Db=array(array("json"));}$zb["mongo"]="MongoDB (beta)";if(isset($_GET["mongo"])){$ae=array("mongo");define("DRIVER","mongo");if(class_exists('MongoDB')){class
...
as$y=>$X){if(is_a($X,'MongoBinData'))$this->_charset[$y]=63;$L[$y]=(is_a($X,'MongoDate')?gmdate("Y-m-d H:i:s",$X->sec)." GMT":(is_a($X,'MongoBinData')?$X->bin:(is_a($X,'MongoRegex')?strval($X):(is_object($X)?get_class($X):$X))));}$this->_rows[]=$L;foreach($L
as$y=>$X){if(!isset($this->_rows[0][$y]))$this->_rows[0][$y]=null;}}$this->num_rows=count($this->_rows);}function
fetch_assoc(){$L=current($this->_rows);if(!$L)return$L;$K=array();foreach($this->_rows[0]as$y=>$X)$K[$y]=$L[$y];next($this->_rows);return$K;}function
fetch_row(){$K=$this->fetch_assoc();if(!$K)return$K;return
array_values($K);}function
...
extends
Min_SQL{function
select($R,$N,$Z,$r,$E,$z,$F){global$h;if($N==array("*"))$N=array();else$N=array_fill_keys($N,true);$K=array();foreach($h->_db->selectCollection($R)->find(array(),$N)as$X)$K[]=$X;return
new
...
as$m){$ue=$h->_link->selectDB($m)->drop();if(!$ue['ok'])return
false;}return
true;}function
...
null;}function
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){global$h;if($R==""){$h->_db->createCollection($D);return
true;}}function
drop_tables($T){global$h;foreach($T
as$R){$ue=$h->_db->selectCollection($R)->drop();if(!$ue['ok'])return
false;}return
true;}function
truncate_tables($T){global$h;foreach($T
as$R){$ue=$h->_db->selectCollection($R)->remove();if(!$ue['ok'])return
false;}return
true;}function
table($u){return$u;}function
idf_escape($u){return$u;}function
support($ac){return
preg_match("~database|table|indexes~",$ac);}$x="mongo";$Id=array("=");$sc=array();$uc=array();$Db=array(array("json"));}$zb["elastic"]="Elasticsearch (beta)";if(isset($_GET["elastic"])){$ae=array("json");define("DRIVER","elastic");if(function_exists('json_decode')){class
Min_DB{var$extension="JSON",$server_info,$errno,$error,$_url;function
query($Td,$fb=array()){@ini_set('track_errors',1);$bc=@file_get_contents($this->_url.($this->_db!=""?"$this->_db/":"").$Td,false,stream_context_create(array('http'=>array('content'=>json_encode($fb),'ignore_errors'=>1,))));if(!$bc){$this->error=$Wd;return$bc;}if(!preg_match('~^HTTP/[0-9.]+ 2~i',$Dc[0])){$this->error=$bc;return
...
extends
Min_SQL{function
select($R,$N,$Z,$r,$E,$z,$F){global$b;$I=$b->selectQueryBuild($N,$Z,$r,$E,$z,$F);$lb=array();if(!$I){$I="$R/_search";if($N!=array("*"))$lb["fields"]=$N;if($E){$Le=array();foreach($E
as$Xa){$Xa=preg_replace('~ DESC$~','',$Xa,1,$hb);$Le[]=($hb?array($Xa=>"desc"):$Xa);}$lb["sort"]=$Le;}if($z){$lb["size"]=+$z;if($F)$lb["from"]=($F*$z);}foreach((array)$_GET["where"]as$X){if("$X[col]$X[val]"!=""){$df=array("match"=>array(($X["col"]!=""?$X["col"]:"_all")=>$X["val"]));if($X["op"]=="=")$lb["query"]["filtered"]["filter"]["and"][]=$df;else$lb["query"]["filtered"]["query"]["bool"]["must"][]=$df;}}if($lb["query"]&&!$lb["query"]["filtered"]["query"])$lb["query"]["filtered"]["query"]=array("match_all"=>array());}echo$b->selectQuery($I);$Be=$this->_conn->query($I,$lb);if(!$Be)return
false;$K=array();foreach($Be['hits']['hits']as$Ac){$L=array();$p=$Ac['_source'];if($N!=array("*")){$p=array();foreach($N
as$y)$p[$y]=$Ac['fields'][$y];}foreach($p
...
fk_support($S){}function
found_rows($S,$Z){return
null;}$x="elastic";$Id=array("=","query");$sc=array();$uc=array();$Db=array(array("json"));}$zb=array("server"=>"MySQL")+$zb;if(!defined("DRIVER")){$ae=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
query($I,$yf=false){$J=@($yf?mysql_unbuffered_query($I,$this->_link):mysql_query($I,$this->_link));$this->error="";if(!$J){$this->errno=mysql_errno($this->_link);$this->error=mysql_error($this->_link);return
...
true;}function
select_db($k){return$this->query("USE ".idf_escape($k));}function
query($I,$yf=false){$this->setAttribute(1000,!$yf);return
parent::query($I,$yf);}}}class
Min_Driver
extends
Min_SQL{function
insert($R,$P){return($P?parent::insert($R,$P):queries("INSERT INTO ".table($R)." ()\nVALUES ()"));}function
insertUpdate($R,$M,$ce){$f=array_keys(reset($M));$be="INSERT INTO ".table($R)." (".implode(", ",$f).") VALUES\n";$Jf=array();foreach($f
as$y)$Jf[$y]="$y = VALUES($y)";$Ve="\nON DUPLICATE KEY UPDATE ".implode(", ",$Jf);$Jf=array();$ed=0;foreach($M
as$P){$Y="(".implode(", ",$P).")";if($Jf&&(strlen($be)+$ed+strlen($Y)+strlen($Ve)>1e6)){if(!queries($be.implode(",\n",$Jf).$Ve))return
false;$Jf=array();$ed=0;}$Jf[]=$Y;$ed+=strlen($Y)+2;}return
queries($be.implode(",\n",$Jf).$Ve);}}function
idf_escape($u){return"`".str_replace("`","``",$u)."`";}function
table($u){return
idf_escape($u);}function
connect(){global$b;$h=new
Min_DB;$j=$b->credentials();if($h->connect($j[0],$j[1],$j[2])){$h->query("SET sql_quote_show_create = 1, autocommit = 1");return$h;}$K=$h->error;if(function_exists('iconv')&&!is_utf8($K)&&strlen($ze=iconv("windows-1250","utf-8",$K))>strlen($K))$K=$ze;return$K;}function
...
foreign_keys($R){global$h,$Dd;static$Ud='`(?:[^`]|``)+`';$K=array();$jb=$h->result("SHOW CREATE TABLE ".table($R),1);if($jb){preg_match_all("~CONSTRAINT ($Ud) FOREIGN KEY \\(((?:$Ud,? ?)+)\\) REFERENCES ($Ud)(?:\\.($Ud))? \\(((?:$Ud,? ?)+)\\)(?: ON DELETE ($Dd))?(?: ON UPDATE ($Dd))?~",$jb,$B,PREG_SET_ORDER);foreach($B
as$A){preg_match_all("~$Ud~",$A[2],$Me);preg_match_all("~$Ud~",$A[5],$cf);$K[idf_unescape($A[1])]=array("db"=>idf_unescape($A[4]!=""?$A[3]:$A[4]),"table"=>idf_unescape($A[4]!=""?$A[4]:$A[3]),"source"=>array_map('idf_unescape',$Me[0]),"target"=>array_map('idf_unescape',$cf[0]),"on_delete"=>($A[6]?$A[6]:"RESTRICT"),"on_update"=>($A[7]?$A[7]:"RESTRICT"),);}}return$K;}function
view($D){global$h;return
...
error_line(){global$h;if(preg_match('~ at line ([0-9]+)$~',$h->error,$pe))return$pe[1]-1;}function
create_database($m,$d){set_session("dbs",null);return
queries("CREATE DATABASE ".idf_escape($m).($d?" COLLATE ".q($d):""));}function
drop_databases($l){restart_session();set_session("dbs",null);return
apply_queries("DROP DATABASE",$l,'idf_escape');}function
rename_database($D,$d){if(create_database($D,$d)){$re=array();foreach(tables_list()as$R=>$U)$re[]=table($R)." TO ".idf_escape($D).".".table($R);if(!$re||queries("RENAME TABLE ".implode(", ",$re))){queries("DROP DATABASE ".idf_escape(DB));return
true;}}return
false;}function
...
alter_table($R,$D,$p,$hc,$bb,$Lb,$d,$Da,$Sd){$c=array();foreach($p
as$o)$c[]=($o[1]?($R!=""?($o[0]!=""?"CHANGE ".idf_escape($o[0]):"ADD"):" ")." ".implode($o[1]).($R!=""?$o[2]:""):"DROP ".idf_escape($o[0]));$c=array_merge($c,$hc);$Qe="COMMENT=".q($bb).($Lb?" ENGINE=".q($Lb):"").($d?" COLLATE ".q($d):"").($Da!=""?" AUTO_INCREMENT=$Da":"").$Sd;if($R=="")return
queries("CREATE TABLE ".table($D)." (\n".implode(",\n",$c)."\n) $Qe");if($R!=$D)$c[]="RENAME TO ".table($D);$c[]=$Qe;return
...
drop_tables($T){return
queries("DROP TABLE ".implode(", ",array_map('table',$T)));}function
move_tables($T,$Nf,$cf){$re=array();foreach(array_merge($T,$Nf)as$R)$re[]=table($R)." TO ".idf_escape($cf).".".table($R);return
queries("RENAME TABLE ".implode(", ",$re));}function
copy_tables($T,$Nf,$cf){queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");foreach($T
as$R){$D=($cf==DB?table("copy_$R"):idf_escape($cf).".".table($R));if(!queries("DROP TABLE IF EXISTS $D")||!queries("CREATE TABLE $D LIKE ".table($R))||!queries("INSERT INTO $D SELECT * FROM ".table($R)))return
false;}foreach($Nf
as$R){$D=($cf==DB?table("copy_$R"):idf_escape($cf).".".table($R));$Mf=view($R);if(!queries("DROP VIEW IF EXISTS $D")||!queries("CREATE VIEW $D AS $Mf[select]"))return
false;}return
true;}function
trigger($D){if($D=="")return
...
routine($D,$U){global$h,$Mb,$Nc,$xf;$va=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$wf="((".implode("|",array_merge(array_keys($xf),$va)).")\\b(?:\\s*\\(((?:[^'\")]*|$Mb)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$Ud="\\s*(".($U=="FUNCTION"?"":$Nc).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$wf";$ib=$h->result("SHOW CREATE $U ".idf_escape($D),2);preg_match("~\\(((?:$Ud\\s*,?)*)\\)\\s*".($U=="FUNCTION"?"RETURNS\\s+$wf\\s+":"")."(.*)~is",$ib,$A);$p=array();preg_match_all("~$Ud\\s*,?~is",$A[1],$B,PREG_SET_ORDER);foreach($B
as$Qd){$D=str_replace("``","`",$Qd[2]).$Qd[3];$p[]=array("field"=>$D,"type"=>strtolower($Qd[5]),"length"=>preg_replace_callback("~$Mb~s",'normalize_enum',$Qd[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$Qd[8] $Qd[7]"))),"null"=>1,"full_type"=>$Qd[4],"inout"=>strtoupper($Qd[1]),"collation"=>strtolower($Qd[9]),);}if($U!="FUNCTION")return
...
use_sql($k){return"USE ".idf_escape($k);}function
trigger_sql($R,$Te){$K="";foreach(get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($R,"%_\\")),null,"-- ")as$L)$K.="\n".($Te=='CREATE+ALTER'?"DROP TRIGGER IF EXISTS ".idf_escape($L["Trigger"]).";;\n":"")."CREATE TRIGGER ".idf_escape($L["Trigger"])." $L[Timing] $L[Event] ON ".table($L["Table"])." FOR EACH ROW\n$L[Statement];;\n";return$K;}function
show_variables(){return
...
true;}function
tableName($Ye){return
h($Ye["Comment"]!=""?$Ye["Comment"]:$Ye["Name"]);}function
fieldName($o,$E=0){return
h($o["comment"]!=""?$o["comment"]:$o["field"]);}function
selectLinks($Ye,$P=""){$a=$Ye["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.lang(26)."</a>\n";}function
foreignKeys($R){return
foreign_keys($R);}function
backwardKeys($R,$Xe){$K=array();foreach(get_rows("SELECT TABLE_NAME, CONSTRAINT_NAME, COLUMN_NAME, REFERENCED_COLUMN_NAME
...
as$y=>$X){$D=$this->tableName(table_status($y,true));if($D!=""){$Be=preg_quote($Xe);$Ge="(:|\\s*-)?\\s+";$K[$y]["name"]=(preg_match("(^$Be$Ge(.+)|^(.+?)$Ge$Be\$)iu",$D,$A)?$A[2].$A[3]:$D);}else
...
selectVal($X,$_,$o){$K=($X===null?"&nbsp;":$X);$_=h($_);if(preg_match('~blob|bytea~',$o["type"])&&!is_utf8($X)){$K=lang(27,strlen($X));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$X))$K="<img src='$_' alt='$K'>";}if(like_bool($o)&&$K!="&nbsp;")$K=($X?lang(28):lang(29));if($_)$K="<a href='$_'>$K</a>";if(!$_&&!like_bool($o)&&preg_match('~int|float|double|decimal~',$o["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$o["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchProcess($p,$w){$K=array();foreach((array)$_GET["where"]as$y=>$Z){$Xa=$Z["col"];$Gd=$Z["op"];$X=$Z["val"];if(($y<0?"":$Xa).$X!=""){$cb=array();foreach(($Xa!=""?array($Xa=>$p[$Xa]):$p)as$D=>$o){if($Xa!=""||is_numeric($X)||!preg_match('~int|float|double|decimal~',$o["type"])){$D=idf_escape($D);if($Xa!=""&&$o["type"]=="enum")$cb[]=(in_array(0,$X)?"$D IS NULL OR ":"")."$D IN (".implode(", ",array_map('intval',$X)).")";else{$gf=preg_match('~char|text|enum|set~',$o["type"]);$Y=$this->processInput($o,(!$Gd&&$gf&&preg_match('~^[^%]+$~',$X)?"%$X%":$X));$cb[]=$D.($Y=="NULL"?" IS".($Gd==">="?" NOT":"")." $Y":(in_array($Gd,$this->operators)||$Gd=="="?" $Gd $Y":($gf?" LIKE $Y":" IN (".str_replace(",","', '",$Y).")")));if($y<0&&$X=="0")$cb[]="$D IS NULL";}}}$K[]=($cb?"(".implode(" OR ",$cb).")":"0");}}return$K;}function
...
true;if($_POST["email"]){$Fe=0;if($_POST["all"]||$_POST["check"]){$o=idf_escape($_POST["email_field"]);$Ue=$_POST["email_subject"];$rd=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$Ue.$rd",$B);$M=get_rows("SELECT DISTINCT $o".($B[1]?", ".implode(", ",array_map('idf_escape',array_unique($B[1]))):"")." FROM ".table($_GET["select"])." WHERE $o IS NOT NULL AND $o != ''".($Z?" AND ".implode(" AND ",$Z):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$p=fields($_GET["select"]);foreach($this->rowDescriptions($M,$jc)as$L){$se=array('{\\'=>'{');foreach($B[1]as$X)$se['{$'."$X}"]=$this->editVal($L[$X],$p[$X]);$Gb=$L[$_POST["email_field"]];if(is_mail($Gb)&&send_mail($Gb,strtr($Ue,$se),strtr($rd,$se),$_POST["email_from"],$_FILES["email_files"]))$Fe++;}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(43,$Fe));}return
false;}function
...
dumpData($R,$Te,$I){global$h;$J=$h->query($I,1);if($J){while($L=$J->fetch_assoc()){if($Te=="table"){dump_csv(array_keys($L));$Te="INSERT";}dump_csv($L);}}}function
...
';if($vd=="auth"){$fc=true;foreach((array)$_SESSION["pwds"]as$Kf=>$Ie){foreach($Ie[""]as$V=>$H){if($H!==null){if($fc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$fc=false;}echo"<a href='".h(auth_url($Kf,"",$V))."'>".($V!=""?h($V):"<i>".lang(5)."</i>")."</a><br>\n";}}}}else{$this->databasesPrint($vd);if($vd!="db"&&$vd!="ns"){$S=table_status('',true);if(!$S)echo"<p class='message'>".lang(7)."\n";else$this->tablesPrint($S);}}}function
...
_foreignKeyOptions($R,$e,$Y=null){global$h;if(list($cf,$t,$D)=$this->_foreignColumn(column_foreign_keys($R),$e)){$K=&$this->_values[$cf];if($K===null){$S=table_status($cf);$K=($S["Rows"]>1000?"":array(""=>"")+get_key_vals("SELECT $t, $D FROM ".table($cf)." ORDER BY 2"));}if(!$K&&$Y!==null)return$h->result("SELECT $D FROM ".table($cf)." WHERE $t = ".q($Y));return$K;}}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);function
page_header($kf,$n="",$Pa=array(),$lf=""){global$ba,$ca,$b,$h,$zb,$x;page_headers();$mf=$kf.($lf!=""?": $lf":"");$nf=strip_tags($mf.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE html>
...
as$y=>$X){$qb=(is_array($X)?$X[1]:h($X));if($qb!="")echo"<a href='".h(ME."$y=").urlencode(is_array($X)?$X[0]:$X)."'>$qb</a> &raquo; ";}}echo"$kf\n";}}echo"<h2>$mf</h2>\n";restart_session();page_messages($n);$l=&get_session("dbs");if(DB!=""&&$l&&!in_array(DB,$l,true))$l=null;stop_session();define("PAGE_HEADER",1);}function
page_headers(){global$b;header("Content-Type: text/html; charset=utf-8");header("Cache-Control: no-cache");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}}function
page_messages($n){$Ff=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$sd=$_SESSION["messages"][$Ff];if($sd){echo"<div class='message'>".implode("</div>\n<div class='message'>",$sd)."</div>\n";unset($_SESSION["messages"][$Ff]);}if($n)echo"<div class='error'>$n</div>\n";}function
page_footer($vd=""){global$b,$pf;echo'</div>

';switch_lang();if($vd!="auth"){echo'<form action="" method="post">
<p class="logout">
<input type="submit" name="logout" value="',lang(50),'" id="logout">
<input type="hidden" name="token" value="',$pf,'">
</p>
</form>
';}echo'<div id="menu">
...
long2str($W,$Pf){$ze='';foreach($W
as$X)$ze.=pack('V',$X);if($Pf)return
substr($ze,0,end($W));return$ze;}function
str2long($ze,$Pf){$W=array_values(unpack('V*',str_pad($ze,4*ceil(strlen($ze)/4),"\0")));if($Pf)$W[]=strlen($ze);return$W;}function
xxtea_mx($Vf,$Uf,$We,$Sc){return
int32((($Vf>>5&0x7FFFFFF)^$Uf<<2)+(($Uf>>3&0x1FFFFFFF)^$Vf<<4))^int32(($We^$Uf)+($Sc^$Vf));}function
encrypt_string($Re,$y){if($Re=="")return"";$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($Re,true);$C=count($W)-1;$Vf=$W[$C];$Uf=$W[0];$he=floor(6+52/($C+1));$We=0;while($he-->0){$We=int32($We+0x9E3779B9);$Cb=$We>>2&3;for($Pd=0;$Pd<$C;$Pd++){$Uf=$W[$Pd+1];$xd=xxtea_mx($Vf,$Uf,$We,$y[$Pd&3^$Cb]);$Vf=int32($W[$Pd]+$xd);$W[$Pd]=$Vf;}$Uf=$W[0];$xd=xxtea_mx($Vf,$Uf,$We,$y[$Pd&3^$Cb]);$Vf=int32($W[$C]+$xd);$W[$C]=$Vf;}return
long2str($W,false);}function
decrypt_string($Re,$y){if($Re=="")return"";if(!$y)return
false;$y=array_values(unpack("V*",pack("H*",md5($y))));$W=str2long($Re,false);$C=count($W)-1;$Vf=$W[$C];$Uf=$W[0];$he=floor(6+52/($C+1));$We=int32($he*0x9E3779B9);while($We){$Cb=$We>>2&3;for($Pd=$C;$Pd>0;$Pd--){$Vf=$W[$Pd-1];$xd=xxtea_mx($Vf,$Uf,$We,$y[$Pd&3^$Cb]);$Uf=int32($W[$Pd]-$xd);$W[$Pd]=$Uf;}$Vf=$W[$C];$xd=xxtea_mx($Vf,$Uf,$We,$y[$Pd&3^$Cb]);$Uf=int32($W[0]-$xd);$W[0]=$Uf;$We=int32($We-0x9E3779B9);}return
long2str($W,true);}$h='';$xc=$_SESSION["token"];if(!$xc)$_SESSION["token"]=rand(1,1e6);$pf=get_token();$Vd=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($y)=explode(":",$X);$Vd[$y]=$X;}}$Ca=$_POST["auth"];if($Ca){session_regenerate_id();$yb=$Ca["driver"];$O=$Ca["server"];$V=$Ca["username"];$H=$Ca["password"];$m=$Ca["db"];set_password($yb,$O,$V,$H);$_SESSION["db"][$yb][$O][$V][$m]=true;if($Ca["permanent"]){$y=base64_encode($yb)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($m);$ee=$b->permanentLogin(true);$Vd[$y]="$y:".base64_encode($ee?encrypt_string($H,$ee):"");cookie("adminer_permanent",implode(" ",$Vd));}if(count($_POST)==1||DRIVER!=$yb||SERVER!=$O||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($yb,$O,$V,$m));}elseif($_POST["logout"]){if($xc&&!verify_token()){page_header(lang(50),lang(51));page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),lang(52));}}elseif($Vd&&!$_SESSION["pwds"]){session_regenerate_id();$ee=$b->permanentLogin();foreach($Vd
as$y=>$X){list(,$Ta)=explode(":",$X);list($Kf,$O,$V,$m)=array_map('base64_decode',explode("-",$y));set_password($Kf,$O,$V,decrypt_string(base64_decode($Ta),$ee));$_SESSION["db"][$Kf][$O][$V][$m]=true;}}function
unset_permanent(){global$Vd;foreach($Vd
as$y=>$X){list($Kf,$O,$V,$m)=array_map('base64_decode',explode("-",$y));if($Kf==DRIVER&&$O==SERVER&&$V==$_GET["username"]&&$m==DB)unset($Vd[$y]);}cookie("adminer_permanent",implode(" ",$Vd));}function
auth_error($Sb=null){global$h,$b,$xc;$Je=session_name();$n="";if(!$_COOKIE[$Je]&&$_GET[$Je]&&ini_bool("session.use_only_cookies"))$n=lang(53);elseif(isset($_GET["username"])){if(($_COOKIE[$Je]||$_GET[$Je])&&!$xc)$n=lang(54);else{$H=get_password();if($H!==null){$n=h($Sb?$Sb->getMessage():(is_string($h)?$h:lang(55)));if($H===false)$n.='<br>'.lang(56,'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$G=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$G["lifetime"]);page_header(lang(24),$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
set_password($Kf,$O,$V,$H){$_SESSION["pwds"][$Kf][$O][$V]=($_COOKIE["adminer_key"]?array(encrypt_string($H,$_COOKIE["adminer_key"])):$H);}function
...
Min_Driver($h);if($Ca&&$_POST["token"])$_POST["token"]=$pf;$n='';if($_POST){if(!verify_token()){$Mc="max_input_vars";$pd=ini_get($Mc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$pd||$X<$pd)){$Mc=$y;$pd=$X;}}}$n=(!$_POST["token"]&&$pd?lang(59,"'$Mc'"):lang(51));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(60,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(61);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();$h->select_db($b->database());function
email_header($yc){return"=?UTF-8?B?".base64_encode($yc)."?=";}function
send_mail($Gb,$Ue,$rd,$qc="",$dc=array()){$Nb=(DIRECTORY_SEPARATOR=="/"?"\n":"\r\n");$rd=str_replace("\n",$Nb,wordwrap(str_replace("\r","","$rd\n")));$Oa=uniqid("boundary");$_a="";foreach((array)$dc["error"]as$y=>$X){if(!$X)$_a.="--$Oa$Nb"."Content-Type: ".str_replace("\n","",$dc["type"][$y]).$Nb."Content-Disposition: attachment; filename=\"".preg_replace('~["\\n]~','',$dc["name"][$y])."\"$Nb"."Content-Transfer-Encoding: base64$Nb$Nb".chunk_split(base64_encode(file_get_contents($dc["tmp_name"][$y])),76,$Nb).$Nb;}$Ja="";$zc="Content-Type: text/plain; charset=utf-8$Nb"."Content-Transfer-Encoding: 8bit";if($_a){$_a.="--$Oa--$Nb";$Ja="--$Oa$Nb$zc$Nb$Nb";$zc="Content-Type: multipart/mixed; boundary=\"$Oa\"";}$zc.=$Nb."MIME-Version: 1.0$Nb"."X-Mailer: Adminer Editor".($qc?$Nb."From: ".str_replace("\n","",$qc):"");return
mail($Gb,email_header($Ue),$Ja.$rd.$_a,$zc);}function
like_bool($o){return
preg_match("~bool|(tinyint|bit)\\(1\\)~",$o["full_type"]);}$Dd="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$zb[DRIVER]=lang(24);if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$p=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$h->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$p),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$p=fields($a);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$p):""):where($_GET,$p));$Ef=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($p
as$D=>$o){if(!isset($o["privileges"][$Ef?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$hd=$_POST["referer"];if($_POST["insert"])$hd=($Ef?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$hd))$hd=ME."select=".urlencode($a);$w=indexes($a);$_f=unique_array($_GET["where"],$w);$ke="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($hd,lang(62),$yb->delete($a,$ke,!$_f));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Ef){if(!$P)redirect($hd);queries_redirect($hd,lang(63),$yb->update($a,$P,$ke,!$_f));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$yb->insert($a,$P);$cd=($J?last_id():0);queries_redirect($hd,lang(64,($cd?" $cd":"")),$J);}}}$Ze=$b->tableName(table_status1($a,true));page_header(($Ef?lang(65):lang(40)),$n,array("select"=>array($a,$Ze)),$Ze);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$y=>$X){if(!$Z)$L[$y]=null;$p[$y]=array("field"=>$y,"null"=>($y!="itemName()"),"auto_increment"=>($y=="itemName()"));}}}if($L===false)echo"<p class='error'>".lang(66)."\n";echo'
<div id="message"></div>

<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$p)echo"<p class='error'>".lang(67)."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($p
as$D=>$o){echo"<tr><th>".$b->fieldName($o);$ob=$_GET["set"][bracket_escape($D)];if($ob===null){$ob=$o["default"];if($o["type"]=="bit"&&preg_match("~^b'([01]*)'\$~",$ob,$pe))$ob=$pe[1];}$Y=($L!==null?($L[$D]!=""&&$x=="sql"&&preg_match("~enum|set~",$o["type"])?(is_array($L[$D])?array_sum($L[$D]):+$L[$D]):$L[$D]):(!$Ef&&$o["auto_increment"]?"":(isset($_GET["select"])?false:$ob)));if(!$_POST["save"]&&is_string($Y))$Y=$b->editVal($Y,$o);$q=($_POST["save"]?(string)$_POST["function"][$D]:($Ef&&$o["on_update"]=="CURRENT_TIMESTAMP"?"now":($Y===false?null:($Y!==null?'':'NULL'))));if(preg_match("~time~",$o["type"])&&$Y=="CURRENT_TIMESTAMP"){$Y="";$q="now";}input($o,$Y,$q);echo"\n";}if(!support("table"))echo"<tr>"."<th><input name='field_keys[]' value='".h($_POST["field_keys"][0])."'>"."<td class='function'>".html_select("field_funs[]",$b->editFunctions(array()),$_POST["field_funs"][0])."<td><input name='field_vals[]' value='".h($_POST["field_vals"][0])."'>"."\n";echo"</table>\n";}echo'<p>
';if($p){echo"<input type='submit' value='".lang(68)."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Ef?lang(69)."' onclick='return !ajaxForm(this.form, \"".lang(70).'...", this)':lang(71))."' title='Ctrl+Shift+Enter'>\n";}echo($Ef?"<input type='submit' name='delete' value='".lang(72)."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$pf,'">
</form>
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status1($a);$w=indexes($a);$p=fields($a);$lc=column_foreign_keys($a);$Cd="";if($S["Oid"]){$Cd=($x=="sqlite"?"rowid":"oid");$w[]=array("type"=>"PRIMARY","columns"=>array($Cd));}parse_str($_COOKIE["adminer_import"],$ra);$xe=array();$f=array();$ff=null;foreach($p
as$y=>$o){$D=$b->fieldName($o);if(isset($o["privileges"]["select"])&&$D!=""){$f[$y]=html_entity_decode(strip_tags($D),ENT_QUOTES);if(is_shortable($o))$ff=$b->selectLengthProcess();}$xe+=$o["privileges"];}list($N,$r)=$b->selectColumnsProcess($f,$w);$Qc=count($r)<count($N);$Z=$b->selectSearchProcess($p,$w);$E=$b->selectOrderProcess($p,$w);$z=$b->selectLimitProcess();$qc=($N?implode(", ",$N):"*".($Cd?", $Cd":"")).convert_fields($f,$p,$N)."\nFROM ".table($a);$tc=($r&&$Qc?"\nGROUP BY ".implode(", ",$r):"").($E?"\nORDER BY ".implode(", ",$E):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Af=>$L){$ya=convert_field($p[key($L)]);echo$h->result("SELECT".limit($ya?$ya:idf_escape(key($L))." FROM ".table($a)," WHERE ".where_check($Af,$p).($Z?" AND ".implode(" AND ",$Z):"").($E?" ORDER BY ".implode(", ",$E):""),1));}exit;}if($_POST&&!$n){$Rf=$Z;if(!$_POST["all"]&&is_array($_POST["check"]))$Rf[]="((".implode(") OR (",array_map('where_check',$_POST["check"]))."))";$Rf=($Rf?"\nWHERE ".implode(" AND ",$Rf):"");$ce=$Cf=null;foreach($w
as$v){if($v["type"]=="PRIMARY"){$ce=array_flip($v["columns"]);$Cf=($N?$ce:array());break;}}foreach((array)$Cf
as$y=>$X){if(in_array(idf_escape($y),$N))unset($Cf[$y]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Cf===array())$I="SELECT $qc$Rf$tc";else{$zf=array();foreach($_POST["check"]as$X)$zf[]="(SELECT".limit($qc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p).$tc,1).")";$I=implode(" UNION ALL ",$zf);}$b->dumpData($a,"table",$I);exit;}if(!$b->selectEmailProcess($Z,$lc)){if($_POST["save"]||$_POST["delete"]){$J=true;$sa=0;$P=array();if(!$_POST["delete"]){foreach($f
as$D=>$X){$X=process_input($p[$D]);if($X!==null&&($_POST["clone"]||$X!==false))$P[idf_escape($D)]=($X!==false?$X:idf_escape($D));}}if($_POST["delete"]||$P){if($_POST["clone"])$I="INTO ".table($a)." (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a);if($_POST["all"]||($Cf===array()&&is_array($_POST["check"]))||$Qc){$J=($_POST["delete"]?$yb->delete($a,$Rf):($_POST["clone"]?queries("INSERT $I$Rf"):$yb->update($a,$P,$Rf)));$sa=$h->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Qf="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$J=($_POST["delete"]?$yb->delete($a,$Qf,1):($_POST["clone"]?queries("INSERT".limit1($I,$Qf)):$yb->update($a,$P,$Qf)));if(!$J)break;$sa+=$h->affected_rows;}}}$rd=lang(73,$sa);if($_POST["clone"]&&$J&&$sa==1){$cd=last_id();if($cd)$rd=lang(64," $cd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$rd,$J);}elseif(!$_POST["import"]){if(!$_POST["val"])$n=lang(74);else{$J=true;$sa=0;foreach($_POST["val"]as$Af=>$L){$P=array();foreach($L
as$y=>$X){$y=bracket_escape($y,1);$P[idf_escape($y)]=(preg_match('~char|text~',$p[$y]["type"])||$X!=""?$b->processInput($p[$y],$X):"NULL");}$J=$yb->update($a,$P," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Af,$p),!($Qc||$Cf===array())," ");if(!$J)break;$sa+=$h->affected_rows;}queries_redirect(remove_from_uri(),lang(73,$sa),$J);}}elseif(!is_string($bc=get_file("csv_file",true)))$n=upload_error($bc);elseif(!preg_match('~~u',$bc))$n=lang(75);else{cookie("adminer_import","output=".urlencode($ra["output"])."&format=".urlencode($_POST["separator"]));$J=true;$Za=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$bc,$B);$sa=count($B[0]);$yb->begin();$Ge=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$M=array();foreach($B[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Ge]*)$Ge~",$X.$Ge,$md);if(!$y&&!array_diff($md[1],$Za)){$Za=$md[1];$sa--;}else{$P=array();foreach($md[1]as$s=>$Xa)$P[idf_escape($Za[$s])]=($Xa==""&&$p[$Za[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Xa))));$M[]=$P;}}$J=(!$M||$yb->insertUpdate($a,$M,$ce));if($J)$yb->commit();queries_redirect(remove_from_uri("page"),lang(76,$sa),$J);$yb->rollback();}}}$Ze=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header(lang(36).": $Ze",$n);$P=null;if(isset($xe["insert"])||!support("table")){$P="";foreach((array)$_GET["where"]as$X){if(count($lc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$P.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$P);if(!$f&&support("table"))echo"<p class='error'>".lang(77).($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($N,$f);$b->selectSearchPrint($Z,$f,$w);$b->selectOrderPrint($E,$f,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($ff);$b->selectActionPrint($w);echo"</form>\n";$F=$_GET["page"];if($F=="last"){$oc=$h->result(count_rows($a,$Z,$Qc,$r));$F=floor(max(0,$oc-1)/$z);}$De=$N;if(!$De){$De[]="*";if($Cd)$De[]=$Cd;}$gb=convert_fields($f,$p,$N);if($gb)$De[]=substr($gb,2);$J=$yb->select($a,$De,$Z,$r,$E,$z,$F);if(!$J)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$F)$J->seek($z*$F);$Ib=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$M=array();while($L=$J->fetch_assoc()){if($F&&$x=="oracle")unset($L["RNUM"]);$M[]=$L;}if($_GET["page"]!="last"&&+$z&&$r&&$Qc&&$x=="sql")$oc=$h->result(" SELECT FOUND_ROWS()");if(!$M)echo"<p class='message'>".lang(66)."\n";else{$Ia=$b->backwardKeys($a,$Ze);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$r&&$N?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".lang(78)."</a>");$yd=array();$sc=array();reset($N);$me=1;foreach($M[0]as$y=>$X){if($y!=$Cd){$X=$_GET["columns"][key($N)];$o=$p[$N?($X?$X["col"]:current($N)):$y];$D=($o?$b->fieldName($o,$me):($X["fun"]?"*":$y));if($D!=""){$me++;$yd[$y]=$D;$e=idf_escape($y);$Cc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$qb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Cc.($E[0]==$e||$E[0]==$y||(!$E&&$Qc&&$r[0]==$e)?$qb:'')).'">';echo
apply_sql_function($X["fun"],$D)."</a>";echo"<span class='column hidden'>","<a href='".h($Cc.$qb)."' title='".lang(79)."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($y)).'\'); return false;" title="'.lang(31).'" class="text jsonly"> =</a>';echo"</span>";}$sc[$y]=$X["fun"];next($N);}}$fd=array();if($_GET["modify"]){foreach($M
as$L){foreach($L
as$y=>$X)$fd[$y]=max($fd[$y],min(40,strlen(utf8_decode($X))));}}echo($Ia?"<th>".lang(80):"")."</thead>\n";if(is_ajax()){if($z%2==1&&$F%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($M,$lc)as$C=>$L){$_f=unique_array($M[$C],$w);if(!$_f){$_f=array();foreach($M[$C]as$y=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$y))$_f[$y]=$X;}}$Af="";foreach($_f
as$y=>$X){if(strlen($X)>64&&($x=="sql"||$x=="pgsql")){$y="MD5(".(strpos($y,'(')?$y:idf_escape($y)).")";$X=md5($X);}$Af.="&".($X!==null?urlencode("where[".bracket_escape($y)."]")."=".urlencode($X):"null%5B%5D=".urlencode($y));}echo"<tr".odd().">".(!$r&&$N?"":"<td>".checkbox("check[]",substr($Af,1),in_array(substr($Af,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Qc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Af)."'>".lang(81)."</a>"));foreach($L
as$y=>$X){if(isset($yd[$y])){$o=$p[$y];if($X!=""&&(!isset($Ib[$y])||$Ib[$y]!=""))$Ib[$y]=(is_mail($X)?$yd[$y]:"");$_="";if(preg_match('~blob|bytea|raw|file~',$o["type"])&&$X!="")$_=ME.'download='.urlencode($a).'&field='.urlencode($y).$Af;if(!$_){foreach((array)$lc[$y]as$kc){if(count($lc[$y])==1||end($kc["source"])==$y){$_="";foreach($kc["source"]as$s=>$Me)$_.=where_link($s,$kc["target"][$s],$M[$C][$Me]);$_=($kc["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($kc["db"]),ME):ME).'select='.urlencode($kc["table"]).$_;if(count($kc["source"])==1)break;}}}if($y=="COUNT(*)"){$_=ME."select=".urlencode($a);$s=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$_f))$_.=where_link($s++,$W["col"],$W["val"],$W["op"]);}foreach($_f
as$Sc=>$W)$_.=where_link($s++,$Sc,$W);}$X=select_value($X,$_,$o,$ff);$t=h("val[$Af][".bracket_escape($y)."]");$Y=$_POST["val"][$Af][bracket_escape($y)];$Eb=!is_array($L[$y])&&is_utf8($X)&&$M[$C][$y]==$L[$y]&&!$sc[$y];$ef=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$Eb)||$Y!==null){$vc=h($Y!==null?$Y:$L[$y]);echo"<td>".($ef?"<textarea name='$t' cols='30' rows='".(substr_count($L[$y],"\n")+1)."'>$vc</textarea>":"<input name='$t' value='$vc' size='$fd[$y]'>");}else{$jd=strpos($X,"<i>...</i>");echo"<td id='$t' onclick=\"selectClick(this, event, ".($jd?2:($ef?1:0)).($Eb?"":", '".h(lang(82))."'").");\">$X";}}}if($Ia)echo"<td>";$b->backwardKeysPrint($Ia,$M[$C]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($M||$F)&&!is_ajax()){$Rb=true;if($_GET["page"]!="last"){if(!+$z)$oc=count($M);elseif($x!="sql"||!$Qc){$oc=($Qc?false:found_rows($S,$Z));if($oc<max(1e4,2*($F+1)*$z))$oc=reset(slow_query(count_rows($a,$Z,$Qc,$r)));else$Rb=false;}}if(+$z&&($oc===false||$oc>$z||$F)){echo"<p class='pages'>";$nd=($oc===false?$F+(count($M)>=$z?2:1):floor(($oc-1)/$z));if(support("table")){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".lang(83)."', '".($F+1)."'), event); return false;\">".lang(83)."</a>:",pagination(0,$F).($F>5?" ...":"");for($s=max(1,$F-4);$s<min($nd,$F+5);$s++)echo
