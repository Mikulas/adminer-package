...
set_schema($ye){global$h,$vf,$Qe;$K=$h->query("SET search_path TO ".idf_escape($ye));foreach(types()as$U){if(!isset($vf[$U])){$vf[$U]=0;$Qe['UÅ¾ivatelskÃ© typy'][]=$U;}}return$K;}function
...
as$D=>$o){if(!isset($o["privileges"][$Cf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$dd=$_POST["referer"];if($_POST["insert"])$dd=($Cf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$dd))$dd=ME."select=".urlencode($a);$x=indexes($a);$yf=unique_array($_GET["where"],$x);$ie="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($dd,'PoloÅ¾ka byla smazÃ¡na.',$wb->delete($a,$ie,!$yf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Cf){if(!$P)redirect($dd);queries_redirect($dd,'PoloÅ¾ka byla aktualizovÃ¡na.',$wb->update($a,$P,$ie,!$yf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$wb->insert($a,$P);$Yc=($J?last_id():0);queries_redirect($dd,sprintf('PoloÅ¾ka%s byla vloÅ¾ena.',($Yc?" $Yc":"")),$J);}}}$Xe=$b->tableName(table_status1($a,true));page_header(($Cf?'Upravit':'VloÅ¾it'),$n,array("select"=>array($a,$Xe)),$Xe);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$z=>$X){if(!$Z)$L[$z]=null;$p[$z]=array("field"=>$z,"null"=>($z!=$u),"auto_increment"=>($z==$u));}}}if($L===false)echo"<p class='error'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";echo'
<div id="message"></div>
