...
input($c,$p,$O){global$T,$r,$_;$f=h(bracket_escape($c["field"]));echo"<td class='function'>";$V=(isset($_GET["select"])?array("orig"=>'původní'):array())+$r->editFunctions($c);$Qa=" name='fields[$f]'";if($c["type"]=="enum"){echo
...
lang($Ve,$nc){$jc=($nc==1||(!$nc&&'cs'=='fr')?0:((!$nc||$nc>=5)&&ereg('cs|sk|ru','cs')?2:1));return
...
q($b);}function
check_sqlite_name($f){global$g;$re="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($re)\$~",$f)){$g->error=sprintf('Prosím použijte jednu z koncovek %s.',str_replace("|",", ",$re));return
false;}return
true;}function
create_database($s,$S){global$g;if(file_exists($s)){$g->error='Soubor existuje.';return
...
set_schema($Ja){global$g,$T,$Oa;$d=$g->query("SET search_path TO ".idf_escape($Ja));foreach(types()as$y){if(!isset($T[$y])){$T[$y]=0;$Oa['Uživatelské typy'][]=$y;}}return$d;}function
...
reset($F);}function
support($ob){return
ereg("view|drop_col|variables|status",$ob);}$_="oracle";$T=array();$Oa=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$T+=$b;$Oa[$e]=array_keys($b);}$nb=array();$Nb=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL");$V=array("length","lower","round","upper");$qb=array("avg","count","count distinct","max","min","sum");$Vb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ha[]="SQLSRV";$Ha[]="MSSQL";if(extension_loaded("sqlsrv")||extension_loaded("mssql")){$ja["mssql"]="MS SQL";}if(isset($_GET["mssql"])){define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Xc,$q=""){echo'<p class="tabs">';$Ka=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Xc)){$Ka["view"]='Pozměnit pohled';}else{$Ka["create"]='Pozměnit tabulku';}if(isset($q)){$Ka["edit"]='Nová položka';}foreach($Ka
...
selectColumnsPrint($C,$A){global$V,$qb;print_fieldset("select",'Vypsat',$C);$k=0;$ne=array('Funkce'=>$V,'Agregace'=>$qb);foreach($C
...
selectSearchPrint($t,$A,$K){print_fieldset("search",'Vyhledat',$t);foreach($K
as$k=>$v){if($v["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$v["columns"]))."</i>) AGAINST"," <input name='fulltext[$k]' value='".h($_GET["fulltext"][$k])."'>",checkbox("boolean[$k]",1,isset($_GET["boolean"][$k]),"BOOL"),"<br>\n";}}$k=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$k][col]'><option value=''>(".'kdekoliv'.")".optionlist($A,$b["col"],true)."</select>",html_select("where[$k][op]",$this->operators,$b["op"]),"<input name='where[$k][val]' value='".h($b["val"])."'></div>\n";$k++;}}echo"<div><select name='where[$k][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoliv'.")".optionlist($A,null,true)."</select>",html_select("where[$k][op]",$this->operators),"<input name='where[$k][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($ub,$A,$K){print_fieldset("sort",'Seřadit',$ub);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($A[$b])){echo"<div><select name='order[$k]'><option>".optionlist($A,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'sestupně')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($A,null,true)."</select>",checkbox("desc[$k]",1,0,'sestupně')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($_b){if(isset($_b)){echo"<fieldset><legend>".'Délka textů'."</legend><div>",'<input name="text_length" size="3" value="'.h($_b).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$_;restart_session();$U="sql-".count($_SESSION["messages"]);$eb=&get_session("queries");$eb[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$U' onclick=\"return !toggle('$U');\">".'SQL příkaz'."</a><div id='$U' class='hidden'><pre class='jush-$_'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($eb[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$d);}function
editInput($h,$c,$Qa,$p){if($c["type"]=="enum"){return($c["null"]?"<label><input type='radio'$Qa value=''".(isset($p)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Qa value='0'".($p===0?" checked":"")."><i>".'prázdné'."</i></label>";}return"";}function
...
';if($ac!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$ac){$D=tables_list();if(!$D){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($D);$Ka=array();foreach($D
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Be[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($T[$e]==254){$b="<code>$b</code>";}}if(isset($Ka[$e])&&!$A[$Ka[$e]]){$x="edit=".urlencode($Ka[$e]);foreach($K[$Ka[$e]]as$yc=>$la){$x.="&where".urlencode("[".bracket_escape($yc)."]")."=".urlencode($a[$la]);}$b="<a href='".h(ME.$x)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($e,$c,$W,$ca=array()){global$Oa,$T,$nb,$fb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$c["type"]||isset($T[$c["type"]])?array():array($c["type"]))+$Oa+($ca?array('Cizí klíče'=>$ca):array()),$c["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($c["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$c["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($W,$c["collation"]).'</select>',($nb?"<select name='$e"."[unsigned]'".(!$c["type"]||ereg('(int|float|double|decimal)$',$c["type"])?"":" class='hidden'").'><option>'.optionlist($nb,$c["unsigned"]).'</select>':''),($ca?"<select name='$e"."[on_delete]'".(ereg("`",$c["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist($fb,$c["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Cb?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($Cb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($c["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $Pe, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.1' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.1' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Cd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.1' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Cb;}function
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě');print_fieldset("webfile",'Ze serveru',$_POST["webfile"]);$md=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$md[]=".$e";}}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($md?"[".implode("|",$md)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($eb){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($eb
...
as$f=>$c){if(!isset($c["privileges"][$na?"update":"insert"])||$r->fieldName($c)==""){unset($o[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ka=$_POST["referer"];if($_POST["insert"]){$ka=($na?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ka)){$ka=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$ka,'Položka byla smazána.');}else{$q=array();foreach($o
as$f=>$c){$b=process_input($c);if($b!==false&&$b!==null){$q[idf_escape($f)]=($na?"\n".idf_escape($f)." = $b":$b);}}if($na){if(!$q){redirect($ka);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$q),"\nWHERE $t"),$ka,'Položka byla aktualizována.');}else{$i=insert_into($m,$q);$ce=($i?last_id():0);queries_redirect($ka,sprintf('Položka%s byla vložena.',($ce?" $ce":"")),$i);}}}$Da=$r->tableName(table_status($m));page_header(($na?'Upravit':'Vložit'),$n,array("select"=>array($m,$Da)),$Da);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$C=array();foreach($o
...
<?php echo(support("comment")?checkbox("","",$Cb,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Cb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložit">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstranit"',$Xa,'>';}if(support("partitioning")){$Vd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdělit podle',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$de,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Vd||!$a["partition_by"]?" class='hidden'":""),'>
...
as$s){if(count($z)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$Md=false;}$rb=$s;}}queries_redirect(ME."db=".urlencode($rb),'Databáze byla vytvořena.',$Md);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$n,array(),DB);$W=collations();$f=DB;$xc=null;if($_POST){$f=$_POST["name"];$xc=$_POST["collation"];}elseif(DB!=""){$xc=db_collation(DB,$W);}elseif($_=="sql"){foreach(get_vals("SHOW GRANTS")as$ga){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ga,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea name="name" rows="10" cols="40" onkeydown="return textareaKeydown(this, event);">'.h($f).'</textarea><br>':'<input name="name" value="'.h($f).'" maxlength="64">')."\n".($W?html_select("collation",array(""=>"(".'porovnávání'.")")+$W,$xc):""),'<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložit">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'$Xa>\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Přidat další'."'>\n";}echo'</form>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+$fb,$a["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+$fb,$a["on_update"]),'<p>
<input type="submit" value="Uložit">
<noscript><p><input type="submit" name="add" value="Přidat sloupec"></noscript>
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',$Xa,'>';}echo'<input type="hidden" name="token" value="',$L,'">
</form>
...
';}elseif(isset($_GET["event"])){$lb=$_GET["event"];$fe=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$zd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($lb),substr(ME,0,-1),'Událost byla odstraněna.');}elseif(in_array($_POST["INTERVAL_FIELD"],$fe)&&isset($zd[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($lb!=""?"ALTER EVENT ".idf_escape($lb).$Td.($lb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$zd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($lb!=""?'Událost byla změněna.':'Událost byla vytvořena.'));}}page_header(($lb!=""?'Pozměnit událost'.": ".h($lb):'Vytvořit událost'),$n);$a=array();if($_POST){$a=$_POST;}elseif($lb!=""){$F=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($lb));$a=reset($F);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$fe,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$zd,$a["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložit">
';if($lb!=""){echo'<input type="submit" name="drop" value="Odstranit"',$Xa,'>';}echo'</form>
...
<input type="hidden" name="token" value="',$L,'">
';if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Odstranit"',$Xa,'>';}echo'</form>
...
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložit">
';if($yb!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'$Xa>\n";}echo'</form>
';}elseif(isset($_GET["type"])){$Yc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Yc),$x,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ byl vytvořen.');}}page_header($Yc!=""?'Pozměnit typ'.": ".h($Yc):'Vytvořit typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$L,'">
';if($Yc!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'$Xa>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložit'."'>\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Kc["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($m,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Událost<td>',html_select("Event",$ee,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Kc["Type"],$a["Type"]),'</table>
<p>Název: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$L,'">
';if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',$Xa,'>';}echo'</form>
...
as$b){if($b[1]!="USAGE"){$bb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$bb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$Nc=$l[1];}}}if($_POST&&!$n){$bc=(isset($_GET["host"])?q($ud)."@".q($_GET["host"]):"''");$cb=q($_POST["user"])."@".q($_POST["host"]);$Dd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $bc",ME."privileges=",'Uživatel byl odstraněn.');}else{if($bc!=$cb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $cb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Dd");}elseif($_POST["pass"]!=$Nc||!$_POST["hashed"]){queries("SET PASSWORD FOR $cb = ".($_POST["hashed"]?$Dd:"PASSWORD($Dd)"));}if(!$n){$vc=array();foreach($qc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html'>".'Oprávnění'."</a>";$k=0;foreach($bb
as$Za=>$ga){echo'<th>'.($Za!="*.*"?"<input name='objects[$k]' value='".h($Za)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Uc=>$hc){foreach((array)$ma[$Uc]as$uc=>$ua){echo"<tr".odd()."><td".($hc?">$hc<td":" colspan='2'").' lang="en" title="'.h($ua).'">'.h($uc);$k=0;foreach($bb
...
<input type="hidden" name="token" value="',$L,'">
<input type="submit" value="Uložit">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit"',$Xa,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$bd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$bd++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$bd),$bd||!$_POST["kill"]);}page_header('Seznam procesů',$n);echo'
<form action="" method="post">
