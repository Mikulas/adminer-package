...
upload_error($k){$xd=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Nepodařilo se nahrát soubor.'.($xd?" ".sprintf('Maximální povolená velikost souboru je %sB.',$xd):""):'Soubor neexistuje.');}function
...
enum_input($S,$za,$l,$X,$Ob=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$sd);$G=($Ob!==null?"<label><input type='$S'$za value='$Ob'".((is_array($X)?in_array($Ob,$X):$X===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($sd[1]as$p=>$W){$W=stripcslashes(str_replace("''","'",$W));$Pa=(is_int($X)?$X==$p+1:(is_array($X)?in_array($p+1,$X):$X===$W));$G.=" <label><input type='$S'$za value='".($p+1)."'".($Pa?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$u;$_=h(bracket_escape($l["field"]));echo"<td class='function'>";$Ye=($u=="mssql"&&$l["auto_increment"]);if($Ye&&!$_POST["save"])$o=null;$xc=(isset($_GET["select"])||$Ye?array("orig"=>'původní'):array())+$b->editFunctions($l);$za=" name='fields[$_]'";if($l["type"]=="enum")echo
...
lang($cg,$Nd){$ze=($Nd==1?0:($Nd&&$Nd<5?1:2));$cg=str_replace("%d","%s",$cg[$ze]);$Nd=number_format($Nd,0,".",' ');return
...
q($W);}function
check_sqlite_name($_){global$g;$gc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($gc)\$~",$_)){$g->error=sprintf('Prosím použijte jednu z koncovek %s.',str_replace("|",", ",$gc));return
false;}return
true;}function
...
set_schema($kf){global$g,$T,$Af;$G=$g->query("SET search_path TO ".idf_escape($kf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$Af['Uživatelské typy'][]=$S;}}return$G;}function
...
unconvert_field($l,$G){return$G;}function
support($ic){return
ereg("view|scheme|processlist|drop_col|variables|status",$ic);}$u="oracle";$T=array();$Af=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$v=>$W){$T+=$W;$Af[$v]=array_keys($W);}$rg=array();$Yd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$xc=array("length","lower","round","upper");$Bc=array("avg","count","count distinct","max","min","sum");$Ib=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Cb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Be=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Hf,$L=""){echo'<p class="tabs">';$nd=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Hf))$nd["view"]='Pozměnit pohled';else$nd["create"]='Pozměnit tabulku';if($L!==null)$nd["edit"]='Nová položka';foreach($nd
...
selectColumnsPrint($J,$f){global$xc,$Bc;print_fieldset("select",'Vypsat',$J);$p=0;$wc=array('Funkce'=>$xc,'Agregace'=>$Bc);foreach($J
...
selectOrderPrint($ce,$f,$t){print_fieldset("sort",'Seřadit',$ce);$p=0;foreach((array)$_GET["order"]as$v=>$W){if(isset($f[$W])){echo"<div><select name='order[$p]' onchange='selectFieldChange(this.form);'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$p]",1,isset($_GET["desc"][$v]),'sestupně')."</div>\n";$p++;}}echo"<div><select name='order[$p]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$p]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
...
messageQuery($E){global$u;static$ib=0;restart_session();$q="sql-".($ib++);$Dc=&get_session("queries");if(strlen($E)>1e6)$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";$Dc[$_GET["db"]][]=array($E,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$q' onclick=\"return !toggle('$q');\">".'SQL příkaz'."</a><div id='$q' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Dc[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
edit_type($v,$l,$Wa,$qc=array()){global$Af,$T,$rg,$Ud;echo'<td><select name="',$v,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$Af+($qc?array('Cizí klíče'=>$qc):array()),$l["type"]),'</select>
<td><input name="',$v,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$v"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($Wa,$l["collation"]).'</select>',($rg?"<select name='$v"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($rg,$l["unsigned"]).'</select>':''),($qc?"<select name='$v"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$Ud),$l["on_delete"])."</select> ":" ");}function
...
<td>Výchozí hodnoty
',(support("comment")?"<td".($bb?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.3' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($bb?"":" class='hidden'")."><input name='fields[$p][comment]' value='".h($l["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.3' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $wa, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.3' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.3' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($ge==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.3' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$v=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($v)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($v))."'>".'Změnit'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Kf=array();$Lf=array();$_="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$_-".DB]?"$_-".DB:$_)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$sd,PREG_SET_ORDER);foreach($sd
...
as$_=>$l){if(!isset($l["privileges"][$sg?"update":"insert"])||$b->fieldName($l)=="")unset($m[$_]);}if($_POST&&!$k&&!isset($_GET["select"])){$od=$_POST["referer"];if($_POST["insert"])$od=($sg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$od))$od=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$od,'Položka byla smazána.');else{$L=array();foreach($m
as$_=>$l){$W=process_input($l);if($W!==false&&$W!==null)$L[idf_escape($_)]=($sg?"\n".idf_escape($_)." = $W":$W);}if($sg){if(!$L)redirect($od);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$od,'Položka byla aktualizována.');}else{$F=insert_into($a,$L);$gd=($F?last_id():0);queries_redirect($od,sprintf('Položka%s byla vložena.',($gd?" $gd":"")),$F);}}}$If=$b->tableName(table_status($a));page_header(($sg?'Upravit':'Vložit'),$k,array("select"=>array($a,$If)),$If);$H=null;if($_POST["save"])$H=(array)$_POST["fields"];elseif($Z){$J=array();foreach($m
...
';if(support("partitioning")){$se=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdělit podle',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$re,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddíly: <input type="number" name="partitions" class="size" value="',h($H["partitions"]),'"',($se||!$H["partition_by"]?" class='hidden'":""),'>
...
<input type="submit" value="Uložit">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.3' alt='+' title='".'Přidat další'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$Ud),$H["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$Ud),$H["on_update"]),'<p>
<input type="submit" value="Uložit">
...
<tr><th>Každých<td><input type="number" name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Rc,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$zf,$H["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<form action="" method="post" id="form">
<p>Název: <input name="name" value="',h($H["name"]),'" maxlength="64">
',($ff?'Jazyk'.": ".html_select("language",$ff,$H["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($H["fields"],$Wa,$ef);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$H["returns"],$Wa);}echo'</table>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$hg["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Událost<td>',html_select("Event",$fg,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$hg["Type"],$H["Type"]),'</table>
...
as$W){if($W[1]!="USAGE")$zc["$z[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$H[0]))$zc["$z[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$z))$Rd=$z[1];}}if($_POST&&!$k){$Sd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$Ld=q($_POST["user"])."@".q($_POST["host"]);$ve=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $Sd",ME."privileges=",'Uživatel byl odstraněn.');else{$lb=false;if($Sd!=$Ld){$lb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Ld IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $ve");$k=!$lb;}elseif($_POST["pass"]!=$Rd||!$_POST["hashed"])queries("SET PASSWORD FOR $Ld = ".($_POST["hashed"]?$ve:"PASSWORD($ve)"));if(!$k){$bf=array();foreach($Kd
...
as$Od=>$bf){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Od,$z))grant("REVOKE",array_keys($bf),$z[2]," ON $z[1] FROM $Ld");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Uživatel byl změněn.':'Uživatel byl vytvořen.'),!$k);if($lb)$g->query("DROP USER $Ld");}}page_header((isset($_GET["host"])?'Uživatel'.": ".h("$ha@$_GET[host]"):'Vytvořit uživatele'),$k,array("privileges"=>array('','Oprávnění')));if($_POST){$H=$_POST;$zc=$Kd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$Rd;if($Rd!="")$H["hashed"]=true;$zc[(DB==""||$zc?"":idf_escape(addcslashes(DB,"%_"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$p=0;foreach($zc
as$Od=>$yc){echo'<th>'.($Od!="*.*"?"<input name='objects[$p]' value='".h($Od)."' size='10'>":"<input type='hidden' name='objects[$p]' value='*.*' size='10'>*.*");$p++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$gb=>$wb){foreach((array)$Je[$gb]as$Ie=>$ab){echo"<tr".odd()."><td".($wb?">$wb<td":" colspan='2'").' lang="en" title="'.h($ab).'">'.h($Ie);$p=0;foreach($zc
as$Od=>$yc){$_="'grants[$p][".h(strtoupper($Ie))."]'";$X=$yc[strtoupper($Ie)];if($gb=="Server Admin"&&$Od!=(isset($zc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$_><option><option value='1'".($X?" selected":"").">".'Povolit'."<option value='0'".($X=="0"?" selected":"").">".'Zakázat'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$cd=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$cd++;}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$cd),$cd||!$_POST["kill"]);}page_header('Seznam procesů',$k);echo'
<form action="" method="post">
