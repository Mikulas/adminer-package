...
confirm($Oc="",$Sf=false){return" onclick=\"".($Sf?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Oc?" (' + $Oc + ')":"")."');\"";}function
js_escape($K){return
addcslashes($K,"\r\n'\\/");}function
...
enum_input($z,$Ta,$d,$q,$cc=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);$c=(isset($cc)?"<label><input type='$z'$Ta value='$cc'".((is_array($q)?in_array($cc,$q):$q===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($sa[1]as$k=>$b){$b=stripcslashes(str_replace("''","'",$b));$fb=(is_int($q)?$q==$k+1:(is_array($q)?in_array($k+1,$q):$q===$b));$c.=" <label><input type='$z'$Ta value='".($k+1)."'".($fb?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$L){global$Q,$o,$u;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Fe=($u=="mssql"&&$d["auto_increment"]);if($Fe&&!$_POST["save"]){$L=null;}$aa=(isset($_GET["select"])||$Fe?array("orig"=>'původní'):array())+$o->editFunctions($d);$Ta=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
set_schema($Ia){global$g,$Q,$La;$c=$g->query("SET search_path TO ".idf_escape($Ia));foreach(types()as$z){if(!isset($Q[$z])){$Q[$z]=0;$La['Uživatelské typy'][]=$z;}}return$c;}function
...
reset($H);}function
support($xb){return
ereg("view|drop_col|variables|status",$xb);}$u="oracle";$Q=array();$La=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$Q+=$b;$La[$e]=array_keys($b);}$ib=array();$bc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$aa=array("length","lower","round","upper");$gb=array("avg","count","count distinct","max","min","sum");$Pb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$na["mssql"]="MS SQL";if(isset($_GET["mssql"])){$xc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($Zc,$r=""){echo'<p class="tabs">';$ta=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Zc)){$ta["view"]='Pozměnit pohled';}else{$ta["create"]='Pozměnit tabulku';}if(isset($r)){$ta["edit"]='Nová položka';}foreach($ta
...
selectColumnsPrint($I,$B){global$aa,$gb;print_fieldset("select",'Vypsat',$I);$k=0;$Qe=array('Funkce'=>$aa,'Agregace'=>$gb);foreach($I
...
selectSearchPrint($s,$B,$J){print_fieldset("search",'Vyhledat',$s);foreach($J
...
selectOrderPrint($cb,$B,$J){print_fieldset("sort",'Seřadit',$cb);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'sestupně')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>","<label><input type='checkbox' name='desc[$k]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
...
false;}function
messageQuery($i){global$u;static$Oc=0;restart_session();$S="sql-".($Oc++);$Ya=&get_session("queries");if(strlen($i)>1e6){$i=ereg_replace('[\x80-\xFF]+$','',substr($i,0,1e6))."\n...";}$Ya[$_GET["db"]][]=$i;return" <a href='#$S' onclick=\"return !toggle('$S');\">".'SQL příkaz'."</a><div id='$S' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($i,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ya[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$c);}function
editInput($h,$d,$Ta,$q){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ta value='-1' checked><i>".'původní'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ta value=''".(isset($q)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ta,$d,$q,0);}return"";}function
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($_e[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($Q[$e]==254){$b="<code>$b</code>";}}if(isset($ta[$e])&&!$B[$ta[$e]]){if($Ec){$x=$ta[$e].urlencode($a[array_search("table=",$ta)]);}else{$x="edit=".urlencode($ta[$e]);foreach($J[$ta[$e]]as$Nc=>$la){$x.="&where".urlencode("[".bracket_escape($Nc)."]")."=".urlencode($a[$la]);}}$b="<a href='".h(ME.$x)."'>$b</a>";}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($e,$d,$Z,$da=array()){global$La,$Q,$ib,$eb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($Q[$d["type"]])?array():array($d["type"]))+$La+($da?array('Cizí klíče'=>$da):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($Z,$d["collation"]).'</select>',($ib?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($ib,$d["unsigned"]).'</select>':''),($da?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist($eb,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Db?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($Db?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $ef, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.0' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.0' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($nd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.0' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Změnit'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB);$vb=array();$Re=array();$f="adminer_schema";$We=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$We,$sa,PREG_SET_ORDER);foreach($sa
...
<p><a href="',h(ME."schema=".urlencode($We)),'" id="schema-link">Trvalý odkaz</a>
...
';$ya=true;foreach($kd
as$e=>$b){if($e!=""&&$b>1){echo($ya?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$e%")."'>".h($e)."</a>";$ya=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnění');$j=$g->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q($_GET["db"])." LIKE Db")." ORDER BY Host, User");if(!$j){echo'<form action=""><p>
';hidden_fields_get();echo'Uživatel: <input name="user">
Server: <input name="host" value="localhost">
<input type="hidden" name="grant" value="">
<input type="submit" value="Upravit">
</form>
';$j=$g->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");}echo"<table cellspacing='0'>\n","<thead><tr><th>&nbsp;<th>".'Uživatel'."<th>".'Server'."</thead>\n";while($a=$j->fetch_assoc()){echo'<tr'.odd().'><td><a href="'.h(ME.'user='.urlencode($a["User"]).'&host='.urlencode($a["Host"])).'">'.'upravit'.'</a><td>'.h($a["User"])."<td>".h($a["Host"])."\n";}echo"</table>\n",'<p><a href="'.h(ME).'user=">'.'Vytvořit uživatele'."</a>";}elseif(isset($_GET["sql"])){if(!$n&&$_POST["export"]){dump_headers("sql");$o->dumpTable("","");$o->dumpData("","table",$_POST["query"]);exit;}restart_session();$vf=&get_session("queries");$Ya=&$vf[DB];if(!$n&&$_POST["clear"]){$Ya=array();redirect(remove_from_uri("history"));}page_header('SQL příkaz',$n);if(!$n&&$_POST){$Ka=false;$i=$_POST["query"];if($_POST["webfile"]){$Ka=@fopen((file_exists("adminer.sql")?"adminer.sql":(file_exists("adminer.sql.gz")?"compress.zlib://adminer.sql.gz":"compress.bzip2://adminer.sql.bz2")),"rb");$i=($Ka?fread($Ka,1e6):false);}elseif($_FILES&&$_FILES["sql_file"]["error"]!=4){$i=get_file("sql_file",true);}if(is_string($i)){if(function_exists('memory_get_usage')){@ini_set("memory_limit",max(ini_get("memory_limit"),2*strlen($i)+memory_get_usage()+8e6));}if($i!=""&&strlen($i)<1e6){$ba=$i.(ereg(';$',$i)?"":";");if(!$Ya||end($Ya)!=$ba){$Ya[]=$ba;}}$kc="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\n)";if(!ini_bool("session.use_cookies")){session_write_close();}$qd=";";$M=0;$cc=true;$G=connect();if(is_object($G)&&DB!=""){$G->select_db(DB);}$zb=0;$Tc=array();$Mf='[\'`"]'.($u=="pgsql"?'|\\$[^$]*\\$':($u=="mssql"||$u=="sqlite"?'|\\[':'')).'|/\\*|-- |#';$Rf=microtime();parse_str($_COOKIE["adminer_export"],$ne);$be=$o->dumpFormat();unset($be["sql"]);while($i!=""){if(!$M&&$u=="sql"&&preg_match("~^$kc*DELIMITER\\s+(.+)~i",$i,$l)){$qd=$l[1];$i=substr($i,strlen($l[0]));}else{preg_match('('.preg_quote($qd)."|$Mf|\$)",$i,$l,PREG_OFFSET_CAPTURE,$M);$qa=$l[0][0];$M=$l[0][1]+strlen($qa);if(!$qa&&$Ka&&!feof($Ka)){$i.=fread($Ka,1e5);}else{if(!$qa&&rtrim($i)==""){break;}if($qa&&$qa!=$qd){while(preg_match('('.($qa=='/*'?'\\*/':($qa=='['?']':(ereg('^-- |^#',$qa)?"\n":preg_quote($qa)."|\\\\."))).'|$)s',$i,$l,PREG_OFFSET_CAPTURE,$M)){$ma=$l[0][0];$M=$l[0][1]+strlen($ma);if(!$ma&&$Ka&&!feof($Ka)){$i.=fread($Ka,1e6);}elseif($ma[0]!="\\"){break;}}}else{$cc=false;$ba=substr($i,0,$l[0][1]);$zb++;$yb="<pre id='sql-$zb'><code class='jush-$u'>".shorten_utf8(trim($ba),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$yb;ob_flush();flush();}$Eb=microtime();if($g->multi_query($ba)&&is_object($G)&&preg_match("~^$kc*USE\\b~isU",$ba)){$G->query($ba);}do{$j=$g->store_result();$Qc=microtime();$pe=format_time($Eb,$Qc).(strlen($ba)<1000?" <a href='".h(ME)."sql=".urlencode(trim($ba))."'>".'Upravit'."</a>":"");if($g->error){echo($_POST["only_errors"]?$yb:""),"<p class='error'>".'Chyba v dotazu'.": ".error()."\n";$Tc[]=" <a href='#sql-$zb'>$zb</a>";if($_POST["error_stops"]){break;}}elseif(is_object($j)){select($j,$G);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n","<p>".($j->num_rows?lang(array('%d řádek','%d řádky','%d řádků'),$j->num_rows):"").$pe;$S="export-$zb";$re=", <a href='#$S' onclick=\"return !toggle('$S');\">".'Export'."</a><span id='$S' class='hidden'>: ".html_select("output",$o->dumpOutput(),$ne["output"])." ".html_select("format",$be,$ne["format"])."<input type='hidden' name='query' value='".h($ba)."'>"." <input type='submit' name='export' value='".'Export'."' onclick='eventStop(event);'><input type='hidden' name='token' value='$O'></span>";if($G&&preg_match("~^($kc|\\()*SELECT\\b~isU",$ba)&&($Df=explain($G,$ba))){$S="explain-$zb";echo", <a href='#$S' onclick=\"return !toggle('$S');\">EXPLAIN</a>$re\n","<div id='$S' class='hidden'>\n";select($Df,$G,($u=="sql"?"http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/explain-output.html#explain_":""));echo"</div>\n";}else{echo"$re\n";}echo"</form>\n";}}else{if(preg_match("~^$kc*(CREATE|DROP|ALTER)$kc+(DATABASE|SCHEMA)\\b~isU",$ba)){restart_session();set_session("dbs",null);session_write_close();}if(!$_POST["only_errors"]){echo"<p class='message' title='".h($g->info)."'>".lang(array('Příkaz proběhl v pořádku, byl změněn %d záznam.','Příkaz proběhl v pořádku, byly změněny %d záznamy.','Příkaz proběhl v pořádku, bylo změněno %d záznamů.'),$g->affected_rows)."$pe\n";}}$Eb=$Qc;}while($g->next_result());$i=substr($i,$M);$M=0;}}}}if($cc){echo"<p class='message'>".'Žádné příkazy k vykonání.'."\n";}elseif($_POST["only_errors"]){echo"<p class='message'>".lang(array('%d příkaz proběhl v pořádku.','%d příkazy proběhly v pořádku.','%d příkazů proběhlo v pořádku.'),$zb-count($Tc)).format_time($Rf,microtime())."\n";}elseif($Tc&&$zb>1){echo"<p class='error'>".'Chyba v dotazu'.": ".implode("",$Tc)."\n";}}else{echo"<p class='error'>".upload_error($i)."\n";}}echo'
...
<input type="hidden" name="token" value="',$O,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$pd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$pd[]=".$e";}}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($pd?"[".implode("|",$pd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($Ya){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($Ya
...
as$f=>$d){if(!isset($d["privileges"][$oa?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ga=$_POST["referer"];if($_POST["insert"]){$ga=($oa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ga)){$ga=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$ga,'Položka byla smazána.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($oa?"\n".idf_escape($f)." = $b":$b);}}if($oa){if(!$r){redirect($ga);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$ga,'Položka byla aktualizována.');}else{$j=insert_into($m,$r);$je=($j?last_id():0);queries_redirect($ga,sprintf('Položka%s byla vložena.',($je?" $je":"")),$j);}}}$Ea=$o->tableName(table_status($m));page_header(($oa?'Upravit':'Vložit'),$n,array("select"=>array($m,$Ea)),$Ea);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$I=array();foreach($p
...
as$t){if(count($A)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$me=false;}$pb=$t;}}queries_redirect(ME."db=".urlencode($pb),'Databáze byla vytvořena.',$me);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$n,array(),DB);$Z=collations();$f=DB;$wc=null;if($_POST){$f=$_POST["name"];$wc=$_POST["collation"];}elseif(DB!=""){$wc=db_collation(DB,$Z);}elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$ka){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ka,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Z?html_select("collation",array(""=>"(".'porovnávání'.")")+$Z,$wc):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložit">
<?php
if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Přidat další'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+$eb,$a["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+$eb,$a["on_update"]),'<p>
<input type="submit" value="Uložit">
...
';if($Xa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["event"])){$mb=$_GET["event"];$ke=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Gd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($mb),substr(ME,0,-1),'Událost byla odstraněna.');}elseif(in_array($_POST["INTERVAL_FIELD"],$ke)&&isset($Gd[$_POST["STATUS"]])){$oe="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($mb!=""?'Událost byla změněna.':'Událost byla vytvořena.'),queries(($mb!=""?"ALTER EVENT ".idf_escape($mb).$oe.($mb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$oe)."\n".$Gd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($mb!=""?'Pozměnit událost'.": ".h($mb):'Vytvořit událost'),$n);$a=array();if($_POST){$a=$_POST;}elseif($mb!=""){$H=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($mb));$a=reset($H);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$ke,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Gd,$a["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($mb!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
<input type="submit" value="Uložit">
';if($Pa!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($Xa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
';}elseif(isset($_GET["type"])){$gd=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($gd),$x,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ byl vytvořen.');}}page_header($gd!=""?'Pozměnit typ'.": ".h($gd):'Vytvořit typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($gd!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložit'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
as$b){if($b[1]!="USAGE"){$kb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$kb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$ad=$l[1];}}}if($_POST&&!$n){$Wb=(isset($_GET["host"])?q($Dd)."@".q($_GET["host"]):"''");$nb=q($_POST["user"])."@".q($_POST["host"]);$yd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $Wb",ME."privileges=",'Uživatel byl odstraněn.');}else{if($Wb!=$nb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $nb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $yd");}elseif($_POST["pass"]!=$ad||!$_POST["hashed"]){queries("SET PASSWORD FOR $nb = ".($_POST["hashed"]?$yd:"PASSWORD($yd)"));}if(!$n){$zc=array();foreach($lc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$k=0;foreach($kb
as$ab=>$ka){echo'<th>'.($ab!="*.*"?"<input name='objects[$k]' value='".h($ab)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Yc=>$tc){foreach((array)$ra[$Yc]as$_c=>$xa){echo"<tr".odd()."><td".($tc?">$tc<td":" colspan='2'").' lang="en" title="'.h($xa).'">'.h($_c);$k=0;foreach($kb
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$_d=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$_d++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$_d),$_d||!$_POST["kill"]);}page_header('Seznam procesů',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".($e=="Info"&&$b!=""?"<code class='jush-$u'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Upravit'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>',($k+1)."/".sprintf('%d celkem',$g->result("SELECT @@max_connections")),'<p>
