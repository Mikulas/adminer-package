...
confirm($Fc="",$yf=false){return" onclick=\"".($yf?"eventStop(event); ":"")."return confirm('".'Are you sure?'.($Fc?" (' + $Fc + ')":"")."');\"";}function
...
upload_error($q){$wd=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Unable to upload a file.'.($wd?" ".sprintf('Maximum allowed file size is %sB.',$wd):""):'File does not exist.');}function
...
enum_input($M,$Fa,$d,$o,$Eb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$da);$e=(isset($Eb)?"<label><input type='$M'$Fa value='$Eb'".((is_array($o)?in_array($Eb,$o):$o===0)?" checked":"")."><i>".'empty'."</i></label>":"");foreach($da[1]as$g=>$b){$b=stripcslashes(str_replace("''","'",$b));$Oa=(is_int($o)?$o==$g+1:(is_array($o)?in_array($g+1,$o):$o===$b));$e.=" <label><input type='$M'$Fa value='".($g+1)."'".($Oa?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
...
selectLinks($Ec,$u=""){echo'<p class="tabs">';$ha=array("select"=>'Select data',"table"=>'Show structure');if(is_view($Ec)){$ha["view"]='Alter view';}else{$ha["create"]='Alter table';}if(isset($u)){$ha["edit"]='New item';}foreach($ha
...
selectColumnsPrint($z,$y){global$Y,$Gb;print_fieldset("select",'Select',$z);$g=0;$ee=array('Functions'=>$Y,'Aggregation'=>$Gb);foreach($z
...
selectSearchPrint($x,$y,$_){print_fieldset("search",'Search',$x);foreach($_
as$g=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input name='fulltext[$g]' value='".h($_GET["fulltext"][$g])."'>",checkbox("boolean[$g]",1,isset($_GET["boolean"][$g]),"BOOL"),"<br>\n";}}$g=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$g][col]'><option value=''>(".'anywhere'.")".optionlist($y,$b["col"],true)."</select>",html_select("where[$g][op]",$this->operators,$b["op"]),"<input name='where[$g][val]' value='".h($b["val"])."'></div>\n";$g++;}}echo"<div><select name='where[$g][col]' onchange='selectAddRow(this);'><option value=''>(".'anywhere'.")".optionlist($y,null,true)."</select>",html_select("where[$g][op]",$this->operators,"="),"<input name='where[$g][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Ga,$y,$_){print_fieldset("sort",'Sort',$Ga);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($y[$b])){echo"<div><select name='order[$g]'><option>".optionlist($y,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'descending')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($y,null,true)."</select>","<label><input type='checkbox' name='desc[$g]' value='1'>".'descending'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($W){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($W)."'>","</div></fieldset>\n";}function
selectLengthPrint($sb){if(isset($sb)){echo"<fieldset><legend>".'Text length'."</legend><div>",'<input name="text_length" size="3" value="'.h($sb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$r;static$Fc=0;restart_session();$E="sql-".($Fc++);$Na=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Na[$_GET["db"]][]=$j;return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL command'."</a><div id='$E' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Na[$_GET["db"]])-1)).'">'.'Edit'.'</a></div>';}function
...
';if($zb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$zb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Create new table'."</a>\n";$qa=tables_list();if(!$qa){echo"<p class='message'>".'No tables.'."\n";}else{$this->tablesPrint($qa);$ha=array();foreach($qa
...
as$l=>$M){echo'<a href="'.h(ME).'select='.urlencode($l).'"'.bold($_GET["select"]==$l).">".'select'."</a> ",'<a href="'.h(ME).'table='.urlencode($l).'"'.bold($_GET["table"]==$l).">".$this->tableName(array("Name"=>$l))."</a><br>\n";}}}$n=(function_exists('adminer_object')?adminer_object():new
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($te[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d byte','%d bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ya[$c]==254){$b="<code>$b</code>";}}if(isset($ha[$c])&&!$y[$ha[$c]]){if($zc){$C=$ha[$c].urlencode($a[array_search("table=",$ha)]);}else{$C="edit=".urlencode($ha[$c]);foreach($_[$ha[$c]]as$Ac=>$U){$C.="&where".urlencode("[".bracket_escape($Ac)."]")."=".urlencode($a[$U]);}}$b="<a href='".h(ME.$C)."'>$b</a>";}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'No rows.')."\n";}function
...
edit_type($c,$d,$S,$N=array()){global$Ic,$ya,$Db,$Ha;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ya[$d["type"]])?array():array($d["type"]))+$Ic+($N?array('Foreign keys'=>$N):array()),$d["type"]),'</select>
...
';if($M=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($M=="TABLE"?'Column name':'Parameter name'),'<td>Type<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Length
<td>Options
...
',(support("comment")?"<td".($hb?"":" class='hidden'").">".'Comment':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Add next'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($hb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Add next'."' onclick='return !editingAddRow(this, $Ce, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.2' alt='^' title='".'Move up'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.2' alt='v' title='".'Move down'."'>&nbsp;":""),($Qc==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.2' alt='x' title='".'Remove'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$c=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($c)."<td><a href='".h(ME.'trigger='.urlencode($h).'&name='.urlencode($c))."'>".'Alter'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($h).'">'.'Add trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Database schema',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$ab=array();$ye=array();$f="adminer_schema";$se=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$se,$da,PREG_SET_ORDER);foreach($da
...
<p><a href="',h(ME."schema=".urlencode($se)),'" id="schema-link">Permanent link</a>
...
<input type="hidden" name="token" value="',$F,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Stop on error')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Show only errors')."\n";print_fieldset("webfile",'From server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Uc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Uc[]=".$c";}}echo
...
as$f=>$d){if(!isset($d["privileges"][$Ca?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$P=$_POST["referer"];if($_POST["insert"]){$P=($Ca?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$P)){$P=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$P,'Item has been deleted.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Ca?"\n".idf_escape($f)." = $b":$b);}}if($Ca){if(!$u){redirect($P);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$P,'Item has been updated.');}else{$k=insert_into($h,$u);$Fd=($k?last_id():0);queries_redirect($P,sprintf('Item%s has been inserted.',($Fd?" $Fd":"")),$k);}}}$wa=$n->tableName(table_status($h));page_header(($Ca?'Edit':'Insert'),$q,array("select"=>array($h,$wa)),$wa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$z=array();foreach($p
as$f=>$d){if(isset($d["privileges"]["select"])){$z[]=($_POST["clone"]&&$d["auto_increment"]?"'' AS ":(ereg("enum|set",$d["type"])?"1*".idf_escape($f)." AS ":"")).idf_escape($f);}}$a=array();if($z){$G=get_rows("SELECT".limit(implode(", ",$z)." FROM ".table($h)," WHERE $x",(isset($_GET["select"])?2:1)));$a=(isset($_GET["select"])&&count($G)!=1?null:reset($G));}}if($a===false){echo"<p class='error'>".'No rows.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($p){echo"<input type='submit' value='".'Save'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Ca?'Save and continue edit':'Save and insert next')."' title='Ctrl+Shift+Enter'>\n";}}echo($Ca?"<input type='submit' name='delete' value='".'Delete'."' onclick=\"return confirm('".'Are you sure?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($h==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($qd?html_select("Engine",array(""=>"(".'engine'.")")+$qd,$a["Engine"]):""),' ',($S&&!ereg("sqlite|mssql",$r)?html_select("Collation",array(""=>"(".'collation'.")")+$S,$a["Collation"]):""),' <input type="submit" value="Save">
...
',(support("comment")?checkbox("comments",1,$hb,'Comment',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($hb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Save">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$F,'">
...
as$w){if(count($A)==1||$w!=""){if(!create_database($w,$_POST["collation"])){$xd=false;}$Xa=$w;}}queries_redirect(ME."db=".urlencode($Xa),'Database has been created.',$xd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Database has been altered.');}}page_header(DB!=""?'Alter database':'Create database',$q,array(),DB);$S=collations();$f=DB;$ec=null;if($_POST){$f=$_POST["name"];$ec=$_POST["collation"];}elseif(DB!=""){$ec=db_collation(DB,$S);}elseif($r=="sql"){foreach(get_vals("SHOW GRANTS")as$Q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Q,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($S?html_select("collation",array(""=>"(".'collation'.")")+$S,$ec):"");?>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$F,'">
</form>
...
';if($La){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Save">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$F,'">
</form>
';}elseif(isset($_GET["event"])){$Ua=$_GET["event"];$_d=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$ld=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ua),substr(ME,0,-1),'Event has been dropped.');}elseif(in_array($_POST["INTERVAL_FIELD"],$_d)&&isset($ld[$_POST["STATUS"]])){$Dd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ua!=""?'Event has been altered.':'Event has been created.'),queries(($Ua!=""?"ALTER EVENT ".idf_escape($Ua).$Dd.($Ua!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Dd)."\n".$ld[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ua!=""?'Alter event'.": ".h($Ua):'Create event'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ua!=""){$G=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ua));$a=reset($G);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Every<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_d,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$ld,$a["STATUS"]),'<tr><th>Comment<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Save">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$F,'">
</form>
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
',($kd?'Language'.": ".html_select("language",$kd,$a["language"]):""),'<table cellspacing="0" class="nowrap">
...
<input type="submit" value="Save">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}if($La){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$F,'">
</form>
...
<input type="submit" value="Save">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}if($La){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$F,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Privileges'."</a>";$g=0;foreach($Sa
as$Pa=>$Q){echo'<th>'.($Pa!="*.*"?"<input name='objects[$g]' value='".h($Pa)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Database',"Tables"=>'Table',"Columns"=>'Column',"Procedures"=>'Routine',)as$xc=>$Sb){foreach((array)$ba[$xc]as$Ob=>$cb){echo"<tr".odd()."><td".($Sb?">$Sb<td":" colspan='2'").' lang="en" title="'.h($cb).'">'.h($Ob);$g=0;foreach($Sa
...
<input type="submit" value="Save">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Drop"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$F,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$q){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('%d process has been killed.','%d processes have been killed.'),$jd),$jd||!$_POST["kill"]);}page_header('Process list',$q);echo'
<form action="" method="post">
