...
upload_error($p){$je=($p==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($p?'Unable to upload a file.'.($je?" ".sprintf('Maximum allowed file size is %sB.',$je):""):'File does not exist.');}function
...
lang($Te,$Qb){$Pb=($Qb==1||(!$Qb&&'en'=='fr')?0:((!$Qb||$Qb>=5)&&ereg('cs|sk|ru','en')?2:1));return
...
selectLinks($Dc,$t=""){echo'<p class="tabs">';$ya=array("select"=>'Select data',"table"=>'Show structure');if(is_view($Dc)){$ya["view"]='Alter view';}else{$ya["create"]='Alter table';}if(isset($t)){$ya["edit"]='New item';}foreach($ya
...
selectQuery($j){global$u;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Page'.": ".'last'."'>&gt;&gt;</a> <code class='jush-$u'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Edit'."</a>\n";}function
...
selectColumnsPrint($v,$w){global$Q,$Jb;print_fieldset("select",'Select',$v);$g=0;$oe=array('Functions'=>$Q,'Aggregation'=>$Jb);foreach($v
...
selectSearchPrint($y,$w,$_){print_fieldset("search",'Search',$y);foreach($_
...
selectOrderPrint($Za,$w,$_){print_fieldset("sort",'Sort',$Za);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$g]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'descending')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>",checkbox("desc[$g]",1,0,'descending')."</div>\n","</div></fieldset>\n";}function
...
false;}function
messageQuery($j){global$u;restart_session();$E="sql-".count($_SESSION["messages"]);$Na=&get_session("queries");$Na[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL command'."</a><div id='$E' class='hidden'><pre class='jush-$u'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Na[$_GET["db"]])-1)).'">'.'Edit'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Aa,$n){if($d["type"]=="enum"){return($d["null"]?"<label><input type='radio'$Aa value=''".(isset($n)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Aa value='0'".($n===0?" checked":"")."><i>".'empty'."</i></label>";}return"";}function
...
dumpOutput($v,$n=""){$e=array('text'=>'open','file'=>'save');if(function_exists('gzencode')){$e['gz']='gzip';}if(function_exists('bzcompress')){$e['bz2']='bzip2';}return
...
';if($ub!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$ub){$xa=tables_list();if(!$xa){echo"<p class='message'>".'No tables.'."\n";}else{$this->tablesPrint($xa);$ya=array();foreach($xa
...
as$m=>$O){echo'<a href="'.h(ME).'select='.urlencode($m).'">'.bold('select',$_GET["select"]==$m).'</a> ','<a href="'.h(ME).'table='.urlencode($m).'">'.bold($this->tableName(array("Name"=>$m)),$_GET["table"]==$m)."</a><br>\n";}}}$q=(function_exists('adminer_object')?adminer_object():new
...
edit_type($c,$d,$R,$J=array()){global$gd,$na,$Cb,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($na[$d["type"]])?array():array($d["type"]))+$gd+($J?array('Foreign keys'=>$J):array()),$d["type"]),'</select>
...
',(support("comment")?"<td".($qb?"":" class='hidden'").">".'Comment':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Add next'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($qb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Add next'."' onclick='return !editingAddRow(this, $Ce, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.1' alt='^' title='".'Move up'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.1' alt='v' title='".'Move down'."'>&nbsp;":""),($nd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.1' alt='x' title='".'Remove'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$qb;}function
...
',checkbox("error_stops",1,$_POST["error_stops"],'Stop on error');print_fieldset("webfile",'From server',$_POST["webfile"]);$Yc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Yc[]=".$c";}}echo
sprintf('Webserver file %s',"<code>adminer.sql".($Yc?"[".implode("|",$Yc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Run file'.'">',"</div></fieldset>\n";if($Na){print_fieldset("history",'History',$_GET["history"]!="");foreach($Na
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Edit'."</a> <code class='jush-$u'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Clear'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Ga?"update":"insert"])||$q->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$p&&!isset($_GET["select"])){$U=$_POST["referer"];if($_POST["insert"]){$U=($Ga?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$U)){$U=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $y"),$U,'Item has been deleted.');}else{$t=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$t[idf_escape($f)]=($Ga?"\n".idf_escape($f)." = $b":$b);}}if($Ga){if(!$t){redirect($U);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$t),"\nWHERE $y"),$U,'Item has been updated.');}else{$k=insert_into($h,$t);$Id=($k?last_id():0);queries_redirect($U,sprintf('Item%s has been inserted.',($Id?" $Id":"")),$k);}}}$oa=$q->tableName(table_status($h));page_header(($Ga?'Edit':'Insert'),$p,array("select"=>array($h,$oa)),$oa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($y){$v=array();foreach($o
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($o){echo"<input type='submit' value='".'Save'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($Ga?'Save and continue edit':'Save and insert next')."\">\n";}}if($Ga){echo"<input type='submit' name='delete' value='".'Delete'."'$db>\n";}echo'</form>
...
<?php echo(support("comment")?checkbox("","",$qb,'Comment',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($qb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Save">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Drop"',$db,'>';}if(support("partitioning")){$Ad=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partition by',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Ld,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partitions: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Ad||!$a["partition_by"]?" class='hidden'":""),'>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Drop"',$db,'>';}echo'<input type="hidden" name="token" value="',$C,'">
</form>
';}elseif(isset($_GET["view"])){$h=$_GET["view"];$Ea=false;if($_POST&&!$p){$Ea=drop_create("DROP VIEW ".table($h),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'View has been dropped.','View has been altered.','View has been created.',$h);}page_header(($h!=""?'Alter view':'Create view'),$p,array("table"=>$h),$h);$a=array();if($_POST){$a=$_POST;}elseif($h!=""){$a=view($h);$a["name"]=$h;}echo'
<form action="" method="post">
...
<tr><th>Every<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Fd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Lc,$a["STATUS"]),'<tr><th>Comment<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Save">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Drop"',$db,'>';}echo'</form>
...
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Save">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Drop"',$db,'>';}echo'</form>
';}elseif(isset($_GET["type"])){$sc=$_GET["type"];if($_POST&&!$p){$A=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($sc),$A,'Type has been dropped.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$A,'Type has been created.');}}page_header($sc!=""?'Alter type'.": ".h($sc):'Create type',$p);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$C,'">
';if($sc!=""){echo"<input type='submit' name='drop' value='".'Drop'."'$db>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Save'."'>\n";}echo'</form>
';}elseif(isset($_GET["trigger"])){$h=$_GET["trigger"];$Fc=trigger_options();$zd=array("INSERT","UPDATE","DELETE");$Ea=false;if($_POST&&!$p&&in_array($_POST["Timing"],$Fc["Timing"])&&in_array($_POST["Event"],$zd)&&in_array($_POST["Type"],$Fc["Type"])){$_d=" $_POST[Timing] $_POST[Event]";$eb=" ON ".table($h);$Ea=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$eb:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$eb.$_d:$_d.$eb)." $_POST[Type]\n$_POST[Statement]",ME."table=".urlencode($h),'Trigger has been dropped.','Trigger has been altered.','Trigger has been created.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Alter trigger'.": ".h($_GET["name"]):'Create trigger'),$p,array("table"=>$h));$a=array("Trigger"=>$h."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Time<td>',html_select("Timing",$Fc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($h,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Event<td>',html_select("Event",$zd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Type<td>',html_select("Type",$Fc["Type"],$a["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Save">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Drop"',$db,'>';}echo'</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Privileges'."</a>";$g=0;foreach($Oa
as$Ha=>$N){echo'<th>'.($Ha!="*.*"?"<input name='objects[$g]' value='".h($Ha)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Database',"Tables"=>'Table',"Columns"=>'Column',"Procedures"=>'Routine',)as$Hc=>$Fb){foreach((array)$W[$Hc]as$dc=>$Ya){echo"<tr".odd()."><td".($Fb?">$Fb<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($dc);$g=0;foreach($Oa
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Save">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Drop"',$db,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$p){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('%d process has been killed.','%d processes have been killed.'),$jd),$jd||!$_POST["kill"]);}page_header('Process list',$p);echo'
<form action="" method="post">
...
';}print_fieldset("export",'Export');echo$q->dumpOutput(1,$Oc["output"])." ".$q->dumpFormat(1,$Oc["format"]);echo" <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}print_fieldset("import",'CSV Import',!$D);echo"<input type='hidden' name='token' value='$C'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;"),$Oc["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>\n","</div></fieldset>\n";$q->selectEmailPrint(array_filter($_c,'strlen'),$w);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$tb=isset($_GET["status"]);page_header($tb?'Status':'Variables');$sd=($tb?show_status():show_variables());if(!$sd){echo"<p class='message'>".'No rows.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($sd
