...
set_schema($ye){global$h,$vf,$Qe;$K=$h->query("SET search_path TO ".idf_escape($ye));foreach(types()as$U){if(!isset($vf[$U])){$vf[$U]=0;$Qe['Benutzer-definierte Typen'][]=$U;}}return$K;}function
...
unconvert_field($o,$K){return$K;}function
support($Yb){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Yb);}$y="oracle";$vf=array();$Qe=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'BinÃ¤r'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$z=>$X){$vf+=$X;$Qe[$z]=array_keys($X);}$Bf=array();$Fd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$pc=array("length","lower","round","upper");$rc=array("avg","count","count distinct","max","min","sum");$Bb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$xb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Xd=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($We,$P=""){$a=$We["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Neuer Datensatz'."</a>\n";}function
...
as$D=>$o){if(!isset($o["privileges"][$Cf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$dd=$_POST["referer"];if($_POST["insert"])$dd=($Cf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$dd))$dd=ME."select=".urlencode($a);$x=indexes($a);$yf=unique_array($_GET["where"],$x);$ie="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($dd,'Datensatz gelÃ¶scht.',$wb->delete($a,$ie,!$yf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Cf){if(!$P)redirect($dd);queries_redirect($dd,'Datensatz geÃ¤ndert.',$wb->update($a,$P,$ie,!$yf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$wb->insert($a,$P);$Yc=($J?last_id():0);queries_redirect($dd,sprintf('Datensatz%s hinzugefÃ¼gt.',($Yc?" $Yc":"")),$J);}}}$Xe=$b->tableName(table_status1($a,true));page_header(($Cf?'Ã„ndern':'HinzufÃ¼gen'),$n,array("select"=>array($a,$Xe)),$Xe);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$z=>$X){if(!$Z)$L[$z]=null;$p[$z]=array("field"=>$z,"null"=>($z!=$u),"auto_increment"=>($z==$u));}}}if($L===false)echo"<p class='error'>".'Keine Daten.'."\n";echo'
<div id="message"></div>
...
';if($p){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Cf?'Speichern und weiter bearbeiten'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'Speichern und nÃ¤chsten hinzufÃ¼gen')."' title='Ctrl+Shift+Enter'>\n";}echo($Cf?"<input type='submit' name='delete' value='".'Entfernen'."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
