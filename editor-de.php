...
upload_error($n){$ld=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?'Hochladen von Datei fehlgeschlagen.'.($ld?" ".sprintf('Maximal erlaubte DateigrÃ¶sse ist %sB.',$ld):""):'Datei existiert nicht.');}function
...
lang($of,$yd=null){if(is_array($of)){$Wd=($yd==1?0:1);$of=$of[$Wd];}$of=str_replace("%d","%s",$of);$yd=number_format($yd,0,".",' ');return
...
h($h->error);}function
check_sqlite_name($D){global$h;$Vb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Vb)\$~",$D)){$h->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Vb));return
false;}return
true;}function
create_database($m,$e){global$h;if(file_exists($m)){$h->error='Datei existiert schon.';return
...
as$m){if(!@unlink($m)){$h->error='Datei existiert schon.';return
false;}}return
true;}function
rename_database($D,$e){global$h;if(!check_sqlite_name($D))return
false;$h->Min_SQLite(":memory:");$h->error='Datei existiert schon.';return@rename(DB,$D);}function
...
set_schema($xe){global$h,$uf,$Pe;$K=$h->query("SET search_path TO ".idf_escape($xe));foreach(types()as$U){if(!isset($uf[$U])){$uf[$U]=0;$Pe['Benutzer-definierte Typen'][]=$U;}}return$K;}function
...
selectLinks($Ve,$P=""){$a=$Ve["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Neuer Datensatz'."</a>\n";}function
...
as$f=>$X)$A.="&set".urlencode("[".bracket_escape($f)."]")."=".urlencode($L[$X]);echo"<a href='".h($A)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
...
selectVal($X,$A,$o){$K=($X===null?"&nbsp;":$X);$A=h($A);if(preg_match('~blob|bytea~',$o["type"])&&!is_utf8($X)){$K=lang(array('%d Byte','%d Bytes'),strlen($X));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$X))$K="<img src='$A' alt='$K'>";}if(like_bool($o)&&$K!="&nbsp;")$K=($X?'yes':'no');if($A)$K="<a href='$A'>$K</a>";if(!$A&&!like_bool($o)&&preg_match('~int|float|double|decimal~',$o["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$o["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($Z,$g,$x){$Z=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Suchen'."</legend><div>\n";$Tc=array();foreach($Z
as$z=>$X)$Tc[$X["col"]]=$z;$t=0;$p=fields($_GET["select"]);foreach($g
as$D=>$ob){$o=$p[$D];if(preg_match("~enum~",$o["type"])||like_bool($o)){$z=$Tc[$D];$t--;echo"<div>".h($ob)."<input type='hidden' name='where[$t][col]' value='".h($D)."'>:",(like_bool($o)?" <select name='where[$t][val]'>".optionlist(array(""=>"",'no','yes'),$Z[$z]["val"],true)."</select>":enum_input("checkbox"," name='where[$t][val][]'",$o,(array)$Z[$z]["val"],($o["null"]?0:null))),"</div>\n";unset($g[$D]);}elseif(is_array($Hd=$this->_foreignKeyOptions($_GET["select"],$D))){if($p[$D]["null"])$Hd[0]='('.'leer'.')';$z=$Tc[$D];$t--;echo"<div>".h($ob)."<input type='hidden' name='where[$t][col]' value='".h($D)."'><input type='hidden' name='where[$t][op]' value='='>: <select name='where[$t][val]'>".optionlist($Hd,$Z[$z]["val"],true)."</select></div>\n";unset($g[$D]);}}$t=0;foreach($Z
as$X){if(($X["col"]==""||$g[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$t][col]'><option value=''>(".'beliebig'.")".optionlist($g,$X["col"],true)."</select>",html_select("where[$t][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$t][val]' value='".h($X["val"])."' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";$t++;}}echo"<div><select name='where[$t][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($g,null,true)."</select>",html_select("where[$t][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$t][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($E,$g,$x){$Jd=array();foreach($x
as$z=>$w){$E=array();foreach($w["columns"]as$X)$E[]=$g[$X];if(count(array_filter($E,'strlen'))>1&&$z!="PRIMARY")$Jd[$z]=implode(", ",$E);}if($Jd){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Jd,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
long2str($W,true);}$h='';$uc=$_SESSION["token"];if(!$uc)$_SESSION["token"]=rand(1,1e6);$mf=get_token();$Sd=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($z)=explode(":",$X);$Sd[$z]=$X;}}$Aa=$_POST["auth"];if($Aa){session_regenerate_id();$wb=$Aa["driver"];$O=$Aa["server"];$V=$Aa["username"];$H=$Aa["password"];$m=$Aa["db"];set_password($wb,$O,$V,$H);$_SESSION["db"][$wb][$O][$V][$m]=true;if($Aa["permanent"]){$z=base64_encode($wb)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($m);$be=$b->permanentLogin(true);$Sd[$z]="$z:".base64_encode($be?encrypt_string($H,$be):"");cookie("adminer_permanent",implode(" ",$Sd));}if(count($_POST)==1||DRIVER!=$wb||SERVER!=$O||$_GET["username"]!==$V||DB!=$m)redirect(auth_url($wb,$O,$V,$m));}elseif($_POST["logout"]){if($uc&&!verify_token()){page_header('Abmelden','CSRF Token ungÃ¼ltig. Bitte die Formulardaten erneut abschicken.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$z)set_session($z,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Abmeldung erfolgreich.');}}elseif($Sd&&!$_SESSION["pwds"]){session_regenerate_id();$be=$b->permanentLogin();foreach($Sd
...
as$D=>$o){if(!isset($o["privileges"][$Bf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$dd=$_POST["referer"];if($_POST["insert"])$dd=($Bf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$dd))$dd=ME."select=".urlencode($a);$x=indexes($a);$xf=unique_array($_GET["where"],$x);$he="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($dd,'Datensatz gelÃ¶scht.',$wb->delete($a,$he,!$xf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Bf){if(!$P)redirect($dd);queries_redirect($dd,'Datensatz geÃ¤ndert.',$wb->update($a,$P,$he,!$xf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$wb->insert($a,$P);$Yc=($J?last_id():0);queries_redirect($dd,sprintf('Datensatz%s hinzugefÃ¼gt.',($Yc?" $Yc":"")),$J);}}}$We=$b->tableName(table_status1($a,true));page_header(($Bf?'Ã„ndern':'HinzufÃ¼gen'),$n,array("select"=>array($a,$We)),$We);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$z=>$X){if(!$Z)$L[$z]=null;$p[$z]=array("field"=>$z,"null"=>($z!="itemName()"),"auto_increment"=>($z=="itemName()"));}}}if($L===false)echo"<p class='error'>".'Keine Daten.'."\n";echo'
<div id="message"></div>
...
';if($p){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Bf?'Speichern und weiter bearbeiten'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'Speichern und nÃ¤chsten hinzufÃ¼gen')."' title='Ctrl+Shift+Enter'>\n";}echo($Bf?"<input type='submit' name='delete' value='".'Entfernen'."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
