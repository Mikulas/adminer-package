...
upload_error($m){$pd=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Hochladen von Datei fehlgeschlagen.'.($pd?" ".sprintf('Maximal erlaubte Dateigröße ist %sB.',$pd):""):'Datei existiert nicht.');}function
...
lang($vf,$Cd=null){if(is_array($vf)){$be=($Cd==1?0:1);$vf=$vf[$be];}$vf=str_replace("%d","%s",$vf);$Cd=format_number($Cd);return
...
h($g->error);}function
check_sqlite_name($C){global$g;$Xb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Xb)\$~",$C)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Xb));return
false;}return
true;}function
...
set_schema($Ce){global$g,$Bf,$We;$K=$g->query("SET search_path TO ".idf_escape($Ce));foreach(types()as$U){if(!isset($Bf[$U])){$Bf[$U]=0;$We['Benutzerdefinierte Typen'][]=$U;}}return$K;}function
...
selectLinks($cf,$P=""){$a=$cf["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Neuer Datensatz'."</a>\n";}function
...
as$e=>$X)$_.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($L[$X]);echo"<a href='".h($_)."' title='".'Neuer Datensatz'."'>+</a> ";}}}function
...
selectVal($X,$_,$n,$Pd){$K=($X===null?"&nbsp;":$X);$_=h($_);if(preg_match('~blob|bytea~',$n["type"])&&!is_utf8($X)){$K=lang(array('%d Byte','%d Bytes'),strlen($Pd));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$Pd))$K="<img src='$_' alt='$K'>";}if(like_bool($n)&&$K!="&nbsp;")$K=($X?'ja':'nein');if($_)$K="<a href='$_'".(is_url($_)?" rel='noreferrer'":"").">$K</a>";if(!$_&&!like_bool($n)&&preg_match('~int|float|double|decimal~',$n["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$n["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($Z,$f,$w){$Z=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Suchen'."</legend><div>\n";$Xc=array();foreach($Z
as$y=>$X)$Xc[$X["col"]]=$y;$s=0;$o=fields($_GET["select"]);foreach($f
as$C=>$tb){$n=$o[$C];if(preg_match("~enum~",$n["type"])||like_bool($n)){$y=$Xc[$C];$s--;echo"<div>".h($tb)."<input type='hidden' name='where[$s][col]' value='".h($C)."'>:",(like_bool($n)?" <select name='where[$s][val]'>".optionlist(array(""=>"",'nein','ja'),$Z[$y]["val"],true)."</select>":enum_input("checkbox"," name='where[$s][val][]'",$n,(array)$Z[$y]["val"],($n["null"]?0:null))),"</div>\n";unset($f[$C]);}elseif(is_array($Md=$this->_foreignKeyOptions($_GET["select"],$C))){if($o[$C]["null"])$Md[0]='('.'leer'.')';$y=$Xc[$C];$s--;echo"<div>".h($tb)."<input type='hidden' name='where[$s][col]' value='".h($C)."'><input type='hidden' name='where[$s][op]' value='='>: <select name='where[$s][val]'>".optionlist($Md,$Z[$y]["val"],true)."</select></div>\n";unset($f[$C]);}}$s=0;foreach($Z
as$X){if(($X["col"]==""||$f[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$s][col]'><option value=''>(".'beliebig'.")".optionlist($f,$X["col"],true)."</select>",html_select("where[$s][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$s][val]' value='".h($X["val"])."' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";$s++;}}echo"<div><select name='where[$s][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($f,null,true)."</select>",html_select("where[$s][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$s][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($E,$f,$w){$Od=array();foreach($w
as$y=>$v){$E=array();foreach($v["columns"]as$X)$E[]=$f[$X];if(count(array_filter($E,'strlen'))>1&&$y!="PRIMARY")$Od[$y]=implode(", ",$E);}if($Od){echo'<fieldset><legend>'.'Ordnen'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Od,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
selectLimitPrint($z){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo
html_select("limit",array("","50","100"),$z),"</div></fieldset>\n";}function
selectLengthPrint($jf){}function
selectActionPrint($w){echo"<fieldset><legend>".'Aktion'."</legend><div>","<input type='submit' value='".'Daten zeigen von'."'>","</div></fieldset>\n";}function
selectCommandPrint(){return
true;}function
selectImportPrint(){return
true;}function
selectEmailPrint($Jb,$f){if($Jb){print_fieldset("email",'E-Mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Von'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Betreff'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Einfügen'."'>\n";echo"<p>".'Anhänge'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($Jb)==1?'<input type="hidden" name="email_field" value="'.h(key($Jb)).'">':html_select("email_field",$Jb)),"<input type='submit' name='email' value='".'Abschicken'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
...
';if($wd=="auth"){$fc=true;foreach((array)$_SESSION["pwds"]as$Qf=>$Me){foreach($Me[""]as$V=>$H){if($H!==null){if($fc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$fc=false;}echo"<a href='".h(auth_url($Qf,"",$V))."'>".($V!=""?h($V):"<i>".'leer'."</i>")."</a><br>\n";}}}}else{$this->databasesPrint($wd);if($wd!="db"&&$wd!="ns"){$S=table_status('',true);if(!$S)echo"<p class='message'>".'Keine Tabellen.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($wd){}function
tablesPrint($T){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($T
as$L){$C=$this->tableName($L);if(isset($L["Engine"])&&$C!="")echo"<a href='".h(ME).'select='.urlencode($L["Name"])."'".bold($_GET["select"]==$L["Name"]||$_GET["edit"]==$L["Name"],"select")." title='".'Daten auswählen'."'>$C</a><br>\n";}}function
...
as$Qc=>$X){if($X[0]<$lf)unset($Pc[$Qc]);}}$Oc=&$Pc[$b->bruteForceKey()];if(!$Oc)$Oc=array($lf+30*60,0);$Oc[1]++;$Ke=serialize($Pc);rewind($oc);fwrite($oc,$Ke);ftruncate($oc,strlen($Ke));flock($oc,LOCK_UN);fclose($oc);}$Ca=$_POST["auth"];if($Ca){$Pc=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$Oc=$Pc[$b->bruteForceKey()];$Ad=($Oc[1]>30?$Oc[0]-time():0);if($Ad>0)auth_error(lang(array('Zu viele erfolglose Login-Versuche. Bitte probieren Sie es in %d Minute noch einmal.','Zu viele erfolglose Login-Versuche. Bitte probieren Sie es in %d Minuten noch einmal.'),ceil($Ad/60)));session_regenerate_id();$Qf=$Ca["driver"];$O=$Ca["server"];$V=$Ca["username"];$H=(string)$Ca["password"];$l=$Ca["db"];set_password($Qf,$O,$V,$H);$_SESSION["db"][$Qf][$O][$V][$l]=true;if($Ca["permanent"]){$y=base64_encode($Qf)."-".base64_encode($O)."-".base64_encode($V)."-".base64_encode($l);$he=$b->permanentLogin(true);$Yd[$y]="$y:".base64_encode($he?encrypt_string($H,$he):"");cookie("adminer_permanent",implode(" ",$Yd));}if(count($_POST)==1||DRIVER!=$Qf||SERVER!=$O||$_GET["username"]!==$V||DB!=$l)redirect(auth_url($Qf,$O,$V,$l));}elseif($_POST["logout"]){if($wc&&!verify_token()){page_header('Abmelden','CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Abmeldung erfolgreich.');}}elseif($Yd&&!$_SESSION["pwds"]){session_regenerate_id();$he=$b->permanentLogin();foreach($Yd
...
auth_error($m){global$b,$wc;$m=h($m);$Ne=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Ne]||$_GET[$Ne])&&!$wc)$m='Sitzungsdauer abgelaufen, bitte erneut anmelden.';else{add_invalid_login();$H=get_password();if($H!==null){if($H===false)$m.='<br>'.sprintf('Das Master-Passwort ist abgelaufen.  <a href="http://www.adminer.org/de/extension/" target="_blank">Implementieren</a> Sie die %s Methode, um es permanent zu machen.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Ne]&&$_GET[$Ne]&&ini_bool("session.use_only_cookies"))$m='Unterstüzung für PHP-Sessions muss aktiviert sein.';$G=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$G["lifetime"]);page_header('Login',$m,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstützten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$ce)),false);page_footer("auth");exit;}$g=connect();}$_b=new
Min_Driver($g);if(!is_object($g)||!$b->login($_GET["username"],get_password()))auth_error((is_string($g)?$g:'Ungültige Anmelde-Informationen.'));if($Ca&&$_POST["token"])$_POST["token"]=$tf;$m='';if($_POST){if(!verify_token()){$Kc="max_input_vars";$qd=ini_get($Kc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$qd||$X<$qd)){$Kc=$y;$qd=$X;}}}$m=(!$_POST["token"]&&$qd?sprintf('Die maximal erlaubte Anzahl der Felder ist überschritten. Bitte %s erhöhen.',"'$Kc'"):'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.'.' '.'Wenn Sie diese Anfrage nicht von Adminer gesendet haben, schließen Sie diese Seite.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('POST-Daten sind zu groß. Reduzieren Sie die Größe oder vergrößern Sie den Wert %s in der Konfiguration.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'Sie können eine große SQL-Datei per FTP hochladen und dann vom Server importieren.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
as$Tc=>$W)$_.=where_link($s++,$Tc,$W);}$X=select_value($X,$_,$n,$jf);$t=h("val[$Ff][".bracket_escape($y)."]");$Y=$_POST["val"][$Ff][bracket_escape($y)];$Gb=!is_array($L[$y])&&is_utf8($X)&&$M[$B][$y]==$L[$y]&&!$qc[$y];$if=preg_match('~text|lob~',$n["type"]);if(($_GET["modify"]&&$Gb)||$Y!==null){$tc=h($Y!==null?$Y:$L[$y]);echo"<td>".($if?"<textarea name='$t' cols='30' rows='".(substr_count($L[$y],"\n")+1)."'>$tc</textarea>":"<input name='$t' value='$tc' size='$fd[$y]'>");}else{$jd=strpos($X,"<i>...</i>");echo"<td id='$t' onclick=\"selectClick(this, event, ".($jd?2:($if?1:0)).($Gb?"":", '".h('Benutzen Sie den Link zum Bearbeiten dieses Wertes.')."'").");\">$X";}}}if($Ia)echo"<td>";$b->backwardKeysPrint($Ia,$M[$B]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($M||$F)&&!is_ajax()){$Sb=true;if($_GET["page"]!="last"){if(!+$z)$nc=count($M);elseif($x!="sql"||!$Rc){$nc=($Rc?false:found_rows($S,$Z));if($nc<max(1e4,2*($F+1)*$z))$nc=reset(slow_query(count_rows($a,$Z,$Rc,$r)));else$Sb=false;}}if(+$z&&($nc===false||$nc>$z||$F)){echo"<p class='pages'>";$od=($nc===false?$F+(count($M)>=$z?2:1):floor(($nc-1)/$z));if($x!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Seite'."', '".($F+1)."'), event); return false;\">".'Seite'."</a>:",pagination(0,$F).($F>5?" ...":"");for($s=max(1,$F-4);$s<min($od,$F+5);$s++)echo
pagination($s,$F);if($od>0){echo($F+5<$od?" ...":""),($Sb&&$nc!==false?pagination($od,$F):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$od'>".'letzte'."</a>");}echo(($nc===false?count($M)+1:$nc-$F*$z)>$z?' <a href="'.h(remove_from_uri("page")."&page=".($F+1)).'" onclick="return !selectLoadMore(this, '.(+$z).', \''.'Lade'.'...\');" class="loadmore">'.'Mehr Daten laden'.'</a>':'');}else{echo'Seite'.":",pagination(0,$F).($F>1?" ...":""),($F?pagination($F,$F):""),($od>$F?pagination($F+1,$F).($od>$F+1?" ...":""):"");}}echo"<p class='count'>\n",($nc!==false?"(".($Sb?"":"~ ").lang(array('%d Datensatz','%d Datensätze'),$nc).") ":"");$yb=($Sb?"":"~ ").$nc;echo
checkbox("all",1,0,'Gesamtergebnis',"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$yb' : checked); selectCount('selected2', this.checked || !checked ? '$yb' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Ändern</legend><div>
