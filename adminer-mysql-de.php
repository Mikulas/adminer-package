...
confirm($Mc="",$Se=false){return" onclick=\"".($Se?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Mc?" (' + $Mc + ')":"")."');\"";}function
...
upload_error($q){$be=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Hochladen von Datei fehlgeschlagen.'.($be?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$be):""):'Datei existiert nicht.');}function
...
enum_input($O,$Ea,$d,$o,$Eb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$ea);$e=(isset($Eb)?"<label><input type='$O'$Ea value='$Eb'".((is_array($o)?in_array($Eb,$o):$o===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($ea[1]as$g=>$b){$b=stripcslashes(str_replace("''","'",$b));$Qa=(is_int($o)?$o==$g+1:(is_array($o)?in_array($g+1,$o):$o===$b));$e.=" <label><input type='$O'$Ea value='".($g+1)."'".($Qa?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
...
selectLinks($Ic,$u=""){echo'<p class="tabs">';$ga=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Ic)){$ga["view"]='View ändern';}else{$ga["create"]='Tabelle ändern';}if(isset($u)){$ga["edit"]='Neuer Datensatz';}foreach($ga
...
selectColumnsPrint($z,$w){global$X,$Cb;print_fieldset("select",'Daten zeigen von',$z);$g=0;$Rd=array('Funktionen'=>$X,'Agregationen'=>$Cb);foreach($z
...
selectSearchPrint($x,$w,$y){print_fieldset("search",'Suchen',$x);foreach($y
...
selectOrderPrint($Ga,$w,$y){print_fieldset("sort",'Ordnen',$Ga);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$g]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'absteigend')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>","<label><input type='checkbox' name='desc[$g]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($W){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($W)."'>","</div></fieldset>\n";}function
selectLengthPrint($qb){if(isset($qb)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($qb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$r;static$Mc=0;restart_session();$E="sql-".($Mc++);$Pa=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Pa[$_GET["db"]][]=$j;return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL-Query'."</a><div id='$E' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Pa[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($_e[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($sa[$c]==254){$b="<code>$b</code>";}}if(isset($ga[$c])&&!$w[$ga[$c]]){if($zc){$B=$ga[$c].urlencode($a[array_search("table=",$ga)]);}else{$B="edit=".urlencode($ga[$c]);foreach($y[$ga[$c]]as$Ac=>$T){$B.="&where".urlencode("[".bracket_escape($Ac)."]")."=".urlencode($a[$T]);}}$b="<a href='".h(ME.$B)."'>$b</a>";}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($c,$d,$S,$M=array()){global$Jc,$sa,$Bb,$La;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($sa[$d["type"]])?array():array($d["type"]))+$Jc+($M?array('Fremdschlüssel'=>$M):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($S,$d["collation"]).'</select>',($Bb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Bb,$d["unsigned"]).'</select>':''),($M?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($La,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ib?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($ib?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $Ie, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.1' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.1' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Qc==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.1' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$c=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($c)."<td><a href='".h(ME.'trigger='.urlencode($h).'&name='.urlencode($c))."'>".'Ändern'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($h).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB);$Za=array();$Fe=array();$f="adminer_schema";$we=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$we,$ea,PREG_SET_ORDER);foreach($ea
...
<p><a href="',h(ME."schema=".urlencode($we)),'" id="schema-link">Dauerhafter Link</a>
...
<input type="hidden" name="token" value="',$D,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Im Server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Zc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Zc[]=".$c";}}echo
...
as$f=>$d){if(!isset($d["privileges"][$Da?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$N=$_POST["referer"];if($_POST["insert"]){$N=($Da?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$N)){$N=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$N,'Datensatz gelöscht.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Da?"\n".idf_escape($f)." = $b":$b);}}if($Da){if(!$u){redirect($N);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$N,'Datensatz geändert.');}else{$k=insert_into($h,$u);$Fd=($k?last_id():0);queries_redirect($N,sprintf('Datensatz%s hinzugefügt.',($Fd?" $Fd":"")),$k);}}}$ua=$n->tableName(table_status($h));page_header(($Da?'Ändern':'Hinzufügen'),$q,array("select"=>array($h,$ua)),$ua);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$z=array();foreach($p
...
';if($p){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Da?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}}echo($Da?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($h==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Vc?html_select("Engine",array(""=>"(".'Motor'.")")+$Vc,$a["Engine"]):""),' ',($S&&!ereg("sqlite|mssql",$r)?html_select("Collation",array(""=>"(".'Kollation'.")")+$S,$a["Collation"]):""),' <input type="submit" value="Speichern">
...
',(support("comment")?checkbox("comments",1,$ib,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ib?'':' class="hidden"').'>':''),'<p>
...
';}elseif(isset($_GET["event"])){$Ra=$_GET["event"];$yd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$qd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ra),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$yd)&&isset($qd[$_POST["STATUS"]])){$zd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ra!=""?'Ereignis geändert.':'Ereignis erstellt.'),queries(($Ra!=""?"ALTER EVENT ".idf_escape($Ra).$zd.($Ra!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$zd)."\n".$qd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ra!=""?'Ereignis ändern'.": ".h($Ra):'Ereignis erstellen'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ra!=""){$F=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ra));$a=reset($F);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$yd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$qd,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Speichern">
';if($Ra!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
...
as$Ja=>$Q){echo'<th>'.($Ja!="*.*"?"<input name='objects[$g]' value='".h($Ja)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$xc=>$Nb){foreach((array)$aa[$xc]as$Ub=>$db){echo"<tr".odd()."><td".($Nb?">$Nb<td":" colspan='2'").' lang="en" title="'.h($db).'">'.h($Ub);$g=0;foreach($Ta
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$q){$od=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$od++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$od),$od||!$_POST["kill"]);}page_header('Prozessliste',$q);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$Ia,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Exportieren');$za=$n->dumpOutput();echo($za?html_select("output",$za,$kc["output"])." ":""),html_select("format",$n->dumpFormat(),$kc["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($n->selectImportPrint()){print_fieldset("import",'Importieren',!$F);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$kc["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$D'>\n","</div></fieldset>\n";}$n->selectEmailPrint(array_filter($lc,'strlen'),$w);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$yb=isset($_GET["status"]);page_header($yb?'Status':'Variablen');$ce=($yb?show_status():show_variables());if(!$ce){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($ce
