...
confirm($Ec="",$kf=false){return" onclick=\"".($kf?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Ec?" (' + $Ec + ')":"")."');\"";}function
js_escape($T){return
addcslashes($T,"\r\n'\\/");}function
...
upload_error($q){$le=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Hochladen von Datei fehlgeschlagen.'.($le?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$le):""):'Datei existiert nicht.');}function
...
enum_input($N,$Ca,$d,$o,$Bb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$fa);$e=(isset($Bb)?"<label><input type='$N'$Ca value='$Bb'".((is_array($o)?in_array($Bb,$o):$o===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($fa[1]as$h=>$b){$b=stripcslashes(str_replace("''","'",$b));$Ja=(is_int($o)?$o==$h+1:(is_array($o)?in_array($h+1,$o):$o===$b));$e.=" <label><input type='$N'$Ca value='".($h+1)."'".($Ja?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
...
lang($tf,$wb){$cc=($wb==1||(!$wb&&'de'=='fr')?0:('de'=='sl'&&(!$wb||$wb>2)?1:0)+((!$wb||$wb>=5)&&ereg('cs|sk|ru|sl|pl','de')?2:1));return
...
<tr><th>Datenbank System<td>',html_select("driver",$ra,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'">
...
selectLinks($Jc,$u=""){echo'<p class="tabs">';$za=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Jc)){$za["view"]='View ändern';}else{$za["create"]='Tabelle ändern';}if(isset($u)){$za["edit"]='Neuer Datensatz';}foreach($za
...
selectColumnsPrint($C,$w){global$W,$zb;print_fieldset("select",'Daten zeigen von',$C);$h=0;$Vd=array('Funktionen'=>$W,'Agregationen'=>$zb);foreach($C
...
selectSearchPrint($x,$w,$z){print_fieldset("search",'Suchen',$x);foreach($z
as$h=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$h]' value='".h($_GET["fulltext"][$h])."'>",checkbox("boolean[$h]",1,isset($_GET["boolean"][$h]),"BOOL"),"<br>\n";}}$h=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$h][col]'><option value=''>(".'beliebig'.")".optionlist($w,$b["col"],true)."</select>",html_select("where[$h][op]",$this->operators,$b["op"]),"<input name='where[$h][val]' value='".h($b["val"])."'></div>\n";$h++;}}echo"<div><select name='where[$h][col]' onchange='selectAddRow(this);'><option value=''>(".'beliebig'.")".optionlist($w,null,true)."</select>",html_select("where[$h][op]",$this->operators,"="),"<input name='where[$h][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Fa,$w,$z){print_fieldset("sort",'Ordnen',$Fa);$h=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$h]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$h]",1,isset($_GET["desc"][$c]),'absteigend')."</div>\n";$h++;}}echo"<div><select name='order[$h]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>","<label><input type='checkbox' name='desc[$h]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($R){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($R)."'>","</div></fieldset>\n";}function
selectLengthPrint($kb){if(isset($kb)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($kb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$r;static$Ec=0;restart_session();$D="sql-".($Ec++);$Oa=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Oa[$_GET["db"]][]=$j;return" <a href='#$D' onclick=\"return !toggle('$D');\">".'SQL-Query'."</a><div id='$D' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Oa[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
';if($Ib!="db"&&DB!=""&&$g->select_db(DB)){if($_GET["ns"]!==""&&!$Ib){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle'."</a>\n";$la=tables_list();if(!$la){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($la);$za=array();foreach($la
...
auth_error($wd=null){global$g,$n,$B;$Cc=session_name();$q="";if(!$_COOKIE[$Cc]&&$_GET[$Cc]&&ini_bool("session.use_only_cookies")){$q='Sitzungen müssen aktiviert sein.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Cc]||$_GET[$Cc])&&!$B){$q='Sitzungsdauer abgelaufen, bitte erneut anmelden.';}else{$ja=&get_session("pwds");if(isset($ja)){$q=h($wd?$wd->getMessage():(is_string($g)?$g:'Ungültige Anmelde-Informationen.'));$ja=null;}}}page_header('Login',$q,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$n->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstützten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$df)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$n->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$B=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$B;}$q=($_POST?($_POST["token"]==$B?"":'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('POST data zu gross. Reduzieren Sie die Grösse oder vergrössern Sie den Wert %s in der Konfiguration.','"post_max_size"')));function
connect_error(){global$g,$B,$q,$ra;$A=array();if(DB!=""){page_header('Datenbank'.": ".h(DB),'Datenbank ungültig.',true);}else{if($_POST["db"]&&!$q){queries_redirect(substr(ME,0,-1),'Datenbanken entfernt.',drop_databases($_POST["db"]));}page_header('Datenbank auswählen',$q,false);echo"<p><a href='".h(ME)."database='>".'Neue Datenbank'."</a>\n";foreach(array('privileges'=>'Rechte','processlist'=>'Prozessliste','variables'=>'Variablen','status'=>'Status',)as$c=>$b){if(support($c)){echo"<a href='".h(ME)."$c='>$b</a>\n";}}echo"<p>".sprintf('Version %s: %s, mit PHP-Erweiterung %s',$ra[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Angemeldet als: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"]){set_session("dbs",null);}$A=get_databases();if($A){$gf=support("scheme");$S=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td>&nbsp;<th>".'Datenbank'."<td>".'Collation'."<td>".'Tabellen'."</thead>\n";foreach($A
...
edit_type($c,$d,$S,$P=array()){global$Gc,$ba,$Db,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ba[$d["type"]])?array():array($d["type"]))+$Gc+($P?array('Fremdschlüssel'=>$P):array()),$d["type"]),'</select>
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$h][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $pe, 1);'>&nbsp;"."<input type='image' name='up[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.2' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.2' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Tc==""||support("drop_col")?"<input type='image' name='drop_col[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.2' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$ob;}function
...
<input type="hidden" name="token" value="',$B,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Auf Server',$_POST["webfile"]);$Uc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Uc[]=".$c";}}echo
...
',(support("comment")?checkbox("","",$ob,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
...
';}elseif(isset($_GET["foreign"])){$i=$_GET["foreign"];if($_POST&&!$q&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"]){query_redirect("ALTER TABLE ".table($i)."\nDROP ".($r=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($i),'Fremdschlüssel entfernt.');}else{$ua=array_filter($_POST["source"],'strlen');ksort($ua);$qa=array();foreach($ua
as$c=>$b){$qa[$c]=$_POST["target"][$c];}query_redirect("ALTER TABLE ".table($i).($_GET["name"]!=""?"\nDROP FOREIGN KEY ".idf_escape($_GET["name"]).",":"")."\nADD FOREIGN KEY (".implode(", ",array_map('idf_escape',$ua)).") REFERENCES ".table($_POST["table"])." (".implode(", ",array_map('idf_escape',$qa)).")".(in_array($_POST["on_delete"],$Ma)?" ON DELETE $_POST[on_delete]":"").(in_array($_POST["on_update"],$Ma)?" ON UPDATE $_POST[on_update]":""),ME."table=".urlencode($i),($_GET["name"]!=""?'Fremdschlüssel geändert.':'Fremdschlüssel erstellt.'));$q='Spalten des Ursprungs und des Zieles müssen vom gleichen Datentyp sein, es muss unter den Zielspalten ein Index existieren und die referenzierten Daten müssen existieren.'."<br>$q";}}page_header('Fremdschlüssel',$q,array("table"=>$i),$i);$a=array("table"=>$i,"source"=>array(""));if($_POST){$a=$_POST;ksort($a["source"]);if($_POST["add"]){$a["source"][]="";}elseif($_POST["change"]||$_POST["change-js"]){$a["target"]=array();}}elseif($_GET["name"]!=""){$P=foreign_keys($i);$a=$P[$_GET["name"]];$a["source"][]="";}$ua=array_keys(fields($i));$qa=($i===$a["table"]?$ua:array_keys(fields($a["table"])));$Nd=array();foreach(table_status()as$f=>$H){if(fk_support($H)){$Nd[]=$f;}}echo'
<form action="" method="post">
<p>
...
';if($Ka){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["event"])){$Ua=$_GET["event"];$Md=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$jd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ua),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Md)&&isset($jd[$_POST["STATUS"]])){$Od="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ua!=""?'Ereignis geändert.':'Ereignis erstellt.'),queries(($Ua!=""?"ALTER EVENT ".idf_escape($Ua).$Od.($Ua!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Od)."\n".$jd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ua!=""?'Ereignis ändern'.": ".h($Ua):'Ereignis erstellen'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ua!=""){$E=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ua));$a=reset($E);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Md,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$jd,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Speichern">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$yc["Timing"],$a["Timing"],"if (/^".h(preg_quote($i,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($i))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Sd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$yc["Type"],$a["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($Ka){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$h=0;foreach($Ta
as$Qa=>$Q){echo'<th>'.($Qa!="*.*"?"<input name='objects[$h]' value='".h($Qa)."' size='10'>":"<input type='hidden' name='objects[$h]' value='*.*' size='10'>*.*");$h++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Bc=>$Pb){foreach((array)$Y[$Bc]as$Ub=>$cb){echo"<tr".odd()."><td".($Pb?">$Pb<td":" colspan='2'").' lang="en" title="'.h($cb).'">'.h($Ub);$h=0;foreach($Ta
...
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$ld=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$ld++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$ld),$ld||!$_POST["kill"]);}page_header('Prozessliste',$q);echo'
<form action="" method="post">
...
as$c=>$b){echo"<td>".($c=="Info"&&$b!=""?"<code class='jush-$r'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Ändern'.'</a>':nbsp($b));}echo"\n";}echo'</table>
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$Ha,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Exportieren');$Aa=$n->dumpOutput();echo($Aa?html_select("output",$Aa,$Hb["output"])." ":""),html_select("format",$n->dumpFormat(),$Hb["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}print_fieldset("import",'Importieren',!$E);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Hb["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$B'>\n","</div></fieldset>\n";$n->selectEmailPrint(array_filter($mc,'strlen'),$w);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ab=isset($_GET["status"]);page_header($Ab?'Status':'Variablen');$he=($Ab?show_status():show_variables());if(!$he){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($he
