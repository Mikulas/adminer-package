...
upload_error($j){$md=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Hochladen von Datei fehlgeschlagen.'.($md?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$md):""):'Datei existiert nicht.');}function
...
selectLinks($pf,$O=""){echo'<p class="tabs">';$ed=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($pf))$ed["view"]='View ändern';else$ed["create"]='Tabelle ändern';if($O!==null)$ed["edit"]='Neuer Datensatz';foreach($ed
...
selectQuery($G){global$v;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a> <code class='jush-$v'>".h(str_replace("\n"," ",$G))."</code> <a href='".h(ME)."sql=".urlencode($G)."'>".'Ändern'."</a></p>\n";}function
...
selectColumnsPrint($M,$e){global$rc,$vc;print_fieldset("select",'Daten zeigen von',$M);$q=0;$qc=array('Funktionen'=>$rc,'Agregationen'=>$vc);foreach($M
...
selectSearchPrint($Z,$e,$t){print_fieldset("search",'Suchen',$Z);foreach($t
as$q=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ia="this.nextSibling.onchange();";for($q=0;$q<=count($_GET["where"]);$q++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div><select name='where[$q][col]' onchange='$Ia'><option value=''>(".'beliebig'.")".optionlist($e,$X["col"],true)."</select>",html_select("where[$q][op]",$this->operators,$X["op"],$Ia),"<input name='where[$q][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Od,$e,$t){print_fieldset("sort",'Ordnen',$Od);$q=0;foreach((array)$_GET["order"]as$w=>$X){if(isset($e[$X])){echo"<div><select name='order[$q]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'absteigend')."</div>\n";$q++;}}echo"<div><select name='order[$q]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$q]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($_f){if($_f!==null){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($_f).'">',"</div></fieldset>\n";}}function
...
messageQuery($G){global$v;static$eb=0;restart_session();$r="sql-".($eb++);$xc=&get_session("queries");if(strlen($G)>1e6)$G=ereg_replace('[\x80-\xFF]+$','',substr($G,0,1e6))."\n...";$xc[$_GET["db"]][]=array($G,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL-Query'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($G,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($xc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
dumpOutput(){$I=array('text'=>'anzeigen','file'=>'Datei');if(function_exists('gzencode'))$I['gz']='gzip';if(function_exists('bzcompress'))$I['bz2']='bzip2';return$I;}function
dumpFormat(){return
...
as$w=>$X){if($X===null)$X="<i>NULL</i>";elseif($Da[$w]&&!is_utf8($X))$X="<i>".lang(array('%d Byte','%d Bytes'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Qf[$w]==254)$X="<code>$X</code>";}if(isset($ed[$w])&&!$e[$ed[$w]]){if($_c){$R=$J[array_search("table=",$ed)];$y=$ed[$w].urlencode($Rd[$R]!=""?$Rd[$R]:$R);}else{$y="edit=".urlencode($ed[$w]);foreach($t[$ed[$w]]as$Pa=>$u)$y.="&where".urlencode("[".bracket_escape($Pa)."]")."=".urlencode($J[$u]);}$X="<a href='".h(ME.$y)."'>$X</a>";}echo"<td>$X";}}echo($q?"</table>":"<p class='message'>".'Keine Daten.')."\n";return$I;}function
...
edit_type($w,$k,$d,$n=array()){global$if,$Qf,$Xf,$Hd;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($Qf[$k["type"]])?array():array($k["type"]))+$if+($n?array('Fremdschlüssel'=>$n):array()),$k["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($d,$k["collation"]).'</select>',($Xf?"<select name='$w"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Xf,$k["unsigned"]).'</select>':''),($n?"<select name='$w"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Hd),$k["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Xa?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Xa?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($k["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.1' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.1' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Sd==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.1' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$T,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Im Server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ya=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$X){if(extension_loaded($X))$Ya[]=".$w";}echo
...
as$w=>$X){list($F,$Af)=$X;echo'<a href="'.h(ME."sql=&history=$w").'">'.'Ändern'."</a> <span class='time'>".@date("H:i:s",$Af)."</span> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$F)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$C=>$k){if(!isset($k["privileges"][$Yf?"update":"insert"])||$b->fieldName($k)=="")unset($l[$C]);}if($_POST&&!$j&&!isset($_GET["select"])){$z=$_POST["referer"];if($_POST["insert"])$z=($Yf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$z))$z=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$z,'Datensatz gelöscht.');else{$O=array();foreach($l
as$C=>$k){$X=process_input($k);if($X!==false&&$X!==null)$O[idf_escape($C)]=($Yf?"\n".idf_escape($C)." = $X":$X);}if($Yf){if(!$O)redirect($z);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$z,'Datensatz geändert.');}else{$H=insert_into($a,$O);$Wc=($H?last_id():0);queries_redirect($z,sprintf('Datensatz%s hinzugefügt.',($Wc?" $Wc":"")),$H);}}}$qf=$b->tableName(table_status($a));page_header(($Yf?'Ändern':'Hinzufügen'),$j,array("select"=>array($a,$qf)),$qf);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($Z){$M=array();foreach($l
as$C=>$k){if(isset($k["privileges"]["select"])){$ta=convert_field($k);if($_POST["clone"]&&$k["auto_increment"])$ta="''";if($v=="sql"&&ereg("enum|set",$k["type"]))$ta="1*".idf_escape($C);$M[]=($ta?"$ta AS ":"").idf_escape($C);}}$J=array();if($M){$K=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false)echo"<p class='error'>".'Keine Daten.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
<p>
Auto-Inkrement: <input name="Auto_increment" size="6" value="',h($J["Auto_increment"]),'">
...
',(support("comment")?checkbox("comments",1,$Xa,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($J["Comment"]).'" maxlength="60"'.($Xa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
';if(support("partitioning")){$de=ereg('RANGE|LIST',$J["partition_by"]);print_fieldset("partition",'Partitionieren um',$J["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$ce,$J["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($J["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($J["partitions"]),'"',($de||!$J["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$j&&!isset($_POST["add_x"])){restart_session();$C=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($C),'Datenbank umbenannt.',rename_database($C,$_POST["collation"]));}else{$h=explode("\n",str_replace("\r","",$C));$jf=true;$Vc="";foreach($h
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$jf=false;$Vc=$i;}}queries_redirect(ME."db=".urlencode($Vc),'Datenbank erstellt.',$jf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$j,array(),DB);$d=collations();$C=DB;$Qa=null;if($_POST){$C=$_POST["name"];$Qa=$_POST["collation"];}elseif(DB!="")$Qa=db_collation(DB,$d);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$_)&&$_[1]){$C=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$Ab=false;if($_POST&&!$j){$C=trim($_POST["name"]);$Ab=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($C)),'View entfernt.','View geändert.','View erstellt.',$a);}page_header(($a!=""?'View ändern':'Neue View erstellen'),$j,array("table"=>$a),$a);$J=$_POST;if(!$J&&$a!=""){$J=view($a);$J["name"]=$a;}echo'
<form action="" method="post">
...
<tr><th>Ende<td><input name="ENDS" value="',h($J["ENDS"]),'">
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Jc,$J["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$gf,$J["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'Nach der Ausführung erhalten'),'</table>
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($J["name"]),'" maxlength="64">
',($Pe?'Sprache'.": ".html_select("language",$Pe,$J["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$d,$Oe);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$J["returns"],$d);}echo'</table>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$Mf=trigger_options();$Lf=array("INSERT","UPDATE","DELETE");$Ab=false;if($_POST&&!$j&&in_array($_POST["Timing"],$Mf["Timing"])&&in_array($_POST["Event"],$Lf)&&in_array($_POST["Type"],$Mf["Type"])){$Bf=" $_POST[Timing] $_POST[Event]";$Gd=" ON ".table($a);$Ab=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($v=="pgsql"?$Gd:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($v=="mssql"?$Gd.$Bf:$Bf.$Gd).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger entfernt.','Trigger geändert.','Trigger erstellt.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Trigger ändern'.": ".h($_GET["name"]):'Trigger hinzufügen'),$j,array("table"=>$a));$J=$_POST;if(!$J)$J=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Mf["Timing"],$J["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Lf,$J["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Mf["Type"],$J["Type"]),'</table>
...
as$X){if($X[1]!="USAGE")$sc["$_[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$J[0]))$sc["$_[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$J[0],$_))$Ed=$_[1];}}if($_POST&&!$j){$Fd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$yd=q($_POST["user"])."@".q($_POST["host"]);$ge=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $Fd",ME."privileges=",'Benutzer entfernt.');else{$hb=false;if($Fd!=$yd){$hb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $yd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $ge");$j=!$hb;}elseif($_POST["pass"]!=$Ed||!$_POST["hashed"])queries("SET PASSWORD FOR $yd = ".($_POST["hashed"]?$ge:"PASSWORD($ge)"));if(!$j){$Le=array();foreach($xd
...
as$Ad=>$Le){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Ad,$_))grant("REVOKE",array_keys($Le),$_[2]," ON $_[1] FROM $yd");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Benutzer geändert.':'Benutzer erstellt.'),!$j);if($hb)$f->query("DROP USER $yd");}}page_header((isset($_GET["host"])?'Benutzer'.": ".h("$fa@$_GET[host]"):'Neuer Benutzer'),$j,array("privileges"=>array('','Rechte')));if($_POST){$J=$_POST;$sc=$xd;}else{$J=$_GET+array("host"=>$f->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$Ed;if($Ed!="")$J["hashed"]=true;$sc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>Server<td><input name="host" maxlength="60" value="',h($J["host"]),'">
<tr><th>Benutzer<td><input name="user" maxlength="16" value="',h($J["user"]),'">
<tr><th>Passwort<td><input id="pass" name="pass" value="',h($J["pass"]),'">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$q=0;foreach($sc
as$Ad=>$p){echo'<th>'.($Ad!="*.*"?"<input name='objects[$q]' value='".h($Ad)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$cb=>$sb){foreach((array)$ue[$cb]as$te=>$Wa){echo"<tr".odd()."><td".($sb?">$sb<td":" colspan='2'").' lang="en" title="'.h($Wa).'">'.h($te);$q=0;foreach($sc
as$Ad=>$p){$C="'grants[$q][".h(strtoupper($te))."]'";$Y=$p[strtoupper($te)];if($cb=="Server Admin"&&$Ad!=(isset($sc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Verbieten'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Sc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Sc++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Sc),$Sc||!$_POST["kill"]);}page_header('Prozessliste',$j);echo'
<form action="" method="post">
...
as$w=>$X)echo"<td>".(($v=="sql"&&$w=="Info"&&ereg("Query|Killed",$J["Command"])&&$X!="")||($v=="pgsql"&&$w=="current_query"&&$X!="<IDLE>")||($v=="oracle"&&$w=="sql_text"&&$X!="")?"<code class='jush-$v'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($J["db"]!=""?"db=".urlencode($J["db"])."&":"")."sql=".urlencode($X)).'">'.'Ändern'.'</a>':nbsp($X));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($q+1)."/".sprintf('%d insgesamt',$f->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Anhalten'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}$lc=$b->dumpFormat();if($lc){print_fieldset("export",'Exportieren');$Xd=$b->dumpOutput();echo($Xd?html_select("output",$Xd,$la["output"])." ":""),html_select("format",$lc,$la["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$K);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Jb,'strlen'),$e);echo"</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$ff=isset($_GET["status"]);page_header($ff?'Status':'Variablen');$eg=($ff?show_status():show_variables());if(!$eg)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($eg
...
as$w=>$X)json_row("sum-$w",number_format($X,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$f->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$i=>$X)json_row("tables-".js_escape($i),$X);json_row("");}exit;}else{$wf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($wf&&!$j&&!$_POST["search"]){$H=true;$A="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$H=truncate_tables($_POST["tables"]);$A='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$H=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$A='Tabellen verschoben.';}elseif($_POST["copy"]){$H=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$A='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"])$H=drop_views($_POST["views"]);if($H&&$_POST["tables"])$H=drop_tables($_POST["tables"]);$A='Tabellen wurden entfernt (drop).';}elseif($v!="sql"){$H=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$A='Tables have been optimized.';}elseif($_POST["tables"]&&($H=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($J=$H->fetch_assoc())$A.="<b>".h($J["Table"])."</b>: ".h($J["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$A,$H);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$j,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabellen und Views'."</h3>\n";$vf=tables_list();if(!$vf)echo"<p class='message'>".'Keine Tabellen.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'Datengrösse','<td>'.'Indexgrösse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'Datensätze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";foreach($vf
as$C=>$U){$fg=($U!==null&&!eregi("table",$U));echo'<tr'.odd().'><td>'.checkbox(($fg?"views[]":"tables[]"),$C,in_array($C,$wf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($C).'" title="'.'Struktur anzeigen'.'">'.h($C).'</a>';if($fg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'View ändern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Daten auswählen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle ändern'),"Index_length"=>array("indexes",'Indizes ändern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle ändern'),"Rows"=>array("select",'Daten auswählen'),)as$w=>$y)echo($y?"<td align='right'><a href='".h(ME."$y[0]=").urlencode($C)."' id='$w-".h($C)."' title='$y[1]'>?</a>":"<td id='$w-".h($C)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($vf)),"<td>".nbsp($v=="sql"?$f->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w)echo"<td align='right' id='sum-$w'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$v)?($v!="sqlite"?"<input type='submit' value='".'Analysieren'."'> ":"")."<input type='submit' name='optimize' value='".'Optimieren'."'> ":"").($v=="sql"?"<input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)").">\n";$h=(support("scheme")?schemas():$b->databases());if(count($h)!=1&&$v!="sqlite"){$i=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($h?html_select("target",$h,$i):'<input name="target" value="'.h($i).'">')," <input type='submit' name='move' value='".'Verschieben'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$T'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$Qe=routines();if($Qe){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($Qe
as$J){echo'<tr'.odd().'>','<th><a href="'.h(ME).($J["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($J["ROUTINE_NAME"]).'">'.h($J["ROUTINE_NAME"]).'</a>','<td>'.h($J["ROUTINE_TYPE"]),'<td>'.h($J["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($J["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($J["ROUTINE_NAME"]).'">'.'Ändern'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Neue Prozedur'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Neue Funktion'."</a>\n";}if(support("event")){echo"<h3>".'Ereignisse'."</h3>\n";$K=get_rows("SHOW EVENTS");if($K){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Name'."<td>".'Zeitplan'."<td>".'Start'."<td>".'Ende'."</thead>\n";foreach($K
as$J){echo"<tr>",'<th><a href="'.h(ME).'event='.urlencode($J["Name"]).'">'.h($J["Name"])."</a>","<td>".($J["Execute at"]?'Zur angegebenen Zeit'."<td>".$J["Execute at"]:'Jede'." ".$J["Interval value"]." ".$J["Interval field"]."<td>$J[Starts]"),"<td>$J[Ends]";}echo"</table>\n";$Sb=$f->result("SELECT @@event_scheduler");if($Sb&&$Sb!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Sb)."\n";}echo'<p><a href="'.h(ME).'event=">'.'Ereignis erstellen'."</a>\n";}if($vf)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}}}page_footer();