...
confirm($Za=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($Za?" (' + $Za + ')":"")."');\"";}function
...
upload_error($j){$Zc=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Hochladen von Datei fehlgeschlagen.'.($Zc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Zc):""):'Datei existiert nicht.');}function
...
<tr><th>Datenbank System<td>',html_select("auth[driver]",$rb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Ze,$P=""){echo'<p class="tabs">';$Rc=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Ze))$Rc["view"]='View ändern';else$Rc["create"]='Tabelle ändern';if($P!==null)$Rc["edit"]='Neuer Datensatz';foreach($Rc
...
selectColumnsPrint($N,$e){global$ic,$lc;print_fieldset("select",'Daten zeigen von',$N);$r=0;$hc=array('Funktionen'=>$ic,'Agregationen'=>$lc);foreach($N
...
selectLengthPrint($kf){if($kf!==null){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($kf).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($H){global$w;static$Za=0;restart_session();$s="sql-".($Za++);$nc=&get_session("queries");if(strlen($H)>1e6)$H=ereg_replace('[\x80-\xFF]+$','',substr($H,0,1e6))."\n...";$nc[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL-Query'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($nc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
as$S=>$V){echo'<a href="'.h(ME).'select='.urlencode($S).'"'.bold($_GET["select"]==$S).">".'zeigen'."</a> ",'<a href="'.h(ME).'table='.urlencode($S).'"'.bold($_GET["table"]==$S)." title='".'Struktur anzeigen'."'>".$this->tableName(array("Name"=>$S))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
edit_type($x,$k,$d,$n=array()){global$Se,$Af,$Hf,$vd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($Af[$k["type"]])?array():array($k["type"]))+$Se+($n?array('Fremdschlüssel'=>$n):array()),$k["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($d,$k["collation"]).'</select>',($Hf?"<select name='$x"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Hf,$k["unsigned"]).'</select>':''),($n?"<select name='$x"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$vd),$k["on_delete"])."</select> ":" ");}function
...
';if($V=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($V=="TABLE"?'Spaltenname':'Name des Parameters'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Länge
<td>Optionen
...
',(support("comment")?"<td".($Sa?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Sa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.1' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.1' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.1' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Fd==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.1' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$U,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Im Server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ta=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$x=>$X){if(extension_loaded($X))$Ta[]=".$x";}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Ta?"[".implode("|",$Ta)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($nc){print_fieldset("history",'History',$_GET["history"]!="");foreach($nc
as$x=>$X){list($G,$lf)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Ändern'."</a> <span class='time'>".@date("H:i:s",$lf)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$If?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($If?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Datensatz gelöscht.');else{$P=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$P[idf_escape($D)]=($If?"\n".idf_escape($D)." = $X":$X);}if($If){if(!$P)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $Z"),$_,'Datensatz geändert.');}else{$I=insert_into($a,$P);$Jc=($I?last_id():0);queries_redirect($_,sprintf('Datensatz%s hinzugefügt.',($Jc?" $Jc":"")),$I);}}}$af=$b->tableName(table_status($a));page_header(($If?'Ändern':'Hinzufügen'),$j,array("select"=>array($a,$af)),$af);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$N=array();foreach($l
...
';if($l){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($If?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}echo($If?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Sa,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($K["Comment"]).'" maxlength="60"'.($Sa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
';if(support("partitioning")){$Qd=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Partitionieren um',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Pd,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($K["partitions"]),'"',($Qd||!$K["partition_by"]?" class='hidden'":""),'>
...
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$Te=false;$Ic=$i;}}queries_redirect(ME."db=".urlencode($Ic),'Datenbank erstellt.',$Te);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$j,array(),DB);$d=collations();$D=DB;$La=null;if($_POST){$D=$_POST["name"];$La=$_POST["collation"];}elseif(DB!="")$La=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($D,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($D).'</textarea><br>':'<input id="name" name="name" value="'.h($D).'" maxlength="64">')."\n".($d?html_select("collation",array(""=>"(".'Kollation'.")")+$d,$La):"");?>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$tb=false;if($_POST&&!$j){$D=trim($_POST["name"]);$tb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($D)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($D)),'View entfernt.','View geändert.','View erstellt.',$a);}page_header(($a!=""?'View ändern':'Neue View erstellen'),$j,array("table"=>$a),$a);$K=$_POST;if(!$K&&$a!=""){$K=view($a);$K["name"]=$a;}echo'
<form action="" method="post">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_c,$K["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Qe,$K["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",'Nach der Ausführung erhalten'),'</table>
<p>';textarea("EVENT_DEFINITION",$K["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="Speichern">
';if($aa!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($K["name"]),'" maxlength="64">
',($_e?'Sprache'.": ".html_select("language",$_e,$K["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$d,$ze);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$K["returns"],$d);}echo'</table>
<p>';textarea("definition",$K["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$xf=trigger_options();$wf=array("INSERT","UPDATE","DELETE");$tb=false;if($_POST&&!$j&&in_array($_POST["Timing"],$xf["Timing"])&&in_array($_POST["Event"],$wf)&&in_array($_POST["Type"],$xf["Type"])){$mf=" $_POST[Timing] $_POST[Event]";$ud=" ON ".table($a);$tb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($w=="pgsql"?$ud:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($w=="mssql"?$ud.$mf:$mf.$ud).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger entfernt.','Trigger geändert.','Trigger erstellt.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Trigger ändern'.": ".h($_GET["name"]):'Trigger hinzufügen'),$j,array("table"=>$a));$K=$_POST;if(!$K)$K=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$xf["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$wf,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$xf["Type"],$K["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($K["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$K["Statement"]);echo'<p>
<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
...
as$X){if($X[1]!="USAGE")$jc["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$jc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$sd=$A[1];}}if($_POST&&!$j){$td=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$ld=q($_POST["user"])."@".q($_POST["host"]);$Td=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $td",ME."privileges=",'Benutzer entfernt.');else{$cb=false;if($td!=$ld){$cb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ld IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Td");$j=!$cb;}elseif($_POST["pass"]!=$sd||!$_POST["hashed"])queries("SET PASSWORD FOR $ld = ".($_POST["hashed"]?$Td:"PASSWORD($Td)"));if(!$j){$we=array();foreach($kd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$r=0;foreach($jc
as$od=>$p){echo'<th>'.($od!="*.*"?"<input name='objects[$r]' value='".h($od)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Xa=>$mb){foreach((array)$he[$Xa]as$ge=>$Ra){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Ra).'">'.h($ge);$r=0;foreach($jc
as$od=>$p){$D="'grants[$r][".h(strtoupper($ge))."]'";$Y=$p[strtoupper($ge)];if($Xa=="Server Admin"&&$od!=(isset($jc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Verbieten'."</select>";else
...
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Fc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Fc++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Fc),$Fc||!$_POST["kill"]);}page_header('Prozessliste',$j);echo'
<form action="" method="post">
...
as$x=>$X)echo"<td>".(($w=="sql"&&$x=="Info"&&ereg("Query|Killed",$K["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($K["db"]!=""?"db=".urlencode($K["db"])."&":"")."sql=".urlencode($X)).'">'.'Ändern'.'</a>':nbsp($X));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$ec,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$cc=$b->dumpFormat();if($cc){print_fieldset("export",'Exportieren');$Kd=$b->dumpOutput();echo($Kd?html_select("output",$Kd,$la["output"])." ":""),html_select("format",$cc,$la["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$U'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Ab,'strlen'),$e);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Pe=isset($_GET["status"]);page_header($Pe?'Status':'Variablen');$Of=($Pe?show_status():show_variables());if(!$Of)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Of
