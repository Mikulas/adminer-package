...
confirm($Xa="",$Ke=false){return" onclick=\"".($Ke?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Xa?" (' + $Xa + ')":"")."');\"";}function
...
upload_error($i){$Tc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($i?'Hochladen von Datei fehlgeschlagen.'.($Tc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Tc):""):'Datei existiert nicht.');}function
...
<tr><th>Datenbank System<td>',html_select("driver",$pb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Te,$O=""){echo'<p class="tabs">';$Lc=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Te)){$Lc["view"]='View ändern';}else{$Lc["create"]='Tabelle ändern';}if(isset($O)){$Lc["edit"]='Neuer Datensatz';}foreach($Lc
...
selectQuery($G){global$w;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a> <code class='jush-$w'>".h(str_replace("\n"," ",$G))."</code> <a href='".h(ME)."sql=".urlencode($G)."'>".'Ändern'."</a></p>\n";}function
...
selectColumnsPrint($M,$d){global$o,$hc;print_fieldset("select",'Daten zeigen von',$M);$r=0;$ec=array('Funktionen'=>$o,'Agregationen'=>$hc);foreach($M
...
selectSearchPrint($Z,$d,$u){print_fieldset("search",'Suchen',$Z);foreach($u
as$r=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."'>",checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"<br>\n";}}$r=0;foreach((array)$_GET["where"]as$X){if("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators)){echo"<div><select name='where[$r][col]'><option value=''>(".'beliebig'.")".optionlist($d,$X["col"],true)."</select>",html_select("where[$r][op]",$this->operators,$X["op"]),"<input name='where[$r][val]' value='".h($X["val"])."'></div>\n";$r++;}}echo"<div><select name='where[$r][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'beliebig'.")".optionlist($d,null,true)."</select>",html_select("where[$r][op]",$this->operators,"="),"<input name='where[$r][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($wd,$d,$u){print_fieldset("sort",'Ordnen',$wd);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($d[$X])){echo"<div><select name='order[$r]'><option>".optionlist($d,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'absteigend')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($d,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($y){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($y)."'>","</div></fieldset>\n";}function
selectLengthPrint($ef){if(isset($ef)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($ef).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($G){global$w;static$Xa=0;restart_session();$s="sql-".($Xa++);$jc=&get_session("queries");if(strlen($G)>1e6){$G=ereg_replace('[\x80-\xFF]+$','',substr($G,0,1e6))."\n...";}$jc[$_GET["db"]][]=$G;return" <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL-Query'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($G,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($jc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
<p class="logout">
';if(DB==""||!$bd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL-Query'."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}}echo'<input type="submit" name="logout" value="Abmelden" onclick="eventStop(event);">
<input type="hidden" name="token" value="',$T,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($g?html_select("db",array(""=>"(".'Datenbank'.")")+$g,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Benutzung"',($g?" class='hidden'":""),' onclick="eventStop(event);">
';if($bd!="db"&&DB!=""&&$e->select_db(DB)){if($_GET["ns"]!==""&&!$bd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle'."</a>\n";$Ye=tables_list();if(!$Ye){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($Ye);$Lc=array();foreach($Ye
...
as$x=>$X){if(!isset($X)){$X="<i>NULL</i>";}elseif($_a[$x]&&!is_utf8($X)){$X="<i>".lang(array('%d Byte','%d Bytes'),strlen($X))."</i>";}elseif(!strlen($X)){$X="&nbsp;";}else{$X=h($X);if($vf[$x]==254){$X="<code>$X</code>";}}if(isset($Lc[$x])&&!$d[$Lc[$x]]){if($mc){$z=$Lc[$x].urlencode($J[array_search("table=",$Lc)]);}else{$z="edit=".urlencode($Lc[$x]);foreach($u[$Lc[$x]]as$Ia=>$v){$z.="&where".urlencode("[".bracket_escape($Ia)."]")."=".urlencode($J[$v]);}}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($x,$j,$c,$m=array()){global$Me,$vf,$Bf,$pd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$j["type"]||isset($vf[$j["type"]])?array():array($j["type"]))+$Me+($m?array('Fremdschlüssel'=>$m):array()),$j["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($j["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$j["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($c,$j["collation"]).'</select>',($Bf?"<select name='$x"."[unsigned]'".(!$j["type"]||ereg('(int|float|double|decimal)$',$j["type"])?"":" class='hidden'").'><option>'.optionlist($Bf,$j["unsigned"]).'</select>':''),($m?"<select name='$x"."[on_delete]'".(ereg("`",$j["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$pd),$j["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Qa?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($k))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($k),';</script>
</thead>
...
',(support("comment")?"<td".($Qa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($j["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $qa, 1);'>&nbsp;"."<input type='image' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Nach unten'."'>&nbsp;":""),($zd==""||support("drop_col")?"<input type='image' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$T,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Im Server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ra=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$x=>$X){if(extension_loaded($X)){$Ra[]=".$x";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Ra?"[".implode("|",$Ra)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($jc){print_fieldset("history",'History',$_GET["history"]!="");foreach($jc
as$x=>$X){echo'<a href="'.h(ME."sql=&history=$x").'">'.'Ändern'."</a> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$X)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$C=>$j){if(!isset($j["privileges"][$Cf?"update":"insert"])||$b->fieldName($j)==""){unset($k[$C]);}}if($_POST&&!$i&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"]){$_=($Cf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$_)){$_=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Datensatz gelöscht.');}else{$O=array();foreach($k
as$C=>$j){$X=process_input($j);if($X!==false&&$X!==null){$O[idf_escape($C)]=($Cf?"\n".idf_escape($C)." = $X":$X);}}if($Cf){if(!$O){redirect($_);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Datensatz geändert.');}else{$H=insert_into($a,$O);$Ec=($H?last_id():0);queries_redirect($_,sprintf('Datensatz%s hinzugefügt.',($Ec?" $Ec":"")),$H);}}}$Ue=$b->tableName(table_status($a));page_header(($Cf?'Ändern':'Hinzufügen'),$i,array("select"=>array($a,$Ue)),$Ue);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($Z){$M=array();foreach($k
as$C=>$j){if(isset($j["privileges"]["select"])){$M[]=($_POST["clone"]&&$j["auto_increment"]?"'' AS ":(ereg("enum|set",$j["type"])?"1*".idf_escape($C)." AS ":"")).idf_escape($C);}}$J=array();if($M){$K=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Keine Daten.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($k){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Cf?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}}echo($Cf?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Qa,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($J["Comment"]).'" maxlength="60"'.($Qa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
';if(support("partitioning")){$Jd=ereg('RANGE|LIST',$J["partition_by"]);print_fieldset("partition",'Partitionieren um',$J["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Id,$J["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($J["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($J["partitions"]),'"',($Jd||!$J["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$i&&!isset($_POST["add_x"])){restart_session();$C=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($C),'Datenbank umbenannt.',rename_database($C,$_POST["collation"]));}else{$g=explode("\n",str_replace("\r","",$C));$Ne=true;$Dc="";foreach($g
as$h){if(count($g)==1||$h!=""){if(!create_database($h,$_POST["collation"])){$Ne=false;}$Dc=$h;}}queries_redirect(ME."db=".urlencode($Dc),'Datenbank erstellt.',$Ne);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($C).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$i,array(),DB);$c=collations();$C=DB;$Ja=null;if($_POST){$C=$_POST["name"];$Ja=$_POST["collation"];}elseif(DB!=""){$Ja=db_collation(DB,$c);}elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$C=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input id="name" name="name" value="'.h($C).'" maxlength="64">')."\n".($c?html_select("collation",array(""=>"(".'Kollation'.")")+$c,$Ja):"");?>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$rb=false;if($_POST&&!$i){$C=trim($_POST["name"]);$rb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($C)),'View entfernt.','View geändert.','View erstellt.',$a);}page_header(($a!=""?'View ändern':'Neue View erstellen'),$i,array("table"=>$a),$a);$J=$_POST;if(!$J&&$a!=""){$J=view($a);$J["name"]=$a;}echo'
<form action="" method="post">
<p>Name: <input name="name" value="',h($J["name"]),'" maxlength="64">
<p>';textarea("select",$J["select"]);echo'<p>
';if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$wc=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Je=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$i){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$wc)&&isset($Je[$_POST["STATUS"]])){$we="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Ereignis geändert.':'Ereignis erstellt.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$we.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$we)."\n".$Je[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Ereignis ändern'.": ".h($aa):'Ereignis erstellen'),$i);$J=$_POST;if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$wc,$J["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Je,$J["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
...
as$j){if($j["field"]!=""){$O[]=(ereg("^($tc)\$",$j["inout"])?"$j[inout] ":"").idf_escape($j["field"]).process_type($j,"CHARACTER SET");}}$rb=drop_create("DROP $se ".idf_escape($da),"CREATE $se ".idf_escape(trim($_POST["name"]))." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$te)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Prozedur entfernt.','Prozedur geändert.','Prozedur erstellt.',$da);}page_header(($da!=""?(isset($_GET["function"])?'Funktion ändern':'Prozedur ändern').": ".h($da):(isset($_GET["function"])?'Neue Funktion':'Neue Prozedur')),$i);$c=get_vals("SHOW CHARACTER SET");sort($c);$J=array("fields"=>array());if($_POST){$J=$_POST;$J["fields"]=(array)$J["fields"];process_fields($J["fields"]);}elseif($da!=""){$J=routine($da,$se);$J["name"]=$da;}echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($J["name"]),'" maxlength="64">
',($te?'Sprache'.": ".html_select("language",$te,$J["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$c,$se);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$J["returns"],$c);}echo'</table>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$rf=trigger_options();$qf=array("INSERT","UPDATE","DELETE");$rb=false;if($_POST&&!$i&&in_array($_POST["Timing"],$rf["Timing"])&&in_array($_POST["Event"],$qf)&&in_array($_POST["Type"],$rf["Type"])){$gf=" $_POST[Timing] $_POST[Event]";$od=" ON ".table($a);$rb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($w=="pgsql"?$od:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($w=="mssql"?$od.$gf:$gf.$od).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger entfernt.','Trigger geändert.','Trigger erstellt.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Trigger ändern'.": ".h($_GET["name"]):'Trigger hinzufügen'),$i,array("table"=>$a));$J=$_POST;if(!$J){$J=trigger($_GET["name"])+array("Trigger"=>$a."_bi");}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$rf["Timing"],$J["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$qf,$J["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$rf["Type"],$J["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($J["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$J["Statement"]);echo'<p>
<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE"){$fc["$A[2]$X[2]"][$X[1]]=true;}if(ereg(' WITH GRANT OPTION',$J[0])){$fc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$J[0],$A)){$md=$A[1];}}}if($_POST&&!$i){$nd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$gd=q($_POST["user"])."@".q($_POST["host"]);$Md=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $nd",ME."privileges=",'Benutzer entfernt.');}else{$ab=false;if($nd!=$gd){$ab=queries(($e->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $gd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Md");$i=!$ab;}elseif($_POST["pass"]!=$md||!$_POST["hashed"]){queries("SET PASSWORD FOR $gd = ".($_POST["hashed"]?$Md:"PASSWORD($Md)"));}if(!$i){$pe=array();foreach($fd
...
as$jd=>$pe){if(preg_match('~^(.+)(\\(.*\\))?$~U',$jd,$A)){grant("REVOKE",array_keys($pe),$A[2]," ON $A[1] FROM $gd");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Benutzer geändert.':'Benutzer erstellt.'),!$i);if($ab){$e->query("DROP USER $gd");}}}page_header((isset($_GET["host"])?'Benutzer'.": ".h("$fa@$_GET[host]"):'Neuer Benutzer'),$i,array("privileges"=>array('','Rechte')));if($_POST){$J=$_POST;$fc=$fd;}else{$J=$_GET+array("host"=>$e->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$md;if($md!=""){$J["hashed"]=true;}$fc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($e->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$r=0;foreach($fc
as$jd=>$p){echo'<th>'.($jd!="*.*"?"<input name='objects[$r]' value='".h($jd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Va=>$kb){foreach((array)$ae[$Va]as$Zd=>$Pa){echo"<tr".odd()."><td".($kb?">$kb<td":" colspan='2'").' lang="en" title="'.h($Pa).'">'.h($Zd);$r=0;foreach($fc
as$jd=>$p){$C="'grants[$r][".h(strtoupper($Zd))."]'";$Y=$p[strtoupper($Zd)];if($Va=="Server Admin"&&$jd!=(isset($fc["*.*"])?"*.*":".*")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Verbieten'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Zd=="All privileges"?" id='grants-$r-all'":($Zd=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$r-all');\"")).">";}$r++;}}}echo"</table>\n",'<p>
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$i){$Ac=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X))){$Ac++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Ac),$Ac||!$_POST["kill"]);}page_header('Prozessliste',$i);echo'
<form action="" method="post">
...
as$x=>$X){echo"<td>".(($w=="sql"?$x=="Info"&&$X!="":$x=="current_query"&&$X!="<IDLE>")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($J["db"]!=""?"db=".urlencode($J["db"])."&":"")."sql=".urlencode($X)).'">'.'Ändern'.'</a>':nbsp($X));}echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('%d insgesamt',$e->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Anhalten'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$bc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$Zb=$b->dumpFormat();if($Zb){print_fieldset("export",'Exportieren');$Dd=$b->dumpOutput();echo($Dd?html_select("output",$Dd,$la["output"])." ":""),html_select("format",$Zb,$la["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$K);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($zb,'strlen'),$d);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ie=isset($_GET["status"]);page_header($Ie?'Status':'Variablen');$Gf=($Ie?show_status():show_variables());if(!$Gf){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Gf
...
as$x=>$X){json_row("sum-$x",number_format($X,0,'.',' '));}json_row("");}else{foreach(count_tables($b->databases())as$h=>$X){json_row("tables-".js_escape($h),$X);}json_row("");}exit;}else{$af=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($af&&!$i&&!$_POST["search"]){$H=true;$Vc="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$H=truncate_tables($_POST["tables"]);}$Vc='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$H=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Vc='Tabellen verschoben.';}elseif($_POST["copy"]){$H=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Vc='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"]){$H=drop_views($_POST["views"]);}if($H&&$_POST["tables"]){$H=drop_tables($_POST["tables"]);}$Vc='Tabellen wurden entfernt (drop).';}elseif($w!="sql"){$H=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$Vc='Tables have been optimized.';}elseif($_POST["tables"]&&($H=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($J=$H->fetch_assoc()){$Vc.="<b>".h($J["Table"])."</b>: ".h($J["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$Vc,$H);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$i,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabellen und Views'."</h3>\n";$Ze=tables_list();if(!$Ze){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'Datengrösse','<td>'.'Indexgrösse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'Datensätze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";foreach($Ze
as$C=>$U){$Hf=(isset($U)&&!eregi("table",$U));echo'<tr'.odd().'><td>'.checkbox(($Hf?"views[]":"tables[]"),$C,in_array($C,$af,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($C).'" title="'.'Struktur anzeigen'.'">'.h($C).'</a>';if($Hf){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'View ändern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Daten auswählen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle ändern'),"Index_length"=>array("indexes",'Indizes ändern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle ändern'),"Rows"=>array("select",'Daten auswählen'),)as$x=>$z){echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($C)."' id='$x-".h($C)."' title='$z[1]'>?</a>":"<td id='$x-".h($C)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($Ze)),"<td>".nbsp($w=="sql"?$e->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x){echo"<td align='right' id='sum-$x'>&nbsp;";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$w)?($w!="sqlite"?"<input type='submit' value='".'Analysieren'."'> ":"")."<input type='submit' name='optimize' value='".'Optimieren'."'> ":"").($w=="sql"?"<input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$g=(support("scheme")?schemas():$b->databases());if(count($g)!=1&&$w!="sqlite"){$h=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($g?html_select("target",$g,$h):'<input name="target" value="'.h($h).'">')," <input type='submit' name='move' value='".'Verschieben'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$T'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";}if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$ue=routines();if($ue){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($ue
as$J){echo'<tr'.odd().'>','<th><a href="'.h(ME).($J["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($J["ROUTINE_NAME"]).'">'.h($J["ROUTINE_NAME"]).'</a>','<td>'.h($J["ROUTINE_TYPE"]),'<td>'.h($J["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($J["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($J["ROUTINE_NAME"]).'">'.'Ändern'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Neue Prozedur'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Neue Funktion'."</a>\n";}if(support("event")){echo"<h3>".'Ereignisse'."</h3>\n";$K=get_rows("SHOW EVENTS");if($K){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Name'."<td>".'Zeitplan'."<td>".'Start'."<td>".'Ende'."</thead>\n";foreach($K
