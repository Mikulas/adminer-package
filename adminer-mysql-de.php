...
confirm($eb=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($eb?" (' + $eb + ')":"")."');\"";}function
...
upload_error($j){$ld=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Hochladen von Datei fehlgeschlagen.'.($ld?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$ld):""):'Datei existiert nicht.');}function
...
<tr><th>Datenbank System<td>',html_select("auth[driver]",$zb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($pf,$O=""){echo'<p class="tabs">';$dd=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($pf))$dd["view"]='View ändern';else$dd["create"]='Tabelle ändern';if($O!==null)$dd["edit"]='Neuer Datensatz';foreach($dd
...
selectQuery($H){global$w;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a> <code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".'Ändern'."</a></p>\n";}function
...
selectColumnsPrint($M,$e){global$p,$uc;print_fieldset("select",'Daten zeigen von',$M);$r=0;$qc=array('Funktionen'=>$p,'Agregationen'=>$uc);foreach($M
...
selectSearchPrint($Z,$e,$u){print_fieldset("search",'Suchen',$Z);foreach($u
as$r=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ia="this.nextSibling.onchange();";for($r=0;$r<=count($_GET["where"]);$r++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div><select name='where[$r][col]' onchange='$Ia'><option value=''>(".'beliebig'.")".optionlist($e,$X["col"],true)."</select>",html_select("where[$r][op]",$this->operators,$X["op"],$Ia),"<input type='search' name='where[$r][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Nd,$e,$u){print_fieldset("sort",'Ordnen',$Nd);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($e[$X])){echo"<div><select name='order[$r]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'absteigend')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($_f){if($_f!==null){echo"<fieldset><legend>".'Textlänge'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($_f)."'>","</div></fieldset>\n";}}function
...
messageQuery($H){global$w;static$eb=0;restart_session();$s="sql-".($eb++);$wc=&get_session("queries");if(strlen($H)>1e6)$H=ereg_replace('[\x80-\xFF]+$','',substr($H,0,1e6))."\n...";$wc[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL-Query'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($wc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
dumpOutput(){$J=array('text'=>'anzeigen','file'=>'Datei');if(function_exists('gzencode'))$J['gz']='gzip';if(function_exists('bzcompress'))$J['bz2']='bzip2';return$J;}function
dumpFormat(){return
...
<p id="dbs">
';hidden_fields_get();echo($h?html_select("db",array(""=>"(".'Datenbank'.")")+$h,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Benutzung"',($h?" class='hidden'":""),'>
...
as$R=>$U){echo'<a href="'.h(ME).'select='.urlencode($R).'"'.bold($_GET["select"]==$R).">".'zeigen'."</a> ",'<a href="'.h(ME).'table='.urlencode($R).'"'.bold($_GET["table"]==$R)." title='".'Struktur anzeigen'."'>".$this->tableName(array("Name"=>$R))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
auth_error($Ub=null){global$f,$b,$T;$af=session_name();$j="";if(!$_COOKIE[$af]&&$_GET[$af]&&ini_bool("session.use_only_cookies"))$j='Sitzungen müssen aktiviert sein.';elseif(isset($_GET["username"])){if(($_COOKIE[$af]||$_GET[$af])&&!$T)$j='Sitzungsdauer abgelaufen, bitte erneut anmelden.';else{$ge=&get_session("pwds");if($ge!==null){$j=h($Ub?$Ub->getMessage():(is_string($f)?$f:'Ungültige Anmelde-Informationen.'));$ge=null;}unset_permanent();}}page_header('Login',$j,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstützten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$me)),false);page_footer("auth");exit;}$f=connect();}if(is_string($f)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$T=$_SESSION["token"];if($c&&$_POST["token"])$_POST["token"]=$T;$j=($_POST?($_POST["token"]==$T?"":'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('POST data zu gross. Reduzieren Sie die Grösse oder vergrössern Sie den Wert %s in der Konfiguration.','"post_max_size"')));if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}function
connect_error(){global$b,$f,$T,$j,$zb;$h=array();if(DB!="")page_header('Datenbank'.": ".h(DB),'Datenbank ungültig.',true);else{if($_POST["db"]&&!$j)queries_redirect(substr(ME,0,-1),'Datenbanken entfernt.',drop_databases($_POST["db"]));page_header('Datenbank auswählen',$j,false);echo"<p><a href='".h(ME)."database='>".'Neue Datenbank'."</a>\n";foreach(array('privileges'=>'Rechte','processlist'=>'Prozessliste','variables'=>'Variablen','status'=>'Status',)as$x=>$X){if(support($x))echo"<a href='".h(ME)."$x='>$X</a>\n";}echo"<p>".sprintf('Version %s: %s, mit PHP-Erweiterung %s',$zb[DRIVER],"<b>$f->server_info</b>","<b>$f->extension</b>")."\n","<p>".sprintf('Angemeldet als: %s',"<b>".h(logged_user())."</b>")."\n";$De="<a href='".h(ME)."refresh=1'>".'Aktualisieren'."</a>\n";$h=$b->databases();if($h){$Ue=support("scheme");$d=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr><td>&nbsp;<th>".'Datenbank'."<td>".'Collation'."<td>".'Tabellen'."</thead>\n";foreach($h
as$i){$Me=h(ME)."db=".urlencode($i);echo"<tr".odd()."><td>".checkbox("db[]",$i,in_array($i,(array)$_POST["db"])),"<th><a href='$Me'>".h($i)."</a>","<td><a href='$Me".($Ue?"&amp;ns=":"")."&amp;database=' title='".'Datenbank ändern'."'>".nbsp(db_collation($i,$d))."</a>","<td align='right'><a href='$Me&amp;schema=' id='tables-".h($i)."' title='".'Datenbankschema'."'>?</a>","\n";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","<p><input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /db/)").">\n","<input type='hidden' name='token' value='$T'>\n",$De,"</form>\n";}else
...
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Da[$x]&&!is_utf8($X))$X="<i>".lang(array('%d Byte','%d Bytes'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Qf[$x]==254)$X="<code>$X</code>";}if(isset($dd[$x])&&!$e[$dd[$x]]){if($zc){$R=$K[array_search("table=",$dd)];$z=$dd[$x].urlencode($Qd[$R]!=""?$Qd[$R]:$R);}else{$z="edit=".urlencode($dd[$x]);foreach($u[$dd[$x]]as$Pa=>$v)$z.="&where".urlencode("[".bracket_escape($Pa)."]")."=".urlencode($K[$v]);}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Keine Daten.')."\n";return$J;}function
...
edit_type($x,$k,$d,$n=array()){global$if,$Qf,$Xf,$Gd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($Qf[$k["type"]])?array():array($k["type"]))+$if+($n?array('Fremdschlüssel'=>$n):array()),$k["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($d,$k["collation"]).'</select>',($Xf?"<select name='$x"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Xf,$k["unsigned"]).'</select>':''),($n?"<select name='$x"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Gd),$k["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Xa?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.2' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Xa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='".($f->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.2' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.2' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.2' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Rd==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.2' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
';$hc=true;foreach($oe
as$x=>$X){if($x!=""&&$X>1){echo($hc?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$x%")."'>".h($x)."</a>";$hc=false;}}}elseif(isset($_GET["privileges"])){page_header('Rechte');$I=$f->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q(DB)." LIKE Db")." ORDER BY Host, User");$q=$I;if(!$I)$I=$f->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");echo"<form action=''><p>\n";hidden_fields_get();echo"<input type='hidden' name='db' value='".h(DB)."'>\n",($q?"":"<input type='hidden' name='grant' value=''>\n"),"<table cellspacing='0'>\n","<thead><tr><th>".'Benutzer'."<th>".'Server'."<th>&nbsp;</thead>\n";while($K=$I->fetch_assoc())echo'<tr'.odd().'><td>'.h($K["User"])."<td>".h($K["Host"]).'<td><a href="'.h(ME.'user='.urlencode($K["User"]).'&host='.urlencode($K["Host"])).'">'.'Ändern'."</a>\n";if(!$q||DB!="")echo"<tr".odd()."><td><input name='user'><td><input name='host' value='localhost'><td><input type='submit' value='".'Ändern'."'>\n";echo"</table>\n","</form>\n",'<p><a href="'.h(ME).'user=">'.'Neuer Benutzer'."</a>";}elseif(isset($_GET["sql"])){if(!$j&&$_POST["export"]){dump_headers("sql");$b->dumpTable("","");$b->dumpData("","table",$_POST["query"]);exit;}restart_session();$xc=&get_session("queries");$wc=&$xc[DB];if(!$j&&$_POST["clear"]){$wc=array();redirect(remove_from_uri("history"));}page_header('SQL-Query',$j);if(!$j&&$_POST){$oc=false;$H=$_POST["query"];if($_POST["webfile"]){$oc=@fopen((file_exists("adminer.sql")?"adminer.sql":(file_exists("adminer.sql.gz")?"compress.zlib://adminer.sql.gz":"compress.bzip2://adminer.sql.bz2")),"rb");$H=($oc?fread($oc,1e6):false);}elseif($_FILES&&$_FILES["sql_file"]["error"]!=UPLOAD_ERR_NO_FILE)$H=get_file("sql_file",true);if(is_string($H)){if(function_exists('memory_get_usage'))@ini_set("memory_limit",max(ini_bytes("memory_limit"),2*strlen($H)+memory_get_usage()+8e6));if($H!=""&&strlen($H)<1e6){$G=$H.(ereg(";[ \t\r\n]*\$",$H)?"":";");if(!$wc||reset(end($wc))!=$G){restart_session();$wc[]=array($G,time());set_session("queries",$xc);stop_session();}}$cf="(?:\\s|/\\*.*\\*/|(?:#|-- )[^\n]*\n|--\n)";$rb=";";$Ad=0;$Kb=true;$g=connect();if(is_object($g)&&DB!="")$g->select_db(DB);$Va=0;$Qb=array();$cd=0;$ae='[\'"'.($w=="sql"?'`#':($w=="sqlite"?'`[':($w=="mssql"?'[':''))).']|/\\*|-- |$'.($w=="pgsql"?'|\\$[^$]*\\$':'');$Hf=microtime();parse_str($_COOKIE["adminer_export"],$ka);$Cb=$b->dumpFormat();unset($Cb["sql"]);while($H!=""){if(!$Ad&&preg_match("~^$cf*DELIMITER\\s+(\\S+)~i",$H,$A)){$rb=$A[1];$H=substr($H,strlen($A[0]));}else{preg_match('('.preg_quote($rb)."\\s*|$ae)",$H,$A,PREG_OFFSET_CAPTURE,$Ad);list($mc,$je)=$A[0];if(!$mc&&$oc&&!feof($oc))$H.=fread($oc,1e5);else{if(!$mc&&rtrim($H)=="")break;$Ad=$je+strlen($mc);if($mc&&rtrim($mc)!=$rb){while(preg_match('('.($mc=='/*'?'\\*/':($mc=='['?']':(ereg('^-- |^#',$mc)?"\n":preg_quote($mc)."|\\\\."))).'|$)s',$H,$A,PREG_OFFSET_CAPTURE,$Ad)){$Re=$A[0][0];if(!$Re&&$oc&&!feof($oc))$H.=fread($oc,1e5);else{$Ad=$A[0][1]+strlen($Re);if($Re[0]!="\\")break;}}}else{$Kb=false;$G=substr($H,0,$je);$Va++;$qe="<pre id='sql-$Va'><code class='jush-$w'>".shorten_utf8(trim($G),1000)."</code></pre>\n";if(!$_POST["only_errors"]){echo$qe;ob_flush();flush();}$ef=microtime();if($f->multi_query($G)&&is_object($g)&&preg_match("~^$cf*USE\\b~isU",$G))$g->query($G);do{$I=$f->store_result();$Lb=microtime();$Af=format_time($ef,$Lb).(strlen($G)<1000?" <a href='".h(ME)."sql=".urlencode(trim($G))."'>".'Ändern'."</a>":"");if($f->error){echo($_POST["only_errors"]?$qe:""),"<p class='error'>".'Fehler in der SQL-Abfrage'.": ".error()."\n";$Qb[]=" <a href='#sql-$Va'>$Va</a>";if($_POST["error_stops"])break
2;}elseif(is_object($I)){$Qd=select($I,$g);if(!$_POST["only_errors"]){echo"<form action='' method='post'>\n","<p>".($I->num_rows?lang(array('%d Datensatz','%d Datensätze'),$I->num_rows):"").$Af;$s="export-$Va";$Zb=", <a href='#$s' onclick=\"return !toggle('$s');\">".'Exportieren'."</a><span id='$s' class='hidden'>: ".html_select("output",$b->dumpOutput(),$ka["output"])." ".html_select("format",$Cb,$ka["format"])."<input type='hidden' name='query' value='".h($G)."'>"." <input type='submit' name='export' value='".'Exportieren'."'><input type='hidden' name='token' value='$T'></span>\n";if($g&&preg_match("~^($cf|\\()*SELECT\\b~isU",$G)&&($Yb=explain($g,$G))){$s="explain-$Va";echo", <a href='#$s' onclick=\"return !toggle('$s');\">EXPLAIN</a>$Zb","<div id='$s' class='hidden'>\n";select($Yb,$g,($w=="sql"?"http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/explain-output.html#explain_":""),$Qd);echo"</div>\n";}else
echo$Zb;echo"</form>\n";}}else{if(preg_match("~^$cf*(CREATE|DROP|ALTER)$cf+(DATABASE|SCHEMA)\\b~isU",$G)){restart_session();set_session("dbs",null);stop_session();}if(!$_POST["only_errors"])echo"<p class='message' title='".h($f->info)."'>".lang(array('Abfrage ausgeführt, %d Datensatz betroffen.','Abfrage ausgeführt, %d Datensätze betroffen.'),$f->affected_rows)."$Af\n";}$ef=$Lb;}while($f->next_result());$cd+=substr_count($G.$mc,"\n");$H=substr($H,$Ad);$Ad=0;}}}}if($Kb)echo"<p class='message'>".'Kein Kommando vorhanden.'."\n";elseif($_POST["only_errors"])echo"<p class='message'>".lang(array('SQL-Query erfolgreich ausgeführt.','%d SQL-Queries erfolgreich ausgeführt.'),$Va-count($Qb)).format_time($Hf,microtime())."\n";elseif($Qb&&$Va>1)echo"<p class='error'>".'Fehler in der SQL-Abfrage'.": ".implode("",$Qb)."\n";}else
...
<input type="hidden" name="token" value="',$T,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Im Server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ya=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$x=>$X){if(extension_loaded($X))$Ya[]=".$x";}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Ya?"[".implode("|",$Ya)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($wc){print_fieldset("history",'History',$_GET["history"]!="");foreach($wc
as$x=>$X){list($G,$Af)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Ändern'."</a> <span class='time' title='".@date('Y-m-d',$Af)."'>".@date("H:i:s",$Af)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$Yf?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Yf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Datensatz gelöscht.');else{$O=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$O[idf_escape($D)]=($Yf?"\n".idf_escape($D)." = $X":$X);}if($Yf){if(!$O)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Datensatz geändert.');}else{$I=insert_into($a,$O);$Vc=($I?last_id():0);queries_redirect($_,sprintf('Datensatz%s hinzugefügt.',($Vc?" $Vc":"")),$I);}}}$qf=$b->tableName(table_status($a));page_header(($Yf?'Ändern':'Hinzufügen'),$j,array("select"=>array($a,$qf)),$qf);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($l
as$D=>$k){if(isset($k["privileges"]["select"])){$ta=convert_field($k);if($_POST["clone"]&&$k["auto_increment"])$ta="''";if($w=="sql"&&ereg("enum|set",$k["type"]))$ta="1*".idf_escape($D);$M[]=($ta?"$ta AS ":"").idf_escape($D);}}$K=array();if($M){$L=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$K=(isset($_GET["select"])&&count($L)!=1?null:reset($L));}}if($K===false)echo"<p class='error'>".'Keine Daten.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($l){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Yf?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}echo($Yf?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<p>
Auto-Inkrement: <input name="Auto_increment" size="6" value="',h($K["Auto_increment"]),'">
...
';if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?checkbox("comments",1,$Xa,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($K["Comment"]).'" maxlength="'.($f->server_info>=5.5?2048:60).'"'.($Xa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
';if(support("partitioning")){$ce=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Partitionieren um',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$be,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Partitionen: <input type="number" name="partitions" class="size" value="',h($K["partitions"]),'"',($ce||!$K["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$j&&!isset($_POST["add_x"])){restart_session();$D=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$D){if(DB!=""){$_GET["db"]=$D;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($D),'Datenbank umbenannt.',rename_database($D,$_POST["collation"]));}else{$h=explode("\n",str_replace("\r","",$D));$jf=true;$Uc="";foreach($h
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$jf=false;$Uc=$i;}}queries_redirect(ME."db=".urlencode($Uc),'Datenbank erstellt.',$jf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$j,array(),DB);$d=collations();$D=DB;$Qa=null;if($_POST){$D=$_POST["name"];$Qa=$_POST["collation"];}elseif(DB!="")$Qa=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$q,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($D,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($D).'</textarea><br>':'<input id="name" name="name" value="'.h($D).'" maxlength="64">')."\n".($d?html_select("collation",array(""=>"(".'Kollation'.")")+$d,$Qa):"");?>
...
<tr><th>Ende<td><input name="ENDS" value="',h($K["ENDS"]),'">
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Ic,$K["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$gf,$K["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",'Nach der Ausführung erhalten'),'</table>
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($K["name"]),'" maxlength="64">
',($Oe?'Sprache'.": ".html_select("language",$Oe,$K["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$d,$Ne);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$K["returns"],$d);}echo'</table>
<p>';textarea("definition",$K["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($Ab){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$Mf=trigger_options();$Lf=array("INSERT","UPDATE","DELETE");$Ab=false;if($_POST&&!$j&&in_array($_POST["Timing"],$Mf["Timing"])&&in_array($_POST["Event"],$Lf)&&in_array($_POST["Type"],$Mf["Type"])){$Bf=" $_POST[Timing] $_POST[Event]";$Fd=" ON ".table($a);$Ab=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($w=="pgsql"?$Fd:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($w=="mssql"?$Fd.$Bf:$Bf.$Fd).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger entfernt.','Trigger geändert.','Trigger erstellt.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Trigger ändern'.": ".h($_GET["name"]):'Trigger hinzufügen'),$j,array("table"=>$a));$K=$_POST;if(!$K)$K=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Mf["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Lf,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Mf["Type"],$K["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($K["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$K["Statement"]);echo'<p>
<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($Ab){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE")$rc["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$rc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$Dd=$A[1];}}if($_POST&&!$j){$Ed=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$xd=q($_POST["user"])."@".q($_POST["host"]);$fe=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $Ed",ME."privileges=",'Benutzer entfernt.');else{$hb=false;if($Ed!=$xd){$hb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $xd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $fe");$j=!$hb;}elseif($_POST["pass"]!=$Dd||!$_POST["hashed"])queries("SET PASSWORD FOR $xd = ".($_POST["hashed"]?$fe:"PASSWORD($fe)"));if(!$j){$Ke=array();foreach($wd
...
as$_d=>$Ke){if(preg_match('~^(.+)(\\(.*\\))?$~U',$_d,$A))grant("REVOKE",array_keys($Ke),$A[2]," ON $A[1] FROM $xd");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Benutzer geändert.':'Benutzer erstellt.'),!$j);if($hb)$f->query("DROP USER $xd");}}page_header((isset($_GET["host"])?'Benutzer'.": ".h("$fa@$_GET[host]"):'Neuer Benutzer'),$j,array("privileges"=>array('','Rechte')));if($_POST){$K=$_POST;$rc=$wd;}else{$K=$_GET+array("host"=>$f->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$K["pass"]=$Dd;if($Dd!="")$K["hashed"]=true;$rc[(DB==""||$rc?"":idf_escape(addcslashes(DB,"%_"))).".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>Server<td><input name="host" maxlength="60" value="',h($K["host"]),'">
<tr><th>Benutzer<td><input name="user" maxlength="16" value="',h($K["user"]),'">
<tr><th>Passwort<td><input id="pass" name="pass" value="',h($K["pass"]),'">
';if(!$K["hashed"]){echo'<script type="text/javascript">typePassword(document.getElementById(\'pass\'));</script>';}echo
checkbox("hashed",1,$K["hashed"],'Hashed',"typePassword(this.form['pass'], this.checked);"),'</table>

';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$r=0;foreach($rc
as$_d=>$q){echo'<th>'.($_d!="*.*"?"<input name='objects[$r]' value='".h($_d)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$cb=>$sb){foreach((array)$te[$cb]as$se=>$Wa){echo"<tr".odd()."><td".($sb?">$sb<td":" colspan='2'").' lang="en" title="'.h($Wa).'">'.h($se);$r=0;foreach($rc
as$_d=>$q){$D="'grants[$r][".h(strtoupper($se))."]'";$Y=$q[strtoupper($se)];if($cb=="Server Admin"&&$_d!=(isset($rc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Verbieten'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Rc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Rc++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Rc),$Rc||!$_POST["kill"]);}page_header('Prozessliste',$j);echo'
<form action="" method="post">
...
as$x=>$X)echo"<td>".(($w=="sql"&&$x=="Info"&&ereg("Query|Killed",$K["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($K["db"]!=""?"db=".urlencode($K["db"])."&":"")."sql=".urlencode($X)).'">'.'Ändern'.'</a>':nbsp($X));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('%d insgesamt',$f->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Anhalten'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$D=>$X){$X=process_input($l[$D]);if($X!==null){if($_POST["clone"])$O[idf_escape($D)]=($X!==false?$X:idf_escape($D));elseif($X!==false)$O[]=idf_escape($D)." = $X";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ua="UPDATE";if($_POST["delete"]){$Ua="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ua="INSERT";$H="INTO $H";}if($_POST["all"]||($Wf===array()&&$_POST["check"])||$Jc){$I=queries("$Ua $H".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $kg"));$ma=$f->affected_rows;}else{foreach((array)$_POST["check"]as$X){$I=queries($Ua.limit1($H,"\nWHERE ".where_check($X,$l)));if(!$I)break;$ma+=$f->affected_rows;}}}$B=sprintf('%d Artikel betroffen.',$ma);if($_POST["clone"]&&$I&&$ma==1){$Vc=last_id();if($Vc)$B=sprintf('Datensatz%s hinzugefügt.'," $Vc");}queries_redirect(remove_from_uri("page"),$B,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$j='Ctrl+klick zum Bearbeiten des Wertes.';else{$I=true;$ma=0;foreach($_POST["val"]as$Uf=>$K){$O=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$O[]=idf_escape($x)." = ".(ereg('char|text',$l[$x]["type"])||$X!=""?$b->processInput($l[$x],$X):"NULL");}$H=table($a)." SET ".implode(", ",$O);$jg=" WHERE ".where_check($Uf,$l).($Z?" AND ".implode(" AND ",$Z):"");$I=queries("UPDATE".($Jc?" $H$jg":limit1($H,$jg)));if(!$I)break;$ma+=$f->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d Artikel betroffen.',$ma),$I);}}elseif(is_string($ec=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($la["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Sa=array_keys($l);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$ec,$gd);$ma=count($gd[0]);begin();$Xe=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($gd[0]as$x=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Xe]*)$Xe~",$X.$Xe,$hd);if(!$x&&!array_diff($hd[1],$Sa)){$Sa=$hd[1];$ma--;}else{$O=array();foreach($hd[1]as$r=>$Pa)$O[idf_escape($Sa[$r])]=($Pa==""&&$l[$Sa[$r]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Pa))));$I=insert_update($a,$O,$pe);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('%d Datensatz importiert.','%d Datensätze wurden importiert.'),$ma),$I);queries("ROLLBACK");}else$j=upload_error($ec);}}$qf=$b->tableName($S);if(is_ajax())ob_start();page_header('Daten zeigen von'.": $qf",$j);$O=null;if(isset($Le["insert"])){$O="";foreach((array)$_GET["where"]as$X){if(count($n[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!ereg('[_%]',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e)echo"<p class='error'>".'Auswahl der Tabelle fehlgeschlagen'.($l?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$u);$b->selectOrderPrint($Nd,$e,$u);$b->selectLimitPrint($y);$b->selectLengthPrint($_f);$b->selectActionPrint($u);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$nc=$f->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$E=floor(max(0,$nc-1)/$y);}$H=$b->selectQueryBuild($M,$Z,$sc,$Nd,$y,$E);if(!$H)$H="SELECT".limit((+$y&&$sc&&$Jc&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").$pc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$tc,($y!=""?+$y:null),($E?$y*$E:0),"\n");echo$b->selectQuery($H);$I=$f->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql")$I->seek($y*$E);$Jb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$nc=(+$y&&$sc&&$Jc?($w=="sql"?$f->result(" SELECT FOUND_ROWS()"):$f->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Keine Daten.'."\n";else{$_a=$b->backwardKeys($a,$qf);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$sc&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ändern'."</a>");$vd=array();$p=array();reset($M);$ye=1;foreach($L[0]as$x=>$X){if($x!=$Bd){$X=$_GET["columns"][key($M)];$k=$l[$M?($X?$X["col"]:current($M)):$x];$D=($k?$b->fieldName($k,$ye):"*");if($D!=""){$ye++;$vd[$x]=$D;$Ta=idf_escape($x);$zc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$sb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($zc.($Nd[0]==$Ta||$Nd[0]==$x||(!$Nd&&$Jc&&$sc[0]==$Ta)?$sb:'')).'">';echo(!$M||$X?apply_sql_function($X["fun"],$D):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($zc.$sb)."' title='".'absteigend'."' class='text'> ↓</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.'Suchen'.'" class="text jsonly"> =</a>';echo"</span>";}$p[$x]=$X["fun"];next($M);}}$bd=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$bd[$x]=max($bd[$x],min(40,strlen(utf8_decode($X))));}}echo($_a?"<th>".'Relationen':"")."</thead>\n";if(is_ajax()){if($y%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$n)as$C=>$K){$Tf=unique_array($L[$C],$u);$Uf="";foreach($Tf
as$x=>$X)$Uf.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));echo"<tr".odd().">".(!$sc&&$M?"":"<td>".checkbox("check[]",substr($Uf,1),in_array(substr($Uf,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Jc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Uf)."'>".'ändern'."</a>"));foreach($K
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$nc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$lc=$b->dumpFormat();if($lc){print_fieldset("export",'Exportieren');$Wd=$b->dumpOutput();echo($Wd?html_select("output",$Wd,$la["output"])." ":""),html_select("format",$lc,$la["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Jb,'strlen'),$e);echo"<p><input type='hidden' name='token' value='$T'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$ff=isset($_GET["status"]);page_header($ff?'Status':'Variablen');$eg=($ff?show_status():show_variables());if(!$eg)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($eg
...
as$x=>$X)json_row("sum-$x",number_format($X,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$f->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$i=>$X)json_row("tables-".js_escape($i),$X);json_row("");}exit;}else{$wf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($wf&&!$j&&!$_POST["search"]){$I=true;$B="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$B='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$B='Tabellen verschoben.';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$B='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$B='Tabellen wurden entfernt (drop).';}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$B='Tables have been optimized.';}elseif($_POST["tables"]&&($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($K=$I->fetch_assoc())$B.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$B,$I);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$j,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabellen und Views'."</h3>\n";$vf=tables_list();if(!$vf)echo"<p class='message'>".'Keine Tabellen.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'Datengrösse','<td>'.'Indexgrösse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'Datensätze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";foreach($vf
as$D=>$U){$fg=($U!==null&&!eregi("table",$U));echo'<tr'.odd().'><td>'.checkbox(($fg?"views[]":"tables[]"),$D,in_array($D,$wf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($D).'" title="'.'Struktur anzeigen'.'">'.h($D).'</a>';if($fg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($D).'" title="'.'View ändern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($D).'" title="'.'Daten auswählen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle ändern'),"Index_length"=>array("indexes",'Indizes ändern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle ändern'),"Rows"=>array("select",'Daten auswählen'),)as$x=>$z)echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($D)."' id='$x-".h($D)."' title='$z[1]'>?</a>":"<td id='$x-".h($D)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($D)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($vf)),"<td>".nbsp($w=="sql"?$f->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$w)?($w!="sqlite"?"<input type='submit' value='".'Analysieren'."'> ":"")."<input type='submit' name='optimize' value='".'Optimieren'."'> ":"").($w=="sql"?"<input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)").">\n";$h=(support("scheme")?schemas():$b->databases());if(count($h)!=1&&$w!="sqlite"){$i=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($h?html_select("target",$h,$i):'<input name="target" value="'.h($i).'">')," <input type='submit' name='move' value='".'Verschieben'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$T'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$Pe=routines();if($Pe){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($Pe
as$K){echo'<tr'.odd().'>','<th><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($K["ROUTINE_NAME"]).'">'.h($K["ROUTINE_NAME"]).'</a>','<td>'.h($K["ROUTINE_TYPE"]),'<td>'.h($K["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($K["ROUTINE_NAME"]).'">'.'Ändern'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Neue Prozedur'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Neue Funktion'."</a>\n";}if(support("event")){echo"<h3>".'Ereignisse'."</h3>\n";$L=get_rows("SHOW EVENTS");if($L){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Name'."<td>".'Zeitplan'."<td>".'Start'."<td>".'Ende'."</thead>\n";foreach($L
as$K){echo"<tr>",'<th><a href="'.h(ME).'event='.urlencode($K["Name"]).'">'.h($K["Name"])."</a>","<td>".($K["Execute at"]?'Zur angegebenen Zeit'."<td>".$K["Execute at"]:'Jede'." ".$K["Interval value"]." ".$K["Interval field"]."<td>$K[Starts]"),"<td>$K[Ends]";}echo"</table>\n";$Sb=$f->result("SELECT @@event_scheduler");if($Sb&&$Sb!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Sb)."\n";}echo'<p><a href="'.h(ME).'event=">'.'Ereignis erstellen'."</a>\n";}if($vf)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}}}page_footer();