...
upload_error($p){$Fd=($p==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($p?'Hochladen von Datei fehlgeschlagen.'.($Fd?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Fd):""):'Datei existiert nicht.');}function
...
input($d,$n,$B){global$na,$q,$x;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$R=(isset($_GET["select"])?array("orig"=>'Original'):array())+$q->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($kf,$Lb){$Kb=($Lb==1||(!$Lb&&'de'=='fr')?0:((!$Lb||$Lb>=5)&&ereg('cs|sk|ru','de')?2:1));return
...
loginForm(){global$ya;?>
<table cellspacing="0">
<tr><th>Datenbank System<td><input type='hidden' name='driver' value='server'>MySQL
...
selectLinks($zc,$t=""){echo'<p class="tabs">';$ua=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($zc)){$ua["view"]='View ändern';}else{$ua["create"]='Tabelle ändern';}if(isset($t)){$ua["edit"]='Neuer Datensatz';}foreach($ua
...
selectQuery($k){global$x;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Seite'.": ".'letzte'."'>&gt;&gt;</a> <code class='jush-$x'>".h(str_replace("\n"," ",$k))."</code> <a href='".h(ME)."sql=".urlencode($k)."'>".'Ändern'."</a>\n";}function
rowDescription($m){return"";}function
rowDescriptions($D,$Xe){return$D;}function
selectVal($b,$_,$d){$e=($b!="<i>NULL</i>"&&ereg("^char|binary",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$e=lang(array('%d Byte','%d Bytes'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($_?"<a href='$_'>$e</a>":$e);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($u,$v){global$R,$Hb;print_fieldset("select",'Daten zeigen von',$u);$h=0;$ae=array('Funktionen'=>$R,'Agregationen'=>$Hb);foreach($u
...
selectSearchPrint($y,$v,$z){print_fieldset("search",'Suchen',$y);foreach($z
...
selectOrderPrint($Za,$v,$z){print_fieldset("sort",'Ordnen',$Za);$h=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$h]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$h]",1,isset($_GET["desc"][$c]),'absteigend')."</div>\n";$h++;}}echo"<div><select name='order[$h]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$h]",1,0,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($Q){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($Q)."'>","</div></fieldset>\n";}function
selectLengthPrint($fb){if(isset($fb)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($fb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($k){global$x;restart_session();$E="sql-".count($_SESSION["messages"]);$Ta=&get_session("queries");$Ta[$_GET["db"]][]=(strlen($k)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($k,0,1e6))."\n...":$k);return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL-Query'."</a><div id='$E' class='hidden'><pre class='jush-$x'>".shorten_utf8($k,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ta[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Ca,$n){if($d["type"]=="enum"){return($d["null"]?"<label><input type='radio'$Ca value=''".(isset($n)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Ca value='0'".($n===0?" checked":"")."><i>".'leer'."</i></label>";}return"";}function
...
dumpOutput($u,$n=""){$e=array('text'=>'anzeigen','file'=>'Datei');if(function_exists('gzencode')){$e['gz']='gzip';}if(function_exists('bzcompress')){$e['bz2']='bzip2';}return
...
<form action="">
<p>
';hidden_fields_get();echo($A?html_select("db",array(""=>"(".'Datenbank'.")")+$A,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Benutzung"',($A?" class='hidden'":""),'>
';if($xc!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""){$wa=tables_list();if(!$wa){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($wa);$ua=array();foreach($wa
...
as$m=>$N){echo'<a href="'.h(ME).'select='.urlencode($m).'">'.bold('zeigen',$_GET["select"]==$m).'</a> ','<a href="'.h(ME).'table='.urlencode($m).'">'.bold($this->tableName(array("Name"=>$m)),$_GET["table"]==$m)."</a><br>\n";}}}$q=(function_exists('adminer_object')?adminer_object():new
...
auth_error($he=null){global$i,$q,$C;$rc=session_name();$p="";if(!$_COOKIE[$rc]&&$_GET[$rc]&&ini_bool("session.use_only_cookies")){$p='Sitzungen müssen aktiviert sein.';}elseif(isset($_GET["username"])){if(($_COOKIE[$rc]||$_GET[$rc])&&!$C){$p='Sitzungsdauer abgelaufen, bitte erneut anmelden.';}else{$ka=&get_session("pwds");if(isset($ka)){$p=h($he?$he->getMessage():(is_string($i)?$i:'Ungültige Anmelde-Informationen.'));$ka=null;}}}page_header('Login',$p,null);echo"<form action='' method='post'>\n";$q->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])&&class_exists("Min_DB")){$i=connect();}if(is_string($i)||!$q->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$C=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$C;}$p=($_POST?($_POST["token"]==$C?"":'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('POST data zu gross. Reduzieren Sie die Grösse oder vergrössern Sie den Wert %s in der Konfiguration.','"post_max_size"')));function
connect_error(){global$i,$C,$p,$ya;$A=array();if(DB!=""){page_header('Datenbank'.": ".h(DB),'Datenbank ungültig.',true);}else{if($_POST["db"]&&!$p){set_session("dbs",null);queries_redirect(substr(ME,0,-1),'Datenbanken entfernt.',drop_databases($_POST["db"]));}page_header('Datenbank auswählen',$p,false);echo"<p><a href='".h(ME)."database='>".'Neue Datenbank'."</a>\n";foreach(array('privileges'=>'Rechte','processlist'=>'Prozessliste','variables'=>'Variablen','status'=>'Status',)as$c=>$b){if(support($c)){echo"<a href='".h(ME)."$c='>$b</a>\n";}}echo"<p>".sprintf('Version %s: %s, mit PHP-Erweiterung %s',$ya[DRIVER],"<b>$i->server_info</b>","<b>$i->extension</b>")."\n","<p>".sprintf('Angemeldet als: %s',"<b>".h(logged_user())."</b>")."\n";$A=get_databases();if($A){$T=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td><input type='hidden' name='token' value='$C'>&nbsp;<th>".'Datenbank'."<td>".'Collation'."<td>".'Tabellen'."</thead>\n";foreach($A
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Zd[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($na[$c]==254){$b="<code>$b</code>";}}if(isset($ua[$c])&&!$v[$ua[$c]]){$_="edit=".urlencode($ua[$c]);foreach($z[$ua[$c]]as$nc=>$W){$_.="&where".urlencode("[".bracket_escape($nc)."]")."=".urlencode($a[$W]);}$b="<a href='".h(ME.$_)."'>$b</a>";}}echo"<td>$b";}}echo($h?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($c,$d,$T,$M=array()){global$ld,$na,$Ib,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($na[$d["type"]])?array():array($d["type"]))+$ld+($M?array('Fremdschlüssel'=>$M):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($T,$d["collation"]).'</select>',($Ib?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Ib,$d["unsigned"]).'</select>':''),($M?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($Ma,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($nb?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($nb?"":" class='hidden'")."><input name='fields[$h][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $Td, 1);'>&nbsp;"."<input type='image' name='up[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($kd==""||support("drop_col")?"<input type='image' name='drop_col[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$nb;}function
...
dump_headers($ze,$Ke=false){$Ua=($ze!=""?friendly_url($ze):"adminer");$lb=$_POST["output"];$eb=($_POST["format"]=="sql"?"sql":($Ke?"tar":"csv"));header("Content-Type: ".($lb=="bz2"?"application/x-bzip":($lb=="gz"?"application/x-gzip":($eb=="tar"?"application/x-tar":($eb=="sql"||$lb!="file"?"text/plain":"text/csv")."; charset=utf-8"))));if($lb!="text"){header("Content-Disposition: attachment; filename=$Ua.$eb".($lb!="file"&&!ereg('[^0-9a-z]',$lb)?".$lb":""));}session_write_close();if($_POST["output"]=="bz2"){ob_start('bzcompress',1e6);}if($_POST["output"]=="gz"){ob_start('gzencode',1e6);}return$eb;}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Ma=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$bb=" onclick=\"return confirm('".'Sind Sie sicher ?'."');\"";$ob='\'(?:\'\'|[^\'\\\\]|\\\\.)*\'|"(?:""|[^"\\\\]|\\\\.)*"';$Pb=array("IN","OUT","INOUT");if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["callf"])){$_GET["call"]=$_GET["callf"];}if(isset($_GET["function"])){$_GET["procedure"]=$_GET["function"];}if(isset($_GET["download"])){$g=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$g-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$i->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($g)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["table"])){$g=$_GET["table"];$o=fields($g);if(!$o){$p=error();}$L=($o?table_status($g):array());page_header(($o&&is_view($L)?'View':'Tabelle').": ".h($g),$p);$q->selectLinks($L);$Xa=$L["Comment"];if($Xa!=""){echo"<p>".'Kommentar'.": ".h($Xa)."\n";}if($o){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Spalte'."<td>".'Typ'.(support("comment")?"<td>".'Kommentar':"")."</thead>\n";foreach($o
as$d){echo"<tr".odd()."><th>".h($d["field"]),"<td>".h($d["full_type"]).($d["null"]?" <i>NULL</i>":"").($d["auto_increment"]?" <i>".'Auto-Inkrement'."</i>":""),(support("comment")?"<td>".nbsp($d["comment"]):""),"\n";}echo"</table>\n";if(!is_view($L)){echo"<h3>".'Indizes'."</h3>\n";$z=indexes($g);if($z){echo"<table cellspacing='0'>\n";foreach($z
as$f=>$s){ksort($s["columns"]);$cc=array();foreach($s["columns"]as$c=>$b){$cc[]="<i>".h($b)."</i>".($s["lengths"][$c]?"(".$s["lengths"][$c].")":"");}echo"<tr title='".h($f)."'><th>$s[type]<td>".implode(", ",$cc)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($g).'">'.'Indizes ändern'."</a>\n";if(fk_support($L)){echo"<h3>".'Fremdschlüssel'."</h3>\n";$M=foreign_keys($g);if($M){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Ursprung'."<td>".'Ziel'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($x!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($M
as$f=>$F){$_=($F["db"]!=""?"<b>".h($F["db"])."</b>.":"").h($F["table"]);echo"<tr>","<th><i>".implode("</i>, <i>",array_map('h',$F["source"]))."</i>","<td><a href='".h($F["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($F["db"]),ME):ME)."table=".urlencode($F["table"])."'>$_</a>","(<i>".implode("</i>, <i>",array_map('h',$F["target"]))."</i>)","<td>$F[on_delete]\n","<td>$F[on_update]\n";if($x!="sqlite"){echo'<td><a href="'.h(ME.'foreign='.urlencode($g).'&name='.urlencode($f)).'">'.'Ändern'.'</a>';}}echo"</table>\n";}if($x!="sqlite"){echo'<p><a href="'.h(ME).'foreign='.urlencode($g).'">'.'Fremdschlüssel hinzufügen'."</a>\n";}}if(support("trigger")){echo"<h3>".'Trigger'."</h3>\n";$Wb=triggers($g);if($Wb){echo"<table cellspacing='0'>\n";foreach($Wb
as$c=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($c)."<td><a href='".h(ME.'trigger='.urlencode($g).'&name='.urlencode($c))."'>".'Ändern'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($g).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB);$Va=array();$we=array();preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$_COOKIE["adminer_schema"],$aa,PREG_SET_ORDER);foreach($aa
...
<table cellspacing="0">
';$ud=array('','USE','DROP+CREATE','CREATE');$zd=array('','DROP+CREATE','CREATE');$vd=array('','TRUNCATE+INSERT','INSERT');if($x=="sql"){$ud[]='CREATE+ALTER';$zd[]='CREATE+ALTER';$vd[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$a);if(!$a){$a=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}$sa=($_GET["dump"]=="");echo"<tr><th>".'Ergebnis'."<td>".$q->dumpOutput(0,$a["output"])."\n","<tr><th>".'Format'."<td>".$q->dumpFormat(0,$a["format"])."\n",($x=="sqlite"?"":"<tr><th>".'Datenbank'."<td>".html_select('db_style',$ud,$a["db_style"]).(support("routine")?checkbox("routines",1,$sa,'Prozeduren'):"").(support("event")?checkbox("events",1,$sa,'Ereignisse'):"")),"<tr><th>".'Tabellen'."<td>".html_select('table_style',$zd,$a["table_style"]).checkbox("auto_increment",1,$a["table_style"],'Auto-Inkrement').(support("trigger")?checkbox("triggers",1,$a["table_style"],'Trigger'):""),"<tr><th>".'Daten'."<td>".html_select('data_style',$vd,$a["data_style"]),'</table>
<p><input type="submit" value="Exportieren">

<table cellspacing="0">
';$Ic=array();if(DB!=""){$sa=($g!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$sa onclick='formCheck(this, /^tables\\[/);'>".'Tabellen'."</label>","<th style='text-align: right;'><label>".'Daten'."<input type='checkbox' id='check-data'$sa onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$db="";foreach(table_status()as$a){$f=$a["Name"];$Tb=ereg_replace("_.*","",$f);$sa=($g==""||$g==(substr($g,-1)=="%"?"$Tb%":$f));$cc="<tr><td>".checkbox("tables[]",$f,$sa,$f,"formUncheck('check-tables');");if(is_view($a)){$db.="$cc\n";}else{echo"$cc<td align='right'><label>".($a["Engine"]=="InnoDB"&&$a["Rows"]?"~ ":"").$a["Rows"].checkbox("data[]",$f,$sa,"","formUncheck('check-data');")."</label>\n";}$Ic[$Tb]++;}echo$db;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($g==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Datenbank'."</label></thead>\n";$A=get_databases();if($A){foreach($A
...
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten');print_fieldset("webfile",'Auf Server',$_POST["webfile"]);$Vc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Vc[]=".$c";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Vc?"[".implode("|",$Vc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($Ta){print_fieldset("history",'History',$_GET["history"]!="");foreach($Ta
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Ändern'."</a> <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Ha?"update":"insert"])||$q->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$p&&!isset($_GET["select"])){$U=$_POST["referer"];if($_POST["insert"]){$U=($Ha?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$U)){$U=ME."select=".urlencode($g);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($g)," WHERE $y"),$U,'Datensatz gelöscht.');}else{$t=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$t[idf_escape($f)]=($Ha?"\n".idf_escape($f)." = $b":$b);}}if($Ha){if(!$t){redirect($U);}query_redirect("UPDATE".limit1(table($g)." SET".implode(",",$t),"\nWHERE $y"),$U,'Datensatz geändert.');}else{$j=insert_into($g,$t);$Bd=($j?last_id():0);queries_redirect($U,sprintf('Datensatz%s hinzugefügt.',($Bd?" $Bd":"")),$j);}}}$oa=$q->tableName(table_status($g));page_header(($Ha?'Ändern':'Hinzufügen'),$p,array("select"=>array($g,$oa)),$oa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($y){$u=array();foreach($o
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($o){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($Ha?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."\">\n";}}if($Ha){echo"<input type='submit' name='delete' value='".'Entfernen'."'$bb>\n";}echo'</form>
...
<?php echo(support("comment")?checkbox("","",$nb,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($nb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Entfernen"',$bb,'>';}if(support("partitioning")){$od=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partitionieren um',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Ad,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($od||!$a["partition_by"]?" class='hidden'":""),'>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',$bb,'>';}echo'<input type="hidden" name="token" value="',$C,'">
</form>
';}elseif(isset($_GET["view"])){$g=$_GET["view"];$Ea=false;if($_POST&&!$p){$Ea=drop_create("DROP VIEW ".table($g),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'View entfernt.','View geändert.','View erstellt.',$g);}page_header(($g!=""?'View ändern':'Neue View erstellen'),$p,array("table"=>$g),$g);$a=array();if($_POST){$a=$_POST;}elseif($g!=""){$a=view($g);$a["name"]=$g;}echo'
<form action="" method="post">
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<p>';textarea("select",$a["select"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
</form>
';}elseif(isset($_GET["event"])){$Oa=$_GET["event"];$_d=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$ed=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$p){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Oa),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$_d)&&isset($ed[$_POST["STATUS"]])){$rd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Oa!=""?"ALTER EVENT ".idf_escape($Oa).$rd.($Oa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$rd)."\n".$ed[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Oa!=""?'Ereignis geändert.':'Ereignis erstellt.'));}}page_header(($Oa!=""?'Ereignis ändern'.": ".h($Oa):'Ereignis erstellen'),$p);$a=array();if($_POST){$a=$_POST;}elseif($Oa!=""){$D=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Oa));$a=reset($D);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_d,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$ed,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if($Oa!=""){echo'<input type="submit" name="drop" value="Entfernen"',$bb,'>';}echo'</form>
...
as$d){if($d["field"]!=""){$t[]=(in_array($d["inout"],$Pb)?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Ea=drop_create("DROP $za ".idf_escape($Aa),"CREATE $za ".idf_escape($_POST["name"])." (".implode(", ",$t).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Prozedur entfernt.','Prozedur geändert.','Prozedur erstellt.',$Aa);}page_header(($Aa!=""?(isset($_GET["function"])?'Funktion ändern':'Prozedur ändern').": ".h($Aa):(isset($_GET["function"])?'Neue Funktion':'Neue Prozedur')),$p);$T=get_vals("SHOW CHARACTER SET");sort($T);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Aa!=""){$a=routine($Aa,$za);$a["name"]=$Aa;}echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$T,$za);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$a["returns"],$T);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($Aa!=""){echo'<input type="submit" name="drop" value="Entfernen"',$bb,'>';}echo'</form>
';}elseif(isset($_GET["type"])){$dc=$_GET["type"];if($_POST&&!$p){$_=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($dc),$_,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$_,'Typ erstellt.');}}page_header($dc!=""?'Typ ändern'.": ".h($dc):'Typ erstellen',$p);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$C,'">
';if($dc!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'$bb>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Speichern'."'>\n";}echo'</form>
';}elseif(isset($_GET["trigger"])){$g=$_GET["trigger"];$fc=trigger_options();$qd=array("INSERT","UPDATE","DELETE");$Ea=false;if($_POST&&!$p&&in_array($_POST["Timing"],$fc["Timing"])&&in_array($_POST["Event"],$qd)&&in_array($_POST["Type"],$fc["Type"])){$td=" $_POST[Timing] $_POST[Event]";$gb=" ON ".table($g);$Ea=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($x=="pgsql"?$gb:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($x=="mssql"?$gb.$td:$td.$gb)." $_POST[Type]\n$_POST[Statement]",ME."table=".urlencode($g),'Trigger entfernt.','Trigger geändert.','Trigger erstellt.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Trigger ändern'.": ".h($_GET["name"]):'Trigger hinzufügen'),$p,array("table"=>$g));$a=array("Trigger"=>$g."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$fc["Timing"],$a["Timing"],"if (/^".h(preg_quote($g,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($g,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$qd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$fc["Type"],$a["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',$bb,'>';}echo'</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Rechte'."</a>";$h=0;foreach($Sa
as$Ia=>$O){echo'<th>'.($Ia!="*.*"?"<input name='objects[$h]' value='".h($Ia)."' size='10'>":"<input type='hidden' name='objects[$h]' value='*.*' size='10'>*.*");$h++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$ec=>$Bb){foreach((array)$X[$ec]as$Nb=>$Xa){echo"<tr".odd()."><td".($Bb?">$Bb<td":" colspan='2'").' lang="en" title="'.h($Xa).'">'.h($Nb);$h=0;foreach($Sa
as$Ia=>$O){$f="'grants[$h][".h(strtoupper($Nb))."]'";$n=$O[strtoupper($Nb)];if($ec=="Server Admin"&&$Ia!=(isset($Sa["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($n?" selected":"").">".'Erlauben'."<option value='0'".($n=="0"?" selected":"").">".'Verbieten'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($n?" checked":"").($Nb=="All privileges"?" id='grants-$h-all'":($Nb=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$h-all');\"")).">";}$h++;}}}echo"</table>\n",'<p>
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',$bb,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$p){$Mc=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$Mc++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Mc),$Mc||!$_POST["kill"]);}page_header('Prozessliste',$p);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$Ba,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Exportieren');echo$q->dumpOutput(1,$dd["output"])." ".$q->dumpFormat(1,$dd["format"]);echo" <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}print_fieldset("import",'Importiere CSV',!$D);echo"<input type='hidden' name='token' value='$C'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;"),$dd["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>\n","</div></fieldset>\n";$q->selectEmailPrint(array_filter($hc,'strlen'),$v);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$sb=isset($_GET["status"]);page_header($sb?'Status':'Variablen');$ie=($sb?show_status():show_variables());if(!$ie){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($ie
