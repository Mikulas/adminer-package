...
upload_error($j){$Yc=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Hochladen von Datei fehlgeschlagen.'.($Yc?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Yc):""):'Datei existiert nicht.');}function
...
selectLinks($Ye,$P=""){echo'<p class="tabs">';$Qc=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Ye))$Qc["view"]='View ändern';else$Qc["create"]='Tabelle ändern';if($P!==null)$Qc["edit"]='Neuer Datensatz';foreach($Qc
...
selectColumnsPrint($N,$e){global$hc,$lc;print_fieldset("select",'Daten zeigen von',$N);$r=0;$gc=array('Funktionen'=>$hc,'Agregationen'=>$lc);foreach($N
...
selectOrderPrint($Ad,$e,$u){print_fieldset("sort",'Ordnen',$Ad);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($e[$X])){echo"<div><select name='order[$r]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'absteigend')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($jf){if($jf!==null){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($jf).'">',"</div></fieldset>\n";}}function
...
edit_type($x,$k,$d,$n=array()){global$Re,$_f,$Gf,$ud;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($_f[$k["type"]])?array():array($k["type"]))+$Re+($n?array('Fremdschlüssel'=>$n):array()),$k["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($d,$k["collation"]).'</select>',($Gf?"<select name='$x"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Gf,$k["unsigned"]).'</select>':''),($n?"<select name='$x"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$ud),$k["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Sa?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Sa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Ed==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$x=>$X)echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Ändern'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$af=array();$bf=array();$D="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$D-".DB]?"$D-".DB:$D)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Tc,PREG_SET_ORDER);foreach($Tc
...
as$x=>$X){list($G,$kf)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Ändern'."</a> <span class='time'>".@date("H:i:s",$kf)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$Hf?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Hf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Datensatz gelöscht.');else{$P=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$P[idf_escape($D)]=($Hf?"\n".idf_escape($D)." = $X":$X);}if($Hf){if(!$P)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $Z"),$_,'Datensatz geändert.');}else{$I=insert_into($a,$P);$Ic=($I?last_id():0);queries_redirect($_,sprintf('Datensatz%s hinzugefügt.',($Ic?" $Ic":"")),$I);}}}$Ze=$b->tableName(table_status($a));page_header(($Hf?'Ändern':'Hinzufügen'),$j,array("select"=>array($a,$Ze)),$Ze);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$N=array();foreach($l
...
';if($l){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Hf?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}echo($Hf?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Pd=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Partitionieren um',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Od,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($K["partitions"]),'"',($Pd||!$K["partition_by"]?" class='hidden'":""),'>
...
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$Se=false;$Hc=$i;}}queries_redirect(ME."db=".urlencode($Hc),'Datenbank erstellt.',$Se);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$j,array(),DB);$d=collations();$D=DB;$La=null;if($_POST){$D=$_POST["name"];$La=$_POST["collation"];}elseif(DB!="")$La=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$q,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_c,$K["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Pe,$K["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($K["name"]),'" maxlength="64">
',($ze?'Sprache'.": ".html_select("language",$ze,$K["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$d,$ye);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$K["returns"],$d);}echo'</table>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$wf["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$vf,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$wf["Type"],$K["Type"]),'</table>
...
as$X){if($X[1]!="USAGE")$ic["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$ic["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$rd=$A[1];}}if($_POST&&!$j){$sd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$kd=q($_POST["user"])."@".q($_POST["host"]);$Sd=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $sd",ME."privileges=",'Benutzer entfernt.');else{$cb=false;if($sd!=$kd){$cb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $kd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Sd");$j=!$cb;}elseif($_POST["pass"]!=$rd||!$_POST["hashed"])queries("SET PASSWORD FOR $kd = ".($_POST["hashed"]?$Sd:"PASSWORD($Sd)"));if(!$j){$ve=array();foreach($jd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$r=0;foreach($ic
as$nd=>$q){echo'<th>'.($nd!="*.*"?"<input name='objects[$r]' value='".h($nd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Xa=>$mb){foreach((array)$ge[$Xa]as$fe=>$Ra){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Ra).'">'.h($fe);$r=0;foreach($ic
as$nd=>$q){$D="'grants[$r][".h(strtoupper($fe))."]'";$Y=$q[strtoupper($fe)];if($Xa=="Server Admin"&&$nd!=(isset($ic["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Erlauben'."<option value='0'".($Y=="0"?" selected":"").">".'Verbieten'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Ec=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Ec++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Ec),$Ec||!$_POST["kill"]);}page_header('Prozessliste',$j);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$dc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$cc=$b->dumpFormat();if($cc){print_fieldset("export",'Exportieren');$Jd=$b->dumpOutput();echo($Jd?html_select("output",$Jd,$la["output"])." ":""),html_select("format",$cc,$la["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$U'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Ab,'strlen'),$e);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Oe=isset($_GET["status"]);page_header($Oe?'Status':'Variablen');$Nf=($Oe?show_status():show_variables());if(!$Nf)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Nf
