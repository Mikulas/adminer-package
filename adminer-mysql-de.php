...
confirm($yc="",$pf=false){return" onclick=\"".($pf?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($yc?" (' + $yc + ')":"")."');\"";}function
js_escape($T){return
addcslashes($T,"\r\n'\\/");}function
...
upload_error($q){$me=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Hochladen von Datei fehlgeschlagen.'.($me?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$me):""):'Datei existiert nicht.');}function
...
enum_input($O,$Ca,$d,$o,$xb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$fa);$e=(isset($xb)?"<label><input type='$O'$Ca value='$xb'".((is_array($o)?in_array($xb,$o):$o===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($fa[1]as$g=>$b){$b=stripcslashes(str_replace("''","'",$b));$Ha=(is_int($o)?$o==$g+1:(is_array($o)?in_array($g+1,$o):$o===$b));$e.=" <label><input type='$O'$Ca value='".($g+1)."'".($Ha?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
...
lang($Qe,$tb){$hc=($tb==1||(!$tb&&'de'=='fr')?0:('de'=='sl'&&(!$tb||$tb>2)?1:0)+((!$tb||$tb>=5)&&ereg('cs|sk|ru|sl','de')?2:1));return
...
<tr><th>Datenbank System<td>',html_select("driver",$va,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'">
...
selectLinks($Ec,$u=""){echo'<p class="tabs">';$Aa=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Ec)){$Aa["view"]='View ändern';}else{$Aa["create"]='Tabelle ändern';}if(isset($u)){$Aa["edit"]='Neuer Datensatz';}foreach($Aa
...
selectColumnsPrint($C,$v){global$W,$Fb;print_fieldset("select",'Daten zeigen von',$C);$g=0;$ye=array('Funktionen'=>$W,'Agregationen'=>$Fb);foreach($C
...
as$g=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$g]' value='".h($_GET["fulltext"][$g])."'>",checkbox("boolean[$g]",1,isset($_GET["boolean"][$g]),"BOOL"),"<br>\n";}}$g=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$g][col]'><option value=''>(".'beliebig'.")".optionlist($v,$b["col"],true)."</select>",html_select("where[$g][op]",$this->operators,$b["op"]),"<input name='where[$g][val]' value='".h($b["val"])."'></div>\n";$g++;}}echo"<div><select name='where[$g][col]' onchange='selectAddRow(this);'><option value=''>(".'beliebig'.")".optionlist($v,null,true)."</select>",html_select("where[$g][op]",$this->operators,"="),"<input name='where[$g][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($La,$v,$z){print_fieldset("sort",'Ordnen',$La);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$g]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'absteigend')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$g]",1,0,'absteigend')."</div>\n","</div></fieldset>\n";}function
...
false;}function
messageQuery($j){global$s;static$yc=0;restart_session();$D="sql-".($yc++);$Ia=&get_session("queries");$Ia[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$D' onclick=\"return !toggle('$D');\">".'SQL-Query'."</a><div id='$D' class='hidden'><pre><code class='jush-$s'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ia[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
as$l=>$O){echo'<a href="'.h(ME).'select='.urlencode($l).'"'.bold($_GET["select"]==$l).">".'zeigen'."</a> ",'<a href="'.h(ME).'table='.urlencode($l).'"'.bold($_GET["table"]==$l).">".$this->tableName(array("Name"=>$l))."</a><br>\n";}}}$n=(function_exists('adminer_object')?adminer_object():new
...
auth_error($pe=null){global$i,$n,$B;$Fc=session_name();$q="";if(!$_COOKIE[$Fc]&&$_GET[$Fc]&&ini_bool("session.use_only_cookies")){$q='Sitzungen müssen aktiviert sein.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Fc]||$_GET[$Fc])&&!$B){$q='Sitzungsdauer abgelaufen, bitte erneut anmelden.';}else{$na=&get_session("pwds");if(isset($na)){$q=h($pe?$pe->getMessage():(is_string($i)?$i:'Ungültige Anmelde-Informationen.'));$na=null;}}}page_header('Login',$q,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$n->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstützten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$Ze)),false);page_footer("auth");exit;}$i=connect();}if(is_string($i)||!$n->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$B=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$B;}$q=($_POST?($_POST["token"]==$B?"":'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('POST data zu gross. Reduzieren Sie die Grösse oder vergrössern Sie den Wert %s in der Konfiguration.','"post_max_size"')));function
connect_error(){global$i,$B,$q,$va;$_=array();if(DB!=""){page_header('Datenbank'.": ".h(DB),'Datenbank ungültig.',true);}else{if($_POST["db"]&&!$q){queries_redirect(substr(ME,0,-1),'Datenbanken entfernt.',drop_databases($_POST["db"]));}page_header('Datenbank auswählen',$q,false);echo"<p><a href='".h(ME)."database='>".'Neue Datenbank'."</a>\n";foreach(array('privileges'=>'Rechte','processlist'=>'Prozessliste','variables'=>'Variablen','status'=>'Status',)as$c=>$b){if(support($c)){echo"<a href='".h(ME)."$c='>$b</a>\n";}}echo"<p>".sprintf('Version %s: %s, mit PHP-Erweiterung %s',$va[DRIVER],"<b>$i->server_info</b>","<b>$i->extension</b>")."\n","<p>".sprintf('Angemeldet als: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"]){set_session("dbs",null);}$_=get_databases();if($_){$wf=support("scheme");$U=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td><input type='hidden' name='token' value='$B'>&nbsp;<th>".'Datenbank'."<td>".'Collation'."<td>".'Tabellen'."</thead>\n";foreach($_
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($ne[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ba[$c]==254){$b="<code>$b</code>";}}if(isset($Aa[$c])&&!$v[$Aa[$c]]){$y="edit=".urlencode($Aa[$c]);foreach($z[$Aa[$c]]as$Mc=>$ca){$y.="&where".urlencode("[".bracket_escape($Mc)."]")."=".urlencode($a[$ca]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($c,$d,$U,$L=array()){global$Oc,$ba,$Gb,$Ka;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ba[$d["type"]])?array():array($d["type"]))+$Oc+($L?array('Fremdschlüssel'=>$L):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($U,$d["collation"]).'</select>',($Gb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Gb,$d["unsigned"]).'</select>':''),($L?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($Ka,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($mb?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($mb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $de, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($cd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$mb;}function
...
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Auf Server',$_POST["webfile"]);$Wc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Wc[]=".$c";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Wc?"[".implode("|",$Wc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($Ia){print_fieldset("history",'History',$_GET["history"]!="");foreach($Ia
...
as$f=>$d){if(!isset($d["privileges"][$Ja?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$M=$_POST["referer"];if($_POST["insert"]){$M=($Ja?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$M)){$M=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$M,'Datensatz gelöscht.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Ja?"\n".idf_escape($f)." = $b":$b);}}if($Ja){if(!$u){redirect($M);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$M,'Datensatz geändert.');}else{$k=insert_into($h,$u);$Sd=($k?last_id():0);queries_redirect($M,sprintf('Datensatz%s hinzugefügt.',($Sd?" $Sd":"")),$k);}}}$wa=$n->tableName(table_status($h));page_header(($Ja?'Ändern':'Hinzufügen'),$q,array("select"=>array($h,$wa)),$wa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$C=array();foreach($p
...
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if(support("partitioning")){$Jd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partitionieren um',$a["partition_by"]);echo'<p>
...
as$w){if(count($_)==1||$w!=""){if(!create_database($w,$_POST["collation"])){$Ld=false;}$bb=$w;}}queries_redirect(ME."db=".urlencode($bb),'Datenbank erstellt.',$Ld);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$q,array(),DB);$U=collations();$f=DB;$Mb=null;if($_POST){$f=$_POST["name"];$Mb=$_POST["collation"];}elseif(DB!=""){$Mb=db_collation(DB,$U);}elseif($s=="sql"){foreach(get_vals("SHOW GRANTS")as$P){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$P,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeypress="return textareaKeypress(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($U?html_select("collation",array(""=>"(".'Kollation'.")")+$U,$Mb):"");?>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
<input type="hidden" name="token" value="',$B,'">
';if($Qa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
</form>
';}elseif(isset($_GET["event"])){$Sa=$_GET["event"];$oe=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Vc=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Sa),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$oe)&&isset($Vc[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Sa!=""?"ALTER EVENT ".idf_escape($Sa).$Td.($Sa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$Vc[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Sa!=""?'Ereignis geändert.':'Ereignis erstellt.'));}}page_header(($Sa!=""?'Ereignis ändern'.": ".h($Sa):'Ereignis erstellen'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Sa!=""){$E=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Sa));$a=reset($E);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$oe,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Vc,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Speichern">
';if($Sa!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
...
as$d){if($d["field"]!=""){$u[]=(in_array($d["inout"],$wb)?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Qa=drop_create("DROP $Da ".idf_escape($Fa),"CREATE $Da ".idf_escape($_POST["name"])." (".implode(", ",$u).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Prozedur entfernt.','Prozedur geändert.','Prozedur erstellt.',$Fa);}page_header(($Fa!=""?(isset($_GET["function"])?'Funktion ändern':'Prozedur ändern').": ".h($Fa):(isset($_GET["function"])?'Neue Funktion':'Neue Prozedur')),$q);$U=get_vals("SHOW CHARACTER SET");sort($U);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Fa!=""){$a=routine($Fa,$Da);$a["name"]=$Fa;}echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$U,$Da);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$a["returns"],$U);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$B,'">
';if($Qa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($Fa!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["type"])){$zc=$_GET["type"];if($_POST&&!$q){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($zc),$y,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ erstellt.');}}page_header($zc!=""?'Typ ändern'.": ".h($zc):'Typ erstellen',$q);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$B,'">
';if($zc!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Speichern'."'>\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$vc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($h))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Dd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$vc["Type"],$a["Type"]),'</table>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$g=0;foreach($Ta
as$Ga=>$P){echo'<th>'.($Ga!="*.*"?"<input name='objects[$g]' value='".h($Ga)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$lc=>$fc){foreach((array)$Y[$lc]as$Zb=>$eb){echo"<tr".odd()."><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($eb).'">'.h($Zb);$g=0;foreach($Ta
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$kd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$kd++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$kd),$kd||!$_POST["kill"]);}page_header('Prozessliste',$q);echo'
<form action="" method="post">
...
<p>',($g+1)."/".sprintf('%d insgesamt',$i->result("SELECT @@max_connections")),'<p>
<input type="hidden" name="token" value="',$B,'">
...
';}print_fieldset("export",'Exportieren');$ya=$n->dumpOutput();echo($ya?html_select("output",$ya,$yb["output"])." ":""),html_select("format",$n->dumpFormat(),$yb["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}print_fieldset("import",'Importieren',!$E);echo"<input type='hidden' name='token' value='$B'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$yb["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>\n","</div></fieldset>\n";$n->selectEmailPrint(array_filter($Lc,'strlen'),$v);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Eb=isset($_GET["status"]);page_header($Eb?'Status':'Variablen');$Pd=($Eb?show_status():show_variables());if(!$Pd){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Pd
