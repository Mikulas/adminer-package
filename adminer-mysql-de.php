...
confirm($ce=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($ce?" (' + $ce + ')":"")."');\"";}function
js_escape($J){return
addcslashes($J,"\r\n'\\/");}function
...
upload_error($q){$ie=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Hochladen von Datei fehlgeschlagen.'.($ie?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$ie):""):'Datei existiert nicht.');}function
...
input($d,$p,$C){global$oa,$n,$t;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$X=(isset($_GET["select"])?array("orig"=>'Original'):array())+$n->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Fe,$Pb){$Eb=($Pb==1||(!$Pb&&'de'=='fr')?0:((!$Pb||$Pb>=5)&&ereg('cs|sk|ru','de')?2:1));return
...
selectLinks($Fc,$u=""){echo'<p class="tabs">';$va=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Fc)){$va["view"]='View ändern';}else{$va["create"]='Tabelle ändern';}if(isset($u)){$va["edit"]='Neuer Datensatz';}foreach($va
...
selectQuery($j){global$t;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a> <code class='jush-$t'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Ändern'."</a>\n";}function
...
selectColumnsPrint($A,$v){global$X,$Zb;print_fieldset("select",'Daten zeigen von',$A);$g=0;$pe=array('Funktionen'=>$X,'Agregationen'=>$Zb);foreach($A
...
selectSearchPrint($x,$v,$_){print_fieldset("search",'Suchen',$x);foreach($_
...
selectOrderPrint($Xa,$v,$_){print_fieldset("sort",'Ordnen',$Xa);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$g]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'absteigend')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$g]",1,0,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($S){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($S)."'>","</div></fieldset>\n";}function
selectLengthPrint($jb){if(isset($jb)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($jb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$t;restart_session();$F="sql-".count($_SESSION["messages"]);$Ka=&get_session("queries");$Ka[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$F' onclick=\"return !toggle('$F');\">".'SQL-Query'."</a><div id='$F' class='hidden'><pre class='jush-$t'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ka[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Ca,$p){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ca value='-1' checked><i>".'Original'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ca value=''".(isset($p)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Ca value='0'".($p===0?" checked":"")."><i>".'leer'."</i></label>".enum_input("radio",$Ca,$d,$p);}return"";}function
...
';if($wb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$wb){echo'<p><a href="'.h(ME).'create=">'.bold('Neue Tabelle',$_GET["create"]==="")."</a>\n";$xa=tables_list();if(!$xa){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($xa);$va=array();foreach($xa
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($de[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($oa[$c]==254){$b="<code>$b</code>";}}if(isset($va[$c])&&!$v[$va[$c]]){$y="edit=".urlencode($va[$c]);foreach($_[$va[$c]]as$vc=>$V){$y.="&where".urlencode("[".bracket_escape($vc)."]")."=".urlencode($a[$V]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($c,$d,$R,$K=array()){global$ad,$oa,$Yb,$Sa;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($oa[$d["type"]])?array():array($d["type"]))+$ad+($K?array('Fremdschlüssel'=>$K):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($R,$d["collation"]).'</select>',($Yb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Yb,$d["unsigned"]).'</select>':''),($K?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($Sa,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $ae, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.1.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.1.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($bd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.1.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$ob;}function
...
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Auf Server',$_POST["webfile"]);$Xc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Xc[]=".$c";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Xc?"[".implode("|",$Xc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($Ka){print_fieldset("history",'History',$_GET["history"]!="");foreach($Ka
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Ändern'."</a> <code class='jush-$t'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Da?"update":"insert"])||$n->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$T=$_POST["referer"];if($_POST["insert"]){$T=($Da?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$T)){$T=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$T,'Datensatz gelöscht.');}else{$u=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Da?"\n".idf_escape($f)." = $b":$b);}}if($Da){if(!$u){redirect($T);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$T,'Datensatz geändert.');}else{$k=insert_into($h,$u);$Ld=($k?last_id():0);queries_redirect($T,sprintf('Datensatz%s hinzugefügt.',($Ld?" $Ld":"")),$k);}}}$ta=$n->tableName(table_status($h));page_header(($Da?'Ändern':'Hinzufügen'),$q,array("select"=>array($h,$ta)),$ta);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$A=array();foreach($o
...
<?php echo(support("comment")?checkbox("","",$ob,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Speichern">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if(support("partitioning")){$Cd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partitionieren um',$a["partition_by"]);echo'<p>
...
as$w){if(count($z)==1||$w!=""){if(!create_database($w,$_POST["collation"])){$Kd=false;}$ab=$w;}}queries_redirect(ME."db=".urlencode($ab),'Datenbank erstellt.',$Kd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$q,array(),DB);$R=collations();$f=DB;$Nb=null;if($_POST){$f=$_POST["name"];$Nb=$_POST["collation"];}elseif(DB!=""){$Nb=db_collation(DB,$R);}elseif($t=="sql"){foreach(get_vals("SHOW GRANTS")as$P){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$P,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeydown="return textareaKeydown(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($R?html_select("collation",array(""=>"(".'Kollation'.")")+$R,$Nb):"");?>
...
<input type="hidden" name="token" value="',$B,'">
';if($Fa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
</form>
';}elseif(isset($_GET["event"])){$Ua=$_GET["event"];$_d=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$dd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ua),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$_d)&&isset($dd[$_POST["STATUS"]])){$Hd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Ua!=""?"ALTER EVENT ".idf_escape($Ua).$Hd.($Ua!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Hd)."\n".$dd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Ua!=""?'Ereignis geändert.':'Ereignis erstellt.'));}}page_header(($Ua!=""?'Ereignis ändern'.": ".h($Ua):'Ereignis erstellen'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ua!=""){$D=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ua));$a=reset($D);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_d,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$dd,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Speichern">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
...
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$R,$_a);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$a["returns"],$R);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$B,'">
';if($Fa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["type"])){$tc=$_GET["type"];if($_POST&&!$q){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($tc),$y,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ erstellt.');}}page_header($tc!=""?'Typ ändern'.": ".h($tc):'Typ erstellen',$q);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$B,'">
';if($tc!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Speichern'."'>\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$qc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($h))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Qd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$qc["Type"],$a["Type"]),'</table>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Rechte'."</a>";$g=0;foreach($Ta
as$Ea=>$P){echo'<th>'.($Ea!="*.*"?"<input name='objects[$g]' value='".h($Ea)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$wc=>$Lb){foreach((array)$Y[$wc]as$Ob=>$eb){echo"<tr".odd()."><td".($Lb?">$Lb<td":" colspan='2'").' lang="en" title="'.h($eb).'">'.h($Ob);$g=0;foreach($Ta
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$jd),$jd||!$_POST["kill"]);}page_header('Prozessliste',$q);echo'
<form action="" method="post">
...
as$c=>$b){echo"<td>".($c=="Info"&&$b!=""?"<code class='jush-$t'>".nbsp($b).'</code> <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Ändern'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
<input type="hidden" name="token" value="',$B,'">
...
<input type="submit" name="delete" value="Entfernen"',confirm("(this.form['all'].checked ? $za : formChecked(this, /check/))"),'>
</div></fieldset>
';}print_fieldset("export",'Exportieren');$Na=$n->dumpOutput();echo($Na?html_select("output",$Na,$Wc["output"])." ":"").html_select("format",$n->dumpFormat(),$Wc["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}print_fieldset("import",'Importiere CSV',!$D);echo"<input type='hidden' name='token' value='$B'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Wc["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>\n","</div></fieldset>\n";$n->selectEmailPrint(array_filter($Bc,'strlen'),$v);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$_b=isset($_GET["status"]);page_header($_b?'Status':'Variablen');$qd=($_b?show_status():show_variables());if(!$qd){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($qd
...
as$c=>$b){echo"setHtml('sum-$c', '".number_format($b,0,'.',' ')."');\n";}}else{foreach(count_tables(get_databases())as$w=>$b){echo"setHtml('tables-".js_escape($w)."', '$b');\n";}}exit;}else{$rd=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($rd&&!$q&&!$_POST["search"]){$k=true;$la="";if($t=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$k=truncate_tables($_POST["tables"]);}$la='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$k=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$la='Tabellen verschoben.';}elseif($_POST["drop"]){if($_POST["views"]){$k=drop_views($_POST["views"]);}if($k&&$_POST["tables"]){$k=drop_tables($_POST["tables"]);}$la='Tabellen wurden entfernt (drop).';}elseif($_POST["tables"]&&($k=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$k->fetch_assoc()){$la.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$la,$k);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$q,true);echo'<p>'.($_GET["ns"]==""?'<a href="'.h(ME).'database=">'.'Datenbank ändern'."</a>\n":"");if($_GET["ns"]!==""){echo'<a href="'.h(ME).'schema=">'.'Datenbankschema'."</a>\n","<h3>".'Tabellen und Views'."</h3>\n";$Hc=tables_list();if(!$Hc){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabelle'.'<td>'.'Motor'.'<td>'.'Collation'.'<td>'.'Datengrösse'.'<td>'.'Indexgrösse'.'<td>'.'Freier Bereich'.'<td>'.'Auto-Inkrement'.'<td>'.'Datensätze'.(support("comment")?'<td>'.'Kommentar':'')."</thead>\n";foreach($Hc
as$f=>$O){$mc=(isset($O)&&!eregi("table",$O));echo'<tr'.odd().'><td>'.checkbox(($mc?"views[]":"tables[]"),$f,in_array($f,$rd,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($mc){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{echo"<td id='Engine-".h($f)."'>&nbsp;<td id='Collation-".h($f)."'>&nbsp;";foreach(array("Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$c=>$y){echo"<td align='right'><a href='".h(ME."$y=").urlencode($f)."' id='$c-".h($f)."'>?</a>";}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($Hc)),"<td>".nbsp($i->result("SELECT @@storage_engine")),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$c){echo"<td align='right' id='sum-$c'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p><input type='hidden' name='token' value='$B'>".($t=="sql"?"<input type='submit' value='".'Analysieren'."'> <input type='submit' name='optimize' value='".'Optimieren'."'> <input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)").">\n";$z=(support("scheme")?schemas():get_databases());if(count($z)!=1&&$t!="sqlite"){$w=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.($z?": ".html_select("target",$z,$w):': <input name="target" value="'.h($w).'">')." <input type='submit' name='move' value='".'Verschieben'."'>\n";}}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";}if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$vd=routines();if($vd){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($vd
