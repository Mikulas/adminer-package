...
upload_error($p){$je=($p==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($p?'Hochladen von Datei fehlgeschlagen.'.($je?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$je):""):'Datei existiert nicht.');}function
...
input($d,$n,$B){global$na,$q,$u;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Q=(isset($_GET["select"])?array("orig"=>'Original'):array())+$q->editFunctions($d);$Aa=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Te,$Qb){$Pb=($Qb==1||(!$Qb&&'de'=='fr')?0:((!$Qb||$Qb>=5)&&ereg('cs|sk|ru','de')?2:1));return
...
selectLinks($Dc,$t=""){echo'<p class="tabs">';$ya=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Dc)){$ya["view"]='View ändern';}else{$ya["create"]='Tabelle ändern';}if(isset($t)){$ya["edit"]='Neuer Datensatz';}foreach($ya
...
selectQuery($j){global$u;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Seite'.": ".'letzte'."'>&gt;&gt;</a> <code class='jush-$u'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Ändern'."</a>\n";}function
rowDescription($m){return"";}function
rowDescriptions($D,$gf){return$D;}function
selectVal($b,$A,$d){$e=($b!="<i>NULL</i>"&&ereg("^char|binary",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$e=lang(array('%d Byte','%d Bytes'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($A?"<a href='$A'>$e</a>":$e);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($v,$w){global$Q,$Jb;print_fieldset("select",'Daten zeigen von',$v);$g=0;$oe=array('Funktionen'=>$Q,'Agregationen'=>$Jb);foreach($v
...
selectSearchPrint($y,$w,$_){print_fieldset("search",'Suchen',$y);foreach($_
as$g=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input name='fulltext[$g]' value='".h($_GET["fulltext"][$g])."'>",checkbox("boolean[$g]",1,isset($_GET["boolean"][$g]),"BOOL"),"<br>\n";}}$g=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$g][col]'><option value=''>(".'beliebig'.")".optionlist($w,$b["col"],true)."</select>",html_select("where[$g][op]",$this->operators,$b["op"]),"<input name='where[$g][val]' value='".h($b["val"])."'></div>\n";$g++;}}echo"<div><select name='where[$g][col]' onchange='selectAddRow(this);'><option value=''>(".'beliebig'.")".optionlist($w,null,true)."</select>",html_select("where[$g][op]",$this->operators),"<input name='where[$g][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Za,$w,$_){print_fieldset("sort",'Ordnen',$Za);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$g]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'absteigend')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>",checkbox("desc[$g]",1,0,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($T){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($T)."'>","</div></fieldset>\n";}function
selectLengthPrint($hb){if(isset($hb)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($hb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$u;restart_session();$E="sql-".count($_SESSION["messages"]);$Na=&get_session("queries");$Na[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL-Query'."</a><div id='$E' class='hidden'><pre class='jush-$u'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Na[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Aa,$n){if($d["type"]=="enum"){return($d["null"]?"<label><input type='radio'$Aa value=''".(isset($n)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Aa value='0'".($n===0?" checked":"")."><i>".'leer'."</i></label>";}return"";}function
...
dumpOutput($v,$n=""){$e=array('text'=>'anzeigen','file'=>'Datei');if(function_exists('gzencode')){$e['gz']='gzip';}if(function_exists('bzcompress')){$e['bz2']='bzip2';}return
...
';if($ub!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$ub){$xa=tables_list();if(!$xa){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($xa);$ya=array();foreach($xa
...
as$m=>$O){echo'<a href="'.h(ME).'select='.urlencode($m).'">'.bold('zeigen',$_GET["select"]==$m).'</a> ','<a href="'.h(ME).'table='.urlencode($m).'">'.bold($this->tableName(array("Name"=>$m)),$_GET["table"]==$m)."</a><br>\n";}}}$q=(function_exists('adminer_object')?adminer_object():new
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($ne[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($na[$c]==254){$b="<code>$b</code>";}}if(isset($ya[$c])&&!$w[$ya[$c]]){$A="edit=".urlencode($ya[$c]);foreach($_[$ya[$c]]as$kc=>$X){$A.="&where".urlencode("[".bracket_escape($kc)."]")."=".urlencode($a[$X]);}$b="<a href='".h(ME.$A)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($c,$d,$R,$J=array()){global$gd,$na,$Cb,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($na[$d["type"]])?array():array($d["type"]))+$gd+($J?array('Fremdschlüssel'=>$J):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($R,$d["collation"]).'</select>',($Cb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Cb,$d["unsigned"]).'</select>':''),($J?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($Ma,$d["on_delete"])."</select> ":" ");}function
...
';if($O=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($O=="TABLE"?'Spaltenname':'Name des Parameters'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Länge
<td>Optionen
...
',(support("comment")?"<td".($qb?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($qb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $Ce, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.1' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.1' alt='v' title='".'Nach unten'."'>&nbsp;":""),($nd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.1' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$qb;}function
...
';$Qa=true;foreach($id
as$c=>$b){if($c!=""&&$b>1){echo($Qa?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$c%")."'>".h($c)."</a>";$Qa=false;}}}elseif(isset($_GET["privileges"])){page_header('Rechte');$k=$i->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$k){echo'<form action=""><p>
';hidden_fields_get();echo'Benutzer: <input name="user">
Server: <input name="host" value="localhost">
<input type="hidden" name="grant" value="">
<input type="submit" value="Ändern">
</form>
';$k=$i->query("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', 1) AS User, SUBSTRING_INDEX(CURRENT_USER, '@', -1) AS Host");}echo"<table cellspacing='0'>\n","<thead><tr><th>&nbsp;<th>".'Benutzer'."<th>".'Server'."</thead>\n";while($a=$k->fetch_assoc()){echo'<tr'.odd().'><td><a href="'.h(ME.'user='.urlencode($a["User"]).'&host='.urlencode($a["Host"])).'">'.'ändern'.'</a><td>'.h($a["User"])."<td>".h($a["Host"])."\n";}echo"</table>\n",'<p><a href="'.h(ME).'user=">'.'Neuer Benutzer'."</a>";}elseif(isset($_GET["sql"])){restart_session();$Je=&get_session("queries");$Na=&$Je[DB];if(!$p&&$_POST["clear"]){$Na=array();redirect(remove_from_uri("history"));}page_header('SQL-Query',$p);if(!$p&&$_POST){$ta=false;$j=$_POST["query"];if($_POST["webfile"]){$ta=@fopen((file_exists("adminer.sql")?"adminer.sql":(file_exists("adminer.sql.gz")?"compress.zlib://adminer.sql.gz":"compress.bzip2://adminer.sql.bz2")),"rb");$j=($ta?fread($ta,1e6):false);}elseif($_FILES["sql_file"]["error"]!=4){$j=get_file("sql_file",true);}if(is_string($j)){if(function_exists('memory_get_usage')){@ini_set("memory_limit",2*strlen($j)+memory_get_usage()+8e6);}if($j!=""&&strlen($j)<1e6&&(!$Na||end($Na)!=$j)){$Na[]=$j;}$qc="(\\s|/\\*.*\\*/|(#|-- )[^\n]*\n|--\n)";if(!ini_bool("session.use_cookies")){session_write_close();}$Ic=";";$ea=0;$td=true;$L=connect();if(is_object($L)&&DB!=""){$L->select_db(DB);}$Bb=0;$Qc="";while($j!=""){if(!$ea&&preg_match('~^\\s*DELIMITER\\s+(.+)~i',$j,$l)){$Ic=$l[1];$j=substr($j,strlen($l[0]));}else{preg_match('('.preg_quote($Ic).'|[\'`"]|/\\*|-- |#|$)',$j,$l,PREG_OFFSET_CAPTURE,$ea);$da=$l[0][0];$ea=$l[0][1]+strlen($da);if(!$da&&$ta&&!feof($ta)){$j.=fread($ta,1e5);}else{if(!$da&&rtrim($j)==""){break;}if(!$da||$da==$Ic){$td=false;$ja=substr($j,0,$l[0][1]);$Bb++;echo"<pre class='jush-$u' id='sql-$Bb'>".shorten_utf8(trim($ja),1000)."</pre>\n";ob_flush();flush();$Uc=explode(" ",microtime());if(!$i->multi_query($ja)){echo"<p class='error'>".'Fehler in der SQL-Abfrage'.": ".error()."\n";$Qc.=" <a href='#sql-$Bb'>$Bb</a>";if($_POST["error_stops"]){break;}}else{if(is_object($L)&&preg_match("~^$qc*(USE)\\b~isU",$ja)){$L->query($ja);}do{$k=$i->store_result();$Xc=explode(" ",microtime());$Kd=" <span class='time'>(".sprintf('%.3f s',max(0,$Xc[0]-$Uc[0]+$Xc[1]-$Uc[1])).")</span>";if(is_object($k)){select($k,$L);echo"<p>".($k->num_rows?lang(array('%d Datensatz','%d Datensätze'),$k->num_rows):"").$Kd;if($L&&preg_match("~^($qc|\\()*SELECT\\b~isU",$ja)){$E="explain-$Bb";echo", <a href='#$E' onclick=\"return !toggle('$E');\">EXPLAIN</a>\n","<div id='$E' class='hidden'>\n";select(explain($L,$ja));echo"</div>\n";}}else{if(preg_match("~^$qc*(CREATE|DROP|ALTER)$qc+(DATABASE|SCHEMA)\\b~isU",$ja)){restart_session();set_session("dbs",null);session_write_close();}echo"<p class='message' title='".h($i->info)."'>".lang(array('Abfrage ausgeführt, %d Datensatz betroffen.','Abfrage ausgeführt, %d Datensätze betroffen.'),$i->affected_rows)."$Kd\n";}$Uc=$Xc;}while($i->next_result());}$j=substr($j,$ea);$ea=0;}else{while(preg_match('~'.($da=='/*'?'\\*/':(ereg('-- |#',$da)?"\n":"$da|\\\\.")).'|$~s',$j,$l,PREG_OFFSET_CAPTURE,$ea)){$S=$l[0][0];$ea=$l[0][1]+strlen($S);if(!$S&&$ta&&!feof($ta)){$j.=fread($ta,1e6);}elseif($S[0]!="\\"){break;}}}}}}if($Qc&&$Bb>1){echo"<p class='error'>".'Fehler in der SQL-Abfrage'.": $Qc\n";}if($td){echo"<p class='message'>".'Kein Kommando vorhanden.'."\n";}}else{echo"<p class='error'>".upload_error($j)."\n";}}echo'
...
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten');print_fieldset("webfile",'Auf Server',$_POST["webfile"]);$Yc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Yc[]=".$c";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Yc?"[".implode("|",$Yc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($Na){print_fieldset("history",'History',$_GET["history"]!="");foreach($Na
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Ändern'."</a> <code class='jush-$u'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Ga?"update":"insert"])||$q->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$p&&!isset($_GET["select"])){$U=$_POST["referer"];if($_POST["insert"]){$U=($Ga?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$U)){$U=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $y"),$U,'Datensatz gelöscht.');}else{$t=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$t[idf_escape($f)]=($Ga?"\n".idf_escape($f)." = $b":$b);}}if($Ga){if(!$t){redirect($U);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$t),"\nWHERE $y"),$U,'Datensatz geändert.');}else{$k=insert_into($h,$t);$Id=($k?last_id():0);queries_redirect($U,sprintf('Datensatz%s hinzugefügt.',($Id?" $Id":"")),$k);}}}$oa=$q->tableName(table_status($h));page_header(($Ga?'Ändern':'Hinzufügen'),$p,array("select"=>array($h,$oa)),$oa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($y){$v=array();foreach($o
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($o){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($Ga?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."\">\n";}}if($Ga){echo"<input type='submit' name='delete' value='".'Entfernen'."'$db>\n";}echo'</form>
...
<?php echo(support("comment")?checkbox("","",$qb,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($qb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Entfernen"',$db,'>';}if(support("partitioning")){$Ad=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partitionieren um',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Ld,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Ad||!$a["partition_by"]?" class='hidden'":""),'>
...
as$x){if(count($z)==1||$x!=""){if(!create_database($x,$_POST["collation"])){$Dd=false;}$Va=$x;}}queries_redirect(ME."db=".urlencode($Va),'Datenbank erstellt.',$Dd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$p,array(),DB);$R=collations();$f=DB;$Kb=null;if($_POST){$f=$_POST["name"];$Kb=$_POST["collation"];}elseif(DB!=""){$Kb=db_collation(DB,$R);}elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$N){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$N,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea name="name" rows="10" cols="40" onkeydown="return textareaKeydown(this, event);">'.h($f).'</textarea><br>':'<input name="name" value="'.h($f).'" maxlength="64">')."\n".($R?html_select("collation",array(""=>"(".'Kollation'.")")+$R,$Kb):""),'<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'$db>\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Hinzufügen'."'>\n";}echo'</form>
';}elseif(isset($_GET["call"])){$Ba=$_GET["call"];page_header('Aufrufen'.": ".h($Ba),$p);$Ca=routine($Ba,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$mb=array();$Pa=array();foreach($Ca["fields"]as$g=>$d){if(substr($d["inout"],-3)=="OUT"){$Pa[$g]="@".idf_escape($d["field"])." AS ".idf_escape($d["field"]);}if(!$d["inout"]||substr($d["inout"],0,2)=="IN"){$mb[]=$g;}}if(!$p&&$_POST){$Md=array();foreach($Ca["fields"]as$c=>$d){if(in_array($c,$mb)){$b=process_input($d);if($b===false){$b="''";}if(isset($Pa[$c])){$i->query("SET @".idf_escape($d["field"])." = $b");}}$Md[]=(isset($Pa[$c])?"@".idf_escape($d["field"]):$b);}$j=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($Ba)."(".implode(", ",$Md).")";echo"<p><code class='jush-$u'>".h($j)."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Ändern'."</a>\n";if(!$i->multi_query($j)){echo"<p class='error'>".error()."\n";}else{do{$k=$i->store_result();if(is_object($k)){select($k);}else{echo"<p class='message'>".lang(array('Kommando SQL ausgeführt, %d Datensatz betroffen.','Kommando SQL ausgeführt, %d Datensätze betroffen.'),$i->affected_rows)."\n";}}while($i->next_result());if($Pa){select($i->query("SELECT ".implode(", ",$Pa)));}}}echo'
<form action="" method="post">
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',$db,'>';}echo'<input type="hidden" name="token" value="',$C,'">
</form>
';}elseif(isset($_GET["view"])){$h=$_GET["view"];$Ea=false;if($_POST&&!$p){$Ea=drop_create("DROP VIEW ".table($h),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'View entfernt.','View geändert.','View erstellt.',$h);}page_header(($h!=""?'View ändern':'Neue View erstellen'),$p,array("table"=>$h),$h);$a=array();if($_POST){$a=$_POST;}elseif($h!=""){$a=view($h);$a["name"]=$h;}echo'
<form action="" method="post">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Fd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Lc,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Entfernen"',$db,'>';}echo'</form>
...
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Entfernen"',$db,'>';}echo'</form>
';}elseif(isset($_GET["type"])){$sc=$_GET["type"];if($_POST&&!$p){$A=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($sc),$A,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$A,'Typ erstellt.');}}page_header($sc!=""?'Typ ändern'.": ".h($sc):'Typ erstellen',$p);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$C,'">
';if($sc!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'$db>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Speichern'."'>\n";}echo'</form>
';}elseif(isset($_GET["trigger"])){$h=$_GET["trigger"];$Fc=trigger_options();$zd=array("INSERT","UPDATE","DELETE");$Ea=false;if($_POST&&!$p&&in_array($_POST["Timing"],$Fc["Timing"])&&in_array($_POST["Event"],$zd)&&in_array($_POST["Type"],$Fc["Type"])){$_d=" $_POST[Timing] $_POST[Event]";$eb=" ON ".table($h);$Ea=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$eb:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$eb.$_d:$_d.$eb)." $_POST[Type]\n$_POST[Statement]",ME."table=".urlencode($h),'Trigger entfernt.','Trigger geändert.','Trigger erstellt.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Trigger ändern'.": ".h($_GET["name"]):'Trigger hinzufügen'),$p,array("table"=>$h));$a=array("Trigger"=>$h."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Fc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($h,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$zd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Fc["Type"],$a["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',$db,'>';}echo'</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Rechte'."</a>";$g=0;foreach($Oa
as$Ha=>$N){echo'<th>'.($Ha!="*.*"?"<input name='objects[$g]' value='".h($Ha)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Hc=>$Fb){foreach((array)$W[$Hc]as$dc=>$Ya){echo"<tr".odd()."><td".($Fb?">$Fb<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($dc);$g=0;foreach($Oa
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',$db,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$p){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$jd),$jd||!$_POST["kill"]);}page_header('Prozessliste',$p);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$za,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Exportieren');echo$q->dumpOutput(1,$Oc["output"])." ".$q->dumpFormat(1,$Oc["format"]);echo" <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}print_fieldset("import",'Importiere CSV',!$D);echo"<input type='hidden' name='token' value='$C'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;"),$Oc["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>\n","</div></fieldset>\n";$q->selectEmailPrint(array_filter($_c,'strlen'),$w);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$tb=isset($_GET["status"]);page_header($tb?'Status':'Variablen');$sd=($tb?show_status():show_variables());if(!$sd){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($sd
...
as$c=>$b){echo"setHtml('sum-$c', '".number_format($b,0,'.',' ')."');\n";}}else{foreach(count_tables(get_databases())as$x=>$b){echo"setHtml('tables-".addcslashes($x,"\\'/")."', '$b');\n";}}exit;}else{$qd=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($qd&&!$p&&!$_POST["search"]){$k=true;$ma="";if($u=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$k=truncate_tables($_POST["tables"]);}$ma='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$k=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ma='Tabellen verschoben.';}elseif($_POST["drop"]){if($_POST["views"]){$k=drop_views($_POST["views"]);}if($k&&$_POST["tables"]){$k=drop_tables($_POST["tables"]);}$ma='Tabellen wurden entfernt (drop).';}elseif($_POST["tables"]&&($k=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$k->fetch_assoc()){$ma.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ma,$k);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$p,true);echo'<p>'.($_GET["ns"]==""?'<a href="'.h(ME).'database=">'.'Datenbank ändern'."</a>\n":"");if($_GET["ns"]!==""){echo'<a href="'.h(ME).'schema=">'.'Datenbankschema'."</a>\n","<h3>".'Tabellen und Views'."</h3>\n";$Gc=tables_list();if(!$Gc){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabelle'.'<td>'.'Motor'.'<td>'.'Collation'.'<td>'.'Datengrösse'.'<td>'.'Indexgrösse'.'<td>'.'Freier Bereich'.'<td>'.'Auto-Inkrement'.'<td>'.'Datensätze'.(support("comment")?'<td>'.'Kommentar':'')."</thead>\n";foreach($Gc
as$f=>$O){$hc=(isset($O)&&!eregi("table",$O));echo'<tr'.odd().'><td>'.checkbox(($hc?"views[]":"tables[]"),$f,in_array($f,$qd,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($hc){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{echo"<td id='Engine-".h($f)."'>&nbsp;<td id='Collation-".h($f)."'>&nbsp;";foreach(array("Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$c=>$A){echo"<td align='right'><a href='".h(ME."$A=").urlencode($f)."' id='$c-".h($f)."'>?</a>";}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($Gc)),"<td>".nbsp($i->result("SELECT @@storage_engine")),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$c){echo"<td align='right' id='sum-$c'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p><input type='hidden' name='token' value='$C'>".($u=="sql"?"<input type='submit' value='".'Analysieren'."'> <input type='submit' name='optimize' value='".'Optimieren'."'> <input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."' onclick=\"return confirm('".'Sind Sie sicher ?'." (' + formChecked(this, /tables/) + ')');\"> <input type='submit' name='drop' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'." (' + formChecked(this, /tables|views/) + ')');\">\n";$z=(support("scheme")?schemas():get_databases());if(count($z)!=1&&$u!="sqlite"){$x=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.($z?": ".html_select("target",$z,$x):': <input name="target" value="'.h($x).'">')." <input type='submit' name='move' value='".'Verschieben'."'>\n";}}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";}if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$od=routines();if($od){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($od
