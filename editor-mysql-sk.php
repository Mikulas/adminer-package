...
selectVal($W,$y,$l,$Yc){$L=($W===null?"&nbsp;":$W);$y=h($y);if(preg_match('~blob|bytea~',$l["type"])&&!is_utf8($W)){$L=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($Yc));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$Yc))$L="<img src='$y' alt='$L'>";}if(like_bool($l)&&$L!="&nbsp;")$L=($W?'yes':'no');if($y)$L="<a href='$y'>$L</a>";if(!$y&&!like_bool($l)&&preg_match('~int|float|double|decimal~',$l["type"]))$L="<div class='number'>$L</div>";elseif(preg_match('~date~',$l["type"]))$L="<div class='datetime'>$L</div>";return$L;}function
...
as$C=>$bb){$l=$m[$C];if(preg_match("~enum~",$l["type"])||like_bool($l)){$w=$qc[$C];$q--;echo"<div>".h($bb)."<input type='hidden' name='where[$q][col]' value='".h($C)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'no','yes'),$Y[$w]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$Y[$w]["val"],($l["null"]?0:null))),"</div>\n";unset($e[$C]);}elseif(is_array($D=$this->_foreignKeyOptions($_GET["select"],$C))){if($m[$C]["null"])$D[0]='('.'prÃ¡zdne'.')';$w=$qc[$C];$q--;echo"<div>".h($bb)."<input type='hidden' name='where[$q][col]' value='".h($C)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($D,$Y[$w]["val"],true)."</select></div>\n";unset($e[$C]);}}$q=0;foreach($Y
...
as$w=>$s){$E=array();foreach($s["columns"]as$W)$E[]=$e[$W];if(count(array_filter($E,'strlen'))>1&&$w!="PRIMARY")$Xc[$w]=implode(", ",$E);}if($Xc){echo'<fieldset><legend>'.'ZotriediÅ¥'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Xc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
long2str($V,true);}$f='';$bc=$_SESSION["token"];if(!$bc)$_SESSION["token"]=rand(1,1e6);$le=get_token();$dd=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$W){list($w)=explode(":",$W);$dd[$w]=$W;}}$ra=$_POST["auth"];if($ra){session_regenerate_id();$i=$ra["driver"];$P=$ra["server"];$U=$ra["username"];$H=$ra["password"];$h=$ra["db"];set_password($i,$P,$U,$H);$_SESSION["db"][$i][$P][$U][$h]=true;if($ra["permanent"]){$w=base64_encode($i)."-".base64_encode($P)."-".base64_encode($U)."-".base64_encode($h);$ld=$b->permanentLogin(true);$dd[$w]="$w:".base64_encode($ld?encrypt_string($H,$ld):"");cookie("adminer_permanent",implode(" ",$dd));}if(count($_POST)==1||DRIVER!=$i||SERVER!=$P||$_GET["username"]!==$U||DB!=$h)redirect(auth_url($i,$P,$U,$h));}elseif($_POST["logout"]){if($bc&&!verify_token()){page_header('OdhlÃ¡siÅ¥','NeplatnÃ½ token CSRF. OdoÅ¡lite formulÃ¡r znova.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$w)set_session($w,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡senie prebehlo v poriadku.');}}elseif($dd&&!$_SESSION["pwds"]){session_regenerate_id();$ld=$b->permanentLogin();foreach($dd
...
auth_error($zb=null){global$f,$b,$bc;$Kd=session_name();$k="";if(!$_COOKIE[$Kd]&&$_GET[$Kd]&&ini_bool("session.use_only_cookies"))$k='Session premennÃ© musia byÅ¥ povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$Kd]||$_GET[$Kd])&&!$bc)$k='Session vyprÅ¡ala, prihlÃ¡ste sa prosÃ­m znova.';else{$H=get_password();if($H!==null){$k=h($zb?$zb->getMessage():(is_string($f)?$f:'NeplatnÃ© prihlasovacie Ãºdaje.'));if($H===false)$k.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$bd=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$bd["lifetime"]);page_header('PrihlÃ¡siÅ¥ sa',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
...
as$C=>$l){if(!isset($l["privileges"][$ze?"update":"insert"])||$b->fieldName($l)=="")unset($m[$C]);}if($_POST&&!$k&&!isset($_GET["select"])){$z=$_POST["referer"];if($_POST["insert"])$z=($ze?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$z))$z=ME."select=".urlencode($a);$t=indexes($a);$ue=unique_array($_GET["where"],$t);$rd="\nWHERE $Y";if(isset($_POST["delete"]))queries_redirect($z,'PoloÅ¾ka bola vymazanÃ¡.',$i->delete($a,$rd,!$ue));else{$Q=array();foreach($m
as$C=>$l){$W=process_input($l);if($W!==false&&$W!==null)$Q[idf_escape($C)]=$W;}if($ze){if(!$Q)redirect($z);queries_redirect($z,'PoloÅ¾ka bola aktualizovanÃ¡.',$i->update($a,$Q,$rd,!$ue));if(is_ajax()){page_headers();page_messages($k);exit;}}else{$K=$i->insert($a,$Q);$vc=($K?last_id():0);queries_redirect($z,sprintf('PoloÅ¾ka%s bola vloÅ¾enÃ¡.',($vc?" $vc":"")),$K);}}}$Yd=$b->tableName(table_status1($a,true));page_header(($ze?'UpraviÅ¥':'VloÅ¾iÅ¥'),$k,array("select"=>array($a,$Yd)),$Yd);$M=null;if($_POST["save"])$M=(array)$_POST["fields"];elseif($Y){$O=array();foreach($m
...
as$w=>$W){if(!$Y)$M[$w]=null;$m[$w]=array("field"=>$w,"null"=>($w!=$i->primary),"auto_increment"=>($w==$i->primary));}}}if($M===false)echo"<p class='error'>".'Å½iadne riadky.'."\n";echo'
<div id="message"></div>
...
';if($m){echo"<input type='submit' value='".'UloÅ¾iÅ¥'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($ze?'UloÅ¾iÅ¥ a pokraÄovaÅ¥ v ÃºpravÃ¡ch'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'UloÅ¾iÅ¥ a vloÅ¾iÅ¥ ÄalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($ze?"<input type='submit' name='delete' value='".'ZmazaÅ¥'."'".confirm().">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
