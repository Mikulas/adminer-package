...
confirm($wc="",$jd=false){return" onclick=\"".($jd?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($wc?" (' + $wc + ')":"")."');\"";}function
js_escape($C){return
addcslashes($C,"\r\n'\\/");}function
...
upload_error($u){$Yb=($u==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($u?'Súbor sa nepodarilo nahrať.'.($Yb?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Yb):""):'Súbor neexistuje.');}function
...
enum_input($xa,$M,$e,$i,$Wb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($Wb)?"<label><input type='$xa'$M value='$Wb'".((is_array($i)?in_array($Wb,$i):$i===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($U[1]as$o=>$a){$a=stripcslashes(str_replace("''","'",$a));$Ua=(is_int($i)?$i==$o+1:(is_array($i)?in_array($o+1,$i):$i===$a));$b.=" <label><input type='$xa'$M value='".($o+1)."'".($Ua?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$x){global$Ea,$j,$qa;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$Zb=($qa=="mssql"&&$e["auto_increment"]);if($Zb&&!$_POST["save"]){$x=null;}$Z=(isset($_GET["select"])||$Zb?array("orig"=>'originál'):array())+$j->editFunctions($e);$M=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($Zc,$za){$Bd=($za==1||(!$za&&'sk'=='fr')?0:('sk'=='sl'&&(!$za||$za>2)?1:0)+((!$za||$za>=5)&&ereg('cs|sk|ru|sl','sk')?2:1));return
...
selectSearchPrint($p,$z,$F){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$Kb=array();foreach($p
...
as$f=>$oa){$ka=$this->_foreignKeyOptions($_GET["select"],$f);if($ka){if($q[$f]["null"]){$ka[0]='('.'prázdne'.')';}$c=$Kb[$f];$o--;echo"<div>".h($oa)."<input type='hidden' name='where[$o][col]' value='".h($f)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($ka,$p[$c]["val"],true)."</select></div>\n";unset($z[$f]);}}$o=0;foreach($p
...
as$c=>$T){$S=array();foreach($T["columns"]as$a){$S[]=$z[$a];}if(count(array_filter($S,'strlen'))>1&&$c!="PRIMARY"){$Eb[$c]=implode(", ",$S);}}if($Eb){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Eb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
selectEmailPrint($ib,$z){if($ib){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<p>".'Odosielateľ'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Predmet'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75' onkeypress='return textareaKeypress(this, event, false, this.form.email);'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea><br>\n",html_select("email_addition",$z,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložiť'."'>\n";echo"<p>".'Prílohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($ib)==1?'<input type="hidden" name="email_field" value="'.h(key($ib)).'">':html_select("email_field",$ib)),"<input type='submit' name='email' value='".'Odoslať'."' onclick=\"return this.form['delete'].onclick();\">\n","</div></fieldset>\n";}}function
...
';if($Ma!="db"&&$Ma!="ns"){$A=table_status();if(!$A){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($A);}}}}function
...
mail($Ia,email_header($vb),$ic.$W.$jb,$nb);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$wb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$va[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$r=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$r-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($r)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$r=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$la=(isset($_GET["select"])?$_POST["edit"]:$p);$q=fields($r);foreach($q
as$f=>$e){if(!isset($e["privileges"][$la?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$u&&!isset($_GET["select"])){$G=$_POST["referer"];if($_POST["insert"]){$G=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$G)){$G=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$G,'Položka bola vymazaná.');}else{$t=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($la?"\n".idf_escape($f)." = $a":$a);}}if($la){if(!$t){redirect($G);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$t),"\nWHERE $p"),$G,'Položka bola aktualizovaná.');}else{$k=insert_into($r,$t);$zc=($k?last_id():0);queries_redirect($G,sprintf('Položka%s bola vložená.',($zc?" $zc":"")),$k);}}}$Xa=$j->tableName(table_status($r));page_header(($la?'Upraviť':'Vložiť'),$u,array("select"=>array($r,$Xa)),$Xa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$y=array();foreach($q
