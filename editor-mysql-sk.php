...
enum_input($Ed,$c,$j,$W,$gb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$z);$I=($gb!==null?"<label><input type='$Ed'$c value='$gb'".((is_array($W)?in_array($gb,$W):$W===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($z[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$za=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$I.=" <label><input type='$Ed'$c value='".($p+1)."'".($za?' checked':'').'>'.h($b->editVal($V,$j)).'</label>';}return$I;}function
input($j,$W,$m){global$Gd,$b,$Yb;$B=h(bracket_escape($j["field"]));echo"<td class='function'>";$Xc=($Yb=="mssql"&&$j["auto_increment"]);if($Xc&&!$_POST["save"])$m=null;$n=(isset($_GET["select"])||$Xc?array("orig"=>'originál'):array())+$b->editFunctions($j);$c=" name='fields[$B]'";if($j["type"]=="enum")echo
...
lang($Cd,$tc){$Kc=($tc==1?0:($tc&&$tc<5?1:2));$Cd=str_replace("%d","%s",$Cd[$Kc]);$tc=number_format($tc,0,".",' ');return
...
selectLinks($rd,$N=""){$a=$rd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Nová položka'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a>\n";}function
...
selectSearchPrint($X,$f,$s){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$Zb=array();foreach($X
as$u=>$V)$Zb[$V["col"]]=$u;$p=0;$k=fields($_GET["select"]);foreach($f
as$B=>$Ta){$j=$k[$B];if(ereg("enum",$j["type"])||like_bool($j)){$u=$Zb[$B];$p--;echo"<div>".h($Ta)."<input type='hidden' name='where[$p][col]' value='".h($B)."'>:",(like_bool($j)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$j,(array)$X[$u]["val"],($j["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Ac=$this->_foreignKeyOptions($_GET["select"],$B))){if($k[$B]["null"])$Ac[0]='('.'prázdne'.')';$u=$Zb[$B];$p--;echo"<div>".h($Ta)."<input type='hidden' name='where[$p][col]' value='".h($B)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Ac,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$p][val]' value='".h($V["val"])."'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoľvek'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input name='where[$p][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Cc,$f,$s){$Dc=array();foreach($s
as$u=>$r){$Cc=array();foreach($r["columns"]as$V)$Cc[]=$f[$V];if(count(array_filter($Cc,'strlen'))>1&&$u!="PRIMARY")$Dc[$u]=implode(", ",$Cc);}if($Dc){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Dc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectActionPrint($s){echo"<fieldset><legend>".'Akcia'."</legend><div>","<input type='submit' value='".'Vypísať'."'>","</div></fieldset>\n";}function
...
like_bool($j){return
ereg("bool|(tinyint|bit)\\(1\\)",$j["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();$vc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$Ya[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Od=(isset($_GET["select"])?$_POST["edit"]:$X);$k=fields($a);foreach($k
as$B=>$j){if(!isset($j["privileges"][$Od?"update":"insert"])||$b->fieldName($j)=="")unset($k[$B]);}if($_POST&&!$i&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($Od?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$x,'Položka bola vymazaná.');else{$N=array();foreach($k
as$B=>$j){$V=process_input($j);if($V!==false&&$V!==null)$N[idf_escape($B)]=($Od?"\n".idf_escape($B)." = $V":$V);}if($Od){if(!$N)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$x,'Položka bola aktualizovaná.');}else{$H=insert_into($a,$N);$dc=($H?last_id():0);queries_redirect($x,sprintf('Položka%s bola vložená.',($dc?" $dc":"")),$H);}}}$sd=$b->tableName(table_status($a));page_header(($Od?'Upraviť':'Vložiť'),$i,array("select"=>array($a,$sd)),$sd);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($k
as$B=>$j){if(isset($j["privileges"]["select"]))$L[]=($_POST["clone"]&&$j["auto_increment"]?"'' AS ":(ereg("enum|set",$j["type"])?"1*".idf_escape($B)." AS ":"")).idf_escape($B);}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false)echo"<p class='error'>".'Žiadne riadky.'."\n";echo'
...
';if($k){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Od?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($Od?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
