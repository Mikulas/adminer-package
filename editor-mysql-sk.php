...
upload_error($k){$wc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'SÃºbor sa nepodarilo nahraÅ¥.'.($wc?" ".sprintf('MaximÃ¡lna povolenÃ¡ veÄ¾kosÅ¥ sÃºboru je %sB.',$wc):""):'SÃºbor neexistuje.');}function
...
enum_input($Td,$c,$l,$W,$mb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$z);$J=($mb!==null?"<label><input type='$Td'$c value='$mb'".((is_array($W)?in_array($mb,$W):$W===0)?" checked":"")."><i>".'prÃ¡zdne'."</i></label>":"");foreach($z[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$J.=" <label><input type='$Td'$c value='".($p+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$J;}function
input($l,$W,$o){global$g,$Vd,$b,$t;$B=h(bracket_escape($l["field"]));echo"<td class='function'>";$jd=($t=="mssql"&&$l["auto_increment"]);if($jd&&!$_POST["save"])$o=null;$Ob=(isset($_GET["select"])||$jd?array("orig"=>'originÃ¡l'):array())+$b->editFunctions($l);$c=" name='fields[$B]'";if($l["type"]=="enum")echo
...
lang($Rd,$Ec){$Wc=($Ec==1?0:($Ec&&$Ec<5?1:2));$Rd=str_replace("%d","%s",$Rd[$Wc]);$Ec=number_format($Ec,0,".",' ');return
...
selectLinks($Ed,$O=""){$a=$Ed["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'NovÃ¡ poloÅ¾ka'."</a>\n";}function
...
as$B=>$Wa){$l=$m[$B];if(ereg("enum",$l["type"])||like_bool($l)){$u=$jc[$B];$p--;echo"<div>".h($Wa)."<input type='hidden' name='where[$p][col]' value='".h($B)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Nc=$this->_foreignKeyOptions($_GET["select"],$B))){if($m[$B]["null"])$Nc[0]='('.'prÃ¡zdne'.')';$u=$jc[$B];$p--;echo"<div>".h($Wa)."<input type='hidden' name='where[$p][col]' value='".h($B)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Nc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'kdekoÄ¾vek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$p][val]' value='".h($V["val"])."' onsearch='selectSearch(this);'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoÄ¾vek'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$p][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($C,$f,$s){$Pc=array();foreach($s
as$u=>$r){$C=array();foreach($r["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Pc[$u]=implode(", ",$C);}if($Pc){echo'<fieldset><legend>'.'ZotriediÅ¥'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Pc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
like_bool($l){return
ereg("bool|(tinyint|bit)\\(1\\)",$l["full_type"]);}$Hc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$db[DRIVER]='PrihlÃ¡siÅ¥ sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$m=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$m),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$m=fields($a);$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$m):""):where($_GET,$m));$de=(isset($_GET["select"])?$_POST["edit"]:$X);foreach($m
as$B=>$l){if(!isset($l["privileges"][$de?"update":"insert"])||$b->fieldName($l)=="")unset($m[$B]);}if($_POST&&!$k&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($de?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$x,'PoloÅ¾ka bola vymazanÃ¡.');else{$O=array();foreach($m
as$B=>$l){$V=process_input($l);if($V!==false&&$V!==null)$O[idf_escape($B)]=($de?"\n".idf_escape($B)." = $V":$V);}if($de){if(!$O)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $X"),$x,'PoloÅ¾ka bola aktualizovanÃ¡.');}else{$I=insert_into($a,$O);$oc=($I?last_id():0);queries_redirect($x,sprintf('PoloÅ¾ka%s bola vloÅ¾enÃ¡.',($oc?" $oc":"")),$I);}}}$Fd=$b->tableName(table_status($a));page_header(($de?'UpraviÅ¥':'VloÅ¾iÅ¥'),$k,array("select"=>array($a,$Fd)),$Fd);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($X){$M=array();foreach($m
as$B=>$l){if(isset($l["privileges"]["select"])){$ma=convert_field($l);if($_POST["clone"]&&$l["auto_increment"])$ma="''";if($t=="sql"&&ereg("enum|set",$l["type"]))$ma="1*".idf_escape($B);$M[]=($ma?"$ma AS ":"").idf_escape($B);}}$K=array();if($M){$L=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$K=(isset($_GET["select"])&&count($L)!=1?null:reset($L));}}if($K===false)echo"<p class='error'>".'Å½iadne riadky.'."\n";echo'
...
';if($m){echo"<input type='submit' value='".'UloÅ¾iÅ¥'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($de?'UloÅ¾iÅ¥ a pokraÄovaÅ¥ v ÃºpravÃ¡ch':'UloÅ¾iÅ¥ a vloÅ¾iÅ¥ ÄalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($de?"<input type='submit' name='delete' value='".'ZmazaÅ¥'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$B=>$V){$V=process_input($m[$B]);if($V!==null){if($_POST["clone"])$O[idf_escape($B)]=($V!==false?$V:idf_escape($B));elseif($V!==false)$O[]=idf_escape($B)." = $V";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ja="UPDATE";if($_POST["delete"]){$Ja="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ja="INSERT";$H="INTO $H";}if($_POST["all"]||($be===array()&&$_POST["check"])||$gc){$I=queries("$Ja $H".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $le"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$I=queries($Ja.limit1($H,"\nWHERE ".where_check($V,$m)));if(!$I)break;$ia+=$g->affected_rows;}}}$_=sprintf('%d poloÅ¾iek bolo ovplyvnenÃ½ch.',$ia);if($_POST["clone"]&&$I&&$ia==1){$oc=last_id();if($oc)$_=sprintf('PoloÅ¾ka%s bola vloÅ¾enÃ¡.'," $oc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$_,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+kliknite na polÃ­Äko, ktorÃ© chcete zmeniÅ¥.';else{$I=true;$ia=0;foreach($_POST["val"]as$Zd=>$K){$O=array();foreach($K
as$u=>$V){$u=bracket_escape($u,1);$O[]=idf_escape($u)." = ".(ereg('char|text',$m[$u]["type"])||$V!=""?$b->processInput($m[$u],$V):"NULL");}$H=table($a)." SET ".implode(", ",$O);$ke=" WHERE ".where_check($Zd,$m).($X?" AND ".implode(" AND ",$X):"");$I=queries("UPDATE".($gc?" $H$ke":limit1($H,$ke)));if(!$I)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d poloÅ¾iek bolo ovplyvnenÃ½ch.',$ia),$I);}}elseif(is_string($zb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Ia=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$zb,$z);$ia=count($z[0]);begin();$sd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($z[0]as$u=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$sd]*)$sd~",$V.$sd,$uc);if(!$u&&!array_diff($uc[1],$Ia)){$Ia=$uc[1];$ia--;}else{$O=array();foreach($uc[1]as$p=>$Fa)$O[idf_escape($Ia[$p])]=($Fa==""&&$m[$Ia[$p]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Fa))));$I=insert_update($a,$O,$Yc);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovanÃ½ %d zÃ¡znam.','Boli importovanÃ© %d zÃ¡znamy.','Bolo importovanÃ½ch %d zÃ¡znamov.'),$ia),$I);queries("ROLLBACK");}else$k=upload_error($zb);}}$Fd=$b->tableName($S);if(is_ajax())ob_start();page_header('VypÃ­saÅ¥'.": $Fd",$k);$O=null;if(isset($md["insert"])){$O="";foreach((array)$_GET["where"]as$V){if(count($Ib[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$O.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($S,$O);if(!$f)echo"<p class='error'>".'TabuÄ¾ku sa nepodarilo vypÃ­saÅ¥'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($X,$f,$s);$b->selectOrderPrint($C,$f,$s);$b->selectLimitPrint($v);$b->selectLengthPrint($Jd);$b->selectActionPrint($s);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$n=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$E=floor(max(0,$n-1)/$v);}$H=$b->selectQueryBuild($M,$X,$Pb,$C,$v,$E);if(!$H)$H="SELECT".limit((+$v&&$Pb&&$gc&&$t=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Mb,($X?"\nWHERE ".implode(" AND ",$X):"").$Qb,($v!=""?+$v:null),($E?$v*$E:0),"\n");echo$b->selectQuery($H);$I=$g->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($t=="mssql"&&$E)$I->seek($v*$E);$lb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$t=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$n=(+$v&&$Pb&&$gc?($t=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Å½iadne riadky.'."\n";else{$ua=$b->backwardKeys($a,$Fd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Pb&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviÅ¥'."</a>");$Cc=array();$Ob=array();reset($M);$ed=1;foreach($L[0]as$u=>$V){if($u!=$Gc){$V=$_GET["columns"][key($M)];$l=$m[$M?($V?$V["col"]:current($M)):$u];$B=($l?$b->fieldName($l,$ed):"*");if($B!=""){$ed++;$Cc[$u]=$B;$e=idf_escape($u);$Xb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Wa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Xb.($C[0]==$e||$C[0]==$u||(!$C&&$gc&&$Pb[0]==$e)?$Wa:'')).'">';echo(!$M||$V?apply_sql_function($V["fun"],$B):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($Xb.$Wa)."' title='".'zostupne'."' class='text'> â†“</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'VyhÄ¾adaÅ¥'.'" class="text jsonly"> =</a>';echo"</span>";}$Ob[$u]=$V["fun"];next($M);}}$rc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$u=>$V)$rc[$u]=max($rc[$u],min(40,strlen(utf8_decode($V))));}}echo($ua?"<th>".'VzÅ¥ahy':"")."</thead>\n";if(is_ajax()){if($v%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Ib)as$A=>$K){$Yd=unique_array($L[$A],$s);$Zd="";foreach($Yd
as$u=>$V){if(strlen($V)>64){$u="MD5(".(strpos($u,'(')?$u:idf_escape($u)).")";$V=md5($V);}$Zd.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));}echo"<tr".odd().">".(!$Pb&&$M?"":"<td>".checkbox("check[]",substr($Zd,1),in_array(substr($Zd,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($gc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Zd)."'>".'upraviÅ¥'."</a>"));foreach($K
