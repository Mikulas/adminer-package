...
confirm($Ja=""){return" onclick=\"return confirm('".'Naozaj?'.($Ja?" (' + $Ja + ')":"")."');\"";}function
...
upload_error($i){$lc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($i?'Súbor sa nepodarilo nahrať.'.($lc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$lc):""):'Súbor neexistuje.');}function
...
enum_input($Fd,$c,$j,$W,$gb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$z);$I=($gb!==null?"<label><input type='$Fd'$c value='$gb'".((is_array($W)?in_array($gb,$W):$W===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($z[1]as$o=>$V){$V=stripcslashes(str_replace("''","'",$V));$za=(is_int($W)?$W==$o+1:(is_array($W)?in_array($o+1,$W):$W===$V));$I.=" <label><input type='$Fd'$c value='".($o+1)."'".($za?' checked':'').'>'.h($b->editVal($V,$j)).'</label>';}return$I;}function
input($j,$W,$m){global$Hd,$b,$s;$B=h(bracket_escape($j["field"]));echo"<td class='function'>";$Yc=($s=="mssql"&&$j["auto_increment"]);if($Yc&&!$_POST["save"])$m=null;$Hb=(isset($_GET["select"])||$Yc?array("orig"=>'originál'):array())+$b->editFunctions($j);$c=" name='fields[$B]'";if($j["type"]=="enum")echo
...
lang($Dd,$tc){$Lc=($tc==1?0:($tc&&$tc<5?1:2));$Dd=str_replace("%d","%s",$Dd[$Lc]);$tc=number_format($tc,0,".",' ');return
...
<tr><th>Používateľ<td><input type="hidden" name="auth[driver]" value="server"><input id="username" name="auth[username]" value="',h($_GET["username"]),'">
...
as$e=>$V)$w.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($J[$V]);echo"<a href='".h($w)."' title='".'Nová položka'."'>+</a> ";}}}function
...
selectVal($V,$w,$j){$I=($V===null?"&nbsp;":$V);if(ereg('blob|bytea',$j["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$I="<img src='$w' alt='$I'>";}if(like_bool($j)&&$I!="&nbsp;")$I=($V?'yes':'no');if($w)$I="<a href='$w'>$I</a>";if(!$w&&!like_bool($j)&&ereg('int|float|double|decimal',$j["type"]))$I="<div class='number'>$I</div>";elseif(ereg('date',$j["type"]))$I="<div class='datetime'>$I</div>";return$I;}function
...
selectSearchPrint($X,$f,$r){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Vyhľadať'."</legend><div>\n";$Zb=array();foreach($X
as$u=>$V)$Zb[$V["col"]]=$u;$o=0;$k=fields($_GET["select"]);foreach($f
as$B=>$Ta){$j=$k[$B];if(ereg("enum",$j["type"])||like_bool($j)){$u=$Zb[$B];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($B)."'>:",(like_bool($j)?" <select name='where[$o][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$o][val][]'",$j,(array)$X[$u]["val"],($j["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Bc=$this->_foreignKeyOptions($_GET["select"],$B))){if($k[$B]["null"])$Bc[0]='('.'prázdne'.')';$u=$Zb[$B];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($B)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($Bc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$o=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$o][val]' value='".h($V["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoľvek'.")".optionlist($f,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Dc,$f,$r){$Ec=array();foreach($r
as$u=>$q){$Dc=array();foreach($q["columns"]as$V)$Dc[]=$f[$V];if(count(array_filter($Dc,'strlen'))>1&&$u!="PRIMARY")$Ec[$u]=implode(", ",$Dc);}if($Ec){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Ec,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';if($oc!="db"&&$oc!="ns"){$R=table_status();if(!$R)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else$this->tablesPrint($R);}}}function
tablesPrint($ud){echo'<p id="tables" onmouseover="this.style.overflow = \'visible\';" onmouseout="this.style.overflow = \'auto\';">'."\n";foreach($ud
as$J){$B=$this->tableName($J);if(isset($J["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($J["Name"])."'".bold($_GET["select"]==$J["Name"])." title='".'Vypísať dáta'."'>$B</a><br>\n";}}function
...
like_bool($j){return
ereg("bool|(tinyint|bit)\\(1\\)",$j["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();$wc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$Ya[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Pd=(isset($_GET["select"])?$_POST["edit"]:$X);$k=fields($a);foreach($k
as$B=>$j){if(!isset($j["privileges"][$Pd?"update":"insert"])||$b->fieldName($j)=="")unset($k[$B]);}if($_POST&&!$i&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($Pd?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$x,'Položka bola vymazaná.');else{$N=array();foreach($k
as$B=>$j){$V=process_input($j);if($V!==false&&$V!==null)$N[idf_escape($B)]=($Pd?"\n".idf_escape($B)." = $V":$V);}if($Pd){if(!$N)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$x,'Položka bola aktualizovaná.');}else{$H=insert_into($a,$N);$dc=($H?last_id():0);queries_redirect($x,sprintf('Položka%s bola vložená.',($dc?" $dc":"")),$H);}}}$td=$b->tableName(table_status($a));page_header(($Pd?'Upraviť':'Vložiť'),$i,array("select"=>array($a,$td)),$td);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($k
as$B=>$j){if(isset($j["privileges"]["select"]))$L[]=($_POST["clone"]&&$j["auto_increment"]?"'' AS ":($s=="sql"&&ereg("enum|set",$j["type"])?"1*".idf_escape($B)." AS ":"")).idf_escape($B);}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false)echo"<p class='error'>".'Žiadne riadky.'."\n";echo'
...
';if($k){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Pd?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($Pd?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$B=>$V){$V=process_input($k[$B]);if($V!==null){if($_POST["clone"])$N[idf_escape($B)]=($V!==false?$V:idf_escape($B));elseif($V!==false)$N[]=idf_escape($B)." = $V";}}$G.=($_POST["clone"]?" (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a):" SET\n".implode(",\n",$N));}if($_POST["delete"]||$N){$Ea="UPDATE";if($_POST["delete"]){$Ea="DELETE";$G="FROM $G";}if($_POST["clone"]){$Ea="INSERT";$G="INTO $G";}if($_POST["all"]||($Nd===array()&&$_POST["check"])||count($n)<count($L)){$H=queries("$Ea $G".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $Xd"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$H=queries($Ea.limit1($G,"\nWHERE ".where_check($V)));if(!$H)break;$ia+=$g->affected_rows;}}}$_=sprintf('%d položiek bolo ovplyvnených.',$ia);if($_POST["clone"]&&$H&&$ia==1){$dc=last_id();if($dc)$_=sprintf('Položka%s bola vložená.'," $dc");}queries_redirect(remove_from_uri("page"),$_,$H);}elseif(!$_POST["import"]){if(!$_POST["val"])$i='Dvojkliknite na políčko, ktoré chcete zmeniť.';else{$H=true;$ia=0;foreach($_POST["val"]as$Ld=>$J){$N=array();foreach($J
as$u=>$V){$u=bracket_escape($u,1);$N[]=idf_escape($u)." = ".(ereg('char|text',$k[$u]["type"])||$V!=""?$b->processInput($k[$u],$V):"NULL");}$G=table($a)." SET ".implode(", ",$N);$Wd=" WHERE ".where_check($Ld).($X?" AND ".implode(" AND ",$X):"");$H=queries("UPDATE".(count($n)<count($L)?" $G$Wd":limit1($G,$Wd)));if(!$H)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$ia),$H);}}elseif(is_string($l=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$H=true;$Da=array_keys($k);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$l,$z);$ia=count($z[0]);begin();$fd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($z[0]as$u=>$V){preg_match_all("~((\"[^\"]*\")+|[^$fd]*)$fd~",$V.$fd,$jc);if(!$u&&!array_diff($jc[1],$Da)){$Da=$jc[1];$ia--;}else{$N=array();foreach($jc[1]as$o=>$Aa)$N[idf_escape($Da[$o])]=($Aa==""&&$k[$Da[$o]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Aa))));$H=insert_update($a,$N,$Nc);if(!$H)break;}}if($H)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$ia),$H);queries("ROLLBACK");}else$i=upload_error($l);}}$td=$b->tableName($R);page_header('Vypísať'.": $td",$i);session_write_close();$N=null;if(isset($Zc["insert"])){$N="";foreach((array)$_GET["where"]as$V){if(count($Bb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$N.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($R,$N);if(!$f)echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($k?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($X,$f,$r);$b->selectOrderPrint($Dc,$f,$r);$b->selectLimitPrint($v);$b->selectLengthPrint($wd);$b->selectActionPrint($r);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$Eb=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$D=floor(max(0,$Eb-1)/$v);}$G="SELECT".limit((+$v&&$n&&count($n)<count($L)&&$s=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Gb,($X?"\nWHERE ".implode(" AND ",$X):"").$Ib,($v!=""?+$v:null),($D?$v*$D:0),"\n");echo$b->selectQuery($G);$H=$g->query($G);if(!$H)echo"<p class='error'>".error()."\n";else{if($s=="mssql")$H->seek($v*$D);$fb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($D&&$s=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last")$Eb=(+$v&&$n&&count($n)<count($L)?($s=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($G) x")):count($K));if(!$K)echo"<p class='message'>".'Žiadne riadky.'."\n";else{$ta=$b->backwardKeys($a,$td);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$n&&$L?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$rc=array();$Hb=array();reset($L);$Tc=1;foreach($K[0]as$u=>$V){if($u!=$vc){$V=$_GET["columns"][key($L)];$j=$k[$L?($V?$V["col"]:current($L)):$u];$B=($j?$b->fieldName($j,$Tc):"*");if($B!=""){$Tc++;$rc[$u]=$B;$Pb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Pb).'">'.(!$L||$V?apply_sql_function($V["fun"],$B):h(current($L)))."</a>";echo"<span class='column hidden'>","<a href='".h("$Pb&desc%5B0%5D=1")."' title='".'zostupne'."' class='text'> ↓</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Vyhľadať'.'" class="text jsonly"> =</a>';echo"</span>";}$Hb[$u]=$V["fun"];next($L);}}$gc=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
as$u=>$V)$gc[$u]=max($gc[$u],min(40,strlen(utf8_decode($V))));}}echo($ta?"<th>".'Vzťahy':"")."</thead>\n";foreach($b->rowDescriptions($K,$Bb)as$A=>$J){$Kd=unique_array($K[$A],$r);$Ld="";foreach($Kd
as$u=>$V)$Ld.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$n&&$L?"":"<td>".checkbox("check[]",substr($Ld,1),in_array(substr($Ld,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($n)<count($L)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Ld)."'>".'upraviť'."</a>"));foreach($J
