...
idf_unescape($cc){$qc=substr($cc,-1);return
str_replace($qc.$qc,$qc,substr($cc,1,-1));}function
escape_string($V){return
substr(q($V),1,-1);}function
remove_slashes($ed,$Eb=false){if(get_magic_quotes_gpc()){while(list($u,$V)=each($ed)){foreach($V
as$lc=>$U){unset($ed[$u][$lc]);if(is_array($U)){$ed[$u][stripslashes($lc)]=$U;$ed[]=&$ed[$u][stripslashes($lc)];}else$ed[$u][stripslashes($lc)]=($Eb?$U:stripslashes($U));}}}}function
...
nl_br($O){return
str_replace("\n","<br>",$O);}function
checkbox($A,$W,$Ba,$oc="",$Nc="",$kc=false){static$q=0;$q++;$I="<input type='checkbox' name='$A' value='".h($W)."'".($Ba?" checked":"").($Nc?' onclick="'.h($Nc).'"':'').($kc?" class='jsonly'":"")." id='checkbox-$q'>";return($oc!=""?"<label for='checkbox-$q'>$I".h($oc)."</label>":$I);}function
optionlist($Rc,$vd=null,$le=false){$I="";foreach($Rc
as$lc=>$U){$Sc=array($lc=>$U);if(is_array($U)){$I.='<optgroup label="'.h($lc).'">';$Sc=$U;}foreach($Sc
as$u=>$V)$I.='<option'.($le||is_string($u)?' value="'.h($u).'"':'').(($le||is_string($u)?(string)$u:$V)===$vd?' selected':'').'>'.h($V);if(is_array($U))$I.='</optgroup>';}return$I;}function
...
confirm($Oa=""){return" onclick=\"return confirm('".'Naozaj?'.($Oa?" (' + $Oa + ')":"")."');\"";}function
print_fieldset($q,$sc,$oe=false,$Nc=""){echo"<fieldset><legend><a href='#fieldset-$q' onclick=\"".h($Nc)."return !toggle('fieldset-$q');\">$sc</a></legend><div id='fieldset-$q'".($oe?"":" class='hidden'").">\n";}function
bold($ya){return($ya?" class='active'":"");}function
odd($I=' class="odd"'){static$p=0;if(!$I)$p=-1;return($p++%2?$I:'');}function
js_escape($O){return
addcslashes($O,"\r\n'\\/");}function
json_row($u,$V=null){static$Fb=true;if($Fb)echo"{";if($u!=""){echo($Fb?"":",")."\n\t\"".addcslashes($u,"\r\n\"\\").'": '.($V!==null?'"'.addcslashes($V,"\r\n\"\\").'"':'undefined');$Fb=false;}else{echo"\n}\n";$Fb=true;}}function
...
2;$I[$u]=$J[$u];}return$I;}}}function
where($X,$m=array()){global$t;$I=array();$Qb='(^[\w\(]+'.str_replace("_",".*",preg_quote(idf_escape("_"))).'\)+$)';foreach((array)$X["where"]as$u=>$V){$u=bracket_escape($u,1);$I[]=(preg_match($Qb,$u)?$u:idf_escape($u)).(($t=="sql"&&ereg('\\.',$V))||$t=="mssql"?" LIKE ".exact_value(addcslashes($V,"%_\\")):" = ".unconvert_field($m[$u],exact_value($V)));}foreach((array)$X["null"]as$u)$I[]=idf_escape($u)." IS NULL";return
...
as$u=>$V){if($L&&!in_array(idf_escape($u),$L))continue;$ma=convert_field($m[$u]);if($ma)$I.=", $ma AS ".idf_escape($u);}return$I;}function
...
auth_url($db,$M,$T,$i=null){global$eb;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($eb))."|username|".($i!==null?"db|":"").session_name()),$y);return"$y[1]?".(sid()?SID."&":"").($db!="server"||$M!=""?urlencode($db)."=".urlencode($M)."&":"")."username=".urlencode($T).($i!=""?"&db=".urlencode($i):"").($y[2]?"&$y[2]":"");}function
...
query_redirect($G,$x,$z,$kd=true,$wb=true,$zb=false){global$g,$k,$b;$Qd="";if($wb){$Bd=microtime();$zb=!$g->query($G);$Qd="; -- ".format_time($Bd,microtime());}$Ad="";if($G)$Ad=$b->messageQuery($G.$Qd);if($zb){$k=error().$Ad;return
false;}if($kd)redirect($x,$z.$Ad);return
true;}function
...
false;}return
true;}function
queries_redirect($x,$z,$kd){return
query_redirect(queries(),$x,$z,$kd,false,!$kd);}function
...
get_file($u,$Va=false){$Bb=$_FILES[$u];if(!$Bb)return
null;foreach($Bb
as$u=>$V)$Bb[$u]=(array)$V;$I='';foreach($Bb["error"]as$u=>$k){if($k)return$k;$A=$Bb["name"][$u];$Vd=$Bb["tmp_name"][$u];$Na=file_get_contents($Va&&ereg('\\.gz$',$A)?"compress.zlib://$Vd":$Vd);if($Va){$Bd=substr($Na,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$Bd,$ld))$Na=iconv("utf-16","utf-8",$Na);elseif($Bd=="\xEF\xBB\xBF")$Na=substr($Na,3);}$I.=$Na."\n\n";}return$I;}function
upload_error($k){$_c=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'SÃºbor sa nepodarilo nahraÅ¥.'.($_c?" ".sprintf('MaximÃ¡lna povolenÃ¡ veÄ¾kosÅ¥ sÃºboru je %sB.',$_c):""):'SÃºbor neexistuje.');}function
repeat_pattern($F,$tc){return
str_repeat("$F{0,65535}",$tc/65535)."$F{0,".($tc%65535)."}";}function
is_utf8($V){return(preg_match('~~u',$V)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$V));}function
shorten_utf8($O,$tc=80,$Hd=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{FFFF}]",$tc).")($)?)u",$O,$y))preg_match("(^(".repeat_pattern("[\t\r\n -~]",$tc).")($)?)",$O,$y);return
h($y[1]).$Hd.(isset($y[2])?"":"<i>...</i>");}function
friendly_url($V){return
preg_replace('~[^a-z0-9_]~i','-',$V);}function
hidden_fields($ed,$ec=array()){while(list($u,$V)=each($ed)){if(is_array($V)){foreach($V
as$lc=>$U)$ed[$u."[$lc]"]=$U;}elseif(!in_array($u,$ec))echo'<input type="hidden" name="'.h($u).'" value="'.h($V).'">';}}function
...
enum_input($Zd,$c,$l,$W,$nb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$xc);$I=($nb!==null?"<label><input type='$Zd'$c value='$nb'".((is_array($W)?in_array($nb,$W):$W===0)?" checked":"")."><i>".'prÃ¡zdne'."</i></label>":"");foreach($xc[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$I.=" <label><input type='$Zd'$c value='".($p+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$I;}function
input($l,$W,$o){global$g,$be,$b,$t;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$od=($t=="mssql"&&$l["auto_increment"]);if($od&&!$_POST["save"])$o=null;$Rb=(isset($_GET["select"])||$od?array("orig"=>'originÃ¡l'):array())+$b->editFunctions($l);$c=" name='fields[$A]'";if($l["type"]=="enum")echo
nbsp($Rb[""])."<td>".$b->editInput($_GET["edit"],$l,$c,$W);else{$Fb=0;foreach($Rb
as$u=>$V){if($u===""||!$V)break;$Fb++;}$Mc=($Fb?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($l["field"])))."]']; if ($Fb > f.selectedIndex) f.selectedIndex = $Fb;\"":"");$c.=$Mc;echo(count($Rb)>1?html_select("function[$A]",$Rb,$o===null||in_array($o,$Rb)||isset($Rb[$o])?$o:"","functionChange(this);"):nbsp(reset($Rb))).'<td>';$ic=$b->editInput($_GET["edit"],$l,$c,$W);if($ic!="")echo$ic;elseif($l["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$xc);foreach($xc[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?($W>>$p)&1:in_array($V,explode(",",$W),true));echo" <label><input type='checkbox' name='fields[$A][$p]' value='".(1<<$p)."'".($Ba?' checked':'')."$Mc>".h($b->editVal($V,$l)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$l["type"])&&ini_bool("file_uploads"))echo"<input type='file' name='fields-$A'$Mc>";elseif(($Nd=ereg('text|lob',$l["type"]))||ereg("\n",$W)){if($Nd&&$t!="sqlite")$c.=" cols='50' rows='12'";else{$K=min(12,substr_count($W,"\n")+1);$c.=" cols='30' rows='$K'".($K==1?" style='height: 1.2em;'":"");}echo"<textarea$c>".h($W).'</textarea>';}else{$Bc=(!ereg('int',$l["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$l["length"],$y)?((ereg("binary",$l["type"])?2:1)*$y[1]+($y[3]?1:0)+($y[2]&&!$l["unsigned"]?1:0)):($be[$l["type"]]?$be[$l["type"]]+($l["unsigned"]?0:1):0));if($g->server_info>=5.6&&ereg('time',$l["type"]))$Bc+=7;echo"<input".(ereg('int',$l["type"])?" type='number'":"")." value='".h($W)."'".($Bc?" maxlength='$Bc'":"").(ereg('char|binary',$l["type"])&&$Bc>20?" size='40'":"")."$c>";}}}function
...
is_url($O){$cb='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($cb?\\.)+$cb(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$O,$y)?strtolower($y[1]):"");}function
is_shortable($l){return
ereg('char|text|lob|geometry|point|linestring|polygon',$l["type"]);}function
slow_query($G){global$b,$S;$i=$b->database();if(support("kill")&&is_object($h=connect())&&($i==""||$h->select_db($i))){$nc=$h->result("SELECT CONNECTION_ID()");echo'<script type="text/javascript">
...
as$p=>$Da){$jb=$Za[$Da];if(!isset($jb))$jb=$se.$se[0];$I.=$jb;if($p)$Za[]=$se.$jb[0];$se=$jb;}return$I;}global$b,$g,$eb,$hb,$qb,$k,$Rb,$Ub,$aa,$hc,$t,$ba,$pc,$Lc,$Yc,$Ed,$S,$Yd,$be,$ie,$ca;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$aa=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$Wc=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$aa);if(version_compare(PHP_VERSION,'5.2.0')>=0)$Wc[]=true;call_user_func_array('session_set_cookie_params',$Wc);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Eb);if(function_exists("set_magic_quotes_runtime"))set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'sk';}function
lang($Xd,$Ic=null){if(is_array($Xd)){$ad=($Ic==1?0:($Ic&&$Ic<5?1:2));$Xd=$Xd[$ad];}$Xd=str_replace("%d","%s",$Xd);$Ic=number_format($Ic,0,".",' ');return
...
limit($G,$X,$v,$Jc=0,$xd=" "){return" $G$X".($v!==null?$xd."LIMIT $v".($Jc?" OFFSET $Jc":""):"");}function
limit1($G,$X){return
limit($G,$X,1);}function
db_collation($i,$Ha){global$g;$I=null;$Pa=$g->result("SHOW CREATE DATABASE ".idf_escape($i),1);if(preg_match('~ COLLATE ([^ ]+)~',$Pa,$y))$I=$y[1];elseif(preg_match('~ CHARACTER SET ([^ ]+)~',$Pa,$y))$I=$Ha[$y[1]][-1];return$I;}function
...
foreign_keys($Q){global$g,$Lc;static$F='`(?:[^`]|``)+`';$I=array();$Qa=$g->result("SHOW CREATE TABLE ".table($Q),1);if($Qa){preg_match_all("~CONSTRAINT ($F) FOREIGN KEY \\(((?:$F,? ?)+)\\) REFERENCES ($F)(?:\\.($F))? \\(((?:$F,? ?)+)\\)(?: ON DELETE ($Lc))?(?: ON UPDATE ($Lc))?~",$Qa,$xc,PREG_SET_ORDER);foreach($xc
as$y){preg_match_all("~$F~",$y[2],$_d);preg_match_all("~$F~",$y[5],$Md);$I[idf_unescape($y[1])]=array("db"=>idf_unescape($y[4]!=""?$y[3]:$y[4]),"table"=>idf_unescape($y[4]!=""?$y[4]:$y[3]),"source"=>array_map('idf_unescape',$_d[0]),"target"=>array_map('idf_unescape',$Md[0]),"on_delete"=>($y[6]?$y[6]:"RESTRICT"),"on_update"=>($y[7]?$y[7]:"RESTRICT"),);}}return$I;}function
...
routine($A,$Zd){global$g,$qb,$hc,$be;$ka=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$ae="((".implode("|",array_merge(array_keys($be),$ka)).")\\b(?:\\s*\\(((?:[^'\")]*|$qb)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$F="\\s*(".($Zd=="FUNCTION"?"":$hc).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$ae";$Pa=$g->result("SHOW CREATE $Zd ".idf_escape($A),2);preg_match("~\\(((?:$F\\s*,?)*)\\)\\s*".($Zd=="FUNCTION"?"RETURNS\\s+$ae\\s+":"")."(.*)~is",$Pa,$y);$m=array();preg_match_all("~$F\\s*,?~is",$y[1],$xc,PREG_SET_ORDER);foreach($xc
...
as$u=>$V)$N[$u]="$u = $V";$je=implode(", ",$N);return
...
login($vc,$E){global$g;$g->query("SET time_zone = ".q(substr_replace(@date("O"),":",-2,0)));return
true;}function
tableName($Jd){return
h($Jd["Comment"]!=""?$Jd["Comment"]:$Jd["Name"]);}function
fieldName($l,$B=0){return
h($l["comment"]!=""?$l["comment"]:$l["field"]);}function
selectLinks($Jd,$N=""){$a=$Jd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'NovÃ¡ poloÅ¾ka'."</a>\n";}function
...
as$u=>$V){$A=$this->tableName(table_status($u,true));if($A!=""){$ud=preg_quote($Id);$xd="(:|\\s*-)?\\s+";$I[$u]["name"]=(preg_match("(^$ud$xd(.+)|^(.+?)$xd$ud\$)iu",$A,$y)?$y[2].$y[3]:$A);}else
...
rowDescriptions($K,$Jb){$I=$K;foreach($K[0]as$u=>$V){if(list($Q,$q,$A)=$this->_foreignColumn($Jb,$u)){$dc=array();foreach($K
as$J)$dc[$J[$u]]=exact_value($J[$u]);$Ya=$this->_values[$Q];if(!$Ya)$Ya=get_key_vals("SELECT $q, $A FROM ".table($Q)." WHERE $q IN (".implode(", ",$dc).")");foreach($K
as$_=>$J){if(isset($J[$u]))$I[$_][$u]=(string)$Ya[$J[$u]];}}}return$I;}function
selectLink($V,$l){}function
selectVal($V,$w,$l){$I=($V===null?"&nbsp;":$V);$w=h($w);if(ereg('blob|bytea',$l["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$I="<img src='$w' alt='$I'>";}if(like_bool($l)&&$I!="&nbsp;")$I=($V?'yes':'no');if($w)$I="<a href='$w'>$I</a>";if(!$w&&!like_bool($l)&&ereg('int|float|double|decimal',$l["type"]))$I="<div class='number'>$I</div>";elseif(ereg('date',$l["type"]))$I="<div class='datetime'>$I</div>";return$I;}function
...
selectSearchPrint($X,$f,$s){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'VyhÄ¾adaÅ¥'."</legend><div>\n";$mc=array();foreach($X
as$u=>$V)$mc[$V["col"]]=$u;$p=0;$m=fields($_GET["select"]);foreach($f
as$A=>$Xa){$l=$m[$A];if(ereg("enum",$l["type"])||like_bool($l)){$u=$mc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$A]);}elseif(is_array($Rc=$this->_foreignKeyOptions($_GET["select"],$A))){if($m[$A]["null"])$Rc[0]='('.'prÃ¡zdne'.')';$u=$mc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Rc,$X[$u]["val"],true)."</select></div>\n";unset($f[$A]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'kdekoÄ¾vek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$p][val]' value='".h($V["val"])."' onsearch='selectSearchSearch(this);'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoÄ¾vek'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$p][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$f,$s){$Tc=array();foreach($s
as$u=>$r){$B=array();foreach($r["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$u!="PRIMARY")$Tc[$u]=implode(", ",$B);}if($Tc){echo'<fieldset><legend>'.'ZotriediÅ¥'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectActionPrint($s){echo"<fieldset><legend>".'Akcia'."</legend><div>","<input type='submit' value='".'VypÃ­saÅ¥'."'>","</div></fieldset>\n";}function
selectCommandPrint(){return
true;}function
selectImportPrint(){return
true;}function
selectEmailPrint($lb,$f){if($lb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'OdosielateÄ¾'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Predmet'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'VloÅ¾iÅ¥'."'>\n";echo"<p>".'PrÃ­lohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($lb)==1?'<input type="hidden" name="email_field" value="'.h(key($lb)).'">':html_select("email_field",$lb)),"<input type='submit' name='email' value='".'OdoslaÅ¥'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
selectColumnsProcess($f,$s){return
array(array(),array());}function
selectSearchProcess($m,$s){$I=array();foreach((array)$_GET["where"]as$u=>$X){$Fa=$X["col"];$Oc=$X["op"];$V=$X["val"];if(($u<0?"":$Fa).$V!=""){$La=array();foreach(($Fa!=""?array($Fa=>$m[$Fa]):$m)as$A=>$l){if($Fa!=""||is_numeric($V)||!ereg('int|float|double|decimal',$l["type"])){$A=idf_escape($A);if($Fa!=""&&$l["type"]=="enum")$La[]=(in_array(0,$V)?"$A IS NULL OR ":"")."$A IN (".implode(", ",array_map('intval',$V)).")";else{$Pd=ereg('char|text|enum|set',$l["type"]);$W=$this->processInput($l,(!$Oc&&$Pd&&ereg('^[^%]+$',$V)?"%$V%":$V));$La[]=$A.($W=="NULL"?" IS".($Oc==">="?" NOT":"")." $W":(in_array($Oc,$this->operators)||$Oc=="="?" $Oc $W":($Pd?" LIKE $W":" IN (".str_replace(",","', '",$W).")")));if($u<0&&$V=="0")$La[]="$A IS NULL";}}}$I[]=($La?"(".implode(" OR ",$La).")":"0");}}return$I;}function
...
true;if($_POST["email"]){$wd=0;if($_POST["all"]||$_POST["check"]){$l=idf_escape($_POST["email_field"]);$Gd=$_POST["email_subject"];$z=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$Gd.$z",$xc);$K=get_rows("SELECT DISTINCT $l".($xc[1]?", ".implode(", ",array_map('idf_escape',array_unique($xc[1]))):"")." FROM ".table($_GET["select"])." WHERE $l IS NOT NULL AND $l != ''".($X?" AND ".implode(" AND ",$X):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$m=fields($_GET["select"]);foreach($this->rowDescriptions($K,$Jb)as$J){$nd=array('{\\'=>'{');foreach($xc[1]as$V)$nd['{$'."$V}"]=$this->editVal($J[$V],$m[$V]);$kb=$J[$_POST["email_field"]];if(is_mail($kb)&&send_mail($kb,strtr($Gd,$nd),strtr($z,$nd),$_POST["email_from"],$_FILES["email_files"]))$wd++;}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(array('Bol odoslanÃ½ %d e-mail.','Boli odoslanÃ© %d e-maily.','Bolo odoslanÃ½ch %d e-mailov.'),$wd));}return
false;}function
selectQueryBuild($L,$X,$Sb,$B,$v,$D){return"";}function
messageQuery($G){return" <span class='time'>".@date("H:i:s")."</span><!--\n".str_replace("--","--><!-- ",$G)."\n-->";}function
editFunctions($l){$I=array();if($l["null"]&&ereg('blob',$l["type"]))$I["NULL"]='prÃ¡zdne';$I[""]=($l["null"]||$l["auto_increment"]||like_bool($l)?"":"*");if(ereg('date|time',$l["type"]))$I["now"]='teraz';if(eregi('_(md5|sha1)$',$l["field"],$y))$I[]=strtolower($y[1]);return$I;}function
editInput($Q,$l,$c,$W){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$c value='-1' checked><i>".'originÃ¡l'."</i></label> ":"").enum_input("radio",$c,$l,($W||isset($_GET["select"])?$W:0),($l["null"]?"":null));$Rc=$this->_foreignKeyOptions($Q,$l["field"],$W);if($Rc!==null)return(is_array($Rc)?"<select$c>".optionlist($Rc,$W,true)."</select>":"<input value='".h($W)."'$c class='hidden'><input value='".h($Rc)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($Q)."&field=".urlencode($l["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");if(like_bool($l))return'<input type="checkbox" value="'.h($W?$W:1).'"'.($W?' checked':'')."$c>";$Yb="";if(ereg('time',$l["type"]))$Yb='HH:MM:SS';if(ereg('date|timestamp',$l["type"]))$Yb='d.m.[rrrr]'.($Yb?" [$Yb]":"");if($Yb)return"<input value='".h($W)."'$c> ($Yb)";if(eregi('_(md5|sha1)$',$l["field"]))return"<input type='password' value='".h($W)."'$c>";return'';}function
...
xxtea_mx($Z,$Y,$P,$lc){return
int32((($Z>>5&0x7FFFFFF)^$Y<<2)+(($Y>>3&0x1FFFFFFF)^$Z<<4))^int32(($P^$Y)+($lc^$Z));}function
encrypt_string($Dd,$u){if($Dd=="")return"";$u=array_values(unpack("V*",pack("H*",md5($u))));$U=str2long($Dd,true);$_=count($U)-1;$Z=$U[$_];$Y=$U[0];$gd=floor(6+52/($_+1));$P=0;while($gd-->0){$P=int32($P+0x9E3779B9);$gb=$P>>2&3;for($C=0;$C<$_;$C++){$Y=$U[$C+1];$Fc=xxtea_mx($Z,$Y,$P,$u[$C&3^$gb]);$Z=int32($U[$C]+$Fc);$U[$C]=$Z;}$Y=$U[0];$Fc=xxtea_mx($Z,$Y,$P,$u[$C&3^$gb]);$Z=int32($U[$_]+$Fc);$U[$_]=$Z;}return
long2str($U,false);}function
decrypt_string($Dd,$u){if($Dd=="")return"";$u=array_values(unpack("V*",pack("H*",md5($u))));$U=str2long($Dd,false);$_=count($U)-1;$Z=$U[$_];$Y=$U[0];$gd=floor(6+52/($_+1));$P=int32($gd*0x9E3779B9);while($P){$gb=$P>>2&3;for($C=$_;$C>0;$C--){$Z=$U[$C-1];$Fc=xxtea_mx($Z,$Y,$P,$u[$C&3^$gb]);$Y=int32($U[$C]-$Fc);$U[$C]=$Y;}$Z=$U[$_];$Fc=xxtea_mx($Z,$Y,$P,$u[$C&3^$gb]);$Y=int32($U[0]-$Fc);$U[0]=$Y;$P=int32($P-0x9E3779B9);}return
long2str($U,true);}$g='';$S=$_SESSION["token"];if(!$_SESSION["token"])$_SESSION["token"]=rand(1,1e6);$Yc=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$V){list($u)=explode(":",$V);$Yc[$u]=$V;}}$d=$_POST["auth"];if($d){session_regenerate_id();$_SESSION["pwds"][$d["driver"]][$d["server"]][$d["username"]]=$d["password"];$_SESSION["db"][$d["driver"]][$d["server"]][$d["username"]][$d["db"]]=true;if($d["permanent"]){$u=base64_encode($d["driver"])."-".base64_encode($d["server"])."-".base64_encode($d["username"])."-".base64_encode($d["db"]);$dd=$b->permanentLogin();$Yc[$u]="$u:".base64_encode($dd?encrypt_string($d["password"],$dd):"");cookie("adminer_permanent",implode(" ",$Yc));}if(count($_POST)==1||DRIVER!=$d["driver"]||SERVER!=$d["server"]||$_GET["username"]!==$d["username"]||DB!=$d["db"])redirect(auth_url($d["driver"],$d["server"],$d["username"],$d["db"]));}elseif($_POST["logout"]){if($S&&$_POST["token"]!=$S){page_header('OdhlÃ¡siÅ¥','NeplatnÃ½ token CSRF. OdoÅ¡lite formulÃ¡r znova.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$u)set_session($u,null);unset_permanent();redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡senie prebehlo v poriadku.');}}elseif($Yc&&!$_SESSION["pwds"]){session_regenerate_id();$dd=$b->permanentLogin();foreach($Yc
as$u=>$V){list(,$Ca)=explode(":",$V);list($db,$M,$T,$i)=array_map('base64_decode',explode("-",$u));$_SESSION["pwds"][$db][$M][$T]=decrypt_string(base64_decode($Ca),$dd);$_SESSION["db"][$db][$M][$T][$i]=true;}}function
unset_permanent(){global$Yc;foreach($Yc
as$u=>$V){list($db,$M,$T,$i)=array_map('base64_decode',explode("-",$u));if($db==DRIVER&&$M==SERVER&&$T==$_GET["username"]&&$i==DB)unset($Yc[$u]);}cookie("adminer_permanent",implode(" ",$Yc));}function
auth_error($ub=null){global$g,$b,$S;$zd=session_name();$k="";if(!$_COOKIE[$zd]&&$_GET[$zd]&&ini_bool("session.use_only_cookies"))$k='Session premennÃ© musia byÅ¥ povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$zd]||$_GET[$zd])&&!$S)$k='Session vyprÅ¡ala, prihlÃ¡ste sa prosÃ­m znova.';else{$E=&get_session("pwds");if($E!==null){$k=h($ub?$ub->getMessage():(is_string($g)?$g:'NeplatnÃ© prihlasovacie Ãºdaje.'));$E=null;}unset_permanent();}}page_header('PrihlÃ¡siÅ¥ sa',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½iadne rozÅ¡Ã­renie',sprintf('Nie je dostupnÃ© Å¾iadne z podporovanÃ½ch rozÅ¡Ã­renÃ­ (%s).',implode(", ",$bd)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$S=$_SESSION["token"];if($d&&$_POST["token"])$_POST["token"]=$S;$k='';if($_POST){if($_POST["token"]!=$S){$gc="max_input_vars";$Ac=ini_get($gc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$u){$V=ini_get($u);if($V&&(!$Ac||$V<$Ac)){$gc=$u;$Ac=$V;}}}$k=(!$_POST["token"]&&$Ac?sprintf('Bol prekroÄenÃ½ maximÃ¡lny poÄet povolenÃ½ch polÃ­. ZvÃ½Å¡te prosÃ­m %s.',"'$gc'"):'NeplatnÃ½ token CSRF. OdoÅ¡lite formulÃ¡r znova.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST")$k=sprintf('PrÃ­liÅ¡ veÄ¾kÃ© POST dÃ¡ta. ZmenÅ¡ite dÃ¡ta alebo zvÃ½Å¡te hodnotu konfiguraÄej direktÃ­vy %s.',"'post_max_size'");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}$g->select_db($b->database());function
email_header($Wb){return"=?UTF-8?B?".base64_encode($Wb)."?=";}function
send_mail($kb,$Gd,$z,$Pb="",$Db=array()){$j=(strncasecmp(PHP_OS,"win",3)?"\n":"\r\n");$z=str_replace("\n",$j,wordwrap(str_replace("\r","","$z\n")));$za=uniqid("boundary");$oa="";foreach((array)$Db["error"]as$u=>$V){if(!$V)$oa.="--$za$j"."Content-Type: ".str_replace("\n","",$Db["type"][$u]).$j."Content-Disposition: attachment; filename=\"".preg_replace('~["\\n]~','',$Db["name"][$u])."\"$j"."Content-Transfer-Encoding: base64$j$j".chunk_split(base64_encode(file_get_contents($Db["tmp_name"][$u])),76,$j).$j;}$va="";$Xb="Content-Type: text/plain; charset=utf-8$j"."Content-Transfer-Encoding: 8bit";if($oa){$oa.="--$za--$j";$va="--$za$j$Xb$j$j";$Xb="Content-Type: multipart/mixed; boundary=\"$za\"";}$Xb.=$j."MIME-Version: 1.0$j"."X-Mailer: Adminer Editor".($Pb?$j."From: ".str_replace("\n","",$Pb):"");return
mail($kb,email_header($Gd),$va.$z.$oa,$Xb);}function
like_bool($l){return
ereg("bool|(tinyint|bit)\\(1\\)",$l["full_type"]);}$Lc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$eb[DRIVER]='PrihlÃ¡siÅ¥ sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$m=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$m),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$m=fields($a);$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$m):""):where($_GET,$m));$je=(isset($_GET["select"])?$_POST["edit"]:$X);foreach($m
as$A=>$l){if(!isset($l["privileges"][$je?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($je?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);$s=indexes($a);$ee=unique_array($_GET["where"],$s);$id="\nWHERE $X";if(isset($_POST["delete"])){$G="FROM ".table($a);query_redirect("DELETE".($ee?" $G$id":limit1($G,$id)),$x,'PoloÅ¾ka bola vymazanÃ¡.');}else{$N=array();foreach($m
as$A=>$l){$V=process_input($l);if($V!==false&&$V!==null)$N[idf_escape($A)]=($je?"\n".idf_escape($A)." = $V":$V);}if($je){if(!$N)redirect($x);$G=table($a)." SET".implode(",",$N);query_redirect("UPDATE".($ee?" $G$id":limit1($G,$id)),$x,'PoloÅ¾ka bola aktualizovanÃ¡.');}else{$H=insert_into($a,$N);$rc=($H?last_id():0);queries_redirect($x,sprintf('PoloÅ¾ka%s bola vloÅ¾enÃ¡.',($rc?" $rc":"")),$H);}}}$Kd=$b->tableName(table_status($a,true));page_header(($je?'UpraviÅ¥':'VloÅ¾iÅ¥'),$k,array("select"=>array($a,$Kd)),$Kd);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($m
as$A=>$l){if(isset($l["privileges"]["select"])){$ma=convert_field($l);if($_POST["clone"]&&$l["auto_increment"])$ma="''";if($t=="sql"&&ereg("enum|set",$l["type"]))$ma="1*".idf_escape($A);$L[]=($ma?"$ma AS ":"").idf_escape($A);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false)echo"<p class='error'>".'Å½iadne riadky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$m)echo"<p class='error'>".'You have no privileges to update this table.'."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($m
as$A=>$l){echo"<tr><th>".$b->fieldName($l);$Wa=$_GET["set"][bracket_escape($A)];if($Wa===null){$Wa=$l["default"];if($l["type"]=="bit"&&ereg("^b'([01]*)'\$",$Wa,$ld))$Wa=$ld[1];}$W=($J!==null?($J[$A]!=""&&$t=="sql"&&ereg("enum|set",$l["type"])?(is_array($J[$A])?array_sum($J[$A]):+$J[$A]):$J[$A]):(!$je&&$l["auto_increment"]?"":(isset($_GET["select"])?false:$Wa)));if(!$_POST["save"]&&is_string($W))$W=$b->editVal($W,$l);$o=($_POST["save"]?(string)$_POST["function"][$A]:($je&&$l["on_update"]=="CURRENT_TIMESTAMP"?"now":($W===false?null:($W!==null?'':'NULL'))));if(ereg("time",$l["type"])&&$W=="CURRENT_TIMESTAMP"){$W="";$o="now";}input($l,$W,$o);echo"\n";}echo"</table>\n";}echo'<p>
';if($m){echo"<input type='submit' value='".'UloÅ¾iÅ¥'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($je?'UloÅ¾iÅ¥ a pokraÄovaÅ¥ v ÃºpravÃ¡ch':'UloÅ¾iÅ¥ a vloÅ¾iÅ¥ ÄalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($je?"<input type='submit' name='delete' value='".'ZmazaÅ¥'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$S,'">
</form>
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$R=table_status($a);$s=indexes($a);$m=fields($a);$Lb=column_foreign_keys($a);$Kc="";if($R["Oid"]=="t"){$Kc=($t=="sqlite"?"rowid":"oid");$s[]=array("type"=>"PRIMARY","columns"=>array($Kc));}parse_str($_COOKIE["adminer_import"],$ha);$rd=array();$f=array();$Od=null;foreach($m
as$u=>$l){$A=$b->fieldName($l);if(isset($l["privileges"]["select"])&&$A!=""){$f[$u]=html_entity_decode(strip_tags($A),ENT_QUOTES);if(is_shortable($l))$Od=$b->selectLengthProcess();}$rd+=$l["privileges"];}list($L,$Sb)=$b->selectColumnsProcess($f,$s);$jc=count($Sb)<count($L);$X=$b->selectSearchProcess($m,$s);$B=$b->selectOrderProcess($m,$s);$v=$b->selectLimitProcess();$Pb=($L?implode(", ",$L):"*".($Kc?", $Kc":"")).convert_fields($f,$m,$L)."\nFROM ".table($a);$Tb=($Sb&&$jc?"\nGROUP BY ".implode(", ",$Sb):"").($B?"\nORDER BY ".implode(", ",$B):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$fe=>$J){$ma=convert_field($m[key($J)]);echo$g->result("SELECT".limit($ma?$ma:idf_escape(key($J))." FROM ".table($a)," WHERE ".where_check($fe,$m).($X?" AND ".implode(" AND ",$X):"").($B?" ORDER BY ".implode(", ",$B):""),1));}exit;}if($_POST&&!$k){$re=$X;if(is_array($_POST["check"]))$re[]="((".implode(") OR (",array_map('where_check',$_POST["check"]))."))";$re=($re?"\nWHERE ".implode(" AND ",$re):"");$cd=$he=null;foreach($s
as$r){if($r["type"]=="PRIMARY"){$cd=array_flip($r["columns"]);$he=($L?$cd:array());break;}}foreach((array)$he
as$u=>$V){if(in_array(idf_escape($u),$L))unset($he[$u]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$he===array())$G="SELECT $Pb$re$Tb";else{$de=array();foreach($_POST["check"]as$V)$de[]="(SELECT".limit($Pb,"\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V,$m).$Tb,1).")";$G=implode(" UNION ALL ",$de);}$b->dumpData($a,"table",$G);exit;}if(!$b->selectEmailProcess($X,$Lb)){if($_POST["save"]||$_POST["delete"]){$H=true;$ia=0;$G=table($a);$N=array();if(!$_POST["delete"]){foreach($f
as$A=>$V){$V=process_input($m[$A]);if($V!==null){if($_POST["clone"])$N[idf_escape($A)]=($V!==false?$V:idf_escape($A));elseif($V!==false)$N[]=idf_escape($A)." = $V";}}$G.=($_POST["clone"]?" (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a):" SET\n".implode(",\n",$N));}if($_POST["delete"]||$N){$Ja="UPDATE";if($_POST["delete"]){$Ja="DELETE";$G="FROM $G";}if($_POST["clone"]){$Ja="INSERT";$G="INTO $G";}if($_POST["all"]||($he===array()&&$_POST["check"])||$jc){$H=queries("$Ja $G$re");$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$H=queries($Ja.limit1($G,"\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V,$m)));if(!$H)break;$ia+=$g->affected_rows;}}}$z=sprintf('%d poloÅ¾iek bolo ovplyvnenÃ½ch.',$ia);if($_POST["clone"]&&$H&&$ia==1){$rc=last_id();if($rc)$z=sprintf('PoloÅ¾ka%s bola vloÅ¾enÃ¡.'," $rc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$z,$H);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+kliknite na polÃ­Äko, ktorÃ© chcete zmeniÅ¥.';else{$H=true;$ia=0;foreach($_POST["val"]as$fe=>$J){$N=array();foreach($J
as$u=>$V){$u=bracket_escape($u,1);$N[]=idf_escape($u)." = ".(ereg('char|text',$m[$u]["type"])||$V!=""?$b->processInput($m[$u],$V):"NULL");}$G=table($a)." SET ".implode(", ",$N);$qe=" WHERE ".where_check($fe,$m).($X?" AND ".implode(" AND ",$X):"");$H=queries("UPDATE".($jc?" $G$qe":limit1($G,$qe)));if(!$H)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d poloÅ¾iek bolo ovplyvnenÃ½ch.',$ia),$H);}}elseif(is_string($Bb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$H=true;$Ia=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Bb,$xc);$ia=count($xc[0]);begin();$xd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($xc[0]as$u=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$xd]*)$xd~",$V.$xd,$yc);if(!$u&&!array_diff($yc[1],$Ia)){$Ia=$yc[1];$ia--;}else{$N=array();foreach($yc[1]as$p=>$Fa)$N[idf_escape($Ia[$p])]=($Fa==""&&$m[$Ia[$p]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Fa))));$H=insert_update($a,$N,$cd);if(!$H)break;}}if($H)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovanÃ½ %d zÃ¡znam.','Boli importovanÃ© %d zÃ¡znamy.','Bolo importovanÃ½ch %d zÃ¡znamov.'),$ia),$H);queries("ROLLBACK");}else$k=upload_error($Bb);}}$Kd=$b->tableName($R);if(is_ajax())ob_start();page_header('VypÃ­saÅ¥'.": $Kd",$k);$N=null;if(isset($rd["insert"])){$N="";foreach((array)$_GET["where"]as$V){if(count($Lb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$N.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($R,$N);if(!$f)echo"<p class='error'>".'TabuÄ¾ku sa nepodarilo vypÃ­saÅ¥'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($X,$f,$s);$b->selectOrderPrint($B,$f,$s);$b->selectLimitPrint($v);$b->selectLengthPrint($Od);$b->selectActionPrint($s);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$n=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$D=floor(max(0,$n-1)/$v);}$G=$b->selectQueryBuild($L,$X,$Sb,$B,$v,$D);if(!$G)$G="SELECT".limit((+$v&&$Sb&&$jc&&$t=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Pb,($X?"\nWHERE ".implode(" AND ",$X):"").$Tb,($v!=""?+$v:null),($D?$v*$D:0),"\n");echo$b->selectQuery($G);$H=$g->query($G);if(!$H)echo"<p class='error'>".error()."\n";else{if($t=="mssql"&&$D)$H->seek($v*$D);$mb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($D&&$t=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last")$n=(+$v&&$Sb&&$jc?($t=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($G) x")):count($K));if(!$K)echo"<p class='message'>".'Å½iadne riadky.'."\n";else{$ua=$b->backwardKeys($a,$Kd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Sb&&$L?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviÅ¥'."</a>");$Gc=array();$Rb=array();reset($L);$jd=1;foreach($K[0]as$u=>$V){if($u!=$Kc){$V=$_GET["columns"][key($L)];$l=$m[$L?($V?$V["col"]:current($L)):$u];$A=($l?$b->fieldName($l,$jd):"*");if($A!=""){$jd++;$Gc[$u]=$A;$e=idf_escape($u);$ac=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Xa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($ac.($B[0]==$e||$B[0]==$u||(!$B&&$jc&&$Sb[0]==$e)?$Xa:'')).'">';echo(!$L||$V?apply_sql_function($V["fun"],$A):h(current($L)))."</a>";echo"<span class='column hidden'>","<a href='".h($ac.$Xa)."' title='".'zostupne'."' class='text'> â†“</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'VyhÄ¾adaÅ¥'.'" class="text jsonly"> =</a>';echo"</span>";}$Rb[$u]=$V["fun"];next($L);}}$uc=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
as$u=>$V)$uc[$u]=max($uc[$u],min(40,strlen(utf8_decode($V))));}}echo($ua?"<th>".'VzÅ¥ahy':"")."</thead>\n";if(is_ajax()){if($v%2==1&&$D%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($K,$Lb)as$_=>$J){$ee=unique_array($K[$_],$s);if(!$ee){$ee=array();foreach($K[$_]as$u=>$V){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$u))$ee[$u]=$V;}}$fe="";foreach($ee
as$u=>$V){if(strlen($V)>64){$u="MD5(".(strpos($u,'(')?$u:idf_escape($u)).")";$V=md5($V);}$fe.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));}echo"<tr".odd().">".(!$Sb&&$L?"":"<td>".checkbox("check[]",substr($fe,1),in_array(substr($fe,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($jc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$fe)."'>".'upraviÅ¥'."</a>"));foreach($J
as$u=>$V){if(isset($Gc[$u])){$l=$m[$u];if($V!=""&&(!isset($mb[$u])||$mb[$u]!=""))$mb[$u]=(is_mail($V)?$Gc[$u]:"");$w="";$V=$b->editVal($V,$l);if($V!==null){if(ereg('blob|bytea|raw|file',$l["type"])&&$V!="")$w=ME.'download='.urlencode($a).'&field='.urlencode($u).$fe;if($V==="")$V="&nbsp;";elseif($Od!=""&&is_shortable($l))$V=shorten_utf8($V,max(0,+$Od));else$V=h($V);if(!$w){foreach((array)$Lb[$u]as$Kb){if(count($Lb[$u])==1||end($Kb["source"])==$u){$w="";foreach($Kb["source"]as$p=>$_d)$w.=where_link($p,$Kb["target"][$p],$K[$_][$_d]);$w=($Kb["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($Kb["db"]),ME):ME).'select='.urlencode($Kb["table"]).$w;if(count($Kb["source"])==1)break;}}}if($u=="COUNT(*)"){$w=ME."select=".urlencode($a);$p=0;foreach((array)$_GET["where"]as$U){if(!array_key_exists($U["col"],$ee))$w.=where_link($p++,$U["col"],$U["val"],$U["op"]);}foreach($ee
as$lc=>$U)$w.=where_link($p++,$lc,$U);}}if(!$w&&($w=$b->selectLink($J[$u],$l))===null){if(is_mail($J[$u]))$w="mailto:$J[$u]";if($fd=is_url($J[$u]))$w=($fd=="http"&&$aa?$J[$u]:"$fd://www.adminer.org/redirect/?url=".urlencode($J[$u]));}$q=h("val[$fe][".bracket_escape($u)."]");$W=$_POST["val"][$fe][bracket_escape($u)];$Vb=h($W!==null?$W:$J[$u]);$wc=strpos($V,"<i>...</i>");$ib=is_utf8($V)&&$K[$_][$u]==$J[$u]&&!$Rb[$u];$Nd=ereg('text|lob',$l["type"]);echo(($_GET["modify"]&&$ib)||$W!==null?"<td>".($Nd?"<textarea name='$q' cols='30' rows='".(substr_count($J[$u],"\n")+1)."'>$Vb</textarea>":"<input name='$q' value='$Vb' size='$uc[$u]'>"):"<td id='$q' onclick=\"selectClick(this, event, ".($wc?2:($Nd?1:0)).($ib?"":", '".h('Pre zmenu tejto hodnoty pouÅ¾ite odkaz upraviÅ¥.')."'").");\">".$b->selectVal($V,$w,$l));}}if($ua)echo"<td>";$b->backwardKeysPrint($ua,$K[$_]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n",(!$Sb&&$L?"":"<script type='text/javascript'>tableCheck();</script>\n");}if(($K||$D)&&!is_ajax()){$tb=true;if($_GET["page"]!="last"&&+$v&&!$jc&&($n>=$v||$D)){$n=found_rows($R,$X);if($n<max(1e4,2*($D+1)*$v))$n=reset(slow_query("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):"")));else$tb=false;}if(+$v&&($n===false||$n>$v||$D)){echo"<p class='pages'>";$zc=($n===false?$D+(count($K)>=$v?2:1):floor(($n-1)/$v));echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'StrÃ¡nka'."', '".($D+1)."'), event); return false;\">".'StrÃ¡nka'."</a>:",pagination(0,$D).($D>5?" ...":"");for($p=max(1,$D-4);$p<min($zc,$D+5);$p++)echo
pagination($p,$D);if($zc>0){echo($D+5<$zc?" ...":""),($tb&&$n!==false?pagination($zc,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$zc'>".'poslednÃ½'."</a>");}echo(($n===false?count($K)+1:$n-$D*$v)>$v?' <a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" onclick="return !selectLoadMore(this, '.(+$v).', \''.'Loading'.'\');">'.'Load more data'.'</a>':'');}echo"<p>\n",($n!==false?"(".($tb?"":"~ ").lang(array('%d riadok','%d riadky','%d riadkov'),$n).") ":""),checkbox("all",1,0,'celÃ½ vÃ½sledok')."\n";if($b->selectCommandPrint()){echo'<fieldset><legend>UpraviÅ¥</legend><div>
...
<input type="submit" name="delete" value="ZmazaÅ¥" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$n,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
