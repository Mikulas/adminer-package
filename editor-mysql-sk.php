...
upload_error($w){$zc=($w==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($w?'Súbor sa nepodarilo nahrať.'.($zc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$zc):""):'Súbor neexistuje.');}function
...
input($e,$i,$y){global$Ea,$m,$Ra;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$U=(isset($_GET["select"])?array("orig"=>'originál'):array())+$m->editFunctions($e);$P=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($Sc,$Va){$Rc=($Va==1||(!$Va&&'sk'=='fr')?0:((!$Va||$Va>=5)&&ereg('cs|sk|ru','sk')?2:1));return
...
selectSearchPrint($p,$_,$E){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$yb=array();foreach($p
...
as$c=>$R){$Y=array();foreach($R["columns"]as$a){$Y[]=$this->fieldName(array("field"=>$a,"comment"=>$_[$a]));}if(count(array_filter($Y,'strlen'))>1&&$c!="PRIMARY"){$tb[$c]=implode(", ",$Y);}}if($tb){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$tb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
';if($jb!="db"){$G=table_status();if(!$G){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($G);}}}}function
...
auth_error($mc=null){global$g,$m,$ga;$mb=session_name();$w="";if(!$_COOKIE[$mb]&&$_GET[$mb]&&ini_bool("session.use_only_cookies")){$w='Session premenné musia byť povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$mb]||$_GET[$mb])&&!$ga){$w='Session vypršala, prihláste sa prosím znova.';}else{$M=&get_session("pwds");if(isset($M)){$w=h($mc?$mc->getMessage():(is_string($g)?$g:'Neplatné prihlasovacie údaje.'));$M=null;}}}page_header('Prihlásiť sa',$w,null);echo"<form action='' method='post'>\n";$m->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])&&class_exists("Min_DB")){$g=connect();}if(is_string($g)||!$m->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$ga=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$ga;}$w=($_POST?($_POST["token"]==$ga?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));$g->select_db($m->database());function
...
dump_headers($bc){$qa=($bc!=""?friendly_url($bc):"dump");$jc="csv";header("Content-Type: text/csv; charset=utf-8");header("Content-Disposition: attachment; filename=$qa.$jc");session_write_close();return$jc;}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Sb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$Nc=" onclick=\"return confirm('".'Naozaj?'."');\"";$ta[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$q=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$q-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($q)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$q=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$da=(isset($_GET["select"])?$_POST["edit"]:$p);$r=fields($q);foreach($r
as$f=>$e){if(!isset($e["privileges"][$da?"update":"insert"])||$m->fieldName($e)==""){unset($r[$f]);}}if($_POST&&!$w&&!isset($_GET["select"])){$K=$_POST["referer"];if($_POST["insert"]){$K=($da?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$K)){$K=ME."select=".urlencode($q);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($q)," WHERE $p"),$K,'Položka bola vymazaná.');}else{$s=array();foreach($r
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($da?"\n".idf_escape($f)." = $a":$a);}}if($da){if(!$s){redirect($K);}query_redirect("UPDATE".limit1(table($q)." SET".implode(",",$s),"\nWHERE $p"),$K,'Položka bola aktualizovaná.');}else{$h=insert_into($q,$s);$gc=($h?last_id():0);queries_redirect($K,sprintf('Položka%s bola vložená.',($gc?" $gc":"")),$h);}}}$Ua=$m->tableName(table_status($q));page_header(($da?'Upraviť':'Vložiť'),$w,array("select"=>array($q,$Ua)),$Ua);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$u=array();foreach($r
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$aa,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
