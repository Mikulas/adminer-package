...
upload_error($i){$mc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($i?'Súbor sa nepodarilo nahrať.'.($mc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$mc):""):'Súbor neexistuje.');}function
...
input($j,$W,$m){global$Hd,$b,$s;$A=h(bracket_escape($j["field"]));echo"<td class='function'>";$Yc=($s=="mssql"&&$j["auto_increment"]);if($Yc&&!$_POST["save"])$m=null;$Hb=(isset($_GET["select"])||$Yc?array("orig"=>'originál'):array())+$b->editFunctions($j);$c=" name='fields[$A]'";if($j["type"]=="enum")echo
...
selectVal($V,$v,$j){$I=($V===null?"&nbsp;":$V);if(ereg('blob|bytea',$j["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$I="<img src='$v' alt='$I'>";}if(like_bool($j)&&$I!="&nbsp;")$I=($V?'yes':'no');if($v)$I="<a href='$v'>$I</a>";if(!$v&&!like_bool($j)&&ereg('int|float|double|decimal',$j["type"]))$I="<div class='number'>$I</div>";elseif(ereg('date',$j["type"]))$I="<div class='datetime'>$I</div>";return$I;}function
...
as$A=>$Ta){$j=$k[$A];if(ereg("enum",$j["type"])||like_bool($j)){$t=$ac[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'>:",(like_bool($j)?" <select name='where[$o][val]'>".optionlist(array(""=>"",'no','yes'),$X[$t]["val"],true)."</select>":enum_input("checkbox"," name='where[$o][val][]'",$j,(array)$X[$t]["val"],($j["null"]?0:null))),"</div>\n";unset($f[$A]);}elseif(is_array($Cc=$this->_foreignKeyOptions($_GET["select"],$A))){if($k[$A]["null"])$Cc[0]='('.'prázdne'.')';$t=$ac[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($Cc,$X[$t]["val"],true)."</select></div>\n";unset($f[$A]);}}$o=0;foreach($X
...
as$t=>$q){$B=array();foreach($q["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$t!="PRIMARY")$Ec[$t]=implode(", ",$B);}if($Ec){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Ec,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
editFunctions($j){$I=array();if($j["null"]&&ereg('blob',$j["type"]))$I["NULL"]='prázdne';$I[""]=($j["null"]||$j["auto_increment"]||like_bool($j)?"":"*");if(ereg('date|time',$j["type"]))$I["now"]='teraz';if(eregi('_(md5|sha1)$',$j["field"],$x))$I[]=strtolower($x[1]);return$I;}function
...
';if($pc!="db"&&$pc!="ns"){$R=table_status();if(!$R)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else$this->tablesPrint($R);}}}function
...
like_bool($j){return
ereg("bool|(tinyint|bit)\\(1\\)",$j["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();$xc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$Ya[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Pd=(isset($_GET["select"])?$_POST["edit"]:$X);$k=fields($a);foreach($k
as$A=>$j){if(!isset($j["privileges"][$Pd?"update":"insert"])||$b->fieldName($j)=="")unset($k[$A]);}if($_POST&&!$i&&!isset($_GET["select"])){$w=$_POST["referer"];if($_POST["insert"])$w=($Pd?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$w))$w=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$w,'Položka bola vymazaná.');else{$N=array();foreach($k
as$A=>$j){$V=process_input($j);if($V!==false&&$V!==null)$N[idf_escape($A)]=($Pd?"\n".idf_escape($A)." = $V":$V);}if($Pd){if(!$N)redirect($w);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$w,'Položka bola aktualizovaná.');}else{$H=insert_into($a,$N);$ec=($H?last_id():0);queries_redirect($w,sprintf('Položka%s bola vložená.',($ec?" $ec":"")),$H);}}}$td=$b->tableName(table_status($a));page_header(($Pd?'Upraviť':'Vložiť'),$i,array("select"=>array($a,$td)),$td);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($k
