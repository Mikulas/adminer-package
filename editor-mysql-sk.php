...
confirm($Ja="",$jd=false){return" onclick=\"".($jd?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Ja?" (' + $Ja + ')":"")."');\"";}function
...
upload_error($h){$kc=($h==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($h?'Súbor sa nepodarilo nahrať.'.($kc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$kc):""):'Súbor neexistuje.');}function
...
enum_input($Dd,$c,$i,$W,$gb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$i["length"],$y);$I=(isset($gb)?"<label><input type='$Dd'$c value='$gb'".((is_array($W)?in_array($gb,$W):$W===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($y[1]as$o=>$V){$V=stripcslashes(str_replace("''","'",$V));$za=(is_int($W)?$W==$o+1:(is_array($W)?in_array($o+1,$W):$W===$V));$I.=" <label><input type='$Dd'$c value='".($o+1)."'".($za?' checked':'').'>'.h($b->editVal($V,$i)).'</label>';}return$I;}function
input($i,$W,$l){global$Fd,$b,$Xb;$A=h(bracket_escape($i["field"]));echo"<td class='function'>";$Vc=($Xb=="mssql"&&$i["auto_increment"]);if($Vc&&!$_POST["save"]){$l=null;}$m=(isset($_GET["select"])||$Vc?array("orig"=>'originál'):array())+$b->editFunctions($i);$c=" name='fields[$A]'";if($i["type"]=="enum"){echo
...
lang($Bd,$sc){$Ic=($sc==1?0:($sc&&$sc<5?1:2));return
...
selectLinks($qd,$N=""){$a=$qd["Name"];if(isset($N)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Nová položka'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a>\n";}function
...
as$d=>$V){$v.="&set".urlencode("[".bracket_escape($d)."]")."=".urlencode($J[$V]);}echo"<a href='".h($v)."' title='".'Nová položka'."'>+</a> ";}}}function
...
selectVal($V,$v,$i){$I=($V=="<i>NULL</i>"?"&nbsp;":$V);if(ereg('blob|bytea',$i["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V)){$I="<img src='$v' alt='$I'>";}}if(like_bool($i)&&$I!="&nbsp;"){$I=($V?'yes':'no');}if($v){$I="<a href='$v'>$I</a>";}if(!$v&&!like_bool($i)&&ereg('int|float|double|decimal',$i["type"])){$I="<div class='number'>$I</div>";}elseif(ereg('date',$i["type"])){$I="<div class='datetime'>$I</div>";}return$I;}function
...
selectSearchPrint($X,$e,$r){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$Yb=array();foreach($X
as$t=>$V){$Yb[$V["col"]]=$t;}$o=0;$j=fields($_GET["select"]);foreach($e
as$A=>$Ta){$i=$j[$A];if(ereg("enum",$i["type"])||like_bool($i)){$t=$Yb[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'>:",(like_bool($i)?" <select name='where[$o][val]'>".optionlist(array(""=>"",'no','yes'),$X[$t]["val"],true)."</select>":enum_input("checkbox"," name='where[$o][val][]'",$i,(array)$X[$t]["val"],($i["null"]?0:null))),"</div>\n";unset($e[$A]);}elseif(is_array($_c=$this->_foreignKeyOptions($_GET["select"],$A))){if($j[$A]["null"]){$_c[0]='('.'prázdne'.')';}$t=$Yb[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($_c,$X[$t]["val"],true)."</select></div>\n";unset($e[$A]);}}$o=0;foreach($X
as$V){if(($V["col"]==""||$e[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'kdekoľvek'.")".optionlist($e,$V["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$o][val]' value='".h($V["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoľvek'.")".optionlist($e,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$e,$r){$Bc=array();foreach($r
as$t=>$q){$B=array();foreach($q["columns"]as$V){$B[]=$e[$V];}if(count(array_filter($B,'strlen'))>1&&$t!="PRIMARY"){$Bc[$t]=implode(", ",$B);}}if($Bc){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Bc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
true;}function
selectEmailPrint($eb,$e){if($eb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Odosielateľ'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Predmet'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n";echo"<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$e,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložiť'."'>\n";echo"<p>".'Prílohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($eb)==1?'<input type="hidden" name="email_field" value="'.h(key($eb)).'">':html_select("email_field",$eb)),"<input type='submit' name='email' value='".'Odoslať'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
...
auth_error($mb=null){global$f,$b,$_d;$ed=session_name();$h="";if(!$_COOKIE[$ed]&&$_GET[$ed]&&ini_bool("session.use_only_cookies")){$h='Session premenné musia byť povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$ed]||$_GET[$ed])&&!$_d){$h='Session vypršala, prihláste sa prosím znova.';}else{$E=&get_session("pwds");if(isset($E)){$h=h($mb?$mb->getMessage():(is_string($f)?$f:'Neplatné prihlasovacie údaje.'));$E=null;}}}page_header('Prihlásiť sa',$h,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$Jc)),false);page_footer("auth");exit;}$f=connect();}if(is_string($f)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$_d=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$_d;}$h=($_POST?($_POST["token"]==$_d?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));$f->select_db($b->database());function
...
like_bool($i){return
ereg("bool|(tinyint|bit)\\(1\\)",$i["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$uc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$Ya[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$f->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Nd=(isset($_GET["select"])?$_POST["edit"]:$X);$j=fields($a);foreach($j
as$A=>$i){if(!isset($i["privileges"][$Nd?"update":"insert"])||$b->fieldName($i)==""){unset($j[$A]);}}if($_POST&&!$h&&!isset($_GET["select"])){$w=$_POST["referer"];if($_POST["insert"]){$w=($Nd?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$w)){$w=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$w,'Položka bola vymazaná.');}else{$N=array();foreach($j
as$A=>$i){$V=process_input($i);if($V!==false&&$V!==null){$N[idf_escape($A)]=($Nd?"\n".idf_escape($A)." = $V":$V);}}if($Nd){if(!$N){redirect($w);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$w,'Položka bola aktualizovaná.');}else{$H=insert_into($a,$N);$cc=($H?last_id():0);queries_redirect($w,sprintf('Položka%s bola vložená.',($cc?" $cc":"")),$H);}}}$rd=$b->tableName(table_status($a));page_header(($Nd?'Upraviť':'Vložiť'),$h,array("select"=>array($a,$rd)),$rd);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($X){$L=array();foreach($j
as$A=>$i){if(isset($i["privileges"]["select"])){$L[]=($_POST["clone"]&&$i["auto_increment"]?"'' AS ":(ereg("enum|set",$i["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Žiadne riadky.'."\n";}echo'
...
';if($j){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Nd?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($Nd?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$j?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$Eb,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
