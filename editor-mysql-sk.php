...
remove_slashes($Yc,$Bb=false){if(get_magic_quotes_gpc()){while(list($u,$V)=each($Yc)){foreach($V
as$hc=>$U){unset($Yc[$u][$hc]);if(is_array($U)){$Yc[$u][stripslashes($hc)]=$U;$Yc[]=&$Yc[$u][stripslashes($hc)];}else$Yc[$u][stripslashes($hc)]=($Bb?$U:stripslashes($U));}}}}function
...
nl_br($P){return
str_replace("\n","<br>",$P);}function
checkbox($B,$W,$Ba,$kc="",$Hc="",$gc=false){static$q=0;$q++;$J="<input type='checkbox' name='$B' value='".h($W)."'".($Ba?" checked":"").($Hc?' onclick="'.h($Hc).'"':'').($gc?" class='jsonly'":"")." id='checkbox-$q'>";return($kc!=""?"<label for='checkbox-$q'>$J".h($kc)."</label>":$J);}function
...
confirm($Na=""){return" onclick=\"return confirm('".'Naozaj?'.($Na?" (' + $Na + ')":"")."');\"";}function
print_fieldset($q,$oc,$ge=false,$Hc=""){echo"<fieldset><legend><a href='#fieldset-$q' onclick=\"".h($Hc)."return !toggle('fieldset-$q');\">$oc</a></legend><div id='fieldset-$q'".($ge?"":" class='hidden'").">\n";}function
...
where($Aa);}function
where_link($p,$e,$W,$Jc="="){return"&where%5B$p%5D%5Bcol%5D=".urlencode($e)."&where%5B$p%5D%5Bop%5D=".urlencode(($W!==null?$Jc:"IS NULL"))."&where%5B$p%5D%5Bval%5D=".urlencode($W);}function
...
get_file($u,$Ua=false){$zb=$_FILES[$u];if(!$zb||$zb["error"])return$zb["error"];$J=file_get_contents($Ua&&ereg('\\.gz$',$zb["name"])?"compress.zlib://$zb[tmp_name]":($Ua&&ereg('\\.bz2$',$zb["name"])?"compress.bzip2://$zb[tmp_name]":$zb["tmp_name"]));if($Ua){$vd=substr($J,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$vd,$ed))$J=iconv("utf-16","utf-8",$J);elseif($vd=="\xEF\xBB\xBF")$J=substr($J,3);}return$J;}function
upload_error($k){$vc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Súbor sa nepodarilo nahrať.'.($vc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$vc):""):'Súbor neexistuje.');}function
...
hidden_fields($Yc,$ac=array()){while(list($u,$V)=each($Yc)){if(is_array($V)){foreach($V
as$hc=>$U)$Yc[$u."[$hc]"]=$U;}elseif(!in_array($u,$ac))echo'<input type="hidden" name="'.h($u).'" value="'.h($V).'">';}}function
...
lzw_decompress($wa){$Za=256;$xa=8;$Ea=array();$id=0;$jd=0;for($p=0;$p<strlen($wa);$p++){$id=($id<<8)+ord($wa[$p]);$jd+=8;if($jd>=$xa){$jd-=$xa;$Ea[]=$id>>$jd;$id&=(1<<$jd)-1;$Za++;if($Za>>$xa)$xa++;}}$Ya=range("\0","\xFF");$J="";foreach($Ea
as$p=>$Da){$ib=$Ya[$Da];if(!isset($ib))$ib=$ke.$ke[0];$J.=$ib;if($p)$Ya[]=$ke.$ib[0];$ke=$ib;}return$J;}global$b,$g,$db,$gb,$ob,$k,$Nb,$Qb,$aa,$dc,$t,$ba,$lc,$Fc,$Sc,$yd,$Nd,$Qd,$Td,$ae,$ca;if(!$_SERVER["REQUEST_URI"])$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"];if(!strpos($_SERVER["REQUEST_URI"],'?')&&$_SERVER["QUERY_STRING"]!="")$_SERVER["REQUEST_URI"].="?$_SERVER[QUERY_STRING]";$aa=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$Qc=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$aa);if(version_compare(PHP_VERSION,'5.2.0')>=0)$Qc[]=true;call_user_func_array('session_set_cookie_params',$Qc);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$Bb);if(function_exists("set_magic_quotes_runtime"))set_magic_quotes_runtime(false);@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'sk';}function
lang($Pd,$Cc){$Uc=($Cc==1?0:($Cc&&$Cc<5?1:2));$Pd=str_replace("%d","%s",$Pd[$Uc]);$Cc=number_format($Cc,0,".",' ');return
...
alter_table($R,$B,$m,$Eb,$Ka,$nb,$Ga,$pa,$Rc){$la=array();foreach($m
as$l)$la[]=($l[1]?($R!=""?($l[0]!=""?"CHANGE ".idf_escape($l[0]):"ADD"):" ")." ".implode($l[1]).($R!=""?$l[2]:""):"DROP ".idf_escape($l[0]));$la=array_merge($la,$Eb);$wd="COMMENT=".q($Ka).($nb?" ENGINE=".q($nb):"").($Ga?" COLLATE ".q($Ga):"").($pa!=""?" AUTO_INCREMENT=$pa":"").$Rc;if($R=="")return
...
<tr><th>Používateľ<td><input type="hidden" name="auth[driver]" value="server"><input id="username" name="auth[username]" value="',h($_GET["username"]),'">
<tr><th>Heslo<td><input type="password" name="auth[password]">
</table>
<script type="text/javascript">
document.getElementById(\'username\').focus();
</script>
',"<p><input type='submit' value='".'Prihlásiť sa'."'>\n",checkbox("auth[permanent]",1,$_COOKIE["adminer_permanent"],'Trvalé prihlásenie')."\n";}function
...
selectLinks($Cd,$O=""){$a=$Cd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'Nová položka'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a>\n";}function
...
as$e=>$V)$w.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($K[$V]);echo"<a href='".h($w)."' title='".'Nová položka'."'>+</a> ";}}}function
selectQuery($H){return"<!--\n".str_replace("--","--><!-- ",$H)."\n-->\n";}function
rowDescription($R){foreach(fields($R)as$l){if(ereg("varchar|character varying",$l["type"]))return
idf_escape($l["field"]);}return"";}function
rowDescriptions($L,$Gb){$J=$L;foreach($L[0]as$u=>$V){if(list($R,$q,$B)=$this->_foreignColumn($Gb,$u)){$Zb=array();foreach($L
as$K)$Zb[$K[$u]]=exact_value($K[$u]);$Xa=$this->_values[$R];if(!$Xa)$Xa=get_key_vals("SELECT $q, $B FROM ".table($R)." WHERE $q IN (".implode(", ",$Zb).")");foreach($L
as$A=>$K){if(isset($K[$u]))$J[$A][$u]=(string)$Xa[$K[$u]];}}}return$J;}function
selectVal($V,$w,$l){$J=($V===null?"&nbsp;":$V);if(ereg('blob|bytea',$l["type"])&&!is_utf8($V)){$J=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$J="<img src='$w' alt='$J'>";}if(like_bool($l)&&$J!="&nbsp;")$J=($V?'yes':'no');if($w)$J="<a href='$w'>$J</a>";if(!$w&&!like_bool($l)&&ereg('int|float|double|decimal',$l["type"]))$J="<div class='number'>$J</div>";elseif(ereg('date',$l["type"]))$J="<div class='datetime'>$J</div>";return$J;}function
...
selectSearchPrint($X,$f,$s){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Vyhľadať'."</legend><div>\n";$ic=array();foreach($X
as$u=>$V)$ic[$V["col"]]=$u;$p=0;$m=fields($_GET["select"]);foreach($f
as$B=>$Wa){$l=$m[$B];if(ereg("enum",$l["type"])||like_bool($l)){$u=$ic[$B];$p--;echo"<div>".h($Wa)."<input type='hidden' name='where[$p][col]' value='".h($B)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'no','yes'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Lc=$this->_foreignKeyOptions($_GET["select"],$B))){if($m[$B]["null"])$Lc[0]='('.'prázdne'.')';$u=$ic[$B];$p--;echo"<div>".h($Wa)."<input type='hidden' name='where[$p][col]' value='".h($B)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Lc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'kdekoľvek'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$p][val]' value='".h($V["val"])."'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoľvek'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input name='where[$p][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($C,$f,$s){$Nc=array();foreach($s
as$u=>$r){$C=array();foreach($r["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Nc[$u]=implode(", ",$C);}if($Nc){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Nc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectActionPrint($s){echo"<fieldset><legend>".'Akcia'."</legend><div>","<input type='submit' value='".'Vypísať'."'>","</div></fieldset>\n";}function
selectCommandPrint(){return
true;}function
selectImportPrint(){return
true;}function
selectEmailPrint($kb,$f){if($kb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Odosielateľ'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Predmet'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložiť'."'>\n";echo"<p>".'Prílohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($kb)==1?'<input type="hidden" name="email_field" value="'.h(key($kb)).'">':html_select("email_field",$kb)),"<input type='submit' name='email' value='".'Odoslať'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
...
array(idf_escape(reset($_GET["order"])).($_GET["desc"]?" DESC":""));foreach(($bc!=""?array($s[$bc]):$s)as$r){if($bc!=""||$r["type"]=="INDEX"){$Wa=false;foreach($r["columns"]as$V){if(ereg('date|timestamp',$m[$V]["type"])){$Wa=true;break;}}$J=array();foreach($r["columns"]as$V)$J[]=idf_escape($V).($Wa?" DESC":"");return$J;}}return
array();}function
selectLimitProcess(){return(isset($_GET["limit"])?$_GET["limit"]:"30");}function
selectLengthProcess(){return"100";}function
selectEmailProcess($X,$Gb){if($_POST["email_append"])return
true;if($_POST["email"]){$pd=0;if($_POST["all"]||$_POST["check"]){$l=idf_escape($_POST["email_field"]);$_d=$_POST["email_subject"];$_=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$_d.$_",$z);$L=get_rows("SELECT DISTINCT $l".($z[1]?", ".implode(", ",array_map('idf_escape',array_unique($z[1]))):"")." FROM ".table($_GET["select"])." WHERE $l IS NOT NULL AND $l != ''".($X?" AND ".implode(" AND ",$X):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$m=fields($_GET["select"]);foreach($this->rowDescriptions($L,$Gb)as$K){$gd=array('{\\'=>'{');foreach($z[1]as$V)$gd['{$'."$V}"]=$this->editVal($K[$V],$m[$V]);$jb=$K[$_POST["email_field"]];if(is_mail($jb)&&send_mail($jb,strtr($_d,$gd),strtr($_,$gd),$_POST["email_from"],$_FILES["email_files"]))$pd++;}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(array('Bol odoslaný %d e-mail.','Boli odoslané %d e-maily.','Bolo odoslaných %d e-mailov.'),$pd));}return
false;}function
selectQueryBuild($M,$X,$Ob,$C,$v,$E){return"";}function
messageQuery($H){return" <span class='time'>".@date("H:i:s")."</span><!--\n".str_replace("--","--><!-- ",$H)."\n-->";}function
editFunctions($l){$J=array();if($l["null"]&&ereg('blob',$l["type"]))$J["NULL"]='prázdne';$J[""]=($l["null"]||$l["auto_increment"]||like_bool($l)?"":"*");if(ereg('date|time',$l["type"]))$J["now"]='teraz';if(eregi('_(md5|sha1)$',$l["field"],$y))$J[]=strtolower($y[1]);return$J;}function
editInput($R,$l,$c,$W){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$c value='-1' checked><i>".'originál'."</i></label> ":"").enum_input("radio",$c,$l,($W||isset($_GET["select"])?$W:0),($l["null"]?"":null));$Lc=$this->_foreignKeyOptions($R,$l["field"],$W);if($Lc!==null)return(is_array($Lc)?"<select$c>".optionlist($Lc,$W,true)."</select>":"<input value='".h($W)."'$c class='hidden'><input value='".h($Lc)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($R)."&field=".urlencode($l["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");if(like_bool($l))return'<input type="checkbox" value="'.h($W?$W:1).'"'.($W?' checked':'')."$c>";$Ub="";if(ereg('time',$l["type"]))$Ub='HH:MM:SS';if(ereg('date|timestamp',$l["type"]))$Ub='d.m.[rrrr]'.($Ub?" [$Ub]":"");if($Ub)return"<input value='".h($W)."'$c> ($Ub)";if(eregi('_(md5|sha1)$',$l["field"]))return"<input type='password' value='".h($W)."'$c>";return'';}function
...
dumpHeaders($Xb,$zc=false){$vb="csv";header("Content-Type: text/csv; charset=utf-8");return$vb;}function
homepage(){return
true;}function
...
';if($yc=="auth"){$Cb=true;foreach((array)$_SESSION["pwds"]["server"][""]as$T=>$F){if($F!==null){if($Cb){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Cb=false;}echo"<a href='".h(auth_url("server","",$T))."'>".($T!=""?h($T):"<i>".'prázdne'."</i>")."</a><br>\n";}}}else{echo'<form action="" method="post">
<p class="logout">
<input type="submit" name="logout" value="Odhlásiť" id="logout">
<input type="hidden" name="token" value="',$Nd,'">
</p>
</form>
';$this->databasesPrint($yc);if($yc!="db"&&$yc!="ns"){$S=table_status();if(!$S)echo"<p class='message'>".'Žiadne tabuľky.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($yc){}function
tablesPrint($Ed){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Ed
as$K){$B=$this->tableName($K);if(isset($K["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($K["Name"])."'".bold($_GET["select"]==$K["Name"])." title='".'Vypísať dáta'."'>$B</a><br>\n";}}function
...
_foreignKeyOptions($R,$e,$W=null){global$g;if(list($Fd,$q,$B)=$this->_foreignColumn(column_foreign_keys($R),$e)){$J=&$this->_values[$Fd];if($J===null){$S=table_status($Fd);$J=($S["Rows"]>1000?"":array(""=>"")+get_key_vals("SELECT $q, $B FROM ".table($Fd)." ORDER BY 2"));}if(!$J&&$W!==null)return$g->result("SELECT $B FROM ".table($Fd)." WHERE $q = ".q($W));return$J;}}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);function
...
as$B=>$l){echo"<tr><th>".$b->fieldName($l);$Va=$_GET["set"][bracket_escape($B)];$W=($K!==null?($K[$B]!=""&&$t=="sql"&&ereg("enum|set",$l["type"])?(is_array($K[$B])?array_sum($K[$B]):+$K[$B]):$K[$B]):(!$be&&$l["auto_increment"]?"":(isset($_GET["select"])?false:($Va!==null?$Va:$l["default"]))));if(!$_POST["save"]&&is_string($W))$W=$b->editVal($W,$l);$o=($_POST["save"]?(string)$_POST["function"][$B]:($be&&$l["on_update"]=="CURRENT_TIMESTAMP"?"now":($W===false?null:($W!==null?'':'NULL'))));if($l["type"]=="timestamp"&&$W=="CURRENT_TIMESTAMP"){$W="";$o="now";}input($l,$W,$o);echo"\n";}echo"</table>\n";}echo'<p>
';if($m){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($be?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($be?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$Nd,'">
</form>
';}elseif(isset($_GET["select"])){$a=$_GET["select"];$S=table_status($a);$s=indexes($a);$m=fields($a);$Ib=column_foreign_keys($a);$Ec="";if($S["Oid"]=="t"){$Ec=($t=="sqlite"?"rowid":"oid");$s[]=array("type"=>"PRIMARY","columns"=>array($Ec));}parse_str($_COOKIE["adminer_import"],$ha);$kd=array();$f=array();$Hd=null;foreach($m
as$u=>$l){$B=$b->fieldName($l);if(isset($l["privileges"]["select"])&&$B!=""){$f[$u]=html_entity_decode(strip_tags($B));if(ereg('text|lob|geometry|point|linestring|polygon',$l["type"]))$Hd=$b->selectLengthProcess();}$kd+=$l["privileges"];}list($M,$Ob)=$b->selectColumnsProcess($f,$s);$fc=count($Ob)<count($M);$X=$b->selectSearchProcess($m,$s);$C=$b->selectOrderProcess($m,$s);$v=$b->selectLimitProcess();$Mb=($M?implode(", ",$M):"*".($Ec?", $Ec":""));if($t=="sql"){foreach($f
as$u=>$V){$ma=convert_field($m[$u]);if($ma)$Mb.=", $ma AS ".idf_escape($u);}}$Mb.="\nFROM ".table($a);$Pb=($Ob&&$fc?"\nGROUP BY ".implode(", ",$Ob):"").($C?"\nORDER BY ".implode(", ",$C):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Xd=>$K){$ma=convert_field($m[key($K)]);echo$g->result("SELECT".limit(($ma?$ma:idf_escape(key($K)))." FROM ".table($a)," WHERE ".where_check($Xd).($X?" AND ".implode(" AND ",$X):"").($C?" ORDER BY ".implode(", ",$C):""),1));}exit;}if($_POST&&!$k){$je="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$Wc=$Zd=null;foreach($s
as$r){if($r["type"]=="PRIMARY"){$Wc=array_flip($r["columns"]);$Zd=($M?$Wc:array());break;}}foreach((array)$Zd
as$u=>$V){if(in_array(idf_escape($u),$M))unset($Zd[$u]);}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");if(!is_array($_POST["check"])||$Zd===array()){$ie=$X;if(is_array($_POST["check"]))$ie[]="($je)";$H="SELECT $Mb".($ie?"\nWHERE ".implode(" AND ",$ie):"").$Pb;}else{$Vd=array();foreach($_POST["check"]as$V)$Vd[]="(SELECT".limit($Mb,"\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V).$Pb,1).")";$H=implode(" UNION ALL ",$Vd);}$b->dumpData($a,"table",$H);exit;}if(!$b->selectEmailProcess($X,$Ib)){if($_POST["save"]||$_POST["delete"]){$I=true;$ia=0;$H=table($a);$O=array();if(!$_POST["delete"]){foreach($f
as$B=>$V){$V=process_input($m[$B]);if($V!==null){if($_POST["clone"])$O[idf_escape($B)]=($V!==false?$V:idf_escape($B));elseif($V!==false)$O[]=idf_escape($B)." = $V";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ja="UPDATE";if($_POST["delete"]){$Ja="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ja="INSERT";$H="INTO $H";}if($_POST["all"]||($Zd===array()&&$_POST["check"])||$fc){$I=queries("$Ja $H".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $je"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$I=queries($Ja.limit1($H,"\nWHERE ".where_check($V)));if(!$I)break;$ia+=$g->affected_rows;}}}$_=sprintf('%d položiek bolo ovplyvnených.',$ia);if($_POST["clone"]&&$I&&$ia==1){$nc=last_id();if($nc)$_=sprintf('Položka%s bola vložená.'," $nc");}queries_redirect(remove_from_uri("page"),$_,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Dvojkliknite na políčko, ktoré chcete zmeniť.';else{$I=true;$ia=0;foreach($_POST["val"]as$Xd=>$K){$O=array();foreach($K
as$u=>$V){$u=bracket_escape($u,1);$O[]=idf_escape($u)." = ".(ereg('char|text',$m[$u]["type"])||$V!=""?$b->processInput($m[$u],$V):"NULL");}$H=table($a)." SET ".implode(", ",$O);$ie=" WHERE ".where_check($Xd).($X?" AND ".implode(" AND ",$X):"");$I=queries("UPDATE".($fc?" $H$ie":limit1($H,$ie)));if(!$I)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$ia),$I);}}elseif(is_string($zb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Ia=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$zb,$z);$ia=count($z[0]);begin();$qd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($z[0]as$u=>$V){preg_match_all("~((\"[^\"]*\")+|[^$qd]*)$qd~",$V.$qd,$tc);if(!$u&&!array_diff($tc[1],$Ia)){$Ia=$tc[1];$ia--;}else{$O=array();foreach($tc[1]as$p=>$Fa)$O[idf_escape($Ia[$p])]=($Fa==""&&$m[$Ia[$p]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Fa))));$I=insert_update($a,$O,$Wc);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$ia),$I);queries("ROLLBACK");}else$k=upload_error($zb);}}$Dd=$b->tableName($S);if(is_ajax())ob_start();page_header('Vypísať'.": $Dd",$k);$O=null;if(isset($kd["insert"])){$O="";foreach((array)$_GET["where"]as$V){if(count($Ib[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$O.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($S,$O);if(!$f)echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($X,$f,$s);$b->selectOrderPrint($C,$f,$s);$b->selectLimitPrint($v);$b->selectLengthPrint($Hd);$b->selectActionPrint($s);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$n=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$E=floor(max(0,$n-1)/$v);}$H=$b->selectQueryBuild($M,$X,$Ob,$C,$v,$E);if(!$H)$H="SELECT".limit((+$v&&$Ob&&$fc&&$t=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Mb,($X?"\nWHERE ".implode(" AND ",$X):"").$Pb,($v!=""?+$v:null),($E?$v*$E:0),"\n");echo$b->selectQuery($H);$I=$g->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($t=="mssql")$I->seek($v*$E);$lb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$t=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$n=(+$v&&$Ob&&$fc?($t=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Žiadne riadky.'."\n";else{$ua=$b->backwardKeys($a,$Dd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Ob&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$Ac=array();$Nb=array();reset($M);$cd=1;foreach($L[0]as$u=>$V){if($u!=$Ec){$V=$_GET["columns"][key($M)];$l=$m[$M?($V?$V["col"]:current($M)):$u];$B=($l?$b->fieldName($l,$cd):"*");if($B!=""){$cd++;$Ac[$u]=$B;$e=idf_escape($u);$Wb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Wa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Wb.($C[0]==$e||$C[0]==$u||(!$C&&$fc&&$Ob[0]==$e)?$Wa:'')).'">';echo(!$M||$V?apply_sql_function($V["fun"],$B):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($Wb.$Wa)."' title='".'zostupne'."' class='text'> ↓</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Vyhľadať'.'" class="text jsonly"> =</a>';echo"</span>";}$Nb[$u]=$V["fun"];next($M);}}$qc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$u=>$V)$qc[$u]=max($qc[$u],min(40,strlen(utf8_decode($V))));}}echo($ua?"<th>".'Vzťahy':"")."</thead>\n";if(is_ajax()){if($v%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Ib)as$A=>$K){$Wd=unique_array($L[$A],$s);$Xd="";foreach($Wd
as$u=>$V)$Xd.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$Ob&&$M?"":"<td>".checkbox("check[]",substr($Xd,1),in_array(substr($Xd,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($fc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Xd)."'>".'upraviť'."</a>"));foreach($K
as$u=>$V){if(isset($Ac[$u])){$l=$m[$u];if($V!=""&&(!isset($lb[$u])||$lb[$u]!=""))$lb[$u]=(is_mail($V)?$Ac[$u]:"");$w="";$V=$b->editVal($V,$l);if($V!==null){if(ereg('blob|bytea|raw|file',$l["type"])&&$V!="")$w=h(ME.'download='.urlencode($a).'&field='.urlencode($u).$Xd);if($V==="")$V="&nbsp;";elseif(is_utf8($V)){if($Hd!=""&&ereg('text|lob|geometry|point|linestring|polygon',$l["type"]))$V=shorten_utf8($V,max(0,+$Hd));else$V=h($V);}if(!$w){foreach((array)$Ib[$u]as$Hb){if(count($Ib[$u])==1||end($Hb["source"])==$u){$w="";foreach($Hb["source"]as$p=>$td)$w.=where_link($p,$Hb["target"][$p],$L[$A][$td]);$w=h(($Hb["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($Hb["db"]),ME):ME).'select='.urlencode($Hb["table"]).$w);if(count($Hb["source"])==1)break;}}}if($u=="COUNT(*)"){$w=h(ME."select=".urlencode($a));$p=0;foreach((array)$_GET["where"]as$U){if(!array_key_exists($U["col"],$Wd))$w.=h(where_link($p++,$U["col"],$U["val"],$U["op"]));}foreach($Wd
as$hc=>$U)$w.=h(where_link($p++,$hc,$U));}}if(!$w){if(is_mail($V))$w="mailto:$V";if($Zc=is_url($K[$u]))$w=($Zc=="http"&&$aa?$K[$u]:"$Zc://www.adminer.org/redirect/?url=".urlencode($K[$u]));}$q=h("val[$Xd][".bracket_escape($u)."]");$W=$_POST["val"][$Xd][bracket_escape($u)];$Rb=h($W!==null?$W:$K[$u]);$sc=strpos($V,"<i>...</i>");$hb=is_utf8($V)&&$L[$A][$u]==$K[$u]&&!$Nb[$u];$Gd=ereg('text|lob',$l["type"]);echo(($_GET["modify"]&&$hb)||$W!==null?"<td>".($Gd?"<textarea name='$q' cols='30' rows='".(substr_count($K[$u],"\n")+1)."'>$Rb</textarea>":"<input name='$q' value='$Rb' size='$qc[$u]'>"):"<td id='$q' ondblclick=\"".($hb?"selectDblClick(this, event".($sc?", 2":($Gd?", 1":"")).")":"alert('".h('Pre zmenu tejto hodnoty použite odkaz upraviť.')."')").";\">".$b->selectVal($V,$w,$l));}}if($ua)echo"<td>";$b->backwardKeysPrint($ua,$L[$A]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n",(!$Ob&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if(($L||$E)&&!is_ajax()){$rb=true;if($_GET["page"]!="last"&&+$v&&!$fc&&($n>=$v||$E)){$n=found_rows($S,$X);if($n<max(1e4,2*($E+1)*$v))$n=reset(slow_query("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):"")));else$rb=false;}echo"<p class='pages'>";if(+$v&&($n===false||$n>$v)){$uc=($n===false?$E+(count($L)>=$v?2:1):floor(($n-1)/$v));echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($E+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$E).($E>5?" ...":"");for($p=max(1,$E-4);$p<min($uc,$E+5);$p++)echo
pagination($p,$E);echo($E+5<$uc?" ...":"").($rb&&$n!==false?pagination($uc,$E):' <a href="'.h(remove_from_uri("page")."&page=last").'">'.'posledný'."</a>");}echo($n!==false?" (".($rb?"":"~ ").lang(array('%d riadok','%d riadky','%d riadkov'),$n).")":""),(+$v&&($n===false?count($L)+1:$n-$E*$v)>$v?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$v).', \''.'Loading'.'\');">'.'Load more data'.'</a>':'')," ".checkbox("all",1,0,'celý výsledok')."\n";if($b->selectCommandPrint()){echo'<fieldset><legend>Upraviť</legend><div>
