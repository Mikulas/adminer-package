...
confirm($Ia="",$hd=false){return" onclick=\"".($hd?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Ia?" (' + $Ia + ')":"")."');\"";}function
...
upload_error($h){$jc=($h==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($h?'Súbor sa nepodarilo nahrať.'.($jc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$jc):""):'Súbor neexistuje.');}function
...
enum_input($Bd,$c,$i,$W,$fb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$i["length"],$x);$I=(isset($fb)?"<label><input type='$Bd'$c value='$fb'".((is_array($W)?in_array($fb,$W):$W===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($x[1]as$n=>$V){$V=stripcslashes(str_replace("''","'",$V));$za=(is_int($W)?$W==$n+1:(is_array($W)?in_array($n+1,$W):$W===$V));$I.=" <label><input type='$Bd'$c value='".($n+1)."'".($za?' checked':'').'>'.h($b->editVal($V,$i)).'</label>';}return$I;}function
input($i,$W,$l){global$Dd,$b,$Wb;$_=h(bracket_escape($i["field"]));echo"<td class='function'>";$Tc=($Wb=="mssql"&&$i["auto_increment"]);if($Tc&&!$_POST["save"]){$l=null;}$Fb=(isset($_GET["select"])||$Tc?array("orig"=>'originál'):array())+$b->editFunctions($i);$c=" name='fields[$_]'";if($i["type"]=="enum"){echo
...
selectVal($V,$u,$i){$I=($V=="<i>NULL</i>"?"&nbsp;":$V);if(ereg('blob|bytea',$i["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V)){$I="<img src='$u' alt='$I'>";}}if(like_bool($i)&&$I!="&nbsp;"){$I='<img src="'.($V?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.3").'" alt="'.h($V).'">';}if($u){$I="<a href='$u'>$I</a>";}if(!$u&&!like_bool($i)&&ereg('int|float|double|decimal',$i["type"])){$I="<div class='number'>$I</div>";}elseif(ereg('date',$i["type"])){$I="<div class='datetime'>$I</div>";}return$I;}function
...
selectSearchPrint($X,$e,$q){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$Xb=array();foreach($X
...
as$_=>$Sa){$A=$this->_foreignKeyOptions($_GET["select"],$_);if(is_array($A)){if($j[$_]["null"]){$A[0]='('.'prázdne'.')';}$s=$Xb[$_];$n--;echo"<div>".h($Sa)."<input type='hidden' name='where[$n][col]' value='".h($_)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($A,$X[$s]["val"],true)."</select></div>\n";unset($e[$_]);}}$n=0;foreach($X
as$V){if(($V["col"]==""||$e[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$n][col]'><option value=''>(".'kdekoľvek'.")".optionlist($e,$V["col"],true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$n][val]' value='".h($V["val"])."'></div>\n";$n++;}}echo"<div><select name='where[$n][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoľvek'.")".optionlist($e,null,true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators),"<input name='where[$n][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$e,$q){$_c=array();foreach($q
as$s=>$p){$B=array();foreach($p["columns"]as$V){$B[]=$e[$V];}if(count(array_filter($B,'strlen'))>1&&$s!="PRIMARY"){$_c[$s]=implode(", ",$B);}}if($_c){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$_c,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
like_bool($i){return
ereg("bool|(tinyint|bit)\\(1\\)",$i["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$tc="RESTRICT|CASCADE|SET NULL|NO ACTION";$Xa[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$f->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Ld=(isset($_GET["select"])?$_POST["edit"]:$X);$j=fields($a);foreach($j
as$_=>$i){if(!isset($i["privileges"][$Ld?"update":"insert"])||$b->fieldName($i)==""){unset($j[$_]);}}if($_POST&&!$h&&!isset($_GET["select"])){$v=$_POST["referer"];if($_POST["insert"]){$v=($Ld?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$v)){$v=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$v,'Položka bola vymazaná.');}else{$N=array();foreach($j
as$_=>$i){$V=process_input($i);if($V!==false&&$V!==null){$N[idf_escape($_)]=($Ld?"\n".idf_escape($_)." = $V":$V);}}if($Ld){if(!$N){redirect($v);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$v,'Položka bola aktualizovaná.');}else{$H=insert_into($a,$N);$bc=($H?last_id():0);queries_redirect($v,sprintf('Položka%s bola vložená.',($bc?" $bc":"")),$H);}}}$pd=$b->tableName(table_status($a));page_header(($Ld?'Upraviť':'Vložiť'),$h,array("select"=>array($a,$pd)),$pd);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($X){$L=array();foreach($j
as$_=>$i){if(isset($i["privileges"]["select"])){$L[]=($_POST["clone"]&&$i["auto_increment"]?"'' AS ":(ereg("enum|set",$i["type"])?"1*".idf_escape($_)." AS ":"")).idf_escape($_);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Žiadne riadky.'."\n";}echo'
...
';if($j){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Ld?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($Ld?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$j?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$Cb,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
