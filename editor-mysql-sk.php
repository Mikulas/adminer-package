...
confirm($Pa=""){return" onclick=\"return confirm('".'Naozaj?'.($Pa?" (' + $Pa + ')":"")."');\"";}function
...
enum_input($ae,$c,$l,$W,$ob=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$z);$J=($ob!==null?"<label><input type='$ae'$c value='$ob'".((is_array($W)?in_array($ob,$W):$W===0)?" checked":"")."><i>".'prÃ¡zdne'."</i></label>":"");foreach($z[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$J.=" <label><input type='$ae'$c value='".($p+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$J;}function
input($l,$W,$o){global$g,$ce,$b,$t;$B=h(bracket_escape($l["field"]));echo"<td class='function'>";$od=($t=="mssql"&&$l["auto_increment"]);if($od&&!$_POST["save"])$o=null;$Sb=(isset($_GET["select"])||$od?array("orig"=>'originÃ¡l'):array())+$b->editFunctions($l);$c=" name='fields[$B]'";if($l["type"]=="enum")echo
...
as$u=>$r){$C=array();foreach($r["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Tc[$u]=implode(", ",$C);}if($Tc){echo'<fieldset><legend>'.'ZotriediÅ¥'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';if($m){echo"<input type='submit' value='".'UloÅ¾iÅ¥'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($ke?'UloÅ¾iÅ¥ a pokraÄovaÅ¥ v ÃºpravÃ¡ch':'UloÅ¾iÅ¥ a vloÅ¾iÅ¥ ÄalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($ke?"<input type='submit' name='delete' value='".'ZmazaÅ¥'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
