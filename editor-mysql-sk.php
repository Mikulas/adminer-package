...
confirm($nc="",$Kd=false){return" onclick=\"".($Kd?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($nc?" (' + $nc + ')":"")."');\"";}function
js_escape($C){return
addcslashes($C,"\r\n'\\/");}function
...
upload_error($u){$Tc=($u==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($u?'Súbor sa nepodarilo nahrať.'.($Tc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Tc):""):'Súbor neexistuje.');}function
...
enum_input($sa,$N,$e,$i,$Ib=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$R);$b=(isset($Ib)?"<label><input type='$sa'$N value='$Ib'".((is_array($i)?in_array($Ib,$i):$i===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($R[1]as$p=>$a){$a=stripcslashes(str_replace("''","'",$a));$db=(is_int($i)?$i==$p+1:(is_array($i)?in_array($p+1,$i):$i===$a));$b.=" <label><input type='$sa'$N value='".($p+1)."'".($db?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$w){global$Ca,$j,$na;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$Ec=($na=="mssql"&&$e["auto_increment"]);if($Ec&&!$_POST["save"]){$w=null;}$Z=(isset($_GET["select"])||$Ec?array("orig"=>'originál'):array())+$j->editFunctions($e);$N=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($gd,$_a){$fd=($_a==1||(!$_a&&'sk'=='fr')?0:('sk'=='sl'&&(!$_a||$_a>2)?1:0)+((!$_a||$_a>=5)&&ereg('cs|sk|ru|sl|pl','sk')?2:1));return
...
selectVal($a,$t,$e){$b=($a=="<i>NULL</i>"?"&nbsp;":$a);if(ereg('blob|bytea',$e["type"])&&!is_utf8($a)){$b=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($a));if(ereg("^(GIF|\xFF\xD8\xFF|\x89\x50\x4E\x47\x0D\x0A\x1A\x0A)",$a)){$b="<img src='$t' alt='$b'>";}}if(like_bool($e)&&$b!="&nbsp;"){$b='<img src="'.($a?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.2").'" alt="'.h($a).'">';}if($t){$b="<a href='$t'>$b</a>";}if(!$t&&!like_bool($e)&&ereg('int|float|double|decimal',$e["type"])){$b="<div class='number'>$b</div>";}elseif(ereg('date',$e["type"])){$b="<div class='datetime'>$b</div>";}return$b;}function
...
selectSearchPrint($o,$_,$F){$o=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$Kb=array();foreach($o
...
as$f=>$qa){$ga=$this->_foreignKeyOptions($_GET["select"],$f);if($ga){if($q[$f]["null"]){$ga[0]='('.'prázdne'.')';}$c=$Kb[$f];$p--;echo"<div>".h($qa)."<input type='hidden' name='where[$p][col]' value='".h($f)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($ga,$o[$c]["val"],true)."</select></div>\n";unset($_[$f]);}}$p=0;foreach($o
as$a){if(($a["col"]==""||$_[$a["col"]])&&"$a[col]$a[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'kdekoľvek'.")".optionlist($_,$a["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$a["op"]),"<input name='where[$p][val]' value='".h($a["val"])."'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoľvek'.")".optionlist($_,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input name='where[$p][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($V,$_,$F){$Qb=array();foreach($F
as$c=>$S){$V=array();foreach($S["columns"]as$a){$V[]=$_[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Qb[$c]=implode(", ",$V);}}if($Qb){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Qb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
selectEmailPrint($ob,$_){if($ob){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<p>".'Odosielateľ'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Predmet'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea><br>\n";echo
html_select("email_addition",$_,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložiť'."'>\n";echo"<p>".'Prílohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($ob)==1?'<input type="hidden" name="email_field" value="'.h(key($ob)).'">':html_select("email_field",$ob)),"<input type='submit' name='email' value='".'Odoslať'."' onclick=\"return this.form['delete'].onclick();\">\n","</div></fieldset>\n";}}function
...
like_bool($e){return
ereg("bool|(tinyint|bit)\\(1\\)",$e["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$_b=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$ta[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$r=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$r-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($r)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$r=$_GET["edit"];$o=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$la=(isset($_GET["select"])?$_POST["edit"]:$o);$q=fields($r);foreach($q
as$f=>$e){if(!isset($e["privileges"][$la?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$u&&!isset($_GET["select"])){$G=$_POST["referer"];if($_POST["insert"]){$G=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$G)){$G=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $o"),$G,'Položka bola vymazaná.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($la?"\n".idf_escape($f)." = $a":$a);}}if($la){if(!$s){redirect($G);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $o"),$G,'Položka bola aktualizovaná.');}else{$k=insert_into($r,$s);$bc=($k?last_id():0);queries_redirect($G,sprintf('Položka%s bola vložená.',($bc?" $bc":"")),$k);}}}$Qa=$j->tableName(table_status($r));page_header(($la?'Upraviť':'Vložiť'),$u,array("select"=>array($r,$Qa)),$Qa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($o){$x=array();foreach($q
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$da,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
