...
confirm($Pc="",$xd=false){return" onclick=\"".($xd?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Pc?" (' + $Pc + ')":"")."');\"";}function
...
upload_error($v){$Nc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Súbor sa nepodarilo nahrať.'.($Nc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Nc):""):'Súbor neexistuje.');}function
...
enum_input($ua,$L,$e,$i,$Hb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($Hb)?"<label><input type='$ua'$L value='$Hb'".((is_array($i)?in_array($Hb,$i):$i===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($U[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Za=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ua'$L value='".($n+1)."'".($Za?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$z){global$Ba,$j,$ka;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$rc=($ka=="mssql"&&$e["auto_increment"]);if($rc&&!$_POST["save"]){$z=null;}$da=(isset($_GET["select"])||$rc?array("orig"=>'originál'):array())+$j->editFunctions($e);$L=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
selectVal($a,$t,$e){$b=($a=="<i>NULL</i>"?"&nbsp;":$a);if(ereg('blob|bytea',$e["type"])&&!is_utf8($a)){$b=lang(array('%d bajt','%d bajty','%d bajtov'),strlen($a));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$a)){$b="<img src='$t' alt='$b'>";}}if(like_bool($e)&&$b!="&nbsp;"){$b='<img src="'.($a?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.2").'" alt="'.h($a).'">';}if($t){$b="<a href='$t'>$b</a>";}if(!$t&&!like_bool($e)&&ereg('int|float|double|decimal',$e["type"])){$b="<div class='number'>$b</div>";}elseif(ereg('date',$e["type"])){$b="<div class='datetime'>$b</div>";}return$b;}function
...
selectSearchPrint($o,$A,$G){$o=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhľadať'."</legend><div>\n";$Lb=array();foreach($o
...
as$f=>$na){$Z=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Z)){if($q[$f]["null"]){$Z[0]='('.'prázdne'.')';}$c=$Lb[$f];$n--;echo"<div>".h($na)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Z,$o[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($o
...
as$c=>$T){$V=array();foreach($T["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Mb[$c]=implode(", ",$V);}}if($Mb){echo'<fieldset><legend>'.'Zotriediť'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Mb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
like_bool($e){return
ereg("bool|(tinyint|bit)\\(1\\)",$e["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$qb="RESTRICT|CASCADE|SET NULL|NO ACTION";$va[DRIVER]='Prihlásiť sa';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$r=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$r-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($r)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$r=$_GET["edit"];$o=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$ba=(isset($_GET["select"])?$_POST["edit"]:$o);$q=fields($r);foreach($q
as$f=>$e){if(!isset($e["privileges"][$ba?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$I=$_POST["referer"];if($_POST["insert"]){$I=($ba?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$I)){$I=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $o"),$I,'Položka bola vymazaná.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($ba?"\n".idf_escape($f)." = $a":$a);}}if($ba){if(!$s){redirect($I);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $o"),$I,'Položka bola aktualizovaná.');}else{$k=insert_into($r,$s);$Ic=($k?last_id():0);queries_redirect($I,sprintf('Položka%s bola vložená.',($Ic?" $Ic":"")),$k);}}}$La=$j->tableName(table_status($r));page_header(($ba?'Upraviť':'Vložiť'),$v,array("select"=>array($r,$La)),$La);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($o){$u=array();foreach($q
as$f=>$e){if(isset($e["privileges"]["select"])){$u[]=($_POST["clone"]&&$e["auto_increment"]?"'' AS ":(ereg("enum|set",$e["type"])?"1*".idf_escape($f)." AS ":"")).idf_escape($f);}}$d=array();if($u){$w=get_rows("SELECT".limit(implode(", ",$u)." FROM ".table($r)," WHERE $o",(isset($_GET["select"])?2:1)));$d=(isset($_GET["select"])&&count($w)!=1?null:reset($w));}}if($d===false){echo"<p class='error'>".'Žiadne riadky.'."\n";}echo'
...
';if($q){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($ba?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($ba?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$q?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<input type="submit" name="delete" value="Zmazať" onclick="return confirm('Naozaj? (' + (this.form['all'].checked ? <?php echo$ma,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
