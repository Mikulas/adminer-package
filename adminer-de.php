...
confirm($Za="",$Ze=false){return" onclick=\"".($Ze?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Za?" (' + $Za + ')":"")."');\"";}function
...
upload_error($k){$cd=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($k?'Hochladen von Datei fehlgeschlagen.'.($cd?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$cd):""):'Datei existiert nicht.');}function
...
enum_input($S,$xa,$l,$X,$Bb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Xc);$G=(isset($Bb)?"<label><input type='$S'$xa value='$Bb'".((is_array($X)?in_array($Bb,$X):$X===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($Xc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ja=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($q+1)."'".($Ja?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
...
q($W);}function
check_sqlite_name($A){global$g;$Tb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Tb)\$~",$A)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Tb));return
false;}return
true;}function
...
set_schema($Je){global$g,$T,$bf;$G=$g->query("SET search_path TO ".idf_escape($Je));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$bf['Benutzer-definierte Typen'][]=$S;}}return$G;}function
...
reset($I);}function
support($Vb){return
ereg("view|drop_col|variables|status",$Vb);}$v="oracle";$T=array();$bf=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binär'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$bf[$w]=array_keys($W);}$Pf=array();$Cd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$jc=array("length","lower","round","upper");$nc=array("avg","count","count distinct","max","min","sum");$xb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$rb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ee=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("driver",$rb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($if,$L=""){echo'<p class="tabs">';$Sc=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($if)){$Sc["view"]='View ändern';}else{$Sc["create"]='Tabelle ändern';}if(isset($L)){$Sc["edit"]='Neuer Datensatz';}foreach($Sc
...
selectColumnsPrint($J,$f){global$jc,$nc;print_fieldset("select",'Daten zeigen von',$J);$q=0;$ic=array('Funktionen'=>$jc,'Agregationen'=>$nc);foreach($J
...
as$q=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$q=0;foreach((array)$_GET["where"]as$W){if("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators)){echo"<div><select name='where[$q][col]'><option value=''>(".'beliebig'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$q][op]",$this->operators,$W["op"]),"<input name='where[$q][val]' value='".h($W["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='selectAddRow(this);'><option value=''>(".'beliebig'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",$this->operators,"="),"<input name='where[$q][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Gd,$f,$u){print_fieldset("sort",'Ordnen',$Gd);$q=0;foreach((array)$_GET["order"]as$w=>$W){if(isset($f[$W])){echo"<div><select name='order[$q]'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'absteigend')."</div>\n";$q++;}}echo"<div><select name='order[$q]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$q]' value='1'>".'absteigend'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($y){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($y)."'>","</div></fieldset>\n";}function
selectLengthPrint($tf){if(isset($tf)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($tf).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($E){global$v;static$Za=0;restart_session();$r="sql-".($Za++);$pc=&get_session("queries");if(strlen($E)>1e6){$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";}$pc[$_GET["db"]][]=$E;return" <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL-Query'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($pc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
<p class="logout">
';if(DB==""||!$ld){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL-Query'."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}}echo'<input type="submit" name="logout" value="Abmelden" onclick="eventStop(event);">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($i?html_select("db",array(""=>"(".'Datenbank'.")")+$i,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Benutzung"',($i?" class='hidden'":""),' onclick="eventStop(event);">
';if($ld!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'Schema'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$ld){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle'."</a>\n";$Q=tables_list();if(!$Q){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($Q);$Sc=array();foreach($Q
...
edit_type($w,$l,$Oa,$dc=array()){global$bf,$T,$Pf,$zd;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$bf+($dc?array('Fremdschlüssel'=>$dc):array()),$l["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($Oa,$l["collation"]).'</select>',($Pf?"<select name='$w"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Pf,$l["unsigned"]).'</select>':''),($dc?"<select name='$w"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$zd),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Ta?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.3' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.3' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Jd==""||support("drop_col")?"<input type='image' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.3' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Im Server',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ua=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W)){$Ua[]=".$w";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($Ua?"[".implode("|",$Ua)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($pc){print_fieldset("history",'History',$_GET["history"]!="");foreach($pc
as$w=>$W){echo'<a href="'.h(ME."sql=&history=$w").'">'.'Ändern'."</a> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$W)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Alle ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$A=>$l){if(!isset($l["privileges"][$Qf?"update":"insert"])||$b->fieldName($l)==""){unset($m[$A]);}}if($_POST&&!$k&&!isset($_GET["select"])){$Tc=$_POST["referer"];if($_POST["insert"]){$Tc=($Qf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$Tc)){$Tc=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$Tc,'Datensatz gelöscht.');}else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null){$L[idf_escape($A)]=($Qf?"\n".idf_escape($A)." = $W":$W);}}if($Qf){if(!$L){redirect($Tc);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$Tc,'Datensatz geändert.');}else{$F=insert_into($a,$L);$Mc=($F?last_id():0);queries_redirect($Tc,sprintf('Datensatz%s hinzugefügt.',($Mc?" $Mc":"")),$F);}}}$jf=$b->tableName(table_status($a));page_header(($Qf?'Ändern':'Hinzufügen'),$k,array("select"=>array($a,$jf)),$jf);$H=null;if($_POST["save"]){$H=(array)$_POST["fields"];}elseif($Z){$J=array();foreach($m
as$A=>$l){if(isset($l["privileges"]["select"])){$J[]=($_POST["clone"]&&$l["auto_increment"]?"'' AS ":(ereg("enum|set",$l["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false){echo"<p class='error'>".'Keine Daten.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($m){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Qf?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}}echo($Qf?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($a==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Eb?html_select("Engine",array(""=>"(".'Motor'.")")+$Eb,$H["Engine"]):""),' ',($Oa&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'Kollation'.")")+$Oa,$H["Collation"]):""),' <input type="submit" value="Speichern">
...
',(support("comment")?checkbox("comments",1,$Ta,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($H["Comment"]).'" maxlength="60"'.($Ta?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
';if(support("partitioning")){$Ud=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Partitionieren um',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Td,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($Ud||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$_POST["name"]){if(DB!=""){$_GET["db"]=$_POST["name"];queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_POST["name"]),'Datenbank umbenannt.',rename_database($_POST["name"],$_POST["collation"]));}else{$i=explode("\n",str_replace("\r","",$_POST["name"]));$cf=true;$Lc="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"])){$cf=false;}$Lc=$j;}}queries_redirect(ME."db=".urlencode($Lc),'Datenbank erstellt.',$cf);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$k,array(),DB);$Oa=collations();$A=DB;$Na=null;if($_POST){$A=$_POST["name"];$Na=$_POST["collation"];}elseif(DB!=""){$Na=db_collation(DB,$Oa);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input id="name" name="name" value="'.h($A).'" maxlength="64">')."\n".($Oa?html_select("collation",array(""=>"(".'Kollation'.")")+$Oa,$Na):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Speichern">
<?php
if(DB!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Hinzufügen'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$k){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"]){query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'Schema wurde gelöscht.');}else{$z.=urlencode($_POST["name"]);if($_GET["ns"]==""){query_redirect("CREATE SCHEMA ".idf_escape($_POST["name"]),$z,'Neues Schema erstellt.');}elseif($_GET["ns"]!=$_POST["name"]){query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($_POST["name"]),$z,'Schema geändert.');}else{redirect($z);}}}page_header($_GET["ns"]!=""?'Schema ändern':'Neues Schema',$k);$H=$_POST;if(!$H){$H=array("name"=>$_GET["ns"]);}echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Speichern">
';if($_GET["ns"]!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header('Aufrufen'.": ".h($da),$k);$De=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$vc=array();$Nd=array();foreach($De["fields"]as$q=>$l){if(substr($l["inout"],-3)=="OUT"){$Nd[$q]="@".idf_escape($l["field"])." AS ".idf_escape($l["field"]);}if(!$l["inout"]||substr($l["inout"],0,2)=="IN"){$vc[]=$q;}}if(!$k&&$_POST){$Ha=array();foreach($De["fields"]as$w=>$l){if(in_array($w,$vc)){$W=process_input($l);if($W===false){$W="''";}if(isset($Nd[$w])){$g->query("SET @".idf_escape($l["field"])." = $W");}}$Ha[]=(isset($Nd[$w])?"@".idf_escape($l["field"]):$W);}$E=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$Ha).")";echo"<p><code class='jush-$v'>".h($E)."</code> <a href='".h(ME)."sql=".urlencode($E)."'>".'Ändern'."</a>\n";if(!$g->multi_query($E)){echo"<p class='error'>".error()."\n";}else{$h=connect();if(is_object($h)){$h->select_db(DB);}do{$F=$g->store_result();if(is_object($F)){select($F,$h);}else{echo"<p class='message'>".lang(array('Kommando SQL ausgeführt, %d Datensatz betroffen.','Kommando SQL ausgeführt, %d Datensätze betroffen.'),$g->affected_rows)."\n";}}while($g->next_result());if($Nd){select($g->query("SELECT ".implode(", ",$Nd)));}}}echo'
<form action="" method="post">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Cc,$H["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Ye,$H["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
as$l){if($l["field"]!=""){$L[]=(ereg("^($_c)\$",$l["inout"])?"$l[inout] ":"").idf_escape($l["field"]).process_type($l,"CHARACTER SET");}}$tb=drop_create("DROP $De ".idf_escape($da),"CREATE $De ".idf_escape($_POST["name"])." (".implode(", ",$L).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$Ee)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Prozedur entfernt.','Prozedur geändert.','Prozedur erstellt.',$da);}page_header(($da!=""?(isset($_GET["function"])?'Funktion ändern':'Prozedur ändern').": ".h($da):(isset($_GET["function"])?'Neue Funktion':'Neue Prozedur')),$k);$Oa=get_vals("SHOW CHARACTER SET");sort($Oa);$H=array("fields"=>array());if($_POST){$H=$_POST;$H["fields"]=(array)$H["fields"];process_fields($H["fields"]);}elseif($da!=""){$H=routine($da,$De);$H["name"]=$da;}echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($H["name"]),'" maxlength="64">
',($Ee?'Sprache'.": ".html_select("language",$Ee,$H["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($H["fields"],$Oa,$De);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$H["returns"],$Oa);}echo'</table>
<p>';textarea("definition",$H["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sequenz gelöscht.');}elseif($fa==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$z,'Neue Sequenz erstellt.');}elseif($fa!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($_POST["name"]),$z,'Sequenz geändert.');}else{redirect($z);}}page_header($fa!=""?'Sequenz ändern'.": ".h($fa):'Neue Sequenz',$k);$H=$_POST;if(!$H){$H=array("name"=>$fa);}echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Speichern">
';if($fa!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$z,'Typ erstellt.');}}page_header($ga!=""?'Typ ändern'.": ".h($ga):'Typ erstellen',$k);$H=$_POST;if(!$H){$H=array("as"=>"AS ");}echo'
<form action="" method="post">
<p>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Ff["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Ef,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Ff["Type"],$H["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$W){if($W[1]!="USAGE"){$kc["$_[2]$W[2]"][$W[1]]=true;}if(ereg(' WITH GRANT OPTION',$H[0])){$kc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_)){$wd=$_[1];}}}if($_POST&&!$k){$xd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$rd=q($_POST["user"])."@".q($_POST["host"]);$Xd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $xd",ME."privileges=",'Benutzer entfernt.');}else{$cb=false;if($xd!=$rd){$cb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $rd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Xd");$k=!$cb;}elseif($_POST["pass"]!=$wd||!$_POST["hashed"]){queries("SET PASSWORD FOR $rd = ".($_POST["hashed"]?$Xd:"PASSWORD($Xd)"));}if(!$k){$Ae=array();foreach($qd
...
as$ud=>$Ae){if(preg_match('~^(.+)(\\(.*\\))?$~U',$ud,$_)){grant("REVOKE",array_keys($Ae),$_[2]," ON $_[1] FROM $rd");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Benutzer geändert.':'Benutzer erstellt.'),!$k);if($cb){$g->query("DROP USER $rd");}}}page_header((isset($_GET["host"])?'Benutzer'.": ".h("$ha@$_GET[host]"):'Neuer Benutzer'),$k,array("privileges"=>array('','Rechte')));if($_POST){$H=$_POST;$kc=$qd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$wd;if($wd!=""){$H["hashed"]=true;}$kc[DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$q=0;foreach($kc
as$ud=>$p){echo'<th>'.($ud!="*.*"?"<input name='objects[$q]' value='".h($ud)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Xa=>$mb){foreach((array)$le[$Xa]as$ke=>$Sa){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Sa).'">'.h($ke);$q=0;foreach($kc
as$ud=>$p){$A="'grants[$q][".h(strtoupper($ke))."]'";$X=$p[strtoupper($ke)];if($Xa=="Server Admin"&&$ud!=(isset($kc["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Erlauben'."<option value='0'".($X=="0"?" selected":"").">".'Verbieten'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$A value='1'".($X?" checked":"").($ke=="All privileges"?" id='grants-$q-all'":($ke=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$q-all');\"")).">";}$q++;}}}echo"</table>\n",'<p>
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Ic=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W))){$Ic++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Ic),$Ic||!$_POST["kill"]);}page_header('Prozessliste',$k);echo'
<form action="" method="post">
...
as$w=>$W){echo"<td>".(($v=="sql"?$w=="Info"&&$W!="":$w=="current_query"&&$W!="<IDLE>")?"<code class='jush-$v'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($H["db"]!=""?"db=".urlencode($H["db"])."&":"")."sql=".urlencode($W)).'">'.'Ändern'.'</a>':nbsp($W));}echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$fc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Exportieren');$Od=$b->dumpOutput();echo($Od?html_select("output",$Od,$qa["output"])." ":""),html_select("format",$b->dumpFormat(),$qa["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$I);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$R'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Ab,'strlen'),$f);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Xe=isset($_GET["status"]);page_header($Xe?'Status':'Variablen');$Vf=($Xe?show_status():show_variables());if(!$Vf){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Vf
...
as$w=>$W){json_row("sum-$w",number_format($W,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$j=>$W){json_row("tables-".js_escape($j),$W);}json_row("");}exit;}else{$pf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($pf&&!$k&&!$_POST["search"]){$F=true;$ed="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$F=truncate_tables($_POST["tables"]);}$ed='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ed='Tabellen verschoben.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ed='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"]){$F=drop_views($_POST["views"]);}if($F&&$_POST["tables"]){$F=drop_tables($_POST["tables"]);}$ed='Tabellen wurden entfernt (drop).';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc()){$ed.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ed,$F);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabellen und Views'."</h3>\n";$of=tables_list();if(!$of){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'Datengrösse','<td>'.'Indexgrösse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'Datensätze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";foreach($of
as$A=>$S){$Xf=(isset($S)&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Xf?"views[]":"tables[]"),$A,in_array($A,$pf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Struktur anzeigen'.'">'.h($A).'</a>';if($Xf){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'View ändern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Daten auswählen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle ändern'),"Index_length"=>array("indexes",'Indizes ändern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle ändern'),"Rows"=>array("select",'Daten auswählen'),)as$w=>$z){echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($A)."' id='$w-".h($A)."' title='$z[1]'>?</a>":"<td id='$w-".h($A)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($of)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w){echo"<td align='right' id='sum-$w'>&nbsp;";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".($v=="sql"?"<input type='submit' value='".'Analysieren'."'> <input type='submit' name='optimize' value='".'Optimieren'."'> <input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$i=(support("scheme")?schemas():get_databases());if(count($i)!=1&&$v!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'">')," <input type='submit' name='move' value='".'Verschieben'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";}if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$Fe=routines();if($Fe){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($Fe
