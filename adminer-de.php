...
confirm($Ec="",$Mf=false){return" onclick=\"".($Mf?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($Ec?" (' + $Ec + ')":"")."');\"";}function
js_escape($J){return
addcslashes($J,"\r\n'\\/");}function
...
upload_error($n){$Ld=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Hochladen von Datei fehlgeschlagen.'.($Ld?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Ld):""):'Datei existiert nicht.');}function
...
enum_input($z,$Ta,$d,$q,$gc=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);$c=(isset($gc)?"<label><input type='$z'$Ta value='$gc'".((is_array($q)?in_array($gc,$q):$q===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($sa[1]as$k=>$b){$b=stripcslashes(str_replace("''","'",$b));$fb=(is_int($q)?$q==$k+1:(is_array($q)?in_array($k+1,$q):$q===$b));$c.=" <label><input type='$z'$Ta value='".($k+1)."'".($fb?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$N){global$Q,$o,$v;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Qd=($v=="mssql"&&$d["auto_increment"]);if($Qd&&!$_POST["save"]){$N=null;}$Z=(isset($_GET["select"])||$Qd?array("orig"=>'Original'):array())+$o->editFunctions($d);$Ta=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($ff,$Pb){$rc=($Pb==1||(!$Pb&&'de'=='fr')?0:('de'=='sl'&&(!$Pb||$Pb>2)?1:0)+((!$Pb||$Pb>=5)&&ereg('cs|sk|ru|sl','de')?2:1));return
...
q($b);}function
check_sqlite_name($f){global$g;$Pd="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Pd)\$~",$f)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Pd));return
false;}return
true;}function
...
reset($G);}function
support($tb){return
ereg("view|drop_col|variables|status",$tb);}$v="oracle";$Q=array();$Ka=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binär'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$Q+=$b;$Ka[$e]=array_keys($b);}$kb=array();$fc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$Z=array("length","lower","round","upper");$ib=array("avg","count","count distinct","max","min","sum");$Ob=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ka["mssql"]="MS SQL";if(isset($_GET["mssql"])){$nc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("driver",$ka,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'">
...
selectLinks($dd,$r=""){echo'<p class="tabs">';$La=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($dd)){$La["view"]='View ändern';}else{$La["create"]='Tabelle ändern';}if(isset($r)){$La["edit"]='Neuer Datensatz';}foreach($La
...
selectColumnsPrint($P,$B){global$Z,$ib;print_fieldset("select",'Daten zeigen von',$P);$k=0;$Ze=array('Funktionen'=>$Z,'Agregationen'=>$ib);foreach($P
...
selectOrderPrint($ab,$B,$I){print_fieldset("sort",'Ordnen',$ab);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'absteigend')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>",checkbox("desc[$k]",1,0,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($Db){if(isset($Db)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($Db).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$v;static$Ec=0;restart_session();$R="sql-".($Ec++);$Xa=&get_session("queries");$Xa[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$R' onclick=\"return !toggle('$R');\">".'SQL-Query'."</a><div id='$R' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Xa[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
';if($cc!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'Schema'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$cc){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle'."</a>\n";$D=tables_list();if(!$D){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($D);$La=array();foreach($D
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Be[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($Q[$e]==254){$b="<code>$b</code>";}}if(isset($La[$e])&&!$B[$La[$e]]){$w="edit=".urlencode($La[$e]);foreach($I[$La[$e]]as$Wc=>$pa){$w.="&where".urlencode("[".bracket_escape($Wc)."]")."=".urlencode($a[$pa]);}$b="<a href='".h(ME.$w)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($e,$d,$Y,$da=array()){global$Ka,$Q,$kb,$bb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($Q[$d["type"]])?array():array($d["type"]))+$Ka+($da?array('Fremdschlüssel'=>$da):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($Y,$d["collation"]).'</select>',($kb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($kb,$d["unsigned"]).'</select>':''),($da?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($bb,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Nb?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($Nb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $ce, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($ld==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Nb;}function
...
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Ändern'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB);$ub=array();$Re=array();$f="adminer_schema";preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]),$sa,PREG_SET_ORDER);foreach($sa
...
',checkbox("error_stops",1,$_POST["error_stops"],'Bei Fehler anhalten')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Nur Fehler anzeigen')."\n";print_fieldset("webfile",'Auf Server',$_POST["webfile"]);$qd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$qd[]=".$e";}}echo
sprintf('Webserver Datei %s',"<code>adminer.sql".($qd?"[".implode("|",$qd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Datei ausführen'.'">',"</div></fieldset>\n";if($Xa){print_fieldset("history",'History',$_GET["history"]!="");foreach($Xa
as$e=>$b){echo'<a href="'.h(ME."sql=&history=$e").'">'.'Ändern'."</a> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Entleeren'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Ändern'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$na?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ea=$_POST["referer"];if($_POST["insert"]){$ea=($na?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ea)){$ea=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$ea,'Datensatz gelöscht.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($na?"\n".idf_escape($f)." = $b":$b);}}if($na){if(!$r){redirect($ea);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$ea,'Datensatz geändert.');}else{$i=insert_into($m,$r);$Vd=($i?last_id():0);queries_redirect($ea,sprintf('Datensatz%s hinzugefügt.',($Vd?" $Vd":"")),$i);}}}$Ga=$o->tableName(table_status($m));page_header(($na?'Ändern':'Hinzufügen'),$n,array("select"=>array($m,$Ga)),$Ga);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$P=array();foreach($p
...
<label><input type="checkbox" onclick="columnShow(this.checked, 5);">Vorgabewerte festlegen</label>
',(support("comment")?checkbox("","",$Nb,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Nb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$K,'">
...
as$t){if(count($_)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$Kd=false;}$rb=$t;}}queries_redirect(ME."db=".urlencode($rb),'Datenbank erstellt.',$Kd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$n,array(),DB);$Y=collations();$f=DB;$kc=null;if($_POST){$f=$_POST["name"];$kc=$_POST["collation"];}elseif(DB!=""){$kc=db_collation(DB,$Y);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$ia){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ia,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeypress="return textareaKeypress(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Y?html_select("collation",array(""=>"(".'Kollation'.")")+$Y,$kc):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="hidden" name="token" value="<?php echo$K,'">
<input type="submit" value="Speichern">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Hinzufügen'."'>\n";}echo'</form>
...
';}elseif(isset($_GET["foreign"])){$m=$_GET["foreign"];if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"]){query_redirect("ALTER TABLE ".table($m)."\nDROP ".($v=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($m),'Fremdschlüssel entfernt.');}else{$Fa=array_filter($_POST["source"],'strlen');ksort($Fa);$ca=array();foreach($Fa
as$e=>$b){$ca[$e]=$_POST["target"][$e];}query_redirect("ALTER TABLE ".table($m).($_GET["name"]!=""?"\nDROP FOREIGN KEY ".idf_escape($_GET["name"]).",":"")."\nADD FOREIGN KEY (".implode(", ",array_map('idf_escape',$Fa)).") REFERENCES ".table($_POST["table"])." (".implode(", ",array_map('idf_escape',$ca)).")".(in_array($_POST["on_delete"],$bb)?" ON DELETE $_POST[on_delete]":"").(in_array($_POST["on_update"],$bb)?" ON UPDATE $_POST[on_update]":""),ME."table=".urlencode($m),($_GET["name"]!=""?'Fremdschlüssel geändert.':'Fremdschlüssel erstellt.'));$n='Spalten des Ursprungs und des Zieles müssen vom gleichen Datentyp sein, es muss unter den Zielspalten ein Index existieren und die referenzierten Daten müssen existieren.'."<br>$n";}}page_header('Fremdschlüssel',$n,array("table"=>$m),$m);$a=array("table"=>$m,"source"=>array(""));if($_POST){$a=$_POST;ksort($a["source"]);if($_POST["add"]){$a["source"][]="";}elseif($_POST["change"]||$_POST["change-js"]){$a["target"]=array();}}elseif($_GET["name"]!=""){$da=foreign_keys($m);$a=$da[$_GET["name"]];$a["source"][]="";}$Fa=array_keys(fields($m));$ca=($m===$a["table"]?$Fa:array_keys(fields($a["table"])));$Md=array();foreach(table_status()as$f=>$E){if(fk_support($E)){$Md[]=$f;}}echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["event"])){$lb=$_GET["event"];$Sd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Ad=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($lb),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Sd)&&isset($Ad[$_POST["STATUS"]])){$Rd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($lb!=""?"ALTER EVENT ".idf_escape($lb).$Rd.($lb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Rd)."\n".$Ad[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($lb!=""?'Ereignis geändert.':'Ereignis erstellt.'));}}page_header(($lb!=""?'Ereignis ändern'.": ".h($lb):'Ereignis erstellen'),$n);$a=array();if($_POST){$a=$_POST;}elseif($lb!=""){$G=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($lb));$a=reset($G);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Sd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Ad,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Speichern">
';if($lb!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
...
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$Y,$Ra);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$a["returns"],$Y);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$K,'">
';if($eb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($Qa!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["sequence"])){$Cb=$_GET["sequence"];if($_POST&&!$n){$w=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($Cb),$w,'Sequenz gelöscht.');}elseif($Cb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$w,'Neue Sequenz erstellt.');}elseif($Cb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($Cb)." RENAME TO ".idf_escape($_POST["name"]),$w,'Sequenz geändert.');}else{redirect($w);}}page_header($Cb!=""?'Sequenz ändern'.": ".h($Cb):'Neue Sequenz',$n);$a=array("name"=>$Cb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Speichern">
';if($Cb!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Nc["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($m))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Od,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Nc["Type"],$a["Type"]),'</table>
...
as$b){if($b[1]!="USAGE"){$jb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$jb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$Hc=$l[1];}}}if($_POST&&!$n){$ac=(isset($_GET["host"])?q($rd)."@".q($_GET["host"]):"''");$gb=q($_POST["user"])."@".q($_POST["host"]);$Cd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $ac",ME."privileges=",'Benutzer entfernt.');}else{if($ac!=$gb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $gb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Cd");}elseif($_POST["pass"]!=$Hc||!$_POST["hashed"]){queries("SET PASSWORD FOR $gb = ".($_POST["hashed"]?$Cd:"PASSWORD($Cd)"));}if(!$n){$oc=array();foreach($pc
...
as$db=>$ia){echo'<th>'.($db!="*.*"?"<input name='objects[$k]' value='".h($db)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Ic=>$_c){foreach((array)$ra[$Ic]as$qc=>$_a){echo"<tr".odd()."><td".($_c?">$_c<td":" colspan='2'").' lang="en" title="'.h($_a).'">'.h($qc);$k=0;foreach($jb
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$zd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$zd++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$zd),$zd||!$_POST["kill"]);}page_header('Prozessliste',$n);echo'
<form action="" method="post">
...
';}print_fieldset("export",'Exportieren');$Oa=$o->dumpOutput();echo($Oa?html_select("output",$Oa,$Yb["output"])." ":""),html_select("format",$o->dumpFormat(),$Yb["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}print_fieldset("import",'Importieren',!$G);echo"<input type='hidden' name='token' value='$K'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Yb["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>\n","</div></fieldset>\n";$o->selectEmailPrint(array_filter($Kc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Sb=isset($_GET["status"]);page_header($Sb?'Status':'Variablen');$pe=($Sb?show_status():show_variables());if(!$pe){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($pe
