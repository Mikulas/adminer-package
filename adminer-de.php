...
confirm($qe=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($qe?" (' + $qe + ')":"")."');\"";}function
js_escape($D){return
addcslashes($D,"\r\n'\\/");}function
...
upload_error($n){$xe=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Hochladen von Datei fehlgeschlagen.'.($xe?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$xe):""):'Datei existiert nicht.');}function
...
input($d,$r,$O){global$T,$p,$y;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$ca=(isset($_GET["select"])?array("orig"=>'Original'):array())+$p->editFunctions($d);$Sa=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($We,$pc){$oc=($pc==1||(!$pc&&'de'=='fr')?0:((!$pc||$pc>=5)&&ereg('cs|sk|ru','de')?2:1));return
...
q($b);}function
check_sqlite_name($f){global$g;$ie="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($ie)\$~",$f)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$ie));return
false;}return
true;}function
...
reset($G);}function
support($mb){return
ereg("view|drop_col|variables|status",$mb);}$y="oracle";$T=array();$Oa=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binär'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$T+=$b;$Oa[$e]=array_keys($b);}$sb=array();$Zb=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL");$ca=array("length","lower","round","upper");$pb=array("avg","count","count distinct","max","min","sum");$Wb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ka["mssql"]="MS SQL";if(isset($_GET["mssql"])){$mc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("driver",$ka,DRIVER),'<tr><th>Server<td><input name="server" value="',h(SERVER),'">
...
selectLinks($Nc,$q=""){echo'<p class="tabs">';$Ka=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Nc)){$Ka["view"]='View ändern';}else{$Ka["create"]='Tabelle ändern';}if(isset($q)){$Ka["edit"]='Neuer Datensatz';}foreach($Ka
...
selectQuery($j){global$y;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Letzte Seite'."'>&gt;&gt;</a> <code class='jush-$y'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Ändern'."</a>\n";}function
...
selectColumnsPrint($N,$B){global$ca,$pb;print_fieldset("select",'Daten zeigen von',$N);$k=0;$he=array('Funktionen'=>$ca,'Agregationen'=>$pb);foreach($N
...
selectSearchPrint($t,$B,$J){print_fieldset("search",'Suchen',$t);foreach($J
...
selectOrderPrint($nb,$B,$J){print_fieldset("sort",'Ordnen',$nb);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'absteigend')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>",checkbox("desc[$k]",1,0,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($zb){if(isset($zb)){echo"<fieldset><legend>".'Textlänge'."</legend><div>",'<input name="text_length" size="3" value="'.h($zb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$y;restart_session();$U="sql-".count($_SESSION["messages"]);$ib=&get_session("queries");$ib[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$U' onclick=\"return !toggle('$U');\">".'SQL-Query'."</a><div id='$U' class='hidden'><pre class='jush-$y'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($ib[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
explode("/",$c);}function
editInput($h,$d,$Sa,$r){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Sa value='-1' checked><i>".'Original'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Sa value=''".(isset($r)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Sa value='0'".($r===0?" checked":"")."><i>".'leer'."</i></label>".enum_input("radio",$Sa,$d,$r);}return"";}function
...
';if($Pb!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'Schema'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$Pb){echo'<p><a href="'.h(ME).'create=">'.bold('Neue Tabelle',$_GET["create"]==="")."</a>\n";$F=tables_list();if(!$F){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($F);$Ka=array();foreach($F
...
as$h=>$_){echo'<a href="'.h(ME).'select='.urlencode($h).'">'.bold('zeigen',$_GET["select"]==$h).'</a> ','<a href="'.h(ME).'table='.urlencode($h).'">'.bold($this->tableName(array("Name"=>$h)),$_GET["table"]==$h)."</a><br>\n";}}}$p=(function_exists('adminer_object')?adminer_object():new
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Ee[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d Byte','%d Bytes'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($T[$e]==254){$b="<code>$b</code>";}}if(isset($Ka[$e])&&!$B[$Ka[$e]]){$x="edit=".urlencode($Ka[$e]);foreach($J[$Ka[$e]]as$_c=>$oa){$x.="&where".urlencode("[".bracket_escape($_c)."]")."=".urlencode($a[$oa]);}$b="<a href='".h(ME.$x)."'>$b</a>";}}echo"<td>$b";}}echo($k?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($e,$d,$W,$ba=array()){global$Oa,$T,$sb,$bb;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($T[$d["type"]])?array():array($d["type"]))+$Oa+($ba?array('Fremdschlüssel'=>$ba):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($W,$d["collation"]).'</select>',($sb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($sb,$d["unsigned"]).'</select>':''),($ba?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($bb,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Kb?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($Kb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $Ne, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.1.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.1.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($hd==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.1.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Kb;}function
...
<table cellspacing="0">
';$Oe=array('','USE','DROP+CREATE','CREATE');$te=array('','DROP+CREATE','CREATE');$Id=array('','TRUNCATE+INSERT','INSERT');if($y=="sql"){$Oe[]='CREATE+ALTER';$te[]='CREATE+ALTER';$Id[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$a);if(!$a){$a=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}$Ea=($_GET["dump"]=="");echo"<tr><th>".'Ergebnis'."<td>".html_select("output",$p->dumpOutput(),$a["output"],0)."\n";echo"<tr><th>".'Format'."<td>".html_select("format",$p->dumpFormat(),$a["format"],0)."\n";echo($y=="sqlite"?"":"<tr><th>".'Datenbank'."<td>".html_select('db_style',$Oe,$a["db_style"]).(support("routine")?checkbox("routines",1,$Ea,'Prozeduren'):"").(support("event")?checkbox("events",1,$Ea,'Ereignisse'):"")),"<tr><th>".'Tabellen'."<td>".html_select('table_style',$te,$a["table_style"]).checkbox("auto_increment",1,$a["table_style"],'Auto-Inkrement').(support("trigger")?checkbox("triggers",1,$a["table_style"],'Trigger'):""),"<tr><th>".'Daten'."<td>".html_select('data_style',$Id,$a["data_style"]),'</table>
<p><input type="submit" value="Exportieren">

<table cellspacing="0">
';$ed=array();if(DB!=""){$Ea=($m!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Ea onclick='formCheck(this, /^tables\\[/);'>".'Tabellen'."</label>","<th style='text-align: right;'><label>".'Daten'."<input type='checkbox' id='check-data'$Ea onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Z="";foreach(table_status()as$a){$f=$a["Name"];$uc=ereg_replace("_.*","",$f);$Ea=($m==""||$m==(substr($m,-1)=="%"?"$uc%":$f));$db="<tr><td>".checkbox("tables[]",$f,$Ea,$f,"formUncheck('check-tables');");if(is_view($a)){$Z.="$db\n";}else{echo"$db<td align='right'><label>".($a["Engine"]=="InnoDB"&&$a["Rows"]?"~ ":"").$a["Rows"].checkbox("data[]",$f,$Ea,"","formUncheck('check-data');")."</label>\n";}$ed[$uc]++;}echo$Z;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($m==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Datenbank'."</label></thead>\n";$z=get_databases();if($z){foreach($z
...
as$f=>$d){if(!isset($d["privileges"][$na?"update":"insert"])||$p->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ja=$_POST["referer"];if($_POST["insert"]){$ja=($na?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ja)){$ja=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$ja,'Datensatz gelöscht.');}else{$q=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$q[idf_escape($f)]=($na?"\n".idf_escape($f)." = $b":$b);}}if($na){if(!$q){redirect($ja);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$q),"\nWHERE $t"),$ja,'Datensatz geändert.');}else{$i=insert_into($m,$q);$ee=($i?last_id():0);queries_redirect($ja,sprintf('Datensatz%s hinzugefügt.',($ee?" $ee":"")),$i);}}}$Ba=$p->tableName(table_status($m));page_header(($na?'Ändern':'Hinzufügen'),$n,array("select"=>array($m,$Ba)),$Ba);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$N=array();foreach($o
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($o){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($na?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."\">\n";}}if($na){echo"<input type='submit' name='delete' value='".'Entfernen'."'".confirm().">\n";}echo'</form>
...
<?php echo(support("comment")?checkbox("","",$Kb,'Kommentar',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Kb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Speichern">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if(support("partitioning")){$Td=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partitionieren um',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Yd,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Td||!$a["partition_by"]?" class='hidden'":""),'>
...
as$s){if(count($z)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$Qd=false;}$wb=$s;}}queries_redirect(ME."db=".urlencode($wb),'Datenbank erstellt.',$Qd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$n,array(),DB);$W=collations();$f=DB;$dc=null;if($_POST){$f=$_POST["name"];$dc=$_POST["collation"];}elseif(DB!=""){$dc=db_collation(DB,$W);}elseif($y=="sql"){foreach(get_vals("SHOW GRANTS")as$fa){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$fa,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeydown="return textareaKeydown(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($W?html_select("collation",array(""=>"(".'Kollation'.")")+$W,$dc):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="hidden" name="token" value="<?php echo$K,'">
<input type="submit" value="Speichern">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Hinzufügen'."'>\n";}echo'</form>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$K,'">
</form>
...
<input type="hidden" name="token" value="',$K,'">
';if($Ua){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
</form>
';}elseif(isset($_GET["event"])){$gb=$_GET["event"];$Jd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$vd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($gb),substr(ME,0,-1),'Ereignis entfernt.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Jd)&&isset($vd[$_POST["STATUS"]])){$Gd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($gb!=""?"ALTER EVENT ".idf_escape($gb).$Gd.($gb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Gd)."\n".$vd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($gb!=""?'Ereignis geändert.':'Ereignis erstellt.'));}}page_header(($gb!=""?'Ereignis ändern'.": ".h($gb):'Ereignis erstellen'),$n);$a=array();if($_POST){$a=$_POST;}elseif($gb!=""){$G=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($gb));$a=reset($G);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Jd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$vd,$a["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Speichern">
';if($gb!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
...
as$d){if($d["field"]!=""){$q[]=(in_array($d["inout"],$nc)?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Ua=drop_create("DROP $Ma ".idf_escape($Ra),"CREATE $Ma ".idf_escape($_POST["name"])." (".implode(", ",$q).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Prozedur entfernt.','Prozedur geändert.','Prozedur erstellt.',$Ra);}page_header(($Ra!=""?(isset($_GET["function"])?'Funktion ändern':'Prozedur ändern').": ".h($Ra):(isset($_GET["function"])?'Neue Funktion':'Neue Prozedur')),$n);$W=get_vals("SHOW CHARACTER SET");sort($W);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Ra!=""){$a=routine($Ra,$Ma);$a["name"]=$Ra;}echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$W,$Ma);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$a["returns"],$W);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$K,'">
';if($Ua){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($Ra!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["sequence"])){$yb=$_GET["sequence"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($yb),$x,'Sequenz gelöscht.');}elseif($yb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$x,'Neue Sequenz erstellt.');}elseif($yb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($yb)." RENAME TO ".idf_escape($_POST["name"]),$x,'Sequenz geändert.');}else{redirect($x);}}page_header($yb!=""?'Sequenz ändern'.": ".h($yb):'Neue Sequenz',$n);$a=array("name"=>$yb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Speichern">
';if($yb!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}echo'</form>
';}elseif(isset($_GET["type"])){$Oc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Oc),$x,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ erstellt.');}}page_header($Oc!=""?'Typ ändern'.": ".h($Oc):'Typ erstellen',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$K,'">
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Jc["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($m))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Fd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Jc["Type"],$a["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$K,'">
';if($Ua){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
...
as$b){if($b[1]!="USAGE"){$Za["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$Za["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$zc=$l[1];}}}if($_POST&&!$n){$Yb=(isset($_GET["host"])?q($nd)."@".q($_GET["host"]):"''");$jb=q($_POST["user"])."@".q($_POST["host"]);$zd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $Yb",ME."privileges=",'Benutzer entfernt.');}else{if($Yb!=$jb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $jb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $zd");}elseif($_POST["pass"]!=$zc||!$_POST["hashed"]){queries("SET PASSWORD FOR $jb = ".($_POST["hashed"]?$zd:"PASSWORD($zd)"));}if(!$n){$vc=array();foreach($rc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html'>".'Rechte'."</a>";$k=0;foreach($Za
as$Wa=>$fa){echo'<th>'.($Wa!="*.*"?"<input name='objects[$k]' value='".h($Wa)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Ec=>$hc){foreach((array)$la[$Ec]as$sc=>$ya){echo"<tr".odd()."><td".($hc?">$hc<td":" colspan='2'").' lang="en" title="'.h($ya).'">'.h($sc);$k=0;foreach($Za
as$Wa=>$fa){$f="'grants[$k][".h(strtoupper($sc))."]'";$r=$fa[strtoupper($sc)];if($Ec=="Server Admin"&&$Wa!=(isset($Za["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($r?" selected":"").">".'Erlauben'."<option value='0'".($r=="0"?" selected":"").">".'Verbieten'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($r?" checked":"").($sc=="All privileges"?" id='grants-$k-all'":($sc=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$k-all');\"")).">";}$k++;}}}echo"</table>\n",'<p>
<input type="hidden" name="token" value="',$K,'">
<input type="submit" value="Speichern">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$jd),$jd||!$_POST["kill"]);}page_header('Prozessliste',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".($e=="Info"&&$b!=""?"<code class='jush-$y'>".nbsp($b).'</code> <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Ändern'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
<input type="hidden" name="token" value="',$K,'">
...
<input type="submit" name="delete" value="Entfernen"',confirm("(this.form['all'].checked ? $Qa : formChecked(this, /check/))"),'>
</div></fieldset>
';}print_fieldset("export",'Exportieren');$fb=$p->dumpOutput();echo($fb?html_select("output",$fb,$fd["output"])." ":"").html_select("format",$p->dumpFormat(),$fd["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}print_fieldset("import",'Importiere CSV',!$G);echo"<input type='hidden' name='token' value='$K'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$fd["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>\n","</div></fieldset>\n";$p->selectEmailPrint(array_filter($Qc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Sb=isset($_GET["status"]);page_header($Sb?'Status':'Variablen');$Rd=($Sb?show_status():show_variables());if(!$Rd){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Rd
