...
confirm($ib=""){return" onclick=\"return confirm('".'Sind Sie sicher ?'.($ib?" (' + $ib + ')":"")."');\"";}function
...
upload_error($k){$Ed=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($Ed?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$Ed):""):'Datei existiert nicht.');}function
...
enum_input($S,$ya,$l,$X,$Rb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$_d);$H=($Rb!==null?"<label><input type='$S'$ya value='$Rb'".((is_array($X)?in_array($Rb,$X):$X===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($_d[1]as$p=>$W){$W=stripcslashes(str_replace("''","'",$W));$Oa=(is_int($X)?$X==$p+1:(is_array($X)?in_array($p+1,$X):$X===$W));$H.=" <label><input type='$S'$ya value='".($p+1)."'".($Oa?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$H;}function
...
h($g->error);}function
check_sqlite_name($A){global$g;$jc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($jc)\$~",$A)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$jc));return
false;}return
true;}function
create_database($j,$d){global$g;if(file_exists($j)){$g->error='Datei existiert schon.';return
...
as$j){if(!@unlink($j)){$g->error='Datei existiert schon.';return
false;}}return
true;}function
rename_database($A,$d){global$g;if(!check_sqlite_name($A))return
false;$g->Min_SQLite(":memory:");$g->error='Datei existiert schon.';return@rename(DB,$A);}function
...
set_schema($vf){global$g,$T,$Nf;$H=$g->query("SET search_path TO ".idf_escape($vf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$Nf['Benutzer-definierte Typen'][]=$S;}}return$H;}function
...
unconvert_field($l,$H){return$H;}function
support($mc){return
ereg("view|scheme|processlist|drop_col|variables|status",$mc);}$u="oracle";$T=array();$Nf=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binär'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$v=>$W){$T+=$W;$Nf[$v]=array_keys($W);}$Hg=array();$ie=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Cc=array("length","lower","round","upper");$Hc=array("avg","count","count distinct","max","min","sum");$Lb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Eb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Ke=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("auth[driver]",$Eb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Uf,$M=""){echo'<p class="tabs">';$wd=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Uf))$wd["view"]='View ändern';else$wd["create"]='Tabelle ändern';if($M!==null)$wd["edit"]='Neuer Datensatz';foreach($wd
...
selectQuery($F){global$u,$R;return"<form action='".h(ME)."sql=' method='post'><p><span onclick=\"return !selectEditSql(event, this, '".'Ausführen'."');\">"."<code class='jush-$u'>".h(str_replace("\n"," ",$F))."</code>"." <a href='".h(ME)."sql=".urlencode($F)."'>".'Ändern'."</a>"."</span><input type='hidden' name='token' value='$R'></p></form>\n";}function
...
selectColumnsPrint($K,$f){global$Cc,$Hc;print_fieldset("select",'Daten zeigen von',$K);$p=0;$Ac=array('Funktionen'=>$Cc,'Agregationen'=>$Hc);foreach($K
...
selectSearchPrint($Z,$f,$t){print_fieldset("search",'Suchen',$Z);foreach($t
as$p=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$p]' value='".h($_GET["fulltext"][$p])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$p]",1,isset($_GET["boolean"][$p]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ma="this.nextSibling.onchange();";for($p=0;$p<=count($_GET["where"]);$p++){list(,$W)=each($_GET["where"]);if(!$W||("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators))){echo"<div><select name='where[$p][col]' onchange='$Ma'><option value=''>(".'beliebig'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$p][op]",$this->operators,$W["op"],$Ma),"<input type='search' name='where[$p][val]' value='".h($W["val"])."' onchange='".($W?"selectFieldChange(this.form)":"selectAddRow(this)").";' onsearch='selectSearchSearch(this);'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($me,$f,$t){print_fieldset("sort",'Ordnen',$me);$p=0;foreach((array)$_GET["order"]as$v=>$W){if(isset($f[$W])){echo"<div><select name='order[$p]' onchange='selectFieldChange(this.form);'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$p]",1,isset($_GET["desc"][$v]),'absteigend')."</div>\n";$p++;}}echo"<div><select name='order[$p]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>",checkbox("desc[$p]",1,false,'absteigend')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($w){echo"<fieldset><legend>".'Begrenzung'."</legend><div>";echo"<input type='number' name='limit' class='size' value='".h($w)."' onchange='selectFieldChange(this.form);'>","</div></fieldset>\n";}function
selectLengthPrint($hg){if($hg!==null){echo"<fieldset><legend>".'Textlänge'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($hg)."'>","</div></fieldset>\n";}}function
selectActionPrint($t){echo"<fieldset><legend>".'Aktion'."</legend><div>","<input type='submit' value='".'Daten zeigen von'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script type='text/javascript'>\n","var indexColumns = ";$f=array();foreach($t
...
messageQuery($F){global$u;restart_session();$Kc=&get_session("queries");$q="sql-".count($Kc[$_GET["db"]]);if(strlen($F)>1e6)$F=ereg_replace('[\x80-\xFF]+$','',substr($F,0,1e6))."\n...";$Kc[$_GET["db"]][]=array($F,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$q' onclick=\"return !toggle('$q');\">".'SQL-Query'."</a><div id='$q' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($F,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Kc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
dumpOutput(){$H=array('text'=>'anzeigen','file'=>'Datei');if(function_exists('gzencode'))$H['gz']='gzip';return$H;}function
dumpFormat(){return
...
<p class="logout">
';if(DB==""||!$Nd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"]))." title='".'Importieren'."'>".'SQL-Query'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}echo'<input type="submit" name="logout" value="Abmelden" id="logout">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
';$this->databasesPrint($Nd);if($_GET["ns"]!==""&&!$Nd&&DB!=""){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle'."</a>\n";$Q=table_status('',true);if(!$Q)echo"<p class='message'>".'Keine Tabellen.'."\n";else{$this->tablesPrint($Q);$wd=array();foreach($Q
...
<p id="dbs">
';hidden_fields_get();$rb=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo($i?"<select name='db'$rb>".optionlist(array(""=>"(".'Datenbank'.")")+$i,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Benutzung'."'".($i?" class='hidden'":"").">\n";if($Nd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br><select name='ns'$rb>".optionlist(array(""=>"(".'Schema'.")")+schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':""))),"</p></form>\n";}function
tablesPrint($Q){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($Q
as$O=>$Kf){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O||$_GET["edit"]==$O).">".'zeigen'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold(in_array($O,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])))." title='".'Struktur anzeigen'."'>".$this->tableName($Kf)."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
auth_error($bc=null){global$g,$b,$R;$Df=session_name();$k="";if(!$_COOKIE[$Df]&&$_GET[$Df]&&ini_bool("session.use_only_cookies"))$k='Sitzungen müssen aktiviert sein.';elseif(isset($_GET["username"])){if(($_COOKIE[$Df]||$_GET[$Df])&&!$R)$k='Sitzungsdauer abgelaufen, bitte erneut anmelden.';else{$D=&get_session("pwds");if($D!==null){$k=h($bc?$bc->getMessage():(is_string($g)?$g:'Ungültige Anmelde-Informationen.'));if($D===false)$k.='<br>'.sprintf('Master password expired. <a href="http://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to make it permanent.','<code>permanentLogin()</code>');$D=null;}unset_permanent();}}page_header('Login',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Keine Erweiterungen installiert',sprintf('Keine der unterstützten PHP-Erweiterungen (%s) ist vorhanden.',implode(", ",$Ke)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$R=$_SESSION["token"];if($za&&$_POST["token"])$_POST["token"]=$R;$k='';if($_POST){if($_POST["token"]!=$R){$Wc="max_input_vars";$Fd=ini_get($Wc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$v){$W=ini_get($v);if($W&&(!$Fd||$W<$Fd)){$Wc=$v;$Fd=$W;}}}$k=(!$_POST["token"]&&$Fd?sprintf('Die maximal erlaubte Anzahl der Felder ist überschritten. Bitte %s erhöhen.',"'$Wc'"):'CSRF Token ungültig. Bitte die Formulardaten erneut abschicken.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$k=sprintf('POST data zu gross. Reduzieren Sie die Grösse oder vergrössern Sie den Wert %s in der Konfiguration.',"'post_max_size'");if(isset($_GET["sql"]))$k.=' '.'You can upload a big SQL file via FTP and import it from server.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}function
connect_error(){global$b,$g,$R,$k,$Eb;$i=array();if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('Datenbank'.": ".h(DB),'Datenbank ungültig.',true);}else{if($_POST["db"]&&!$k)queries_redirect(substr(ME,0,-1),'Datenbanken entfernt.',drop_databases($_POST["db"]));page_header('Datenbank auswählen',$k,false);echo"<p><a href='".h(ME)."database='>".'Neue Datenbank'."</a>\n";foreach(array('privileges'=>'Rechte','processlist'=>'Prozessliste','variables'=>'Variablen','status'=>'Status',)as$v=>$W){if(support($v))echo"<a href='".h(ME)."$v='>$W</a>\n";}echo"<p>".sprintf('Version %s: %s, mit PHP-Erweiterung %s',$Eb[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Angemeldet als: %s',"<b>".h(logged_user())."</b>")."\n";$ef="<a href='".h(ME)."refresh=1'>".'Aktualisieren'."</a>\n";$i=$b->databases();if($i){$wf=support("scheme");$Wa=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr><td>&nbsp;<th>".'Datenbank'."<td>".'Collation'."<td>".'Tabellen'."</thead>\n";foreach($i
as$j){$of=h(ME)."db=".urlencode($j);echo"<tr".odd()."><td>".checkbox("db[]",$j,in_array($j,(array)$_POST["db"])),"<th><a href='$of'>".h($j)."</a>","<td><a href='$of".($wf?"&amp;ns=":"")."&amp;database=' title='".'Datenbank ändern'."'>".nbsp(db_collation($j,$Wa))."</a>","<td align='right'><a href='$of&amp;schema=' id='tables-".h($j)."' title='".'Datenbankschema'."'>?</a>","\n";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","<p><input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /db/)").">\n","<input type='hidden' name='token' value='$R'>\n",$ef,"</form>\n";}else
echo"<p>$ef";}page_footer("db");if($i)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('Schema'.": ".h($_GET["ns"]),'Schema nicht gültig.',true);page_footer("ns");exit;}}function
...
edit_type($v,$l,$Wa,$vc=array()){global$Nf,$T,$Hg,$ee;echo'<td><select name="',$v,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$Nf+($vc?array('Fremdschlüssel'=>$vc):array()),$l["type"]),'</select>
<td><input name="',$v,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$v"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($Wa,$l["collation"]).'</select>',($Hg?"<select name='$v"."[unsigned]'".(!$l["type"]||ereg('((^|[^o])int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Hg,$l["unsigned"]).'</select>':''),(isset($l['on_update'])?"<select name='$v"."[on_update]'".($l["type"]=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),$l["on_update"]).'</select>':''),($vc?"<select name='$v"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$ee),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($bb?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.1' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($bb?"":" class='hidden'")."><input name='fields[$p][comment]' value='".h($l["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.1' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.7.1' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.7.1' alt='v' title='".'Nach unten'."'>&nbsp;":""),($qe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.7.1' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
send(){fseek($this->handler,0);fpassthru($this->handler);fclose($this->handler);}}$Vb="'(?:''|[^'\\\\]|\\\\.)*+'";$Xc="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["callf"]))$_GET["call"]=$_GET["callf"];if(isset($_GET["function"]))$_GET["procedure"]=$_GET["function"];if(isset($_GET["download"])){$a=$_GET["download"];$m=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$m),1));exit;}elseif(isset($_GET["table"])){$a=$_GET["table"];$m=fields($a);if(!$m)$k=error();$P=table_status1($a,true);page_header(($m&&is_view($P)?'View':'Tabelle').": ".h($a),$k);$b->selectLinks($P);$ab=$P["Comment"];if($ab!="")echo"<p>".'Kommentar'.": ".h($ab)."\n";if($m){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Spalte'."<td>".'Typ'.(support("comment")?"<td>".'Kommentar':"")."</thead>\n";foreach($m
as$l){echo"<tr".odd()."><th>".h($l["field"]),"<td title='".h($l["collation"])."'>".h($l["full_type"]).($l["null"]?" <i>NULL</i>":"").($l["auto_increment"]?" <i>".'Auto-Inkrement'."</i>":""),(isset($l["default"])?" [<b>".h($l["default"])."</b>]":""),(support("comment")?"<td>".nbsp($l["comment"]):""),"\n";}echo"</table>\n";if(!is_view($P)){echo"<h3 id='indexes'>".'Indizes'."</h3>\n";$t=indexes($a);if($t){echo"<table cellspacing='0'>\n";foreach($t
as$A=>$s){ksort($s["columns"]);$Pe=array();foreach($s["columns"]as$v=>$W)$Pe[]="<i>".h($W)."</i>".($s["lengths"][$v]?"(".$s["lengths"][$v].")":"").($s["descs"][$v]?" DESC":"");echo"<tr title='".h($A)."'><th>$s[type]<td>".implode(", ",$Pe)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($a).'">'.'Indizes ändern'."</a>\n";if(fk_support($P)){echo"<h3 id='foreign-keys'>".'Fremdschlüssel'."</h3>\n";$vc=foreign_keys($a);if($vc){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Ursprung'."<td>".'Ziel'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($u!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($vc
as$A=>$n){echo"<tr title='".h($A)."'>","<th><i>".implode("</i>, <i>",array_map('h',$n["source"]))."</i>","<td><a href='".h($n["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($n["db"]),ME):($n["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($n["ns"]),ME):ME))."table=".urlencode($n["table"])."'>".($n["db"]!=""?"<b>".h($n["db"])."</b>.":"").($n["ns"]!=""?"<b>".h($n["ns"])."</b>.":"").h($n["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$n["target"]))."</i>)","<td>".nbsp($n["on_delete"])."\n","<td>".nbsp($n["on_update"])."\n",($u=="sqlite"?"":'<td><a href="'.h(ME.'foreign='.urlencode($a).'&name='.urlencode($A)).'">'.'Ändern'.'</a>');}echo"</table>\n";}if($u!="sqlite")echo'<p><a href="'.h(ME).'foreign='.urlencode($a).'">'.'Fremdschlüssel hinzufügen'."</a>\n";}if(support("trigger")){echo"<h3 id='triggers'>".'Trigger'."</h3>\n";$zg=triggers($a);if($zg){echo"<table cellspacing='0'>\n";foreach($zg
as$v=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($v)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($v))."'>".'Ändern'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Xf=array();$Yf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$_d,PREG_SET_ORDER);foreach($_d
...
as$A=>$l){if(!isset($l["privileges"][$Ig?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$y=$_POST["referer"];if($_POST["insert"])$y=($Ig?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$y))$y=ME."select=".urlencode($a);$t=indexes($a);$Dg=unique_array($_GET["where"],$t);$Xe="\nWHERE $Z";if(isset($_POST["delete"])){$F="FROM ".table($a);query_redirect("DELETE".($Dg?" $F$Xe":limit1($F,$Xe)),$y,'Datensatz gelöscht.');}else{$M=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null)$M[idf_escape($A)]=($Ig?"\n".idf_escape($A)." = $W":$W);}if($Ig){if(!$M)redirect($y);$F=table($a)." SET".implode(",",$M);query_redirect("UPDATE".($Dg?" $F$Xe":limit1($F,$Xe)),$y,'Datensatz geändert.');}else{$G=insert_into($a,$M);$od=($G?last_id():0);queries_redirect($y,sprintf('Datensatz%s hinzugefügt.',($od?" $od":"")),$G);}}}$Vf=$b->tableName(table_status1($a,true));page_header(($Ig?'Ändern':'Hinzufügen'),$k,array("select"=>array($a,$Vf)),$Vf);$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Ig?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}echo($Ig?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($Ub?html_select("Engine",array(""=>"(".'Motor'.")")+$Ub,$I["Engine"]):""),' ',($Wa&&!ereg("sqlite|mssql",$u)?html_select("Collation",array(""=>"(".'Kollation'.")")+$Wa,$I["Collation"]):""),' <input type="submit" value="Speichern">
...
',checkbox("defaults",1,true,'Vorgabewerte festlegen',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($bb?" checked":"").">".'Kommentar'."</label>".' <input name="Comment" id="Comment" value="'.h($I["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($bb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Speichern">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if(support("partitioning")){$Ae=ereg('RANGE|LIST',$I["partition_by"]);print_fieldset("partition",'Partitionieren um',$I["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$_e,$I["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($I["partition"]),'">)
Partitionen: <input type="number" name="partitions" class="size" value="',h($I["partitions"]),'"',($Ae||!$I["partition_by"]?" class='hidden'":""),'>
...
';$fd=1;foreach($I["indexes"]as$s){echo"<tr><td>".html_select("indexes[$fd][type]",array(-1=>"")+$Sc,$s["type"],($fd==count($I["indexes"])?"indexesAddRow(this);":1))."<td>";ksort($s["columns"]);$p=1;foreach($s["columns"]as$v=>$e){echo"<span>".html_select("indexes[$fd][columns][$p]",array(-1=>"")+$m,$e,($p==count($s["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($u=="sql"?"":$_GET["indexes"]."_")."');"),($u=="sql"||$u=="mssql"?"<input type='number' name='indexes[$fd][lengths][$p]' class='size' value='".h($s["lengths"][$v])."'>":""),($u!="sql"?checkbox("indexes[$fd][descs][$p]",1,$s["descs"][$v],'absteigend'):"")," </span>";$p++;}echo"<td><input name='indexes[$fd][name]' value='".h($s["name"])."' autocapitalize='off'>\n";$fd++;}echo'</table>
<p>
<input type="submit" value="Speichern">
<noscript><p><input type="submit" name="add" value="Hinzufügen"></noscript>
<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["database"])){$I=$_POST;if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();$A=trim($I["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Datenbank umbenannt.',rename_database($A,$I["collation"]));}else{$i=explode("\n",str_replace("\r","",$A));$Pf=true;$nd="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$I["collation"]))$Pf=false;$nd=$j;}}queries_redirect(ME."db=".urlencode($nd),'Datenbank erstellt.',$Pf);}}else{if(!$I["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$k,array(),DB);$Wa=collations();$A=DB;if($_POST)$A=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$Wa);elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$Dc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Dc,$z)&&$z[1]){$A=stripcslashes(idf_unescape("`$z[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input name="name" id="name" value="'.h($A).'" maxlength="64" autocapitalize="off">')."\n".($Wa?html_select("collation",array(""=>"(".'Kollation'.")")+$Wa,$I["collation"]):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Speichern">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.1' alt='+' title='".'Hinzufügen'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){$I=$_POST;if($_POST&&!$k){$x=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$x,'Schema wurde gelöscht.');else{$A=trim($I["name"]);$x.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$x,'Neues Schema erstellt.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$x,'Schema geändert.');else
redirect($x);}}page_header($_GET["ns"]!=""?'Schema ändern':'Neues Schema',$k);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
';}if($A!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=$_POST;if($_POST&&!$k){$A=trim($I["name"]);$wa=" AS\n$I[select]";$y=ME."table=".urlencode($A);$_='View geändert.';if(!$_POST["drop"]&&$a==$A&&$u!="sqlite")query_redirect(($u=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($A).$wa,$y,$_);else{$eg=$A."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A).$wa,"DROP VIEW ".table($A),"CREATE VIEW ".table($eg).$wa,"DROP VIEW ".table($eg),($_POST["drop"]?substr(ME,0,-1):$y),'View entfernt.',$_,'View erstellt.',$a,$A);}}if(!$_POST&&$a!=""){$I=view($a);$I["name"]=$a;if(!$k)$k=$g->error;}page_header(($a!=""?'View ändern':'Neue View erstellen'),$k,array("table"=>$a),$a);echo'
<form action="" method="post">
...
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$ad,$I["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Lf,$I["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$I=$_POST;if($_POST&&!$k){$x=substr(ME,0,-1);$A=trim($I["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$x,'Sequenz gelöscht.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$x,'Neue Sequenz erstellt.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$x,'Sequenz geändert.');else
redirect($x);}page_header($fa!=""?'Sequenz ändern'.": ".h($fa):'Neue Sequenz',$k);if(!$I)$I["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="Speichern">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$I=$_POST;if($_POST&&!$k){$x=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$x,'Typ gelöscht.');else
query_redirect("CREATE TYPE ".idf_escape(trim($I["name"]))." $I[as]",$x,'Typ erstellt.');}page_header($ga!=""?'Typ ändern'.": ".h($ga):'Typ erstellen',$k);if(!$I)$I["as"]="AS ";echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$A=$_GET["name"];$yg=trigger_options();$wg=array("INSERT","UPDATE","DELETE");$I=(array)trigger($A)+array("Trigger"=>$a."_bi");if($_POST){if(!$k&&in_array($_POST["Timing"],$yg["Timing"])&&in_array($_POST["Event"],$wg)&&in_array($_POST["Type"],$yg["Type"])){$de=" ON ".table($a);$Fb="DROP TRIGGER ".idf_escape($A).($u=="pgsql"?$de:"");$y=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Fb,$y,'Trigger entfernt.');else{if($A!="")queries($Fb);queries_redirect($y,($A!=""?'Trigger geändert.':'Trigger erstellt.'),queries(create_trigger($de,$_POST)));if($A!="")queries(create_trigger($de,$I+array("Type"=>reset($yg["Type"]))));}}$I=$_POST;}page_header(($A!=""?'Trigger ändern'.": ".h($A):'Trigger hinzufügen'),$k,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$yg["Timing"],$I["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$wg,$I["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$yg["Type"],$I["Type"]),'</table>
...
as$W){if($W[1]!="USAGE")$Ec["$z[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$I[0]))$Ec["$z[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$I[0],$z))$be=$z[1];}}if($_POST&&!$k){$ce=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $ce",ME."privileges=",'Benutzer entfernt.');else{$Ud=q($_POST["user"])."@".q($_POST["host"]);$De=$_POST["pass"];if($De!=''&&!$_POST["hashed"]){$De=$g->result("SELECT PASSWORD(".q($De).")");$k=!$De;}$lb=false;if(!$k){if($ce!=$Ud){$lb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Ud IDENTIFIED BY PASSWORD ".q($De));$k=!$lb;}elseif($De!=$be)queries("SET PASSWORD FOR $Ud = ".q($De));}if(!$k){$mf=array();foreach($Sd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer' class='help'>".'Rechte'."</a>";$p=0;foreach($Ec
as$Xd=>$Dc){echo'<th>'.($Xd!="*.*"?"<input name='objects[$p]' value='".h($Xd)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$p]' value='*.*' size='10'>*.*");$p++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$gb=>$xb){foreach((array)$Se[$gb]as$Re=>$ab){echo"<tr".odd()."><td".($xb?">$xb<td":" colspan='2'").' lang="en" title="'.h($ab).'">'.h($Re);$p=0;foreach($Ec
as$Xd=>$Dc){$A="'grants[$p][".h(strtoupper($Re))."]'";$X=$Dc[strtoupper($Re)];if($gb=="Server Admin"&&$Xd!=(isset($Ec["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Erlauben'."<option value='0'".($X=="0"?" selected":"").">".'Verbieten'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$kd=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$kd++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$kd),$kd||!$_POST["kill"]);}page_header('Prozessliste',$k);echo'
<form action="" method="post">
...
as$v=>$W)echo"<td>".(($u=="sql"&&$v=="Info"&&ereg("Query|Killed",$I["Command"])&&$W!="")||($u=="pgsql"&&$v=="current_query"&&$W!="<IDLE>")||($u=="oracle"&&$v=="sql_text"&&$W!="")?"<code class='jush-$u'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($I["db"]!=""?"db=".urlencode($I["db"])."&":"")."sql=".urlencode($W)).'">'.'Klonen'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($p+1)."/".sprintf('%d insgesamt',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Anhalten'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$yc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$wc=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($wc['sql']);break;}}if($wc){print_fieldset("export",'Exportieren');$ve=$b->dumpOutput();echo($ve?html_select("output",$ve,$qa["output"])." ":""),html_select("format",$wc,$qa["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$J);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Qb,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$R'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$Kf=isset($_GET["status"]);page_header($Kf?'Status':'Variablen');$Qg=($Kf?show_status():show_variables());if(!$Qg)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Qg
...
as$v=>$W)json_row("sum-$v",number_format($W,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$j=>$W)json_row("tables-".js_escape($j),$W);json_row("");}exit;}else{$bg=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($bg&&!$k&&!$_POST["search"]){$G=true;$_="";if($u=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$G=truncate_tables($_POST["tables"]);$_='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$G=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$_='Tabellen verschoben.';}elseif($_POST["copy"]){$G=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$_='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"])$G=drop_views($_POST["views"]);if($G&&$_POST["tables"])$G=drop_tables($_POST["tables"]);$_='Tabellen wurden entfernt (drop).';}elseif($u!="sql"){$G=($u=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$_='Tables have been optimized.';}elseif(!$_POST["tables"])$_='Keine Tabellen.';elseif($G=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($I=$G->fetch_assoc())$_.="<b>".h($I["Table"])."</b>: ".h($I["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$_,$G);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabellen und Views'."</h3>\n";$ag=tables_list();if(!$ag)echo"<p class='message'>".'Keine Tabellen.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'Datengrösse','<td>'.'Indexgrösse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'Datensätze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";foreach($ag
as$A=>$S){$Sg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Sg?"views[]":"tables[]"),$A,in_array($A,$bg,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Struktur anzeigen'.'">'.h($A).'</a>';if($Sg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'View ändern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Daten auswählen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle ändern'),"Index_length"=>array("indexes",'Indizes ändern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle ändern'),"Rows"=>array("select",'Daten auswählen'),)as$v=>$x)echo($x?"<td align='right'><a href='".h(ME."$x[0]=").urlencode($A)."' id='$v-".h($A)."' title='$x[1]'>?</a>":"<td id='$v-".h($A)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($ag)),"<td>".nbsp($u=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$v)echo"<td align='right' id='sum-$v'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$u)?($u!="sqlite"?"<input type='submit' value='".'Analysieren'."'> ":"")."<input type='submit' name='optimize' value='".'Optimieren'."'> ":"").($u=="sql"?"<input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)").">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$u!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'" autocapitalize="off">')," <input type='submit' name='move' value='".'Verschieben'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";if(support("routine")){echo"<h3 id='routines'>".'Prozeduren'."</h3>\n";$rf=routines();if($rf){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($rf
