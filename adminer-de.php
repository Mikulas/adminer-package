...
confirm($ab="",$bf=false){return" onclick=\"".($bf?"eventStop(event); ":"")."return confirm('".'Sind Sie sicher ?'.($ab?" (' + $ab + ')":"")."');\"";}function
...
upload_error($k){$ed=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Hochladen von Datei fehlgeschlagen.'.($ed?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$ed):""):'Datei existiert nicht.');}function
...
enum_input($S,$xa,$l,$X,$Cb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Zc);$G=(isset($Cb)?"<label><input type='$S'$xa value='$Cb'".((is_array($X)?in_array($Cb,$X):$X===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($Zc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ja=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($q+1)."'".($Ja?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
...
q($W);}function
check_sqlite_name($A){global$g;$Ub="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ub)\$~",$A)){$g->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$Ub));return
false;}return
true;}function
...
set_schema($Le){global$g,$T,$df;$G=$g->query("SET search_path TO ".idf_escape($Le));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$df['Benutzer-definierte Typen'][]=$S;}}return$G;}function
...
reset($I);}function
support($Wb){return
ereg("view|drop_col|variables|status",$Wb);}$v="oracle";$T=array();$df=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binär'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$df[$w]=array_keys($W);}$Rf=array();$Ed=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$lc=array("length","lower","round","upper");$pc=array("avg","count","count distinct","max","min","sum");$yb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ge=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("driver",$sb,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($kf,$L=""){echo'<p class="tabs">';$Uc=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($kf)){$Uc["view"]='View ändern';}else{$Uc["create"]='Tabelle ändern';}if(isset($L)){$Uc["edit"]='Neuer Datensatz';}foreach($Uc
...
selectColumnsPrint($J,$f){global$lc,$pc;print_fieldset("select",'Daten zeigen von',$J);$q=0;$kc=array('Funktionen'=>$lc,'Agregationen'=>$pc);foreach($J
...
false;}function
messageQuery($E){global$v;static$ab=0;restart_session();$r="sql-".($ab++);$rc=&get_session("queries");if(strlen($E)>1e6){$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";}$rc[$_GET["db"]][]=$E;return" <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL-Query'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($rc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
';if($nd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'Schema'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$nd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Neue Tabelle'."</a>\n";$Q=tables_list();if(!$Q){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{$this->tablesPrint($Q);$Uc=array();foreach($Q
...
as$w=>$W){if(!isset($W)){$W="<i>NULL</i>";}elseif($Da[$w]&&!is_utf8($W)){$W="<i>".lang(array('%d Byte','%d Bytes'),strlen($W))."</i>";}elseif(!strlen($W)){$W="&nbsp;";}else{$W=h($W);if($T[$w]==254){$W="<code>$W</code>";}}if(isset($Uc[$w])&&!$f[$Uc[$w]]){if($uc){$z=$Uc[$w].urlencode($H[array_search("table=",$Uc)]);}else{$z="edit=".urlencode($Uc[$w]);foreach($u[$Uc[$w]]as$Ma=>$Hc){$z.="&where".urlencode("[".bracket_escape($Ma)."]")."=".urlencode($H[$Hc]);}}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Keine Daten.')."\n";}function
...
edit_type($w,$l,$Oa,$ec=array()){global$df,$T,$Rf,$Ad;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$df+($ec?array('Fremdschlüssel'=>$ec):array()),$l["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($Oa,$l["collation"]).'</select>',($Rf?"<select name='$w"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Rf,$l["unsigned"]).'</select>':''),($ec?"<select name='$w"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$Ad),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Ta?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Nach unten'."'>&nbsp;":""),($Ld==""||support("drop_col")?"<input type='image' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W){echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Ändern'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$nf=array();$of=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Zc,PREG_SET_ORDER);foreach($Zc
...
as$A=>$l){if(!isset($l["privileges"][$Sf?"update":"insert"])||$b->fieldName($l)==""){unset($m[$A]);}}if($_POST&&!$k&&!isset($_GET["select"])){$Vc=$_POST["referer"];if($_POST["insert"]){$Vc=($Sf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$Vc)){$Vc=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$Vc,'Datensatz gelöscht.');}else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null){$L[idf_escape($A)]=($Sf?"\n".idf_escape($A)." = $W":$W);}}if($Sf){if(!$L){redirect($Vc);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$Vc,'Datensatz geändert.');}else{$F=insert_into($a,$L);$Oc=($F?last_id():0);queries_redirect($Vc,sprintf('Datensatz%s hinzugefügt.',($Oc?" $Oc":"")),$F);}}}$lf=$b->tableName(table_status($a));page_header(($Sf?'Ändern':'Hinzufügen'),$k,array("select"=>array($a,$lf)),$lf);$H=null;if($_POST["save"]){$H=(array)$_POST["fields"];}elseif($Z){$J=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Speichern'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Sf?'Speichern und weiter bearbeiten':'Speichern und nächsten hinzufügen')."' title='Ctrl+Shift+Enter'>\n";}}echo($Sf?"<input type='submit' name='delete' value='".'Entfernen'."' onclick=\"return confirm('".'Sind Sie sicher ?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Wd=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Partitionieren um',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Vd,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Partitionen: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($Wd||!$H["partition_by"]?" class='hidden'":""),'>
...
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"])){$ef=false;}$Nc=$j;}}queries_redirect(ME."db=".urlencode($Nc),'Datenbank erstellt.',$ef);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$k,array(),DB);$Oa=collations();$A=DB;$Na=null;if($_POST){$A=$_POST["name"];$Na=$_POST["collation"];}elseif(DB!=""){$Na=db_collation(DB,$Oa);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<tr><th>Jede<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ec,$H["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$af,$H["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($H["name"]),'" maxlength="64">
',($Ge?'Sprache'.": ".html_select("language",$Ge,$H["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($H["fields"],$Oa,$Fe);if(isset($_GET["function"])){echo"<tr><td>".'Typ des Rückgabewertes';edit_type("returns",$H["returns"],$Oa);}echo'</table>
<p>';textarea("definition",$H["definition"]);echo'<p>
<input type="submit" value="Speichern">
';if($da!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($ub){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$z=substr(ME,0,-1);$A=trim($_POST["name"]);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sequenz gelöscht.');}elseif($fa==""){query_redirect("CREATE SEQUENCE ".idf_escape($A),$z,'Neue Sequenz erstellt.');}elseif($fa!=$A){query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$z,'Sequenz geändert.');}else{redirect($z);}}page_header($fa!=""?'Sequenz ändern'.": ".h($fa):'Neue Sequenz',$k);$H=$_POST;if(!$H){$H=array("name"=>$fa);}echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Speichern">
';if($fa!=""){echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ gelöscht.');}else{query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$z,'Typ erstellt.');}}page_header($ga!=""?'Typ ändern'.": ".h($ga):'Typ erstellen',$k);$H=$_POST;if(!$H){$H=array("as"=>"AS ");}echo'
<form action="" method="post">
<p>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$Hf["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$Gf,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Hf["Type"],$H["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Speichern">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if($ub){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$W){if($W[1]!="USAGE"){$mc["$_[2]$W[2]"][$W[1]]=true;}if(ereg(' WITH GRANT OPTION',$H[0])){$mc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_)){$yd=$_[1];}}}if($_POST&&!$k){$zd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$td=q($_POST["user"])."@".q($_POST["host"]);$Zd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $zd",ME."privileges=",'Benutzer entfernt.');}else{$db=false;if($zd!=$td){$db=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $td IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Zd");$k=!$db;}elseif($_POST["pass"]!=$yd||!$_POST["hashed"]){queries("SET PASSWORD FOR $td = ".($_POST["hashed"]?$Zd:"PASSWORD($Zd)"));}if(!$k){$Ce=array();foreach($sd
...
as$wd=>$Ce){if(preg_match('~^(.+)(\\(.*\\))?$~U',$wd,$_)){grant("REVOKE",array_keys($Ce),$_[2]," ON $_[1] FROM $td");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Benutzer geändert.':'Benutzer erstellt.'),!$k);if($db){$g->query("DROP USER $td");}}}page_header((isset($_GET["host"])?'Benutzer'.": ".h("$ha@$_GET[host]"):'Neuer Benutzer'),$k,array("privileges"=>array('','Rechte')));if($_POST){$H=$_POST;$mc=$sd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$yd;if($yd!=""){$H["hashed"]=true;}$mc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Rechte'."</a>";$q=0;foreach($mc
as$wd=>$p){echo'<th>'.($wd!="*.*"?"<input name='objects[$q]' value='".h($wd)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$Ya=>$nb){foreach((array)$ne[$Ya]as$me=>$Sa){echo"<tr".odd()."><td".($nb?">$nb<td":" colspan='2'").' lang="en" title="'.h($Sa).'">'.h($me);$q=0;foreach($mc
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Kc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W))){$Kc++;}}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$Kc),$Kc||!$_POST["kill"]);}page_header('Prozessliste',$k);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Entfernen" onclick="return confirm('Sind Sie sicher ? (' + (this.form['all'].checked ? <?php echo$hc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$fc=$b->dumpFormat();if($fc){print_fieldset("export",'Exportieren');$Qd=$b->dumpOutput();echo($Qd?html_select("output",$Qd,$qa["output"])." ":""),html_select("format",$fc,$qa["format"])," <input type='submit' name='export' value='".'Exportieren'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$I);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","<input type='hidden' name='token' value='$R'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Bb,'strlen'),$f);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ze=isset($_GET["status"]);page_header($Ze?'Status':'Variablen');$Xf=($Ze?show_status():show_variables());if(!$Xf){echo"<p class='message'>".'Keine Daten.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Xf
...
as$w=>$W){json_row("sum-$w",number_format($W,0,'.',' '));}json_row("");}else{foreach(count_tables($b->databases())as$j=>$W){json_row("tables-".js_escape($j),$W);}json_row("");}exit;}else{$rf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($rf&&!$k&&!$_POST["search"]){$F=true;$gd="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$F=truncate_tables($_POST["tables"]);}$gd='Tabellen sind entleert worden (truncate).';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$gd='Tabellen verschoben.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$gd='Tabellen wurden kopiert.';}elseif($_POST["drop"]){if($_POST["views"]){$F=drop_views($_POST["views"]);}if($F&&$_POST["tables"]){$F=drop_tables($_POST["tables"]);}$gd='Tabellen wurden entfernt (drop).';}elseif($v!="sql"){$F=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$gd='Tables have been optimized.';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc()){$gd.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$gd,$F);}page_header(($_GET["ns"]==""?'Datenbank'.": ".h(DB):'Schema'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabellen und Views'."</h3>\n";$qf=tables_list();if(!$qf){echo"<p class='message'>".'Keine Tabellen.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Suche in Tabellen'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Suchen'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabelle','<td>'.'Motor','<td>'.'Collation','<td>'.'Datengrösse','<td>'.'Indexgrösse','<td>'.'Freier Bereich','<td>'.'Auto-Inkrement','<td>'.'Datensätze',(support("comment")?'<td>'.'Kommentar':''),"</thead>\n";foreach($qf
as$A=>$S){$Zf=(isset($S)&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Zf?"views[]":"tables[]"),$A,in_array($A,$rf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Struktur anzeigen'.'">'.h($A).'</a>';if($Zf){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'View ändern'.'">'.'View'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Daten auswählen'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Tabelle ändern'),"Index_length"=>array("indexes",'Indizes ändern'),"Data_free"=>array("edit",'Neuer Datensatz'),"Auto_increment"=>array("auto_increment=1&create",'Tabelle ändern'),"Rows"=>array("select",'Daten auswählen'),)as$w=>$z){echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($A)."' id='$w-".h($A)."' title='$z[1]'>?</a>":"<td id='$w-".h($A)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d insgesamt',count($qf)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w){echo"<td align='right' id='sum-$w'>&nbsp;";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$v)?($v!="sqlite"?"<input type='submit' value='".'Analysieren'."'> ":"")."<input type='submit' name='optimize' value='".'Optimieren'."'> ":"").($v=="sql"?"<input type='submit' name='check' value='".'Prüfen'."'> <input type='submit' name='repair' value='".'Reparieren'."'> ":"")."<input type='submit' name='truncate' value='".'Entleeren (truncate)'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Entfernen'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$v!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'In andere Datenbank verschieben'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'">')," <input type='submit' name='move' value='".'Verschieben'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopieren'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Neue Tabelle erstellen'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Neue View erstellen'."</a>\n";}if(support("routine")){echo"<h3>".'Prozeduren'."</h3>\n";$He=routines();if($He){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Name'.'<td>'.'Typ'.'<td>'.'Typ des Rückgabewertes'."<td>&nbsp;</thead>\n";odd('');foreach($He
