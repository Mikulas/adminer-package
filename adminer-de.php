...
upload_error($l){$_d=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Hochladen von Datei fehlgeschlagen.'.($_d?" ".sprintf('Maximal erlaubte Dateigrösse ist %sB.',$_d):""):'Datei existiert nicht.');}function
...
enum_input($S,$ya,$m,$X,$Nb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$m["length"],$vd);$H=($Nb!==null?"<label><input type='$S'$ya value='$Nb'".((is_array($X)?in_array($Nb,$X):$X===0)?" checked":"")."><i>".'leer'."</i></label>":"");foreach($vd[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Na=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$H.=" <label><input type='$S'$ya value='".($q+1)."'".($Na?' checked':'').'>'.h($b->editVal($W,$m)).'</label>';}return$H;}function
...
q($W);}function
check_sqlite_name($A){global$h;$fc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($fc)\$~",$A)){$h->error=sprintf('Bitte einen der Dateitypen %s benutzen.',str_replace("|",", ",$fc));return
false;}return
true;}function
...
set_schema($rf){global$h,$T,$If;$H=$h->query("SET search_path TO ".idf_escape($rf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$If['Benutzer-definierte Typen'][]=$S;}}return$H;}function
...
unconvert_field($m,$H){return$H;}function
support($ic){return
ereg("view|scheme|processlist|drop_col|variables|status",$ic);}$v="oracle";$T=array();$If=array();foreach(array('Zahlen'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum oder Zeit'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Zeichenketten'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binär'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$If[$w]=array_keys($W);}$Cg=array();$ee=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$zc=array("length","lower","round","upper");$Dc=array("avg","count","count distinct","max","min","sum");$Hb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ab["mssql"]="MS SQL";if(isset($_GET["mssql"])){$He=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Datenbank System<td>',html_select("auth[driver]",$Ab,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Pf,$M=""){echo'<p class="tabs">';$rd=array("select"=>'Daten auswählen',"table"=>'Struktur anzeigen');if(is_view($Pf))$rd["view"]='View ändern';else$rd["create"]='Tabelle ändern';if($M!==null)$rd["edit"]='Neuer Datensatz';foreach($rd
...
selectColumnsPrint($K,$g){global$zc,$Dc;print_fieldset("select",'Daten zeigen von',$K);$q=0;$xc=array('Funktionen'=>$zc,'Agregationen'=>$Dc);foreach($K
...
selectLengthPrint($cg){if($cg!==null){echo"<fieldset><legend>".'Textlänge'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($cg)."'>","</div></fieldset>\n";}}function
...
messageQuery($F){global$v;restart_session();$Gc=&get_session("queries");$r="sql-".count($Gc[$_GET["db"]]);if(strlen($F)>1e6)$F=ereg_replace('[\x80-\xFF]+$','',substr($F,0,1e6))."\n...";$Gc[$_GET["db"]][]=array($F,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL-Query'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($F,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Gc[$_GET["db"]])-1)).'">'.'Ändern'.'</a></div>';}function
...
as$O=>$Ff){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O).">".'zeigen'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold($_GET["table"]==$O)." title='".'Struktur anzeigen'."'>".$this->tableName($Ff)."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$w=>$W){if($W===null)$W="<i>NULL</i>";elseif($Ga[$w]&&!is_utf8($W))$W="<i>".lang(array('%d Byte','%d Bytes'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$w]==254)$W="<code>$W</code>";}if(isset($rd[$w])&&!$g[$rd[$w]]){if($Jc){$O=$I[array_search("table=",$rd)];$y=$rd[$w].urlencode($le[$O]!=""?$le[$O]:$O);}else{$y="edit=".urlencode($rd[$w]);foreach($u[$rd[$w]]as$Sa=>$Zc)$y.="&where".urlencode("[".bracket_escape($Sa)."]")."=".urlencode($I[$Zc]);}$W="<a href='".h(ME.$y)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Keine Daten.')."\n";return$H;}function
...
edit_type($w,$m,$Ua,$rc=array()){global$If,$T,$Cg,$ae;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$m["type"]||isset($T[$m["type"]])?array():array($m["type"]))+$If+($rc?array('Fremdschlüssel'=>$rc):array()),$m["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$m["type"])?"":" class='hidden'").'><option value="">('.'Kollation'.')'.optionlist($Ua,$m["collation"]).'</select>',($Cg?"<select name='$w"."[unsigned]'".(!$m["type"]||ereg('((^|[^o])int|float|double|decimal)$',$m["type"])?"":" class='hidden'").'><option>'.optionlist($Cg,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='$w"."[on_update]'".($m["type"]=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'ON UPDATE'.")","CURRENT_TIMESTAMP"),$m["on_update"]).'</select>':''),($rc?"<select name='$w"."[on_delete]'".(ereg("`",$m["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$ae),$m["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Za?"":" class='hidden'").">".'Kommentar':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.0' alt='+' title='".'Hinzufügen'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($Za?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.0' alt='+' title='".'Hinzufügen'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.7.0' alt='^' title='".'Nach oben'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.7.0' alt='v' title='".'Nach unten'."'>&nbsp;":""),($me==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.7.0' alt='x' title='".'Entfernen'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Ändern'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Trigger hinzufügen'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Datenbankschema',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Sf=array();$Tf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$vd,PREG_SET_ORDER);foreach($vd
...
as$A=>$m){if(!isset($m["privileges"][$Dg?"update":"insert"])||$b->fieldName($m)=="")unset($n[$A]);}if($_POST&&!$l&&!isset($_GET["select"])){$z=$_POST["referer"];if($_POST["insert"])$z=($Dg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$z))$z=ME."select=".urlencode($a);$u=indexes($a);$zg=unique_array($_GET["where"],$u);$Ue="\nWHERE $Z";if(isset($_POST["delete"])){$F="FROM ".table($a);query_redirect("DELETE".($zg?" $F$Ue":limit1($F,$Ue)),$z,'Datensatz gelöscht.');}else{$M=array();foreach($n
as$A=>$m){$W=process_input($m);if($W!==false&&$W!==null)$M[idf_escape($A)]=($Dg?"\n".idf_escape($A)." = $W":$W);}if($Dg){if(!$M)redirect($z);$F=table($a)." SET".implode(",",$M);query_redirect("UPDATE".($zg?" $F$Ue":limit1($F,$Ue)),$z,'Datensatz geändert.');}else{$G=insert_into($a,$M);$jd=($G?last_id():0);queries_redirect($z,sprintf('Datensatz%s hinzugefügt.',($jd?" $jd":"")),$G);}}}$Qf=$b->tableName(table_status($a,true));page_header(($Dg?'Ändern':'Hinzufügen'),$l,array("select"=>array($a,$Qf)),$Qf);$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($n
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($Qb?html_select("Engine",array(""=>"(".'Motor'.")")+$Qb,$I["Engine"]):""),' ',($Ua&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'Kollation'.")")+$Ua,$I["Collation"]):""),' <input type="submit" value="Speichern">
...
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Entfernen"',confirm(),'>';}if(support("partitioning")){$ye=ereg('RANGE|LIST',$I["partition_by"]);print_fieldset("partition",'Partitionieren um',$I["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$xe,$I["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($I["partition"]),'">)
Partitionen: <input type="number" name="partitions" class="size" value="',h($I["partitions"]),'"',($ye||!$I["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){$I=$_POST;if($_POST&&!$l&&!isset($_POST["add_x"])){restart_session();$A=trim($I["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Datenbank entfernt.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Datenbank umbenannt.',rename_database($A,$I["collation"]));}else{$j=explode("\n",str_replace("\r","",$A));$Kf=true;$id="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$I["collation"]))$Kf=false;$id=$k;}}queries_redirect(ME."db=".urlencode($id),'Datenbank erstellt.',$Kf);}}else{if(!$I["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Datenbank geändert.');}}page_header(DB!=""?'Datenbank ändern':'Neue Datenbank',$l,array(),DB);$Ua=collations();$A=DB;if($_POST)$A=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$Ua);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$_c){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$_c,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input name="name" id="name" value="'.h($A).'" maxlength="64" autocapitalize="off">')."\n".($Ua?html_select("collation",array(""=>"(".'Kollation'.")")+$Ua,$I["collation"]):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Speichern">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.0' alt='+' title='".'Hinzufügen'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){$I=$_POST;if($_POST&&!$l){$y=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$y,'Schema wurde gelöscht.');else{$A=trim($I["name"]);$y.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$y,'Neues Schema erstellt.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$y,'Schema geändert.');else
redirect($y);}}page_header($_GET["ns"]!=""?'Schema ändern':'Neues Schema',$l);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=$_POST;if($_POST&&!$l){$A=trim($I["name"]);$wa=" AS\n$I[select]";$z=ME."table=".urlencode($A);$Cd='View geändert.';if(!$_POST["drop"]&&$a==$A&&$v!="sqlite")query_redirect(($v=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($A).$wa,$z,$Cd);else{$Zf=$A."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A).$wa,"DROP VIEW ".table($A),"CREATE VIEW ".table($Zf).$wa,"DROP VIEW ".table($Zf),($_POST["drop"]?substr(ME,0,-1):$z),'View entfernt.',$Cd,'View erstellt.',$a,$A);}}page_header(($a!=""?'View ändern':'Neue View erstellen'),$l,array("table"=>$a),$a);if(!$_POST&&$a!=""){$I=view($a);$I["name"]=$a;}echo'
<form action="" method="post">
...
<tr><th>Jede<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Uc,$I["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Gf,$I["STATUS"]),'<tr><th>Kommentar<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$I=$_POST;if($_POST&&!$l){$y=substr(ME,0,-1);$A=trim($I["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$y,'Sequenz gelöscht.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$y,'Neue Sequenz erstellt.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$y,'Sequenz geändert.');else
redirect($y);}page_header($fa!=""?'Sequenz ändern'.": ".h($fa):'Neue Sequenz',$l);if(!$I)$I["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="Speichern">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Entfernen'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$I=$_POST;if($_POST&&!$l){$y=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$y,'Typ gelöscht.');else
query_redirect("CREATE TYPE ".idf_escape(trim($I["name"]))." $I[as]",$y,'Typ erstellt.');}page_header($ga!=""?'Typ ändern'.": ".h($ga):'Typ erstellen',$l);if(!$I)$I["as"]="AS ";echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$A=$_GET["name"];$tg=trigger_options();$rg=array("INSERT","UPDATE","DELETE");$I=(array)trigger($A)+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$tg["Timing"])&&in_array($_POST["Event"],$rg)&&in_array($_POST["Type"],$tg["Type"])){$Zd=" ON ".table($a);$Bb="DROP TRIGGER ".idf_escape($A).($v=="pgsql"?$Zd:"");$z=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Bb,$z,'Trigger entfernt.');else{if($A!="")queries($Bb);queries_redirect($z,($A!=""?'Trigger geändert.':'Trigger erstellt.'),queries(create_trigger($Zd,$_POST)));if($A!="")queries(create_trigger($Zd,$I+array("Type"=>reset($tg["Type"]))));}}$I=$_POST;}page_header(($A!=""?'Trigger ändern'.": ".h($A):'Trigger hinzufügen'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Zeitpunkt<td>',html_select("Timing",$tg["Timing"],$I["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Ereignis<td>',html_select("Event",$rg,$I["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$tg["Type"],$I["Type"]),'</table>
...
as$W){if($W[1]!="USAGE")$Ac["$_[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$I[0]))$Ac["$_[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$I[0],$_))$Xd=$_[1];}}if($_POST&&!$l){$Yd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Yd",ME."privileges=",'Benutzer entfernt.');else{$Qd=q($_POST["user"])."@".q($_POST["host"]);$Ae=$_POST["pass"];if($Ae!=''&&!$_POST["hashed"]){$Ae=$h->result("SELECT PASSWORD(".q($Ae).")");$l=!$Ae;}$jb=false;if(!$l){if($Yd!=$Qd){$jb=queries(($h->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Qd IDENTIFIED BY PASSWORD ".q($Ae));$l=!$jb;}elseif($Ae!=$Xd)queries("SET PASSWORD FOR $Qd = ".q($Ae));}if(!$l){$if=array();foreach($Od
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer' class='help'>".'Rechte'."</a>";$q=0;foreach($Ac
as$Td=>$_c){echo'<th>'.($Td!="*.*"?"<input name='objects[$q]' value='".h($Td)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Datenbank',"Tables"=>'Tabelle',"Columns"=>'Spalte',"Procedures"=>'Rutine',)as$eb=>$ub){foreach((array)$Pe[$eb]as$Oe=>$Ya){echo"<tr".odd()."><td".($ub?">$ub<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($Oe);$q=0;foreach($Ac
as$Td=>$_c){$A="'grants[$q][".h(strtoupper($Oe))."]'";$X=$_c[strtoupper($Oe)];if($eb=="Server Admin"&&$Td!=(isset($Ac["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Erlauben'."<option value='0'".($X=="0"?" selected":"").">".'Verbieten'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$fd=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$fd++;}queries_redirect(ME."processlist=",lang(array('%d Prozess gestoppt.','%d Prozesse gestoppt.'),$fd),$fd||!$_POST["kill"]);}page_header('Prozessliste',$l);echo'
<form action="" method="post">
...
';}$sc=$b->dumpFormat();foreach((array)$_GET["columns"]as$f){if($f["fun"]){unset($sc['sql']);break;}}if($sc){print_fieldset("export",'Exportieren');$se=$b->dumpOutput();echo($se?html_select("output",$se,$qa["output"])." ":""),html_select("format",$sc,$qa["format"])," <input type='submit' name='export' value='".'Exportieren'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Importieren',!$J);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Importieren'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Mb,'strlen'),$g);echo"<p><input type='hidden' name='token' value='$R'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$Ff=isset($_GET["status"]);page_header($Ff?'Status':'Variablen');$Lg=($Ff?show_status():show_variables());if(!$Lg)echo"<p class='message'>".'Keine Daten.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Lg
