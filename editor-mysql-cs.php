...
confirm($tc="",$Ld=false){return" onclick=\"".($Ld?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($tc?" (' + $tc + ')":"")."');\"";}function
js_escape($F){return
addcslashes($F,"\r\n'\\/");}function
...
upload_error($v){$Pc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Nepodařilo se nahrát soubor.'.($Pc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Pc):""):'Soubor neexistuje.');}function
...
enum_input($ta,$L,$e,$i,$Ib=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$T);$b=(isset($Ib)?"<label><input type='$ta'$L value='$Ib'".((is_array($i)?in_array($Ib,$i):$i===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($T[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Za=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ta'$L value='".($n+1)."'".($Za?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$x){global$ya,$j,$ma;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$Ec=($ma=="mssql"&&$e["auto_increment"]);if($Ec&&!$_POST["save"]){$x=null;}$ba=(isset($_GET["select"])||$Ec?array("orig"=>'původní'):array())+$j->editFunctions($e);$L=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
selectVal($a,$s,$e){$b=($a=="<i>NULL</i>"?"&nbsp;":$a);if(ereg('blob|bytea',$e["type"])&&!is_utf8($a)){$b=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($a));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$a)){$b="<img src='$s' alt='$b'>";}}if(like_bool($e)&&$b!="&nbsp;"){$b='<img src="'.($a?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.0").'" alt="'.h($a).'">';}if($s){$b="<a href='$s'>$b</a>";}if(!$s&&!like_bool($e)&&ereg('int|float|double|decimal',$e["type"])){$b="<div class='number'>$b</div>";}elseif(ereg('date',$e["type"])){$b="<div class='datetime'>$b</div>";}return$b;}function
...
as$f=>$pa){$Y=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Y)){if($q[$f]["null"]){$Y[0]='('.'prázdné'.')';}$c=$Fb[$f];$n--;echo"<div>".h($pa)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Y,$p[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($p
...
as$c=>$R){$V=array();foreach($R["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Gb[$c]=implode(", ",$V);}}if($Gb){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Gb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
';if($Qa!="db"&&$Qa!="ns"){$C=table_status();if(!$C){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($C);}}}}function
...
like_bool($e){return
ereg("bool|(tinyint|bit)\\(1\\)",$e["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$ob=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$sa[DRIVER]='Přihlásit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$r=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$r-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($r)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$r=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$aa=(isset($_GET["select"])?$_POST["edit"]:$p);$q=fields($r);foreach($q
as$f=>$e){if(!isset($e["privileges"][$aa?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$I=$_POST["referer"];if($_POST["insert"]){$I=($aa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$I)){$I=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$I,'Položka byla smazána.');}else{$t=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($aa?"\n".idf_escape($f)." = $a":$a);}}if($aa){if(!$t){redirect($I);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$t),"\nWHERE $p"),$I,'Položka byla aktualizována.');}else{$k=insert_into($r,$t);$dc=($k?last_id():0);queries_redirect($I,sprintf('Položka%s byla vložena.',($dc?" $dc":"")),$k);}}}$Na=$j->tableName(table_status($r));page_header(($aa?'Upravit':'Vložit'),$v,array("select"=>array($r,$Na)),$Na);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$u=array();foreach($q
...
';if($q){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($aa?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}}echo($aa?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$q?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
<input type="submit" name="delete" value="Smazat" onclick="return confirm('Opravdu? (' + (this.form['all'].checked ? <?php echo$ga,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
