...
confirm($Ja="",$jd=false){return" onclick=\"".($jd?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Ja?" (' + $Ja + ')":"")."');\"";}function
...
upload_error($h){$kc=($h==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($h?'Nepodařilo se nahrát soubor.'.($kc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$kc):""):'Soubor neexistuje.');}function
...
input($i,$W,$l){global$Fd,$b,$Xb;$A=h(bracket_escape($i["field"]));echo"<td class='function'>";$Vc=($Xb=="mssql"&&$i["auto_increment"]);if($Vc&&!$_POST["save"]){$l=null;}$m=(isset($_GET["select"])||$Vc?array("orig"=>'původní'):array())+$b->editFunctions($i);$c=" name='fields[$A]'";if($i["type"]=="enum"){echo
...
selectVal($V,$v,$i){$I=($V=="<i>NULL</i>"?"&nbsp;":$V);if(ereg('blob|bytea',$i["type"])&&!is_utf8($V)){$I=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V)){$I="<img src='$v' alt='$I'>";}}if(like_bool($i)&&$I!="&nbsp;"){$I=($V?'ano':'ne');}if($v){$I="<a href='$v'>$I</a>";}if(!$v&&!like_bool($i)&&ereg('int|float|double|decimal',$i["type"])){$I="<div class='number'>$I</div>";}elseif(ereg('date',$i["type"])){$I="<div class='datetime'>$I</div>";}return$I;}function
...
selectSearchPrint($X,$e,$r){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhledat'."</legend><div>\n";$Yb=array();foreach($X
as$t=>$V){$Yb[$V["col"]]=$t;}$o=0;$j=fields($_GET["select"]);foreach($e
as$A=>$Ta){$i=$j[$A];if(ereg("enum",$i["type"])||like_bool($i)){$t=$Yb[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'>:",(like_bool($i)?" <select name='where[$o][val]'>".optionlist(array(""=>"",'ne','ano'),$X[$t]["val"],true)."</select>":enum_input("checkbox"," name='where[$o][val][]'",$i,(array)$X[$t]["val"],($i["null"]?0:null))),"</div>\n";unset($e[$A]);}elseif(is_array($_c=$this->_foreignKeyOptions($_GET["select"],$A))){if($j[$A]["null"]){$_c[0]='('.'prázdné'.')';}$t=$Yb[$A];$o--;echo"<div>".h($Ta)."<input type='hidden' name='where[$o][col]' value='".h($A)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($_c,$X[$t]["val"],true)."</select></div>\n";unset($e[$A]);}}$o=0;foreach($X
as$V){if(($V["col"]==""||$e[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'kdekoliv'.")".optionlist($e,$V["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$o][val]' value='".h($V["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoliv'.")".optionlist($e,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$e,$r){$Bc=array();foreach($r
as$t=>$q){$B=array();foreach($q["columns"]as$V){$B[]=$e[$V];}if(count(array_filter($B,'strlen'))>1&&$t!="PRIMARY"){$Bc[$t]=implode(", ",$B);}}if($Bc){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Bc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
';if($nc!="db"&&$nc!="ns"){$R=table_status();if(!$R){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($R);}}}}function
...
like_bool($i){return
ereg("bool|(tinyint|bit)\\(1\\)",$i["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$uc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$Ya[DRIVER]='Přihlásit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$a=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$f->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$Nd=(isset($_GET["select"])?$_POST["edit"]:$X);$j=fields($a);foreach($j
as$A=>$i){if(!isset($i["privileges"][$Nd?"update":"insert"])||$b->fieldName($i)==""){unset($j[$A]);}}if($_POST&&!$h&&!isset($_GET["select"])){$w=$_POST["referer"];if($_POST["insert"]){$w=($Nd?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$w)){$w=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$w,'Položka byla smazána.');}else{$N=array();foreach($j
as$A=>$i){$V=process_input($i);if($V!==false&&$V!==null){$N[idf_escape($A)]=($Nd?"\n".idf_escape($A)." = $V":$V);}}if($Nd){if(!$N){redirect($w);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$w,'Položka byla aktualizována.');}else{$H=insert_into($a,$N);$cc=($H?last_id():0);queries_redirect($w,sprintf('Položka%s byla vložena.',($cc?" $cc":"")),$H);}}}$rd=$b->tableName(table_status($a));page_header(($Nd?'Upravit':'Vložit'),$h,array("select"=>array($a,$rd)),$rd);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($X){$L=array();foreach($j
as$A=>$i){if(isset($i["privileges"]["select"])){$L[]=($_POST["clone"]&&$i["auto_increment"]?"'' AS ":(ereg("enum|set",$i["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Žádné řádky.'."\n";}echo'
...
';if($j){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Nd?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}}echo($Nd?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$j?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
