...
upload_error($k){$Ec=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Nepodařilo se nahrát soubor.'.($Ec?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Ec):""):'Soubor neexistuje.');}function
...
lang($qe,$Oc=null){if(is_array($qe)){$id=($Oc==1?0:($Oc&&$Oc<5?1:2));$qe=$qe[$id];}$qe=str_replace("%d","%s",$qe);$Oc=format_number($Oc);return
...
selectLinks($ae,$Q=""){$a=$ae["Name"];if($Q!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$Q).'">'.'Nová položka'."</a>\n";}function
...
as$d=>$W)$z.="&set".urlencode("[".bracket_escape($d)."]")."=".urlencode($M[$W]);echo"<a href='".h($z)."' title='".'Nová položka'."'>+</a> ";}}}function
...
selectVal($W,$z,$l,$ad){$L=($W===null?"&nbsp;":$W);$z=h($z);if(preg_match('~blob|bytea~',$l["type"])&&!is_utf8($W)){$L=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($ad));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$ad))$L="<img src='$z' alt='$L'>";}if(like_bool($l)&&$L!="&nbsp;")$L=($W?'ano':'ne');if($z)$L="<a href='$z'".(is_url($z)?" rel='noreferrer'":"").">$L</a>";if(!$z&&!like_bool($l)&&preg_match('~int|float|double|decimal~',$l["type"]))$L="<div class='number'>$L</div>";elseif(preg_match('~date~',$l["type"]))$L="<div class='datetime'>$L</div>";return$L;}function
...
selectSearchPrint($Z,$e,$t){$Z=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Vyhledat'."</legend><div>\n";$qc=array();foreach($Z
as$x=>$W)$qc[$W["col"]]=$x;$q=0;$m=fields($_GET["select"]);foreach($e
as$D=>$db){$l=$m[$D];if(preg_match("~enum~",$l["type"])||like_bool($l)){$x=$qc[$D];$q--;echo"<div>".h($db)."<input type='hidden' name='where[$q][col]' value='".h($D)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'ne','ano'),$Z[$x]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$Z[$x]["val"],($l["null"]?0:null))),"</div>\n";unset($e[$D]);}elseif(is_array($Xc=$this->_foreignKeyOptions($_GET["select"],$D))){if($m[$D]["null"])$Xc[0]='('.'prázdné'.')';$x=$qc[$D];$q--;echo"<div>".h($db)."<input type='hidden' name='where[$q][col]' value='".h($D)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Xc,$Z[$x]["val"],true)."</select></div>\n";unset($e[$D]);}}$q=0;foreach($Z
...
as$x=>$s){$E=array();foreach($s["columns"]as$W)$E[]=$e[$W];if(count(array_filter($E,'strlen'))>1&&$x!="PRIMARY")$Zc[$x]=implode(", ",$E);}if($Zc){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Zc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
true;}function
selectEmailPrint($qb,$e){if($qb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Odesílatel'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Předmět'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$e,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložit'."'>\n";echo"<p>".'Přílohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($qb)==1?'<input type="hidden" name="email_field" value="'.h(key($qb)).'">':html_select("email_field",$qb)),"<input type='submit' name='email' value='".'Odeslat'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
...
';if($Ic=="auth"){$Ib=true;foreach((array)$_SESSION["pwds"]as$Y=>$Md){foreach($Md[""]as$U=>$H){if($H!==null){if($Ib){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Ib=false;}echo"<a href='".h(auth_url($Y,"",$U))."'>".($U!=""?h($U):"<i>".'prázdné'."</i>")."</a><br>\n";}}}}else{$this->databasesPrint($Ic);if($Ic!="db"&&$Ic!="ns"){$T=table_status('',true);if(!$T)echo"<p class='message'>".'Žádné tabulky.'."\n";else$this->tablesPrint($T);}}}function
databasesPrint($Ic){}function
tablesPrint($ce){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($ce
as$M){$D=$this->tableName($M);if(isset($M["Engine"])&&$D!="")echo"<a href='".h(ME).'select='.urlencode($M["Name"])."'".bold($_GET["select"]==$M["Name"]||$_GET["edit"]==$M["Name"],"select")." title='".'Vypsat data'."'>$D</a><br>\n";}}function
...
auth_error($k){global$b,$Zb;$k=h($k);$Nd=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Nd]||$_GET[$Nd])&&!$Zb)$k='Session vypršela, přihlašte se prosím znovu.';else{add_invalid_login();$H=get_password();if($H!==null){if($H===false)$k.='<br>'.sprintf('Platnost hlavního hesla vypršela. <a href="http://www.adminer.org/cs/extension/" target="_blank">Implementujte</a> metodu %s, aby platilo stále.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Nd]&&$_GET[$Nd]&&ini_bool("session.use_only_cookies"))$k='Session proměnné musí být povolené.';$dd=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$dd["lifetime"]);page_header('Přihlásit se',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Žádné rozšíření',sprintf('Není dostupné žádné z podporovaných PHP rozšíření (%s).',implode(", ",$jd)),false);page_footer("auth");exit;}$f=connect();}$i=new
Min_Driver($f);if(!is_object($f)||!$b->login($_GET["username"],get_password()))auth_error((is_string($f)?$f:'Neplatné přihlašovací údaje.'));if($ra&&$_POST["token"])$_POST["token"]=$oe;$k='';if($_POST){if(!verify_token()){$kc="max_input_vars";$Fc=ini_get($kc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$W=ini_get($x);if($W&&(!$Fc||$W<$Fc)){$kc=$x;$Fc=$W;}}}$k=(!$_POST["token"]&&$Fc?sprintf('Byl překročen maximální povolený počet polí. Zvyšte prosím %s.',"'$kc'"):'Neplatný token CSRF. Odešlete formulář znovu.'.' '.'Pokud jste tento požadavek neposlali z Adminera, tak tuto stránku zavřete.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$k=sprintf('Příliš velká POST data. Zmenšete data nebo zvyšte hodnotu konfigurační direktivy %s.',"'post_max_size'");if(isset($_GET["sql"]))$k.=' '.'Velký SQL soubor můžete nahrát pomocí FTP a importovat ho ze serveru.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
...
as$w=>$V)$z.=where_link($q++,$w,$V);}$W=select_value($W,$z,$l,$fe);$r=h("val[$ye][".bracket_escape($x)."]");$X=$_POST["val"][$ye][bracket_escape($x)];$nb=!is_array($M[$x])&&is_utf8($W)&&$N[$C][$x]==$M[$x]&&!$Tb[$x];$ee=preg_match('~text|lob~',$l["type"]);if(($_GET["modify"]&&$nb)||$X!==null){$Wb=h($X!==null?$X:$M[$x]);echo"<td>".($ee?"<textarea name='$r' cols='30' rows='".(substr_count($M[$x],"\n")+1)."'>$Wb</textarea>":"<input name='$r' value='$Wb' size='$yc[$x]'>");}else{$Ac=strpos($W,"<i>...</i>");echo"<td id='$r' onclick=\"selectClick(this, event, ".($Ac?2:($ee?1:0)).($nb?"":", '".h('Ke změně této hodnoty použijte odkaz upravit.')."'").");\">$W";}}}if($xa)echo"<td>";$b->backwardKeysPrint($xa,$N[$C]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($N||$G)&&!is_ajax()){$yb=true;if($_GET["page"]!="last"){if(!+$y)$n=count($N);elseif($v!="sql"||!$u){$n=($u?false:found_rows($T,$Z));if($n<max(1e4,2*($G+1)*$y))$n=reset(slow_query(count_rows($a,$Z,$u,$p)));else$yb=false;}}if(+$y&&($n===false||$n>$y||$G)){echo"<p class='pages'>";$Dc=($n===false?$G+(count($N)>=$y?2:1):floor(($n-1)/$y));if($v!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($G+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$G).($G>5?" ...":"");for($q=max(1,$G-4);$q<min($Dc,$G+5);$q++)echo
pagination($q,$G);if($Dc>0){echo($G+5<$Dc?" ...":""),($yb&&$n!==false?pagination($Dc,$G):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Dc'>".'poslední'."</a>");}echo(($n===false?count($N)+1:$n-$G*$y)>$y?' <a href="'.h(remove_from_uri("page")."&page=".($G+1)).'" onclick="return !selectLoadMore(this, '.(+$y).', \''.'Nahrává se'.'...\');" class="loadmore">'.'Nahrát další data'.'</a>':'');}else{echo'Stránka'.":",pagination(0,$G).($G>1?" ...":""),($G?pagination($G,$G):""),($Dc>$G?pagination($G+1,$G).($Dc>$G+1?" ...":""):"");}}echo"<p class='count'>\n",($n!==false?"(".($yb?"":"~ ").lang(array('%d řádek','%d řádky','%d řádků'),$n).") ":"");$hb=($yb?"":"~ ").$n;echo
checkbox("all",1,0,'celý výsledek',"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$hb' : checked); selectCount('selected2', this.checked || !checked ? '$hb' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Změnit</legend><div>
