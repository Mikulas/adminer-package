...
confirm($Ub=""){return" onclick=\"return confirm('".'Opravdu?'.($Ub?" (' + $Ub + ')":"")."');\"";}function
js_escape($A){return
addcslashes($A,"\r\n'\\/");}function
...
upload_error($v){$Fc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Nepodařilo se nahrát soubor.'.($Fc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Fc):""):'Soubor neexistuje.');}function
...
input($e,$l,$_){global$Da,$j,$ya;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$ea=(isset($_GET["select"])?array("orig"=>'původní'):array())+$j->editFunctions($e);$P=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($sd,$Wa){$td=($Wa==1||(!$Wa&&'cs'=='fr')?0:((!$Wa||$Wa>=5)&&ereg('cs|sk|ru','cs')?2:1));return
...
selectSearchPrint($p,$z,$E){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhledat'."</legend><div>\n";$Cb=array();foreach($p
...
as$c=>$Q){$X=array();foreach($Q["columns"]as$a){$X[]=$this->fieldName(array("field"=>$a,"comment"=>$z[$a]));}if(count(array_filter($X,'strlen'))>1&&$c!="PRIMARY"){$Bb[$c]=implode(", ",$X);}}if($Bb){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Bb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
mail($Ga,email_header($ib),$qc.$V.$db,$mb);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Mb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$Aa[DRIVER]='Přihlásit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$q=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$q-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($q)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$q=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$ga=(isset($_GET["select"])?$_POST["edit"]:$p);$r=fields($q);foreach($r
as$f=>$e){if(!isset($e["privileges"][$ga?"update":"insert"])||$j->fieldName($e)==""){unset($r[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$O=$_POST["referer"];if($_POST["insert"]){$O=($ga?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$O)){$O=ME."select=".urlencode($q);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($q)," WHERE $p"),$O,'Položka byla smazána.');}else{$t=array();foreach($r
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($ga?"\n".idf_escape($f)." = $a":$a);}}if($ga){if(!$t){redirect($O);}query_redirect("UPDATE".limit1(table($q)." SET".implode(",",$t),"\nWHERE $p"),$O,'Položka byla aktualizována.');}else{$h=insert_into($q,$t);$sc=($h?last_id():0);queries_redirect($O,sprintf('Položka%s byla vložena.',($sc?" $sc":"")),$h);}}}$Ta=$j->tableName(table_status($q));page_header(($ga?'Upravit':'Vložit'),$v,array("select"=>array($q,$Ta)),$Ta);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$w=array();foreach($r
...
<input type="submit" name="delete" value="Smazat"',confirm("(this.form['all'].checked ? $Z : formChecked(this, /check/))"),'>
</div></fieldset>
