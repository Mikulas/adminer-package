...
confirm($Pa=""){return" onclick=\"return confirm('".'Opravdu?'.($Pa?" (' + $Pa + ')":"")."');\"";}function
...
enum_input($ae,$c,$l,$W,$ob=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$z);$J=($ob!==null?"<label><input type='$ae'$c value='$ob'".((is_array($W)?in_array($ob,$W):$W===0)?" checked":"")."><i>".'prÃ¡zdnÃ©'."</i></label>":"");foreach($z[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$J.=" <label><input type='$ae'$c value='".($p+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$J;}function
input($l,$W,$o){global$g,$ce,$b,$t;$B=h(bracket_escape($l["field"]));echo"<td class='function'>";$od=($t=="mssql"&&$l["auto_increment"]);if($od&&!$_POST["save"])$o=null;$Sb=(isset($_GET["select"])||$od?array("orig"=>'pÅ¯vodnÃ­'):array())+$b->editFunctions($l);$c=" name='fields[$B]'";if($l["type"]=="enum")echo
...
lang($Yd,$Ic=null){if(is_array($Yd)){$ad=($Ic==1?0:($Ic&&$Ic<5?1:2));$Yd=$Yd[$ad];}$Yd=str_replace("%d","%s",$Yd);$Ic=number_format($Ic,0,".",' ');return
...
selectLinks($Jd,$O=""){$a=$Jd["Name"];if($O!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$O).'">'.'NovÃ¡ poloÅ¾ka'."</a>\n";}function
...
as$e=>$V)$w.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($K[$V]);echo"<a href='".h($w)."' title='".'NovÃ¡ poloÅ¾ka'."'>+</a> ";}}}function
...
as$B=>$Ya){$l=$m[$B];if(ereg("enum",$l["type"])||like_bool($l)){$u=$nc[$B];$p--;echo"<div>".h($Ya)."<input type='hidden' name='where[$p][col]' value='".h($B)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'ne','ano'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($Rc=$this->_foreignKeyOptions($_GET["select"],$B))){if($m[$B]["null"])$Rc[0]='('.'prÃ¡zdnÃ©'.')';$u=$nc[$B];$p--;echo"<div>".h($Ya)."<input type='hidden' name='where[$p][col]' value='".h($B)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Rc,$X[$u]["val"],true)."</select></div>\n";unset($f[$B]);}}$p=0;foreach($X
...
as$u=>$r){$C=array();foreach($r["columns"]as$V)$C[]=$f[$V];if(count(array_filter($C,'strlen'))>1&&$u!="PRIMARY")$Tc[$u]=implode(", ",$C);}if($Tc){echo'<fieldset><legend>'.'SeÅ™adit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
editFunctions($l){$J=array();if($l["null"]&&ereg('blob',$l["type"]))$J["NULL"]='prÃ¡zdnÃ©';$J[""]=($l["null"]||$l["auto_increment"]||like_bool($l)?"":"*");if(ereg('date|time',$l["type"]))$J["now"]='teÄ';if(eregi('_(md5|sha1)$',$l["field"],$y))$J[]=strtolower($y[1]);return$J;}function
editInput($R,$l,$c,$W){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$c value='-1' checked><i>".'pÅ¯vodnÃ­'."</i></label> ":"").enum_input("radio",$c,$l,($W||isset($_GET["select"])?$W:0),($l["null"]?"":null));$Rc=$this->_foreignKeyOptions($R,$l["field"],$W);if($Rc!==null)return(is_array($Rc)?"<select$c>".optionlist($Rc,$W,true)."</select>":"<input value='".h($W)."'$c class='hidden'><input value='".h($Rc)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($R)."&field=".urlencode($l["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");if(like_bool($l))return'<input type="checkbox" value="'.h($W?$W:1).'"'.($W?' checked':'')."$c>";$ac="";if(ereg('time',$l["type"]))$ac='HH:MM:SS';if(ereg('date|timestamp',$l["type"]))$ac='d.m.[rrrr]'.($ac?" [$ac]":"");if($ac)return"<input value='".h($W)."'$c> ($ac)";if(eregi('_(md5|sha1)$',$l["field"]))return"<input type='password' value='".h($W)."'$c>";return'';}function
...
';if($m){echo"<input type='submit' value='".'UloÅ¾it'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($ke?'UloÅ¾it a pokraÄovat v editaci':'UloÅ¾it a vloÅ¾it dalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($ke?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$B=>$V){$V=process_input($m[$B]);if($V!==null){if($_POST["clone"])$O[idf_escape($B)]=($V!==false?$V:idf_escape($B));elseif($V!==false)$O[]=idf_escape($B)." = $V";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ka="UPDATE";if($_POST["delete"]){$Ka="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ka="INSERT";$H="INTO $H";}if($_POST["all"]||($ie===array()&&is_array($_POST["check"]))||$lc){$I=queries("$Ka $H$se");$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$I=queries($Ka.limit1($H,"\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V,$m)));if(!$I)break;$ia+=$g->affected_rows;}}}$_=lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$ia);if($_POST["clone"]&&$I&&$ia==1){$sc=last_id();if($sc)$_=sprintf('PoloÅ¾ka%s byla vloÅ¾ena.'," $sc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$_,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+kliknÄ›te na polÃ­Äko, kterÃ© chcete zmÄ›nit.';else{$I=true;$ia=0;foreach($_POST["val"]as$ge=>$K){$O=array();foreach($K
as$u=>$V){$u=bracket_escape($u,1);$O[]=idf_escape($u)." = ".(ereg('char|text',$m[$u]["type"])||$V!=""?$b->processInput($m[$u],$V):"NULL");}$H=table($a)." SET ".implode(", ",$O);$re=" WHERE ".where_check($ge,$m).($X?" AND ".implode(" AND ",$X):"");$I=queries("UPDATE".($lc||$ie===array()?" $H$re":limit1($H,$re)));if(!$I)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$ia),$I);}}elseif(!is_string($Cb=get_file("csv_file",true)))$k=upload_error($Cb);elseif(!preg_match('~~u',$Cb))$k='Soubor musÃ­ bÃ½t v kÃ³dovÃ¡nÃ­ UTF-8.';else{cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Ja=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Cb,$z);$ia=count($z[0]);begin();$xd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($z[0]as$u=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$xd]*)$xd~",$V.$xd,$yc);if(!$u&&!array_diff($yc[1],$Ja)){$Ja=$yc[1];$ia--;}else{$O=array();foreach($yc[1]as$p=>$Ga)$O[idf_escape($Ja[$p])]=($Ga==""&&$m[$Ja[$p]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Ga))));$I=insert_update($a,$O,$cd);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Byl importovÃ¡n %d zÃ¡znam.','Byly importovÃ¡ny %d zÃ¡znamy.','Bylo importovÃ¡no %d zÃ¡znamÅ¯.'),$ia),$I);queries("ROLLBACK");}}}$Kd=$b->tableName($S);if(is_ajax())ob_start();page_header('Vypsat'.": $Kd",$k);$O=null;if(isset($rd["insert"])){$O="";foreach((array)$_GET["where"]as$V){if(count($Mb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$O.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($S,$O);if(!$f)echo"<p class='error'>".'NepodaÅ™ilo se vypsat tabulku'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$f);$b->selectSearchPrint($X,$f,$s);$b->selectOrderPrint($C,$f,$s);$b->selectLimitPrint($v);$b->selectLengthPrint($Od);$b->selectActionPrint($s);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$n=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$E=floor(max(0,$n-1)/$v);}$H=$b->selectQueryBuild($M,$X,$Tb,$C,$v,$E);if(!$H)$H="SELECT".limit((+$v&&$Tb&&$lc&&$t=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Qb,($X?"\nWHERE ".implode(" AND ",$X):"").$Ub,($v!=""?+$v:null),($E?$v*$E:0),"\n");echo$b->selectQuery($H);$I=$g->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($t=="mssql"&&$E)$I->seek($v*$E);$nb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$t=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$n=(+$v&&$Tb&&$lc?($t=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";else{$ua=$b->backwardKeys($a,$Kd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Tb&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upravit'."</a>");$Gc=array();$Sb=array();reset($M);$jd=1;foreach($L[0]as$u=>$V){if($u!=$Kc){$V=$_GET["columns"][key($M)];$l=$m[$M?($V?$V["col"]:current($M)):$u];$B=($l?$b->fieldName($l,$jd):"*");if($B!=""){$jd++;$Gc[$u]=$B;$e=idf_escape($u);$cc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Ya="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($cc.($C[0]==$e||$C[0]==$u||(!$C&&$lc&&$Tb[0]==$e)?$Ya:'')).'">';echo(!$M||$V?apply_sql_function($V["fun"],$B):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($cc.$Ya)."' title='".'sestupnÄ›'."' class='text'> â†“</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Vyhledat'.'" class="text jsonly"> =</a>';echo"</span>";}$Sb[$u]=$V["fun"];next($M);}}$vc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$u=>$V)$vc[$u]=max($vc[$u],min(40,strlen(utf8_decode($V))));}}echo($ua?"<th>".'Vztahy':"")."</thead>\n";if(is_ajax()){if($v%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$Mb)as$A=>$K){$fe=unique_array($L[$A],$s);if(!$fe){$fe=array();foreach($L[$A]as$u=>$V){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$u))$fe[$u]=$V;}}$ge="";foreach($fe
as$u=>$V){if(strlen($V)>64){$u="MD5(".(strpos($u,'(')?$u:idf_escape($u)).")";$V=md5($V);}$ge.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));}echo"<tr".odd().">".(!$Tb&&$M?"":"<td>".checkbox("check[]",substr($ge,1),in_array(substr($ge,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($lc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$ge)."'>".'upravit'."</a>"));foreach($K
