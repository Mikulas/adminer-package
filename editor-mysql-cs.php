...
as$W=>$o){unset($ra[$c][$W]);if(is_array($o)){$ra[$c][stripslashes($W)]=$o;$ra[]=&$ra[$c][stripslashes($W)];}else{$ra[$c][stripslashes($W)]=($vb?$o:stripslashes($o));}}}}}function
...
nl_br($F){return
str_replace("\n","<br>",$F);}function
checkbox($f,$i,$Za,$Vc="",$ub="",$Zc=false){static$z=0;$z++;$b="<input type='checkbox' name='$f' value='".h($i)."'".($Za?" checked":"").($ub?' onclick="'.h($ub).'"':'').($Zc?" class='jsonly'":"")." id='checkbox-$z'>";return($Vc!=""?"<label for='checkbox-$z'>$b".h($Vc)."</label>":$b);}function
optionlist($Z,$Xc=null,$mc=false){$b="";foreach($Z
as$W=>$o){$jc=array($W=>$o);if(is_array($o)){$b.='<optgroup label="'.h($W).'">';$jc=$o;}foreach($jc
as$c=>$a){$b.='<option'.($mc||is_string($c)?' value="'.h($c).'"':'').(($mc||is_string($c)?(string)$c:$a)===$Xc?' selected':'').'>'.h($a);}if(is_array($o)){$b.='</optgroup>';}}return$b;}function
...
print_fieldset($z,$Dd,$Ed=false,$ub=""){echo"<fieldset><legend><a href='#fieldset-$z' onclick=\"".h($ub)."return !toggle('fieldset-$z');\">$Dd</a></legend><div id='fieldset-$z'".($Ed?"":" class='hidden'").">\n";}function
...
get_file($c,$Jb=false){$T=$_FILES[$c];if(!$T||$T["error"]){return$T["error"];}$b=file_get_contents($Jb&&ereg('\\.gz$',$T["name"])?"compress.zlib://$T[tmp_name]":($Jb&&ereg('\\.bz2$',$T["name"])?"compress.bzip2://$T[tmp_name]":$T["tmp_name"]));if($Jb){$xc=substr($b,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$xc,$Qd)){$b=iconv("utf-16","utf-8",$b);}elseif($xc=="\xEF\xBB\xBF"){$b=substr($b,3);}}return$b;}function
...
enum_input($ua,$J,$e,$i,$Hb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$R);$b=(isset($Hb)?"<label><input type='$ua'$J value='$Hb'".((is_array($i)?in_array($Hb,$i):$i===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($R[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Za=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ua'$J value='".($n+1)."'".($Za?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$x){global$Ba,$j,$la;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$rc=($la=="mssql"&&$e["auto_increment"]);if($rc&&!$_POST["save"]){$x=null;}$ca=(isset($_GET["select"])||$rc?array("orig"=>'původní'):array())+$j->editFunctions($e);$J=" name='fields[$f]'";if($e["type"]=="enum"){echo
nbsp($ca[""])."<td>".$j->editInput($_GET["edit"],$e,$J,$i);}else{$da=0;foreach($ca
as$c=>$a){if($c===""||!$a){break;}$da++;}$za=($da?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($e["field"])))."]']; if ($da > f.selectedIndex) f.selectedIndex = $da;\"":"");$J.=$za;echo(count($ca)>1?html_select("function[$f]",$ca,!isset($x)||in_array($x,$ca)||isset($ca[$x])?$x:"","functionChange(this);"):nbsp(reset($ca))).'<td>';$Tc=$j->editInput($_GET["edit"],$e,$J,$i);if($Tc!=""){echo$Tc;}elseif($e["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$R);foreach($R[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Za=(is_int($i)?($i>>$n)&1:in_array($a,explode(",",$i),true));echo" <label><input type='checkbox' name='fields[$f][$n]' value='".(1<<$n)."'".($Za?' checked':'')."$za>".h($j->editVal($a,$e)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$e["type"])&&ini_bool("file_uploads")){echo"<input type='file' name='fields-$f'$za>";}elseif(ereg('text|lob',$e["type"])){echo"<textarea ".($la!="sqlite"||ereg("\n",$i)?"cols='50' rows='12'":"cols='30' rows='1' style='height: 1.2em;'")."$J>".h($i).'</textarea>';}else{$Gb=(!ereg('int',$e["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$e["length"],$m)?((ereg("binary",$e["type"])?2:1)*$m[1]+($m[3]?1:0)+($m[2]&&!$e["unsigned"]?1:0)):($Ba[$e["type"]]?$Ba[$e["type"]]+($e["unsigned"]?0:1):0));echo"<input value='".h($i)."'".($Gb?" maxlength='$Gb'":"").(ereg('char|binary',$e["type"])&&$Gb>20?" size='40'":"")."$J>";}}}function
...
limit($l,$p,$C,$sc=0,$sa=" "){return" $l$p".(isset($C)?$sa."LIMIT $C".($sc?" OFFSET $sc":""):"");}function
limit1($l,$p){return
...
foreign_keys($h){global$g,$qb;static$L='`(?:[^`]|``)+`';$b=array();$yc=$g->result("SHOW CREATE TABLE ".table($h),1);if($yc){preg_match_all("~CONSTRAINT ($L) FOREIGN KEY \\(((?:$L,? ?)+)\\) REFERENCES ($L)(?:\\.($L))? \\(((?:$L,? ?)+)\\)(?: ON DELETE (".implode("|",$qb)."))?(?: ON UPDATE (".implode("|",$qb)."))?~",$yc,$R,PREG_SET_ORDER);foreach($R
as$m){preg_match_all("~$L~",$m[2],$Tb);preg_match_all("~$L~",$m[5],$Y);$b[idf_unescape($m[1])]=array("db"=>idf_unescape($m[4]!=""?$m[3]:$m[4]),"table"=>idf_unescape($m[4]!=""?$m[4]:$m[3]),"source"=>array_map('idf_unescape',$Tb[0]),"target"=>array_map('idf_unescape',$Y[0]),"on_delete"=>$m[6],"on_update"=>$m[7],);}}return$b;}function
...
alter_table($h,$f,$q,$ld,$kd,$Sc,$Aa,$db,$jd){$ha=array();foreach($q
as$e){$ha[]=($e[1]?($h!=""?($e[0]!=""?"CHANGE ".idf_escape($e[0]):"ADD"):" ")." ".implode($e[1]).($h!=""?" $e[2]":""):"DROP ".idf_escape($e[0]));}$ha=array_merge($ha,$ld);$Uc="COMMENT=".q($kd).($Sc?" ENGINE=".q($Sc):"").($Aa?" COLLATE ".q($Aa):"").($db!=""?" AUTO_INCREMENT=$db":"").$jd;if($h==""){return
queries("CREATE TABLE ".table($f)." (\n".implode(",\n",$ha)."\n) $Uc");}if($h!=$f){$ha[]="RENAME TO ".table($f);}$ha[]=$Uc;return
...
routine($f,$ua){global$g,$Vb,$_c,$Ba;$Gd=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Rc="((".implode("|",array_merge(array_keys($Ba),$Gd)).")(?:\\s*\\(((?:[^'\")]*|$Vb)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$L="\\s*(".($ua=="FUNCTION"?"":$_c).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Rc";$ib=$g->result("SHOW CREATE $ua ".idf_escape($f),2);preg_match("~\\(((?:$L\\s*,?)*)\\)".($ua=="FUNCTION"?"\\s*RETURNS\\s+$Rc":"")."\\s*(.*)~is",$ib,$m);$q=array();preg_match_all("~$L\\s*,?~is",$m[1],$R,PREG_SET_ORDER);foreach($R
as$ja){$f=str_replace("``","`",$ja[2]).$ja[3];$q[]=array("field"=>$f,"type"=>strtolower($ja[5]),"length"=>preg_replace_callback("~$Vb~s",'normalize_enum',$ja[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$ja[8] $ja[7]"))),"full_type"=>$ja[4],"inout"=>strtoupper($ja[1]),"collation"=>strtolower($ja[9]),);}if($ua!="FUNCTION"){return
...
rowDescriptions($w,$Qa){$b=$w;foreach($w[0]as$c=>$a){if(list($h,$z,$f)=$this->_foreignColumn($Qa,$c)){$nc=array();foreach($w
as$d){$nc[$d[$c]]=exact_value($d[$c]);}$Kb=$this->_values[$h];if(!$Kb){$Kb=get_key_vals("SELECT $z, $f FROM ".table($h)." WHERE $z IN (".implode(", ",$nc).")");}foreach($w
as$y=>$d){if(isset($d[$c])){$b[$y][$c]=(string)$Kb[$d[$c]];}}}}return$b;}function
selectVal($a,$t,$e){$b=($a=="<i>NULL</i>"?"&nbsp;":$a);if(ereg('blob|bytea',$e["type"])&&!is_utf8($a)){$b=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($a));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$a)){$b="<img src='$t' alt='$b'>";}}if(like_bool($e)&&$b!="&nbsp;"){$b='<img src="'.($a?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.1").'" alt="'.h($a).'">';}if($t){$b="<a href='$t'>$b</a>";}if(!$t&&!like_bool($e)&&ereg('int|float|double|decimal',$e["type"])){$b="<div class='number'>$b</div>";}elseif(ereg('date',$e["type"])){$b="<div class='datetime'>$b</div>";}return$b;}function
...
selectSearchPrint($p,$A,$G){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhledat'."</legend><div>\n";$Lb=array();foreach($p
as$c=>$a){$Lb[$a["col"]]=$c;}$n=0;$q=fields($_GET["select"]);foreach($q
as$f=>$e){$ma=$A[$f];if(ereg("enum",$e["type"])&&$ma!=""){$c=$Lb[$f];$n--;echo"<div>".h($ma)."<input type='hidden' name='where[$n][col]' value='".h($f)."'>:",enum_input("checkbox"," name='where[$n][val][]'",$e,(array)$p[$c]["val"],($e["null"]?0:null)),"</div>\n";unset($A[$f]);}}foreach($A
as$f=>$ma){$Z=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Z)){if($q[$f]["null"]){$Z[0]='('.'prázdné'.')';}$c=$Lb[$f];$n--;echo"<div>".h($ma)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Z,$p[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($p
...
as$c=>$S){$V=array();foreach($S["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Mb[$c]=implode(", ",$V);}}if($Mb){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Mb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
true;}if($_POST["email"]){$fc=0;if($_POST["all"]||$_POST["check"]){$e=idf_escape($_POST["email_field"]);$gb=$_POST["email_subject"];$X=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$gb.$X",$R);$w=get_rows("SELECT DISTINCT $e".($R[1]?", ".implode(", ",array_map('idf_escape',array_unique($R[1]))):"")." FROM ".table($_GET["select"])." WHERE $e IS NOT NULL AND $e != ''".($p?" AND ".implode(" AND ",$p):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$q=fields($_GET["select"]);foreach($this->rowDescriptions($w,$Qa)as$d){$ac=array('{\\'=>'{');foreach($R[1]as$a){$ac['{$'."$a}"]=$this->editVal($d[$a],$q[$a]);}$Fa=$d[$_POST["email_field"]];if(is_mail($Fa)&&send_mail($Fa,strtr($gb,$ac),strtr($X,$ac),$_POST["email_from"],$_FILES["email_files"])){$fc++;}}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(array('Byl odeslán %d e-mail.','Byly odeslány %d e-maily.','Bylo odesláno %d e-mailů.'),$fc));}return
false;}function
...
editInput($h,$e,$J,$i){if($e["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$J value='-1' checked><i>".'původní'."</i></label> ":"").enum_input("radio",$J,$e,($i||isset($_GET["select"])?$i:0),($e["null"]?"":null));}$Z=$this->_foreignKeyOptions($h,$e["field"],$i);if(isset($Z)){return(is_array($Z)?"<select$J>".optionlist($Z,$i,true)."</select>":"<input value='".h($i)."'$J class='hidden'><input value='".h($Z)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($h)."&field=".urlencode($e["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");}if(like_bool($e)){return'<input type="checkbox" value="'.h($i?$i:1).'"'.($i?' checked':'')."$J>";}$Ia="";if(ereg('time',$e["type"])){$Ia='HH:MM:SS';}if(ereg('date|timestamp',$e["type"])){$Ia='d.m.[rrrr]'.($Ia?" [$Ia]":"");}if($Ia){return"<input value='".h($i)."'$J> ($Ia)";}if(eregi('_(md5|sha1)$',$e["field"])){return"<input type='password' value='".h($i)."'$J>";}return'';}function
...
_foreignColumn($Qa,$H){foreach((array)$Qa[$H]as$jb){if(count($jb["source"])==1){$f=$this->rowDescription($jb["table"]);if($f!=""){$z=idf_escape($jb["target"][0]);return
array($jb["table"],$z,$f);}}}}function
...
as$c=>$a){$ma=(is_array($a)?$a[1]:$a);if($ma!=""){echo'<a href="'.h(ME."$c=").urlencode(is_array($a)?$a[0]:$a).'">'.h($ma).'</a> &raquo; ';}}}echo"$oc\n";}}echo"<span id='loader'></span>\n","<h2>$qc</h2>\n";restart_session();$pc=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$kc=$_SESSION["messages"][$pc];if($kc){echo"<div class='message'>".implode("</div>\n<div class='message'>",$kc)."</div>\n";unset($_SESSION["messages"][$pc]);}$ka=&get_session("dbs");if(DB!=""&&$ka&&!in_array(DB,$ka,true)){$ka=null;}if($v){echo"<div class='error'>$v</div>\n";}define("PAGE_HEADER",1);}function
...
long2str($o,$Nb){$xa='';foreach($o
as$a){$xa.=pack('V',$a);}if($Nb){return
substr($xa,0,end($o));}return$xa;}function
str2long($xa,$Nb){$o=array_values(unpack('V*',str_pad($xa,4*ceil(strlen($xa)/4),"\0")));if($Nb){$o[]=strlen($xa);}return$o;}function
xxtea_mx($N,$Q,$U,$W){return
int32((($N>>5&0x7FFFFFF)^$Q<<2)+(($Q>>3&0x1FFFFFFF)^$N<<4))^int32(($U^$Q)+($W^$N));}function
encrypt_string($Ua,$c){if($Ua==""){return"";}$c=array_values(unpack("V*",pack("H*",md5($c))));$o=str2long($Ua,true);$y=count($o)-1;$N=$o[$y];$Q=$o[0];$Yb=floor(6+52/($y+1));$U=0;while($Yb-->0){$U=int32($U+0x9E3779B9);$Na=$U>>2&3;for($P=0;$P<$y;$P++){$Q=$o[$P+1];$ya=xxtea_mx($N,$Q,$U,$c[$P&3^$Na]);$N=int32($o[$P]+$ya);$o[$P]=$N;}$Q=$o[0];$ya=xxtea_mx($N,$Q,$U,$c[$P&3^$Na]);$N=int32($o[$y]+$ya);$o[$y]=$N;}return
long2str($o,false);}function
decrypt_string($Ua,$c){if($Ua==""){return"";}$c=array_values(unpack("V*",pack("H*",md5($c))));$o=str2long($Ua,false);$y=count($o)-1;$N=$o[$y];$Q=$o[0];$Yb=floor(6+52/($y+1));$U=int32($Yb*0x9E3779B9);while($U){$Na=$U>>2&3;for($P=$y;$P>0;$P--){$N=$o[$P-1];$ya=xxtea_mx($N,$Q,$U,$c[$P&3^$Na]);$Q=int32($o[$P]-$ya);$o[$P]=$Q;}$N=$o[$y];$ya=xxtea_mx($N,$Q,$U,$c[$P&3^$Na]);$Q=int32($o[0]-$ya);$o[0]=$Q;$U=int32($U-0x9E3779B9);}return
...
as$c=>$e){$f=$j->fieldName($e);if(isset($e["privileges"]["select"])&&$f!=""){$A[$c]=html_entity_decode(strip_tags($f));if(ereg('text|lob',$e["type"])){$Pa=$j->selectLengthProcess();}}$Qc+=$e["privileges"];}list($u,$M)=$j->selectColumnsProcess($A,$G);$p=$j->selectSearchProcess($q,$G);$V=$j->selectOrderProcess($q,$G);$C=$j->selectLimitProcess();$Ha=($u?implode(", ",$u):($B["Oid"]=="t"?"oid, ":"")."*")."\nFROM ".table($r);$Db=($M&&count($M)<count($u)?"\nGROUP BY ".implode(", ",$M):"").($V?"\nORDER BY ".implode(", ",$V):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$ia=>$d){echo$g->result("SELECT".limit(idf_escape(key($d))." FROM ".table($r)," WHERE ".where_check($ia).($p?" AND ".implode(" AND ",$p):"").($V?" ORDER BY ".implode(", ",$V):""),1));}exit;}if($_POST&&!$v){$Kc="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$lb=$Ja=null;foreach($G
as$S){if($S["type"]=="PRIMARY"){$lb=array_flip($S["columns"]);$Ja=($u?$lb:array());break;}}foreach((array)$Ja
as$c=>$a){if(in_array(idf_escape($c),$u)){unset($Ja[$c]);}}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($r);$j->dumpTable($r,"");if(!is_array($_POST["check"])||$Ja===array()){$Ga=$p;if(is_array($_POST["check"])){$Ga[]="($Kc)";}$l="SELECT $Ha".($Ga?"\nWHERE ".implode(" AND ",$Ga):"").$Db;}else{$vc=array();foreach($_POST["check"]as$a){$vc[]="(SELECT".limit($Ha,"\nWHERE ".($p?implode(" AND ",$p)." AND ":"").where_check($a).$Db,1).")";}$l=implode(" UNION ALL ",$vc);}$j->dumpData($r,"table",$l);exit;}if(!$j->selectEmailProcess($p,$Ka)){if($_POST["save"]||$_POST["delete"]){$k=true;$qa=0;$l=table($r);$s=array();if(!$_POST["delete"]){foreach($A
as$f=>$a){$a=process_input($q[$f]);if($a!==null){if($_POST["clone"]){$s[idf_escape($f)]=($a!==false?$a:idf_escape($f));}elseif($a!==false){$s[]=idf_escape($f)." = $a";}}}$l.=($_POST["clone"]?" (".implode(", ",array_keys($s)).")\nSELECT ".implode(", ",$s)."\nFROM ".table($r):" SET\n".implode(",\n",$s));}if($_POST["delete"]||$s){$xb="UPDATE";if($_POST["delete"]){$xb="DELETE";$l="FROM $l";}if($_POST["clone"]){$xb="INSERT";$l="INTO $l";}if($_POST["all"]||($Ja===array()&&$_POST["check"])||count($M)<count($u)){$k=queries($xb." $l".($_POST["all"]?($p?"\nWHERE ".implode(" AND ",$p):""):"\nWHERE $Kc"));$qa=$g->affected_rows;}else{foreach((array)$_POST["check"]as$a){$k=queries($xb.limit1($l,"\nWHERE ".where_check($a)));if(!$k){break;}$qa+=$g->affected_rows;}}}queries_redirect(remove_from_uri("page"),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$qa),$k);}elseif(!$_POST["import"]){if(!$_POST["val"]){$v='Dvojklikněte na políčko, které chcete změnit.';}else{$k=true;$qa=0;foreach($_POST["val"]as$ia=>$d){$s=array();foreach($d
as$c=>$a){$c=bracket_escape($c,1);$s[]=idf_escape($c)." = ".(ereg('char|text',$q[$c]["type"])||$a!=""?$j->processInput($q[$c],$a):"NULL");}$l=table($r)." SET ".implode(", ",$s);$Ga=" WHERE ".where_check($ia).($p?" AND ".implode(" AND ",$p):"");$k=queries("UPDATE".(count($M)<count($u)?" $l$Ga":limit1($l,$Ga)));if(!$k){break;}$qa+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$qa),$k);}}elseif(is_string($T=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($tb["output"])."&format=".urlencode($_POST["separator"]));$k=true;$Ca=array_keys($q);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$T,$R);$qa=count($R[0]);begin();$sa=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($R[0]as$c=>$a){preg_match_all("~((\"[^\"]*\")+|[^$sa]*)$sa~",$a.$sa,$Sb);if(!$c&&!array_diff($Sb[1],$Ca)){$Ca=$Sb[1];$qa--;}else{$s=array();foreach($Sb[1]as$n=>$oa){$s[idf_escape($Ca[$n])]=($oa==""&&$q[$Ca[$n]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$oa))));}$k=insert_update($r,$s,$lb);if(!$k){break;}}}if($k){queries("COMMIT");}queries_redirect(remove_from_uri("page"),lang(array('Byl importován %d záznam.','Byly importovány %d záznamy.','Bylo importováno %d záznamů.'),$qa),$k);queries("ROLLBACK");}else{$v=upload_error($T);}}}$La=$j->tableName($B);page_header('Vypsat'.": $La",$v);session_write_close();$s=null;if(isset($Qc["insert"])){$s="";foreach((array)$_GET["where"]as$a){if(count($Ka[$a["col"]])==1&&($a["op"]=="="||(!$a["op"]&&!ereg('[_%]',$a["val"])))){$s.="&set".urlencode("[".bracket_escape($a["col"])."]")."=".urlencode($a["val"]);}}}$j->selectLinks($B,$s);if(!$A){echo"<p class='error'>".'Nepodařilo se vypsat tabulku'.($q?".":": ".error())."\n";}else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($r).'">',"</div>\n";$j->selectColumnsPrint($u,$A);$j->selectSearchPrint($p,$A,$G);$j->selectOrderPrint($V,$A,$G);$j->selectLimitPrint($C);$j->selectLengthPrint($Pa);$j->selectActionPrint();echo"</form>\n";$_=$_GET["page"];if($_=="last"){$fa=$g->result("SELECT COUNT(*) FROM ".table($r).($p?" WHERE ".implode(" AND ",$p):""));$_=floor(max(0,$fa-1)/$C);}$l="SELECT".limit((+$C&&$M&&count($M)<count($u)&&$la=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Ha,($p?"\nWHERE ".implode(" AND ",$p):"").$Db,($C!=""?+$C:null),($_?$C*$_:0),"\n");echo$j->selectQuery($l);$k=$g->query($l);if(!$k){echo"<p class='error'>".error()."\n";}else{if($la=="mssql"){$k->seek($C*$_);}$Cb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$w=array();while($d=$k->fetch_assoc()){if($_&&$la=="oracle"){unset($d["RNUM"]);}$w[]=$d;}if($_GET["page"]!="last"){$fa=(+$C&&$M&&count($M)<count($u)?($la=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($l) x")):count($w));}if(!$w){echo"<p class='message'>".'Žádné řádky.'."\n";}else{$Ob=$j->backwardKeys($r,$La);echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$M&&$u?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upravit'."</a>");$Qb=array();$ca=array();reset($u);$zc=1;foreach($w[0]as$c=>$a){if($B["Oid"]!="t"||$c!="oid"){$a=$_GET["columns"][key($u)];$e=$q[$u?($a?$a["col"]:current($u)):$c];$f=($e?$j->fieldName($e,$zc):"*");if($f!=""){$zc++;$Qb[$c]=$f;$H=idf_escape($c);echo'<th><a href="'.h(remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($c).($V[0]==$H||$V[0]==$c||(!$V&&count($M)<count($u)&&$M[0]==$H)?'&desc%5B0%5D=1':'')).'">'.(!$u||$a?apply_sql_function($a["fun"],$f):h(current($u)))."</a>";}$ca[$c]=$a["fun"];next($u);}}$Eb=array();if($_GET["modify"]){foreach($w
as$d){foreach($d
as$c=>$a){$Eb[$c]=max($Eb[$c],min(40,strlen(utf8_decode($a))));}}}echo($Ob?"<th>".'Vztahy':"")."</thead>\n";foreach($j->rowDescriptions($w,$Ka)as$y=>$d){$Pb=unique_array($w[$y],$G);$ia="";foreach($Pb
as$c=>$a){$ia.="&".(isset($a)?urlencode("where[".bracket_escape($c)."]")."=".urlencode($a):"null%5B%5D=".urlencode($c));}echo"<tr".odd().">".(!$M&&$u?"":"<td>".checkbox("check[]",substr($ia,1),in_array(substr($ia,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($M)<count($u)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($r).$ia)."'>".'upravit'."</a>"));foreach($d
as$c=>$a){if(isset($Qb[$c])){$e=$q[$c];if($a!=""&&(!isset($Cb[$c])||$Cb[$c]!="")){$Cb[$c]=(is_mail($a)?$Qb[$c]:"");}$t="";$a=$j->editVal($a,$e);if(!isset($a)){$a="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$e["type"])&&$a!=""){$t=h(ME.'download='.urlencode($r).'&field='.urlencode($c).$ia);}if($a===""){$a="&nbsp;";}elseif($Pa!=""&&ereg('text|blob',$e["type"])&&is_utf8($a)){$a=shorten_utf8($a,max(0,+$Pa));}else{$a=h($a);}if(!$t){foreach((array)$Ka[$c]as$na){if(count($Ka[$c])==1||end($na["source"])==$c){$t="";foreach($na["source"]as$n=>$Tb){$t.=where_link($n,$na["target"][$n],$w[$y][$Tb]);}$t=h(($na["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($na["db"]),ME):ME).'select='.urlencode($na["table"]).$t);if(count($na["source"])==1){break;}}}}if($c=="COUNT(*)"){$t=h(ME."select=".urlencode($r));$n=0;foreach((array)$_GET["where"]as$o){if(!array_key_exists($o["col"],$Pb)){$t.=h(where_link($n++,$o["col"],$o["val"],$o["op"]));}}foreach($Pb
as$W=>$o){$t.=h(where_link($n++,$W,$o));}}}if(!$t){if(is_mail($a)){$t="mailto:$a";}if($Ec=is_url($d[$c])){$t=($Ec=="http"&&$bb?$d[$c]:"$Ec://www.adminer.org/redirect/?url=".urlencode($d[$c]));}}$z=h("val[$ia][".bracket_escape($c)."]");$i=$_POST["val"][$ia][bracket_escape($c)];$Fc=h(isset($i)?$i:$d[$c]);$ad=strpos($a,"<i>...</i>");$Cc=is_utf8($a)&&$w[$y][$c]==$d[$c]&&!$ca[$c];$Ac=ereg('text|lob',$e["type"]);echo(($_GET["modify"]&&$Cc)||isset($i)?"<td>".($Ac?"<textarea name='$z' cols='30' rows='".(substr_count($d[$c],"\n")+1)."'>$Fc</textarea>":"<input name='$z' value='$Fc' size='$Eb[$c]'>"):"<td id='$z' ondblclick=\"".($Cc?"selectDblClick(this, event".($ad?", 2":($Ac?", 1":"")).")":"alert('".h('Ke změně této hodnoty použijte odkaz upravit.')."')").";\">".$j->selectVal($a,$t,$e));}}if($Ob){echo"<td>";}$j->backwardKeysPrint($Ob,$w[$y]);echo"</tr>\n";}echo"</table>\n";}if($w||$_){$Ub=true;if($_GET["page"]!="last"&&+$C&&count($M)>=count($u)&&($fa>=$C||$_)){$fa=$B["Rows"];if(!isset($fa)||$p||($B["Engine"]=="InnoDB"&&$fa<max(1e4,2*($_+1)*$C))){ob_flush();flush();$fa=$g->result("SELECT COUNT(*) FROM ".table($r).($p?" WHERE ".implode(" AND ",$p):""));}else{$Ub=false;}}echo"<p class='pages'>";if(+$C&&$fa>$C){$Rb=floor(($fa-1)/$C);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($_+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$_).($_>5?" ...":"");for($n=max(1,$_-4);$n<min($Rb,$_+5);$n++){echo
pagination($n,$_);}echo($_+5<$Rb?" ...":"").($Ub?pagination($Rb,$_):' <a href="'.h(remove_from_uri()."&page=last").'">'.'poslední'."</a>");}echo" (".($Ub?"":"~ ").lang(array('%d řádek','%d řádky','%d řádků'),$fa).") ".checkbox("all",1,0,'celý výsledek')."\n";if($j->selectCommandPrint()){?>
...
<input type="submit" name="delete" value="Smazat" onclick="return confirm('Opravdu? (' + (this.form['all'].checked ? <?php echo$fa,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
