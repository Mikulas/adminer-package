...
confirm($Oa=""){return" onclick=\"return confirm('".'Opravdu?'.($Oa?" (' + $Oa + ')":"")."');\"";}function
...
upload_error($k){$_c=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'NepodaÅ™ilo se nahrÃ¡t soubor.'.($_c?" ".sprintf('MaximÃ¡lnÃ­ povolenÃ¡ velikost souboru je %sB.',$_c):""):'Soubor neexistuje.');}function
...
enum_input($Zd,$c,$l,$W,$nb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$xc);$I=($nb!==null?"<label><input type='$Zd'$c value='$nb'".((is_array($W)?in_array($nb,$W):$W===0)?" checked":"")."><i>".'prÃ¡zdnÃ©'."</i></label>":"");foreach($xc[1]as$p=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$p+1:(is_array($W)?in_array($p+1,$W):$W===$V));$I.=" <label><input type='$Zd'$c value='".($p+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$I;}function
input($l,$W,$o){global$g,$be,$b,$t;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$od=($t=="mssql"&&$l["auto_increment"]);if($od&&!$_POST["save"])$o=null;$Rb=(isset($_GET["select"])||$od?array("orig"=>'pÅ¯vodnÃ­'):array())+$b->editFunctions($l);$c=" name='fields[$A]'";if($l["type"]=="enum")echo
...
as$A=>$Xa){$l=$m[$A];if(ereg("enum",$l["type"])||like_bool($l)){$u=$mc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'>:",(like_bool($l)?" <select name='where[$p][val]'>".optionlist(array(""=>"",'ne','ano'),$X[$u]["val"],true)."</select>":enum_input("checkbox"," name='where[$p][val][]'",$l,(array)$X[$u]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$A]);}elseif(is_array($Rc=$this->_foreignKeyOptions($_GET["select"],$A))){if($m[$A]["null"])$Rc[0]='('.'prÃ¡zdnÃ©'.')';$u=$mc[$A];$p--;echo"<div>".h($Xa)."<input type='hidden' name='where[$p][col]' value='".h($A)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($Rc,$X[$u]["val"],true)."</select></div>\n";unset($f[$A]);}}$p=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'kdekoliv'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$p][val]' value='".h($V["val"])."' onsearch='selectSearchSearch(this);'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoliv'.")".optionlist($f,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$p][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$f,$s){$Tc=array();foreach($s
as$u=>$r){$B=array();foreach($r["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$u!="PRIMARY")$Tc[$u]=implode(", ",$B);}if($Tc){echo'<fieldset><legend>'.'SeÅ™adit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Tc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
auth_error($ub=null){global$g,$b,$S;$zd=session_name();$k="";if(!$_COOKIE[$zd]&&$_GET[$zd]&&ini_bool("session.use_only_cookies"))$k='Session promÄ›nnÃ© musÃ­ bÃ½t povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$zd]||$_GET[$zd])&&!$S)$k='Session vyprÅ¡ela, pÅ™ihlaÅ¡te se prosÃ­m znovu.';else{$E=&get_session("pwds");if($E!==null){$k=h($ub?$ub->getMessage():(is_string($g)?$g:'NeplatnÃ© pÅ™ihlaÅ¡ovacÃ­ Ãºdaje.'));$E=null;}unset_permanent();}}page_header('PÅ™ihlÃ¡sit se',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½Ã¡dnÃ© rozÅ¡Ã­Å™enÃ­',sprintf('NenÃ­ dostupnÃ© Å¾Ã¡dnÃ© z podporovanÃ½ch PHP rozÅ¡Ã­Å™enÃ­ (%s).',implode(", ",$bd)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$S=$_SESSION["token"];if($d&&$_POST["token"])$_POST["token"]=$S;$k='';if($_POST){if($_POST["token"]!=$S){$gc="max_input_vars";$Ac=ini_get($gc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$u){$V=ini_get($u);if($V&&(!$Ac||$V<$Ac)){$gc=$u;$Ac=$V;}}}$k=(!$_POST["token"]&&$Ac?sprintf('Byl pÅ™ekroÄen maximÃ¡lnÃ­ povolenÃ½ poÄet polÃ­. ZvyÅ¡te prosÃ­m %s.',"'$gc'"):'NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST")$k=sprintf('PÅ™Ã­liÅ¡ velkÃ¡ POST data. ZmenÅ¡ete data nebo zvyÅ¡te hodnotu konfiguraÄnÃ­ direktivy %s.',"'post_max_size'");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}$g->select_db($b->database());function
...
like_bool($l){return
ereg("bool|(tinyint|bit)\\(1\\)",$l["full_type"]);}$Lc="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$eb[DRIVER]='PÅ™ihlÃ¡sit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$m=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$m),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$m=fields($a);$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$m):""):where($_GET,$m));$je=(isset($_GET["select"])?$_POST["edit"]:$X);foreach($m
as$A=>$l){if(!isset($l["privileges"][$je?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($je?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);$s=indexes($a);$ee=unique_array($_GET["where"],$s);$id="\nWHERE $X";if(isset($_POST["delete"])){$G="FROM ".table($a);query_redirect("DELETE".($ee?" $G$id":limit1($G,$id)),$x,'PoloÅ¾ka byla smazÃ¡na.');}else{$N=array();foreach($m
as$A=>$l){$V=process_input($l);if($V!==false&&$V!==null)$N[idf_escape($A)]=($je?"\n".idf_escape($A)." = $V":$V);}if($je){if(!$N)redirect($x);$G=table($a)." SET".implode(",",$N);query_redirect("UPDATE".($ee?" $G$id":limit1($G,$id)),$x,'PoloÅ¾ka byla aktualizovÃ¡na.');}else{$H=insert_into($a,$N);$rc=($H?last_id():0);queries_redirect($x,sprintf('PoloÅ¾ka%s byla vloÅ¾ena.',($rc?" $rc":"")),$H);}}}$Kd=$b->tableName(table_status($a,true));page_header(($je?'Upravit':'VloÅ¾it'),$k,array("select"=>array($a,$Kd)),$Kd);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'UloÅ¾it'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($je?'UloÅ¾it a pokraÄovat v editaci':'UloÅ¾it a vloÅ¾it dalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($je?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
