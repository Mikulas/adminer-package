...
upload_error($v){$uc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Nepodařilo se nahrát soubor.'.($uc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$uc):""):'Soubor neexistuje.');}function
...
input($e,$i,$_){global$Ea,$l,$_a;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$U=(isset($_GET["select"])?array("orig"=>'původní'):array())+$l->editFunctions($e);$P=" name='fields[$f]'";if($e["type"]=="enum"){echo
...
lang($Yc,$Ra){$Zc=($Ra==1||(!$Ra&&'cs'=='fr')?0:((!$Ra||$Ra>=5)&&ereg('cs|sk|ru','cs')?2:1));return
...
selectLinks($Qa,$s=""){$q=$Qa["Name"];if(isset($s)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($q).$s).'">'.'Nová položka'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Stránka'.": ".'poslední'."'>&gt;&gt;</a>\n";}function
...
as$W=>$a){$u.="&set".urlencode("[".bracket_escape($W)."]")."=".urlencode($d[$a]);}echo"<a href='".h($u)."' title='".'Nová položka'."'>+</a> ";}}}}function
...
selectSearchPrint($p,$y,$E){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhledat'."</legend><div>\n";$Ib=array();foreach($p
...
as$a){if(($a["col"]==""||$y[$a["col"]])&&"$a[col]$a[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'kdekoliv'.")".optionlist($y,$a["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$a["op"]),"<input name='where[$o][val]' value='".h($a["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoliv'.")".optionlist($y,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($X,$y,$E){$Db=array();foreach($E
as$b=>$Q){$X=array();foreach($Q["columns"]as$a){$X[]=$this->fieldName(array("field"=>$a,"comment"=>$y[$a]));}if(count(array_filter($X,'strlen'))>1&&$b!="PRIMARY"){$Db[$b]=implode(", ",$X);}}if($Db){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Db,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
long2str($n,true);}$g='';if(!$ua){page_header('Žádná extenze',sprintf('Není dostupná žádná z podporovaných PHP extenzí (%s).',implode(", ",$kb)),null);page_footer("auth");exit;}$ga=$_SESSION["token"];if(!$_SESSION["token"]){$_SESSION["token"]=rand(1,1e6);}$ra=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$a){list($b)=explode(":",$a);$ra[$b]=$a;}}if(isset($_POST["server"])){session_regenerate_id();$_SESSION["pwds"][$_POST["driver"]][$_POST["server"]][$_POST["username"]]=$_POST["password"];if($_POST["permanent"]){$b=base64_encode($_POST["driver"])."-".base64_encode($_POST["server"])."-".base64_encode($_POST["username"]);$ab=$l->permanentLogin();$ra[$b]="$b:".base64_encode($ab?encrypt_string($_POST["password"],$ab):"");cookie("adminer_permanent",implode(" ",$ra));}if(count($_POST)==($_POST["permanent"]?5:4)||DRIVER!=$_POST["driver"]||SERVER!=$_POST["server"]||$_GET["username"]!==$_POST["username"]){redirect(auth_url($_POST["driver"],$_POST["server"],$_POST["username"]));}}elseif($_POST["logout"]){if($ga&&$_POST["token"]!=$ga){page_header('Odhlásit','Neplatný token CSRF. Odešlete formulář znovu.');page_footer("db");exit;}else{foreach(array("pwds","dbs","queries")as$b){set_session($b,null);}$b=base64_encode(DRIVER)."-".base64_encode(SERVER)."-".base64_encode($_GET["username"]);if($ra[$b]){unset($ra[$b]);cookie("adminer_permanent",implode(" ",$ra));}redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),'Odhlášení proběhlo v pořádku.');}}elseif($ra&&!$_SESSION["pwds"]){session_regenerate_id();$ab=$l->permanentLogin();foreach($ra
...
auth_error($mc=null){global$g,$l,$ga;$cb=session_name();$v="";if(!$_COOKIE[$cb]&&$_GET[$cb]&&ini_bool("session.use_only_cookies")){$v='Session proměnné musí být povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$cb]||$_GET[$cb])&&!$ga){$v='Session vypršela, přihlašte se prosím znovu.';}else{$M=&get_session("pwds");if(isset($M)){$v=h($mc?$mc->getMessage():(is_string($g)?$g:'Neplatné přihlašovací údaje.'));$M=null;}}}page_header('Přihlásit se',$v,null);echo"<form action='' method='post'>\n";$l->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])&&class_exists("Min_DB")){$g=connect();}if(is_string($g)||!$l->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$ga=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$ga;}$v=($_POST?($_POST["token"]==$ga?"":'Neplatný token CSRF. Odešlete formulář znovu.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Příliš velká POST data. Zmenšete data nebo zvyšte hodnotu konfigurační direktivy %s.','"post_max_size"')));$g->select_db($l->database());function
...
dump_headers($oc){$qa=($oc!=""?friendly_url($oc):"dump");$Wb="csv";header("Content-Type: text/csv; charset=utf-8");header("Content-Disposition: attachment; filename=$qa.$Wb");session_write_close();return$Wb;}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Kb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$qd=" onclick=\"return confirm('".'Opravdu?'."');\"";$ua[DRIVER]='Přihlásit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$q=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$q-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($q)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$q=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$ea=(isset($_GET["select"])?$_POST["edit"]:$p);$r=fields($q);foreach($r
as$f=>$e){if(!isset($e["privileges"][$ea?"update":"insert"])||$l->fieldName($e)==""){unset($r[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$K=$_POST["referer"];if($_POST["insert"]){$K=($ea?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$K)){$K=ME."select=".urlencode($q);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($q)," WHERE $p"),$K,'Položka byla smazána.');}else{$s=array();foreach($r
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($ea?"\n".idf_escape($f)." = $a":$a);}}if($ea){if(!$s){redirect($K);}query_redirect("UPDATE".limit1(table($q)." SET".implode(",",$s),"\nWHERE $p"),$K,'Položka byla aktualizována.');}else{$h=insert_into($q,$s);$nc=($h?last_id():0);queries_redirect($K,sprintf('Položka%s byla vložena.',($nc?" $nc":"")),$h);}}}$Oa=$l->tableName(table_status($q));page_header(($ea?'Upravit':'Vložit'),$v,array("select"=>array($q,$Oa)),$Oa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$t=array();foreach($r
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($r){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($ea?'Uložit a pokračovat v editaci':'Uložit a vložit další')."\">\n";}}if($ea){echo"<input type='submit' name='delete' value='".'Smazat'."'$qd>\n";}echo'</form>
