...
upload_error($k){$uc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Nepodařilo se nahrát soubor.'.($uc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$uc):""):'Soubor neexistuje.');}function
...
enum_input($Rd,$c,$l,$W,$mb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$_);$K=($mb!==null?"<label><input type='$Rd'$c value='$mb'".((is_array($W)?in_array($mb,$W):$W===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($_[1]as$q=>$V){$V=stripcslashes(str_replace("''","'",$V));$Ba=(is_int($W)?$W==$q+1:(is_array($W)?in_array($q+1,$W):$W===$V));$K.=" <label><input type='$Rd'$c value='".($q+1)."'".($Ba?' checked':'').'>'.h($b->editVal($V,$l)).'</label>';}return$K;}function
input($l,$W,$p){global$Td,$b,$u;$C=h(bracket_escape($l["field"]));echo"<td class='function'>";$gd=($u=="mssql"&&$l["auto_increment"]);if($gd&&!$_POST["save"])$p=null;$Mb=(isset($_GET["select"])||$gd?array("orig"=>'původní'):array())+$b->editFunctions($l);$c=" name='fields[$C]'";if($l["type"]=="enum")echo
...
selectVal($V,$x,$l){$K=($V===null?"&nbsp;":$V);$x=h($x);if(ereg('blob|bytea',$l["type"])&&!is_utf8($V)){$K=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($V));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$V))$K="<img src='$x' alt='$K'>";}if(like_bool($l)&&$K!="&nbsp;")$K=($V?'ano':'ne');if($x)$K="<a href='$x'>$K</a>";if(!$x&&!like_bool($l)&&ereg('int|float|double|decimal',$l["type"]))$K="<div class='number'>$K</div>";elseif(ereg('date',$l["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($X,$f,$t){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Vyhledat'."</legend><div>\n";$hc=array();foreach($X
as$v=>$V)$hc[$V["col"]]=$v;$q=0;$m=fields($_GET["select"]);foreach($f
as$C=>$Wa){$l=$m[$C];if(ereg("enum",$l["type"])||like_bool($l)){$v=$hc[$C];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($C)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'ne','ano'),$X[$v]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$v]["val"],($l["null"]?0:null))),"</div>\n";unset($f[$C]);}elseif(is_array($Kc=$this->_foreignKeyOptions($_GET["select"],$C))){if($m[$C]["null"])$Kc[0]='('.'prázdné'.')';$v=$hc[$C];$q--;echo"<div>".h($Wa)."<input type='hidden' name='where[$q][col]' value='".h($C)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Kc,$X[$v]["val"],true)."</select></div>\n";unset($f[$C]);}}$q=0;foreach($X
as$V){if(($V["col"]==""||$f[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$q][col]'><option value=''>(".'kdekoliv'.")".optionlist($f,$V["col"],true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators,$V["op"]),"<input type='search' name='where[$q][val]' value='".h($V["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoliv'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($D,$f,$t){$Mc=array();foreach($t
as$v=>$s){$D=array();foreach($s["columns"]as$V)$D[]=$f[$V];if(count(array_filter($D,'strlen'))>1&&$v!="PRIMARY")$Mc[$v]=implode(", ",$D);}if($Mc){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Mc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
like_bool($l){return
ereg("bool|(tinyint|bit)\\(1\\)",$l["full_type"]);}$Ec="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$db[DRIVER]='Přihlásit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$m=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$m),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$m=fields($a);$X=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$m):""):where($_GET,$m));$be=(isset($_GET["select"])?$_POST["edit"]:$X);foreach($m
as$C=>$l){if(!isset($l["privileges"][$be?"update":"insert"])||$b->fieldName($l)=="")unset($m[$C]);}if($_POST&&!$k&&!isset($_GET["select"])){$y=$_POST["referer"];if($_POST["insert"])$y=($be?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$y))$y=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$y,'Položka byla smazána.');else{$P=array();foreach($m
as$C=>$l){$V=process_input($l);if($V!==false&&$V!==null)$P[idf_escape($C)]=($be?"\n".idf_escape($C)." = $V":$V);}if($be){if(!$P)redirect($y);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $X"),$y,'Položka byla aktualizována.');}else{$J=insert_into($a,$P);$mc=($J?last_id():0);queries_redirect($y,sprintf('Položka%s byla vložena.',($mc?" $mc":"")),$J);}}}$Dd=$b->tableName(table_status($a));page_header(($be?'Upravit':'Vložit'),$k,array("select"=>array($a,$Dd)),$Dd);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($X){$N=array();foreach($m
as$C=>$l){if(isset($l["privileges"]["select"])){$ma=convert_field($l);if($_POST["clone"]&&$l["auto_increment"])$ma="''";if($u=="sql"&&ereg("enum|set",$l["type"]))$ma="1*".idf_escape($C);$N[]=($ma?"$ma AS ":"").idf_escape($C);}}$L=array();if($N){$M=get_rows("SELECT".limit(implode(", ",$N)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$L=(isset($_GET["select"])&&count($M)!=1?null:reset($M));}}if($L===false)echo"<p class='error'>".'Žádné řádky.'."\n";echo'
...
<input type="submit" name="delete" value="Smazat" onclick="return confirm('Opravdu? (' + (this.form['all'].checked ? <?php echo$o,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
