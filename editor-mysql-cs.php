...
remove_slashes($ra,$vb=false){if(get_magic_quotes_gpc()){while(list($c,$a)=each($ra)){foreach($a
as$W=>$p){unset($ra[$c][$W]);if(is_array($p)){$ra[$c][stripslashes($W)]=$p;$ra[]=&$ra[$c][stripslashes($W)];}else{$ra[$c][stripslashes($W)]=($vb?$p:stripslashes($p));}}}}}function
bracket_escape($aa,$bd=false){static$Gc=array(':'=>':1',']'=>':2','['=>':3');return
strtr($aa,($bd?array_flip($Gc):$Gc));}function
h($F){return
htmlspecialchars(str_replace("\0","",$F),ENT_QUOTES);}function
nbsp($F){return(trim($F)!=""?h($F):"&nbsp;");}function
nl_br($F){return
str_replace("\n","<br>",$F);}function
checkbox($f,$i,$Za,$Vc="",$ub="",$Zc=false){static$_=0;$_++;$b="<input type='checkbox' name='$f' value='".h($i)."'".($Za?" checked":"").($ub?' onclick="'.h($ub).'"':'').($Zc?" class='jsonly'":"")." id='checkbox-$_'>";return($Vc!=""?"<label for='checkbox-$_'>$b".h($Vc)."</label>":$b);}function
optionlist($Z,$Xc=null,$mc=false){$b="";foreach($Z
as$W=>$p){$jc=array($W=>$p);if(is_array($p)){$b.='<optgroup label="'.h($W).'">';$jc=$p;}foreach($jc
as$c=>$a){$b.='<option'.($mc||is_string($c)?' value="'.h($c).'"':'').(($mc||is_string($c)?(string)$c:$a)===$Xc?' selected':'').'>'.h($a);}if(is_array($p)){$b.='</optgroup>';}}return$b;}function
...
confirm($Pc="",$xd=false){return" onclick=\"".($xd?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Pc?" (' + $Pc + ')":"")."');\"";}function
print_fieldset($_,$Dd,$Ed=false,$ub=""){echo"<fieldset><legend><a href='#fieldset-$_' onclick=\"".h($ub)."return !toggle('fieldset-$_');\">$Dd</a></legend><div id='fieldset-$_'".($Ed?"":" class='hidden'").">\n";}function
...
where_check($a){parse_str($a,$gc);remove_slashes(array(&$gc));return
where($gc);}function
...
cookie($f,$i){global$bb;$cb=array($f,(ereg("\n",$i)?"":$i),time()+2592000,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$bb);if(version_compare(PHP_VERSION,'5.2.0')>=0){$cb[]=true;}return
...
auth_url($kb,$E,$D){global$va;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($va))."|username|".session_name()),$m);return"$m[1]?".(sid()?SID."&":"").($kb!="server"||$E!=""?urlencode($kb)."=".urlencode($E)."&":"")."username=".urlencode($D).($m[2]?"&$m[2]":"");}function
...
query_redirect($l,$I,$X,$fb=true,$_d=true,$lc=false){global$g,$v,$j;if($_d){$lc=!$g->query($l);}$Ib="";if($l){$Ib=$j->messageQuery("$l;");}if($lc){$v=error().$Ib;return
false;}if($fb){redirect($I,$X.$Ib);}return
true;}function
queries($l=null){global$g;static$ic=array();if(!isset($l)){return
implode(";\n",$ic);}$ic[]=(ereg(';$',$l)?"DELIMITER ;;\n$l;\nDELIMITER ":$l);return$g->query($l);}function
apply_queries($l,$ga,$cd='table'){foreach($ga
as$h){if(!queries("$l ".$cd($h))){return
false;}}return
true;}function
queries_redirect($I,$X,$fb){return
query_redirect(queries(),$I,$X,$fb,false,!$fb);}function
...
get_file($c,$Jb=false){$R=$_FILES[$c];if(!$R||$R["error"]){return$R["error"];}$b=file_get_contents($Jb&&ereg('\\.gz$',$R["name"])?"compress.zlib://$R[tmp_name]":($Jb&&ereg('\\.bz2$',$R["name"])?"compress.bzip2://$R[tmp_name]":$R["tmp_name"]));if($Jb){$xc=substr($b,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$xc,$Qd)){$b=iconv("utf-16","utf-8",$b);}elseif($xc=="\xEF\xBB\xBF"){$b=substr($b,3);}}return$b;}function
upload_error($v){$Nc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Nepodařilo se nahrát soubor.'.($Nc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Nc):""):'Soubor neexistuje.');}function
repeat_pattern($K,$ab){return
str_repeat("$K{0,65535}",$ab/65535)."$K{0,".($ab
%
65535)."}";}function
is_utf8($a){return(preg_match('~~u',$a)&&!preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~',$a));}function
shorten_utf8($F,$ab=80,$Kd=""){if(!preg_match("(^(".repeat_pattern("[\t\r\n -\x{FFFF}]",$ab).")($)?)u",$F,$m)){preg_match("(^(".repeat_pattern("[\t\r\n -~]",$ab).")($)?)",$F,$m);}return
...
enum_input($ua,$L,$e,$i,$Hb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($Hb)?"<label><input type='$ua'$L value='$Hb'".((is_array($i)?in_array($Hb,$i):$i===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($U[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Za=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ua'$L value='".($n+1)."'".($Za?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
input($e,$i,$z){global$Ba,$j,$ka;$f=h(bracket_escape($e["field"]));echo"<td class='function'>";$rc=($ka=="mssql"&&$e["auto_increment"]);if($rc&&!$_POST["save"]){$z=null;}$da=(isset($_GET["select"])||$rc?array("orig"=>'původní'):array())+$j->editFunctions($e);$L=" name='fields[$f]'";if($e["type"]=="enum"){echo
nbsp($da[""])."<td>".$j->editInput($_GET["edit"],$e,$L,$i);}else{$ca=0;foreach($da
as$c=>$a){if($c===""||!$a){break;}$ca++;}$za=($ca?" onchange=\"var f = this.form['function[".h(js_escape(bracket_escape($e["field"])))."]']; if ($ca > f.selectedIndex) f.selectedIndex = $ca;\"":"");$L.=$za;echo(count($da)>1?html_select("function[$f]",$da,!isset($z)||in_array($z,$da)||isset($da[$z])?$z:"","functionChange(this);"):nbsp(reset($da))).'<td>';$Tc=$j->editInput($_GET["edit"],$e,$L,$i);if($Tc!=""){echo$Tc;}elseif($e["type"]=="set"){preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);foreach($U[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Za=(is_int($i)?($i>>$n)&1:in_array($a,explode(",",$i),true));echo" <label><input type='checkbox' name='fields[$f][$n]' value='".(1<<$n)."'".($Za?' checked':'')."$za>".h($j->editVal($a,$e)).'</label>';}}elseif(ereg('blob|bytea|raw|file',$e["type"])&&ini_bool("file_uploads")){echo"<input type='file' name='fields-$f'$za>";}elseif(ereg('text|lob',$e["type"])){echo"<textarea ".($ka!="sqlite"||ereg("\n",$i)?"cols='50' rows='12'":"cols='30' rows='1' style='height: 1.2em;'")."$L>".h($i).'</textarea>';}else{$Gb=(!ereg('int',$e["type"])&&preg_match('~^(\\d+)(,(\\d+))?$~',$e["length"],$m)?((ereg("binary",$e["type"])?2:1)*$m[1]+($m[3]?1:0)+($m[2]&&!$e["unsigned"]?1:0)):($Ba[$e["type"]]?$Ba[$e["type"]]+($e["unsigned"]?0:1):0));echo"<input value='".h($i)."'".($Gb?" maxlength='$Gb'":"").(ereg('char|binary',$e["type"])&&$Gb>20?" size='40'":"")."$L>";}}}function
...
password_file(){$wb=ini_get("upload_tmp_dir");if(!$wb){if(function_exists('sys_get_temp_dir')){$wb=sys_get_temp_dir();}else{$Ea=@tempnam("","");if(!$Ea){return
false;}$wb=dirname($Ea);unlink($Ea);}}$Ea="$wb/adminer.key";$b=@file_get_contents($Ea);if($b){return$b;}$Zb=@fopen($Ea,"w");if($Zb){$b=md5(uniqid(mt_rand(),true));fwrite($Zb,$b);fclose($Zb);}return$b;}function
is_mail($Fa){$wc='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$Ya='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$K="$wc+(\\.$wc+)*@($Ya?\\.)+$Ya";return
preg_match("(^$K(,\\s*$K)*\$)i",$Fa);}function
is_url($F){$Ya='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($Ya?\\.)+$Ya(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$F,$m)?strtolower($m[1]):"");}global$j,$g,$va,$hd,$Vb,$v,$da,$ud,$bb,$_c,$ka,$rd,$Rd,$qb,$Bc,$ea,$Nd,$Ba,$qd,$Bb;if(!isset($_SERVER["REQUEST_URI"])){$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"].($_SERVER["QUERY_STRING"]!=""?"?$_SERVER[QUERY_STRING]":"");}$bb=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$cb=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$bb);if(version_compare(PHP_VERSION,'5.2.0')>=0){$cb[]=true;}call_user_func_array('session_set_cookie_params',$cb);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$vb);if(function_exists("set_magic_quotes_runtime")){set_magic_quotes_runtime(false);}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'cs';}function
lang($pd,$Ab){$od=($Ab==1?0:($Ab&&$Ab<5?1:2));return
sprintf($pd[$od],$Ab);}if(extension_loaded('pdo')){class
Min_PDO
extends
...
Min_DB;$Wb=$j->credentials();if($g->connect($Wb[0],$Wb[1],$Wb[2])){$g->query("SET sql_quote_show_create = 1");return$g;}return$g->error;}function
...
limit($l,$o,$C,$sc=0,$sa=" "){return" $l$o".(isset($C)?$sa."LIMIT $C".($sc?" OFFSET $sc":""):"");}function
limit1($l,$o){return
limit($l,$o,1);}function
db_collation($ta,$id){global$g;$b=null;$ib=$g->result("SHOW CREATE DATABASE ".idf_escape($ta),1);if(preg_match('~ COLLATE ([^ ]+)~',$ib,$m)){$b=$m[1];}elseif(preg_match('~ CHARACTER SET ([^ ]+)~',$ib,$m)){$b=$id[$m[1]][-1];}return$b;}function
...
foreign_keys($h){global$g,$qb;static$K='`(?:[^`]|``)+`';$b=array();$yc=$g->result("SHOW CREATE TABLE ".table($h),1);if($yc){preg_match_all("~CONSTRAINT ($K) FOREIGN KEY \\(((?:$K,? ?)+)\\) REFERENCES ($K)(?:\\.($K))? \\(((?:$K,? ?)+)\\)(?: ON DELETE ($qb))?(?: ON UPDATE ($qb))?~",$yc,$U,PREG_SET_ORDER);foreach($U
as$m){preg_match_all("~$K~",$m[2],$Tb);preg_match_all("~$K~",$m[5],$Y);$b[idf_unescape($m[1])]=array("db"=>idf_unescape($m[4]!=""?$m[3]:$m[4]),"table"=>idf_unescape($m[4]!=""?$m[4]:$m[3]),"source"=>array_map('idf_unescape',$Tb[0]),"target"=>array_map('idf_unescape',$Y[0]),"on_delete"=>$m[6],"on_update"=>$m[7],);}}return$b;}function
...
true;}}return
false;}function
auto_increment(){$bc=" PRIMARY KEY";if($_GET["create"]!=""&&$_POST["auto_increment_col"]){foreach(indexes($_GET["create"])as$T){if(in_array($_POST["fields"][$_POST["auto_increment_col"]]["orig"],$T["columns"],true)){$bc="";break;}if($T["type"]=="PRIMARY"){$bc=" UNIQUE";}}}return" AUTO_INCREMENT$bc";}function
alter_table($h,$f,$q,$ld,$kd,$Sc,$Aa,$db,$jd){$ia=array();foreach($q
as$e){$ia[]=($e[1]?($h!=""?($e[0]!=""?"CHANGE ".idf_escape($e[0]):"ADD"):" ")." ".implode($e[1]).($h!=""?" $e[2]":""):"DROP ".idf_escape($e[0]));}$ia=array_merge($ia,$ld);$Uc="COMMENT=".q($kd).($Sc?" ENGINE=".q($Sc):"").($Aa?" COLLATE ".q($Aa):"").($db!=""?" AUTO_INCREMENT=$db":"").$jd;if($h==""){return
queries("CREATE TABLE ".table($f)." (\n".implode(",\n",$ia)."\n) $Uc");}if($h!=$f){$ia[]="RENAME TO ".table($f);}$ia[]=$Uc;return
...
routine($f,$ua){global$g,$Vb,$_c,$Ba;$Gd=array("bool","boolean","integer","double precision","real","dec","numeric","fixed","national char","national varchar");$Rc="((".implode("|",array_merge(array_keys($Ba),$Gd)).")(?:\\s*\\(((?:[^'\")]*|$Vb)+)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s]+)['\"]?)?";$K="\\s*(".($ua=="FUNCTION"?"":$_c).")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$Rc";$ib=$g->result("SHOW CREATE $ua ".idf_escape($f),2);preg_match("~\\(((?:$K\\s*,?)*)\\)".($ua=="FUNCTION"?"\\s*RETURNS\\s+$Rc":"")."\\s*(.*)~is",$ib,$m);$q=array();preg_match_all("~$K\\s*,?~is",$m[1],$U,PREG_SET_ORDER);foreach($U
as$ha){$f=str_replace("``","`",$ha[2]).$ha[3];$q[]=array("field"=>$f,"type"=>strtolower($ha[5]),"length"=>preg_replace_callback("~$Vb~s",'normalize_enum',$ha[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$ha[8] $ha[7]"))),"full_type"=>$ha[4],"inout"=>strtoupper($ha[1]),"collation"=>strtolower($ha[9]),);}if($ua!="FUNCTION"){return
...
true;}function
create_sql($h,$db){global$g;$b=$g->result("SHOW CREATE TABLE ".table($h),1);if(!$db){$b=preg_replace('~ AUTO_INCREMENT=\\d+~','',$b);}return$b;}function
truncate_sql($h){return"TRUNCATE ".table($h);}function
use_sql($Xa){return"USE ".idf_escape($Xa);}function
trigger_sql($h,$sb){$b="";foreach(get_rows("SHOW TRIGGERS LIKE ".q(addcslashes($h,"%_")),null,"-- ")as$d){$b.="\n".($sb=='CREATE+ALTER'?"DROP TRIGGER IF EXISTS ".idf_escape($d["Trigger"]).";;\n":"")."CREATE TRIGGER ".idf_escape($d["Trigger"])." $d[Timing] $d[Event] ON ".table($d["Table"])." FOR EACH ROW\n$d[Statement];;\n";}return$b;}function
...
selectLinks($Wa,$s=""){$r=$Wa["Name"];if(isset($s)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($r).$s).'">'.'Nová položka'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Poslední stránka'."'>&gt;&gt;</a>\n";}function
...
as$c=>$a){$f=$this->tableName(table_status($c));if($f!=""){$Mc=preg_quote($Md);$sa="(:|\\s*-)?\\s+";$b[$c]["name"]=(preg_match("(^$Mc$sa(.+)|^(.+?)$sa$Mc\$)iu",$f,$m)?$m[2].$m[3]:$f);}else{unset($b[$c]);}}return$b;}function
...
as$H=>$a){$t.="&set".urlencode("[".bracket_escape($H)."]")."=".urlencode($d[$a]);}echo"<a href='".h($t)."' title='".'Nová položka'."'>+</a> ";}}}function
...
rowDescriptions($w,$Qa){$b=$w;foreach($w[0]as$c=>$a){if(list($h,$_,$f)=$this->_foreignColumn($Qa,$c)){$nc=array();foreach($w
as$d){$nc[$d[$c]]=exact_value($d[$c]);}$Kb=$this->_values[$h];if(!$Kb){$Kb=get_key_vals("SELECT $_, $f FROM ".table($h)." WHERE $_ IN (".implode(", ",$nc).")");}foreach($w
as$x=>$d){if(isset($d[$c])){$b[$x][$c]=(string)$Kb[$d[$c]];}}}}return$b;}function
selectVal($a,$t,$e){$b=($a=="<i>NULL</i>"?"&nbsp;":$a);if(ereg('blob|bytea',$e["type"])&&!is_utf8($a)){$b=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($a));if(ereg("^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)",$a)){$b="<img src='$t' alt='$b'>";}}if(like_bool($e)&&$b!="&nbsp;"){$b='<img src="'.($a?"".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2":"".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.2").'" alt="'.h($a).'">';}if($t){$b="<a href='$t'>$b</a>";}if(!$t&&!like_bool($e)&&ereg('int|float|double|decimal',$e["type"])){$b="<div class='number'>$b</div>";}elseif(ereg('date',$e["type"])){$b="<div class='datetime'>$b</div>";}return$b;}function
...
selectSearchPrint($o,$A,$G){$o=(array)$_GET["where"];echo'<fieldset><legend>'.'Vyhledat'."</legend><div>\n";$Lb=array();foreach($o
as$c=>$a){$Lb[$a["col"]]=$c;}$n=0;$q=fields($_GET["select"]);foreach($q
as$f=>$e){$na=$A[$f];if(ereg("enum",$e["type"])&&$na!=""){$c=$Lb[$f];$n--;echo"<div>".h($na)."<input type='hidden' name='where[$n][col]' value='".h($f)."'>:",enum_input("checkbox"," name='where[$n][val][]'",$e,(array)$o[$c]["val"],($e["null"]?0:null)),"</div>\n";unset($A[$f]);}}foreach($A
as$f=>$na){$Z=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Z)){if($q[$f]["null"]){$Z[0]='('.'prázdné'.')';}$c=$Lb[$f];$n--;echo"<div>".h($na)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Z,$o[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($o
as$a){if(($a["col"]==""||$A[$a["col"]])&&"$a[col]$a[val]"!=""){echo"<div><select name='where[$n][col]'><option value=''>(".'kdekoliv'.")".optionlist($A,$a["col"],true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators,$a["op"]),"<input name='where[$n][val]' value='".h($a["val"])."'></div>\n";$n++;}}echo"<div><select name='where[$n][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoliv'.")".optionlist($A,null,true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators),"<input name='where[$n][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($V,$A,$G){$Mb=array();foreach($G
as$c=>$T){$V=array();foreach($T["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Mb[$c]=implode(", ",$V);}}if($Mb){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Mb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
true;}function
selectEmailPrint($mb,$A){if($mb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Odesílatel'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Předmět'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n";echo"<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$A,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložit'."'>\n";echo"<p>".'Přílohy'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($mb)==1?'<input type="hidden" name="email_field" value="'.h(key($mb)).'">':html_select("email_field",$mb)),"<input type='submit' name='email' value='".'Odeslat'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
selectColumnsProcess($A,$G){return
array(array(),array());}function
selectSearchProcess($q,$G){$b=array();foreach((array)$_GET["where"]as$c=>$o){$qa=$o["col"];$Oa=$o["op"];$a=$o["val"];if(($c<0?"":$qa).$a!=""){$Ra=array();foreach(($qa!=""?array($qa=>$q[$qa]):$q)as$f=>$e){if($qa!=""||is_numeric($a)||!ereg('int|float|double|decimal',$e["type"])){$f=idf_escape($f);if($qa!=""&&$e["type"]=="enum"){$Ra[]=(in_array(0,$a)?"$f IS NULL OR ":"")."$f IN (".implode(", ",array_map('intval',$a)).")";}else{$cc=ereg('char|text|enum|set',$e["type"]);$i=$this->processInput($e,(!$Oa&&$cc&&ereg('^[^%]+$',$a)?"%$a%":$a));$Ra[]=$f.($i=="NULL"?" IS".($Oa==">="?" NOT":"")." $i":(in_array($Oa,$this->operators)||$Oa=="="?" $Oa $i":($cc?" LIKE $i":" IN (".str_replace(",","', '",$i).")")));if($c<0&&$a=="0"){$Ra[]="$f IS NULL";}}}}$b[]=($Ra?"(".implode(" OR ",$Ra).")":"0");}}return$b;}function
...
true;}if($_POST["email"]){$fc=0;if($_POST["all"]||$_POST["check"]){$e=idf_escape($_POST["email_field"]);$gb=$_POST["email_subject"];$X=$_POST["email_message"];preg_match_all('~\\{\\$([a-z0-9_]+)\\}~i',"$gb.$X",$U);$w=get_rows("SELECT DISTINCT $e".($U[1]?", ".implode(", ",array_map('idf_escape',array_unique($U[1]))):"")." FROM ".table($_GET["select"])." WHERE $e IS NOT NULL AND $e != ''".($o?" AND ".implode(" AND ",$o):"").($_POST["all"]?"":" AND ((".implode(") OR (",array_map('where_check',(array)$_POST["check"]))."))"));$q=fields($_GET["select"]);foreach($this->rowDescriptions($w,$Qa)as$d){$ac=array('{\\'=>'{');foreach($U[1]as$a){$ac['{$'."$a}"]=$this->editVal($d[$a],$q[$a]);}$Fa=$d[$_POST["email_field"]];if(is_mail($Fa)&&send_mail($Fa,strtr($gb,$ac),strtr($X,$ac),$_POST["email_from"],$_FILES["email_files"])){$fc++;}}}cookie("adminer_email",$_POST["email_from"]);redirect(remove_from_uri(),lang(array('Byl odeslán %d e-mail.','Byly odeslány %d e-maily.','Bylo odesláno %d e-mailů.'),$fc));}return
false;}function
messageQuery($l){return"<!--\n".str_replace("--","--><!-- ",$l)."\n-->";}function
editFunctions($e){$b=array();if($e["null"]&&ereg('blob',$e["type"])){$b["NULL"]='prázdné';}$b[""]=($e["null"]||$e["auto_increment"]||like_bool($e)?"":"*");if(ereg('date|time',$e["type"])){$b["now"]='teď';}if(eregi('_(md5|sha1)$',$e["field"],$m)){$b[]=strtolower($m[1]);}return$b;}function
editInput($h,$e,$L,$i){if($e["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$L value='-1' checked><i>".'původní'."</i></label> ":"").enum_input("radio",$L,$e,($i||isset($_GET["select"])?$i:0),($e["null"]?"":null));}$Z=$this->_foreignKeyOptions($h,$e["field"],$i);if(isset($Z)){return(is_array($Z)?"<select$L>".optionlist($Z,$i,true)."</select>":"<input value='".h($i)."'$L class='hidden'><input value='".h($Z)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($h)."&field=".urlencode($e["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");}if(like_bool($e)){return'<input type="checkbox" value="'.h($i?$i:1).'"'.($i?' checked':'')."$L>";}$Ia="";if(ereg('time',$e["type"])){$Ia='HH:MM:SS';}if(ereg('date|timestamp',$e["type"])){$Ia='d.m.[rrrr]'.($Ia?" [$Ia]":"");}if($Ia){return"<input value='".h($i)."'$L> ($Ia)";}if(eregi('_(md5|sha1)$',$e["field"])){return"<input type='password' value='".h($i)."'$L>";}return'';}function
...
dumpData($h,$sb,$l){global$g;$k=$g->query($l,1);if($k){while($d=$k->fetch_assoc()){if($sb=="table"){dump_csv(array_keys($d));$sb="INSERT";}dump_csv($d);}}}function
dumpHeaders($hb,$Hc=false){$zd="csv";header("Content-Type: text/csv; charset=utf-8");return$zd;}function
homepage(){return
true;}function
...
_foreignColumn($Qa,$H){foreach((array)$Qa[$H]as$jb){if(count($jb["source"])==1){$f=$this->rowDescription($jb["table"]);if($f!=""){$_=idf_escape($jb["target"][0]);return
array($jb["table"],$_,$f);}}}}function
...
Adminer);function
page_header($oc,$v="",$Va=array(),$hc=""){global$rd,$j,$g,$va;header("Content-Type: text/html; charset=utf-8");if($j->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}$qc=$oc.($hc!=""?": ".h($hc):"");$ec=strip_tags($qc.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$j->name());if(is_ajax()){header("X-AJAX-Title: ".rawurlencode($ec));}else{echo'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN">
...
as$c=>$a){$na=(is_array($a)?$a[1]:$a);if($na!=""){echo'<a href="'.h(ME."$c=").urlencode(is_array($a)?$a[0]:$a).'">'.h($na).'</a> &raquo; ';}}}echo"$oc\n";}}echo"<span id='loader'></span>\n","<h2>$qc</h2>\n";restart_session();$pc=preg_replace('~^[^?]*~','',$_SERVER["REQUEST_URI"]);$kc=$_SESSION["messages"][$pc];if($kc){echo"<div class='message'>".implode("</div>\n<div class='message'>",$kc)."</div>\n";unset($_SESSION["messages"][$pc]);}$fa=&get_session("dbs");if(DB!=""&&$fa&&!in_array(DB,$fa,true)){$fa=null;}if($v){echo"<div class='error'>$v</div>\n";}define("PAGE_HEADER",1);}function
...
long2str($p,$Nb){$xa='';foreach($p
as$a){$xa.=pack('V',$a);}if($Nb){return
substr($xa,0,end($p));}return$xa;}function
str2long($xa,$Nb){$p=array_values(unpack('V*',str_pad($xa,4*ceil(strlen($xa)/4),"\0")));if($Nb){$p[]=strlen($xa);}return$p;}function
xxtea_mx($M,$N,$S,$W){return
int32((($M>>5&0x7FFFFFF)^$N<<2)+(($N>>3&0x1FFFFFFF)^$M<<4))^int32(($S^$N)+($W^$M));}function
encrypt_string($Ua,$c){if($Ua==""){return"";}$c=array_values(unpack("V*",pack("H*",md5($c))));$p=str2long($Ua,true);$x=count($p)-1;$M=$p[$x];$N=$p[0];$Yb=floor(6+52/($x+1));$S=0;while($Yb-->0){$S=int32($S+0x9E3779B9);$Na=$S>>2&3;for($O=0;$O<$x;$O++){$N=$p[$O+1];$ya=xxtea_mx($M,$N,$S,$c[$O&3^$Na]);$M=int32($p[$O]+$ya);$p[$O]=$M;}$N=$p[0];$ya=xxtea_mx($M,$N,$S,$c[$O&3^$Na]);$M=int32($p[$x]+$ya);$p[$x]=$M;}return
long2str($p,false);}function
decrypt_string($Ua,$c){if($Ua==""){return"";}$c=array_values(unpack("V*",pack("H*",md5($c))));$p=str2long($Ua,false);$x=count($p)-1;$M=$p[$x];$N=$p[0];$Yb=floor(6+52/($x+1));$S=int32($Yb*0x9E3779B9);while($S){$Na=$S>>2&3;for($O=$x;$O>0;$O--){$M=$p[$O-1];$ya=xxtea_mx($M,$N,$S,$c[$O&3^$Na]);$N=int32($p[$O]-$ya);$p[$O]=$N;}$M=$p[$x];$ya=xxtea_mx($M,$N,$S,$c[$O&3^$Na]);$N=int32($p[0]-$ya);$p[0]=$N;$S=int32($S-0x9E3779B9);}return
long2str($p,true);}$g='';$ea=$_SESSION["token"];if(!$_SESSION["token"]){$_SESSION["token"]=rand(1,1e6);}$wa=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$a){list($c)=explode(":",$a);$wa[$c]=$a;}}if(isset($_POST["server"])){session_regenerate_id();$_SESSION["pwds"][$_POST["driver"]][$_POST["server"]][$_POST["username"]]=$_POST["password"];if($_POST["permanent"]){$c=base64_encode($_POST["driver"])."-".base64_encode($_POST["server"])."-".base64_encode($_POST["username"]);$_b=$j->permanentLogin();$wa[$c]="$c:".base64_encode($_b?encrypt_string($_POST["password"],$_b):"");cookie("adminer_permanent",implode(" ",$wa));}if(count($_POST)==($_POST["permanent"]?5:4)||DRIVER!=$_POST["driver"]||SERVER!=$_POST["server"]||$_GET["username"]!==$_POST["username"]){redirect(auth_url($_POST["driver"],$_POST["server"],$_POST["username"]));}}elseif($_POST["logout"]){if($ea&&$_POST["token"]!=$ea){page_header('Odhlásit','Neplatný token CSRF. Odešlete formulář znovu.');page_footer("db");exit;}else{foreach(array("pwds","dbs","queries")as$c){set_session($c,null);}$c=base64_encode(DRIVER)."-".base64_encode(SERVER)."-".base64_encode($_GET["username"]);if($wa[$c]){unset($wa[$c]);cookie("adminer_permanent",implode(" ",$wa));}redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),'Odhlášení proběhlo v pořádku.');}}elseif($wa&&!$_SESSION["pwds"]){session_regenerate_id();$_b=$j->permanentLogin();foreach($wa
as$c=>$a){list(,$Yc)=explode(":",$a);list($kb,$E,$D)=array_map('base64_decode',explode("-",$c));$_SESSION["pwds"][$kb][$E][$D]=decrypt_string(base64_decode($Yc),$_b);}}function
auth_error($dc=null){global$g,$j,$ea;$zb=session_name();$v="";if(!$_COOKIE[$zb]&&$_GET[$zb]&&ini_bool("session.use_only_cookies")){$v='Session proměnné musí být povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$zb]||$_GET[$zb])&&!$ea){$v='Session vypršela, přihlašte se prosím znovu.';}else{$P=&get_session("pwds");if(isset($P)){$v=h($dc?$dc->getMessage():(is_string($g)?$g:'Neplatné přihlašovací údaje.'));$P=null;}}}page_header('Přihlásit se',$v,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$j->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Žádná extenze',sprintf('Není dostupná žádná z podporovaných PHP extenzí (%s).',implode(", ",$fd)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$j->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$ea=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$ea;}$v=($_POST?($_POST["token"]==$ea?"":'Neplatný token CSRF. Odešlete formulář znovu.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Příliš velká POST data. Zmenšete data nebo zvyšte hodnotu konfigurační direktivy %s.','"post_max_size"')));$g->select_db($j->database());function
email_header($Wc){return"=?UTF-8?B?".base64_encode($Wc)."?=";}function
send_mail($Fa,$gb,$X,$Ha="",$nb=array()){$J=(strncasecmp(PHP_OS,"win",3)?"\n":"\r\n");$X=str_replace("\n",$J,wordwrap(str_replace("\r","","$X\n")));$pb=uniqid("boundary");$ob="";foreach((array)$nb["error"]as$c=>$a){if(!$a){$ob.="--$pb$J"."Content-Type: ".str_replace("\n","",$nb["type"][$c]).$J."Content-Disposition: attachment; filename=\"".preg_replace('~["\\n]~','',$nb["name"][$c])."\"$J"."Content-Transfer-Encoding: base64$J$J".chunk_split(base64_encode(file_get_contents($nb["tmp_name"][$c])),76,$J).$J;}}$Lc="";$rb="Content-Type: text/plain; charset=utf-8$J"."Content-Transfer-Encoding: 8bit";if($ob){$ob.="--$pb--$J";$Lc="--$pb$J$rb$J$J";$rb="Content-Type: multipart/mixed; boundary=\"$pb\"";}$rb.=$J."MIME-Version: 1.0$J"."X-Mailer: Adminer Editor".($Ha?$J."From: ".str_replace("\n","",$Ha):"");return
mail($Fa,email_header($gb),$Lc.$X.$ob,$rb);}function
like_bool($e){return
ereg("bool|(tinyint|bit)\\(1\\)",$e["full_type"]);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$qb="RESTRICT|CASCADE|SET NULL|NO ACTION";$va[DRIVER]='Přihlásit se';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$r=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$r-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($r)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$r=$_GET["edit"];$o=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$ba=(isset($_GET["select"])?$_POST["edit"]:$o);$q=fields($r);foreach($q
as$f=>$e){if(!isset($e["privileges"][$ba?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$I=$_POST["referer"];if($_POST["insert"]){$I=($ba?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$I)){$I=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $o"),$I,'Položka byla smazána.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($ba?"\n".idf_escape($f)." = $a":$a);}}if($ba){if(!$s){redirect($I);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $o"),$I,'Položka byla aktualizována.');}else{$k=insert_into($r,$s);$Ic=($k?last_id():0);queries_redirect($I,sprintf('Položka%s byla vložena.',($Ic?" $Ic":"")),$k);}}}$La=$j->tableName(table_status($r));page_header(($ba?'Upravit':'Vložit'),$v,array("select"=>array($r,$La)),$La);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($o){$u=array();foreach($q
as$f=>$e){if(isset($e["privileges"]["select"])){$u[]=($_POST["clone"]&&$e["auto_increment"]?"'' AS ":(ereg("enum|set",$e["type"])?"1*".idf_escape($f)." AS ":"")).idf_escape($f);}}$d=array();if($u){$w=get_rows("SELECT".limit(implode(", ",$u)." FROM ".table($r)," WHERE $o",(isset($_GET["select"])?2:1)));$d=(isset($_GET["select"])&&count($w)!=1?null:reset($w));}}if($d===false){echo"<p class='error'>".'Žádné řádky.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';if($q){echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($q
as$f=>$e){echo"<tr><th>".$j->fieldName($e);$Jc=$_GET["set"][bracket_escape($f)];$i=(isset($d)?($d[$f]!=""&&ereg("enum|set",$e["type"])?(is_array($d[$f])?array_sum($d[$f]):+$d[$f]):$d[$f]):(!$ba&&$e["auto_increment"]?"":(isset($_GET["select"])?false:(isset($Jc)?$Jc:$e["default"]))));if(!$_POST["save"]&&is_string($i)){$i=$j->editVal($i,$e);}$z=($_POST["save"]?(string)$_POST["function"][$f]:($ba&&$e["on_update"]=="CURRENT_TIMESTAMP"?"now":($i===false?null:(isset($i)?'':'NULL'))));if($e["type"]=="timestamp"&&$i=="CURRENT_TIMESTAMP"){$i="";$z="now";}input($e,$i,$z);echo"\n";}echo"</table>\n";}echo'<p>
';if($q){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($ba?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}}echo($ba?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$q?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
<input type="hidden" name="save" value="1">
<input type="hidden" name="token" value="',$ea,'">
</form>
';}elseif(isset($_GET["select"])){$r=$_GET["select"];$B=table_status($r);$G=indexes($r);$q=fields($r);$Ka=column_foreign_keys($r);if($B["Oid"]=="t"){$G[]=array("type"=>"PRIMARY","columns"=>array("oid"));}parse_str($_COOKIE["adminer_import"],$tb);$Qc=array();$A=array();$Pa=null;foreach($q
as$c=>$e){$f=$j->fieldName($e);if(isset($e["privileges"]["select"])&&$f!=""){$A[$c]=html_entity_decode(strip_tags($f));if(ereg('text|lob',$e["type"])){$Pa=$j->selectLengthProcess();}}$Qc+=$e["privileges"];}list($u,$Q)=$j->selectColumnsProcess($A,$G);$o=$j->selectSearchProcess($q,$G);$V=$j->selectOrderProcess($q,$G);$C=$j->selectLimitProcess();$Ha=($u?implode(", ",$u):($B["Oid"]=="t"?"oid, ":"")."*")."\nFROM ".table($r);$Db=($Q&&count($Q)<count($u)?"\nGROUP BY ".implode(", ",$Q):"").($V?"\nORDER BY ".implode(", ",$V):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$ja=>$d){echo$g->result("SELECT".limit(idf_escape(key($d))." FROM ".table($r)," WHERE ".where_check($ja).($o?" AND ".implode(" AND ",$o):"").($V?" ORDER BY ".implode(", ",$V):""),1));}exit;}if($_POST&&!$v){$Kc="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$lb=$Ja=null;foreach($G
as$T){if($T["type"]=="PRIMARY"){$lb=array_flip($T["columns"]);$Ja=($u?$lb:array());break;}}foreach((array)$Ja
as$c=>$a){if(in_array(idf_escape($c),$u)){unset($Ja[$c]);}}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($r);$j->dumpTable($r,"");if(!is_array($_POST["check"])||$Ja===array()){$Ga=$o;if(is_array($_POST["check"])){$Ga[]="($Kc)";}$l="SELECT $Ha".($Ga?"\nWHERE ".implode(" AND ",$Ga):"").$Db;}else{$vc=array();foreach($_POST["check"]as$a){$vc[]="(SELECT".limit($Ha,"\nWHERE ".($o?implode(" AND ",$o)." AND ":"").where_check($a).$Db,1).")";}$l=implode(" UNION ALL ",$vc);}$j->dumpData($r,"table",$l);exit;}if(!$j->selectEmailProcess($o,$Ka)){if($_POST["save"]||$_POST["delete"]){$k=true;$oa=0;$l=table($r);$s=array();if(!$_POST["delete"]){foreach($A
as$f=>$a){$a=process_input($q[$f]);if($a!==null){if($_POST["clone"]){$s[idf_escape($f)]=($a!==false?$a:idf_escape($f));}elseif($a!==false){$s[]=idf_escape($f)." = $a";}}}$l.=($_POST["clone"]?" (".implode(", ",array_keys($s)).")\nSELECT ".implode(", ",$s)."\nFROM ".table($r):" SET\n".implode(",\n",$s));}if($_POST["delete"]||$s){$xb="UPDATE";if($_POST["delete"]){$xb="DELETE";$l="FROM $l";}if($_POST["clone"]){$xb="INSERT";$l="INTO $l";}if($_POST["all"]||($Ja===array()&&$_POST["check"])||count($Q)<count($u)){$k=queries($xb." $l".($_POST["all"]?($o?"\nWHERE ".implode(" AND ",$o):""):"\nWHERE $Kc"));$oa=$g->affected_rows;}else{foreach((array)$_POST["check"]as$a){$k=queries($xb.limit1($l,"\nWHERE ".where_check($a)));if(!$k){break;}$oa+=$g->affected_rows;}}}queries_redirect(remove_from_uri("page"),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$oa),$k);}elseif(!$_POST["import"]){if(!$_POST["val"]){$v='Dvojklikněte na políčko, které chcete změnit.';}else{$k=true;$oa=0;foreach($_POST["val"]as$ja=>$d){$s=array();foreach($d
as$c=>$a){$c=bracket_escape($c,1);$s[]=idf_escape($c)." = ".(ereg('char|text',$q[$c]["type"])||$a!=""?$j->processInput($q[$c],$a):"NULL");}$l=table($r)." SET ".implode(", ",$s);$Ga=" WHERE ".where_check($ja).($o?" AND ".implode(" AND ",$o):"");$k=queries("UPDATE".(count($Q)<count($u)?" $l$Ga":limit1($l,$Ga)));if(!$k){break;}$oa+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$oa),$k);}}elseif(is_string($R=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($tb["output"])."&format=".urlencode($_POST["separator"]));$k=true;$Ca=array_keys($q);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$R,$U);$oa=count($U[0]);begin();$sa=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($U[0]as$c=>$a){preg_match_all("~((\"[^\"]*\")+|[^$sa]*)$sa~",$a.$sa,$Sb);if(!$c&&!array_diff($Sb[1],$Ca)){$Ca=$Sb[1];$oa--;}else{$s=array();foreach($Sb[1]as$n=>$qa){$s[idf_escape($Ca[$n])]=($qa==""&&$q[$Ca[$n]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$qa))));}$k=insert_update($r,$s,$lb);if(!$k){break;}}}if($k){queries("COMMIT");}queries_redirect(remove_from_uri("page"),lang(array('Byl importován %d záznam.','Byly importovány %d záznamy.','Bylo importováno %d záznamů.'),$oa),$k);queries("ROLLBACK");}else{$v=upload_error($R);}}}$La=$j->tableName($B);page_header('Vypsat'.": $La",$v);session_write_close();$s=null;if(isset($Qc["insert"])){$s="";foreach((array)$_GET["where"]as$a){if(count($Ka[$a["col"]])==1&&($a["op"]=="="||(!$a["op"]&&!ereg('[_%]',$a["val"])))){$s.="&set".urlencode("[".bracket_escape($a["col"])."]")."=".urlencode($a["val"]);}}}$j->selectLinks($B,$s);if(!$A){echo"<p class='error'>".'Nepodařilo se vypsat tabulku'.($q?".":": ".error())."\n";}else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($r).'">',"</div>\n";$j->selectColumnsPrint($u,$A);$j->selectSearchPrint($o,$A,$G);$j->selectOrderPrint($V,$A,$G);$j->selectLimitPrint($C);$j->selectLengthPrint($Pa);$j->selectActionPrint();echo"</form>\n";$y=$_GET["page"];if($y=="last"){$ma=$g->result("SELECT COUNT(*) FROM ".table($r).($o?" WHERE ".implode(" AND ",$o):""));$y=floor(max(0,$ma-1)/$C);}$l="SELECT".limit((+$C&&$Q&&count($Q)<count($u)&&$ka=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Ha,($o?"\nWHERE ".implode(" AND ",$o):"").$Db,($C!=""?+$C:null),($y?$C*$y:0),"\n");echo$j->selectQuery($l);$k=$g->query($l);if(!$k){echo"<p class='error'>".error()."\n";}else{if($ka=="mssql"){$k->seek($C*$y);}$Cb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$w=array();while($d=$k->fetch_assoc()){if($y&&$ka=="oracle"){unset($d["RNUM"]);}$w[]=$d;}if($_GET["page"]!="last"){$ma=(+$C&&$Q&&count($Q)<count($u)?($ka=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($l) x")):count($w));}if(!$w){echo"<p class='message'>".'Žádné řádky.'."\n";}else{$Ob=$j->backwardKeys($r,$La);echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Q&&$u?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upravit'."</a>");$Qb=array();$da=array();reset($u);$zc=1;foreach($w[0]as$c=>$a){if($B["Oid"]!="t"||$c!="oid"){$a=$_GET["columns"][key($u)];$e=$q[$u?($a?$a["col"]:current($u)):$c];$f=($e?$j->fieldName($e,$zc):"*");if($f!=""){$zc++;$Qb[$c]=$f;$H=idf_escape($c);echo'<th><a href="'.h(remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($c).($V[0]==$H||$V[0]==$c||(!$V&&count($Q)<count($u)&&$Q[0]==$H)?'&desc%5B0%5D=1':'')).'">'.(!$u||$a?apply_sql_function($a["fun"],$f):h(current($u)))."</a>";}$da[$c]=$a["fun"];next($u);}}$Eb=array();if($_GET["modify"]){foreach($w
as$d){foreach($d
as$c=>$a){$Eb[$c]=max($Eb[$c],min(40,strlen(utf8_decode($a))));}}}echo($Ob?"<th>".'Vztahy':"")."</thead>\n";foreach($j->rowDescriptions($w,$Ka)as$x=>$d){$Pb=unique_array($w[$x],$G);$ja="";foreach($Pb
as$c=>$a){$ja.="&".(isset($a)?urlencode("where[".bracket_escape($c)."]")."=".urlencode($a):"null%5B%5D=".urlencode($c));}echo"<tr".odd().">".(!$Q&&$u?"":"<td>".checkbox("check[]",substr($ja,1),in_array(substr($ja,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($Q)<count($u)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($r).$ja)."'>".'upravit'."</a>"));foreach($d
as$c=>$a){if(isset($Qb[$c])){$e=$q[$c];if($a!=""&&(!isset($Cb[$c])||$Cb[$c]!="")){$Cb[$c]=(is_mail($a)?$Qb[$c]:"");}$t="";$a=$j->editVal($a,$e);if(!isset($a)){$a="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$e["type"])&&$a!=""){$t=h(ME.'download='.urlencode($r).'&field='.urlencode($c).$ja);}if($a===""){$a="&nbsp;";}elseif($Pa!=""&&ereg('text|blob',$e["type"])&&is_utf8($a)){$a=shorten_utf8($a,max(0,+$Pa));}else{$a=h($a);}if(!$t){foreach((array)$Ka[$c]as$la){if(count($Ka[$c])==1||end($la["source"])==$c){$t="";foreach($la["source"]as$n=>$Tb){$t.=where_link($n,$la["target"][$n],$w[$x][$Tb]);}$t=h(($la["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($la["db"]),ME):ME).'select='.urlencode($la["table"]).$t);if(count($la["source"])==1){break;}}}}if($c=="COUNT(*)"){$t=h(ME."select=".urlencode($r));$n=0;foreach((array)$_GET["where"]as$p){if(!array_key_exists($p["col"],$Pb)){$t.=h(where_link($n++,$p["col"],$p["val"],$p["op"]));}}foreach($Pb
as$W=>$p){$t.=h(where_link($n++,$W,$p));}}}if(!$t){if(is_mail($a)){$t="mailto:$a";}if($Ec=is_url($d[$c])){$t=($Ec=="http"&&$bb?$d[$c]:"$Ec://www.adminer.org/redirect/?url=".urlencode($d[$c]));}}$_=h("val[$ja][".bracket_escape($c)."]");$i=$_POST["val"][$ja][bracket_escape($c)];$Fc=h(isset($i)?$i:$d[$c]);$ad=strpos($a,"<i>...</i>");$Cc=is_utf8($a)&&$w[$x][$c]==$d[$c]&&!$da[$c];$Ac=ereg('text|lob',$e["type"]);echo(($_GET["modify"]&&$Cc)||isset($i)?"<td>".($Ac?"<textarea name='$_' cols='30' rows='".(substr_count($d[$c],"\n")+1)."'>$Fc</textarea>":"<input name='$_' value='$Fc' size='$Eb[$c]'>"):"<td id='$_' ondblclick=\"".($Cc?"selectDblClick(this, event".($ad?", 2":($Ac?", 1":"")).")":"alert('".h('Ke změně této hodnoty použijte odkaz upravit.')."')").";\">".$j->selectVal($a,$t,$e));}}if($Ob){echo"<td>";}$j->backwardKeysPrint($Ob,$w[$x]);echo"</tr>\n";}echo"</table>\n";}if($w||$y){$Ub=true;if($_GET["page"]!="last"&&+$C&&count($Q)>=count($u)&&($ma>=$C||$y)){$ma=found_rows($B,$o);if($ma<max(1e4,2*($y+1)*$C)){ob_flush();flush();$ma=$g->result("SELECT COUNT(*) FROM ".table($r).($o?" WHERE ".implode(" AND ",$o):""));}else{$Ub=false;}}echo"<p class='pages'>";if(+$C&&$ma>$C){$Rb=floor(($ma-1)/$C);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($y+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$y).($y>5?" ...":"");for($n=max(1,$y-4);$n<min($Rb,$y+5);$n++){echo
pagination($n,$y);}echo($y+5<$Rb?" ...":"").($Ub?pagination($Rb,$y):' <a href="'.h(remove_from_uri()."&page=last").'">'.'poslední'."</a>");}echo" (".($Ub?"":"~ ").lang(array('%d řádek','%d řádky','%d řádků'),$ma).") ".checkbox("all",1,0,'celý výsledek')."\n";if($j->selectCommandPrint()){?>
