...
selectVal($V,$y,$l,$Zc){$K=($V===null?"&nbsp;":$V);$y=h($y);if(preg_match('~blob|bytea~',$l["type"])&&!is_utf8($V)){$K=lang(array('%d bajt','%d bajty','%d bajtÅ¯'),strlen($Zc));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$Zc))$K="<img src='$y' alt='$K'>";}if(like_bool($l)&&$K!="&nbsp;")$K=($V?'ano':'ne');if($y)$K="<a href='$y'>$K</a>";if(!$y&&!like_bool($l)&&preg_match('~int|float|double|decimal~',$l["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$l["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($X,$e,$t){$X=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Vyhledat'."</legend><div>\n";$qc=array();foreach($X
as$w=>$V)$qc[$V["col"]]=$w;$q=0;$m=fields($_GET["select"]);foreach($e
as$C=>$bb){$l=$m[$C];if(preg_match("~enum~",$l["type"])||like_bool($l)){$w=$qc[$C];$q--;echo"<div>".h($bb)."<input type='hidden' name='where[$q][col]' value='".h($C)."'>:",(like_bool($l)?" <select name='where[$q][val]'>".optionlist(array(""=>"",'ne','ano'),$X[$w]["val"],true)."</select>":enum_input("checkbox"," name='where[$q][val][]'",$l,(array)$X[$w]["val"],($l["null"]?0:null))),"</div>\n";unset($e[$C]);}elseif(is_array($Wc=$this->_foreignKeyOptions($_GET["select"],$C))){if($m[$C]["null"])$Wc[0]='('.'prÃ¡zdnÃ©'.')';$w=$qc[$C];$q--;echo"<div>".h($bb)."<input type='hidden' name='where[$q][col]' value='".h($C)."'><input type='hidden' name='where[$q][op]' value='='>: <select name='where[$q][val]'>".optionlist($Wc,$X[$w]["val"],true)."</select></div>\n";unset($e[$C]);}}$q=0;foreach($X
...
as$w=>$s){$D=array();foreach($s["columns"]as$V)$D[]=$e[$V];if(count(array_filter($D,'strlen'))>1&&$w!="PRIMARY")$Yc[$w]=implode(", ",$D);}if($Yc){echo'<fieldset><legend>'.'SeÅ™adit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Yc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';if($Ic=="auth"){$Jb=true;foreach((array)$_SESSION["pwds"]as$De=>$Kd){foreach($Kd[""]as$T=>$G){if($G!==null){if($Jb){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$Jb=false;}echo"<a href='".h(auth_url($De,"",$T))."'>".($T!=""?h($T):"<i>".'prÃ¡zdnÃ©'."</i>")."</a><br>\n";}}}}else{$this->databasesPrint($Ic);if($Ic!="db"&&$Ic!="ns"){$S=table_status('',true);if(!$S)echo"<p class='message'>".'Å½Ã¡dnÃ© tabulky.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($Ic){}function
tablesPrint($ae){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($ae
as$L){$C=$this->tableName($L);if(isset($L["Engine"])&&$C!="")echo"<a href='".h(ME).'select='.urlencode($L["Name"])."'".bold($_GET["select"]==$L["Name"]||$_GET["edit"]==$L["Name"])." title='".'Vypsat data'."'>$C</a><br>\n";}}function
...
long2str($U,true);}$f='';$bc=$_SESSION["token"];if(!$bc)$_SESSION["token"]=rand(1,1e6);$me=get_token();$ed=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$V){list($w)=explode(":",$V);$ed[$w]=$V;}}$ra=$_POST["auth"];if($ra){session_regenerate_id();$i=$ra["driver"];$O=$ra["server"];$T=$ra["username"];$G=$ra["password"];$h=$ra["db"];set_password($i,$O,$T,$G);$_SESSION["db"][$i][$O][$T][$h]=true;if($ra["permanent"]){$w=base64_encode($i)."-".base64_encode($O)."-".base64_encode($T)."-".base64_encode($h);$md=$b->permanentLogin(true);$ed[$w]="$w:".base64_encode($md?encrypt_string($G,$md):"");cookie("adminer_permanent",implode(" ",$ed));}if(count($_POST)==1||DRIVER!=$i||SERVER!=$O||$_GET["username"]!==$T||DB!=$h)redirect(auth_url($i,$O,$T,$h));}elseif($_POST["logout"]){if($bc&&!verify_token()){page_header('OdhlÃ¡sit','NeplatnÃ½ token CSRF. OdeÅ¡lete formulÃ¡Å™ znovu.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$w)set_session($w,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'OdhlÃ¡Å¡enÃ­ probÄ›hlo v poÅ™Ã¡dku.');}}elseif($ed&&!$_SESSION["pwds"]){session_regenerate_id();$md=$b->permanentLogin();foreach($ed
...
auth_error($zb=null){global$f,$b,$bc;$Ld=session_name();$k="";if(!$_COOKIE[$Ld]&&$_GET[$Ld]&&ini_bool("session.use_only_cookies"))$k='Session promÄ›nnÃ© musÃ­ bÃ½t povolenÃ©.';elseif(isset($_GET["username"])){if(($_COOKIE[$Ld]||$_GET[$Ld])&&!$bc)$k='Session vyprÅ¡ela, pÅ™ihlaÅ¡te se prosÃ­m znovu.';else{$G=get_password();if($G!==null){$k=h($zb?$zb->getMessage():(is_string($f)?$f:'NeplatnÃ© pÅ™ihlaÅ¡ovacÃ­ Ãºdaje.'));if($G===false)$k.='<br>'.sprintf('Platnost hlavnÃ­ho hesla vyprÅ¡ela. <a href="http://www.adminer.org/cs/extension/" target="_blank">Implementujte</a> metodu %s, aby platilo stÃ¡le.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$cd=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$cd["lifetime"]);page_header('PÅ™ihlÃ¡sit se',$k,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
set_password($De,$O,$T,$G){$_SESSION["pwds"][$De][$O][$T]=($_COOKIE["adminer_key"]?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
get_password(){$K=get_session("pwds");if(is_array($K))$K=($_COOKIE["adminer_key"]?decrypt_string($K[0],$_COOKIE["adminer_key"]):false);return$K;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Å½Ã¡dnÃ© rozÅ¡Ã­Å™enÃ­',sprintf('NenÃ­ dostupnÃ© Å¾Ã¡dnÃ© z podporovanÃ½ch PHP rozÅ¡Ã­Å™enÃ­ (%s).',implode(", ",$id)),false);page_footer("auth");exit;}$f=connect();}if(!is_object($f)||!$b->login($_GET["username"],get_password())){auth_error();exit;}$i=new
...
as$C=>$l){if(!isset($l["privileges"][$_e?"update":"insert"])||$b->fieldName($l)=="")unset($m[$C]);}if($_POST&&!$k&&!isset($_GET["select"])){$z=$_POST["referer"];if($_POST["insert"])$z=($_e?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$z))$z=ME."select=".urlencode($a);$t=indexes($a);$ve=unique_array($_GET["where"],$t);$sd="\nWHERE $X";if(isset($_POST["delete"]))queries_redirect($z,'PoloÅ¾ka byla smazÃ¡na.',$i->delete($a,$sd,!$ve));else{$P=array();foreach($m
as$C=>$l){$V=process_input($l);if($V!==false&&$V!==null)$P[idf_escape($C)]=$V;}if($_e){if(!$P)redirect($z);queries_redirect($z,'PoloÅ¾ka byla aktualizovÃ¡na.',$i->update($a,$P,$sd,!$ve));if(is_ajax()){page_headers();page_messages($k);exit;}}else{$J=$i->insert($a,$P);$vc=($J?last_id():0);queries_redirect($z,sprintf('PoloÅ¾ka%s byla vloÅ¾ena.',($vc?" $vc":"")),$J);}}}$Zd=$b->tableName(table_status1($a,true));page_header(($_e?'Upravit':'VloÅ¾it'),$k,array("select"=>array($a,$Zd)),$Zd);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($X){$N=array();foreach($m
...
as$w=>$V){if(!$X)$L[$w]=null;$m[$w]=array("field"=>$w,"null"=>($w!=$r),"auto_increment"=>($w==$r));}}}if($L===false)echo"<p class='error'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";echo'
<div id="message"></div>
...
';if($m){echo"<input type='submit' value='".'UloÅ¾it'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($_e?'UloÅ¾it a pokraÄovat v editaci'."' onclick='return !ajaxForm(this.form, \"".'UklÃ¡dÃ¡ se'.'...", this)':'UloÅ¾it a vloÅ¾it dalÅ¡Ã­')."' title='Ctrl+Shift+Enter'>\n";}echo($_e?"<input type='submit' name='delete' value='".'Smazat'."'".confirm().">\n":($_POST||!$m?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$C=>$V){$V=process_input($m[$C]);if($V!==null&&($_POST["clone"]||$V!==false))$P[idf_escape($C)]=($V!==false?$V:idf_escape($C));}}if($_POST["delete"]||$P){if($_POST["clone"])$I="INTO ".table($a)." (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a);if($_POST["all"]||($ye===array()&&is_array($_POST["check"]))||$u){$J=($_POST["delete"]?$i->delete($a,$Je):($_POST["clone"]?queries("INSERT $I$Je"):$i->update($a,$P,$Je)));$ja=$f->affected_rows;}else{foreach((array)$_POST["check"]as$V){$Ie="\nWHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($V,$m);$J=($_POST["delete"]?$i->delete($a,$Ie,1):($_POST["clone"]?queries("INSERT".limit1($I,$Ie)):$i->update($a,$P,$Ie)));if(!$J)break;$ja+=$f->affected_rows;}}}$A=lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$ja);if($_POST["clone"]&&$J&&$ja==1){$vc=last_id();if($vc)$A=sprintf('PoloÅ¾ka%s byla vloÅ¾ena.'," $vc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$A,$J);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+kliknÄ›te na polÃ­Äko, kterÃ© chcete zmÄ›nit.';else{$J=true;$ja=0;foreach($_POST["val"]as$we=>$L){$P=array();foreach($L
as$w=>$V){$w=bracket_escape($w,1);$P[idf_escape($w)]=(preg_match('~char|text~',$m[$w]["type"])||$V!=""?$b->processInput($m[$w],$V):"NULL");}$J=$i->update($a,$P," WHERE ".($X?implode(" AND ",$X)." AND ":"").where_check($we,$m),!($u||$ye===array())," ");if(!$J)break;$ja+=$f->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivnÄ›n %d zÃ¡znam.','Byly ovlivnÄ›ny %d zÃ¡znamy.','Bylo ovlivnÄ›no %d zÃ¡znamÅ¯.'),$ja),$J);}}elseif(!is_string($Fb=get_file("csv_file",true)))$k=upload_error($Fb);elseif(!preg_match('~~u',$Fb))$k='Soubor musÃ­ bÃ½t v kÃ³dovÃ¡nÃ­ UTF-8.';else{cookie("adminer_import","output=".urlencode($ia["output"])."&format=".urlencode($_POST["separator"]));$J=true;$Ma=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Fb,$Bc);$ja=count($Bc[0]);$i->begin();$Id=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$M=array();foreach($Bc[0]as$w=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$Id]*)$Id~",$V.$Id,$Cc);if(!$w&&!array_diff($Cc[1],$Ma)){$Ma=$Cc[1];$ja--;}else{$P=array();foreach($Cc[1]as$q=>$Ja)$P[idf_escape($Ma[$q])]=($Ja==""&&$m[$Ma[$q]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Ja))));$M[]=$P;}}$J=(!$M||$i->insertUpdate($a,$M,$kd));if($J)$i->commit();queries_redirect(remove_from_uri("page"),lang(array('Byl importovÃ¡n %d zÃ¡znam.','Byly importovÃ¡ny %d zÃ¡znamy.','Bylo importovÃ¡no %d zÃ¡znamÅ¯.'),$ja),$J);$i->rollback();}}}$Zd=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header('Vypsat'.": $Zd",$k);$P=null;if(isset($Bd["insert"])||!support("table")){$P="";foreach((array)$_GET["where"]as$V){if(count($Pb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!preg_match('~[_%]~',$V["val"]))))$P.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($S,$P);if(!$e&&support("table"))echo"<p class='error'>".'NepodaÅ™ilo se vypsat tabulku'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($N,$e);$b->selectSearchPrint($X,$e,$t);$b->selectOrderPrint($D,$e,$t);$b->selectLimitPrint($x);$b->selectLengthPrint($de);$b->selectActionPrint($t);echo"</form>\n";$F=$_GET["page"];if($F=="last"){$n=$f->result(count_rows($a,$X,$u,$p));$F=floor(max(0,$n-1)/$x);}$Fd=$N;if(!$Fd){$Fd[]="*";if($Pc)$Fd[]=$Pc;}$Sa=convert_fields($e,$m,$N);if($Sa)$Fd[]=substr($Sa,2);$J=$i->select($a,$Fd,$X,$p,$D,$x,$F,true);if(!$J)echo"<p class='error'>".error()."\n";else{if($v=="mssql"&&$F)$J->seek($x*$F);$qb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$M=array();while($L=$J->fetch_assoc()){if($F&&$v=="oracle")unset($L["RNUM"]);$M[]=$L;}if($_GET["page"]!="last"&&+$x&&$p&&$u&&$v=="sql")$n=$f->result(" SELECT FOUND_ROWS()");if(!$M)echo"<p class='message'>".'Å½Ã¡dnÃ© Å™Ã¡dky.'."\n";else{$xa=$b->backwardKeys($a,$Zd);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$p&&$N?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'ZmÄ›nit'."</a>");$Lc=array();$Vb=array();reset($N);$ud=1;foreach($M[0]as$w=>$V){if($w!=$Pc){$V=$_GET["columns"][key($N)];$l=$m[$N?($V?$V["col"]:current($N)):$w];$C=($l?$b->fieldName($l,$ud):($V["fun"]?"*":$w));if($C!=""){$ud++;$Lc[$w]=$C;$d=idf_escape($w);$gc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($w);$bb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($gc.($D[0]==$d||$D[0]==$w||(!$D&&$u&&$p[0]==$d)?$bb:'')).'">';echo
apply_sql_function($V["fun"],$C)."</a>";echo"<span class='column hidden'>","<a href='".h($gc.$bb)."' title='".'sestupnÄ›'."' class='text'> â†“</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($w)).'\'); return false;" title="'.'Vyhledat'.'" class="text jsonly"> =</a>';echo"</span>";}$Vb[$w]=$V["fun"];next($N);}}$yc=array();if($_GET["modify"]){foreach($M
as$L){foreach($L
as$w=>$V)$yc[$w]=max($yc[$w],min(40,strlen(utf8_decode($V))));}}echo($xa?"<th>".'Vztahy':"")."</thead>\n";if(is_ajax()){if($x%2==1&&$F%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($M,$Pb)as$B=>$L){$ve=unique_array($M[$B],$t);if(!$ve){$ve=array();foreach($M[$B]as$w=>$V){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$w))$ve[$w]=$V;}}$we="";foreach($ve
as$w=>$V){if(($v=="sql"||$v=="pgsql")&&strlen($V)>64){$w="MD5(".(strpos($w,'(')?$w:idf_escape($w)).")";$V=md5($V);}$we.="&".($V!==null?urlencode("where[".bracket_escape($w)."]")."=".urlencode($V):"null%5B%5D=".urlencode($w));}echo"<tr".odd().">".(!$p&&$N?"":"<td>".checkbox("check[]",substr($we,1),in_array(substr($we,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($u||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$we)."'>".'upravit'."</a>"));foreach($L
