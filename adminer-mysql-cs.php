...
upload_error($p){$Fd=($p==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($p?'Nepodařilo se nahrát soubor.'.($Fd?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Fd):""):'Soubor neexistuje.');}function
...
input($d,$n,$B){global$na,$q,$x;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$R=(isset($_GET["select"])?array("orig"=>'původní'):array())+$q->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($kf,$Lb){$Kb=($Lb==1||(!$Lb&&'cs'=='fr')?0:((!$Lb||$Lb>=5)&&ereg('cs|sk|ru','cs')?2:1));return
...
selectLinks($zc,$t=""){echo'<p class="tabs">';$ua=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($zc)){$ua["view"]='Pozměnit pohled';}else{$ua["create"]='Pozměnit tabulku';}if(isset($t)){$ua["edit"]='Nová položka';}foreach($ua
...
selectQuery($k){global$x;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Stránka'.": ".'poslední'."'>&gt;&gt;</a> <code class='jush-$x'>".h(str_replace("\n"," ",$k))."</code> <a href='".h(ME)."sql=".urlencode($k)."'>".'Upravit'."</a>\n";}function
rowDescription($m){return"";}function
rowDescriptions($D,$Xe){return$D;}function
selectVal($b,$_,$d){$e=($b!="<i>NULL</i>"&&ereg("^char|binary",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$e=lang(array('%d bajt','%d bajty','%d bajtů'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($_?"<a href='$_'>$e</a>":$e);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($u,$v){global$R,$Hb;print_fieldset("select",'Vypsat',$u);$h=0;$ae=array('Funkce'=>$R,'Agregace'=>$Hb);foreach($u
...
selectSearchPrint($y,$v,$z){print_fieldset("search",'Vyhledat',$y);foreach($z
...
selectOrderPrint($Za,$v,$z){print_fieldset("sort",'Seřadit',$Za);$h=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$h]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$h]",1,isset($_GET["desc"][$c]),'sestupně')."</div>\n";$h++;}}echo"<div><select name='order[$h]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$h]",1,0,'sestupně')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($Q){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($Q)."'>","</div></fieldset>\n";}function
selectLengthPrint($fb){if(isset($fb)){echo"<fieldset><legend>".'Délka textů'."</legend><div>",'<input name="text_length" size="3" value="'.h($fb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($k){global$x;restart_session();$E="sql-".count($_SESSION["messages"]);$Ta=&get_session("queries");$Ta[$_GET["db"]][]=(strlen($k)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($k,0,1e6))."\n...":$k);return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL příkaz'."</a><div id='$E' class='hidden'><pre class='jush-$x'>".shorten_utf8($k,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ta[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Ca,$n){if($d["type"]=="enum"){return($d["null"]?"<label><input type='radio'$Ca value=''".(isset($n)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Ca value='0'".($n===0?" checked":"")."><i>".'prázdné'."</i></label>";}return"";}function
...
dumpOutput($u,$n=""){$e=array('text'=>'otevřít','file'=>'uložit');if(function_exists('gzencode')){$e['gz']='gzip';}if(function_exists('bzcompress')){$e['bz2']='bzip2';}return
...
<form action="">
<p>
';hidden_fields_get();echo($A?html_select("db",array(""=>"(".'databáze'.")")+$A,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrat"',($A?" class='hidden'":""),'>
';if($xc!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""){$wa=tables_list();if(!$wa){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($wa);$ua=array();foreach($wa
...
as$m=>$N){echo'<a href="'.h(ME).'select='.urlencode($m).'">'.bold('vypsat',$_GET["select"]==$m).'</a> ','<a href="'.h(ME).'table='.urlencode($m).'">'.bold($this->tableName(array("Name"=>$m)),$_GET["table"]==$m)."</a><br>\n";}}}$q=(function_exists('adminer_object')?adminer_object():new
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Zd[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($na[$c]==254){$b="<code>$b</code>";}}if(isset($ua[$c])&&!$v[$ua[$c]]){$_="edit=".urlencode($ua[$c]);foreach($z[$ua[$c]]as$nc=>$W){$_.="&where".urlencode("[".bracket_escape($nc)."]")."=".urlencode($a[$W]);}$b="<a href='".h(ME.$_)."'>$b</a>";}}echo"<td>$b";}}echo($h?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($c,$d,$T,$M=array()){global$ld,$na,$Ib,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($na[$d["type"]])?array():array($d["type"]))+$ld+($M?array('Cizí klíče'=>$M):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($T,$d["collation"]).'</select>',($Ib?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Ib,$d["unsigned"]).'</select>':''),($M?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist($Ma,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($nb?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($nb?"":" class='hidden'")."><input name='fields[$h][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $Td, 1);'>&nbsp;"."<input type='image' name='up[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.0' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.0' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($kd==""||support("drop_col")?"<input type='image' name='drop_col[$h]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.0' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$nb;}function
...
dump_headers($ze,$Ke=false){$Ua=($ze!=""?friendly_url($ze):"adminer");$lb=$_POST["output"];$eb=($_POST["format"]=="sql"?"sql":($Ke?"tar":"csv"));header("Content-Type: ".($lb=="bz2"?"application/x-bzip":($lb=="gz"?"application/x-gzip":($eb=="tar"?"application/x-tar":($eb=="sql"||$lb!="file"?"text/plain":"text/csv")."; charset=utf-8"))));if($lb!="text"){header("Content-Disposition: attachment; filename=$Ua.$eb".($lb!="file"&&!ereg('[^0-9a-z]',$lb)?".$lb":""));}session_write_close();if($_POST["output"]=="bz2"){ob_start('bzcompress',1e6);}if($_POST["output"]=="gz"){ob_start('gzencode',1e6);}return$eb;}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Ma=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$bb=" onclick=\"return confirm('".'Opravdu?'."');\"";$ob='\'(?:\'\'|[^\'\\\\]|\\\\.)*\'|"(?:""|[^"\\\\]|\\\\.)*"';$Pb=array("IN","OUT","INOUT");if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["callf"])){$_GET["call"]=$_GET["callf"];}if(isset($_GET["function"])){$_GET["procedure"]=$_GET["function"];}if(isset($_GET["download"])){$g=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$g-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$i->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($g)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["table"])){$g=$_GET["table"];$o=fields($g);if(!$o){$p=error();}$L=($o?table_status($g):array());page_header(($o&&is_view($L)?'Pohled':'Tabulka').": ".h($g),$p);$q->selectLinks($L);$Xa=$L["Comment"];if($Xa!=""){echo"<p>".'Komentář'.": ".h($Xa)."\n";}if($o){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Sloupec'."<td>".'Typ'.(support("comment")?"<td>".'Komentář':"")."</thead>\n";foreach($o
...
as$f=>$s){ksort($s["columns"]);$cc=array();foreach($s["columns"]as$c=>$b){$cc[]="<i>".h($b)."</i>".($s["lengths"][$c]?"(".$s["lengths"][$c].")":"");}echo"<tr title='".h($f)."'><th>$s[type]<td>".implode(", ",$cc)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($g).'">'.'Pozměnit indexy'."</a>\n";if(fk_support($L)){echo"<h3>".'Cizí klíče'."</h3>\n";$M=foreign_keys($g);if($M){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'Cíl'."<td>".'Při smazání'."<td>".'Při změně'.($x!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($M
as$f=>$F){$_=($F["db"]!=""?"<b>".h($F["db"])."</b>.":"").h($F["table"]);echo"<tr>","<th><i>".implode("</i>, <i>",array_map('h',$F["source"]))."</i>","<td><a href='".h($F["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($F["db"]),ME):ME)."table=".urlencode($F["table"])."'>$_</a>","(<i>".implode("</i>, <i>",array_map('h',$F["target"]))."</i>)","<td>$F[on_delete]\n","<td>$F[on_update]\n";if($x!="sqlite"){echo'<td><a href="'.h(ME.'foreign='.urlencode($g).'&name='.urlencode($f)).'">'.'Změnit'.'</a>';}}echo"</table>\n";}if($x!="sqlite"){echo'<p><a href="'.h(ME).'foreign='.urlencode($g).'">'.'Přidat cizí klíč'."</a>\n";}}if(support("trigger")){echo"<h3>".'Triggery'."</h3>\n";$Wb=triggers($g);if($Wb){echo"<table cellspacing='0'>\n";foreach($Wb
as$c=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($c)."<td><a href='".h(ME.'trigger='.urlencode($g).'&name='.urlencode($c))."'>".'Změnit'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($g).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB);$Va=array();$we=array();preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$_COOKIE["adminer_schema"],$aa,PREG_SET_ORDER);foreach($aa
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě');print_fieldset("webfile",'Ze serveru',$_POST["webfile"]);$Vc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Vc[]=".$c";}}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($Vc?"[".implode("|",$Vc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($Ta){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($Ta
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Upravit'."</a> <code class='jush-$x'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistit'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Ha?"update":"insert"])||$q->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$p&&!isset($_GET["select"])){$U=$_POST["referer"];if($_POST["insert"]){$U=($Ha?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$U)){$U=ME."select=".urlencode($g);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($g)," WHERE $y"),$U,'Položka byla smazána.');}else{$t=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$t[idf_escape($f)]=($Ha?"\n".idf_escape($f)." = $b":$b);}}if($Ha){if(!$t){redirect($U);}query_redirect("UPDATE".limit1(table($g)." SET".implode(",",$t),"\nWHERE $y"),$U,'Položka byla aktualizována.');}else{$j=insert_into($g,$t);$Bd=($j?last_id():0);queries_redirect($U,sprintf('Položka%s byla vložena.',($Bd?" $Bd":"")),$j);}}}$oa=$q->tableName(table_status($g));page_header(($Ha?'Upravit':'Vložit'),$p,array("select"=>array($g,$oa)),$oa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($y){$u=array();foreach($o
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($o){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($Ha?'Uložit a pokračovat v editaci':'Uložit a vložit další')."\">\n";}}if($Ha){echo"<input type='submit' name='delete' value='".'Smazat'."'$bb>\n";}echo'</form>
...
<?php echo(support("comment")?checkbox("","",$nb,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($nb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložit">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstranit"',$bb,'>';}if(support("partitioning")){$od=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdělit podle',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Ad,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($od||!$a["partition_by"]?" class='hidden'":""),'>
...
as$w){if(count($A)==1||$w!=""){if(!create_database($w,$_POST["collation"])){$Ed=false;}$ab=$w;}}queries_redirect(ME."db=".urlencode($ab),'Databáze byla vytvořena.',$Ed);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"])." COLLATE ".q($_POST["collation"]),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$p,array(),DB);$T=collations();$f=DB;$Gb=null;if($_POST){$f=$_POST["name"];$Gb=$_POST["collation"];}elseif(DB!=""){$Gb=db_collation(DB,$T);}elseif($x=="sql"){foreach(get_vals("SHOW GRANTS")as$O){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$O,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input name="name" value="'.h($f).'" maxlength="64">')."\n".($T?html_select("collation",array(""=>"(".'porovnávání'.")")+$T,$Gb):""),'<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložit">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'$bb>\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Přidat další'."'>\n";}echo'</form>
';}elseif(isset($_GET["call"])){$Aa=$_GET["call"];page_header('Zavolat'.": ".h($Aa),$p);$za=routine($Aa,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$rb=array();$Qa=array();foreach($za["fields"]as$h=>$d){if(substr($d["inout"],-3)=="OUT"){$Qa[$h]="@".idf_escape($d["field"])." AS ".idf_escape($d["field"]);}if(!$d["inout"]||substr($d["inout"],0,2)=="IN"){$rb[]=$h;}}if(!$p&&$_POST){$Gd=array();foreach($za["fields"]as$c=>$d){if(in_array($c,$rb)){$b=process_input($d);if($b===false){$b="''";}if(isset($Qa[$c])){$i->query("SET @".idf_escape($d["field"])." = $b");}}$Gd[]=(isset($Qa[$c])?"@".idf_escape($d["field"]):$b);}$k=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($Aa)."(".implode(", ",$Gd).")";echo"<p><code class='jush-$x'>".h($k)."</code> <a href='".h(ME)."sql=".urlencode($k)."'>".'Upravit'."</a>\n";if(!$i->multi_query($k)){echo"<p class='error'>".error()."\n";}else{do{$j=$i->store_result();if(is_object($j)){select($j);}else{echo"<p class='message'>".lang(array('Procedura byla zavolána, byl změněn %d záznam.','Procedura byla zavolána, byly změněny %d záznamy.','Procedura byla zavolána, bylo změněno %d záznamů.'),$i->affected_rows)."\n";}}while($i->next_result());if($Qa){select($i->query("SELECT ".implode(", ",$Qa)));}}}echo'
<form action="" method="post">
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',$bb,'>';}echo'<input type="hidden" name="token" value="',$C,'">
</form>
';}elseif(isset($_GET["view"])){$g=$_GET["view"];$Ea=false;if($_POST&&!$p){$Ea=drop_create("DROP VIEW ".table($g),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'Pohled byl odstraněn.','Pohled byl změněn.','Pohled byl vytvořen.',$g);}page_header(($g!=""?'Pozměnit pohled':'Vytvořit pohled'),$p,array("table"=>$g),$g);$a=array();if($_POST){$a=$_POST;}elseif($g!=""){$a=view($g);$a["name"]=$g;}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["event"])){$Oa=$_GET["event"];$_d=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$ed=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$p){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Oa),substr(ME,0,-1),'Událost byla odstraněna.');}elseif(in_array($_POST["INTERVAL_FIELD"],$_d)&&isset($ed[$_POST["STATUS"]])){$rd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Oa!=""?"ALTER EVENT ".idf_escape($Oa).$rd.($Oa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$rd)."\n".$ed[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Oa!=""?'Událost byla změněna.':'Událost byla vytvořena.'));}}page_header(($Oa!=""?'Pozměnit událost'.": ".h($Oa):'Vytvořit událost'),$p);$a=array();if($_POST){$a=$_POST;}elseif($Oa!=""){$D=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Oa));$a=reset($D);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_d,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$ed,$a["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložit">
';if($Oa!=""){echo'<input type="submit" name="drop" value="Odstranit"',$bb,'>';}echo'</form>
...
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($Aa!=""){echo'<input type="submit" name="drop" value="Odstranit"',$bb,'>';}echo'</form>
';}elseif(isset($_GET["type"])){$dc=$_GET["type"];if($_POST&&!$p){$_=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($dc),$_,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$_,'Typ byl vytvořen.');}}page_header($dc!=""?'Pozměnit typ'.": ".h($dc):'Vytvořit typ',$p);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$C,'">
';if($dc!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'$bb>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložit'."'>\n";}echo'</form>
';}elseif(isset($_GET["trigger"])){$g=$_GET["trigger"];$fc=trigger_options();$qd=array("INSERT","UPDATE","DELETE");$Ea=false;if($_POST&&!$p&&in_array($_POST["Timing"],$fc["Timing"])&&in_array($_POST["Event"],$qd)&&in_array($_POST["Type"],$fc["Type"])){$td=" $_POST[Timing] $_POST[Event]";$gb=" ON ".table($g);$Ea=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($x=="pgsql"?$gb:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($x=="mssql"?$gb.$td:$td.$gb)." $_POST[Type]\n$_POST[Statement]",ME."table=".urlencode($g),'Trigger byl odstraněn.','Trigger byl změněn.','Trigger byl vytvořen.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Změnit trigger'.": ".h($_GET["name"]):'Vytvořit trigger'),$p,array("table"=>$g));$a=array("Trigger"=>$g."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$fc["Timing"],$a["Timing"],"if (/^".h(preg_quote($g,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($g,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Událost<td>',html_select("Event",$qd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$fc["Type"],$a["Type"]),'</table>
<p>Název: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',$bb,'>';}echo'</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Oprávnění'."</a>";$h=0;foreach($Sa
as$Ia=>$O){echo'<th>'.($Ia!="*.*"?"<input name='objects[$h]' value='".h($Ia)."' size='10'>":"<input type='hidden' name='objects[$h]' value='*.*' size='10'>*.*");$h++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$ec=>$Bb){foreach((array)$X[$ec]as$Nb=>$Xa){echo"<tr".odd()."><td".($Bb?">$Bb<td":" colspan='2'").' lang="en" title="'.h($Xa).'">'.h($Nb);$h=0;foreach($Sa
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložit">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit"',$bb,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$p){$Mc=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$Mc++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Mc),$Mc||!$_POST["kill"]);}page_header('Seznam procesů',$p);echo'
<form action="" method="post">
