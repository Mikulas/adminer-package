...
confirm($Za=""){return" onclick=\"return confirm('".'Opravdu?'.($Za?" (' + $Za + ')":"")."');\"";}function
...
upload_error($j){$Yc=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Nepodařilo se nahrát soubor.'.($Yc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Yc):""):'Soubor neexistuje.');}function
...
enum_input($V,$ua,$k,$Y,$Bb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$k["length"],$Tc);$J=($Bb!==null?"<label><input type='$V'$ua value='$Bb'".((is_array($Y)?in_array($Bb,$Y):$Y===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($Tc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Ha=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$J.=" <label><input type='$V'$ua value='".($r+1)."'".($Ha?' checked':'').'>'.h($b->editVal($X,$k)).'</label>';}return$J;}function
input($k,$Y,$p){global$_f,$b,$w;$D=h(bracket_escape($k["field"]));echo"<td class='function'>";$ue=($w=="mssql"&&$k["auto_increment"]);if($ue&&!$_POST["save"])$p=null;$hc=(isset($_GET["select"])||$ue?array("orig"=>'původní'):array())+$b->editFunctions($k);$ua=" name='fields[$D]'";if($k["type"]=="enum")echo
...
selectLinks($Ye,$P=""){echo'<p class="tabs">';$Qc=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Ye))$Qc["view"]='Pozměnit pohled';else$Qc["create"]='Pozměnit tabulku';if($P!==null)$Qc["edit"]='Nová položka';foreach($Qc
...
selectQuery($H){global$w;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Poslední stránka'."'>&gt;&gt;</a> <code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".'Upravit'."</a></p>\n";}function
rowDescription($S){return"";}function
rowDescriptions($L,$bc){return$L;}function
selectVal($X,$z,$k){$J=($X===null?"<i>NULL</i>":(ereg("char|binary",$k["type"])&&!ereg("var",$k["type"])?"<code>$X</code>":$X));if(ereg('blob|bytea|raw|file',$k["type"])&&!is_utf8($X))$J=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($X));return($z?"<a href='$z'>$J</a>":$J);}function
editVal($X,$k){return(ereg("binary",$k["type"])?reset(unpack("H*",$X)):$X);}function
selectColumnsPrint($N,$e){global$hc,$lc;print_fieldset("select",'Vypsat',$N);$r=0;$gc=array('Funkce'=>$hc,'Agregace'=>$lc);foreach($N
...
as$r=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Fa="this.nextSibling.onchange();";for($r=0;$r<=count($_GET["where"]);$r++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div><select name='where[$r][col]' onchange='$Fa'><option value=''>(".'kdekoliv'.")".optionlist($e,$X["col"],true)."</select>",html_select("where[$r][op]",$this->operators,$X["op"],$Fa),"<input name='where[$r][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Ad,$e,$u){print_fieldset("sort",'Seřadit',$Ad);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($e[$X])){echo"<div><select name='order[$r]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'sestupně')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($jf){if($jf!==null){echo"<fieldset><legend>".'Délka textů'."</legend><div>",'<input name="text_length" size="3" value="'.h($jf).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($H){global$w;static$Za=0;restart_session();$s="sql-".($Za++);$nc=&get_session("queries");if(strlen($H)>1e6)$H=ereg_replace('[\x80-\xFF]+$','',substr($H,0,1e6))."\n...";$nc[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL příkaz'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($nc[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$J);}function
editInput($S,$k,$ua,$Y){if($k["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$ua value='-1' checked><i>".'původní'."</i></label> ":"").($k["null"]?"<label><input type='radio'$ua value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ua,$k,$Y,0);return"";}function
...
dumpOutput(){$J=array('text'=>'otevřít','file'=>'uložit');if(function_exists('gzencode'))$J['gz']='gzip';if(function_exists('bzcompress'))$J['bz2']='bzip2';return$J;}function
dumpFormat(){return
...
as$S=>$V){echo'<a href="'.h(ME).'select='.urlencode($S).'"'.bold($_GET["select"]==$S).">".'vypsat'."</a> ",'<a href="'.h(ME).'table='.urlencode($S).'"'.bold($_GET["table"]==$S)." title='".'Zobrazit strukturu'."'>".$this->tableName(array("Name"=>$S))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Aa[$x]&&!is_utf8($X))$X="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($_f[$x]==254)$X="<code>$X</code>";}if(isset($Qc[$x])&&!$e[$Qc[$x]]){if($qc){$S=$K[array_search("table=",$Qc)];$z=$Qc[$x].urlencode($Dd[$S]!=""?$Dd[$S]:$S);}else{$z="edit=".urlencode($Qc[$x]);foreach($u[$Qc[$x]]as$Ka=>$v)$z.="&where".urlencode("[".bracket_escape($Ka)."]")."=".urlencode($K[$v]);}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Žádné řádky.')."\n";return$J;}function
...
edit_type($x,$k,$d,$n=array()){global$Re,$_f,$Gf,$ud;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($_f[$k["type"]])?array():array($k["type"]))+$Re+($n?array('Cizí klíče'=>$n):array()),$k["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($d,$k["collation"]).'</select>',($Gf?"<select name='$x"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Gf,$k["unsigned"]).'</select>':''),($n?"<select name='$x"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$ud),$k["on_delete"])."</select> ":" ");}function
...
';if($V=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($V=="TABLE"?'Název sloupce':'Název parametru'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Délka
<td>Volby
...
',(support("comment")?"<td".($Sa?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Sa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.0' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.0' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Ed==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.0' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$U,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ta=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$x=>$X){if(extension_loaded($X))$Ta[]=".$x";}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($Ta?"[".implode("|",$Ta)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($nc){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($nc
as$x=>$X){list($G,$kf)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Upravit'."</a> <span class='time'>".@date("H:i:s",$kf)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistit'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upravit vše'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$Hf?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Hf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Položka byla smazána.');else{$P=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$P[idf_escape($D)]=($Hf?"\n".idf_escape($D)." = $X":$X);}if($Hf){if(!$P)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $Z"),$_,'Položka byla aktualizována.');}else{$I=insert_into($a,$P);$Ic=($I?last_id():0);queries_redirect($_,sprintf('Položka%s byla vložena.',($Ic?" $Ic":"")),$I);}}}$Ze=$b->tableName(table_status($a));page_header(($Hf?'Upravit':'Vložit'),$j,array("select"=>array($a,$Ze)),$Ze);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$N=array();foreach($l
as$D=>$k){if(isset($k["privileges"]["select"]))$N[]=($_POST["clone"]&&$k["auto_increment"]?"'' AS ":($w=="sql"&&ereg("enum|set",$k["type"])?"1*".idf_escape($D)." AS ":"")).idf_escape($D);}$K=array();if($N){$L=get_rows("SELECT".limit(implode(", ",$N)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$K=(isset($_GET["select"])&&count($L)!=1?null:reset($L));}}if($K===false)echo"<p class='error'>".'Žádné řádky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($l){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Hf?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}echo($Hf?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Sa,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($K["Comment"]).'" maxlength="60"'.($Sa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložit">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
';if(support("partitioning")){$Pd=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Rozdělit podle',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Od,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($K["partitions"]),'"',($Pd||!$K["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$j&&!isset($_POST["add_x"])){restart_session();$D=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáze byla odstraněna.',drop_databases(array(DB)));}elseif(DB!==$D){if(DB!=""){$_GET["db"]=$D;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($D),'Databáze byla přejmenována.',rename_database($D,$_POST["collation"]));}else{$h=explode("\n",str_replace("\r","",$D));$Se=true;$Hc="";foreach($h
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$Se=false;$Hc=$i;}}queries_redirect(ME."db=".urlencode($Hc),'Databáze byla vytvořena.',$Se);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$j,array(),DB);$d=collations();$D=DB;$La=null;if($_POST){$D=$_POST["name"];$La=$_POST["collation"];}elseif(DB!="")$La=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$q,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($D,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($D).'</textarea><br>':'<input id="name" name="name" value="'.h($D).'" maxlength="64">')."\n".($d?html_select("collation",array(""=>"(".'porovnávání'.")")+$d,$La):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Uložit">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Přidat další'."'>\n";echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header('Zavolat'.": ".h($da),$j);$ye=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$uc=array();$Id=array();foreach($ye["fields"]as$r=>$k){if(substr($k["inout"],-3)=="OUT")$Id[$r]="@".idf_escape($k["field"])." AS ".idf_escape($k["field"]);if(!$k["inout"]||substr($k["inout"],0,2)=="IN")$uc[]=$r;}if(!$j&&$_POST){$Ea=array();foreach($ye["fields"]as$x=>$k){if(in_array($x,$uc)){$X=process_input($k);if($X===false)$X="''";if(isset($Id[$x]))$f->query("SET @".idf_escape($k["field"])." = $X");}$Ea[]=(isset($Id[$x])?"@".idf_escape($k["field"]):$X);}$H=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$Ea).")";echo"<p><code class='jush-$w'>".h($H)."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".'Upravit'."</a>\n";if(!$f->multi_query($H))echo"<p class='error'>".error()."\n";else{$g=connect();if(is_object($g))$g->select_db(DB);do{$I=$f->store_result();if(is_object($I))select($I,$g);else
echo"<p class='message'>".lang(array('Procedura byla zavolána, byl změněn %d záznam.','Procedura byla zavolána, byly změněny %d záznamy.','Procedura byla zavolána, bylo změněno %d záznamů.'),$f->affected_rows)."\n";}while($f->next_result());if($Id)select($f->query("SELECT ".implode(", ",$Id)));}}echo'
<form action="" method="post">
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$ud),$K["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$ud),$K["on_update"]),'<p>
<input type="submit" value="Uložit">
<noscript><p><input type="submit" name="add" value="Přidat sloupec"></noscript>
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$tb=false;if($_POST&&!$j){$D=trim($_POST["name"]);$tb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($D)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($D)),'Pohled byl odstraněn.','Pohled byl změněn.','Pohled byl vytvořen.',$a);}page_header(($a!=""?'Pozměnit pohled':'Vytvořit pohled'),$j,array("table"=>$a),$a);$K=$_POST;if(!$K&&$a!=""){$K=view($a);$K["name"]=$a;}echo'
<form action="" method="post">
<p>Název: <input name="name" value="',h($K["name"]),'" maxlength="64">
<p>';textarea("select",$K["select"]);echo'<p>
';if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$_c=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Pe=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$j){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Událost byla odstraněna.');elseif(in_array($_POST["INTERVAL_FIELD"],$_c)&&isset($Pe[$_POST["STATUS"]])){$Be="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Událost byla změněna.':'Událost byla vytvořena.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Be.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Be)."\n".$Pe[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Pozměnit událost'.": ".h($aa):'Vytvořit událost'),$j);$K=$_POST;if(!$K&&$aa!=""){$L=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$K=reset($L);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>Název<td><input name="EVENT_NAME" value="',h($K["EVENT_NAME"]),'" maxlength="64">
<tr><th>Začátek<td><input name="STARTS" value="',h("$K[EXECUTE_AT]$K[STARTS]"),'">
<tr><th>Konec<td><input name="ENDS" value="',h($K["ENDS"]),'">
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_c,$K["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Pe,$K["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$K["ON_COMPLETION"]=="PRESERVE",'Po dokončení zachovat'),'</table>
<p>';textarea("EVENT_DEFINITION",$K["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="Uložit">
';if($aa!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
...
<input type="submit" value="Uložit">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$wf=trigger_options();$vf=array("INSERT","UPDATE","DELETE");$tb=false;if($_POST&&!$j&&in_array($_POST["Timing"],$wf["Timing"])&&in_array($_POST["Event"],$vf)&&in_array($_POST["Type"],$wf["Type"])){$lf=" $_POST[Timing] $_POST[Event]";$td=" ON ".table($a);$tb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($w=="pgsql"?$td:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($w=="mssql"?$td.$lf:$lf.$td).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger byl odstraněn.','Trigger byl změněn.','Trigger byl vytvořen.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Změnit trigger'.": ".h($_GET["name"]):'Vytvořit trigger'),$j,array("table"=>$a));$K=$_POST;if(!$K)$K=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$wf["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Událost<td>',html_select("Event",$vf,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$wf["Type"],$K["Type"]),'</table>
<p>Název: <input name="Trigger" value="',h($K["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$K["Statement"]);echo'<p>
<input type="submit" value="Uložit">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($tb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
...
as$X){if($X[1]!="USAGE")$ic["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$ic["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$rd=$A[1];}}if($_POST&&!$j){$sd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$kd=q($_POST["user"])."@".q($_POST["host"]);$Sd=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $sd",ME."privileges=",'Uživatel byl odstraněn.');else{$cb=false;if($sd!=$kd){$cb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $kd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Sd");$j=!$cb;}elseif($_POST["pass"]!=$rd||!$_POST["hashed"])queries("SET PASSWORD FOR $kd = ".($_POST["hashed"]?$Sd:"PASSWORD($Sd)"));if(!$j){$ve=array();foreach($jd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$r=0;foreach($ic
as$nd=>$q){echo'<th>'.($nd!="*.*"?"<input name='objects[$r]' value='".h($nd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Xa=>$mb){foreach((array)$ge[$Xa]as$fe=>$Ra){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Ra).'">'.h($fe);$r=0;foreach($ic
as$nd=>$q){$D="'grants[$r][".h(strtoupper($fe))."]'";$Y=$q[strtoupper($fe)];if($Xa=="Server Admin"&&$nd!=(isset($ic["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Povolit'."<option value='0'".($Y=="0"?" selected":"").">".'Zakázat'."</select>";else
...
<input type="submit" value="Uložit">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$U,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Ec=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Ec++;}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Ec),$Ec||!$_POST["kill"]);}page_header('Seznam procesů',$j);echo'
<form action="" method="post">
...
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('%d celkem',$f->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Ukončit'."'>\n";}echo'<input type="hidden" name="token" value="',$U,'">
</form>
...
as$D=>$X){$X=process_input($l[$D]);if($X!==null){if($_POST["clone"])$P[idf_escape($D)]=($X!==false?$X:idf_escape($D));elseif($X!==false)$P[]=idf_escape($D)." = $X";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a):" SET\n".implode(",\n",$P));}if($_POST["delete"]||$P){$Pa="UPDATE";if($_POST["delete"]){$Pa="DELETE";$H="FROM $H";}if($_POST["clone"]){$Pa="INSERT";$H="INTO $H";}if($_POST["all"]||($Ff===array()&&$_POST["check"])||count($jc)<count($N)){$I=queries("$Pa $H".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $Tf"));$ma=$f->affected_rows;}else{foreach((array)$_POST["check"]as$X){$I=queries($Pa.limit1($H,"\nWHERE ".where_check($X)));if(!$I)break;$ma+=$f->affected_rows;}}}$B=lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ma);if($_POST["clone"]&&$I&&$ma==1){$Ic=last_id();if($Ic)$B=sprintf('Položka%s byla vložena.'," $Ic");}queries_redirect(remove_from_uri("page"),$B,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$j='Dvojklikněte na políčko, které chcete změnit.';else{$I=true;$ma=0;foreach($_POST["val"]as$Df=>$K){$P=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$P[]=idf_escape($x)." = ".(ereg('char|text',$l[$x]["type"])||$X!=""?$b->processInput($l[$x],$X):"NULL");}$H=table($a)." SET ".implode(", ",$P);$Sf=" WHERE ".where_check($Df).($Z?" AND ".implode(" AND ",$Z):"");$I=queries("UPDATE".(count($jc)<count($N)?" $H$Sf":limit1($H,$Sf)));if(!$I)break;$ma+=$f->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ma),$I);}}elseif(is_string($Vb=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($la["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Na=array_keys($l);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Vb,$Tc);$ma=count($Tc[0]);begin();$Ge=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($Tc[0]as$x=>$X){preg_match_all("~((\"[^\"]*\")+|[^$Ge]*)$Ge~",$X.$Ge,$Uc);if(!$x&&!array_diff($Uc[1],$Na)){$Na=$Uc[1];$ma--;}else{$P=array();foreach($Uc[1]as$r=>$Ka)$P[idf_escape($Na[$r])]=($Ka==""&&$l[$Na[$r]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Ka))));$I=insert_update($a,$P,$ce);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Byl importován %d záznam.','Byly importovány %d záznamy.','Bylo importováno %d záznamů.'),$ma),$I);queries("ROLLBACK");}else$j=upload_error($Vb);}}$Ze=$b->tableName($T);page_header('Vypsat'.": $Ze",$j);session_write_close();$P=null;if(isset($we["insert"])){$P="";foreach((array)$_GET["where"]as$X){if(count($n[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!ereg('[_%]',$X["val"]))))$P.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($T,$P);if(!$e)echo"<p class='error'>".'Nepodařilo se vypsat tabulku'.($l?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($N,$e);$b->selectSearchPrint($Z,$e,$u);$b->selectOrderPrint($Ad,$e,$u);$b->selectLimitPrint($y);$b->selectLengthPrint($jf);$b->selectActionPrint($u);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$dc=$f->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$E=floor(max(0,$dc-1)/$y);}$H="SELECT".limit((+$y&&$jc&&count($jc)<count($N)&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").$fc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$kc,($y!=""?+$y:null),($E?$y*$E:0),"\n");echo$b->selectQuery($H);$I=$f->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql")$I->seek($y*$E);$Ab=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$dc=(+$y&&$jc&&count($jc)<count($N)?($w=="sql"?$f->result(" SELECT FOUND_ROWS()"):$f->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Žádné řádky.'."\n";else{$za=$b->backwardKeys($a,$Ze);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$jc&&$N?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upravit'."</a>");$id=array();$hc=array();reset($N);$le=1;foreach($L[0]as$x=>$X){if($x!=$pd){$X=$_GET["columns"][key($N)];$k=$l[$N?($X?$X["col"]:current($N)):$x];$D=($k?$b->fieldName($k,$le):"*");if($D!=""){$le++;$id[$x]=$D;$qc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($qc).'">'.(!$N||$X?apply_sql_function($X["fun"],$D):h(current($N)))."</a>";echo"<span class='column hidden'>","<a href='".h("$qc&desc%5B0%5D=1")."' title='".'sestupně'."' class='text'> ↓</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.'Vyhledat'.'" class="text jsonly"> =</a>';echo"</span>";}$hc[$x]=$X["fun"];next($N);}}$Oc=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$Oc[$x]=max($Oc[$x],min(40,strlen(utf8_decode($X))));}}echo($za?"<th>".'Vztahy':"")."</thead>\n";foreach($b->rowDescriptions($L,$n)as$C=>$K){$Cf=unique_array($L[$C],$u);$Df="";foreach($Cf
as$x=>$X)$Df.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));echo"<tr".odd().">".(!$jc&&$N?"":"<td>".checkbox("check[]",substr($Df,1),in_array(substr($Df,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($jc)<count($N)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Df)."'>".'upravit'."</a>"));foreach($K
