...
confirm($eb=""){return" onclick=\"return confirm('".'Opravdu?'.($eb?" (' + $eb + ')":"")."');\"";}function
...
upload_error($j){$ld=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Nepodařilo se nahrát soubor.'.($ld?" ".sprintf('Maximální povolená velikost souboru je %sB.',$ld):""):'Soubor neexistuje.');}function
...
query($H,$Rf=false){$I=parent::query($H);$this->error="";if(!$I){$Pb=$this->errorInfo();$this->error=$Pb[2];return
...
query($H,$Rf=false){$I=@($Rf?mysql_unbuffered_query($H,$this->_link):mysql_query($H,$this->_link));$this->error="";if(!$I){$this->error=mysql_error($this->_link);return
...
selectLinks($pf,$O=""){echo'<p class="tabs">';$dd=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($pf))$dd["view"]='Pozměnit pohled';else$dd["create"]='Pozměnit tabulku';if($O!==null)$dd["edit"]='Nová položka';foreach($dd
...
selectQuery($H){global$w;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Poslední stránka'."'>&gt;&gt;</a> <code class='jush-$w'>".h(str_replace("\n"," ",$H))."</code> <a href='".h(ME)."sql=".urlencode($H)."'>".'Upravit'."</a></p>\n";}function
rowDescription($R){return"";}function
rowDescriptions($L,$kc){return$L;}function
selectVal($X,$z,$k){$J=($X===null?"<i>NULL</i>":(ereg("char|binary",$k["type"])&&!ereg("var",$k["type"])?"<code>$X</code>":$X));if(ereg('blob|bytea|raw|file',$k["type"])&&!is_utf8($X))$J=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($X));return($z?"<a href='".h($z)."'>$J</a>":$J);}function
editVal($X,$k){return$X;}function
selectColumnsPrint($M,$e){global$p,$uc;print_fieldset("select",'Vypsat',$M);$r=0;$qc=array('Funkce'=>$p,'Agregace'=>$uc);foreach($M
...
selectSearchPrint($Z,$e,$u){print_fieldset("search",'Vyhledat',$Z);foreach($u
as$r=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ia="this.nextSibling.onchange();";for($r=0;$r<=count($_GET["where"]);$r++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div><select name='where[$r][col]' onchange='$Ia'><option value=''>(".'kdekoliv'.")".optionlist($e,$X["col"],true)."</select>",html_select("where[$r][op]",$this->operators,$X["op"],$Ia),"<input type='search' name='where[$r][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Nd,$e,$u){print_fieldset("sort",'Seřadit',$Nd);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($e[$X])){echo"<div><select name='order[$r]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'sestupně')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($_f){if($_f!==null){echo"<fieldset><legend>".'Délka textů'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($_f)."'>","</div></fieldset>\n";}}function
...
messageQuery($H){global$w;static$eb=0;restart_session();$s="sql-".($eb++);$wc=&get_session("queries");if(strlen($H)>1e6)$H=ereg_replace('[\x80-\xFF]+$','',substr($H,0,1e6))."\n...";$wc[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL příkaz'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($wc[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$J);}function
editInput($R,$k,$va,$Y){if($k["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$va value='-1' checked><i>".'původní'."</i></label> ":"").($k["null"]?"<label><input type='radio'$va value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$va,$k,$Y,0);return"";}function
...
dumpOutput(){$J=array('text'=>'otevřít','file'=>'uložit');if(function_exists('gzencode'))$J['gz']='gzip';if(function_exists('bzcompress'))$J['bz2']='bzip2';return$J;}function
dumpFormat(){return
...
<p class="logout">
';if(DB==""||!$sd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL příkaz'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}echo'<input type="submit" name="logout" value="Odhlásit" id="logout">
<input type="hidden" name="token" value="',$T,'">
</p>
</form>
';$this->databasesPrint($sd);if($_GET["ns"]!==""&&!$sd&&DB!=""){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$uf=tables_list();if(!$uf)echo"<p class='message'>".'Žádné tabulky.'."\n";else{$this->tablesPrint($uf);$dd=array();foreach($uf
...
<p id="dbs">
';hidden_fields_get();echo($h?html_select("db",array(""=>"(".'databáze'.")")+$h,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrat"',($h?" class='hidden'":""),'>
...
auth_error($Ub=null){global$f,$b,$T;$af=session_name();$j="";if(!$_COOKIE[$af]&&$_GET[$af]&&ini_bool("session.use_only_cookies"))$j='Session proměnné musí být povolené.';elseif(isset($_GET["username"])){if(($_COOKIE[$af]||$_GET[$af])&&!$T)$j='Session vypršela, přihlašte se prosím znovu.';else{$ge=&get_session("pwds");if($ge!==null){$j=h($Ub?$Ub->getMessage():(is_string($f)?$f:'Neplatné přihlašovací údaje.'));$ge=null;}unset_permanent();}}page_header('Přihlásit se',$j,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Žádné rozšíření',sprintf('Není dostupné žádné z podporovaných PHP rozšíření (%s).',implode(", ",$me)),false);page_footer("auth");exit;}$f=connect();}if(is_string($f)||!$b->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$T=$_SESSION["token"];if($c&&$_POST["token"])$_POST["token"]=$T;$j=($_POST?($_POST["token"]==$T?"":'Neplatný token CSRF. Odešlete formulář znovu.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Příliš velká POST data. Zmenšete data nebo zvyšte hodnotu konfigurační direktivy %s.','"post_max_size"')));if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_cache_limiter("");session_write_close();}function
connect_error(){global$b,$f,$T,$j,$zb;$h=array();if(DB!="")page_header('Databáze'.": ".h(DB),'Nesprávná databáze.',true);else{if($_POST["db"]&&!$j)queries_redirect(substr(ME,0,-1),'Databáze byly odstraněny.',drop_databases($_POST["db"]));page_header('Vybrat databázi',$j,false);echo"<p><a href='".h(ME)."database='>".'Vytvořit novou databázi'."</a>\n";foreach(array('privileges'=>'Oprávnění','processlist'=>'Seznam procesů','variables'=>'Proměnné','status'=>'Stav',)as$x=>$X){if(support($x))echo"<a href='".h(ME)."$x='>$X</a>\n";}echo"<p>".sprintf('Verze %s: %s přes PHP rozšíření %s',$zb[DRIVER],"<b>$f->server_info</b>","<b>$f->extension</b>")."\n","<p>".sprintf('Přihlášen jako: %s',"<b>".h(logged_user())."</b>")."\n";$De="<a href='".h(ME)."refresh=1'>".'Obnovit'."</a>\n";$h=$b->databases();if($h){$Ue=support("scheme");$d=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr><td>&nbsp;<th>".'Databáze'."<td>".'Porovnávání'."<td>".'Tabulky'."</thead>\n";foreach($h
as$i){$Me=h(ME)."db=".urlencode($i);echo"<tr".odd()."><td>".checkbox("db[]",$i,in_array($i,(array)$_POST["db"])),"<th><a href='$Me'>".h($i)."</a>","<td><a href='$Me".($Ue?"&amp;ns=":"")."&amp;database=' title='".'Pozměnit databázi'."'>".nbsp(db_collation($i,$d))."</a>","<td align='right'><a href='$Me&amp;schema=' id='tables-".h($i)."' title='".'Schéma databáze'."'>?</a>","\n";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n","<p><input type='submit' name='drop' value='".'Odstranit'."'".confirm("formChecked(this, /db/)").">\n","<input type='hidden' name='token' value='$T'>\n",$De,"</form>\n";}else
...
as$x=>$X){if($X===null)$X="<i>NULL</i>";elseif($Da[$x]&&!is_utf8($X))$X="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($Qf[$x]==254)$X="<code>$X</code>";}if(isset($dd[$x])&&!$e[$dd[$x]]){if($zc){$R=$K[array_search("table=",$dd)];$z=$dd[$x].urlencode($Qd[$R]!=""?$Qd[$R]:$R);}else{$z="edit=".urlencode($dd[$x]);foreach($u[$dd[$x]]as$Pa=>$v)$z.="&where".urlencode("[".bracket_escape($Pa)."]")."=".urlencode($K[$v]);}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Žádné řádky.')."\n";return$J;}function
...
edit_type($x,$k,$d,$n=array()){global$if,$Qf,$Xf,$Gd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($Qf[$k["type"]])?array():array($k["type"]))+$if+($n?array('Cizí klíče'=>$n):array()),$k["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($k["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$k["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($d,$k["collation"]).'</select>',($Xf?"<select name='$x"."[unsigned]'".(!$k["type"]||ereg('(int|float|double|decimal)$',$k["type"])?"":" class='hidden'").'><option>'.optionlist($Xf,$k["unsigned"]).'</select>':''),($n?"<select name='$x"."[on_delete]'".(ereg("`",$k["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$Gd),$k["on_delete"])."</select> ":" ");}function
...
';if($U=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($U=="TABLE"?'Název sloupce':'Název parametru'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Délka
<td>Volby
';if($U=="TABLE"){echo'<td>NULL
<td><input type="radio" name="auto_increment_col" value=""><acronym title="Auto Increment">AI</acronym>
<td>Výchozí hodnoty
',(support("comment")?"<td".($Xa?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.2' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Xa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='".($f->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.2' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.2' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.2' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Rd==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.2' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$xf)echo"\n<div class='references' title='".h($yf)."' id='refd$Wc-".($r++)."' style='left: $Xc"."em; top: ".$R["fields"][$xf]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=3.6.2'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$Xc)."em;'></div></div>";}}echo"\n</div>\n";}foreach($Te
...
<input type="hidden" name="token" value="',$T,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ya=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$x=>$X){if(extension_loaded($X))$Ya[]=".$x";}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($Ya?"[".implode("|",$Ya)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($wc){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($wc
as$x=>$X){list($G,$Af)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Upravit'."</a> <span class='time' title='".@date('Y-m-d',$Af)."'>".@date("H:i:s",$Af)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistit'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upravit vše'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$Yf?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Yf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Položka byla smazána.');else{$O=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$O[idf_escape($D)]=($Yf?"\n".idf_escape($D)." = $X":$X);}if($Yf){if(!$O)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Položka byla aktualizována.');}else{$I=insert_into($a,$O);$Vc=($I?last_id():0);queries_redirect($_,sprintf('Položka%s byla vložena.',($Vc?" $Vc":"")),$I);}}}$qf=$b->tableName(table_status($a));page_header(($Yf?'Upravit':'Vložit'),$j,array("select"=>array($a,$qf)),$qf);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($l
as$D=>$k){if(isset($k["privileges"]["select"])){$ta=convert_field($k);if($_POST["clone"]&&$k["auto_increment"])$ta="''";if($w=="sql"&&ereg("enum|set",$k["type"]))$ta="1*".idf_escape($D);$M[]=($ta?"$ta AS ":"").idf_escape($D);}}$K=array();if($M){$L=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$K=(isset($_GET["select"])&&count($L)!=1?null:reset($L));}}if($K===false)echo"<p class='error'>".'Žádné řádky.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$l)echo"<p class='error'>".'Nemáte oprávnění editovat tuto tabulku.'."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($l
...
';if($l){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Yf?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}echo($Yf?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?checkbox("comments",1,$Xa,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($K["Comment"]).'" maxlength="'.($f->server_info>=5.5?2048:60).'"'.($Xa?'':' class="hidden"').'>':''),'<p>
...
';if(support("partitioning")){$ce=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Rozdělit podle',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$be,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Oddíly: <input type="number" name="partitions" class="size" value="',h($K["partitions"]),'"',($ce||!$K["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$j&&!isset($_POST["add_x"])){restart_session();$D=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáze byla odstraněna.',drop_databases(array(DB)));}elseif(DB!==$D){if(DB!=""){$_GET["db"]=$D;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($D),'Databáze byla přejmenována.',rename_database($D,$_POST["collation"]));}else{$h=explode("\n",str_replace("\r","",$D));$jf=true;$Uc="";foreach($h
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$jf=false;$Uc=$i;}}queries_redirect(ME."db=".urlencode($Uc),'Databáze byla vytvořena.',$jf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$j,array(),DB);$d=collations();$D=DB;$Qa=null;if($_POST){$D=$_POST["name"];$Qa=$_POST["collation"];}elseif(DB!="")$Qa=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$q,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($D,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($D).'</textarea><br>':'<input id="name" name="name" value="'.h($D).'" maxlength="64">')."\n".($d?html_select("collation",array(""=>"(".'porovnávání'.")")+$d,$Qa):"");?>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$Gd),$K["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$Gd),$K["on_update"]),'<p>
<input type="submit" value="Uložit">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$Ab=false;if($_POST&&!$j){$D=trim($_POST["name"]);$Ab=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($D)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($D)),'Pohled byl odstraněn.','Pohled byl změněn.','Pohled byl vytvořen.',$a);}page_header(($a!=""?'Pozměnit pohled':'Vytvořit pohled'),$j,array("table"=>$a),$a);$K=$_POST;if(!$K&&$a!=""){$K=view($a);$K["name"]=$a;}echo'
<form action="" method="post">
<p>Název: <input name="name" value="',h($K["name"]),'" maxlength="64">
<p>';textarea("select",$K["select"]);echo'<p>
';if($Ab){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Ic=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$gf=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$j){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Událost byla odstraněna.');elseif(in_array($_POST["INTERVAL_FIELD"],$Ic)&&isset($gf[$_POST["STATUS"]])){$Se="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Událost byla změněna.':'Událost byla vytvořena.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Se.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Se)."\n".$gf[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Pozměnit událost'.": ".h($aa):'Vytvořit událost'),$j);$K=$_POST;if(!$K&&$aa!=""){$L=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$K=reset($L);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>Název<td><input name="EVENT_NAME" value="',h($K["EVENT_NAME"]),'" maxlength="64">
<tr><th>Začátek<td><input name="STARTS" value="',h("$K[EXECUTE_AT]$K[STARTS]"),'">
<tr><th>Konec<td><input name="ENDS" value="',h($K["ENDS"]),'">
<tr><th>Každých<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Ic,$K["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$gf,$K["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($Ab){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$Mf=trigger_options();$Lf=array("INSERT","UPDATE","DELETE");$Ab=false;if($_POST&&!$j&&in_array($_POST["Timing"],$Mf["Timing"])&&in_array($_POST["Event"],$Lf)&&in_array($_POST["Type"],$Mf["Type"])){$Bf=" $_POST[Timing] $_POST[Event]";$Fd=" ON ".table($a);$Ab=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($w=="pgsql"?$Fd:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($w=="mssql"?$Fd.$Bf:$Bf.$Fd).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Trigger byl odstraněn.','Trigger byl změněn.','Trigger byl vytvořen.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Změnit trigger'.": ".h($_GET["name"]):'Vytvořit trigger'),$j,array("table"=>$a));$K=$_POST;if(!$K)$K=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Mf["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Událost<td>',html_select("Event",$Lf,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Mf["Type"],$K["Type"]),'</table>
<p>Název: <input name="Trigger" value="',h($K["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$K["Statement"]);echo'<p>
<input type="submit" value="Uložit">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($Ab){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE")$rc["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$rc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$Dd=$A[1];}}if($_POST&&!$j){$Ed=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$xd=q($_POST["user"])."@".q($_POST["host"]);$fe=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $Ed",ME."privileges=",'Uživatel byl odstraněn.');else{$hb=false;if($Ed!=$xd){$hb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $xd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $fe");$j=!$hb;}elseif($_POST["pass"]!=$Dd||!$_POST["hashed"])queries("SET PASSWORD FOR $xd = ".($_POST["hashed"]?$fe:"PASSWORD($fe)"));if(!$j){$Ke=array();foreach($wd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$r=0;foreach($rc
as$_d=>$q){echo'<th>'.($_d!="*.*"?"<input name='objects[$r]' value='".h($_d)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$cb=>$sb){foreach((array)$te[$cb]as$se=>$Wa){echo"<tr".odd()."><td".($sb?">$sb<td":" colspan='2'").' lang="en" title="'.h($Wa).'">'.h($se);$r=0;foreach($rc
as$_d=>$q){$D="'grants[$r][".h(strtoupper($se))."]'";$Y=$q[strtoupper($se)];if($cb=="Server Admin"&&$_d!=(isset($rc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Povolit'."<option value='0'".($Y=="0"?" selected":"").">".'Zakázat'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Rc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Rc++;}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Rc),$Rc||!$_POST["kill"]);}page_header('Seznam procesů',$j);echo'
<form action="" method="post">
...
as$x=>$X)echo"<td>".(($w=="sql"&&$x=="Info"&&ereg("Query|Killed",$K["Command"])&&$X!="")||($w=="pgsql"&&$x=="current_query"&&$X!="<IDLE>")||($w=="oracle"&&$x=="sql_text"&&$X!="")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($K["db"]!=""?"db=".urlencode($K["db"])."&":"")."sql=".urlencode($X)).'">'.'Upravit'.'</a>':nbsp($X));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('%d celkem',$f->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Ukončit'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$D=>$X){$X=process_input($l[$D]);if($X!==null){if($_POST["clone"])$O[idf_escape($D)]=($X!==false?$X:idf_escape($D));elseif($X!==false)$O[]=idf_escape($D)." = $X";}}$H.=($_POST["clone"]?" (".implode(", ",array_keys($O)).")\nSELECT ".implode(", ",$O)."\nFROM ".table($a):" SET\n".implode(",\n",$O));}if($_POST["delete"]||$O){$Ua="UPDATE";if($_POST["delete"]){$Ua="DELETE";$H="FROM $H";}if($_POST["clone"]){$Ua="INSERT";$H="INTO $H";}if($_POST["all"]||($Wf===array()&&$_POST["check"])||$Jc){$I=queries("$Ua $H".($_POST["all"]?($Z?"\nWHERE ".implode(" AND ",$Z):""):"\nWHERE $kg"));$ma=$f->affected_rows;}else{foreach((array)$_POST["check"]as$X){$I=queries($Ua.limit1($H,"\nWHERE ".where_check($X,$l)));if(!$I)break;$ma+=$f->affected_rows;}}}$B=lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ma);if($_POST["clone"]&&$I&&$ma==1){$Vc=last_id();if($Vc)$B=sprintf('Položka%s byla vložena.'," $Vc");}queries_redirect(remove_from_uri("page"),$B,$I);}elseif(!$_POST["import"]){if(!$_POST["val"])$j='Ctrl+klikněte na políčko, které chcete změnit.';else{$I=true;$ma=0;foreach($_POST["val"]as$Uf=>$K){$O=array();foreach($K
as$x=>$X){$x=bracket_escape($x,1);$O[]=idf_escape($x)." = ".(ereg('char|text',$l[$x]["type"])||$X!=""?$b->processInput($l[$x],$X):"NULL");}$H=table($a)." SET ".implode(", ",$O);$jg=" WHERE ".where_check($Uf,$l).($Z?" AND ".implode(" AND ",$Z):"");$I=queries("UPDATE".($Jc?" $H$jg":limit1($H,$jg)));if(!$I)break;$ma+=$f->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ma),$I);}}elseif(is_string($ec=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($la["output"])."&format=".urlencode($_POST["separator"]));$I=true;$Sa=array_keys($l);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$ec,$gd);$ma=count($gd[0]);begin();$Xe=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($gd[0]as$x=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$Xe]*)$Xe~",$X.$Xe,$hd);if(!$x&&!array_diff($hd[1],$Sa)){$Sa=$hd[1];$ma--;}else{$O=array();foreach($hd[1]as$r=>$Pa)$O[idf_escape($Sa[$r])]=($Pa==""&&$l[$Sa[$r]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Pa))));$I=insert_update($a,$O,$pe);if(!$I)break;}}if($I)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('Byl importován %d záznam.','Byly importovány %d záznamy.','Bylo importováno %d záznamů.'),$ma),$I);queries("ROLLBACK");}else$j=upload_error($ec);}}$qf=$b->tableName($S);if(is_ajax())ob_start();page_header('Vypsat'.": $qf",$j);$O=null;if(isset($Le["insert"])){$O="";foreach((array)$_GET["where"]as$X){if(count($n[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!ereg('[_%]',$X["val"]))))$O.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$O);if(!$e)echo"<p class='error'>".'Nepodařilo se vypsat tabulku'.($l?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($M,$e);$b->selectSearchPrint($Z,$e,$u);$b->selectOrderPrint($Nd,$e,$u);$b->selectLimitPrint($y);$b->selectLengthPrint($_f);$b->selectActionPrint($u);echo"</form>\n";$E=$_GET["page"];if($E=="last"){$nc=$f->result("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):""));$E=floor(max(0,$nc-1)/$y);}$H=$b->selectQueryBuild($M,$Z,$sc,$Nd,$y,$E);if(!$H)$H="SELECT".limit((+$y&&$sc&&$Jc&&$w=="sql"?"SQL_CALC_FOUND_ROWS ":"").$pc,($Z?"\nWHERE ".implode(" AND ",$Z):"").$tc,($y!=""?+$y:null),($E?$y*$E:0),"\n");echo$b->selectQuery($H);$I=$f->query($H);if(!$I)echo"<p class='error'>".error()."\n";else{if($w=="mssql")$I->seek($y*$E);$Jb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$L=array();while($K=$I->fetch_assoc()){if($E&&$w=="oracle")unset($K["RNUM"]);$L[]=$K;}if($_GET["page"]!="last")$nc=(+$y&&$sc&&$Jc?($w=="sql"?$f->result(" SELECT FOUND_ROWS()"):$f->result("SELECT COUNT(*) FROM ($H) x")):count($L));if(!$L)echo"<p class='message'>".'Žádné řádky.'."\n";else{$_a=$b->backwardKeys($a,$qf);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$sc&&$M?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upravit'."</a>");$vd=array();$p=array();reset($M);$ye=1;foreach($L[0]as$x=>$X){if($x!=$Bd){$X=$_GET["columns"][key($M)];$k=$l[$M?($X?$X["col"]:current($M)):$x];$D=($k?$b->fieldName($k,$ye):"*");if($D!=""){$ye++;$vd[$x]=$D;$Ta=idf_escape($x);$zc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($x);$sb="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($zc.($Nd[0]==$Ta||$Nd[0]==$x||(!$Nd&&$Jc&&$sc[0]==$Ta)?$sb:'')).'">';echo(!$M||$X?apply_sql_function($X["fun"],$D):h(current($M)))."</a>";echo"<span class='column hidden'>","<a href='".h($zc.$sb)."' title='".'sestupně'."' class='text'> ↓</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($x)).'\'); return false;" title="'.'Vyhledat'.'" class="text jsonly"> =</a>';echo"</span>";}$p[$x]=$X["fun"];next($M);}}$bd=array();if($_GET["modify"]){foreach($L
as$K){foreach($K
as$x=>$X)$bd[$x]=max($bd[$x],min(40,strlen(utf8_decode($X))));}}echo($_a?"<th>".'Vztahy':"")."</thead>\n";if(is_ajax()){if($y%2==1&&$E%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($L,$n)as$C=>$K){$Tf=unique_array($L[$C],$u);$Uf="";foreach($Tf
as$x=>$X)$Uf.="&".($X!==null?urlencode("where[".bracket_escape($x)."]")."=".urlencode($X):"null%5B%5D=".urlencode($x));echo"<tr".odd().">".(!$sc&&$M?"":"<td>".checkbox("check[]",substr($Uf,1),in_array(substr($Uf,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Jc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Uf)."'>".'upravit'."</a>"));foreach($K
as$x=>$X){if(isset($vd[$x])){$k=$l[$x];if($X!=""&&(!isset($Jb[$x])||$Jb[$x]!=""))$Jb[$x]=(is_mail($X)?$vd[$x]:"");$z="";$X=$b->editVal($X,$k);if($X!==null){if(ereg('blob|bytea|raw|file',$k["type"])&&$X!="")$z=ME.'download='.urlencode($a).'&field='.urlencode($x).$Uf;if($X==="")$X="&nbsp;";elseif(is_utf8($X)){if($_f!=""&&is_shortable($k))$X=shorten_utf8($X,max(0,+$_f));else$X=h($X);}if(!$z){foreach((array)$n[$x]as$m){if(count($n[$x])==1||end($m["source"])==$x){$z="";foreach($m["source"]as$r=>$bf)$z.=where_link($r,$m["target"][$r],$L[$C][$bf]);$z=($m["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($m["db"]),ME):ME).'select='.urlencode($m["table"]).$z;if(count($m["source"])==1)break;}}}if($x=="COUNT(*)"){$z=ME."select=".urlencode($a);$r=0;foreach((array)$_GET["where"]as$W){if(!array_key_exists($W["col"],$Tf))$z.=where_link($r++,$W["col"],$W["val"],$W["op"]);}foreach($Tf
as$Oc=>$W)$z.=where_link($r++,$Oc,$W);}}if(!$z){if(is_mail($X))$z="mailto:$X";if($we=is_url($K[$x]))$z=($we=="http"&&$ba?$K[$x]:"$we://www.adminer.org/redirect/?url=".urlencode($K[$x]));}$s=h("val[$Uf][".bracket_escape($x)."]");$Y=$_POST["val"][$Uf][bracket_escape($x)];$vc=h($Y!==null?$Y:$K[$x]);$fd=strpos($X,"<i>...</i>");$Fb=is_utf8($X)&&$L[$C][$x]==$K[$x]&&!$p[$x];$zf=ereg('text|lob',$k["type"]);echo(($_GET["modify"]&&$Fb)||$Y!==null?"<td>".($zf?"<textarea name='$s' cols='30' rows='".(substr_count($K[$x],"\n")+1)."'>$vc</textarea>":"<input name='$s' value='$vc' size='$bd[$x]'>"):"<td id='$s' onclick=\"selectClick(this, event, ".($fd?2:($zf?1:0)).($Fb?"":", '".h('Ke změně této hodnoty použijte odkaz upravit.')."'").");\">".$b->selectVal($X,$z,$k));}}if($_a)echo"<td>";$b->backwardKeysPrint($_a,$L[$C]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n",(!$sc&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if(($L||$E)&&!is_ajax()){$Tb=true;if($_GET["page"]!="last"&&+$y&&!$Jc&&($nc>=$y||$E)){$nc=found_rows($S,$Z);if($nc<max(1e4,2*($E+1)*$y))$nc=reset(slow_query("SELECT COUNT(*) FROM ".table($a).($Z?" WHERE ".implode(" AND ",$Z):"")));else$Tb=false;}echo"<p class='pages'>";if(+$y&&($nc===false||$nc>$y)){$jd=($nc===false?$E+(count($L)>=$y?2:1):floor(($nc-1)/$y));echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($E+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$E).($E>5?" ...":"");for($r=max(1,$E-4);$r<min($jd,$E+5);$r++)echo
pagination($r,$E);echo($E+5<$jd?" ...":"").($Tb&&$nc!==false?pagination($jd,$E):' <a href="'.h(remove_from_uri("page")."&page=last").'">'.'poslední'."</a>");}echo($nc!==false?" (".($Tb?"":"~ ").lang(array('%d řádek','%d řádky','%d řádků'),$nc).")":""),(+$y&&($nc===false?count($L)+1:$nc-$E*$y)>$y?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$y).', \''.'Nahrává se'.'\');">'.'Nahrát další data'.'</a>':'')," ".checkbox("all",1,0,'celý výsledek')."\n";if($b->selectCommandPrint()){echo'<fieldset><legend>Upravit</legend><div>
...
as$x=>$X)json_row("sum-$x",number_format($X,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$f->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$i=>$X)json_row("tables-".js_escape($i),$X);json_row("");}exit;}else{$wf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($wf&&!$j&&!$_POST["search"]){$I=true;$B="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$B='Tabulky byly vyprázdněny.';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$B='Tabulky byly přesunuty.';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$B='Tabulky byly zkopírovány.';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$B='Tabulky byly odstraněny.';}elseif($w!="sql"){$I=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$B='Tabulky byly optimalizovány.';}elseif($_POST["tables"]&&($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($K=$I->fetch_assoc())$B.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$B,$I);}page_header(($_GET["ns"]==""?'Databáze'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$j,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabulky a pohledy'."</h3>\n";$vf=tables_list();if(!$vf)echo"<p class='message'>".'Žádné tabulky.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Vyhledat data v tabulkách'.": <input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhledat'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabulka','<td>'.'Úložiště','<td>'.'Porovnávání','<td>'.'Velikost dat','<td>'.'Velikost indexů','<td>'.'Volné místo','<td>'.'Auto Increment','<td>'.'Řádků',(support("comment")?'<td>'.'Komentář':''),"</thead>\n";foreach($vf
as$D=>$U){$fg=($U!==null&&!eregi("table",$U));echo'<tr'.odd().'><td>'.checkbox(($fg?"views[]":"tables[]"),$D,in_array($D,$wf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($D).'" title="'.'Zobrazit strukturu'.'">'.h($D).'</a>';if($fg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($D).'" title="'.'Pozměnit pohled'.'">'.'Pohled'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($D).'" title="'.'Vypsat data'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Pozměnit tabulku'),"Index_length"=>array("indexes",'Pozměnit indexy'),"Data_free"=>array("edit",'Nová položka'),"Auto_increment"=>array("auto_increment=1&create",'Pozměnit tabulku'),"Rows"=>array("select",'Vypsat data'),)as$x=>$z)echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($D)."' id='$x-".h($D)."' title='$z[1]'>?</a>":"<td id='$x-".h($D)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($D)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkem',count($vf)),"<td>".nbsp($w=="sql"?$f->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x)echo"<td align='right' id='sum-$x'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$w)?($w!="sqlite"?"<input type='submit' value='".'Analyzovat'."'> ":"")."<input type='submit' name='optimize' value='".'Optimalizovat'."'> ":"").($w=="sql"?"<input type='submit' name='check' value='".'Zkontrolovat'."'> <input type='submit' name='repair' value='".'Opravit'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdnit'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstranit'."'".confirm("formChecked(this, /tables|views/)").">\n";$h=(support("scheme")?schemas():$b->databases());if(count($h)!=1&&$w!="sqlite"){$i=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Přesunout do jiné databáze'.": ",($h?html_select("target",$h,$i):'<input name="target" value="'.h($i).'">')," <input type='submit' name='move' value='".'Přesunout'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Zkopírovat'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$T'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvořit tabulku'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Vytvořit pohled'."</a>\n";if(support("routine")){echo"<h3>".'Procedury a funkce'."</h3>\n";$Pe=routines();if($Pe){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Název'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($Pe
as$K){echo'<tr'.odd().'>','<th><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($K["ROUTINE_NAME"]).'">'.h($K["ROUTINE_NAME"]).'</a>','<td>'.h($K["ROUTINE_TYPE"]),'<td>'.h($K["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($K["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($K["ROUTINE_NAME"]).'">'.'Změnit'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Vytvořit proceduru'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Vytvořit funkci'."</a>\n";}if(support("event")){echo"<h3>".'Události'."</h3>\n";$L=get_rows("SHOW EVENTS");if($L){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Název'."<td>".'Plán'."<td>".'Začátek'."<td>".'Konec'."</thead>\n";foreach($L
as$K){echo"<tr>",'<th><a href="'.h(ME).'event='.urlencode($K["Name"]).'">'.h($K["Name"])."</a>","<td>".($K["Execute at"]?'V daný čas'."<td>".$K["Execute at"]:'Každých'." ".$K["Interval value"]." ".$K["Interval field"]."<td>$K[Starts]"),"<td>$K[Ends]";}echo"</table>\n";$Sb=$f->result("SELECT @@event_scheduler");if($Sb&&$Sb!="ON")echo"<p class='error'><code class='jush-sqlset'>event_scheduler</code>: ".h($Sb)."\n";}echo'<p><a href="'.h(ME).'event=">'.'Vytvořit událost'."</a>\n";}if($vf)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}}}page_footer();