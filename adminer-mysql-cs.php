...
confirm($Ec="",$kf=false){return" onclick=\"".($kf?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Ec?" (' + $Ec + ')":"")."');\"";}function
js_escape($T){return
addcslashes($T,"\r\n'\\/");}function
...
upload_error($q){$le=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Nepodařilo se nahrát soubor.'.($le?" ".sprintf('Maximální povolená velikost souboru je %sB.',$le):""):'Soubor neexistuje.');}function
...
enum_input($N,$Ca,$d,$o,$Bb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$fa);$e=(isset($Bb)?"<label><input type='$N'$Ca value='$Bb'".((is_array($o)?in_array($Bb,$o):$o===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($fa[1]as$h=>$b){$b=stripcslashes(str_replace("''","'",$b));$Ja=(is_int($o)?$o==$h+1:(is_array($o)?in_array($h+1,$o):$o===$b));$e.=" <label><input type='$N'$Ca value='".($h+1)."'".($Ja?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
input($d,$o,$_){global$ba,$n,$r;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Ad=($r=="mssql"&&$d["auto_increment"]);if($Ad&&!$_POST["save"]){$_=null;}$W=(isset($_GET["select"])||$Ad?array("orig"=>'původní'):array())+$n->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($tf,$wb){$cc=($wb==1||(!$wb&&'cs'=='fr')?0:('cs'=='sl'&&(!$wb||$wb>2)?1:0)+((!$wb||$wb>=5)&&ereg('cs|sk|ru|sl|pl','cs')?2:1));return
...
selectLinks($Jc,$u=""){echo'<p class="tabs">';$za=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Jc)){$za["view"]='Pozměnit pohled';}else{$za["create"]='Pozměnit tabulku';}if(isset($u)){$za["edit"]='Nová položka';}foreach($za
...
selectColumnsPrint($C,$w){global$W,$zb;print_fieldset("select",'Vypsat',$C);$h=0;$Vd=array('Funkce'=>$W,'Agregace'=>$zb);foreach($C
...
as$h=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$h]' value='".h($_GET["fulltext"][$h])."'>",checkbox("boolean[$h]",1,isset($_GET["boolean"][$h]),"BOOL"),"<br>\n";}}$h=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$h][col]'><option value=''>(".'kdekoliv'.")".optionlist($w,$b["col"],true)."</select>",html_select("where[$h][op]",$this->operators,$b["op"]),"<input name='where[$h][val]' value='".h($b["val"])."'></div>\n";$h++;}}echo"<div><select name='where[$h][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoliv'.")".optionlist($w,null,true)."</select>",html_select("where[$h][op]",$this->operators,"="),"<input name='where[$h][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Fa,$w,$z){print_fieldset("sort",'Seřadit',$Fa);$h=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$h]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$h]",1,isset($_GET["desc"][$c]),'sestupně')."</div>\n";$h++;}}echo"<div><select name='order[$h]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>","<label><input type='checkbox' name='desc[$h]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
...
false;}function
messageQuery($j){global$r;static$Ec=0;restart_session();$D="sql-".($Ec++);$Oa=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Oa[$_GET["db"]][]=$j;return" <a href='#$D' onclick=\"return !toggle('$D');\">".'SQL příkaz'."</a><div id='$D' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Oa[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$e);}function
editInput($l,$d,$Ca,$o){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ca value='-1' checked><i>".'původní'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ca value=''".(isset($o)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ca,$d,$o,0);}return"";}function
...
';if($Ib!="db"&&DB!=""&&$g->select_db(DB)){if($_GET["ns"]!==""&&!$Ib){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$la=tables_list();if(!$la){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($la);$za=array();foreach($la
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Ke[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ba[$c]==254){$b="<code>$b</code>";}}if(isset($za[$c])&&!$w[$za[$c]]){$y="edit=".urlencode($za[$c]);foreach($z[$za[$c]]as$sc=>$aa){$y.="&where".urlencode("[".bracket_escape($sc)."]")."=".urlencode($a[$aa]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($h?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($c,$d,$S,$P=array()){global$Gc,$ba,$Db,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ba[$d["type"]])?array():array($d["type"]))+$Gc+($P?array('Cizí klíče'=>$P):array()),$d["type"]),'</select>
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$h][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $pe, 1);'>&nbsp;"."<input type='image' name='up[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.2' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.2' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Tc==""||support("drop_col")?"<input type='image' name='drop_col[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.2' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$ob;}function
...
<p><a href="',h(ME."schema=".urlencode($Ae)),'" id="schema-link">Trvalý odkaz</a>
...
<input type="hidden" name="token" value="',$B,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"]);$Uc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Uc[]=".$c";}}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($Uc?"[".implode("|",$Uc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($Oa){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($Oa
...
',(support("comment")?checkbox("","",$ob,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložit">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
...
';if($Ka){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["event"])){$Ua=$_GET["event"];$Md=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$jd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ua),substr(ME,0,-1),'Událost byla odstraněna.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Md)&&isset($jd[$_POST["STATUS"]])){$Od="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ua!=""?'Událost byla změněna.':'Událost byla vytvořena.'),queries(($Ua!=""?"ALTER EVENT ".idf_escape($Ua).$Od.($Ua!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Od)."\n".$jd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ua!=""?'Pozměnit událost'.": ".h($Ua):'Vytvořit událost'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ua!=""){$E=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ua));$a=reset($E);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Md,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$jd,$a["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
<input type="submit" value="Uložit">
';if($Ea!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($Ka){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["type"])){$zc=$_GET["type"];if($_POST&&!$q){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($zc),$y,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ byl vytvořen.');}}page_header($zc!=""?'Pozměnit typ'.": ".h($zc):'Vytvořit typ',$q);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($zc!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložit'."'>\n";}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$h=0;foreach($Ta
as$Qa=>$Q){echo'<th>'.($Qa!="*.*"?"<input name='objects[$h]' value='".h($Qa)."' size='10'>":"<input type='hidden' name='objects[$h]' value='*.*' size='10'>*.*");$h++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Bc=>$Pb){foreach((array)$Y[$Bc]as$Ub=>$cb){echo"<tr".odd()."><td".($Pb?">$Pb<td":" colspan='2'").' lang="en" title="'.h($cb).'">'.h($Ub);$h=0;foreach($Ta
...
<input type="submit" value="Uložit">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$ld=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$ld++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$ld),$ld||!$_POST["kill"]);}page_header('Seznam procesů',$q);echo'
<form action="" method="post">
