...
confirm($Xa="",$Ke=false){return" onclick=\"".($Ke?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($Xa?" (' + $Xa + ')":"")."');\"";}function
...
upload_error($i){$Tc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($i?'Nepodařilo se nahrát soubor.'.($Tc?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Tc):""):'Soubor neexistuje.');}function
...
enum_input($U,$ta,$j,$Y,$_b=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$Oc);$I=(isset($_b)?"<label><input type='$U'$ta value='$_b'".((is_array($Y)?in_array($_b,$Y):$Y===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($Oc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Fa=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$I.=" <label><input type='$U'$ta value='".($r+1)."'".($Fa?' checked':'').'>'.h($b->editVal($X,$j)).'</label>';}return$I;}function
input($j,$Y,$n){global$vf,$b,$w;$C=h(bracket_escape($j["field"]));echo"<td class='function'>";$oe=($w=="mssql"&&$j["auto_increment"]);if($oe&&!$_POST["save"]){$n=null;}$o=(isset($_GET["select"])||$oe?array("orig"=>'původní'):array())+$b->editFunctions($j);$ta=" name='fields[$C]'";if($j["type"]=="enum"){echo
...
selectLinks($Te,$O=""){echo'<p class="tabs">';$Lc=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Te)){$Lc["view"]='Pozměnit pohled';}else{$Lc["create"]='Pozměnit tabulku';}if(isset($O)){$Lc["edit"]='Nová položka';}foreach($Lc
...
selectColumnsPrint($M,$d){global$o,$hc;print_fieldset("select",'Vypsat',$M);$r=0;$ec=array('Funkce'=>$o,'Agregace'=>$hc);foreach($M
...
false;}function
messageQuery($G){global$w;static$Xa=0;restart_session();$s="sql-".($Xa++);$jc=&get_session("queries");if(strlen($G)>1e6){$G=ereg_replace('[\x80-\xFF]+$','',substr($G,0,1e6))."\n...";}$jc[$_GET["db"]][]=$G;return" <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL příkaz'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($G,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($jc[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
';if($bd!="db"&&DB!=""&&$e->select_db(DB)){if($_GET["ns"]!==""&&!$bd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$Ye=tables_list();if(!$Ye){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($Ye);$Lc=array();foreach($Ye
...
var noResponse = 'Server neodpověděl.';
</script>
...
as$x=>$X){if(!isset($X)){$X="<i>NULL</i>";}elseif($_a[$x]&&!is_utf8($X)){$X="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($X))."</i>";}elseif(!strlen($X)){$X="&nbsp;";}else{$X=h($X);if($vf[$x]==254){$X="<code>$X</code>";}}if(isset($Lc[$x])&&!$d[$Lc[$x]]){if($mc){$z=$Lc[$x].urlencode($J[array_search("table=",$Lc)]);}else{$z="edit=".urlencode($Lc[$x]);foreach($u[$Lc[$x]]as$Ia=>$v){$z.="&where".urlencode("[".bracket_escape($Ia)."]")."=".urlencode($J[$v]);}}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($x,$j,$c,$m=array()){global$Me,$vf,$Bf,$pd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$j["type"]||isset($vf[$j["type"]])?array():array($j["type"]))+$Me+($m?array('Cizí klíče'=>$m):array()),$j["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($j["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$j["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($c,$j["collation"]).'</select>',($Bf?"<select name='$x"."[unsigned]'".(!$j["type"]||ereg('(int|float|double|decimal)$',$j["type"])?"":" class='hidden'").'><option>'.optionlist($Bf,$j["unsigned"]).'</select>':''),($m?"<select name='$x"."[on_delete]'".(ereg("`",$j["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$pd),$j["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Qa?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($k))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($k),';</script>
</thead>
...
',(support("comment")?"<td".($Qa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($j["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $qa, 1);'>&nbsp;"."<input type='image' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($zd==""||support("drop_col")?"<input type='image' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$x=>$X){echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Změnit'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Přidat trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databáze',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Ve=array();$We=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Oc,PREG_SET_ORDER);foreach($Oc
...
as$C=>$j){if(!isset($j["privileges"][$Cf?"update":"insert"])||$b->fieldName($j)==""){unset($k[$C]);}}if($_POST&&!$i&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"]){$_=($Cf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$_)){$_=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Položka byla smazána.');}else{$O=array();foreach($k
as$C=>$j){$X=process_input($j);if($X!==false&&$X!==null){$O[idf_escape($C)]=($Cf?"\n".idf_escape($C)." = $X":$X);}}if($Cf){if(!$O){redirect($_);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Položka byla aktualizována.');}else{$H=insert_into($a,$O);$Ec=($H?last_id():0);queries_redirect($_,sprintf('Položka%s byla vložena.',($Ec?" $Ec":"")),$H);}}}$Ue=$b->tableName(table_status($a));page_header(($Cf?'Upravit':'Vložit'),$i,array("select"=>array($a,$Ue)),$Ue);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($Z){$M=array();foreach($k
...
';if($k){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Cf?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}}echo($Cf?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Jd=ereg('RANGE|LIST',$J["partition_by"]);print_fieldset("partition",'Rozdělit podle',$J["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Id,$J["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($J["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($J["partitions"]),'"',($Jd||!$J["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$i&&!isset($_POST["add_x"])){restart_session();$C=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáze byla odstraněna.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($C),'Databáze byla přejmenována.',rename_database($C,$_POST["collation"]));}else{$g=explode("\n",str_replace("\r","",$C));$Ne=true;$Dc="";foreach($g
as$h){if(count($g)==1||$h!=""){if(!create_database($h,$_POST["collation"])){$Ne=false;}$Dc=$h;}}queries_redirect(ME."db=".urlencode($Dc),'Databáze byla vytvořena.',$Ne);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($C).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$i,array(),DB);$c=collations();$C=DB;$Ja=null;if($_POST){$C=$_POST["name"];$Ja=$_POST["collation"];}elseif(DB!=""){$Ja=db_collation(DB,$c);}elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$C=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$pd),$J["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$pd),$J["on_update"]),'<p>
<input type="submit" value="Uložit">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$wc,$J["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Je,$J["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE"){$fc["$A[2]$X[2]"][$X[1]]=true;}if(ereg(' WITH GRANT OPTION',$J[0])){$fc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$J[0],$A)){$md=$A[1];}}}if($_POST&&!$i){$nd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$gd=q($_POST["user"])."@".q($_POST["host"]);$Md=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $nd",ME."privileges=",'Uživatel byl odstraněn.');}else{$ab=false;if($nd!=$gd){$ab=queries(($e->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $gd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Md");$i=!$ab;}elseif($_POST["pass"]!=$md||!$_POST["hashed"]){queries("SET PASSWORD FOR $gd = ".($_POST["hashed"]?$Md:"PASSWORD($Md)"));}if(!$i){$pe=array();foreach($fd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($e->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$r=0;foreach($fc
as$jd=>$p){echo'<th>'.($jd!="*.*"?"<input name='objects[$r]' value='".h($jd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Va=>$kb){foreach((array)$ae[$Va]as$Zd=>$Pa){echo"<tr".odd()."><td".($kb?">$kb<td":" colspan='2'").' lang="en" title="'.h($Pa).'">'.h($Zd);$r=0;foreach($fc
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$i){$Ac=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X))){$Ac++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Ac),$Ac||!$_POST["kill"]);}page_header('Seznam procesů',$i);echo'
<form action="" method="post">
