...
confirm($yc="",$pf=false){return" onclick=\"".($pf?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($yc?" (' + $yc + ')":"")."');\"";}function
js_escape($T){return
addcslashes($T,"\r\n'\\/");}function
...
upload_error($q){$me=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Nepodařilo se nahrát soubor.'.($me?" ".sprintf('Maximální povolená velikost souboru je %sB.',$me):""):'Soubor neexistuje.');}function
...
enum_input($O,$Ca,$d,$o,$xb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$fa);$e=(isset($xb)?"<label><input type='$O'$Ca value='$xb'".((is_array($o)?in_array($xb,$o):$o===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($fa[1]as$g=>$b){$b=stripcslashes(str_replace("''","'",$b));$Ha=(is_int($o)?$o==$g+1:(is_array($o)?in_array($g+1,$o):$o===$b));$e.=" <label><input type='$O'$Ca value='".($g+1)."'".($Ha?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
input($d,$o,$A){global$ba,$n,$s;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$ke=($s=="mssql"&&$d["auto_increment"]);if($ke&&!$_POST["save"]){$A=null;}$W=(isset($_GET["select"])||$ke?array("orig"=>'původní'):array())+$n->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Qe,$tb){$hc=($tb==1||(!$tb&&'cs'=='fr')?0:('cs'=='sl'&&(!$tb||$tb>2)?1:0)+((!$tb||$tb>=5)&&ereg('cs|sk|ru|sl','cs')?2:1));return
...
selectLinks($Ec,$u=""){echo'<p class="tabs">';$Aa=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($Ec)){$Aa["view"]='Pozměnit pohled';}else{$Aa["create"]='Pozměnit tabulku';}if(isset($u)){$Aa["edit"]='Nová položka';}foreach($Aa
...
selectColumnsPrint($C,$v){global$W,$Fb;print_fieldset("select",'Vypsat',$C);$g=0;$ye=array('Funkce'=>$W,'Agregace'=>$Fb);foreach($C
...
selectOrderPrint($La,$v,$z){print_fieldset("sort",'Seřadit',$La);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$g]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'sestupně')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$g]",1,0,'sestupně')."</div>\n","</div></fieldset>\n";}function
...
false;}function
messageQuery($j){global$s;static$yc=0;restart_session();$D="sql-".($yc++);$Ia=&get_session("queries");$Ia[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$D' onclick=\"return !toggle('$D');\">".'SQL příkaz'."</a><div id='$D' class='hidden'><pre><code class='jush-$s'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ia[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
';if($Cb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$Cb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$qa=tables_list();if(!$qa){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($qa);$Aa=array();foreach($qa
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($ne[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ba[$c]==254){$b="<code>$b</code>";}}if(isset($Aa[$c])&&!$v[$Aa[$c]]){$y="edit=".urlencode($Aa[$c]);foreach($z[$Aa[$c]]as$Mc=>$ca){$y.="&where".urlencode("[".bracket_escape($Mc)."]")."=".urlencode($a[$ca]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($c,$d,$U,$L=array()){global$Oc,$ba,$Gb,$Ka;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ba[$d["type"]])?array():array($d["type"]))+$Oc+($L?array('Cizí klíče'=>$L):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($U,$d["collation"]).'</select>',($Gb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Gb,$d["unsigned"]).'</select>':''),($L?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist($Ka,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($mb?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($mb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $de, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.0' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.0' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($cd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.0' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$mb;}function
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"]);$Wc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Wc[]=".$c";}}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($Wc?"[".implode("|",$Wc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($Ia){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($Ia
...
as$f=>$d){if(!isset($d["privileges"][$Ja?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$M=$_POST["referer"];if($_POST["insert"]){$M=($Ja?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$M)){$M=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$M,'Položka byla smazána.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Ja?"\n".idf_escape($f)." = $b":$b);}}if($Ja){if(!$u){redirect($M);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$M,'Položka byla aktualizována.');}else{$k=insert_into($h,$u);$Sd=($k?last_id():0);queries_redirect($M,sprintf('Položka%s byla vložena.',($Sd?" $Sd":"")),$k);}}}$wa=$n->tableName(table_status($h));page_header(($Ja?'Upravit':'Vložit'),$q,array("select"=>array($h,$wa)),$wa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$C=array();foreach($p
...
';if($h==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Uc?html_select("Engine",array(""=>"(".'úložiště'.")")+$Uc,$a["Engine"]):""),' ',($U&&!ereg("sqlite|mssql",$s)?html_select("Collation",array(""=>"(".'porovnávání'.")")+$U,$a["Collation"]):""),' <input type="submit" value="Uložit">
...
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if(support("partitioning")){$Jd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdělit podle',$a["partition_by"]);echo'<p>
...
as$w){if(count($_)==1||$w!=""){if(!create_database($w,$_POST["collation"])){$Ld=false;}$bb=$w;}}queries_redirect(ME."db=".urlencode($bb),'Databáze byla vytvořena.',$Ld);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$q,array(),DB);$U=collations();$f=DB;$Mb=null;if($_POST){$f=$_POST["name"];$Mb=$_POST["collation"];}elseif(DB!=""){$Mb=db_collation(DB,$U);}elseif($s=="sql"){foreach(get_vals("SHOW GRANTS")as$P){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$P,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeypress="return textareaKeypress(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($U?html_select("collation",array(""=>"(".'porovnávání'.")")+$U,$Mb):"");?>
...
<input type="hidden" name="token" value="',$B,'">
';if($Qa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
</form>
';}elseif(isset($_GET["event"])){$Sa=$_GET["event"];$oe=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Vc=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Sa),substr(ME,0,-1),'Událost byla odstraněna.');}elseif(in_array($_POST["INTERVAL_FIELD"],$oe)&&isset($Vc[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Sa!=""?"ALTER EVENT ".idf_escape($Sa).$Td.($Sa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$Vc[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Sa!=""?'Událost byla změněna.':'Událost byla vytvořena.'));}}page_header(($Sa!=""?'Pozměnit událost'.": ".h($Sa):'Vytvořit událost'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Sa!=""){$E=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Sa));$a=reset($E);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$oe,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Vc,$a["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Uložit">
';if($Sa!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'</form>
...
<input type="hidden" name="token" value="',$B,'">
';if($Qa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložit">
';if($Fa!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'</form>
...
as$Ga=>$P){echo'<th>'.($Ga!="*.*"?"<input name='objects[$g]' value='".h($Ga)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$lc=>$fc){foreach((array)$Y[$lc]as$Zb=>$eb){echo"<tr".odd()."><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($eb).'">'.h($Zb);$g=0;foreach($Ta
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$kd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$kd++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$kd),$kd||!$_POST["kill"]);}page_header('Seznam procesů',$q);echo'
<form action="" method="post">
