...
confirm($Xa="",$Ke=false){return" onclick=\"".($Ke?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($Xa?" (' + $Xa + ')":"")."');\"";}function
...
upload_error($i){$Tc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($i?'Wgranie pliku było niemożliwe.'.($Tc?" ".sprintf('Maksymalna wielkość pliku to %sB.',$Tc):""):'Plik nie istnieje.');}function
...
enum_input($U,$ta,$j,$Y,$_b=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$Oc);$I=(isset($_b)?"<label><input type='$U'$ta value='$_b'".((is_array($Y)?in_array($_b,$Y):$Y===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($Oc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Fa=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$I.=" <label><input type='$U'$ta value='".($r+1)."'".($Fa?' checked':'').'>'.h($b->editVal($X,$j)).'</label>';}return$I;}function
input($j,$Y,$n){global$vf,$b,$w;$C=h(bracket_escape($j["field"]));echo"<td class='function'>";$oe=($w=="mssql"&&$j["auto_increment"]);if($oe&&!$_POST["save"]){$n=null;}$o=(isset($_GET["select"])||$oe?array("orig"=>'bez zmian'):array())+$b->editFunctions($j);$ta=" name='fields[$C]'";if($j["type"]=="enum"){echo
...
<tr><th>Rodzaj bazy<td>',html_select("driver",$pb,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Te,$O=""){echo'<p class="tabs">';$Lc=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($Te)){$Lc["view"]='Zmień perspektywę';}else{$Lc["create"]='Zmień tabelę';}if(isset($O)){$Lc["edit"]='Nowy rekord';}foreach($Lc
...
selectQuery($G){global$w;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Ostatnia strona'."'>&gt;&gt;</a> <code class='jush-$w'>".h(str_replace("\n"," ",$G))."</code> <a href='".h(ME)."sql=".urlencode($G)."'>".'Edytuj'."</a></p>\n";}function
rowDescription($R){return"";}function
rowDescriptions($K,$Yb){return$K;}function
selectVal($X,$z,$j){$I=($X!="<i>NULL</i>"&&ereg("char|binary",$j["type"])&&!ereg("var",$j["type"])?"<code>$X</code>":$X);if(ereg('blob|bytea|raw|file',$j["type"])&&!is_utf8($X)){$I=lang(array('%d bajt','%d bajty','%d bajtów'),strlen(html_entity_decode($X,ENT_QUOTES)));}return($z?"<a href='$z'>$I</a>":$I);}function
editVal($X,$j){return(ereg("binary",$j["type"])?reset(unpack("H*",$X)):$X);}function
selectColumnsPrint($M,$d){global$o,$hc;print_fieldset("select",'pokaż',$M);$r=0;$ec=array('Funkcje'=>$o,'Agregacje'=>$hc);foreach($M
...
selectSearchPrint($Z,$d,$u){print_fieldset("search",'Szukaj',$Z);foreach($u
as$r=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$r]' value='".h($_GET["fulltext"][$r])."'>",checkbox("boolean[$r]",1,isset($_GET["boolean"][$r]),"BOOL"),"<br>\n";}}$r=0;foreach((array)$_GET["where"]as$X){if("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators)){echo"<div><select name='where[$r][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($d,$X["col"],true)."</select>",html_select("where[$r][op]",$this->operators,$X["op"]),"<input name='where[$r][val]' value='".h($X["val"])."'></div>\n";$r++;}}echo"<div><select name='where[$r][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'gdziekolwiek'.")".optionlist($d,null,true)."</select>",html_select("where[$r][op]",$this->operators,"="),"<input name='where[$r][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($wd,$d,$u){print_fieldset("sort",'Sortuj',$wd);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($d[$X])){echo"<div><select name='order[$r]'><option>".optionlist($d,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'malejąco')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($d,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'malejąco'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($y){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($y)."'>","</div></fieldset>\n";}function
selectLengthPrint($ef){if(isset($ef)){echo"<fieldset><legend>".'Długość tekstu'."</legend><div>",'<input name="text_length" size="3" value="'.h($ef).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($G){global$w;static$Xa=0;restart_session();$s="sql-".($Xa++);$jc=&get_session("queries");if(strlen($G)>1e6){$G=ereg_replace('[\x80-\xFF]+$','',substr($G,0,1e6))."\n...";}$jc[$_GET["db"]][]=$G;return" <a href='#$s' onclick=\"return !toggle('$s');\">".'Zapytanie SQL'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($G,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($jc[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
explode("/",$I);}function
editInput($R,$j,$ta,$Y){if($j["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$ta value='-1' checked><i>".'bez zmian'."</i></label> ":"").($j["null"]?"<label><input type='radio'$ta value=''".(isset($Y)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$ta,$j,$Y,0);}return"";}function
...
dumpOutput(){$I=array('text'=>'otwórz','file'=>'zapisz');if(function_exists('gzencode')){$I['gz']='gzip';}if(function_exists('bzcompress')){$I['bz2']='bzip2';}return$I;}function
dumpFormat(){return
...
<p class="logout">
';if(DB==""||!$bd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'Zapytanie SQL'."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Eksport'."</a>\n";}}echo'<input type="submit" name="logout" value="Wyloguj" onclick="eventStop(event);">
<input type="hidden" name="token" value="',$T,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($g?html_select("db",array(""=>"(".'baza danych'.")")+$g,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Wybierz"',($g?" class='hidden'":""),' onclick="eventStop(event);">
';if($bd!="db"&&DB!=""&&$e->select_db(DB)){if($_GET["ns"]!==""&&!$bd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$Ye=tables_list();if(!$Ye){echo"<p class='message'>".'Brak tabel.'."\n";}else{$this->tablesPrint($Ye);$Lc=array();foreach($Ye
...
as$R=>$U){echo'<a href="'.h(ME).'select='.urlencode($R).'"'.bold($_GET["select"]==$R).">".'przeglądaj'."</a> ",'<a href="'.h(ME).'table='.urlencode($R).'"'.bold($_GET["table"]==$R)." title='".'Struktura tabeli'."'>".$this->tableName(array("Name"=>$R))."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$x=>$X){if(!isset($X)){$X="<i>NULL</i>";}elseif($_a[$x]&&!is_utf8($X)){$X="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($X))."</i>";}elseif(!strlen($X)){$X="&nbsp;";}else{$X=h($X);if($vf[$x]==254){$X="<code>$X</code>";}}if(isset($Lc[$x])&&!$d[$Lc[$x]]){if($mc){$z=$Lc[$x].urlencode($J[array_search("table=",$Lc)]);}else{$z="edit=".urlencode($Lc[$x]);foreach($u[$Lc[$x]]as$Ia=>$v){$z.="&where".urlencode("[".bracket_escape($Ia)."]")."=".urlencode($J[$v]);}}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Brak rekordów.')."\n";}function
...
edit_type($x,$j,$c,$m=array()){global$Me,$vf,$Bf,$pd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$j["type"]||isset($vf[$j["type"]])?array():array($j["type"]))+$Me+($m?array('Klucze obce'=>$m):array()),$j["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($j["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$j["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($c,$j["collation"]).'</select>',($Bf?"<select name='$x"."[unsigned]'".(!$j["type"]||ereg('(int|float|double|decimal)$',$j["type"])?"":" class='hidden'").'><option>'.optionlist($Bf,$j["unsigned"]).'</select>':''),($m?"<select name='$x"."[on_delete]'".(ereg("`",$j["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist(explode("|",$pd),$j["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Qa?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($k))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($k),';</script>
</thead>
...
',(support("comment")?"<td".($Qa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($j["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $qa, 1);'>&nbsp;"."<input type='image' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($zd==""||support("drop_col")?"<input type='image' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<table cellspacing="0">
';$gb=array('','USE','DROP+CREATE','CREATE');$Xe=array('','DROP+CREATE','CREATE');$eb=array('','TRUNCATE+INSERT','INSERT');if($w=="sql"){$gb[]='CREATE+ALTER';$Xe[]='CREATE+ALTER';$eb[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$J);if(!$J){$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'Rezultat'."<td>".html_select("output",$b->dumpOutput(),$J["output"],0)."\n";echo"<tr><th>".'Format'."<td>".html_select("format",$b->dumpFormat(),$J["format"],0)."\n";echo($w=="sqlite"?"":"<tr><th>".'Baza danych'."<td>".html_select('db_style',$gb,$J["db_style"]).(support("routine")?checkbox("routines",1,$J["routines"],'Procedury i funkcje'):"").(support("event")?checkbox("events",1,$J["events"],'Wydarzenia'):"")),"<tr><th>".'Tabele'."<td>".html_select('table_style',$Xe,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$J["triggers"],'Wyzwalacze'):""),"<tr><th>".'Dane'."<td>".html_select('data_style',$eb,$J["data_style"]),'</table>
<p><input type="submit" value="Eksport">

<table cellspacing="0">
';$Vd=array();if(DB!=""){$Fa=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Fa onclick='formCheck(this, /^tables\\[/);'>".'Tabele'."</label>","<th style='text-align: right;'><label>".'Dane'."<input type='checkbox' id='check-data'$Fa onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$If="";foreach(table_status()as$S){$C=$S["Name"];$Ud=ereg_replace("_.*","",$C);$Fa=($a==""||$a==(substr($a,-1)=="%"?"$Ud%":$C));$Xd="<tr><td>".checkbox("tables[]",$C,$Fa,$C,"formUncheck('check-tables');");if(is_view($S)){$If.="$Xd\n";}else{echo"$Xd<td align='right'><label>".($S["Engine"]=="InnoDB"&&$S["Rows"]?"~ ":"").$S["Rows"].checkbox("data[]",$C,$Fa,"","formUncheck('check-data');")."</label>\n";}$Vd[$Ud]++;}echo$If;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Baza danych'."</label></thead>\n";$g=$b->databases();if($g){foreach($g
...
<input type="hidden" name="token" value="',$T,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zatrzymaj w przypadku błędu')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Pokaż tylko błędy')."\n";print_fieldset("webfile",'Z serwera',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ra=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$x=>$X){if(extension_loaded($X)){$Ra[]=".$x";}}echo
sprintf('Plik %s na serwerze',"<code>adminer.sql".($Ra?"[".implode("|",$Ra)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Uruchom z pliku'.'">',"</div></fieldset>\n";if($jc){print_fieldset("history",'Historia',$_GET["history"]!="");foreach($jc
as$x=>$X){echo'<a href="'.h(ME."sql=&history=$x").'">'.'Edytuj'."</a> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$X)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Wyczyść'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Edytuj wszystkie'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$C=>$j){if(!isset($j["privileges"][$Cf?"update":"insert"])||$b->fieldName($j)==""){unset($k[$C]);}}if($_POST&&!$i&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"]){$_=($Cf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$_)){$_=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Rekord został usunięty.');}else{$O=array();foreach($k
as$C=>$j){$X=process_input($j);if($X!==false&&$X!==null){$O[idf_escape($C)]=($Cf?"\n".idf_escape($C)." = $X":$X);}}if($Cf){if(!$O){redirect($_);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Rekord został zaktualizowany.');}else{$H=insert_into($a,$O);$Ec=($H?last_id():0);queries_redirect($_,sprintf('Rekord%s został dodany.',($Ec?" $Ec":"")),$H);}}}$Ue=$b->tableName(table_status($a));page_header(($Cf?'Edytuj':'Dodaj'),$i,array("select"=>array($a,$Ue)),$Ue);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($Z){$M=array();foreach($k
as$C=>$j){if(isset($j["privileges"]["select"])){$M[]=($_POST["clone"]&&$j["auto_increment"]?"'' AS ":(ereg("enum|set",$j["type"])?"1*".idf_escape($C)." AS ":"")).idf_escape($C);}}$J=array();if($M){$K=get_rows("SELECT".limit(implode(", ",$M)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'Brak rekordów.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($k){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Cf?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}}echo($Cf?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$Qa,'Komentarz',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($J["Comment"]).'" maxlength="60"'.($Qa?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
';if(support("partitioning")){$Jd=ereg('RANGE|LIST',$J["partition_by"]);print_fieldset("partition",'Partycjonowanie',$J["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Id,$J["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($J["partition"]),'">)
Partycje: <input name="partitions" size="2" value="',h($J["partitions"]),'"',($Jd||!$J["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$i&&!isset($_POST["add_x"])){restart_session();$C=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Baza danych została usunięta.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($C),'Nazwa bazy danych została zmieniona.',rename_database($C,$_POST["collation"]));}else{$g=explode("\n",str_replace("\r","",$C));$Ne=true;$Dc="";foreach($g
as$h){if(count($g)==1||$h!=""){if(!create_database($h,$_POST["collation"])){$Ne=false;}$Dc=$h;}}queries_redirect(ME."db=".urlencode($Dc),'Baza danych została utworzona.',$Ne);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($C).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$i,array(),DB);$c=collations();$C=DB;$Ja=null;if($_POST){$C=$_POST["name"];$Ja=$_POST["collation"];}elseif(DB!=""){$Ja=db_collation(DB,$c);}elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$C=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input id="name" name="name" value="'.h($C).'" maxlength="64">')."\n".($c?html_select("collation",array(""=>"(".'porównywanie znaków'.")")+$c,$Ja):"");?>
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+explode("|",$pd),$J["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+explode("|",$pd),$J["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
<noscript><p><input type="submit" name="add" value="Dodaj kolumnę"></noscript>
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$rb=false;if($_POST&&!$i){$C=trim($_POST["name"]);$rb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($C)),'Perspektywa została usunięta.','Perspektywa została zmieniona.','Perspektywa została utworzona.',$a);}page_header(($a!=""?'Zmień perspektywę':'Utwórz perspektywę'),$i,array("table"=>$a),$a);$J=$_POST;if(!$J&&$a!=""){$J=view($a);$J["name"]=$a;}echo'
<form action="" method="post">
<p>Nazwa: <input name="name" value="',h($J["name"]),'" maxlength="64">
<p>';textarea("select",$J["select"]);echo'<p>
';if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Zapisz zmiany">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$wc=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Je=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$i){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Wydarzenie zostało usunięte.');}elseif(in_array($_POST["INTERVAL_FIELD"],$wc)&&isset($Je[$_POST["STATUS"]])){$we="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Wydarzenie zostało zmienione.':'Wydarzenie zostało utworzone.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$we.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$we)."\n".$Je[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Zmień wydarzenie'.": ".h($aa):'Utwórz wydarzenie'),$i);$J=$_POST;if(!$J&&$aa!=""){$K=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$J=reset($K);}echo'
<form action="" method="post">
<table cellspacing="0">
<tr><th>Nazwa<td><input name="EVENT_NAME" value="',h($J["EVENT_NAME"]),'" maxlength="64">
<tr><th>Początek<td><input name="STARTS" value="',h("$J[EXECUTE_AT]$J[STARTS]"),'">
<tr><th>Koniec<td><input name="ENDS" value="',h($J["ENDS"]),'">
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$wc,$J["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Je,$J["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
<tr><th>&nbsp;<td>',checkbox("ON_COMPLETION","PRESERVE",$J["ON_COMPLETION"]=="PRESERVE",'Nie kasuj wydarzenia po przeterminowaniu'),'</table>
<p>';textarea("EVENT_DEFINITION",$J["EVENT_DEFINITION"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($aa!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$j){if($j["field"]!=""){$O[]=(ereg("^($tc)\$",$j["inout"])?"$j[inout] ":"").idf_escape($j["field"]).process_type($j,"CHARACTER SET");}}$rb=drop_create("DROP $se ".idf_escape($da),"CREATE $se ".idf_escape(trim($_POST["name"]))." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$te)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedura została usunięta.','Procedura została zmieniona.','Procedura została utworzona.',$da);}page_header(($da!=""?(isset($_GET["function"])?'Zmień funkcję':'Zmień procedurę').": ".h($da):(isset($_GET["function"])?'Utwórz funkcję':'Utwórz procedurę')),$i);$c=get_vals("SHOW CHARACTER SET");sort($c);$J=array("fields"=>array());if($_POST){$J=$_POST;$J["fields"]=(array)$J["fields"];process_fields($J["fields"]);}elseif($da!=""){$J=routine($da,$se);$J["name"]=$da;}echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($J["name"]),'" maxlength="64">
',($te?'Język'.": ".html_select("language",$te,$J["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$c,$se);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$J["returns"],$c);}echo'</table>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($da!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$rf=trigger_options();$qf=array("INSERT","UPDATE","DELETE");$rb=false;if($_POST&&!$i&&in_array($_POST["Timing"],$rf["Timing"])&&in_array($_POST["Event"],$qf)&&in_array($_POST["Type"],$rf["Type"])){$gf=" $_POST[Timing] $_POST[Event]";$od=" ON ".table($a);$rb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($w=="pgsql"?$od:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($w=="mssql"?$od.$gf:$gf.$od).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Wyzwalacz został usunięty.','Wyzwalacz został zmieniony.','Wyzwalacz został utworzony.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmień wyzwalacz'.": ".h($_GET["name"]):'Utwórz wyzwalacz'),$i,array("table"=>$a));$J=$_POST;if(!$J){$J=trigger($_GET["name"])+array("Trigger"=>$a."_bi");}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$rf["Timing"],$J["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$qf,$J["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$rf["Type"],$J["Type"]),'</table>
<p>Nazwa: <input name="Trigger" value="',h($J["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$J["Statement"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($rb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE"){$fc["$A[2]$X[2]"][$X[1]]=true;}if(ereg(' WITH GRANT OPTION',$J[0])){$fc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$J[0],$A)){$md=$A[1];}}}if($_POST&&!$i){$nd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$gd=q($_POST["user"])."@".q($_POST["host"]);$Md=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $nd",ME."privileges=",'Użytkownik został usunięty.');}else{$ab=false;if($nd!=$gd){$ab=queries(($e->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $gd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Md");$i=!$ab;}elseif($_POST["pass"]!=$md||!$_POST["hashed"]){queries("SET PASSWORD FOR $gd = ".($_POST["hashed"]?$Md:"PASSWORD($Md)"));}if(!$i){$pe=array();foreach($fd
...
as$jd=>$pe){if(preg_match('~^(.+)(\\(.*\\))?$~U',$jd,$A)){grant("REVOKE",array_keys($pe),$A[2]," ON $A[1] FROM $gd");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Użytkownik został zmieniony.':'Użytkownik został dodany.'),!$i);if($ab){$e->query("DROP USER $gd");}}}page_header((isset($_GET["host"])?'Użytkownik'.": ".h("$fa@$_GET[host]"):'Dodaj użytkownika'),$i,array("privileges"=>array('','Uprawnienia użytkowników')));if($_POST){$J=$_POST;$fc=$fd;}else{$J=$_GET+array("host"=>$e->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$md;if($md!=""){$J["hashed"]=true;}$fc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
<tr><th>Serwer<td><input name="host" maxlength="60" value="',h($J["host"]),'">
<tr><th>Użytkownik<td><input name="user" maxlength="16" value="',h($J["user"]),'">
<tr><th>Hasło<td><input id="pass" name="pass" value="',h($J["pass"]),'">
';if(!$J["hashed"]){echo'<script type="text/javascript">typePassword(document.getElementById(\'pass\'));</script>';}echo
checkbox("hashed",1,$J["hashed"],'Zahashowane',"typePassword(this.form['pass'], this.checked);"),'</table>

';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($e->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$r=0;foreach($fc
as$jd=>$p){echo'<th>'.($jd!="*.*"?"<input name='objects[$r]' value='".h($jd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$Va=>$kb){foreach((array)$ae[$Va]as$Zd=>$Pa){echo"<tr".odd()."><td".($kb?">$kb<td":" colspan='2'").' lang="en" title="'.h($Pa).'">'.h($Zd);$r=0;foreach($fc
as$jd=>$p){$C="'grants[$r][".h(strtoupper($Zd))."]'";$Y=$p[strtoupper($Zd)];if($Va=="Server Admin"&&$jd!=(isset($fc["*.*"])?"*.*":".*")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Uprawnienia'."<option value='0'".($Y=="0"?" selected":"").">".'Usuń uprawnienia'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$C value='1'".($Y?" checked":"").($Zd=="All privileges"?" id='grants-$r-all'":($Zd=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$r-all');\"")).">";}$r++;}}}echo"</table>\n",'<p>
<input type="submit" value="Zapisz zmiany">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$i){$Ac=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X))){$Ac++;}}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$Ac),$Ac||!$_POST["kill"]);}page_header('Lista procesów',$i);echo'
<form action="" method="post">
...
as$x=>$X){echo"<td>".(($w=="sql"?$x=="Info"&&$X!="":$x=="current_query"&&$X!="<IDLE>")?"<code class='jush-$w'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($J["db"]!=""?"db=".urlencode($J["db"])."&":"")."sql=".urlencode($X)).'">'.'Edytuj'.'</a>':nbsp($X));}echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($r+1)."/".sprintf('%d w sumie',$e->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Przerwij wykonywanie'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$bc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$Zb=$b->dumpFormat();if($Zb){print_fieldset("export",'Eksport');$Dd=$b->dumpOutput();echo($Dd?html_select("output",$Dd,$la["output"])." ":""),html_select("format",$Zb,$la["format"])," <input type='submit' name='export' value='".'Eksport'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Import',!$K);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($zb,'strlen'),$d);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ie=isset($_GET["status"]);page_header($Ie?'Status':'Zmienne');$Gf=($Ie?show_status():show_variables());if(!$Gf){echo"<p class='message'>".'Brak rekordów.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Gf
...
as$x=>$X){json_row("sum-$x",number_format($X,0,'.',' '));}json_row("");}else{foreach(count_tables($b->databases())as$h=>$X){json_row("tables-".js_escape($h),$X);}json_row("");}exit;}else{$af=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($af&&!$i&&!$_POST["search"]){$H=true;$Vc="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$H=truncate_tables($_POST["tables"]);}$Vc='Tabele zostały opróżnione.';}elseif($_POST["move"]){$H=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Vc='Tabele zostały przeniesione.';}elseif($_POST["copy"]){$H=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Vc='Tabele zostały skopiowane.';}elseif($_POST["drop"]){if($_POST["views"]){$H=drop_views($_POST["views"]);}if($H&&$_POST["tables"]){$H=drop_tables($_POST["tables"]);}$Vc='Tabele zostały usunięte.';}elseif($w!="sql"){$H=($w=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$Vc='Tables have been optimized.';}elseif($_POST["tables"]&&($H=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($J=$H->fetch_assoc()){$Vc.="<b>".h($J["Table"])."</b>: ".h($J["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$Vc,$H);}page_header(($_GET["ns"]==""?'Baza danych'.": ".h(DB):'Schemat'.": ".h($_GET["ns"])),$i,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabele i perspektywy'."</h3>\n";$Ze=tables_list();if(!$Ze){echo"<p class='message'>".'Brak tabel.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Wyszukaj we wszystkich tabelach'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Szukaj'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabela','<td>'.'Składowanie','<td>'.'Porównywanie znaków','<td>'.'Rozmiar danych','<td>'.'Rozmiar indeksów','<td>'.'Wolne miejsce','<td>'.'Auto Increment','<td>'.'Liczba rekordów',(support("comment")?'<td>'.'Komentarz':''),"</thead>\n";foreach($Ze
as$C=>$U){$Hf=(isset($U)&&!eregi("table",$U));echo'<tr'.odd().'><td>'.checkbox(($Hf?"views[]":"tables[]"),$C,in_array($C,$af,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($C).'" title="'.'Struktura tabeli'.'">'.h($C).'</a>';if($Hf){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'Zmień perspektywę'.'">'.'Perspektywa'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Pokaż dane'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmień tabelę'),"Index_length"=>array("indexes",'Zmień indeksy'),"Data_free"=>array("edit",'Nowy rekord'),"Auto_increment"=>array("auto_increment=1&create",'Zmień tabelę'),"Rows"=>array("select",'Pokaż dane'),)as$x=>$z){echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($C)."' id='$x-".h($C)."' title='$z[1]'>?</a>":"<td id='$x-".h($C)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d w sumie',count($Ze)),"<td>".nbsp($w=="sql"?$e->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x){echo"<td align='right' id='sum-$x'>&nbsp;";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$w)?($w!="sqlite"?"<input type='submit' value='".'Analizuj'."'> ":"")."<input type='submit' name='optimize' value='".'Optymalizuj'."'> ":"").($w=="sql"?"<input type='submit' name='check' value='".'Sprawdź'."'> <input type='submit' name='repair' value='".'Napraw'."'> ":"")."<input type='submit' name='truncate' value='".'Opróżnij'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Usuń'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$g=(support("scheme")?schemas():$b->databases());if(count($g)!=1&&$w!="sqlite"){$h=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Przenieś do innej bazy danych'.": ",($g?html_select("target",$g,$h):'<input name="target" value="'.h($h).'">')," <input type='submit' name='move' value='".'Przenieś'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopiuj'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$T'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Utwórz tabelę'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Utwórz perspektywę'."</a>\n";}if(support("routine")){echo"<h3>".'Procedury i funkcje'."</h3>\n";$ue=routines();if($ue){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Nazwa'.'<td>'.'Typ'.'<td>'.'Zwracany typ'."<td>&nbsp;</thead>\n";odd('');foreach($ue
as$J){echo'<tr'.odd().'>','<th><a href="'.h(ME).($J["ROUTINE_TYPE"]!="PROCEDURE"?'callf=':'call=').urlencode($J["ROUTINE_NAME"]).'">'.h($J["ROUTINE_NAME"]).'</a>','<td>'.h($J["ROUTINE_TYPE"]),'<td>'.h($J["DTD_IDENTIFIER"]),'<td><a href="'.h(ME).($J["ROUTINE_TYPE"]!="PROCEDURE"?'function=':'procedure=').urlencode($J["ROUTINE_NAME"]).'">'.'Zmień'."</a>";}echo"</table>\n";}echo'<p>'.(support("procedure")?'<a href="'.h(ME).'procedure=">'.'Utwórz procedurę'.'</a> ':'').'<a href="'.h(ME).'function=">'.'Utwórz funkcję'."</a>\n";}if(support("event")){echo"<h3>".'Wydarzenia'."</h3>\n";$K=get_rows("SHOW EVENTS");if($K){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Nazwa'."<td>".'Harmonogram'."<td>".'Początek'."<td>".'Koniec'."</thead>\n";foreach($K
as$J){echo"<tr>",'<th><a href="'.h(ME).'event='.urlencode($J["Name"]).'">'.h($J["Name"])."</a>","<td>".($J["Execute at"]?'O danym czasie'."<td>".$J["Execute at"]:'Wykonuj co'." ".$J["Interval value"]." ".$J["Interval field"]."<td>$J[Starts]"),"<td>$J[Ends]";}echo"</table>\n";}echo'<p><a href="'.h(ME).'event=">'.'Utwórz wydarzenie'."</a>\n";}if($Ze){echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=db');</script>\n";}}}}page_footer();