...
confirm($Wa="",$Ie=false){return" onclick=\"".($Ie?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($Wa?" (' + $Wa + ')":"")."');\"";}function
...
upload_error($i){$Rc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($i?'Wgranie pliku było niemożliwe.'.($Rc?" ".sprintf('Maksymalna wielkość pliku to %sB.',$Rc):""):'Plik nie istnieje.');}function
...
enum_input($U,$ta,$j,$Y,$zb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$Mc);$I=(isset($zb)?"<label><input type='$U'$ta value='$zb'".((is_array($Y)?in_array($zb,$Y):$Y===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($Mc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Fa=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$I.=" <label><input type='$U'$ta value='".($r+1)."'".($Fa?' checked':'').'>'.h($b->editVal($X,$j)).'</label>';}return$I;}function
input($j,$Y,$n){global$tf,$b,$w;$C=h(bracket_escape($j["field"]));echo"<td class='function'>";$me=($w=="mssql"&&$j["auto_increment"]);if($me&&!$_POST["save"]){$n=null;}$o=(isset($_GET["select"])||$me?array("orig"=>'bez zmian'):array())+$b->editFunctions($j);$ta=" name='fields[$C]'";if($j["type"]=="enum"){echo
...
<tr><th>Rodzaj bazy<td>',html_select("driver",$ob,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Re,$O=""){echo'<p class="tabs">';$Jc=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($Re)){$Jc["view"]='Zmień perspektywę';}else{$Jc["create"]='Zmień tabelę';}if(isset($O)){$Jc["edit"]='Nowy rekord';}foreach($Jc
...
selectColumnsPrint($M,$d){global$o,$fc;print_fieldset("select",'pokaż',$M);$r=0;$cc=array('Funkcje'=>$o,'Agregacje'=>$fc);foreach($M
...
false;}function
messageQuery($G){global$w;static$Wa=0;restart_session();$s="sql-".($Wa++);$hc=&get_session("queries");if(strlen($G)>1e6){$G=ereg_replace('[\x80-\xFF]+$','',substr($G,0,1e6))."\n...";}$hc[$_GET["db"]][]=$G;return" <a href='#$s' onclick=\"return !toggle('$s');\">".'Zapytanie SQL'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($G,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($hc[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
';if($Zc!="db"&&DB!=""&&$e->select_db(DB)){if($_GET["ns"]!==""&&!$Zc){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$We=tables_list();if(!$We){echo"<p class='message'>".'Brak tabel.'."\n";}else{$this->tablesPrint($We);$Jc=array();foreach($We
...
as$x=>$X){if(!isset($X)){$X="<i>NULL</i>";}elseif($_a[$x]&&!is_utf8($X)){$X="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($X))."</i>";}elseif(!strlen($X)){$X="&nbsp;";}else{$X=h($X);if($tf[$x]==254){$X="<code>$X</code>";}}if(isset($Jc[$x])&&!$d[$Jc[$x]]){if($kc){$z=$Jc[$x].urlencode($J[array_search("table=",$Jc)]);}else{$z="edit=".urlencode($Jc[$x]);foreach($u[$Jc[$x]]as$Ia=>$v){$z.="&where".urlencode("[".bracket_escape($Ia)."]")."=".urlencode($J[$v]);}}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Brak rekordów.')."\n";}function
...
edit_type($x,$j,$c,$m=array()){global$Ke,$tf,$_f,$nd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$j["type"]||isset($tf[$j["type"]])?array():array($j["type"]))+$Ke+($m?array('Klucze obce'=>$m):array()),$j["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($j["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$j["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($c,$j["collation"]).'</select>',($_f?"<select name='$x"."[unsigned]'".(!$j["type"]||ereg('(int|float|double|decimal)$',$j["type"])?"":" class='hidden'").'><option>'.optionlist($_f,$j["unsigned"]).'</select>':''),($m?"<select name='$x"."[on_delete]'".(ereg("`",$j["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist(explode("|",$nd),$j["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Qa?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($k))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($k),';</script>
</thead>
...
',(support("comment")?"<td".($Qa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($j["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $qa, 1);'>&nbsp;"."<input type='image' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.3' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.3' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($xd==""||support("drop_col")?"<input type='image' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.3' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$x=>$X){echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Zmień'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Dodaj wyzwalacz'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Te=array();$Ue=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Mc,PREG_SET_ORDER);foreach($Mc
...
<table cellspacing="0">
';$fb=array('','USE','DROP+CREATE','CREATE');$Ve=array('','DROP+CREATE','CREATE');$db=array('','TRUNCATE+INSERT','INSERT');if($w=="sql"){$fb[]='CREATE+ALTER';$Ve[]='CREATE+ALTER';$db[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$J);if(!$J){$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'Rezultat'."<td>".html_select("output",$b->dumpOutput(),$J["output"],0)."\n";echo"<tr><th>".'Format'."<td>".html_select("format",$b->dumpFormat(),$J["format"],0)."\n";echo($w=="sqlite"?"":"<tr><th>".'Baza danych'."<td>".html_select('db_style',$fb,$J["db_style"]).(support("routine")?checkbox("routines",1,$J["routines"],'Procedury i funkcje'):"").(support("event")?checkbox("events",1,$J["events"],'Wydarzenia'):"")),"<tr><th>".'Tabele'."<td>".html_select('table_style',$Ve,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$J["triggers"],'Wyzwalacze'):""),"<tr><th>".'Dane'."<td>".html_select('data_style',$db,$J["data_style"]),'</table>
<p><input type="submit" value="Eksport">

<table cellspacing="0">
';$Td=array();if(DB!=""){$Fa=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Fa onclick='formCheck(this, /^tables\\[/);'>".'Tabele'."</label>","<th style='text-align: right;'><label>".'Dane'."<input type='checkbox' id='check-data'$Fa onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Gf="";foreach(table_status()as$S){$C=$S["Name"];$Sd=ereg_replace("_.*","",$C);$Fa=($a==""||$a==(substr($a,-1)=="%"?"$Sd%":$C));$Vd="<tr><td>".checkbox("tables[]",$C,$Fa,$C,"formUncheck('check-tables');");if(is_view($S)){$Gf.="$Vd\n";}else{echo"$Vd<td align='right'><label>".($S["Engine"]=="InnoDB"&&$S["Rows"]?"~ ":"").$S["Rows"].checkbox("data[]",$C,$Fa,"","formUncheck('check-data');")."</label>\n";}$Td[$Sd]++;}echo$Gf;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Baza danych'."</label></thead>\n";$g=get_databases();if($g){foreach($g
...
as$C=>$j){if(!isset($j["privileges"][$Af?"update":"insert"])||$b->fieldName($j)==""){unset($k[$C]);}}if($_POST&&!$i&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"]){$_=($Af?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$_)){$_=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Rekord został usunięty.');}else{$O=array();foreach($k
as$C=>$j){$X=process_input($j);if($X!==false&&$X!==null){$O[idf_escape($C)]=($Af?"\n".idf_escape($C)." = $X":$X);}}if($Af){if(!$O){redirect($_);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Rekord został zaktualizowany.');}else{$H=insert_into($a,$O);$Cc=($H?last_id():0);queries_redirect($_,sprintf('Rekord%s został dodany.',($Cc?" $Cc":"")),$H);}}}$Se=$b->tableName(table_status($a));page_header(($Af?'Edytuj':'Dodaj'),$i,array("select"=>array($a,$Se)),$Se);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($Z){$M=array();foreach($k
...
';if($k){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Af?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}}echo($Af?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Hd=ereg('RANGE|LIST',$J["partition_by"]);print_fieldset("partition",'Partycjonowanie',$J["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Gd,$J["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($J["partition"]),'">)
Partycje: <input name="partitions" size="2" value="',h($J["partitions"]),'"',($Hd||!$J["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$i&&!isset($_POST["add_x"])){restart_session();if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Baza danych została usunięta.',drop_databases(array(DB)));}elseif(DB!==$_POST["name"]){if(DB!=""){$_GET["db"]=$_POST["name"];queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_POST["name"]),'Nazwa bazy danych została zmieniona.',rename_database($_POST["name"],$_POST["collation"]));}else{$g=explode("\n",str_replace("\r","",$_POST["name"]));$Le=true;$Bc="";foreach($g
as$h){if(count($g)==1||$h!=""){if(!create_database($h,$_POST["collation"])){$Le=false;}$Bc=$h;}}queries_redirect(ME."db=".urlencode($Bc),'Baza danych została utworzona.',$Le);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$i,array(),DB);$c=collations();$C=DB;$Ja=null;if($_POST){$C=$_POST["name"];$Ja=$_POST["collation"];}elseif(DB!=""){$Ja=db_collation(DB,$c);}elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$C=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+explode("|",$nd),$J["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+explode("|",$nd),$J["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$uc,$J["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$He,$J["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
...
as$j){if($j["field"]!=""){$O[]=(ereg("^($rc)\$",$j["inout"])?"$j[inout] ":"").idf_escape($j["field"]).process_type($j,"CHARACTER SET");}}$qb=drop_create("DROP $qe ".idf_escape($da),"CREATE $qe ".idf_escape($_POST["name"])." (".implode(", ",$O).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$re)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedura została usunięta.','Procedura została zmieniona.','Procedura została utworzona.',$da);}page_header(($da!=""?(isset($_GET["function"])?'Zmień funkcję':'Zmień procedurę').": ".h($da):(isset($_GET["function"])?'Utwórz funkcję':'Utwórz procedurę')),$i);$c=get_vals("SHOW CHARACTER SET");sort($c);$J=array("fields"=>array());if($_POST){$J=$_POST;$J["fields"]=(array)$J["fields"];process_fields($J["fields"]);}elseif($da!=""){$J=routine($da,$qe);$J["name"]=$da;}echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($J["name"]),'" maxlength="64">
',($re?'Język'.": ".html_select("language",$re,$J["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($J["fields"],$c,$qe);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$J["returns"],$c);}echo'</table>
<p>';textarea("definition",$J["definition"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($da!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($qb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$pf["Timing"],$J["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$of,$J["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$pf["Type"],$J["Type"]),'</table>
<p>Nazwa: <input name="Trigger" value="',h($J["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$J["Statement"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($qb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE"){$dc["$A[2]$X[2]"][$X[1]]=true;}if(ereg(' WITH GRANT OPTION',$J[0])){$dc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$J[0],$A)){$kd=$A[1];}}}if($_POST&&!$i){$ld=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$ed=q($_POST["user"])."@".q($_POST["host"]);$Kd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $ld",ME."privileges=",'Użytkownik został usunięty.');}else{$Za=false;if($ld!=$ed){$Za=queries(($e->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ed IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Kd");$i=!$Za;}elseif($_POST["pass"]!=$kd||!$_POST["hashed"]){queries("SET PASSWORD FOR $ed = ".($_POST["hashed"]?$Kd:"PASSWORD($Kd)"));}if(!$i){$ne=array();foreach($dd
...
as$hd=>$ne){if(preg_match('~^(.+)(\\(.*\\))?$~U',$hd,$A)){grant("REVOKE",array_keys($ne),$A[2]," ON $A[1] FROM $ed");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Użytkownik został zmieniony.':'Użytkownik został dodany.'),!$i);if($Za){$e->query("DROP USER $ed");}}}page_header((isset($_GET["host"])?'Użytkownik'.": ".h("$fa@$_GET[host]"):'Dodaj użytkownika'),$i,array("privileges"=>array('','Uprawnienia użytkowników')));if($_POST){$J=$_POST;$dc=$dd;}else{$J=$_GET+array("host"=>$e->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$kd;if($kd!=""){$J["hashed"]=true;}$dc[DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($e->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$r=0;foreach($dc
as$hd=>$p){echo'<th>'.($hd!="*.*"?"<input name='objects[$r]' value='".h($hd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$Ua=>$jb){foreach((array)$Yd[$Ua]as$Xd=>$Pa){echo"<tr".odd()."><td".($jb?">$jb<td":" colspan='2'").' lang="en" title="'.h($Pa).'">'.h($Xd);$r=0;foreach($dc
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$i){$zc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X))){$zc++;}}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$zc),$zc||!$_POST["kill"]);}page_header('Lista procesów',$i);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$Zb,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Eksport');$Bd=$b->dumpOutput();echo($Bd?html_select("output",$Bd,$la["output"])." ":""),html_select("format",$b->dumpFormat(),$la["format"])," <input type='submit' name='export' value='".'Eksport'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($b->selectImportPrint()){print_fieldset("import",'Import',!$K);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$la["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($yb,'strlen'),$d);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ge=isset($_GET["status"]);page_header($Ge?'Status':'Zmienne');$Ef=($Ge?show_status():show_variables());if(!$Ef){echo"<p class='message'>".'Brak rekordów.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Ef
