...
confirm($Ec="",$kf=false){return" onclick=\"".($kf?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($Ec?" (' + $Ec + ')":"")."');\"";}function
js_escape($T){return
addcslashes($T,"\r\n'\\/");}function
...
upload_error($q){$le=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Wgranie pliku było niemożliwe.'.($le?" ".sprintf('Maksymalna wielkość pliku to %sB.',$le):""):'Plik nie istnieje.');}function
...
enum_input($N,$Ca,$d,$o,$Bb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$fa);$e=(isset($Bb)?"<label><input type='$N'$Ca value='$Bb'".((is_array($o)?in_array($Bb,$o):$o===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($fa[1]as$h=>$b){$b=stripcslashes(str_replace("''","'",$b));$Ja=(is_int($o)?$o==$h+1:(is_array($o)?in_array($h+1,$o):$o===$b));$e.=" <label><input type='$N'$Ca value='".($h+1)."'".($Ja?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
input($d,$o,$_){global$ba,$n,$r;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Ad=($r=="mssql"&&$d["auto_increment"]);if($Ad&&!$_POST["save"]){$_=null;}$W=(isset($_GET["select"])||$Ad?array("orig"=>'bez zmian'):array())+$n->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($tf,$wb){$cc=($wb==1||(!$wb&&'pl'=='fr')?0:('pl'=='sl'&&(!$wb||$wb>2)?1:0)+((!$wb||$wb>=5)&&ereg('cs|sk|ru|sl|pl','pl')?2:1));return
...
<tr><th>Rodzaj bazy<td>',html_select("driver",$ra,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="server" value="',h(SERVER),'">
...
selectLinks($Jc,$u=""){echo'<p class="tabs">';$za=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($Jc)){$za["view"]='Zmień perspektywę';}else{$za["create"]='Zmień tabelę';}if(isset($u)){$za["edit"]='Nowy rekord';}foreach($za
...
selectVal($b,$y,$d){$e=($b!="<i>NULL</i>"&&ereg("char|binary",$d["type"])&&!ereg("var",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$e=lang(array('%d bajt','%d bajty','%d bajtów'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($y?"<a href='$y'>$e</a>":$e);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($C,$w){global$W,$zb;print_fieldset("select",'pokaż',$C);$h=0;$Vd=array('Funkcje'=>$W,'Agregacje'=>$zb);foreach($C
...
as$h=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$h]' value='".h($_GET["fulltext"][$h])."'>",checkbox("boolean[$h]",1,isset($_GET["boolean"][$h]),"BOOL"),"<br>\n";}}$h=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$h][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($w,$b["col"],true)."</select>",html_select("where[$h][op]",$this->operators,$b["op"]),"<input name='where[$h][val]' value='".h($b["val"])."'></div>\n";$h++;}}echo"<div><select name='where[$h][col]' onchange='selectAddRow(this);'><option value=''>(".'gdziekolwiek'.")".optionlist($w,null,true)."</select>",html_select("where[$h][op]",$this->operators,"="),"<input name='where[$h][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Fa,$w,$z){print_fieldset("sort",'Sortuj',$Fa);$h=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$h]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$h]",1,isset($_GET["desc"][$c]),'malejąco')."</div>\n";$h++;}}echo"<div><select name='order[$h]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>","<label><input type='checkbox' name='desc[$h]' value='1'>".'malejąco'."</label></div>\n";echo"</div></fieldset>\n";}function
...
false;}function
messageQuery($j){global$r;static$Ec=0;restart_session();$D="sql-".($Ec++);$Oa=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Oa[$_GET["db"]][]=$j;return" <a href='#$D' onclick=\"return !toggle('$D');\">".'Zapytanie SQL'."</a><div id='$D' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Oa[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
';if($Ib!="db"&&DB!=""&&$g->select_db(DB)){if($_GET["ns"]!==""&&!$Ib){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$la=tables_list();if(!$la){echo"<p class='message'>".'Brak tabel.'."\n";}else{$this->tablesPrint($la);$za=array();foreach($la
...
auth_error($wd=null){global$g,$n,$B;$Cc=session_name();$q="";if(!$_COOKIE[$Cc]&&$_GET[$Cc]&&ini_bool("session.use_only_cookies")){$q='Wymagana jest obsługa sesji w PHP.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Cc]||$_GET[$Cc])&&!$B){$q='Sesja wygasła, zaloguj się ponownie.';}else{$ja=&get_session("pwds");if(isset($ja)){$q=h($wd?$wd->getMessage():(is_string($g)?$g:'Nieprawidłowe dane logowania.'));$ja=null;}}}page_header('Zaloguj się',$q,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$n->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Brak rozszerzenia',sprintf('Żadne z rozszerzeń PHP umożliwiających połączenie się z bazą danych (%s) nie jest dostępne.',implode(", ",$df)),false);page_footer("auth");exit;}$g=connect();}if(is_string($g)||!$n->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$B=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$B;}$q=($_POST?($_POST["token"]==$B?"":'Nieprawidłowy token CSRF. Spróbuj wysłać formularz ponownie.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Przesłano zbyt dużo danych. Zmniejsz objętość danych lub zwiększ zmienną konfiguracyjną %s.','"post_max_size"')));function
connect_error(){global$g,$B,$q,$ra;$A=array();if(DB!=""){page_header('Baza danych'.": ".h(DB),'Nie znaleziono bazy danych.',true);}else{if($_POST["db"]&&!$q){queries_redirect(substr(ME,0,-1),'Bazy danych zostały usunięte.',drop_databases($_POST["db"]));}page_header('Wybierz bazę danych',$q,false);echo"<p><a href='".h(ME)."database='>".'Utwórz nową bazę danych'."</a>\n";foreach(array('privileges'=>'Uprawnienia użytkowników','processlist'=>'Lista procesów','variables'=>'Zmienne','status'=>'Status',)as$c=>$b){if(support($c)){echo"<a href='".h(ME)."$c='>$b</a>\n";}}echo"<p>".sprintf('Wersja %s: %s za pomocą %s',$ra[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Zalogowany jako: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"]){set_session("dbs",null);}$A=get_databases();if($A){$gf=support("scheme");$S=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td>&nbsp;<th>".'Baza danych'."<td>".'Porównywanie znaków'."<td>".'Tabele'."</thead>\n";foreach($A
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($Ke[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ba[$c]==254){$b="<code>$b</code>";}}if(isset($za[$c])&&!$w[$za[$c]]){$y="edit=".urlencode($za[$c]);foreach($z[$za[$c]]as$sc=>$aa){$y.="&where".urlencode("[".bracket_escape($sc)."]")."=".urlencode($a[$aa]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($h?"</table>":"<p class='message'>".'Brak rekordów.')."\n";}function
...
edit_type($c,$d,$S,$P=array()){global$Gc,$ba,$Db,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ba[$d["type"]])?array():array($d["type"]))+$Gc+($P?array('Klucze obce'=>$P):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($S,$d["collation"]).'</select>',($Db?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Db,$d["unsigned"]).'</select>':''),($P?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist($Ma,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$h][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.2' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $pe, 1);'>&nbsp;"."<input type='image' name='up[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.2' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.2' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($Tc==""||support("drop_col")?"<input type='image' name='drop_col[$h]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.2' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$ob;}function
...
<p><a href="',h(ME."schema=".urlencode($Ae)),'" id="schema-link">Trwały link</a>
...
<input type="hidden" name="token" value="',$B,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zatrzymaj w przypadku błędu')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Pokaż tylko błędy')."\n";print_fieldset("webfile",'Z serwera',$_POST["webfile"]);$Uc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Uc[]=".$c";}}echo
sprintf('Plik %s na serwerze',"<code>adminer.sql".($Uc?"[".implode("|",$Uc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Uruchom z pliku'.'">',"</div></fieldset>\n";if($Oa){print_fieldset("history",'Historia',$_GET["history"]!="");foreach($Oa
...
',(support("comment")?checkbox("","",$ob,'Komentarz',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
...
as$v){if(count($A)==1||$v!=""){if(!create_database($v,$_POST["collation"])){$ae=false;}$bb=$v;}}queries_redirect(ME."db=".urlencode($bb),'Baza danych została utworzona.',$ae);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$q,array(),DB);$S=collations();$f=DB;$Sb=null;if($_POST){$f=$_POST["name"];$Sb=$_POST["collation"];}elseif(DB!=""){$Sb=db_collation(DB,$S);}elseif($r=="sql"){foreach(get_vals("SHOW GRANTS")as$Q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Q,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($S?html_select("collation",array(""=>"(".'porównywanie znaków'.")")+$S,$Sb):"");?>
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+$Ma,$a["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+$Ma,$a["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
<noscript><p><input type="submit" name="add" value="Dodaj kolumnę"></noscript>
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["view"])){$i=$_GET["view"];$Ka=false;if($_POST&&!$q){$Ka=drop_create("DROP VIEW ".table($i),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'Perspektywa została usunięta.','Perspektywa została zmieniona.','Perspektywa została utworzona.',$i);}page_header(($i!=""?'Zmień perspektywę':'Utwórz perspektywę'),$q,array("table"=>$i),$i);$a=array();if($_POST){$a=$_POST;}elseif($i!=""){$a=view($i);$a["name"]=$i;}echo'
<form action="" method="post">
...
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["event"])){$Ua=$_GET["event"];$Md=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$jd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ua),substr(ME,0,-1),'Wydarzenie zostało usunięte.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Md)&&isset($jd[$_POST["STATUS"]])){$Od="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ua!=""?'Wydarzenie zostało zmienione.':'Wydarzenie zostało utworzone.'),queries(($Ua!=""?"ALTER EVENT ".idf_escape($Ua).$Od.($Ua!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Od)."\n".$jd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ua!=""?'Zmień wydarzenie'.": ".h($Ua):'Utwórz wydarzenie'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ua!=""){$E=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ua));$a=reset($E);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Md,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$jd,$a["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Zapisz zmiany">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
as$d){if($d["field"]!=""){$u[]=(in_array($d["inout"],$Eb)?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Ka=drop_create("DROP $Da ".idf_escape($Ea),"CREATE $Da ".idf_escape($_POST["name"])." (".implode(", ",$u).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedura została usunięta.','Procedura została zmieniona.','Procedura została utworzona.',$Ea);}page_header(($Ea!=""?(isset($_GET["function"])?'Zmień funkcję':'Zmień procedurę').": ".h($Ea):(isset($_GET["function"])?'Utwórz funkcję':'Utwórz procedurę')),$q);$S=get_vals("SHOW CHARACTER SET");sort($S);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Ea!=""){$a=routine($Ea,$Da);$a["name"]=$Ea;}echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$S,$Da);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$a["returns"],$S);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($Ea!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($Ka){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["type"])){$zc=$_GET["type"];if($_POST&&!$q){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($zc),$y,'Typ został usunięty.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ został utworzony.');}}page_header($zc!=""?'Zmień typ'.": ".h($zc):'Utwórz typ',$q);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($zc!=""){echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Zapisz zmiany'."'>\n";}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["trigger"])){$i=$_GET["trigger"];$yc=trigger_options();$Sd=array("INSERT","UPDATE","DELETE");$Ka=false;if($_POST&&!$q&&in_array($_POST["Timing"],$yc["Timing"])&&in_array($_POST["Event"],$Sd)&&in_array($_POST["Type"],$yc["Type"])){$Rd=" $_POST[Timing] $_POST[Event]";$mb=" ON ".table($i);$Ka=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($r=="pgsql"?$mb:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($r=="mssql"?$mb.$Rd:$Rd.$mb).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($i),'Wyzwalacz został usunięty.','Wyzwalacz został zmieniony.','Wyzwalacz został utworzony.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmień wyzwalacz'.": ".h($_GET["name"]):'Utwórz wyzwalacz'),$q,array("table"=>$i));$a=array("Trigger"=>$i."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$yc["Timing"],$a["Timing"],"if (/^".h(preg_quote($i,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($i))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$Sd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$yc["Type"],$a["Type"]),'</table>
<p>Nazwa: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($Ka){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$h=0;foreach($Ta
as$Qa=>$Q){echo'<th>'.($Qa!="*.*"?"<input name='objects[$h]' value='".h($Qa)."' size='10'>":"<input type='hidden' name='objects[$h]' value='*.*' size='10'>*.*");$h++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$Bc=>$Pb){foreach((array)$Y[$Bc]as$Ub=>$cb){echo"<tr".odd()."><td".($Pb?">$Pb<td":" colspan='2'").' lang="en" title="'.h($cb).'">'.h($Ub);$h=0;foreach($Ta
...
<input type="submit" value="Zapisz zmiany">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$B,'">
</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$ld=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$ld++;}}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$ld),$ld||!$_POST["kill"]);}page_header('Lista procesów',$q);echo'
<form action="" method="post">
...
<p>',($h+1)."/".sprintf('%d w sumie',$g->result("SELECT @@max_connections")),'<p>
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$Ha,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Eksport');$Aa=$n->dumpOutput();echo($Aa?html_select("output",$Aa,$Hb["output"])." ":""),html_select("format",$n->dumpFormat(),$Hb["format"])," <input type='submit' name='export' value='".'Eksport'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}print_fieldset("import",'Import',!$E);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Hb["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$B'>\n","</div></fieldset>\n";$n->selectEmailPrint(array_filter($mc,'strlen'),$w);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ab=isset($_GET["status"]);page_header($Ab?'Status':'Zmienne');$he=($Ab?show_status():show_variables());if(!$he){echo"<p class='message'>".'Brak rekordów.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($he
