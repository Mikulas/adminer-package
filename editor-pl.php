...
h($g->error);}function
check_sqlite_name($D){global$g;$Vb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Vb)\$~",$D)){$g->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$Vb));return
false;}return
true;}function
...
false;$g->Min_SQLite(":memory:");$g->error='Plik juÅ¼ istnieje.';return@rename(DB,$D);}function
...
set_schema($ye){global$g,$vf,$Qe;$K=$g->query("SET search_path TO ".idf_escape($ye));foreach(types()as$U){if(!isset($vf[$U])){$vf[$U]=0;$Qe['Typy uÅ¼ytkownika'][]=$U;}}return$K;}function
...
as$e=>$X)$A.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($L[$X]);echo"<a href='".h($A)."' title='".'Nowy rekord'."'>+</a> ";}}}function
...
selectSearchPrint($Z,$f,$x){$Z=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Szukaj'."</legend><div>\n";$Tc=array();foreach($Z
as$z=>$X)$Tc[$X["col"]]=$z;$t=0;$p=fields($_GET["select"]);foreach($f
as$D=>$pb){$o=$p[$D];if(preg_match("~enum~",$o["type"])||like_bool($o)){$z=$Tc[$D];$t--;echo"<div>".h($pb)."<input type='hidden' name='where[$t][col]' value='".h($D)."'>:",(like_bool($o)?" <select name='where[$t][val]'>".optionlist(array(""=>"",'nie','tak'),$Z[$z]["val"],true)."</select>":enum_input("checkbox"," name='where[$t][val][]'",$o,(array)$Z[$z]["val"],($o["null"]?0:null))),"</div>\n";unset($f[$D]);}elseif(is_array($Hd=$this->_foreignKeyOptions($_GET["select"],$D))){if($p[$D]["null"])$Hd[0]='('.'puste'.')';$z=$Tc[$D];$t--;echo"<div>".h($pb)."<input type='hidden' name='where[$t][col]' value='".h($D)."'><input type='hidden' name='where[$t][op]' value='='>: <select name='where[$t][val]'>".optionlist($Hd,$Z[$z]["val"],true)."</select></div>\n";unset($f[$D]);}}$t=0;foreach($Z
as$X){if(($X["col"]==""||$f[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$t][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($f,$X["col"],true)."</select>",html_select("where[$t][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$t][val]' value='".h($X["val"])."' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";$t++;}}echo"<div><select name='where[$t][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'gdziekolwiek'.")".optionlist($f,null,true)."</select>",html_select("where[$t][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$t][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($E,$f,$x){$Jd=array();foreach($x
as$z=>$w){$E=array();foreach($w["columns"]as$X)$E[]=$f[$X];if(count(array_filter($E,'strlen'))>1&&$z!="PRIMARY")$Jd[$z]=implode(", ",$E);}if($Jd){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Jd,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
editInput($R,$o,$Aa,$Y){if($o["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Aa value='-1' checked><i>".'bez zmian'."</i></label> ":"").enum_input("radio",$Aa,$o,($Y||isset($_GET["select"])?$Y:0),($o["null"]?"":null));$Hd=$this->_foreignKeyOptions($R,$o["field"],$Y);if($Hd!==null)return(is_array($Hd)?"<select$Aa>".optionlist($Hd,$Y,true)."</select>":"<input value='".h($Y)."'$Aa class='hidden'><input value='".h($Hd)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($R)."&field=".urlencode($o["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");if(like_bool($o))return'<input type="checkbox" value="'.h($Y?$Y:1).'"'.($Y?' checked':'')."$Aa>";$yc="";if(preg_match('~time~',$o["type"]))$yc='HH:MM:SS';if(preg_match('~date|timestamp~',$o["type"]))$yc='d.m.[rrrr]'.($yc?" [$yc]":"");if($yc)return"<input value='".h($Y)."'$Aa> ($yc)";if(preg_match('~_(md5|sha1)$~i',$o["field"]))return"<input type='password' value='".h($Y)."'$Aa>";return'';}function
...
as$D=>$o){if(!isset($o["privileges"][$Cf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$dd=$_POST["referer"];if($_POST["insert"])$dd=($Cf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$dd))$dd=ME."select=".urlencode($a);$x=indexes($a);$yf=unique_array($_GET["where"],$x);$ie="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($dd,'Rekord zostaÅ‚ usuniÄ™ty.',$m->delete($a,$ie,!$yf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Cf){if(!$P)redirect($dd);queries_redirect($dd,'Rekord zostaÅ‚ zaktualizowany.',$m->update($a,$P,$ie,!$yf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$m->insert($a,$P);$Yc=($J?last_id():0);queries_redirect($dd,sprintf('Rekord%s zostaÅ‚ dodany.',($Yc?" $Yc":"")),$J);}}}$Xe=$b->tableName(table_status1($a,true));page_header(($Cf?'Edytuj':'Dodaj'),$n,array("select"=>array($a,$Xe)),$Xe);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$z=>$X){if(!$Z)$L[$z]=null;$p[$z]=array("field"=>$z,"null"=>($z!=$m->primary),"auto_increment"=>($z==$m->primary));}}}if($L===false)echo"<p class='error'>".'Brak rekordÃ³w.'."\n";echo'
<div id="message"></div>
