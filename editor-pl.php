...
upload_error($n){$ld=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($n?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($ld?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$ld):""):'Plik nie istnieje.');}function
...
lang($of,$yd=null){if(is_array($of)){$Wd=($yd==1?0:($yd%10>1&&$yd%10<5&&$yd/10%10!=1?1:2));$of=$of[$Wd];}$of=str_replace("%d","%s",$of);$yd=number_format($yd,0,".",' ');return
...
h($h->error);}function
check_sqlite_name($D){global$h;$Vb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Vb)\$~",$D)){$h->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$Vb));return
false;}return
true;}function
...
false;$h->Min_SQLite(":memory:");$h->error='Plik juÅ¼ istnieje.';return@rename(DB,$D);}function
...
set_schema($xe){global$h,$uf,$Pe;$K=$h->query("SET search_path TO ".idf_escape($xe));foreach(types()as$U){if(!isset($uf[$U])){$uf[$U]=0;$Pe['Typy uÅ¼ytkownika'][]=$U;}}return$K;}function
...
selectLinks($Ve,$P=""){$a=$Ve["Name"];if($P!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$P).'">'.'Nowy rekord'."</a>\n";}function
...
as$f=>$X)$A.="&set".urlencode("[".bracket_escape($f)."]")."=".urlencode($L[$X]);echo"<a href='".h($A)."' title='".'Nowy rekord'."'>+</a> ";}}}function
...
selectVal($X,$A,$o){$K=($X===null?"&nbsp;":$X);$A=h($A);if(preg_match('~blob|bytea~',$o["type"])&&!is_utf8($X)){$K=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($X));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$X))$K="<img src='$A' alt='$K'>";}if(like_bool($o)&&$K!="&nbsp;")$K=($X?'tak':'nie');if($A)$K="<a href='$A'>$K</a>";if(!$A&&!like_bool($o)&&preg_match('~int|float|double|decimal~',$o["type"]))$K="<div class='number'>$K</div>";elseif(preg_match('~date~',$o["type"]))$K="<div class='datetime'>$K</div>";return$K;}function
...
selectSearchPrint($Z,$g,$x){$Z=(array)$_GET["where"];echo'<fieldset id="fieldset-search"><legend>'.'Szukaj'."</legend><div>\n";$Tc=array();foreach($Z
as$z=>$X)$Tc[$X["col"]]=$z;$t=0;$p=fields($_GET["select"]);foreach($g
as$D=>$ob){$o=$p[$D];if(preg_match("~enum~",$o["type"])||like_bool($o)){$z=$Tc[$D];$t--;echo"<div>".h($ob)."<input type='hidden' name='where[$t][col]' value='".h($D)."'>:",(like_bool($o)?" <select name='where[$t][val]'>".optionlist(array(""=>"",'nie','tak'),$Z[$z]["val"],true)."</select>":enum_input("checkbox"," name='where[$t][val][]'",$o,(array)$Z[$z]["val"],($o["null"]?0:null))),"</div>\n";unset($g[$D]);}elseif(is_array($Hd=$this->_foreignKeyOptions($_GET["select"],$D))){if($p[$D]["null"])$Hd[0]='('.'puste'.')';$z=$Tc[$D];$t--;echo"<div>".h($ob)."<input type='hidden' name='where[$t][col]' value='".h($D)."'><input type='hidden' name='where[$t][op]' value='='>: <select name='where[$t][val]'>".optionlist($Hd,$Z[$z]["val"],true)."</select></div>\n";unset($g[$D]);}}$t=0;foreach($Z
as$X){if(($X["col"]==""||$g[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$t][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($g,$X["col"],true)."</select>",html_select("where[$t][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$t][val]' value='".h($X["val"])."' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";$t++;}}echo"<div><select name='where[$t][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'gdziekolwiek'.")".optionlist($g,null,true)."</select>",html_select("where[$t][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$t][val]' onchange='selectAddRow(this);' onsearch='selectSearch(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($E,$g,$x){$Jd=array();foreach($x
as$z=>$w){$E=array();foreach($w["columns"]as$X)$E[]=$g[$X];if(count(array_filter($E,'strlen'))>1&&$z!="PRIMARY")$Jd[$z]=implode(", ",$E);}if($Jd){echo'<fieldset><legend>'.'Sortuj'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Jd,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
selectActionPrint($x){echo"<fieldset><legend>".'CzynnoÅ›Ä‡'."</legend><div>","<input type='submit' value='".'pokaÅ¼'."'>","</div></fieldset>\n";}function
selectCommandPrint(){return
true;}function
selectImportPrint(){return
true;}function
selectEmailPrint($Fb,$g){if($Fb){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div onkeydown=\"eventStop(event); return bodyKeydown(event, 'email');\">\n","<p>".'Nadawca'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Temat'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p onkeydown=\"eventStop(event); return bodyKeydown(event, 'email_append');\">".html_select("email_addition",$g,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Dodaj'."'>\n";echo"<p>".'ZaÅ‚Ä…czniki'.": <input type='file' name='email_files[]' onchange=\"this.onchange = function () { }; var el = this.cloneNode(true); el.value = ''; this.parentNode.appendChild(el);\">","<p>".(count($Fb)==1?'<input type="hidden" name="email_field" value="'.h(key($Fb)).'">':html_select("email_field",$Fb)),"<input type='submit' name='email' value='".'WyÅ›lij'."' onclick=\"return this.form['delete'].onclick();\">\n","</div>\n","</div></fieldset>\n";}}function
...
';if($sd=="auth"){$dc=true;foreach((array)$_SESSION["pwds"]as$Hf=>$Fe){foreach($Fe[""]as$V=>$H){if($H!==null){if($dc){echo"<p id='logins' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";$dc=false;}echo"<a href='".h(auth_url($Hf,"",$V))."'>".($V!=""?h($V):"<i>".'puste'."</i>")."</a><br>\n";}}}}else{$this->databasesPrint($sd);if($sd!="db"&&$sd!="ns"){$S=table_status('',true);if(!$S)echo"<p class='message'>".'Brak tabel.'."\n";else$this->tablesPrint($S);}}}function
databasesPrint($sd){}function
tablesPrint($T){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($T
as$L){$D=$this->tableName($L);if(isset($L["Engine"])&&$D!="")echo"<a href='".h(ME).'select='.urlencode($L["Name"])."'".bold($_GET["select"]==$L["Name"]||$_GET["edit"]==$L["Name"])." title='".'PokaÅ¼ dane'."'>$D</a><br>\n";}}function
...
auth_error($Qb=null){global$h,$b,$uc;$Ge=session_name();$n="";if(!$_COOKIE[$Ge]&&$_GET[$Ge]&&ini_bool("session.use_only_cookies"))$n='Wymagana jest obsÅ‚uga sesji w PHP.';elseif(isset($_GET["username"])){if(($_COOKIE[$Ge]||$_GET[$Ge])&&!$uc)$n='Sesja wygasÅ‚a, zaloguj siÄ™ ponownie.';else{$H=get_password();if($H!==null){$n=h($Qb?$Qb->getMessage():(is_string($h)?$h:'NieprawidÅ‚owe dane logowania.'));if($H===false)$n.='<br>'.sprintf('WaÅ¼noÅ›Ä‡ hasÅ‚a gÅ‚Ã³wnego wygasÅ‚a. <a href="http://www.adminer.org/pl/extension/" target="_blank">Zaimplementuj</a> wÅ‚asnÄ… metodÄ™ %s, aby ustawiÄ‡ je na staÅ‚e.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$G=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$G["lifetime"]);page_header('Zaloguj siÄ™',$n,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
set_password($Hf,$O,$V,$H){$_SESSION["pwds"][$Hf][$O][$V]=($_COOKIE["adminer_key"]?array(encrypt_string($H,$_COOKIE["adminer_key"])):$H);}function
get_password(){$K=get_session("pwds");if(is_array($K))$K=($_COOKIE["adminer_key"]?decrypt_string($K[0],$_COOKIE["adminer_key"]):false);return$K;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Brak rozszerzenia',sprintf('Å»adne z rozszerzeÅ„ PHP umoÅ¼liwiajÄ…cych poÅ‚Ä…czenie siÄ™ z bazÄ… danych (%s) nie jest dostÄ™pne.',implode(", ",$Xd)),false);page_footer("auth");exit;}$h=connect();}if(is_string($h)||!$b->login($_GET["username"],get_password())){auth_error();exit;}$wb=new
Min_Driver($h);if($Aa&&$_POST["token"])$_POST["token"]=$mf;$n='';if($_POST){if(!verify_token()){$Jc="max_input_vars";$md=ini_get($Jc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$z){$X=ini_get($z);if($X&&(!$md||$X<$md)){$Jc=$z;$md=$X;}}}$n=(!$_POST["token"]&&$md?sprintf('Przekroczono maksymalnÄ… liczbÄ™ pÃ³l. ZwiÄ™ksz %s.',"'$Jc'"):'NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=sprintf('PrzesÅ‚ano zbyt duÅ¼o danych. Zmniejsz objÄ™toÅ›Ä‡ danych lub zwiÄ™ksz zmiennÄ… konfiguracyjnÄ… %s.',"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.'WiÄ™ksze pliki SQL moÅ¼esz wgraÄ‡ na serwer poprzez FTP przed zaimportowaniem.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();$h->select_db($b->database());function
...
like_bool($o){return
preg_match("~bool|(tinyint|bit)\\(1\\)~",$o["full_type"]);}$Ad="RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";$xb[DRIVER]='Zaloguj siÄ™';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"])$_GET["edit"]=$_GET["select"];if(isset($_GET["download"])){$a=$_GET["download"];$p=fields($a);header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$a-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$h->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($a)," WHERE ".where($_GET,$p),1));exit;}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$p=fields($a);$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0],$p):""):where($_GET,$p));$Bf=(isset($_GET["select"])?$_POST["edit"]:$Z);foreach($p
as$D=>$o){if(!isset($o["privileges"][$Bf?"update":"insert"])||$b->fieldName($o)=="")unset($p[$D]);}if($_POST&&!$n&&!isset($_GET["select"])){$dd=$_POST["referer"];if($_POST["insert"])$dd=($Bf?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$dd))$dd=ME."select=".urlencode($a);$x=indexes($a);$xf=unique_array($_GET["where"],$x);$he="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($dd,'Rekord zostaÅ‚ usuniÄ™ty.',$wb->delete($a,$he,!$xf));else{$P=array();foreach($p
as$D=>$o){$X=process_input($o);if($X!==false&&$X!==null)$P[idf_escape($D)]=$X;}if($Bf){if(!$P)redirect($dd);queries_redirect($dd,'Rekord zostaÅ‚ zaktualizowany.',$wb->update($a,$P,$he,!$xf));if(is_ajax()){page_headers();page_messages($n);exit;}}else{$J=$wb->insert($a,$P);$Yc=($J?last_id():0);queries_redirect($dd,sprintf('Rekord%s zostaÅ‚ dodany.',($Yc?" $Yc":"")),$J);}}}$We=$b->tableName(table_status1($a,true));page_header(($Bf?'Edytuj':'Dodaj'),$n,array("select"=>array($a,$We)),$We);$L=null;if($_POST["save"])$L=(array)$_POST["fields"];elseif($Z){$N=array();foreach($p
...
as$z=>$X){if(!$Z)$L[$z]=null;$p[$z]=array("field"=>$z,"null"=>($z!="itemName()"),"auto_increment"=>($z=="itemName()"));}}}if($L===false)echo"<p class='error'>".'Brak rekordÃ³w.'."\n";echo'
<div id="message"></div>

<form action="" method="post" enctype="multipart/form-data" id="form">
';if(!$p)echo"<p class='error'>".'Brak uprawnieÅ„ do edycji tej tabeli'."\n";else{echo"<table cellspacing='0' onkeydown='return editingKeydown(event);'>\n";foreach($p
...
';if($p){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Bf?'Zapisz i kontynuuj edycjÄ™'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'Zapisz i dodaj nastÄ™pny')."' title='Ctrl+Shift+Enter'>\n";}echo($Bf?"<input type='submit' name='delete' value='".'UsuÅ„'."'".confirm().">\n":($_POST||!$p?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
as$D=>$X){$X=process_input($p[$D]);if($X!==null&&($_POST["clone"]||$X!==false))$P[idf_escape($D)]=($X!==false?$X:idf_escape($D));}}if($_POST["delete"]||$P){if($_POST["clone"])$I="INTO ".table($a)." (".implode(", ",array_keys($P)).")\nSELECT ".implode(", ",$P)."\nFROM ".table($a);if($_POST["all"]||($_f===array()&&is_array($_POST["check"]))||$Nc){$J=($_POST["delete"]?$wb->delete($a,$Of):($_POST["clone"]?queries("INSERT $I$Of"):$wb->update($a,$P,$Of)));$ra=$h->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Nf="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$p);$J=($_POST["delete"]?$wb->delete($a,$Nf,1):($_POST["clone"]?queries("INSERT".limit1($I,$Nf)):$wb->update($a,$P,$Nf)));if(!$J)break;$ra+=$h->affected_rows;}}}$od=lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordÃ³w.'),$ra);if($_POST["clone"]&&$J&&$ra==1){$Yc=last_id();if($Yc)$od=sprintf('Rekord%s zostaÅ‚ dodany.'," $Yc");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$od,$J);}elseif(!$_POST["import"]){if(!$_POST["val"])$n='Ctrl+kliknij wartoÅ›Ä‡, aby jÄ… edytowaÄ‡.';else{$J=true;$ra=0;foreach($_POST["val"]as$yf=>$L){$P=array();foreach($L
as$z=>$X){$z=bracket_escape($z,1);$P[idf_escape($z)]=(preg_match('~char|text~',$p[$z]["type"])||$X!=""?$b->processInput($p[$z],$X):"NULL");}$J=$wb->update($a,$P," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($yf,$p),!($Nc||$_f===array())," ");if(!$J)break;$ra+=$h->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Zmieniono %d rekord.','Zmieniono %d rekordy.','Zmieniono %d rekordÃ³w.'),$ra),$J);}}elseif(!is_string($Zb=get_file("csv_file",true)))$n=upload_error($Zb);elseif(!preg_match('~~u',$Zb))$n='Kodowanie pliku musi byÄ‡ ustawione na UTF-8.';else{cookie("adminer_import","output=".urlencode($qa["output"])."&format=".urlencode($_POST["separator"]));$J=true;$Xa=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$Zb,$id);$ra=count($id[0]);$wb->begin();$De=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$M=array();foreach($id[0]as$z=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$De]*)$De~",$X.$De,$jd);if(!$z&&!array_diff($jd[1],$Xa)){$Xa=$jd[1];$ra--;}else{$P=array();foreach($jd[1]as$t=>$Va)$P[idf_escape($Xa[$t])]=($Va==""&&$p[$Xa[$t]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Va))));$M[]=$P;}}$J=(!$M||$wb->insertUpdate($a,$M,$Zd));if($J)$wb->commit();queries_redirect(remove_from_uri("page"),lang(array('%d rekord zostaÅ‚ zaimportowany.','%d rekordy zostaÅ‚y zaimportowane.','%d rekordÃ³w zostaÅ‚o zaimportowanych.'),$ra),$J);$wb->rollback();}}}$We=$b->tableName($S);if(is_ajax()){page_headers();ob_start();}else
page_header('pokaÅ¼'.": $We",$n);$P=null;if(isset($ue["insert"])||!support("table")){$P="";foreach((array)$_GET["where"]as$X){if(count($ic[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$P.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($S,$P);if(!$g&&support("table"))echo"<p class='error'>".'Nie udaÅ‚o siÄ™ pobraÄ‡ danych z tabeli'.($p?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($N,$g);$b->selectSearchPrint($Z,$g,$x);$b->selectOrderPrint($E,$g,$x);$b->selectLimitPrint($_);$b->selectLengthPrint($cf);$b->selectActionPrint($x);echo"</form>\n";$F=$_GET["page"];if($F=="last"){$lc=$h->result(count_rows($a,$Z,$Nc,$s));$F=floor(max(0,$lc-1)/$_);}$Ae=$N;if(!$Ae){$Ae[]="*";if($_d)$Ae[]=$_d;}$eb=convert_fields($g,$p,$N);if($eb)$Ae[]=substr($eb,2);$J=$wb->select($a,$Ae,$Z,$s,$E,$_,$F);if(!$J)echo"<p class='error'>".error()."\n";else{if($y=="mssql"&&$F)$J->seek($_*$F);$Gb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$M=array();while($L=$J->fetch_assoc()){if($F&&$y=="oracle")unset($L["RNUM"]);$M[]=$L;}if($_GET["page"]!="last"&&+$_&&$s&&$Nc&&$y=="sql")$lc=$h->result(" SELECT FOUND_ROWS()");if(!$M)echo"<p class='message'>".'Brak rekordÃ³w.'."\n";else{$Ga=$b->backwardKeys($a,$We);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$s&&$N?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'Modify'."</a>");$vd=array();$pc=array();reset($N);$je=1;foreach($M[0]as$z=>$X){if($z!=$_d){$X=$_GET["columns"][key($N)];$o=$p[$N?($X?$X["col"]:current($N)):$z];$D=($o?$b->fieldName($o,$je):($X["fun"]?"*":$z));if($D!=""){$je++;$vd[$z]=$D;$f=idf_escape($z);$_c=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($z);$ob="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($_c.($E[0]==$f||$E[0]==$z||(!$E&&$Nc&&$s[0]==$f)?$ob:'')).'">';echo
apply_sql_function($X["fun"],$D)."</a>";echo"<span class='column hidden'>","<a href='".h($_c.$ob)."' title='".'malejÄ…co'."' class='text'> â†“</a>";if(!$X["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($z)).'\'); return false;" title="'.'Szukaj'.'" class="text jsonly"> =</a>';echo"</span>";}$pc[$z]=$X["fun"];next($N);}}$bd=array();if($_GET["modify"]){foreach($M
as$L){foreach($L
as$z=>$X)$bd[$z]=max($bd[$z],min(40,strlen(utf8_decode($X))));}}echo($Ga?"<th>".'Relacje':"")."</thead>\n";if(is_ajax()){if($_%2==1&&$F%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($M,$ic)as$C=>$L){$xf=unique_array($M[$C],$x);if(!$xf){$xf=array();foreach($M[$C]as$z=>$X){if(!preg_match('~^(COUNT\\((\\*|(DISTINCT )?`(?:[^`]|``)+`)\\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\\(`(?:[^`]|``)+`\\))$~',$z))$xf[$z]=$X;}}$yf="";foreach($xf
as$z=>$X){if(strlen($X)>64&&($y=="sql"||$y=="pgsql")){$z="MD5(".(strpos($z,'(')?$z:idf_escape($z)).")";$X=md5($X);}$yf.="&".($X!==null?urlencode("where[".bracket_escape($z)."]")."=".urlencode($X):"null%5B%5D=".urlencode($z));}echo"<tr".odd().">".(!$s&&$N?"":"<td>".checkbox("check[]",substr($yf,1),in_array(substr($yf,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($Nc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$yf)."'>".'edytuj'."</a>"));foreach($L
...
as$Pc=>$W)$A.=where_link($t++,$Pc,$W);}$X=select_value($X,$A,$o,$cf);$u=h("val[$yf][".bracket_escape($z)."]");$Y=$_POST["val"][$yf][bracket_escape($z)];$Cb=!is_array($L[$z])&&is_utf8($X)&&$M[$C][$z]==$L[$z]&&!$pc[$z];$bf=preg_match('~text|lob~',$o["type"]);if(($_GET["modify"]&&$Cb)||$Y!==null){$sc=h($Y!==null?$Y:$L[$z]);echo"<td>".($bf?"<textarea name='$u' cols='30' rows='".(substr_count($L[$z],"\n")+1)."'>$sc</textarea>":"<input name='$u' value='$sc' size='$bd[$z]'>");}else{$fd=strpos($X,"<i>...</i>");echo"<td id='$u' onclick=\"selectClick(this, event, ".($fd?2:($bf?1:0)).($Cb?"":", '".h('UÅ¼yj linku edycji aby zmieniÄ‡ tÄ™ wartoÅ›Ä‡.')."'").");\">$X";}}}if($Ga)echo"<td>";$b->backwardKeysPrint($Ga,$M[$C]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($M||$F)&&!is_ajax()){$Pb=true;if($_GET["page"]!="last"){if(!+$_)$lc=count($M);elseif($y!="sql"||!$Nc){$lc=($Nc?false:found_rows($S,$Z));if($lc<max(1e4,2*($F+1)*$_))$lc=reset(slow_query(count_rows($a,$Z,$Nc,$s)));else$Pb=false;}}if(+$_&&($lc===false||$lc>$_||$F)){echo"<p class='pages'>";$kd=($lc===false?$F+(count($M)>=$_?2:1):floor(($lc-1)/$_));if(support("table")){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Strona'."', '".($F+1)."'), event); return false;\">".'Strona'."</a>:",pagination(0,$F).($F>5?" ...":"");for($t=max(1,$F-4);$t<min($kd,$F+5);$t++)echo
pagination($t,$F);if($kd>0){echo($F+5<$kd?" ...":""),($Pb&&$lc!==false?pagination($kd,$F):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$kd'>".'ostatni'."</a>");}echo(($lc===false?count($M)+1:$lc-$F*$_)>$_?' <a href="'.h(remove_from_uri("page")."&page=".($F+1)).'" onclick="return !selectLoadMore(this, '.(+$_).', \''.'Wczytywanie'.'...\');">'.'Wczytaj wiÄ™cej danych'.'</a>':'');}else{echo'Strona'.":",pagination(0,$F).($F>1?" ...":""),($F?pagination($F,$F):""),($kd>$F?pagination($F+1,$F).($kd>$F+1?" ...":""):"");}}echo"<p class='count'>\n",($lc!==false?"(".($Pb?"":"~ ").lang(array('%d rekord','%d rekordy','%d rekordÃ³w'),$lc).") ":"");$ub=($Pb?"":"~ ").$lc;echo
checkbox("all",1,0,'wybierz wszystkie',"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$ub' : checked); selectCount('selected2', this.checked || !checked ? '$ub' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Modify</legend><div>
