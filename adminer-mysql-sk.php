...
confirm($yc="",$pf=false){return" onclick=\"".($pf?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($yc?" (' + $yc + ')":"")."');\"";}function
js_escape($T){return
addcslashes($T,"\r\n'\\/");}function
...
upload_error($q){$me=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Súbor sa nepodarilo nahrať.'.($me?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$me):""):'Súbor neexistuje.');}function
...
enum_input($O,$Ca,$d,$o,$xb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$fa);$e=(isset($xb)?"<label><input type='$O'$Ca value='$xb'".((is_array($o)?in_array($xb,$o):$o===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($fa[1]as$g=>$b){$b=stripcslashes(str_replace("''","'",$b));$Ha=(is_int($o)?$o==$g+1:(is_array($o)?in_array($g+1,$o):$o===$b));$e.=" <label><input type='$O'$Ca value='".($g+1)."'".($Ha?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
input($d,$o,$A){global$ba,$n,$s;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$ke=($s=="mssql"&&$d["auto_increment"]);if($ke&&!$_POST["save"]){$A=null;}$W=(isset($_GET["select"])||$ke?array("orig"=>'originál'):array())+$n->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Qe,$tb){$hc=($tb==1||(!$tb&&'sk'=='fr')?0:('sk'=='sl'&&(!$tb||$tb>2)?1:0)+((!$tb||$tb>=5)&&ereg('cs|sk|ru|sl','sk')?2:1));return
...
<tr><th>Systém<td>',html_select("driver",$va,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'">
...
selectLinks($Ec,$u=""){echo'<p class="tabs">';$Aa=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Ec)){$Aa["view"]='Zmeniť pohľad';}else{$Aa["create"]='Zmeniť tabuľku';}if(isset($u)){$Aa["edit"]='Nová položka';}foreach($Aa
...
selectColumnsPrint($C,$v){global$W,$Fb;print_fieldset("select",'Vypísať',$C);$g=0;$ye=array('Funkcie'=>$W,'Agregácia'=>$Fb);foreach($C
...
as$g=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$g]' value='".h($_GET["fulltext"][$g])."'>",checkbox("boolean[$g]",1,isset($_GET["boolean"][$g]),"BOOL"),"<br>\n";}}$g=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$g][col]'><option value=''>(".'kdekoľvek'.")".optionlist($v,$b["col"],true)."</select>",html_select("where[$g][op]",$this->operators,$b["op"]),"<input name='where[$g][val]' value='".h($b["val"])."'></div>\n";$g++;}}echo"<div><select name='where[$g][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoľvek'.")".optionlist($v,null,true)."</select>",html_select("where[$g][op]",$this->operators,"="),"<input name='where[$g][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($La,$v,$z){print_fieldset("sort",'Zotriediť',$La);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$g]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'zostupne')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$g]",1,0,'zostupne')."</div>\n","</div></fieldset>\n";}function
...
false;}function
messageQuery($j){global$s;static$yc=0;restart_session();$D="sql-".($yc++);$Ia=&get_session("queries");$Ia[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$D' onclick=\"return !toggle('$D');\">".'SQL príkaz'."</a><div id='$D' class='hidden'><pre><code class='jush-$s'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ia[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
';if($Cb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$Cb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$qa=tables_list();if(!$qa){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($qa);$Aa=array();foreach($qa
...
auth_error($pe=null){global$i,$n,$B;$Fc=session_name();$q="";if(!$_COOKIE[$Fc]&&$_GET[$Fc]&&ini_bool("session.use_only_cookies")){$q='Session premenné musia byť povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Fc]||$_GET[$Fc])&&!$B){$q='Session vypršala, prihláste sa prosím znova.';}else{$na=&get_session("pwds");if(isset($na)){$q=h($pe?$pe->getMessage():(is_string($i)?$i:'Neplatné prihlasovacie údaje.'));$na=null;}}}page_header('Prihlásiť sa',$q,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$n->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$Ze)),false);page_footer("auth");exit;}$i=connect();}if(is_string($i)||!$n->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$B=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$B;}$q=($_POST?($_POST["token"]==$B?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));function
connect_error(){global$i,$B,$q,$va;$_=array();if(DB!=""){page_header('Databáza'.": ".h(DB),'Nesprávna databáza.',true);}else{if($_POST["db"]&&!$q){queries_redirect(substr(ME,0,-1),'Databázy boli odstránené.',drop_databases($_POST["db"]));}page_header('Vybrať databázu',$q,false);echo"<p><a href='".h(ME)."database='>".'Vytvoriť novú databázu'."</a>\n";foreach(array('privileges'=>'Oprávnenia','processlist'=>'Zoznam procesov','variables'=>'Premenné','status'=>'Stav',)as$c=>$b){if(support($c)){echo"<a href='".h(ME)."$c='>$b</a>\n";}}echo"<p>".sprintf('Verzia %s: %s cez PHP rozšírenie %s',$va[DRIVER],"<b>$i->server_info</b>","<b>$i->extension</b>")."\n","<p>".sprintf('Prihlásený ako: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"]){set_session("dbs",null);}$_=get_databases();if($_){$wf=support("scheme");$U=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td><input type='hidden' name='token' value='$B'>&nbsp;<th>".'Databáza'."<td>".'Porovnávanie'."<td>".'Tabuľky'."</thead>\n";foreach($_
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($ne[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ba[$c]==254){$b="<code>$b</code>";}}if(isset($Aa[$c])&&!$v[$Aa[$c]]){$y="edit=".urlencode($Aa[$c]);foreach($z[$Aa[$c]]as$Mc=>$ca){$y.="&where".urlencode("[".bracket_escape($Mc)."]")."=".urlencode($a[$ca]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($c,$d,$U,$L=array()){global$Oc,$ba,$Gb,$Ka;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ba[$d["type"]])?array():array($d["type"]))+$Oc+($L?array('Cudzie kľúče'=>$L):array()),$d["type"]),'</select>
...
',(support("comment")?"<td".($mb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($mb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.2.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $de, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.2.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.2.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($cd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.2.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$mb;}function
...
';$ja=true;foreach($bd
as$c=>$b){if($c!=""&&$b>1){echo($ja?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$c%")."'>".h($c)."</a>";$ja=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$k=$i->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$k){echo'<form action=""><p>
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"]);$Wc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Wc[]=".$c";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($Wc?"[".implode("|",$Wc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Ia){print_fieldset("history",'História',$_GET["history"]!="");foreach($Ia
...
as$f=>$d){if(!isset($d["privileges"][$Ja?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$M=$_POST["referer"];if($_POST["insert"]){$M=($Ja?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$M)){$M=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$M,'Položka bola vymazaná.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Ja?"\n".idf_escape($f)." = $b":$b);}}if($Ja){if(!$u){redirect($M);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$M,'Položka bola aktualizovaná.');}else{$k=insert_into($h,$u);$Sd=($k?last_id():0);queries_redirect($M,sprintf('Položka%s bola vložená.',($Sd?" $Sd":"")),$k);}}}$wa=$n->tableName(table_status($h));page_header(($Ja?'Upraviť':'Vložiť'),$q,array("select"=>array($h,$wa)),$wa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$C=array();foreach($p
...
';if($h==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Uc?html_select("Engine",array(""=>"(".'úložisko'.")")+$Uc,$a["Engine"]):""),' ',($U&&!ereg("sqlite|mssql",$s)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$U,$a["Collation"]):""),' <input type="submit" value="Uložiť">
...
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if(support("partitioning")){$Jd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
...
as$w){if(count($_)==1||$w!=""){if(!create_database($w,$_POST["collation"])){$Ld=false;}$bb=$w;}}queries_redirect(ME."db=".urlencode($bb),'Databáza bola vytvorená.',$Ld);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$q,array(),DB);$U=collations();$f=DB;$Mb=null;if($_POST){$f=$_POST["name"];$Mb=$_POST["collation"];}elseif(DB!=""){$Mb=db_collation(DB,$U);}elseif($s=="sql"){foreach(get_vals("SHOW GRANTS")as$P){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$P,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40" onkeypress="return textareaKeypress(this, event);">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($U?html_select("collation",array(""=>"(".'porovnávanie'.")")+$U,$Mb):"");?>
...
<input type="hidden" name="token" value="',$B,'">
';if($Qa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
</form>
';}elseif(isset($_GET["event"])){$Sa=$_GET["event"];$oe=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Vc=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Sa),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$oe)&&isset($Vc[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Sa!=""?"ALTER EVENT ".idf_escape($Sa).$Td.($Sa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$Vc[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Sa!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'));}}page_header(($Sa!=""?'Upraviť udalosť'.": ".h($Sa):'Vytvoriť udalosť'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Sa!=""){$E=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Sa));$a=reset($E);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$oe,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Vc,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Uložiť">
';if($Sa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
...
as$d){if($d["field"]!=""){$u[]=(in_array($d["inout"],$wb)?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Qa=drop_create("DROP $Da ".idf_escape($Fa),"CREATE $Da ".idf_escape($_POST["name"])." (".implode(", ",$u).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$Fa);}page_header(($Fa!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($Fa):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$q);$U=get_vals("SHOW CHARACTER SET");sort($U);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Fa!=""){$a=routine($Fa,$Da);$a["name"]=$Fa;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$U,$Da);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$a["returns"],$U);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$B,'">
';if($Qa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($Fa!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$vc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($h))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$Dd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$vc["Type"],$a["Type"]),'</table>
...
as$Ga=>$P){echo'<th>'.($Ga!="*.*"?"<input name='objects[$g]' value='".h($Ga)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$lc=>$fc){foreach((array)$Y[$lc]as$Zb=>$eb){echo"<tr".odd()."><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($eb).'">'.h($Zb);$g=0;foreach($Ta
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$kd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$kd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$kd),$kd||!$_POST["kill"]);}page_header('Zoznam procesov',$q);echo'
<form action="" method="post">
