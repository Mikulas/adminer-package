...
upload_error($p){$je=($p==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($p?'Súbor sa nepodarilo nahrať.'.($je?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$je):""):'Súbor neexistuje.');}function
...
input($d,$n,$B){global$na,$q,$u;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Q=(isset($_GET["select"])?array("orig"=>'originál'):array())+$q->editFunctions($d);$Aa=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Te,$Qb){$Pb=($Qb==1||(!$Qb&&'sk'=='fr')?0:((!$Qb||$Qb>=5)&&ereg('cs|sk|ru','sk')?2:1));return
...
selectLinks($Dc,$t=""){echo'<p class="tabs">';$ya=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Dc)){$ya["view"]='Zmeniť pohľad';}else{$ya["create"]='Zmeniť tabuľku';}if(isset($t)){$ya["edit"]='Nová položka';}foreach($ya
...
selectQuery($j){global$u;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Stránka'.": ".'posledný'."'>&gt;&gt;</a> <code class='jush-$u'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Upraviť'."</a>\n";}function
rowDescription($m){return"";}function
rowDescriptions($D,$gf){return$D;}function
selectVal($b,$A,$d){$e=($b!="<i>NULL</i>"&&ereg("^char|binary",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$e=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($A?"<a href='$A'>$e</a>":$e);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($v,$w){global$Q,$Jb;print_fieldset("select",'Vypísať',$v);$g=0;$oe=array('Funkcie'=>$Q,'Agregácia'=>$Jb);foreach($v
...
selectSearchPrint($y,$w,$_){print_fieldset("search",'Vyhľadať',$y);foreach($_
...
selectOrderPrint($Za,$w,$_){print_fieldset("sort",'Zotriediť',$Za);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$g]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'zostupne')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>",checkbox("desc[$g]",1,0,'zostupne')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($T){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($T)."'>","</div></fieldset>\n";}function
selectLengthPrint($hb){if(isset($hb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($hb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$u;restart_session();$E="sql-".count($_SESSION["messages"]);$Na=&get_session("queries");$Na[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL príkaz'."</a><div id='$E' class='hidden'><pre class='jush-$u'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Na[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Aa,$n){if($d["type"]=="enum"){return($d["null"]?"<label><input type='radio'$Aa value=''".(isset($n)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Aa value='0'".($n===0?" checked":"")."><i>".'prázdne'."</i></label>";}return"";}function
...
dumpOutput($v,$n=""){$e=array('text'=>'otvoriť','file'=>'uložiť');if(function_exists('gzencode')){$e['gz']='gzip';}if(function_exists('bzcompress')){$e['bz2']='bzip2';}return
...
';if($ub!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$ub){$xa=tables_list();if(!$xa){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($xa);$ya=array();foreach($xa
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($ne[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($na[$c]==254){$b="<code>$b</code>";}}if(isset($ya[$c])&&!$w[$ya[$c]]){$A="edit=".urlencode($ya[$c]);foreach($_[$ya[$c]]as$kc=>$X){$A.="&where".urlencode("[".bracket_escape($kc)."]")."=".urlencode($a[$X]);}$b="<a href='".h(ME.$A)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($c,$d,$R,$J=array()){global$gd,$na,$Cb,$Ma;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($na[$d["type"]])?array():array($d["type"]))+$gd+($J?array('Cudzie kľúče'=>$J):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($R,$d["collation"]).'</select>',($Cb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Cb,$d["unsigned"]).'</select>':''),($J?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($Ma,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($qb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($qb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $Ce, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($nd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$qb;}function
...
';$Qa=true;foreach($id
as$c=>$b){if($c!=""&&$b>1){echo($Qa?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$c%")."'>".h($c)."</a>";$Qa=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$k=$i->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$k){echo'<form action=""><p>
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe');print_fieldset("webfile",'Zo serveru',$_POST["webfile"]);$Yc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Yc[]=".$c";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($Yc?"[".implode("|",$Yc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Na){print_fieldset("history",'História',$_GET["history"]!="");foreach($Na
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Upraviť'."</a> <code class='jush-$u'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Ga?"update":"insert"])||$q->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$p&&!isset($_GET["select"])){$U=$_POST["referer"];if($_POST["insert"]){$U=($Ga?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$U)){$U=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $y"),$U,'Položka bola vymazaná.');}else{$t=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$t[idf_escape($f)]=($Ga?"\n".idf_escape($f)." = $b":$b);}}if($Ga){if(!$t){redirect($U);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$t),"\nWHERE $y"),$U,'Položka bola aktualizovaná.');}else{$k=insert_into($h,$t);$Id=($k?last_id():0);queries_redirect($U,sprintf('Položka%s bola vložená.',($Id?" $Id":"")),$k);}}}$oa=$q->tableName(table_status($h));page_header(($Ga?'Upraviť':'Vložiť'),$p,array("select"=>array($h,$oa)),$oa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($y){$v=array();foreach($o
...
<?php echo(support("comment")?checkbox("","",$qb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($qb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložiť">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstrániť"',$db,'>';}if(support("partitioning")){$Ad=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Ld,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Ad||!$a["partition_by"]?" class='hidden'":""),'>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$db,'>';}echo'<input type="hidden" name="token" value="',$C,'">
</form>
';}elseif(isset($_GET["view"])){$h=$_GET["view"];$Ea=false;if($_POST&&!$p){$Ea=drop_create("DROP VIEW ".table($h),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'Pohľad bol odstránený.','Pohľad bol zmenený.','Pohľad bol vytvorený.',$h);}page_header(($h!=""?'Zmeniť pohľad':'Vytvoriť pohľad'),$p,array("table"=>$h),$h);$a=array();if($_POST){$a=$_POST;}elseif($h!=""){$a=view($h);$a["name"]=$h;}echo'
<form action="" method="post">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Fd,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Lc,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložiť">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$db,'>';}echo'</form>
...
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$db,'>';}echo'</form>
';}elseif(isset($_GET["type"])){$sc=$_GET["type"];if($_POST&&!$p){$A=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($sc),$A,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$A,'Typ bol vytvorený.');}}page_header($sc!=""?'Pozmeniť typ'.": ".h($sc):'Vytvoriť typ',$p);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$C,'">
';if($sc!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'$db>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'</form>
';}elseif(isset($_GET["trigger"])){$h=$_GET["trigger"];$Fc=trigger_options();$zd=array("INSERT","UPDATE","DELETE");$Ea=false;if($_POST&&!$p&&in_array($_POST["Timing"],$Fc["Timing"])&&in_array($_POST["Event"],$zd)&&in_array($_POST["Type"],$Fc["Type"])){$_d=" $_POST[Timing] $_POST[Event]";$eb=" ON ".table($h);$Ea=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$eb:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$eb.$_d:$_d.$eb)." $_POST[Type]\n$_POST[Statement]",ME."table=".urlencode($h),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$p,array("table"=>$h));$a=array("Trigger"=>$h."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$Fc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($h,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$zd,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Fc["Type"],$a["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$C,'">
';if($Ea){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',$db,'>';}echo'</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Oprávnenia'."</a>";$g=0;foreach($Oa
as$Ha=>$N){echo'<th>'.($Ha!="*.*"?"<input name='objects[$g]' value='".h($Ha)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Hc=>$Fb){foreach((array)$W[$Hc]as$dc=>$Ya){echo"<tr".odd()."><td".($Fb?">$Fb<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($dc);$g=0;foreach($Oa
...
<input type="hidden" name="token" value="',$C,'">
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',$db,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$p){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$jd),$jd||!$_POST["kill"]);}page_header('Zoznam procesov',$p);echo'
<form action="" method="post">
