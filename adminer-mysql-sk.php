...
confirm($ce=""){return" onclick=\"return confirm('".'Naozaj?'.($ce?" (' + $ce + ')":"")."');\"";}function
js_escape($J){return
addcslashes($J,"\r\n'\\/");}function
...
upload_error($q){$ie=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Súbor sa nepodarilo nahrať.'.($ie?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$ie):""):'Súbor neexistuje.');}function
...
input($d,$p,$C){global$oa,$n,$t;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$X=(isset($_GET["select"])?array("orig"=>'originál'):array())+$n->editFunctions($d);$Ca=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
lang($Fe,$Pb){$Eb=($Pb==1||(!$Pb&&'sk'=='fr')?0:((!$Pb||$Pb>=5)&&ereg('cs|sk|ru','sk')?2:1));return
...
selectLinks($Fc,$u=""){echo'<p class="tabs">';$va=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Fc)){$va["view"]='Zmeniť pohľad';}else{$va["create"]='Zmeniť tabuľku';}if(isset($u)){$va["edit"]='Nová položka';}foreach($va
...
selectQuery($j){global$t;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$t'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Upraviť'."</a>\n";}function
...
selectColumnsPrint($A,$v){global$X,$Zb;print_fieldset("select",'Vypísať',$A);$g=0;$pe=array('Funkcie'=>$X,'Agregácia'=>$Zb);foreach($A
...
selectSearchPrint($x,$v,$_){print_fieldset("search",'Vyhľadať',$x);foreach($_
...
selectOrderPrint($Xa,$v,$_){print_fieldset("sort",'Zotriediť',$Xa);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($v[$b])){echo"<div><select name='order[$g]'><option>".optionlist($v,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'zostupne')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($v,null,true)."</select>",checkbox("desc[$g]",1,0,'zostupne')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($S){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($S)."'>","</div></fieldset>\n";}function
selectLengthPrint($jb){if(isset($jb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($jb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$t;restart_session();$F="sql-".count($_SESSION["messages"]);$Ka=&get_session("queries");$Ka[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$F' onclick=\"return !toggle('$F');\">".'SQL príkaz'."</a><div id='$F' class='hidden'><pre class='jush-$t'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ka[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$e);}function
editInput($m,$d,$Ca,$p){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ca value='-1' checked><i>".'originál'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ca value=''".(isset($p)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Ca value='0'".($p===0?" checked":"")."><i>".'prázdne'."</i></label>".enum_input("radio",$Ca,$d,$p);}return"";}function
...
';if($wb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$wb){echo'<p><a href="'.h(ME).'create=">'.bold('Vytvoriť novú tabuľku',$_GET["create"]==="")."</a>\n";$xa=tables_list();if(!$xa){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($xa);$va=array();foreach($xa
...
as$m=>$O){echo'<a href="'.h(ME).'select='.urlencode($m).'">'.bold('vypísať',$_GET["select"]==$m).'</a> ','<a href="'.h(ME).'table='.urlencode($m).'">'.bold($this->tableName(array("Name"=>$m)),$_GET["table"]==$m)."</a><br>\n";}}}$n=(function_exists('adminer_object')?adminer_object():new
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}else{if($de[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($oa[$c]==254){$b="<code>$b</code>";}}if(isset($va[$c])&&!$v[$va[$c]]){$y="edit=".urlencode($va[$c]);foreach($_[$va[$c]]as$vc=>$V){$y.="&where".urlencode("[".bracket_escape($vc)."]")."=".urlencode($a[$V]);}$b="<a href='".h(ME.$y)."'>$b</a>";}}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($c,$d,$R,$K=array()){global$ad,$oa,$Yb,$Sa;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($oa[$d["type"]])?array():array($d["type"]))+$ad+($K?array('Cudzie kľúče'=>$K):array()),$d["type"]),'</select>
<td><input name="',$c,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td>',"<select name='$c"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($R,$d["collation"]).'</select>',($Yb?"<select name='$c"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($Yb,$d["unsigned"]).'</select>':''),($K?"<select name='$c"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist($Sa,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($ob?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($ob?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.1.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $ae, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.1.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.1.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($bd==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.1.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$ob;}function
...
';$Va=true;foreach($Sc
as$c=>$b){if($c!=""&&$b>1){echo($Va?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$c%")."'>".h($c)."</a>";$Va=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$k=$i->query("SELECT User, Host FROM mysql.user ORDER BY Host, User");if(!$k){echo'<form action=""><p>
...
as$f=>$d){if(!isset($d["privileges"][$Da?"update":"insert"])||$n->fieldName($d)==""){unset($o[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$T=$_POST["referer"];if($_POST["insert"]){$T=($Da?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$T)){$T=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$T,'Položka bola vymazaná.');}else{$u=array();foreach($o
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Da?"\n".idf_escape($f)." = $b":$b);}}if($Da){if(!$u){redirect($T);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$T,'Položka bola aktualizovaná.');}else{$k=insert_into($h,$u);$Ld=($k?last_id():0);queries_redirect($T,sprintf('Položka%s bola vložená.',($Ld?" $Ld":"")),$k);}}}$ta=$n->tableName(table_status($h));page_header(($Da?'Upraviť':'Vložiť'),$q,array("select"=>array($h,$ta)),$ta);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$A=array();foreach($o
...
';if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}if($o){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo'<input type="submit" name="insert" value="'.($Da?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."\">\n";}}if($Da){echo"<input type='submit' name='delete' value='".'Zmazať'."'".confirm().">\n";}echo'</form>
...
<?php echo(support("comment")?checkbox("","",$ob,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ob?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Uložiť">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if(support("partitioning")){$Cd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$zd,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Cd||!$a["partition_by"]?" class='hidden'":""),'>
...
<input type="hidden" name="token" value="',$B,'">
';if($Fa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
</form>
';}elseif(isset($_GET["event"])){$Ua=$_GET["event"];$_d=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$dd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ua),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$_d)&&isset($dd[$_POST["STATUS"]])){$Hd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($Ua!=""?"ALTER EVENT ".idf_escape($Ua).$Hd.($Ua!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Hd)."\n".$dd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($Ua!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'));}}page_header(($Ua!=""?'Upraviť udalosť'.": ".h($Ua):'Vytvoriť udalosť'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ua!=""){$D=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ua));$a=reset($D);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_d,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$dd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$B,'">
<input type="submit" value="Uložiť">
';if($Ua!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
...
<input type="hidden" name="token" value="',$B,'">
';if($Fa){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'</form>
';}elseif(isset($_GET["type"])){$tc=$_GET["type"];if($_POST&&!$q){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($tc),$y,'Typ bol odstránený.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ bol vytvorený.');}}page_header($tc!=""?'Pozmeniť typ'.": ".h($tc):'Vytvoriť typ',$q);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$B,'">
';if($tc!=""){echo"<input type='submit' name='drop' value='".'Odstrániť'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Uložiť'."'>\n";}echo'</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html'>".'Oprávnenia'."</a>";$g=0;foreach($Ta
as$Ea=>$P){echo'<th>'.($Ea!="*.*"?"<input name='objects[$g]' value='".h($Ea)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$wc=>$Lb){foreach((array)$Y[$wc]as$Ob=>$eb){echo"<tr".odd()."><td".($Lb?">$Lb<td":" colspan='2'").' lang="en" title="'.h($eb).'">'.h($Ob);$g=0;foreach($Ta
...
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$jd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$jd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$jd),$jd||!$_POST["kill"]);}page_header('Zoznam procesov',$q);echo'
<form action="" method="post">
...
as$c=>$b){echo"<td>".($c=="Info"&&$b!=""?"<code class='jush-$t'>".nbsp($b).'</code> <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Upraviť'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
<input type="hidden" name="token" value="',$B,'">
...
as$c=>$b){echo"setHtml('sum-$c', '".number_format($b,0,'.',' ')."');\n";}}else{foreach(count_tables(get_databases())as$w=>$b){echo"setHtml('tables-".js_escape($w)."', '$b');\n";}}exit;}else{$rd=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($rd&&!$q&&!$_POST["search"]){$k=true;$la="";if($t=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$k=truncate_tables($_POST["tables"]);}$la='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$k=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$la='Tabuľka bola presunutá.';}elseif($_POST["drop"]){if($_POST["views"]){$k=drop_views($_POST["views"]);}if($k&&$_POST["tables"]){$k=drop_tables($_POST["tables"]);}$la='Tabuľka bola odstránená.';}elseif($_POST["tables"]&&($k=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$k->fetch_assoc()){$la.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$la,$k);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$q,true);echo'<p>'.($_GET["ns"]==""?'<a href="'.h(ME).'database=">'.'Zmeniť databázu'."</a>\n":"");if($_GET["ns"]!==""){echo'<a href="'.h(ME).'schema=">'.'Schéma databázy'."</a>\n","<h3>".'Tabuľky a pohľady'."</h3>\n";$Hc=tables_list();if(!$Hc){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabuľka'.'<td>'.'Typ'.'<td>'.'Porovnávanie'.'<td>'.'Veľkosť dát'.'<td>'.'Veľkosť indexu'.'<td>'.'Voľné miesto'.'<td>'.'Auto Increment'.'<td>'.'Riadky'.(support("comment")?'<td>'.'Komentár':'')."</thead>\n";foreach($Hc
as$f=>$O){$mc=(isset($O)&&!eregi("table",$O));echo'<tr'.odd().'><td>'.checkbox(($mc?"views[]":"tables[]"),$f,in_array($f,$rd,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($mc){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{echo"<td id='Engine-".h($f)."'>&nbsp;<td id='Collation-".h($f)."'>&nbsp;";foreach(array("Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$c=>$y){echo"<td align='right'><a href='".h(ME."$y=").urlencode($f)."' id='$c-".h($f)."'>?</a>";}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($Hc)),"<td>".nbsp($i->result("SELECT @@storage_engine")),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$c){echo"<td align='right' id='sum-$c'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p><input type='hidden' name='token' value='$B'>".($t=="sql"?"<input type='submit' value='".'Analyzovať'."'> <input type='submit' name='optimize' value='".'Optimalizovať'."'> <input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)").">\n";$z=(support("scheme")?schemas():get_databases());if(count($z)!=1&&$t!="sqlite"){$w=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.($z?": ".html_select("target",$z,$w):': <input name="target" value="'.h($w).'">')." <input type='submit' name='move' value='".'Presunúť'."'>\n";}}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";}if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$vd=routines();if($vd){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($vd
