...
confirm($Wa="",$Ie=false){return" onclick=\"".($Ie?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Wa?" (' + $Wa + ')":"")."');\"";}function
...
upload_error($i){$Rc=($i==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($i?'Súbor sa nepodarilo nahrať.'.($Rc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Rc):""):'Súbor neexistuje.');}function
...
enum_input($U,$ta,$j,$Y,$zb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$j["length"],$Mc);$I=(isset($zb)?"<label><input type='$U'$ta value='$zb'".((is_array($Y)?in_array($zb,$Y):$Y===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($Mc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Fa=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$I.=" <label><input type='$U'$ta value='".($r+1)."'".($Fa?' checked':'').'>'.h($b->editVal($X,$j)).'</label>';}return$I;}function
input($j,$Y,$n){global$tf,$b,$w;$C=h(bracket_escape($j["field"]));echo"<td class='function'>";$me=($w=="mssql"&&$j["auto_increment"]);if($me&&!$_POST["save"]){$n=null;}$o=(isset($_GET["select"])||$me?array("orig"=>'originál'):array())+$b->editFunctions($j);$ta=" name='fields[$C]'";if($j["type"]=="enum"){echo
...
selectLinks($Re,$O=""){echo'<p class="tabs">';$Jc=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Re)){$Jc["view"]='Zmeniť pohľad';}else{$Jc["create"]='Zmeniť tabuľku';}if(isset($O)){$Jc["edit"]='Nová položka';}foreach($Jc
...
selectColumnsPrint($M,$d){global$o,$fc;print_fieldset("select",'Vypísať',$M);$r=0;$cc=array('Funkcie'=>$o,'Agregácia'=>$fc);foreach($M
...
false;}function
messageQuery($G){global$w;static$Wa=0;restart_session();$s="sql-".($Wa++);$hc=&get_session("queries");if(strlen($G)>1e6){$G=ereg_replace('[\x80-\xFF]+$','',substr($G,0,1e6))."\n...";}$hc[$_GET["db"]][]=$G;return" <a href='#$s' onclick=\"return !toggle('$s');\">".'SQL príkaz'."</a><div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($G,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($hc[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
';if($Zc!="db"&&DB!=""&&$e->select_db(DB)){if($_GET["ns"]!==""&&!$Zc){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$We=tables_list();if(!$We){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($We);$Jc=array();foreach($We
...
as$x=>$X){if(!isset($X)){$X="<i>NULL</i>";}elseif($_a[$x]&&!is_utf8($X)){$X="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($X))."</i>";}elseif(!strlen($X)){$X="&nbsp;";}else{$X=h($X);if($tf[$x]==254){$X="<code>$X</code>";}}if(isset($Jc[$x])&&!$d[$Jc[$x]]){if($kc){$z=$Jc[$x].urlencode($J[array_search("table=",$Jc)]);}else{$z="edit=".urlencode($Jc[$x]);foreach($u[$Jc[$x]]as$Ia=>$v){$z.="&where".urlencode("[".bracket_escape($Ia)."]")."=".urlencode($J[$v]);}}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($r?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($x,$j,$c,$m=array()){global$Ke,$tf,$_f,$nd;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$j["type"]||isset($tf[$j["type"]])?array():array($j["type"]))+$Ke+($m?array('Cudzie kľúče'=>$m):array()),$j["type"]),'</select>
<td><input name="',$x,'[length]" value="',h($j["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$x"."[collation]'".(ereg('(char|text|enum|set)$',$j["type"])?"":" class='hidden'").'><option value="">('.'porovnávanie'.')'.optionlist($c,$j["collation"]).'</select>',($_f?"<select name='$x"."[unsigned]'".(!$j["type"]||ereg('(int|float|double|decimal)$',$j["type"])?"":" class='hidden'").'><option>'.optionlist($_f,$j["unsigned"]).'</select>':''),($m?"<select name='$x"."[on_delete]'".(ereg("`",$j["type"])?"":" class='hidden'")."><option value=''>(".'ON DELETE'.")".optionlist(explode("|",$nd),$j["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Qa?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($k))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($k),';</script>
</thead>
...
',(support("comment")?"<td".($Qa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($j["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.3' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $qa, 1);'>&nbsp;"."<input type='image' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.3' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.3' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($xd==""||support("drop_col")?"<input type='image' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.3' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$x=>$X){echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Zmeniť'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Te=array();$Ue=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Mc,PREG_SET_ORDER);foreach($Mc
...
<table cellspacing="0">
';$fb=array('','USE','DROP+CREATE','CREATE');$Ve=array('','DROP+CREATE','CREATE');$db=array('','TRUNCATE+INSERT','INSERT');if($w=="sql"){$fb[]='CREATE+ALTER';$Ve[]='CREATE+ALTER';$db[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$J);if(!$J){$J=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}if(!isset($J["events"])){$J["routines"]=$J["events"]=($_GET["dump"]=="");$J["triggers"]=$J["table_style"];}echo"<tr><th>".'Výstup'."<td>".html_select("output",$b->dumpOutput(),$J["output"],0)."\n";echo"<tr><th>".'Formát'."<td>".html_select("format",$b->dumpFormat(),$J["format"],0)."\n";echo($w=="sqlite"?"":"<tr><th>".'Databáza'."<td>".html_select('db_style',$fb,$J["db_style"]).(support("routine")?checkbox("routines",1,$J["routines"],'Procedúry'):"").(support("event")?checkbox("events",1,$J["events"],'Udalosti'):"")),"<tr><th>".'Tabuľky'."<td>".html_select('table_style',$Ve,$J["table_style"]).checkbox("auto_increment",1,$J["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$J["triggers"],'Triggery'):""),"<tr><th>".'Dáta'."<td>".html_select('data_style',$db,$J["data_style"]),'</table>
<p><input type="submit" value="Export">

<table cellspacing="0">
';$Td=array();if(DB!=""){$Fa=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Fa onclick='formCheck(this, /^tables\\[/);'>".'Tabuľky'."</label>","<th style='text-align: right;'><label>".'Dáta'."<input type='checkbox' id='check-data'$Fa onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Gf="";foreach(table_status()as$S){$C=$S["Name"];$Sd=ereg_replace("_.*","",$C);$Fa=($a==""||$a==(substr($a,-1)=="%"?"$Sd%":$C));$Vd="<tr><td>".checkbox("tables[]",$C,$Fa,$C,"formUncheck('check-tables');");if(is_view($S)){$Gf.="$Vd\n";}else{echo"$Vd<td align='right'><label>".($S["Engine"]=="InnoDB"&&$S["Rows"]?"~ ":"").$S["Rows"].checkbox("data[]",$C,$Fa,"","formUncheck('check-data');")."</label>\n";}$Td[$Sd]++;}echo$Gf;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($a==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Databáza'."</label></thead>\n";$g=get_databases();if($g){foreach($g
...
as$C=>$j){if(!isset($j["privileges"][$Af?"update":"insert"])||$b->fieldName($j)==""){unset($k[$C]);}}if($_POST&&!$i&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"]){$_=($Af?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$_)){$_=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Položka bola vymazaná.');}else{$O=array();foreach($k
as$C=>$j){$X=process_input($j);if($X!==false&&$X!==null){$O[idf_escape($C)]=($Af?"\n".idf_escape($C)." = $X":$X);}}if($Af){if(!$O){redirect($_);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$O),"\nWHERE $Z"),$_,'Položka bola aktualizovaná.');}else{$H=insert_into($a,$O);$Cc=($H?last_id():0);queries_redirect($_,sprintf('Položka%s bola vložená.',($Cc?" $Cc":"")),$H);}}}$Se=$b->tableName(table_status($a));page_header(($Af?'Upraviť':'Vložiť'),$i,array("select"=>array($a,$Se)),$Se);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($Z){$M=array();foreach($k
...
';if($k){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Af?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($Af?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$k?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Hd=ereg('RANGE|LIST',$J["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$J["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Gd,$J["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($J["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($J["partitions"]),'"',($Hd||!$J["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$i&&!isset($_POST["add_x"])){restart_session();if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáza bola odstránená.',drop_databases(array(DB)));}elseif(DB!==$_POST["name"]){if(DB!=""){$_GET["db"]=$_POST["name"];queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_POST["name"]),'Databáza bola premenovaná.',rename_database($_POST["name"],$_POST["collation"]));}else{$g=explode("\n",str_replace("\r","",$_POST["name"]));$Le=true;$Bc="";foreach($g
as$h){if(count($g)==1||$h!=""){if(!create_database($h,$_POST["collation"])){$Le=false;}$Bc=$h;}}queries_redirect(ME."db=".urlencode($Bc),'Databáza bola vytvorená.',$Le);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$i,array(),DB);$c=collations();$C=DB;$Ja=null;if($_POST){$C=$_POST["name"];$Ja=$_POST["collation"];}elseif(DB!=""){$Ja=db_collation(DB,$c);}elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$A)&&$A[1]){$C=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<p>
ON DELETE: ',html_select("on_delete",array(-1=>"")+explode("|",$nd),$J["on_delete"]),' ON UPDATE: ',html_select("on_update",array(-1=>"")+explode("|",$nd),$J["on_update"]),'<p>
<input type="submit" value="Uložiť">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($J["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$uc,$J["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$He,$J["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($J["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($qb){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$X){if($X[1]!="USAGE"){$dc["$A[2]$X[2]"][$X[1]]=true;}if(ereg(' WITH GRANT OPTION',$J[0])){$dc["$A[2]$X[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$J[0],$A)){$kd=$A[1];}}}if($_POST&&!$i){$ld=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$ed=q($_POST["user"])."@".q($_POST["host"]);$Kd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $ld",ME."privileges=",'Používateľ bol odstránený.');}else{$Za=false;if($ld!=$ed){$Za=queries(($e->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $ed IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Kd");$i=!$Za;}elseif($_POST["pass"]!=$kd||!$_POST["hashed"]){queries("SET PASSWORD FOR $ed = ".($_POST["hashed"]?$Kd:"PASSWORD($Kd)"));}if(!$i){$ne=array();foreach($dd
...
as$hd=>$ne){if(preg_match('~^(.+)(\\(.*\\))?$~U',$hd,$A)){grant("REVOKE",array_keys($ne),$A[2]," ON $A[1] FROM $ed");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Používateľ bol zmenený.':'Používateľ bol vytvorený.'),!$i);if($Za){$e->query("DROP USER $ed");}}}page_header((isset($_GET["host"])?'Používateľ'.": ".h("$fa@$_GET[host]"):'Vytvoriť používateľa'),$i,array("privileges"=>array('','Oprávnenia')));if($_POST){$J=$_POST;$dc=$dd;}else{$J=$_GET+array("host"=>$e->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$J["pass"]=$kd;if($kd!=""){$J["hashed"]=true;}$dc[DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($e->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$r=0;foreach($dc
as$hd=>$p){echo'<th>'.($hd!="*.*"?"<input name='objects[$r]' value='".h($hd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Ua=>$jb){foreach((array)$Yd[$Ua]as$Xd=>$Pa){echo"<tr".odd()."><td".($jb?">$jb<td":" colspan='2'").' lang="en" title="'.h($Pa).'">'.h($Xd);$r=0;foreach($dc
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$i){$zc=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X))){$zc++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$zc),$zc||!$_POST["kill"]);}page_header('Zoznam procesov',$i);echo'
<form action="" method="post">
...
as$x=>$X){json_row("sum-$x",number_format($X,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$h=>$X){json_row("tables-".js_escape($h),$X);}json_row("");}exit;}else{$Ye=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Ye&&!$i&&!$_POST["search"]){$H=true;$Tc="";if($w=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$H=truncate_tables($_POST["tables"]);}$Tc='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$H=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Tc='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$H=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$Tc='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"]){$H=drop_views($_POST["views"]);}if($H&&$_POST["tables"]){$H=drop_tables($_POST["tables"]);}$Tc='Tabuľka bola odstránená.';}elseif($_POST["tables"]&&($H=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($J=$H->fetch_assoc()){$Tc.="<b>".h($J["Table"])."</b>: ".h($J["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$Tc,$H);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$i,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$Xe=tables_list();if(!$Xe){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabuľka','<td>'.'Typ','<td>'.'Porovnávanie','<td>'.'Veľkosť dát','<td>'.'Veľkosť indexu','<td>'.'Voľné miesto','<td>'.'Auto Increment','<td>'.'Riadky',(support("comment")?'<td>'.'Komentár':''),"</thead>\n";foreach($Xe
as$C=>$U){$Ff=(isset($U)&&!eregi("table",$U));echo'<tr'.odd().'><td>'.checkbox(($Ff?"views[]":"tables[]"),$C,in_array($C,$Ye,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($C).'" title="'.'Zobraziť štruktúru'.'">'.h($C).'</a>';if($Ff){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'Zmeniť pohľad'.'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'Vypísať dáta'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmeniť tabuľku'),"Index_length"=>array("indexes",'Zmeniť indexy'),"Data_free"=>array("edit",'Nová položka'),"Auto_increment"=>array("auto_increment=1&create",'Zmeniť tabuľku'),"Rows"=>array("select",'Vypísať dáta'),)as$x=>$z){echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($C)."' id='$x-".h($C)."' title='$z[1]'>?</a>":"<td id='$x-".h($C)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($Xe)),"<td>".nbsp($w=="sql"?$e->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$x){echo"<td align='right' id='sum-$x'>&nbsp;";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".($w=="sql"?"<input type='submit' value='".'Analyzovať'."'> <input type='submit' name='optimize' value='".'Optimalizovať'."'> <input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$g=(support("scheme")?schemas():get_databases());if(count($g)!=1&&$w!="sqlite"){$h=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($g?html_select("target",$g,$h):'<input name="target" value="'.h($h).'">')," <input type='submit' name='move' value='".'Presunúť'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$T'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";}if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$se=routines();if($se){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($se
