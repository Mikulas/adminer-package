...
upload_error($j){$Yc=($j==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($j?'Súbor sa nepodarilo nahrať.'.($Yc?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Yc):""):'Súbor neexistuje.');}function
...
enum_input($V,$ua,$k,$Y,$Bb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$k["length"],$Tc);$J=($Bb!==null?"<label><input type='$V'$ua value='$Bb'".((is_array($Y)?in_array($Bb,$Y):$Y===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($Tc[1]as$r=>$X){$X=stripcslashes(str_replace("''","'",$X));$Ha=(is_int($Y)?$Y==$r+1:(is_array($Y)?in_array($r+1,$Y):$Y===$X));$J.=" <label><input type='$V'$ua value='".($r+1)."'".($Ha?' checked':'').'>'.h($b->editVal($X,$k)).'</label>';}return$J;}function
input($k,$Y,$p){global$_f,$b,$w;$D=h(bracket_escape($k["field"]));echo"<td class='function'>";$ue=($w=="mssql"&&$k["auto_increment"]);if($ue&&!$_POST["save"])$p=null;$hc=(isset($_GET["select"])||$ue?array("orig"=>'originál'):array())+$b->editFunctions($k);$ua=" name='fields[$D]'";if($k["type"]=="enum")echo
...
lang($tf,$md){$Wd=($md==1?0:($md&&$md<5?1:2));$tf=str_replace("%d","%s",$tf[$Wd]);$md=number_format($md,0,".",' ');return
...
selectLinks($Ye,$P=""){echo'<p class="tabs">';$Qc=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Ye))$Qc["view"]='Zmeniť pohľad';else$Qc["create"]='Zmeniť tabuľku';if($P!==null)$Qc["edit"]='Nová položka';foreach($Qc
...
selectColumnsPrint($N,$e){global$hc,$lc;print_fieldset("select",'Vypísať',$N);$r=0;$gc=array('Funkcie'=>$hc,'Agregácia'=>$lc);foreach($N
...
selectOrderPrint($Ad,$e,$u){print_fieldset("sort",'Zotriediť',$Ad);$r=0;foreach((array)$_GET["order"]as$x=>$X){if(isset($e[$X])){echo"<div><select name='order[$r]' onchange='selectFieldChange(this.form);'><option>".optionlist($e,$X,true)."</select>",checkbox("desc[$r]",1,isset($_GET["desc"][$x]),'zostupne')."</div>\n";$r++;}}echo"<div><select name='order[$r]' onchange='selectAddRow(this);'><option>".optionlist($e,null,true)."</select>","<label><input type='checkbox' name='desc[$r]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
...
selectLengthPrint($jf){if($jf!==null){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($jf).'">',"</div></fieldset>\n";}}function
...
edit_type($x,$k,$d,$n=array()){global$Re,$_f,$Gf,$ud;echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$k["type"]||isset($_f[$k["type"]])?array():array($k["type"]))+$Re+($n?array('Cudzie kľúče'=>$n):array()),$k["type"]),'</select>
...
',(support("comment")?"<td".($Sa?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($l))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($l),';</script>
</thead>
...
',(support("comment")?"<td".($Sa?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($k["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.5.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $ra, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.5.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.5.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Ed==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.5.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$x=>$X)echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($x)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($x))."'>".'Zmeniť'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$af=array();$bf=array();$D="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$D-".DB]?"$D-".DB:$D)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Tc,PREG_SET_ORDER);foreach($Tc
...
as$x=>$X){list($G,$kf)=$X;echo'<a href="'.h(ME."sql=&history=$x").'">'.'Upraviť'."</a> <span class='time'>".@date("H:i:s",$kf)."</span> <code class='jush-$w'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$G)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$D=>$k){if(!isset($k["privileges"][$Hf?"update":"insert"])||$b->fieldName($k)=="")unset($l[$D]);}if($_POST&&!$j&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($Hf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$_))$_=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$_,'Položka bola vymazaná.');else{$P=array();foreach($l
as$D=>$k){$X=process_input($k);if($X!==false&&$X!==null)$P[idf_escape($D)]=($Hf?"\n".idf_escape($D)." = $X":$X);}if($Hf){if(!$P)redirect($_);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$P),"\nWHERE $Z"),$_,'Položka bola aktualizovaná.');}else{$I=insert_into($a,$P);$Ic=($I?last_id():0);queries_redirect($_,sprintf('Položka%s bola vložená.',($Ic?" $Ic":"")),$I);}}}$Ze=$b->tableName(table_status($a));page_header(($Hf?'Upraviť':'Vložiť'),$j,array("select"=>array($a,$Ze)),$Ze);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$N=array();foreach($l
...
';if($l){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Hf?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}echo($Hf?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$l?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Pd=ereg('RANGE|LIST',$K["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$K["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Od,$K["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($K["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($K["partitions"]),'"',($Pd||!$K["partition_by"]?" class='hidden'":""),'>
...
as$i){if(count($h)==1||$i!=""){if(!create_database($i,$_POST["collation"]))$Se=false;$Hc=$i;}}queries_redirect(ME."db=".urlencode($Hc),'Databáza bola vytvorená.',$Se);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($D).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$j,array(),DB);$d=collations();$D=DB;$La=null;if($_POST){$D=$_POST["name"];$La=$_POST["collation"];}elseif(DB!="")$La=db_collation(DB,$d);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$q,$A)&&$A[1]){$D=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$_c,$K["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$Pe,$K["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($K["name"]),'" maxlength="64">
',($ze?'Jazyk'.": ".html_select("language",$ze,$K["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$d,$ye);if(isset($_GET["function"])){echo"<tr><td>".'Návratový typ';edit_type("returns",$K["returns"],$d);}echo'</table>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$wf["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$vf,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$wf["Type"],$K["Type"]),'</table>
...
as$X){if($X[1]!="USAGE")$ic["$A[2]$X[2]"][$X[1]]=true;if(ereg(' WITH GRANT OPTION',$K[0]))$ic["$A[2]$X[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$K[0],$A))$rd=$A[1];}}if($_POST&&!$j){$sd=(isset($_GET["host"])?q($fa)."@".q($_GET["host"]):"''");$kd=q($_POST["user"])."@".q($_POST["host"]);$Sd=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $sd",ME."privileges=",'Používateľ bol odstránený.');else{$cb=false;if($sd!=$kd){$cb=queries(($f->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $kd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Sd");$j=!$cb;}elseif($_POST["pass"]!=$rd||!$_POST["hashed"])queries("SET PASSWORD FOR $kd = ".($_POST["hashed"]?$Sd:"PASSWORD($Sd)"));if(!$j){$ve=array();foreach($jd
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($f->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$r=0;foreach($ic
as$nd=>$q){echo'<th>'.($nd!="*.*"?"<input name='objects[$r]' value='".h($nd)."' size='10'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$Xa=>$mb){foreach((array)$ge[$Xa]as$fe=>$Ra){echo"<tr".odd()."><td".($mb?">$mb<td":" colspan='2'").' lang="en" title="'.h($Ra).'">'.h($fe);$r=0;foreach($ic
as$nd=>$q){$D="'grants[$r][".h(strtoupper($fe))."]'";$Y=$q[strtoupper($fe)];if($Xa=="Server Admin"&&$nd!=(isset($ic["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$D><option><option value='1'".($Y?" selected":"").">".'Povoliť'."<option value='0'".($Y=="0"?" selected":"").">".'Zakázať'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$j){$Ec=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Ec++;}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$Ec),$Ec||!$_POST["kill"]);}page_header('Zoznam procesov',$j);echo'
<form action="" method="post">
