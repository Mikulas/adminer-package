...
* @version 3.3.0
*/error_reporting(6135);$Fd=(!ereg('^(unsafe_raw)?$',ini_get("filter.default"))||ini_get("filter.default_flags"));if($Fd){foreach(array('_GET','_POST','_COOKIE','_SERVER')as$b){$ke=filter_input_array(constant("INPUT$b"),FILTER_UNSAFE_RAW);if($ke){$$b=$ke;}}}if(isset($_GET["file"])){header("Expires: ".gmdate("D, d M Y H:i:s",time()+365*24*60*60)." GMT");if($_GET["file"]=="favicon.ico"){header("Content-Type: image/x-icon");echo
...
as$za=>$s){unset($eb[$c][$za]);if(is_array($s)){$eb[$c][stripslashes($za)]=$s;$eb[]=&$eb[$c][stripslashes($za)];}else{$eb[$c][stripslashes($za)]=($Fd?$s:stripslashes($s));}}}}}function
bracket_escape($Ca,$Ve=false){static$ge=array(':'=>':1',']'=>':2','['=>':3');return
strtr($Ca,($Ve?array_flip($ge):$ge));}function
h($V){return
htmlspecialchars(str_replace("\0","",$V),ENT_QUOTES);}function
nbsp($V){return(trim($V)!=""?h($V):"&nbsp;");}function
nl_br($V){return
str_replace("\n","<br>",$V);}function
checkbox($f,$o,$Oa,$Nd="",$Nc="",$rf=false){static$E=0;$E++;$e="<input type='checkbox' name='$f' value='".h($o)."'".($Oa?" checked":"").($Nc?" onclick=\"$Nc\"":"").($rf?" class='jsonly'":"")." id='checkbox-$E'>";return($Nd!=""?"<label for='checkbox-$E'>$e".h($Nd)."</label>":$e);}function
optionlist($Kc,$vf=null,$ve=false){$e="";foreach($Kc
as$za=>$s){$pe=array($za=>$s);if(is_array($s)){$e.='<optgroup label="'.h($za).'">';$pe=$s;}foreach($pe
as$c=>$b){$e.='<option'.($ve||is_string($c)?' value="'.h($c).'"':'').(($ve||is_string($c)?(string)$c:$b)===$vf?' selected':'').'>'.h($b);}if(is_array($s)){$e.='</optgroup>';}}return$e;}function
...
confirm($Ic="",$jf=false){return" onclick=\"".($jf?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Ic?" (' + $Ic + ')":"")."');\"";}function
js_escape($V){return
addcslashes($V,"\r\n'\\/");}function
...
query_redirect($j,$O,$ga,$yc=true,$sf=true,$Me=false){global$i,$q,$n;if($sf){$Me=!$i->query($j);}$td="";if($j){$td=$n->messageQuery("$j;");}if($Me){$q=error().$td;return
...
apply_queries($j,$ia,$xf='table'){foreach($ia
as$l){if(!queries("$j ".$xf($l))){return
false;}}return
true;}function
...
pagination($K,$qf){return" ".($K==$qf?$K+1:'<a href="'.h(remove_from_uri("page").($K?"&page=$K":"")).'">'.($K+1)."</a>");}function
get_file($c,$Vc=false){$va=$_FILES[$c];if(!$va||$va["error"]){return$va["error"];}$e=file_get_contents($Vc&&ereg('\\.gz$',$va["name"])?"compress.zlib://$va[tmp_name]":($Vc&&ereg('\\.bz2$',$va["name"])?"compress.bzip2://$va[tmp_name]":$va["tmp_name"]));if($Vc){$rb=substr($e,0,3);if(function_exists("iconv")&&ereg("^\xFE\xFF|^\xFF\xFE",$rb,$If)){$e=iconv("utf-16","utf-8",$e);}elseif($rb=="\xEF\xBB\xBF"){$e=substr($e,3);}}return$e;}function
upload_error($q){$Ed=($q==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($q?'Súbor sa nepodarilo nahrať.'.($Ed?" ".sprintf('Maximálna povolená veľkosť súboru je %sB.',$Ed):""):'Súbor neexistuje.');}function
...
hidden_fields($eb,$We=array()){while(list($c,$b)=each($eb)){if(is_array($b)){foreach($b
as$za=>$s){$eb[$c."[$za]"]=$s;}}elseif(!in_array($c,$We)){echo'<input type="hidden" name="'.h($c).'" value="'.h($b).'">';}}}function
...
password_file(){$Mc=ini_get("upload_tmp_dir");if(!$Mc){if(function_exists('sys_get_temp_dir')){$Mc=sys_get_temp_dir();}else{$kb=@tempnam("","");if(!$kb){return
false;}$Mc=dirname($kb);unlink($kb);}}$kb="$Mc/adminer.key";$e=@file_get_contents($kb);if($e){return$e;}$Aa=@fopen($kb,"w");if($Aa){$e=md5(uniqid(mt_rand(),true));fwrite($Aa,$e);fclose($Aa);}return$e;}function
...
print_fieldset($E,$Qe,$Re=false,$Nc=""){echo"<fieldset><legend><a href='#fieldset-$E' onclick=\"$Nc"."return !toggle('fieldset-$E');\">$Qe</a></legend><div id='fieldset-$E'".($Re?"":" class='hidden'").">\n";}function
bold($Se){return($Se?" class='active'":"");}global$n,$i,$ya,$gd,$lb,$q,$X,$Ab,$Nb,$yb,$r,$Cf,$Gf,$Ga,$Fc,$D,$Ff,$ua,$wb,$fc;if(!isset($_SERVER["REQUEST_URI"])){$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"].($_SERVER["QUERY_STRING"]!=""?"?$_SERVER[QUERY_STRING]":"");}$Nb=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$Pb=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$Nb);if(version_compare(PHP_VERSION,'5.2.0')>=0){$Pb[]=true;}call_user_func_array('session_set_cookie_params',$Pb);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE));if(function_exists("set_magic_quotes_runtime")){set_magic_quotes_runtime(false);}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'sk';}function
lang($zf,$Ec){$Zb=($Ec==1?0:($Ec&&$Ec<5?1:2));return
sprintf($zf[$Zb],$Ec);}if(extension_loaded('pdo')){class
Min_PDO
extends
...
query($j,$Yb=false){$k=parent::query($j);if(!$k){$hf=$this->errorInfo();$this->error=$hf[2];return
...
fetch_field(){$a=(object)$this->getColumnMeta($this->_offset++);$a->orgtable=$a->table;$a->orgname=$a->name;$a->charsetnr=(in_array("blob",$a->flags)?63:0);return$a;}}}$ya=array();$ya=array("server"=>"MySQL")+$ya;if(!defined("DRIVER")){$bf=array("MySQLi","MySQL","PDO_MySQL");define("DRIVER","server");if(extension_loaded("mysqli")){class
Min_DB
extends
...
foreign_keys($l){global$i,$Ga;static$R='`(?:[^`]|``)+`';$e=array();$ye=$i->result("SHOW CREATE TABLE ".table($l),1);if($ye){preg_match_all("~CONSTRAINT ($R) FOREIGN KEY \\(((?:$R,? ?)+)\\) REFERENCES ($R)(?:\\.($R))? \\(((?:$R,? ?)+)\\)(?: ON DELETE (".implode("|",$Ga)."))?(?: ON UPDATE (".implode("|",$Ga)."))?~",$ye,$fa,PREG_SET_ORDER);foreach($fa
...
alter_table($l,$f,$p,$Jc,$cb,$cc,$mb,$Gc,$Ob){$ca=array();foreach($p
as$d){$ca[]=($d[1]?($l!=""?($d[0]!=""?"CHANGE ".idf_escape($d[0]):"ADD"):" ")." ".implode($d[1]).($l!=""?" $d[2]":""):"DROP ".idf_escape($d[0]));}$ca=array_merge($ca,$Jc);$Db="COMMENT=".q($cb).($cc?" ENGINE=".q($cc):"").($mb?" COLLATE ".q($mb):"").($Gc!=""?" AUTO_INCREMENT=$Gc":"").$Ob;if($l==""){return
...
<tr><th>Systém<td>',html_select("driver",$ya,DRIVER,"loginDriver(this);"),'<tr><th>Server<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Dc,$u=""){echo'<p class="tabs">';$ha=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Dc)){$ha["view"]='Zmeniť pohľad';}else{$ha["create"]='Zmeniť tabuľku';}if(isset($u)){$ha["edit"]='Nová položka';}foreach($ha
...
selectQuery($j){global$r;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Posledná stránka'."'>&gt;&gt;</a> <code class='jush-$r'>".h(str_replace("\n"," ",$j))."</code> <a href='".h(ME)."sql=".urlencode($j)."'>".'Upraviť'."</a></p>\n";}function
rowDescription($l){return"";}function
rowDescriptions($F,$wf){return$F;}function
selectVal($b,$B,$d){$e=($b!="<i>NULL</i>"&&ereg("char|binary",$d["type"])&&!ereg("var",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$e=lang(array('%d bajt','%d bajty','%d bajtov'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($B?"<a href='$B'>$e</a>":$e);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($y,$w){global$X,$Ab;print_fieldset("select",'Vypísať',$y);$g=0;$Rd=array('Funkcie'=>$X,'Agregácia'=>$Ab);foreach($y
...
selectSearchPrint($x,$w,$z){print_fieldset("search",'Vyhľadať',$x);foreach($z
as$g=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$g]' value='".h($_GET["fulltext"][$g])."'>",checkbox("boolean[$g]",1,isset($_GET["boolean"][$g]),"BOOL"),"<br>\n";}}$g=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$g][col]'><option value=''>(".'kdekoľvek'.")".optionlist($w,$b["col"],true)."</select>",html_select("where[$g][op]",$this->operators,$b["op"]),"<input name='where[$g][val]' value='".h($b["val"])."'></div>\n";$g++;}}echo"<div><select name='where[$g][col]' onchange='selectAddRow(this);'><option value=''>(".'kdekoľvek'.")".optionlist($w,null,true)."</select>",html_select("where[$g][op]",$this->operators,"="),"<input name='where[$g][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Ma,$w,$z){print_fieldset("sort",'Zotriediť',$Ma);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$g]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'zostupne')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>","<label><input type='checkbox' name='desc[$g]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($W){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($W)."'>","</div></fieldset>\n";}function
selectLengthPrint($pb){if(isset($pb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($pb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$r;static$Ic=0;restart_session();$E="sql-".($Ic++);$Ia=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Ia[$_GET["db"]][]=$j;return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL príkaz'."</a><div id='$E' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Ia[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
editFunctions($d){global$gd;$e=($d["null"]?"NULL/":"");foreach($gd
...
dumpHeaders($Pc,$id=false){$_a=$_POST["output"];$vb=($_POST["format"]=="sql"?"sql":($id?"tar":"csv"));header("Content-Type: ".($_a=="bz2"?"application/x-bzip":($_a=="gz"?"application/x-gzip":($vb=="tar"?"application/x-tar":($vb=="sql"||$_a!="file"?"text/plain":"text/csv")."; charset=utf-8"))));if($_a=="bz2"){ob_start('bzcompress',1e6);}if($_a=="gz"){ob_start('gzencode',1e6);}return$vb;}function
...
';if($Fb=="auth"){$qa=true;foreach((array)$_SESSION["pwds"]as$ub=>$Xe){foreach($Xe
as$J=>$Ye){foreach($Ye
...
';if($Fb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$Fb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$ia=tables_list();if(!$ia){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($ia);$ha=array();foreach($ia
...
page_header($Ee,$q="",$Vb=array(),$Dd=""){global$Cf,$n,$i,$ya;header("Content-Type: text/html; charset=utf-8");if($n->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}$xd=$Ee.($Dd!=""?": ".h($Dd):"");$Ld=strip_tags($xd.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$n->name());if(is_ajax()){header("X-AJAX-Title: ".rawurlencode($Ld));}else{echo'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN">
...
long2str($s,$sd){$U='';foreach($s
as$b){$U.=pack('V',$b);}if($sd){return
substr($U,0,end($s));}return$U;}function
str2long($U,$sd){$s=array_values(unpack('V*',str_pad($U,4*ceil(strlen($U)/4),"\0")));if($sd){$s[]=strlen($U);}return$s;}function
...
long2str($s,true);}$i='';$D=$_SESSION["token"];if(!$_SESSION["token"]){$_SESSION["token"]=rand(1,1e6);}$fb=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$b){list($c)=explode(":",$b);$fb[$c]=$b;}}if(isset($_POST["server"])){session_regenerate_id();$_SESSION["pwds"][$_POST["driver"]][$_POST["server"]][$_POST["username"]]=$_POST["password"];if($_POST["permanent"]){$c=base64_encode($_POST["driver"])."-".base64_encode($_POST["server"])."-".base64_encode($_POST["username"]);$Cc=$n->permanentLogin();$fb[$c]="$c:".base64_encode($Cc?encrypt_string($_POST["password"],$Cc):"");cookie("adminer_permanent",implode(" ",$fb));}if(count($_POST)==($_POST["permanent"]?5:4)||DRIVER!=$_POST["driver"]||SERVER!=$_POST["server"]||$_GET["username"]!==$_POST["username"]){redirect(auth_url($_POST["driver"],$_POST["server"],$_POST["username"]));}}elseif($_POST["logout"]){if($D&&$_POST["token"]!=$D){page_header('Odhlásiť','Neplatný token CSRF. Odošlite formulár znova.');page_footer("db");exit;}else{foreach(array("pwds","dbs","queries")as$c){set_session($c,null);}$c=base64_encode(DRIVER)."-".base64_encode(SERVER)."-".base64_encode($_GET["username"]);if($fb[$c]){unset($fb[$c]);cookie("adminer_permanent",implode(" ",$fb));}redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),'Odhlásenie prebehlo v poriadku.');}}elseif($fb&&!$_SESSION["pwds"]){session_regenerate_id();$Cc=$n->permanentLogin();foreach($fb
as$c=>$b){list(,$af)=explode(":",$b);list($ub,$J,$Z)=array_map('base64_decode',explode("-",$c));$_SESSION["pwds"][$ub][$J][$Z]=decrypt_string(base64_decode($af),$Cc);}}function
auth_error($yd=null){global$i,$n,$D;$Bc=session_name();$q="";if(!$_COOKIE[$Bc]&&$_GET[$Bc]&&ini_bool("session.use_only_cookies")){$q='Session premenné musia byť povolené.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Bc]||$_GET[$Bc])&&!$D){$q='Session vypršala, prihláste sa prosím znova.';}else{$oa=&get_session("pwds");if(isset($oa)){$q=h($yd?$yd->getMessage():(is_string($i)?$i:'Neplatné prihlasovacie údaje.'));$oa=null;}}}page_header('Prihlásiť sa',$q,null);echo"<form action='' method='post' onclick='eventStop(event);'>\n";$n->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);page_header('Žiadne rozšírenie',sprintf('Nie je dostupné žiadne z podporovaných rozšírení (%s).',implode(", ",$bf)),false);page_footer("auth");exit;}$i=connect();}if(is_string($i)||!$n->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$D=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$D;}$q=($_POST?($_POST["token"]==$D?"":'Neplatný token CSRF. Odošlite formulár znova.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Príliš veľké POST dáta. Zmenšite dáta alebo zvýšte hodnotu konfiguračej direktívy %s.','"post_max_size"')));function
connect_error(){global$i,$D,$q,$ya;$_=array();if(DB!=""){page_header('Databáza'.": ".h(DB),'Nesprávna databáza.',true);}else{if($_POST["db"]&&!$q){queries_redirect(substr(ME,0,-1),'Databázy boli odstránené.',drop_databases($_POST["db"]));}page_header('Vybrať databázu',$q,false);echo"<p><a href='".h(ME)."database='>".'Vytvoriť novú databázu'."</a>\n";foreach(array('privileges'=>'Oprávnenia','processlist'=>'Zoznam procesov','variables'=>'Premenné','status'=>'Stav',)as$c=>$b){if(support($c)){echo"<a href='".h(ME)."$c='>$b</a>\n";}}echo"<p>".sprintf('Verzia %s: %s cez PHP rozšírenie %s',$ya[DRIVER],"<b>$i->server_info</b>","<b>$i->extension</b>")."\n","<p>".sprintf('Prihlásený ako: %s',"<b>".h(logged_user())."</b>")."\n";if($_GET["refresh"]){set_session("dbs",null);}$_=get_databases();if($_){$gf=support("scheme");$S=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td>&nbsp;<th>".'Databáza'."<td>".'Porovnávanie'."<td>".'Tabuľky'."</thead>\n";foreach($_
as$v){$ud=h(ME)."db=".urlencode($v);echo"<tr".odd()."><td>".checkbox("db[]",$v,in_array($v,(array)$_POST["db"])),"<th><a href='$ud'>".h($v)."</a>","<td><a href='$ud".($gf?"&amp;ns=":"")."&amp;database='>".nbsp(db_collation($v,$S))."</a>","<td align='right'><a href='$ud&amp;schema=' id='tables-".h($v)."'>?</a>","\n";}echo"</table>\n","<p><input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /db/)",1).">\n";echo"<input type='hidden' name='token' value='$D'>\n","<a href='".h(ME)."refresh=1' onclick='eventStop(event);'>".'Obnoviť'."</a>\n","</form>\n";}}page_footer("db");if($_){echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}}if(isset($_GET["status"])){$_GET["variables"]=$_GET["status"];}if(!(DB!=""?$i->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect")){if(DB!=""){set_session("dbs",null);}connect_error();exit;}function
select($k,$G=null,$wc=""){$ha=array();$z=array();$w=array();$Ke=array();$ua=array();odd('');for($g=0;$a=$k->fetch_row();$g++){if(!$g){echo"<table cellspacing='0' class='nowrap'>\n","<thead><tr>";for($T=0;$T<count($a);$T++){$d=$k->fetch_field();$f=$d->name;$ta=$d->orgtable;$Mb=$d->orgname;if($wc){$ha[$T]=($f=="table"?"table=":($f=="possible_keys"?"indexes=":null));}elseif($ta!=""){if(!isset($z[$ta])){$z[$ta]=array();foreach(indexes($ta,$G)as$t){if($t["type"]=="PRIMARY"){$z[$ta]=array_flip($t["columns"]);break;}}$w[$ta]=$z[$ta];}if(isset($w[$ta][$Mb])){unset($w[$ta][$Mb]);$z[$ta][$Mb]=$T;$ha[$T]=$ta;}}if($d->charsetnr==63){$Ke[$T]=true;}$ua[$T]=$d->type;$f=h($f);echo"<th".($ta!=""||$d->name!=$Mb?" title='".h(($ta!=""?"$ta.":"").$Mb)."'":"").">".($wc?"<a href='$wc".strtolower($f)."' target='_blank' rel='noreferrer'>$f</a>":$f);}echo"</thead>\n";}echo"<tr".odd().">";foreach($a
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($Ke[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($ua[$c]==254){$b="<code>$b</code>";}}if(isset($ha[$c])&&!$w[$ha[$c]]){if($wc){$B=$ha[$c].urlencode($a[array_search("table=",$ha)]);}else{$B="edit=".urlencode($ha[$c]);foreach($z[$ha[$c]]as$vc=>$T){$B.="&where".urlencode("[".bracket_escape($vc)."]")."=".urlencode($a[$T]);}}$b="<a href='".h(ME.$B)."'>$b</a>";}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($c,$d,$S,$M=array()){global$Fc,$ua,$wb,$Ga;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($ua[$d["type"]])?array():array($d["type"]))+$Fc+($M?array('Cudzie kľúče'=>$M):array()),$d["type"]),'</select>
...
edit_fields($p,$S,$N="TABLE",$te=0,$M=array(),$jb=false){global$yb;echo'<thead><tr class="wrap">
...
',(support("comment")?"<td".($jb?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$g=>$d){$g++;$Xc=$d[($_POST?"orig":"field")];$ue=(isset($_POST["add"][$g-1])||(isset($d["field"])&&!$_POST["drop_col"][$g]))&&(support("drop_col")||$Xc=="");echo'<tr',($ue?"":" style='display: none;'"),'>
',($N=="PROCEDURE"?"<td>".html_select("fields[$g][inout]",explode("|",$yb),$d["inout"]):""),'<th>';if($ue){echo'<input name="fields[',$g,'][field]" value="',h($d["field"]),'" onchange="',($d["field"]!=""||count($p)>1?"":"editingAddRow(this, $te); "),'editingNameChange(this);" maxlength="64">';}echo'<input type="hidden" name="fields[',$g,'][orig]" value="',h($Xc),'">
...
',(support("comment")?"<td".($jb?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.0' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $te, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.0' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.0' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Xc==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.0' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
drop_create($xe,$da,$O,$we,$lf,$of,$f){if($_POST["drop"]){return
query_redirect($xe,$O,$we,true,!$_POST["dropped"]);}$Ja=$f!=""&&($_POST["dropped"]||queries($xe));$pf=queries($da);if(!queries_redirect($O,($f!=""?$lf:$of),$pf)&&$Ja){redirect(null,$we);}return$Ja;}function
tar_file($kb,$Wc){$e=pack("a100a8a8a8a12a12",$kb,644,0,0,decoct(strlen($Wc)),decoct(time()));$he=8*32;for($g=0;$g<strlen($e);$g++){$he+=ord($e{$g});}$e.=sprintf("%06o",$he)."\0 ";return$e.str_repeat("\0",512-strlen($e)).$Wc.str_repeat("\0",511-(strlen($Wc)+511)%
512);}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Ga=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$lb="'(?:''|[^'\\\\]|\\\\.)*+'";$yb="IN|OUT|INOUT";if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["callf"])){$_GET["call"]=$_GET["callf"];}if(isset($_GET["function"])){$_GET["procedure"]=$_GET["function"];}if(isset($_GET["download"])){$h=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$h-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$i->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($h)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["table"])){$h=$_GET["table"];$p=fields($h);if(!$p){$q=error();}$H=($p?table_status($h):array());page_header(($p&&is_view($H)?'Pohľad':'Tabuľka').": ".h($h),$q);$n->selectLinks($H);$cb=$H["Comment"];if($cb!=""){echo"<p>".'Komentár'.": ".h($cb)."\n";}if($p){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Stĺpec'."<td>".'Typ'.(support("comment")?"<td>".'Komentár':"")."</thead>\n";foreach($p
as$d){echo"<tr".odd()."><th>".h($d["field"]),"<td title='".h($d["collation"])."'>".h($d["full_type"]).($d["null"]?" <i>NULL</i>":"").($d["auto_increment"]?" <i>".'Auto Increment'."</i>":""),(isset($d["default"])?" [<b>".h($d["default"])."</b>]":""),(support("comment")?"<td>".nbsp($d["comment"]):""),"\n";}echo"</table>\n";if(!is_view($H)){echo"<h3>".'Indexy'."</h3>\n";$z=indexes($h);if($z){echo"<table cellspacing='0'>\n";foreach($z
as$f=>$t){ksort($t["columns"]);$hb=array();foreach($t["columns"]as$c=>$b){$hb[]="<i>".h($b)."</i>".($t["lengths"][$c]?"(".$t["lengths"][$c].")":"");}echo"<tr title='".h($f)."'><th>$t[type]<td>".implode(", ",$hb)."\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'indexes='.urlencode($h).'">'.'Zmeniť indexy'."</a>\n";if(fk_support($H)){echo"<h3>".'Cudzie kľúče'."</h3>\n";$M=foreign_keys($h);if($M){echo"<table cellspacing='0'>\n","<thead><tr><th>".'Zdroj'."<td>".'Cieľ'."<td>".'ON DELETE'."<td>".'ON UPDATE'.($r!="sqlite"?"<td>&nbsp;":"")."</thead>\n";foreach($M
as$f=>$C){echo"<tr title='".h($f)."'>","<th><i>".implode("</i>, <i>",array_map('h',$C["source"]))."</i>","<td><a href='".h($C["db"]!=""?preg_replace('~db=[^&]*~',"db=".urlencode($C["db"]),ME):($C["ns"]!=""?preg_replace('~ns=[^&]*~',"ns=".urlencode($C["ns"]),ME):ME))."table=".urlencode($C["table"])."'>".($C["db"]!=""?"<b>".h($C["db"])."</b>.":"").($C["ns"]!=""?"<b>".h($C["ns"])."</b>.":"").h($C["table"])."</a>","(<i>".implode("</i>, <i>",array_map('h',$C["target"]))."</i>)","<td>".nbsp($C["on_delete"])."\n","<td>".nbsp($C["on_update"])."\n";if($r!="sqlite"){echo'<td><a href="'.h(ME.'foreign='.urlencode($h).'&name='.urlencode($f)).'">'.'Zmeniť'.'</a>';}}echo"</table>\n";}if($r!="sqlite"){echo'<p><a href="'.h(ME).'foreign='.urlencode($h).'">'.'Pridať cudzí kľúč'."</a>\n";}}if(support("trigger")){echo"<h3>".'Triggery'."</h3>\n";$Ac=triggers($h);if($Ac){echo"<table cellspacing='0'>\n";foreach($Ac
as$c=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($c)."<td><a href='".h(ME.'trigger='.urlencode($h).'&name='.urlencode($c))."'>".'Zmeniť'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($h).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB);$db=array();$_e=array();$f="adminer_schema";$me=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$me,$fa,PREG_SET_ORDER);foreach($fa
as$g=>$m){$db[$m[1]]=array($m[2],$m[3]);$_e[]="\n\t'".js_escape($m[1])."': [ $m[2], $m[3] ]";}$Ib=0;$He=-1;$Wa=array();$Ne=array();$Le=array();foreach(table_status()as$a){if(!isset($a["Engine"])){continue;}$Zb=0;$Wa[$a["Name"]]["fields"]=array();foreach(fields($a["Name"])as$f=>$d){$Zb+=1.25;$d["pos"]=$Zb;$Wa[$a["Name"]]["fields"][$f]=$d;}$Wa[$a["Name"]]["pos"]=($db[$a["Name"]]?$db[$a["Name"]]:array($Ib,0));foreach($n->foreignKeys($a["Name"])as$b){if(!$b["db"]){$ka=$He;if($db[$a["Name"]][1]||$db[$b["table"]][1]){$ka=min(floatval($db[$a["Name"]][1]),floatval($db[$b["table"]][1]))-1;}else{$He-=.1;}while($Le[(string)$ka]){$ka-=.0001;}$Wa[$a["Name"]]["references"][$b["table"]][(string)$ka]=array($b["source"],$b["target"]);$Ne[$b["table"]][$a["Name"]][(string)$ka]=$b["target"];$Le[(string)$ka]=true;}}$Ib=max($Ib,$Wa[$a["Name"]]["pos"][0]+2.5+$Zb);}echo'<div id="schema" style="height: ',$Ib,'em;">
<script type="text/javascript">
tablePos = {',implode(",",$_e)."\n",'};
...
as$ka=>$ic){$_c=$Ib;$Sc=-10;foreach($ic[0]as$c=>$xa){$Ie=$l["pos"][0]+$l["fields"][$xa]["pos"];$Ce=$Wa[$Hb]["pos"][0]+$Wa[$Hb]["fields"][$ic[1][$c]]["pos"];$_c=min($_c,$Ie,$Ce);$Sc=max($Sc,$Ie,$Ce);}echo"<div class='references' id='refl$ka' style='left: $ka"."em; top: $_c"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($Sc-$_c)."em;'></div></div>\n";}}}echo'</div>
<p><a href="',h(ME."schema=".urlencode($me)),'" id="schema-link">Permanentný odkaz</a>
';}elseif(isset($_GET["dump"])){$h=$_GET["dump"];if($_POST){$Be="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$c){$Be.="&$c=".urlencode($_POST[$c]);}cookie("adminer_export",substr($Be,1));$vb=dump_headers(($h!=""?$h:DB),(DB==""||count((array)$_POST["tables"]+(array)$_POST["data"])>1));$Ya=($_POST["format"]=="sql");if($Ya){echo"-- Adminer $fc ".$ya[DRIVER]." dump
...
<table cellspacing="0">
';$Fe=array('','USE','DROP+CREATE','CREATE');$ne=array('','DROP+CREATE','CREATE');$Zd=array('','TRUNCATE+INSERT','INSERT');if($r=="sql"){$Fe[]='CREATE+ALTER';$ne[]='CREATE+ALTER';$Zd[]='INSERT+UPDATE';}parse_str($_COOKIE["adminer_export"],$a);if(!$a){$a=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");}if(!isset($a["events"])){$a["routines"]=$a["events"]=($_GET["dump"]=="");$a["triggers"]=$a["table_style"];}echo"<tr><th>".'Výstup'."<td>".html_select("output",$n->dumpOutput(),$a["output"],0)."\n";echo"<tr><th>".'Formát'."<td>".html_select("format",$n->dumpFormat(),$a["format"],0)."\n";echo($r=="sqlite"?"":"<tr><th>".'Databáza'."<td>".html_select('db_style',$Fe,$a["db_style"]).(support("routine")?checkbox("routines",1,$a["routines"],'Procedúry'):"").(support("event")?checkbox("events",1,$a["events"],'Udalosti'):"")),"<tr><th>".'Tabuľky'."<td>".html_select('table_style',$ne,$a["table_style"]).checkbox("auto_increment",1,$a["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$a["triggers"],'Triggery'):""),"<tr><th>".'Dáta'."<td>".html_select('data_style',$Zd,$a["data_style"]),'</table>
<p><input type="submit" value="Export">

<table cellspacing="0">
';$rd=array();if(DB!=""){$Oa=($h!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Oa onclick='formCheck(this, /^tables\\[/);'>".'Tabuľky'."</label>","<th style='text-align: right;'><label>".'Dáta'."<input type='checkbox' id='check-data'$Oa onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Qa="";foreach(table_status()as$a){$f=$a["Name"];$ec=ereg_replace("_.*","",$f);$Oa=($h==""||$h==(substr($h,-1)=="%"?"$ec%":$f));$hb="<tr><td>".checkbox("tables[]",$f,$Oa,$f,"formUncheck('check-tables');");if(is_view($a)){$Qa.="$hb\n";}else{echo"$hb<td align='right'><label>".($a["Engine"]=="InnoDB"&&$a["Rows"]?"~ ":"").$a["Rows"].checkbox("data[]",$f,$Oa,"","formUncheck('check-data');")."</label>\n";}$rd[$ec]++;}echo$Qa;}else{echo"<thead><tr><th style='text-align: left;'><label><input type='checkbox' id='check-databases'".($h==""?" checked":"")." onclick='formCheck(this, /^databases\\[/);'>".'Databáza'."</label></thead>\n";$_=get_databases();if($_){foreach($_
as$v){if(!information_schema($v)){$ec=ereg_replace("_.*","",$v);echo"<tr><td>".checkbox("databases[]",$v,$h==""||$h=="$ec%",$v,"formUncheck('check-databases');")."</label>\n";$rd[$ec]++;}}}else{echo"<tr><td><textarea name='databases' rows='10' cols='20'></textarea>";}}echo'</table>
</form>
';$qa=true;foreach($rd
as$c=>$b){if($c!=""&&$b>1){echo($qa?"<p>":" ")."<a href='".h(ME)."dump=".urlencode("$c%")."'>".h($c)."</a>";$qa=false;}}}elseif(isset($_GET["privileges"])){page_header('Oprávnenia');$k=$i->query("SELECT User, Host FROM mysql.".(DB==""?"user":"db WHERE ".q($_GET["db"])." LIKE Db")." ORDER BY Host, User");if(!$k){echo'<form action=""><p>
...
<input type="hidden" name="token" value="',$D,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$bd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$bd[]=".$c";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($bd?"[".implode("|",$bd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Ia){print_fieldset("history",'História',$_GET["history"]!="");foreach($Ia
as$c=>$b){echo'<a href="'.h(ME."sql=&history=$c").'">'.'Upraviť'."</a> <code class='jush-$r'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistiť'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upraviť všetko'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$Fa?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$O=$_POST["referer"];if($_POST["insert"]){$O=($Fa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$O)){$O=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$O,'Položka bola vymazaná.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Fa?"\n".idf_escape($f)." = $b":$b);}}if($Fa){if(!$u){redirect($O);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$O,'Položka bola aktualizovaná.');}else{$k=insert_into($h,$u);$Bd=($k?last_id():0);queries_redirect($O,sprintf('Položka%s bola vložená.',($Bd?" $Bd":"")),$k);}}}$ra=$n->tableName(table_status($h));page_header(($Fa?'Upraviť':'Vložiť'),$q,array("select"=>array($h,$ra)),$ra);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$y=array();foreach($p
...
as$ra=>$d){$M[str_replace("`","``",$ra)."`".str_replace("`","``",$d["field"])]=$ra;}$tc=array();$sc=array();if($h!=""){$tc=fields($h);$sc=table_status($h);}if($_POST&&!$_POST["fields"]){$_POST["fields"]=array();}if($_POST&&!$q&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"]){if($_POST["drop"]){query_redirect("DROP TABLE ".table($h),substr(ME,0,-1),'Tabuľka bola odstránená.');}else{$p=array();$Jc=array();ksort($_POST["fields"]);$dd=reset($tc);$sb="FIRST";foreach($_POST["fields"]as$c=>$d){$C=$M[$d["type"]];$xc=(isset($C)?$_d[$C]:$d);if($d["field"]!=""){if(!$d["has_default"]){$d["default"]=null;}$La=eregi_replace(" *on update CURRENT_TIMESTAMP","",$d["default"]);if($La!=$d["default"]){$d["on_update"]="CURRENT_TIMESTAMP";$d["default"]=$La;}if($c==$_POST["auto_increment_col"]){$d["auto_increment"]=true;}$Md=process_field($d,$xc);if($Md!=process_field($dd,$dd)){$p[]=array($d["orig"],$Md,$sb);}if(isset($C)){$Jc[idf_escape($d["field"])]=($h!=""?"ADD":" ")." FOREIGN KEY (".idf_escape($d["field"]).") REFERENCES ".table($M[$d["type"]])." (".idf_escape($xc["field"]).")".(in_array($d["on_delete"],$Ga)?" ON DELETE $d[on_delete]":"");}$sb="AFTER ".idf_escape($d["field"]);}elseif($d["orig"]!=""){$p[]=array($d["orig"]);}if($d["orig"]!=""){$dd=next($tc);}}$Ob="";if(in_array($_POST["partition_by"],$Ad)){$md=array();if($_POST["partition_by"]=='RANGE'||$_POST["partition_by"]=='LIST'){foreach(array_filter($_POST["partition_names"])as$c=>$b){$o=$_POST["partition_values"][$c];$md[]="\nPARTITION ".idf_escape($b)." VALUES ".($_POST["partition_by"]=='RANGE'?"LESS THAN":"IN").($o!=""?" ($o)":" MAXVALUE");}}$Ob.="\nPARTITION BY $_POST[partition_by]($_POST[partition])".($md?" (".implode(",",$md)."\n)":($_POST["partitions"]?" PARTITIONS ".(+$_POST["partitions"]):""));}elseif($h!=""&&support("partitioning")){$Ob.="\nREMOVE PARTITIONING";}$ga='Tabuľka bola zmenená.';if($h==""){cookie("adminer_engine",$_POST["Engine"]);$ga='Tabuľka bola vytvorená.';}queries_redirect(ME."table=".urlencode($_POST["name"]),$ga,alter_table($h,$_POST["name"],$p,$Jc,$_POST["Comment"],($_POST["Engine"]&&$_POST["Engine"]!=$sc["Engine"]?$_POST["Engine"]:""),($_POST["Collation"]&&$_POST["Collation"]!=$sc["Collation"]?$_POST["Collation"]:""),($_POST["Auto_increment"]!=""?+$_POST["Auto_increment"]:""),$Ob));}}page_header(($h!=""?'Zmeniť tabuľku':'Vytvoriť tabuľku'),$q,array("table"=>$h),$h);$a=array("Engine"=>$_COOKIE["adminer_engine"],"fields"=>array(array("field"=>"","type"=>(isset($ua["int"])?"int":(isset($ua["integer"])?"integer":"")))),"partition_names"=>array(""),);if($_POST){$a=$_POST;if($a["auto_increment_col"]){$a["fields"][$a["auto_increment_col"]]["auto_increment"]=true;}process_fields($a["fields"]);}elseif($h!=""){$a=$sc;$a["name"]=$h;$a["fields"]=array();if(!$_GET["auto_increment"]){$a["Auto_increment"]="";}foreach($tc
as$d){$d["has_default"]=isset($d["default"]);if($d["on_update"]){$d["default"].=" ON UPDATE $d[on_update]";}$a["fields"][]=$d;}if(support("partitioning")){$Eb="FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = ".q(DB)." AND TABLE_NAME = ".q($h);$k=$i->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $Eb ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");list($a["partition_by"],$a["partitions"],$a["partition"])=$k->fetch_row();$a["partition_names"]=array();$a["partition_values"]=array();foreach(get_rows("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $Eb AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION")as$ae){$a["partition_names"][]=$ae["PARTITION_NAME"];$a["partition_values"][]=$ae["PARTITION_DESCRIPTION"];}$a["partition_names"][]="";}}$S=collations();$ld=floor(extension_loaded("suhosin")?(min(ini_get("suhosin.request.max_vars"),ini_get("suhosin.post.max_vars"))-13)/10:0);if($ld&&count($a["fields"])>$ld){echo"<p class='error'>".h(sprintf('Bol prekročený maximálny počet povolených polí. Zvýšte prosím %s a %s.','suhosin.post.max_vars','suhosin.request.max_vars'))."\n";}$nd=engines();foreach($nd
as$cc){if(!strcasecmp($cc,$a["Engine"])){$a["Engine"]=$cc;break;}}echo'
<form action="" method="post" id="form">
<p>
Názov tabuľky: <input name="name" maxlength="64" value="',h($a["name"]),'">
';if($h==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($nd?html_select("Engine",array(""=>"(".'úložisko'.")")+$nd,$a["Engine"]):""),' ',($S&&!ereg("sqlite|mssql",$r)?html_select("Collation",array(""=>"(".'porovnávanie'.")")+$S,$a["Collation"]):""),' <input type="submit" value="Uložiť">
<table cellspacing="0" id="edit-fields" class="nowrap">
';$jb=($_POST?$_POST["comments"]:$a["Comment"]!="");if(!$_POST&&!$jb){foreach($a["fields"]as$d){if($d["comment"]!=""){$jb=true;break;}}}edit_fields($a["fields"],$S,"TABLE",$ld,$M,$jb);echo'</table>
<p>
...
',(support("comment")?checkbox("comments",1,$jb,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($jb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložiť">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
';if(support("partitioning")){$de=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Rozdeliť podľa',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Ad,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Oddiely: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($de||!$a["partition_by"]?" class='hidden'":""),'>
...
as$v){if(count($_)==1||$v!=""){if(!create_database($v,$_POST["collation"])){$ce=false;}$Xa=$v;}}queries_redirect(ME."db=".urlencode($Xa),'Databáza bola vytvorená.',$ce);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$q,array(),DB);$S=collations();$f=DB;$Jb=null;if($_POST){$f=$_POST["name"];$Jb=$_POST["collation"];}elseif(DB!=""){$Jb=db_collation(DB,$S);}elseif($r=="sql"){foreach(get_vals("SHOW GRANTS")as$Q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Q,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($S?html_select("collation",array(""=>"(".'porovnávanie'.")")+$S,$Jb):"");?>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
';}elseif(isset($_GET["view"])){$h=$_GET["view"];$Ja=false;if($_POST&&!$q){$Ja=drop_create("DROP VIEW ".table($h),"CREATE VIEW ".table($_POST["name"])." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_POST["name"])),'Pohľad bol odstránený.','Pohľad bol zmenený.','Pohľad bol vytvorený.',$h);}page_header(($h!=""?'Zmeniť pohľad':'Vytvoriť pohľad'),$q,array("table"=>$h),$h);$a=array();if($_POST){$a=$_POST;}elseif($h!=""){$a=view($h);$a["name"]=$h;}echo'
<form action="" method="post">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
<p>';textarea("select",$a["select"]);echo'<p>
';if($Ja){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Uložiť">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
';}elseif(isset($_GET["event"])){$Ta=$_GET["event"];$Od=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$jd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ta),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Od)&&isset($jd[$_POST["STATUS"]])){$Qd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ta!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($Ta!=""?"ALTER EVENT ".idf_escape($Ta).$Qd.($Ta!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Qd)."\n".$jd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ta!=""?'Upraviť udalosť'.": ".h($Ta):'Vytvoriť udalosť'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ta!=""){$F=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ta));$a=reset($F);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Od,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$jd,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($Ta!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
';}elseif(isset($_GET["procedure"])){$Ba=$_GET["procedure"];$Ea=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$fd=routine_languages();$Ja=false;if($_POST&&!$q&&!$_POST["add"]&&!$_POST["drop_col"]&&!$_POST["up"]&&!$_POST["down"]){$u=array();$p=(array)$_POST["fields"];ksort($p);foreach($p
as$d){if($d["field"]!=""){$u[]=(ereg("^($yb)\$",$d["inout"])?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Ja=drop_create("DROP $Ea ".idf_escape($Ba),"CREATE $Ea ".idf_escape($_POST["name"])." (".implode(", ",$u).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$fd)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedúra bola odstránená.','Procedúra bola zmenená.','Procedúra bola vytvorená.',$Ba);}page_header(($Ba!=""?(isset($_GET["function"])?'Zmeniť funkciu':'Zmeniť procedúru').": ".h($Ba):(isset($_GET["function"])?'Vytvoriť funkciu':'Vytvoriť procedúru')),$q);$S=get_vals("SHOW CHARACTER SET");sort($S);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Ba!=""){$a=routine($Ba,$Ea);$a["name"]=$Ba;}echo'
<form action="" method="post" id="form">
<p>Názov: <input name="name" value="',h($a["name"]),'" maxlength="64">
',($fd?'Jazyk'.": ".html_select("language",$fd,$a["language"]):""),'<table cellspacing="0" class="nowrap">
...
<input type="submit" value="Uložiť">
';if($Ba!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Ja){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
';}elseif(isset($_GET["trigger"])){$h=$_GET["trigger"];$lc=trigger_options();$Ud=array("INSERT","UPDATE","DELETE");$Ja=false;if($_POST&&!$q&&in_array($_POST["Timing"],$lc["Timing"])&&in_array($_POST["Event"],$Ud)&&in_array($_POST["Type"],$lc["Type"])){$Td=" $_POST[Timing] $_POST[Event]";$ib=" ON ".table($h);$Ja=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($r=="pgsql"?$ib:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($r=="mssql"?$ib.$Td:$Td.$ib).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($h),'Trigger bol odstránený.','Trigger bol zmenený.','Trigger bol vytvorený.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmeniť trigger'.": ".h($_GET["name"]):'Vytvoriť trigger'),$q,array("table"=>$h));$a=array("Trigger"=>$h."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Čas<td>',html_select("Timing",$lc["Timing"],$a["Timing"],"if (/^".h(preg_quote($h,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(js_escape($h))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Udalosť<td>',html_select("Event",$Ud,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$lc["Type"],$a["Type"]),'</table>
<p>Názov: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Uložiť">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Ja){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$g=0;foreach($Va
as$Ha=>$Q){echo'<th>'.($Ha!="*.*"?"<input name='objects[$g]' value='".h($Ha)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$kc=>$Wb){foreach((array)$aa[$kc]as$hc=>$cb){echo"<tr".odd()."><td".($Wb?">$Wb<td":" colspan='2'").' lang="en" title="'.h($cb).'">'.h($hc);$g=0;foreach($Va
...
<input type="submit" value="Uložiť">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$q){$qd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$qd++;}}queries_redirect(ME."processlist=",lang(array('Bol ukončený %d proces.','Boli ukončené %d procesy.','Bolo ukončených %d procesov.'),$qd),$qd||!$_POST["kill"]);}page_header('Zoznam procesov',$q);echo'
<form action="" method="post">
...
as$c=>$b){echo"<td>".($c=="Info"&&$b!=""?"<code class='jush-$r'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Upraviť'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>',($g+1)."/".sprintf('%d celkom',$i->result("SELECT @@max_connections")),'<p>
<input type="submit" value="Ukončiť">
<input type="hidden" name="token" value="',$D,'">
</form>
';}elseif(isset($_GET["select"])){$h=$_GET["select"];$H=table_status($h);$z=indexes($h);$p=fields($h);$M=column_foreign_keys($h);if($H["Oid"]=="t"){$z[]=array("type"=>"PRIMARY","columns"=>array("oid"));}parse_str($_COOKIE["adminer_import"],$nc);$fe=array();$w=array();$pb=null;foreach($p
as$c=>$d){$f=$n->fieldName($d);if(isset($d["privileges"]["select"])&&$f!=""){$w[$c]=html_entity_decode(strip_tags($f));if(ereg('text|lob',$d["type"])){$pb=$n->selectLengthProcess();}}$fe+=$d["privileges"];}list($y,$Y)=$n->selectColumnsProcess($w,$z);$x=$n->selectSearchProcess($p,$z);$Ma=$n->selectOrderProcess($p,$z);$W=$n->selectLimitProcess();$Eb=($y?implode(", ",$y):($H["Oid"]=="t"?"oid, ":"")."*")."\nFROM ".table($h);$kd=($Y&&count($Y)<count($y)?"\nGROUP BY ".implode(", ",$Y):"").($Ma?"\nORDER BY ".implode(", ",$Ma):"");if($_GET["val"]&&is_ajax()){header("Content-Type: text/plain; charset=utf-8");foreach($_GET["val"]as$Pa=>$a){echo$i->result("SELECT".limit(idf_escape(key($a))." FROM ".table($h)," WHERE ".where_check($Pa).($x?" AND ".implode(" AND ",$x):"").($Ma?" ORDER BY ".implode(", ",$Ma):""),1));}exit;}if($_POST&&!$q){$zd="(".implode(") OR (",array_map('where_check',(array)$_POST["check"])).")";$Lc=$Qb=null;foreach($z
as$t){if($t["type"]=="PRIMARY"){$Lc=array_flip($t["columns"]);$Qb=($y?$Lc:array());break;}}foreach((array)$Qb
as$c=>$b){if(in_array(idf_escape($c),$y)){unset($Qb[$c]);}}if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($h);$n->dumpTable($h,"");if(!is_array($_POST["check"])||$Qb===array()){$Cb=$x;if(is_array($_POST["check"])){$Cb[]="($zd)";}$j="SELECT $Eb".($Cb?"\nWHERE ".implode(" AND ",$Cb):"").$kd;}else{$Kd=array();foreach($_POST["check"]as$b){$Kd[]="(SELECT".limit($Eb,"\nWHERE ".($x?implode(" AND ",$x)." AND ":"").where_check($b).$kd,1).")";}$j=implode(" UNION ALL ",$Kd);}$n->dumpData($h,"table",$j);exit;}if(!$n->selectEmailProcess($x,$M)){if($_POST["save"]||$_POST["delete"]){$k=true;$ab=0;$j=table($h);$u=array();if(!$_POST["delete"]){foreach($w
as$f=>$b){$b=process_input($p[$f]);if($b!==null){if($_POST["clone"]){$u[idf_escape($f)]=($b!==false?$b:idf_escape($f));}elseif($b!==false){$u[]=idf_escape($f)." = $b";}}}$j.=($_POST["clone"]?" (".implode(", ",array_keys($u)).")\nSELECT ".implode(", ",$u)."\nFROM ".table($h):" SET\n".implode(",\n",$u));}if($_POST["delete"]||$u){$oc="UPDATE";if($_POST["delete"]){$oc="DELETE";$j="FROM $j";}if($_POST["clone"]){$oc="INSERT";$j="INTO $j";}if($_POST["all"]||($Qb===array()&&$_POST["check"])||count($Y)<count($y)){$k=queries($oc." $j".($_POST["all"]?($x?"\nWHERE ".implode(" AND ",$x):""):"\nWHERE $zd"));$ab=$i->affected_rows;}else{foreach((array)$_POST["check"]as$b){$k=queries($oc.limit1($j,"\nWHERE ".where_check($b)));if(!$k){break;}$ab+=$i->affected_rows;}}}queries_redirect(remove_from_uri("page"),sprintf('%d položiek bolo ovplyvnených.',$ab),$k);}elseif(!$_POST["import"]){if(!$_POST["val"]){$q='Dvojkliknite na políčko, ktoré chcete zmeniť.';}else{$k=true;$ab=0;foreach($_POST["val"]as$Pa=>$a){$u=array();foreach($a
as$c=>$b){$c=bracket_escape($c,1);$u[]=idf_escape($c)." = ".(ereg('char|text',$p[$c]["type"])||$b!=""?$n->processInput($p[$c],$b):"NULL");}$j=table($h)." SET ".implode(", ",$u);$Cb=" WHERE ".where_check($Pa).($x?" AND ".implode(" AND ",$x):"");$k=queries("UPDATE".(count($Y)<count($y)?" $j$Cb":limit1($j,$Cb)));if(!$k){break;}$ab+=$i->affected_rows;}queries_redirect(remove_from_uri(),sprintf('%d položiek bolo ovplyvnených.',$ab),$k);}}elseif(is_string($va=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($nc["output"])."&format=".urlencode($_POST["separator"]));$k=true;$Sa=array_keys($p);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$va,$fa);$ab=count($fa[0]);begin();$ac=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($fa[0]as$c=>$b){preg_match_all("~((\"[^\"]*\")+|[^$ac]*)$ac~",$b.$ac,$od);if(!$c&&!array_diff($od[1],$Sa)){$Sa=$od[1];$ab--;}else{$u=array();foreach($od[1]as$g=>$vc){$u[idf_escape($Sa[$g])]=($vc==""&&$p[$Sa[$g]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$vc))));}$k=insert_update($h,$u,$Lc);if(!$k){break;}}}if($k){queries("COMMIT");}queries_redirect(remove_from_uri("page"),lang(array('Bol importovaný %d záznam.','Boli importované %d záznamy.','Bolo importovaných %d záznamov.'),$ab),$k);queries("ROLLBACK");}else{$q=upload_error($va);}}}$ra=$n->tableName($H);page_header('Vypísať'.": $ra",$q);session_write_close();$u=null;if(isset($fe["insert"])){$u="";foreach((array)$_GET["where"]as$b){if(count($M[$b["col"]])==1&&($b["op"]=="="||(!$b["op"]&&!ereg('[_%]',$b["val"])))){$u.="&set".urlencode("[".bracket_escape($b["col"])."]")."=".urlencode($b["val"]);}}}$n->selectLinks($H,$u);if(!$w){echo"<p class='error'>".'Tabuľku sa nepodarilo vypísať'.($p?".":": ".error())."\n";}else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($h).'">',"</div>\n";$n->selectColumnsPrint($y,$w);$n->selectSearchPrint($x,$w,$z);$n->selectOrderPrint($Ma,$w,$z);$n->selectLimitPrint($W);$n->selectLengthPrint($pb);$n->selectActionPrint();echo"</form>\n";$K=$_GET["page"];if($K=="last"){$Ka=$i->result("SELECT COUNT(*) FROM ".table($h).($x?" WHERE ".implode(" AND ",$x):""));$K=floor(max(0,$Ka-1)/$W);}$j="SELECT".limit((+$W&&$Y&&count($Y)<count($y)&&$r=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Eb,($x?"\nWHERE ".implode(" AND ",$x):"").$kd,($W!=""?+$W:null),($K?$W*$K:0),"\n");echo$n->selectQuery($j);$k=$i->query($j);if(!$k){echo"<p class='error'>".error()."\n";}else{if($r=="mssql"){$k->seek($W*$K);}$rc=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$F=array();while($a=$k->fetch_assoc()){if($K&&$r=="oracle"){unset($a["RNUM"]);}$F[]=$a;}if($_GET["page"]!="last"){$Ka=(+$W&&$Y&&count($Y)<count($y)?($r=="sql"?$i->result(" SELECT FOUND_ROWS()"):$i->result("SELECT COUNT(*) FROM ($j) x")):count($F));}if(!$F){echo"<p class='message'>".'Žiadne riadky.'."\n";}else{$Uc=$n->backwardKeys($h,$ra);echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Y&&$y?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'upraviť'."</a>");$Rc=array();$X=array();reset($y);$De=1;foreach($F[0]as$c=>$b){if($H["Oid"]!="t"||$c!="oid"){$b=$_GET["columns"][key($y)];$d=$p[$y?($b?$b["col"]:current($y)):$c];$f=($d?$n->fieldName($d,$De):"*");if($f!=""){$De++;$Rc[$c]=$f;$ea=idf_escape($c);echo'<th><a href="'.h(remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($c).($Ma[0]==$ea||$Ma[0]==$c||(!$Ma&&count($Y)<count($y)&&$Y[0]==$ea)?'&desc%5B0%5D=1':'')).'">'.(!$y||$b?apply_sql_function($b["fun"],$f):h(current($y)))."</a>";}$X[$c]=$b["fun"];next($y);}}$Bb=array();if($_GET["modify"]){foreach($F
as$a){foreach($a
as$c=>$b){$Bb[$c]=max($Bb[$c],min(40,strlen(utf8_decode($b))));}}}echo($Uc?"<th>".'Vzťahy':"")."</thead>\n";foreach($n->rowDescriptions($F,$M)as$P=>$a){$cd=unique_array($F[$P],$z);$Pa="";foreach($cd
as$c=>$b){$Pa.="&".(isset($b)?urlencode("where[".bracket_escape($c)."]")."=".urlencode($b):"null%5B%5D=".urlencode($c));}echo"<tr".odd().">".(!$Y&&$y?"":"<td>".checkbox("check[]",substr($Pa,1),in_array(substr($Pa,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($Y)<count($y)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($h).$Pa)."'>".'upraviť'."</a>"));foreach($a
as$c=>$b){if(isset($Rc[$c])){$d=$p[$c];if($b!=""&&(!isset($rc[$c])||$rc[$c]!="")){$rc[$c]=(is_mail($b)?$Rc[$c]:"");}$B="";$b=$n->editVal($b,$d);if(!isset($b)){$b="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$d["type"])&&$b!=""){$B=h(ME.'download='.urlencode($h).'&field='.urlencode($c).$Pa);}if($b===""){$b="&nbsp;";}elseif($pb!=""&&ereg('text|blob',$d["type"])&&is_utf8($b)){$b=shorten_utf8($b,max(0,+$pb));}else{$b=h($b);}if(!$B){foreach((array)$M[$c]as$C){if(count($M[$c])==1||end($C["source"])==$c){$B="";foreach($C["source"]as$g=>$xa){$B.=where_link($g,$C["target"][$g],$F[$P][$xa]);}$B=h(($C["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($C["db"]),ME):ME).'select='.urlencode($C["table"]).$B);if(count($C["source"])==1){break;}}}}if($c=="COUNT(*)"){$B=h(ME."select=".urlencode($h));$g=0;foreach((array)$_GET["where"]as$s){if(!array_key_exists($s["col"],$cd)){$B.=h(where_link($g++,$s["col"],$s["val"],$s["op"]));}}foreach($cd
as$za=>$s){$B.=h(where_link($g++,$za,$s));}}}if(!$B){if(is_mail($b)){$B="mailto:$b";}if($Oe=is_url($a[$c])){$B=($Oe=="http"&&$Nb?$a[$c]:"$Oe://www.adminer.org/redirect/?url=".urlencode($a[$c]));}}$E=h("val[$Pa][".bracket_escape($c)."]");$o=$_POST["val"][$Pa][bracket_escape($c)];$Je=h(isset($o)?$o:$a[$c]);$Te=strpos($b,"<i>...</i>");$ze=is_utf8($b)&&$F[$P][$c]==$a[$c]&&!$X[$c];$le=ereg('text|lob',$d["type"]);echo(($_GET["modify"]&&$ze)||isset($o)?"<td>".($le?"<textarea name='$E' cols='30' rows='".(substr_count($a[$c],"\n")+1)."'>$Je</textarea>":"<input name='$E' value='$Je' size='$Bb[$c]'>"):"<td id='$E' ondblclick=\"".($ze?"selectDblClick(this, event".($Te?", 2":($le?", 1":"")).")":"alert('".h('Pre zmenu tejto hodnoty použite odkaz upraviť.')."')").";\">".$n->selectVal($b,$B,$d));}}if($Uc){echo"<td>";}$n->backwardKeysPrint($Uc,$F[$P]);echo"</tr>\n";}echo"</table>\n";}if($F||$K){$ad=true;if($_GET["page"]!="last"&&+$W&&count($Y)>=count($y)&&($Ka>=$W||$K)){$Ka=$H["Rows"];if(!isset($Ka)||$x||($H["Engine"]=="InnoDB"&&$Ka<max(1e4,2*($K+1)*$W))){ob_flush();flush();$Ka=$i->result("SELECT COUNT(*) FROM ".table($h).($x?" WHERE ".implode(" AND ",$x):""));}else{$ad=false;}}echo"<p class='pages'>";if(+$W&&$Ka>$W){$Yc=floor(($Ka-1)/$W);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Stránka'."', '".($K+1)."'), event); return false;\">".'Stránka'."</a>:",pagination(0,$K).($K>5?" ...":"");for($g=max(1,$K-4);$g<min($Yc,$K+5);$g++){echo
pagination($g,$K);}echo($K+5<$Yc?" ...":"").($ad?pagination($Yc,$K):' <a href="'.h(remove_from_uri()."&page=last").'">'.'posledný'."</a>");}echo" (".($ad?"":"~ ").lang(array('%d riadok','%d riadky','%d riadkov'),$Ka).") ".checkbox("all",1,0,'celý výsledok')."\n";if($n->selectCommandPrint()){?>
...
as$c=>$b){json_row("sum-$c",number_format($b,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$v=>$b){json_row("tables-".js_escape($v),$b);}json_row("");}exit;}else{$qe=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($qe&&!$q&&!$_POST["search"]){$k=true;$ga="";if($r=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$k=truncate_tables($_POST["tables"]);}$ga='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$k=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ga='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$k=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ga='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"]){$k=drop_views($_POST["views"]);}if($k&&$_POST["tables"]){$k=drop_tables($_POST["tables"]);}$ga='Tabuľka bola odstránená.';}elseif($_POST["tables"]&&($k=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$k->fetch_assoc()){$ga.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ga,$k);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$q,true);if($n->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$pc=tables_list();if(!$pc){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabuľka'.'<td>'.'Typ'.'<td>'.'Porovnávanie'.'<td>'.'Veľkosť dát'.'<td>'.'Veľkosť indexu'.'<td>'.'Voľné miesto'.'<td>'.'Auto Increment'.'<td>'.'Riadky'.(support("comment")?'<td>'.'Komentár':'')."</thead>\n";foreach($pc
as$f=>$N){$xb=(isset($N)&&!eregi("table",$N));echo'<tr'.odd().'><td>'.checkbox(($xb?"views[]":"tables[]"),$f,in_array($f,$qe,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($xb){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{foreach(array("Engine"=>"","Collation"=>"","Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$c=>$B){echo($B?"<td align='right'><a href='".h(ME."$B=").urlencode($f)."' id='$c-".h($f)."'>?</a>":"<td id='$c-".h($f)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($pc)),"<td>".nbsp($r=="sql"?$i->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$c){echo"<td align='right' id='sum-$c'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p>".($r=="sql"?"<input type='submit' value='".'Analyzovať'."'> <input type='submit' name='optimize' value='".'Optimalizovať'."'> <input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$_=(support("scheme")?schemas():get_databases());if(count($_)!=1&&$r!="sqlite"){$v=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($_?html_select("target",$_,$v):'<input name="target" value="'.h($v).'">')," <input type='submit' name='move' value='".'Presunúť'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$D'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";}if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$se=routines();if($se){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($se
