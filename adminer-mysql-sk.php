...
confirm($Pc="",$Re=false){return" onclick=\"".($Re?"eventStop(event); ":"")."return confirm('".'Naozaj?'.($Pc?" (' + $Pc + ')":"")."');\"";}function
...
enum_input($O,$Ea,$d,$o,$Eb=null){global$n;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$ea);$e=(isset($Eb)?"<label><input type='$O'$Ea value='$Eb'".((is_array($o)?in_array($Eb,$o):$o===0)?" checked":"")."><i>".'prázdne'."</i></label>":"");foreach($ea[1]as$g=>$b){$b=stripcslashes(str_replace("''","'",$b));$Qa=(is_int($o)?$o==$g+1:(is_array($o)?in_array($g+1,$o):$o===$b));$e.=" <label><input type='$O'$Ea value='".($g+1)."'".($Qa?' checked':'').'>'.h($n->editVal($b,$d)).'</label>';}return$e;}function
input($d,$o,$_){global$sa,$n,$r;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$oe=($r=="mssql"&&$d["auto_increment"]);if($oe&&!$_POST["save"]){$_=null;}$X=(isset($_GET["select"])||$oe?array("orig"=>'originál'):array())+$n->editFunctions($d);$Ea=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
selectLinks($Ic,$u=""){echo'<p class="tabs">';$ga=array("select"=>'Vypísať dáta',"table"=>'Zobraziť štruktúru');if(is_view($Ic)){$ga["view"]='Zmeniť pohľad';}else{$ga["create"]='Zmeniť tabuľku';}if(isset($u)){$ga["edit"]='Nová položka';}foreach($ga
...
selectColumnsPrint($z,$w){global$X,$Cb;print_fieldset("select",'Vypísať',$z);$g=0;$ie=array('Funkcie'=>$X,'Agregácia'=>$Cb);foreach($z
...
selectSearchPrint($x,$w,$y){print_fieldset("search",'Vyhľadať',$x);foreach($y
...
selectOrderPrint($Ga,$w,$y){print_fieldset("sort",'Zotriediť',$Ga);$g=0;foreach((array)$_GET["order"]as$c=>$b){if(isset($w[$b])){echo"<div><select name='order[$g]'><option>".optionlist($w,$b,true)."</select>",checkbox("desc[$g]",1,isset($_GET["desc"][$c]),'zostupne')."</div>\n";$g++;}}echo"<div><select name='order[$g]' onchange='selectAddRow(this);'><option>".optionlist($w,null,true)."</select>","<label><input type='checkbox' name='desc[$g]' value='1'>".'zostupne'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($W){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($W)."'>","</div></fieldset>\n";}function
selectLengthPrint($qb){if(isset($qb)){echo"<fieldset><legend>".'Dĺžka textov'."</legend><div>",'<input name="text_length" size="3" value="'.h($qb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$r;static$Pc=0;restart_session();$E="sql-".($Pc++);$Pa=&get_session("queries");if(strlen($j)>1e6){$j=ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...";}$Pa[$_GET["db"]][]=$j;return" <a href='#$E' onclick=\"return !toggle('$E');\">".'SQL príkaz'."</a><div id='$E' class='hidden'><pre><code class='jush-$r'>".shorten_utf8($j,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Pa[$_GET["db"]])-1)).'">'.'Upraviť'.'</a></div>';}function
...
explode("/",$e);}function
editInput($l,$d,$Ea,$o){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ea value='-1' checked><i>".'originál'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ea value=''".(isset($o)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ea,$d,$o,0);}return"";}function
...
';if($xb!="db"&&DB!=""&&$i->select_db(DB)){if($_GET["ns"]!==""&&!$xb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvoriť novú tabuľku'."</a>\n";$qa=tables_list();if(!$qa){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{$this->tablesPrint($qa);$ga=array();foreach($qa
...
as$l=>$O){echo'<a href="'.h(ME).'select='.urlencode($l).'"'.bold($_GET["select"]==$l).">".'vypísať'."</a> ",'<a href="'.h(ME).'table='.urlencode($l).'"'.bold($_GET["table"]==$l).">".$this->tableName(array("Name"=>$l))."</a><br>\n";}}}$n=(function_exists('adminer_object')?adminer_object():new
...
as$c=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($xe[$c]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtov'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($sa[$c]==254){$b="<code>$b</code>";}}if(isset($ga[$c])&&!$w[$ga[$c]]){if($zc){$B=$ga[$c].urlencode($a[array_search("table=",$ga)]);}else{$B="edit=".urlencode($ga[$c]);foreach($y[$ga[$c]]as$Ac=>$T){$B.="&where".urlencode("[".bracket_escape($Ac)."]")."=".urlencode($a[$T]);}}$b="<a href='".h(ME.$B)."'>$b</a>";}echo"<td>$b";}}echo($g?"</table>":"<p class='message'>".'Žiadne riadky.')."\n";}function
...
edit_type($c,$d,$S,$M=array()){global$Jc,$sa,$Bb,$La;echo'<td><select name="',$c,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($sa[$d["type"]])?array():array($d["type"]))+$Jc+($M?array('Cudzie kľúče'=>$M):array()),$d["type"]),'</select>
...
';if($O=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($O=="TABLE"?'Názov stĺpca':'Názov parametra'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Dĺžka
<td>Voľby
...
',(support("comment")?"<td".($ib?"":" class='hidden'").">".'Komentár':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1' alt='+' title='".'Pridať ďalší'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($ib?"":" class='hidden'")."><input name='fields[$g][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1' alt='+' title='".'Pridať ďalší'."' onclick='return !editingAddRow(this, $Nd, 1);'>&nbsp;"."<input type='image' name='up[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.1' alt='^' title='".'Presunúť hore'."'>&nbsp;"."<input type='image' name='down[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.1' alt='v' title='".'Presunúť dolu'."'>&nbsp;":""),($Rc==""||support("drop_col")?"<input type='image' name='drop_col[$g]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.1' alt='x' title='".'Odobrať'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$c=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($c)."<td><a href='".h(ME.'trigger='.urlencode($h).'&name='.urlencode($c))."'>".'Zmeniť'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($h).'">'.'Pridať trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schéma databázy',"",array(),DB);$Za=array();$De=array();$f="adminer_schema";$Be=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$Be,$ea,PREG_SET_ORDER);foreach($ea
...
<p><a href="',h(ME."schema=".urlencode($Be)),'" id="schema-link">Permanentný odkaz</a>
...
<input type="hidden" name="token" value="',$D,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastaviť pri chybe')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobraziť iba chyby')."\n";print_fieldset("webfile",'Zo serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Xc=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$c=>$b){if(extension_loaded($b)){$Xc[]=".$c";}}echo
sprintf('Súbor %s na webovom serveri',"<code>adminer.sql".($Xc?"[".implode("|",$Xc)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustiť súbor'.'">',"</div></fieldset>\n";if($Pa){print_fieldset("history",'História',$_GET["history"]!="");foreach($Pa
...
as$f=>$d){if(!isset($d["privileges"][$Da?"update":"insert"])||$n->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$q&&!isset($_GET["select"])){$N=$_POST["referer"];if($_POST["insert"]){$N=($Da?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$N)){$N=ME."select=".urlencode($h);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($h)," WHERE $x"),$N,'Položka bola vymazaná.');}else{$u=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$u[idf_escape($f)]=($Da?"\n".idf_escape($f)." = $b":$b);}}if($Da){if(!$u){redirect($N);}query_redirect("UPDATE".limit1(table($h)." SET".implode(",",$u),"\nWHERE $x"),$N,'Položka bola aktualizovaná.');}else{$k=insert_into($h,$u);$Ld=($k?last_id():0);queries_redirect($N,sprintf('Položka%s bola vložená.',($Ld?" $Ld":"")),$k);}}}$ua=$n->tableName(table_status($h));page_header(($Da?'Upraviť':'Vložiť'),$q,array("select"=>array($h,$ua)),$ua);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($x){$z=array();foreach($p
...
';if($p){echo"<input type='submit' value='".'Uložiť'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Da?'Uložiť a pokračovať v úpravách':'Uložiť a vložiť ďalší')."' title='Ctrl+Shift+Enter'>\n";}}echo($Da?"<input type='submit' name='delete' value='".'Zmazať'."' onclick=\"return confirm('".'Naozaj?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$ib,'Komentár',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($ib?'':' class="hidden"').'>':''),'<p>
...
as$v){if(count($A)==1||$v!=""){if(!create_database($v,$_POST["collation"])){$xd=false;}$Ya=$v;}}queries_redirect(ME."db=".urlencode($Ya),'Databáza bola vytvorená.',$xd);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáza bola zmenená.');}}page_header(DB!=""?'Zmeniť databázu':'Vytvoriť databázu',$q,array(),DB);$S=collations();$f=DB;$hc=null;if($_POST){$f=$_POST["name"];$hc=$_POST["collation"];}elseif(DB!=""){$hc=db_collation(DB,$S);}elseif($r=="sql"){foreach(get_vals("SHOW GRANTS")as$Q){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Q,$m)&&$m[1]){$f=stripcslashes(idf_unescape("`$m[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($S?html_select("collation",array(""=>"(".'porovnávanie'.")")+$S,$hc):"");?>
...
';}elseif(isset($_GET["event"])){$Ra=$_GET["event"];$Ad=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$md=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$q){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($Ra),substr(ME,0,-1),'Udalosť bola odstránená.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Ad)&&isset($md[$_POST["STATUS"]])){$Dd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($Ra!=""?'Udalosť bola zmenená.':'Udalosť bola vytvorená.'),queries(($Ra!=""?"ALTER EVENT ".idf_escape($Ra).$Dd.($Ra!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Dd)."\n".$md[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($Ra!=""?'Upraviť udalosť'.": ".h($Ra):'Vytvoriť udalosť'),$q);$a=array();if($_POST){$a=$_POST;}elseif($Ra!=""){$F=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($Ra));$a=reset($F);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ad,$a["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$md,$a["STATUS"]),'<tr><th>Komentár<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložiť">
';if($Ra!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
...
<input type="submit" value="Uložiť">
';if($Ca!=""){echo'<input type="submit" name="drop" value="Odstrániť"',confirm(),'>';}if($Na){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$D,'">
</form>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($i->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnenia'."</a>";$g=0;foreach($Ta
as$Ja=>$Q){echo'<th>'.($Ja!="*.*"?"<input name='objects[$g]' value='".h($Ja)."' size='10'>":"<input type='hidden' name='objects[$g]' value='*.*' size='10'>*.*");$g++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáza',"Tables"=>'Tabuľka',"Columns"=>'Stĺpec',"Procedures"=>'Procedúra',)as$xc=>$Nb){foreach((array)$aa[$xc]as$Ub=>$db){echo"<tr".odd()."><td".($Nb?">$Nb<td":" colspan='2'").' lang="en" title="'.h($db).'">'.h($Ub);$g=0;foreach($Ta
...
as$c=>$b){json_row("sum-$c",number_format($b,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$v=>$b){json_row("tables-".js_escape($v),$b);}json_row("");}exit;}else{$yd=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($yd&&!$q&&!$_POST["search"]){$k=true;$ha="";if($r=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$k=truncate_tables($_POST["tables"]);}$ha='Tabuľka bola vyprázdnená.';}elseif($_POST["move"]){$k=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ha='Tabuľka bola presunutá.';}elseif($_POST["copy"]){$k=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ha='Tabuľky boli skopírované.';}elseif($_POST["drop"]){if($_POST["views"]){$k=drop_views($_POST["views"]);}if($k&&$_POST["tables"]){$k=drop_tables($_POST["tables"]);}$ha='Tabuľka bola odstránená.';}elseif($_POST["tables"]&&($k=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$k->fetch_assoc()){$ha.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ha,$k);}page_header(($_GET["ns"]==""?'Databáza'.": ".h(DB):'Schéma'.": ".h($_GET["ns"])),$q,true);if($n->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabuľky a pohľady'."</h3>\n";$ic=tables_list();if(!$ic){echo"<p class='message'>".'Žiadne tabuľky.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Vyhľadať dáta v tabuľkách'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Vyhľadať'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabuľka'.'<td>'.'Typ'.'<td>'.'Porovnávanie'.'<td>'.'Veľkosť dát'.'<td>'.'Veľkosť indexu'.'<td>'.'Voľné miesto'.'<td>'.'Auto Increment'.'<td>'.'Riadky'.(support("comment")?'<td>'.'Komentár':'')."</thead>\n";foreach($ic
as$f=>$O){$wb=(isset($O)&&!eregi("table",$O));echo'<tr'.odd().'><td>'.checkbox(($wb?"views[]":"tables[]"),$f,in_array($f,$yd,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($wb){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'Pohľad'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{foreach(array("Engine"=>"","Collation"=>"","Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$c=>$B){echo($B?"<td align='right'><a href='".h(ME."$B=").urlencode($f)."' id='$c-".h($f)."'>?</a>":"<td id='$c-".h($f)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d celkom',count($ic)),"<td>".nbsp($r=="sql"?$i->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$c){echo"<td align='right' id='sum-$c'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p>".($r=="sql"?"<input type='submit' value='".'Analyzovať'."'> <input type='submit' name='optimize' value='".'Optimalizovať'."'> <input type='submit' name='check' value='".'Skontrolovať'."'> <input type='submit' name='repair' value='".'Opraviť'."'> ":"")."<input type='submit' name='truncate' value='".'Vyprázdniť'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Odstrániť'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$A=(support("scheme")?schemas():get_databases());if(count($A)!=1&&$r!="sqlite"){$v=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Presunúť do inej databázy'.": ",($A?html_select("target",$A,$v):'<input name="target" value="'.h($v).'">')," <input type='submit' name='move' value='".'Presunúť'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopírovať'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$D'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Vytvoriť tabuľku'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Vytvoriť pohľad'."</a>\n";}if(support("routine")){echo"<h3>".'Procedúry'."</h3>\n";$Hd=routines();if($Hd){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Názov'.'<td>'.'Typ'.'<td>'.'Návratový typ'."<td>&nbsp;</thead>\n";odd('');foreach($Hd
