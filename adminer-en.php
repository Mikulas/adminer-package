...
upload_error($n){$Qe=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Unable to upload a file.'.($Qe?" ".sprintf('Maximum allowed file size is %sB.',$Qe):""):'File does not exist.');}function
...
lang($Se,$hc){$qc=($hc==1||(!$hc&&'en'=='fr')?0:((!$hc||$hc>=5)&&ereg('cs|sk|ru','en')?2:1));return
...
false;}}return
true;}function
rename_database($f,$R){global$g;$g->Min_SQLite(":memory:");$g->error='File exists.';return@rename(DB,$f);}function
...
selectLinks($Wc,$q=""){echo'<p class="tabs">';$Ia=array("select"=>'Select data',"table"=>'Show structure');if(is_view($Wc)){$Ia["view"]='Alter view';}else{$Ia["create"]='Alter table';}if(isset($q)){$Ia["edit"]='New item';}foreach($Ia
...
selectColumnsPrint($C,$B){global$V,$ob;print_fieldset("select",'Select',$C);$k=0;$ge=array('Functions'=>$V,'Aggregation'=>$ob);foreach($C
...
selectOrderPrint($ub,$B,$K){print_fieldset("sort",'Sort',$ub);$k=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$k]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$k]",1,isset($_GET["desc"][$e]),'descending')."</div>\n";$k++;}}echo"<div><select name='order[$k]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>",checkbox("desc[$k]",1,0,'descending')."</div>\n","</div></fieldset>\n";}function
selectLimitPrint($L){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($L)."'>","</div></fieldset>\n";}function
selectLengthPrint($Bb){if(isset($Bb)){echo"<fieldset><legend>".'Text length'."</legend><div>",'<input name="text_length" size="3" value="'.h($Bb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($j){global$_;restart_session();$U="sql-".count($_SESSION["messages"]);$lb=&get_session("queries");$lb[$_GET["db"]][]=(strlen($j)>1e6?ereg_replace('[\x80-\xFF]+$','',substr($j,0,1e6))."\n...":$j);return" <a href='#$U' onclick=\"return !toggle('$U');\">".'SQL command'."</a><div id='$U' class='hidden'><pre class='jush-$_'>".shorten_utf8($j,1000).'</pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($lb[$_GET["db"]])-1)).'">'.'Edit'.'</a></div>';}function
...
explode("/",$d);}function
editInput($h,$c,$Pa,$p){if($c["type"]=="enum"){return($c["null"]?"<label><input type='radio'$Pa value=''".(isset($p)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"")."<label><input type='radio'$Pa value='0'".($p===0?" checked":"")."><i>".'empty'."</i></label>";}return"";}function
...
';if($Lc!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schema'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""){$E=tables_list();if(!$E){echo"<p class='message'>".'No tables.'."\n";}else{$this->tablesPrint($E);$Ia=array();foreach($E
...
auth_error($le=null){global$g,$r,$P;$Fc=session_name();$n="";if(!$_COOKIE[$Fc]&&$_GET[$Fc]&&ini_bool("session.use_only_cookies")){$n='Session support must be enabled.';}elseif(isset($_GET["username"])){if(($_COOKIE[$Fc]||$_GET[$Fc])&&!$P){$n='Session expired, please login again.';}else{$S=&get_session("pwds");if(isset($S)){$n=h($le?$le->getMessage():(is_string($g)?$g:'Invalid credentials.'));$S=null;}}}page_header('Login',$n,null);echo"<form action='' method='post'>\n";$r->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])&&class_exists("Min_DB")){$g=connect();}if(is_string($g)||!$r->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$P=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$P;}$n=($_POST?($_POST["token"]==$P?"":'Invalid CSRF token. Send the form again.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Too big POST data. Reduce the data or increase the %s configuration directive.','"post_max_size"')));function
connect_error(){global$g,$P,$n,$ja;$z=array();if(DB!=""){page_header('Database'.": ".h(DB),'Invalid database.',true);}else{if($_POST["db"]&&!$n){set_session("dbs",null);queries_redirect(substr(ME,0,-1),'Databases have been dropped.',drop_databases($_POST["db"]));}page_header('Select database',$n,false);echo"<p><a href='".h(ME)."database='>".'Create new database'."</a>\n";foreach(array('privileges'=>'Privileges','processlist'=>'Process list','variables'=>'Variables','status'=>'Status',)as$e=>$b){if(support($e)){echo"<a href='".h(ME)."$e='>$b</a>\n";}}echo"<p>".sprintf('%s version: %s through PHP extension %s',$ja[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Logged as: %s',"<b>".h(logged_user())."</b>")."\n";$z=get_databases();if($z){$X=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' onclick='tableClick(event);'>\n","<thead><tr><td><input type='hidden' name='token' value='$P'>&nbsp;<th>".'Database'."<td>".'Collation'."<td>".'Tables'."</thead>\n";foreach($z
...
edit_type($e,$c,$X,$ba=array()){global$Sa,$T,$pb,$db;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$c["type"]||isset($T[$c["type"]])?array():array($c["type"]))+$Sa+($ba?array('Foreign keys'=>$ba):array()),$c["type"]),'</select>
...
',(support("comment")?"<td".($Fb?"":" class='hidden'").">".'Comment':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Add next'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
';foreach($o
...
',(support("comment")?"<td".($Fb?"":" class='hidden'")."><input name='fields[$k][comment]' value='".h($c["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Add next'."' onclick='return !editingAddRow(this, $me, 1);'>&nbsp;"."<input type='image' name='up[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=up.gif&amp;version=3.0.0' alt='^' title='".'Move up'."'>&nbsp;"."<input type='image' name='down[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=down.gif&amp;version=3.0.0' alt='v' title='".'Move down'."'>&nbsp;":""),($td==""||support("drop_col")?"<input type='image' name='drop_col[$k]' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=cross.gif&amp;version=3.0.0' alt='x' title='".'Remove'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}return$Fb;}function
...
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Alter'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Add trigger'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Database schema',"",array(),DB);$rb=array();$Ie=array();preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$_COOKIE["adminer_schema"],$ra,PREG_SET_ORDER);foreach($ra
...
',checkbox("error_stops",1,$_POST["error_stops"],'Stop on error');print_fieldset("webfile",'From server',$_POST["webfile"]);$cd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$cd[]=".$e";}}echo
sprintf('Webserver file %s',"<code>adminer.sql".($cd?"[".implode("|",$cd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Run file'.'">',"</div></fieldset>\n";if($lb){print_fieldset("history",'History',$_GET["history"]!="");foreach($lb
...
as$f=>$c){if(!isset($c["privileges"][$la?"update":"insert"])||$r->fieldName($c)==""){unset($o[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$ka=$_POST["referer"];if($_POST["insert"]){$ka=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$ka)){$ka=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$ka,'Item has been deleted.');}else{$q=array();foreach($o
as$f=>$c){$b=process_input($c);if($b!==false&&$b!==null){$q[idf_escape($f)]=($la?"\n".idf_escape($f)." = $b":$b);}}if($la){if(!$q){redirect($ka);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$q),"\nWHERE $t"),$ka,'Item has been updated.');}else{$i=insert_into($m,$q);$Xd=($i?last_id():0);queries_redirect($ka,sprintf('Item%s has been inserted.',($Xd?" $Xd":"")),$i);}}}$Fa=$r->tableName(table_status($m));page_header(($la?'Edit':'Insert'),$n,array("select"=>array($m,$Fa)),$Fa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$C=array();foreach($o
...
<?php echo(support("comment")?checkbox("","",$Fb,'Comment',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();").' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($Fb?'':' class="hidden"').'>':''),'<p>
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Save">
';if(strlen($_GET["create"])){echo'<input type="submit" name="drop" value="Drop"',$Va,'>';}if(support("partitioning")){$Wd=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partition by',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Rd,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partitions: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($Wd||!$a["partition_by"]?" class='hidden'":""),'>
...
as$s){if(count($z)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$Od=false;}$mb=$s;}}queries_redirect(ME."db=".urlencode($mb),'Database has been created.',$Od);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"])." COLLATE ".q($_POST["collation"]),substr(ME,0,-1),'Database has been altered.');}}page_header(DB!=""?'Alter database':'Create database',$n,array(),DB);$X=collations();$f=DB;$kc=null;if($_POST){$f=$_POST["name"];$kc=$_POST["collation"];}elseif(DB!=""){$kc=db_collation(DB,$X);}elseif($_=="sql"){foreach(get_vals("SHOW GRANTS")as$ga){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ga,$l)&&$l[1]){$f=stripcslashes(idf_unescape("`$l[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input name="name" value="'.h($f).'" maxlength="64">')."\n".($X?html_select("collation",array(""=>"(".'collation'.")")+$X,$kc):""),'<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Save">
';if(DB!=""){echo"<input type='submit' name='drop' value='".'Drop'."'$Va>\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",$_SERVER["REQUEST_URI"]))."?file=plus.gif&amp;version=3.0.0' alt='+' title='".'Add next'."'>\n";}echo'</form>
...
';}if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Drop"',$Va,'>';}echo'<input type="hidden" name="token" value="',$P,'">
</form>
...
';}elseif(isset($_GET["event"])){$gb=$_GET["event"];$Gd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$Dd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($gb),substr(ME,0,-1),'Event has been dropped.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Gd)&&isset($Dd[$_POST["STATUS"]])){$Md="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";query_redirect(($gb!=""?"ALTER EVENT ".idf_escape($gb).$Md.($gb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Md)."\n".$Dd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"])." DO\n$_POST[EVENT_DEFINITION]",substr(ME,0,-1),($gb!=""?'Event has been altered.':'Event has been created.'));}}page_header(($gb!=""?'Alter event'.": ".h($gb):'Create event'),$n);$a=array();if($_POST){$a=$_POST;}elseif($gb!=""){$D=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($gb));$a=reset($D);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Every<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Gd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Dd,$a["STATUS"]),'<tr><th>Comment<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Save">
';if($gb!=""){echo'<input type="submit" name="drop" value="Drop"',$Va,'>';}echo'</form>
...
as$c){if($c["field"]!=""){$q[]=(in_array($c["inout"],$tc)?"$c[inout] ":"").idf_escape($c["field"]).process_type($c,"CHARACTER SET");}}$Za=drop_create("DROP $Ta ".idf_escape($Oa),"CREATE $Ta ".idf_escape($_POST["name"])." (".implode(", ",$q).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"")."\n$_POST[definition]",substr(ME,0,-1),'Routine has been dropped.','Routine has been altered.','Routine has been created.',$Oa);}page_header(($Oa!=""?(isset($_GET["function"])?'Alter function':'Alter procedure').": ".h($Oa):(isset($_GET["function"])?'Create function':'Create procedure')),$n);$X=get_vals("SHOW CHARACTER SET");sort($X);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Oa!=""){$a=routine($Oa,$Ta);$a["name"]=$Oa;}echo'
<form action="" method="post" id="form">
<p>Name: <input name="name" value="',h($a["name"]),'" maxlength="64">
<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$X,$Ta);if(isset($_GET["function"])){echo"<tr><td>".'Return type';edit_type("returns",$a["returns"],$X);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="hidden" name="token" value="',$P,'">
';if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Save">
';if($Oa!=""){echo'<input type="submit" name="drop" value="Drop"',$Va,'>';}echo'</form>
';}elseif(isset($_GET["sequence"])){$zb=$_GET["sequence"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($zb),$x,'Sequence has been dropped.');}elseif($zb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$x,'Sequence has been created.');}elseif($zb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($zb)." RENAME TO ".idf_escape($_POST["name"]),$x,'Sequence has been altered.');}else{redirect($x);}}page_header($zb!=""?'Alter sequence'.": ".h($zb):'Create sequence',$n);$a=array("name"=>$zb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Save">
';if($zb!=""){echo"<input type='submit' name='drop' value='".'Drop'."'$Va>\n";}echo'</form>
';}elseif(isset($_GET["type"])){$Uc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Uc),$x,'Type has been dropped.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Type has been created.');}}page_header($Uc!=""?'Alter type'.": ".h($Uc):'Create type',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
<input type="hidden" name="token" value="',$P,'">
';if($Uc!=""){echo"<input type='submit' name='drop' value='".'Drop'."'$Va>\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Save'."'>\n";}echo'</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Time<td>',html_select("Timing",$Kc["Timing"],$a["Timing"],"if (/^".h(preg_quote($m,"/"))."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".h(addcslashes($m,"\r\n'\\"))."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Event<td>',html_select("Event",$Ld,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Type<td>',html_select("Type",$Kc["Type"],$a["Type"]),'</table>
<p>Name: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="hidden" name="token" value="',$P,'">
';if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Save">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Drop"',$Va,'>';}echo'</form>
...
as$b){if($b[1]!="USAGE"){$cb["$l[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$cb["$l[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$l)){$Hc=$l[1];}}}if($_POST&&!$n){$Xb=(isset($_GET["host"])?q($zd)."@".q($_GET["host"]):"''");$jb=q($_POST["user"])."@".q($_POST["host"]);$ud=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $Xb",ME."privileges=",'User has been dropped.');}else{if($Xb!=$jb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $jb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $ud");}elseif($_POST["pass"]!=$Hc||!$_POST["hashed"]){queries("SET PASSWORD FOR $jb = ".($_POST["hashed"]?$ud:"PASSWORD($ud)"));}if(!$n){$wc=array();foreach($uc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html'>".'Privileges'."</a>";$k=0;foreach($cb
as$Wa=>$ga){echo'<th>'.($Wa!="*.*"?"<input name='objects[$k]' value='".h($Wa)."' size='10'>":"<input type='hidden' name='objects[$k]' value='*.*' size='10'>*.*");$k++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Database',"Tables"=>'Table',"Columns"=>'Column',"Procedures"=>'Routine',)as$Ac=>$fc){foreach((array)$ma[$Ac]as$dc=>$_a){echo"<tr".odd()."><td".($fc?">$fc<td":" colspan='2'").' lang="en" title="'.h($_a).'">'.h($dc);$k=0;foreach($cb
...
<input type="hidden" name="token" value="',$P,'">
<input type="submit" value="Save">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Drop"',$Va,'>';}echo'</form>
';}elseif(isset($_GET["processlist"])){if($_POST&&!$n){$rd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".ereg_replace("[^0-9]+","",$b))){$rd++;}}queries_redirect(ME."processlist=",lang(array('%d process has been killed.','%d processes have been killed.'),$rd),$rd||!$_POST["kill"]);}page_header('Process list',$n);echo'
<form action="" method="post">
...
as$Ma=>$v){$x.=h(where_link($k++,$Ma,$v,(isset($v)?"=":"IS NULL")));}}}if(!$x){if(is_email($b)){$x="mailto:$b";}if($cc=is_url($a[$e])){$x=($cc=="http"&&$Zb?$a[$e]:"$cc://www.adminer.org/redirect/?url=".urlencode($a[$e]));}}$U=h("val[$sb][".bracket_escape($e)."]");$p=$_POST["val"][$sb][bracket_escape($e)];$Sd=h(isset($p)?$p:$a[$e]);$Ud=strpos($b,"<i>...</i>");$Vd=is_utf8($b)&&!$Ud&&$D[$da][$e]==$a[$e]&&!$V[$e];$ee=ereg('text|lob',$c["type"]);echo(($_GET["modify"]&&$Vd)||isset($p)?"<td>".($ee?"<textarea name='$U' cols='30' rows='".(substr_count($a[$e],"\n")+1)."'>$Sd</textarea>":"<input name='$U' value='$Sd' size='$Ub[$e]'>"):"<td id='$U' ondblclick=\"".($Vd?"selectDblClick(this, event".($ee?", 1":"").")":"alert('".h($Ud?'Increase text length to modify this value.':'Use edit link to modify this value.')."')").";\">".$r->selectVal($b,$x,$c));}}$r->backwardKeysPrint($Pd,$D[$da]);echo"</tr>\n";}echo"</table>\n";}parse_str($_COOKIE["adminer_export"],$md);if($D||$Z){$kd=true;if($_GET["page"]!="last"&&intval($L)&&count($va)>=count($C)&&($Na>=$L||$Z)){$Na=$H["Rows"];if(!isset($Na)||$t||2*$Z*$L>$Na||($H["Engine"]=="InnoDB"&&$Na<1e4)){ob_flush();flush();$Na=$g->result("SELECT COUNT(*) FROM ".table($m).($t?" WHERE ".implode(" AND ",$t):""));}else{$kd=false;}}echo"<p class='pages'>";if(intval($L)&&$Na>$L){$ld=floor(($Na-1)/$L);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"var page = +prompt('".'Page'."', '".($Z+1)."'); if (!isNaN(page) &amp;&amp; page) location.href = this.href + (page != 1 ? '&amp;page=' + (page - 1) : ''); return false;\">".'Page'."</a>:".pagination(0,$Z).($Z>5?" ...":"");for($k=max(1,$Z-4);$k<min($ld,$Z+5);$k++){echo
pagination($k,$Z);}echo($Z+5<$ld?" ...":"").($kd?pagination($ld,$Z):' <a href="'.h(remove_from_uri()."&page=last").'">'.'last'."</a>");}echo" (".($kd?"":"~ ").lang(array('%d row','%d rows'),$Na).") ".checkbox("all",1,0,'whole result')."\n";if(!information_schema(DB)){?>
<fieldset><legend>Edit</legend><div>
<input type="submit" value="Save" title="Double click on a value to modify it.">
<input type="submit" name="edit" value="Edit">
<input type="submit" name="clone" value="Clone">
<input type="submit" name="delete" value="Delete" onclick="return confirm('Are you sure? (' + (this.form['all'].checked ? <?php echo$Na,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Export');echo$r->dumpOutput(1,$md["output"])." ".$r->dumpFormat(1,$md["format"]);echo" <input type='submit' name='export' value='".'Export'."'>\n","</div></fieldset>\n";}print_fieldset("import",'CSV Import',!$D);echo"<input type='hidden' name='token' value='$P'><input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;"),$md["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>\n","</div></fieldset>\n";$r->selectEmailPrint(array_filter($Tc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Pb=isset($_GET["status"]);page_header($Pb?'Status':'Variables');$be=($Pb?show_status():show_variables());if(!$be){echo"<p class='message'>".'No rows.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($be
