...
confirm($gb=""){return" onclick=\"return confirm('".'Czy jesteś pewien?'.($gb?" (' + $gb + ')":"")."');\"";}function
...
upload_error($l){$_d=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Wgranie pliku było niemożliwe.'.($_d?" ".sprintf('Maksymalna wielkość pliku to %sB.',$_d):""):'Plik nie istnieje.');}function
...
enum_input($S,$ya,$m,$X,$Nb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$m["length"],$vd);$H=($Nb!==null?"<label><input type='$S'$ya value='$Nb'".((is_array($X)?in_array($Nb,$X):$X===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($vd[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Na=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$H.=" <label><input type='$S'$ya value='".($q+1)."'".($Na?' checked':'').'>'.h($b->editVal($W,$m)).'</label>';}return$H;}function
input($m,$X,$p){global$h,$T,$b,$v;$A=h(bracket_escape($m["field"]));echo"<td class='function'>";$ff=($v=="mssql"&&$m["auto_increment"]);if($ff&&!$_POST["save"])$p=null;$zc=(isset($_GET["select"])||$ff?array("orig"=>'bez zmian'):array())+$b->editFunctions($m);$ya=" name='fields[$A]'";if($m["type"]=="enum")echo
...
lang($og,$Sd=null){if(is_array($og)){$Ee=($Sd==1?0:($Sd%10>1&&$Sd%10<5&&$Sd/10%10!=1?1:2));$og=$og[$Ee];}$og=str_replace("%d","%s",$og);$Sd=number_format($Sd,0,".",' ');return
...
set_schema($rf){global$h,$T,$If;$H=$h->query("SET search_path TO ".idf_escape($rf));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$If['Typy użytkownika'][]=$S;}}return$H;}function
...
unconvert_field($m,$H){return$H;}function
support($ic){return
ereg("view|scheme|processlist|drop_col|variables|status",$ic);}$v="oracle";$T=array();$If=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$If[$w]=array_keys($W);}$Cg=array();$ee=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$zc=array("length","lower","round","upper");$Dc=array("avg","count","count distinct","max","min","sum");$Hb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ab["mssql"]="MS SQL";if(isset($_GET["mssql"])){$He=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("auth[driver]",$Ab,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Pf,$M=""){echo'<p class="tabs">';$rd=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($Pf))$rd["view"]='Zmień perspektywę';else$rd["create"]='Zmień tabelę';if($M!==null)$rd["edit"]='Nowy rekord';foreach($rd
...
selectQuery($F){global$v,$R;return"<form action='".h(ME)."sql=' method='post'><p><span onclick=\"return !selectEditSql(event, this, '".'Wykonaj'."');\">"."<code class='jush-$v'>".h(str_replace("\n"," ",$F))."</code>"." <a href='".h(ME)."sql=".urlencode($F)."'>".'Edytuj'."</a>"."</span><input type='hidden' name='token' value='$R'></p></form>\n";}function
rowDescription($O){return"";}function
rowDescriptions($J,$qc){return$J;}function
selectLink($W,$m){}function
selectVal($W,$y,$m){$H=($W===null?"<i>NULL</i>":(ereg("char|binary",$m["type"])&&!ereg("var",$m["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$m["type"])&&!is_utf8($W))$H=lang(array('%d bajt','%d bajty','%d bajtów'),strlen(html_entity_decode($W,ENT_QUOTES)));return($y?"<a href='".h($y)."'>$H</a>":$H);}function
editVal($W,$m){return$W;}function
selectColumnsPrint($K,$g){global$zc,$Dc;print_fieldset("select",'pokaż',$K);$q=0;$xc=array('Funkcje'=>$zc,'Agregacje'=>$Dc);foreach($K
...
selectLengthPrint($cg){if($cg!==null){echo"<fieldset><legend>".'Długość tekstu'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($cg)."'>","</div></fieldset>\n";}}function
...
messageQuery($F){global$v;restart_session();$Gc=&get_session("queries");$r="sql-".count($Gc[$_GET["db"]]);if(strlen($F)>1e6)$F=ereg_replace('[\x80-\xFF]+$','',substr($F,0,1e6))."\n...";$Gc[$_GET["db"]][]=array($F,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'Zapytanie SQL'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($F,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Gc[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
<p id="dbs">
';hidden_fields_get();echo($j?'<select name="db" onmousedown="dbMouseDown(event, this);" onchange="dbChange(this);">'.optionlist(array(""=>"(".'baza danych'.")")+$j,DB).'</select>':'<input name="db" value="'.h(DB).'" autocapitalize="off">'),'<input type="submit" value="Wybierz"',($j?" class='hidden'":""),'>
...
as$O=>$Ff){echo'<a href="'.h(ME).'select='.urlencode($O).'"'.bold($_GET["select"]==$O).">".'przeglądaj'."</a> ",'<a href="'.h(ME).'table='.urlencode($O).'"'.bold($_GET["table"]==$O)." title='".'Struktura tabeli'."'>".$this->tableName($Ff)."</a><br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
as$w=>$W){if($W===null)$W="<i>NULL</i>";elseif($Ga[$w]&&!is_utf8($W))$W="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$w]==254)$W="<code>$W</code>";}if(isset($rd[$w])&&!$g[$rd[$w]]){if($Jc){$O=$I[array_search("table=",$rd)];$y=$rd[$w].urlencode($le[$O]!=""?$le[$O]:$O);}else{$y="edit=".urlencode($rd[$w]);foreach($u[$rd[$w]]as$Sa=>$Zc)$y.="&where".urlencode("[".bracket_escape($Sa)."]")."=".urlencode($I[$Zc]);}$W="<a href='".h(ME.$y)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Brak rekordów.')."\n";return$H;}function
...
edit_type($w,$m,$Ua,$rc=array()){global$If,$T,$Cg,$ae;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$m["type"]||isset($T[$m["type"]])?array():array($m["type"]))+$If+($rc?array('Klucze obce'=>$rc):array()),$m["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$m["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($Ua,$m["collation"]).'</select>',($Cg?"<select name='$w"."[unsigned]'".(!$m["type"]||ereg('((^|[^o])int|float|double|decimal)$',$m["type"])?"":" class='hidden'").'><option>'.optionlist($Cg,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='$w"."[on_update]'".($m["type"]=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'W przypadku zmiany'.")","CURRENT_TIMESTAMP"),$m["on_update"]).'</select>':''),($rc?"<select name='$w"."[on_delete]'".(ereg("`",$m["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist(explode("|",$ae),$m["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Za?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.0' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($Za?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.0' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.7.0' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.7.0' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($me==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.7.0' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Zmień'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Dodaj wyzwalacz'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Sf=array();$Tf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$vd,PREG_SET_ORDER);foreach($vd
...
as$A=>$m){if(!isset($m["privileges"][$Dg?"update":"insert"])||$b->fieldName($m)=="")unset($n[$A]);}if($_POST&&!$l&&!isset($_GET["select"])){$z=$_POST["referer"];if($_POST["insert"])$z=($Dg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$z))$z=ME."select=".urlencode($a);$u=indexes($a);$zg=unique_array($_GET["where"],$u);$Ue="\nWHERE $Z";if(isset($_POST["delete"])){$F="FROM ".table($a);query_redirect("DELETE".($zg?" $F$Ue":limit1($F,$Ue)),$z,'Rekord został usunięty.');}else{$M=array();foreach($n
as$A=>$m){$W=process_input($m);if($W!==false&&$W!==null)$M[idf_escape($A)]=($Dg?"\n".idf_escape($A)." = $W":$W);}if($Dg){if(!$M)redirect($z);$F=table($a)." SET".implode(",",$M);query_redirect("UPDATE".($zg?" $F$Ue":limit1($F,$Ue)),$z,'Rekord został zaktualizowany.');}else{$G=insert_into($a,$M);$jd=($G?last_id():0);queries_redirect($z,sprintf('Rekord%s został dodany.',($jd?" $jd":"")),$G);}}}$Qf=$b->tableName(table_status($a,true));page_header(($Dg?'Edytuj':'Dodaj'),$l,array("select"=>array($a,$Qf)),$Qf);$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($n
...
';if($n){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Dg?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}echo($Dg?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$n?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($Qb?html_select("Engine",array(""=>"(".'składowanie'.")")+$Qb,$I["Engine"]):""),' ',($Ua&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porównywanie znaków'.")")+$Ua,$I["Collation"]):""),' <input type="submit" value="Zapisz zmiany">
...
';if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?checkbox("comments",1,$Za,'Komentarz',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input name="Comment" id="Comment" value="'.h($I["Comment"]).'" maxlength="'.($h->server_info>=5.5?2048:60).'"'.($Za?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if(support("partitioning")){$ye=ereg('RANGE|LIST',$I["partition_by"]);print_fieldset("partition",'Partycjonowanie',$I["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$xe,$I["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($I["partition"]),'">)
Partycje: <input type="number" name="partitions" class="size" value="',h($I["partitions"]),'"',($ye||!$I["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){$I=$_POST;if($_POST&&!$l&&!isset($_POST["add_x"])){restart_session();$A=trim($I["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Baza danych została usunięta.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Nazwa bazy danych została zmieniona.',rename_database($A,$I["collation"]));}else{$j=explode("\n",str_replace("\r","",$A));$Kf=true;$id="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$I["collation"]))$Kf=false;$id=$k;}}queries_redirect(ME."db=".urlencode($id),'Baza danych została utworzona.',$Kf);}}else{if(!$I["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$I["collation"])?" COLLATE $I[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$l,array(),DB);$Ua=collations();$A=DB;if($_POST)$A=$I["name"];elseif(DB!="")$I["collation"]=db_collation(DB,$Ua);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$_c){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$_c,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input name="name" id="name" value="'.h($A).'" maxlength="64" autocapitalize="off">')."\n".($Ua?html_select("collation",array(""=>"(".'porównywanie znaków'.")")+$Ua,$I["collation"]):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.7.0' alt='+' title='".'Dodaj następny'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){$I=$_POST;if($_POST&&!$l){$y=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$y,'Schemat został usunięty.');else{$A=trim($I["name"]);$y.=urlencode($A);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($A),$y,'Schemat został utworzony.');elseif($_GET["ns"]!=$A)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($A),$y,'Schemat został zmieniony.');else
redirect($y);}}page_header($_GET["ns"]!=""?'Zmień schemat':'Utwórz schemat',$l);if(!$I)$I["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+explode("|",$ae),$I["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+explode("|",$ae),$I["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=$_POST;if($_POST&&!$l){$A=trim($I["name"]);$wa=" AS\n$I[select]";$z=ME."table=".urlencode($A);$Cd='Perspektywa została zmieniona.';if(!$_POST["drop"]&&$a==$A&&$v!="sqlite")query_redirect(($v=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($A).$wa,$z,$Cd);else{$Zf=$A."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A).$wa,"DROP VIEW ".table($A),"CREATE VIEW ".table($Zf).$wa,"DROP VIEW ".table($Zf),($_POST["drop"]?substr(ME,0,-1):$z),'Perspektywa została usunięta.',$Cd,'Perspektywa została utworzona.',$a,$A);}}page_header(($a!=""?'Zmień perspektywę':'Utwórz perspektywę'),$l,array("table"=>$a),$a);if(!$_POST&&$a!=""){$I=view($a);$I["name"]=$a;}echo'
<form action="" method="post">
...
<tr><th>Wykonuj co<td><input type="number" name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$Uc,$I["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$Gf,$I["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$lf=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$I=$_POST;$I["fields"]=(array)$I["fields"];if($_POST&&!process_fields($I["fields"])&&!$l){$Zf="$I[name]_adminer_".uniqid();drop_create("DROP $lf ".idf_escape($da),create_routine($lf,$I),"DROP $lf ".idf_escape($I["name"]),create_routine($lf,array("name"=>$Zf)+$I),"DROP $lf ".idf_escape($Zf),substr(ME,0,-1),'Procedura została usunięta.','Procedura została zmieniona.','Procedura została utworzona.',$da,$I["name"]);}page_header(($da!=""?(isset($_GET["function"])?'Zmień funkcję':'Zmień procedurę').": ".h($da):(isset($_GET["function"])?'Utwórz funkcję':'Utwórz procedurę')),$l);if(!$_POST&&$da!=""){$I=routine($da,$lf);$I["name"]=$da;}$Ua=get_vals("SHOW CHARACTER SET");sort($Ua);$mf=routine_languages();echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($I["name"]),'" maxlength="64" autocapitalize="off">
',($mf?'Język'.": ".html_select("language",$mf,$I["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($I["fields"],$Ua,$lf);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$I["returns"],$Ua);}echo'</table>
...
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$I=$_POST;if($_POST&&!$l){$y=substr(ME,0,-1);$A=trim($I["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$y,'Sekwencja została usunięta.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($A),$y,'Sekwencja została utworzona.');elseif($fa!=$A)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$y,'Sekwencja została zmieniona.');else
redirect($y);}page_header($fa!=""?'Zmień sekwencję'.": ".h($fa):'Utwórz sekwencję',$l);if(!$I)$I["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($I["name"]),'" autocapitalize="off">
<input type="submit" value="Zapisz zmiany">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$I=$_POST;if($_POST&&!$l){$y=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$y,'Typ został usunięty.');else
query_redirect("CREATE TYPE ".idf_escape(trim($I["name"]))." $I[as]",$y,'Typ został utworzony.');}page_header($ga!=""?'Zmień typ'.": ".h($ga):'Utwórz typ',$l);if(!$I)$I["as"]="AS ";echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$A=$_GET["name"];$tg=trigger_options();$rg=array("INSERT","UPDATE","DELETE");$I=(array)trigger($A)+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$tg["Timing"])&&in_array($_POST["Event"],$rg)&&in_array($_POST["Type"],$tg["Type"])){$Zd=" ON ".table($a);$Bb="DROP TRIGGER ".idf_escape($A).($v=="pgsql"?$Zd:"");$z=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Bb,$z,'Wyzwalacz został usunięty.');else{if($A!="")queries($Bb);queries_redirect($z,($A!=""?'Wyzwalacz został zmieniony.':'Wyzwalacz został utworzony.'),queries(create_trigger($Zd,$_POST)));if($A!="")queries(create_trigger($Zd,$I+array("Type"=>reset($tg["Type"]))));}}$I=$_POST;}page_header(($A!=""?'Zmień wyzwalacz'.": ".h($A):'Utwórz wyzwalacz'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$tg["Timing"],$I["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$rg,$I["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$tg["Type"],$I["Type"]),'</table>
...
as$W){if($W[1]!="USAGE")$Ac["$_[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$I[0]))$Ac["$_[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$I[0],$_))$Xd=$_[1];}}if($_POST&&!$l){$Yd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");if($_POST["drop"])query_redirect("DROP USER $Yd",ME."privileges=",'Użytkownik został usunięty.');else{$Qd=q($_POST["user"])."@".q($_POST["host"]);$Ae=$_POST["pass"];if($Ae!=''&&!$_POST["hashed"]){$Ae=$h->result("SELECT PASSWORD(".q($Ae).")");$l=!$Ae;}$jb=false;if(!$l){if($Yd!=$Qd){$jb=queries(($h->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $Qd IDENTIFIED BY PASSWORD ".q($Ae));$l=!$jb;}elseif($Ae!=$Xd)queries("SET PASSWORD FOR $Qd = ".q($Ae));}if(!$l){$if=array();foreach($Od
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer' class='help'>".'Uprawnienia użytkowników'."</a>";$q=0;foreach($Ac
as$Td=>$_c){echo'<th>'.($Td!="*.*"?"<input name='objects[$q]' value='".h($Td)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$eb=>$ub){foreach((array)$Pe[$eb]as$Oe=>$Ya){echo"<tr".odd()."><td".($ub?">$ub<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($Oe);$q=0;foreach($Ac
as$Td=>$_c){$A="'grants[$q][".h(strtoupper($Oe))."]'";$X=$_c[strtoupper($Oe)];if($eb=="Server Admin"&&$Td!=(isset($Ac["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Uprawnienia'."<option value='0'".($X=="0"?" selected":"").">".'Usuń uprawnienia'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$fd=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$fd++;}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$fd),$fd||!$_POST["kill"]);}page_header('Lista procesów',$l);echo'
<form action="" method="post">
...
as$w=>$W)echo"<td>".(($v=="sql"&&$w=="Info"&&ereg("Query|Killed",$I["Command"])&&$W!="")||($v=="pgsql"&&$w=="current_query"&&$W!="<IDLE>")||($v=="oracle"&&$w=="sql_text"&&$W!="")?"<code class='jush-$v'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($I["db"]!=""?"db=".urlencode($I["db"])."&":"")."sql=".urlencode($W)).'">'.'Duplikuj'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$uc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$sc=$b->dumpFormat();foreach((array)$_GET["columns"]as$f){if($f["fun"]){unset($sc['sql']);break;}}if($sc){print_fieldset("export",'Eksport');$se=$b->dumpOutput();echo($se?html_select("output",$se,$qa["output"])." ":""),html_select("format",$sc,$qa["format"])," <input type='submit' name='export' value='".'Eksport'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Import',!$J);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Mb,'strlen'),$g);echo"<p><input type='hidden' name='token' value='$R'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$Ff=isset($_GET["status"]);page_header($Ff?'Status':'Zmienne');$Lg=($Ff?show_status():show_variables());if(!$Lg)echo"<p class='message'>".'Brak rekordów.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Lg
