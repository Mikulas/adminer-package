...
upload_error($l){$ce=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($ce?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$ce):""):'Plik nie istnieje.');}function
...
lang($ih,$we=null){if(is_array($ih)){$mf=($we==1?0:($we%10>1&&$we%10<5&&$we/10%10!=1?1:2));$ih=$ih[$mf];}$ih=str_replace("%d","%s",$ih);$we=number_format($we,0,".",' ');return
...
h($g->error);}function
check_sqlite_name($C){global$g;$_c="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($_c)\$~",$C)){$g->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$_c));return
false;}return
true;}function
create_database($k,$d){global$g;if(file_exists($k)){$g->error='Plik juÅ¼ istnieje.';return
...
as$k){if(!@unlink($k)){$g->error='Plik juÅ¼ istnieje.';return
false;}}return
true;}function
...
set_schema($dg){global$g,$rh,$_g;$J=$g->query("SET search_path TO ".idf_escape($dg));foreach(types()as$U){if(!isset($rh[$U])){$rh[$U]=0;$_g['Typy uÅ¼ytkownika'][]=$U;}}return$J;}function
...
unconvert_field($m,$J){return$J;}function
support($Cc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Cc);}$v="oracle";$rh=array();$_g=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$X){$rh+=$X;$_g[$w]=array_keys($X);}$yh=array();$He=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Sc=array("length","lower","round","upper");$Xc=array("avg","count","count distinct","max","min","sum");$bc=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Tb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$pf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("auth[driver]",$Tb,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Gg,$O=""){echo'<p class="links">';$Sd=array("select"=>'PokaÅ¼ dane');if(support("table")||support("indexes"))$Sd["table"]='Struktura tabeli';if(support("table")){if(is_view($Gg))$Sd["view"]='ZmieÅ„ perspektywÄ™';else$Sd["create"]='ZmieÅ„ tabelÄ™';}if($O!==null)$Sd["edit"]='Nowy rekord';foreach($Sd
...
selectQuery($H){global$v;return"<p><code class='jush-$v'>".h(str_replace("\n"," ",$H))."</code>".(support("sql")?" <a href='".h(ME)."sql=".urlencode($H)."'>".'Edytuj'."</a>":"")."</p>";}function
rowDescription($Q){return"";}function
rowDescriptions($L,$Kc){return$L;}function
selectLink($X,$m){}function
selectVal($X,$z,$m,$Te){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$m["type"])&&!preg_match("~var~",$m["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$m["type"])&&!is_utf8($X))$J=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($Te));return($z?"<a href='".h($z)."'>$J</a>":$J);}function
editVal($X,$m){return$X;}function
selectColumnsPrint($M,$f){global$Sc,$Xc;print_fieldset("select",'pokaÅ¼',$M);$q=0;$M[""]=array();foreach($M
as$w=>$X){$X=$_GET["columns"][$w];$e=select_input(" name='columns[$q][col]' onchange='".($w!==""?"selectFieldChange(this.form)":"selectAddRow(this)").";'",$f,$X["col"]);echo"<div>".($Sc||$Xc?"<select name='columns[$q][fun]' onchange='helpClose();".($w!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).">".optionlist(array(-1=>"")+array_filter(array('Funkcje'=>$Sc,'Agregacje'=>$Xc)),$X["fun"])."</select>"."($e)":$e)."</div>\n";$q++;}echo"</div></fieldset>\n";}function
selectSearchPrint($Z,$f,$u){print_fieldset("search",'Szukaj',$Z);foreach($u
as$q=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input type='search' name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Xa="this.nextSibling.onchange();";for($q=0;$q<=count($_GET["where"]);$q++){list(,$X)=each($_GET["where"]);if(!$X||("$X[col]$X[val]"!=""&&in_array($X["op"],$this->operators))){echo"<div>".select_input(" name='where[$q][col]' onchange='$Xa'",$f,$X["col"],"(".'gdziekolwiek'.")"),html_select("where[$q][op]",$this->operators,$X["op"],$Xa),"<input type='search' name='where[$q][val]' value='".h($X["val"])."' onchange='".($X?"selectFieldChange(this.form)":"selectAddRow(this)").";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Me,$f,$u){print_fieldset("sort",'Sortuj',$Me);$q=0;foreach((array)$_GET["order"]as$w=>$X){if($X!=""){echo"<div>".select_input(" name='order[$q]' onchange='selectFieldChange(this.form);'",$f,$X),checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'malejÄ…co')."</div>\n";$q++;}}echo"<div>".select_input(" name='order[$q]' onchange='selectAddRow(this);'",$f),checkbox("desc[$q]",1,false,'malejÄ…co')."</div>\n","</div></fieldset>\n";}function
...
selectLengthPrint($Vg){if($Vg!==null){echo"<fieldset><legend>".'DÅ‚ugoÅ›Ä‡ tekstu'."</legend><div>","<input type='number' name='text_length' class='size' value='".h($Vg)."'>","</div></fieldset>\n";}}function
...
messageQuery($H){global$v;restart_session();$cd=&get_session("queries");$r="sql-".count($cd[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$cd[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'Zapytanie SQL'."</a>"."<div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($H,1000).'</code></pre>'.(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($cd[$_GET["db"]])-1)).'">'.'Edytuj'.'</a>':'').'</div>';}function
...
explode("/",$J);}function
editInput($Q,$m,$Ha,$Y){if($m["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ha value='-1' checked><i>".'bez zmian'."</i></label> ":"").($m["null"]?"<label><input type='radio'$Ha value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ha,$m,$Y,0);return"";}function
...
<p id="dbs">
';hidden_fields_get();$Db=" onmousedown='dbMouseDown(event, this);' onchange='dbChange(this);'";echo"<span title='".'baza danych'."'>DB</span>: ".($j?"<select name='db'$Db>".optionlist(array(""=>"")+$j,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Wybierz'."'".($j?" class='hidden'":"").">\n";if($me!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br><select name='ns'$Db>".optionlist(array(""=>"(".'schemat'.")")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($S){echo"<p id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($S
as$Q=>$xg){echo'<a href="'.h(ME).'select='.urlencode($Q).'"'.bold($_GET["select"]==$Q||$_GET["edit"]==$Q).">".'przeglÄ…daj'."</a> ";$C=$this->tableName($xg);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($Q).'"'.bold(in_array($Q,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($xg)?"view":""))." title='".'Struktura tabeli'."'>$C</a>":"<span>$C</span>")."<br>\n";}}}$b=(function_exists('adminer_object')?adminer_object():new
...
long2str($W,true);}$g='';$bd=$_SESSION["token"];if(!$bd)$_SESSION["token"]=rand(1,1e6);$T=get_token();$jf=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$X){list($w)=explode(":",$X);$jf[$w]=$X;}}$Ia=$_POST["auth"];if($Ia){session_regenerate_id();$Sb=$Ia["driver"];$N=$Ia["server"];$V=$Ia["username"];$G=$Ia["password"];$k=$Ia["db"];set_password($Sb,$N,$V,$G);$_SESSION["db"][$Sb][$N][$V][$k]=true;if($Ia["permanent"]){$w=base64_encode($Sb)."-".base64_encode($N)."-".base64_encode($V)."-".base64_encode($k);$vf=$b->permanentLogin(true);$jf[$w]="$w:".base64_encode($vf?encrypt_string($G,$vf):"");cookie("adminer_permanent",implode(" ",$jf));}if(count($_POST)==1||DRIVER!=$Sb||SERVER!=$N||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($Sb,$N,$V,$k));}elseif($_POST["logout"]){if($bd&&!verify_token()){page_header('Wyloguj','NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');page_footer("db");exit;}else{foreach(array("pwds","db","dbs","queries")as$w)set_session($w,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Wylogowano pomyÅ›lnie.');}}elseif($jf&&!$_SESSION["pwds"]){session_regenerate_id();$vf=$b->permanentLogin();foreach($jf
...
auth_error($sc=null){global$g,$b,$bd;$og=session_name();$l="";if(!$_COOKIE[$og]&&$_GET[$og]&&ini_bool("session.use_only_cookies"))$l='Wymagana jest obsÅ‚uga sesji w PHP.';elseif(isset($_GET["username"])){if(($_COOKIE[$og]||$_GET[$og])&&!$bd)$l='Sesja wygasÅ‚a, zaloguj siÄ™ ponownie.';else{$G=get_password();if($G!==null){$l=h($sc?$sc->getMessage():(is_string($g)?$g:'NieprawidÅ‚owe dane logowania.'));if($G===false)$l.='<br>'.sprintf('WaÅ¼noÅ›Ä‡ hasÅ‚a gÅ‚Ã³wnego wygasÅ‚a. <a href="http://www.adminer.org/pl/extension/" target="_blank">Zaimplementuj</a> wÅ‚asnÄ… metodÄ™ %s, aby ustawiÄ‡ je na staÅ‚e.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header('Zaloguj siÄ™',$l,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
set_password($Jh,$N,$V,$G){$_SESSION["pwds"][$Jh][$N][$V]=($_COOKIE["adminer_key"]?array(encrypt_string($G,$_COOKIE["adminer_key"])):$G);}function
get_password(){$J=get_session("pwds");if(is_array($J))$J=($_COOKIE["adminer_key"]?decrypt_string($J[0],$_COOKIE["adminer_key"]):false);return$J;}if(isset($_GET["username"])){if(!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Brak rozszerzenia',sprintf('Å»adne z rozszerzeÅ„ PHP umoÅ¼liwiajÄ…cych poÅ‚Ä…czenie siÄ™ z bazÄ… danych (%s) nie jest dostÄ™pne.',implode(", ",$pf)),false);page_footer("auth");exit;}$g=connect();}if(!is_object($g)||!$b->login($_GET["username"],get_password())){auth_error();exit;}$Sb=new
Min_Driver($g);if($Ia&&$_POST["token"])$_POST["token"]=$T;$l='';if($_POST){if(!verify_token()){$qd="max_input_vars";$de=ini_get($qd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$w){$X=ini_get($w);if($X&&(!$de||$X<$de)){$qd=$w;$de=$X;}}}$l=(!$_POST["token"]&&$de?sprintf('Przekroczono maksymalnÄ… liczbÄ™ pÃ³l. ZwiÄ™ksz %s.',"'$qd'"):'NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$l=sprintf('PrzesÅ‚ano zbyt duÅ¼o danych. Zmniejsz objÄ™toÅ›Ä‡ danych lub zwiÄ™ksz zmiennÄ… konfiguracyjnÄ… %s.',"'post_max_size'");if(isset($_GET["sql"]))$l.=' '.'WiÄ™ksze pliki SQL moÅ¼esz wgraÄ‡ na serwer poprzez FTP przed zaimportowaniem.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
connect_error(){global$b,$g,$T,$l,$Tb;$j=array();if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('Baza danych'.": ".h(DB),'Nie znaleziono bazy danych.',true);}else{if($_POST["db"]&&!$l)queries_redirect(substr(ME,0,-1),'Bazy danych zostaÅ‚y usuniÄ™te.',drop_databases($_POST["db"]));page_header('Wybierz bazÄ™ danych',$l,false);echo"<p class='links'>\n";foreach(array('database'=>'UtwÃ³rz nowÄ… bazÄ™ danych','privileges'=>'Uprawnienia uÅ¼ytkownikÃ³w','processlist'=>'Lista procesÃ³w','variables'=>'Zmienne','status'=>'Status',)as$w=>$X){if(support($w))echo"<a href='".h(ME)."$w='>$X</a>\n";}echo"<p>".sprintf('Wersja %s: %s za pomocÄ… %s',$Tb[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Zalogowany jako: %s',"<b>".h(logged_user())."</b>")."\n";$j=$b->databases();if($j){$eg=support("scheme");$ib=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".'Baza danych'."<td>".'PorÃ³wnywanie znakÃ³w'."<td>".'Tabele'."</thead>\n";foreach($j
as$k){$Wf=h(ME)."db=".urlencode($k);echo"<tr".odd().">".(support("database")?"<td>".checkbox("db[]",$k,in_array($k,(array)$_POST["db"])):""),"<th><a href='$Wf'>".h($k)."</a>";$d=nbsp(db_collation($k,$ib));echo"<td>".(support("database")?"<a href='$Wf".($eg?"&amp;ns=":"")."&amp;database=' title='".'ZmieÅ„ bazÄ™ danych'."'>$d</a>":$d),"<td align='right'><a href='$Wf&amp;schema=' id='tables-".h($k)."' title='".'Schemat bazy danych'."'>?</a>","\n";}echo"</table>\n",(support("database")?"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>\n"."<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^db/));\">\n"."<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().">\n"."</div></fieldset>\n":""),"<script type='text/javascript'>tableCheck();</script>\n","<input type='hidden' name='token' value='$T'>\n","</form>\n";}echo"<p><a href='".h(ME)."refresh=1'>".'OdÅ›wieÅ¼'."</a>\n";}page_footer("db");if($j)echo"<script type='text/javascript'>ajaxSetHtml('".js_escape(ME)."script=connect');</script>\n";}if(isset($_GET["status"]))$_GET["variables"]=$_GET["status"];if(isset($_GET["import"]))$_GET["sql"]=$_GET["import"];if(!(DB!=""?$g->select_db(DB):isset($_GET["sql"])||isset($_GET["dump"])||isset($_GET["database"])||isset($_GET["processlist"])||isset($_GET["privileges"])||isset($_GET["user"])||isset($_GET["variables"])||$_GET["script"]=="connect"||$_GET["script"]=="kill")){if(DB!=""||$_GET["refresh"]){restart_session();set_session("dbs",null);}connect_error();exit;}if(support("scheme")&&DB!=""&&$_GET["ns"]!==""){if(!isset($_GET["ns"]))redirect(preg_replace('~ns=[^&]*&~','',ME)."ns=".get_schema());if(!set_schema($_GET["ns"])){header("HTTP/1.1 404 Not Found");page_header('Schemat'.": ".h($_GET["ns"]),'NieprawidÅ‚owy schemat.',true);page_footer("ns");exit;}}function
...
as$w=>$X){if($X===null)$X="<i>NULL</i>";elseif($Ra[$w]&&!is_utf8($X))$X="<i>".lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($X))."</i>";elseif(!strlen($X))$X="&nbsp;";else{$X=h($X);if($rh[$w]==254)$X="<code>$X</code>";}if(isset($Sd[$w])&&!$f[$Sd[$w]]){if($Pe&&$v=="sql"){$Q=$K[array_search("table=",$Sd)];$z=$Sd[$w].urlencode($Pe[$Q]!=""?$Pe[$Q]:$Q);}else{$z="edit=".urlencode($Sd[$w]);foreach($u[$Sd[$w]]as$gb=>$_d)$z.="&where".urlencode("[".bracket_escape($gb)."]")."=".urlencode($K[$_d]);}$X="<a href='".h(ME.$z)."'>$X</a>";}echo"<td>$X";}}echo($q?"</table>":"<p class='message'>".'Brak rekordÃ³w.')."\n";return$J;}function
...
edit_type($w,$m,$ib,$Lc=array()){global$_g,$rh,$yh,$De;$U=$m["type"];echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($rh[$U])&&!isset($Lc[$U]))array_unshift($_g,$U);if($Lc)$_g['Klucze obce']=$Lc;echo
optionlist($_g,$U),'</select>
<td><input name="',$w,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$m["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='$w"."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'porÃ³wnywanie znakÃ³w'.')'.optionlist($ib,$m["collation"]).'</select>',($yh?"<select name='$w"."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($yh,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='$w"."[on_update]'".($U=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'W przypadku zmiany'.")","CURRENT_TIMESTAMP"),$m["on_update"]).'</select>':''),($Lc?"<select name='$w"."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'W przypadku usuniÄ™cia'.")".optionlist(explode("|",$De),$m["on_delete"])."</select> ":" ");}function
...
<td><input type="radio" name="auto_increment_col" value=""><acronym title="Auto Increment">AI</acronym>',doc_link(array('sql'=>"example-auto-increment.html",'sqlite'=>"autoinc.html",'pgsql'=>"datatype.html#DATATYPE-SERIAL",'mssql'=>"ms186775.aspx",)),'<td>WartoÅ›ci domyÅ›lne
',(support("comment")?"<td".($nb?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.2' alt='+' title='".'Dodaj nastÄ™pny'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($nb?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.2' alt='+' title='".'Dodaj nastÄ™pny'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.0.2' alt='^' title='".'PrzesuÅ„ w gÃ³rÄ™'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.0.2' alt='v' title='".'PrzesuÅ„ w dÃ³Å‚'."'>&nbsp;":""),($Qe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.0.2' alt='x' title='".'UsuÅ„'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
as$w=>$X)echo"<tr valign='top'><td>$X[0]<td>$X[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'ZmieÅ„'."</a>\n";echo"</table>\n";}echo'<p class="links"><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Dodaj wyzwalacz'."</a>\n";}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),h(DB.($_GET["ns"]?".$_GET[ns]":"")));$Jg=array();$Kg=array();$C="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$C-".DB]?"$C-".DB:$C)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Xd,PREG_SET_ORDER);foreach($Xd
...
pack("x512");}}}if($wd)echo"-- ".$g->result("SELECT NOW()")."\n";exit;}page_header('Eksport',$l,($_GET["export"]!=""?array("table"=>$_GET["export"]):array()),h(DB));echo'
<form action="" method="post">
<table cellspacing="0">
';$Eb=array('','USE','DROP+CREATE','CREATE');$Lg=array('','DROP+CREATE','CREATE');$Bb=array('','TRUNCATE+INSERT','INSERT');if($v=="sql")$Bb[]='INSERT+UPDATE';parse_str($_COOKIE["adminer_export"],$K);if(!$K)$K=array("output"=>"text","format"=>"sql","db_style"=>(DB!=""?"":"CREATE"),"table_style"=>"DROP+CREATE","data_style"=>"INSERT");if(!isset($K["events"])){$K["routines"]=$K["events"]=($_GET["dump"]=="");$K["triggers"]=$K["table_style"];}echo"<tr><th>".'Rezultat'."<td>".html_select("output",$b->dumpOutput(),$K["output"],0)."\n";echo"<tr><th>".'Format'."<td>".html_select("format",$b->dumpFormat(),$K["format"],0)."\n";echo($v=="sqlite"?"":"<tr><th>".'Baza danych'."<td>".html_select('db_style',$Eb,$K["db_style"]).(support("routine")?checkbox("routines",1,$K["routines"],'Procedury i funkcje'):"").(support("event")?checkbox("events",1,$K["events"],'Wydarzenia'):"")),"<tr><th>".'Tabele'."<td>".html_select('table_style',$Lg,$K["table_style"]).checkbox("auto_increment",1,$K["auto_increment"],'Auto Increment').(support("trigger")?checkbox("triggers",1,$K["triggers"],'Wyzwalacze'):""),"<tr><th>".'Dane'."<td>".html_select('data_style',$Bb,$K["data_style"]),'</table>
<p><input type="submit" value="Eksport">
<input type="hidden" name="token" value="',$T,'">

<table cellspacing="0">
';$rf=array();if(DB!=""){$Za=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Za onclick='formCheck(this, /^tables\\[/);'>".'Tabele'."</label>","<th style='text-align: right;'><label class='block'>".'Dane'."<input type='checkbox' id='check-data'$Za onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Mh="";$Mg=tables_list();foreach($Mg
...
as$C=>$m){if(!isset($m["privileges"][$zh?"update":"insert"])||$b->fieldName($m)=="")unset($n[$C]);}if($_POST&&!$l&&!isset($_GET["select"])){$_=$_POST["referer"];if($_POST["insert"])$_=($zh?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$_))$_=ME."select=".urlencode($a);$u=indexes($a);$uh=unique_array($_GET["where"],$u);$Df="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($_,'Rekord zostaÅ‚ usuniÄ™ty.',$Sb->delete($a,$Df,!$uh));else{$O=array();foreach($n
as$C=>$m){$X=process_input($m);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($zh){if(!$O)redirect($_);queries_redirect($_,'Rekord zostaÅ‚ zaktualizowany.',$Sb->update($a,$O,$Df,!$uh));if(is_ajax()){page_headers();page_messages($l);exit;}}else{$I=$Sb->insert($a,$O);$Kd=($I?last_id():0);queries_redirect($_,sprintf('Rekord%s zostaÅ‚ dodany.',($Kd?" $Kd":"")),$I);}}}$Hg=$b->tableName(table_status1($a,true));page_header(($zh?'Edytuj':'Dodaj'),$l,array("select"=>array($a,$Hg)),$Hg);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($n
...
as$w=>$X){if(!$Z)$K[$w]=null;$n[$w]=array("field"=>$w,"null"=>($w!=$r),"auto_increment"=>($w==$r));}}}if($K===false)echo"<p class='error'>".'Brak rekordÃ³w.'."\n";echo'
<div id="message"></div>
...
';if($n){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($zh?'Zapisz i kontynuuj edycjÄ™'."' onclick='return !ajaxForm(this.form, \"".'Saving'.'...", this)':'Zapisz i dodaj nastÄ™pny')."' title='Ctrl+Shift+Enter'>\n";}echo($zh?"<input type='submit' name='delete' value='".'UsuÅ„'."'".confirm().">\n":($_POST||!$n?"":"<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($a==""&&!$_POST){?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php }echo($kc?"<select name='Engine' onchange='helpClose();'".on_help("getTarget(event).value",1).">".optionlist(array(""=>"(".'skÅ‚adowanie'.")")+$kc,$K["Engine"])."</select>":""),' ',($ib&&!preg_match("~sqlite|mssql~",$v)?html_select("Collation",array(""=>"(".'porÃ³wnywanie znakÃ³w'.")")+$ib,$K["Collation"]):""),' <input type="submit" value="Zapisz zmiany">
';}echo'
...
',checkbox("defaults",1,true,'WartoÅ›ci domyÅ›lne',"columnShow(this.checked, 5)","jsonly");if(!$_POST["defaults"]){echo'<script type="text/javascript">editingHideDefaults()</script>';}echo(support("comment")?"<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"".($nb?" checked":"").">".'Komentarz'."</label>".' <input name="Comment" id="Comment" value="'.h($K["Comment"]).'" maxlength="'.($g->server_info>=5.5?2048:60).'"'.($nb?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Zapisz zmiany">
';}echo'
';if($a!=""){echo'<input type="submit" name="drop" value="UsuÅ„"',confirm(),'>';}if(support("partitioning")){$cf=preg_match('~RANGE|LIST~',$K["partition_by"]);print_fieldset("partition",'Partycjonowanie',$K["partition_by"]);echo'<p>
...
Partycje: <input type="number" name="partitions" class="size',($cf||!$K["partition_by"]?" hidden":""),'" value="',h($K["partitions"]),'">
...
<th>Nazwa
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.0.2' alt='+' title='Dodaj nastÄ™pny'></noscript>&nbsp;
</thead>
<?php
if($sf){echo"<tr><td>PRIMARY<td>";foreach($sf["columns"]as$w=>$e){echo"<select disabled>".optionlist($n,$e)."</select>","<label><input disabled type='checkbox'>".'malejÄ…co'."</label> ";}echo"<td><td>\n";}$_d=1;foreach($K["indexes"]as$t){if(!$_POST["drop_col"]||$_d!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$_d][type]",array(-1=>"")+$md,$t["type"],($_d==count($K["indexes"])?"indexesAddRow(this);":1)),"<td>";ksort($t["columns"]);$q=1;foreach($t["columns"]as$w=>$e){echo"<span>".html_select("indexes[$_d][columns][$q]",array(-1=>"")+$n,$e,($q==count($t["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($v=="sql"?"":$_GET["indexes"]."_")."');"),($v=="sql"||$v=="mssql"?"<input type='number' name='indexes[$_d][lengths][$q]' class='size' value='".h($t["lengths"][$w])."'>":""),($v!="sql"?checkbox("indexes[$_d][descs][$q]",1,$t["descs"][$w],'malejÄ…co'):"")," </span>";$q++;}echo"<td><input name='indexes[$_d][name]' value='".h($t["name"])."' autocapitalize='off'>\n","<td><input type='image' class='icon' name='drop_col[$_d]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.0.2' alt='x' title='".'UsuÅ„'."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$_d++;}echo'</table>
<p>
<input type="submit" value="Zapisz zmiany">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$l&&!isset($_POST["add_x"])){restart_session();$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Baza danych zostaÅ‚a usuniÄ™ta.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),'Nazwa bazy danych zostaÅ‚a zmieniona.',rename_database($C,$K["collation"]));}else{$j=explode("\n",str_replace("\r","",$C));$Bg=true;$Jd="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Bg=false;$Jd=$k;}}queries_redirect(ME."db=".urlencode($Jd),'Baza danych zostaÅ‚a utworzona.',$Bg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'Baza danych zostaÅ‚a zmieniona.');}}page_header(DB!=""?'ZmieÅ„ bazÄ™ danych':'UtwÃ³rz bazÄ™ danych',$l,array(),h(DB));$ib=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$ib);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$Tc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Tc,$A)&&$A[1]){$C=stripcslashes(idf_unescape("`$A[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($C,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($C).'</textarea><br>':'<input name="name" id="name" value="'.h($C).'" maxlength="64" autocapitalize="off">')."\n".($ib?html_select("collation",array(""=>"(".'porÃ³wnywanie znakÃ³w'.")")+$ib,$K["collation"]).doc_link(array('sql'=>"charset-charsets.html",'mssql'=>"ms187963.aspx",)):"");?>
<script type='text/javascript'>focus(document.getElementById('name'));</script>
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.2' alt='+' title='".'Dodaj nastÄ™pny'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["scheme"])){$K=$_POST;if($_POST&&!$l){$z=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$z,'Schemat zostaÅ‚ usuniÄ™ty.');else{$C=trim($K["name"]);$z.=urlencode($C);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($C),$z,'Schemat zostaÅ‚ utworzony.');elseif($_GET["ns"]!=$C)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($C),$z,'Schemat zostaÅ‚ zmieniony.');else
redirect($z);}}page_header($_GET["ns"]!=""?'ZmieÅ„ schemat':'UtwÃ³rz schemat',$l);if(!$K)$K["name"]=$_GET["ns"];echo'
<form action="" method="post">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$l){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$_=ME."table=".urlencode($C);$B='Perspektywa zostaÅ‚a zmieniona.';if(!$_POST["drop"]&&$a==$C&&$v!="sqlite")query_redirect(($v=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$_,$B);else{$Rg=$C."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Rg).$Ea,"DROP VIEW ".table($Rg),($_POST["drop"]?substr(ME,0,-1):$_),'Perspektywa zostaÅ‚a usuniÄ™ta.',$B,'Perspektywa zostaÅ‚a utworzona.',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$l)$l=$g->error;}page_header(($a!=""?'ZmieÅ„ perspektywÄ™':'UtwÃ³rz perspektywÄ™'),$l,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
<tr><th>Wykonuj co<td><input type="number" name="INTERVAL_VALUE" value="',h($K["INTERVAL_VALUE"]),'" class="size"> ',html_select("INTERVAL_FIELD",$ud,$K["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$yg,$K["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($K["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["procedure"])){$da=$_GET["procedure"];$Xf=(isset($_GET["function"])?"FUNCTION":"PROCEDURE");$K=$_POST;$K["fields"]=(array)$K["fields"];if($_POST&&!process_fields($K["fields"])&&!$l){$Rg="$K[name]_adminer_".uniqid();drop_create("DROP $Xf ".idf_escape($da),create_routine($Xf,$K),"DROP $Xf ".idf_escape($K["name"]),create_routine($Xf,array("name"=>$Rg)+$K),"DROP $Xf ".idf_escape($Rg),substr(ME,0,-1),'Procedura zostaÅ‚a usuniÄ™ta.','Procedura zostaÅ‚a zmieniona.','Procedura zostaÅ‚a utworzona.',$da,$K["name"]);}page_header(($da!=""?(isset($_GET["function"])?'ZmieÅ„ funkcjÄ™':'ZmieÅ„ procedurÄ™').": ".h($da):(isset($_GET["function"])?'UtwÃ³rz funkcjÄ™':'UtwÃ³rz procedurÄ™')),$l);if(!$_POST&&$da!=""){$K=routine($da,$Xf);$K["name"]=$da;}$ib=get_vals("SHOW CHARACTER SET");sort($ib);$Yf=routine_languages();echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($K["name"]),'" maxlength="64" autocapitalize="off">
',($Yf?'JÄ™zyk'.": ".html_select("language",$Yf,$K["language"]):""),'<input type="submit" value="Zapisz zmiany">
<table cellspacing="0" class="nowrap">
';edit_fields($K["fields"],$ib,$Xf);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$K["returns"],$ib);}echo'</table>
<p>';textarea("definition",$K["definition"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($da!=""){echo'<input type="submit" name="drop" value="UsuÅ„"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];$K=$_POST;if($_POST&&!$l){$z=substr(ME,0,-1);$C=trim($K["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekwencja zostaÅ‚a usuniÄ™ta.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($C),$z,'Sekwencja zostaÅ‚a utworzona.');elseif($fa!=$C)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($C),$z,'Sekwencja zostaÅ‚a zmieniona.');else
redirect($z);}page_header($fa!=""?'ZmieÅ„ sekwencjÄ™'.": ".h($fa):'UtwÃ³rz sekwencjÄ™',$l);if(!$K)$K["name"]=$fa;echo'
<form action="" method="post">
<p><input name="name" value="',h($K["name"]),'" autocapitalize="off">
<input type="submit" value="Zapisz zmiany">
';if($fa!="")echo"<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];$K=$_POST;if($_POST&&!$l){$z=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ zostaÅ‚ usuniÄ™ty.');else
query_redirect("CREATE TYPE ".idf_escape(trim($K["name"]))." $K[as]",$z,'Typ zostaÅ‚ utworzony.');}page_header($ga!=""?'ZmieÅ„ typ'.": ".h($ga):'UtwÃ³rz typ',$l);if(!$K)$K["as"]="AS ";echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$C=$_GET["name"];$nh=trigger_options();$lh=array("INSERT","UPDATE","DELETE");$K=(array)trigger($C)+array("Trigger"=>$a."_bi");if($_POST){if(!$l&&in_array($_POST["Timing"],$nh["Timing"])&&in_array($_POST["Event"],$lh)&&in_array($_POST["Type"],$nh["Type"])){$Ce=" ON ".table($a);$Ub="DROP TRIGGER ".idf_escape($C).($v=="pgsql"?$Ce:"");$_=ME."table=".urlencode($a);if($_POST["drop"])query_redirect($Ub,$_,'Wyzwalacz zostaÅ‚ usuniÄ™ty.');else{if($C!="")queries($Ub);queries_redirect($_,($C!=""?'Wyzwalacz zostaÅ‚ zmieniony.':'Wyzwalacz zostaÅ‚ utworzony.'),queries(create_trigger($Ce,$_POST)));if($C!="")queries(create_trigger($Ce,$K+array("Type"=>reset($nh["Type"]))));}}$K=$_POST;}page_header(($C!=""?'ZmieÅ„ wyzwalacz'.": ".h($C):'UtwÃ³rz wyzwalacz'),$l,array("table"=>$a));echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$nh["Timing"],$K["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$lh,$K["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$nh["Type"],$K["Type"]),'</table>
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'Uprawnienia uÅ¼ytkownikÃ³w'.doc_link(array('sql'=>"grant.html#priv_level"));$q=0;foreach($Uc
as$xe=>$Tc){echo'<th>'.($xe!="*.*"?"<input name='objects[$q]' value='".h($xe)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$tb=>$Kb){foreach((array)$xf[$tb]as$wf=>$mb){echo"<tr".odd()."><td".($Kb?">$Kb<td":" colspan='2'").' lang="en" title="'.h($mb).'">'.h($wf);$q=0;foreach($Uc
as$xe=>$Tc){$C="'grants[$q][".h(strtoupper($wf))."]'";$Y=$Tc[strtoupper($wf)];if($tb=="Server Admin"&&$xe!=(isset($Uc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Uprawnienia'."<option value='0'".($Y=="0"?" selected":"").">".'UsuÅ„ uprawnienia'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$Gd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Gd++;}queries_redirect(ME."processlist=",lang(array('Przerwano %d wÄ…tek.','Przerwano %d wÄ…tki.','Przerwano %d wÄ…tkÃ³w.'),$Gd),$Gd||!$_POST["kill"]);}page_header('Lista procesÃ³w',$l);echo'
<form action="" method="post">
...
as$w=>$X)echo"<td>".(($v=="sql"&&$w=="Info"&&preg_match("~Query|Killed~",$K["Command"])&&$X!="")||($v=="pgsql"&&$w=="current_query"&&$X!="<IDLE>")||($v=="oracle"&&$w=="sql_text"&&$X!="")?"<code class='jush-$v'>".shorten_utf8($X,100,"</code>").' <a href="'.h(ME.($K["db"]!=""?"db=".urlencode($K["db"])."&":"")."sql=".urlencode($X)).'">'.'Duplikuj'.'</a>':nbsp($X));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
';if(support("kill")){echo($q+1)."/".sprintf('%d w sumie',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Przerwij wykonywanie'."'>\n";}echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
as$Ad=>$W)$z.=where_link($q++,$Ad,$W);}$X=select_value($X,$z,$m,$Vg);$r=h("val[$vh][".bracket_escape($w)."]");$Y=$_POST["val"][$vh][bracket_escape($w)];$cc=!is_array($K[$w])&&is_utf8($X)&&$L[$pe][$w]==$K[$w]&&!$Sc[$w];$Ug=preg_match('~text|lob~',$m["type"]);if(($_GET["modify"]&&$cc)||$Y!==null){$Yc=h($Y!==null?$Y:$K[$w]);echo"<td>".($Ug?"<textarea name='$r' cols='30' rows='".(substr_count($K[$w],"\n")+1)."'>$Yc</textarea>":"<input name='$r' value='$Yc' size='$Pd[$w]'>");}else{$Ud=strpos($X,"<i>...</i>");echo"<td id='$r' onclick=\"selectClick(this, event, ".($Ud?2:($Ug?1:0)).($cc?"":", '".h('UÅ¼yj linku edycji aby zmieniÄ‡ tÄ™ wartoÅ›Ä‡.')."'").");\">$X";}}}if($Na)echo"<td>";$b->backwardKeysPrint($Na,$L[$pe]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n";}if(($L||$E)&&!is_ajax()){$rc=true;if($_GET["page"]!="last"){if(!+$y)$Oc=count($L);elseif($v!="sql"||!$vd){$Oc=($vd?false:found_rows($R,$Z));if($Oc<max(1e4,2*($E+1)*$y))$Oc=reset(slow_query(count_rows($a,$Z,$vd,$Vc)));else$rc=false;}}if(+$y&&($Oc===false||$Oc>$y||$E)){echo"<p class='pages'>";$ae=($Oc===false?$E+(count($L)>=$y?2:1):floor(($Oc-1)/$y));if($v!="simpledb"){echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Strona'."', '".($E+1)."'), event); return false;\">".'Strona'."</a>:",pagination(0,$E).($E>5?" ...":"");for($q=max(1,$E-4);$q<min($ae,$E+5);$q++)echo
pagination($q,$E);if($ae>0){echo($E+5<$ae?" ...":""),($rc&&$Oc!==false?pagination($ae,$E):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$ae'>".'ostatni'."</a>");}echo(($Oc===false?count($L)+1:$Oc-$E*$y)>$y?' <a href="'.h(remove_from_uri("page")."&page=".($E+1)).'" onclick="return !selectLoadMore(this, '.(+$y).', \''.'Wczytywanie'.'...\');">'.'Wczytaj wiÄ™cej danych'.'</a>':'');}else{echo'Strona'.":",pagination(0,$E).($E>1?" ...":""),($E?pagination($E,$E):""),($ae>$E?pagination($E+1,$E).($ae>$E+1?" ...":""):"");}}echo"<p class='count'>\n",($Oc!==false?"(".($rc?"":"~ ").lang(array('%d rekord','%d rekordy','%d rekordÃ³w'),$Oc).") ":"");$Qb=($rc?"":"~ ").$Oc;echo
checkbox("all",1,0,'wybierz wszystkie',"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Qb' : checked); selectCount('selected2', this.checked || !checked ? '$Qb' : checked);")."\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Modify</legend><div>
...
';}$Mc=$b->dumpFormat();foreach((array)$_GET["columns"]as$e){if($e["fun"]){unset($Mc['sql']);break;}}if($Mc){print_fieldset("export",'Eksport'." <span id='selected2'></span>");$We=$b->dumpOutput();echo($We?html_select("output",$We,$wa["output"])." ":""),html_select("format",$Mc,$wa["format"])," <input type='submit' name='export' value='".'Eksport'."'>\n","</div></fieldset>\n";}echo(!$Vc&&$M?"":"<script type='text/javascript'>tableCheck();</script>\n");}if($b->selectImportPrint()){print_fieldset("import",'Import',!$L);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$wa["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($gc,'strlen'),$f);echo"<p><input type='hidden' name='token' value='$T'></p>\n","</form>\n";}}if(is_ajax()){ob_end_clean();exit;}}elseif(isset($_GET["variables"])){$xg=isset($_GET["status"]);page_header($xg?'Status':'Zmienne');$Ih=($xg?show_status():show_variables());if(!$Ih)echo"<p class='message'>".'Brak rekordÃ³w.'."\n";else{echo"<table cellspacing='0'>\n";foreach($Ih
...
as$w=>$X)json_row("sum-$w",number_format($X,0,'.',' '));json_row("");}elseif($_GET["script"]=="kill")$g->query("KILL ".(+$_POST["kill"]));else{foreach(count_tables($b->databases())as$k=>$X)json_row("tables-".js_escape($k),$X);json_row("");}exit;}else{$Ng=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($Ng&&!$l&&!$_POST["search"]){$I=true;$B="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$I=truncate_tables($_POST["tables"]);$B='Tabele zostaÅ‚y oprÃ³Å¼nione.';}elseif($_POST["move"]){$I=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$B='Tabele zostaÅ‚y przeniesione.';}elseif($_POST["copy"]){$I=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$B='Tabele zostaÅ‚y skopiowane.';}elseif($_POST["drop"]){if($_POST["views"])$I=drop_views($_POST["views"]);if($I&&$_POST["tables"])$I=drop_tables($_POST["tables"]);$B='Tabele zostaÅ‚y usuniÄ™te.';}elseif($v!="sql"){$I=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$B='Tabele zostaÅ‚y zoptymalizowane.';}elseif(!$_POST["tables"])$B='Brak tabel.';elseif($I=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"])))){while($K=$I->fetch_assoc())$B.="<b>".h($K["Table"])."</b>: ".h($K["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$B,$I);}page_header(($_GET["ns"]==""?'Baza danych'.": ".h(DB):'Schemat'.": ".h($_GET["ns"])),$l,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3 id='tables-views'>".'Tabele i perspektywy'."</h3>\n";$Mg=tables_list();if(!$Mg)echo"<p class='message'>".'Brak tabel.'."\n";else{echo"<form action='' method='post'>\n";if(support("table")){echo"<fieldset><legend>".'Wyszukaj we wszystkich tabelach'." <span id='selected2'></span></legend><div>","<input type='search' name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Szukaj'."'>\n","</div></fieldset>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabela','<td>'.'SkÅ‚adowanie','<td>'.'PorÃ³wnywanie znakÃ³w','<td>'.'Rozmiar danych','<td>'.'Rozmiar indeksÃ³w','<td>'.'Wolne miejsce','<td>'.'Auto Increment','<td>'.'Liczba rekordÃ³w',(support("comment")?'<td>'.'Komentarz':''),"</thead>\n";$S=0;foreach($Mg
as$C=>$U){$Lh=($U!==null&&!preg_match('~table~i',$U));echo'<tr'.odd().'><td>'.checkbox(($Lh?"views[]":"tables[]"),$C,in_array($C,$Ng,true),"","formUncheck('check-all');"),'<th>'.(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($C).'" title="'.'Struktura tabeli'.'">'.h($C).'</a>':h($C));if($Lh){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($C).'" title="'.'ZmieÅ„ perspektywÄ™'.'">'.'Perspektywa'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($C).'" title="'.'PokaÅ¼ dane'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'ZmieÅ„ tabelÄ™'),"Index_length"=>array("indexes",'ZmieÅ„ indeksy'),"Data_free"=>array("edit",'Nowy rekord'),"Auto_increment"=>array("auto_increment=1&create",'ZmieÅ„ tabelÄ™'),"Rows"=>array("select",'PokaÅ¼ dane'),)as$w=>$z){$r=" id='$w-".h($C)."'";echo($z?"<td align='right'>".(support("table")||$w=="Rows"||(support("indexes")&&$w!="Data_length")?"<a href='".h(ME."$z[0]=").urlencode($C)."'$r title='$z[1]'>?</a>":"<span$r>?</span>"):"<td id='$w-".h($C)."'>&nbsp;");}$S++;}echo(support("comment")?"<td id='Comment-".h($C)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d w sumie',count($Mg)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w)echo"<td align='right' id='sum-$w'>&nbsp;";echo"</table>\n";if(!information_schema(DB)){$Fh="<input type='submit' value='".'Vacuum'."'".on_help("'VACUUM'")."> ";$Ie="<input type='submit' name='optimize' value='".'Optymalizuj'."'".on_help($v=="sql"?"'OPTIMIZE TABLE'":"'VACUUM OPTIMIZE'")."> ";echo"<fieldset><legend>".'Selected'." <span id='selected'></span></legend><div>".($v=="sqlite"?$Fh:($v=="pgsql"?$Fh.$Ie:($v=="sql"?"<input type='submit' value='".'Analizuj'."'".on_help("'ANALYZE TABLE'")."> ".$Ie."<input type='submit' name='check' value='".'SprawdÅº'."'".on_help("'CHECK TABLE'")."> "."<input type='submit' name='repair' value='".'Napraw'."'".on_help("'REPAIR TABLE'")."> ":""))).(support("table")?"<input type='submit' name='truncate' value='".'OprÃ³Å¼nij'."'".confirm().on_help($v=="sqlite"?"'DELETE'":"'TRUNCATE".($v=="pgsql"?"'":" TABLE'"))."> ":"")."<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().on_help("'DROP TABLE'").">\n";$j=(support("scheme")?$b->schemas():$b->databases());if(count($j)!=1&&$v!="sqlite"){$k=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'PrzenieÅ› do innej bazy danych'.": ",($j?html_select("target",$j,$k):'<input name="target" value="'.h($k).'" autocapitalize="off">')," <input type='submit' name='move' value='".'PrzenieÅ›'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopiuj'."'>":""),"\n";}echo"<input type='hidden' name='all' value='' onclick=\"selectCount('selected', formChecked(this, /^(tables|views)\[/));".(support("table")?" selectCount('selected2', formChecked(this, /^tables\[/) || $S);":"")."\">\n";echo"<input type='hidden' name='token' value='$T'>\n","</div></fieldset>\n";}echo"</form>\n","<script type='text/javascript'>tableCheck();</script>\n";}echo'<p class="links"><a href="'.h(ME).'create=">'.'UtwÃ³rz tabelÄ™'."</a>\n",(support("view")?'<a href="'.h(ME).'view=">'.'UtwÃ³rz perspektywÄ™'."</a>\n":"");if(support("routine")){echo"<h3 id='routines'>".'Procedury i funkcje'."</h3>\n";$Zf=routines();if($Zf){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Nazwa'.'<td>'.'Typ'.'<td>'.'Zwracany typ'."<td>&nbsp;</thead>\n";odd('');foreach($Zf
