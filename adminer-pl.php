...
confirm($gb=""){return" onclick=\"return confirm('".'Czy jesteÅ› pewien?'.($gb?" (' + $gb + ')":"")."');\"";}function
...
upload_error($l){$vd=($l==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($l?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($vd?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$vd):""):'Plik nie istnieje.');}function
...
enum_input($S,$ya,$m,$X,$Lb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$m["length"],$qd);$H=($Lb!==null?"<label><input type='$S'$ya value='$Lb'".((is_array($X)?in_array($Lb,$X):$X===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($qd[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Na=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$H.=" <label><input type='$S'$ya value='".($q+1)."'".($Na?' checked':'').'>'.h($b->editVal($W,$m)).'</label>';}return$H;}function
input($m,$X,$p){global$h,$T,$b,$v;$A=h(bracket_escape($m["field"]));echo"<td class='function'>";$Xe=($v=="mssql"&&$m["auto_increment"]);if($Xe&&!$_POST["save"])$p=null;$vc=(isset($_GET["select"])||$Xe?array("orig"=>'bez zmian'):array())+$b->editFunctions($m);$ya=" name='fields[$A]'";if($m["type"]=="enum")echo
...
q($W);}function
check_sqlite_name($A){global$h;$dc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($dc)\$~",$A)){$h->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$dc));return
false;}return
true;}function
...
unconvert_field($m,$H){return$H;}function
support($fc){return
ereg("view|scheme|processlist|drop_col|variables|status",$fc);}$v="oracle";$T=array();$Af=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$Af[$w]=array_keys($W);}$rg=array();$Xd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$vc=array("length","lower","round","upper");$_c=array("avg","count","count distinct","max","min","sum");$Fb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Ab["mssql"]="MS SQL";if(isset($_GET["mssql"])){$Ae=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("auth[driver]",$Ab,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" autocapitalize="off">
...
selectLinks($Hf,$M=""){echo'<p class="tabs">';$ld=array("select"=>'PokaÅ¼ dane',"table"=>'Struktura tabeli');if(is_view($Hf))$ld["view"]='ZmieÅ„ perspektywÄ™';else$ld["create"]='ZmieÅ„ tabelÄ™';if($M!==null)$ld["edit"]='Nowy rekord';foreach($ld
...
selectVal($W,$z,$m){$H=($W===null?"<i>NULL</i>":(ereg("char|binary",$m["type"])&&!ereg("var",$m["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$m["type"])&&!is_utf8($W))$H=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen(html_entity_decode($W,ENT_QUOTES)));return($z?"<a href='".h($z)."'>$H</a>":$H);}function
editVal($W,$m){return$W;}function
selectColumnsPrint($K,$g){global$vc,$_c;print_fieldset("select",'pokaÅ¼',$K);$q=0;$tc=array('Funkcje'=>$vc,'Agregacje'=>$_c);foreach($K
...
messageQuery($F){global$v;restart_session();$Bc=&get_session("queries");$r="sql-".count($Bc[$_GET["db"]]);if(strlen($F)>1e6)$F=ereg_replace('[\x80-\xFF]+$','',substr($F,0,1e6))."\n...";$Bc[$_GET["db"]][]=array($F,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$r' onclick=\"return !toggle('$r');\">".'Zapytanie SQL'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($F,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Bc[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
as$w=>$W){if($W===null)$W="<i>NULL</i>";elseif($Ga[$w]&&!is_utf8($W))$W="<i>".lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$w]==254)$W="<code>$W</code>";}if(isset($ld[$w])&&!$g[$ld[$w]]){if($Ec){$O=$I[array_search("table=",$ld)];$z=$ld[$w].urlencode($ee[$O]!=""?$ee[$O]:$O);}else{$z="edit=".urlencode($ld[$w]);foreach($u[$ld[$w]]as$Sa=>$Uc)$z.="&where".urlencode("[".bracket_escape($Sa)."]")."=".urlencode($I[$Uc]);}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Brak rekordÃ³w.')."\n";return$H;}function
...
edit_type($w,$m,$Ua,$nc=array()){global$Af,$T,$rg,$Td;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$m["type"]||isset($T[$m["type"]])?array():array($m["type"]))+$Af+($nc?array('Klucze obce'=>$nc):array()),$m["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($m["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">';echo"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$m["type"])?"":" class='hidden'").'><option value="">('.'porÃ³wnywanie znakÃ³w'.')'.optionlist($Ua,$m["collation"]).'</select>',($rg?"<select name='$w"."[unsigned]'".(!$m["type"]||ereg('(int|float|double|decimal)$',$m["type"])?"":" class='hidden'").'><option>'.optionlist($rg,$m["unsigned"]).'</select>':''),(isset($m['on_update'])?"<select name='$w"."[on_update]'".($m["type"]=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'W przypadku zmiany'.")","CURRENT_TIMESTAMP"),$m["on_update"]).'</select>':''),($nc?"<select name='$w"."[on_delete]'".(ereg("`",$m["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usuniÄ™cia'.")".optionlist(explode("|",$Td),$m["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Za?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.4' alt='+' title='".'Dodaj nastÄ™pny'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($Za?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='".($h->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.4' alt='+' title='".'Dodaj nastÄ™pny'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.4' alt='^' title='".'PrzesuÅ„ w gÃ³rÄ™'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.4' alt='v' title='".'PrzesuÅ„ w dÃ³Å‚'."'>&nbsp;":""),($fe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.4' alt='x' title='".'UsuÅ„'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W)echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'ZmieÅ„'."</a>\n";echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Dodaj wyzwalacz'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$Kf=array();$Lf=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$qd,PREG_SET_ORDER);foreach($qd
...
<table cellspacing="0">
';$Ce=array();if(DB!=""){$Na=($a!=""?"":" checked");echo"<thead><tr>","<th style='text-align: left;'><label><input type='checkbox' id='check-tables'$Na onclick='formCheck(this, /^tables\\[/);'>".'Tabele'."</label>","<th style='text-align: right;'><label>".'Dane'."<input type='checkbox' id='check-data'$Na onclick='formCheck(this, /^data\\[/);'></label>","</thead>\n";$Y="";foreach(table_status()as$P){$A=$P["Name"];$Be=ereg_replace("_.*","",$A);$Na=($a==""||$a==(substr($a,-1)=="%"?"$Be%":$A));$Fe="<tr><td>".checkbox("tables[]",$A,$Na,$A,"checkboxClick(event, this); formUncheck('check-tables');");if(is_view($P))$Y.="$Fe\n";else
...
',checkbox("error_stops",1,$_POST["error_stops"],'Zatrzymaj w przypadku bÅ‚Ä™du')."\n",checkbox("only_errors",1,$_POST["only_errors"],'PokaÅ¼ tylko bÅ‚Ä™dy')."\n";print_fieldset("webfile",'Z serwera',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$ab=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W))$ab[]=".$w";}echo
sprintf('Plik %s na serwerze',"<code>adminer.sql".($ab?"[".implode("|",$ab)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Uruchom z pliku'.'">',"</div></fieldset>\n";if($Bc){print_fieldset("history",'Historia',$_GET["history"]!="");for($W=end($Bc);$W;$W=prev($Bc)){$w=key($Bc);list($E,$Tf)=$W;echo'<a href="'.h(ME."sql=&history=$w").'">'.'Edytuj'."</a> <span class='time' title='".@date('Y-m-d',$Tf)."'>".@date("H:i:s",$Tf)."</span> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$E)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'WyczyÅ›Ä‡'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Edytuj wszystkie'."</a>\n","</div></fieldset>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$A=>$m){if(!isset($m["privileges"][$sg?"update":"insert"])||$b->fieldName($m)=="")unset($n[$A]);}if($_POST&&!$l&&!isset($_GET["select"])){$md=$_POST["referer"];if($_POST["insert"])$md=($sg?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$md))$md=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$md,'Rekord zostaÅ‚ usuniÄ™ty.');else{$M=array();foreach($n
as$A=>$m){$W=process_input($m);if($W!==false&&$W!==null)$M[idf_escape($A)]=($sg?"\n".idf_escape($A)." = $W":$W);}if($sg){if(!$M)redirect($md);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$M),"\nWHERE $Z"),$md,'Rekord zostaÅ‚ zaktualizowany.');}else{$G=insert_into($a,$M);$ed=($G?last_id():0);queries_redirect($md,sprintf('Rekord%s zostaÅ‚ dodany.',($ed?" $ed":"")),$G);}}}$If=$b->tableName(table_status($a));page_header(($sg?'Edytuj':'Dodaj'),$l,array("select"=>array($a,$If)),$If);$I=null;if($_POST["save"])$I=(array)$_POST["fields"];elseif($Z){$K=array();foreach($n
...
';if($a==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Ob?html_select("Engine",array(""=>"(".'skÅ‚adowanie'.")")+$Ob,$I["Engine"]):""),' ',($Ua&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porÃ³wnywanie znakÃ³w'.")")+$Ua,$I["Collation"]):""),' <input type="submit" value="Zapisz zmiany">
...
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="UsuÅ„"',confirm(),'>';}if(support("partitioning")){$re=ereg('RANGE|LIST',$I["partition_by"]);print_fieldset("partition",'Partycjonowanie',$I["partition_by"]);echo'<p>
...
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$_POST["collation"]))$Cf=false;$dd=$k;}}queries_redirect(ME."db=".urlencode($dd),'Baza danych zostaÅ‚a utworzona.',$Cf);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych zostaÅ‚a zmieniona.');}}page_header(DB!=""?'ZmieÅ„ bazÄ™ danych':'UtwÃ³rz bazÄ™ danych',$l,array(),DB);$Ua=collations();$A=DB;$Ta=null;if($_POST){$A=$_POST["name"];$Ta=$_POST["collation"];}elseif(DB!="")$Ta=db_collation(DB,$Ua);elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$wc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$wc,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input name="name" id="name" value="'.h($A).'" maxlength="64" autocapitalize="off">')."\n".($Ua?html_select("collation",array(""=>"(".'porÃ³wnywanie znakÃ³w'.")")+$Ua,$Ta):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.4' alt='+' title='".'Dodaj nastÄ™pny'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];if($_POST&&!$l&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"])query_redirect("ALTER TABLE ".table($a)."\nDROP ".($v=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($a),'Klucz obcy zostaÅ‚ usuniÄ™ty.');else{$uf=array_filter($_POST["source"],'strlen');ksort($uf);$Pf=array();foreach($uf
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$I=($a==""?array():view($a));$I["name"]=$a;if($_POST){if(!$l){$A=trim($_POST["name"]);drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($A)." AS\n$_POST[select]","CREATE VIEW ".table($a)." AS\n$I[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($A)),'Perspektywa zostaÅ‚a usuniÄ™ta.','Perspektywa zostaÅ‚a zmieniona.','Perspektywa zostaÅ‚a utworzona.',$a);}$I=$_POST;}page_header(($a!=""?'ZmieÅ„ perspektywÄ™':'UtwÃ³rz perspektywÄ™'),$l,array("table"=>$a),$a);echo'
<form action="" method="post">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($h->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer' class='help'>".'Uprawnienia uÅ¼ytkownikÃ³w'."</a>";$q=0;foreach($xc
as$Nd=>$wc){echo'<th>'.($Nd!="*.*"?"<input name='objects[$q]' value='".h($Nd)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$eb=>$ub){foreach((array)$Ie[$eb]as$He=>$Ya){echo"<tr".odd()."><td".($ub?">$ub<td":" colspan='2'").' lang="en" title="'.h($Ya).'">'.h($He);$q=0;foreach($xc
as$Nd=>$wc){$A="'grants[$q][".h(strtoupper($He))."]'";$X=$wc[strtoupper($He)];if($eb=="Server Admin"&&$Nd!=(isset($xc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Uprawnienia'."<option value='0'".($X=="0"?" selected":"").">".'UsuÅ„ uprawnienia'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$l){$ad=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$ad++;}queries_redirect(ME."processlist=",lang(array('Przerwano %d wÄ…tek.','Przerwano %d wÄ…tki.','Przerwano %d wÄ…tkÃ³w.'),$ad),$ad||!$_POST["kill"]);}page_header('Lista procesÃ³w',$l);echo'
<form action="" method="post">
