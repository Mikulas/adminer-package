...
confirm($Gc="",$Jf=false){return" onclick=\"".($Jf?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($Gc?" (' + $Gc + ')":"")."');\"";}function
...
enum_input($z,$Ra,$d,$q,$Wb=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$ta);$c=(isset($Wb)?"<label><input type='$z'$Ra value='$Wb'".((is_array($q)?in_array($Wb,$q):$q===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($ta[1]as$l=>$b){$b=stripcslashes(str_replace("''","'",$b));$eb=(is_int($q)?$q==$l+1:(is_array($q)?in_array($l+1,$q):$q===$b));$c.=" <label><input type='$z'$Ra value='".($l+1)."'".($eb?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$L){global$Q,$o,$u;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Fe=($u=="mssql"&&$d["auto_increment"]);if($Fe&&!$_POST["save"]){$L=null;}$ba=(isset($_GET["select"])||$Fe?array("orig"=>'bez zmian'):array())+$o->editFunctions($d);$Ra=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
%
10!=1?1:2));return
sprintf($qf[$_c],$xc);}if(extension_loaded('pdo')){class
Min_PDO
extends
...
false;}return
true;}function
create_database($s,$U){global$g;if(file_exists($s)){$g->error='Plik już istnieje.';return
...
as$s){if(!@unlink($s)){$g->error='Plik już istnieje.';return
false;}}return
true;}function
...
set_schema($Ja){global$g,$Q,$Ia;$c=$g->query("SET search_path TO ".idf_escape($Ja));foreach(types()as$z){if(!isset($Q[$z])){$Q[$z]=0;$Ia['Typy użytkownika'][]=$z;}}return$c;}function
...
reset($H);}function
support($qb){return
ereg("view|drop_col|variables|status",$qb);}$u="oracle";$Q=array();$Ia=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$Q+=$b;$Ia[$e]=array_keys($b);}$jb=array();$ec=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$ba=array("length","lower","round","upper");$gb=array("avg","count","count distinct","max","min","sum");$Ob=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$la["mssql"]="MS SQL";if(isset($_GET["mssql"])){$oc=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("driver",$la,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($Yc,$r=""){echo'<p class="tabs">';$sa=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($Yc)){$sa["view"]='Zmień perspektywę';}else{$sa["create"]='Zmień tabelę';}if(isset($r)){$sa["edit"]='Nowy rekord';}foreach($sa
...
selectColumnsPrint($J,$B){global$ba,$gb;print_fieldset("select",'pokaż',$J);$l=0;$Ee=array('Funkcje'=>$ba,'Agregacje'=>$gb);foreach($J
...
selectSearchPrint($t,$B,$K){print_fieldset("search",'Szukaj',$t);foreach($K
...
selectLengthPrint($Kb){if(isset($Kb)){echo"<fieldset><legend>".'Długość tekstu'."</legend><div>",'<input name="text_length" size="3" value="'.h($Kb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($i){global$u;static$Gc=0;restart_session();$R="sql-".($Gc++);$Za=&get_session("queries");if(strlen($i)>1e6){$i=ereg_replace('[\x80-\xFF]+$','',substr($i,0,1e6))."\n...";}$Za[$_GET["db"]][]=$i;return" <a href='#$R' onclick=\"return !toggle('$R');\">".'Zapytanie SQL'."</a><div id='$R' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($i,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Za[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
explode("/",$c);}function
editInput($h,$d,$Ra,$q){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ra value='-1' checked><i>".'bez zmian'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ra value=''".(isset($q)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ra,$d,$q,0);}return"";}function
...
';if($Zb!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schemat'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$Zb){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$E=tables_list();if(!$E){echo"<p class='message'>".'Brak tabel.'."\n";}else{$this->tablesPrint($E);$sa=array();foreach($E
...
as$e=>$b){if(!isset($b)){$b="<i>NULL</i>";}elseif($Ae[$e]&&!is_utf8($b)){$b="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($b))."</i>";}elseif(!strlen($b)){$b="&nbsp;";}else{$b=h($b);if($Q[$e]==254){$b="<code>$b</code>";}}if(isset($sa[$e])&&!$B[$sa[$e]]){if($Kc){$x=$sa[$e].urlencode($a[array_search("table=",$sa)]);}else{$x="edit=".urlencode($sa[$e]);foreach($K[$sa[$e]]as$Ic=>$na){$x.="&where".urlencode("[".bracket_escape($Ic)."]")."=".urlencode($a[$na]);}}$b="<a href='".h(ME.$x)."'>$b</a>";}echo"<td>$b";}}echo($l?"</table>":"<p class='message'>".'Brak rekordów.')."\n";}function
...
format_time($Eb,$Cc){return" <span class='time'>(".sprintf('%.3f s',max(0,array_sum(explode(" ",$Cc))-array_sum(explode(" ",$Eb)))).")</span>";}function
edit_type($e,$d,$Z,$ga=array()){global$Ia,$Q,$jb,$db;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($Q[$d["type"]])?array():array($d["type"]))+$Ia+($ga?array('Klucze obce'=>$ga):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($Z,$d["collation"]).'</select>',($jb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($jb,$d["unsigned"]).'</select>':''),($ga?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist($db,$d["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Db?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
...
',(support("comment")?"<td".($Db?"":" class='hidden'")."><input name='fields[$l][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.1' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $Pe, 1);'>&nbsp;"."<input type='image' name='up[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.1' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.1' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($ld==""||support("drop_col")?"<input type='image' name='drop_col[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.1' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$e=>$b){echo"<tr valign='top'><td>$b[0]<td>$b[1]<th>".h($e)."<td><a href='".h(ME.'trigger='.urlencode($m).'&name='.urlencode($e))."'>".'Zmień'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($m).'">'.'Dodaj wyzwalacz'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),DB);$vb=array();$We=array();$f="adminer_schema";$Te=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$f-".DB]?"$f-".DB:$f)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$Te,$ta,PREG_SET_ORDER);foreach($ta
as$l=>$k){$vb[$k[1]]=array($k[2],$k[3]);$We[]="\n\t'".js_escape($k[1])."': [ $k[2], $k[3] ]";}$ac=0;$Ue=-1;$Ja=array();$bf=array();$df=array();foreach(table_status()as$a){if(!isset($a["Engine"])){continue;}$_c=0;$Ja[$a["Name"]]["fields"]=array();foreach(fields($a["Name"])as$f=>$d){$_c+=1.25;$d["pos"]=$_c;$Ja[$a["Name"]]["fields"][$f]=$d;}$Ja[$a["Name"]]["pos"]=($vb[$a["Name"]]?$vb[$a["Name"]]:array($ac,0));foreach($o->foreignKeys($a["Name"])as$b){if(!$b["db"]){$xa=$Ue;if($vb[$a["Name"]][1]||$vb[$b["table"]][1]){$xa=min(floatval($vb[$a["Name"]][1]),floatval($vb[$b["table"]][1]))-1;}else{$Ue-=.1;}while($df[(string)$xa]){$xa-=.0001;}$Ja[$a["Name"]]["references"][$b["table"]][(string)$xa]=array($b["source"],$b["target"]);$bf[$b["table"]][$a["Name"]][(string)$xa]=$b["target"];$df[(string)$xa]=true;}}$ac=max($ac,$Ja[$a["Name"]]["pos"][0]+2.5+$_c);}echo'<div id="schema" style="height: ',$ac,'em;">
<script type="text/javascript">
tablePos = {',implode(",",$We)."\n",'};
...
as$f=>$h){foreach((array)$h["references"]as$fc=>$yc){foreach($yc
...
as$f=>$d){if(!isset($d["privileges"][$oa?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$fa=$_POST["referer"];if($_POST["insert"]){$fa=($oa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$fa)){$fa=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $t"),$fa,'Rekord został usunięty.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($oa?"\n".idf_escape($f)." = $b":$b);}}if($oa){if(!$r){redirect($fa);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $t"),$fa,'Rekord został zaktualizowany.');}else{$j=insert_into($m,$r);$Yd=($j?last_id():0);queries_redirect($fa,sprintf('Rekord%s został dodany.',($Yd?" $Yd":"")),$j);}}}$Ca=$o->tableName(table_status($m));page_header(($oa?'Edytuj':'Dodaj'),$n,array("select"=>array($m,$Ca)),$Ca);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($t){$J=array();foreach($p
...
';if(support("partitioning")){$ke=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partycjonowanie',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$oe,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partycje: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($ke||!$a["partition_by"]?" class='hidden'":""),'>
...
as$s){if(count($_)==1||$s!=""){if(!create_database($s,$_POST["collation"])){$ge=false;}$xb=$s;}}queries_redirect(ME."db=".urlencode($xb),'Baza danych została utworzona.',$ge);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$n,array(),DB);$Z=collations();$f=DB;$mc=null;if($_POST){$f=$_POST["name"];$mc=$_POST["collation"];}elseif(DB!=""){$mc=db_collation(DB,$Z);}elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$ia){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ia,$k)&&$k[1]){$f=stripcslashes(idf_unescape("`$k[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Z?html_select("collation",array(""=>"(".'porównywanie znaków'.")")+$Z,$mc):"");?>
...
';}elseif(isset($_GET["foreign"])){$m=$_GET["foreign"];if($_POST&&!$n&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"]){query_redirect("ALTER TABLE ".table($m)."\nDROP ".($u=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($m),'Klucz obcy został usunięty.');}else{$Fa=array_filter($_POST["source"],'strlen');ksort($Fa);$da=array();foreach($Fa
as$e=>$b){$da[$e]=$_POST["target"][$e];}query_redirect("ALTER TABLE ".table($m).($_GET["name"]!=""?"\nDROP FOREIGN KEY ".idf_escape($_GET["name"]).",":"")."\nADD FOREIGN KEY (".implode(", ",array_map('idf_escape',$Fa)).") REFERENCES ".table($_POST["table"])." (".implode(", ",array_map('idf_escape',$da)).")".(in_array($_POST["on_delete"],$db)?" ON DELETE $_POST[on_delete]":"").(in_array($_POST["on_update"],$db)?" ON UPDATE $_POST[on_update]":""),ME."table=".urlencode($m),($_GET["name"]!=""?'Klucz obcy został zmieniony.':'Klucz obcy został utworzony.'));$n='Źródłowa i docelowa kolumna muszą być tego samego typu, powinien istnieć indeks na docelowej kolumnie oraz muszą istnieć dane referencyjne.'."<br>$n";}}page_header('Klucz obcy',$n,array("table"=>$m),$m);$a=array("table"=>$m,"source"=>array(""));if($_POST){$a=$_POST;ksort($a["source"]);if($_POST["add"]){$a["source"][]="";}elseif($_POST["change"]||$_POST["change-js"]){$a["target"]=array();}}elseif($_GET["name"]!=""){$ga=foreign_keys($m);$a=$ga[$_GET["name"]];$a["source"][]="";}$Fa=array_keys(fields($m));$da=($m===$a["table"]?$Fa:array_keys(fields($a["table"])));$ee=array();foreach(table_status()as$f=>$D){if(fk_support($D)){$ee[]=$f;}}echo'
<form action="" method="post">
<p>
...
';if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Zapisz zmiany">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["event"])){$ib=$_GET["event"];$de=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$td=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($ib),substr(ME,0,-1),'Wydarzenie zostało usunięte.');}elseif(in_array($_POST["INTERVAL_FIELD"],$de)&&isset($td[$_POST["STATUS"]])){$Zd="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($ib!=""?'Wydarzenie zostało zmienione.':'Wydarzenie zostało utworzone.'),queries(($ib!=""?"ALTER EVENT ".idf_escape($ib).$Zd.($ib!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Zd)."\n".$td[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($ib!=""?'Zmień wydarzenie'.": ".h($ib):'Utwórz wydarzenie'),$n);$a=array();if($_POST){$a=$_POST;}elseif($ib!=""){$H=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($ib));$a=reset($H);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$de,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$td,$a["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Zapisz zmiany">
';if($ib!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
<input type="submit" value="Zapisz zmiany">
';if($Pa!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
';}elseif(isset($_GET["type"])){$Dc=$_GET["type"];if($_POST&&!$n){$x=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Dc),$x,'Typ został usunięty.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$x,'Typ został utworzony.');}}page_header($Dc!=""?'Zmień typ'.": ".h($Dc):'Utwórz typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($Dc!=""){echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Zapisz zmiany'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["trigger"])){$m=$_GET["trigger"];$Hc=trigger_options();$be=array("INSERT","UPDATE","DELETE");$Ya=false;if($_POST&&!$n&&in_array($_POST["Timing"],$Hc["Timing"])&&in_array($_POST["Event"],$be)&&in_array($_POST["Type"],$Hc["Type"])){$ce=" $_POST[Timing] $_POST[Event]";$_b=" ON ".table($m);$Ya=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$_b:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$_b.$ce:$ce.$_b).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($m),'Wyzwalacz został usunięty.','Wyzwalacz został zmieniony.','Wyzwalacz został utworzony.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmień wyzwalacz'.": ".h($_GET["name"]):'Utwórz wyzwalacz'),$n,array("table"=>$m));$a=array("Trigger"=>$m."_bi");if($_POST){$a=$_POST;}elseif($_GET["name"]!=""){$a=trigger($_GET["name"]);}echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$Hc["Timing"],$a["Timing"],"if (/^".preg_quote($m,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($m)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$be,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Hc["Type"],$a["Type"]),'</table>
<p>Nazwa: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($Ya){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
as$b){if($b[1]!="USAGE"){$nb["$k[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$nb["$k[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$k)){$Sc=$k[1];}}}if($_POST&&!$n){$cc=(isset($_GET["host"])?q($xd)."@".q($_GET["host"]):"''");$lb=q($_POST["user"])."@".q($_POST["host"]);$ud=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $cc",ME."privileges=",'Użytkownik został usunięty.');}else{if($cc!=$lb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $lb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $ud");}elseif($_POST["pass"]!=$Sc||!$_POST["hashed"]){queries("SET PASSWORD FOR $lb = ".($_POST["hashed"]?$ud:"PASSWORD($ud)"));}if(!$n){$tc=array();foreach($lc
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$l=0;foreach($nb
as$ab=>$ia){echo'<th>'.($ab!="*.*"?"<input name='objects[$l]' value='".h($ab)."' size='10'>":"<input type='hidden' name='objects[$l]' value='*.*' size='10'>*.*");$l++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$hd=>$sc){foreach((array)$ra[$hd]as$zc=>$va){echo"<tr".odd()."><td".($sc?">$sc<td":" colspan='2'").' lang="en" title="'.h($va).'">'.h($zc);$l=0;foreach($nb
as$ab=>$ia){$f="'grants[$l][".h(strtoupper($zc))."]'";$q=$ia[strtoupper($zc)];if($hd=="Server Admin"&&$ab!=(isset($nb["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($q?" selected":"").">".'Uprawnienia'."<option value='0'".($q=="0"?" selected":"").">".'Usuń uprawnienia'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($q?" checked":"").($zc=="All privileges"?" id='grants-$l-all'":($zc=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$l-all');\"")).">";}$l++;}}}echo"</table>\n",'<p>
<input type="submit" value="Zapisz zmiany">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Bd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$Bd++;}}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$Bd),$Bd||!$_POST["kill"]);}page_header('Lista procesów',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".(($u=="sql"?$e=="Info"&&$b!="":$e=="current_query"&&$b!="<IDLE>")?"<code class='jush-$u'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Edytuj'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$Va,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Eksport');$Na=$o->dumpOutput();echo($Na?html_select("output",$Na,$Fc["output"])." ":""),html_select("format",$o->dumpFormat(),$Fc["format"])," <input type='submit' name='export' value='".'Eksport'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($o->selectImportPrint()){print_fieldset("import",'Import',!$H);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$Fc["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$O'>\n","</div></fieldset>\n";}$o->selectEmailPrint(array_filter($Oc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Tb=isset($_GET["status"]);page_header($Tb?'Status':'Zmienne');$fe=($Tb?show_status():show_variables());if(!$fe){echo"<p class='message'>".'Brak rekordów.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($fe
