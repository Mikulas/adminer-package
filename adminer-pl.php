...
confirm($ab="",$bf=false){return" onclick=\"".($bf?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($ab?" (' + $ab + ')":"")."');\"";}function
...
upload_error($k){$ed=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Wgranie pliku było niemożliwe.'.($ed?" ".sprintf('Maksymalna wielkość pliku to %sB.',$ed):""):'Plik nie istnieje.');}function
...
enum_input($S,$xa,$l,$X,$Cb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Zc);$G=(isset($Cb)?"<label><input type='$S'$xa value='$Cb'".((is_array($X)?in_array($Cb,$X):$X===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($Zc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ja=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($q+1)."'".($Ja?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$v;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$Be=($v=="mssql"&&$l["auto_increment"]);if($Be&&!$_POST["save"]){$o=null;}$lc=(isset($_GET["select"])||$Be?array("orig"=>'bez zmian'):array())+$b->editFunctions($l);$xa=" name='fields[$A]'";if($l["type"]=="enum"){echo
...
q($W);}function
check_sqlite_name($A){global$g;$Ub="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ub)\$~",$A)){$g->error=sprintf('Proszę użyć jednego z rozszerzeń: %s.',str_replace("|",", ",$Ub));return
false;}return
true;}function
...
set_schema($Le){global$g,$T,$df;$G=$g->query("SET search_path TO ".idf_escape($Le));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$df['Typy użytkownika'][]=$S;}}return$G;}function
...
reset($I);}function
support($Wb){return
ereg("view|drop_col|variables|status",$Wb);}$v="oracle";$T=array();$df=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$df[$w]=array_keys($W);}$Rf=array();$Ed=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$lc=array("length","lower","round","upper");$pc=array("avg","count","count distinct","max","min","sum");$yb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ge=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("driver",$sb,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($kf,$L=""){echo'<p class="tabs">';$Uc=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($kf)){$Uc["view"]='Zmień perspektywę';}else{$Uc["create"]='Zmień tabelę';}if(isset($L)){$Uc["edit"]='Nowy rekord';}foreach($Uc
...
selectColumnsPrint($J,$f){global$lc,$pc;print_fieldset("select",'pokaż',$J);$q=0;$kc=array('Funkcje'=>$lc,'Agregacje'=>$pc);foreach($J
...
false;}function
messageQuery($E){global$v;static$ab=0;restart_session();$r="sql-".($ab++);$rc=&get_session("queries");if(strlen($E)>1e6){$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";}$rc[$_GET["db"]][]=$E;return" <a href='#$r' onclick=\"return !toggle('$r');\">".'Zapytanie SQL'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($rc[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
';if($nd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schemat'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$nd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$Q=tables_list();if(!$Q){echo"<p class='message'>".'Brak tabel.'."\n";}else{$this->tablesPrint($Q);$Uc=array();foreach($Q
...
as$w=>$W){if(!isset($W)){$W="<i>NULL</i>";}elseif($Da[$w]&&!is_utf8($W)){$W="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($W))."</i>";}elseif(!strlen($W)){$W="&nbsp;";}else{$W=h($W);if($T[$w]==254){$W="<code>$W</code>";}}if(isset($Uc[$w])&&!$f[$Uc[$w]]){if($uc){$z=$Uc[$w].urlencode($H[array_search("table=",$Uc)]);}else{$z="edit=".urlencode($Uc[$w]);foreach($u[$Uc[$w]]as$Ma=>$Hc){$z.="&where".urlencode("[".bracket_escape($Ma)."]")."=".urlencode($H[$Hc]);}}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Brak rekordów.')."\n";}function
...
edit_type($w,$l,$Oa,$ec=array()){global$df,$T,$Rf,$Ad;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$df+($ec?array('Klucze obce'=>$ec):array()),$l["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($Oa,$l["collation"]).'</select>',($Rf?"<select name='$w"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Rf,$l["unsigned"]).'</select>':''),($ec?"<select name='$w"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist(explode("|",$Ad),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Ta?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($Ld==""||support("drop_col")?"<input type='image' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$w=>$W){echo"<tr valign='top'><td>$W[0]<td>$W[1]<th>".h($w)."<td><a href='".h(ME.'trigger='.urlencode($a).'&name='.urlencode($w))."'>".'Zmień'."</a>\n";}echo"</table>\n";}echo'<p><a href="'.h(ME).'trigger='.urlencode($a).'">'.'Dodaj wyzwalacz'."</a>\n";}}}}elseif(isset($_GET["schema"])){page_header('Schemat bazy danych',"",array(),DB.($_GET["ns"]?".$_GET[ns]":""));$nf=array();$of=array();$A="adminer_schema";$ea=($_GET["schema"]?$_GET["schema"]:$_COOKIE[($_COOKIE["$A-".DB]?"$A-".DB:$A)]);preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~',$ea,$Zc,PREG_SET_ORDER);foreach($Zc
...
as$A=>$l){if(!isset($l["privileges"][$Sf?"update":"insert"])||$b->fieldName($l)==""){unset($m[$A]);}}if($_POST&&!$k&&!isset($_GET["select"])){$Vc=$_POST["referer"];if($_POST["insert"]){$Vc=($Sf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$Vc)){$Vc=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$Vc,'Rekord został usunięty.');}else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null){$L[idf_escape($A)]=($Sf?"\n".idf_escape($A)." = $W":$W);}}if($Sf){if(!$L){redirect($Vc);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$Vc,'Rekord został zaktualizowany.');}else{$F=insert_into($a,$L);$Oc=($F?last_id():0);queries_redirect($Vc,sprintf('Rekord%s został dodany.',($Oc?" $Oc":"")),$F);}}}$lf=$b->tableName(table_status($a));page_header(($Sf?'Edytuj':'Dodaj'),$k,array("select"=>array($a,$lf)),$lf);$H=null;if($_POST["save"]){$H=(array)$_POST["fields"];}elseif($Z){$J=array();foreach($m
...
';if($m){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Sf?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}}echo($Sf?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$Wd=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Partycjonowanie',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Vd,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Partycje: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($Wd||!$H["partition_by"]?" class='hidden'":""),'>
...
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!=""){echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";}elseif(!$_POST["add_x"]&&$_GET["db"]==""){echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Dodaj następny'."'>\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+explode("|",$Ad),$H["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+explode("|",$Ad),$H["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ec,$H["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$af,$H["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($H["name"]),'" maxlength="64">
',($Ge?'Język'.": ".html_select("language",$Ge,$H["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($H["fields"],$Oa,$Fe);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$H["returns"],$Oa);}echo'</table>
<p>';textarea("definition",$H["definition"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($da!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($ub){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$z=substr(ME,0,-1);$A=trim($_POST["name"]);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekwencja została usunięta.');}elseif($fa==""){query_redirect("CREATE SEQUENCE ".idf_escape($A),$z,'Sekwencja została utworzona.');}elseif($fa!=$A){query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$z,'Sekwencja została zmieniona.');}else{redirect($z);}}page_header($fa!=""?'Zmień sekwencję'.": ".h($fa):'Utwórz sekwencję',$k);$H=$_POST;if(!$H){$H=array("name"=>$fa);}echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Zapisz zmiany">
';if($fa!=""){echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ został usunięty.');}else{query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$z,'Typ został utworzony.');}}page_header($ga!=""?'Zmień typ'.": ".h($ga):'Utwórz typ',$k);$H=$_POST;if(!$H){$H=array("as"=>"AS ");}echo'
<form action="" method="post">
<p>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$Hf["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$Gf,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Hf["Type"],$H["Type"]),'</table>
<p>Nazwa: <input name="Trigger" value="',h($H["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$H["Statement"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($ub){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
as$W){if($W[1]!="USAGE"){$mc["$_[2]$W[2]"][$W[1]]=true;}if(ereg(' WITH GRANT OPTION',$H[0])){$mc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_)){$yd=$_[1];}}}if($_POST&&!$k){$zd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$td=q($_POST["user"])."@".q($_POST["host"]);$Zd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $zd",ME."privileges=",'Użytkownik został usunięty.');}else{$db=false;if($zd!=$td){$db=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $td IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Zd");$k=!$db;}elseif($_POST["pass"]!=$yd||!$_POST["hashed"]){queries("SET PASSWORD FOR $td = ".($_POST["hashed"]?$Zd:"PASSWORD($Zd)"));}if(!$k){$Ce=array();foreach($sd
...
as$wd=>$Ce){if(preg_match('~^(.+)(\\(.*\\))?$~U',$wd,$_)){grant("REVOKE",array_keys($Ce),$_[2]," ON $_[1] FROM $td");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Użytkownik został zmieniony.':'Użytkownik został dodany.'),!$k);if($db){$g->query("DROP USER $td");}}}page_header((isset($_GET["host"])?'Użytkownik'.": ".h("$ha@$_GET[host]"):'Dodaj użytkownika'),$k,array("privileges"=>array('','Uprawnienia użytkowników')));if($_POST){$H=$_POST;$mc=$sd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$yd;if($yd!=""){$H["hashed"]=true;}$mc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$q=0;foreach($mc
as$wd=>$p){echo'<th>'.($wd!="*.*"?"<input name='objects[$q]' value='".h($wd)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$Ya=>$nb){foreach((array)$ne[$Ya]as$me=>$Sa){echo"<tr".odd()."><td".($nb?">$nb<td":" colspan='2'").' lang="en" title="'.h($Sa).'">'.h($me);$q=0;foreach($mc
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Kc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W))){$Kc++;}}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$Kc),$Kc||!$_POST["kill"]);}page_header('Lista procesów',$k);echo'
<form action="" method="post">
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$hc,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}$fc=$b->dumpFormat();if($fc){print_fieldset("export",'Eksport');$Qd=$b->dumpOutput();echo($Qd?html_select("output",$Qd,$qa["output"])." ":""),html_select("format",$fc,$qa["format"])," <input type='submit' name='export' value='".'Eksport'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Import',!$I);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$R'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Bb,'strlen'),$f);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Ze=isset($_GET["status"]);page_header($Ze?'Status':'Zmienne');$Xf=($Ze?show_status():show_variables());if(!$Xf){echo"<p class='message'>".'Brak rekordów.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($Xf
...
as$w=>$W){json_row("sum-$w",number_format($W,0,'.',' '));}json_row("");}else{foreach(count_tables($b->databases())as$j=>$W){json_row("tables-".js_escape($j),$W);}json_row("");}exit;}else{$rf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($rf&&!$k&&!$_POST["search"]){$F=true;$gd="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$F=truncate_tables($_POST["tables"]);}$gd='Tabele zostały opróżnione.';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$gd='Tabele zostały przeniesione.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$gd='Tabele zostały skopiowane.';}elseif($_POST["drop"]){if($_POST["views"]){$F=drop_views($_POST["views"]);}if($F&&$_POST["tables"]){$F=drop_tables($_POST["tables"]);}$gd='Tabele zostały usunięte.';}elseif($v!="sql"){$F=($v=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$gd='Tables have been optimized.';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc()){$gd.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$gd,$F);}page_header(($_GET["ns"]==""?'Baza danych'.": ".h(DB):'Schemat'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabele i perspektywy'."</h3>\n";$qf=tables_list();if(!$qf){echo"<p class='message'>".'Brak tabel.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Wyszukaj we wszystkich tabelach'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Szukaj'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabela','<td>'.'Składowanie','<td>'.'Porównywanie znaków','<td>'.'Rozmiar danych','<td>'.'Rozmiar indeksów','<td>'.'Wolne miejsce','<td>'.'Auto Increment','<td>'.'Liczba rekordów',(support("comment")?'<td>'.'Komentarz':''),"</thead>\n";foreach($qf
as$A=>$S){$Zf=(isset($S)&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($Zf?"views[]":"tables[]"),$A,in_array($A,$rf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($A).'" title="'.'Struktura tabeli'.'">'.h($A).'</a>';if($Zf){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($A).'" title="'.'Zmień perspektywę'.'">'.'Perspektywa'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($A).'" title="'.'Pokaż dane'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmień tabelę'),"Index_length"=>array("indexes",'Zmień indeksy'),"Data_free"=>array("edit",'Nowy rekord'),"Auto_increment"=>array("auto_increment=1&create",'Zmień tabelę'),"Rows"=>array("select",'Pokaż dane'),)as$w=>$z){echo($z?"<td align='right'><a href='".h(ME."$z[0]=").urlencode($A)."' id='$w-".h($A)."' title='$z[1]'>?</a>":"<td id='$w-".h($A)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($A)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d w sumie',count($qf)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$w){echo"<td align='right' id='sum-$w'>&nbsp;";}echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$v)?($v!="sqlite"?"<input type='submit' value='".'Analizuj'."'> ":"")."<input type='submit' name='optimize' value='".'Optymalizuj'."'> ":"").($v=="sql"?"<input type='submit' name='check' value='".'Sprawdź'."'> <input type='submit' name='repair' value='".'Napraw'."'> ":"")."<input type='submit' name='truncate' value='".'Opróżnij'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Usuń'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$v!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Przenieś do innej bazy danych'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'">')," <input type='submit' name='move' value='".'Przenieś'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopiuj'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Utwórz tabelę'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Utwórz perspektywę'."</a>\n";}if(support("routine")){echo"<h3>".'Procedury i funkcje'."</h3>\n";$He=routines();if($He){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Nazwa'.'<td>'.'Typ'.'<td>'.'Zwracany typ'."<td>&nbsp;</thead>\n";odd('');foreach($He
