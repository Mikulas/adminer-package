...
nl_br($I){return
str_replace("\n","<br>",$I);}function
checkbox($f,$q,$db,$Ke="",$Jc="",$Ff=false){static$R=0;$R++;$c="<input type='checkbox' name='$f' value='".h($q)."'".($db?" checked":"").($Jc?' onclick="'.h($Jc).'"':'').($Ff?" class='jsonly'":"")." id='checkbox-$R'>";return($Ke!=""?"<label for='checkbox-$R'>$c".h($Ke)."</label>":$c);}function
optionlist($Dc,$Lf=null,$Ue=false){$c="";foreach($Dc
...
html_select($f,$Dc,$q="",$Gb=true){if($Gb){return"<select name='".h($f)."'".(is_string($Gb)?' onchange="'.h($Gb).'"':"").">".optionlist($Dc,$q)."</select>";}$c="";foreach($Dc
as$e=>$b){$c.="<label><input type='radio' name='".h($f)."' value='".h($e)."'".($e==$q?" checked":"").">".h($b)."</label>";}return$c;}function
confirm($Sc="",$Tf=false){return" onclick=\"".($Tf?"eventStop(event); ":"")."return confirm('".'Czy jesteś pewien?'.($Sc?" (' + $Sc + ')":"")."');\"";}function
print_fieldset($R,$Uf,$Vf=false,$Jc=""){echo"<fieldset><legend><a href='#fieldset-$R' onclick=\"".h($Jc)."return !toggle('fieldset-$R');\">$Uf</a></legend><div id='fieldset-$R'".($Vf?"":" class='hidden'").">\n";}function
...
query_redirect($i,$fa,$ta,$fd=true,$lf=true,$ne=false){global$g,$n,$o;if($lf){$ne=!$g->query($i);}$Ld="";if($i){$Ld=$o->messageQuery("$i;");}if($ne){$n=error().$Ld;return
false;}if($fd){redirect($fa,$ta.$Ld);}return
true;}function
...
upload_error($n){$ye=($n==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($n?'Wgranie pliku było niemożliwe.'.($ye?" ".sprintf('Maksymalna wielkość pliku to %sB.',$ye):""):'Plik nie istnieje.');}function
...
enum_input($_,$Ua,$d,$q,$ac=null){global$o;preg_match_all("~'((?:[^']|'')*)'~",$d["length"],$sa);$c=(isset($ac)?"<label><input type='$_'$Ua value='$ac'".((is_array($q)?in_array($ac,$q):$q===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($sa[1]as$l=>$b){$b=stripcslashes(str_replace("''","'",$b));$db=(is_int($q)?$q==$l+1:(is_array($q)?in_array($l+1,$q):$q===$b));$c.=" <label><input type='$_'$Ua value='".($l+1)."'".($db?' checked':'').'>'.h($o->editVal($b,$d)).'</label>';}return$c;}function
input($d,$q,$N){global$S,$o,$v;$f=h(bracket_escape($d["field"]));echo"<td class='function'>";$Fe=($v=="mssql"&&$d["auto_increment"]);if($Fe&&!$_POST["save"]){$N=null;}$aa=(isset($_GET["select"])||$Fe?array("orig"=>'bez zmian'):array())+$o->editFunctions($d);$Ua=" name='fields[$f]'";if($d["type"]=="enum"){echo
...
password_file(){$Rc=ini_get("upload_tmp_dir");if(!$Rc){if(function_exists('sys_get_temp_dir')){$Rc=sys_get_temp_dir();}else{$ka=@tempnam("","");if(!$ka){return
false;}$Rc=dirname($ka);unlink($ka);}}$ka="$Rc/adminer.key";$c=@file_get_contents($ka);if($c){return$c;}$Ja=@fopen($ka,"w");if($Ja){$c=md5(uniqid(mt_rand(),true));fwrite($Ja,$c);fclose($Ja);}return$c;}function
is_mail($uf){$Ce='[-a-z0-9!#$%&\'*+/=?^_`{|}~]';$zc='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';$ca="$Ce+(\\.$Ce+)*@($zc?\\.)+$zc";return
preg_match("(^$ca(,\\s*$ca)*\$)i",$uf);}function
is_url($I){$zc='[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';return(preg_match("~^(https?)://($zc?\\.)+$zc(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i",$I,$k)?strtolower($k[1]):"");}global$o,$g,$ma,$Ob,$Cb,$n,$aa,$kb,$xc,$cc,$v,$Bf,$Wf,$Ea,$Na,$O,$Xf,$S,$jb,$Bc;if(!isset($_SERVER["REQUEST_URI"])){$_SERVER["REQUEST_URI"]=$_SERVER["ORIG_PATH_INFO"].($_SERVER["QUERY_STRING"]!=""?"?$_SERVER[QUERY_STRING]":"");}$xc=$_SERVER["HTTPS"]&&strcasecmp($_SERVER["HTTPS"],"off");@ini_set("session.use_trans_sid",false);if(!defined("SID")){session_name("adminer_sid");$mc=array(0,preg_replace('~\\?.*~','',$_SERVER["REQUEST_URI"]),"",$xc);if(version_compare(PHP_VERSION,'5.2.0')>=0){$mc[]=true;}call_user_func_array('session_set_cookie_params',$mc);session_start();}remove_slashes(array(&$_GET,&$_POST,&$_COOKIE),$cd);if(function_exists("set_magic_quotes_runtime")){set_magic_quotes_runtime(false);}@set_time_limit(0);@ini_set("zend.ze1_compatibility_mode",false);@ini_set("precision",20);function
get_lang(){return'pl';}function
lang($Qf,$Ac){$yc=($Ac==1?0:($Ac
%
10>1&&$Ac
%
10<5&&$Ac/10
%
10!=1?1:2));return
sprintf($Qf[$yc],$Ac);}if(extension_loaded('pdo')){class
Min_PDO
extends
...
q($b);}function
check_sqlite_name($f){global$g;$He="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($He)\$~",$f)){$g->error=sprintf('Proszę użyć jednego z rozszerzeń: %s.',str_replace("|",", ",$He));return
false;}return
true;}function
...
set_schema($Oa){global$g,$S,$Na;$c=$g->query("SET search_path TO ".idf_escape($Oa));foreach(types()as$_){if(!isset($S[$_])){$S[$_]=0;$Na['Typy użytkownika'][]=$_;}}return$c;}function
...
reset($H);}function
support($ub){return
ereg("view|drop_col|variables|status",$ub);}$v="oracle";$S=array();$Na=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$e=>$b){$S+=$b;$Na[$e]=array_keys($b);}$jb=array();$gc=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$aa=array("length","lower","round","upper");$kb=array("avg","count","count distinct","max","min","sum");$Ob=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ma["mssql"]="MS SQL";if(isset($_GET["mssql"])){$_c=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("driver",$ma,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="server" value="',h(SERVER),'" title="hostname[:port]">
...
selectLinks($bd,$r=""){echo'<p class="tabs">';$ua=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($bd)){$ua["view"]='Zmień perspektywę';}else{$ua["create"]='Zmień tabelę';}if(isset($r)){$ua["edit"]='Nowy rekord';}foreach($ua
...
selectQuery($i){global$v;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Ostatnia strona'."'>&gt;&gt;</a> <code class='jush-$v'>".h(str_replace("\n"," ",$i))."</code> <a href='".h(ME)."sql=".urlencode($i)."'>".'Edytuj'."</a></p>\n";}function
rowDescription($h){return"";}function
rowDescriptions($H,$Cf){return$H;}function
selectVal($b,$y,$d){$c=($b!="<i>NULL</i>"&&ereg("char|binary",$d["type"])&&!ereg("var",$d["type"])?"<code>$b</code>":$b);if(ereg('blob|bytea|raw|file',$d["type"])&&!is_utf8($b)){$c=lang(array('%d bajt','%d bajty','%d bajtów'),strlen(html_entity_decode($b,ENT_QUOTES)));}return($y?"<a href='$y'>$c</a>":$c);}function
editVal($b,$d){return(ereg("binary",$d["type"])?reset(unpack("H*",$b)):$b);}function
selectColumnsPrint($J,$B){global$aa,$kb;print_fieldset("select",'pokaż',$J);$l=0;$Je=array('Funkcje'=>$aa,'Agregacje'=>$kb);foreach($J
...
selectSearchPrint($s,$B,$K){print_fieldset("search",'Szukaj',$s);foreach($K
as$l=>$x){if($x["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$x["columns"]))."</i>) AGAINST"," <input name='fulltext[$l]' value='".h($_GET["fulltext"][$l])."'>",checkbox("boolean[$l]",1,isset($_GET["boolean"][$l]),"BOOL"),"<br>\n";}}$l=0;foreach((array)$_GET["where"]as$b){if("$b[col]$b[val]"!=""&&in_array($b["op"],$this->operators)){echo"<div><select name='where[$l][col]'><option value=''>(".'gdziekolwiek'.")".optionlist($B,$b["col"],true)."</select>",html_select("where[$l][op]",$this->operators,$b["op"]),"<input name='where[$l][val]' value='".h($b["val"])."'></div>\n";$l++;}}echo"<div><select name='where[$l][col]' onchange='selectAddRow(this);'><option value=''>(".'gdziekolwiek'.")".optionlist($B,null,true)."</select>",html_select("where[$l][op]",$this->operators,"="),"<input name='where[$l][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($cb,$B,$K){print_fieldset("sort",'Sortuj',$cb);$l=0;foreach((array)$_GET["order"]as$e=>$b){if(isset($B[$b])){echo"<div><select name='order[$l]'><option>".optionlist($B,$b,true)."</select>",checkbox("desc[$l]",1,isset($_GET["desc"][$e]),'malejąco')."</div>\n";$l++;}}echo"<div><select name='order[$l]' onchange='selectAddRow(this);'><option>".optionlist($B,null,true)."</select>","<label><input type='checkbox' name='desc[$l]' value='1'>".'malejąco'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($M){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($M)."'>","</div></fieldset>\n";}function
selectLengthPrint($Nb){if(isset($Nb)){echo"<fieldset><legend>".'Długość tekstu'."</legend><div>",'<input name="text_length" size="3" value="'.h($Nb).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($i){global$v;static$Sc=0;restart_session();$R="sql-".($Sc++);$Wa=&get_session("queries");if(strlen($i)>1e6){$i=ereg_replace('[\x80-\xFF]+$','',substr($i,0,1e6))."\n...";}$Wa[$_GET["db"]][]=$i;return" <a href='#$R' onclick=\"return !toggle('$R');\">".'Zapytanie SQL'."</a><div id='$R' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($i,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($Wa[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
explode("/",$c);}function
editInput($h,$d,$Ua,$q){if($d["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$Ua value='-1' checked><i>".'bez zmian'."</i></label> ":"").($d["null"]?"<label><input type='radio'$Ua value=''".(isset($q)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ua,$d,$q,0);}return"";}function
...
true;}function
navigation($Ub){global$Bc,$g,$O,$v,$ma;echo'<h1>
',$this->name(),' <span class="version">',$Bc,'</span>
...
';if($Ub!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schemat'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$Ub){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$E=tables_list();if(!$E){echo"<p class='message'>".'Brak tabel.'."\n";}else{$this->tablesPrint($E);$ua=array();foreach($E
...
as$h=>$_){echo'<a href="'.h(ME).'select='.urlencode($h).'"'.bold($_GET["select"]==$h).">".'przeglądaj'."</a> ",'<a href="'.h(ME).'table='.urlencode($h).'"'.bold($_GET["table"]==$h).">".$this->tableName(array("Name"=>$h))."</a><br>\n";}}}$o=(function_exists('adminer_object')?adminer_object():new
...
edit_type($e,$d,$Z,$ha=array()){global$Na,$S,$jb,$Ea;echo'<td><select name="',$e,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$d["type"]||isset($S[$d["type"]])?array():array($d["type"]))+$Na+($ha?array('Klucze obce'=>$ha):array()),$d["type"]),'</select>
<td><input name="',$e,'[length]" value="',h($d["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$e"."[collation]'".(ereg('(char|text|enum|set)$',$d["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($Z,$d["collation"]).'</select>',($jb?"<select name='$e"."[unsigned]'".(!$d["type"]||ereg('(int|float|double|decimal)$',$d["type"])?"":" class='hidden'").'><option>'.optionlist($jb,$d["unsigned"]).'</select>':''),($ha?"<select name='$e"."[on_delete]'".(ereg("`",$d["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist(explode("|",$Ea),$d["on_delete"])."</select> ":" ");}function
...
';if($_=="PROCEDURE"){echo'<td>&nbsp;';}echo'<th>',($_=="TABLE"?'Nazwa kolumny':'Nazwa parametru'),'<td>Typ<textarea id="enum-edit" rows="4" cols="12" wrap="off" style="display: none;" onblur="editingLengthBlur(this);"></textarea>
<td>Długość
<td>Opcje
...
',(support("comment")?"<td".($_b?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($p))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($p),';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';foreach($p
as$l=>$d){$l++;$pd=$d[($_POST?"orig":"field")];$Me=(isset($_POST["add"][$l-1])||(isset($d["field"])&&!$_POST["drop_col"][$l]))&&(support("drop_col")||$pd=="");echo'<tr',($Me?"":" style='display: none;'"),'>
',($_=="PROCEDURE"?"<td>".html_select("fields[$l][inout]",explode("|",$cc),$d["inout"]):""),'<th>';if($Me){echo'<input name="fields[',$l,'][field]" value="',h($d["field"]),'" onchange="',($d["field"]!=""||count($p)>1?"":"editingAddRow(this, $Ne); "),'editingNameChange(this);" maxlength="64">';}echo'<input type="hidden" name="fields[',$l,'][orig]" value="',h($pd),'">
...
',(support("comment")?"<td".($_b?"":" class='hidden'")."><input name='fields[$l][comment]' value='".h($d["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.2' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $Ne, 1);'>&nbsp;"."<input type='image' name='up[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.2' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.2' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($pd==""||support("drop_col")?"<input type='image' name='drop_col[$l]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.2' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$ya=>$ad){$Yc=$Yb;$nd=-10;foreach($ad[0]as$e=>$Ca){$Qd=$h["pos"][0]+$h["fields"][$Ca]["pos"];$Vd=$Oa[$bc]["pos"][0]+$Oa[$bc]["fields"][$ad[1][$e]]["pos"];$Yc=min($Yc,$Qd,$Vd);$nd=max($nd,$Qd,$Vd);}echo"<div class='references' id='refl$ya' style='left: $ya"."em; top: $Yc"."em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: ".($nd-$Yc)."em;'></div></div>\n";}}}echo'</div>
<p><a href="',h(ME."schema=".urlencode($Te)),'" id="schema-link">Trwały link</a>
';}elseif(isset($_GET["dump"])){$m=$_GET["dump"];if($_POST){$Xd="";foreach(array("output","format","db_style","routines","events","table_style","auto_increment","triggers","data_style")as$e){$Xd.="&$e=".urlencode($_POST[$e]);}cookie("adminer_export",substr($Xd,1));$Hb=dump_headers(($m!=""?$m:DB),(DB==""||count((array)$_POST["tables"]+(array)$_POST["data"])>1));$qb=($_POST["format"]=="sql");if($qb){echo"-- Adminer $Bc ".$ma[DRIVER]." dump
...
<input type="hidden" name="token" value="',$O,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zatrzymaj w przypadku błędu')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Pokaż tylko błędy')."\n";print_fieldset("webfile",'Z serwera',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$yd=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$e=>$b){if(extension_loaded($b)){$yd[]=".$e";}}echo
sprintf('Plik %s na serwerze',"<code>adminer.sql".($yd?"[".implode("|",$yd)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Uruchom z pliku'.'">',"</div></fieldset>\n";if($Wa){print_fieldset("history",'Historia',$_GET["history"]!="");foreach($Wa
as$e=>$b){echo'<a href="'.h(ME."sql=&history=$e").'">'.'Edytuj'."</a> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$b)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Wyczyść'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Edytuj wszystkie'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$f=>$d){if(!isset($d["privileges"][$pa?"update":"insert"])||$o->fieldName($d)==""){unset($p[$f]);}}if($_POST&&!$n&&!isset($_GET["select"])){$fa=$_POST["referer"];if($_POST["insert"]){$fa=($pa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$fa)){$fa=ME."select=".urlencode($m);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($m)," WHERE $s"),$fa,'Rekord został usunięty.');}else{$r=array();foreach($p
as$f=>$d){$b=process_input($d);if($b!==false&&$b!==null){$r[idf_escape($f)]=($pa?"\n".idf_escape($f)." = $b":$b);}}if($pa){if(!$r){redirect($fa);}query_redirect("UPDATE".limit1(table($m)." SET".implode(",",$r),"\nWHERE $s"),$fa,'Rekord został zaktualizowany.');}else{$j=insert_into($m,$r);$Ud=($j?last_id():0);queries_redirect($fa,sprintf('Rekord%s został dodany.',($Ud?" $Ud":"")),$j);}}}$Fa=$o->tableName(table_status($m));page_header(($pa?'Edytuj':'Dodaj'),$n,array("select"=>array($m,$Fa)),$Fa);$a=null;if($_POST["save"]){$a=(array)$_POST["fields"];}elseif($s){$J=array();foreach($p
as$f=>$d){if(isset($d["privileges"]["select"])){$J[]=($_POST["clone"]&&$d["auto_increment"]?"'' AS ":(ereg("enum|set",$d["type"])?"1*".idf_escape($f)." AS ":"")).idf_escape($f);}}$a=array();if($J){$H=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($m)," WHERE $s",(isset($_GET["select"])?2:1)));$a=(isset($_GET["select"])&&count($H)!=1?null:reset($H));}}if($a===false){echo"<p class='error'>".'Brak rekordów.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($p){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($pa?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}}echo($pa?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$p?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
',(support("comment")?checkbox("comments",1,$_b,'Komentarz',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($a["Comment"]).'" maxlength="60"'.($_b?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
';if(support("partitioning")){$ce=ereg('RANGE|LIST',$a["partition_by"]);print_fieldset("partition",'Partycjonowanie',$a["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Sd,$a["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($a["partition"]),'">)
Partycje: <input name="partitions" size="2" value="',h($a["partitions"]),'"',($ce||!$a["partition_by"]?" class='hidden'":""),'>
...
as$t){if(count($A)==1||$t!=""){if(!create_database($t,$_POST["collation"])){$de=false;}$wb=$t;}}queries_redirect(ME."db=".urlencode($wb),'Baza danych została utworzona.',$de);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($_POST["name"]).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$n,array(),DB);$Z=collations();$f=DB;$lc=null;if($_POST){$f=$_POST["name"];$lc=$_POST["collation"];}elseif(DB!=""){$lc=db_collation(DB,$Z);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$ia){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$ia,$k)&&$k[1]){$f=stripcslashes(idf_unescape("`$k[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($f,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($f).'</textarea><br>':'<input id="name" name="name" value="'.h($f).'" maxlength="64">')."\n".($Z?html_select("collation",array(""=>"(".'porównywanie znaków'.")")+$Z,$lc):"");?>
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+explode("|",$Ea),$a["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+explode("|",$Ea),$a["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
...
';if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="submit" value="Zapisz zmiany">
';if($_GET["view"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["event"])){$lb=$_GET["event"];$Pd=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$rd=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$n){if($_POST["drop"]){query_redirect("DROP EVENT ".idf_escape($lb),substr(ME,0,-1),'Wydarzenie zostało usunięte.');}elseif(in_array($_POST["INTERVAL_FIELD"],$Pd)&&isset($rd[$_POST["STATUS"]])){$Td="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($lb!=""?'Wydarzenie zostało zmienione.':'Wydarzenie zostało utworzone.'),queries(($lb!=""?"ALTER EVENT ".idf_escape($lb).$Td.($lb!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Td)."\n".$rd[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($lb!=""?'Zmień wydarzenie'.": ".h($lb):'Utwórz wydarzenie'),$n);$a=array();if($_POST){$a=$_POST;}elseif($lb!=""){$H=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($lb));$a=reset($H);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($a["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Pd,$a["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$rd,$a["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($a["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Zapisz zmiany">
';if($lb!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
as$d){if($d["field"]!=""){$r[]=(ereg("^($cc)\$",$d["inout"])?"$d[inout] ":"").idf_escape($d["field"]).process_type($d,"CHARACTER SET");}}$Za=drop_create("DROP $Ra ".idf_escape($Qa),"CREATE $Ra ".idf_escape($_POST["name"])." (".implode(", ",$r).")".(isset($_GET["function"])?" RETURNS".process_type($_POST["returns"],"CHARACTER SET"):"").(in_array($_POST["language"],$xd)?" LANGUAGE $_POST[language]":"").rtrim("\n$_POST[definition]",";").";",substr(ME,0,-1),'Procedura została usunięta.','Procedura została zmieniona.','Procedura została utworzona.',$Qa);}page_header(($Qa!=""?(isset($_GET["function"])?'Zmień funkcję':'Zmień procedurę').": ".h($Qa):(isset($_GET["function"])?'Utwórz funkcję':'Utwórz procedurę')),$n);$Z=get_vals("SHOW CHARACTER SET");sort($Z);$a=array("fields"=>array());if($_POST){$a=$_POST;$a["fields"]=(array)$a["fields"];process_fields($a["fields"]);}elseif($Qa!=""){$a=routine($Qa,$Ra);$a["name"]=$Qa;}echo'
<form action="" method="post" id="form">
<p>Nazwa: <input name="name" value="',h($a["name"]),'" maxlength="64">
',($xd?'Język'.": ".html_select("language",$xd,$a["language"]):""),'<table cellspacing="0" class="nowrap">
';edit_fields($a["fields"],$Z,$Ra);if(isset($_GET["function"])){echo"<tr><td>".'Zwracany typ';edit_type("returns",$a["returns"],$Z);}echo'</table>
<p>';textarea("definition",$a["definition"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($Qa!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["sequence"])){$Bb=$_GET["sequence"];if($_POST&&!$n){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($Bb),$y,'Sekwencja została usunięta.');}elseif($Bb==""){query_redirect("CREATE SEQUENCE ".idf_escape($_POST["name"]),$y,'Sekwencja została utworzona.');}elseif($Bb!=$_POST["name"]){query_redirect("ALTER SEQUENCE ".idf_escape($Bb)." RENAME TO ".idf_escape($_POST["name"]),$y,'Sekwencja została zmieniona.');}else{redirect($y);}}page_header($Bb!=""?'Zmień sekwencję'.": ".h($Bb):'Utwórz sekwencję',$n);$a=array("name"=>$Bb);if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p><input name="name" value="',h($a["name"]),'">
<input type="submit" value="Zapisz zmiany">
';if($Bb!=""){echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["type"])){$Gc=$_GET["type"];if($_POST&&!$n){$y=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($Gc),$y,'Typ został usunięty.');}else{query_redirect("CREATE TYPE ".idf_escape($_POST["name"])." $_POST[as]",$y,'Typ został utworzony.');}}page_header($Gc!=""?'Zmień typ'.": ".h($Gc):'Utwórz typ',$n);$a["as"]="AS ";if($_POST){$a=$_POST;}echo'
<form action="" method="post">
<p>
';if($Gc!=""){echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";}else{echo"<input name='name' value='".h($a['name'])."'>\n";textarea("as",$a["as"]);echo"<p><input type='submit' value='".'Zapisz zmiany'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$Ic["Timing"],$a["Timing"],"if (/^".preg_quote($m,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($m)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$be,$a["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Ic["Type"],$a["Type"]),'</table>
<p>Nazwa: <input name="Trigger" value="',h($a["Trigger"]),'" maxlength="64">
<p>';textarea("Statement",$a["Statement"]);echo'<p>
<input type="submit" value="Zapisz zmiany">
';if($_GET["name"]!=""){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}if($Za){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["user"])){$qd=$_GET["user"];$ra=array(""=>array("All privileges"=>""));foreach(get_rows("SHOW PRIVILEGES")as$a){foreach(explode(",",($a["Privilege"]=="Grant option"?"":$a["Context"]))as$Qc){$ra[$Qc][$a["Privilege"]]=$a["Comment"];}}$ra["Server Admin"]+=$ra["File access on server"];$ra["Databases"]["Create routine"]=$ra["Procedures"]["Create routine"];unset($ra["Procedures"]["Create routine"]);$ra["Columns"]=array();foreach(array("Select","Insert","Update","References")as$b){$ra["Columns"][$b]=$ra["Tables"][$b];}unset($ra["Server Admin"]["Usage"]);foreach($ra["Tables"]as$e=>$b){unset($ra["Databases"][$e]);}$hc=array();if($_POST){foreach($_POST["objects"]as$e=>$b){$hc[$b]=(array)$hc[$b]+(array)$_POST["grants"][$e];}}$hb=array();$Tc="";if(isset($_GET["host"])&&($j=$g->query("SHOW GRANTS FOR ".q($qd)."@".q($_GET["host"])))){while($a=$j->fetch_row()){if(preg_match('~GRANT (.*) ON (.*) TO ~',$a[0],$k)&&preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~',$k[1],$sa,PREG_SET_ORDER)){foreach($sa
as$b){if($b[1]!="USAGE"){$hb["$k[2]$b[2]"][$b[1]]=true;}if(ereg(' WITH GRANT OPTION',$a[0])){$hb["$k[2]$b[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$a[0],$k)){$Tc=$k[1];}}}if($_POST&&!$n){$ec=(isset($_GET["host"])?q($qd)."@".q($_GET["host"]):"''");$nb=q($_POST["user"])."@".q($_POST["host"]);$Nd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $ec",ME."privileges=",'Użytkownik został usunięty.');}else{if($ec!=$nb){$n=!queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $nb IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Nd");}elseif($_POST["pass"]!=$Tc||!$_POST["hashed"]){queries("SET PASSWORD FOR $nb = ".($_POST["hashed"]?$Nd:"PASSWORD($Nd)"));}if(!$n){$oc=array();foreach($hc
...
as$Xa=>$oc){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Xa,$k)){grant("REVOKE",array_keys($oc),$k[2]," ON $k[1] FROM $nb");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Użytkownik został zmieniony.':'Użytkownik został dodany.'),!$n);if($ec!=$nb){$g->query("DROP USER $nb");}}}page_header((isset($_GET["host"])?'Użytkownik'.": ".h("$qd@$_GET[host]"):'Dodaj użytkownika'),$n,array("privileges"=>array('','Uprawnienia użytkowników')));if($_POST){$a=$_POST;$hb=$hc;}else{$a=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$a["pass"]=$Tc;if($Tc!=""){$a["hashed"]=true;}$hb[DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")).".*":""]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$l=0;foreach($hb
as$Xa=>$ia){echo'<th>'.($Xa!="*.*"?"<input name='objects[$l]' value='".h($Xa)."' size='10'>":"<input type='hidden' name='objects[$l]' value='*.*' size='10'>*.*");$l++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$Qc=>$uc){foreach((array)$ra[$Qc]as$pc=>$za){echo"<tr".odd()."><td".($uc?">$uc<td":" colspan='2'").' lang="en" title="'.h($za).'">'.h($pc);$l=0;foreach($hb
as$Xa=>$ia){$f="'grants[$l][".h(strtoupper($pc))."]'";$q=$ia[strtoupper($pc)];if($Qc=="Server Admin"&&$Xa!=(isset($hb["*.*"])?"*.*":"")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$f><option><option value='1'".($q?" selected":"").">".'Uprawnienia'."<option value='0'".($q=="0"?" selected":"").">".'Usuń uprawnienia'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$f value='1'".($q?" checked":"").($pc=="All privileges"?" id='grants-$l-all'":($pc=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$l-all');\"")).">";}$l++;}}}echo"</table>\n",'<p>
<input type="submit" value="Zapisz zmiany">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Usuń"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$O,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$n){$Cd=0;foreach((array)$_POST["kill"]as$b){if(queries("KILL ".(+$b))){$Cd++;}}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$Cd),$Cd||!$_POST["kill"]);}page_header('Lista procesów',$n);echo'
<form action="" method="post">
...
as$e=>$b){echo"<td>".(($v=="sql"?$e=="Info"&&$b!="":$e=="current_query"&&$b!="<IDLE>")?"<code class='jush-$v'>".shorten_utf8($b,100,"</code>").' <a href="'.h(ME.($a["db"]!=""?"db=".urlencode($a["db"])."&":"")."sql=".urlencode($b)).'">'.'Edytuj'.'</a>':nbsp($b));}echo"\n";}echo'</table>
<p>
';if(support("kill")){echo($l+1)."/".sprintf('%d w sumie',$g->result("SELECT @@max_connections")),"<p><input type='submit' value='".'Przerwij wykonywanie'."'>\n";}echo'<input type="hidden" name="token" value="',$O,'">
</form>
...
as$e=>$b){if(isset($_d[$e])){$d=$p[$e];if($b!=""&&(!isset($Uc[$e])||$Uc[$e]!="")){$Uc[$e]=(is_mail($b)?$_d[$e]:"");}$y="";$b=$o->editVal($b,$d);if(!isset($b)){$b="<i>NULL</i>";}else{if(ereg('blob|bytea|raw|file',$d["type"])&&$b!=""){$y=h(ME.'download='.urlencode($m).'&field='.urlencode($e).$Va);}if($b===""){$b="&nbsp;";}elseif($Nb!=""&&ereg('text|blob',$d["type"])&&is_utf8($b)){$b=shorten_utf8($b,max(0,+$Nb));}else{$b=h($b);}if(!$y){foreach((array)$ha[$e]as$C){if(count($ha[$e])==1||end($C["source"])==$e){$y="";foreach($C["source"]as$l=>$Ca){$y.=where_link($l,$C["target"][$l],$H[$ga][$Ca]);}$y=h(($C["db"]!=""?preg_replace('~([?&]db=)[^&]+~','\\1'.urlencode($C["db"]),ME):ME).'select='.urlencode($C["table"]).$y);if(count($C["source"])==1){break;}}}}if($e=="COUNT(*)"){$y=h(ME."select=".urlencode($m));$l=0;foreach((array)$_GET["where"]as$z){if(!array_key_exists($z["col"],$vd)){$y.=h(where_link($l++,$z["col"],$z["val"],$z["op"]));}}foreach($vd
as$Ka=>$z){$y.=h(where_link($l++,$Ka,$z));}}}if(!$y){if(is_mail($b)){$y="mailto:$b";}if($pe=is_url($a[$e])){$y=($pe=="http"&&$xc?$a[$e]:"$pe://www.adminer.org/redirect/?url=".urlencode($a[$e]));}}$R=h("val[$Va][".bracket_escape($e)."]");$q=$_POST["val"][$Va][bracket_escape($e)];$qe=h(isset($q)?$q:$a[$e]);$hf=strpos($b,"<i>...</i>");$Ee=is_utf8($b)&&$H[$ga][$e]==$a[$e]&&!$aa[$e];$De=ereg('text|lob',$d["type"]);echo(($_GET["modify"]&&$Ee)||isset($q)?"<td>".($De?"<textarea name='$R' cols='30' rows='".(substr_count($a[$e],"\n")+1)."'>$qe</textarea>":"<input name='$R' value='$qe' size='$Xb[$e]'>"):"<td id='$R' ondblclick=\"".($Ee?"selectDblClick(this, event".($hf?", 2":($De?", 1":"")).")":"alert('".h('Użyj linku edycji aby zmienić tę wartość.')."')").";\">".$o->selectVal($b,$y,$d));}}if($Id){echo"<td>";}$o->backwardKeysPrint($Id,$H[$ga]);echo"</tr>\n";}echo"</table>\n";}if($H||$Y){$td=true;if($_GET["page"]!="last"&&+$M&&count($qa)>=count($J)&&($fb>=$M||$Y)){$fb=found_rows($u,$s);if($fb<max(1e4,2*($Y+1)*$M)){ob_flush();flush();$fb=$g->result("SELECT COUNT(*) FROM ".table($m).($s?" WHERE ".implode(" AND ",$s):""));}else{$td=false;}}echo"<p class='pages'>";if(+$M&&$fb>$M){$md=floor(($fb-1)/$M);echo'<a href="'.h(remove_from_uri("page"))."\" onclick=\"pageClick(this.href, +prompt('".'Strona'."', '".($Y+1)."'), event); return false;\">".'Strona'."</a>:",pagination(0,$Y).($Y>5?" ...":"");for($l=max(1,$Y-4);$l<min($md,$Y+5);$l++){echo
pagination($l,$Y);}echo($Y+5<$md?" ...":"").($td?pagination($md,$Y):' <a href="'.h(remove_from_uri()."&page=last").'">'.'ostatni'."</a>");}echo" (".($td?"":"~ ").lang(array('%d rekord','%d rekordy','%d rekordów'),$fb).") ".checkbox("all",1,0,'wybierz wszystkie')."\n";if($o->selectCommandPrint()){?>
...
<input type="submit" name="delete" value="Usuń" onclick="return confirm('Czy jesteś pewien? (' + (this.form['all'].checked ? <?php echo$fb,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
';}print_fieldset("export",'Eksport');$Ma=$o->dumpOutput();echo($Ma?html_select("output",$Ma,$ed["output"])." ":""),html_select("format",$o->dumpFormat(),$ed["format"])," <input type='submit' name='export' value='".'Eksport'."' onclick='eventStop(event);'>\n","</div></fieldset>\n";}if($o->selectImportPrint()){print_fieldset("import",'Import',!$H);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$ed["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$O'>\n","</div></fieldset>\n";}$o->selectEmailPrint(array_filter($Uc,'strlen'),$B);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$Sb=isset($_GET["status"]);page_header($Sb?'Status':'Zmienne');$ze=($Sb?show_status():show_variables());if(!$ze){echo"<p class='message'>".'Brak rekordów.'."\n";}else{echo"<table cellspacing='0'>\n";foreach($ze
...
as$e=>$b){json_row("sum-$e",number_format($b,0,'.',' '));}json_row("");}else{foreach(count_tables(get_databases())as$t=>$b){json_row("tables-".js_escape($t),$b);}json_row("");}exit;}else{$_e=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($_e&&!$n&&!$_POST["search"]){$j=true;$ta="";if($v=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"])){queries("SET foreign_key_checks = 0");}if($_POST["truncate"]){if($_POST["tables"]){$j=truncate_tables($_POST["tables"]);}$ta='Tabele zostały opróżnione.';}elseif($_POST["move"]){$j=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ta='Tabele zostały przeniesione.';}elseif($_POST["copy"]){$j=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ta='Tabele zostały skopiowane.';}elseif($_POST["drop"]){if($_POST["views"]){$j=drop_views($_POST["views"]);}if($j&&$_POST["tables"]){$j=drop_tables($_POST["tables"]);}$ta='Tabele zostały usunięte.';}elseif($_POST["tables"]&&($j=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($a=$j->fetch_assoc()){$ta.="<b>".h($a["Table"])."</b>: ".h($a["Msg_text"])."<br>";}}queries_redirect(substr(ME,0,-1),$ta,$j);}page_header(($_GET["ns"]==""?'Baza danych'.": ".h(DB):'Schemat'.": ".h($_GET["ns"])),$n,true);if($o->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabele i perspektywy'."</h3>\n";$Xc=tables_list();if(!$Xc){echo"<p class='message'>".'Brak tabel.'."\n";}else{echo"<form action='' method='post'>\n","<p>".'Wyszukaj we wszystkich tabelach'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Szukaj'."'>\n";if($_POST["search"]&&$_POST["query"]!=""){search_tables();}echo"<table cellspacing='0' class='nowrap' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);"><th>'.'Tabela'.'<td>'.'Składowanie'.'<td>'.'Porównywanie znaków'.'<td>'.'Rozmiar danych'.'<td>'.'Rozmiar indeksów'.'<td>'.'Wolne miejsce'.'<td>'.'Auto Increment'.'<td>'.'Liczba rekordów'.(support("comment")?'<td>'.'Komentarz':'')."</thead>\n";foreach($Xc
as$f=>$_){$Zb=(isset($_)&&!eregi("table",$_));echo'<tr'.odd().'><td>'.checkbox(($Zb?"views[]":"tables[]"),$f,in_array($f,$_e,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($f).'">'.h($f).'</a>';if($Zb){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($f).'">'.'Perspektywa'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($f).'">?</a>';}else{foreach(array("Engine"=>"","Collation"=>"","Data_length"=>"create","Index_length"=>"indexes","Data_free"=>"edit","Auto_increment"=>"auto_increment=1&create","Rows"=>"select")as$e=>$y){echo($y?"<td align='right'><a href='".h(ME."$y=").urlencode($f)."' id='$e-".h($f)."'>?</a>":"<td id='$e-".h($f)."'>&nbsp;");}}echo(support("comment")?"<td id='Comment-".h($f)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d w sumie',count($Xc)),"<td>".nbsp($v=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$e){echo"<td align='right' id='sum-$e'>&nbsp;";}echo"</table>\n";if(!information_schema(DB)){echo"<p>".($v=="sql"?"<input type='submit' value='".'Analizuj'."'> <input type='submit' name='optimize' value='".'Optymalizuj'."'> <input type='submit' name='check' value='".'Sprawdź'."'> <input type='submit' name='repair' value='".'Napraw'."'> ":"")."<input type='submit' name='truncate' value='".'Opróżnij'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Usuń'."'".confirm("formChecked(this, /tables|views/)",1).">\n";$A=(support("scheme")?schemas():get_databases());if(count($A)!=1&&$v!="sqlite"){$t=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Przenieś do innej bazy danych'.": ",($A?html_select("target",$A,$t):'<input name="target" value="'.h($t).'">')," <input type='submit' name='move' value='".'Przenieś'."' onclick='eventStop(event);'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopiuj'."' onclick='eventStop(event);'>":""),"\n";}echo"<input type='hidden' name='token' value='$O'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Utwórz tabelę'."</a>\n";if(support("view")){echo'<a href="'.h(ME).'view=">'.'Utwórz perspektywę'."</a>\n";}if(support("routine")){echo"<h3>".'Procedury i funkcje'."</h3>\n";$Ae=routines();if($Ae){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Nazwa'.'<td>'.'Typ'.'<td>'.'Zwracany typ'."<td>&nbsp;</thead>\n";odd('');foreach($Ae
