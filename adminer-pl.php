...
upload_error($k){$jd=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Wgranie pliku było niemożliwe.'.($jd?" ".sprintf('Maksymalna wielkość pliku to %sB.',$jd):""):'Plik nie istnieje.');}function
...
enum_input($S,$xa,$l,$X,$Eb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$ed);$G=($Eb!==null?"<label><input type='$S'$xa value='$Eb'".((is_array($X)?in_array($Eb,$X):$X===0)?" checked":"")."><i>".'puste'."</i></label>":"");foreach($ed[1]as$p=>$W){$W=stripcslashes(str_replace("''","'",$W));$La=(is_int($X)?$X==$p+1:(is_array($X)?in_array($p+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($p+1)."'".($La?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$u;$_=h(bracket_escape($l["field"]));echo"<td class='function'>";$Ge=($u=="mssql"&&$l["auto_increment"]);if($Ge&&!$_POST["save"])$o=null;$oc=(isset($_GET["select"])||$Ge?array("orig"=>'bez zmian'):array())+$b->editFunctions($l);$xa=" name='fields[$_]'";if($l["type"]=="enum")echo
...
lang($If,$_d){$je=($_d==1?0:($_d
%
10>1&&$_d
%
10<5&&$_d/10
%
10!=1?1:2));$If=str_replace("%d","%s",$If[$je]);$_d=number_format($_d,0,".",' ');return
...
q($W);}function
check_sqlite_name($_){global$g;$Xb="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Xb)\$~",$_)){$g->error=sprintf('Proszę użyć jednego z rozszerzeń: %s.',str_replace("|",", ",$Xb));return
false;}return
true;}function
create_database($j,$d){global$g;if(file_exists($j)){$g->error='Plik już istnieje.';return
...
as$j){if(!@unlink($j)){$g->error='Plik już istnieje.';return
false;}}return
true;}function
rename_database($_,$d){global$g;if(!check_sqlite_name($_))return
false;$g->Min_SQLite(":memory:");$g->error='Plik już istnieje.';return@rename(DB,$_);}function
...
set_schema($Qe){global$g,$T,$hf;$G=$g->query("SET search_path TO ".idf_escape($Qe));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$hf['Typy użytkownika'][]=$S;}}return$G;}function
...
reset($I);}function
support($Zb){return
ereg("view|scheme|processlist|drop_col|variables|status",$Zb);}$u="oracle";$T=array();$hf=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$v=>$W){$T+=$W;$hf[$v]=array_keys($W);}$Vf=array();$Jd=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$oc=array("length","lower","round","upper");$tc=array("avg","count","count distinct","max","min","sum");$_b=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$ub["mssql"]="MS SQL";if(isset($_GET["mssql"])){$me=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($of,$L=""){echo'<p class="tabs">';$Zc=array("select"=>'Pokaż dane',"table"=>'Struktura tabeli');if(is_view($of))$Zc["view"]='Zmień perspektywę';else$Zc["create"]='Zmień tabelę';if($L!==null)$Zc["edit"]='Nowy rekord';foreach($Zc
...
selectQuery($E){global$u;return"<p><a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Ostatnia strona'."'>&gt;&gt;</a> <code class='jush-$u'>".h(str_replace("\n"," ",$E))."</code> <a href='".h(ME)."sql=".urlencode($E)."'>".'Edytuj'."</a></p>\n";}function
rowDescription($O){return"";}function
rowDescriptions($I,$gc){return$I;}function
selectVal($W,$y,$l){$G=($W===null?"<i>NULL</i>":(ereg("char|binary",$l["type"])&&!ereg("var",$l["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$l["type"])&&!is_utf8($W))$G=lang(array('%d bajt','%d bajty','%d bajtów'),strlen($W));return($y?"<a href='$y'>$G</a>":$G);}function
editVal($W,$l){return(ereg("binary",$l["type"])?reset(unpack("H*",$W)):$W);}function
selectColumnsPrint($J,$f){global$oc,$tc;print_fieldset("select",'pokaż',$J);$p=0;$nc=array('Funkcje'=>$oc,'Agregacje'=>$tc);foreach($J
...
as$p=>$s){if($s["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$s["columns"]))."</i>) AGAINST"," <input name='fulltext[$p]' value='".h($_GET["fulltext"][$p])."' onchange='selectFieldChange(this.form);'>",checkbox("boolean[$p]",1,isset($_GET["boolean"][$p]),"BOOL"),"<br>\n";}}$_GET["where"]=(array)$_GET["where"];reset($_GET["where"]);$Ja="this.nextSibling.onchange();";for($p=0;$p<=count($_GET["where"]);$p++){list(,$W)=each($_GET["where"]);if(!$W||("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators))){echo"<div><select name='where[$p][col]' onchange='$Ja'><option value=''>(".'gdziekolwiek'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$p][op]",$this->operators,$W["op"],$Ja),"<input name='where[$p][val]' value='".h($W["val"])."' onchange='".($W?"selectFieldChange(this.form)":"selectAddRow(this)").";'></div>\n";}}echo"</div></fieldset>\n";}function
selectOrderPrint($Nd,$f,$t){print_fieldset("sort",'Sortuj',$Nd);$p=0;foreach((array)$_GET["order"]as$v=>$W){if(isset($f[$W])){echo"<div><select name='order[$p]' onchange='selectFieldChange(this.form);'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$p]",1,isset($_GET["desc"][$v]),'malejąco')."</div>\n";$p++;}}echo"<div><select name='order[$p]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$p]' value='1'>".'malejąco'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($x){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($x)."'>","</div></fieldset>\n";}function
selectLengthPrint($zf){if($zf!==null){echo"<fieldset><legend>".'Długość tekstu'."</legend><div>",'<input name="text_length" size="3" value="'.h($zf).'">',"</div></fieldset>\n";}}function
selectActionPrint($t){echo"<fieldset><legend>".'Czynność'."</legend><div>","<input type='submit' value='".'pokaż'."'>"," <span id='noindex' title='".'Full table scan'."'></span>","<script type='text/javascript'>\n","var indexColumns = ";$f=array();foreach($t
...
false;}function
messageQuery($E){global$u;static$cb=0;restart_session();$q="sql-".($cb++);$vc=&get_session("queries");if(strlen($E)>1e6)$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";$vc[$_GET["db"]][]=array($E,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$q' onclick=\"return !toggle('$q');\">".'Zapytanie SQL'."</a><div id='$q' class='hidden'><pre><code class='jush-$u'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($vc[$_GET["db"]])-1)).'">'.'Edytuj'.'</a></div>';}function
...
explode("/",$G);}function
editInput($O,$l,$xa,$X){if($l["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$xa value='-1' checked><i>".'bez zmian'."</i></label> ":"").($l["null"]?"<label><input type='radio'$xa value=''".($X!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$xa,$l,$X,0);return"";}function
...
<p class="logout">
';if(DB==""||!$sd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'Zapytanie SQL'."</a>\n";if(support("dump"))echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Eksport'."</a>\n";}echo'<input type="submit" name="logout" value="Wyloguj">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($i?html_select("db",array(""=>"(".'baza danych'.")")+$i,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Wybierz"',($i?" class='hidden'":""),'>
';if($sd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schemat'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!="")set_schema($_GET["ns"]);}if($_GET["ns"]!==""&&!$sd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Utwórz nową tabelę'."</a>\n";$Q=tables_list();if(!$Q)echo"<p class='message'>".'Brak tabel.'."\n";else{$this->tablesPrint($Q);$Zc=array();foreach($Q
...
as$v=>$W){if($W===null)$W="<i>NULL</i>";elseif($Ea[$v]&&!is_utf8($W))$W="<i>".lang(array('%d bajt','%d bajty','%d bajtów'),strlen($W))."</i>";elseif(!strlen($W))$W="&nbsp;";else{$W=h($W);if($T[$v]==254)$W="<code>$W</code>";}if(isset($Zc[$v])&&!$f[$Zc[$v]]){if($yc){$O=$H[array_search("table=",$Zc)];$y=$Zc[$v].urlencode($Qd[$O]!=""?$Qd[$O]:$O);}else{$y="edit=".urlencode($Zc[$v]);foreach($t[$Zc[$v]]as$Oa=>$Lc)$y.="&where".urlencode("[".bracket_escape($Oa)."]")."=".urlencode($H[$Lc]);}$W="<a href='".h(ME.$y)."'>$W</a>";}echo"<td>$W";}}echo($p?"</table>":"<p class='message'>".'Brak rekordów.')."\n";return$G;}function
...
edit_type($v,$l,$Qa,$hc=array()){global$hf,$T,$Vf,$Fd;echo'<td><select name="',$v,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$hf+($hc?array('Klucze obce'=>$hc):array()),$l["type"]),'</select>
<td><input name="',$v,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$v"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porównywanie znaków'.')'.optionlist($Qa,$l["collation"]).'</select>',($Vf?"<select name='$v"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Vf,$l["unsigned"]).'</select>':''),($hc?"<select name='$v"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'W przypadku usunięcia'.")".optionlist(explode("|",$Fd),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Va?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.4.0' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Va?"":" class='hidden'")."><input name='fields[$p][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.4.0' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.4.0' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' name='down[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.4.0' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($Rd==""||support("drop_col")?"<input type='image' name='drop_col[$p]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.4.0' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zatrzymaj w przypadku błędu')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Pokaż tylko błędy')."\n";print_fieldset("webfile",'Z serwera',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Wa=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$v=>$W){if(extension_loaded($W))$Wa[]=".$v";}echo
...
as$v=>$W){list($D,$_f)=$W;echo'<a href="'.h(ME."sql=&history=$v").'">'.'Edytuj'."</a> <span class='time'>".@date("H:i:s",$_f)."</span> <code class='jush-$u'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$D)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Wyczyść'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Edytuj wszystkie'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$_=>$l){if(!isset($l["privileges"][$Wf?"update":"insert"])||$b->fieldName($l)=="")unset($m[$_]);}if($_POST&&!$k&&!isset($_GET["select"])){$ad=$_POST["referer"];if($_POST["insert"])$ad=($Wf?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$ad))$ad=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$ad,'Rekord został usunięty.');else{$L=array();foreach($m
as$_=>$l){$W=process_input($l);if($W!==false&&$W!==null)$L[idf_escape($_)]=($Wf?"\n".idf_escape($_)." = $W":$W);}if($Wf){if(!$L)redirect($ad);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$ad,'Rekord został zaktualizowany.');}else{$F=insert_into($a,$L);$Sc=($F?last_id():0);queries_redirect($ad,sprintf('Rekord%s został dodany.',($Sc?" $Sc":"")),$F);}}}$pf=$b->tableName(table_status($a));page_header(($Wf?'Edytuj':'Dodaj'),$k,array("select"=>array($a,$pf)),$pf);$H=null;if($_POST["save"])$H=(array)$_POST["fields"];elseif($Z){$J=array();foreach($m
as$_=>$l){if(isset($l["privileges"]["select"]))$J[]=($_POST["clone"]&&$l["auto_increment"]?"'' AS ":(ereg("enum|set",$l["type"])?"1*".idf_escape($_)." AS ":"")).idf_escape($_);}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false)echo"<p class='error'>".'Brak rekordów.'."\n";echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($m){echo"<input type='submit' value='".'Zapisz zmiany'."'>\n";if(!isset($_GET["select"]))echo"<input type='submit' name='insert' value='".($Wf?'Zapisz i kontynuuj edycję':'Zapisz i dodaj następny')."' title='Ctrl+Shift+Enter'>\n";}echo($Wf?"<input type='submit' name='delete' value='".'Usuń'."' onclick=\"return confirm('".'Czy jesteś pewien?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"]))hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if(support("partitioning")){$ce=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Partycjonowanie',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$be,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Partycje: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($ce||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();$_=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Baza danych została usunięta.',drop_databases(array(DB)));}elseif(DB!==$_){if(DB!=""){$_GET["db"]=$_;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($_),'Nazwa bazy danych została zmieniona.',rename_database($_,$_POST["collation"]));}else{$i=explode("\n",str_replace("\r","",$_));$if=true;$Rc="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"]))$if=false;$Rc=$j;}}queries_redirect(ME."db=".urlencode($Rc),'Baza danych została utworzona.',$if);}}else{if(!$_POST["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($_).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Baza danych została zmieniona.');}}page_header(DB!=""?'Zmień bazę danych':'Utwórz bazę danych',$k,array(),DB);$Qa=collations();$_=DB;$Pa=null;if($_POST){$_=$_POST["name"];$Pa=$_POST["collation"];}elseif(DB!="")$Pa=db_collation(DB,$Qa);elseif($u=="sql"){foreach(get_vals("SHOW GRANTS")as$pc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$pc,$z)&&$z[1]){$_=stripcslashes(idf_unescape("`$z[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($_,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($_).'</textarea><br>':'<input id="name" name="name" value="'.h($_).'" maxlength="64">')."\n".($Qa?html_select("collation",array(""=>"(".'porównywanie znaków'.")")+$Qa,$Pa):"");?>
<script type='text/javascript'>document.getElementById('name').focus();</script>
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.4.0' alt='+' title='".'Dodaj następny'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["scheme"])){if($_POST&&!$k){$y=preg_replace('~ns=[^&]*&~','',ME)."ns=";if($_POST["drop"])query_redirect("DROP SCHEMA ".idf_escape($_GET["ns"]),$y,'Schemat został usunięty.');else{$_=trim($_POST["name"]);$y.=urlencode($_);if($_GET["ns"]=="")query_redirect("CREATE SCHEMA ".idf_escape($_),$y,'Schemat został utworzony.');elseif($_GET["ns"]!=$_)query_redirect("ALTER SCHEMA ".idf_escape($_GET["ns"])." RENAME TO ".idf_escape($_),$y,'Schemat został zmieniony.');else
redirect($y);}}page_header($_GET["ns"]!=""?'Zmień schemat':'Utwórz schemat',$k);$H=$_POST;if(!$H)$H=array("name"=>$_GET["ns"]);echo'
<form action="" method="post">
...
<p>
W przypadku usunięcia: ',html_select("on_delete",array(-1=>"")+explode("|",$Fd),$H["on_delete"]),' W przypadku zmiany: ',html_select("on_update",array(-1=>"")+explode("|",$Fd),$H["on_update"]),'<p>
<input type="submit" value="Zapisz zmiany">
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$wb=false;if($_POST&&!$k){$_=trim($_POST["name"]);$wb=drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($_)." AS\n$_POST[select]",($_POST["drop"]?substr(ME,0,-1):ME."table=".urlencode($_)),'Perspektywa została usunięta.','Perspektywa została zmieniona.','Perspektywa została utworzona.',$a);}page_header(($a!=""?'Zmień perspektywę':'Utwórz perspektywę'),$k,array("table"=>$a),$a);$H=$_POST;if(!$H&&$a!=""){$H=view($a);$H["name"]=$a;}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["event"])){$aa=$_GET["event"];$Ic=array("YEAR","QUARTER","MONTH","DAY","HOUR","MINUTE","WEEK","SECOND","YEAR_MONTH","DAY_HOUR","DAY_MINUTE","DAY_SECOND","HOUR_MINUTE","HOUR_SECOND","MINUTE_SECOND");$ff=array("ENABLED"=>"ENABLE","DISABLED"=>"DISABLE","SLAVESIDE_DISABLED"=>"DISABLE ON SLAVE");if($_POST&&!$k){if($_POST["drop"])query_redirect("DROP EVENT ".idf_escape($aa),substr(ME,0,-1),'Wydarzenie zostało usunięte.');elseif(in_array($_POST["INTERVAL_FIELD"],$Ic)&&isset($ff[$_POST["STATUS"]])){$Pe="\nON SCHEDULE ".($_POST["INTERVAL_VALUE"]?"EVERY ".q($_POST["INTERVAL_VALUE"])." $_POST[INTERVAL_FIELD]".($_POST["STARTS"]?" STARTS ".q($_POST["STARTS"]):"").($_POST["ENDS"]?" ENDS ".q($_POST["ENDS"]):""):"AT ".q($_POST["STARTS"]))." ON COMPLETION".($_POST["ON_COMPLETION"]?"":" NOT")." PRESERVE";queries_redirect(substr(ME,0,-1),($aa!=""?'Wydarzenie zostało zmienione.':'Wydarzenie zostało utworzone.'),queries(($aa!=""?"ALTER EVENT ".idf_escape($aa).$Pe.($aa!=$_POST["EVENT_NAME"]?"\nRENAME TO ".idf_escape($_POST["EVENT_NAME"]):""):"CREATE EVENT ".idf_escape($_POST["EVENT_NAME"]).$Pe)."\n".$ff[$_POST["STATUS"]]." COMMENT ".q($_POST["EVENT_COMMENT"]).rtrim(" DO\n$_POST[EVENT_DEFINITION]",";").";"));}}page_header(($aa!=""?'Zmień wydarzenie'.": ".h($aa):'Utwórz wydarzenie'),$k);$H=$_POST;if(!$H&&$aa!=""){$I=get_rows("SELECT * FROM information_schema.EVENTS WHERE EVENT_SCHEMA = ".q(DB)." AND EVENT_NAME = ".q($aa));$H=reset($I);}echo'
<form action="" method="post">
<table cellspacing="0">
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ic,$H["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$ff,$H["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$y=substr(ME,0,-1);$_=trim($_POST["name"]);if($_POST["drop"])query_redirect("DROP SEQUENCE ".idf_escape($fa),$y,'Sekwencja została usunięta.');elseif($fa=="")query_redirect("CREATE SEQUENCE ".idf_escape($_),$y,'Sekwencja została utworzona.');elseif($fa!=$_)query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($_),$y,'Sekwencja została zmieniona.');else
redirect($y);}page_header($fa!=""?'Zmień sekwencję'.": ".h($fa):'Utwórz sekwencję',$k);$H=$_POST;if(!$H)$H=array("name"=>$fa);echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Zapisz zmiany">
';if($fa!="")echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$y=substr(ME,0,-1);if($_POST["drop"])query_redirect("DROP TYPE ".idf_escape($ga),$y,'Typ został usunięty.');else
query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$y,'Typ został utworzony.');}page_header($ga!=""?'Zmień typ'.": ".h($ga):'Utwórz typ',$k);$H=$_POST;if(!$H)$H=array("as"=>"AS ");echo'
<form action="" method="post">
<p>
...
';}elseif(isset($_GET["trigger"])){$a=$_GET["trigger"];$Lf=trigger_options();$Kf=array("INSERT","UPDATE","DELETE");$wb=false;if($_POST&&!$k&&in_array($_POST["Timing"],$Lf["Timing"])&&in_array($_POST["Event"],$Kf)&&in_array($_POST["Type"],$Lf["Type"])){$Af=" $_POST[Timing] $_POST[Event]";$Ed=" ON ".table($a);$wb=drop_create("DROP TRIGGER ".idf_escape($_GET["name"]).($u=="pgsql"?$Ed:""),"CREATE TRIGGER ".idf_escape($_POST["Trigger"]).($u=="mssql"?$Ed.$Af:$Af.$Ed).rtrim(" $_POST[Type]\n$_POST[Statement]",";").";",ME."table=".urlencode($a),'Wyzwalacz został usunięty.','Wyzwalacz został zmieniony.','Wyzwalacz został utworzony.',$_GET["name"]);}page_header(($_GET["name"]!=""?'Zmień wyzwalacz'.": ".h($_GET["name"]):'Utwórz wyzwalacz'),$k,array("table"=>$a));$H=$_POST;if(!$H)$H=trigger($_GET["name"])+array("Trigger"=>$a."_bi");echo'
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Czas<td>',html_select("Timing",$Lf["Timing"],$H["Timing"],"if (/^".preg_quote($a,"/")."_[ba][iud]$/.test(this.form['Trigger'].value)) this.form['Trigger'].value = '".js_escape($a)."_' + selectValue(this).charAt(0).toLowerCase() + selectValue(this.form['Event']).charAt(0).toLowerCase();"),'<tr><th>Wydarzenie<td>',html_select("Event",$Kf,$H["Event"],"this.form['Timing'].onchange();"),'<tr><th>Typ<td>',html_select("Type",$Lf["Type"],$H["Type"]),'</table>
...
as$W){if($W[1]!="USAGE")$qc["$z[2]$W[2]"][$W[1]]=true;if(ereg(' WITH GRANT OPTION',$H[0]))$qc["$z[2]$W[2]"]["GRANT OPTION"]=true;}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$z))$Cd=$z[1];}}if($_POST&&!$k){$Dd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$yd=q($_POST["user"])."@".q($_POST["host"]);$fe=q($_POST["pass"]);if($_POST["drop"])query_redirect("DROP USER $Dd",ME."privileges=",'Użytkownik został usunięty.');else{$fb=false;if($Dd!=$yd){$fb=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $yd IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $fe");$k=!$fb;}elseif($_POST["pass"]!=$Cd||!$_POST["hashed"])queries("SET PASSWORD FOR $yd = ".($_POST["hashed"]?$fe:"PASSWORD($fe)"));if(!$k){$He=array();foreach($xd
...
as$Ad=>$He){if(preg_match('~^(.+)(\\(.*\\))?$~U',$Ad,$z))grant("REVOKE",array_keys($He),$z[2]," ON $z[1] FROM $yd");}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Użytkownik został zmieniony.':'Użytkownik został dodany.'),!$k);if($fb)$g->query("DROP USER $yd");}}page_header((isset($_GET["host"])?'Użytkownik'.": ".h("$ha@$_GET[host]"):'Dodaj użytkownika'),$k,array("privileges"=>array('','Uprawnienia użytkowników')));if($_POST){$H=$_POST;$qc=$xd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$Cd;if($Cd!="")$H["hashed"]=true;$qc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Uprawnienia użytkowników'."</a>";$p=0;foreach($qc
as$Ad=>$pc){echo'<th>'.($Ad!="*.*"?"<input name='objects[$p]' value='".h($Ad)."' size='10'>":"<input type='hidden' name='objects[$p]' value='*.*' size='10'>*.*");$p++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$ab=>$pb){foreach((array)$te[$ab]as$se=>$Ua){echo"<tr".odd()."><td".($pb?">$pb<td":" colspan='2'").' lang="en" title="'.h($Ua).'">'.h($se);$p=0;foreach($qc
as$Ad=>$pc){$_="'grants[$p][".h(strtoupper($se))."]'";$X=$pc[strtoupper($se)];if($ab=="Server Admin"&&$Ad!=(isset($qc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$_><option><option value='1'".($X?" selected":"").">".'Uprawnienia'."<option value='0'".($X=="0"?" selected":"").">".'Usuń uprawnienia'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Oc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W)))$Oc++;}queries_redirect(ME."processlist=",lang(array('Przerwano %d wątek.','Przerwano %d wątki.','Przerwano %d wątków.'),$Oc),$Oc||!$_POST["kill"]);}page_header('Lista procesów',$k);echo'
<form action="" method="post">
...
as$v=>$W)echo"<td>".(($u=="sql"&&$v=="Info"&&$H["Command"]=="Query"&&$W!="")||($u=="pgsql"&&$v=="current_query"&&$W!="<IDLE>")||($u=="oracle"&&$v=="sql_text"&&$W!="")?"<code class='jush-$u'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($H["db"]!=""?"db=".urlencode($H["db"])."&":"")."sql=".urlencode($W)).'">'.'Edytuj'.'</a>':nbsp($W));echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
...
';}$ic=$b->dumpFormat();if($ic){print_fieldset("export",'Eksport');$Wd=$b->dumpOutput();echo($Wd?html_select("output",$Wd,$qa["output"])." ":""),html_select("format",$ic,$qa["format"])," <input type='submit' name='export' value='".'Eksport'."'>\n","</div></fieldset>\n";}}if($b->selectImportPrint()){print_fieldset("import",'Import',!$I);echo"<input type='file' name='csv_file'> ",html_select("separator",array("csv"=>"CSV,","csv;"=>"CSV;","tsv"=>"TSV"),$qa["format"],1);echo" <input type='submit' name='import' value='".'Import'."'>","<input type='hidden' name='token' value='$R'>\n","</div></fieldset>\n";}$b->selectEmailPrint(array_filter($Db,'strlen'),$f);echo"</form>\n";}}}elseif(isset($_GET["variables"])){$ef=isset($_GET["status"]);page_header($ef?'Status':'Zmienne');$bg=($ef?show_status():show_variables());if(!$bg)echo"<p class='message'>".'Brak rekordów.'."\n";else{echo"<table cellspacing='0'>\n";foreach($bg
...
as$v=>$W)json_row("sum-$v",number_format($W,0,'.',' '));json_row("");}else{foreach(count_tables($b->databases())as$j=>$W)json_row("tables-".js_escape($j),$W);json_row("");}exit;}else{$vf=array_merge((array)$_POST["tables"],(array)$_POST["views"]);if($vf&&!$k&&!$_POST["search"]){$F=true;$ld="";if($u=="sql"&&count($_POST["tables"])>1&&($_POST["drop"]||$_POST["truncate"]||$_POST["copy"]))queries("SET foreign_key_checks = 0");if($_POST["truncate"]){if($_POST["tables"])$F=truncate_tables($_POST["tables"]);$ld='Tabele zostały opróżnione.';}elseif($_POST["move"]){$F=move_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ld='Tabele zostały przeniesione.';}elseif($_POST["copy"]){$F=copy_tables((array)$_POST["tables"],(array)$_POST["views"],$_POST["target"]);$ld='Tabele zostały skopiowane.';}elseif($_POST["drop"]){if($_POST["views"])$F=drop_views($_POST["views"]);if($F&&$_POST["tables"])$F=drop_tables($_POST["tables"]);$ld='Tabele zostały usunięte.';}elseif($u!="sql"){$F=($u=="sqlite"?queries("VACUUM"):apply_queries("VACUUM".($_POST["optimize"]?"":" ANALYZE"),$_POST["tables"]));$ld='Tabele zostały zoptymalizowane.';}elseif($_POST["tables"]&&($F=queries(($_POST["optimize"]?"OPTIMIZE":($_POST["check"]?"CHECK":($_POST["repair"]?"REPAIR":"ANALYZE")))." TABLE ".implode(", ",array_map('idf_escape',$_POST["tables"]))))){while($H=$F->fetch_assoc())$ld.="<b>".h($H["Table"])."</b>: ".h($H["Msg_text"])."<br>";}queries_redirect(substr(ME,0,-1),$ld,$F);}page_header(($_GET["ns"]==""?'Baza danych'.": ".h(DB):'Schemat'.": ".h($_GET["ns"])),$k,true);if($b->homepage()){if($_GET["ns"]!==""){echo"<h3>".'Tabele i perspektywy'."</h3>\n";$uf=tables_list();if(!$uf)echo"<p class='message'>".'Brak tabel.'."\n";else{echo"<form action='' method='post'>\n","<p>".'Wyszukaj we wszystkich tabelach'.": <input name='query' value='".h($_POST["query"])."'> <input type='submit' name='search' value='".'Szukaj'."'>\n";if($_POST["search"]&&$_POST["query"]!="")search_tables();echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);'>\n",'<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);">','<th>'.'Tabela','<td>'.'Składowanie','<td>'.'Porównywanie znaków','<td>'.'Rozmiar danych','<td>'.'Rozmiar indeksów','<td>'.'Wolne miejsce','<td>'.'Auto Increment','<td>'.'Liczba rekordów',(support("comment")?'<td>'.'Komentarz':''),"</thead>\n";foreach($uf
as$_=>$S){$dg=($S!==null&&!eregi("table",$S));echo'<tr'.odd().'><td>'.checkbox(($dg?"views[]":"tables[]"),$_,in_array($_,$vf,true),"","formUncheck('check-all');"),'<th><a href="'.h(ME).'table='.urlencode($_).'" title="'.'Struktura tabeli'.'">'.h($_).'</a>';if($dg){echo'<td colspan="6"><a href="'.h(ME)."view=".urlencode($_).'" title="'.'Zmień perspektywę'.'">'.'Perspektywa'.'</a>','<td align="right"><a href="'.h(ME)."select=".urlencode($_).'" title="'.'Pokaż dane'.'">?</a>';}else{foreach(array("Engine"=>array(),"Collation"=>array(),"Data_length"=>array("create",'Zmień tabelę'),"Index_length"=>array("indexes",'Zmień indeksy'),"Data_free"=>array("edit",'Nowy rekord'),"Auto_increment"=>array("auto_increment=1&create",'Zmień tabelę'),"Rows"=>array("select",'Pokaż dane'),)as$v=>$y)echo($y?"<td align='right'><a href='".h(ME."$y[0]=").urlencode($_)."' id='$v-".h($_)."' title='$y[1]'>?</a>":"<td id='$v-".h($_)."'>&nbsp;");}echo(support("comment")?"<td id='Comment-".h($_)."'>&nbsp;":"");}echo"<tr><td>&nbsp;<th>".sprintf('%d w sumie',count($uf)),"<td>".nbsp($u=="sql"?$g->result("SELECT @@storage_engine"):""),"<td>".nbsp(db_collation(DB,collations()));foreach(array("Data_length","Index_length","Data_free")as$v)echo"<td align='right' id='sum-$v'>&nbsp;";echo"</table>\n","<script type='text/javascript'>tableCheck();</script>\n";if(!information_schema(DB)){echo"<p>".(ereg('^(sql|sqlite|pgsql)$',$u)?($u!="sqlite"?"<input type='submit' value='".'Analizuj'."'> ":"")."<input type='submit' name='optimize' value='".'Optymalizuj'."'> ":"").($u=="sql"?"<input type='submit' name='check' value='".'Sprawdź'."'> <input type='submit' name='repair' value='".'Napraw'."'> ":"")."<input type='submit' name='truncate' value='".'Opróżnij'."'".confirm("formChecked(this, /tables/)")."> <input type='submit' name='drop' value='".'Usuń'."'".confirm("formChecked(this, /tables|views/)").">\n";$i=(support("scheme")?schemas():$b->databases());if(count($i)!=1&&$u!="sqlite"){$j=(isset($_POST["target"])?$_POST["target"]:(support("scheme")?$_GET["ns"]:DB));echo"<p>".'Przenieś do innej bazy danych'.": ",($i?html_select("target",$i,$j):'<input name="target" value="'.h($j).'">')," <input type='submit' name='move' value='".'Przenieś'."'>",(support("copy")?" <input type='submit' name='copy' value='".'Kopiuj'."'>":""),"\n";}echo"<input type='hidden' name='token' value='$R'>\n";}echo"</form>\n";}echo'<p><a href="'.h(ME).'create=">'.'Utwórz tabelę'."</a>\n";if(support("view"))echo'<a href="'.h(ME).'view=">'.'Utwórz perspektywę'."</a>\n";if(support("routine")){echo"<h3>".'Procedury i funkcje'."</h3>\n";$Me=routines();if($Me){echo"<table cellspacing='0'>\n",'<thead><tr><th>'.'Nazwa'.'<td>'.'Typ'.'<td>'.'Zwracany typ'."<td>&nbsp;</thead>\n";odd('');foreach($Me
