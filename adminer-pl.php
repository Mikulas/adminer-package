...
upload_error($m){$be=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Wgranie pliku byÅ‚o niemoÅ¼liwe.'.($be?" ".sprintf('Maksymalna wielkoÅ›Ä‡ pliku to %sB.',$be):""):'Plik nie istnieje.');}function
...
h($g->error);}function
check_sqlite_name($C){global$g;$zc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($zc)\$~",$C)){$g->error=sprintf('ProszÄ™ uÅ¼yÄ‡ jednego z rozszerzeÅ„: %s.',str_replace("|",", ",$zc));return
false;}return
true;}function
...
unconvert_field($n,$J){return$J;}function
support($Bc){return
preg_match('~^(columns|database|drop_col|indexes|processlist|scheme|sql|status|table|variables|view|view_trigger)$~',$Bc);}$w="oracle";$rh=array();$_g=array();foreach(array('Numeryczne'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Data i czas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Tekstowe'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binarne'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$x=>$X){$rh+=$X;$_g[$x]=array_keys($X);}$yh=array();$He=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","SQL");$Rc=array("length","lower","round","upper");$Wc=array("avg","count","count distinct","max","min","sum");$ac=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$Sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$pf=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
<tr><th>Rodzaj bazy<td>',html_select("auth[driver]",$Sb,DRIVER,"loginDriver(this);"),'<tr><th>Serwer<td><input name="auth[server]" value="',h(SERVER),'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
selectLinks($Gg,$O=""){echo'<p class="links">';$Rd=array("select"=>'PokaÅ¼ dane');if(support("table")||support("indexes"))$Rd["table"]='Struktura tabeli';if(support("table")){if(is_view($Gg))$Rd["view"]='ZmieÅ„ perspektywÄ™';else$Rd["create"]='ZmieÅ„ tabelÄ™';}if($O!==null)$Rd["edit"]='Nowy rekord';foreach($Rd
...
selectVal($X,$_,$n,$Te){$J=($X===null?"<i>NULL</i>":(preg_match("~char|binary~",$n["type"])&&!preg_match("~var~",$n["type"])?"<code>$X</code>":$X));if(preg_match('~blob|bytea|raw|file~',$n["type"])&&!is_utf8($X))$J=lang(array('%d bajt','%d bajty','%d bajtÃ³w'),strlen($Te));return($_?"<a href='".h($_)."'>$J</a>":$J);}function
editVal($X,$n){return$X;}function
selectColumnsPrint($M,$f){global$Rc,$Wc;print_fieldset("select",'pokaÅ¼',$M);$r=0;$M[""]=array();foreach($M
as$x=>$X){$X=$_GET["columns"][$x];$e=select_input(" name='columns[$r][col]' onchange='".($x!==""?"selectFieldChange(this.form)":"selectAddRow(this)").";'",$f,$X["col"]);echo"<div>".($Rc||$Wc?"<select name='columns[$r][fun]' onchange='helpClose();".($x!==""?"":" this.nextSibling.nextSibling.onchange();")."'".on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'",1).">".optionlist(array(-1=>"")+array_filter(array('Funkcje'=>$Rc,'Agregacje'=>$Wc)),$X["fun"])."</select>"."($e)":$e)."</div>\n";$r++;}echo"</div></fieldset>\n";}function
...
messageQuery($H){global$w;restart_session();$bd=&get_session("queries");$s="sql-".count($bd[$_GET["db"]]);if(strlen($H)>1e6)$H=preg_replace('~[\x80-\xFF]+$~','',substr($H,0,1e6))."\n...";$bd[$_GET["db"]][]=array($H,time());return" <span class='time'>".@date("H:i:s")."</span> <a href='#$s' onclick=\"return !toggle('$s');\">".'Zapytanie SQL'."</a>"."<div id='$s' class='hidden'><pre><code class='jush-$w'>".shorten_utf8($H,1000).'</code></pre>'.(support("sql")?'<p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($bd[$_GET["db"]])-1)).'">'.'Edytuj'.'</a>':'').'</div>';}function
...
explode("/",$J);}function
editInput($Q,$n,$Ha,$Y){if($n["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Ha value='-1' checked><i>".'bez zmian'."</i></label> ":"").($n["null"]?"<label><input type='radio'$Ha value=''".($Y!==null||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$Ha,$n,$Y,0);return"";}function
...
auth_error($rc=null){global$g,$b,$ad;$og=session_name();$m="";if(!$_COOKIE[$og]&&$_GET[$og]&&ini_bool("session.use_only_cookies"))$m='Wymagana jest obsÅ‚uga sesji w PHP.';elseif(isset($_GET["username"])){if(($_COOKIE[$og]||$_GET[$og])&&!$ad)$m='Sesja wygasÅ‚a, zaloguj siÄ™ ponownie.';else{$G=get_password();if($G!==null){$m=h($rc?$rc->getMessage():(is_string($g)?$g:'NieprawidÅ‚owe dane logowania.'));if($G===false)$m.='<br>'.sprintf('WaÅ¼noÅ›Ä‡ hasÅ‚a gÅ‚Ã³wnego wygasÅ‚a. <a href="http://www.adminer.org/pl/extension/" target="_blank">Zaimplementuj</a> wÅ‚asnÄ… metodÄ™ %s, aby ustawiÄ‡ je na staÅ‚e.','<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}$F=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$F["lifetime"]);page_header('Zaloguj siÄ™',$m,null);echo"<form action='' method='post'>\n";$b->loginForm();echo"<div>";hidden_fields($_POST,array("auth"));echo"</div>\n","</form>\n";page_footer("auth");}function
...
Min_Driver($g);if($Ia&&$_POST["token"])$_POST["token"]=$T;$m='';if($_POST){if(!verify_token()){$pd="max_input_vars";$ce=ini_get($pd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$x){$X=ini_get($x);if($X&&(!$ce||$X<$ce)){$pd=$x;$ce=$X;}}}$m=(!$_POST["token"]&&$ce?sprintf('Przekroczono maksymalnÄ… liczbÄ™ pÃ³l. ZwiÄ™ksz %s.',"'$pd'"):'NieprawidÅ‚owy token CSRF. SprÃ³buj wysÅ‚aÄ‡ formularz ponownie.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('PrzesÅ‚ano zbyt duÅ¼o danych. Zmniejsz objÄ™toÅ›Ä‡ danych lub zwiÄ™ksz zmiennÄ… konfiguracyjnÄ… %s.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'WiÄ™ksze pliki SQL moÅ¼esz wgraÄ‡ na serwer poprzez FTP przed zaimportowaniem.';}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
connect_error(){global$b,$g,$T,$m,$Sb;$j=array();if(DB!=""){header("HTTP/1.1 404 Not Found");page_header('Baza danych'.": ".h(DB),'Nie znaleziono bazy danych.',true);}else{if($_POST["db"]&&!$m)queries_redirect(substr(ME,0,-1),'Bazy danych zostaÅ‚y usuniÄ™te.',drop_databases($_POST["db"]));page_header('Wybierz bazÄ™ danych',$m,false);echo"<p class='links'>\n";foreach(array('database'=>'UtwÃ³rz nowÄ… bazÄ™ danych','privileges'=>'Uprawnienia uÅ¼ytkownikÃ³w','processlist'=>'Lista procesÃ³w','variables'=>'Zmienne','status'=>'Status',)as$x=>$X){if(support($x))echo"<a href='".h(ME)."$x='>$X</a>\n";}echo"<p>".sprintf('Wersja %s: %s za pomocÄ… %s',$Sb[DRIVER],"<b>$g->server_info</b>","<b>$g->extension</b>")."\n","<p>".sprintf('Zalogowany jako: %s',"<b>".h(logged_user())."</b>")."\n";$j=$b->databases();if($j){$eg=support("scheme");$ib=collations();echo"<form action='' method='post'>\n","<table cellspacing='0' class='checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n","<thead><tr>".(support("database")?"<td>&nbsp;":"")."<th>".'Baza danych'."<td>".'PorÃ³wnywanie znakÃ³w'."<td>".'Tabele'."</thead>\n";foreach($j
...
edit_type($x,$n,$ib,$Kc=array()){global$_g,$rh,$yh,$De;$U=$n["type"];echo'<td><select name="',$x,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);"',on_help("getTarget(event).value",1),'>';if($U&&!isset($rh[$U])&&!isset($Kc[$U]))array_unshift($_g,$U);if($Kc)$_g['Klucze obce']=$Kc;echo
optionlist($_g,$U),'</select>
<td><input name="',$x,'[length]" value="',h($n["length"]),'" size="3" onfocus="editingLengthFocus(this);"',(!$n["length"]&&preg_match('~var(char|binary)$~',$U)?" class='required'":""),' onchange="editingLengthChange(this);" onkeyup="this.onchange();"><td class="options">';echo"<select name='$x"."[collation]'".(preg_match('~(char|text|enum|set)$~',$U)?"":" class='hidden'").'><option value="">('.'porÃ³wnywanie znakÃ³w'.')'.optionlist($ib,$n["collation"]).'</select>',($yh?"<select name='$x"."[unsigned]'".(!$U||preg_match('~((^|[^o])int|float|double|decimal)$~',$U)?"":" class='hidden'").'><option>'.optionlist($yh,$n["unsigned"]).'</select>':''),(isset($n['on_update'])?"<select name='$x"."[on_update]'".($U=="timestamp"?"":" class='hidden'").'>'.optionlist(array(""=>"(".'W przypadku zmiany'.")","CURRENT_TIMESTAMP"),$n["on_update"]).'</select>':''),($Kc?"<select name='$x"."[on_delete]'".(preg_match("~`~",$U)?"":" class='hidden'")."><option value=''>(".'W przypadku usuniÄ™cia'.")".optionlist(explode("|",$De),$n["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($nb?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($o))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.3' alt='+' title='".'Dodaj nastÄ™pny'."'>",'<script type="text/javascript">row_count = ',count($o),';</script>
</thead>
...
',(support("comment")?"<td".($nb?"":" class='hidden'")."><input name='fields[$r][comment]' value='".h($n["comment"])."' maxlength='".($g->server_info>=5.5?1024:255)."'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.3' alt='+' title='".'Dodaj nastÄ™pny'."' onclick='return !editingAddRow(this, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=4.0.3' alt='^' title='".'PrzesuÅ„ w gÃ³rÄ™'."'>&nbsp;"."<input type='image' class='icon' name='down[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=4.0.3' alt='v' title='".'PrzesuÅ„ w dÃ³Å‚'."'>&nbsp;":""),($Qe==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$r]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.0.3' alt='x' title='".'UsuÅ„'."' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">":""),"\n";}}function
...
as$C=>$n){if(!isset($n["privileges"][$zh?"update":"insert"])||$b->fieldName($n)=="")unset($o[$C]);}if($_POST&&!$m&&!isset($_GET["select"])){$A=$_POST["referer"];if($_POST["insert"])$A=($zh?null:$_SERVER["REQUEST_URI"]);elseif(!preg_match('~^.+&select=.+$~',$A))$A=ME."select=".urlencode($a);$v=indexes($a);$uh=unique_array($_GET["where"],$v);$Df="\nWHERE $Z";if(isset($_POST["delete"]))queries_redirect($A,'Rekord zostaÅ‚ usuniÄ™ty.',$l->delete($a,$Df,!$uh));else{$O=array();foreach($o
as$C=>$n){$X=process_input($n);if($X!==false&&$X!==null)$O[idf_escape($C)]=$X;}if($zh){if(!$O)redirect($A);queries_redirect($A,'Rekord zostaÅ‚ zaktualizowany.',$l->update($a,$O,$Df,!$uh));if(is_ajax()){page_headers();page_messages($m);exit;}}else{$I=$l->insert($a,$O);$Jd=($I?last_id():0);queries_redirect($A,sprintf('Rekord%s zostaÅ‚ dodany.',($Jd?" $Jd":"")),$I);}}}$Hg=$b->tableName(table_status1($a,true));page_header(($zh?'Edytuj':'Dodaj'),$m,array("select"=>array($a,$Hg)),$Hg);$K=null;if($_POST["save"])$K=(array)$_POST["fields"];elseif($Z){$M=array();foreach($o
...
as$x=>$X){if(!$Z)$K[$x]=null;$o[$x]=array("field"=>$x,"null"=>($x!=$l->primary),"auto_increment"=>($x==$l->primary));}}}if($K===false)echo"<p class='error'>".'Brak rekordÃ³w.'."\n";echo'
<div id="message"></div>
...
<th>Nazwa
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.0.3' alt='+' title='Dodaj nastÄ™pny'></noscript>&nbsp;
</thead>
<?php
if($sf){echo"<tr><td>PRIMARY<td>";foreach($sf["columns"]as$x=>$e){echo
select_input(" disabled",$o,$e),"<label><input disabled type='checkbox'>".'malejÄ…co'."</label> ";}echo"<td><td>\n";}$zd=1;foreach($K["indexes"]as$u){if(!$_POST["drop_col"]||$zd!=key($_POST["drop_col"])){echo"<tr><td>".html_select("indexes[$zd][type]",array(-1=>"")+$ld,$u["type"],($zd==count($K["indexes"])?"indexesAddRow(this);":1)),"<td>";ksort($u["columns"]);$r=1;foreach($u["columns"]as$x=>$e){echo"<span>".select_input(" name='indexes[$zd][columns][$r]' onchange=\"".($r==count($u["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($w=="sql"?"":$_GET["indexes"]."_")."');\"",($o?array_combine($o,$o):$o),$e),($w=="sql"||$w=="mssql"?"<input type='number' name='indexes[$zd][lengths][$r]' class='size' value='".h($u["lengths"][$x])."'>":""),($w!="sql"?checkbox("indexes[$zd][descs][$r]",1,$u["descs"][$x],'malejÄ…co'):"")," </span>";$r++;}echo"<td><input name='indexes[$zd][name]' value='".h($u["name"])."' autocapitalize='off'>\n","<td><input type='image' class='icon' name='drop_col[$zd]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=4.0.3' alt='x' title='".'UsuÅ„'."' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";}$zd++;}echo'</table>
<p>
<input type="submit" value="Zapisz zmiany">
<input type="hidden" name="token" value="',$T,'">
</form>
';}elseif(isset($_GET["database"])){$K=$_POST;if($_POST&&!$m&&!isset($_POST["add_x"])){restart_session();$C=trim($K["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Baza danych zostaÅ‚a usuniÄ™ta.',drop_databases(array(DB)));}elseif(DB!==$C){if(DB!=""){$_GET["db"]=$C;queries_redirect(preg_replace('~\bdb=[^&]*&~','',ME)."db=".urlencode($C),'Nazwa bazy danych zostaÅ‚a zmieniona.',rename_database($C,$K["collation"]));}else{$j=explode("\n",str_replace("\r","",$C));$Bg=true;$Id="";foreach($j
as$k){if(count($j)==1||$k!=""){if(!create_database($k,$K["collation"]))$Bg=false;$Id=$k;}}queries_redirect(ME."db=".urlencode($Id),'Baza danych zostaÅ‚a utworzona.',$Bg);}}else{if(!$K["collation"])redirect(substr(ME,0,-1));query_redirect("ALTER DATABASE ".idf_escape($C).(preg_match('~^[a-z0-9_]+$~i',$K["collation"])?" COLLATE $K[collation]":""),substr(ME,0,-1),'Baza danych zostaÅ‚a zmieniona.');}}page_header(DB!=""?'ZmieÅ„ bazÄ™ danych':'UtwÃ³rz bazÄ™ danych',$m,array(),h(DB));$ib=collations();$C=DB;if($_POST)$C=$K["name"];elseif(DB!="")$K["collation"]=db_collation(DB,$ib);elseif($w=="sql"){foreach(get_vals("SHOW GRANTS")as$Sc){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$Sc,$B)&&$B[1]){$C=stripcslashes(idf_unescape("`$B[2]`"));break;}}}echo'
<form action="" method="post">
<p>
...
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'UsuÅ„'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' class='icon' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=4.0.3' alt='+' title='".'Dodaj nastÄ™pny'."'>\n";echo'<input type="hidden" name="token" value="',$T,'">
</form>
...
';}elseif(isset($_GET["view"])){$a=$_GET["view"];$K=$_POST;if($_POST&&!$m){$C=trim($K["name"]);$Ea=" AS\n$K[select]";$A=ME."table=".urlencode($C);$ee='Perspektywa zostaÅ‚a zmieniona.';if(!$_POST["drop"]&&$a==$C&&$w!="sqlite")query_redirect(($w=="mssql"?"ALTER":"CREATE OR REPLACE")." VIEW ".table($C).$Ea,$A,$ee);else{$Rg=$C."_adminer_".uniqid();drop_create("DROP VIEW ".table($a),"CREATE VIEW ".table($C).$Ea,"DROP VIEW ".table($C),"CREATE VIEW ".table($Rg).$Ea,"DROP VIEW ".table($Rg),($_POST["drop"]?substr(ME,0,-1):$A),'Perspektywa zostaÅ‚a usuniÄ™ta.',$ee,'Perspektywa zostaÅ‚a utworzona.',$a,$C);}}if(!$_POST&&$a!=""){$K=view($a);$K["name"]=$a;if(!$m)$m=$g->error;}page_header(($a!=""?'ZmieÅ„ perspektywÄ™':'UtwÃ³rz perspektywÄ™'),$m,array("table"=>$a),h($a));echo'
<form action="" method="post">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'>".'Uprawnienia uÅ¼ytkownikÃ³w'.doc_link(array('sql'=>"grant.html#priv_level"));$r=0;foreach($Tc
as$xe=>$Sc){echo'<th>'.($xe!="*.*"?"<input name='objects[$r]' value='".h($xe)."' size='10' autocapitalize='off'>":"<input type='hidden' name='objects[$r]' value='*.*' size='10'>*.*");$r++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Serwer',"Databases"=>'Baza danych',"Tables"=>'Tabela',"Columns"=>'Kolumna',"Procedures"=>'Procedura',)as$tb=>$Kb){foreach((array)$xf[$tb]as$wf=>$mb){echo"<tr".odd()."><td".($Kb?">$Kb<td":" colspan='2'").' lang="en" title="'.h($mb).'">'.h($wf);$r=0;foreach($Tc
as$xe=>$Sc){$C="'grants[$r][".h(strtoupper($wf))."]'";$Y=$Sc[strtoupper($wf)];if($tb=="Server Admin"&&$xe!=(isset($Tc["*.*"])?"*.*":".*"))echo"<td>&nbsp;";elseif(isset($_GET["grant"]))echo"<td><select name=$C><option><option value='1'".($Y?" selected":"").">".'Uprawnienia'."<option value='0'".($Y=="0"?" selected":"").">".'UsuÅ„ uprawnienia'."</select>";else
...
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$m){$Fd=0;foreach((array)$_POST["kill"]as$X){if(queries("KILL ".(+$X)))$Fd++;}queries_redirect(ME."processlist=",lang(array('Przerwano %d wÄ…tek.','Przerwano %d wÄ…tki.','Przerwano %d wÄ…tkÃ³w.'),$Fd),$Fd||!$_POST["kill"]);}page_header('Lista procesÃ³w',$m);echo'
<form action="" method="post">
