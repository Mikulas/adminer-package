...
where($Z){global$v;$H=array();foreach((array)$Z["where"]as$w=>$W)$H[]=idf_escape(bracket_escape($w,1)).(($v=="sql"&&ereg('\\.',$W))||$v=="mssql"?" LIKE ".exact_value(addcslashes($W,"%_\\")):" = ".exact_value($W));foreach((array)$Z["null"]as$w)$H[]=idf_escape($w)." IS NULL";return
...
quote($N){return"'".str_replace("'","''",$N)."'";}function
select_db($pb){return$this->query("USE $pb");}function
...
as$oe){$A=str_replace("``","`",$oe[2]).$oe[3];$n[]=array("field"=>$A,"type"=>strtolower($oe[5]),"length"=>preg_replace_callback("~$Rb~s",'normalize_enum',$oe[6]),"unsigned"=>strtolower(preg_replace('~\\s+~',' ',trim("$oe[8] $oe[7]"))),"full_type"=>$oe[4],"inout"=>strtoupper($oe[1]),"collation"=>strtolower($oe[9]),);}if($S!="FUNCTION")return
...
selectVal($W,$z,$m){$H=($W===null?"<i>NULL</i>":(ereg("char|binary",$m["type"])&&!ereg("var",$m["type"])?"<code>$W</code>":$W));if(ereg('blob|bytea|raw|file',$m["type"])&&!is_utf8($W))$H=lang(array('%d bajt','%d bajty','%d bajtów'),strlen($W));return($z?"<a href='$z'>$H</a>":$H);}function
...
selectLengthPrint($Tf){if($Tf!==null){echo"<fieldset><legend>".'Długość tekstu'."</legend><div>",'<input name="text_length" size="3" value="'.h($Tf).'">',"</div></fieldset>\n";}}function
...
page_header($Wf,$l="",$Ja=array(),$Xf=""){global$ca,$b,$h,$Bb;header("Content-Type: text/html; charset=utf-8");if($b->headers()){header("X-Frame-Options: deny");header("X-XSS-Protection: 0");}$Yf=$Wf.($Xf!=""?": ".h($Xf):"");$Zf=strip_tags($Yf.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());echo'<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN">
...
edit_fields($n,$Va,$S="TABLE",$wa=0,$qc=array(),$ab=false){global$Oc;echo'<thead><tr class="wrap">
...
<td',($_POST["defaults"]?"":" class='hidden'"),'>Wartości domyślne
',(support("comment")?"<td".($ab?"":" class='hidden'").">".'Komentarz':"");}echo'<td>',"<input type='image' class='icon' name='add[".(support("move_col")?0:count($n))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Dodaj następny'."'>",'<script type="text/javascript">row_count = ',count($n),';</script>
</thead>
...
',(support("comment")?"<td".($ab?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($m["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' class='icon' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Dodaj następny'."' onclick='return !editingAddRow(this, $wa, 1);'>&nbsp;"."<input type='image' class='icon' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.6.1' alt='^' title='".'Przesuń w górę'."'>&nbsp;"."<input type='image' class='icon' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.6.1' alt='v' title='".'Przesuń w dół'."'>&nbsp;":""),($ge==""||support("drop_col")?"<input type='image' class='icon' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.6.1' alt='x' title='".'Usuń'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
as$Qf)echo"\n<div class='references' title='".h($Rf)."' id='refd$hd-".($q++)."' style='left: $id"."em; top: ".$O["fields"][$Qf]["pos"]."em; height: 1.25em; background: url(".h(preg_replace("~\\?.*~","",ME))."?file=arrow.gif) no-repeat right center;&amp;version=3.6.1'><div style='height: .5em; border-bottom: 1px solid Gray; width: ".(-$id)."em;'></div></div>";}}echo"\n</div>\n";}foreach($kf
...
as$w=>$W){list($E,$Uf)=$W;echo'<a href="'.h(ME."sql=&history=$w").'">'.'Edytuj'."</a> <span class='time'>".@date("H:i:s",$Uf)."</span> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$E)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Wyczyść'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Edytuj wszystkie'."</a>\n","</div></fieldset>\n";}echo'
</form>
';}elseif(isset($_GET["edit"])){$a=$_GET["edit"];$Z=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$tg=(isset($_GET["select"])?$_POST["edit"]:$Z);$n=fields($a);foreach($n
...
<label class="jsonly"><input type="checkbox" name="defaults" value="1"',($_POST["defaults"]?" checked":""),' onclick="columnShow(this.checked, 5);">Wartości domyślne</label>
',(support("comment")?checkbox("comments",1,$ab,'Komentarz',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($I["Comment"]).'" maxlength="60"'.($ab?'':' class="hidden"').'>':''),'<p>
...
Partycje: <input name="partitions" size="2" value="',h($I["partitions"]),'"',($se||!$I["partition_by"]?" class='hidden'":""),'>
...
';$Wc=1;foreach($I["indexes"]as$t){echo"<tr><td>".html_select("indexes[$Wc][type]",array(-1=>"")+$Kc,$t["type"],($Wc==count($I["indexes"])?"indexesAddRow(this);":1))."<td>";ksort($t["columns"]);$q=1;foreach($t["columns"]as$w=>$f){echo"<span>".html_select("indexes[$Wc][columns][$q]",array(-1=>"")+$n,$f,($q==count($t["columns"])?"indexesAddColumn":"indexesChangeColumn")."(this, '".js_escape($v=="sql"?"":$_GET["indexes"]."_")."');"),"<input name='indexes[$Wc][lengths][$q]' size='2' value='".h($t["lengths"][$w])."'> </span>";$q++;}echo"<td><input name='indexes[$Wc][name]' value='".h($t["name"])."'>\n";$Wc++;}echo'</table>
<p>
<input type="submit" value="Zapisz zmiany">
...
<input type="submit" value="Zapisz zmiany">
<?php
if(DB!="")echo"<input type='submit' name='drop' value='".'Usuń'."'".confirm().">\n";elseif(!$_POST["add_x"]&&$_GET["db"]=="")echo"<input type='image' name='add' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.6.1' alt='+' title='".'Dodaj następny'."'>\n";echo'<input type="hidden" name="token" value="',$R,'">
</form>
...
<tr><th>Wykonuj co<td><input name="INTERVAL_VALUE" value="',h($I["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Rc,$I["INTERVAL_FIELD"]),'<tr><th>Status<td>',html_select("STATUS",$zf,$I["STATUS"]),'<tr><th>Komentarz<td><input name="EVENT_COMMENT" value="',h($I["EVENT_COMMENT"]),'" maxlength="64">
