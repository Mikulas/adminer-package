...
confirm($Ia="",$hd=false){return" onclick=\"".($hd?"eventStop(event); ":"")."return confirm('".'Are you sure?'.($Ia?" (' + $Ia + ')":"")."');\"";}function
...
upload_error($h){$jc=($h==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($h?'Unable to upload a file.'.($jc?" ".sprintf('Maximum allowed file size is %sB.',$jc):""):'File does not exist.');}function
...
selectLinks($od,$N=""){$a=$od["Name"];if(isset($N)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'New item'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Last page'."'>&gt;&gt;</a>\n";}function
...
as$d=>$V){$u.="&set".urlencode("[".bracket_escape($d)."]")."=".urlencode($J[$V]);}echo"<a href='".h($u)."' title='".'New item'."'>+</a> ";}}}function
...
selectSearchPrint($X,$e,$q){$X=(array)$_GET["where"];echo'<fieldset><legend>'.'Search'."</legend><div>\n";$Xb=array();foreach($X
...
as$_=>$Sa){$A=$this->_foreignKeyOptions($_GET["select"],$_);if(is_array($A)){if($j[$_]["null"]){$A[0]='('.'empty'.')';}$s=$Xb[$_];$n--;echo"<div>".h($Sa)."<input type='hidden' name='where[$n][col]' value='".h($_)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($A,$X[$s]["val"],true)."</select></div>\n";unset($e[$_]);}}$n=0;foreach($X
as$V){if(($V["col"]==""||$e[$V["col"]])&&"$V[col]$V[val]"!=""){echo"<div><select name='where[$n][col]'><option value=''>(".'anywhere'.")".optionlist($e,$V["col"],true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators,$V["op"]),"<input name='where[$n][val]' value='".h($V["val"])."'></div>\n";$n++;}}echo"<div><select name='where[$n][col]' onchange='selectAddRow(this);'><option value=''>(".'anywhere'.")".optionlist($e,null,true)."</select>",html_select("where[$n][op]",array(-1=>"")+$this->operators),"<input name='where[$n][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($B,$e,$q){$_c=array();foreach($q
as$s=>$p){$B=array();foreach($p["columns"]as$V){$B[]=$e[$V];}if(count(array_filter($B,'strlen'))>1&&$s!="PRIMARY"){$_c[$s]=implode(", ",$B);}}if($_c){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$_c,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
as$_=>$i){if(!isset($i["privileges"][$Ld?"update":"insert"])||$b->fieldName($i)==""){unset($j[$_]);}}if($_POST&&!$h&&!isset($_GET["select"])){$v=$_POST["referer"];if($_POST["insert"]){$v=($Ld?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$v)){$v=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$v,'Item has been deleted.');}else{$N=array();foreach($j
as$_=>$i){$V=process_input($i);if($V!==false&&$V!==null){$N[idf_escape($_)]=($Ld?"\n".idf_escape($_)." = $V":$V);}}if($Ld){if(!$N){redirect($v);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$v,'Item has been updated.');}else{$H=insert_into($a,$N);$bc=($H?last_id():0);queries_redirect($v,sprintf('Item%s has been inserted.',($bc?" $bc":"")),$H);}}}$pd=$b->tableName(table_status($a));page_header(($Ld?'Edit':'Insert'),$h,array("select"=>array($a,$pd)),$pd);$J=null;if($_POST["save"]){$J=(array)$_POST["fields"];}elseif($X){$L=array();foreach($j
as$_=>$i){if(isset($i["privileges"]["select"])){$L[]=($_POST["clone"]&&$i["auto_increment"]?"'' AS ":(ereg("enum|set",$i["type"])?"1*".idf_escape($_)." AS ":"")).idf_escape($_);}}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false){echo"<p class='error'>".'No rows.'."\n";}echo'
...
';if($j){echo"<input type='submit' value='".'Save'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Ld?'Save and continue edit':'Save and insert next')."' title='Ctrl+Shift+Enter'>\n";}}echo($Ld?"<input type='submit' name='delete' value='".'Delete'."' onclick=\"return confirm('".'Are you sure?'."');\">\n":($_POST||!$j?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
