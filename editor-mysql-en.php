...
selectLinks($sd,$N=""){$a=$sd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'New item'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Last page'."'>&gt;&gt;</a>\n";}function
...
as$u=>$q){$Dc=array();foreach($q["columns"]as$V)$Dc[]=$f[$V];if(count(array_filter($Dc,'strlen'))>1&&$u!="PRIMARY")$Ec[$u]=implode(", ",$Dc);}if($Ec){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Ec,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
as$B=>$j){if(!isset($j["privileges"][$Pd?"update":"insert"])||$b->fieldName($j)=="")unset($k[$B]);}if($_POST&&!$i&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($Pd?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$x,'Item has been deleted.');else{$N=array();foreach($k
as$B=>$j){$V=process_input($j);if($V!==false&&$V!==null)$N[idf_escape($B)]=($Pd?"\n".idf_escape($B)." = $V":$V);}if($Pd){if(!$N)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$x,'Item has been updated.');}else{$H=insert_into($a,$N);$dc=($H?last_id():0);queries_redirect($x,sprintf('Item%s has been inserted.',($dc?" $dc":"")),$H);}}}$td=$b->tableName(table_status($a));page_header(($Pd?'Edit':'Insert'),$i,array("select"=>array($a,$td)),$td);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($k
as$B=>$j){if(isset($j["privileges"]["select"]))$L[]=($_POST["clone"]&&$j["auto_increment"]?"'' AS ":($s=="sql"&&ereg("enum|set",$j["type"])?"1*".idf_escape($B)." AS ":"")).idf_escape($B);}$J=array();if($L){$K=get_rows("SELECT".limit(implode(", ",$L)." FROM ".table($a)," WHERE $X",(isset($_GET["select"])?2:1)));$J=(isset($_GET["select"])&&count($K)!=1?null:reset($K));}}if($J===false)echo"<p class='error'>".'No rows.'."\n";echo'
...
as$B=>$V){$V=process_input($k[$B]);if($V!==null){if($_POST["clone"])$N[idf_escape($B)]=($V!==false?$V:idf_escape($B));elseif($V!==false)$N[]=idf_escape($B)." = $V";}}$G.=($_POST["clone"]?" (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a):" SET\n".implode(",\n",$N));}if($_POST["delete"]||$N){$Ea="UPDATE";if($_POST["delete"]){$Ea="DELETE";$G="FROM $G";}if($_POST["clone"]){$Ea="INSERT";$G="INTO $G";}if($_POST["all"]||($Nd===array()&&$_POST["check"])||count($n)<count($L)){$H=queries("$Ea $G".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $Xd"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$H=queries($Ea.limit1($G,"\nWHERE ".where_check($V)));if(!$H)break;$ia+=$g->affected_rows;}}}$_=lang(array('%d item has been affected.','%d items have been affected.'),$ia);if($_POST["clone"]&&$H&&$ia==1){$dc=last_id();if($dc)$_=sprintf('Item%s has been inserted.'," $dc");}queries_redirect(remove_from_uri("page"),$_,$H);}elseif(!$_POST["import"]){if(!$_POST["val"])$i='Double click on a value to modify it.';else{$H=true;$ia=0;foreach($_POST["val"]as$Ld=>$J){$N=array();foreach($J
as$u=>$V){$u=bracket_escape($u,1);$N[]=idf_escape($u)." = ".(ereg('char|text',$k[$u]["type"])||$V!=""?$b->processInput($k[$u],$V):"NULL");}$G=table($a)." SET ".implode(", ",$N);$Wd=" WHERE ".where_check($Ld).($X?" AND ".implode(" AND ",$X):"");$H=queries("UPDATE".(count($n)<count($L)?" $G$Wd":limit1($G,$Wd)));if(!$H)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('%d item has been affected.','%d items have been affected.'),$ia),$H);}}elseif(is_string($l=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$H=true;$Da=array_keys($k);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$l,$z);$ia=count($z[0]);begin();$fd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($z[0]as$u=>$V){preg_match_all("~((\"[^\"]*\")+|[^$fd]*)$fd~",$V.$fd,$jc);if(!$u&&!array_diff($jc[1],$Da)){$Da=$jc[1];$ia--;}else{$N=array();foreach($jc[1]as$o=>$Aa)$N[idf_escape($Da[$o])]=($Aa==""&&$k[$Da[$o]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Aa))));$H=insert_update($a,$N,$Nc);if(!$H)break;}}if($H)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('%d row has been imported.','%d rows have been imported.'),$ia),$H);queries("ROLLBACK");}else$i=upload_error($l);}}$td=$b->tableName($R);page_header('Select'.": $td",$i);session_write_close();$N=null;if(isset($Zc["insert"])){$N="";foreach((array)$_GET["where"]as$V){if(count($Bb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$N.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($R,$N);if(!$f)echo"<p class='error'>".'Unable to select the table'.($k?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($X,$f,$r);$b->selectOrderPrint($Dc,$f,$r);$b->selectLimitPrint($v);$b->selectLengthPrint($wd);$b->selectActionPrint($r);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$Eb=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$D=floor(max(0,$Eb-1)/$v);}$G="SELECT".limit((+$v&&$n&&count($n)<count($L)&&$s=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Gb,($X?"\nWHERE ".implode(" AND ",$X):"").$Ib,($v!=""?+$v:null),($D?$v*$D:0),"\n");echo$b->selectQuery($G);$H=$g->query($G);if(!$H)echo"<p class='error'>".error()."\n";else{if($s=="mssql")$H->seek($v*$D);$fb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($D&&$s=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last")$Eb=(+$v&&$n&&count($n)<count($L)?($s=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($G) x")):count($K));if(!$K)echo"<p class='message'>".'No rows.'."\n";else{$ta=$b->backwardKeys($a,$td);echo"<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$n&&$L?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'edit'."</a>");$rc=array();$Hb=array();reset($L);$Tc=1;foreach($K[0]as$u=>$V){if($u!=$vc){$V=$_GET["columns"][key($L)];$j=$k[$L?($V?$V["col"]:current($L)):$u];$B=($j?$b->fieldName($j,$Tc):"*");if($B!=""){$Tc++;$rc[$u]=$B;$Pb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Pb).'">'.(!$L||$V?apply_sql_function($V["fun"],$B):h(current($L)))."</a>";echo"<span class='column hidden'>","<a href='".h("$Pb&desc%5B0%5D=1")."' title='".'descending'."' class='text'> ↓</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Search'.'" class="text jsonly"> =</a>';echo"</span>";}$Hb[$u]=$V["fun"];next($L);}}$gc=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
...
as$u=>$V)$Ld.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$n&&$L?"":"<td>".checkbox("check[]",substr($Ld,1),in_array(substr($Ld,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($n)<count($L)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Ld)."'>".'edit'."</a>"));foreach($J
