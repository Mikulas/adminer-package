...
confirm($Ub=""){return" onclick=\"return confirm('".'Are you sure?'.($Ub?" (' + $Ub + ')":"")."');\"";}function
js_escape($A){return
addcslashes($A,"\r\n'\\/");}function
...
upload_error($v){$Fc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Unable to upload a file.'.($Fc?" ".sprintf('Maximum allowed file size is %sB.',$Fc):""):'File does not exist.');}function
...
lang($sd,$Wa){$td=($Wa==1||(!$Wa&&'en'=='fr')?0:((!$Wa||$Wa>=5)&&ereg('cs|sk|ru','en')?2:1));return
...
selectLinks($Ka,$t=""){$q=$Ka["Name"];if(isset($t)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($q).$t).'">'.'New item'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Last page'."'>&gt;&gt;</a>\n";}function
...
as$J=>$a){$s.="&set".urlencode("[".bracket_escape($J)."]")."=".urlencode($d[$a]);}echo"<a href='".h($s)."' title='".'New item'."'>+</a> ";}}}}function
...
selectSearchPrint($p,$z,$E){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Search'."</legend><div>\n";$Cb=array();foreach($p
...
as$c=>$Q){$X=array();foreach($Q["columns"]as$a){$X[]=$this->fieldName(array("field"=>$a,"comment"=>$z[$a]));}if(count(array_filter($X,'strlen'))>1&&$c!="PRIMARY"){$Bb[$c]=implode(", ",$X);}}if($Bb){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Bb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
as$f=>$e){if(!isset($e["privileges"][$ga?"update":"insert"])||$j->fieldName($e)==""){unset($r[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$O=$_POST["referer"];if($_POST["insert"]){$O=($ga?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$O)){$O=ME."select=".urlencode($q);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($q)," WHERE $p"),$O,'Item has been deleted.');}else{$t=array();foreach($r
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($ga?"\n".idf_escape($f)." = $a":$a);}}if($ga){if(!$t){redirect($O);}query_redirect("UPDATE".limit1(table($q)." SET".implode(",",$t),"\nWHERE $p"),$O,'Item has been updated.');}else{$h=insert_into($q,$t);$sc=($h?last_id():0);queries_redirect($O,sprintf('Item%s has been inserted.',($sc?" $sc":"")),$h);}}}$Ta=$j->tableName(table_status($q));page_header(($ga?'Edit':'Insert'),$v,array("select"=>array($q,$Ta)),$Ta);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$w=array();foreach($r
