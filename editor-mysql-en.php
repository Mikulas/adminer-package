...
upload_error($k){$wc=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Unable to upload a file.'.($wc?" ".sprintf('Maximum allowed file size is %sB.',$wc):""):'File does not exist.');}function
...
selectLinks($Dd,$N=""){$a=$Dd["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'New item'."</a>\n";echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Last page'."'>&gt;&gt;</a>\n";}function
...
as$e=>$V)$w.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($J[$V]);echo"<a href='".h($w)."' title='".'New item'."'>+</a> ";}}}function
...
as$u=>$ac){$B=array();foreach($ac["columns"]as$V)$B[]=$f[$V];if(count(array_filter($B,'strlen'))>1&&$u!="PRIMARY")$Oc[$u]=implode(", ",$B);}if($Oc){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Oc,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
';$this->databasesPrint($zc);if($zc!="db"&&$zc!="ns"){$R=table_status();if(!$R)echo"<p class='message'>".'No tables.'."\n";else$this->tablesPrint($R);}}}function
...
as$A=>$l){if(!isset($l["privileges"][$be?"update":"insert"])||$b->fieldName($l)=="")unset($m[$A]);}if($_POST&&!$k&&!isset($_GET["select"])){$x=$_POST["referer"];if($_POST["insert"])$x=($be?null:$_SERVER["REQUEST_URI"]);elseif(!ereg('^.+&select=.+$',$x))$x=ME."select=".urlencode($a);if(isset($_POST["delete"]))query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $X"),$x,'Item has been deleted.');else{$N=array();foreach($m
as$A=>$l){$V=process_input($l);if($V!==false&&$V!==null)$N[idf_escape($A)]=($be?"\n".idf_escape($A)." = $V":$V);}if($be){if(!$N)redirect($x);query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$N),"\nWHERE $X"),$x,'Item has been updated.');}else{$H=insert_into($a,$N);$nc=($H?last_id():0);queries_redirect($x,sprintf('Item%s has been inserted.',($nc?" $nc":"")),$H);}}}$Ed=$b->tableName(table_status($a));page_header(($be?'Edit':'Insert'),$k,array("select"=>array($a,$Ed)),$Ed);$J=null;if($_POST["save"])$J=(array)$_POST["fields"];elseif($X){$L=array();foreach($m
...
as$A=>$V){$V=process_input($m[$A]);if($V!==null){if($_POST["clone"])$N[idf_escape($A)]=($V!==false?$V:idf_escape($A));elseif($V!==false)$N[]=idf_escape($A)." = $V";}}$G.=($_POST["clone"]?" (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a):" SET\n".implode(",\n",$N));}if($_POST["delete"]||$N){$Ja="UPDATE";if($_POST["delete"]){$Ja="DELETE";$G="FROM $G";}if($_POST["clone"]){$Ja="INSERT";$G="INTO $G";}if($_POST["all"]||($Zd===array()&&$_POST["check"])||$fc){$H=queries("$Ja $G".($_POST["all"]?($X?"\nWHERE ".implode(" AND ",$X):""):"\nWHERE $je"));$ia=$g->affected_rows;}else{foreach((array)$_POST["check"]as$V){$H=queries($Ja.limit1($G,"\nWHERE ".where_check($V,$m)));if(!$H)break;$ia+=$g->affected_rows;}}}$z=lang(array('%d item has been affected.','%d items have been affected.'),$ia);if($_POST["clone"]&&$H&&$ia==1){$nc=last_id();if($nc)$z=sprintf('Item%s has been inserted.'," $nc");}queries_redirect(remove_from_uri("page"),$z,$H);}elseif(!$_POST["import"]){if(!$_POST["val"])$k='Ctrl+click on a value to modify it.';else{$H=true;$ia=0;foreach($_POST["val"]as$Xd=>$J){$N=array();foreach($J
as$u=>$V){$u=bracket_escape($u,1);$N[]=idf_escape($u)." = ".(ereg('char|text',$m[$u]["type"])||$V!=""?$b->processInput($m[$u],$V):"NULL");}$G=table($a)." SET ".implode(", ",$N);$ie=" WHERE ".where_check($Xd,$m).($X?" AND ".implode(" AND ",$X):"");$H=queries("UPDATE".($fc?" $G$ie":limit1($G,$ie)));if(!$H)break;$ia+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('%d item has been affected.','%d items have been affected.'),$ia),$H);}}elseif(is_string($n=get_file("csv_file",true))){cookie("adminer_import","output=".urlencode($ha["output"])."&format=".urlencode($_POST["separator"]));$H=true;$Ia=array_keys($m);preg_match_all('~(?>"[^"]*"|[^"\\r\\n]+)+~',$n,$tc);$ia=count($tc[0]);begin();$rd=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));foreach($tc[0]as$u=>$V){preg_match_all("~((?>\"[^\"]*\")+|[^$rd]*)$rd~",$V.$rd,$uc);if(!$u&&!array_diff($uc[1],$Ia)){$Ia=$uc[1];$ia--;}else{$N=array();foreach($uc[1]as$q=>$Fa)$N[idf_escape($Ia[$q])]=($Fa==""&&$m[$Ia[$q]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Fa))));$H=insert_update($a,$N,$Xc);if(!$H)break;}}if($H)queries("COMMIT");queries_redirect(remove_from_uri("page"),lang(array('%d row has been imported.','%d rows have been imported.'),$ia),$H);queries("ROLLBACK");}else$k=upload_error($n);}}$Ed=$b->tableName($R);if(is_ajax())ob_start();page_header('Select'.": $Ed",$k);$N=null;if(isset($ld["insert"])){$N="";foreach((array)$_GET["where"]as$V){if(count($Hb[$V["col"]])==1&&($V["op"]=="="||(!$V["op"]&&!ereg('[_%]',$V["val"]))))$N.="&set".urlencode("[".bracket_escape($V["col"])."]")."=".urlencode($V["val"]);}}$b->selectLinks($R,$N);if(!$f)echo"<p class='error'>".'Unable to select the table'.($m?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($L,$f);$b->selectSearchPrint($X,$f,$s);$b->selectOrderPrint($B,$f,$s);$b->selectLimitPrint($v);$b->selectLengthPrint($Id);$b->selectActionPrint($s);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$o=$g->result("SELECT COUNT(*) FROM ".table($a).($X?" WHERE ".implode(" AND ",$X):""));$D=floor(max(0,$o-1)/$v);}$G=$b->selectQueryBuild($L,$X,$Nb,$B,$v,$D);if(!$G)$G="SELECT".limit((+$v&&$Nb&&$fc&&$t=="sql"?"SQL_CALC_FOUND_ROWS ":"").$Lb,($X?"\nWHERE ".implode(" AND ",$X):"").$Ob,($v!=""?+$v:null),($D?$v*$D:0),"\n");echo$b->selectQuery($G);$H=$g->query($G);if(!$H)echo"<p class='error'>".error()."\n";else{if($t=="mssql")$H->seek($v*$D);$lb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$K=array();while($J=$H->fetch_assoc()){if($D&&$t=="oracle")unset($J["RNUM"]);$K[]=$J;}if($_GET["page"]!="last")$o=(+$v&&$Nb&&$fc?($t=="sql"?$g->result(" SELECT FOUND_ROWS()"):$g->result("SELECT COUNT(*) FROM ($G) x")):count($K));if(!$K)echo"<p class='message'>".'No rows.'."\n";else{$ua=$b->backwardKeys($a,$Ed);echo"<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n","<thead><tr>".(!$Nb&&$L?"":"<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);'> <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'edit'."</a>");$Bc=array();$Mb=array();reset($L);$dd=1;foreach($K[0]as$u=>$V){if($u!=$Fc){$V=$_GET["columns"][key($L)];$l=$m[$L?($V?$V["col"]:current($L)):$u];$A=($l?$b->fieldName($l,$dd):"*");if($A!=""){$dd++;$Bc[$u]=$A;$e=idf_escape($u);$Vb=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($u);$Wa="&desc%5B0%5D=1";echo'<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">','<a href="'.h($Vb.($B[0]==$e||$B[0]==$u||(!$B&&$fc&&$Nb[0]==$e)?$Wa:'')).'">';echo(!$L||$V?apply_sql_function($V["fun"],$A):h(current($L)))."</a>";echo"<span class='column hidden'>","<a href='".h($Vb.$Wa)."' title='".'descending'."' class='text'> ↓</a>";if(!$V["fun"])echo'<a href="#fieldset-search" onclick="selectSearch(\''.h(js_escape($u)).'\'); return false;" title="'.'Search'.'" class="text jsonly"> =</a>';echo"</span>";}$Mb[$u]=$V["fun"];next($L);}}$qc=array();if($_GET["modify"]){foreach($K
as$J){foreach($J
...
as$u=>$V)$Xd.="&".($V!==null?urlencode("where[".bracket_escape($u)."]")."=".urlencode($V):"null%5B%5D=".urlencode($u));echo"<tr".odd().">".(!$Nb&&$L?"":"<td>".checkbox("check[]",substr($Xd,1),in_array(substr($Xd,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").($fc||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($a).$Xd)."'>".'edit'."</a>"));foreach($J
