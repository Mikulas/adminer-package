...
confirm($nc="",$Jd=false){return" onclick=\"".($Jd?"eventStop(event); ":"")."return confirm('".'Are you sure?'.($nc?" (' + $nc + ')":"")."');\"";}function
js_escape($F){return
addcslashes($F,"\r\n'\\/");}function
...
upload_error($v){$Vc=($v==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($v?'Unable to upload a file.'.($Vc?" ".sprintf('Maximum allowed file size is %sB.',$Vc):""):'File does not exist.');}function
...
enum_input($ta,$L,$e,$i,$Fb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$T);$b=(isset($Fb)?"<label><input type='$ta'$L value='$Fb'".((is_array($i)?in_array($Fb,$i):$i===0)?" checked":"")."><i>".'empty'."</i></label>":"");foreach($T[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$Ya=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ta'$L value='".($n+1)."'".($Ya?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
...
as$f=>$pa){$Y=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Y)){if($q[$f]["null"]){$Y[0]='('.'empty'.')';}$c=$Cb[$f];$n--;echo"<div>".h($pa)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Y,$p[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($p
...
as$c=>$R){$V=array();foreach($R["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Db[$c]=implode(", ",$V);}}if($Db){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Db,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
';if($Qa!="db"&&$Qa!="ns"){$C=table_status();if(!$C){echo"<p class='message'>".'No tables.'."\n";}else{$this->tablesPrint($C);}}}}function
...
as$f=>$e){if(!isset($e["privileges"][$aa?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$I=$_POST["referer"];if($_POST["insert"]){$I=($aa?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$I)){$I=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$I,'Item has been deleted.');}else{$t=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$t[idf_escape($f)]=($aa?"\n".idf_escape($f)." = $a":$a);}}if($aa){if(!$t){redirect($I);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$t),"\nWHERE $p"),$I,'Item has been updated.');}else{$k=insert_into($r,$t);$pc=($k?last_id():0);queries_redirect($I,sprintf('Item%s has been inserted.',($pc?" $pc":"")),$k);}}}$Na=$j->tableName(table_status($r));page_header(($aa?'Edit':'Insert'),$v,array("select"=>array($r,$Na)),$Na);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$u=array();foreach($q
...
';if($q){echo"<input type='submit' value='".'Save'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($aa?'Save and continue edit':'Save and insert next')."' title='Ctrl+Shift+Enter'>\n";}}echo($aa?"<input type='submit' name='delete' value='".'Delete'."' onclick=\"return confirm('".'Are you sure?'."');\">\n":($_POST||!$q?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
