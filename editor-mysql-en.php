...
enum_input($ua,$L,$e,$i,$Jb=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($Jb)?"<label><input type='$ua'$L value='$Jb'".((is_array($i)?in_array($Jb,$i):$i===0)?" checked":"")."><i>".'empty'."</i></label>":"");foreach($U[1]as$n=>$a){$a=stripcslashes(str_replace("''","'",$a));$bb=(is_int($i)?$i==$n+1:(is_array($i)?in_array($n+1,$i):$i===$a));$b.=" <label><input type='$ua'$L value='".($n+1)."'".($bb?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
...
selectSearchPrint($o,$A,$G){$o=(array)$_GET["where"];echo'<fieldset><legend>'.'Search'."</legend><div>\n";$Ib=array();foreach($o
...
as$f=>$na){$Z=$this->_foreignKeyOptions($_GET["select"],$f);if(is_array($Z)){if($q[$f]["null"]){$Z[0]='('.'empty'.')';}$c=$Ib[$f];$n--;echo"<div>".h($na)."<input type='hidden' name='where[$n][col]' value='".h($f)."'><input type='hidden' name='where[$n][op]' value='='>: <select name='where[$n][val]'>".optionlist($Z,$o[$c]["val"],true)."</select></div>\n";unset($A[$f]);}}$n=0;foreach($o
...
as$c=>$T){$V=array();foreach($T["columns"]as$a){$V[]=$A[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Nb[$c]=implode(", ",$V);}}if($Nb){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Nb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
editInput($h,$e,$L,$i){if($e["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$L value='-1' checked><i>".'original'."</i></label> ":"").enum_input("radio",$L,$e,($i||isset($_GET["select"])?$i:0),($e["null"]?"":null));}$Z=$this->_foreignKeyOptions($h,$e["field"],$i);if(isset($Z)){return(is_array($Z)?"<select$L>".optionlist($Z,$i,true)."</select>":"<input value='".h($i)."'$L class='hidden'><input value='".h($Z)."' class='jsonly' onkeyup=\"whisper('".h(ME."script=complete&source=".urlencode($h)."&field=".urlencode($e["field"]))."&value=', this);\"><div onclick='return whisperClick(event, this.previousSibling);'></div>");}if(like_bool($e)){return'<input type="checkbox" value="'.h($i?$i:1).'"'.($i?' checked':'')."$L>";}$Ia="";if(ereg('time',$e["type"])){$Ia='HH:MM:SS';}if(ereg('date|timestamp',$e["type"])){$Ia='[yyyy]-mm-dd'.($Ia?" [$Ia]":"");}if($Ia){return"<input value='".h($i)."'$L> ($Ia)";}if(eregi('_(md5|sha1)$',$e["field"])){return"<input type='password' value='".h($i)."'$L>";}return'';}function
processInput($e,$i,$z=""){if($z=="now"){return"$z()";}$b=$i;if(ereg('date|timestamp',$e["type"])&&preg_match('(^'.str_replace('\\$1','(?P<p1>\\d*)',preg_replace('~(\\\\\\$([2-6]))~','(?P<p\\2>\\d{1,2})',preg_quote('$1-$3-$5'))).'(.*))',$i,$m)){$b=($m["p1"]!=""?$m["p1"]:($m["p2"]!=""?($m["p2"]<70?20:19).$m["p2"]:gmdate("Y")))."-$m[p3]$m[p4]-$m[p5]$m[p6]".end($m);}$b=($e["type"]=="bit"&&ereg('^[0-9]+$',$i)?$b:q($b));if($i==""&&($e["null"]||!ereg('char|text',$e["type"]))&&!like_bool($e)){$b="NULL";}elseif(ereg('^(md5|sha1)$',$z)){$b="$z($b)";}if(ereg("binary",$e["type"])){$b="unhex($b)";}return$b;}function
dumpOutput(){return
array();}function
dumpFormat(){return
...
as$f=>$e){if(!isset($e["privileges"][$ba?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$v&&!isset($_GET["select"])){$I=$_POST["referer"];if($_POST["insert"]){$I=($ba?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$I)){$I=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $o"),$I,'Item has been deleted.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($ba?"\n".idf_escape($f)." = $a":$a);}}if($ba){if(!$s){redirect($I);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $o"),$I,'Item has been updated.');}else{$k=insert_into($r,$s);$Lc=($k?last_id():0);queries_redirect($I,sprintf('Item%s has been inserted.',($Lc?" $Lc":"")),$k);}}}$La=$j->tableName(table_status($r));page_header(($ba?'Edit':'Insert'),$v,array("select"=>array($r,$La)),$La);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($o){$u=array();foreach($q
as$f=>$e){if(isset($e["privileges"]["select"])){$u[]=($_POST["clone"]&&$e["auto_increment"]?"'' AS ":(ereg("enum|set",$e["type"])?"1*".idf_escape($f)." AS ":"")).idf_escape($f);}}$d=array();if($u){$w=get_rows("SELECT".limit(implode(", ",$u)." FROM ".table($r)," WHERE $o",(isset($_GET["select"])?2:1)));$d=(isset($_GET["select"])&&count($w)!=1?null:reset($w));}}if($d===false){echo"<p class='error'>".'No rows.'."\n";}echo'
...
as$c=>$a){$ja.="&".(isset($a)?urlencode("where[".bracket_escape($c)."]")."=".urlencode($a):"null%5B%5D=".urlencode($c));}echo"<tr".odd().">".(!$Q&&$u?"":"<td>".checkbox("check[]",substr($ja,1),in_array(substr($ja,1),(array)$_POST["check"]),"","this.form['all'].checked = false; formUncheck('all-page');").(count($Q)<count($u)||information_schema(DB)?"":" <a href='".h(ME."edit=".urlencode($r).$ja)."'>".'edit'."</a>"));foreach($d
...
<input type="submit" name="delete" value="Delete" onclick="return confirm('Are you sure? (' + (this.form['all'].checked ? <?php echo$ma,' : formChecked(this, /check/)) + \')\');">
</div></fieldset>
