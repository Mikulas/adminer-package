...
upload_error($w){$zc=($w==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($w?'Unable to upload a file.'.($zc?" ".sprintf('Maximum allowed file size is %sB.',$zc):""):'File does not exist.');}function
...
lang($Sc,$Va){$Rc=($Va==1||(!$Va&&'en'=='fr')?0:((!$Va||$Va>=5)&&ereg('cs|sk|ru','en')?2:1));return
...
selectLinks($Ma,$s=""){$q=$Ma["Name"];if(isset($s)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($q).$s).'">'.'New item'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Page'.": ".'last'."'>&gt;&gt;</a>\n";}function
...
as$V=>$a){$t.="&set".urlencode("[".bracket_escape($V)."]")."=".urlencode($d[$a]);}echo"<a href='".h($t)."' title='".'New item'."'>+</a> ";}}}}function
...
selectSearchPrint($p,$_,$E){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Search'."</legend><div>\n";$yb=array();foreach($p
...
as$c=>$R){$Y=array();foreach($R["columns"]as$a){$Y[]=$this->fieldName(array("field"=>$a,"comment"=>$_[$a]));}if(count(array_filter($Y,'strlen'))>1&&$c!="PRIMARY"){$tb[$c]=implode(", ",$Y);}}if($tb){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$tb,$_GET["index_order"],true)."</select>","</div></fieldset>\n";}}function
...
processInput($e,$i,$y=""){if($y=="now"){return"$y()";}$b=$i;if(ereg('date|timestamp',$e["type"])&&preg_match('(^'.str_replace('\\$1','(?P<p1>[0-9]*)',preg_replace('~(\\\\\\$([2-6]))~','(?P<p\\2>[0-9]{1,2})',preg_quote('$1-$3-$5'))).'(.*))',$i,$j)){$b=($j["p1"]!=""?$j["p1"]:($j["p2"]!=""?($j["p2"]<70?20:19).$j["p2"]:gmdate("Y")))."-$j[p3]$j[p4]-$j[p5]$j[p6]".end($j);}$b=q($b);if(!ereg('varchar|text',$e["type"])&&$e["full_type"]!="tinyint(1)"&&$i==""){$b="NULL";}elseif(ereg('^(md5|sha1)$',$y)){$b="$y($b)";}if(ereg("binary",$e["type"])){$b="unhex($b)";}return$b;}function
...
';if($jb!="db"){$G=table_status();if(!$G){echo"<p class='message'>".'No tables.'."\n";}else{$this->tablesPrint($G);}}}}function
...
long2str($n,true);}$g='';if(!$ta){page_header('No extension',sprintf('None of the supported PHP extensions (%s) are available.',implode(", ",$ab)),null);page_footer("auth");exit;}$ga=$_SESSION["token"];if(!$_SESSION["token"]){$_SESSION["token"]=rand(1,1e6);}$ra=array();if($_COOKIE["adminer_permanent"]){foreach(explode(" ",$_COOKIE["adminer_permanent"])as$a){list($c)=explode(":",$a);$ra[$c]=$a;}}if(isset($_POST["server"])){session_regenerate_id();$_SESSION["pwds"][$_POST["driver"]][$_POST["server"]][$_POST["username"]]=$_POST["password"];if($_POST["permanent"]){$c=base64_encode($_POST["driver"])."-".base64_encode($_POST["server"])."-".base64_encode($_POST["username"]);$qb=$m->permanentLogin();$ra[$c]="$c:".base64_encode($qb?encrypt_string($_POST["password"],$qb):"");cookie("adminer_permanent",implode(" ",$ra));}if(count($_POST)==($_POST["permanent"]?5:4)||DRIVER!=$_POST["driver"]||SERVER!=$_POST["server"]||$_GET["username"]!==$_POST["username"]){redirect(auth_url($_POST["driver"],$_POST["server"],$_POST["username"]));}}elseif($_POST["logout"]){if($ga&&$_POST["token"]!=$ga){page_header('Logout','Invalid CSRF token. Send the form again.');page_footer("db");exit;}else{foreach(array("pwds","dbs","queries")as$c){set_session($c,null);}$c=base64_encode(DRIVER)."-".base64_encode(SERVER)."-".base64_encode($_GET["username"]);if($ra[$c]){unset($ra[$c]);cookie("adminer_permanent",implode(" ",$ra));}redirect(substr(preg_replace('~(username|db|ns)=[^&]*&~','',ME),0,-1),'Logout successful.');}}elseif($ra&&!$_SESSION["pwds"]){session_regenerate_id();$qb=$m->permanentLogin();foreach($ra
...
auth_error($mc=null){global$g,$m,$ga;$mb=session_name();$w="";if(!$_COOKIE[$mb]&&$_GET[$mb]&&ini_bool("session.use_only_cookies")){$w='Session support must be enabled.';}elseif(isset($_GET["username"])){if(($_COOKIE[$mb]||$_GET[$mb])&&!$ga){$w='Session expired, please login again.';}else{$M=&get_session("pwds");if(isset($M)){$w=h($mc?$mc->getMessage():(is_string($g)?$g:'Invalid credentials.'));$M=null;}}}page_header('Login',$w,null);echo"<form action='' method='post'>\n";$m->loginForm();echo"<div>";hidden_fields($_POST,array("driver","server","username","password","permanent"));echo"</div>\n","</form>\n";page_footer("auth");}if(isset($_GET["username"])&&class_exists("Min_DB")){$g=connect();}if(is_string($g)||!$m->login($_GET["username"],get_session("pwds"))){auth_error();exit;}$ga=$_SESSION["token"];if(isset($_POST["server"])&&$_POST["token"]){$_POST["token"]=$ga;}$w=($_POST?($_POST["token"]==$ga?"":'Invalid CSRF token. Send the form again.'):($_SERVER["REQUEST_METHOD"]!="POST"?"":sprintf('Too big POST data. Reduce the data or increase the %s configuration directive.','"post_max_size"')));$g->select_db($m->database());function
...
dump_headers($bc){$qa=($bc!=""?friendly_url($bc):"dump");$jc="csv";header("Content-Type: text/csv; charset=utf-8");header("Content-Disposition: attachment; filename=$qa.$jc");session_write_close();return$jc;}session_cache_limiter("");if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false){session_write_close();}$Sb=array("RESTRICT","CASCADE","SET NULL","NO ACTION");$Nc=" onclick=\"return confirm('".'Are you sure?'."');\"";$ta[DRIVER]='Login';if(isset($_GET["select"])&&($_POST["edit"]||$_POST["clone"])&&!$_POST["save"]){$_GET["edit"]=$_GET["select"];}if(isset($_GET["download"])){$q=$_GET["download"];header("Content-Type: application/octet-stream");header("Content-Disposition: attachment; filename=".friendly_url("$q-".implode("_",$_GET["where"])).".".friendly_url($_GET["field"]));echo$g->result("SELECT".limit(idf_escape($_GET["field"])." FROM ".table($q)," WHERE ".where($_GET),1));exit;}elseif(isset($_GET["edit"])){$q=$_GET["edit"];$p=(isset($_GET["select"])?(count($_POST["check"])==1?where_check($_POST["check"][0]):""):where($_GET));$da=(isset($_GET["select"])?$_POST["edit"]:$p);$r=fields($q);foreach($r
as$f=>$e){if(!isset($e["privileges"][$da?"update":"insert"])||$m->fieldName($e)==""){unset($r[$f]);}}if($_POST&&!$w&&!isset($_GET["select"])){$K=$_POST["referer"];if($_POST["insert"]){$K=($da?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$K)){$K=ME."select=".urlencode($q);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($q)," WHERE $p"),$K,'Item has been deleted.');}else{$s=array();foreach($r
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($da?"\n".idf_escape($f)." = $a":$a);}}if($da){if(!$s){redirect($K);}query_redirect("UPDATE".limit1(table($q)." SET".implode(",",$s),"\nWHERE $p"),$K,'Item has been updated.');}else{$h=insert_into($q,$s);$gc=($h?last_id():0);queries_redirect($K,sprintf('Item%s has been inserted.',($gc?" $gc":"")),$h);}}}$Ua=$m->tableName(table_status($q));page_header(($da?'Edit':'Insert'),$w,array("select"=>array($q,$Ua)),$Ua);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$u=array();foreach($r
