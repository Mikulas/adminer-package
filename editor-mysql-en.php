...
confirm($nc="",$Kd=false){return" onclick=\"".($Kd?"eventStop(event); ":"")."return confirm('".'Are you sure?'.($nc?" (' + $nc + ')":"")."');\"";}function
js_escape($C){return
addcslashes($C,"\r\n'\\/");}function
...
upload_error($u){$Tc=($u==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($u?'Unable to upload a file.'.($Tc?" ".sprintf('Maximum allowed file size is %sB.',$Tc):""):'File does not exist.');}function
...
enum_input($sa,$N,$e,$i,$Ib=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$R);$b=(isset($Ib)?"<label><input type='$sa'$N value='$Ib'".((is_array($i)?in_array($Ib,$i):$i===0)?" checked":"")."><i>".'empty'."</i></label>":"");foreach($R[1]as$p=>$a){$a=stripcslashes(str_replace("''","'",$a));$db=(is_int($i)?$i==$p+1:(is_array($i)?in_array($p+1,$i):$i===$a));$b.=" <label><input type='$sa'$N value='".($p+1)."'".($db?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
...
lang($gd,$_a){$fd=($_a==1||(!$_a&&'en'=='fr')?0:('en'=='sl'&&(!$_a||$_a>2)?1:0)+((!$_a||$_a>=5)&&ereg('cs|sk|ru|sl|pl','en')?2:1));return
...
selectLinks($Ua,$s=""){$r=$Ua["Name"];if(isset($s)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($r).$s).'">'.'New item'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Last page'."'>&gt;&gt;</a>\n";}function
...
selectSearchPrint($o,$_,$F){$o=(array)$_GET["where"];echo'<fieldset><legend>'.'Search'."</legend><div>\n";$Kb=array();foreach($o
...
as$f=>$qa){$ga=$this->_foreignKeyOptions($_GET["select"],$f);if($ga){if($q[$f]["null"]){$ga[0]='('.'empty'.')';}$c=$Kb[$f];$p--;echo"<div>".h($qa)."<input type='hidden' name='where[$p][col]' value='".h($f)."'><input type='hidden' name='where[$p][op]' value='='>: <select name='where[$p][val]'>".optionlist($ga,$o[$c]["val"],true)."</select></div>\n";unset($_[$f]);}}$p=0;foreach($o
as$a){if(($a["col"]==""||$_[$a["col"]])&&"$a[col]$a[val]"!=""){echo"<div><select name='where[$p][col]'><option value=''>(".'anywhere'.")".optionlist($_,$a["col"],true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators,$a["op"]),"<input name='where[$p][val]' value='".h($a["val"])."'></div>\n";$p++;}}echo"<div><select name='where[$p][col]' onchange='selectAddRow(this);'><option value=''>(".'anywhere'.")".optionlist($_,null,true)."</select>",html_select("where[$p][op]",array(-1=>"")+$this->operators),"<input name='where[$p][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($V,$_,$F){$Qb=array();foreach($F
as$c=>$S){$V=array();foreach($S["columns"]as$a){$V[]=$_[$a];}if(count(array_filter($V,'strlen'))>1&&$c!="PRIMARY"){$Qb[$c]=implode(", ",$V);}}if($Qb){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Qb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
as$f=>$e){if(!isset($e["privileges"][$la?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$u&&!isset($_GET["select"])){$G=$_POST["referer"];if($_POST["insert"]){$G=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$G)){$G=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $o"),$G,'Item has been deleted.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($la?"\n".idf_escape($f)." = $a":$a);}}if($la){if(!$s){redirect($G);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $o"),$G,'Item has been updated.');}else{$k=insert_into($r,$s);$bc=($k?last_id():0);queries_redirect($G,sprintf('Item%s has been inserted.',($bc?" $bc":"")),$k);}}}$Qa=$j->tableName(table_status($r));page_header(($la?'Edit':'Insert'),$u,array("select"=>array($r,$Qa)),$Qa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($o){$x=array();foreach($q
