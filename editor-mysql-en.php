...
confirm($Dc="",$kd=false){return" onclick=\"".($kd?"eventStop(event); ":"")."return confirm('".'Are you sure?'.($Dc?" (' + $Dc + ')":"")."');\"";}function
js_escape($B){return
addcslashes($B,"\r\n'\\/");}function
...
upload_error($u){$dc=($u==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):null);return($u?'Unable to upload a file.'.($dc?" ".sprintf('Maximum allowed file size is %sB.',$dc):""):'File does not exist.');}function
...
enum_input($wa,$P,$e,$h,$ac=null){global$j;preg_match_all("~'((?:[^']|'')*)'~",$e["length"],$U);$b=(isset($ac)?"<label><input type='$wa'$P value='$ac'".((is_array($h)?in_array($ac,$h):$h===0)?" checked":"")."><i>".'empty'."</i></label>":"");foreach($U[1]as$o=>$a){$a=stripcslashes(str_replace("''","'",$a));$Wa=(is_int($h)?$h==$o+1:(is_array($h)?in_array($o+1,$h):$h===$a));$b.=" <label><input type='$wa'$P value='".($o+1)."'".($Wa?' checked':'').'>'.h($j->editVal($a,$e)).'</label>';}return$b;}function
...
lang($_d,$Ca){$wd=($Ca==1||(!$Ca&&'en'=='fr')?0:('en'=='sl'&&(!$Ca||$Ca>2)?1:0)+((!$Ca||$Ca>=5)&&ereg('cs|sk|ru|sl|pl','en')?2:1));return
...
selectLinks($Sa,$s=""){$r=$Sa["Name"];if(isset($s)){echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($r).$s).'">'.'New item'."</a>\n";}echo"<a href='".h(remove_from_uri("page"))."&amp;page=last' title='".'Last page'."'>&gt;&gt;</a>\n";}function
...
as$I=>$a){$t.="&set".urlencode("[".bracket_escape($I)."]")."=".urlencode($d[$a]);}echo"<a href='".h($t)."' title='".'New item'."'>+</a> ";}}}function
...
selectSearchPrint($p,$_,$G){$p=(array)$_GET["where"];echo'<fieldset><legend>'.'Search'."</legend><div>\n";$Ob=array();foreach($p
...
as$f=>$ra){$ja=$this->_foreignKeyOptions($_GET["select"],$f);if($ja){if($q[$f]["null"]){$ja[0]='('.'empty'.')';}$c=$Ob[$f];$o--;echo"<div>".h($ra)."<input type='hidden' name='where[$o][col]' value='".h($f)."'><input type='hidden' name='where[$o][op]' value='='>: <select name='where[$o][val]'>".optionlist($ja,$p[$c]["val"],true)."</select></div>\n";unset($_[$f]);}}$o=0;foreach($p
as$a){if(($a["col"]==""||$_[$a["col"]])&&"$a[col]$a[val]"!=""){echo"<div><select name='where[$o][col]'><option value=''>(".'anywhere'.")".optionlist($_,$a["col"],true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators,$a["op"]),"<input name='where[$o][val]' value='".h($a["val"])."'></div>\n";$o++;}}echo"<div><select name='where[$o][col]' onchange='selectAddRow(this);'><option value=''>(".'anywhere'.")".optionlist($_,null,true)."</select>",html_select("where[$o][op]",array(-1=>"")+$this->operators),"<input name='where[$o][val]'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($R,$_,$G){$Jb=array();foreach($G
as$c=>$V){$R=array();foreach($V["columns"]as$a){$R[]=$_[$a];}if(count(array_filter($R,'strlen'))>1&&$c!="PRIMARY"){$Jb[$c]=implode(", ",$R);}}if($Jb){echo'<fieldset><legend>'.'Sort'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$Jb,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"]){echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}}function
...
editInput($i,$e,$P,$h){if($e["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$P value='-1' checked><i>".'original'."</i></label> ":"").enum_input("radio",$P,$e,($h||isset($_GET["select"])?$h:0),($e["null"]?"":null));}$ja=$this->_foreignKeyOptions($i,$e["field"]);if($ja){return"<select$P>".optionlist($ja,$h,true)."</select>";}if(ereg("(tinyint|bit)\\(1\\)",$e["full_type"])){return'<input type="checkbox" value="'.h($h?$h:1).'"'.($h?' checked':'')."$P>";}$La="";if(ereg('time',$e["type"])){$La='HH:MM:SS';}if(ereg('date|timestamp',$e["type"])){$La='[yyyy]-mm-dd'.($La?" [$La]":"");}if($La){return"<input value='".h($h)."'$P> ($La)";}if(eregi('_(md5|sha1)$',$e["field"])){return"<input type='password' value='".h($h)."'$P>";}return'';}function
processInput($e,$h,$y=""){if($y=="now"){return"$y()";}$b=$h;if(ereg('date|timestamp',$e["type"])&&preg_match('(^'.str_replace('\\$1','(?P<p1>\\d*)',preg_replace('~(\\\\\\$([2-6]))~','(?P<p\\2>\\d{1,2})',preg_quote('$1-$3-$5'))).'(.*))',$h,$m)){$b=($m["p1"]!=""?$m["p1"]:($m["p2"]!=""?($m["p2"]<70?20:19).$m["p2"]:gmdate("Y")))."-$m[p3]$m[p4]-$m[p5]$m[p6]".end($m);}$b=($e["type"]=="bit"&&ereg('^[0-9]+$',$h)?$b:q($b));if(!ereg('char|text',$e["type"])&&!ereg("(tinyint|bit)\\(1\\)",$e["full_type"])&&$h==""){$b="NULL";}elseif(ereg('^(md5|sha1)$',$y)){$b="$y($b)";}if(ereg("binary",$e["type"])){$b="unhex($b)";}return$b;}function
dumpOutput(){return
array();}function
dumpFormat(){return
...
as$f=>$e){if(!isset($e["privileges"][$la?"update":"insert"])||$j->fieldName($e)==""){unset($q[$f]);}}if($_POST&&!$u&&!isset($_GET["select"])){$F=$_POST["referer"];if($_POST["insert"]){$F=($la?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$F)){$F=ME."select=".urlencode($r);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($r)," WHERE $p"),$F,'Item has been deleted.');}else{$s=array();foreach($q
as$f=>$e){$a=process_input($e);if($a!==false&&$a!==null){$s[idf_escape($f)]=($la?"\n".idf_escape($f)." = $a":$a);}}if($la){if(!$s){redirect($F);}query_redirect("UPDATE".limit1(table($r)." SET".implode(",",$s),"\nWHERE $p"),$F,'Item has been updated.');}else{$l=insert_into($r,$s);$Gc=($l?last_id():0);queries_redirect($F,sprintf('Item%s has been inserted.',($Gc?" $Gc":"")),$l);}}}$Xa=$j->tableName(table_status($r));page_header(($la?'Edit':'Insert'),$u,array("select"=>array($r,$Xa)),$Xa);$d=null;if($_POST["save"]){$d=(array)$_POST["fields"];}elseif($p){$w=array();foreach($q
